...
char* Class::javaMakeCodeOutputFileName(boolean donttruncateP) {
  { Class* source = this;
    ...
    { StringWrapper* barefile = javaTranslateClassName(internSymbolInModule(source->classType->symbolName, ((Module*)(source->classType->homeContext)), TRUE));
  ...
  { Cons* functions = NIL;
    ...
    { Module* module = NULL;
      ...
      while (!(iter000 == NIL)) {
        ...
        javaOutputFlotsamUnitsToFileForModule(module, NULL, globals, functions);
      }
    }
  }
}

void javaOutputFlotsamUnitsToFileForModule(Module* module, TranslationUnit* classunit, Cons* globals, Cons* functions) {
  { char* classoutputfile = NULL;
    ...
    { 
      ...
      classoutputfile = javaMakeGlobalOutputFileName(module, FALSE);
      classoutputstream = newOutputFileStream(classoutputfile);
      if (((boolean)(classunit))) {
        clasS = ((Class*)(classunit->theObject));
        std::cout << "Translating " << "`" << clasS << "'" << "..." << std::endl;
        classtranslation = javaTranslateDefineNativeClass(clasS)->rest;
      }
      { 
        ...
        *(oCURRENT_STREAMo.get()->nativeStream) << "{" << std::endl;
        ...
        if (((boolean)(classunit))) {
          javaOutputClassConstructors(((Cons*)(classtranslation->nth(7))), javaYieldFlotsamClassName(module), ((boolean)(clasS)) &&
...
void javaOutputClassToFile(Class* clasS) {
  { 
    ...
    { char* classoutputfile = clasS->javaMakeCodeOutputFileName(FALSE);
      OutputFileStream* classoutputstream = newOutputFileStream(classoutputfile);
      Cons* translation = NIL;
...
boolean javaClassUnitDefinesFlotsamClassP(TranslationUnit* classunit) {
  { Class* clasS = ((Class*)(classunit->theObject));
    ...
    return (stringEqlP(javaTranslateClassNamestring(wrapString(internSymbolInModule(clasS->classType->symbolName, ((Module*)(clasS->classType->homeContext)), TRUE)->symbolName))->wrapperValue, javaYieldFlotsamClassName(module)));
  }
}
      ...
      while (!(iter001 == NIL)) {
        ...
        if (javaClassUnitDefinesFlotsamClassP(classunit)) {
          { Module* module = classunit->theObject->homeModule();
            ...
            javaOutputFlotsamUnitsToFileForModule(module, classunit, globals, functions);
...
void javaTranslateSystem(char* systemname) {
  ...
  translateSystem(systemname, KWD_JAVA_TRANSLATE_FILE_JAVA, 6, KWD_JAVA_TRANSLATE_FILE_TWO_PASSp, TRUE_WRAPPER, KWD_JAVA_TRANSLATE_FILE_FORCE_TRANSLATIONp, TRUE_WRAPPER, KWD_JAVA_TRANSLATE_FILE_PRODUCTION_SETTINGSp, TRUE_WRAPPER);
}
...
void startupJavaTranslateFile() {
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE", "(DEFUN JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE ((MODULE MODULE) (CLASSUNIT TRANSLATION-UNIT) (GLOBALS (CONS OF TRANSLATION-UNIT)) (FUNCTIONS (CONS OF TRANSLATION-UNIT))))", ((cpp_function_code)(&javaOutputFlotsamUnitsToFileForModule)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_JAVA_TRANSLATE_FILE_STELLA_STARTUP_JAVA_TRANSLATE_FILE);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_JAVA_TRANSLATE_FILE_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Java-Translate-File"), NULL_STRING_WRAPPER);
      }
    }
