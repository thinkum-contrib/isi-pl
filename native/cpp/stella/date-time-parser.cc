...
void yesterday(DecodedDateTime* parsedValues) {
  today(parsedValues);
...
void today(DecodedDateTime* parsedValues) {
  { double timeZone = parsedValues->dateTimeZone;
    ...
    { int year = NULL_INTEGER;
      ...
      year = makeCurrentDateTime()->getCalendarDate(timeZone, month, day, dow);
      parsedValues->dateTimeDay = day;
      parsedValues->dateTimeMonth = month;
      parsedValues->dateTimeYear = year;
      parsedValues->dateTimeDow = dow;
...
void tomorrow(DecodedDateTime* parsedValues) {
  today(parsedValues);
  { int jd = computeJulianDay(parsedValues->dateTimeYear, parsedValues->dateTimeMonth, parsedValues->dateTimeDay);
...
void now(DecodedDateTime* parsedValues) {
  { double timeZone = parsedValues->dateTimeZone;
...
void setTimeValues(Cons* stringForm, DecodedDateTime* parsedValues) {
      ...
      while (!(iter000 == NIL)) {
        ...
        { GeneralizedSymbol* testValue000 = ((GeneralizedSymbol*)(formType));
          ...
          if (testValue000 == SYM_DATE_TIME_PARSER_STELLA_SECONDP) {
            ...
            ((void  (*) (DecodedDateTime*))lookupFunction(((Symbol*)(formValue)))->functionCode)(parsedValues);
          }
          else {
            { OutputStringStream* stream001 = newOutputStringStream();
  ...
  { 
    ...
    { Cons* stringParts = decomposeDateTimeString(dateTimeString, start, end);
      ...
      if (stringForm == NIL) {
        ...
        { DecodedDateTime* parsedValues = makeCurrentDateTime()->decodeCalendarDate(((timeZone == NULL_FLOAT) ? getLocalTimeZone() : timeZone));
          ...
          setTimeValues(stringForm, parsedValues);
          if (parsedValues->dateTimeZone == NULL_FLOAT) {
            parsedValues->dateTimeZone = getLocalTimeZoneForDate(parsedValues->dateTimeYear, parsedValues->dateTimeMonth, parsedValues->dateTimeDay, parsedValues->dateTimeHour, parsedValues->dateTimeMinute, parsedValues->dateTimeSecond);
          }
          return (parsedValues);
        }
      }
...
CalendarDate* stringToCalendarDate(char* input) {
  ...
  // `null' is returned.
  { DecodedDateTime* dO = parseDateTimeInTimeZone(input, NULL_FLOAT, 0, NULL_INTEGER, FALSE);

    if (!((boolean)(dO))) {
      return (NULL);
    }
    else {
      return (dO->encodeCalendarDate());
    }
  }
}
...
void startupDateTimeParser() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      ...
      { Symbol* symbol = NULL;
        Cons* iter042 = oSPECIAL_SYMBOLSo.get();
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("YESTERDAY", "(DEFUN YESTERDAY ((PARSED-VALUES DECODED-DATE-TIME)))", ((cpp_function_code)(&yesterday)), NULL);
      defineFunctionObject("TODAY", "(DEFUN TODAY ((PARSED-VALUES DECODED-DATE-TIME)))", ((cpp_function_code)(&today)), NULL);
      defineFunctionObject("TOMORROW", "(DEFUN TOMORROW ((PARSED-VALUES DECODED-DATE-TIME)))", ((cpp_function_code)(&tomorrow)), NULL);
      defineFunctionObject("NOW", "(DEFUN NOW ((PARSED-VALUES DECODED-DATE-TIME)))", ((cpp_function_code)(&now)), NULL);
      ...
      defineFunctionObject("SET-TIME-VALUES", "(DEFUN SET-TIME-VALUES ((STRING-FORM (CONS OF CONS)) (PARSED-VALUES DECODED-DATE-TIME)))", ((cpp_function_code)(&setTimeValues)), NULL);
      defineFunctionObject("PARSE-DATE-TIME-IN-TIME-ZONE", "(DEFUN (PARSE-DATE-TIME-IN-TIME-ZONE DECODED-DATE-TIME) ((DATE-TIME-STRING STRING) (TIME-ZONE FLOAT) (START INTEGER) (END INTEGER) (ERROR-ON-MISMATCH? BOOLEAN)) :PUBLIC? TRUE :DOCUMENTATION \"Tries very hard to make sense out of the argument `date-time-string' and\nreturns a time structure if successful.  If not, it returns `null'.\nIf `error-on-mismatch?' is true, parse-date-time will signal an error instead of\nreturning `null'.  \nDefault values are 00:00:00 in the given timezone on the current date.  If the\ngiven `time-zone' value is `null', then the local time zone for the given date\nand time will be used as determined by the operating system.\")", ((cpp_function_code)(&parseDateTimeInTimeZone)), NULL);
      defineFunctionObject("PARSE-DATE-TIME", "(DEFUN (PARSE-DATE-TIME DECODED-DATE-TIME) ((DATE-TIME-STRING STRING) (START INTEGER) (END INTEGER) (ERROR-ON-MISMATCH? BOOLEAN)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE :DOCUMENTATION \"Tries very hard to make sense out of the argument `date-time-string' and\nreturns a time structure if successful.  If not, it returns `null'.\nIf `error-on-mismatch?' is true, parse-date-time will signal an error instead of\nreturning `null'.  \nDefault values are 00:00:00 local time on the current date\" (RETURN (PARSE-DATE-TIME-IN-TIME-ZONE DATE-TIME-STRING NULL START END ERROR-ON-MISMATCH?)))", ((cpp_function_code)(&parseDateTime)), NULL);
      defineFunctionObject("STRING-TO-CALENDAR-DATE", "(DEFUN (STRING-TO-CALENDAR-DATE CALENDAR-DATE) ((INPUT STRING)) :PUBLIC? TRUE :DOCUMENTATION \"Returns a calendar date object representing the date and time\nparsed from the `input' string.  If no valid parse is found,\n`null' is returned.\")", ((cpp_function_code)(&stringToCalendarDate)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_DATE_TIME_PARSER_STELLA_STARTUP_DATE_TIME_PARSER);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_DATE_TIME_PARSER_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Date-Time-Parser"), NULL_STRING_WRAPPER);
      }
    }
