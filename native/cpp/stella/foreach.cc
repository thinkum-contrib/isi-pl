...
Cons* extractGeneratorsClause(Cons* tail, Cons*& _Return1) {
  { Cons* generatorterm = NULL;
    ...
    _Return1 = residue;
    return (generatorsclause->reverse());
  }
}
...
Cons* walkForeachTree(Cons* tree, StandardObject*& _Return1) {
  { 
    ...
    { GeneralizedSymbol* testValue000 = ((GeneralizedSymbol*)(tree->value));
      ...
      else if ((testValue000 == SYM_FOREACH_STELLA_SETOF) ||
          (testValue000 == SYM_FOREACH_STELLA_SET_OF)) {
        return (walkSetofTree(tree, _Return1));
      }
      else {
...
Object* walkCollectionTree(Object* tree, boolean dontoptimizeP, StandardObject*& _Return1) {
  ...
  return (walkExpressionTree(tree, SGT_FOREACH_STELLA_UNKNOWN, SYM_FOREACH_STELLA_FOREACH, FALSE, _Return1));
}

Cons* yieldInCursorClauses(Cons* intree, boolean dontoptimizeP, Cons*& _Return1, Object*& _Return2) {
  { Object* vartree = intree->rest->value;
    ...
    { Object* collectiontree = NULL;
      ...
      if (safePrimaryType(vartree) == SGT_FOREACH_STELLA_CONS) {
        { Object* vartree000 = vartree;
          ...
          if (!(vartree->length() == 2)) {
            ...
            _Return1 = NIL;
            _Return2 = SYM_FOREACH_STELLA_FALSE;
            return (NIL);
          }
      ...
      if (collectionbasetype == SGT_FOREACH_STELLA_INTEGER_INTERVAL) {
        return (yieldInCursorClausesForIntegerInterval(((Cons*)(collectiontree)), collectiontype, keyvar, valuevar, _Return1, _Return2));
      }
      else if (collectionbasetype == SGT_FOREACH_STELLA_ARGUMENT_LIST) {
        return (yieldInCursorClausesForArgumentList(collectiontree, collectiontype, keyvar, valuevar, _Return1, _Return2));
      }
      else {
        if (useVectorStyleIterationP(collectionbasetype)) {
          return (yieldInCursorClausesForVector(collectiontree, collectiontype, keyvar, valuevar, _Return1, _Return2));
        }
        else {
          return (yieldInCursorClausesForGeneralCollection(collectiontree, collectiontype, keyvar, valuevar, _Return1, _Return2));
        }
      }
    }
  }
...
Cons* yieldInCursorClausesForGeneralCollection(Object* collectiontree, StandardObject* collectiontype, Symbol* keyvar, Symbol* valuevar, Cons*& _Return1, Object*& _Return2) {
  { boolean dummy1;
    ...
    { Surrogate* collectionbasetype = typeSpecToBaseType(collectiontype);
      ...
      valueclauses = cons(cons(valuevar, cons(extractParameterType(collectiontype, SYM_FOREACH_STELLA_ANY_VALUE, dummy1), cons(optimizeProgn(listO(3, SYM_FOREACH_STELLA_PROGN, listO(3, SYM_FOREACH_STELLA_SETQ, valuevar, cons(valuetree, NIL)), bumptrees->concatenate(NIL, 0))), NIL))), valueclauses);
      if (((boolean)(keyvar))) {
        { StandardObject* keytype = NULL;
          ...
          if (parameterexistsP) {
            valueclauses = cons(cons(keyvar, cons(keytype, cons(listO(3, SYM_FOREACH_STELLA_SETQ, keyvar, cons(keytree, NIL)), NIL))), valueclauses);
          }
          else {
            { 
              BIND_STELLA_SPECIAL(oPRINTREADABLYpo, boolean, TRUE);
      ...
      _Return2 = continuationtest;
      return (iteratorclauses);
    }
  }
}

Cons* yieldInCursorClausesForIntegerInterval(Cons* collectiontree, StandardObject* collectiontype, Symbol* keyvar, Symbol* valuevar, Cons*& _Return1, Object*& _Return2) {
  { StandardObject* dummy1;
    ...
    { Object* upperbound = collectiontree->rest->rest->value;
      ...
      { Cons* _return_temp = cons(cons(iteratorvar, listO(3, SGT_FOREACH_STELLA_INTEGER, collectiontree->rest->value, NIL)), ((knownunboundedP ? NIL : cons(cons(upperboundvar, listO(3, SGT_FOREACH_STELLA_INTEGER, upperbound, NIL)), NIL)))->concatenate((((knownunboundedP ||
        ...
        _Return1 = cons(cons(valuevar, listO(3, SGT_FOREACH_STELLA_INTEGER, listO(4, SYM_FOREACH_STELLA_PROGN, listO(3, SYM_FOREACH_STELLA_SETQ, valuevar, cons(iteratorvar, NIL)), listO(3, SYM_FOREACH_STELLA_ii, iteratorvar, NIL), NIL), NIL)), NIL);
        _Return2 = (knownunboundedP ? ((StandardObject*)(SYM_FOREACH_STELLA_TRUE)) : ((StandardObject*)((knownboundedP ? listO(3, SYM_FOREACH_STELLA_le, iteratorvar, cons(upperboundvar, NIL)) : listO(3, SYM_FOREACH_STELLA_OR, unboundedvar, cons(listO(3, SYM_FOREACH_STELLA_le, iteratorvar, cons(upperboundvar, NIL)), NIL))))));
        return (_return_temp);
      }
    }
  }
...
Cons* yieldInCursorClausesForVector(Object* collectiontree, StandardObject* collectiontype, Symbol* keyvar, Symbol* valuevar, Cons*& _Return1, Object*& _Return2) {
  { boolean dummy1;
    ...
    { Symbol* vectorvar = localGensym("VECTOR");
      ...
      { Cons* _return_temp = listO(4, cons(vectorvar, cons(collectiontype, cons(collectiontree, NIL))), cons(indexvar, listO(3, SGT_FOREACH_STELLA_INTEGER, wrapInteger(0), NIL)), cons(lengthvar, listO(3, SGT_FOREACH_STELLA_INTEGER, listO(3, SYM_FOREACH_STELLA_LENGTH, vectorvar, NIL), NIL)), NIL);

        _Return1 = cons(cons(valuevar, cons((((collectiontype == SGT_FOREACH_STELLA_STRING) ||
            (collectiontype == SGT_FOREACH_STELLA_MUTABLE_STRING)) ? SGT_FOREACH_STELLA_CHARACTER : ((StandardObject*)(extractParameterType(collectiontype, SYM_FOREACH_STELLA_ANY_VALUE, dummy1)))), cons(listO(4, SYM_FOREACH_STELLA_PROGN, listO(3, SYM_FOREACH_STELLA_SETQ, valuevar, cons(listO(3, SYM_FOREACH_STELLA_NTH, vectorvar, cons(indexvar, NIL)), NIL)), listO(3, SYM_FOREACH_STELLA_ii, indexvar, NIL), NIL), NIL))), NIL);
        _Return2 = listO(3, SYM_FOREACH_STELLA_l, indexvar, cons(lengthvar, NIL));
        return (_return_temp);
      }
    }
  }
...
Cons* yieldInCursorClausesForArgumentList(Object* collectiontree, StandardObject* collectiontype, Symbol* keyvar, Symbol* valuevar, Cons*& _Return1, Object*& _Return2) {
  { boolean dummy1;
    StandardObject* dummy2;
    boolean dummy3;
    ...
    if (!passVariableArgumentsAsListP(oMETHODBEINGWALKEDo.get())) {
      { ParametricTypeSpecifier* self000 = newParametricTypeSpecifier();
        ...
        { ParametricTypeSpecifier* iteratortype = self000;
          ...
          return (yieldInCursorClausesForGeneralCollection(sysTree(listO(5, SYM_FOREACH_STELLA_SYS_CALL_METHOD, SGT_FOREACH_STELLA_ARGUMENT_LIST_ITERATOR, SYM_FOREACH_STELLA_ALLOCATE_ITERATOR, walkWithoutTypeTree(collectiontree), NIL), iteratortype, dummy2), iteratortype, keyvar, valuevar, _Return1, _Return2));
        }
      }
    }
    { StandardObject* listifiedtype = yieldListifiedVariableArgumentsType(oMETHODBEINGWALKEDo.get());
      ...
      { Cons* iteratorclauses = NULL;
        ...
        iteratorclauses = yieldInCursorClausesForGeneralCollection(sysTree(collectiontree, listifiedtype, dummy4), listifiedtype, keyvar, valuevar, valueclauses, continuationtest);
        if (subTypeSpecOfP(elementtype, SGT_FOREACH_STELLA_LITERAL)) {
          { Cons* clause = NULL;
            Cons* iter000 = ((Cons*)(valueclauses));
...
Cons* yieldOnCursorClauses(Cons* ontree, Cons*& _Return1, Object*& _Return2) {
  { Object* iteratorvar = ontree->rest->value;
    ...
    { Object* collectiontree = NULL;
      ...
      { Cons* _return_temp = cons(cons(iteratorvar, listO(3, SGT_FOREACH_STELLA_ABSTRACT_ITERATOR, collectiontree, NIL)), NIL);

        _Return1 = NIL;
        _Return2 = listO(3, SYM_FOREACH_STELLA_NEXTp, iteratorvar, NIL);
        return (_return_temp);
      }
    }
  }
...
Cons* collectCursorClauses(Cons* cursorstree, boolean dontoptimizeP, Cons*& _Return1, Object*& _Return2) {
  { GeneralizedSymbol* testValue000 = ((GeneralizedSymbol*)(cursorstree->value));

    if (testValue000 == SYM_FOREACH_STELLA_IN) {
      return (yieldInCursorClauses(cursorstree, dontoptimizeP, _Return1, _Return2));
    }
    else if (testValue000 == SYM_FOREACH_STELLA_ON) {
      return (yieldOnCursorClauses(cursorstree, _Return1, _Return2));
    }
      ...
      { Cons* alliteratorclauses = NIL;
        ...
        { Object* clause = NULL;
          ...
          while (!(iter000 == NIL)) {
            ...
            { Cons* iteratorclauses = NULL;
              ...
              iteratorclauses = collectCursorClauses(((Cons*)(clause)), dontoptimizeP, valueclauses, continuationtest);
        ...
        { Cons* _return_temp = alliteratorclauses;
          ...
          _Return2 = ((continuationtests->length() == 1) ? continuationtests->value : cons(SYM_FOREACH_STELLA_AND, continuationtests->reverse()->remove(SYM_FOREACH_STELLA_TRUE)->concatenate(NIL, 0)));
          return (_return_temp);
        }
      }
    }
...
Cons* walkCursorsTree(Cons* cursorstree, boolean dontoptimizeP, Cons*& _Return1, Object*& _Return2) {
  { StandardObject* dummy1;
    ...
    { Cons* iteratorclauses = NULL;
      ...
      iteratorclauses = collectCursorClauses(cursorstree, dontoptimizeP, valueclauses, continuationtest);
      { Cons* clause = NULL;
      ...
      { Cons* clause = NULL;
        ...
        while (!(iter001 == NIL)) {
          ...
          pushVariableBinding(((Symbol*)(clause->value)), ((StandardObject*)(clause->rest->value)));
          clause->thirdSetter(walkWithoutTypeTree(clause->rest->rest->value));
        }
      }
...
Cons* walkPrefixCollectIntoTree(Cons* collectintotree, Cons* iterators, Cons* valuebindings, Cons* docode, Cons*& _Return1) {
  { Object* collecttree = collectintotree->rest->value;
    ...
    { Cons* _return_temp = iterators->concatenate(collectintobindings, 0);
...
Cons* walkPrefixForeachBody(Cons* iteratorclauses, Cons* valueclauses, Object* wheretest, Cons* doactions) {
  { Cons* otree = NULL;
...
Cons* walkPrefixForeachTree(Cons* tree, StandardObject*& _Return1) {
  { Cons* tail = NULL;
    ...
    iteratorclauses = walkCursorsTree(((Cons*)(tree->rest->value)), FALSE, valueclauses, continuationtest);
    ...
    if (((Cons*)(tail->value))->value == SYM_FOREACH_STELLA_DO) {
      ...
      ((Cons*)(tail->value))->rest = NIL;
      tail = tail->rest;
    }
    if (!(tail == NIL)) {
      iteratorclauses = walkPrefixCollectIntoTree(((Cons*)(tail->value)), iteratorclauses, valueclauses, doactions, doactions);
      tail->firstSetter(NULL);
    }
    _Return1 = SGT_FOREACH_STELLA_VOID;
    return (listO(3, SYM_FOREACH_STELLA_SYS_FOREACH, iteratorclauses, cons(valueclauses, cons(continuationtest, walkPrefixForeachBody(iteratorclauses, valueclauses, wheretest, doactions)->concatenate(NIL, 0)))));
  }
}
...
Cons* extractSetofNestedIteratorTree(Cons* iteratorclauses) {
  { Cons* iterclauses = ((Cons*)(iteratorclauses));
    Object* subtree = ((Cons*)(iterclauses->value))->rest->rest->value;

    ((Cons*)(iterclauses->value))->thirdSetter(NULL);
    if (!(iterclauses->rest == NIL)) {
      { 
        BIND_STELLA_SPECIAL(oPRINTREADABLYpo, boolean, TRUE);
        signalTranslationError();
        if (!(suppressWarningsP())) {
          printErrorContext(">> ERROR: ", STANDARD_ERROR);
          *(STANDARD_ERROR->nativeStream) << std::endl << " " << "Too many iterators in 'setof' expression" << "." << std::endl;
        }
      }
    }
    return (((Cons*)(subtree)));
...
Symbol* yieldSetofNextFunctionName(Cons* valueclauses, Cons* wheretest) {
  { Cons* onlyvalueclause = ((Cons*)(valueclauses->value));
    Object* valuevariable = onlyvalueclause->value;
    Object* valuetype = onlyvalueclause->rest->value;
    Symbol* nextfunctionname = NULL;

    nextfunctionname = gensym("SETOF-NEXT-P");
    walkAuxiliaryTree(listO(5, SYM_FOREACH_STELLA_DEFUN, cons(nextfunctionname, cons(SYM_FOREACH_STELLA_BOOLEAN, NIL)), cons(listO(3, SYM_FOREACH_STELLA_SELF, SYM_FOREACH_STELLA_ALL_PURPOSE_ITERATOR, NIL), NIL), listO(5, SYM_FOREACH_STELLA_LET, listO(3, listO(3, SYM_FOREACH_STELLA_NESTEDITERATOR, listO(3, SYM_FOREACH_STELLA_ITERATOR_NESTED_ITERATOR, SYM_FOREACH_STELLA_SELF, NIL), NIL), cons(valuevariable, cons(valuetype, cons(SYM_FOREACH_STELLA_NULL, NIL))), NIL), listO(5, SYM_FOREACH_STELLA_WHILE, listO(3, SYM_FOREACH_STELLA_NEXTp, SYM_FOREACH_STELLA_NESTEDITERATOR, NIL), listO(3, SYM_FOREACH_STELLA_SETQ, valuevariable, cons(listO(3, SYM_FOREACH_STELLA_VALUE, SYM_FOREACH_STELLA_NESTEDITERATOR, NIL), NIL)), listO(3, SYM_FOREACH_STELLA_WHEN, wheretest, listO(3, listO(4, SYM_FOREACH_STELLA_SETF, listO(3, SYM_FOREACH_STELLA_VALUE, SYM_FOREACH_STELLA_SELF, NIL), valuevariable, NIL), listO(3, SYM_FOREACH_STELLA_RETURN, SYM_FOREACH_STELLA_TRUE, NIL), NIL)), NIL), listO(3, SYM_FOREACH_STELLA_RETURN, SYM_FOREACH_STELLA_FALSE, NIL), NIL), NIL));
    return (nextfunctionname);
  }
}

Cons* walkSetofTree(Cons* tree, StandardObject*& _Return1) {
  tree = foreachToPrefixForeachTree(tree);
  { Cons* iteratorssubtree = ((Cons*)(tree->rest->value));
    Symbol* iteratorvariable = localGensym("ITER");
    Cons* tail = NULL;
    Cons* iteratorclauses = NULL;
    Cons* valueclauses = NULL;
    Object* continuationtest = NULL;
    ...
    tree->secondSetter(NULL);
    if (getQuotedTree("((ON AS) \"/STELLA\")", "/STELLA")->memberP(iteratorssubtree->value)) {
      { 
        BIND_STELLA_SPECIAL(oPRINTREADABLYpo, boolean, TRUE);
        signalTranslationError();
        if (!(suppressWarningsP())) {
          printErrorContext(">> ERROR: ", STANDARD_ERROR);
          *(STANDARD_ERROR->nativeStream) << std::endl << " " << "Illegal reserved word " << "`" << iteratorssubtree->value << "'" << " in 'setof' expression." << "." << std::endl;
        }
      }
    }
    iteratorclauses = walkCursorsTree(iteratorssubtree, TRUE, valueclauses, continuationtest);
    continuationtest = continuationtest;
    tail = ((Cons*)(tree->rest->rest));
    if ((!(tail == NIL)) &&
        (((Cons*)(tail->value))->value == SYM_FOREACH_STELLA_WHERE)) {
      wheretest = ((Cons*)(tail->value))->rest->value;
      ((Cons*)(tail->value))->secondSetter(NULL);
      tail = tail->rest;
    }
    if (!(tail == NIL)) {
      { 
        ...
        if (!(suppressWarningsP())) {
          printErrorContext(">> ERROR: ", STANDARD_ERROR);
          *(STANDARD_ERROR->nativeStream) << std::endl << " " << "Illegal 'setof' expression.  Syntax is " << std::endl << "   (SETOF <iterators> WHERE <boolean expression>)" << "." << std::endl;
        }
      }
    }
    _Return1 = SGT_FOREACH_STELLA_ALL_PURPOSE_ITERATOR;
    return (listO(6, SYM_FOREACH_STELLA_VRLET, cons(cons(iteratorvariable, cons(listO(3, SYM_FOREACH_STELLA_NEW, SYM_FOREACH_STELLA_ALL_PURPOSE_ITERATOR, NIL), NIL)), NIL), listO(4, SYM_FOREACH_STELLA_SETF, listO(3, SYM_FOREACH_STELLA_ITERATOR_NESTED_ITERATOR, iteratorvariable, NIL), extractSetofNestedIteratorTree(iteratorclauses), NIL), listO(4, SYM_FOREACH_STELLA_SETF, listO(3, SYM_FOREACH_STELLA_ITERATOR_NEXT_CODE, iteratorvariable, NIL), listO(4, SYM_FOREACH_STELLA_THE_CODE, KWD_FOREACH_FUNCTION, yieldSetofNextFunctionName(valueclauses, ((Cons*)(wheretest))), NIL), NIL), iteratorvariable, NIL));
  }
}
...
void helpStartupForeach1() {
  {
    ...
    SYM_FOREACH_STELLA_SETOF = ((Symbol*)(internRigidSymbolWrtModule("SETOF", NULL, 0)));
    SYM_FOREACH_STELLA_SET_OF = ((Symbol*)(internRigidSymbolWrtModule("SET-OF", NULL, 0)));
...
void helpStartupForeach2() {
  {
    ...
    SYM_FOREACH_STELLA_DEFUN = ((Symbol*)(internRigidSymbolWrtModule("DEFUN", NULL, 0)));
    SYM_FOREACH_STELLA_BOOLEAN = ((Symbol*)(internRigidSymbolWrtModule("BOOLEAN", NULL, 0)));
    SYM_FOREACH_STELLA_SELF = ((Symbol*)(internRigidSymbolWrtModule("SELF", NULL, 0)));
    SYM_FOREACH_STELLA_ALL_PURPOSE_ITERATOR = ((Symbol*)(internRigidSymbolWrtModule("ALL-PURPOSE-ITERATOR", NULL, 0)));
    SYM_FOREACH_STELLA_LET = ((Symbol*)(internRigidSymbolWrtModule("LET", NULL, 0)));
    SYM_FOREACH_STELLA_NESTEDITERATOR = ((Symbol*)(internRigidSymbolWrtModule("NESTEDITERATOR", NULL, 0)));
    SYM_FOREACH_STELLA_ITERATOR_NESTED_ITERATOR = ((Symbol*)(internRigidSymbolWrtModule("ITERATOR-NESTED-ITERATOR", NULL, 0)));
    SYM_FOREACH_STELLA_WHILE = ((Symbol*)(internRigidSymbolWrtModule("WHILE", NULL, 0)));
    SYM_FOREACH_STELLA_RETURN = ((Symbol*)(internRigidSymbolWrtModule("RETURN", NULL, 0)));
    SYM_FOREACH_STELLA_NEW = ((Symbol*)(internRigidSymbolWrtModule("NEW", NULL, 0)));
    SYM_FOREACH_STELLA_ITERATOR_NEXT_CODE = ((Symbol*)(internRigidSymbolWrtModule("ITERATOR-NEXT-CODE", NULL, 0)));
    SYM_FOREACH_STELLA_THE_CODE = ((Symbol*)(internRigidSymbolWrtModule("THE-CODE", NULL, 0)));
    KWD_FOREACH_FUNCTION = ((Keyword*)(internRigidSymbolWrtModule("FUNCTION", NULL, 2)));
    SGT_FOREACH_STELLA_ALL_PURPOSE_ITERATOR = ((Surrogate*)(internRigidSymbolWrtModule("ALL-PURPOSE-ITERATOR", NULL, 1)));
...
void startupForeach() {
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("YIELD-IN-CURSOR-CLAUSES", "(DEFUN (YIELD-IN-CURSOR-CLAUSES CONS CONS OBJECT) ((INTREE CONS) (DONTOPTIMIZE? BOOLEAN)))", ((cpp_function_code)(&yieldInCursorClauses)), NULL);
      ...
      defineFunctionObject("YIELD-IN-CURSOR-CLAUSES-FOR-GENERAL-COLLECTION", "(DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-GENERAL-COLLECTION CONS CONS OBJECT) ((COLLECTIONTREE OBJECT) (COLLECTIONTYPE TYPE-SPEC) (KEYVAR SYMBOL) (VALUEVAR SYMBOL)))", ((cpp_function_code)(&yieldInCursorClausesForGeneralCollection)), NULL);
      defineFunctionObject("YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL", "(DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL CONS CONS OBJECT) ((COLLECTIONTREE CONS) (COLLECTIONTYPE TYPE-SPEC) (KEYVAR SYMBOL) (VALUEVAR SYMBOL)))", ((cpp_function_code)(&yieldInCursorClausesForIntegerInterval)), NULL);
      defineFunctionObject("YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR", "(DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR CONS CONS OBJECT) ((COLLECTIONTREE OBJECT) (COLLECTIONTYPE TYPE-SPEC) (KEYVAR SYMBOL) (VALUEVAR SYMBOL)))", ((cpp_function_code)(&yieldInCursorClausesForVector)), NULL);
      defineFunctionObject("YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST", "(DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST CONS CONS OBJECT) ((COLLECTIONTREE OBJECT) (COLLECTIONTYPE TYPE-SPEC) (KEYVAR SYMBOL) (VALUEVAR SYMBOL)))", ((cpp_function_code)(&yieldInCursorClausesForArgumentList)), NULL);
      defineFunctionObject("YIELD-ON-CURSOR-CLAUSES", "(DEFUN (YIELD-ON-CURSOR-CLAUSES CONS CONS OBJECT) ((ONTREE CONS)))", ((cpp_function_code)(&yieldOnCursorClauses)), NULL);
      defineFunctionObject("COLLECT-CURSOR-CLAUSES", "(DEFUN (COLLECT-CURSOR-CLAUSES CONS CONS OBJECT) ((CURSORSTREE CONS) (DONTOPTIMIZE? BOOLEAN)))", ((cpp_function_code)(&collectCursorClauses)), NULL);
      defineFunctionObject("WALK-CURSORS-TREE", "(DEFUN (WALK-CURSORS-TREE CONS CONS OBJECT) ((CURSORSTREE CONS) (DONTOPTIMIZE? BOOLEAN)))", ((cpp_function_code)(&walkCursorsTree)), NULL);
      ...
      defineFunctionObject("WALK-PREFIX-COLLECT-INTO-TREE", "(DEFUN (WALK-PREFIX-COLLECT-INTO-TREE CONS CONS) ((COLLECTINTOTREE CONS) (ITERATORS CONS) (VALUEBINDINGS CONS) (DOCODE CONS)))", ((cpp_function_code)(&walkPrefixCollectIntoTree)), NULL);
      ...
      defineFunctionObject("WALK-PREFIX-FOREACH-BODY", "(DEFUN (WALK-PREFIX-FOREACH-BODY CONS) ((ITERATORCLAUSES CONS) (VALUECLAUSES CONS) (WHERETEST OBJECT) (DOACTIONS CONS)))", ((cpp_function_code)(&walkPrefixForeachBody)), NULL);
      ...
      defineFunctionObject("EXTRACT-SETOF-NESTED-ITERATOR-TREE", "(DEFUN (EXTRACT-SETOF-NESTED-ITERATOR-TREE CONS) ((ITERATORCLAUSES CONS)))", ((cpp_function_code)(&extractSetofNestedIteratorTree)), NULL);
      defineFunctionObject("YIELD-SETOF-NEXT-FUNCTION-NAME", "(DEFUN (YIELD-SETOF-NEXT-FUNCTION-NAME SYMBOL) ((VALUECLAUSES CONS) (WHERETEST CONS)))", ((cpp_function_code)(&yieldSetofNextFunctionName)), NULL);
      defineFunctionObject("WALK-SETOF-TREE", "(DEFUN (WALK-SETOF-TREE CONS TYPE-SPEC) ((TREE CONS)))", ((cpp_function_code)(&walkSetofTree)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_FOREACH_STELLA_STARTUP_FOREACH);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_FOREACH_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Foreach"), NULL_STRING_WRAPPER);
      }
    }
...
Symbol* SYM_FOREACH_STELLA_SETOF = NULL;

Symbol* SYM_FOREACH_STELLA_SET_OF = NULL;
...
Symbol* SYM_FOREACH_STELLA_DEFUN = NULL;

Symbol* SYM_FOREACH_STELLA_BOOLEAN = NULL;

Symbol* SYM_FOREACH_STELLA_SELF = NULL;

Symbol* SYM_FOREACH_STELLA_ALL_PURPOSE_ITERATOR = NULL;

Symbol* SYM_FOREACH_STELLA_LET = NULL;

Symbol* SYM_FOREACH_STELLA_NESTEDITERATOR = NULL;

Symbol* SYM_FOREACH_STELLA_ITERATOR_NESTED_ITERATOR = NULL;

Symbol* SYM_FOREACH_STELLA_WHILE = NULL;

Symbol* SYM_FOREACH_STELLA_RETURN = NULL;

Symbol* SYM_FOREACH_STELLA_NEW = NULL;

Symbol* SYM_FOREACH_STELLA_ITERATOR_NEXT_CODE = NULL;

Symbol* SYM_FOREACH_STELLA_THE_CODE = NULL;

Keyword* KWD_FOREACH_FUNCTION = NULL;

Surrogate* SGT_FOREACH_STELLA_ALL_PURPOSE_ITERATOR = NULL;
