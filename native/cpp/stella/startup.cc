...
int oSTELLA_MINOR_VERSION_NUMBERo = 3;

char* oSTELLA_RELEASE_STATEo = "";

int oSTELLA_PATCH_LEVELo = 2;
  ...
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_STARTUP_STELLA_STARTUP_STARTUP);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_STARTUP_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Startup"), NULL_STRING_WRAPPER);
      }
    }
    ...
    if (currentStartupTimePhaseP(9)) {
      ...
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *STELLA-MINOR-VERSION-NUMBER* INTEGER 3 :PUBLIC? TRUE)");
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *STELLA-RELEASE-STATE* STRING \"\" :PUBLIC? TRUE)");
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *STELLA-PATCH-LEVEL* INTEGER 2 :PUBLIC? TRUE)");
