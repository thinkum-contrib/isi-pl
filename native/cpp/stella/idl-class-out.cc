...
Cons* idlYieldOperationSignatureTree(MethodSlot* method) {
  { MethodSlot* mostgeneralmethod = method;
    ...
    return (cons((method->methodFunctionP ? NULL : idlTranslateClassName(internSymbolInModule(method->slotOwner->symbolName, ((Module*)(method->slotOwner->homeContext)), TRUE))), listO(3, cons(idlTranslateTypeSpec(returntype), NIL), idlTranslateFunctionName(method), cons(idlTranslateMethodParameters(method), NIL))));
  }
}

Cons* idlYieldConstructorSignatureTree(MethodSlot* method) {
  oMETHODBEINGWALKEDo.set(method);
  return (cons(idlTranslateClassName(internSymbolInModule(method->slotOwner->symbolName, ((Module*)(method->slotOwner->homeContext)), TRUE)), listO(3, NIL, idlTranslateConstructorName(method), cons(idlTranslateMethodParameters(method), NIL))));
}
  ...
  { Class* clasS = ((Class*)(unit->theObject));
    ...
    { Surrogate* super = NULL;
      ...
      while (!(iter000 == NIL)) {
        ...
        if (!((boolean)(collect000))) {
          {
            collect000 = cons(idlTranslateClassName(internSymbolInModule(super->symbolName, ((Module*)(super->homeContext)), TRUE)), NIL);
          ...
          {
            collect000->rest = cons(idlTranslateClassName(internSymbolInModule(super->symbolName, ((Module*)(super->homeContext)), TRUE)), NIL);
            collect000 = collect000->rest;
          }
        }
      }
  ...
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_IDL_CLASS_OUT_STELLA_STARTUP_IDL_CLASS_OUT);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_IDL_CLASS_OUT_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Idl-Class-Out"), NULL_STRING_WRAPPER);
      }
    }
