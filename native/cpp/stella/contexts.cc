...
boolean oUNLINK_DISCARDED_CONTEXTS_ON_READpo = FALSE;

// Eliminate pointers to discarded contexts while
// inserting into a CS-VALUE context table.
boolean oUNLINK_DISCARDED_CONTEXTS_ON_WRITEpo = FALSE;
...
void startupContexts() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oUNLINK_DISCARDED_CONTEXTS_ON_READpo = TRUE;
      oUNLINK_DISCARDED_CONTEXTS_ON_WRITEpo = TRUE;
      oCONTEXT_BACKTRACKING_MODEo = FALSE;
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_CONTEXTS_STELLA_STARTUP_CONTEXTS);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_CONTEXTS_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Contexts"), NULL_STRING_WRAPPER);
      }
    }
