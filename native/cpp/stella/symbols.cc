...
Cons* yieldVisibleRigidSymbolsWrtModule(char* name, Module* module, int kindofsym) {
  { Cons* listofsymbols = NIL;
    ...
    { Module* visiblemodule = NULL;
      Iterator* iter000 = visibleModules(module);
...
int lookupRigidSymbolOffsetWrtModule(char* name, Module* module, int kindofsym) {
  { StringToIntegerHashTable* offsettable = selectSymbolOffsetTable(module, kindofsym);
    ...
    { Module* super = NULL;
      Iterator* iter000 = visibleModules(module)->advance(1);
...
GeneralizedSymbol* lookupRigidSymbol(char* name, int kindofsym) {
  ...
  { Module* module = oMODULEo.get();
    ...
    if (qualifiedNameP(name)) {
      module = computeModuleAndBareName(name, name);
    }
    if (((boolean)(module))) {
      return (lookupRigidSymbolWrtModule(name, module, kindofsym));
    }
    else {
      return (NULL);
    }
  ...
  { Module* currentmodule = oMODULEo.get();
    ...
    { 
      ...
      if (((boolean)(oMODULEo.get()))) {
        return (((Symbol*)(internRigidSymbolLocally((oMODULEo.get()->caseSensitiveP ? newname : stringUpcase(newname)), oMODULEo.get(), SYMBOL_SYM))));
      }
      oMODULEo.set(currentmodule);
      return (internSymbol(newname));
    }
  }
}
...
Surrogate* Cons::surrogatify() {
  { Cons* self = this;
    ...
    if (self == NIL) {
      return (internSurrogate("NIL"));
    }
    else {
      throw *newStellaException("Cons passed to 'surrogatify'");
    }
  }
}
...
Surrogate* Object::surrogatify() {
  { Object* self = this;
    ...
    throw *newStellaException("OBJECT passed to 'surrogatify'");
  }
}
...
boolean qualifiedNameP(char* name) {
  if (strlen(name) == 0) {
    return (FALSE);
  }
  else if (stringMemberP(name, MODULE_SEPARATOR_CHARACTER)) {
    if (strlen(name) > 1) {
      if (oCHARACTER_TYPE_TABLEo[(int)(unsigned char) (name[0])] == KWD_SYMBOLS_DIGIT) {
        return (FALSE);
      }
      if ((name[0] == '.') &&
          (oCHARACTER_TYPE_TABLEo[(int)(unsigned char) (name[1])] == KWD_SYMBOLS_DIGIT)) {
        return (FALSE);
      }
      return (TRUE);
    }
...
GeneralizedSymbol* internGeneralizedSymbol(char* name) {
  if (qualifiedNameP(name)) {
    return (internQualifiedName(name, TRUE));
  }
  else if (keywordNameP(name)) {
    return (internKeyword(stringRest(name)));
  }
  else if (surrogateNameP(name)) {
    return (internSurrogate(stringRest(name)));
  }
  else {
...
void helpStartupSymbols2() {
  {
    ...
    defineFunctionObject("LOOKUP-RIGID-SYMBOL", "(DEFUN (LOOKUP-RIGID-SYMBOL GENERALIZED-SYMBOL) ((NAME STRING) (KINDOFSYM INTEGER)) :DOCUMENTATION \"Return the symbol named `name'.  Case sensitive.\")", ((cpp_function_code)(&lookupRigidSymbol)), NULL);
...
void helpStartupSymbols3() {
  {
    ...
    defineFunctionObject("SYMBOLIZE", "(DEFUN (SYMBOLIZE SYMBOL) ((SURROGATE SURROGATE)) :DOCUMENTATION \"Convert `surrogate' into a symbol with the same name and module.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (TYPE-TO-SYMBOL SURROGATE)))", ((cpp_function_code)(&symbolize)), NULL);
    defineFunctionObject("STRING-TO-SURROGATE", "(DEFUN (STRING-TO-SURROGATE SURROGATE) ((SELF STRING)) :PUBLIC? TRUE)", ((cpp_function_code)(&stringToSurrogate)), NULL);
    defineFunctionObject("SYMBOL-TO-SURROGATE", "(DEFUN (SYMBOL-TO-SURROGATE SURROGATE) ((SELF SYMBOL)) :PUBLIC? TRUE)", ((cpp_function_code)(&symbolToSurrogate)), NULL);
    defineMethodObject("(DEFMETHOD (SURROGATIFY SURROGATE) ((SELF SURROGATE)) :PUBLIC? TRUE)", ((cpp_method_code)(&Surrogate::surrogatify)), ((cpp_method_code)(NULL)));
    defineMethodObject("(DEFMETHOD (SURROGATIFY SURROGATE) ((SELF SYMBOL)) :PUBLIC? TRUE)", ((cpp_method_code)(&Symbol::surrogatify)), ((cpp_method_code)(NULL)));
    defineMethodObject("(DEFMETHOD (SURROGATIFY SURROGATE) ((SELF STRING)) :PUBLIC? TRUE)", ((cpp_method_code)(NULL)), ((cpp_method_code)(NULL)));
    defineMethodObject("(DEFMETHOD (SURROGATIFY SURROGATE) ((SELF CONS)) :PUBLIC? TRUE)", ((cpp_method_code)(&Cons::surrogatify)), ((cpp_method_code)(NULL)));
    ...
    defineFunctionObject("SURROGATE-NAME?", "(DEFUN (SURROGATE-NAME? BOOLEAN) ((NAME STRING)) :PUBLIC? TRUE)", ((cpp_function_code)(&surrogateNameP)), NULL);
    defineFunctionObject("QUALIFIED-NAME?", "(DEFUN (QUALIFIED-NAME? BOOLEAN) ((NAME STRING)) :PUBLIC? TRUE)", ((cpp_function_code)(&qualifiedNameP)), NULL);
    defineFunctionObject("KEYWORD-NAME?", "(DEFUN (KEYWORD-NAME? BOOLEAN) ((NAME STRING)) :PUBLIC? TRUE)", ((cpp_function_code)(&keywordNameP)), NULL);
    defineFunctionObject("INTERN-GENERALIZED-SYMBOL", "(DEFUN (INTERN-GENERALIZED-SYMBOL GENERALIZED-SYMBOL) ((NAME STRING)) :PUBLIC? TRUE)", ((cpp_function_code)(&internGeneralizedSymbol)), NULL);
    ...
    { MethodSlot* function = lookupFunction(SYM_SYMBOLS_STELLA_STARTUP_SYMBOLS);
      ...
      setDynamicSlotValue(function->dynamicSlots, SYM_SYMBOLS_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Symbols"), NULL_STRING_WRAPPER);
    }
  }
}
