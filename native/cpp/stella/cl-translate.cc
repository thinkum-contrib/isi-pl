...
Object* Symbol::clTranslateAtomicTree() {
  { Symbol* tree = this;
    ...
    if (((boolean)(tree->lookupGlobalVariable()))) {
...
Object* yieldGlobalLispSymbol(Module* symbolmodule, char* symbolname) {
  { char* lispsymbolpackage = (((!((boolean)(symbolmodule))) ||
    ...
    char* lispsymbolname = ((((StringWrapper*)(dynamicSlotValue(symbolmodule->dynamicSlots, SYM_CL_TRANSLATE_STELLA_MODULE_LISP_PACKAGE, NULL_STRING_WRAPPER)))->wrapperValue != NULL) ? symbolname : computeFullName(symbolname, symbolmodule));
...
Object* clTranslateLocalSymbol(Symbol* symbol) {
  { Module* symbolmodule = ((Module*)(symbol->homeContext));
    ...
    if ((!oMODULEo.get()->caseSensitiveP) &&
        ((!((boolean)(symbolmodule))) ||
         (symbolmodule == oMODULEo.get()))) {
      return (symbol);
    }
    else if (oMODULEo.get()->caseSensitiveP) {
      return (newVerbatimStringWrapper(stringConcatenate("|", symbol->symbolName, 1, "|")));
    }
    else {
      return (newVerbatimStringWrapper(symbol->symbolName));
    }
  }
}
...
Cons* clTranslateForeachTree(Cons* tree) {
  { Cons* iteratorbindings = ((Cons*)(tree->rest->value));
    ...
    Object* continuationtest = clTranslateBooleanTest(tree->fourth(), FALSE);
    Cons* body = clTranslateListOfTrees(tree->nthRest(4));
    ...
    if (generateClTypeDeclarationsP()) {
      typedeclarations = typedeclarations->concatenate(clYieldLetVariableTypeDeclarationTrees(cursorbindings), 0);
    }
    { Cons* binding = NULL;
      ...
      while (!(iter001 == NIL)) {
        ...
        nextassignments = cons(clTranslateATree(binding->rest->rest->value), nextassignments);
        binding->thirdSetter(NULL);
        if (!(duplicateCursorVariableP(((Symbol*)(binding->value)), iteratorbindings))) {
          declarations = cons(cons(clTranslateLocalSymbol(((Symbol*)(binding->value))), cons(clTranslateATree(typeToWalkedNullValueTree(typeSpecToBaseType(((StandardObject*)(binding->rest->value))))), NIL)), declarations);
        }
      }
    }
    return (listO(3, internCommonLispSymbol("LET*"), declarations, clYieldDeclareTree(typedeclarations, FALSE)->concatenate(cons(listO(4, internCommonLispSymbol("LOOP"), SYM_CL_TRANSLATE_STELLA_WHILE, continuationtest, cons(SYM_CL_TRANSLATE_STELLA_DO, nextassignments->reverse()->concatenate(maybeWrapWithContinueLabel(body)->concatenate(NIL, 0), 0))), NIL), 0)));
  }
}
        ...
        if ((operatoR == SYM_CL_TRANSLATE_STELLA_l) ||
          ...
          if (methodCallInliningEnabledP()) {
            if ((owner == SGT_CL_TRANSLATE_STELLA_OBJECT) ||
              ...
              tree = listO(3, internCommonLispSymbol("EQ"), clTranslateATree(arguments->value), cons(clTranslateATree(typeToWalkedNullValueTree(owner)), NIL));
...
Cons* clTranslateMakeTree(Cons* tree) {
  { Class* clasS = ((Class*)(typify(tree->rest->value)->surrogateValue));
    ...
    if (exceptionClassP(clasS)) {
      otree = listO(4, internCommonLispSymbol("MAKE-CONDITION"), listO(3, internCommonLispSymbol("QUOTE"), classsymbol, NIL), clTranslateGlobalSymbol(SYM_CL_TRANSLATE_STELLA_oCONDITION_MESSAGE_KEYWORDo), cons(clTranslateATree(tree->rest->rest->value), NIL));
    }
...
Cons* clYieldMethodTypeDeclarationTree(MethodSlot* method) {
  ...
  { Object* cltype = lookupClTypeFromStellaType(method->slotBaseType);
    ...
    if (((boolean)(cltype))) {
      return (listO(4, internCommonLispSymbol("FTYPE"), listO(3, internCommonLispSymbol("FUNCTION"), clYieldMethodArglistTypeDeclarations(method), cons(clYieldMethodReturnTypeDeclaration(method), NIL)), clTranslateGlobalSymbol(method->slotName), NIL));
    }
    else {
      return (NULL);
    }
...
Cons* clTranslateDefineMethodUnit(TranslationUnit* unit) {
  { MethodSlot* method = ((MethodSlot*)(unit->theObject));
    ...
    if (((boolean)(methoddeclaration))) {
      otree = listO(3, internCommonLispSymbol("PROGN"), otree, cons(listO(3, internCommonLispSymbol("DECLAIM"), methoddeclaration, NIL), NIL));
    }
    return (otree);
  }
}
...
Cons* yieldClosSlotTypeTree(StorageSlot* slot) {
  { Object* cltype = lookupClTypeFromStellaType(slot->slotBaseType);
    ...
    if (((boolean)(cltype))) {
      return (listO(3, KWD_CL_TRANSLATE_TYPE, cltype, listO(3, KWD_CL_TRANSLATE_INITFORM, clTranslateATree(typeToWalkedNullValueTree(slot->slotBaseType)), NIL)));
    }
    else {
      return (NIL);
    }
...
Cons* yieldStructSlotTree(StorageSlot* slot) {
  { Object* cltype = lookupClTypeFromStellaType(slot->slotBaseType);
    ...
    if (((boolean)(cltype))) {
      return (cons(clTranslateLocalSymbol(slot->slotName), cons(clTranslateATree(typeToWalkedNullValueTree(slot->slotBaseType)), listO(3, KWD_CL_TRANSLATE_TYPE, cltype, NIL))));
    }
    else {
...
void startupClTranslate() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oNEEDEXPLICITRETURNpo.set(FALSE);
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_CL_TRANSLATE_STELLA_STARTUP_CL_TRANSLATE);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_CL_TRANSLATE_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Cl-Translate"), NULL_STRING_WRAPPER);
      }
    }
