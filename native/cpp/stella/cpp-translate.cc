            ...
            switch (body[cursor]) {
              case '/': 
                { InputStringStream* comment = newInputStringStream(stringSubsequence(body, cursor, NULL_INTEGER));
                  ...
                  cursor = cursor + strlen((readLine(comment, dummy2)));
                }
              break;
              case '*': 
  ...
  { GlobalVariable* global = ((GlobalVariable*)(unit->theObject));
    ...
    if (global->variableSpecialP) {
      if ((!((boolean)(initialvaluetree))) &&
        ...
        initialvaluetree = cppTranslateATree(typeToWalkedNullValueTree(typeSpecToBaseType(type)));
      }
...
Cons* cppTranslateForeachTree(Cons* tree) {
  { Cons* iteratorbindings = ((Cons*)(tree->rest->value));
    ...
    Object* continuationtest = cppTranslateATree(tree->fourth());
    Cons* body = cppTranslateListOfTrees(tree->nthRest(4));
    ...
    { Cons* binding = NULL;
      ...
      while (!(iter000 == NIL)) {
        ...
        iter000 = iter000->rest;
        nextassignments = cons(cppTranslateATree(binding->rest->rest->value), nextassignments);
        declaration = cons(cppTranslateAndPointerizeTypeSpec(((StandardObject*)(binding->rest->value))), cons(cppTranslateATree(binding->value), cons(cppTranslateATree(typeToWalkedNullValueTree(typeSpecToBaseType(((StandardObject*)(binding->rest->value))))), NIL)));
        if (!((boolean)(collect000))) {
          {
    ...
    { Cons* binding = NULL;
      ...
      while (!(iter001 == NIL)) {
        ...
        if (subTypeSpecOfP(((StandardObject*)(binding->rest->value)), SGT_CPP_TRANSLATE_STELLA_ARGUMENT_LIST_ITERATOR)) {
          ...
          declarations = declarations->concatenate(cppYieldVaIteratorDeclarations(varargsiterator), 0);
          continue;
        }
        else {
        ...
        if (!((boolean)(collect001))) {
          {
            collect001 = cons(declaration, NIL);
            if (declarations == NIL) {
              declarations = collect001;
            }
            else {
              addConsToEndOfConsList(declarations, collect001);
            }
          }
        }
        else {
          {
            collect001->rest = cons(declaration, NIL);
            collect001 = collect001->rest;
          }
        }
      }
    }
    return (listO(3, SYM_CPP_TRANSLATE_STELLA_CPP_BLOCK, declarations, cons(cons(SYM_CPP_TRANSLATE_STELLA_CPP_STATEMENTS, ((((boolean)(varargsiterator)) ? cppYieldVaStartTree(varargsiterator) : NIL))->concatenate(cons(listO(3, SYM_CPP_TRANSLATE_STELLA_CPP_FOREACH, continuationtest, cons(nextassignments->reverse(), cons(body, NIL))), ((((boolean)(varargsiterator)) ? cppYieldVaEndTree(varargsiterator) : NIL))->concatenate(NIL, 0)), 0)), NIL)));
  }
}
    ...
    if (subtypeOfParametricTypeSpecifierP(testValue000)) {
      ...
      { StandardObject* typespec001 = typespec;
        ...
        if (((StringWrapper*)(dynamicSlotValue(((Class*)(typespec->surrogateValue))->dynamicSlots, SYM_CPP_TRANSLATE_STELLA_CLASS_CPP_NATIVE_TYPE, NULL_STRING_WRAPPER)))->wrapperValue != NULL) {
          ...
          return (cppTranslateClassTypename(typeToSymbol(((Class*)(typespec->surrogateValue))->classType)));
        }
      }
    }
    else {
...
char* cppChangeCase(GeneralizedSymbol* symbol, Keyword* caseconvention) {
  { MemoizationTable* memoTable000 = ((MemoizationTable*)(SGT_CPP_TRANSLATE_STELLA_CPP_CHANGE_CASE_MEMO_TABLE_000->surrogateValue));
    Cons* memoizedEntry000 = NULL;
    Object* memoizedValue000 = NULL;

    if (!((boolean)(memoTable000))) {
      initializeMemoizationTable(SGT_CPP_TRANSLATE_STELLA_CPP_CHANGE_CASE_MEMO_TABLE_000, "(:MAX-VALUES 2000)");
      memoTable000 = ((MemoizationTable*)(SGT_CPP_TRANSLATE_STELLA_CPP_CHANGE_CASE_MEMO_TABLE_000->surrogateValue));
    }
    ...
    { Object* value000 = memoizedValue000;
      ...
      return (((StringWrapper*)(value000))->wrapperValue);
    }
  }
}
...
char* helpCppChangeCase(GeneralizedSymbol* symbol, Keyword* caseconvention) {
  { Module* module = ((Module*)(symbol->homeContext));
    ...
    if (module->caseSensitiveP ||
        (!(computeSymbolEscapeCode(name, FALSE) == KWD_CPP_TRANSLATE_UNESCAPED))) {
    ...
    if (caseconvention == KWD_CPP_TRANSLATE_UPPERCASE) {
      ...
      return (makeMatchingNameFull(name, KWD_CPP_TRANSLATE_CLEVER, KWD_CPP_TRANSLATE_CLEVER, "- ", "", "", "", "", KWD_CPP_TRANSLATE_TITLECASEX, ""));
    }
    else if (caseconvention == KWD_CPP_TRANSLATE_CAPITALIZED) {
      return (makeMatchingNameFull(name, KWD_CPP_TRANSLATE_CLEVER, KWD_CPP_TRANSLATE_CLEVER, "- ", "", "", "", "", KWD_CPP_TRANSLATE_TITLECASE, ""));
    }
    else {
      { OutputStringStream* stream000 = newOutputStringStream();
...
StringWrapper* cppTranslateMethodName(MethodSlot* method) {
  { Surrogate* methodtype = method->slotOwner;
    ...
    if (cppMethodObjectIsFunctionP(method)) {
      ...
      return (wrapString(stringConcatenate(cppTranslateClassName(internSymbolInModule(methodtype->symbolName, ((Module*)(methodtype->homeContext)), TRUE))->wrapperValue, "::", 1, translatedname->wrapperValue)));
    }
  }
}
  ...
  { Cons* otree = NULL;
    ...
    if (returnassignments == NIL) {
      ...
      if (needtemporaryreturnvariableP) {
        { char* tempvar = "_return_temp";

          otree = listO(4, SYM_CPP_TRANSLATE_STELLA_CPP_BLOCK, cons(cons(cppTranslateAndPointerizeTypeSpec(oMETHODBEINGWALKEDo.get()->computeReturnTypeSpec(oMETHODBEINGWALKEDo.get()->slotOwner)), listO(3, listO(3, SYM_CPP_TRANSLATE_STELLA_CPP_IDENT, wrapString(tempvar), NIL), translatedreturnexpression, NIL)), NIL), cons(SYM_CPP_TRANSLATE_STELLA_CPP_STATEMENTS, returnassignments->concatenate(cons(listO(3, SYM_CPP_TRANSLATE_STELLA_CPP_RETURN, listO(3, SYM_CPP_TRANSLATE_STELLA_CPP_IDENT, wrapString(tempvar), NIL), NIL), NIL), 0)), NIL);
        }
      }
      else {
...
Cons* cppTranslateDefinedPMethodCall(Surrogate* argumenttype, Object* argument, boolean nullP) {
  { Object* nullvalue = typeToWalkedNullValueTree(argumenttype);
...
void helpStartupCppTranslate4() {
  {
    ...
    SGT_CPP_TRANSLATE_STELLA_CPP_CHANGE_CASE_MEMO_TABLE_000 = ((Surrogate*)(internRigidSymbolWrtModule("CPP-CHANGE-CASE-MEMO-TABLE-000", NULL, 1)));
    KWD_CPP_TRANSLATE_UNESCAPED = ((Keyword*)(internRigidSymbolWrtModule("UNESCAPED", NULL, 2)));
  ...
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_CPP_TRANSLATE_STELLA_STARTUP_CPP_TRANSLATE);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_CPP_TRANSLATE_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Cpp-Translate"), NULL_STRING_WRAPPER);
      }
    }
...
Surrogate* SGT_CPP_TRANSLATE_STELLA_CPP_CHANGE_CASE_MEMO_TABLE_000 = NULL;

Keyword* KWD_CPP_TRANSLATE_UNESCAPED = NULL;
