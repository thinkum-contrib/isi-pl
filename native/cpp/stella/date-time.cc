...
void formatDatePrintString(char* string, char flag, int start, int end, OutputStream* stream) {
  if (end < 0) {
    end = strlen(string);
  }
  end = end - 1;
...
void formatDateAmPmHandler(DecodedDateTime* date, char cont, char flag, OutputStream* stream) {
  cont = cont;
  { char* ampm = ((date->dateTimeHour < 12) ? (char*)"AM" : (char*)"PM");
...
void formatDateToStream(Object* date, Object* timezone, char* control, OutputStream* stream) {
...
char* formatDate(Object* date, Object* timezone, char* control) {
...
void startupDateTime() {
  { 
    ...
    if (currentStartupTimePhaseP(9)) {
      ...
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *FORMAT-DATE-HANDLERS* (VECTOR OF FUNCTION-CODE-WRAPPER) (NEW VECTOR :ARRAY-SIZE 128) :DOCUMENTATION \"Handler functions indexed by the character code of their control character.\")");
