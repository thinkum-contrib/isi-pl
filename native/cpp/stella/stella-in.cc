...
Symbol* yieldSetterMethodName(Symbol* methodname) {
  return (internDerivedSymbol(methodname, stringConcatenate(methodname->symbolName, "-SETTER", 0)));
}
    ...
    if (testValue000 == SGT_STELLA_IN_STELLA_CONS) {
      { Object* nametree000 = nametree;
        ...
        if (nametree->rest->value == SYM_STELLA_IN_STELLA_SETTER) {
          _Return1 = NIL;
          return (yieldSetterMethodName(((Symbol*)(nametree->value))));
        }
        else {
...
void startupStellaIn() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oREDEFINE_IDENTICAL_CLASSESpo = FALSE;
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_STELLA_IN_STELLA_STARTUP_STELLA_IN);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_STELLA_IN_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Stella-In"), NULL_STRING_WRAPPER);
      }
    }
