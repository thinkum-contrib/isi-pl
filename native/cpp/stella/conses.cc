...
DEFINE_STELLA_SPECIAL(oPRINTPRETTYpo, boolean , FALSE);
...
DEFINE_STELLA_SPECIAL(oPRINTPRETTYCODEpo, boolean , FALSE);
...
void helpStartupConses1() {
  {
    ...
    defineFunctionObject("MAP-NULL-TO-NIL", "(DEFUN (MAP-NULL-TO-NIL CONS) ((SELF CONS)) :DOCUMENTATION \"Return `nil' iff `self' is `null' or `self' otherwise.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (CHOOSE (NULL? SELF) NIL SELF)))", ((cpp_function_code)(&mapNullToNil)), NULL);
...
void startupConses() {
  ...
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oPRINTPRETTYpo.set(TRUE);
      oPRINTREADABLYpo.set(FALSE);
      ELIPSIS = SYM_CONSES_STELLA_ddd;
      oPRINTPRETTYCODEpo.set(TRUE);
      oDEPTHEXCEEDEDpo.set(FALSE);
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_CONSES_STELLA_STARTUP_CONSES);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_CONSES_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Conses"), NULL_STRING_WRAPPER);
      }
    }
