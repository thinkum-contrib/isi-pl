...
Object* collect(Symbol* collectvariable, Cons* body) {
  // Use a VRLET to collect values.  Input has the form
  // '(collect <x> in <expression> where (<test> <x>))'.
  { Symbol* collection = gensym("V");

    return (listO(5, SYM_MACROS_STELLA_VRLET, cons(cons(collection, cons(SYM_MACROS_STELLA_NIL, NIL)), NIL), listO(3, SYM_MACROS_STELLA_FOREACH, collectvariable, body->concatenate(listO(3, SYM_MACROS_STELLA_COLLECT, collectvariable, listO(3, SYM_MACROS_STELLA_INTO, collection, NIL)), 0)), collection, NIL));
  }
}
...
Object* COERCE_AREST_TO_CONS(Symbol* restvariable) {
      ...
      { PropertyList* options = self000;
        ...
        if (!processcmdlineargsP) {
          if (oTRANSLATOROUTPUTLANGUAGEo.get() == KWD_MACROS_COMMON_LISP) {
            return (listO(6, SYM_MACROS_STELLA_DEFUN, SYM_MACROS_STELLA_MAIN, NIL, KWD_MACROS_PUBLICp, SYM_MACROS_STELLA_TRUE, startupfunctioncall->concatenate(body->concatenate(NIL, 0), 0)));
          }
            ...
            return (listO(6, SYM_MACROS_STELLA_DEFUN, listO(3, SYM_MACROS_STELLA_MAIN, SYM_MACROS_STELLA_INTEGER, NIL), NIL, KWD_MACROS_PUBLICp, SYM_MACROS_STELLA_TRUE, startupfunctioncall->concatenate(body->concatenate(cons(listO(3, SYM_MACROS_STELLA_RETURN, wrapInteger(1), NIL), NIL), 0), 0)));
          }
          else if (oTRANSLATOROUTPUTLANGUAGEo.get() == KWD_MACROS_JAVA) {
            return (listO(6, SYM_MACROS_STELLA_DEFUN, SYM_MACROS_STELLA_MAIN, cons(cons(localGensym("ARGV"), cons(listO(5, SYM_MACROS_STELLA_ARRAY, NIL, SYM_MACROS_STELLA_OF, SYM_MACROS_STELLA_STRING, NIL), NIL)), NIL), KWD_MACROS_PUBLICp, SYM_MACROS_STELLA_TRUE, startupfunctioncall->concatenate(body->concatenate(NIL, 0), 0)));
          }
          else {
          }
        }
...
void helpStartupMacros1() {
  {
    ...
    SYM_MACROS_STELLA_MAIN = ((Symbol*)(internRigidSymbolWrtModule("MAIN", NULL, 0)));
...
void startupMacros() {
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("COLLECT", "(DEFUN COLLECT ((COLLECTVARIABLE SYMBOL) |&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE :DOCUMENTATION \"Use a VRLET to collect values.  Input has the form\n'(collect <x> in <expression> where (<test> <x>))'.\")", ((cpp_function_code)(&collect)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_MACROS_STELLA_STARTUP_MACROS);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_MACROS_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Macros"), NULL_STRING_WRAPPER);
      }
    }
...
Symbol* SYM_MACROS_STELLA_MAIN = NULL;
