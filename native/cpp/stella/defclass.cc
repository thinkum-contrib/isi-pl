...
Object* typeToWalkedNullValueTree(Surrogate* type) {
  { StandardObject* dummy1;
    ...
    { Object* nullvalue = typeToNullValueTree(type);
      ...
      if (nullvalue == SYM_DEFCLASS_STELLA_NULL) {
        ...
        return (walkExpressionTree(nullvalue, type, SYM_DEFCLASS_STELLA_NULL, FALSE, dummy1));
      }
    }
  }
}
...
Symbol* yieldConstructorName(Class* clasS) {
  return (internDerivedSymbol(clasS->classType, stringConcatenate("NEW-", className(clasS), 0)));
}
  ...
  { Object* initialvalueexpression = yieldInitialValueExpression(slot);
    ...
    if (((boolean)(initialvalueexpression)) &&
      ...
      if (mode == KWD_DEFCLASS_NULL_VALUE_ONLY) {
        if (requiredslotP ||
            (!equalConsTreesP(typeToNullValueTree(slot->type()), initialvalueexpression))) {
        ...
        initialvalueexpression = typeToNullValueTree(slot->type());
      }
...
Symbol* yieldInitializerName(Class* clasS) {
  return (internDerivedSymbol(clasS->classType, stringConcatenate("initialize-", className(clasS), 0)));
}
...
DEFINE_STELLA_SPECIAL(oRECYCLING_ENABLEDpo, boolean , FALSE);
...
boolean hasNonNullDefaultValueP(StorageSlot* slot) {
  { Object* defaultform = slot->systemDefaultValue();
    ...
    return (((boolean)(defaultform)) &&
        (!equalConsTreesP(typeToNullValueTree(slot->type()), defaultform)));
  }
}
...
Symbol* yieldHardwiredSlotVariable(StorageSlot* slot) {
  return (internDerivedSymbol(slot->slotName, stringConcatenate("*HARDWIRED-", stringConcatenate(slot->slotName->symbolName, stringConcatenate("-ON-", stringConcatenate(slot->slotOwner->symbolName, "*", 0), 0), 0), 0)));
}
...
Cons* yieldHardwiredSlotReaderBody(StorageSlot* slot, Cons*& _Return1) {
  { Symbol* slotvariable = yieldHardwiredSlotVariable(slot);
    ...
    { Cons* _return_temp = listO(3, SYM_DEFCLASS_STELLA_RETURN, slotvariable, NIL);
...
Symbol* yieldGenericSlotAccessorName(Class* clasS) {
  return (internDerivedSymbol(clasS->classType, stringConcatenate("ACCESS-", stringConcatenate(clasS->classType->symbolName, "-SLOT-VALUE", 0), 0)));
}
...
Symbol* yieldMixinSlotReaderName(StorageSlot* slot) {
  return (internDerivedSymbol(slot->slotName, stringConcatenate(slot->slotName->symbolName, stringConcatenate("-ON-", slot->slotOwner->symbolName, 0), 0)));
}
...
Symbol* yieldNativeSlotReaderName(StorageSlot* slot) {
  return (internDerivedSymbol(slot->slotName, stringConcatenate(slot->slotName->symbolName, "-NATIVE-VALUE", 0)));
}
...
void helpStartupDefclass3() {
  {
    ...
    defineFunctionObject("TYPE-TO-WALKED-NULL-VALUE-TREE", "(DEFUN (TYPE-TO-WALKED-NULL-VALUE-TREE OBJECT) ((TYPE TYPE)))", ((cpp_function_code)(&typeToWalkedNullValueTree)), NULL);
...
void helpStartupDefclass4() {
  {
    ...
    { MethodSlot* function = lookupFunction(SYM_DEFCLASS_STELLA_STARTUP_DEFCLASS);
      ...
      setDynamicSlotValue(function->dynamicSlots, SYM_DEFCLASS_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Defclass"), NULL_STRING_WRAPPER);
    }
  }
}
...
void startupDefclass() {
  ...
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oRECYCLING_ENABLEDpo.set(TRUE);
      oRECYCLE_LISTS_MAINTENANCE_TIMERo = oRECYCLE_LISTS_MAINTENANCE_INTERVALo;
      oTRANSLATE_WITH_RECYCLE_LIST_DEBUGGINGpo = FALSE;
    }
