...
Surrogate* makeMemoizationTableSurrogate(Symbol* memoname) {
  if (!((boolean)(memoname))) {
    memoname = (((boolean)(oMETHODBEINGWALKEDo.get())) ? localGensym(stringConcatenate(oMETHODBEINGWALKEDo.get()->slotName->symbolName, "-MEMO-TABLE", 0)) : gensym("MEMO-TABLE"));
  }
...
void startupMemoize() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      ...
      oMEMOIZATION_ENABLEDpo = TRUE;
      { Symbol* self057 = newSymbol("MEMOIZED-NULL-VALUE");

        self057->homeContext = NULL;
        MEMOIZED_NULL_VALUE = self057;
      }
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_MEMOIZE_STELLA_STARTUP_MEMOIZE);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_MEMOIZE_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Memoize"), NULL_STRING_WRAPPER);
      }
    }
    ...
    if (currentStartupTimePhaseP(9)) {
      ...
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *MEMOIZATION-ENABLED?* BOOLEAN TRUE)");
