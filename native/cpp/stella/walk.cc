...
StringHashTable* oQUOTATION_TABLEo = NULL;
...
Cons* getQuotedTree(char* treeName, char* modulename) {
  ...
  { Cons* quotedtree = ((Cons*)(oQUOTATION_TABLEo->lookup(treeName)));
    ...
    if (((boolean)(quotedtree))) {
      return (quotedtree);
    }
    { 
      BIND_STELLA_SPECIAL(oMODULEo, Module*, getStellaModule(modulename, TRUE));
      { Cons* tree = ((Cons*)(readSExpressionFromString(treeName)));

        quotedtree = ((Cons*)(permanentifyForm(tree->value)));
        oQUOTATION_TABLEo->insertAt(treeName, quotedtree);
        return (quotedtree);
      }
    }
  }
...
Cons* walkDontCallMeTree(Object* tree, StandardObject* returntype, StandardObject*& _Return1) {
  _Return1 = returntype;
  return (listO(3, SYM_WALK_STELLA_BAD_SYS, tree, cons(typeSpecToBaseType(returntype), NIL)));
}
...
Surrogate* verifyType(Surrogate* self) {
  ...
  if (translatingCodeP() &&
    ...
    { 
      ...
      signalTranslationWarning();
      if (!(suppressWarningsP())) {
        printErrorContext(">> WARNING: ", STANDARD_OUTPUT);
        std::cout << std::endl << " " << "Reference to undefined class " << "`" << self->symbolName << "'" << "." << std::endl;
      }
    }
    self->surrogateValue = ((Class*)(SGT_WALK_STELLA_UNKNOWN->surrogateValue));
  }
    ...
    { Object* coersionfunction = NULL;
      ...
      if (sourcetype == SGT_WALK_STELLA_UNKNOWN) {
        ...
        if ((tree == SYM_WALK_STELLA_NULL) &&
          ...
          _Return1 = targettype;
          _Return2 = TRUE;
          return (typeToWalkedNullValueTree(targetbasetype));
        }
        else {
          _Return1 = targettype;
          _Return2 = TRUE;
      ...
      if (((boolean)(targetbasetype)) &&
        ...
        if (subtypeOfP(targetbasetype, sourcebasetype)) {
          ...
          _Return1 = targettype;
          _Return2 = TRUE;
          return (walkWithoutTypeTree(cons((((boolean)(realsourcetype)) ? SYM_WALK_STELLA_SAFE_CAST : SYM_WALK_STELLA_CAST), cons(tree, cons(targettype, NIL)))));
        }
    ...
    if (safePrimaryType(otree) == SGT_WALK_STELLA_CONS) {
      ...
      if (warnonatomP &&
        ...
        { 
          ...
          if (!(suppressWarningsP())) {
            printErrorContext(">> WARNING: ", STANDARD_OUTPUT);
            std::cout << std::endl << " " << "Expression found where statement expected: " << "`" << deUglifyParseTree(tree) << "'" << "." << std::endl;
          }
        }
      }
    }
...
Cons* sysTree(Object* tree, StandardObject* typespec, StandardObject*& _Return1) {
  ...
  if (voidP(typespec)) {
    ...
    return (listO(3, SYM_WALK_STELLA_VOID_SYS, tree, NIL));
  }
  else {
    _Return1 = typespec;
    return (listO(3, SYM_WALK_STELLA_TYPED_SYS, tree, cons(typespec, NIL)));
  }
}
...
Symbol* yieldSymbolConstantName(GeneralizedSymbol* symbol) {
  ...
  if (subtypeOfKeywordP(safePrimaryType(symbol))) {
    { GeneralizedSymbol* symbol000 = symbol;
      Keyword* symbol = ((Keyword*)(symbol000));

      return (internSymbolInModule(stringUpcase(constructSymbolConstantName(symbol)), oMODULEo.get(), TRUE));
    }
  }
  else {
    return (internSymbolInModule(stringUpcase(constructSymbolConstantName(symbol)), oMODULEo.get(), TRUE));
  ...
  { Symbol* self = this;
    ...
    { Symbol* truename = trueVariableName(self);
      ...
      if (truename == self) {
        ...
        _Return1 = type;
        return (sysTree(truename, type, dummy1));
      }
    }
  }
}
...
Cons* yieldContextSensitiveSlotValueTree(StorageSlot* slot, Object* objectref, StandardObject* returntype, Cons* csoptions, StandardObject*& _Return1) {
  { StandardObject* dummy1;
    ...
    { boolean contextsensitiveP = !(((boolean)(csoptions)) &&
      ...
      if (onceonlywrapperP) {
        _Return1 = returntype;
        return (listO(4, SYM_WALK_STELLA_VRLET, listO(3, cons(objectref, cons(objectrefcopy, NIL)), cons(oldvalueref, cons(oldvaluetree, NIL)), NIL), accesstree, NIL));
      }
      else {
        _Return1 = returntype;
        return (((Cons*)(walkWithoutTypeTree(accesstree))));
      }
    }
  }
}
...
Cons* yieldSlotValueTree(StorageSlot* slot, Object* objectref, StandardObject* objecttype, StandardObject* returntype, Cons* csoptions, StandardObject*& _Return1) {
  if (slot->slotContextSensitiveP) {
    ...
    if (slot->dynamicStorageP()) {
      _Return1 = returntype;
      return (((Cons*)(walkWithoutTypeTree(yieldDynamicSlotValueTree(slot, objectref, returntype)))));
    }
    else {
      _Return1 = returntype;
      return (yieldNativeSlotValueTree(slot, objectref, objecttype));
    }
  }
}
...
Cons* yieldSlotValueSetterTree(StorageSlot* slot, Object* objectref, StandardObject* objecttype, Object* valueref, StandardObject* returntype, Cons* csoptions, StandardObject*& _Return1) {
  if (slot->slotContextSensitiveP) {
    ...
    if (slot->dynamicStorageP()) {
      _Return1 = returntype;
      return (((Cons*)(walkWithoutTypeTree(yieldDynamicSlotValueSetterTree(slot, objectref, valueref, returntype)))));
    }
    else {
      _Return1 = returntype;
      return (yieldNativeSlotValueSetterTree(slot, objectref, objecttype, valueref));
    }
  }
}
            ...
            if (vrletencounteredP) {
              ...
              if (((boolean)(variable->lookupGlobalVariable()))) {
                { 
                  ...
                  if (!(suppressWarningsP())) {
                    printErrorContext(">> ERROR: ", STANDARD_ERROR);
                    *(STANDARD_ERROR->nativeStream) << std::endl << " " << "Illegal binding of global or special variable " << "in `let' declaration:" << std::endl << "    " << "`" << deUglifyParseTree(decl) << "'" << "." << std::endl;
                  }
                }
                variable = SYM_WALK_STELLA_ILLEGAL_VARIABLE;
              }
...
DEFINE_STELLA_SPECIAL(oSPECIALSENABLEDpo, boolean , FALSE);
...
Cons* walkConditionalTree(Cons* tree, StandardObject*& _Return1) {
  { StandardObject* dummy1;
    ...
    { Object* operatoR = tree->value;
      ...
      if (proceduralExpressionP(test)) {
        _Return1 = SGT_WALK_STELLA_VOID;
        return (transformBooleanProceduralExpression(tree, ((Cons*)(test)), dummy2));
      }
      { GeneralizedSymbol* testValue000 = ((GeneralizedSymbol*)(operatoR));
...
Cons* finishWalkingEqlTree(Cons* tree, Surrogate* type1, Surrogate* type2, StandardObject*& _Return1) {
  { StandardObject* dummy1;
    ...
    _Return1 = SGT_WALK_STELLA_BOOLEAN;
    return (((Cons*)(walkWithoutTypeTree(tree))));
  }
}
...
Cons* walkUndefinedSlotTree(Cons* tree, StandardObject* firstargtype, boolean warnP, Surrogate*& _Return1) {
  { StandardObject* dummy1;
    ...
    { Cons* cursor = tree->rest->rest;
      ...
      _Return1 = SGT_WALK_STELLA_UNKNOWN;
      return (listO(3, SYM_WALK_STELLA_SYS_CALL_METHOD, type, tree->concatenate(NIL, 0)));
    }
  }
}
          ...
          { int nofsubs = NULL_INTEGER;
            ...
            if ((nofsubs == nofmethods) &&
              ...
              { 
                ...
                if (!(suppressWarningsP())) {
                  printErrorContext(">> WARNING: ", STANDARD_OUTPUT);
                  std::cout << std::endl << " " << "Could not infer " << "`" << deUglifyParseTree(methodname) << "'" << " on abstract type " << "`" << deUglifyParseTree(abstracttype) << "'" << "," << std::endl << "   since the implementations of " << "`" << deUglifyParseTree(methodname) << "'" << " on the non-abstract" << std::endl << "   subtypes of " << "`" << deUglifyParseTree(abstracttype) << "'" << " are not all compatible" << "." << std::endl;
                }
              }
            }
            else {
  ...
  { StorageSlot* self = this;
    ...
    { Object* objectref = tree->rest->value;
      ...
      if (systemDefinedSlotReaderP(self)) {
        _Return1 = otype;
        return (listO(3, SYM_WALK_STELLA_SYS_CALL_METHOD, self->slotOwner, cons(self->slotName, cons(objectref, NIL))));
      }
      else {
        _Return1 = otype;
        return (yieldSlotValueTree(self, objectref, firstargtype, otype, NULL, dummy1));
      }
    }
  }
}
...
Object* finishWalkingArgumentListTree(Slot* self, Cons* tree, StandardObject* firstargtype, StandardObject*& _Return1) {
  ...
  if (self->slotOwner == SGT_WALK_STELLA_CONS) {
    { 
      ...
      if (!(suppressWarningsP())) {
        printErrorContext(">> WARNING: ", STANDARD_WARNING);
        *(STANDARD_WARNING->nativeStream) << std::endl << " " << "Applying CONS-methods to &rest-arguments is deprecated." << std::endl << "   " << "`" << deUglifyParseTree(tree) << "'" << std::endl << " Use `foreach' or explicitly coerce with `coerce-&rest-to-cons'" << "." << std::endl;
      }
    }
...
boolean methodInlinableP(MethodSlot* method) {
  ...
  if (method->methodFunctionP ||
    ...
    { 
      ...
      if (!(suppressWarningsP())) {
        printErrorContext(">> WARNING: ", STANDARD_WARNING);
        *(STANDARD_WARNING->nativeStream) << std::endl << " " << "Cannot inline method " << "`" << deUglifyParseTree(method) << "'" << ", since there are" << std::endl << " one or more methods that specialize it" << "." << std::endl;
      }
    }
    return (FALSE);
  }
...
Object* yieldInitialValueExpression(StorageSlot* slot) {
  ...
  { Object* initialvalueexpression = slot->initially();
    ...
    return (typeToNullValueTree(slot->type()));
  }
}
          ...
          if (storageSlotP(slot) &&
            ...
            if (oTRANSLATOROUTPUTLANGUAGEo.get() == KWD_WALK_COMMON_LISP) {
              ...
              { 
                ...
                if (!(suppressWarningsP())) {
                  printErrorContext(">> ERROR: ", STANDARD_ERROR);
                  *(STANDARD_ERROR->nativeStream) << std::endl << " " << "Defining slot " << "`" << deUglifyParseTree(slot) << "'" << " outside the home module of" << std::endl << "   its owner class is illegal in " << "`" << translatorOutputLanguageName() << "'" << "." << std::endl;
                }
              }
    ...
    { GlobalVariable* global = ((GlobalVariable*)(unit->theObject));
      ...
      if (!(initialvaluetree == KWD_WALK_UNBOUND_SPECIAL_VARIABLE)) {
        if (global->variableSpecialP &&
          ...
          initialvaluetree = typeToWalkedNullValueTree(typeSpecToBaseType(global->variableType));
        }
      }
...
char* yieldStartupFunctionClassname(Symbol* functionName) {
  if (systemStartupFileP(oCURRENTFILEo.get())) {
    return (stringCapitalize(functionName->symbolName));
  }
  else {
    return (stringConcatenate("_", stringCapitalize(functionName->symbolName), 0));
  }
}
...
Cons* walkSignalExceptionTree(Cons* tree, StandardObject*& _Return1) {
  { Object* expression = NULL;
    ...
    _Return1 = SGT_WALK_STELLA_VOID;
    return (listO(3, SYM_WALK_STELLA_SYS_SIGNAL, expression, cons(itsType, NIL)));
  }
}
...
Cons* walkExceptionCaseTree(Cons* tree, StandardObject*& _Return1) {
  { StandardObject* dummy1;
    ...
    { Object* protectedtree = walkATree(tree->rest->value, dummy1);
      ...
      _Return1 = SGT_WALK_STELLA_VOID;
      return (listO(3, SYM_WALK_STELLA_SYS_HANDLER_CASE, walkATree(protectedtree, dummy2), handlertrees->concatenate(NIL, 0)));
    }
  }
}
...
Cons* walkUnwindProtectTree(Cons* tree, Surrogate*& _Return1) {
  ...
  _Return1 = SGT_WALK_STELLA_VOID;
  return (cons(SYM_WALK_STELLA_SYS_UNWIND_PROTECT, walkListOfStatements(tree->rest)->concatenate(NIL, 0)));
}
  ...
  { Object* operatoR = tree->value;
    ...
    { Cons* requiredargs = NULL;
      ...
      if (otherassignments == NIL) {
        ...
        _Return1 = classtype;
        return (listO(3, SYM_WALK_STELLA_VRLET, cons(cons(selfvariable, cons(newtree, NIL)), NIL), otherassignments->concatenate(cons(selfvariable, NIL), 0)));
      }
    }
  }
}
...
Symbol* yieldEvaluatorWrapperName(Symbol* methodname) {
  return (internDerivedSymbol(methodname, stringConcatenate(methodname->symbolName, "-EVALUATOR-WRAPPER", 0)));
}
        ...
        { Object* operatorname000 = operatorname;
          ...
          if (operatorname == SYM_WALK_STELLA_QUOTE) {
            if (arguments->length() == 1) {
              { Object* _return_temp = arguments->value;
...
void helpStartupWalk8() {
  {
    ...
    resetStellaFeatures();
    oUSEHARDCODEDSYMBOLSpo.set(FALSE);
    oTRANSLATOROUTPUTLANGUAGEo.set(KWD_WALK_COMMON_LISP);
    oFOUNDRETURNpo.set(FALSE);
    oTARGETTYPEo.set(SGT_WALK_STELLA_VOID);
    oLOG_FUNCTION_CALLSpo = FALSE;
    ...
    oINLININGMETHODCALLpo.set(FALSE);
    oMIXIN_IMPLEMENTATION_STYLEo = KWD_WALK_SECOND_CLASS;
...
void helpStartupWalk14() {
  {
    ...
    { MethodSlot* function = lookupFunction(SYM_WALK_STELLA_STARTUP_WALK);
      ...
      setDynamicSlotValue(function->dynamicSlots, SYM_WALK_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Walk"), NULL_STRING_WRAPPER);
    }
  }
}
...
void startupWalk() {
  if (currentStartupTimePhaseP(0)) {
    oQUOTATION_TABLEo = newStringHashTable();
  }
  { 
    ...
    if (currentStartupTimePhaseP(9)) {
      ...
      defineStellaGlobalVariableFromStringifiedSource("(DEFGLOBAL *QUOTATION-TABLE* (STRING-HASH-TABLE OF STRING CONS) NULL :DOCUMENTATION \"Stores quoted cons trees.\")");
