...
// their own.
DEFINE_STELLA_SPECIAL(oSKOLEMNAMEMAPPINGTABLEo, KeyValueList* , NULL);
...
void startupPrint() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      ...
      oPRINTFUNCTIONSASRELATIONSpo.set(FALSE);
      oDEBUG_MODEpo = FALSE;
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_PRINT_LOGIC_STARTUP_PRINT);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_PRINT_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Print"), NULL_STRING_WRAPPER);
      }
    }
    ...
    if (currentStartupTimePhaseP(9)) {
      ...
      defineStellaGlobalVariableFromStringifiedSource("(DEFSPECIAL *SKOLEMNAMEMAPPINGTABLE* (KEY-VALUE-LIST OF SKOLEM SKOLEM) NULL :DOCUMENTATION \"If set, maps some skolems to names other than\ntheir own.\")");
