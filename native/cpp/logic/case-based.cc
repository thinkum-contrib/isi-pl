...
FloatWrapper* matchInstances(LogicObject* x, LogicObject* y) {
  { Cons* prop = listO(3, SYM_CASE_BASED_PL_KERNEL_KB_CASE_MATCH, x, cons(y, NIL));
    ...
    return (wrapFloat(returnPartialTruth(makeQuery(NIL, coerceToTree(prop), NIL, NIL, "MatchInstances"), true)));
  }
}
...
double trainAndTestCaseMatcher(Symbol* className, Symbol* slotName, int numTrain, int numTest, int cycles) {
  ...
  { Surrogate* clasS = getDescription(className)->surrogateValueInverse;
    ...
    generateCaseRuleWoSlot(instances, className, slotName);
    returnPartialTruth(makeQuery(NIL, coerceToTree(listO(3, SYM_CASE_BASED_LOGIC_CASE_MATCH, objectName(cases->first()), cons(listO(3, SYM_CASE_BASED_LOGIC_OBJECT_NAME, listO(3, SYM_CASE_BASED_STELLA_SECOND, SYM_CASE_BASED_LOGIC_CASES, NIL), NIL), NIL))), NIL, NIL, "CreateNeuralNetwork"), true);
