...
boolean ruleCoversExampleP(Cons* reverseRule, TrainingExample* example) {
  ...
  { QueryIterator* query = NULL;
    ...
    if (oOPTIMIZE_INDUCED_QUERIESo) {
      query = makeQuery(NIL, consQuery, NIL, listO(3, KWD_RULE_INDUCTION_TIMEOUT, wrapInteger(300), NIL), "RuleOptimized");
    }
    else {
      query = makeQuery(NIL, consQuery, NIL, listO(5, KWD_RULE_INDUCTION_TIMEOUT, wrapInteger(300), KWD_RULE_INDUCTION_DONT_OPTIMIZEp, SYM_RULE_INDUCTION_STELLA_TRUE, NIL), "Rule");
    }
