...
Keyword* tryIsaPropositionProof(ControlFrame* frame) {
  { Proposition* proposition = frame->proposition;
    ...
    else if (isaP(member, SGT_STRATEGIES_STELLA_LITERAL_WRAPPER)) {
...
Skolem* createHypothesizedSkolem(char* prefix) {
  { Skolem* skolem = createSkolem(NULL, gensym(prefix));

    setDynamicSlotValue(skolem->dynamicSlots, SYM_STRATEGIES_LOGIC_HYPOTHESIZED_INSTANCEp, (TRUE ? TRUE_WRAPPER : FALSE_WRAPPER), FALSE_WRAPPER);
    return (skolem);
  }
}
    ...
    { Proposition* forallprop = frame->proposition;
      ...
      { PatternVariable* v = NULL;
        ...
        while (index000 < length000) {
          v = ((PatternVariable*)((vector000->theArray)[index000]));
          index000 = index000 + 1;
          { Skolem* skolem = createHypothesizedSkolem("ANONYMOUS");
...
void helpStartupStrategies3() {
  {
    ...
    defineFunctionObject("CREATE-HYPOTHESIZED-SKOLEM", "(DEFUN (CREATE-HYPOTHESIZED-SKOLEM SKOLEM) ((PREFIX STRING)))", ((cpp_function_code)(&createHypothesizedSkolem)), NULL);
  ...
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_STRATEGIES_LOGIC_STARTUP_STRATEGIES);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_STRATEGIES_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Strategies"), NULL_STRING_WRAPPER);
      }
    }
