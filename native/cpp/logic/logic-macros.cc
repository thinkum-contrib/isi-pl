...
DEFINE_STELLA_SPECIAL(oSUPPRESSNONLOGICOBJECTWARNINGpo, boolean , FALSE);
...
Object* applyRetrieve(Cons* body) {
  ...
  { Symbol* gensymname = gensym("QUERY");
...
void helpStartupLogicMacros1() {
  {
    ...
    SYM_LOGIC_MACROS_LOGIC_DEPTH = ((Symbol*)(internRigidSymbolWrtModule("DEPTH", NULL, 0)));
...
void startupLogicMacros() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      oSUPPRESSNONLOGICOBJECTWARNINGpo.set(TRUE);
      oCYC_KLUDGES_ENABLEDpo = FALSE;
    }
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("DEFINE-COMPUTED-CONSTRAINT", "(DEFUN DEFINE-COMPUTED-CONSTRAINT ((NAME SYMBOL) (VAR-LIST CONS) (CONSTRAINT-TEST CONS) |&BODY| (POSITION-COMPUTATIONS CONS)) :TYPE OBJECT :MACRO? TRUE :PUBLIC? TRUE :DOCUMENTATION \"Defines `name' to be a constraint computation which uses `constraint-test'\nto determine if a fully bound set of variables satisfies the constraint.  The forms in\n`position-computations' are used to compute the value for each of the positions. All such\ncomputations must set the variable `value' to be the result computed for the missing position.\nSetting `value' to `null' for any such computation means that that particular argument cannot be\ncomputed from the others.  The input variables in `var-list' will be bound to the N arguments\nto the constraint.\n  The generated function will return a Stella Object and take as inputs the values of\nthe N arguments to the constraint.  A value of `null' means that the value is not available.\nIf all arguments are not `null', then the return value will be a Stella wrapped boolean indicating \nwhether the constraint is satisified or not.\n  If more than one input value is `null', then this constraint code will not be called.\")", ((cpp_function_code)(&defineComputedConstraint)), NULL);
      defineFunctionObject("DEFINE-ARITHMETIC-TEST-ON-WRAPPERS", "(DEFUN DEFINE-ARITHMETIC-TEST-ON-WRAPPERS ((NAME SYMBOL) (TEST-NAME SYMBOL)) :TYPE OBJECT :MACRO? TRUE :DOCUMENTATION \"Defines `name' as an arithmetic comparision operation using the\ntest `test-name'.  It will take two wrapped number parameters and return a `boolean'.\nThe code will use the appropriate test for the specific subtype of wrapped number\nactually passed in.\")", ((cpp_function_code)(&defineArithmeticTestOnWrappers)), NULL);
      defineFunctionObject("DEFINE-ARITHMETIC-OPERATION-ON-WRAPPERS", "(DEFUN DEFINE-ARITHMETIC-OPERATION-ON-WRAPPERS ((NAME SYMBOL) (OPERATION-NAME SYMBOL)) :TYPE OBJECT :MACRO? TRUE :DOCUMENTATION \"Defines `name' as an arithmetic comparision operation using the\ntest `test-name'.  It will take two wrapped number parameters and return a wrapped number.\nThe code will use the appropriate test for the specific subtype of wrapped number\nactually passed in, and return the appropriate subtype of wrapped number based on\nthe normal arithmetic contagion rules.\n  \nFor example, if both input parameters are wrapped integers then the output will be\na wrapped integer.  If the inputs are a wrapped integer and a wrapped float then\nthe output will be a wrapped float, etc.\")", ((cpp_function_code)(&defineArithmeticOperationOnWrappers)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_LOGIC_MACROS_LOGIC_STARTUP_LOGIC_MACROS);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_LOGIC_MACROS_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Logic-Macros"), NULL_STRING_WRAPPER);
      }
    }
...
Symbol* SYM_LOGIC_MACROS_LOGIC_DEPTH = NULL;
