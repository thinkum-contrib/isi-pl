...
Cons* loomRestrictionToKif(Cons* tree, Symbol* implicitvariable) {
  { Symbol* operatoR = ((Symbol*)(tree->value));
    Object* role = (getQuotedTree("((AT-LEAST AT-MOST EXACTLY) \"/LOGIC\")", "/LOGIC")->memberP(operatoR) ? tree->rest->rest->value : tree->rest->value);
    ...
    if ((operatoR == SYM_LOOM_TO_KIF_LOGIC_AT_LEAST) ||
      ...
      return (cons(operatoR, cons(loomArgumentDescriptionToKif(role, KWD_LOOM_TO_KIF_BINARY), cons(implicitvariable, cons(tree->rest->value, ((((boolean)(tree->fourth())) ? consList(1, loomArgumentDescriptionToKif(tree->fourth(), KWD_LOOM_TO_KIF_UNARY)) : NIL))->concatenate(NIL, 0))))));
    }
      ...
      if (((boolean)(tree->fourth()))) {
        {
          ...
          return (listO(4, SYM_LOOM_TO_KIF_STELLA_EXISTS, cons(existsvariable, NIL), listO(4, SYM_LOOM_TO_KIF_STELLA_eg, listO(4, SYM_LOOM_TO_KIF_STELLA_AND, cons(loomArgumentDescriptionToKif(role, KWD_LOOM_TO_KIF_BINARY), cons(implicitvariable, cons(existsvariable, NIL))), cons(loomArgumentDescriptionToKif(tree->fourth(), KWD_LOOM_TO_KIF_UNARY), cons(existsvariable, NIL)), NIL), cons(loomArgumentDescriptionToKif(tree->rest->rest->value, KWD_LOOM_TO_KIF_UNARY), cons(((Symbol*)(SYSTEM_DEFINED_ARGUMENT_NAMES->nth(0))), NIL)), NIL), NIL));
        }
      }
      else {
        return (listO(3, SYM_LOOM_TO_KIF_PL_KERNEL_KB_RANGE_TYPE, loomArgumentDescriptionToKif(role, KWD_LOOM_TO_KIF_BINARY), cons(implicitvariable, cons(loomArgumentDescriptionToKif(tree->rest->rest->value, KWD_LOOM_TO_KIF_UNARY), NIL))));
      }
    }
    else if (operatoR == SYM_LOOM_TO_KIF_STELLA_SOME) {
      return (loomRestrictionToKif(listO(4, SYM_LOOM_TO_KIF_LOGIC_AT_LEAST, wrapInteger(1), role, cons(tree->rest->rest->value, NIL)), implicitvariable));
    }
    else if (operatoR == SYM_LOOM_TO_KIF_LOGIC_THE) {
      return (loomDescriptionToKif(listO(4, SYM_LOOM_TO_KIF_STELLA_AND, listO(4, SYM_LOOM_TO_KIF_LOGIC_EXACTLY, wrapInteger(1), role, NIL), listO(3, SYM_LOOM_TO_KIF_LOGIC_ALL, role, cons(tree->rest->rest->value, NIL)), NIL), consList(1, implicitvariable)));
    }
    ...
    { Object* tree000 = tree;
      ...
      { GeneralizedSymbol* testValue000 = ((GeneralizedSymbol*)(tree->value));
        ...
        if ((testValue000 == SYM_LOOM_TO_KIF_STELLA_AND) ||
          ...
          return (cons(tree->rest->rest->value, cons(tree->rest->value, NIL)));
        }
        else {
          return (tree);
        }
        ...
        if ((testValue000 == SYM_LOOM_TO_KIF_STELLA_PROGN) ||
          ...
          return (translateLoomDefinition(tree));
            ...
            ((testValue000 == SYM_LOOM_TO_KIF_LOGIC_CREATEM) ||
             ((testValue000 == SYM_LOOM_TO_KIF_LOGIC_DEFCONTEXT) ||
...
void translateLoomFile(char* input, char* output) {
  { 
    ...
    { InputFileStream* in = NULL;
      ...
      try {
        in = newInputFileStream(input);
        { OutputFileStream* out = NULL;

          try {
            out = newOutputFileStream(output);
...
void startupLoomToKif() {
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("TRANSLATE-LOOM-FILE", "(DEFUN TRANSLATE-LOOM-FILE ((INPUT FILE-NAME) (OUTPUT FILE-NAME)))", ((cpp_function_code)(&translateLoomFile)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_LOOM_TO_KIF_LOGIC_STARTUP_LOOM_TO_KIF);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_LOOM_TO_KIF_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Loom-To-Kif"), NULL_STRING_WRAPPER);
      }
    }
