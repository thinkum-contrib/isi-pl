...
boolean oEMIT_THINKING_DOTSpo = FALSE;
    ...
    if (subtypeOfP(testValue000, SGT_QUERY_LOGIC_PATTERN_VARIABLE)) {
      ...
      { Object* self002 = self;
        ...
        if (((boolean)(((Vector*)(dynamicSlotValue(self->dynamicSlots, SYM_QUERY_LOGIC_EXTERNAL_VARIABLES, NULL))))) &&
          ...
          { Description* duplicate = findDuplicateDescriptionWithConstantReference(self);

            return ((((boolean)(duplicate)) ? duplicate : self));
          }
        }
        return (valueOf(self));
      }
  ...
  { Object* dummy1;
    ...
    if (oTYPE_CHECK_STRATEGYo.get() == KWD_QUERY_NONE) {
      ...
      { Surrogate* type = v1->skolemType;
        boolean typeisokP = checkTypeP(i2, type, TRUE, dummy1);
...
boolean helpBindVariableToValueP(PatternVariable* variable, Object* value) {
  if (oTYPE_CHECK_STRATEGYo.get() == KWD_QUERY_LOOKUP) {
    { boolean typeisokP = FALSE;
      ...
      typeisokP = checkTypeP(value, variable->skolemType, TRUE, value);
      if ((!typeisokP) &&
  ...
  { Object* boundtovalue = (oQUERYITERATORo.get()->currentPatternRecord->variableBindings->theArray)[(variable->boundToOffset)];
    ...
    else if (equalP(boundtovalue, value)) {
      return (TRUE);
    }
    else {
      return (FALSE);
...
Keyword* continueOrProof(ControlFrame* frame, Keyword* lastmove) {
  { Proposition* proposition = frame->proposition;
    ...
    if (lastmove == KWD_QUERY_DOWN) {
      ...
      frame->truthValue = ((!((boolean)(frame->truthValue))) ? frame->result->truthValue : disjoinTruthValues(frame->truthValue, frame->result->truthValue));
      frame->argumentCursor = frame->argumentCursor + 1;
      if (frame->argumentCursor < proposition->arguments->length()) {
        {
          createChoicePoint(frame);
...
Keyword* continueNotProof(ControlFrame* frame, Keyword* lastmove) {
  if (lastmove == KWD_QUERY_DOWN) {
    ...
    else if (oGENERATE_ALL_PROOFSpo.get()) {
      return (KWD_QUERY_CONTINUING_SUCCESS);
    }
    else {
      popFramesUpTo(frame->down);
...
boolean allKeyArgumentsBoundP(Proposition* goal) {
  { boolean testValue000 = FALSE;
    ...
    testValue000 = (goal->kind == KWD_QUERY_FUNCTION) ||
...
boolean disjointTermsP(Description* d1, Description* d2) {
  ...
  { TruthValue* dummy1;
    ...
    return (applyCachedAsk(listO(3, SYM_QUERY_LOGIC_pD1, SYM_QUERY_LOGIC_pD2, NIL), listO(4, SYM_QUERY_LOGIC_DISJOINT, SYM_QUERY_LOGIC_pD1, SYM_QUERY_LOGIC_pD2, NIL), consList(2, d1, d2), consList(0), SYM_QUERY_LOGIC_QUERY_049, dummy1));
  }
}
  ...
  { Surrogate* testValue000 = safePrimaryType(collection);
    ...
    if (subtypeOfP(testValue000, SGT_QUERY_LOGIC_NAMED_DESCRIPTION)) {
      ...
      { Object* collection003 = collection;
        ...
        *(STANDARD_WARNING->nativeStream) << "Warning: " << "Bug: PowerLoom doesn't know how to handle a LOGIC-OBJECT that " << std::endl << "   denotes a collection.  Please complain to your PowerLoom dealer." << std::endl << std::endl;
        return (FALSE);
      }
    }
    else {
...
void printControlFrame(ControlFrame* self, std::ostream* stream) {
  ...
  { 
    ...
    { boolean printtimesP = self->allottedClockTicks != NULL_INTEGER;
      ...
      *(stream) << " " << self->state << " " << self->currentStrategy << "  UP: " << ((((boolean)(self->up)) ? debugFrameId(self->up) : ((int)(NULL_INTEGER)))) << "  DOWN: " << ((((boolean)(self->down)) ? debugFrameId(self->down) : ((int)(NULL_INTEGER)))) << "  DEPTH: " << computeFrameDepth(self);
      if (printtimesP) {
        ...
        if (((boolean)(outsidebindings))) {
          if (outsidebindings->length() > externalvariables->length()) {
            { OutputStringStream* stream000 = newOutputStringStream();
              ...
              *(stream000->nativeStream) << "Arity mismatch between bindings " << "`" << outsidebindings << "'" << " and external varaibles " << "`" << externalvariables << "'" << ".";
              throw *newFailException(stream000->theStringReader());
            }
          }
...
boolean tryToDefeatLastAnswerP(QueryIterator* self) {
  { QueryIterator* negatedquery = (self->queryIsTrueFalseP() ? self : ((QueryIterator*)(dynamicSlotValue(self->dynamicSlots, SYM_QUERY_LOGIC_AUXILIARY_QUERY, NULL))));
    ...
    if (!((boolean)(negatedquery))) {
      ...
      negatedquery->baseControlFrame->patternRecord->variableBindings = ((QuerySolution*)(self->value))->bindings;
    }
...
Cons* standardizeQueryTree(Object* iovariables, Object* querybody, Cons* externalvariables) {
  ...
  { Cons* standardizedtree = NULL;
    ...
    standardizedtree = canonicalizeQuantificationTree(((Cons*)(standardizeLogicalParseTree(standardizedtree))));
...
TruthValue* callAsk(Object* query) {
  ...
  { 
    ...
    { TruthValue* truthvalue = NULL;
      ...
      truthvalue = oQUERYITERATORo.get()->baseControlFrame->truthValue;
      if (!((boolean)(truthvalue))) {
...
QueryIterator* makeCachedQuery(Cons* variables, Cons* querybody, Cons* inputbindings, Object* options, Symbol* cacheid) {
  { QueryIterator* queryiterator = ((QueryIterator*)(oQUERY_CACHEo->lookup(cacheid)));
    ...
    if (!(variables->length() == inputbindings->length())) {
      std::cout << "Number of variables does not equal number of input bindings." << std::endl;
      return (NULL);
    }
    { ConsIterator* it = inputbindings->allocateIterator();
      ...
      while (it->nextP()) {
        if (((boolean)(it->value))) {
          it->valueSetter(valueOf(it->value));
    ...
    if (((boolean)(queryiterator))) {
      { Object* oldb = NULL;
        Object* newb = NULL;
        Vector* vector000 = queryiterator->currentPatternRecord->variableBindings;
        int index000 = 0;
        int length000 = vector000->length();
    ...
    if (!((boolean)(queryiterator))) {
      {
        try {
          variables = copyConsList(variables);
          querybody = ((Cons*)(copyConsTree(querybody)));
          queryiterator = makeQuery(variables, querybody, inputbindings, options);
        }
        catch (LogicException& _e) {
          LogicException* e = &_e;
      ...
      { 
        ...
        BIND_STELLA_SPECIAL(oCONTEXTo, Context*, getQueryContext());
        queryiterator->reset();
        unbindVariablesBeginningAt(queryiterator->currentPatternRecord, 0);
        if (bindingscount > 0) {
          { Vector* externalvariables = queryiterator->externalVariables;

            { PatternVariable* arg = NULL;
              Object* value = NULL;
              Vector* vector001 = externalvariables;
              int index001 = 0;
              int length001 = vector001->length();
              ...
              while ((index001 < length001) &&
                  (!(iter001 == NIL))) {
                arg = ((PatternVariable*)((vector001->theArray)[index001]));
                index001 = index001 + 1;
                value = iter001->value;
                iter001 = iter001->rest;
                if (((boolean)(value))) {
                  if (!bindArgumentToValueP(arg, value, FALSE)) {
                    std::cout << "Query failure: Can't bind " << "`" << value << "'" << " to " << "`" << arg << "'" << "." << std::endl << std::endl;
                    return (NULL);
                  }
                }
              }
...
boolean applyCachedAsk(Cons* inputvariables, Cons* querybody, Cons* inputbindings, Object* options, Symbol* cacheid, TruthValue*& _Return1) {
  { QueryIterator* queryiterator = makeCachedQuery(inputvariables, querybody, inputbindings, options, cacheid);
    ...
    return (trueTruthValueP(callAsk(queryiterator)));
  }
}
  ...
  { Cons* dummy1;
    ...
    { Object* prototype = accessBinaryValue(description, SGT_QUERY_PL_KERNEL_KB_CONCEPT_PROTOTYPE);
      ...
      if (!((boolean)(prototype))) {
        prototype = applyCachedRetrieve(listO(3, SYM_QUERY_LOGIC_pC, SYM_QUERY_LOGIC_pP, NIL), listO(4, SYM_QUERY_PL_KERNEL_KB_CONCEPT_PROTOTYPE, SYM_QUERY_LOGIC_pC, SYM_QUERY_LOGIC_pP, NIL), consList(2, description, NULL), consList(0), SYM_QUERY_LOGIC_QUERY_053, dummy1)->value;
      }
      { 
  ...
  { Vector* skolemsvector = newVector(sub->ioVariables->length());
    ...
    { int i = NULL_INTEGER;
      ...
      while (iter000 <= upperBound000) {
        i = iter000;
        iter000 = iter000 + 1;
        (skolemsvector->theArray)[i] = (createHypothesizedSkolem("ANONYMOUS"));
      }
    }
    try {
...
TruthValue* vectorSatisfiesDescriptionP(Vector* vector, Description* description) {
  ...
  try {
    { boolean testValue000 = FALSE;
      ...
      testValue000 = vector->length() == description->arity();
      if (testValue000) {
        { boolean alwaysP000 = TRUE;
  ...
  { Surrogate* testValue000 = safePrimaryType(self);
    ...
    if (subtypeOfP(testValue000, SGT_QUERY_LOGIC_LOGIC_OBJECT)) {
      ...
      { Object* self002 = self;
        ...
        return (coerceToInstance(self->surrogateValue, original));
      }
    }
    else if (subtypeOfSymbolP(testValue000)) {
      { Object* self003 = self;
    ...
    { 
      ...
      { QuerySolution* solution = NULL;
        ...
        while (iter000->nextP()) {
          ...
          { Proposition* prop = NULL;
            Cons* iter001 = inheritAsTopLevelProposition(proposition, mapping);
...
Cons* assertFromQuery(Cons* query, Cons* options) {
  ...
  // 
  try {
    { PropertyList* theoptions = parseLogicCommandOptions(options, listO(7, KWD_QUERY_MODULE, SGT_QUERY_STELLA_MODULE, KWD_QUERY_RELATION, SGT_QUERY_LOGIC_NAMED_DESCRIPTION, KWD_QUERY_PATTERN, SGT_QUERY_STELLA_CONS, NIL), TRUE);
...
void helpStartupQuery4() {
  {
    ...
    SYM_QUERY_LOGIC_QUERY_049 = ((Symbol*)(internRigidSymbolWrtModule("QUERY-049", NULL, 0)));
    ...
    SYM_QUERY_LOGIC_QUERY_051 = ((Symbol*)(internRigidSymbolWrtModule("QUERY-051", NULL, 0)));
...
void helpStartupQuery5() {
  {
    ...
    SYM_QUERY_LOGIC_QUERY_053 = ((Symbol*)(internRigidSymbolWrtModule("QUERY-053", NULL, 0)));
...
void helpStartupQuery6() {
  {
    { NormalInferenceLevel* self039 = newNormalInferenceLevel();

      self039->keyword = KWD_QUERY_NORMAL;
      NORMAL_INFERENCE = self039;
    }
    { BacktrackingInferenceLevel* self040 = newBacktrackingInferenceLevel();

      self040->keyword = KWD_QUERY_BACKTRACKING;
      BACKTRACKING_INFERENCE = self040;
    }
    { SubsumptionInferenceLevel* self041 = newSubsumptionInferenceLevel();

      self041->keyword = KWD_QUERY_SUBSUMPTION;
      SUBSUMPTION_INFERENCE = self041;
    }
    { ShallowInferenceLevel* self042 = newShallowInferenceLevel();

      self042->keyword = KWD_QUERY_SHALLOW;
      SHALLOW_INFERENCE = self042;
    }
    { AssertionInferenceLevel* self043 = newAssertionInferenceLevel();

      self043->keyword = KWD_QUERY_ASSERTION;
      ASSERTION_INFERENCE = self043;
    }
    { RefutationInferenceLevel* self044 = newRefutationInferenceLevel();

      self044->keyword = KWD_QUERY_REFUTATION;
      REFUTATION_INFERENCE = self044;
    }
    oINFERENCELEVELo.set(NORMAL_INFERENCE);
    oDONTUSEDEFAULTKNOWLEDGEpo.set(FALSE);
    oMAXIMUM_BACKTRACKING_DEPTHo = oDEFAULT_MAXIMUM_DEPTHo;
    oTYPE_CHECK_STRATEGYo.set(KWD_QUERY_LOOKUP);
    oDUPLICATE_SUBGOAL_STRATEGYo = KWD_QUERY_DUPLICATE_GOALS;
    oGLOBALLY_CLOSED_COLLECTIONSpo = FALSE;
    oGENERATE_ALL_PROOFSpo.set(FALSE);
    oITERATIVE_DEEPENING_MODEpo = FALSE;
    oEMIT_THINKING_DOTSpo = TRUE;
    { PropertyList* self045 = newPropertyList();

      self045->thePlist = listO(3, KWD_QUERY_UPCLASSIFY, wrapCharacter('u'), listO(3, KWD_QUERY_DOWNCLASSIFY, wrapCharacter('d'), listO(3, KWD_QUERY_PROPAGATE, wrapCharacter('f'), listO(3, KWD_QUERY_PARTIAL_MATCH, wrapCharacter('p'), NIL))));
      oTHINKING_DOT_TABLEo = self045;
    }
    { BooleanVectorIndexNode* self046 = newBooleanVectorIndexNode();

      self046->theVector = newBooleanVector(0);
      oBOOLEAN_VECTOR_INDEXo = self046;
    }
    oREVERSEPOLARITYpo.set(FALSE);
    oQUERY_CACHEo = newHashTable();
  }
}
  ...
  { 
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      { MethodSlot* function = lookupFunction(SYM_QUERY_LOGIC_STARTUP_QUERY);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_QUERY_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Query"), NULL_STRING_WRAPPER);
      }
    }
...
Symbol* SYM_QUERY_LOGIC_QUERY_049 = NULL;
...
Symbol* SYM_QUERY_LOGIC_QUERY_051 = NULL;
...
Symbol* SYM_QUERY_LOGIC_QUERY_053 = NULL;
