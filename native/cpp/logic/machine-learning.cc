...
List* getClassificationRelations(Cons* consQuery) {
  { QueryIterator* query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "Antecedent");
    List* result = newList();
...
Vector* createInputSignature(Cons* consQuery) {
  { QueryIterator* query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "Signature");
    IncrementalPartialMatch* pmf = NULL;
    Cons* argScores = NIL;
    ...
    { int i = NULL_INTEGER;
      ...
      for  (i, iter000, upperBound000; 
        ...
        { TrainingExample* example = newTrainingExample();
          ...
          target = trueTruthValueP(callAsk(makeQuery(NIL, consList(2, conceptName, objectName(((LogicObject*)((instances->theArray)[i])))), NIL, NIL, "Training")));
    ...
    { Object* instance = NULL;
      ...
      for (instance, iter000; !(iter000 == NIL); iter000 = iter000->rest) {
        ...
        if (trueP(prop)) {
          ...
          helpUpdateTopLevelProposition(prop, KWD_MACHINE_LEARNING_RETRACT_TRUE);
          query = makeQuery(NIL, ((Cons*)(copyConsTree(listO(3, SYM_MACHINE_LEARNING_STELLA_NOT, consQuery, NIL)))), NIL, NIL, "TestNegated");
          if (trueTruthValueP(callAsk(query))) {
            correctP = false;
            std::cout << "  **Theory disproves " << consQuery << " true" << std::endl;
          }
          query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "TestUnknown1");
        ...
        if (falseP(prop)) {
          ...
          helpUpdateTopLevelProposition(prop, KWD_MACHINE_LEARNING_RETRACT_FALSE);
          query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "TestTrue");
          if (trueTruthValueP(callAsk(query))) {
            correctP = false;
            std::cout << "  **Theory proves " << consQuery << "true" << std::endl;
          }
          query = makeQuery(NIL, ((Cons*)(copyConsTree(listO(3, SYM_MACHINE_LEARNING_STELLA_NOT, consQuery, NIL)))), NIL, NIL, "TestUnknown2");
          if (unknownTruthValueP(callAsk(query))) {
            correctP = false;
  ...
  { double correct = 0.0;
    ...
    { TrainingExample* example = NULL;
      ...
      for (example, iter000; !(iter000 == NIL); iter000 = iter000->rest) {
        ...
        prop = ((Proposition*)(conceiveFormula(((Cons*)(copyConsTree(consQuery))))));
        query = makeQuery(NIL, ((Cons*)(copyConsTree(example->query))), NIL, NIL, "TestTheory");
    ...
    { int i = NULL_INTEGER;
      ...
      for  (i, iter000, upperBound000; 
        ...
        if (trueP(prop)) {
          {
            helpUpdateTopLevelProposition(prop, KWD_MACHINE_LEARNING_RETRACT_TRUE);
            query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "AlreadyTrue");
            if (unknownTruthValueP(callAsk(query))) {
    ...
    { int i = NULL_INTEGER;
      ...
      for  (i, iter000, upperBound000; 
        ...
        if (trueP(prop)) {
          {
            helpUpdateTopLevelProposition(prop, KWD_MACHINE_LEARNING_RETRACT_TRUE);
            query = makeQuery(NIL, ((Cons*)(copyConsTree(consQuery))), NIL, NIL, "AlreadyTrue-Class");
            if (unknownTruthValueP(callAsk(query))) {
              example = newTrainingExample();
...
double generateSlotApproximation(Symbol* instName, Surrogate* slot) {
  { Object* probe = getInstance(instName);
    ...
    range = max - min;
    score = returnPartialTruth(makeQuery(NIL, coerceToTree(prop), NIL, NIL, "ScoreQuery"), true);
