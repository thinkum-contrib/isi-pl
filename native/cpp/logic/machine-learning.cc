...
boolean oCLOSED_WORLD_TRAINING_EXAMPLESo = FALSE;
...
boolean oDROP_FUNCTION_VALUESpo = FALSE;
...
void translateLearning() {
  setOptimizationLevels(1, 0, 3, 3);
  clTranslateFile("machine-learning.ste", TRUE);
  cppTranslateFile("machine-learning.ste");
  clTranslateFile("rule-induction.ste", TRUE);
  cppTranslateFile("rule-induction.ste");
  clTranslateFile("neural-network.ste", TRUE);
  cppTranslateFile("neural-network.ste");
  clTranslateFile("case-based.ste", TRUE);
  cppTranslateFile("case-based.ste");
}
...
void startupMachineLearning() {
  { 
    ...
    if (currentStartupTimePhaseP(4)) {
      ...
      oCLOSED_WORLD_TRAINING_EXAMPLESo = TRUE;
      oDROP_FUNCTION_VALUESpo = TRUE;
    ...
    if (currentStartupTimePhaseP(7)) {
      ...
      defineFunctionObject("TRANSLATE-LEARNING", "(DEFUN TRANSLATE-LEARNING () :COMMAND? TRUE :PUBLIC? TRUE)", ((cpp_function_code)(&translateLearning)), NULL);
      ...
      { MethodSlot* function = lookupFunction(SYM_MACHINE_LEARNING_LOGIC_STARTUP_MACHINE_LEARNING);
        ...
        setDynamicSlotValue(function->dynamicSlots, SYM_MACHINE_LEARNING_STELLA_METHOD_STARTUP_CLASSNAME, wrapString("_Startup-Machine-Learning"), NULL_STRING_WRAPPER);
      }
    }
