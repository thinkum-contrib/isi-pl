...
(CL:DEFUN GET-RPC-METHOD (ENVELOPE)
  (CL:LET* ((CONTENTS (|%contents| (|%body| ENVELOPE))) (TYPE (PRIMARY-TYPE CONTENTS))) (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME TYPE) (%HOME-CONTEXT TYPE) CL:T)))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SLOT (%%VALUE ITER-000))
     (CL:WHEN (ELEMENT-SLOT? SLOT)
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (GET-OBJECT-SLOT-VALUE CONTENTS SLOT) NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
...
(CL:DEFUN DISPATCH-RPC-CALL (ENVELOPE)
  (CL:LET* ((METHOD (GET-RPC-METHOD ENVELOPE)) (ARGUMENTS (GET-RPC-ARGUMENTS ENVELOPE)) (RESULT (DISPATCH-CALL-WITH-METHOD-AND-ARGS METHOD ARGUMENTS))) RESULT))

;;; (DEFUN (DISPATCH-CALL-WITH-METHOD-AND-ARGS |Envelope|) ...)

(CL:DEFUN DISPATCH-CALL-WITH-METHOD-AND-ARGS (METHODNAME ARGUMENTS)
  (CL:LET* ((FUNCTION (%FUNCTION-CODE (LOOKUP-FUNCTION METHODNAME))) (RAWRESULT (APPLY FUNCTION ARGUMENTS)) (PACKAGEDRESULT (CREATE-SOAP-ENVELOPE RAWRESULT)))
   PACKAGEDRESULT))
...
(CL:DEFUN CREATE-SOAP-ENVELOPE (CONTENTS)
  (CL:LET* ((SELF-000 (|new-Body|))) (CL:SETF (|%contents| SELF-000) CONTENTS)
   (CL:LET* ((BODY SELF-000)) (CL:LET* ((SELF-001 (|new-Envelope|))) (CL:SETF (|%body| SELF-001) BODY) (CL:LET* ((RESULT SELF-001)) RESULT)))))
  ...
  (CL:LET* ((SELF-000 (|new-Fault|))) (CL:LET* ((SELF-001 (|new-FaultCode|))) (CL:SETF (|%textContent| SELF-001) FCODE) (CL:SETF (|%faultcode| SELF-000) SELF-001))
   ...
   (CL:LET* ((FAULT SELF-000))
    (CL:WHEN (CL:NOT (CL:EQ FACTOR STELLA::NULL-STRING))
     (CL:LET* ((SELF-003 (|new-FaultActor|))) (CL:SETF (|%textContent| SELF-003) FACTOR) (CL:SETF (|%faultactor| FAULT) SELF-003)))
    (CL:WHEN (CL:NOT (CL:EQ FDETAIL NULL)) (CL:LET* ((SELF-004 (|new-Detail|))) (CL:SETF (|%contents| SELF-004) FDETAIL) (CL:SETF (|%detail| FAULT) SELF-004))) FAULT)))
...
(CL:DEFUN CALL-WEBSERVICE (CALLOBJECT URL)
  ...
  (CL:LET*
   ((DEBUG? CL:T) (REQUESTENVELOPEOBJECT (CREATE-SOAP-ENVELOPE CALLOBJECT)) (REQUESTSTRING (TO-XML-STRING REQUESTENVELOPEOBJECT))
    ...
    (RESPONSEENVELOPESTRING (%DO-HTTP-REQUEST URL (CONS-LIST KWD-SOAP-METHOD KWD-SOAP-POST KWD-SOAP-HEADERS HEADERS KWD-SOAP-CONTENT (WRAP-STRING REQUESTSTRING))))
    (RESPONSEENVELOPEOBJECT (FROM-XML-STRING RESPONSEENVELOPESTRING)) (RESPONSEOBJECT (|%contents| (|%body| RESPONSEENVELOPEOBJECT))))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING REQUESTSTRING RESPONSEENVELOPESTRING))
   (CL:WHEN DEBUG? (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "response received: " EOL RESPONSEENVELOPESTRING EOL)) RESPONSEOBJECT))

;;; (DEFUN (MAKE-SOAP-RPC-REQUEST-HEADERS HTTP-HEADERS) ...)

(CL:DEFUN MAKE-SOAP-RPC-REQUEST-HEADERS ()
  (CL:LET* ((RESULT (NEW-KEY-VALUE-LIST))) (INSERT-AT RESULT (WRAP-STRING "content-type") (WRAP-STRING "text/xml; charset=utf-8"))
   (INSERT-AT RESULT (WRAP-STRING "SoapAction") (WRAP-STRING "")) RESULT))

(CL:DEFUN STARTUP-SOAP ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/SOAP" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ KWD-SOAP-CONTENT-TYPE (INTERN-RIGID-SYMBOL-WRT-MODULE "CONTENT-TYPE" NULL 2))
    (CL:SETQ KWD-SOAP-XML (INTERN-RIGID-SYMBOL-WRT-MODULE "XML" NULL 2)) (CL:SETQ KWD-SOAP-DOCUMENTATION (INTERN-RIGID-SYMBOL-WRT-MODULE "DOCUMENTATION" NULL 2))
    (CL:SETQ KWD-SOAP-METHOD (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD" NULL 2)) (CL:SETQ KWD-SOAP-POST (INTERN-RIGID-SYMBOL-WRT-MODULE "POST" NULL 2))
    (CL:SETQ KWD-SOAP-HEADERS (INTERN-RIGID-SYMBOL-WRT-MODULE "HEADERS" NULL 2)) (CL:SETQ KWD-SOAP-CONTENT (INTERN-RIGID-SYMBOL-WRT-MODULE "CONTENT" NULL 2))
    (CL:SETQ SYM-SOAP-SOAP-STARTUP-SOAP (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-SOAP" NULL 0))
    (CL:SETQ SYM-SOAP-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5)
    (CL:LET*
     ((CLASS (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "SOAP-FAULT-EXCEPTION" "(DEFCLASS SOAP-FAULT-EXCEPTION (STELLA-EXCEPTION) :PUBLIC-SLOTS ((FAULT :TYPE |Fault|)))")))
...
RPC request.  This functions writes a SOAP envelope containing a service response to `stream'.\" :PUBLIC? TRUE)" (CL:FUNCTION PROCESS-SOAP-RPC-REQUEST) NULL)
    (DEFINE-FUNCTION-OBJECT "GET-RPC-METHOD" "(DEFUN (GET-RPC-METHOD SYMBOL) ((ENVELOPE |Envelope|)))" (CL:FUNCTION GET-RPC-METHOD) NULL)
    (DEFINE-FUNCTION-OBJECT "GET-RPC-ARGUMENTS" "(DEFUN (GET-RPC-ARGUMENTS (CONS OF |XMLObject|)) ((ENVELOPE |Envelope|)))" (CL:FUNCTION GET-RPC-ARGUMENTS) NULL)
    (DEFINE-FUNCTION-OBJECT "DISPATCH-RPC-CALL" "(DEFUN (DISPATCH-RPC-CALL |Envelope|) ((ENVELOPE |Envelope|)))" (CL:FUNCTION DISPATCH-RPC-CALL) NULL)
    (DEFINE-FUNCTION-OBJECT "DISPATCH-CALL-WITH-METHOD-AND-ARGS"
     "(DEFUN (DISPATCH-CALL-WITH-METHOD-AND-ARGS |Envelope|) ((METHODNAME SYMBOL) (ARGUMENTS (CONS OF |XMLObject|))))" (CL:FUNCTION DISPATCH-CALL-WITH-METHOD-AND-ARGS)
     NULL)
    (DEFINE-FUNCTION-OBJECT "CREATE-SOAP-ENVELOPE" "(DEFUN (CREATE-SOAP-ENVELOPE |Envelope|) ((CONTENTS |XMLObject|)))" (CL:FUNCTION CREATE-SOAP-ENVELOPE) NULL)
...
`factor' and `fdetail' are optional fields.  Fault code should be one of
\\\"SOAP-ENV:VersionMismatch\\\", \\\"SOAP-ENV:MustUnderstand\\\" \\\"SOAP-ENV:Client\\\" or
\\\"SOAP-ENV:Server\\\".  These may change to keywords in the future.\" :PUBLIC? TRUE)" (CL:FUNCTION CREATE-SOAP-FAULT) NULL)
...
command, unpack the envelope, dispatch the request, and return the result as SOAP envelope.\" :PUBLIC? TRUE)" (CL:FUNCTION SOAP-RPC-REQUEST-HANDLER) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "MAKE-SOAP-RPC-REQUEST-HEADERS" "(DEFUN (MAKE-SOAP-RPC-REQUEST-HEADERS HTTP-HEADERS) ())" (CL:FUNCTION MAKE-SOAP-RPC-REQUEST-HEADERS) NULL)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SOAP-SOAP-STARTUP-SOAP)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SOAP-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupSoap") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "SOAP")))
    (PUBLISH-HANDLER "/ploom/soap-rpc/generic-soap-service" (CL:FUNCTION SOAP-RPC-REQUEST-HANDLER) KWD-SOAP-CONTENT-TYPE
     (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-SOAP-XML NULL)) KWD-SOAP-DOCUMENTATION (WRAP-STRING "Generic SOAP RPC handler.")))))
