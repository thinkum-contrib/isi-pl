...
(CL:DEFCLASS |Envelope| (|XMLObject|)
  ((|encodingStyle| :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR
    |%encodingStyle|)
   (|body| :ALLOCATION :INSTANCE :ACCESSOR |%body|)))

(CL:DEFUN |new-Envelope| ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE |Envelope|)))
   (CL:SETF (|%cdataContent| SELF) STELLA::NULL-STRING) (CL:SETF (|%textContent| SELF) STELLA::NULL-STRING)
   (CL:SETF (|%body| SELF) NULL) (CL:SETF (|%encodingStyle| SELF) "http://schemas.xmlsoap.org/soap/encoding/") SELF))
...
(CL:DEFUN |access-Envelope-Slot-Value| (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-XML-OBJECTS-body|)
    (CL:IF SETVALUE? (CL:SETF (|%body| SELF) VALUE) (CL:SETQ VALUE (|%body| SELF))))
   (CL:T
...
(CL:DEFUN |access-Body-Slot-Value| (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-SOAP-ENV-contents|)
    (CL:IF SETVALUE? (CL:SETF (|%contents| SELF) VALUE) (CL:SETQ VALUE (|%contents| SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN |new-Fault| ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE |Fault|)))
   (CL:SETF (|%cdataContent| SELF) STELLA::NULL-STRING) (CL:SETF (|%textContent| SELF) STELLA::NULL-STRING)
   (CL:SETF (|%detail| SELF) NULL) (CL:SETF (|%faultactor| SELF) NULL) (CL:SETF (|%faultstring| SELF) NULL)
...
(CL:DEFUN |access-Fault-Slot-Value| (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-SOAP-ENV-faultcode|)
    (CL:IF SETVALUE? (CL:SETF (|%faultcode| SELF) VALUE) (CL:SETQ VALUE (|%faultcode| SELF))))
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-SOAP-ENV-faultstring|)
    (CL:IF SETVALUE? (CL:SETF (|%faultstring| SELF) VALUE) (CL:SETQ VALUE (|%faultstring| SELF))))
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-SOAP-ENV-faultactor|)
    (CL:IF SETVALUE? (CL:SETF (|%faultactor| SELF) VALUE) (CL:SETQ VALUE (|%faultactor| SELF))))
   ((CL:EQ SLOTNAME |SYM-SOAP-ENV-SOAP-ENV-detail|)
    (CL:IF SETVALUE? (CL:SETF (|%detail| SELF) VALUE) (CL:SETQ VALUE (|%detail| SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN |new-faultcode| ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE |faultcode|)))
...
(CL:DEFUN |new-faultstring| ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE |faultstring|)))
   (CL:SETF (|%cdataContent| SELF) STELLA::NULL-STRING) (CL:SETF (|%textContent| SELF) STELLA::NULL-STRING) SELF))
...
(CL:DEFUN |new-faultactor| ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE |faultactor|)))
   (CL:SETF (|%cdataContent| SELF) STELLA::NULL-STRING) (CL:SETF (|%textContent| SELF) STELLA::NULL-STRING) SELF))
...
(CL:DEFUN STARTUP-SOAP-ENV ()
  (CL:LET*
   ((*MODULE* (GET-STELLA-MODULE "/STELLA/XML-OBJECTS/SOAP-ENV" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ |SGT-SOAP-ENV-SOAP-ENV-Envelope| (INTERN-RIGID-SYMBOL-WRT-MODULE "Envelope" NULL 1))
    ...
    (CL:SETQ |SYM-SOAP-ENV-XML-OBJECTS-body|
     (INTERN-RIGID-SYMBOL-WRT-MODULE "body" (GET-STELLA-MODULE "/STELLA/XML-OBJECTS" CL:T) 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5)
    (CL:LET*
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION |access-Envelope-Slot-Value|)))
    (CL:LET*
     ((CLASS
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION |access-Body-Slot-Value|)))
    (CL:LET*
     ((CLASS
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION |access-Fault-Slot-Value|)))
    ...
    (CL:LET*
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION |access-detail-Slot-Value|))))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-SOAP-ENV" "(DEFUN STARTUP-SOAP-ENV () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-SOAP-ENV) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SOAP-ENV-SOAP-ENV-STARTUP-SOAP-ENV)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SOAP-ENV-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "SOAP-ENV")))
    ...
    (INSERT-AT *NAMESPACE-URI-PREFIX-TABLE* (WRAP-STRING "http://schemas.xmlsoap.org/soap/envelope/")
