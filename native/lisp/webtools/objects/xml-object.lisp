...
(CL:DEFUN DEFNAMESPACE (|body|)
  (CL:LET*
   ((|namespaceURI| (%%VALUE |body|)) (|namespaceName| (%MODULE-NAME *MODULE*)) (|bodyWithHeader| (CONS NULL |body|)))
     ...
     ((|options| SELF-000) (|invisible?| (LOOKUP |options| KWD-XML-OBJECT-INVISIBLE-ON-OUTPUT?))
      (|invisibleForm|
       (CL:IF (CL:AND (CL:NOT (CL:EQ |invisible?| NULL)) (STRING-EQUAL? (%SYMBOL-NAME |invisible?|) "TRUE"))
        (CONS
         (LIST* SYM-XML-OBJECT-STELLA-PUSHQ SYM-XML-OBJECT-XML-OBJECTS-*INVISIBLE-NAMESPACES-ON-OUTPUT*
          (WRAP-STRING |namespaceName|) NIL)
         NIL)
        NIL)))
     (LIST* SYM-XML-OBJECT-STELLA-STARTUP-TIME-PROGN
      (LIST* SYM-XML-OBJECT-STELLA-INSERT-AT SYM-XML-OBJECT-XML-OBJECTS-*NAMESPACE-PREFIX-URI-TABLE*
       (WRAP-STRING |namespaceName|) (CONS |namespaceURI| NIL))
      (LIST* SYM-XML-OBJECT-STELLA-INSERT-AT SYM-XML-OBJECT-XML-OBJECTS-*NAMESPACE-URI-PREFIX-TABLE* |namespaceURI|
       (CONS (WRAP-STRING |namespaceName|) NIL))
...
(CL:DEFCLASS |XMLObject| (STANDARD-OBJECT)
  ((|textContent| :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR |%textContent|)
   (|cdataContent| :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR
    |%cdataContent|))
...
(CL:DEFUN |access-XMLObject-Slot-Value| (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME |SYM-XML-OBJECT-XML-OBJECTS-cdataContent|)
    (CL:IF SETVALUE? (CL:SETF (|%cdataContent| SELF) (%WRAPPER-VALUE VALUE))
...
whether `CONTENT' needs to be encoded as CDATA or not."
  ...
  (CL:LET* ((FOUND?-000 CL:NIL))
   (CL:LET*
    ((CH NULL-CHARACTER) (VECTOR-000 CONTENT) (INDEX-000 0) (LENGTH-000 (CL:THE CL:FIXNUM (CL:LENGTH VECTOR-000))))
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:WHEN (HTML-CHARACTER-NEEDS-QUOTING? CH) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
     (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
   (CL:IF FOUND?-000
    (CL:IF (CL:NOT (CL:= (STRING-SEARCH CONTENT "]]>" 0) NULL-INTEGER))
     (CL:LET* ((S (NEW-OUTPUT-STRING-STREAM))) (WRITE-HTML-QUOTING-SPECIAL-CHARACTERS (%NATIVE-STREAM S) CONTENT)
...
(CL:DEFUN STARTUP-XML-OBJECT ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/STELLA/XML-OBJECTS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-XML-OBJECT-INVISIBLE-ON-OUTPUT? (INTERN-RIGID-SYMBOL-WRT-MODULE "INVISIBLE-ON-OUTPUT?" NULL 2))
    (CL:SETQ SYM-XML-OBJECT-STELLA-PUSHQ (INTERN-RIGID-SYMBOL-WRT-MODULE "PUSHQ" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ SYM-XML-OBJECT-XML-OBJECTS-*INVISIBLE-NAMESPACES-ON-OUTPUT*
     (INTERN-RIGID-SYMBOL-WRT-MODULE "*INVISIBLE-NAMESPACES-ON-OUTPUT*" NULL 0))
    (CL:SETQ SYM-XML-OBJECT-STELLA-STARTUP-TIME-PROGN
     (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-TIME-PROGN" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ SYM-XML-OBJECT-STELLA-INSERT-AT
     (INTERN-RIGID-SYMBOL-WRT-MODULE "INSERT-AT" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ SYM-XML-OBJECT-XML-OBJECTS-*NAMESPACE-PREFIX-URI-TABLE*
     (INTERN-RIGID-SYMBOL-WRT-MODULE "*NAMESPACE-PREFIX-URI-TABLE*" NULL 0))
    (CL:SETQ SYM-XML-OBJECT-XML-OBJECTS-*NAMESPACE-URI-PREFIX-TABLE*
     (INTERN-RIGID-SYMBOL-WRT-MODULE "*NAMESPACE-URI-PREFIX-TABLE*" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *NAMESPACE-PREFIX-URI-TABLE* (NEW-PROPERTY-LIST))
    (CL:SETQ *NAMESPACE-URI-PREFIX-TABLE* (NEW-PROPERTY-LIST)) (CL:SETQ *INVISIBLE-NAMESPACES-ON-OUTPUT* NIL))
...
of a slot.  The currently understood values are :PLAIN (or NULL) and :BASE64\") \"/STELLA/XML-OBJECTS\")"
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "DEFNAMESPACE" "(DEFUN DEFNAMESPACE (|&BODY| (body CONS)) :TYPE OBJECT :MACRO? TRUE)"
...
whether `CONTENT' needs to be encoded as CDATA or not.\")" (CL:FUNCTION SET-CONTENT) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-XML-OBJECT" "(DEFUN STARTUP-XML-OBJECT () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-XML-OBJECT) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-XML-OBJECT-XML-OBJECTS-STARTUP-XML-OBJECT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-XML-OBJECT-STELLA-METHOD-STARTUP-CLASSNAME
