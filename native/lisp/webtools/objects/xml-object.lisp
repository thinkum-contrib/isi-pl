...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* NULL-CHARACTER NULL-INTEGER *MODULE* NIL))
...
(CL:DEFUN DEFNAMESPACE (|body|)
  (CL:LET*
   ...
   (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
    (CL:SETF (%THE-PLIST SELF-000) (EXTRACT-OPTIONS |bodyWithHeader| NULL))
    (CL:LET*
     ((|options| SELF-000)
      (|invisible?| (LOOKUP |options| KWD-XML-OBJECT-INVISIBLE-ON-OUTPUT?))
      (|invisibleForm|
       (CL:IF
     ...
     (LIST* SYM-XML-OBJECT-STELLA-STARTUP-TIME-PROGN
      ...
      (LIST* SYM-XML-OBJECT-STELLA-INSERT-AT
       SYM-XML-OBJECT-XML-OBJECTS-*NAMESPACE-URI-PREFIX-TABLE* |namespaceURI|
  ...
  (CL:LET* ((FOUND?-000 CL:NIL))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:WHEN (HTML-CHARACTER-NEEDS-QUOTING? CH) (CL:SETQ FOUND?-000 CL:T)
      (CL:RETURN))
     (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
   (CL:IF FOUND?-000
    (CL:IF (CL:NOT (CL:= (STRING-SEARCH CONTENT "]]>" 0) NULL-INTEGER))
     (CL:LET* ((S (NEW-OUTPUT-STRING-STREAM)))
      (WRITE-HTML-QUOTING-SPECIAL-CHARACTERS (%NATIVE-STREAM S) CONTENT)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ SYM-XML-OBJECT-XML-OBJECTS-*INVISIBLE-NAMESPACES-ON-OUTPUT*
     (INTERN-RIGID-SYMBOL-WRT-MODULE "*INVISIBLE-NAMESPACES-ON-OUTPUT*" NULL
      0))
...
whether `CONTENT' needs to be encoded as CDATA or not.\")"
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-XML-OBJECT-XML-OBJECTS-STARTUP-XML-OBJECT)))
