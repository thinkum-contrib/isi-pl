...
(CL:DEFUN STARTUP-WEBTOOLS-SYSTEM ()
  (WITH-PROCESS-LOCK *BOOTSTRAP-LOCK* (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 0) (CL:WHEN (CL:NOT (SYSTEM-STARTED-UP? "stella" "/STELLA")) (STARTUP-STELLA-SYSTEM)))
   ...
   (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP/WEBTOOLS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
     (CL:SETQ SYM-STARTUP-SYSTEM-WEBTOOLS-STARTUP-WEBTOOLS-SYSTEM (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-WEBTOOLS-SYSTEM" NULL 0))
    ...
    (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
     (DEFINE-FUNCTION-OBJECT "STARTUP-WEBTOOLS-SYSTEM" "(DEFUN STARTUP-WEBTOOLS-SYSTEM () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-WEBTOOLS-SYSTEM) NULL)
     (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STARTUP-SYSTEM-WEBTOOLS-STARTUP-WEBTOOLS-SYSTEM)))
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-STARTUP-SYSTEM-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "StartupWebtoolsSystem") NULL-STRING-WRAPPER)))
    (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 8) (FINALIZE-SLOTS) (CLEANUP-UNFINALIZED-CLASSES))
    (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/HTTP/WEBTOOLS")))
     (CL:LET* ((PHASE NULL-INTEGER) (ITER-006 0) (UPPER-BOUND-007 9)) (CL:DECLARE (CL:TYPE CL:FIXNUM PHASE ITER-006 UPPER-BOUND-007))
      (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-007) DO (CL:SETQ PHASE ITER-006) (CL:SETQ *STARTUP-TIME-PHASE* PHASE) (STARTUP-HTTP-CLIENT) (STARTUP-HTTP-SERVER)
       (STARTUP-SESSIONS) (STARTUP-XML-OBJECT) (STARTUP-XML-SCHEMA) (STARTUP-XML-SCHEMA-INSTANCE) (STARTUP-APACHE-SOAP) (STARTUP-SOAP-ENV) (STARTUP-MARSHALLER)
       (STARTUP-SOAP) (CL:SETQ ITER-006 (CL:1+ ITER-006))))
