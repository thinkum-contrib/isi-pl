...
(CL:DEFCLASS SESSION (DICTIONARY)
  ((NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :DOCUMENTATION "UUID uniquely identifying this session." :ALLOCATION :INSTANCE :ACCESSOR %NAME)
   (TIMEOUT :DOCUMENTATION "Timeout duration of session; NULL means no timeout." :ALLOCATION :INSTANCE :ACCESSOR %TIMEOUT)
   (TIMESTAMP :DOCUMENTATION "Time this object was last accessed." :ALLOCATION :INSTANCE :ACCESSOR %TIMESTAMP)
   (OBJECTS :DOCUMENTATION "Table of session-related objects indexed on string keys." :ALLOCATION :INSTANCE :ACCESSOR %OBJECTS))
...
(CL:DEFUN NEW-SESSION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE SESSION))) (CL:SETF (%OBJECTS SELF) (NEW-KEY-VALUE-MAP))
   (CL:SETF (%TIMESTAMP SELF) (MAKE-CURRENT-DATE-TIME)) (CL:SETF (%TIMEOUT SELF) NULL) (CL:SETF (%NAME SELF) STELLA::NULL-STRING) SELF))
...
(CL:DEFUN ACCESS-SESSION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-SESSIONS-STELLA-NAME) (CL:IF SETVALUE? (CL:SETF (%NAME SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%NAME SELF)))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-TIMEOUT) (CL:IF SETVALUE? (CL:SETF (%TIMEOUT SELF) VALUE) (CL:SETQ VALUE (%TIMEOUT SELF))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-TIMESTAMP) (CL:IF SETVALUE? (CL:SETF (%TIMESTAMP SELF) VALUE) (CL:SETQ VALUE (%TIMESTAMP SELF))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-OBJECTS) (CL:IF SETVALUE? (CL:SETF (%OBJECTS SELF) VALUE) (CL:SETQ VALUE (%OBJECTS SELF))))
   (CL:T
...
(CL:DEFUN CLEANUP-SESSION-TABLE ()
  (CL:SETQ *SESSION-TABLE-CLEANUP-COUNTER* (CL:1+ *SESSION-TABLE-CLEANUP-COUNTER*))
  (CL:WHEN (CL:= (MOD *SESSION-TABLE-CLEANUP-COUNTER* 100) 0)
   (WITH-PROCESS-LOCK *SESSIONS-LOCK*
    (CL:LET* ((NAME NULL) (SESSION NULL) (ITER-000 (ALLOCATE-ITERATOR *SESSION-TABLE*)))
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ NAME (%KEY ITER-000)) (CL:SETQ SESSION (%VALUE ITER-000))
...
haven't yet timed out."
  ...
  (CL:LET* ((SESSION (LOOKUP *SESSION-TABLE* (WRAP-STRING NAME))))
   (CL:WHEN (CL:NOT (CL:EQ SESSION NULL))
    (CL:IF (TIMED-OUT? SESSION) (CL:WHEN ERRORIFTIMEOUT? (CL:ERROR (NEW-TIMEOUT-EXCEPTION (CONCATENATE "lookup-session: session timed out: " NAME)))) (REFRESH SESSION)))
   SESSION))
...
Timeout after `timeout' seconds or never in case the value is <= 0 or undefined."
  ...
  (CL:LET* ((SESSION (NEW-SESSION))) (CL:WHEN (CL:EQ NAME STELLA::NULL-STRING) (CL:SETQ NAME (GENERATE-RANDOM-UUID))) (CL:SETF (%NAME SESSION) NAME)
   ...
   (WITH-PROCESS-LOCK *SESSIONS-LOCK* (INSERT-AT *SESSION-TABLE* (WRAP-STRING NAME) SESSION)) (CLEANUP-SESSION-TABLE) SESSION))
...
(CL:DEFMETHOD TIMED-OUT? ((SESSION SESSION))
  ...
  (CL:LET* ((TIMEOUT (%TIMEOUT SESSION)) (TIMESTAMP (%TIMESTAMP SESSION)))
   (CL:IF (CL:EQ TIMEOUT NULL) CL:NIL (GREATER? (TIME-SUBTRACT (MAKE-CURRENT-DATE-TIME) TIMESTAMP) TIMEOUT))))

;;; (DEFMETHOD REFRESH ...)

(CL:DEFMETHOD REFRESH ((SESSION SESSION))
  "Bump the timestamp of `session'."
  (CL:LET* ((TIMEOUT (%TIMEOUT SESSION))) (CL:WHEN (CL:NOT (CL:EQ TIMEOUT NULL)) (CL:SETF (%TIMESTAMP SESSION) (MAKE-CURRENT-DATE-TIME)))))
...
(CL:DEFUN STARTUP-SESSIONS ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ SGT-SESSIONS-HTTP-SESSION (INTERN-RIGID-SYMBOL-WRT-MODULE "SESSION" NULL 1))
    (CL:SETQ SYM-SESSIONS-STELLA-NAME (INTERN-RIGID-SYMBOL-WRT-MODULE "NAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ SYM-SESSIONS-HTTP-TIMEOUT (INTERN-RIGID-SYMBOL-WRT-MODULE "TIMEOUT" NULL 0))
    (CL:SETQ SYM-SESSIONS-HTTP-TIMESTAMP (INTERN-RIGID-SYMBOL-WRT-MODULE "TIMESTAMP" NULL 0))
    (CL:SETQ SYM-SESSIONS-HTTP-OBJECTS (INTERN-RIGID-SYMBOL-WRT-MODULE "OBJECTS" NULL 0))
    (CL:SETQ SYM-SESSIONS-HTTP-STARTUP-SESSIONS (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-SESSIONS" NULL 0))
    (CL:SETQ SYM-SESSIONS-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *SESSION-TABLE* (NEW-KEY-VALUE-MAP)) (CL:SETQ *SESSIONS-LOCK* (MAKE-PROCESS-LOCK)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5)
    (CL:LET*
     ((CLASS
       (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "SESSION" "(DEFCLASS SESSION (DICTIONARY) :DOCUMENTATION \"Object storing the state associated with a single session.
All session-related information is stored in a table indexed on string keys.\" :PARAMETERS ((ANY-KEY :TYPE STRING-WRAPPER) (ANY-VALUE :TYPE OBJECT)) :SLOTS ((NAME :TYPE STRING :INITIALLY NULL :PUBLIC? TRUE :DOCUMENTATION \"UUID uniquely identifying this session.\") (TIMEOUT :TYPE TIME-DURATION :INITIALLY NULL :DOCUMENTATION \"Timeout duration of session; NULL means no timeout.\") (TIMESTAMP :TYPE CALENDAR-DATE :INITIALLY (MAKE-CURRENT-DATE-TIME) :DOCUMENTATION \"Time this object was last accessed.\") (OBJECTS :TYPE (KEY-VALUE-MAP OF STRING-WRAPPER OBJECT) :DOCUMENTATION \"Table of session-related objects indexed on string keys.\" :INITIALLY (NEW KEY-VALUE-MAP))))")))
     (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-SESSION)) (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-SESSION-SLOT-VALUE))))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (DEFINE-FUNCTION-OBJECT "CLEANUP-SESSION-TABLE" "(DEFUN CLEANUP-SESSION-TABLE ())" (CL:FUNCTION CLEANUP-SESSION-TABLE) NULL)
...
sessions with the same name).  If `name' is NULL, generate a random UUID for it.
Timeout after `timeout' seconds or never in case the value is <= 0 or undefined.\" :PUBLIC? TRUE)" (CL:FUNCTION CREATE-SESSION) NULL)
...
If `name' is NULL, generate a random UUID for it.  If an existing object was found but has timed
out, raise an error if `errorIfTimeout?' is true.  If a new object is created, initialize its
timeout field with `timeout' seconds (0 or undefined means no timeout).\" :PUBLIC? TRUE)" (CL:FUNCTION GET-SESSION) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (TIMED-OUT? BOOLEAN) ((SESSION SESSION)) :DOCUMENTATION \"Return TRUE if `session' has timed out.\" :PUBLIC? TRUE)"
     (CL:FUNCTION TIMED-OUT?) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD REFRESH ((SESSION SESSION)) :DOCUMENTATION \"Bump the timestamp of `session'.\" :PUBLIC? TRUE)" (CL:FUNCTION REFRESH) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-SESSIONS" "(DEFUN STARTUP-SESSIONS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-SESSIONS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SESSIONS-HTTP-STARTUP-SESSIONS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SESSIONS-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupSessions") NULL-STRING-WRAPPER)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 8) (FINALIZE-SLOTS) (CLEANUP-UNFINALIZED-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "HTTP")))
...
globally unique session name.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *SESSIONS-LOCK* PROCESS-LOCK-OBJECT (MAKE-PROCESS-LOCK))")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *SESSION-TABLE-CLEANUP-COUNTER* INTEGER 0)"))))
