...
(CL:DEFCLASS SESSION (DICTIONARY)
  ((NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :DOCUMENTATION "UUID uniquely identifying this session."
    :ALLOCATION :INSTANCE :ACCESSOR %NAME)
   (TIMEOUT :DOCUMENTATION "Timeout duration of session; NULL means no timeout." :ALLOCATION :INSTANCE :ACCESSOR
    %TIMEOUT)
   (TIMESTAMP :DOCUMENTATION "Time this object was last accessed." :ALLOCATION :INSTANCE :ACCESSOR %TIMESTAMP)
   (OBJECTS :DOCUMENTATION "Table of session-related objects indexed on string keys." :ALLOCATION :INSTANCE :ACCESSOR
    %OBJECTS))
...
(CL:DEFUN NEW-SESSION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE SESSION)))
   (CL:SETF (%OBJECTS SELF) (NEW-KEY-VALUE-MAP)) (CL:SETF (%TIMESTAMP SELF) (MAKE-CURRENT-DATE-TIME))
   (CL:SETF (%TIMEOUT SELF) NULL) (CL:SETF (%NAME SELF) STELLA::NULL-STRING) SELF))
...
(CL:DEFUN ACCESS-SESSION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SESSIONS-STELLA-NAME)
    (CL:IF SETVALUE? (CL:SETF (%NAME SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%NAME SELF)))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-TIMEOUT)
    (CL:IF SETVALUE? (CL:SETF (%TIMEOUT SELF) VALUE) (CL:SETQ VALUE (%TIMEOUT SELF))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-TIMESTAMP)
    (CL:IF SETVALUE? (CL:SETF (%TIMESTAMP SELF) VALUE) (CL:SETQ VALUE (%TIMESTAMP SELF))))
   ((CL:EQ SLOTNAME SYM-SESSIONS-HTTP-OBJECTS)
    (CL:IF SETVALUE? (CL:SETF (%OBJECTS SELF) VALUE) (CL:SETQ VALUE (%OBJECTS SELF))))
   (CL:T
   ...
   (CL:WHEN (CL:NOT (CL:EQ SESSION NULL))
    (CL:IF (TIMED-OUT? SESSION)
     (CL:WHEN ERRORIFTIMEOUT?
      (CL:ERROR (NEW-TIMEOUT-EXCEPTION (CONCATENATE "lookup-session: session timed out: " NAME))))
     (REFRESH SESSION)))
   SESSION))
...
Timeout after `timeout' seconds or never in case the value is <= 0 or undefined."
  ...
  (CL:LET* ((SESSION (NEW-SESSION))) (CL:WHEN (CL:EQ NAME STELLA::NULL-STRING) (CL:SETQ NAME (GENERATE-RANDOM-UUID)))
   (CL:SETF (%NAME SESSION) NAME)
   (CL:WHEN (CL:> TIMEOUT 0) (CL:SETF (%TIMEOUT SESSION) (MAKE-TIME-DURATION 0 (CL:* TIMEOUT 1000))))
   (WITH-PROCESS-LOCK *SESSIONS-LOCK* (INSERT-AT *SESSION-TABLE* (WRAP-STRING NAME) SESSION)) (CLEANUP-SESSION-TABLE)
   SESSION))
...
timeout field with `timeout' seconds (0 or undefined means no timeout)."
  ...
  (CL:IF (CL:EQ NAME STELLA::NULL-STRING) (CREATE-SESSION STELLA::NULL-STRING TIMEOUT)
   (CL:LET* ((SESSION (LOOKUP-SESSION NAME ERRORIFTIMEOUT?)))
    (CL:WHEN (CL:EQ SESSION NULL) (CL:SETQ SESSION (CREATE-SESSION NAME TIMEOUT))) SESSION)))
...
(CL:DEFMETHOD REFRESH ((SESSION SESSION))
  ...
  (CL:LET* ((TIMEOUT (%TIMEOUT SESSION)))
   (CL:WHEN (CL:NOT (CL:EQ TIMEOUT NULL)) (CL:SETF (%TIMESTAMP SESSION) (MAKE-CURRENT-DATE-TIME)))))
...
(CL:DEFUN STARTUP-SESSIONS ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-SESSIONS-HTTP-SESSION (INTERN-RIGID-SYMBOL-WRT-MODULE "SESSION" NULL 1))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *SESSION-TABLE* (NEW-KEY-VALUE-MAP))
...
All session-related information is stored in a table indexed on string keys.\" :PARAMETERS ((ANY-KEY :TYPE STRING-WRAPPER) (ANY-VALUE :TYPE OBJECT)) :SLOTS ((NAME :TYPE STRING :INITIALLY NULL :PUBLIC? TRUE :DOCUMENTATION \"UUID uniquely identifying this session.\") (TIMEOUT :TYPE TIME-DURATION :INITIALLY NULL :DOCUMENTATION \"Timeout duration of session; NULL means no timeout.\") (TIMESTAMP :TYPE CALENDAR-DATE :INITIALLY (MAKE-CURRENT-DATE-TIME) :DOCUMENTATION \"Time this object was last accessed.\") (OBJECTS :TYPE (KEY-VALUE-MAP OF STRING-WRAPPER OBJECT) :DOCUMENTATION \"Table of session-related objects indexed on string keys.\" :INITIALLY (NEW KEY-VALUE-MAP))))")))
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-SESSION-SLOT-VALUE))))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "CLEANUP-SESSION-TABLE" "(DEFUN CLEANUP-SESSION-TABLE ())"
...
If `name' is NULL, generate a random UUID for it.  If an existing object was found but has timed
out, raise an error if `errorIfTimeout?' is true.  If a new object is created, initialize its
timeout field with `timeout' seconds (0 or undefined means no timeout).\" :PUBLIC? TRUE)" (CL:FUNCTION GET-SESSION)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-SESSIONS" "(DEFUN STARTUP-SESSIONS () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-SESSIONS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SESSIONS-HTTP-STARTUP-SESSIONS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SESSIONS-STELLA-METHOD-STARTUP-CLASSNAME
