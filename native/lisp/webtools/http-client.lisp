...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* NIL STANDARD-OUTPUT NULL-CHARACTER
  NULL-INTEGER))
 ...
 (CL:FTYPE
  (CL:FUNCTION (CL:SIMPLE-STRING)
   (CL:VALUES CL:SIMPLE-STRING CL:SIMPLE-STRING CL:SIMPLE-STRING
    CL:SIMPLE-STRING CL:SIMPLE-STRING CL:SIMPLE-STRING CL:SIMPLE-STRING))
...
References:  http://www.gbiv.com/protocols/uri/rfc/rfc3986.html"
  ...
  (CL:LET*
   ((FIELD-START 0) (FIELD-END (STRING-SEARCH URL (MAKE-STRING 1 #\:) 0))
    (PROTOCOL "") (USER-INFORMATION "") (DOMAIN-NAME "") (PORT-NUMBER "")
    (PATH "") (QUERY "") (FRAGMENT ""))
   ...
   (CL:VALUES PROTOCOL USER-INFORMATION DOMAIN-NAME PORT-NUMBER PATH QUERY
    FRAGMENT)))
...
components.  Undefined portions can be supplied as NULL or the empty string."
  (CL:DECLARE
   (CL:TYPE CL:SIMPLE-STRING PROTOCOL USERINFO HOST PATH QUERY FRAGMENT)
...
(CL:DEFUN URL-PATH-TO-FILENAME (PATH)
  ...
  (CL:LET* ((SEPARATOR (DIRECTORY-SEPARATOR)))
   (CL:IF (CL:EQL SEPARATOR #\/) (UNESCAPE-URL-STRING PATH)
    (CL:PROGN
     (CL:SETQ PATH (UNESCAPE-URL-STRING (SUBSTITUTE PATH SEPARATOR #\/)))
     (CL:WHEN
...
(CL:DEFUN NEW-HTTP-EXCEPTION (MESSAGE)
  ...
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-CONDITION (CL:QUOTE HTTP-EXCEPTION) *CONDITION-MESSAGE-KEYWORD*
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:= SPACE2 NULL-INTEGER)
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Malformed header line: `"
      LINE "'")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
   (INSERT-AT FIELDS (WRAP-STRING "") (WRAP-STRING LINE))
   (CL:VALUES (SUBSEQUENCE LINE 0 SPACE1)
    (CL:TRUNCATE (STRING-TO-INTEGER (SUBSEQUENCE LINE (CL:1+ SPACE1) SPACE2)))
...
(CL:DEFUN READ-HTTP-HEADER-LINE (STREAM)
  (CL:LET*
   ((LINE (NATIVE-READ-LINE STREAM))
    (LENGTH (CL:THE CL:FIXNUM (CL:LENGTH LINE))) (NEXTCHAR NULL-CHARACTER))
...
all values."
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING VERSION-STRING RESPONSE-MESSAGE)
    ...
    ((SUBTYPE-OF-STRING? TEST-VALUE-000)
     (CL:PROGN
      (CL:VALUES CONTENT
       (CL:TRUNCATE
        (CL:THE CL:FIXNUM
         (CL:LENGTH (CL:THE CL:SIMPLE-STRING (UNWRAP-STRING CONTENT))))))))
...
an input stream to read from."
  ...
  (CL:LET* ((CONTENTLENGTH 0))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (%SYMBOL-NAME METHOD) " " URL-PATH
    " HTTP/1.0
")
   ...
   (CL:WHEN (CL:NOT (CL:EQ CONTENT NULL))
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE CONTENT)))
     (CL:COND
      ((SUBTYPE-OF-STRING? TEST-VALUE-000)
       (CL:PROGN
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (UNWRAP-STRING CONTENT))))
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-HTTP-CLIENT-STELLA-INPUT-STREAM)
        ...
        (CL:LET* ((SELF-001 (NEW-HTTP-EXCEPTION RETURN-MESSAGE)))
         ...
         (CL:LET* ((EX SELF-001))
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
           "http-post-data: code=" RETURN-CODE ", message=" RETURN-MESSAGE
           ", version=" VERSION)
     ...
     (CL:COND ((HTTP-SUCCESS-RESPONSE-CODE? RETURN-CODE) IN)
      (CL:T
       (CL:LET* ((SELF-001 (NEW-HTTP-EXCEPTION RETURN-MESSAGE)))
        ...
        (CL:LET* ((EX SELF-001))
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
          "http-get-data: code=" RETURN-CODE ", message=" RETURN-MESSAGE
          ", version=" VERSION)
         (CLOSE-STREAM IN) (CLOSE-STREAM OUT) (CL:ERROR EX)))))))))
  ...
  (CL:LET*
   ...
   (CL:DECLARE
    (CL:TYPE CL:SIMPLE-STRING PROTOCOL USER-INFO HOST PORT PATH QUERY
     FRAGMENT))
   (CL:MULTIPLE-VALUE-SETQ (PROTOCOL USER-INFO HOST PORT PATH QUERY FRAGMENT)
       ...
       (CL:T
        (CL:LET*
         ((THEPORT HTTP-DEFAULT-PORT)
          (THEPATH
           (COMPOSE-INTERNET-URL "" "" "" NULL-INTEGER PATH QUERY FRAGMENT)))
...
Syntax is `(WITH-INPUT-URL (var url) body+)'."
  (CL:LET* ((VAR (%%VALUE BINDING)))
   (LIST* SYM-HTTP-CLIENT-STELLA-LET
    (CONS
     (CONS VAR
      (LIST* SYM-HTTP-CLIENT-STELLA-INPUT-STREAM SYM-HTTP-CLIENT-STELLA-NULL
       NIL))
     NIL)
    (LIST* SYM-HTTP-CLIENT-STELLA-UNWIND-PROTECT
     (LIST* SYM-HTTP-CLIENT-STELLA-PROGN
      (LIST* SYM-HTTP-CLIENT-STELLA-SETQ VAR
       (CONS
        (LIST* SYM-HTTP-CLIENT-HTTP-OPEN-URL-STREAM (%%VALUE (%%REST BINDING))
         NIL)
        NIL))
...
(CL:DEFUN DOWNLOAD-URL-TO-STRING-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:LET*
   ((RESULT (%DOWNLOAD-URL-TO-STRING (%WRAPPER-VALUE (%%VALUE ARGUMENTS)))))
...
(CL:DEFUN %DO-HTTP-REQUEST (URI OPTIONS)
  ...
  (CL:LET*
   ((PROPLIST
     (VET-OPTIONS OPTIONS
      (GET-QUOTED-TREE "((:METHOD :HEADERS :CONTENT) \"/HTTP\")" "/HTTP")))
    (METHOD
     ...
     (CL:LET* ((HTTPHEADERS (NEW-KEY-VALUE-LIST)))
      (CL:LET*
       ...
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000))
        (CL:SETQ VALUE (%VALUE ITER-000)) (INSERT-AT HTTPHEADERS KEY VALUE)))
...
CAUTION: String conversion will not do the right thing for binary data in Java.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-HTTP-CLIENT-HTTP-STARTUP-HTTP-CLIENT)))
