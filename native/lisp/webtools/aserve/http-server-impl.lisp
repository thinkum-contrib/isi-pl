...
(CL:DEFCLASS HTTP-EXCHANGE-ASERVE (HTTP-EXCHANGE)
  ((REQUEST :ALLOCATION :INSTANCE :ACCESSOR %REQUEST) (ENTITY :ALLOCATION :INSTANCE :ACCESSOR %ENTITY)
   (STREAM :ALLOCATION :INSTANCE :ACCESSOR %STREAM)))

(CL:DEFUN NEW-HTTP-EXCHANGE-ASERVE (REQUEST ENTITY)
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE HTTP-EXCHANGE-ASERVE)))
   (CL:SETF (%REQUEST SELF) REQUEST) (CL:SETF (%ENTITY SELF) ENTITY) (CL:SETF (%STREAM SELF) NULL) SELF))
...
(CL:DEFMETHOD GET-REPLY-HEADER-VALUE-IMPL ((SERVER HTTP-SERVER-ASERVE) XCHG KEY)
  (CL:LET* ((LXCHG XCHG) (LREQ (%REQUEST LXCHG)) (LKEY (LISPIFY KEY)))
   (COERCE-LISP-VALUE-TO-STRING (net.aserve:reply-header-slot-value lreq lkey))))
...
(CL:DEFMETHOD GET-REQUEST-URI-QUERY-IMPL ((SERVER HTTP-SERVER-ASERVE) XCHG)
  (CL:LET* ((LXCHG XCHG) (LREQ (%REQUEST LXCHG)))
   (COERCE-LISP-VALUE-TO-STRING (net.uri:uri-query (net.aserve:request-uri lreq)))))

;;; (DEFMETHOD (GET-REQUEST-URI-PATH-IMPL STRING) ...)

(CL:DEFMETHOD GET-REQUEST-URI-PATH-IMPL ((SERVER HTTP-SERVER-ASERVE) XCHG)
  (CL:LET* ((LXCHG XCHG) (LREQ (%REQUEST LXCHG)))
   (COERCE-LISP-VALUE-TO-STRING (net.uri:uri-path (net.aserve:request-uri lreq)))))
...
(CL:DEFMETHOD GET-REPLY-STREAM-IMPL ((SERVER HTTP-SERVER-ASERVE) XCHG)
  (CL:LET* ((LXCHG XCHG) (LSTREAM (%STREAM LXCHG)))
   (CL:WHEN (NULL? LSTREAM) (CL:SETQ LSTREAM (CL:make-string-output-stream)) (CL:SETF (%STREAM LXCHG) LSTREAM)) LSTREAM))

;;; (DEFUN (GET-ASERVE-RESPONSE-CODE LISP-CODE) ...)

(CL:DEFUN GET-ASERVE-RESPONSE-CODE (CODE)
  (CL:LET*
   ((NCODE (GET-HTTP-RESPONSE-CODE CODE NULL)) (LCODE (net.aserve::code-to-response ncode))
    (LCODEDESC (net.aserve::response-desc lcode)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NCODE) (CL:TYPE CL:SIMPLE-STRING LCODEDESC))
   (CL:WHEN (STRING-EQL? LCODEDESC "unknown code") (CL:SETQ LCODEDESC (GET-HTTP-RESPONSE-DESC CODE NULL))
...
(CL:DEFMETHOD SET-RESPONSE-CODE-IMPL ((SERVER HTTP-SERVER-ASERVE) XCHG CODE)
  (CL:LET* ((LXCHG XCHG) (LREQ (%REQUEST LXCHG)) (LCODE (GET-ASERVE-RESPONSE-CODE CODE)))
...
(CL:DEFUN GENERIC-RESPONSE-HANDLER (LREQ LENT)
  (CL:LET*
   ((HANDLER (CL:getf (net.aserve:entity-plist lent) 'response-handler)) (XCHG (NEW-HTTP-EXCHANGE-ASERVE LREQ LENT)))
   (CL:FUNCALL HANDLER XCHG)
   (CL:LET* ((STREAM (%STREAM XCHG)) (OUTPUT "")) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING OUTPUT))
    (CL:WHEN (DEFINED? STREAM) (CL:SETQ OUTPUT (cl:get-output-stream-string stream)))
...
(CL:DEFUN STARTUP-HTTP-SERVER-IMPL ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP/ASERVE" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (START-HTTP-SERVER-IMPL STRING) ((SERVER HTTP-SERVER-ASERVE) (PORT INTEGER)))"
     (CL:FUNCTION START-HTTP-SERVER-IMPL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD STOP-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER-ASERVE)))"
     (CL:FUNCTION STOP-HTTP-SERVER-IMPL) NULL)
    (DEFINE-FUNCTION-OBJECT "COERCE-LISP-VALUE-TO-STRING"
     "(DEFUN (COERCE-LISP-VALUE-TO-STRING STRING) ((LVALUE LISP-CODE)))" (CL:FUNCTION COERCE-LISP-VALUE-TO-STRING)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "GET-ASERVE-RESPONSE-CODE" "(DEFUN (GET-ASERVE-RESPONSE-CODE LISP-CODE) ((CODE KEYWORD)))"
    ...
    (DEFINE-FUNCTION-OBJECT "GENERIC-RESPONSE-HANDLER"
     "(DEFUN GENERIC-RESPONSE-HANDLER ((LREQ LISP-CODE) (LENT LISP-CODE)))" (CL:FUNCTION GENERIC-RESPONSE-HANDLER)
     NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-HTTP-SERVER-IMPL" "(DEFUN STARTUP-HTTP-SERVER-IMPL () :PUBLIC? TRUE)"
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-HTTP-SERVER-IMPL-ASERVE-STARTUP-HTTP-SERVER-IMPL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-HTTP-SERVER-IMPL-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "HTTP/ASERVE")))
    (CL:IF
     (CL:AND (CL:NOT (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL))
      (CL:NOT (ISA? *HTTP-SERVER-IMPLEMENTATION* SGT-HTTP-SERVER-IMPL-ASERVE-HTTP-SERVER-ASERVE)))
     (CL:ERROR (NEW-STELLA-EXCEPTION "Conflicting HTTP server implementation already loaded"))
