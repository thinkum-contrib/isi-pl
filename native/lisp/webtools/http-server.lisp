...
(CL:DECLAIM (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* EOL NULL-INTEGER NIL STANDARD-OUTPUT *STELLA-EXIT-HOOKS*))

;;; (DEFCLASS HTTP-SERVER ...)

(CL:DEFCLASS HTTP-SERVER (STANDARD-OBJECT)
  ()
  (:DOCUMENTATION "Abstract class that will be implemented by specific server implementations
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-DEFAULT-HTTP-SERVER-IMPL))
  ...
  (CL:IF (CL:or (CL:member :allegro CL:*features*)
                               ...
                               (CL:let ((loader-function-symbol (CL:find-symbol "LOAD-ASERVE"
                                   ...
                                   (CL:and loader-function-symbol
                                           (CL:fboundp loader-function-symbol)))) "webtools-aserve"
   (CL:WHEN ERROR? (CL:ERROR (NEW-STELLA-EXCEPTION "HTTP server not supported in this Lisp")))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:FIXNUM) CL:SIMPLE-STRING) %START-HTTP-SERVER))
...
address of the started server instance."
  ...
  (CL:LET* ((TEMP-000 (START-HTTP-SERVER-IMPL *HTTP-SERVER-IMPLEMENTATION* PORT))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-000))
   (CL:LET* ((ADDRESS (CL:IF (CL:NOT (CL:EQ TEMP-000 STELLA::NULL-STRING)) TEMP-000 (CONCATENATE "http://localhost:" (INTEGER-TO-STRING (CL:TRUNCATE PORT)) "/"))))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING ADDRESS)) (ADD-HOOK *STELLA-EXIT-HOOKS* SYM-HTTP-SERVER-HTTP-HTTP-SERVER-EXIT-HOOK) (PUBLISH-REGISTERED-HTTP-HANDLERS) ADDRESS)))

(CL:DEFUN START-HTTP-SERVER-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:LET* ((RESULT (%START-HTTP-SERVER (%WRAPPER-VALUE (%%VALUE ARGUMENTS))))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING RESULT))
...
server instance running, this is a no-op."
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL) (AUTOLOAD "HTTP-SERVER-IMPL-SYSTEM" (GET-DEFAULT-HTTP-SERVER-IMPL CL:T) NULL CL:NIL)
   (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL) (CL:ERROR (NEW-STELLA-EXCEPTION "stop-http-server: no HTTP server implementation loaded"))))
...
(CL:DEFMETHOD STOP-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER))
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "stop-http-server-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
This is a no-op if no server is running or if the server is not supported."
  ...
  (CL:WHEN (CL:NOT (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL))
   (CL:HANDLER-CASE (STOP-HTTP-SERVER) (CL:CONDITION (E) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "ERROR: http-server-exit-hook: " (EXCEPTION-MESSAGE E))))))

;;; (DEFCLASS HTTP-EXCHANGE ...)

(CL:DEFCLASS HTTP-EXCHANGE (STANDARD-OBJECT)
  ()
  (:DOCUMENTATION "Abstract class that represents exchange objects that encapsulate all necessary
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:SIMPLE-STRING) GET-HEADER-VALUE))
...
(CL:DEFMETHOD GET-HEADER-VALUE-IMPL ((SERVER HTTP-SERVER) XCHG KEY)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-header-value-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-URI-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))

;;; (DEFUN (GET-REQUEST-URI-QUERY STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-REQUEST-URI-QUERY))
...
(CL:DEFMETHOD GET-REQUEST-URI-QUERY-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-query-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))

;;; (DEFUN (GET-REQUEST-URI-PATH STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-REQUEST-URI-PATH))
...
(CL:DEFMETHOD GET-REQUEST-URI-PATH-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-path-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-METHOD-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-method-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-BODY-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-body-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))

;;; (DEFUN (GET-REQUEST-LOCAL-ADDRESS STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-REQUEST-LOCAL-ADDRESS))
...
(CL:DEFMETHOD GET-REQUEST-LOCAL-ADDRESS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-local-address-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))

;;; (DEFUN (GET-REQUEST-REMOTE-ADDRESS STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-REQUEST-REMOTE-ADDRESS))
...
(CL:DEFMETHOD GET-REQUEST-REMOTE-ADDRESS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-remote-address-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REPLY-STREAM-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-reply-stream-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD SET-RESPONSE-CODE-IMPL ((SERVER HTTP-SERVER) XCHG CODE)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "set-response-code-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:FIXNUM) GET-HTTP-RESPONSE-CODE))
(CL:DEFUN GET-HTTP-RESPONSE-CODE (CODENAME DEFAULT)
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-RESPONSE-CODES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) CODENAME) (CL:RETURN-FROM GET-HTTP-RESPONSE-CODE (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY))))) (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-response-code: undefined response code: `" CODENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
   (GET-HTTP-RESPONSE-CODE DEFAULT NULL)))

;;; (DEFUN (GET-HTTP-RESPONSE-DESC STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:SIMPLE-STRING) GET-HTTP-RESPONSE-DESC))
(CL:DEFUN GET-HTTP-RESPONSE-DESC (CODENAME DEFAULT)
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-RESPONSE-CODES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) CODENAME) (CL:RETURN-FROM GET-HTTP-RESPONSE-DESC (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ENTRY))))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-response-desc: undefined response code: `" CODENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:SIMPLE-STRING) GET-HTTP-MIME-TYPE))
(CL:DEFUN GET-HTTP-MIME-TYPE (TYPENAME DEFAULT)
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-MIME-TYPES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) TYPENAME) (CL:RETURN-FROM GET-HTTP-MIME-TYPE (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY))))) (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-mime-type: undefined mime type: `" TYPENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:LET* ((FOUND?-000 CL:NIL))
     (CL:LET* ((EXT NULL) (ITER-001 (%%REST (%%REST ENTRY))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ EXT (%%VALUE ITER-001)) (CL:WHEN (EQL-TO-STRING? EXT EXTENSION) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
       (CL:SETQ ITER-001 (%%REST ITER-001))))
     (CL:WHEN FOUND?-000 (CL:RETURN-FROM GET-HTTP-MIME-TYPE-FROM-EXTENSION (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY))))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
...
`separator' and return them as a list of properly decoded strings."
  ...
  (CL:LET* ((RESULT (SPLIT-STRING ARGUMENTS SEPARATOR)) (CURSOR RESULT))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (FIRST-SETTER CURSOR (WRAP-STRING (UNESCAPE-URL-STRING (%WRAPPER-VALUE (%%VALUE CURSOR)))))
...
last input will be recorded in the resulting key-value-list."
  ...
  (CL:LET* ((RESULT (NEW-KEY-VALUE-LIST)) (BAREENTRY STELLA::NULL-STRING) (SPLIT 0)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BAREENTRY) (CL:TYPE CL:FIXNUM SPLIT))
   ...
   (CL:LET* ((ENTRY NULL) (ITER-000 (SPLIT-STRING VALUES #\&)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000)) (CL:SETQ BAREENTRY (UNWRAP-STRING ENTRY))
     (CL:SETQ SPLIT (POSITION BAREENTRY #\= 0))
     (CL:WHEN (CL:NOT (CL:= SPLIT NULL-INTEGER))
      (INSERT-AT RESULT (WRAP-STRING (UNESCAPE-URL-STRING (SUBSEQUENCE BAREENTRY 0 SPLIT)))
       (WRAP-STRING (UNESCAPE-URL-STRING (SUBSEQUENCE BAREENTRY (CL:1+ SPLIT) NULL-INTEGER)))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   RESULT))
...
standard `lookup' calls will return the value of the first key only."
  ...
  (CL:LET* ((RESULT NIL) (BAREENTRY STELLA::NULL-STRING) (SPLIT 0)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BAREENTRY) (CL:TYPE CL:FIXNUM SPLIT))
   ...
   (CL:LET* ((ENTRY NULL) (ITER-000 (SPLIT-STRING VALUES #\&)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000)) (CL:SETQ BAREENTRY (UNWRAP-STRING ENTRY))
     (CL:SETQ SPLIT (POSITION BAREENTRY #\= 0))
     (CL:WHEN (CL:NOT (CL:= SPLIT NULL-INTEGER)) (CL:SETQ RESULT (CONS (WRAP-STRING (UNESCAPE-URL-STRING (SUBSEQUENCE BAREENTRY 0 SPLIT))) RESULT))
      (CL:SETQ RESULT (CONS (WRAP-STRING (UNESCAPE-URL-STRING (SUBSEQUENCE BAREENTRY (CL:1+ SPLIT) NULL-INTEGER))) RESULT)))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST))) (CL:SETF (%THE-PLIST SELF-000) (REVERSE RESULT)) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION () CL:SIMPLE-STRING) GET-HTTP-DOCUMENT-ROOT))
...
handler whose path is identical to `path' or is the longest prefix of `path'."
  ...
  (CL:LET* ((HANDLER (LOOKUP *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH))) (HANDLERPATH STELLA::NULL-STRING) (HANDLERPATHLENGTH 0))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING HANDLERPATH) (CL:TYPE CL:FIXNUM HANDLERPATHLENGTH))
   (CL:WHEN (CL:NOT (CL:EQ HANDLER NULL)) (CL:RETURN-FROM LOOKUP-HANDLER HANDLER))
   (CL:LET* ((P NULL) (H NULL) (ITER-000 (ALLOCATE-ITERATOR *HTTP-HANDLER-REGISTRY*)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ P (%KEY ITER-000)) (CL:SETQ H (%VALUE ITER-000))
     (CL:WHEN (STARTS-WITH? PATH (%WRAPPER-VALUE P) 0)
      (CL:WHEN (CL:OR (CL:EQ HANDLER NULL) (CL:> (CL:THE CL:FIXNUM (CL:LENGTH (CL:THE CL:SIMPLE-STRING (UNWRAP-STRING P)))) HANDLERPATHLENGTH)) (CL:SETQ HANDLER H)
       (CL:SETQ HANDLERPATH (%WRAPPER-VALUE P)) (CL:SETQ HANDLERPATHLENGTH (CL:THE CL:FIXNUM (CL:LENGTH HANDLERPATH)))))))
   HANDLER))
...
This is run every time a server is started with `start-http-server'."
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL) (CL:ERROR (NEW-STELLA-EXCEPTION "publish-registered-http-handlers: no HTTP server implementation loaded")))
  (CL:LET* ((PATH NULL) (OPTIONS NULL) (ITER-000 (ALLOCATE-ITERATOR *HTTP-HANDLER-REGISTRY*)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PATH (%KEY ITER-000)) (CL:SETQ OPTIONS (%VALUE ITER-000))
    (CL:LET* ((TEST-VALUE-000 (LOOKUP OPTIONS KWD-HTTP-SERVER-HANDLER-TYPE)))
     (CL:COND
      ((CL:EQ TEST-VALUE-000 KWD-HTTP-SERVER-FILE)
       (PUBLISH-FILE-IMPL *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH) (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-FILE)) OPTIONS))
      ((CL:EQ TEST-VALUE-000 KWD-HTTP-SERVER-DIRECTORY)
       (PUBLISH-DIRECTORY-IMPL *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH) (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-DIRECTORY)) OPTIONS))
      ((CL:EQ TEST-VALUE-000 KWD-HTTP-SERVER-HANDLER)
       (PUBLISH-HANDLER-IMPL *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH) (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-HANDLER)) OPTIONS))
      (CL:T
...
(CL:DEFUN PRINT-REGISTERED-HTTP-HANDLERS ()
  (CL:LET* ((PATHS NIL))
   (CL:LET* ((PATH NULL) (OPTIONS NULL) (ITER-000 (ALLOCATE-ITERATOR *HTTP-HANDLER-REGISTRY*)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PATH (%KEY ITER-000)) (CL:SETQ OPTIONS (%VALUE ITER-000)) (CL:SETQ OPTIONS OPTIONS) (CL:SETQ PATHS (CONS PATH PATHS))))
   (CL:SETQ PATHS (SORT PATHS NULL))
   (CL:LET* ((*PRINTPRETTY?* CL:NIL) (*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTPRETTY?* *PRINTREADABLY?*))
    (CL:LET* ((PATH NULL) (ITER-001 PATHS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ PATH (%%VALUE ITER-001)) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (UNWRAP-STRING PATH) " ->" EOL)
      (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-002 (%THE-PLIST (LOOKUP-HANDLER *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH)))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-002)) (CL:SETQ VALUE (%%VALUE (%%REST ITER-002)))
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "      " KEY " " VALUE EOL) (CL:SETQ ITER-002 (%%REST (%%REST ITER-002)))))
      (CL:SETQ ITER-001 (%%REST ITER-001)))))))
...
type is guessed from `file's extension."
  ...
  (CL:LET* ((ARGLIST-000 NIL))
   ...
   (CL:LET* ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:CONTENT-TYPE :DOCUMENTATION) \"/HTTP\")" "/HTTP"))))
    (CL:WHEN (CL:EQ (LOOKUP THEOPTIONS KWD-HTTP-SERVER-CONTENT-TYPE) NULL)
     (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-CONTENT-TYPE (WRAP-STRING (GUESS-FILE-CONTENT-TYPE FILE))))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER-TYPE KWD-HTTP-SERVER-FILE) (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-PATH (WRAP-STRING PATH))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-FILE (WRAP-STRING FILE)) (INSERT-AT *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH) THEOPTIONS))))
...
and does not (yet) support directory listings."
  ...
  (CL:WHEN (CL:NOT (PROBE-FILE? DIRECTORY)) (CL:WARN "HTTP/publish-directory: `~A' does not exist" DIRECTORY))
  (CL:WHEN (CL:NOT (ENDS-WITH? PATH "/" NULL-INTEGER)) (CL:SETQ PATH (CONCATENATE PATH "/")))
  (CL:LET* ((ARGLIST-000 NIL))
   (CL:LET* ((ARG-000 NULL) (ITER-000 OPTIONS) (COLLECT-000 NULL))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG-000 (CL:POP ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS ARG-000 NIL))
       (CL:IF (CL:EQ ARGLIST-000 NIL) (CL:SETQ ARGLIST-000 COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST ARGLIST-000 COLLECT-000)))
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS ARG-000 NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET* ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:DOCUMENTATION) \"/HTTP\")" "/HTTP"))))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER-TYPE KWD-HTTP-SERVER-DIRECTORY) (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-PATH (WRAP-STRING PATH))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-DIRECTORY (WRAP-STRING DIRECTORY)) (INSERT-AT *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH) THEOPTIONS))))
...
the content type of the resulting page, otherwise, text/html will be used by default."
  ...
  (CL:LET* ((ARGLIST-000 NIL))
   ...
   (CL:LET* ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:CONTENT-TYPE :DOCUMENTATION) \"/HTTP\")" "/HTTP"))))
    (CL:WHEN (CL:EQ (LOOKUP THEOPTIONS KWD-HTTP-SERVER-CONTENT-TYPE) NULL)
     (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL))))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER-TYPE KWD-HTTP-SERVER-HANDLER) (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-PATH (WRAP-STRING PATH))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER (WRAP-FUNCTION-CODE HANDLER)) (INSERT-AT *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH) THEOPTIONS))))
...
(CL:DEFMETHOD GET-HANDLER-OPTIONS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-handler-options-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFUN GENERATE-ERROR-RESPONSE (XCHG CODE MESSAGE)
  ...
  (CL:LET* ((STREAM (GET-REPLY-STREAM XCHG))) (SET-RESPONSE-CODE XCHG CODE)
   ...
   (CL:PROGN (%%PRINT-STREAM STREAM "<BODY BGCOLOR='FF9999'>")
    (%%PRINT-STREAM STREAM "ERROR: " (GET-HTTP-RESPONSE-CODE CODE NULL) " " (GET-HTTP-RESPONSE-DESC CODE NULL))
    (CL:WHEN (CL:NOT (CL:EQ MESSAGE STELLA::NULL-STRING)) (%%PRINT-STREAM STREAM "<BR><BR>" MESSAGE EOL)) (%%PRINT-STREAM STREAM "</BODY>" EOL))))
...
Simple default implementation for minimal servers such as the Sun JRE HttpServer."
  (CL:LET*
   ((URIPATH (GET-REQUEST-URI-PATH XCHG)) (OPTIONS (GET-HANDLER-OPTIONS XCHG)) (PATH (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-PATH)))
    (FILE (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-FILE)))
    (CONTENTTYPE (LOOKUP-WITH-DEFAULT OPTIONS KWD-HTTP-SERVER-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL))))
    (STREAM (GET-REPLY-STREAM XCHG)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING URIPATH PATH FILE)) (CL:SETQ URIPATH (STRING-TRIM (UNESCAPE-URL-STRING URIPATH)))
   (CL:COND ((CL:NOT (STRING-EQL? URIPATH PATH)) (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-NOT-FOUND STELLA::NULL-STRING))
    (CL:T (SET-REPLY-HEADER-VALUE XCHG KWD-HTTP-SERVER-CONTENT-TYPE (%WRAPPER-VALUE CONTENTTYPE))
     (CL:LET* ((IN NULL))
      (CL:UNWIND-PROTECT
       (CL:PROGN (CL:SETQ IN (OPEN-INPUT-FILE FILE))
        (CL:LET* ((SELF-000 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-000) STREAM) (COPY-STREAM-TO-STREAM IN SELF-000)))
...
servers such as the Sun JRE HttpServer."
  (CL:LET*
   ((URIPATH (GET-REQUEST-URI-PATH XCHG)) (OPTIONS (GET-HANDLER-OPTIONS XCHG)) (PATH (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-PATH)))
    (DIRECTORY (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-DIRECTORY))) (RESOLVEDURI STELLA::NULL-STRING) (ISDIRECTORY? CL:NIL)
    (CONTENTTYPE (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-BINARY NULL)) (STREAM (GET-REPLY-STREAM XCHG)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING URIPATH PATH DIRECTORY RESOLVEDURI CONTENTTYPE)) (CL:SETQ URIPATH (STRING-TRIM (UNESCAPE-URL-STRING URIPATH)))
   (CL:SETQ ISDIRECTORY? (ENDS-WITH? URIPATH "/" NULL-INTEGER))
   (CL:WHEN (CL:NOT (STARTS-WITH? URIPATH PATH NULL-INTEGER)) (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-NOT-FOUND STELLA::NULL-STRING)
    (CL:RETURN-FROM DIRECTORY-HANDLER))
   (CL:SETQ URIPATH (SUBSEQUENCE URIPATH (CL:THE CL:FIXNUM (CL:LENGTH PATH)) NULL-INTEGER))
   (CL:WHEN
    (CL:OR (STARTS-WITH? URIPATH "../" NULL-INTEGER) (STARTS-WITH? URIPATH "/.." NULL-INTEGER) (CL:NOT (CL:= (STRING-SEARCH URIPATH "/../" NULL-INTEGER) NULL-INTEGER)))
    (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-FORBIDDEN STELLA::NULL-STRING) (CL:RETURN-FROM DIRECTORY-HANDLER))
   (CL:SETQ RESOLVEDURI (SUBSTITUTE URIPATH (DIRECTORY-SEPARATOR) #\/)) (CL:SETQ RESOLVEDURI (CONCATENATE DIRECTORY RESOLVEDURI))
   (CL:WHEN ISDIRECTORY?
    (CL:COND ((PROBE-FILE? (CONCATENATE RESOLVEDURI "index.html")) (CL:SETQ RESOLVEDURI (CONCATENATE RESOLVEDURI "index.html")))
     ((PROBE-FILE? (CONCATENATE RESOLVEDURI "index.htm")) (CL:SETQ RESOLVEDURI (CONCATENATE RESOLVEDURI "index.htm")))
     (CL:T (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-FORBIDDEN STELLA::NULL-STRING) (CL:RETURN-FROM DIRECTORY-HANDLER))))
   (CL:WHEN (CL:NOT (PROBE-FILE? RESOLVEDURI)) (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-NOT-FOUND STELLA::NULL-STRING) (CL:RETURN-FROM DIRECTORY-HANDLER))
   (CL:SETQ CONTENTTYPE (GUESS-FILE-CONTENT-TYPE RESOLVEDURI)) (SET-REPLY-HEADER-VALUE XCHG KWD-HTTP-SERVER-CONTENT-TYPE CONTENTTYPE)
   (CL:LET* ((IN NULL))
    (CL:UNWIND-PROTECT
...
(CL:DEFUN LOAD-SYSTEM-HANDLER (XCHG)
  ...
  (CL:LET*
   ((ARGUMENTS (PARSE-AND-DECODE-URL-ARGUMENTS (GET-REQUEST-URI-QUERY XCHG) #\+)) (SYSTEMNAME (%%VALUE ARGUMENTS))
    (LOADCOMMAND (LIST* SYM-HTTP-SERVER-STELLA-LOAD-SYSTEM SYSTEMNAME NIL)) (STREAM (GET-REPLY-STREAM XCHG)))
   (CL:LET* ((ARG NULL) (ITER-000 (%%REST ARGUMENTS)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ARG (%%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE ARG)) NIL))
       (CL:IF (CL:EQ LOADCOMMAND NIL) (CL:SETQ LOADCOMMAND COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST LOADCOMMAND COLLECT-000)))
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE ARG)) NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:HANDLER-CASE
    (CL:PROGN (%EVALUATE LOADCOMMAND)
     (CL:PROGN (%%PRINT-STREAM STREAM "<BODY>") (%%PRINT-STREAM STREAM "Successfully evaluated: " (STRINGIFY LOADCOMMAND)) (%%PRINT-STREAM STREAM "<BR><BR>" EOL)
      (%%PRINT-STREAM STREAM "<HR SIZE=2>" EOL) (%%PRINT-STREAM STREAM *HTTP-SERVER-COPYRIGHT-TRAILER*) (%%PRINT-STREAM STREAM "</BODY>" EOL)))
    (CL:CONDITION (E) (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-BAD-REQUEST (EXCEPTION-MESSAGE E))))))
...
(CL:DEFUN TEST-HTTP-API-HANDLER (XCHG)
  ...
  (CL:LET*
   ((URI (GET-REQUEST-URI XCHG)) (PATH (GET-REQUEST-URI-PATH XCHG)) (QUERY (GET-REQUEST-URI-QUERY XCHG)) (ARGS (PARSE-AND-DECODE-URL-ARGUMENTS QUERY #\+))
    (METHOD (GET-REQUEST-METHOD XCHG)) (LOCALHOST (GET-REQUEST-LOCAL-ADDRESS XCHG)) (REMOTEHOST (GET-REQUEST-REMOTE-ADDRESS XCHG)) (BODY (GET-REQUEST-BODY XCHG))
    (STREAM (GET-REPLY-STREAM XCHG)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING URI PATH QUERY LOCALHOST REMOTEHOST BODY))
   (CL:COND
    ((CL:EQ METHOD KWD-HTTP-SERVER-GET)
     (CL:WHEN (MEMBER? ARGS (WRAP-STRING "plain")) (SET-REPLY-HEADER-VALUE XCHG KWD-HTTP-SERVER-CONTENT-TYPE (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-PLAIN-TEXT NULL)))
     (%%PRINT-STREAM STREAM "<PRE>" EOL) (%%PRINT-STREAM STREAM "It worked, here are some parameters: " EOL) (%%PRINT-STREAM STREAM "URI=`" URI "'" EOL)
     (%%PRINT-STREAM STREAM "Path=`" PATH "'" EOL) (%%PRINT-STREAM STREAM "Query=`" QUERY "'" EOL) (%%PRINT-STREAM STREAM "Args=`" ARGS "'" EOL)
     (%%PRINT-STREAM STREAM "Method=`" METHOD "'" EOL) (%%PRINT-STREAM STREAM "Local host=`" LOCALHOST "'" EOL)
     (%%PRINT-STREAM STREAM "Remote host=`" REMOTEHOST "'" EOL) (%%PRINT-STREAM STREAM "Body=`" BODY "'" EOL) (%%PRINT-STREAM STREAM "</PRE>" EOL)
     (TEST-HTTP-API-HANDLER-SERVE-FORM XCHG))
    ((CL:EQ METHOD KWD-HTTP-SERVER-POST)
     (CL:LET* ((VALUES (PARSE-AND-DECODE-FORM-VALUES BODY))) (%%PRINT-STREAM STREAM "<PRE>" EOL) (%%PRINT-STREAM STREAM "It worked, here are some parameters: " EOL)
      (%%PRINT-STREAM STREAM "URI=`" URI "'" EOL) (%%PRINT-STREAM STREAM "Path=`" PATH "'" EOL) (%%PRINT-STREAM STREAM "Query=`" QUERY "'" EOL)
      (%%PRINT-STREAM STREAM "Args=`" ARGS "'" EOL) (%%PRINT-STREAM STREAM "Method=`" METHOD "'" EOL) (%%PRINT-STREAM STREAM "Local host=`" LOCALHOST "'" EOL)
      (%%PRINT-STREAM STREAM "Remote host=`" REMOTEHOST "'" EOL) (%%PRINT-STREAM STREAM "Body=`" BODY "'" EOL) (%%PRINT-STREAM STREAM "Values=`" VALUES "'" EOL EOL)
      (%%PRINT-STREAM STREAM "</PRE>" EOL)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" METHOD "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))))

(CL:DEFUN HELP-STARTUP-HTTP-SERVER1 ()
  (CL:PROGN (CL:SETQ SYM-HTTP-SERVER-HTTP-HTTP-SERVER-EXIT-HOOK (INTERN-RIGID-SYMBOL-WRT-MODULE "HTTP-SERVER-EXIT-HOOK" NULL 0))
   (CL:SETQ SYM-HTTP-SERVER-HTTP-START-HTTP-SERVER (INTERN-RIGID-SYMBOL-WRT-MODULE "START-HTTP-SERVER" NULL 0))
   (CL:SETQ KWD-HTTP-SERVER-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2)) (CL:SETQ KWD-HTTP-SERVER-OK (INTERN-RIGID-SYMBOL-WRT-MODULE "OK" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-REDIRECT (INTERN-RIGID-SYMBOL-WRT-MODULE "REDIRECT" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-FORBIDDEN (INTERN-RIGID-SYMBOL-WRT-MODULE "FORBIDDEN" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-NOT-FOUND (INTERN-RIGID-SYMBOL-WRT-MODULE "NOT-FOUND" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-BAD-REQUEST (INTERN-RIGID-SYMBOL-WRT-MODULE "BAD-REQUEST" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-INTERNAL-ERROR (INTERN-RIGID-SYMBOL-WRT-MODULE "INTERNAL-ERROR" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-NOT-IMPLEMENTED (INTERN-RIGID-SYMBOL-WRT-MODULE "NOT-IMPLEMENTED" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-PLAIN-TEXT (INTERN-RIGID-SYMBOL-WRT-MODULE "PLAIN-TEXT" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-HTML (INTERN-RIGID-SYMBOL-WRT-MODULE "HTML" NULL 2)) (CL:SETQ KWD-HTTP-SERVER-XML (INTERN-RIGID-SYMBOL-WRT-MODULE "XML" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-BINARY (INTERN-RIGID-SYMBOL-WRT-MODULE "BINARY" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-IMAGE-GIF (INTERN-RIGID-SYMBOL-WRT-MODULE "IMAGE-GIF" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-IMAGE-JPG (INTERN-RIGID-SYMBOL-WRT-MODULE "IMAGE-JPG" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-IMAGE-PNG (INTERN-RIGID-SYMBOL-WRT-MODULE "IMAGE-PNG" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-AUDIO-MPEG (INTERN-RIGID-SYMBOL-WRT-MODULE "AUDIO-MPEG" NULL 2)) (CL:SETQ KWD-HTTP-SERVER-PDF (INTERN-RIGID-SYMBOL-WRT-MODULE "PDF" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-MSWORD (INTERN-RIGID-SYMBOL-WRT-MODULE "MSWORD" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-HANDLER-TYPE (INTERN-RIGID-SYMBOL-WRT-MODULE "HANDLER-TYPE" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-FILE (INTERN-RIGID-SYMBOL-WRT-MODULE "FILE" NULL 2)) (CL:SETQ KWD-HTTP-SERVER-DIRECTORY (INTERN-RIGID-SYMBOL-WRT-MODULE "DIRECTORY" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-HANDLER (INTERN-RIGID-SYMBOL-WRT-MODULE "HANDLER" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-CONTENT-TYPE (INTERN-RIGID-SYMBOL-WRT-MODULE "CONTENT-TYPE" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-PATH (INTERN-RIGID-SYMBOL-WRT-MODULE "PATH" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-DOCUMENTATION (INTERN-RIGID-SYMBOL-WRT-MODULE "DOCUMENTATION" NULL 2))
   (CL:SETQ SYM-HTTP-SERVER-STELLA-LOAD-SYSTEM (INTERN-RIGID-SYMBOL-WRT-MODULE "LOAD-SYSTEM" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   (CL:SETQ KWD-HTTP-SERVER-GET (INTERN-RIGID-SYMBOL-WRT-MODULE "GET" NULL 2)) (CL:SETQ KWD-HTTP-SERVER-POST (INTERN-RIGID-SYMBOL-WRT-MODULE "POST" NULL 2))
   (CL:SETQ SYM-HTTP-SERVER-HTTP-STARTUP-HTTP-SERVER (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-HTTP-SERVER" NULL 0))
   (CL:SETQ SYM-HTTP-SERVER-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0))))
...
where none has been loaded yet.  If `error?' is TRUE, raise an error if none
is defined for the current native language environment.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-DEFAULT-HTTP-SERVER-IMPL) NULL)
...
a server instance running, it will be stopped first.  If `port' is <= 0 or
NULL, use the value of `*default-http-server-port*'.  Returns the listening
address of the started server instance.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %START-HTTP-SERVER) (CL:FUNCTION START-HTTP-SERVER-EVALUATOR-WRAPPER))
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (START-HTTP-SERVER-IMPL STRING) ((SERVER HTTP-SERVER) (PORT INTEGER)))" (CL:FUNCTION START-HTTP-SERVER-IMPL) NULL)
   (DEFINE-FUNCTION-OBJECT "STOP-HTTP-SERVER" "(DEFUN STOP-HTTP-SERVER () :DOCUMENTATION \"Stop any currently running HTTP server.  If there is no
server instance running, this is a no-op.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION STOP-HTTP-SERVER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD STOP-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER)))" (CL:FUNCTION STOP-HTTP-SERVER-IMPL) NULL)
...
This is a no-op if no server is running or if the server is not supported.\" :PUBLIC? TRUE)" (CL:FUNCTION HTTP-SERVER-EXIT-HOOK) NULL)
...
headers represented as a string.  If no value is found, NULL will be returned.
If there are multiple values, an arbitrary one will be returned.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-HEADER-VALUE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-HEADER-VALUE-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE) (KEY KEYWORD)))" (CL:FUNCTION GET-HEADER-VALUE-IMPL) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD SET-REPLY-HEADER-VALUE-IMPL ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE) (KEY KEYWORD) (VALUE STRING)))"
...
This does not include any protocol/scheme and host/port information.  For example, if
the user typed http://a.b.c:42/foo/bar.html#frag?a+b+c into the client, this function
will return /foo/bar.html#frag?a+b+c .\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-URI-IMPL) NULL)
...
return a+b+c .\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI-QUERY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-QUERY-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-URI-QUERY-IMPL) NULL)
...
that any URI-encoded characters are not yet decoded to facilitate further parsing.  For example,
if the user typed http://a.b.c:42/foo/bar.html#frag?a+b+c into the client, this function will
return /foo/bar.html.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI-PATH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-PATH-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-URI-PATH-IMPL) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-METHOD-IMPL KEYWORD) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-METHOD-IMPL) NULL)
...
if this was a GET), the result will be the empty string.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-BODY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-BODY-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-BODY-IMPL) NULL)
...
Concatenates the hostname with a port in standard : notation.  If the hostname
can be resolved it will be, otherwise a numeric representation will be returned.
Note that different implementations have slightly different behavior.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-LOCAL-ADDRESS) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-LOCAL-ADDRESS-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-LOCAL-ADDRESS-IMPL)
    NULL)
...
Concatenates the hostname with a port in standard : notation.  If the hostname
can be resolved it will be, otherwise a numeric representation will be returned.
Note that different implementations have slightly different behavior.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-REMOTE-ADDRESS) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-REMOTE-ADDRESS-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REQUEST-REMOTE-ADDRESS-IMPL)
    NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REPLY-STREAM-IMPL NATIVE-OUTPUT-STREAM) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-REPLY-STREAM-IMPL) NULL)
...
:OK (code 200) is used.\" :PUBLIC? TRUE)" (CL:FUNCTION SET-RESPONSE-CODE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD SET-RESPONSE-CODE-IMPL ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE) (CODE KEYWORD)))" (CL:FUNCTION SET-RESPONSE-CODE-IMPL) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-RESPONSE-CODE" "(DEFUN (GET-HTTP-RESPONSE-CODE INTEGER) ((CODENAME KEYWORD) (DEFAULT KEYWORD)))"
    (CL:FUNCTION GET-HTTP-RESPONSE-CODE) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-RESPONSE-DESC" "(DEFUN (GET-HTTP-RESPONSE-DESC STRING) ((CODENAME KEYWORD) (DEFAULT KEYWORD)))" (CL:FUNCTION GET-HTTP-RESPONSE-DESC)
    NULL)
   (DEFINE-FUNCTION-OBJECT "HTTP-SUCCESS-RESPONSE-CODE?" "(DEFUN (HTTP-SUCCESS-RESPONSE-CODE? BOOLEAN) ((CODE INTEGER)))" (CL:FUNCTION HTTP-SUCCESS-RESPONSE-CODE?) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-MIME-TYPE" "(DEFUN (GET-HTTP-MIME-TYPE STRING) ((TYPENAME KEYWORD) (DEFAULT KEYWORD)))" (CL:FUNCTION GET-HTTP-MIME-TYPE) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-MIME-TYPE-FROM-EXTENSION" "(DEFUN (GET-HTTP-MIME-TYPE-FROM-EXTENSION STRING) ((EXTENSION STRING)))"
    (CL:FUNCTION GET-HTTP-MIME-TYPE-FROM-EXTENSION) NULL)
   (DEFINE-FUNCTION-OBJECT "GUESS-FILE-CONTENT-TYPE" "(DEFUN (GUESS-FILE-CONTENT-TYPE STRING) ((FILE STRING)))" (CL:FUNCTION GUESS-FILE-CONTENT-TYPE) NULL)
...
`separator' and return them as a list of properly decoded strings.\" :PUBLIC? TRUE)" (CL:FUNCTION PARSE-AND-DECODE-URL-ARGUMENTS) NULL)
...
a key-value-list.  NOTE: values will not be trimmed and empty values will be represented by the empty
string rather than NULL.  Also, if the same input name occurs more than once, only the value of the
last input will be recorded in the resulting key-value-list.\" :PUBLIC? TRUE)" (CL:FUNCTION PARSE-AND-DECODE-FORM-VALUES) NULL)
...
string rather than NULL.  Keys will be inserted into the result in the order they appear in the form.
If they are non-unique, multiple entries per key will result which can be iterated over, however,
standard `lookup' calls will return the value of the first key only.\" :PUBLIC? TRUE)" (CL:FUNCTION PARSE-AND-DECODE-NON-UNIQUE-FORM-VALUES) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-DOCUMENT-ROOT" "(DEFUN (GET-HTTP-DOCUMENT-ROOT STRING) () :PUBLIC? TRUE)" (CL:FUNCTION GET-HTTP-DOCUMENT-ROOT) NULL)
...
called before any handlers are published, otherwise it won't be effective.\" :PUBLIC? TRUE)" (CL:FUNCTION SET-HTTP-DOCUMENT-ROOT) NULL)
...
handler whose path is identical to `path' or is the longest prefix of `path'.\" :PUBLIC? TRUE)" (CL:FUNCTION LOOKUP-HANDLER) NULL)
...
This is run every time a server is started with `start-http-server'.\" :PUBLIC? TRUE)" (CL:FUNCTION PUBLISH-REGISTERED-HTTP-HANDLERS) NULL)
   (DEFINE-FUNCTION-OBJECT "PRINT-REGISTERED-HTTP-HANDLERS" "(DEFUN PRINT-REGISTERED-HTTP-HANDLERS () :PUBLIC? TRUE :COMMAND? TRUE)"
...
If :content-type is supplied, it will be used when serving `file', otherwise, a content
type is guessed from `file's extension.\" :PUBLIC? TRUE)" (CL:FUNCTION PUBLISH-FILE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD PUBLISH-FILE-IMPL ((SERVER HTTP-SERVER) (PATH STRING) (FILE STRING) (OPTIONS PROPERTY-LIST)))" (CL:FUNCTION PUBLISH-FILE-IMPL) NULL)
...
This is very rudimentary right now and does not provide any sophisticated access
control.  It recurses into subdirectories of `directory', prevents escape via `..'
and does not (yet) support directory listings.\" :PUBLIC? TRUE)" (CL:FUNCTION PUBLISH-DIRECTORY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD PUBLISH-DIRECTORY-IMPL ((SERVER HTTP-SERVER) (PATH STRING) (DIRECTORY STRING) (OPTIONS PROPERTY-LIST)))"
...
take a single argument of type HTTP-EXCHANGE and use the API functions to read request
information and generate a result page.  If :content-type is supplied, it will be used as
the content type of the resulting page, otherwise, text/html will be used by default.\" :PUBLIC? TRUE)" (CL:FUNCTION PUBLISH-HANDLER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD PUBLISH-HANDLER-IMPL ((SERVER HTTP-SERVER) (PATH STRING) (HANDLER FUNCTION-CODE) (OPTIONS PROPERTY-LIST)))"
...
This includes required as well as optional arguments.  This will never return NULL.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-HANDLER-OPTIONS) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-HANDLER-OPTIONS-IMPL PROPERTY-LIST) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))" (CL:FUNCTION GET-HANDLER-OPTIONS-IMPL) NULL)
   (DEFINE-FUNCTION-OBJECT "GENERATE-ERROR-RESPONSE" "(DEFUN GENERATE-ERROR-RESPONSE ((XCHG HTTP-EXCHANGE) (CODE KEYWORD) (MESSAGE STRING)))"
...
Simple default implementation for minimal servers such as the Sun JRE HttpServer.\" :PUBLIC? TRUE)" (CL:FUNCTION FILE-HANDLER) NULL)
...
handler was published (including subdirectories).  Simple default implementation for minimal
servers such as the Sun JRE HttpServer.\" :PUBLIC? TRUE)" (CL:FUNCTION DIRECTORY-HANDLER) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "TEST-HTTP-API-HANDLER-SERVE-FORM" "(DEFUN TEST-HTTP-API-HANDLER-SERVE-FORM ((XCHG HTTP-EXCHANGE)))"
    (CL:FUNCTION TEST-HTTP-API-HANDLER-SERVE-FORM) NULL)
   (DEFINE-FUNCTION-OBJECT "TEST-HTTP-API-HANDLER" "(DEFUN TEST-HTTP-API-HANDLER ((XCHG HTTP-EXCHANGE)))" (CL:FUNCTION TEST-HTTP-API-HANDLER) NULL)
   (DEFINE-FUNCTION-OBJECT "STARTUP-HTTP-SERVER" "(DEFUN STARTUP-HTTP-SERVER () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-HTTP-SERVER) NULL)
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-HTTP-SERVER-HTTP-STARTUP-HTTP-SERVER)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-HTTP-SERVER-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupHttpServer") NULL-STRING-WRAPPER))))
  ...
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *HTTP-RESPONSE-CODES*
     (LIST* (LIST* KWD-HTTP-SERVER-OK (WRAP-INTEGER 200) (WRAP-STRING "OK") NIL)
      (LIST* KWD-HTTP-SERVER-REDIRECT (WRAP-INTEGER 301) (WRAP-STRING "Moved Permanently") NIL)
      (LIST* KWD-HTTP-SERVER-FORBIDDEN (WRAP-INTEGER 403) (WRAP-STRING "Forbidden") NIL)
      (LIST* KWD-HTTP-SERVER-NOT-FOUND (WRAP-INTEGER 404) (WRAP-STRING "Not Found") NIL)
      (LIST* KWD-HTTP-SERVER-BAD-REQUEST (WRAP-INTEGER 400) (WRAP-STRING "Bad Request") NIL)
      (LIST* KWD-HTTP-SERVER-INTERNAL-ERROR (WRAP-INTEGER 500) (WRAP-STRING "Internal Server Error") NIL)
      (LIST* KWD-HTTP-SERVER-NOT-IMPLEMENTED (WRAP-INTEGER 501) (WRAP-STRING "Not Implemented") NIL) NIL))
    (CL:SETQ *HTTP-MIME-TYPES*
     (LIST* (LIST* KWD-HTTP-SERVER-PLAIN-TEXT (WRAP-STRING "text/plain") (WRAP-STRING "txt") (WRAP-STRING "text") (WRAP-STRING "asc") NIL)
      (LIST* KWD-HTTP-SERVER-HTML (WRAP-STRING "text/html") (WRAP-STRING "htm") (WRAP-STRING "html") NIL)
      (LIST* KWD-HTTP-SERVER-XML (WRAP-STRING "text/xml") (WRAP-STRING "xml") (WRAP-STRING "rdf") (WRAP-STRING "owl") NIL)
      (LIST* KWD-HTTP-SERVER-BINARY (WRAP-STRING "application/octet-stream") (WRAP-STRING "zip") (WRAP-STRING "exe") (WRAP-STRING "class") NIL)
      (LIST* KWD-HTTP-SERVER-IMAGE-GIF (WRAP-STRING "image/gif") (WRAP-STRING "gif") NIL)
      (LIST* KWD-HTTP-SERVER-IMAGE-JPG (WRAP-STRING "image/jpeg") (WRAP-STRING "jpg") (WRAP-STRING "jpeg") NIL)
      (LIST* KWD-HTTP-SERVER-IMAGE-PNG (WRAP-STRING "image/png") (WRAP-STRING "png") NIL)
      (LIST* KWD-HTTP-SERVER-AUDIO-MPEG (WRAP-STRING "audio/mpeg") (WRAP-STRING "mp3") NIL)
      (LIST* KWD-HTTP-SERVER-PDF (WRAP-STRING "application/pdf") (WRAP-STRING "pdf") NIL)
      (LIST* KWD-HTTP-SERVER-MSWORD (WRAP-STRING "application/msword") (WRAP-STRING "doc") NIL) NIL))
    (CL:SETQ *HTTP-DOCUMENT-ROOT*
     (CL:IF (CL:NOT (CL:EQ *HTTP-DOCUMENT-ROOT* STELLA::NULL-STRING)) *HTTP-DOCUMENT-ROOT*
      (CL:IF (PROBE-FILE? "PL:htdocs;powerloom.html") (TRANSLATE-LOGICAL-PATHNAME "PL:htdocs;") (TRANSLATE-LOGICAL-PATHNAME "PL:"))))
    (CL:SETQ *HTTP-HANDLER-REGISTRY* (NEW-KEY-VALUE-MAP))
    (CL:SETQ *HTTP-SERVER-COPYRIGHT-TRAILER*
     (CONCATENATE *STELLA-VERSION-STRING* " HTTP Server" "<BR>" "Copyright 1996-"
      (INTEGER-TO-STRING (CL:TRUNCATE (GET-CALENDAR-DATE (MAKE-CURRENT-DATE-TIME) (GET-LOCAL-TIME-ZONE))))
      " University of Southern California Information Sciences Institute")))
...
implementation in com.sun.net.httpserver.\" :ABSTRACT? TRUE :PUBLIC? TRUE)"))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES)) (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-HTTP-SERVER2))
...
with an actual native server object.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *DEFAULT-HTTP-SERVER-PORT* INTEGER 9090)")
    (REGISTER-NATIVE-NAME SYM-HTTP-SERVER-HTTP-START-HTTP-SERVER KWD-HTTP-SERVER-COMMON-LISP KWD-HTTP-SERVER-FUNCTION)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *HTTP-HANDLER-REGISTRY* (KEY-VALUE-MAP OF STRING-WRAPPER PROPERTY-LIST) (NEW KEY-VALUE-MAP))")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
    ...
    (PUBLISH-DIRECTORY "/ploom/" (GET-HTTP-DOCUMENT-ROOT) KWD-HTTP-SERVER-DOCUMENTATION (WRAP-STRING "Top-level htdocs directory."))
    (PUBLISH-HANDLER "/ploom/load-system" (CL:FUNCTION LOAD-SYSTEM-HANDLER) KWD-HTTP-SERVER-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL))
     KWD-HTTP-SERVER-DOCUMENTATION (WRAP-STRING "Triggers a load-system operation at the server for the argument system."))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *TEST-HTTP-API-HANDLER-LAST-XCHG* HTTP-EXCHANGE NULL)")
    (PUBLISH-HANDLER "/ploom/test-http-api" (CL:FUNCTION TEST-HTTP-API-HANDLER) KWD-HTTP-SERVER-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL))
     KWD-HTTP-SERVER-DOCUMENTATION (WRAP-STRING "Useful for comparing different HTTP API implementations.")))))
