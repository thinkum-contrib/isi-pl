...
address of the started server instance."
  ...
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
   (AUTOLOAD "HTTP-SERVER-IMPL-SYSTEM" (GET-DEFAULT-HTTP-SERVER-IMPL CL:T) NULL CL:NIL)
   (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
    (CL:ERROR (NEW-STELLA-EXCEPTION "start-http-server: no HTTP server implementation loaded"))))
  (CL:WHEN (CL:<= PORT 0) (CL:SETQ PORT *DEFAULT-HTTP-SERVER-PORT*))
  (CL:LET* ((TEMP-000 (START-HTTP-SERVER-IMPL *HTTP-SERVER-IMPLEMENTATION* PORT)))
   ...
   (CL:LET*
    ...
    (ADD-HOOK *STELLA-EXIT-HOOKS* SYM-HTTP-SERVER-HTTP-HTTP-SERVER-EXIT-HOOK) (PUBLISH-REGISTERED-HTTP-HANDLERS)
    ADDRESS)))

(CL:DEFUN START-HTTP-SERVER-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:LET* ((RESULT (%START-HTTP-SERVER (%WRAPPER-VALUE (%%VALUE ARGUMENTS)))))
...
(CL:DEFMETHOD START-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER) PORT)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "start-http-server-impl: not implemented on `" SERVER "'")
...
server instance running, this is a no-op."
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
   (AUTOLOAD "HTTP-SERVER-IMPL-SYSTEM" (GET-DEFAULT-HTTP-SERVER-IMPL CL:T) NULL CL:NIL)
   (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
    (CL:ERROR (NEW-STELLA-EXCEPTION "stop-http-server: no HTTP server implementation loaded"))))
...
(CL:DEFMETHOD STOP-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER))
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "stop-http-server-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
This is a no-op if no server is running or if the server is not supported."
  ...
  (CL:WHEN (CL:NOT (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL))
   (CL:HANDLER-CASE (STOP-HTTP-SERVER)
    (CL:CONDITION (E)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "ERROR: http-server-exit-hook: " (EXCEPTION-MESSAGE E))))))
...
(CL:DEFMETHOD GET-HEADER-VALUE-IMPL ((SERVER HTTP-SERVER) XCHG KEY)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-header-value-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD SET-REPLY-HEADER-VALUE-IMPL ((SERVER HTTP-SERVER) XCHG KEY VALUE)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "set-reply-header-value-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-URI-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-URI-QUERY-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-query-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-URI-PATH-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-uri-path-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-METHOD-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-method-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-BODY-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-body-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-LOCAL-ADDRESS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-local-address-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REQUEST-REMOTE-ADDRESS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-request-remote-address-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-REPLY-STREAM-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-reply-stream-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD SET-RESPONSE-CODE-IMPL ((SERVER HTTP-SERVER) XCHG CODE)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "set-response-code-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
  ...
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-RESPONSE-CODES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) CODENAME)
     (CL:RETURN-FROM GET-HTTP-RESPONSE-CODE (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY)))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-response-code: undefined response code: `" CODENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
...
(CL:DEFUN GET-HTTP-RESPONSE-DESC (CODENAME DEFAULT)
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-RESPONSE-CODES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) CODENAME)
     (CL:RETURN-FROM GET-HTTP-RESPONSE-DESC (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ENTRY))))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-response-desc: undefined response code: `" CODENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
  ...
  (CL:LET* ((ENTRY NULL) (ITER-000 *HTTP-MIME-TYPES*))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ (%%VALUE ENTRY) TYPENAME)
     (CL:RETURN-FROM GET-HTTP-MIME-TYPE (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY)))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:IF (CL:EQ DEFAULT NULL)
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-http-mime-type: undefined mime type: `" TYPENAME "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
    ...
    (CL:LET* ((FOUND?-000 CL:NIL))
     (CL:LET* ((EXT NULL) (ITER-001 (%%REST (%%REST ENTRY))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ EXT (%%VALUE ITER-001))
       (CL:WHEN (EQL-TO-STRING? EXT EXTENSION) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
...
`separator' and return them as a list of properly decoded strings."
  ...
  (CL:LET* ((RESULT (SPLIT-STRING ARGUMENTS SEPARATOR)) (CURSOR RESULT))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (FIRST-SETTER CURSOR (WRAP-STRING (UNESCAPE-URL-STRING (%WRAPPER-VALUE (%%VALUE CURSOR)))))
...
string rather than NULL."
  ...
  (CL:LET* ((RESULT (NEW-KEY-VALUE-LIST)) (BAREENTRY STELLA::NULL-STRING) (SPLIT 0))
   ...
   (CL:LET* ((ENTRY NULL) (ITER-000 (SPLIT-STRING VALUES #\&)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000))
     (CL:SETQ BAREENTRY (UNWRAP-STRING ENTRY)) (CL:SETQ SPLIT (POSITION BAREENTRY #\= 0))
...
handler whose path is identical to `path' or is the longest prefix of `path'."
  ...
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
   (CL:ERROR (NEW-STELLA-EXCEPTION "lookup-handler: no HTTP server implementation loaded")))
  (CL:LET*
   ((HANDLER (LOOKUP *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH))) (HANDLERPATH STELLA::NULL-STRING)
    (HANDLERPATHLENGTH 0))
     ...
     (CL:WHEN (STARTS-WITH? PATH (%WRAPPER-VALUE P) 0)
      (CL:WHEN
       (CL:OR (CL:EQ HANDLER NULL)
        (CL:> (CL:THE CL:FIXNUM (CL:LENGTH (CL:THE CL:SIMPLE-STRING (UNWRAP-STRING P)))) HANDLERPATHLENGTH))
       (CL:SETQ HANDLER H) (CL:SETQ HANDLERPATH (%WRAPPER-VALUE P))
       (CL:SETQ HANDLERPATHLENGTH (CL:THE CL:FIXNUM (CL:LENGTH HANDLERPATH)))))))
   HANDLER))
...
This is run every time a server is started with `start-http-server'."
  (CL:WHEN (CL:EQ *HTTP-SERVER-IMPLEMENTATION* NULL)
   (CL:ERROR (NEW-STELLA-EXCEPTION "publish-registered-http-handlers: no HTTP server implementation loaded")))
    ...
    (CL:LET* ((TEST-VALUE-000 (LOOKUP OPTIONS KWD-HTTP-SERVER-HANDLER-TYPE)))
     (CL:COND
      ...
      ((CL:EQ TEST-VALUE-000 KWD-HTTP-SERVER-DIRECTORY)
       (PUBLISH-DIRECTORY-IMPL *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH)
        (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-DIRECTORY)) OPTIONS))
      ((CL:EQ TEST-VALUE-000 KWD-HTTP-SERVER-HANDLER)
       (PUBLISH-HANDLER-IMPL *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH)
...
(CL:DEFUN PRINT-REGISTERED-HTTP-HANDLERS ()
  (CL:LET* ((PATHS NIL))
   (CL:LET* ((PATH NULL) (OPTIONS NULL) (ITER-000 (ALLOCATE-ITERATOR *HTTP-HANDLER-REGISTRY*)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PATH (%KEY ITER-000)) (CL:SETQ OPTIONS (%VALUE ITER-000))
     (CL:SETQ OPTIONS OPTIONS) (CL:SETQ PATHS (CONS PATH PATHS))))
   ...
   (CL:LET* ((*PRINTPRETTY?* CL:NIL) (*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTPRETTY?* *PRINTREADABLY?*))
    (CL:LET* ((PATH NULL) (ITER-001 PATHS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ PATH (%%VALUE ITER-001))
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (UNWRAP-STRING PATH) " ->" EOL)
      (CL:LET*
       ((KEY NULL) (VALUE NULL)
        (ITER-002 (%THE-PLIST (LOOKUP-HANDLER *HTTP-SERVER-IMPLEMENTATION* (%WRAPPER-VALUE PATH)))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-002))
        (CL:SETQ VALUE (%%VALUE (%%REST ITER-002)))
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "      " KEY " " VALUE EOL)
...
type is guessed from `file's extension."
  ...
  (CL:LET* ((ARGLIST-000 NIL))
   ...
   (CL:LET*
    ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:CONTENT-TYPE :DOCUMENTATION) \"/HTTP\")" "/HTTP"))))
...
(CL:DEFMETHOD PUBLISH-FILE-IMPL ((SERVER HTTP-SERVER) PATH FILE OPTIONS)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "publish-file-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
and does not (yet) support directory listings."
  ...
  (CL:LET* ((ARGLIST-000 NIL))
   ...
   (CL:LET* ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:DOCUMENTATION) \"/HTTP\")" "/HTTP"))))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER-TYPE KWD-HTTP-SERVER-DIRECTORY)
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-PATH (WRAP-STRING PATH))
...
(CL:DEFMETHOD PUBLISH-DIRECTORY-IMPL ((SERVER HTTP-SERVER) PATH DIRECTORY OPTIONS)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "publish-directory-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
the content type of the resulting page, otherwise, text/html will be used by default."
  ...
  (CL:LET* ((ARGLIST-000 NIL))
   ...
   (CL:LET*
    ...
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER-TYPE KWD-HTTP-SERVER-HANDLER)
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-PATH (WRAP-STRING PATH))
    (INSERT-AT THEOPTIONS KWD-HTTP-SERVER-HANDLER (WRAP-FUNCTION-CODE HANDLER))
    (INSERT-AT *HTTP-HANDLER-REGISTRY* (WRAP-STRING PATH) THEOPTIONS))))
...
(CL:DEFMETHOD PUBLISH-HANDLER-IMPL ((SERVER HTTP-SERVER) PATH HANDLER OPTIONS)
  ...
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "publish-handler-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFMETHOD GET-HANDLER-OPTIONS-IMPL ((SERVER HTTP-SERVER) XCHG)
  (CL:SETQ XCHG XCHG)
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-handler-options-impl: not implemented on `" SERVER "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFUN GENERATE-ERROR-RESPONSE (XCHG CODE MESSAGE)
  ...
  (CL:LET* ((STREAM (GET-REPLY-STREAM XCHG))) (SET-RESPONSE-CODE XCHG CODE)
   ...
   (CL:PROGN (%%PRINT-STREAM STREAM "<BODY BGCOLOR='FF9999'>")
    ...
    (CL:WHEN (CL:NOT (CL:EQ MESSAGE STELLA::NULL-STRING)) (%%PRINT-STREAM STREAM "<BR><BR>" MESSAGE EOL))
...
Simple default implementation for minimal servers such as the Sun JRE HttpServer."
  (CL:LET*
   ...
   (CL:COND
    ((CL:NOT (STRING-EQL? URIPATH PATH)) (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-NOT-FOUND STELLA::NULL-STRING))
...
servers such as the Sun JRE HttpServer."
  (CL:LET*
   ((URIPATH (GET-REQUEST-URI-PATH XCHG)) (OPTIONS (GET-HANDLER-OPTIONS XCHG))
    (PATH (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-PATH)))
    (DIRECTORY (%WRAPPER-VALUE (LOOKUP OPTIONS KWD-HTTP-SERVER-DIRECTORY))) (RESOLVEDURI STELLA::NULL-STRING)
    (ISDIRECTORY? CL:NIL) (CONTENTTYPE (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-BINARY NULL))
    (STREAM (GET-REPLY-STREAM XCHG)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING URIPATH PATH DIRECTORY RESOLVEDURI CONTENTTYPE))
   (CL:SETQ URIPATH (STRING-TRIM (UNESCAPE-URL-STRING URIPATH)))
   (CL:SETQ ISDIRECTORY? (ENDS-WITH? URIPATH "/" NULL-INTEGER))
   (CL:WHEN (CL:NOT (STARTS-WITH? URIPATH PATH NULL-INTEGER))
    (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-NOT-FOUND STELLA::NULL-STRING) (CL:RETURN-FROM DIRECTORY-HANDLER))
   (CL:SETQ URIPATH (SUBSEQUENCE URIPATH (CL:THE CL:FIXNUM (CL:LENGTH PATH)) NULL-INTEGER))
   (CL:WHEN
    (CL:OR (STARTS-WITH? URIPATH "../" NULL-INTEGER) (STARTS-WITH? URIPATH "/.." NULL-INTEGER)
   ...
   (CL:WHEN ISDIRECTORY?
    (CL:COND
     ...
     (CL:T (GENERATE-ERROR-RESPONSE XCHG KWD-HTTP-SERVER-FORBIDDEN STELLA::NULL-STRING)
   ...
   (CL:LET* ((ARG NULL) (ITER-000 (%%REST ARGUMENTS)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ARG (%%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE ARG)) NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:HANDLER-CASE
    (CL:PROGN (%EVALUATE LOADCOMMAND)
     (CL:PROGN (%%PRINT-STREAM STREAM "<BODY>")
      (%%PRINT-STREAM STREAM "Successfully evaluated: " (STRINGIFY LOADCOMMAND)) (%%PRINT-STREAM STREAM "<BR><BR>" EOL)
      (%%PRINT-STREAM STREAM "<HR SIZE=2>" EOL) (%%PRINT-STREAM STREAM *HTTP-SERVER-COPYRIGHT-TRAILER*)
      (%%PRINT-STREAM STREAM "</BODY>" EOL)))
...
(CL:DEFUN TEST-HTTP-API-HANDLER (XCHG)
  ...
  (CL:LET*
   ((URI (GET-REQUEST-URI XCHG)) (PATH (GET-REQUEST-URI-PATH XCHG)) (QUERY (GET-REQUEST-URI-QUERY XCHG))
    (ARGS (PARSE-AND-DECODE-URL-ARGUMENTS QUERY #\+)) (METHOD (GET-REQUEST-METHOD XCHG))
    (LOCALHOST (GET-REQUEST-LOCAL-ADDRESS XCHG)) (REMOTEHOST (GET-REQUEST-REMOTE-ADDRESS XCHG))
   ...
   (CL:COND
    ((CL:EQ METHOD KWD-HTTP-SERVER-GET)
     (CL:WHEN (MEMBER? ARGS (WRAP-STRING "plain"))
      (SET-REPLY-HEADER-VALUE XCHG KWD-HTTP-SERVER-CONTENT-TYPE (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-PLAIN-TEXT NULL)))
     (%%PRINT-STREAM STREAM "<PRE>" EOL) (%%PRINT-STREAM STREAM "It worked, here are some parameters: " EOL)
     (%%PRINT-STREAM STREAM "URI=`" URI "'" EOL) (%%PRINT-STREAM STREAM "Path=`" PATH "'" EOL)
     (%%PRINT-STREAM STREAM "Query=`" QUERY "'" EOL) (%%PRINT-STREAM STREAM "Args=`" ARGS "'" EOL)
     (%%PRINT-STREAM STREAM "Method=`" METHOD "'" EOL) (%%PRINT-STREAM STREAM "Local host=`" LOCALHOST "'" EOL)
     (%%PRINT-STREAM STREAM "Remote host=`" REMOTEHOST "'" EOL) (%%PRINT-STREAM STREAM "Body=`" BODY "'" EOL)
     (%%PRINT-STREAM STREAM "</PRE>" EOL) (TEST-HTTP-API-HANDLER-SERVE-FORM XCHG))
    ((CL:EQ METHOD KWD-HTTP-SERVER-POST)
     (CL:LET* ((VALUES (PARSE-AND-DECODE-FORM-VALUES BODY))) (%%PRINT-STREAM STREAM "<PRE>" EOL)
      (%%PRINT-STREAM STREAM "It worked, here are some parameters: " EOL) (%%PRINT-STREAM STREAM "URI=`" URI "'" EOL)
      (%%PRINT-STREAM STREAM "Path=`" PATH "'" EOL) (%%PRINT-STREAM STREAM "Query=`" QUERY "'" EOL)
      (%%PRINT-STREAM STREAM "Args=`" ARGS "'" EOL) (%%PRINT-STREAM STREAM "Method=`" METHOD "'" EOL)
      (%%PRINT-STREAM STREAM "Local host=`" LOCALHOST "'" EOL)
      (%%PRINT-STREAM STREAM "Remote host=`" REMOTEHOST "'" EOL) (%%PRINT-STREAM STREAM "Body=`" BODY "'" EOL)
      (%%PRINT-STREAM STREAM "Values=`" VALUES "'" EOL EOL) (%%PRINT-STREAM STREAM "</PRE>" EOL)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" METHOD "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))))

(CL:DEFUN HELP-STARTUP-HTTP-SERVER1 ()
  (CL:PROGN
   (CL:SETQ SYM-HTTP-SERVER-HTTP-HTTP-SERVER-EXIT-HOOK (INTERN-RIGID-SYMBOL-WRT-MODULE "HTTP-SERVER-EXIT-HOOK" NULL 0))
   ...
   (CL:SETQ KWD-HTTP-SERVER-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   ...
   (CL:SETQ KWD-HTTP-SERVER-HTML (INTERN-RIGID-SYMBOL-WRT-MODULE "HTML" NULL 2))
   ...
   (CL:SETQ KWD-HTTP-SERVER-AUDIO-MPEG (INTERN-RIGID-SYMBOL-WRT-MODULE "AUDIO-MPEG" NULL 2))
   ...
   (CL:SETQ KWD-HTTP-SERVER-FILE (INTERN-RIGID-SYMBOL-WRT-MODULE "FILE" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-DIRECTORY (INTERN-RIGID-SYMBOL-WRT-MODULE "DIRECTORY" NULL 2))
   ...
   (CL:SETQ SYM-HTTP-SERVER-STELLA-LOAD-SYSTEM
    (INTERN-RIGID-SYMBOL-WRT-MODULE "LOAD-SYSTEM" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   (CL:SETQ KWD-HTTP-SERVER-GET (INTERN-RIGID-SYMBOL-WRT-MODULE "GET" NULL 2))
   (CL:SETQ KWD-HTTP-SERVER-POST (INTERN-RIGID-SYMBOL-WRT-MODULE "POST" NULL 2))
...
where none has been loaded yet.  If `error?' is TRUE, raise an error if none
is defined for the current native language environment.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-DEFAULT-HTTP-SERVER-IMPL)
    NULL)
...
a server instance running, it will be stopped first.  If `port' is <= 0 or
NULL, use the value of `*default-http-server-port*'.  Returns the listening
address of the started server instance.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %START-HTTP-SERVER)
    (CL:FUNCTION START-HTTP-SERVER-EVALUATOR-WRAPPER))
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (START-HTTP-SERVER-IMPL STRING) ((SERVER HTTP-SERVER) (PORT INTEGER)))"
...
server instance running, this is a no-op.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION STOP-HTTP-SERVER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD STOP-HTTP-SERVER-IMPL ((SERVER HTTP-SERVER)))" (CL:FUNCTION STOP-HTTP-SERVER-IMPL)
    NULL)
...
will return /foo/bar.html#frag?a+b+c .\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))"
...
return a+b+c .\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI-QUERY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-QUERY-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))"
...
return /foo/bar.html.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-URI-PATH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-URI-PATH-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-METHOD-IMPL KEYWORD) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))"
...
if this was a GET), the result will be the empty string.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-REQUEST-BODY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-REQUEST-BODY-IMPL STRING) ((SERVER HTTP-SERVER) (XCHG HTTP-EXCHANGE)))"
...
:OK (code 200) is used.\" :PUBLIC? TRUE)" (CL:FUNCTION SET-RESPONSE-CODE) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "HTTP-SUCCESS-RESPONSE-CODE?"
    "(DEFUN (HTTP-SUCCESS-RESPONSE-CODE? BOOLEAN) ((CODE INTEGER)))" (CL:FUNCTION HTTP-SUCCESS-RESPONSE-CODE?) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-MIME-TYPE"
    "(DEFUN (GET-HTTP-MIME-TYPE STRING) ((TYPENAME KEYWORD) (DEFAULT KEYWORD)))" (CL:FUNCTION GET-HTTP-MIME-TYPE) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "GUESS-FILE-CONTENT-TYPE" "(DEFUN (GUESS-FILE-CONTENT-TYPE STRING) ((FILE STRING)))"
...
a key-value-list.  NOTE: values will not be trimmed and empty values will be represented by the empty
string rather than NULL.\" :PUBLIC? TRUE)" (CL:FUNCTION PARSE-AND-DECODE-FORM-VALUES) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-HTTP-DOCUMENT-ROOT" "(DEFUN (GET-HTTP-DOCUMENT-ROOT STRING) () :PUBLIC? TRUE)"
...
servers such as the Sun JRE HttpServer.\" :PUBLIC? TRUE)" (CL:FUNCTION DIRECTORY-HANDLER) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "TEST-HTTP-API-HANDLER-SERVE-FORM"
    "(DEFUN TEST-HTTP-API-HANDLER-SERVE-FORM ((XCHG HTTP-EXCHANGE)))" (CL:FUNCTION TEST-HTTP-API-HANDLER-SERVE-FORM)
    NULL)
   (DEFINE-FUNCTION-OBJECT "TEST-HTTP-API-HANDLER" "(DEFUN TEST-HTTP-API-HANDLER ((XCHG HTTP-EXCHANGE)))"
    (CL:FUNCTION TEST-HTTP-API-HANDLER) NULL)
   (DEFINE-FUNCTION-OBJECT "STARTUP-HTTP-SERVER" "(DEFUN STARTUP-HTTP-SERVER () :PUBLIC? TRUE)"
    (CL:FUNCTION STARTUP-HTTP-SERVER) NULL)
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-HTTP-SERVER-HTTP-STARTUP-HTTP-SERVER)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-HTTP-SERVER-STELLA-METHOD-STARTUP-CLASSNAME
...
(CL:DEFUN STARTUP-HTTP-SERVER ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/HTTP" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*)) (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (HELP-STARTUP-HTTP-SERVER1))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *HTTP-MIME-TYPES*
     (LIST*
      (LIST* KWD-HTTP-SERVER-PLAIN-TEXT (WRAP-STRING "text/plain") (WRAP-STRING "txt") (WRAP-STRING "text")
       (WRAP-STRING "asc") NIL)
      (LIST* KWD-HTTP-SERVER-HTML (WRAP-STRING "text/html") (WRAP-STRING "htm") (WRAP-STRING "html") NIL)
      (LIST* KWD-HTTP-SERVER-XML (WRAP-STRING "text/xml") (WRAP-STRING "xml") (WRAP-STRING "rdf") (WRAP-STRING "owl")
       NIL)
      (LIST* KWD-HTTP-SERVER-BINARY (WRAP-STRING "application/octet-stream") (WRAP-STRING "zip") (WRAP-STRING "exe")
       (WRAP-STRING "class") NIL)
    ...
    (CL:SETQ *HTTP-DOCUMENT-ROOT*
     (CL:IF (CL:NOT (CL:EQ *HTTP-DOCUMENT-ROOT* STELLA::NULL-STRING)) *HTTP-DOCUMENT-ROOT*
      (CL:IF (PROBE-FILE? "PL:htdocs;powerloom.html") (TRANSLATE-LOGICAL-PATHNAME "PL:htdocs;")
...
with an actual native server object.\")")
    ...
    (PUBLISH-DIRECTORY "/ploom/" (GET-HTTP-DOCUMENT-ROOT) KWD-HTTP-SERVER-DOCUMENTATION
     (WRAP-STRING "Top-level htdocs directory."))
    (PUBLISH-HANDLER "/ploom/load-system" (CL:FUNCTION LOAD-SYSTEM-HANDLER) KWD-HTTP-SERVER-CONTENT-TYPE
     (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL)) KWD-HTTP-SERVER-DOCUMENTATION
     (WRAP-STRING "Triggers a load-system operation at the server for the argument system."))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *TEST-HTTP-API-HANDLER-LAST-XCHG* HTTP-EXCHANGE NULL)")
    (PUBLISH-HANDLER "/ploom/test-http-api" (CL:FUNCTION TEST-HTTP-API-HANDLER) KWD-HTTP-SERVER-CONTENT-TYPE
     (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTTP-SERVER-HTML NULL)) KWD-HTTP-SERVER-DOCUMENTATION
     (WRAP-STRING "Useful for comparing different HTTP API implementations.")))))
