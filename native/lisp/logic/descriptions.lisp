  ...
  (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
   ...
   (CL:LET* ((DESCRIPTION (CREATE-DESCRIPTION NULL-INTEGER CL:T)))
    ...
    (CL:SETF (%SURROGATE-VALUE-INVERSE DESCRIPTION)
     SGT-DESCRIPTIONS-LOGIC-UNFINALIZED)
    (CL:LET*
     ((*FILLINGCONSTRAINTPROPAGATIONQUEUES?* CL:NIL)
      (*EVALUATIONMODE* KWD-DESCRIPTIONS-EXTENSIONAL-ASSERTION))
     (CL:DECLARE
...
(CL:DEFUN HELP-EQUIVALENT-PROPOSITIONS? (SELF OTHER MAPPING IGNORELASTARG?)
  (CL:WHEN (CL:EQ SELF OTHER)
   (CL:RETURN-FROM HELP-EQUIVALENT-PROPOSITIONS? CL:T))
     ...
     (CL:LET* ((SELFARGS (%ARGUMENTS SELF)) (OTHERARGS (%ARGUMENTS OTHER)))
      (CL:LET* ((TEST-VALUE-002 CL:NIL))
       ...
       (CL:WHEN TEST-VALUE-002
        (CL:LET* ((ALWAYS?-000 CL:T))
         (CL:LET*
          ((ARG1 NULL) (VECTOR-002 SELFARGS) (INDEX-002 0)
           (LENGTH-002 (LENGTH VECTOR-002)) (ARG2 NULL) (VECTOR-003 OTHERARGS)
           (INDEX-003 0) (LENGTH-003 (LENGTH VECTOR-003)) (I NULL-INTEGER)
           (ITER-000 2)
           (UPPER-BOUND-000
            (CL:IF IGNORELASTARG? (LENGTH SELFARGS) NULL-INTEGER))
           (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
          (CL:DECLARE
           (CL:TYPE CL:FIXNUM INDEX-002 LENGTH-002 INDEX-003 LENGTH-003 I
            ITER-000 UPPER-BOUND-000))
          (CL:LOOP WHILE
        ...
        (CL:WHEN TEST-VALUE-002
         (CL:SETQ TEST-VALUE-002
          (CL:= (LENGTH SELFARGS) (LENGTH OTHERARGS)))))
...
(CL:DEFUN FIND-DUPLICATE-COMPLEX-DESCRIPTION (SELF)
  (CL:LET*
   ((INDEX
     (WRAP-INTEGER (PROPOSITION-HASH-INDEX (%PROPOSITION SELF) NULL CL:NIL)))
    ...
    (THEOPTIONS
     (PARSE-OPTIONS
      (CL:IF (KEYWORD? (%%VALUE OPTIONSLIST)) OPTIONSLIST
       (CONS-LIST KWD-DESCRIPTIONS-MODULE (%%VALUE OPTIONSLIST)
        KWD-DESCRIPTIONS-LOCAL? (%%VALUE (%%REST OPTIONSLIST))))
      (LIST* KWD-DESCRIPTIONS-MODULE SGT-DESCRIPTIONS-STELLA-MODULE
       KWD-DESCRIPTIONS-LOCAL? SGT-DESCRIPTIONS-STELLA-BOOLEAN NIL)
      CL:T CL:NIL))
    (THEMODULE
     (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-DESCRIPTIONS-MODULE *MODULE*))
    (LOCAL?
     (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN
      (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-DESCRIPTIONS-LOCAL?
       FALSE-WRAPPER))))
...
(CL:DEFUN HELP-STARTUP-DESCRIPTIONS2 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-DESCRIPTIONS-LOGIC-UNFINALIZED
    (INTERN-RIGID-SYMBOL-WRT-MODULE "UNFINALIZED" NULL 1))
...
(CL:DEFUN HELP-STARTUP-DESCRIPTIONS3 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-DESCRIPTIONS-MODULE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" NULL 2))
   (CL:SETQ KWD-DESCRIPTIONS-LOCAL?
...
the old <module> <local?> arguments specified without keywords.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)"
     (CL:FUNCTION %LIST-UNDEFINED-RELATIONS)
