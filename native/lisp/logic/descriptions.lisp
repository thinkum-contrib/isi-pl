...
(CL:DEFVAR SYM-DESCRIPTIONS-LOGIC-QUERY-037 NULL)
...
(CL:DECLAIM
  ...
  *VARIABLEIDCOUNTER* STANDARD-ERROR *UNIFY-PROPOSITIONS?* *QUERYITERATOR*
  ZERO-VARIABLES-VECTOR TRUE-WRAPPER STANDARD-WARNING
  *SUPPRESSNONLOGICOBJECTWARNING?* STANDARD-OUTPUT *PL-KERNEL-MODULE* NIL-LIST
  EOL *MODULE* TRUE-PROPOSITION TRUE FALSE NIL NULL-INTEGER))
...
(CL:DEFUN MATERIALIZE-PRIMITIVE-DESCRIPTION-BODY (DESCRIPTION)
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LET* ((SACRIFICIALDESCRIPTION (EVALUATE-DESCRIPTION-TERM TREE FALSE)))
     (CL:SETF (%IO-VARIABLES DESCRIPTION)
     ...
     (CL:LET* ((ALWAYS?-000 TRUE))
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
        (CL:PROGN (CL:SETQ I1 ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
        (CL:PROGN
         (CL:SETQ ARG2
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
        ...
        (CL:LET* ((TEST-VALUE-001 FALSE))
         ...
         (CL:WHEN TEST-VALUE-001 (CL:SETQ ALWAYS?-000 FALSE) (CL:RETURN)))))
      (CL:SETQ TEST-VALUE-000 ALWAYS?-000))))
   (CL:LET* ((VALUE-000 TEST-VALUE-000)) VALUE-000)))
...
(CL:DEFUN EQUIVALENT-PROPOSITIONS? (SELF OTHER MAPPING)
  (CL:WHEN (CL:EQ SELF OTHER) (CL:RETURN-FROM EQUIVALENT-PROPOSITIONS? TRUE))
    ...
    (CL:T
        ...
        (CL:LET*
         ...
         (CL:LET*
          ((V2 NULL) (V1 NULL) (VECTOR-000 IOVARS1) (INDEX-000 0)
           (LENGTH-000 (LENGTH VECTOR-000)) (VECTOR-001 IOVARS2) (INDEX-001 0)
           (LENGTH-001 (LENGTH VECTOR-001)))
          (CL:DECLARE
     ...
     (CL:LET* ((TEST-VALUE-002 FALSE))
      ...
      (CL:WHEN TEST-VALUE-002
       (CL:LET* ((ALWAYS?-000 TRUE))
        (CL:LET*
         ((ARG2 NULL) (ARG1 NULL) (VECTOR-002 (%ARGUMENTS SELF)) (INDEX-002 0)
          (LENGTH-002 (LENGTH VECTOR-002)) (VECTOR-003 (%ARGUMENTS OTHER))
...
(CL:DEFUN EQUIVALENT-DESCRIPTIONS? (SELF OTHER MAPPING)
  (CL:WHEN (CL:EQ SELF OTHER) (CL:RETURN-FROM EQUIVALENT-DESCRIPTIONS? TRUE))
  (CL:WHEN
  ...
  (CL:COND ((NAMED-DESCRIPTION? SELF) FALSE)
   (CL:T (CL:WHEN (CL:EQ MAPPING NULL) (CL:SETQ MAPPING (NEW-KEY-VALUE-LIST)))
    (CL:LET*
...
(CL:DEFUN EQUIVALENT-FORMULAE? (SELF OTHER MAPPING)
  ...
  (CL:WHEN
   (CL:AND (CL:NOT (CL:EQ MAPPING NULL)) (EQL? OTHER (LOOKUP MAPPING SELF)))
   (CL:RETURN-FROM EQUIVALENT-FORMULAE? TRUE))
  (CL:WHEN (EQUAL? SELF OTHER) (CL:RETURN-FROM EQUIVALENT-FORMULAE? TRUE))
   ...
   (CL:LET* ((ALWAYS?-000 TRUE))
    (CL:LET*
     ((ARG NULL) (V NULL) (VECTOR-000 VARIABLES) (INDEX-000 0)
      (LENGTH-000 (LENGTH VECTOR-000)) (VECTOR-001 ARGUMENTS) (INDEX-001 0)
      (LENGTH-001 (LENGTH VECTOR-001)))
     (CL:DECLARE
      (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 INDEX-001 LENGTH-001))
     (CL:LOOP WHILE
  ...
  (CL:LET* ((PROPOSITION (%PROPOSITION SELF)))
   (CL:LET* ((TEST-VALUE-000 (%KIND PROPOSITION)))
    (CL:COND
     ...
     (CL:T
      (CL:WHEN (SAME-ARGUMENTS? (%IO-VARIABLES SELF) (%ARGUMENTS PROPOSITION))
       (CL:RETURN-FROM FIND-DUPLICATE-NAMED-DESCRIPTION
        (EXTRACT-GOAL-DESCRIPTION PROPOSITION NULL))))))
   NULL))
...
(CL:DEFUN UNIQUIFY-DESCRIPTION-VARIABLES (SELF)
  (CL:LET* ((VARIABLENAMES SYSTEM-DEFINED-ARGUMENT-NAMES))
   (CL:WHEN
    (CL:>
     (CL:+ (LENGTH (%IO-VARIABLES SELF)) (LENGTH (%INTERNAL-VARIABLES SELF)))
         ...
         (CL:LOOP WHILE
          ...
          (CL:COND
           ...
           ((CONS-TREE-LESS-THAN? V2 V1)
            (CL:RETURN-FROM CONS-TREE-LESS-THAN? FALSE)))))
        FALSE)))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-DESCRIPTIONS-STELLA-GENERALIZED-SYMBOL)
...
(CL:DEFUN UNIQUIFY-PROPOSITION-TREE (TREE)
  (CL:WHEN (CL:EQ TREE NULL) (CL:RETURN-FROM UNIQUIFY-PROPOSITION-TREE TREE))
  (CL:COND
...
(CL:DEFUN FIND-EXACT-DUPLICATE-DESCRIPTION (SELF)
  ...
  (CL:LET*
   ((STRINGIFICATION
     (STRINGIFY
      (UNIQUIFY-PROPOSITION-TREE (GENERATE-PROPOSITION (%PROPOSITION SELF)))))
   ...
   (CL:WHEN (CL:EQ BUCKET NULL)
    (INSERT-AT *STRINGIFIED-DESCRIPTION-INDEX* STRINGIFICATION (LIST SELF))
     ...
     (CL:WHEN
      (CL:AND (EQUIVALENT-DESCRIPTIONS? SELF D NULL)
       (CL:OR (CL:EQ (%HOME-CONTEXT SELF) (%HOME-CONTEXT D))
        (MEMB? (%ALL-SUPER-CONTEXTS (%HOME-CONTEXT SELF)) (%HOME-CONTEXT D))))
...
(CL:DEFUN COLLECT-CONSTANT-DESCRIPTION-REFERENCES (SELF COLLECTION STOPATONE?)
  ...
  (HELP-COLLECT-CONSTANT-REFERENCES (%PROPOSITION SELF) COLLECTION STOPATONE?)
  COLLECTION)
...
(CL:DEFUN FIND-DUPLICATE-DESCRIPTION (SELF)
  (CL:LET* ((DUPLICATE NULL))
   (CL:IF (NAMED-DESCRIPTION? SELF) (CL:PROGN NULL)
    (CL:LET* () (CL:SETQ DUPLICATE (FIND-DUPLICATE-NAMED-DESCRIPTION SELF))
     (CL:IF (CL:NOT (CL:EQ DUPLICATE NULL)) (CL:PROGN DUPLICATE)
      (CL:LET* () (CL:SETQ DUPLICATE (FIND-EXACT-DUPLICATE-DESCRIPTION SELF))
       (CL:IF (CL:NOT (CL:EQ DUPLICATE NULL)) (CL:PROGN DUPLICATE)
        (CL:PROGN NULL))))))))
...
(CL:DEFUN ADD-DEPENDENT-DESCRIPTION-LINK (SELF DESCRIPTION)
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF) SGT-DESCRIPTIONS-LOGIC-LOGIC-OBJECT)
    (CL:PROGN
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:WHEN
      (CL:AND
       (CL:NOT
        (CL:EQ
         (ACCESS-IN-CONTEXT (%VARIABLE-VALUE VBL) (%HOME-CONTEXT VBL) FALSE)
         NULL))
       (VARIABLE?
        (ACCESS-IN-CONTEXT (%VARIABLE-VALUE VBL) (%HOME-CONTEXT VBL) FALSE)))
      (CL:SETQ VALUE
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
     (CL:PROGN
      ...
      (CL:SETQ INDEX-001 (CL:1+ INDEX-001)))
     (CL:PROGN (CL:SETQ I ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
     (CL:WHEN (MEMBER? (%IO-VARIABLES DESCRIPTION) VBL) (CL:SETQ VALUE VBL)
      (CL:LET
       ((SELF (%THE-ARRAY (%INTERNAL-VARIABLES DESCRIPTION))) (VALUE NULL)
        (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
      ...
      (CL:=
       (%ARRAY-SIZE
        (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION)
         SYM-DESCRIPTIONS-LOGIC-IO-VARIABLES NULL))
       0)
      (UNFASTEN-PROPOSITION PROPOSITION) (NORMALIZE-PROPOSITION PROPOSITION)
      ...
      (CL:LOOP WHILE
       ...
       (CL:IF (CL:OR (CL:EQ TYPE NULL) (CLASS-DESCRIPTION? SELF))
        ...
        (CL:SETQ DECLARATIONS
         (CONS (CONS (%SKOLEM-NAME V) (CONS (SURROGATE-TO-SYMBOL TYPE) NIL))
          DECLARATIONS)))
       (CL:IF (CL:EQ COLLECT-000 NULL)
           ...
           (CL:PROGN
            (CL:COND
             ((CL:NOT (CL:EQ (%IO-VARIABLE-TYPES PREDICATE) NULL))
              (CL:LET*
               ((TYPE NULL) (ARG NULL) (VECTOR-000 ARGUMENTS) (INDEX-000 0)
                (LENGTH-000 (LENGTH VECTOR-000))
...
(CL:DEFUN CREATE-DUMMY-RELATION (WAYWARDPROPOSITION)
  (CL:LET*
   ...
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS DESCRIPTION)
    SYM-DESCRIPTIONS-LOGIC-UNDECLARED? (CL:IF TRUE TRUE-WRAPPER FALSE-WRAPPER)
    FALSE-WRAPPER)))
         ...
         (CL:LET*
          ...
          (CL:WHEN
           (APPLY-CACHED-ASK (CONS SYM-DESCRIPTIONS-LOGIC-?X NIL)
            ...
            (CONS-LIST TERM) (CONS-LIST) SYM-DESCRIPTIONS-LOGIC-QUERY-037)
           (CL:SETQ UNDEFINED (CONS TERM UNDEFINED))))))
       (CL:T)))))
   UNDEFINED))
...
(CL:DEFUN HELP-STARTUP-DESCRIPTIONS3 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-DESCRIPTIONS-LOGIC-QUERY-037
    (INTERN-RIGID-SYMBOL-WRT-MODULE "QUERY-037" NULL 0))
