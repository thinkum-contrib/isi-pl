...
(CL:DEFCLASS PROPAGATION-ENVIRONMENT (STANDARD-OBJECT)
  ((EVALUATION-QUEUE :ALLOCATION :INSTANCE :ACCESSOR %EVALUATION-QUEUE)
   (EVALUATION-STATES :ALLOCATION :INSTANCE :ACCESSOR %EVALUATION-STATES)
   (FORWARD-CHAINING-QUEUE :ALLOCATION :INSTANCE :ACCESSOR %FORWARD-CHAINING-QUEUE)
   (FORWARD-CHAINING-SET :ALLOCATION :INSTANCE :ACCESSOR %FORWARD-CHAINING-SET)
...
(CL:DEFUN NEW-PROPAGATION-ENVIRONMENT ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE PROPAGATION-ENVIRONMENT)))
   (CL:SETF (%ELABORATED-OBJECTS SELF) (NEW-HASH-SET)) (CL:SETF (%DEFERRED-DEFAULT-PROPOSITIONS SELF) (LIST))
   (CL:SETF (%FORWARD-CHAINING-SET SELF) (NEW-HASH-SET)) (CL:SETF (%FORWARD-CHAINING-QUEUE SELF) (LIST))
...
(CL:DEFUN ACCESS-PROPAGATION-ENVIRONMENT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-PROPAGATE-LOGIC-EVALUATION-QUEUE)
    (CL:IF SETVALUE? (CL:SETF (%EVALUATION-QUEUE SELF) VALUE) (CL:SETQ VALUE (%EVALUATION-QUEUE SELF))))
   ((CL:EQ SLOTNAME SYM-PROPAGATE-LOGIC-EVALUATION-STATES)
    (CL:IF SETVALUE? (CL:SETF (%EVALUATION-STATES SELF) VALUE) (CL:SETQ VALUE (%EVALUATION-STATES SELF))))
   ...
   ((CL:EQ SLOTNAME SYM-PROPAGATE-LOGIC-DEFERRED-DEFAULT-PROPOSITIONS)
    (CL:IF SETVALUE? (CL:SETF (%DEFERRED-DEFAULT-PROPOSITIONS SELF) VALUE)
     (CL:SETQ VALUE (%DEFERRED-DEFAULT-PROPOSITIONS SELF))))
   ((CL:EQ SLOTNAME SYM-PROPAGATE-LOGIC-ELABORATED-OBJECTS)
    (CL:IF SETVALUE? (CL:SETF (%ELABORATED-OBJECTS SELF) VALUE) (CL:SETQ VALUE (%ELABORATED-OBJECTS SELF))))
   (CL:T
...
(CL:DEFUN PRINT-PROPAGATION-ENVIRONMENT-STATS (SELF)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "  " (LENGTH (%EVALUATION-QUEUE SELF))
   " propositions in evaluation-queue" EOL)
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "  " (LENGTH (%FORWARD-CHAINING-SET SELF))
...
(CL:DEFMETHOD COPY ((SELF PROPAGATION-ENVIRONMENT))
  (CL:LET* ((COPY (NEW-PROPAGATION-ENVIRONMENT))) (CL:SETF (%EVALUATION-QUEUE COPY) (COPY (%EVALUATION-QUEUE SELF)))
   (CL:LET* ((PROP NULL) (STATE NULL) (ITER-000 (ALLOCATE-ITERATOR (%EVALUATION-STATES SELF))))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PROP (%KEY ITER-000)) (CL:SETQ STATE (%VALUE ITER-000))
...
(CL:DEFUN POST-FOR-EVALUATION (SELF WORLD)
  ...
  (CL:LET* ((ENVIRONMENT (GET-PROPAGATION-ENVIRONMENT WORLD)))
   (CL:WHEN (CL:NOT (CL:EQ (LOOKUP (%EVALUATION-STATES ENVIRONMENT) SELF) KWD-PROPAGATE-POSTED))
...
(CL:DEFUN HELP-COLLECT-FORWARD-RULES (DESCRIPTION RULES INDICES TOUCHEDDEFAULT? BEENTHERE)
  ...
  (CL:LET* ((INDEX NULL) (ITER-000 (%THE-CONS-LIST (FORWARD-CHAINING-INDICES DESCRIPTION))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ INDEX (%%VALUE ITER-000))
    (CL:WHEN (CL:AND (CL:NOT (DELETED? (%MASTER-RULE INDEX))) (TRUE? (%MASTER-RULE INDEX)))
  ...
  (CL:LET* ((P NULL) (ITER-001 (APPLICABLE-RULES-OF-DESCRIPTION DESCRIPTION KWD-PROPAGATE-FORWARD CL:T)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ P (%%VALUE ITER-001))
    (CL:WHEN
     (CL:NOT (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS P) SYM-PROPAGATE-LOGIC-BACKWARD-ONLY? FALSE-WRAPPER)))
        ...
        ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPAGATE-LOGIC-NAMED-DESCRIPTION)
         (CL:PROGN
          (CL:LET* ((SUPER (VALUE-OF CONSEQUENT))) (CL:WHEN (DEFAULT-TRUE? P) (CL:SETQ TOUCHEDDEFAULT? CL:T))
           (CL:WHEN
            (CL:OR
             (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS P) SYM-PROPAGATE-LOGIC-FORWARD-ONLY? FALSE-WRAPPER))
...
(CL:DEFUN POST-TO-FORWARD-CHAINING-QUEUE (SELF WORLD)
  (CL:WHEN (CL:AND (UNKNOWN? SELF) (CL:NOT (CL:EQ (%KIND SELF) KWD-PROPAGATE-FUNCTION)))
    ...
    (CL:COND
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-ISA) (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-PREDICATE)
      ...
      (CL:WHEN (CL:AND (CL:NOT (CL:EQ DESCRIPTION NULL)) (HAS-FORWARD-CHAINING-RULES? DESCRIPTION SELF))
       (INSERT (%FORWARD-CHAINING-SET ENVIRONMENT) SELF) (INSERT (%FORWARD-CHAINING-QUEUE ENVIRONMENT) SELF)))
     (CL:T)))))
...
(CL:DEFMETHOD SKOLEM-GENERATION-COUNT ((SELF SKOLEM))
  (CL:LET*
   ((ANSWER
     (%WRAPPER-VALUE
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-SKOLEM-GENERATION-COUNT NULL-INTEGER-WRAPPER))))
      ...
      (CL:WHEN (CL:AND (SKOLEM? ARG) (CL:> (SKOLEM-GENERATION-COUNT ARG) SKOLEMGENERATIONCOUNT))
       ...
       (CL:WHEN (CL:>= SKOLEMGENERATIONCOUNT *MAX-SKOLEM-GENERATION-COUNT*)
        (CL:WHEN (CL:AND (CL:NOT (CL:EQ *TRACED-KEYWORDS* NULL)) (MEMB? *TRACED-KEYWORDS* KWD-PROPAGATE-PROPAGATE))
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "*** cutting off forward skolemization on: "
    ...
    (CL:LET* ((P NULL) (ITER-000 *COLLECTFORWARDPROPOSITIONS*))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ P (%%VALUE ITER-000))
      (CL:LET* ((VALUE-000 NULL))
       (CL:LET* ((ARG NULL) (VECTOR-001 (%ARGUMENTS P)) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
      ...
      (CL:WHEN
       (CL:AND (CL:NOT (CL:EQ SKOLEM NULL)) (CL:EQ (%DEFINING-PROPOSITION SKOLEM) NULL)
        (CL:NOT
         (%WRAPPER-VALUE
          (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SKOLEM) SYM-PROPAGATE-LOGIC-HYPOTHESIZED-INSTANCE? FALSE-WRAPPER))))
...
(CL:DEFUN TRACE-FORWARD-RULE (RULE TRIGGER CONSEQUENTS)
  (CL:WHEN (CL:AND (CL:NOT (CL:EQ CONSEQUENTS NULL)) (CL:NOT (CL:EQ CONSEQUENTS NIL)))
   (CL:LET* ((INDENT 11)) (CL:DECLARE (CL:TYPE CL:FIXNUM INDENT))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "RUNNING FORWARD RULE:" EOL "  TRIGGER: ")
    (PRINT-FORMULA TRIGGER INDENT) (CL:WHEN (FALSE? TRIGGER) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) ")"))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "  RULE:    ") (PRINT-FORMULA RULE INDENT)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "  RESULT:  ")
    (CL:WHEN (CL:NOT (CL:EQ (%%REST CONSEQUENTS) NIL))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "(" (STRINGIFIED-SURROGATE SGT-PROPAGATE-PL-KERNEL-KB-AND) " ")
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%%REST CONSEQUENTS) NIL)) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) ")")
     (CL:SETQ INDENT (CL:- INDENT 5)))
...
(CL:DEFUN APPLY-FORWARD-RULES-TO-VECTOR (TRIGGERDESCRIPTION ARGUMENTS TRIGGERPROPOSITION)
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:COND ((DELETED? ARG) (CL:RETURN-FROM APPLY-FORWARD-RULES-TO-VECTOR))
      ((CL:NOT (EQL? ARG (VALUE-OF ARG)))
       ...
       (CL:LET ((SELF (%THE-ARRAY EVALUATEDARGS)) (VALUE (VALUE-OF ARG)) (POSITION I))
  ...
  (CL:LET* ((RULES NULL) (INDICES NULL))
   ...
   (CL:LET* ((R NULL) (TOUCHEDDEFAULT? NULL) (ITER-001 (%THE-KV-LIST RULES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NULL)) DO (CL:SETQ R (%KEY ITER-001))
     (CL:SETQ TOUCHEDDEFAULT? (%VALUE ITER-001))
     (CL:WHEN (APPLICABLE-FORWARD-RULE? R ARGUMENTS)
      (APPLY-RULE-CONSEQUENT-TO-VECTOR (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS R))) 1) ARGUMENTS R
   ...
   (CL:LET* ((FWDINDEX NULL) (TOUCHEDDEFAULT? NULL) (ITER-002 (%THE-KV-LIST INDICES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NULL)) DO (CL:SETQ FWDINDEX (%KEY ITER-002))
     (CL:SETQ TOUCHEDDEFAULT? (%VALUE ITER-002))
     (CL:TAGBODY
      (CL:LET* ((INPUTARGS NIL))
       (CL:WHEN (CL:NOT (CL:= (LENGTH ARGUMENTS) (LENGTH (%ARGUMENTS (%FORWARD-GOAL FWDINDEX))))) (CL:GO :CONTINUE))
       (CL:LET*
        ((ARG NULL) (VECTOR-001 ARGUMENTS) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)) (GOALARG NULL)
         (VECTOR-002 (%ARGUMENTS (%FORWARD-GOAL FWDINDEX))) (INDEX-002 0) (LENGTH-002 (LENGTH VECTOR-002)))
       ...
       (CL:COND
        (TRACEFORWARDINFERENCE?
         (CL:WHEN TRACEGOALTREE?
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Proving forward goal: " (%QUERY-BODY FWDINDEX) EOL)))
       ...
       (CL:LET* ((*FILLINGCONSTRAINTPROPAGATIONQUEUES?* CL:T))
        ...
        (CL:MULTIPLE-VALUE-SETQ (OUTPUTBINDINGS TRUTHVALUES JUSTIFICATIONS)
         (APPLY-CACHED-RETRIEVE (%IO-VARIABLES FWDINDEX) (%QUERY-BODY FWDINDEX) (%INPUT-BINDINGS FWDINDEX)
          ...
          (%CACHE-ID FWDINDEX))))
       (CL:WHEN TRACEGOALTREE?
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Done proving forward goal: " (%QUERY-BODY FWDINDEX) EOL EOL))
        ...
        (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-004 NIL)) (CL:NOT (CL:EQ ITER-005 NIL))) DO
         ...
         (CL:LET* ((HEAD-000 (%%VALUE JUSTIFICATIONS))) (CL:SETQ JUSTIFICATIONS (%%REST JUSTIFICATIONS))
          (CL:LET* ((JUSTIFICATION HEAD-000))
           (APPLY-RULE-CONSEQUENT-TO-VECTOR (%CONSEQUENT FWDINDEX) (COPY-LIST-TO-ARGUMENTS-VECTOR (LISTIFY OB))
            (%MASTER-RULE FWDINDEX) TRIGGERDESCRIPTION TRIGGERPROPOSITION
      ...
      (CL:LET* ((PROPOSITION NULL) (ITER-001 (%THE-CONS-LIST QUEUE)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ PROPOSITION (%%VALUE ITER-001)) (CL:PROGN)
        ...
        (CL:WHEN (CL:NOT (CL:EQ DESCRIPTION NULL))
         (APPLY-FORWARD-RULES-TO-VECTOR DESCRIPTION (%ARGUMENTS PROPOSITION) PROPOSITION))
        (CL:SETQ ITER-001 (%%REST ITER-001))))))
    (CL:WHEN (CL:AND (EMPTY? (%EVALUATION-QUEUE SELF)) (EMPTY? (%FORWARD-CHAINING-QUEUE SELF)))
...
(CL:DEFUN EVALUATE-NEW-PROPOSITION (SELF)
  (CL:WHEN
   (CL:OR (DESCRIPTION-MODE?)
    (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-DESCRIPTIVE? FALSE-WRAPPER)))
   ...
   (CL:COND
    ((TRUE? SELF)
     (CL:LET* ((ARG NULL) (VECTOR-000 ARGUMENTS) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       (CL:SETQ ARG (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
       (ASSIGN-TRUTH-VALUE ARG (ACCESS-IN-CONTEXT (%TRUTH-VALUE SELF) (%HOME-CONTEXT SELF) CL:NIL))
       (CL:SETQ INDEX-000 (CL:1+ INDEX-000)))))
    ((FALSE? SELF)
  ...
  (CL:LET* ((ARGUMENT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS SELF))) 0)))
   (CL:WHEN (CL:OR (TRUE? ARGUMENT) (FALSE? ARGUMENT))
    (ASSIGN-TRUTH-VALUE SELF
     (INVERT-TRUTH-VALUE (ACCESS-IN-CONTEXT (%TRUTH-VALUE ARGUMENT) (%HOME-CONTEXT ARGUMENT) CL:NIL))))
   (CL:WHEN (CL:OR (TRUE? SELF) (FALSE? SELF))
    (ASSIGN-TRUTH-VALUE ARGUMENT
     (INVERT-TRUTH-VALUE (ACCESS-IN-CONTEXT (%TRUTH-VALUE SELF) (%HOME-CONTEXT SELF) CL:NIL))))))
...
(CL:DEFUN EVALUATE-EQUIVALENT-PROPOSITION (SELF)
  (CL:LET*
   ((ARGUMENTS (%ARGUMENTS SELF))
    (FIRSTARG (INNERMOST-OF (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS)) 0)))
...
(CL:DEFUN EQUATE-EQUIVALENT-FUNCTION-PROPOSITIONS (SELF)
  (CL:LET* ((RELATIONREF (%OPERATOR SELF)) (NOFARGS (LENGTH (%ARGUMENTS SELF))) (CANDIDATEPROPOSITIONS (NEW-LIST)))
         ...
         (CL:PROGN
          (CL:WHEN
           (CL:AND (CL:NOT (CL:EQ (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) CL:NIL) NULL))
            (ISA? (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) CL:NIL)
             SGT-PROPAGATE-LOGIC-LOGIC-OBJECT))
           (CL:LET*
            ((PROP NULL)
             (ITER-001
              (ALLOCATE-ITERATOR
               (UNFILTERED-DEPENDENT-PROPOSITIONS (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) CL:NIL)
                RELATIONREF))))
            (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ PROP (%VALUE ITER-001))
             (CL:WHEN (CL:AND (CL:EQ (%OPERATOR PROP) RELATIONREF) (CL:NOT (CL:EQ PROP SELF)))
   ...
   (CL:LET* ((PROP NULL) (ITER-006 (%THE-CONS-LIST CANDIDATEPROPOSITIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-006 NIL)) DO (CL:SETQ PROP (%%VALUE ITER-006))
     (CL:LET* ((ALWAYS?-000 CL:T))
      (CL:LET*
       ((ARG1 NULL) (VECTOR-001 (%ARGUMENTS SELF)) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)) (ARG2 NULL)
        (VECTOR-002 (%ARGUMENTS PROP)) (INDEX-002 0) (LENGTH-002 (LENGTH VECTOR-002)) (I NULL-INTEGER) (ITER-007 1))
      ...
      (CL:WHEN ALWAYS?-000
       (EQUATE-VALUES SELF (INNERMOST-OF (LAST (%ARGUMENTS SELF))) (INNERMOST-OF (LAST (%ARGUMENTS PROP))))))
...
(CL:DEFUN EVALUATE-FUNCTION-PROPOSITION (SELF)
  ...
  (CL:LET*
   ((DESCRIPTION (GET-DESCRIPTION (%OPERATOR SELF))) (COMPUTATION (LOOKUP-CONSTRAINT-FUNCTION DESCRIPTION))
   ...
   (CL:WHEN (CL:NOT (CL:EQ COMPUTATION NULL))
    ...
    (CL:WHEN (CL:NOT (CL:EQ COMPUTEDVALUE NULL))
     ...
     (CL:SETQ STOREDVALUE
      (VALUE-OF (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS SELF))) MISSINGVALUEINDEX)))
    ...
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-FUNCTION) (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-PREDICATE))
     (CL:LET* ((DESCRIPTION (GET-DESCRIPTION (%OPERATOR SELF))) (EVALUATOR (LOOKUP-EVALUATOR DESCRIPTION)))
      (CL:COND ((CL:NOT (CL:EQ EVALUATOR NULL)) (CL:FUNCALL EVALUATOR SELF))
       ((CL:EQ (%KIND SELF) KWD-PROPAGATE-PREDICATE) (EVALUATE-PREDICATE-PROPOSITION SELF))
    ...
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF) SGT-PROPAGATE-LOGIC-LOGIC-OBJECT)
     (CL:PROGN
      (CL:LET* ((ENVIRONMENT (GET-PROPAGATION-ENVIRONMENT *CONTEXT*)))
       (CL:WHEN (MEMBER? (%ELABORATED-OBJECTS ENVIRONMENT) SELF) (CL:RETURN-FROM ELABORATE-INSTANCE))
       ...
       (CL:LET*
        ...
        (CL:HANDLER-CASE
         ...
         (CLASH (E)
          (CL:LET* ((SELF-000 (NEW-EXCEPTION-RECORD))) (CL:SETF (%EXCEPTION SELF-000) E)
           (CL:SETF (%CONTEXT SELF-000) *CONTEXT*) (CL:SETF (%MODULE SELF-000) *MODULE*)
...
(CL:DEFUN EVALUATE-REACHABLE-INEQUALITIES (SELF VISITEDLIST)
  (CL:LET* ((P NULL) (ITER-000 (ALL-TRUE-DEPENDENT-PROPOSITIONS SELF SGT-PROPAGATE-PL-KERNEL-KB-INEQUALITY CL:T)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ P (%VALUE ITER-000)) (POST-FOR-EVALUATION P *CONTEXT*)
    (PUSH VISITEDLIST SELF)
    (CL:LET* ((ARG NULL) (VECTOR-000 (%ARGUMENTS P)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
     (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
     (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
      (CL:SETQ ARG (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
   ...
   (CL:LET* ((TEST-VALUE-000 (%KIND PROPOSITION)))
    (CL:COND
     ...
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-AND) (CL:EQ TEST-VALUE-000 KWD-PROPAGATE-OR)
      ...
      (CL:LET* ((P NULL) (VECTOR-001 (%ARGUMENTS PROPOSITION)) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-001 LENGTH-001))
       (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
        (CL:SETQ P (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001)) INDEX-001))
...
(CL:DEFUN POST-RELATED-FACTS (SELF ENVIRONMENT)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPAGATE-LOGIC-LOGIC-OBJECT)
     (CL:PROGN (CL:WHEN (MEMBER? (%ELABORATED-OBJECTS ENVIRONMENT) SELF) (CL:RETURN-FROM POST-RELATED-FACTS))
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPAGATE-LOGIC-PROPOSITION)
     (CL:PROGN
      (CL:WHEN (CL:NOT (CL:EQ (LOOKUP (%EVALUATION-STATES ENVIRONMENT) SELF) NULL))
    ...
    ((SUBTYPE-OF-CLASS? TEST-VALUE-000)
     (CL:PROGN
      (CL:WHEN (CL:NOT (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-DESCRIPTION NULL) NULL))
       (POST-RELATED-FACTS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-DESCRIPTION NULL)
        ENVIRONMENT))))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ...
    ((SUBTYPE-OF-CLASS? TEST-VALUE-000)
     (CL:PROGN
      (CL:WHEN (CL:NOT (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-DESCRIPTION NULL) NULL))
       (HELP-COLLECT-FACTS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PROPAGATE-LOGIC-DESCRIPTION NULL) FACTS
...
`fred' as well as a function term such as `(father fred)'."
  (CL:LET* ((INSTANCE (COERCE-TO-INSTANCE INSTANCEREF NULL)))
   (CL:IF (CL:NOT (CL:EQ INSTANCE NULL)) (%THE-CONS-LIST (ALL-FACTS-OF-INSTANCE INSTANCE CL:NIL CL:T)) NULL)))
...
on a separate line on the standard output stream."
  (CL:LET* ((INSTANCE (COERCE-TO-INSTANCE INSTANCEREF NULL)))
   (CL:LET*
    ((FACT NULL)
     (ITER-000
      (%THE-CONS-LIST (SORT (ALL-FACTS-OF-INSTANCE INSTANCE CL:NIL CL:NIL) (CL:FUNCTION PROPOSITION-LESS-THAN?)))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ FACT (%%VALUE ITER-000))
     (PRETTY-PRINT-LOGICAL-FORM FACT STANDARD-OUTPUT) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL)
     (CL:SETQ ITER-000 (%%REST ITER-000))))))
       ...
       (CL:LET* ((SKOLEM (LAST (%ARGUMENTS P)))) (CL:WHEN (CL:EQ SKOLEM SELF) (CL:GO :CONTINUE))
        (CL:WHEN
         (CL:AND (CL:NOT (CL:EQ SKOLEM NULL))
          (CL:NOT (CL:EQ (ACCESS-IN-CONTEXT (%VARIABLE-VALUE SKOLEM) (%HOME-CONTEXT SKOLEM) CL:NIL) NULL)))
     ...
     (CL:LET*
      ...
      (CL:WHEN (CL:NOT (CL:EQ EXPLODEDSTRING NULL))
       (CL:LET* ((TABLE (NEW-KEY-VALUE-LIST)) (SENTENCE "")) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SENTENCE))
        (CL:LET*
         ((VBL NULL) (ITER-000 (%THE-CONS-LIST (%IO-VARIABLE-NAMES RELATION))) (ARG NULL)
          (VECTOR-000 (%ARGUMENTS SELF)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
         (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
         (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:< INDEX-000 LENGTH-000)) DO
          (CL:SETQ VBL (%%VALUE ITER-000))
          (CL:SETQ ARG (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000)) (INSERT-AT TABLE VBL ARG)
           ...
           (CL:LET* ((TEST-VALUE-002 (SAFE-PRIMARY-TYPE ITEM)))
            (CL:COND
                ...
                (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE VALUE)))
                 (CL:COND
                  ((SUBTYPE-OF? TEST-VALUE-001 SGT-PROPAGATE-LOGIC-SKOLEM)
                   (CL:PROGN
                    (CL:LET*
                     ((SURROGATE (%SURROGATE-VALUE-INVERSE VALUE)) (DEFININGPROPOSITION (%DEFINING-PROPOSITION VALUE)))
                     (CL:COND
                  ...
                  ((SUBTYPE-OF? TEST-VALUE-001 SGT-PROPAGATE-LOGIC-LOGIC-OBJECT)
                   (CL:PROGN
                    (CL:LET* ((SURROGATE (%SURROGATE-VALUE-INVERSE VALUE)))
                     (CL:WHEN (CL:NOT (CL:EQ SURROGATE NULL)) (CL:SETQ STRINGVALUE (%SYMBOL-NAME SURROGATE))))))
                  ((SUBTYPE-OF-STRING? TEST-VALUE-001)
                   (CL:PROGN (CL:SETQ STRINGVALUE (CONCATENATE "'" (%WRAPPER-VALUE VALUE) "'"))))
                  ((SUBTYPE-OF-INTEGER? TEST-VALUE-001)
                   (CL:PROGN (CL:SETQ STRINGVALUE (INTEGER-TO-STRING (CL:TRUNCATE (%WRAPPER-VALUE VALUE))))))
                  ((SUBTYPE-OF-FLOAT? TEST-VALUE-001)
                   (CL:PROGN (CL:SETQ STRINGVALUE (FLOAT-TO-STRING (%WRAPPER-VALUE VALUE)))))
                  (CL:T))))
             ...
             ((SUBTYPE-OF-STRING? TEST-VALUE-002)
              (CL:PROGN (CL:SETQ SENTENCE (CONCATENATE SENTENCE (%WRAPPER-VALUE ITEM)))))
             (CL:T
              (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
...
run then in one where it has not (see `run-forward-rules').\")" (CL:FUNCTION %ALL-FACTS-OF-N)
   ...
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-PROPAGATE-LOGIC-STARTUP-PROPAGATE)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-PROPAGATE-STELLA-METHOD-STARTUP-CLASSNAME
