...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* STANDARD-ERROR NIL-LIST
  DEFAULT-FALSE-TRUTH-VALUE *TRACED-KEYWORDS* *INVISIBLEASSERTION?*
  *MODULE* DEFAULT-TRUE-TRUTH-VALUE STANDARD-OUTPUT EOL TRUE-WRAPPER
  FALSE-WRAPPER *CONTEXT*))
...
;;; (DEFUN HELP-COLLECT-FORWARD-RULES ...)
...
(CL:DEFUN COLLECT-FORWARD-CHAINING-RULES (DESCRIPTION)
  (CL:LET*
   ((RULES (NEW-KEY-VALUE-LIST)) (INDICES (NEW-KEY-VALUE-LIST))
    (BEENTHERE (NEW-LIST)))
   (HELP-COLLECT-FORWARD-RULES DESCRIPTION RULES INDICES FALSE BEENTHERE)
   (FREE BEENTHERE) (CL:VALUES RULES INDICES)))
   ...
   (CL:AND
    (CL:EQ
     (APPLICABLE-RULES-OF-DESCRIPTION DESCRIPTION KWD-PROPAGATE-FORWARD
      TRUE)
     NULL)
    (NULL-LIST? (FORWARD-CHAINING-INDICES DESCRIPTION)))
   FALSE
   ...
   (CL:LET* ((P NULL) (ITER-000 *COLLECTFORWARDPROPOSITIONS*))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (TRACE-KEYWORD? KWD-PROPAGATE-PROPAGATE)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "MODUS PONENS:"
       EOL)
     ...
     (CL:PROGN
      (CL:SETQ ARG
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
        INDEX-000))
      (CL:SETQ INDEX-000 (CL:1+ INDEX-000)))
     (CL:WHEN (DELETED? ARG) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
   (CL:WHEN FOUND?-000 (CL:RETURN-FROM APPLY-FORWARD-RULES-TO-VECTOR)))
  (CL:LET* ((RULES NULL) (INDICES NULL))
   (CL:MULTIPLE-VALUE-SETQ (RULES INDICES)
    (COLLECT-FORWARD-CHAINING-RULES TRIGGERDESCRIPTION))
   (CL:LET*
    ((TOUCHEDDEFAULT? NULL) (R NULL) (ITER-000 (%THE-KV-LIST RULES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
     (CL:SETQ R (%KEY ITER-000))
     (CL:PROGN (CL:SETQ TOUCHEDDEFAULT? (%VALUE ITER-000))
      (CL:SETQ ITER-000 (%REST ITER-000)))
   ...
   (CL:LET*
    ((TOUCHEDDEFAULT? NULL) (FWDINDEX NULL)
     (ITER-001 (%THE-KV-LIST INDICES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NULL)) DO
     (CL:SETQ FWDINDEX (%KEY ITER-001))
        ...
        (CL:LOOP WHILE
         ...
         (CL:TAGBODY
          (CL:COND
           ...
           ((CL:NOT (EQL? ARG (VALUE-OF GOALARG))) (CL:GO :CONTINUE)))
          :CONTINUE)))
       (CL:LET*
        ((ARG NULL) (IT (ALLOCATE-ITERATOR (%INPUT-BINDINGS FWDINDEX)))
         (ITER-002 (REVERSE INPUTARGS)))
        (CL:LOOP WHILE (CL:AND (NEXT? IT) (CL:NOT (CL:EQ ITER-002 NIL)))
         DO
      ...
      (CL:LET* ((OUTPUTBINDINGS NULL) (TRUTHVALUES NULL))
       (CL:MULTIPLE-VALUE-SETQ (OUTPUTBINDINGS TRUTHVALUES)
        (APPLY-CACHED-RETRIEVE (%IO-VARIABLES FWDINDEX)
         ...
         (%CACHE-ID FWDINDEX)))
       (CL:LET*
        ((TV NULL) (OB NULL) (ITER-003 OUTPUTBINDINGS)
         (ITER-004 TRUTHVALUES))
        (CL:LOOP WHILE
         (CL:AND (CL:NOT (CL:EQ ITER-003 NIL))
          (CL:NOT (CL:EQ ITER-004 NIL)))
         DO
         (CL:PROGN (CL:SETQ OB (%%VALUE ITER-003))
          (CL:SETQ ITER-003 (%%REST ITER-003)))
         (CL:PROGN (CL:SETQ TV (%%VALUE ITER-004))
...
(CL:DEFUN EVALUATE-FUNCTION-PROPOSITION (SELF)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ CODE NULL))
    (CL:LET*
     ((ARG NULL) (ITER-001 (BUT-LAST (%ARGUMENTS SELF)))
      (COLLECT-001 NULL))
...
(CL:DEFVAR *JUST-IN-TIME-FORWARD-INFERENCE?* TRUE
    ...
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF)
     ...
     (CL:PROGN
      ...
      (CL:WHEN
       ...
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) " ELABORATING:  "
        SELF " in world " *CONTEXT* EOL))
      ...
      (CL:LET*
       ((*EVALUATIONMODE* KWD-PROPAGATE-EXTENSIONAL-ASSERTION)
        (*INHIBITRELATIONFINALIZATION?* TRUE) (*QUERYITERATOR* NULL))
       (CL:DECLARE
        (CL:SPECIAL *EVALUATIONMODE* *INHIBITRELATIONFINALIZATION?*
         *QUERYITERATOR*))
       (CL:HANDLER-CASE
   ...
   (CL:IF (CL:NOT (CL:EQ INSTANCE NULL))
    (CL:LET* ((LIST NIL))
     (CL:LET*
      ((FACT NULL)
       (ITER-000
        (%THE-CONS-LIST (ALL-FACTS-OF-INSTANCE INSTANCE FALSE TRUE)))
       (COLLECT-000 NULL))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
having applicable forward rules.  Rules are triggered for each queue entry.\" :DEFAULT NULL :ALLOCATION :DYNAMIC)")
   (DEFINE-FUNCTION-OBJECT "HELP-COLLECT-FORWARD-RULES"
    "(DEFUN HELP-COLLECT-FORWARD-RULES ((DESCRIPTION DESCRIPTION) (RULES KEY-VALUE-LIST) (INDICES KEY-VALUE-LIST) (TOUCHEDDEFAULT? BOOLEAN) (BEENTHERE LIST)))"
...
attached to `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-FACTS-OF-INSTANCE)
   ...
   (DEFINE-FUNCTION-OBJECT "ALL-FACTS-OF"
    "(DEFUN (ALL-FACTS-OF (LIST OF PROPOSITION)) ((INSTANCEREF OBJECT)) :COMMAND? TRUE :PUBLIC? TRUE :EVALUATE-ARGUMENTS? FALSE :DOCUMENTATION \"Return a list of all definite (TRUE or FALSE) propositions
that reference the instance `instanceRef'.\")" (CL:FUNCTION %ALL-FACTS-OF)
    NULL)
...
that reference the instance `instanceRef'.\")"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Propagate") NULL-STRING-WRAPPER))))
