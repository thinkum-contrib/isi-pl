...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* EOL
  *PRINTFUNCTIONSASRELATIONS?* FALSE-WRAPPER *CURRENTJUSTIFICATION* NIL
  *SKOLEMNAMEMAPPINGTABLE* SYSTEM-DEFINED-ARGUMENT-NAMES NULL-INTEGER))
...
(CL:DEFUN GENERATE-ONE-VARIABLE (SELF TYPED?)
  (CL:LET* ((VALUE NULL))
   ...
   (CL:WHEN (CL:EQ VALUE NULL)
    (CL:SETQ VALUE (SAFE-ARGUMENT-BOUND-TO SELF)))
   (CL:IF
    (CL:AND (CL:NOT (CL:EQ VALUE NULL)) (CL:NOT (CL:EQ VALUE SELF)))
    (GENERATE-TERM VALUE)
  ...
  (CL:LET* ((ARGS (%ARGUMENTS SELF)) (ARGUMENTS NIL))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:- (LENGTH ARGS) 2)) (COLLECT-000 NULL))
...
(CL:DEFUN GENERATE-FUNCTION-PROPOSITION (SELF)
  (CL:LET*
   ...
   (CL:IF *PRINTFUNCTIONSASRELATIONS?*
    (CONCATENATE RESULT (CONS LASTARG NIL))
