...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* EOL *PRINTFUNCTIONSASRELATIONS?*
  FALSE-WRAPPER *CURRENTJUSTIFICATION* NIL *SKOLEMNAMEMAPPINGTABLE*
...
(CL:DEFUN GENERATE-EXPRESSION (SELF CANONICALIZEVARIABLENAMES?)
  ...
  (CL:LET*
   ...
   (CL:DECLARE
    (CL:SPECIAL *CANONICALVARIABLENAMEMAPPING* *CANONICALVARIABLECOUNTER*))
...
(CL:DEFUN GENERATE-NAME-OF-VARIABLE (SELF)
  ...
  (CL:WHEN (CL:EQ *CANONICALVARIABLENAMEMAPPING* NULL)
   (CL:RETURN-FROM GENERATE-NAME-OF-VARIABLE (%SKOLEM-NAME SELF)))
...
(CL:DEFUN GENERATE-ONE-VARIABLE (SELF TYPED?)
  (CL:LET* ((VALUE NULL))
   (CL:WHEN (CL:NOT (CL:EQ *CURRENTJUSTIFICATION* NULL))
    (CL:SETQ VALUE (JUSTIFICATION-ARGUMENT-BOUND-TO SELF NULL)))
   (CL:WHEN (CL:EQ VALUE NULL) (CL:SETQ VALUE (SAFE-ARGUMENT-BOUND-TO SELF)))
   (CL:IF (CL:AND (CL:NOT (CL:EQ VALUE NULL)) (CL:NOT (CL:EQ VALUE SELF)))
    (GENERATE-TERM VALUE)
  ...
  (CL:COND
   ...
   (CL:T
    (LIST* SYM-GENERATE-LOGIC-KAPPA
     (GENERATE-VARIABLES (%IO-VARIABLES SELF) CL:T)
      ...
      (CL:PROGN
       (CL:LET*
        ((DESCRIPTION (GET-DESCRIPTION OPERATOR))
         (SYMBOL
          (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME OPERATOR)
           (%HOME-CONTEXT OPERATOR) CL:NIL)))
   ...
   (CL:COND
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-GENERATE-PREDICATE)
     ...
     (CL:LET* ((OPERATOR (GENERATE-OPERATOR SELF)))
      (CONS OPERATOR
       (CONCATENATE (GENERATE-ARGUMENTS (%ARGUMENTS SELF)) NIL))))
    ((CL:EQ TEST-VALUE-000 KWD-GENERATE-FUNCTION)
     (GENERATE-FUNCTION-PROPOSITION SELF))
...
(CL:DEFUN GENERATE-FUNCTION-AS-TERM (SELF)
  (CL:LET* ((ARGS (%ARGUMENTS SELF)) (ARGUMENTS NIL))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:- (LENGTH ARGS) 2))
     (COLLECT-000 NULL))
...
(CL:DEFUN GENERATE-FUNCTION-PROPOSITION (SELF)
  (CL:LET*
   ((RESULT (GENERATE-FUNCTION-AS-TERM SELF))
    (LASTARG (LAST (%ARGUMENTS SELF))) (LASTARGVALUE (INNERMOST-OF LASTARG)))
   (CL:IF
    (CL:AND (CL:NOT (CL:EQ LASTARGVALUE NULL))
     (CL:NOT (EQL? LASTARGVALUE LASTARG)))
    (CL:SETQ LASTARG (GENERATE-TERM LASTARGVALUE))
...
(CL:DEFUN GENERATE-SKOLEM (SELF)
  (CL:COND
   ...
   ((CL:NOT (CL:EQ (%DEFINING-PROPOSITION SELF) NULL))
    (GENERATE-FUNCTION-AS-TERM (%DEFINING-PROPOSITION SELF)))
  ...
  (CL:IF (CL:NOT (CL:EQ (%SURROGATE-VALUE-INVERSE SELF) NULL))
   ...
   (CL:ERROR
    (NEW-TERM-GENERATION-EXCEPTION SELF
     (CONCATENATE "DON'T KNOW HOW TO GENERATE EXPRESSION FOR THE THING: "
      (STRINGIFY SELF))))))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000 (CONS (GENERATE-ONE-VARIABLE VAR CL:T) NIL))
   ...
   (CL:LET*
    ((*SKOLEMNAMEMAPPINGTABLE*
      (CL:IF MAPHEADVARIABLES?
       (CREATE-SKOLEM-MAPPING-TABLE (%IO-VARIABLES HEAD) (%IO-VARIABLES TAIL))
       NULL)))
   ...
   (CL:LET*
    ((*SKOLEMNAMEMAPPINGTABLE*
      (CL:IF (CL:NOT MAPHEADVARIABLES?)
       (CREATE-SKOLEM-MAPPING-TABLE (%IO-VARIABLES TAIL) (%IO-VARIABLES HEAD))
       NULL)))
    (CL:DECLARE (CL:SPECIAL *SKOLEMNAMEMAPPINGTABLE*))
    (CL:SETQ TAILPROP
         ...
         (CL:IF (CL:EQ COLLECT-000 NULL)
          (CL:PROGN (CL:SETQ COLLECT-000 (CONS (DEOBJECTIFY-TREE ELT) NIL))
           (CL:IF (CL:EQ RESULT NIL) (CL:SETQ RESULT COLLECT-000)
            (ADD-CONS-TO-END-OF-CONS-LIST RESULT COLLECT-000)))
          (CL:PROGN
           (CL:SETF (%%REST COLLECT-000) (CONS (DEOBJECTIFY-TREE ELT) NIL))
           (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
         (CL:SETQ ITER-000 (%%REST ITER-000))))
       RESULT)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-GENERATE-LOGIC-PROPOSITION)
...
(CL:DEFUN STARTUP-GENERATE ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *CANONICALVARIABLECOUNTER* NULL-INTEGER))
