...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* FALSE-WRAPPER EOL TRUE FALSE
  ...
  (CL:COND
   ...
   (CL:T
    (LIST* SYM-GENERATE-LOGIC-KAPPA
     (GENERATE-VARIABLES (%IO-VARIABLES SELF) TRUE)
     (CONS (GENERATE-DESCRIPTION-PROPOSITION SELF FALSE) NIL)))))
...
(CL:DEFUN GENERATE-FUNCTION-AS-TERM (SELF)
  (CL:LET* ((ARGUMENTS NIL))
   (CL:LET*
    ((ARG NULL) (ITER-000 (BUT-LAST (%ARGUMENTS SELF))) (COLLECT-000 NULL))
...
(CL:DEFUN GENERATE-DESCRIPTION-PROPOSITION (SELF INVERT?)
  (CL:LET*
   ...
   (CL:LET* ((I NULL-INTEGER) (VAR NULL) (ITER-000 EXISTENTALS) (ITER-001 0))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (NTH-SETTER EXISTENTALS (GENERATE-ONE-VARIABLE VAR TRUE) I)))
...
(CL:DEFUN GENERATE-DESCRIPTIONS-AS-RULE (HEAD TAIL RULE REVERSEPOLARITY?)
  (CL:LET*
   ((FORWARDARROW?
    ...
    (ARROW (TYPE-TO-SYMBOL (CHOOSE-IMPLICATION-OPERATOR RULE FORWARDARROW?)))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((CL:EQ TEST-VALUE-000 SGT-GENERATE-STELLA-CONS)
     (CL:PROGN
      (CL:IF (CL:EQ SELF NIL) NIL
       (CONS (DEOBJECTIFY-TREE (%%VALUE SELF))
        (DEOBJECTIFY-TREE (%%REST SELF))))))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-GENERATE-LOGIC-PROPOSITION)
...
convert the result of a retrieval query into a regular parse tree.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Generate") NULL-STRING-WRAPPER)))
