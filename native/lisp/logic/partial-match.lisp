...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *ACTIVATED-NETWORKS* *MOST-RECENT-QUERY*
  ...
  TRUE-TRUTH-VALUE NULL-INTEGER TRUE UNKNOWN-TRUTH-VALUE NIL *QUERYITERATOR*
  NULL-FLOAT-WRAPPER STANDARD-OUTPUT EOL TRUE-WRAPPER FALSE-WRAPPER NULL-FLOAT
  FALSE))
...
(CL:DEFVAR *GREEDY-NETWORK-PRUNING* FALSE)
...
(CL:DEFCLASS PARTIAL-MATCH-FRAME (STANDARD-OBJECT)
  ((KIND :ALLOCATION :INSTANCE :ACCESSOR %KIND)
   ...
   (NEGATIVE-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %NEGATIVE-SCORE)
   (DYNAMIC-CUTOFF :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %DYNAMIC-CUTOFF)
...
(CL:DEFMETHOD DEFINED-FRAME-PARTIAL-TRUTH? ((SELF CONTROL-FRAME))
  (CL:AND (CL:NOT (CL:EQ (%PARTIAL-MATCH-FRAME SELF) NULL))
   (CL:OR
    (CL:NOT (CL:= (%POSITIVE-SCORE (%PARTIAL-MATCH-FRAME SELF)) NULL-FLOAT))
...
(CL:DEFMETHOD PROPAGATE-FRAME-PARTIAL-TRUTH ((SOURCE PARTIAL-MATCH-FRAME) TARGET)
  (CL:LET*
     ...
     (FLIPPOLARITY?
      (CL:SETQ TARGETPMF
       (CREATE-OBJECT (PRIMARY-TYPE SOURCE) KWD-PARTIAL-MATCH-CONTROL-FRAME
        TARGET KWD-PARTIAL-MATCH-KIND (%STATE TARGET)))
   ...
   (CL:COND
    (FLIPPOLARITY?
     (CL:SETF (%POSITIVE-SCORE TARGETPMF) (CL:- 0 (%POSITIVE-SCORE SOURCE)))
...
(CL:DEFMETHOD TRACE-FRAME-PARTIAL-TRUTH ((SELF PARTIAL-MATCH-FRAME) LASTMOVE STREAM)
  (CL:LET*
   ((WEIGHT (PROPOSITION-WEIGHT SELF (%PROPOSITION (%CONTROL-FRAME SELF)))))
   (CL:DECLARE (CL:TYPE CL:DOUBLE-FLOAT WEIGHT))
   (CL:COND
...
(CL:DEFCLASS INCREMENTAL-PARTIAL-MATCH (PARTIAL-MATCH-FRAME)
  ((ACCUMULATED-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %ACCUMULATED-SCORE)
   (ACCUMULATED-WEIGHT :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %ACCUMULATED-WEIGHT)
   (TOTAL-WEIGHT :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %TOTAL-WEIGHT)
   (MAXIMUM-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %MAXIMUM-SCORE)))
...
(CL:DEFUN NEW-INCREMENTAL-PARTIAL-MATCH (KIND CONTROL-FRAME)
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%PARENT SELF) NULL) (INITIALIZE-INCREMENTAL-PARTIAL-MATCH SELF)
   SELF))
   ...
   (CL:WHEN (CL:NOT (CL:EQ PARENT NULL))
    ...
    (CL:COND
     ((CL:AND (CL:< CUTOFF 0.0d0)
      ...
      (CL:SETQ CUTOFF 0.0d0))
     ((CL:AND (CL:> CUTOFF 0.0d0) (%REVERSE-POLARITY? (%CONTROL-FRAME SELF)))
      (CL:SETQ CUTOFF 0.0d0))
     ((CL:NOT
...
(CL:DEFMETHOD POP-OR-PARTIAL-MATCH-SCORE ((SELF INCREMENTAL-PARTIAL-MATCH))
  (CL:LET*
   ...
   (CL:WHEN (CL:= LATESTSCORE MAXIMUMSCORE) (CL:SETQ MAXIMUMSCORE 0.0d0)
    (CL:LET*
     ((WEIGHT NULL) (SCORE NULL) (ITER-000 (%ARGUMENT-SCORES SELF))
     ...
     (CL:LOOP WHILE
      ...
      (CL:PROGN (CL:SETQ SCORE (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:PROGN (CL:SETQ WEIGHT (%%VALUE ITER-001))
  ...
  (CL:LET* ((FRAME (%CONTROL-FRAME SELF)))
   (CL:COND
    ...
    ((CL:OR (CL:EQ LASTMOVE KWD-PARTIAL-MATCH-UP-TRUE)
      (CL:EQ LASTMOVE KWD-PARTIAL-MATCH-UP-FAIL))
     (CL:LET*
      ((RESULT (%RESULT FRAME)) (RESULTPMF (%PARTIAL-MATCH-FRAME RESULT)))
...
(CL:DEFMETHOD CONTINUE-PARTIAL-ANTECEDENTS-PROOF ((SELF INCREMENTAL-PARTIAL-MATCH) LASTMOVE)
  (CL:LET*
   ((FRAME (%CONTROL-FRAME SELF)) (GOAL (EXTRACT-SUBGOAL-OF-FRAME FRAME))
    (SCORE NULL-FLOAT) (RESULT NULL))
   (CL:DECLARE (CL:TYPE CL:DOUBLE-FLOAT SCORE))
   (CL:COND
    ((CL:EQ LASTMOVE KWD-PARTIAL-MATCH-DOWN) (SET-DYNAMIC-CUTOFF SELF))
    ((CL:OR (CL:EQ LASTMOVE KWD-PARTIAL-MATCH-UP-TRUE)
   ...
   (CL:WHEN
    ...
    (CL:LET*
     ((R
      ...
      (ANTE
       (%PROPOSITION
        (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS R))) 0))))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE
     ...
     (CL:PROGN
      ...
      (CL:SETQ INDEX-000 (CL:1+ INDEX-000)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ARG)
      SYM-PARTIAL-MATCH-LOGIC-MATCH-SCORE (WRAP-FLOAT (%WRAPPER-VALUE SCORE))
      NULL-FLOAT-WRAPPER)))
      ...
      (CL:LET* ((TEMP-000 (HOME-MODULE MATCHMODE)))
       (CL:LET*
        ...
        (CL:IF
         ...
         (CREATE-OBJECT (%CLASS-TYPE CLASS) KWD-PARTIAL-MATCH-CONTROL-FRAME
          FRAME KWD-PARTIAL-MATCH-KIND KIND)
         (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
  ...
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND (CL:EQ (%KIND PFRAME) KWD-PARTIAL-MATCH-AND) (CL:= SCORE 0.0d0)
     ...
     (CL:AND
      (ISA? (LAST (%ARGUMENTS PROP)) SGT-PARTIAL-MATCH-LOGIC-PATTERN-VARIABLE)
...
(CL:DEFUN COMPUTE-PARTIAL-MATCH-SCORE? (FRAME)
  (CL:LET*
   ...
   (CL:IF (UNBOUND-VARIABLES-IN-FRAME? FRAME) FALSE
...
(CL:DEFMETHOD RETRIEVE-PARTIAL-SOLUTIONS ((SELF INCREMENTAL-PARTIAL-MATCH) QUERY)
  (CL:LET*
   ...
   (CL:LET*
    ((*QUERYITERATOR* QUERY) (*REVERSEPOLARITY?* FALSE)
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:DOUBLE-FLOAT) ASK-PARTIAL))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((*QUERYITERATOR* QUERY) (*GENERATE-ALL-PROOFS?* TRUE)
     (*INFERENCELEVEL* (CURRENT-INFERENCE-LEVEL)) (*REVERSEPOLARITY?* FALSE))
    (CL:DECLARE
...
(CL:DEFMETHOD COMPUTE-PARTIAL-TRUTH ((SELF NN-PARTIAL-MATCH) QUERY)
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ SUPPORT NULL))
    ...
    (CL:LET*
     ((SCORELIST NULL) (NET NULL)
      (ITER-001 (%THE-CONS-LIST (REVERSE NETLIST)))
     ...
     (CL:LOOP WHILE
      ...
      (CL:PROGN (CL:SETQ NET (%%VALUE ITER-001))
       (CL:SETQ ITER-001 (%%REST ITER-001)))
      (CL:PROGN (CL:SETQ SCORELIST (%%VALUE ITER-002))
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE
        ...
        (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROP)
         SYM-PARTIAL-MATCH-LOGIC-MATCH-SCORE (WRAP-FLOAT (%WRAPPER-VALUE K))
         NULL-FLOAT-WRAPPER))))))
   HIGHESTSCORE))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ ITERATOR NULL))
    (CL:LOOP WHILE (NEXT? ITERATOR) DO
     (CL:WHEN
      (PARTIAL-ARGUMENTS-UNIFY-WITH-ARGUMENTS? (%VALUE ITERATOR) PROPOSITION)
   ...
   (CL:WHEN (CL:NOT (CL:EQ RESULT KWD-PARTIAL-MATCH-FAILURE))
    (CL:LET*
     ...
     (SET-FRAME-PARTIAL-TRUTH (%PARTIAL-MATCH-FRAME FRAME) UNKNOWN-TRUTH-VALUE
      (CL:* (%POSITIVE-SCORE TRUTHVALUE) WEIGHT) NULL-FLOAT TRUE)))
   RESULT))
...
(CL:DEFUN HELP-STARTUP-PARTIAL-MATCH1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-PARTIAL-MATCH-STELLA-ARITY
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ARITY" (GET-STELLA-MODULE "/STELLA" TRUE)
     0))
...
(CL:DEFUN HELP-STARTUP-PARTIAL-MATCH3 ()
  (CL:PROGN (CL:SETQ *PARTIAL-MATCH-MODE* KWD-PARTIAL-MATCH-BASIC)
   (CL:SETQ *GREEDY-NETWORK-PRUNING* TRUE)
   (CL:SETQ *RULE-COMBINATION* KWD-PARTIAL-MATCH-MAX)
   (CL:SETQ *PLANNING-MODE* FALSE) (CL:SETQ *PLANABLE-PREDICATES* NIL)
   (CL:SETQ *FAIL-UNBOUND-CLAUSES?* FALSE)
   (CL:SETQ *AND-MISSED-A-BINDING* FALSE)))
...
(CL:DEFUN HELP-STARTUP-PARTIAL-MATCH5 ()
  (CL:PROGN
   (DEFINE-METHOD-OBJECT
...
has been generated.\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "_Startup-Partial-Match") NULL-STRING-WRAPPER))))
...
(CL:DEFUN STARTUP-PARTIAL-MATCH ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (HELP-STARTUP-PARTIAL-MATCH3))
...
user-defined partial match implementations.\" :ABSTRACT? TRUE :SLOTS ((KIND :TYPE KEYWORD :REQUIRED? TRUE) (CONTROL-FRAME :TYPE CONTROL-FRAME :REQUIRED? TRUE) (PARENT :TYPE PARTIAL-MATCH-FRAME) (CHILD :TYPE PARTIAL-MATCH-FRAME) (POSITIVE-SCORE :TYPE PARTIAL-MATCH-SCORE :INITIALLY NULL) (NEGATIVE-SCORE :TYPE PARTIAL-MATCH-SCORE :INITIALLY NULL) (DYNAMIC-CUTOFF :TYPE FLOAT) (ARGUMENT-SCORES :TYPE (CONS OF FLOAT-WRAPPER) :INITIALLY NIL) (ARGUMENT-WEIGHTS :TYPE (CONS OF FLOAT-WRAPPER) :INITIALLY NIL) (UNBOUND-VARS :TYPE CONS :INITIALLY NIL) (SUCCESS? :TYPE BOOLEAN :INITIALLY FALSE)))")))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-PARTIAL-MATCH4)
    (HELP-STARTUP-PARTIAL-MATCH5))
