...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* *LOADINGREGENERABLEOBJECTS?*
  FALSE-WRAPPER *REVERSEPOLARITY?* *CURRENTJUSTIFICATION* *PRINTINFRAME*
  *PRINTREADABLY?* FALSE TRUE STANDARD-OUTPUT))
...
(CL:DEFUN PRINT-LOGIC-OBJECT (SELF STREAM TOPLEVEL?)
  (CL:LET*
   ((PREFIX (CL:IF (CL:AND TOPLEVEL? (CL:NOT *PRINTREADABLY?*)) "|i|" "")))
      ...
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE CURSOR)))
       (CL:COND
        ((SUBTYPE-OF? TEST-VALUE-000 SGT-PRINT-LOGIC-SKOLEM)
         (CL:PROGN
          (CL:SETQ NEXTVALUE
           (ACCESS-IN-CONTEXT (%VARIABLE-VALUE CURSOR) (%HOME-CONTEXT CURSOR)
            FALSE))))
        ((SUBTYPE-OF? TEST-VALUE-000 SGT-PRINT-LOGIC-LOGIC-OBJECT)
...
(CL:DEFUN PRINT-VARIABLE (SELF STREAM)
  (CL:WHEN
   ...
   (PRETTY-PRINT-LOGICAL-FORM (JUSTIFICATION-ARGUMENT-BOUND-TO SELF NULL)
    STREAM)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ VALUE NULL)) (PRINT-VARIABLE-NAME SELF STREAM)
    ...
    (CL:LET* ((*PRINTREADABLY?* TRUE))
     ...
     (PRINT-VARIABLE-VALUE VALUE STREAM) (CL:RETURN-FROM PRINT-VARIABLE))))
  (PRINT-VALUE-OF-CHAIN SELF STREAM (INNERMOST-OF SELF)))
  ...
  (CL:LET* ((PREFIX STELLA::NULL-STRING))
   ...
   (CL:WHEN (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN (UNFASTENED? SELF))
    (CL:SETQ PREFIX
     (CL:IF
      (%WRAPPER-VALUE
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-PRINT-LOGIC-DESCRIPTIVE?
        FALSE-WRAPPER))
      "|uPd|" "|uP|")))
   (CL:WHEN
    (CL:AND
...
(CL:DEFUN STARTUP-PRINT ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *PRINTFUNCTIONSASRELATIONS?* FALSE)
    (CL:SETQ *DEBUG-MODE?* FALSE))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-PRINT-LOGIC-STARTUP-PRINT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-PRINT-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_Startup-Print")
      NULL-STRING-WRAPPER)))
