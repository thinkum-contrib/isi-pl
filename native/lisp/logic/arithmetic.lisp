...
(CL:DEFUN |/PL-KERNEL-KB/NEW-INTERVAL-CACHE| ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%UPPER-BOUND SELF) NULL) (CL:SETF (%LOWER-BOUND SELF) NULL)
   (CL:SETF (%INTERVAL-MEMBER SELF) NULL) (CL:SETF (%HOME-CONTEXT SELF) NULL)
   SELF))
...
")))
   (CL:LET* ((MESSAGE MESSAGE-000) (CLASH (NEW-INTERVAL-CLASH MESSAGE)))
    ...
    (CL:SETF (%STRICT-UPPER-BOUND? CLASH) (%STRICT-UPPER-BOUND? INTERVAL))
    (CL:ERROR CLASH))))
...
(CL:DEFMETHOD |/PL-KERNEL-KB/EVALUATE-TIGHTER-INTEGER-INTERVAL| ((INTERVAL |/PL-KERNEL-KB/INTERVAL-CACHE|))
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ LB NULL)) (CL:NOT (CL:EQ UB NULL)) (EQL? LB UB))
...
(CL:DEFMETHOD |/PL-KERNEL-KB/EVALUATE-TIGHTER-INTERVAL| ((INTERVAL |/PL-KERNEL-KB/INTERVAL-CACHE|))
  (CL:WHEN
   ...
   (CL:IF (EQL? (%LOWER-BOUND INTERVAL) (%UPPER-BOUND INTERVAL))
    (CL:IF
     (CL:OR (%STRICT-LOWER-BOUND? INTERVAL) (%STRICT-UPPER-BOUND? INTERVAL))
     ...
     (CL:PROGN
      (CL:WHEN
       (|/PL-KERNEL-KB/COMPARE-INTERVAL-BOUNDS?|
        |/PL-KERNEL-KB/SGT-ARITHMETIC-PL-KERNEL-KB->| (%LOWER-BOUND INTERVAL)
       ...
       (CL:COND
        ...
        (CL:T
         (CL:LET* ((STREAM-005 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-005) "`" TEST-VALUE-001
           "' is not a valid case option")
          (CL:ERROR
...
(CL:DEFUN |/PL-KERNEL-KB/LESS-SPECIALIST-HELPER| (FRAME RELATION XARG YARG)
  (CL:LET*
   ((X (ARGUMENT-BOUND-TO XARG)) (Y (ARGUMENT-BOUND-TO YARG)) (XBOUND NULL)
    (STRICTX? CL:NIL) (YBOUND NULL) (STRICTY? CL:NIL) (SUCCESS? CL:NIL))
   ...
   (CL:IF SKOLEM1?
    ...
    (CL:IF SKOLEM2?
     (|/PL-KERNEL-KB/PROPAGATE-INEQUALITY-TO-INTERVAL-CACHE|
      ...
      (|/PL-KERNEL-KB/INVERSE-INEQUALITY-OPERATOR| OPERATOR))
     (CL:COND
      ((|/PL-KERNEL-KB/COMPARE-INTERVAL-BOUNDS?| OPERATOR VALUE1 VALUE2)
    ...
    (CL:LET*
     ...
     (CL:WHEN (CL:NOT (CL:EQ P2ARGVALUE NULL))
      (CL:IF (INTEGER? P2ARGVALUE) (CL:SETQ P2 (%WRAPPER-VALUE P2ARGVALUE))
       (CL:RETURN-FROM |/PL-KERNEL-KB/SUBSEQUENCE-SPECIALIST|
        |/PL-KERNEL-KB/KWD-ARITHMETIC-TERMINAL-FAILURE|))
      (CL:WHEN (CL:= P2 -1) (CL:SETQ P2 SUPERLENGTH))
      (CL:WHEN
       ...
       (CL:IF (CL:= P2 NULL-INTEGER)
        ...
        (CL:COND
         ...
         ((STRING-EQL? (SUBSEQUENCE SUPER (CL:- P2 SUBLENGTH) P2) SUB)
          (CL:RETURN-FROM |/PL-KERNEL-KB/SUBSEQUENCE-SPECIALIST|
           (SELECT-PROOF-RESULT
            (BIND-ARGUMENT-TO-VALUE? P1ARG (WRAP-INTEGER (CL:- P2 SUBLENGTH))
             CL:T)
            CL:NIL CL:T)))
         (CL:T
          (CL:RETURN-FROM |/PL-KERNEL-KB/SUBSEQUENCE-SPECIALIST|
      ...
      ((CL:= P2 NULL-INTEGER)
       (CL:COND
        ...
        ((STRING-EQL? SUB (SUBSEQUENCE SUPER P1 (CL:+ P1 SUBLENGTH)))
         (CL:RETURN-FROM |/PL-KERNEL-KB/SUBSEQUENCE-SPECIALIST|
          (SELECT-PROOF-RESULT
           (BIND-ARGUMENT-TO-VALUE? P2ARG (WRAP-INTEGER (CL:+ P1 SUBLENGTH))
            CL:T)
           CL:NIL CL:T)))
        (CL:T
         (CL:RETURN-FROM |/PL-KERNEL-KB/SUBSEQUENCE-SPECIALIST|
...
(CL:DEFUN |/PL-KERNEL-KB/STRING-MATCH-COMPUTATION-HELPER| (PATTERN X START END IGNORE-CASE?)
  (CL:WHEN (CL:NOT (CL:AND (STRING? PATTERN) (INTEGER? START) (INTEGER? END)))
   (CL:RETURN-FROM |/PL-KERNEL-KB/STRING-MATCH-COMPUTATION-HELPER| NULL))
  (CL:LET*
   ((THEPATTERN (%WRAPPER-VALUE PATTERN)) (THESTART (%WRAPPER-VALUE START))
    ...
    (NAMELENGTH (CL:THE CL:FIXNUM (CL:LENGTH NAME)))
    (MATCHPOSITION NULL-INTEGER))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING THEPATTERN NAME)
    (CL:TYPE CL:FIXNUM THESTART THEEND NAMELENGTH MATCHPOSITION))
   ...
   (CL:WHEN (CL:OR (CL:< THEEND 0) (CL:> THEEND NAMELENGTH))
    (CL:RETURN-FROM |/PL-KERNEL-KB/STRING-MATCH-COMPUTATION-HELPER| NULL))
   (CL:IF IGNORE-CASE?
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "STRING-MATCH-COMPUTATION-HELPER"
     "(DEFUN (STRING-MATCH-COMPUTATION-HELPER INTEGER-WRAPPER) ((PATTERN OBJECT) (X OBJECT) (START OBJECT) (END OBJECT) (IGNORE-CASE? BOOLEAN)) :PUBLIC? FALSE)"
