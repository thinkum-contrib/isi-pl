...
(CL:DEFVAR *POWERLOOM-PATCH-LEVEL* 7)
...
Useful when reporting problems."
  (CL:LET* ((SS (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM SS) *POWERLOOM-VERSION-STRING* EOL (STELLA-INFORMATION)) (THE-STRING-READER SS)))
...
(CL:DEFUN POWERLOOM-COPYRIGHT-HEADER ()
  (CL:LET*
   ((LOGICSYSTEMFILE (MAKE-SYSTEM-DEFINITION-FILE-NAME "logic"))
    (LOGICSYSTEM (CL:IF (PROBE-FILE? LOGICSYSTEMFILE) (GET-SYSTEM-DEFINITION "logic") NULL))
    (SUBSTITUTION-LIST (NEW-KEY-VALUE-LIST)))
   (FILL-IN-DATE-SUBSTITUTION SUBSTITUTION-LIST)
   (CL:IF (CL:NOT (CL:EQ LOGICSYSTEM NULL))
    (SUBSTITUTE-TEMPLATE-VARIABLES-IN-STRING (%BANNER LOGICSYSTEM) SUBSTITUTION-LIST)
    "---------------------------- BEGIN LICENSE BLOCK ---------------------------+
 |                                                                            |
 | Version: MPL 1.1/GPL 2.0/LGPL 2.1                                          |
 |                                                                            |
...
(CL:DEFUN COPYRIGHT-YEARS ()
  (CL:LET*
   ((COPYRIGHT (POWERLOOM-COPYRIGHT-HEADER)) (START (STRING-SEARCH COPYRIGHT "1997" 0))
...
entered `bye', `exit', `halt', `quit', or `stop'."
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "    Welcome to " *POWERLOOM-VERSION-STRING* EOL EOL
   "Copyright (C) USC Information Sciences Institute, " (COPYRIGHT-YEARS) "." EOL
   "PowerLoom is a trademark of the University of Southern California." EOL
   "PowerLoom comes with ABSOLUTELY NO WARRANTY!" EOL "Type `(copyright)' for detailed copyright information." EOL
   "Type `(help)' for a list of available commands." EOL "Type `(demo)' for a list of example applications." EOL
   "Type `bye', `exit', `halt', `quit', or `stop', to exit." EOL EOL)
  (CL:HANDLER-CASE
   (CL:PROGN (LOGIC-COMMAND-LOOP NULL) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Bye." EOL EOL))
   (CL:CONDITION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) "Caught native non-STELLA exception " E " at top level." EOL)
    (PRINT-EXCEPTION-CONTEXT E STANDARD-ERROR)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) "Exiting PowerLoom." EOL EOL))))

(CL:DEFUN STARTUP-POWERLOOM ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/LOGIC" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-POWERLOOM-RELEASE (INTERN-RIGID-SYMBOL-WRT-MODULE "RELEASE" NULL 2))
    (CL:SETQ KWD-POWERLOOM-LOG-LEVELS (INTERN-RIGID-SYMBOL-WRT-MODULE "LOG-LEVELS" NULL 2))
    (CL:SETQ KWD-POWERLOOM-LEVEL (INTERN-RIGID-SYMBOL-WRT-MODULE "LEVEL" NULL 2))
    (CL:SETQ KWD-POWERLOOM-DEVELOPMENT (INTERN-RIGID-SYMBOL-WRT-MODULE "DEVELOPMENT" NULL 2))
    (CL:SETQ KWD-POWERLOOM-MEDIUM (INTERN-RIGID-SYMBOL-WRT-MODULE "MEDIUM" NULL 2))
    (CL:SETQ KWD-POWERLOOM-LOW (INTERN-RIGID-SYMBOL-WRT-MODULE "LOW" NULL 2))
    (CL:SETQ KWD-POWERLOOM-PREFIX (INTERN-RIGID-SYMBOL-WRT-MODULE "PREFIX" NULL 2))
    (CL:SETQ KWD-POWERLOOM-MAX-WIDTH (INTERN-RIGID-SYMBOL-WRT-MODULE "MAX-WIDTH" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *POWERLOOM-VERSION-STRING*
     (CONCATENATE "PowerLoom " (INTEGER-TO-STRING (CL:TRUNCATE *POWERLOOM-MAJOR-VERSION-NUMBER*)) "."
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "PL-LOG" "(DEFUN PL-LOG ((LOGLEVEL KEYWORD) |&REST| (MESSAGE OBJECT)))"
...
Useful when reporting problems.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION POWERLOOM-INFORMATION)
     (CL:FUNCTION POWERLOOM-INFORMATION-EVALUATOR-WRAPPER))
    (DEFINE-FUNCTION-OBJECT "POWERLOOM-COPYRIGHT-HEADER" "(DEFUN (POWERLOOM-COPYRIGHT-HEADER STRING) ())"
...
entered `bye', `exit', `halt', `quit', or `stop'.\" :PUBLIC? TRUE)" (CL:FUNCTION POWERLOOM) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-POWERLOOM" "(DEFUN STARTUP-POWERLOOM () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-POWERLOOM) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-POWERLOOM-LOGIC-STARTUP-POWERLOOM)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-POWERLOOM-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "LOGIC")))
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *POWERLOOM-RELEASE-STATE* STRING \".beta\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *POWERLOOM-PATCH-LEVEL* INTEGER 7)")
...
whether certain internal error and warning messages are surfaced to the user.\")")
    (%SET-LOGGING-PARAMETERS "PowerLoom"
     (CONS-LIST KWD-POWERLOOM-LOG-LEVELS (GET-QUOTED-TREE "((:NONE :LOW :MEDIUM :HIGH) \"/LOGIC\")" "/LOGIC")
      KWD-POWERLOOM-LEVEL
      (CL:IF (CL:EQ *POWERLOOM-EXECUTION-MODE* KWD-POWERLOOM-DEVELOPMENT) KWD-POWERLOOM-MEDIUM KWD-POWERLOOM-LOW)
      KWD-POWERLOOM-PREFIX (WRAP-STRING "PL") KWD-POWERLOOM-MAX-WIDTH (WRAP-INTEGER 250)))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
