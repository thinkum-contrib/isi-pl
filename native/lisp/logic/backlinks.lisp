...
(CL:DECLAIM
  ...
  *LOADINGREGENERABLEOBJECTS?* NIL-LIST UNKNOWN-TRUTH-VALUE
  FALSE-WRAPPER TRUE-WRAPPER NULL-INTEGER TRUE NIL-NON-PAGING-INDEX
  STANDARD-OUTPUT EOL NIL *MODULE* FALSE))
  ...
  (CL:LET* ((TABLE (%PREDICATE-PROPOSITIONS-TABLE INDEX)))
   (CL:WHEN (CL:EQ TABLE NULL) (CL:SETQ TABLE (NEW-HASH-TABLE))
    ...
    (CL:LET*
     ((PROP NULL)
      (ITER-000
       (ALLOCATE-ITERATOR (%DEPENDENT-PROPOSITIONS-LIST INDEX))))
      ...
      (CL:LET* ((INDEX (GET-BACKLINKS-INDEX SELF)))
       (CL:WHEN
        ...
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
         "Can we eliminate the skolem `" SELF
         "' from the description???" EOL EOL))
    ...
    (CL:WHEN TEST-VALUE-000 (CL:SETQ TEST-VALUE-000 (CL:= I 0))
     (CL:WHEN TEST-VALUE-000
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((T NULL)
         (ITER-000 (BUT-LAST (%IO-VARIABLE-TYPES DESCRIPTION))))
        (CL:LOOP WHILE (NEXT? ITER-000) DO
      ...
      (CL:AND
       (MEMBER?
        (GET-QUOTED-TREE "((:PREDICATE :FUNCTION) \"/LOGIC\")"
         "/LOGIC")
        (%KIND PROPOSITION))
       (INDEX-ITH-DOMAIN? (GET-DESCRIPTION (%OPERATOR PROPOSITION))
        I)))))))
      ...
      (CL:T
       (CL:LET*
        ...
        (CL:LET*
         ((PROP NULL)
          (ITER-000
           (ALLOCATE-ITERATOR (%DEPENDENT-PROPOSITIONS SELF))))
         (CL:LOOP WHILE (NEXT? ITER-000) DO
          (CL:SETQ PROP (%VALUE ITER-000))
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-BACKLINKS-LOGIC-LOGIC-OBJECT)
     (CL:PROGN
      (CL:LET* ((INDEX (GET-BACKLINKS-INDEX SELF)))
       (CL:IF (CL:EQ INDEX NULL) NIL-NON-PAGING-INDEX
        (CL:LET*
         ((TABLE (%PREDICATE-PROPOSITIONS-TABLE INDEX)) (LIST NULL))
         (CL:WHEN (CL:NOT (CL:EQ TABLE NULL))
          (CL:SETQ LIST
  ...
  (CL:LET*
   ...
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE INDEX)
     ...
     (CL:PROGN
      (CL:WHEN
       (CL:EQ (%%VALUE (%SELECTION-PATTERN INDEX)) KWD-BACKLINKS-ISA)
...
(CL:DEFUN SELECT-ARGUMENT-WITH-BACKLINKS (PROPOSITION)
  (CL:LET*
   ((RELATION (%OPERATOR PROPOSITION)) (KIND (%KIND PROPOSITION))
    (VALUE NULL) (COUNT NULL-INTEGER)
    (LOWESTBACKLINKCOUNT NULL-INTEGER) (BESTARGUMENT NULL)
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ P (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS P NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET* ((TOPROPOSITIONS VALUE-000))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ P (%VALUE ITER-001))
      (CL:WHEN (CL:NOT (MEMBER? EXCEPTPROPOSITIONS P))
       (CL:LET*
        ((I NULL-INTEGER) (ARG NULL) (VECTOR-000 (%ARGUMENTS P))
         (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ITER-002 0))
        (CL:DECLARE
         (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-002 I))
        (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
         (CL:PROGN
          (CL:SETQ ARG
           (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
         ...
         (CL:WHEN (EQL? ARG FROM)
          (CL:LET
           ((SELF (%THE-ARRAY (%ARGUMENTS P))) (VALUE TO) (POSITION I))
           (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
            (CL:TYPE CL:FIXNUM POSITION))
           (CL:SETF (CL:AREF SELF POSITION) VALUE)))))
       (CL:LET* ((TEST-VALUE-000 FALSE))
       ...
       (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
        ...
        (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE ARG)))
         (CL:COND
          ...
          (CL:T)))))
      (CL:LET*
       ((EXTENSION (GET-DESCRIPTION-EXTENSION DESCRIPTION TRUE)))
