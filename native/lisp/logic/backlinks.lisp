...
(CL:DECLAIM
  ...
  *LOADINGREGENERABLEOBJECTS?* *EVALUATIONMODE* NIL-LIST UNKNOWN-TRUTH-VALUE
  *CONTEXT* *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE TRUE-WRAPPER
  NULL-INTEGER STANDARD-OUTPUT FALSE-WRAPPER NIL-NON-PAGING-INDEX
  *POWERLOOM-EXECUTION-MODE* STANDARD-WARNING EOL NIL *MODULE*))

;;; (DEFUN (TYPE-HAS-BACKLINKS? BOOLEAN) ...)

(CL:DEFUN TYPE-HAS-BACKLINKS? (TYPE)
  (CL:IF (CL:EQ (%SURROGATE-VALUE TYPE) NULL) CL:NIL
      ...
      (CL:PROGN
       (CL:WHEN
        ...
        (CL:ERROR
         ...
         SELF PROPOSITION))
       (CL:WHEN
        (CL:AND (CL:EQ *POWERLOOM-EXECUTION-MODE* KWD-BACKLINKS-DEBUGGING)
     ...
     (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CL:TAGBODY
         (CL:WHEN (CL:NOT (EQUAL? RELATION (%OPERATOR PROP)))
          (CL:COND
           ((CL:EQ (%KIND PROP) KWD-BACKLINKS-EQUIVALENT)
            (CL:SETQ FUNCTION-SKOLEM (GET-SKOLEM-AND-VALUE-TERMS PROP))
            (CL:WHEN (CL:NOT (CL:EQ FUNCTION-SKOLEM NULL))
             (CL:SETQ PROP (%DEFINING-PROPOSITION FUNCTION-SKOLEM)))
            (CL:WHEN (CL:NOT (EQUAL? RELATION (%OPERATOR PROP)))
             (CL:GO :CONTINUE)))
           (CL:T (CL:GO :CONTINUE))))
           ...
           (CL:LET* ((KEY NULL) (ITER-002 KEYS) (COLLECT-000 NULL))
            (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
             ...
             (CL:IF (CL:EQ COLLECT-000 NULL)
              ...
              (CL:PROGN
               (CL:SETF (%%REST COLLECT-000) (CONS (VALUE-OF KEY) NIL))
               (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
      ...
      (CL:IF (CL:EQ FILTEREDSEQUENCE NIL) NIL-NON-PAGING-INDEX
       (CL:LET* ((SELF-001 (NEW-NON-PAGING-INDEX)))
        (CL:SETF (%THE-SEQUENCE SELF-001) FILTEREDSEQUENCE)
    ...
    (CL:LET*
     ...
     (CL:WHEN *MEMOIZATION-ENABLED?*
      ...
      (CL:SETQ MEMOIZED-ENTRY-001
       (LOOKUP-MRU-MEMOIZED-VALUE MEMO-TABLE-001 RELATION KEY1 *CONTEXT*
        MEMOIZED-NULL-VALUE 4))
    ...
    (CL:LET*
     ...
     (CL:WHEN *MEMOIZATION-ENABLED?*
      ...
      (CL:SETQ MEMOIZED-ENTRY-002
       (LOOKUP-MRU-MEMOIZED-VALUE MEMO-TABLE-002 RELATION KEY1 KEY2 *CONTEXT*
        8))
  ...
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND (CL:= COUNT 0)
     (CL:NOT
      (CL:EQ
       (ALL-DIRECT-SUBRELATIONS (SURROGATE-TO-DESCRIPTION RELATION) CL:NIL)
       NIL)))
    (CL:SETQ COUNT 1))
...
(CL:DEFUN SELECT-ARGUMENT-WITH-BACKLINKS (PROPOSITION)
  (CL:LET*
      ...
      (CL:WHEN (CL:NOT (CL:EQ VALUE NULL))
       ...
       (CL:WHEN
        (CL:AND (CL:NOT (CL:= COUNT NULL-INTEGER))
         (CL:OR (CL:= LOWESTBACKLINKCOUNT NULL-INTEGER)
          (CL:< COUNT LOWESTBACKLINKCOUNT)))
        (CL:SETQ LOWESTBACKLINKCOUNT COUNT) (CL:SETQ BESTARGUMENT VALUE)))
      :CONTINUE)
     (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
   (CL:WHEN
    (CL:AND (CL:= LOWESTBACKLINKCOUNT NULL-INTEGER) FOUNDDISCOURAGEDARGUMENT?)
    (CL:LET*
   ...
   (CL:COND ((CL:= LOWESTBACKLINKCOUNT NULL-INTEGER) (CL:VALUES NULL CL:NIL))
        ...
        (CL:LET*
         ...
         (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
          ...
          (CL:WHEN (EQL? ARG FROM)
           (CL:LET
            ((SELF (%THE-ARRAY (%ARGUMENTS P))) (VALUE TOVALUE) (POSITION I))
            (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
             (CL:TYPE CL:FIXNUM POSITION))
            (CL:SETF (CL:AREF SELF POSITION) VALUE)))
          (CL:SETQ INDEX-000 (CL:1+ INDEX-000))
  ...
  (CL:PROGN
   ...
   (CL:SETQ SGT-BACKLINKS-STELLA-LITERAL-WRAPPER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "LITERAL-WRAPPER"
     (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ SGT-BACKLINKS-LOGIC-BACKLINKS-MIXIN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "BACKLINKS-MIXIN" NULL 1))
