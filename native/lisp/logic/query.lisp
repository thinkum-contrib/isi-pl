...
(CL:DECLAIM
  ...
  *PRINTREADABLY?* *DEBUG-PRINT-MODE?* NULL-STRING-WRAPPER
  *CYC-KLUDGES-ENABLED?* *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE
  TRUE-TRUTH-VALUE ZERO-WRAPPER *RECORD-JUSTIFICATIONS?*
  UNKNOWN-TRUTH-VALUE *CACHE-SUCCEEDED-GOALS?* *CACHE-FAILED-GOALS?*
  *CONTEXT* NULL-INTEGER-WRAPPER *PARTIAL-SUPPORT-COUNTER*
  NULL-FLOAT-WRAPPER *FAIL-UNBOUND-CLAUSES?* STANDARD-WARNING
  ONE-WRAPPER STANDARD-OUTPUT DEFAULT-TRUE-TRUTH-VALUE
  ...
  (CL:WHEN *DEBUG-PRINT-MODE?*
   (CL:LET*
    ((COUNTER
      (INTEGER-TO-STRING
       (CL:TRUNCATE
        (CL:SETQ *QUERY-ITERATOR-DEBUG-ID-COUNTER*
         (CL:1+ *QUERY-ITERATOR-DEBUG-ID-COUNTER*))))))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING COUNTER))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
     SYM-QUERY-LOGIC-DEBUG-ID
     (CL:IF (BLANK-STRING? ID) (WRAP-STRING COUNTER)
      (WRAP-STRING (CONCATENATE ID ":" COUNTER)))
     NULL-STRING-WRAPPER))))
...
(CL:DEFUN PRINT-QUERY-ITERATOR (SELF STREAM)
  (CL:WHEN *DEBUG-PRINT-MODE?*
   (%%PRINT-STREAM STREAM "["
    (%WRAPPER-VALUE
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-QUERY-LOGIC-DEBUG-ID
      NULL-STRING-WRAPPER))
    "] "))
    ...
    ((CL:EQ NEGATEDQUERY NULL)
     (CL:SETQ NEGATEDQUERY
      (CREATE-QUERY-ITERATOR (QUERY-DESCRIPTION SELF)
       (%BINDINGS (%VALUE SELF))))
     (DEBUG-ID-SETTER NEGATEDQUERY "negatedQuery")
...
with a full stack of control frames.\")" (CL:FUNCTION PRINT-GOAL-STACK)
   ...
   (DEFINE-FUNCTION-OBJECT "VPGS" "(DEFUN VPGS ())" (CL:FUNCTION VPGS)
    NULL)
   (DEFINE-EXTERNAL-SLOT-FROM-STRINGIFIED-SOURCE
    "(DEFSLOT QUERY-ITERATOR DEBUG-ID :TYPE STRING :ALLOCATION :DYNAMIC)")
   (DEFINE-METHOD-OBJECT
...
use the current ID counter and increment it appropriately.\")"
    (CL:FUNCTION DEBUG-ID-SETTER) NULL)
...
description extension iterator is switched from a list to a hash table\")")
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *QUERY-ITERATOR-DEBUG-ID-COUNTER* INTEGER 0)")
