...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *TRACED-KEYWORDS* TRUE-TRUTH-VALUE
  ...
  MEMOIZED-NULL-VALUE TRUE-WRAPPER NIL FALSE-WRAPPER FALSE TRUE))
...
(CL:DEFUN ALL-NAMED-DIRECT-SUBDESCRIPTIONS (SELF)
  (CL:LET* ((RESULT NIL))
   (CL:LET*
    ((D NULL) (ITER-000 (ALL-DIRECT-SUBCOLLECTIONS SELF FALSE)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ D (%VALUE ITER-000))
     (CL:WHEN (ISA? D SGT-CLASSIFY-LOGIC-NAMED-DESCRIPTION)
      (CL:SETQ RESULT (CONS D RESULT)))))
   RESULT))
...
(CL:DEFVAR *CACHE-INFERABLE-SUBCOLLECTIONS?* FALSE
...
(CL:DEFVAR *CLASSIFY-FROM-NON-INFERABLE-PARENTS-ONLY?* FALSE
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (NAMED-DESCRIPTION? E))
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ PD (%%VALUE ITER-001))
         (CL:SETQ ITER-001 (%%REST ITER-001)))
        (ADD-SUBSUMPTION-LINK SELF PD)))))))
  (CL:LET*
   ((SUPER NULL)
    (ITER-002 (ALL-NAMED-DIRECT-SUPERDESCRIPTIONS SELF TRUE)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:COND ((CL:EQ MODULE NULL) (UPCLASSIFY-ALL-DESCRIPTIONS))
     (LOCAL?
      (CL:LET*
       ((D NULL) (ITER-000 (ALL-NAMED-DESCRIPTIONS MODULE TRUE)))
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ D (%VALUE ITER-000))
       ...
       (CL:WHEN (CL:NOT (%EVERYTHING-CLASSIFIED? SESSION))
        (CL:LET* ((M NULL) (ITER-001 (ALL-INCLUDED-MODULES MODULE)))
         (CL:LOOP WHILE (NEXT? ITER-001) DO
          ...
          (CL:LET*
           ((D NULL) (ITER-002 (ALL-NAMED-DESCRIPTIONS M TRUE)))
...
non-inferable/primitive ancestor relations."
  ...
  (FINALIZE-OBJECTS)
  (CL:LET*
   ((THEMODULE (COERCE-TO-MODULE MODULE TRUE)) (UNCLASSIFIED NIL))
   (CL:WHEN (CL:NOT (CL:EQ THEMODULE NULL))
    (CL:WHEN (CL:EQ MODULE NULL) (CL:SETQ THEMODULE NULL))
    (CL:LET*
     ((*CLASSIFICATIONSESSION*
         ...
         (CL:WHEN TEST-VALUE-000
          (CL:LET* ((FOUND?-000 FALSE))
           (CL:LET*
            ...
            (CL:LOOP WHILE (NEXT? ITER-001) DO
             (CL:SETQ SUPER (%VALUE ITER-001))
             (CL:WHEN (NON-INFERABLE? SUPER) (CL:SETQ FOUND?-000 TRUE)
              (CL:RETURN))))
           (CL:SETQ TEST-VALUE-000 FOUND?-000))
          (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000)))
         (CL:WHEN TEST-VALUE-000
          ...
          (CL:LET* ((FOUND?-000 FALSE))
           (CL:LET*
            ...
            (CL:LOOP WHILE (NEXT? ITER-001) DO
             ...
             (CL:WHEN (NON-INFERABLE? TYPE) (CL:SETQ FOUND?-000 TRUE)
              (CL:RETURN))))
           (CL:SETQ TEST-VALUE-000 FOUND?-000))
          (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000)))
         (CL:WHEN TEST-VALUE-000
...
(CL:DEFUN STARTUP-CLASSIFY ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *CACHE-INFERABLE-SUBCOLLECTIONS?* TRUE)
    (CL:SETQ *CLASSIFY-FROM-NON-INFERABLE-PARENTS-ONLY?* TRUE))
...
provably co-extensional.\")" (CL:FUNCTION ALL-CYCLES) NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Classify") NULL-STRING-WRAPPER)))
