...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *TRACED-KEYWORDS* TRUE-TRUTH-VALUE DEFAULT-TRUE-TRUTH-VALUE *MODULE* NULL-INTEGER
  *EMIT-THINKING-DOTS?* STANDARD-OUTPUT EOL *CONTEXT* *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE TRUE-WRAPPER NIL
  FALSE-WRAPPER))
...
(CL:DEFUN HELP-COLLECT-INTENSIONAL-PARENTS (DESCRIPTION PROPOSITION COLLECTION)
  (CL:LET* ((TEST-VALUE-000 (%KIND PROPOSITION)))
   (CL:COND
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-CLASSIFY-PREDICATE) (CL:EQ TEST-VALUE-000 KWD-CLASSIFY-FUNCTION)
      (CL:EQ TEST-VALUE-000 KWD-CLASSIFY-ISA))
     (CL:LET* ((TEST-VALUE-001 CL:NIL))
      (CL:LET* ((ALWAYS?-000 CL:T))
       (CL:LET*
        ((ARG1 NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ARG2 NULL)
         (VECTOR-001 (%IO-VARIABLES DESCRIPTION)) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
        ...
        (CL:LOOP WHILE (CL:AND (CL:< INDEX-000 LENGTH-000) (CL:< INDEX-001 LENGTH-001)) DO
         ...
         (CL:WHEN (CL:NOT (CL:EQ ARG1 ARG2)) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
         (CL:SETQ INDEX-000 (CL:1+ INDEX-000)) (CL:SETQ INDEX-001 (CL:1+ INDEX-001))))
       (CL:SETQ TEST-VALUE-001 ALWAYS?-000))
      (CL:WHEN TEST-VALUE-001
       (CL:SETQ TEST-VALUE-001 (CL:= (LENGTH (%ARGUMENTS PROPOSITION)) (LENGTH (%IO-VARIABLES DESCRIPTION)))))
    ...
    ((CL:EQ TEST-VALUE-000 KWD-CLASSIFY-EXISTS)
     (HELP-COLLECT-INTENSIONAL-PARENTS DESCRIPTION
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS PROPOSITION))) 0) COLLECTION))
    (CL:T))))
    ...
    ((SUBTYPE-OF? TEST-VALUE-001 SGT-CLASSIFY-LOGIC-NAMED-DESCRIPTION)
     (CL:PROGN
      (CL:COND
       ...
       ((CL:NOT (CL:EQ (%KIND (%PROPOSITION SELF)) (%KIND (%PROPOSITION THROUGHCHILD))))
        (CL:RETURN-FROM INFERABLE-THROUGH-BY-SOME-DESCENDANT? CL:T))
      ...
      (CL:LET* ((P NULL) (ITER-000 (APPLICABLE-RULES-OF-DESCRIPTION THROUGHCHILD KWD-CLASSIFY-BACKWARD CL:NIL)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ P (%%VALUE ITER-000))
        (CL:WHEN
         (CL:AND
          (CL:NOT
           (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS P) SYM-CLASSIFY-LOGIC-FORWARD-ONLY? FALSE-WRAPPER)))
          (TRUE? P))
         (CL:WHEN
          (INFERABLE-THROUGH-BY-SOME-DESCENDANT? SELF (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS P))) 0)
           ALREADYVISITEDLIST)
          (CL:RETURN-FROM INFERABLE-THROUGH-BY-SOME-DESCENDANT? CL:T)))
       ...
       (CL:LET* ((FOUND?-000 CL:NIL))
        (CL:LET* ((PARENT NULL) (ITER-001 (%THE-CONS-LIST (ALL-INTENSIONAL-PARENTS THROUGHCHILD))))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ PARENT (%%VALUE ITER-001))
          (CL:WHEN (COLLECTION-IMPLIES-COLLECTION? PARENT SELF) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
      ...
      (CL:LET* ((MEMO-TABLE-000 NULL) (MEMOIZED-ENTRY-000 NULL) (MEMOIZED-VALUE-000 NULL))
       (CL:WHEN *MEMOIZATION-ENABLED?*
        ...
        (CL:WHEN (CL:EQ MEMO-TABLE-000 NULL)
         (INITIALIZE-MEMOIZATION-TABLE SGT-CLASSIFY-LOGIC-F-INFERABLE-WITH-CYCLE-CHECK?-MEMO-TABLE-000
          "(:MAX-VALUES 500 :TIMESTAMPS (:IMPLIES-PROPOSITION-UPDATE))")
         (CL:SETQ MEMO-TABLE-000 (%SURROGATE-VALUE SGT-CLASSIFY-LOGIC-F-INFERABLE-WITH-CYCLE-CHECK?-MEMO-TABLE-000)))
        (CL:SETQ MEMOIZED-ENTRY-000
         (LOOKUP-MRU-MEMOIZED-VALUE MEMO-TABLE-000 SELF *CONTEXT* MEMOIZED-NULL-VALUE NULL -1))
       ...
       (CL:COND
        ...
        (CL:T
         (CL:LET* ((TEST-VALUE-001 CL:NIL))
          (CL:SETQ TEST-VALUE-001 (CL:NOT (CL:EQ (%SURROGATE-VALUE-INVERSE SELF) SGT-CLASSIFY-STELLA-THING)))
          (CL:WHEN TEST-VALUE-001
           (CL:SETQ TEST-VALUE-001 (CL:NOT (CL:EQ (CL:SETQ ALREADYVISITEDLIST (CONS SELF ALREADYVISITEDLIST)) NULL)))
           (CL:WHEN TEST-VALUE-001
              ...
              (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ P (%%VALUE ITER-001))
               (CL:WHEN
                (CL:AND
                 (CL:NOT
                  (%WRAPPER-VALUE
                   (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS P) SYM-CLASSIFY-LOGIC-FORWARD-ONLY? FALSE-WRAPPER)))
                 (TRUE? P)
                 (INFERABLE-THROUGH-BY-SOME-DESCENDANT? SELF
                  (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS P))) 0) ALREADYVISITEDLIST))
...
(CL:DEFUN INFERABLE? (SELF)
  (CL:LET* ((*CONTEXT* (GET-INFERABLE-TEST-CONTEXT)) (*MODULE* (%BASE-MODULE *CONTEXT*)))
   (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*)) (INFERABLE-WITH-CYCLE-CHECK? SELF NIL)))
...
(CL:DEFUN NEW-MARKER-TABLE ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE MARKER-TABLE)))
   (CL:SETF (%SUPPORTS-RECALL? SELF) CL:NIL) (CL:SETF (%RECALL-TABLE SELF) NULL) (CL:SETF (%TEST-TABLE SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-MARKER-TABLE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-TEST-TABLE)
    (CL:IF SETVALUE? (CL:SETF (%TEST-TABLE SELF) VALUE) (CL:SETQ VALUE (%TEST-TABLE SELF))))
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-RECALL-TABLE)
    (CL:IF SETVALUE? (CL:SETF (%RECALL-TABLE SELF) VALUE) (CL:SETQ VALUE (%RECALL-TABLE SELF))))
...
storage object 'self'."
  (CL:IF (%SUPPORTS-RECALL? SELF)
   (CL:WHEN (CL:EQ (LOOKUP (%TEST-TABLE SELF) OBJECT) NULL) (INSERT-AT (%TEST-TABLE SELF) OBJECT TRUE-WRAPPER)
  ...
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((D NULL) (ITER-000 (ALL-DIRECT-SUPERCOLLECTIONS SELF REMOVEEQUIVALENTS?)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ D (%VALUE ITER-000))
     (CL:WHEN (ISA? D SGT-CLASSIFY-LOGIC-NAMED-DESCRIPTION) (CL:SETQ RESULT (CONS D RESULT)))))
   RESULT))
...
(CL:DEFUN ADD-SUBSUMPTION-LINK (SUBDESCRIPTION SUPERDESCRIPTION)
  (CL:LET* ((IMPLIESPROP (ASSERT-DESCRIPTION-IMPLIES-DESCRIPTION SUBDESCRIPTION SUPERDESCRIPTION CL:NIL)))
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS IMPLIESPROP) SYM-CLASSIFY-LOGIC-SUBSUMPTION-LINK? TRUE-WRAPPER
    FALSE-WRAPPER)
   (CL:WHEN (TRACE-KEYWORD? KWD-CLASSIFY-CLASSIFIER-INFERENCES)
    (CL:LET* ((*INDENTCOUNTER* 4) (*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *INDENTCOUNTER* *PRINTREADABLY?*))
...
(CL:DEFCLASS CLASSIFICATION-CACHE (STANDARD-OBJECT)
  ((INFERABLE-DIRECT-SUBDESCRIPTIONS :ALLOCATION :INSTANCE :ACCESSOR %INFERABLE-DIRECT-SUBDESCRIPTIONS)
   (CACHE-UPCLASSIFICATION-TIMESTAMP :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR
    %CACHE-UPCLASSIFICATION-TIMESTAMP)
   (CACHE-DOWNCLASSIFICATION-TIMESTAMP :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR
    %CACHE-DOWNCLASSIFICATION-TIMESTAMP)
   (INTRODUCTION-TIMESTAMP :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR
...
(CL:DEFUN NEW-CLASSIFICATION-CACHE ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE CLASSIFICATION-CACHE)))
   (CL:SETF (%INTRODUCTION-TIMESTAMP SELF) 0) (CL:SETF (%CACHE-DOWNCLASSIFICATION-TIMESTAMP SELF) -1)
   (CL:SETF (%CACHE-UPCLASSIFICATION-TIMESTAMP SELF) -1) (CL:SETF (%INFERABLE-DIRECT-SUBDESCRIPTIONS SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-CLASSIFICATION-CACHE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-CACHE-UPCLASSIFICATION-TIMESTAMP)
    (CL:IF SETVALUE? (CL:SETF (%CACHE-UPCLASSIFICATION-TIMESTAMP SELF) (%WRAPPER-VALUE VALUE))
   ...
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-INTRODUCTION-TIMESTAMP)
    (CL:IF SETVALUE? (CL:SETF (%INTRODUCTION-TIMESTAMP SELF) (%WRAPPER-VALUE VALUE))
...
(CL:DEFCLASS CLASSIFICATION-SESSION (STANDARD-OBJECT)
  ((CLASSIFICATION-WORLD :ALLOCATION :INSTANCE :ACCESSOR %CLASSIFICATION-WORLD)
   (CLASSIFICATION-CACHE-TABLE :ALLOCATION :INSTANCE :ACCESSOR %CLASSIFICATION-CACHE-TABLE)
   (CLASSIFICATION-TIMECLOCK :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR
    %CLASSIFICATION-TIMECLOCK)
   (INTRODUCTION-TIMESTAMP-STACK :ALLOCATION :INSTANCE :ACCESSOR %INTRODUCTION-TIMESTAMP-STACK)
   (EVERYTHING-CLASSIFIED? :ALLOCATION :INSTANCE :ACCESSOR %EVERYTHING-CLASSIFIED?)
   (LAST-CACHE-TABLE-KEY :ALLOCATION :INSTANCE :ACCESSOR %LAST-CACHE-TABLE-KEY)
   (LAST-CACHE-TABLE-VALUE :ALLOCATION :INSTANCE :ACCESSOR %LAST-CACHE-TABLE-VALUE)))

(CL:DEFUN NEW-CLASSIFICATION-SESSION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE CLASSIFICATION-SESSION)))
   (CL:SETF (%LAST-CACHE-TABLE-VALUE SELF) NULL) (CL:SETF (%LAST-CACHE-TABLE-KEY SELF) NULL)
   (CL:SETF (%EVERYTHING-CLASSIFIED? SELF) CL:NIL)
   (CL:SETF (%INTRODUCTION-TIMESTAMP-STACK SELF) (LIST (WRAP-INTEGER 0))) (CL:SETF (%CLASSIFICATION-TIMECLOCK SELF) 0)
   (CL:SETF (%CLASSIFICATION-CACHE-TABLE SELF) (NEW-HASH-TABLE)) (CL:SETF (%CLASSIFICATION-WORLD SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-CLASSIFICATION-SESSION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-CLASSIFICATION-WORLD)
    (CL:IF SETVALUE? (CL:SETF (%CLASSIFICATION-WORLD SELF) VALUE) (CL:SETQ VALUE (%CLASSIFICATION-WORLD SELF))))
   ...
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-CLASSIFICATION-TIMECLOCK)
    (CL:IF SETVALUE? (CL:SETF (%CLASSIFICATION-TIMECLOCK SELF) (%WRAPPER-VALUE VALUE))
   ...
   ((CL:EQ SLOTNAME SYM-CLASSIFY-LOGIC-LAST-CACHE-TABLE-KEY)
    (CL:IF SETVALUE? (CL:SETF (%LAST-CACHE-TABLE-KEY SELF) VALUE) (CL:SETQ VALUE (%LAST-CACHE-TABLE-KEY SELF))))
...
(CL:DEFUN GET-CLASSIFICATION-SESSION (INSTANCEORDESCRIPTION)
  ...
  (CL:LET* ((WORLD NULL) (SESSION NULL))
   (CL:COND
    ...
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" INSTANCEORDESCRIPTION "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
   ...
   (CL:WHEN (CL:EQ SESSION NULL) (CL:SETQ SESSION (NEW-CLASSIFICATION-SESSION))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD) SYM-CLASSIFY-LOGIC-CLASSIFICATION-SESSION SESSION NULL)
...
(CL:DEFUN GET-CLASSIFICATION-CACHE (SELF)
  (CL:LET* ((SESSION *CLASSIFICATIONSESSION*) (CACHE NULL))
   (CL:WHEN (CL:EQ (%LAST-CACHE-TABLE-KEY SESSION) SELF)
    (CL:RETURN-FROM GET-CLASSIFICATION-CACHE (%LAST-CACHE-TABLE-VALUE SESSION)))
...
(CL:DEFUN TOGGLE-INFERABLE-DIRECT-SUBCOLLECTIONS-CACHING ()
  ...
  (CL:IF *CACHE-INFERABLE-SUBCOLLECTIONS?* "Inferable subcollections caching enabled"
   ...
   (CL:WHEN (CL:OR (CL:EQ LIST NULL) (CL:NOT *CACHE-INFERABLE-SUBCOLLECTIONS?*)) (CL:SETQ LIST (NEW-LIST))
    (CL:LET* ((SUB NULL) (ITER-000 (ALL-NAMED-DIRECT-SUBDESCRIPTIONS SELF)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SUB (%%VALUE ITER-000))
      (CL:WHEN (INFERABLE? SUB) (INSERT LIST SUB)) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN FLUSH-INFERABLE-DIRECT-SUBDESCRIPTIONS-CACHE (IMPLIESPROPOSITION)
  ...
  (CL:LET*
   ((CACHE
     (GET-CLASSIFICATION-CACHE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS IMPLIESPROPOSITION))) 1))))
  ...
  (CL:LET* ((WORLD (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS *MODULE*) SYM-CLASSIFY-LOGIC-META-INFERENCE-CACHE NULL)))
   (CL:WHEN
    (CL:OR (CL:EQ WORLD NULL)
     (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD) SYM-CLASSIFY-LOGIC-CLASSIFICATION-SESSION NULL) NULL))
...
(CL:DEFUN FLUSH-CLASSIFICATION-SESSIONS ()
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ JITWORLD NULL))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS JITWORLD) SYM-CLASSIFY-LOGIC-CLASSIFICATION-SESSION NULL NULL))
   (CL:WHEN (CL:NOT (CL:EQ METAWORLD NULL))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METAWORLD) SYM-CLASSIFY-LOGIC-CLASSIFICATION-SESSION NULL NULL))
   (CL:WHEN (CL:NOT (CL:EQ TMSWORLD NULL))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS TMSWORLD) SYM-CLASSIFY-LOGIC-CLASSIFICATION-SESSION NULL NULL))))
...
(CL:DEFUN HELP-COLLECT-PARENTS-BELOW? (SELF SUPER NEWPARENTDESCRIPTIONS ALREADYVISITEDTABLE SELFISBELOWTABLE SUBSUMPTIONTEST)
  ...
  (CL:WHEN (CL:AND (CL:NOT (CL:EQ *TRACED-KEYWORDS* NULL)) (MEMB? *TRACED-KEYWORDS* KWD-CLASSIFY-CLASSIFIER))
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "   TEST-MARKER of 'super': " (TEST-MARKER? SELFISBELOWTABLE SUPER)
    EOL))
     ...
     (CL:LET* ((SUB NULL) (ITER-000 (%THE-CONS-LIST (ALL-INFERABLE-DIRECT-SUBCOLLECTIONS SUPER))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SUB (%%VALUE ITER-000))
       (CL:WHEN (CL:NOT (TEST-MARKER? ALREADYVISITEDTABLE SUB))
        (HELP-COLLECT-PARENTS-BELOW? SELF SUB NEWPARENTDESCRIPTIONS ALREADYVISITEDTABLE SELFISBELOWTABLE
         SUBSUMPTIONTEST))
       (CL:SETQ ITER-000 (%%REST ITER-000)))))
    ((CL:AND (CL:NOT (UPCLASSIFIED-LATER-THAN? SELF SUPER)) (APPLY-SUBSUMPTION-TEST? SUBSUMPTIONTEST SELF SUPER))
     (CL:SETQ FOUNDSUBSUMINGSUB? CL:T) (MARK-UNMARKED-SUPERRELATIONS SUPER SELFISBELOWTABLE)
     (CL:LET* ((FOUND?-000 CL:NIL))
       ...
       (CL:WHEN (CL:NOT EXISTSMORESPECIFICSUB?)
        (CL:WHEN
         (CL:AND (CL:NOT (CL:EQ *TRACED-KEYWORDS* NULL))
          (CL:OR (MEMB? *TRACED-KEYWORDS* KWD-CLASSIFY-CLASSIFIER)
           (MEMB? *TRACED-KEYWORDS* KWD-CLASSIFY-CLASSIFIER-PROFILE)))
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "XXXXXXX PUSHING NEW PARENT: " SUPER "  of " SELF EOL
          EOL))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ S (%%VALUE ITER-000))
     (CL:LET* ((A NULL) (ITER-001 (ALL-SUPERCOLLECTIONS S)))
      (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ A (%VALUE ITER-001))
       (CL:WHEN (CL:NOT (MEMBER? SUPERS A)) (CL:SETQ COLLECTION (CONS A COLLECTION)))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN HELP-UPCLASSIFY-ONE-ENTITY (SELF INSTANCEORDESCRIPTION)
  ...
  (CL:LET*
   ((ALREADYVISITEDTABLE (CREATE-MARKER-STORAGE CL:NIL)) (INSTANCEISBELOWTABLE (CREATE-MARKER-STORAGE CL:NIL))
    ...
    (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ SUPER (%VALUE ITER-001))
     (CL:WHEN
      (CL:AND (ISA? SUPER SGT-CLASSIFY-LOGIC-DESCRIPTION)
       (CL:OR (CL:NOT *CLASSIFY-FROM-NON-INFERABLE-PARENTS-ONLY?*) (NON-INFERABLE? SUPER))
      ...
      (COLLECT-SUBSUMING-PARENTS-BELOW SELF SUPER NEWPARENTDESCRIPTIONS ALREADYVISITEDTABLE INSTANCEISBELOWTABLE
       (CL:IF (CL:EQ INSTANCEORDESCRIPTION KWD-CLASSIFY-INSTANCE) (CL:FUNCTION INSTANCE-SATISFIES-DESCRIPTION?)
...
(CL:DEFUN UPCLASSIFY-ONE-DESCRIPTION (SELF)
  (CL:WHEN (CL:OR (UPCLASSIFIED? SELF) (CL:NOT (ISA? SELF SGT-CLASSIFY-LOGIC-DESCRIPTION)))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ E (%%VALUE ITER-000))
     (CL:WHEN (CL:NOT (NAMED-DESCRIPTION? E))
      (CL:LET* ((PD NULL) (ITER-001 (%THE-CONS-LIST (ALL-INTENSIONAL-PARENTS E))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ PD (%%VALUE ITER-001)) (ADD-SUBSUMPTION-LINK SELF PD)
        (CL:SETQ ITER-001 (%%REST ITER-001)))))
     (CL:SETQ ITER-000 (%%REST ITER-000)))))
  (CL:LET* ((SUPER NULL) (ITER-002 (ALL-NAMED-DIRECT-SUPERDESCRIPTIONS SELF CL:T)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ SUPER (%%VALUE ITER-002))
    (UPCLASSIFY-ONE-DESCRIPTION SUPER) (CL:SETQ ITER-002 (%%REST ITER-002))))
...
(CL:DEFUN HELP-DOWNCLASSIFY-ONE-DESCRIPTION (NODE SELF NECESSARYANCESTORS ALREADYVISITEDTABLE)
  (CL:WHEN (CL:OR (TEST-MARKER? ALREADYVISITEDTABLE NODE) (COLLECTION-IMPLIES-COLLECTION? NODE SELF))
  ...
  (CL:LET* ((NODECOULDBETHEONE? CL:T))
     ...
     (CL:LET* ((FOUND?-000 CL:NIL))
      (CL:LET* ((ANCESTOR NULL) (ITER-000 NECESSARYANCESTORS))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ANCESTOR (%%VALUE ITER-000))
        (CL:WHEN
         (CL:AND (UPCLASSIFIED-LATER-THAN? NODE ANCESTOR) (CL:NOT (COLLECTION-IMPLIES-COLLECTION? NODE ANCESTOR)))
         (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
        (CL:SETQ ITER-000 (%%REST ITER-000))))
      (CL:IF FOUND?-000 (CL:SETQ NODECOULDBETHEONE? CL:NIL) (UPCLASSIFY-ONE-DESCRIPTION NODE)))))
   (CL:WHEN (CL:AND NODECOULDBETHEONE? (COLLECTION-IMPLIES-COLLECTION? NODE SELF))
    (CL:RETURN-FROM HELP-DOWNCLASSIFY-ONE-DESCRIPTION))
   (CL:LET* ((SUBNODE NULL) (ITER-001 (ALL-NAMED-DIRECT-SUBDESCRIPTIONS NODE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ SUBNODE (%%VALUE ITER-001))
     (HELP-DOWNCLASSIFY-ONE-DESCRIPTION SUBNODE SELF NECESSARYANCESTORS ALREADYVISITEDTABLE)
     (CL:SETQ ITER-001 (%%REST ITER-001))))))
     ...
     (CL:WHEN (DOWNCLASSIFIED? P) (CL:PROGN)
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS P NIL))
        (CL:IF (CL:EQ DOWNCLASSIFIEDPARENTS NIL) (CL:SETQ DOWNCLASSIFIEDPARENTS COLLECT-000)
   ...
   (CL:CASE (LENGTH DOWNCLASSIFIEDPARENTS)
    (0
     (CL:LET* ((NAMEDPARENTS NIL))
      ...
      (CL:WHEN (CL:EQ BESTPARENT NULL) (CL:SETQ BESTPARENT (COLLECTION-WITH-FEWEST-CHILDREN PARENTS)))
      (DOWNCLASSIFY-ONE-DESCRIPTION BESTPARENT)))
    (1 (CL:SETQ BESTPARENT (%%VALUE DOWNCLASSIFIEDPARENTS)))
    (CL:OTHERWISE (CL:SETQ BESTPARENT (COLLECTION-WITH-FEWEST-CHILDREN DOWNCLASSIFIEDPARENTS))))
   (CL:WHEN (CL:AND (CL:NOT (CL:EQ *TRACED-KEYWORDS* NULL)) (MEMB? *TRACED-KEYWORDS* KWD-CLASSIFY-CLASSIFIER))
...
(CL:DEFUN DOWNCLASSIFY-ONE-DESCRIPTION (SELF)
  (CL:LET* ((*CLASSIFICATIONSESSION* (GET-CLASSIFICATION-SESSION KWD-CLASSIFY-DESCRIPTION)))
     ...
     (CL:WHEN (CL:NOT (CL:EQ BESTPARENT NULL))
      (CL:LET* ((CHILD NULL) (ITER-000 (ALL-NAMED-DIRECT-SUBDESCRIPTIONS BESTPARENT)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ CHILD (%%VALUE ITER-000))
        (HELP-DOWNCLASSIFY-ONE-DESCRIPTION CHILD SELF OTHERPARENTS ALREADYVISITEDTABLE)
        (CL:SETQ ITER-000 (%%REST ITER-000))))))
  ...
  (CL:LET* ((PARENTIMPLIES (NEW-LIST)) (CHILDIMPLIES (NEW-LIST)) (PARENT NULL) (CHILD NULL) (BRIDGEPROP NULL))
   (CL:LET* ((P NULL) (ITER-000 (ALLOCATE-ITERATOR (UNFILTERED-DEPENDENT-PROPOSITIONS SELF NULL))))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ P (%VALUE ITER-000))
     (CL:IF (CL:EQ (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS P))) 0) SELF) (PUSH PARENTIMPLIES P)
      (PUSH CHILDIMPLIES P))))
   (CL:LET* ((CP NULL) (ITER-001 (%THE-CONS-LIST CHILDIMPLIES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ CP (%%VALUE ITER-001))
     (CL:SETQ CHILD (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS CP))) 0))
     (CL:LET* ((PP NULL) (ITER-002 (%THE-CONS-LIST PARENTIMPLIES)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ PP (%%VALUE ITER-002))
       (CL:SETQ PARENT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS PP))) 1))
       (CL:SETQ BRIDGEPROP (ASSERT-DESCRIPTION-IMPLIES-DESCRIPTION CHILD PARENT CL:T))
       (CL:WHEN
        (CL:AND
         (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CP) SYM-CLASSIFY-LOGIC-SUBRELATION-LINK? FALSE-WRAPPER))
         (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PP) SYM-CLASSIFY-LOGIC-SUBRELATION-LINK? FALSE-WRAPPER)))
        (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS BRIDGEPROP) SYM-CLASSIFY-LOGIC-SUBRELATION-LINK? TRUE-WRAPPER
         FALSE-WRAPPER))
...
it is automatically removed from the hierarchy after classification."
  ...
  (CL:LET* ((*FINDSUPERSANDSUBSDESCRIPTION* SELF)) (CL:DECLARE (CL:SPECIAL *FINDSUPERSANDSUBSDESCRIPTION*))
   (CL:LET*
    ((*CLASSIFICATIONSESSION* (GET-CLASSIFICATION-SESSION KWD-CLASSIFY-DESCRIPTION))
     (*CONTEXT* (%CLASSIFICATION-WORLD *CLASSIFICATIONSESSION*)))
    (CL:DECLARE (CL:SPECIAL *CLASSIFICATIONSESSION* *CONTEXT*))
    (CL:LET*
     ((VIRGIN? (VIRGIN? SELF)) (OLDCURRENTTIME (CURRENT-CLASSIFICATION-TIME)) (SUPERS NIL) (SUBS NIL)
      (EQUIVALENTS NIL))
     (CL:DECLARE (CL:TYPE CL:FIXNUM OLDCURRENTTIME)) (CL:WHEN VIRGIN? (INTRODUCE-INTO-CLASSIFICATION-SESSION SELF))
     (UPCLASSIFY-ONE-DESCRIPTION SELF)
     (CL:LET* ((SUPER NULL) (ITER-000 (ALL-NAMED-DIRECT-SUPERDESCRIPTIONS SELF CL:NIL)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SUPER (%%VALUE ITER-000))
       (UPCLASSIFY-ONE-DESCRIPTION SUPER) (CL:SETQ ITER-000 (%%REST ITER-000))))
     ...
     (CL:LET* ((E NULL) (ITER-001 (ALL-EQUIVALENT-COLLECTIONS SELF CL:NIL)) (COLLECT-000 NULL))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ E (%%VALUE ITER-001))
       ...
       (CL:SETQ ITER-001 (%%REST ITER-001))))
     (CL:WHEN VIRGIN? (EVAPORATE-VIRGIN SELF) (POP (%INTRODUCTION-TIMESTAMP-STACK *CLASSIFICATIONSESSION*)))
...
named descriptions among all descriptions that it satisfies."
  ...
  (CL:LET*
   ((*CLASSIFICATIONSESSION* (GET-CLASSIFICATION-SESSION KWD-CLASSIFY-INSTANCE))
    (*CONTEXT* (%CLASSIFICATION-WORLD *CLASSIFICATIONSESSION*)))
   (CL:DECLARE (CL:SPECIAL *CLASSIFICATIONSESSION* *CONTEXT*)) (UPCLASSIFY-ONE-INSTANCE SELF)
...
'module' is NULL, classify descriptions in all modules."
  (CL:LET*
   ((*CLASSIFICATIONSESSION* (GET-CLASSIFICATION-SESSION KWD-CLASSIFY-DESCRIPTION))
    (*CONTEXT* (%CLASSIFICATION-WORLD *CLASSIFICATIONSESSION*)))
   (CL:DECLARE (CL:SPECIAL *CLASSIFICATIONSESSION* *CONTEXT*))
   (CL:LET* ((*MODULE* (CL:IF (CL:EQ MODULE NULL) *MODULE* MODULE)) (*CONTEXT* *MODULE*))
...
"
  ...
  (CL:LET*
    ...
    (THEOPTIONS
     (PARSE-OPTIONS
      (CL:IF (KEYWORD? (%%VALUE OPTIONSLIST)) OPTIONSLIST
       (CONS-LIST KWD-CLASSIFY-MODULE (%%VALUE OPTIONSLIST) KWD-CLASSIFY-LOCAL? (%%VALUE (%%REST OPTIONSLIST))))
      (LIST* KWD-CLASSIFY-MODULE SGT-CLASSIFY-STELLA-MODULE KWD-CLASSIFY-LOCAL? SGT-CLASSIFY-STELLA-BOOLEAN NIL) CL:T
      CL:NIL))
   ...
   (CL:WHEN (CL:AND (CL:EQ THEMODULE NULL) (CL:NOT (MEMBER? OPTIONSLIST KWD-CLASSIFY-MODULE)))
    (CL:SETQ THEMODULE *MODULE*))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((*CLASSIFICATIONSESSION* (GET-CLASSIFICATION-SESSION KWD-CLASSIFY-DESCRIPTION))
     (*CONTEXT* (%CLASSIFICATION-WORLD *CLASSIFICATIONSESSION*)))
    (CL:DECLARE (CL:SPECIAL *CLASSIFICATIONSESSION* *CONTEXT*))
    (CL:LET* ((*MODULE* (CL:IF (CL:EQ THEMODULE NULL) *MODULE* THEMODULE)) (*CONTEXT* *MODULE*))
     (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
        ...
        (CL:WHEN TEST-VALUE-000
         (CL:LET* ((FOUND?-000 CL:NIL))
          (CL:LET* ((SUPER NULL) (ITER-001 (ALL-ANCESTOR-COLLECTIONS D KWD-CLASSIFY-DESCRIPTION)))
           (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ SUPER (%VALUE ITER-001))
            (CL:WHEN (NON-INFERABLE? SUPER) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))))
          (CL:SETQ TEST-VALUE-000 FOUND?-000))
    ...
    (CL:COND ((CL:EQ MODULE NULL) (UPCLASSIFY-ALL-INSTANCES))
     (LOCAL?
      (CL:LET* ((I NULL) (ITER-000 (ALL-INSTANCES MODULE CL:T)))
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ I (%VALUE ITER-000)) (UPCLASSIFY-ONE-INSTANCE I))))
     (CL:T
      (CL:LET* ((M NULL) (ITER-001 (ALL-INCLUDED-MODULES MODULE)))
       (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ M (%VALUE ITER-001))
        (CL:LET* ((I NULL) (ITER-002 (ALL-INSTANCES M CL:T)))
         (CL:LOOP WHILE (NEXT? ITER-002) DO (CL:SETQ I (%VALUE ITER-002)) (UPCLASSIFY-ONE-INSTANCE I))))))))))
        ...
        (CL:WHEN TEST-VALUE-000
         (CL:LET* ((FOUND?-000 CL:NIL))
          (CL:LET* ((TYPE NULL) (ITER-001 (ALL-ANCESTOR-COLLECTIONS I KWD-CLASSIFY-INSTANCE)))
           (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ TYPE (%VALUE ITER-001))
            (CL:WHEN (NON-INFERABLE? TYPE) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))))
          (CL:SETQ TEST-VALUE-000 FOUND?-000))
         (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000)))
        (CL:WHEN TEST-VALUE-000
         (CL:IF (CL:EQ COLLECT-000 NULL)
    ...
    (CL:LOOP (CL:SETQ U (POP UNSCANNEDEQUIVALENTS))
     (CL:LET* ((PARENT NULL) (ITER-000 (ALL-DIRECT-SUPERCOLLECTIONS U CL:NIL)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PARENT (%VALUE ITER-000))
       (CL:WHEN (CL:AND (CL:NOT (MEMBER? EQUIVALENTS PARENT)) (MEMBER? (ALL-SUPERCOLLECTIONS PARENT) U))
        (CL:SETQ EQUIVALENTS (CONS PARENT EQUIVALENTS)) (PUSH UNSCANNEDEQUIVALENTS PARENT))))
...
(CL:DEFUN HELP-STARTUP-CLASSIFY1 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-CLASSIFY-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   ...
   (CL:SETQ KWD-CLASSIFY-EXISTS (INTERN-RIGID-SYMBOL-WRT-MODULE "EXISTS" NULL 2))
   ...
   (CL:SETQ SGT-CLASSIFY-LOGIC-F-INFERABLE-WITH-CYCLE-CHECK?-MEMO-TABLE-000
    (INTERN-RIGID-SYMBOL-WRT-MODULE "F-INFERABLE-WITH-CYCLE-CHECK?-MEMO-TABLE-000" NULL 1))
   ...
   (CL:SETQ SYM-CLASSIFY-LOGIC-INFERABLE-DIRECT-SUBDESCRIPTIONS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INFERABLE-DIRECT-SUBDESCRIPTIONS" NULL 0))
   (CL:SETQ SYM-CLASSIFY-LOGIC-CACHE-UPCLASSIFICATION-TIMESTAMP
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CACHE-UPCLASSIFICATION-TIMESTAMP" NULL 0))
   (CL:SETQ SYM-CLASSIFY-LOGIC-CACHE-DOWNCLASSIFICATION-TIMESTAMP
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CACHE-DOWNCLASSIFICATION-TIMESTAMP" NULL 0))
   ...
   (CL:SETQ KWD-CLASSIFY-DESCRIPTION (INTERN-RIGID-SYMBOL-WRT-MODULE "DESCRIPTION" NULL 2))
   (CL:SETQ KWD-CLASSIFY-META (INTERN-RIGID-SYMBOL-WRT-MODULE "META" NULL 2))
   (CL:SETQ KWD-CLASSIFY-INSTANCE (INTERN-RIGID-SYMBOL-WRT-MODULE "INSTANCE" NULL 2))
   ...
   (CL:SETQ KWD-CLASSIFY-MODULE (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" NULL 2))
   (CL:SETQ KWD-CLASSIFY-LOCAL? (INTERN-RIGID-SYMBOL-WRT-MODULE "LOCAL?" NULL 2))
   (CL:SETQ SGT-CLASSIFY-STELLA-MODULE (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ SGT-CLASSIFY-STELLA-BOOLEAN
...
(CL:DEFUN HELP-STARTUP-CLASSIFY2 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "INFERABLE?"
    "(DEFUN (INFERABLE? BOOLEAN) ((SELF DESCRIPTION)) :INLINE INFERABLE-WITH-CYCLE-CHECK?)" (CL:FUNCTION INFERABLE?)
    NULL)
   (DEFINE-FUNCTION-OBJECT "NON-INFERABLE?" "(DEFUN (NON-INFERABLE? BOOLEAN) ((SELF DESCRIPTION)) :INLINE INFERABLE?)"
...
recorded in 'self'.\")" (CL:FUNCTION RECALL-MARKED-OBJECTS) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "ADD-ISA-LINK"
    "(DEFUN ADD-ISA-LINK ((INSTANCE LOGIC-OBJECT) (SUPERDESCRIPTION DESCRIPTION)))" (CL:FUNCTION ADD-ISA-LINK) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "NAMED-COLLECTION?"
    "(DEFUN (NAMED-COLLECTION? BOOLEAN) ((SELF LOGIC-OBJECT)) :PUBLIC? TRUE)" (CL:FUNCTION NAMED-COLLECTION?) NULL)
...
state of descriptions participating in a classification session.\" :TYPE CLASSIFICATION-SESSION :ALLOCATION :DYNAMIC)")
   (DEFINE-FUNCTION-OBJECT "GET-CLASSIFICATION-WORLD" "(DEFUN (GET-CLASSIFICATION-WORLD WORLD) () :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "GET-CLASSIFICATION-CACHE"
    "(DEFUN (GET-CLASSIFICATION-CACHE CLASSIFICATION-CACHE) ((SELF OBJECT)))" (CL:FUNCTION GET-CLASSIFICATION-CACHE)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CURRENT-CLASSIFICATION-TIME" "(DEFUN (CURRENT-CLASSIFICATION-TIME TIMESTAMP) ())"
    (CL:FUNCTION CURRENT-CLASSIFICATION-TIME) NULL)
   (DEFINE-FUNCTION-OBJECT "BUMP-CLASSIFICATION-TIMECLOCK" "(DEFUN (BUMP-CLASSIFICATION-TIMECLOCK TIMESTAMP) ())"
    (CL:FUNCTION BUMP-CLASSIFICATION-TIMECLOCK) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (INTRODUCTION-TIMESTAMP TIMESTAMP) ((SELF LOGIC-OBJECT)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "YOUNGEST-INTRODUCTION-TIMESTAMP" "(DEFUN (YOUNGEST-INTRODUCTION-TIMESTAMP INTEGER) ())"
    (CL:FUNCTION YOUNGEST-INTRODUCTION-TIMESTAMP) NULL)
   (DEFINE-FUNCTION-OBJECT "UPCLASSIFICATION-TIMESTAMP" "(DEFUN (UPCLASSIFICATION-TIMESTAMP INTEGER) ((SELF OBJECT)))"
    (CL:FUNCTION UPCLASSIFICATION-TIMESTAMP) NULL)
   (DEFINE-FUNCTION-OBJECT "DOWNCLASSIFICATION-TIMESTAMP"
    "(DEFUN (DOWNCLASSIFICATION-TIMESTAMP INTEGER) ((SELF LOGIC-OBJECT)))" (CL:FUNCTION DOWNCLASSIFICATION-TIMESTAMP)
    NULL)
   (DEFINE-FUNCTION-OBJECT "REFRESH-UPCLASSIFICATION-TIMESTAMP"
    "(DEFUN REFRESH-UPCLASSIFICATION-TIMESTAMP ((SELF OBJECT)))" (CL:FUNCTION REFRESH-UPCLASSIFICATION-TIMESTAMP) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "FLUSH-CLASSIFICATION-SESSIONS" "(DEFUN FLUSH-CLASSIFICATION-SESSIONS ())"
    (CL:FUNCTION FLUSH-CLASSIFICATION-SESSIONS) NULL)
   (DEFINE-FUNCTION-OBJECT "UPCLASSIFIED?" "(DEFUN (UPCLASSIFIED? BOOLEAN) ((SELF OBJECT)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "DOWNCLASSIFIED?" "(DEFUN (DOWNCLASSIFIED? BOOLEAN) ((SELF LOGIC-OBJECT)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "UPCLASSIFY-ONE-INSTANCE" "(DEFUN UPCLASSIFY-ONE-INSTANCE ((SELF LOGIC-OBJECT)))"
    (CL:FUNCTION UPCLASSIFY-ONE-INSTANCE) NULL)
   (DEFINE-FUNCTION-OBJECT "UPCLASSIFY-ONE-DESCRIPTION"
    "(DEFUN UPCLASSIFY-ONE-DESCRIPTION ((SELF LOGIC-OBJECT)) :PUBLIC? TRUE)" (CL:FUNCTION UPCLASSIFY-ONE-DESCRIPTION)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "EVAPORATE-VIRGIN" "(DEFUN EVAPORATE-VIRGIN ((SELF DESCRIPTION)))"
...
or relations that need to be checked for possible subsumption
relationships.
\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)" (CL:FUNCTION %CLASSIFY-RELATIONS)
...
(CL:DEFUN STARTUP-CLASSIFY ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/LOGIC" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*)) (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (HELP-STARTUP-CLASSIFY1))
...
Can be optimized for testing only, or also for recall.\" :SLOTS ((TEST-TABLE :TYPE HASH-TABLE) (RECALL-TABLE :TYPE LIST) (SUPPORTS-RECALL? :TYPE BOOLEAN)))")))
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-MARKER-TABLE-SLOT-VALUE)))
    (CL:LET*
     ((CLASS
...
limiting the number of concepts or relations that need to be checked for
possible is-a relationships.
\" :PUBLIC? TRUE :COMMAND? TRUE :EVALUATE-ARGUMENTS? FALSE)" (CL:FUNCTION %CLASSIFY-INSTANCES)
...
provably co-extensional.\")" (CL:FUNCTION ALL-CYCLES) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CLASSIFY" "(DEFUN STARTUP-CLASSIFY () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-CLASSIFY) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CLASSIFY-LOGIC-STARTUP-CLASSIFY)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CLASSIFY-STELLA-METHOD-STARTUP-CLASSNAME
...
or `list-unclassified-instances'.\" :PUBLIC? TRUE)")
    ...
    (REGISTER-NATIVE-NAME SYM-CLASSIFY-LOGIC-LIST-UNCLASSIFIED-RELATIONS KWD-CLASSIFY-COMMON-LISP
     KWD-CLASSIFY-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-CLASSIFY-LOGIC-CLASSIFY-INSTANCES KWD-CLASSIFY-COMMON-LISP KWD-CLASSIFY-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-CLASSIFY-LOGIC-LIST-UNCLASSIFIED-INSTANCES KWD-CLASSIFY-COMMON-LISP
     KWD-CLASSIFY-FUNCTION))))
