...
(CL:DECLAIM
  ...
  *CONTEXT* *QUERYITERATOR* TRUE NIL STANDARD-OUTPUT *REVERSEPOLARITY?*
  EOL TRUE-WRAPPER FALSE-WRAPPER FALSE NULL-INTEGER))
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        ...
        (CL:LET* ((ALWAYS?-000 TRUE))
         (CL:LET*
          ((V2 NULL) (V1 NULL) (ITER-001 BINDINGS)
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE
     ...
     (CL:PROGN (CL:SETQ ARGVALUE (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (%%PRINT-STREAM STREAM " ")
     (PRINT-AS-KIF-INTERNAL
      (CL:IF (CL:EQ ARGVALUE NULL) ARG ARGVALUE)))))
         ...
         (CL:LET* ((FOUND?-000 FALSE))
          (CL:LET*
           ((BINDING NULL) (ARG NULL)
            (VECTOR-001 (%ARGUMENTS (%PROPOSITION CACHE)))
       ...
       (CL:WHEN TEST-VALUE-001
        ...
        (CL:WHEN TEST-VALUE-001
         (CL:LET* ((FOUND?-001 FALSE))
          (CL:LET*
           ((BINDING NULL) (ARG NULL)
            (VECTOR-003 (%ARGUMENTS (%PROPOSITION CACHE)))
   ...
   (CL:WHEN SUCCESS?
    (CL:WHEN (CL:NOT (CL:EQ (%NEXT CACHE) NULL))
     ...
     (CL:WHEN (CL:EQ (%PREVIOUS CACHE) NULL)
      (SET-GOAL-CACHE-LIST (%PROPOSITION (%NEXT CACHE))
       (%NEXT CACHE))))
    ...
    (CL:WHEN
     ...
     (CL:LET*
      ((I NULL-INTEGER) (CACHEBINDING NULL) (GOALBINDING NULL)
       ...
       (LENGTH-000 (LENGTH VECTOR-000))
       (ITER-000 (%GOAL-BINDINGS GOAL)) (ITER-001 (%BINDINGS CACHE))
       (ITER-002 0))
        ...
        (CL:TAGBODY (CL:SETQ SUBARGBINDING (CL:AREF SUBBINDINGS I))
         ...
         (CL:WHEN (CL:NOT (CL:EQ UNIFIEDARG NULL))
          (CL:IF (CL:NOT (EQL? SUBARGBINDING UNIFIEDARG))
           (CL:RETURN-FROM CACHE-MATCHES-GOAL? FALSE)
           (CL:GO :CONTINUE)))
         ...
         (CL:LET*
          ...
          (CL:DECLARE
           (CL:TYPE CL:FIXNUM INDEX-003 LENGTH-003 ITER-004 I))
