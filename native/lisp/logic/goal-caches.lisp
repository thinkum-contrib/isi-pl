...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* *DONTUSEDEFAULTKNOWLEDGE?*
  DEFAULT-TRUE-TRUTH-VALUE DEFAULT-FALSE-TRUTH-VALUE *RECORD-JUSTIFICATIONS?*
  *INTEGER-MSB-MASK* *FAILED-GOAL-CUTOFFS* *SUCCEEDED-GOAL-CUTOFFS* NULL-FLOAT
  *CACHE-SUCCEEDED-GOALS?* *CACHE-FAILED-GOALS?* *DUPLICATE-SUBGOAL-STRATEGY*
  *TRACED-KEYWORDS* *CONTEXT* *QUERYITERATOR* TRUE NIL STANDARD-OUTPUT
  *REVERSEPOLARITY?* EOL TRUE-WRAPPER FALSE-WRAPPER FALSE NULL-INTEGER))
...
(CL:DEFUN NEW-GOAL-CACHE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%PROPOSITION SELF) NULL) (CL:SETF (%TIMESTAMP SELF) NULL-INTEGER)
  ...
  (CL:LET* ((GOAL (%PROPOSITION FRAME)) (TABLE (FIND-GOAL-CACHE-TABLE GOAL)))
   ...
   (CL:LET* ((GOALCACHE (LOOKUP-GOAL-CACHE TABLE)))
    (CL:WHEN (CL:EQ GOALCACHE NULL)
     ...
     (CL:SETQ GOALCACHE (NEW-GOAL-CACHE)) (REMOVE-OBSOLETE-GOAL-CACHES TABLE)
    ...
    (CL:LET* ((GOALCACHE (FIND-GOAL-CACHE FRAME)) (CACHEDBINDINGS NULL))
     ...
     (CL:CASE (LENGTH CACHEDBINDINGS)
      ...
      (CL:OTHERWISE (TRACE-GOAL-CACHE "USED MANY CACHED BINDINGS: " FRAME)
       ...
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
        SYM-GOAL-CACHES-LOGIC-CACHED-BINDINGS-ITERATOR ITERATOR NULL)))))
   (CREATE-CHOICE-POINT FRAME)
   (CL:LOOP WHILE (NEXT? ITERATOR) DO
    (CL:WHEN
     (BIND-VECTOR-OF-ARGUMENTS-TO-VALUES? (%ARGUMENTS (%PROPOSITION FRAME))
      (%VALUE ITERATOR))
...
(CL:DEFVAR *CACHE-SUCCEEDED-GOALS?* FALSE)

;;; (DEFGLOBAL *CACHE-FAILED-GOALS?* ...)

(CL:DEFVAR *CACHE-FAILED-GOALS?* FALSE)
...
(CL:DEFUN NEW-ATOMIC-GOAL-CACHE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%REVERSE-POLARITY? SELF) FALSE)
   (CL:SETF (%TRUTH-VALUE SELF) NULL) (CL:SETF (%TIMESTAMP SELF) NULL-INTEGER)
...
(CL:DEFUN PRINT-ATOMIC-GOAL-CACHE (SELF STREAM)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((ARGVALUE NULL) (ARG NULL) (VECTOR-000 (%ARGUMENTS (%PROPOSITION SELF)))
...
(CL:DEFUN PRINT-QUERY-RESULTS-CACHE (LIMIT)
  ...
  (CL:LET* ((COUNT LIMIT)) (CL:DECLARE (CL:TYPE CL:FIXNUM COUNT))
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Failed goal cutoffs:    "
    *FAILED-GOAL-CUTOFFS* EOL)
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Succeeded goal cutoffs: "
    *SUCCEEDED-GOAL-CUTOFFS* EOL EOL)
        ...
        (CL:WHEN TEST-VALUE-001
         (CL:LET* ((FOUND?-001 FALSE))
          (CL:LET*
           ...
           (CL:LOOP WHILE
            (CL:AND (CL:< INDEX-003 LENGTH-003) (CL:NOT (CL:EQ ITER-003 NIL)))
            DO
            (CL:PROGN
             (CL:SETQ ARG
...
(CL:DEFUN HASH-GOAL-ARGUMENT (ARG CODE)
  ...
  (CL:LET* ((HASHEDARG ARG))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ARG)))
    (CL:COND
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-GOAL-CACHES-LOGIC-SKOLEM) (CL:PROGN))
...
(CL:DEFUN GET-GOAL-CACHE-LIST (GOAL)
  (CL:LET*
   ((OPERATOR (CACHED-GOAL-OPERATOR GOAL))
    (DESCRIPTION
     (CL:IF (CL:NOT (CL:EQ OPERATOR NULL)) (GET-DESCRIPTION OPERATOR) NULL)))
   (CL:IF
    (CL:AND (CL:NOT (CL:EQ DESCRIPTION NULL)) (CL:NOT (DELETED? DESCRIPTION)))
    (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS DESCRIPTION)
...
(CL:DEFUN CREATE-ATOMIC-GOAL-CACHE (FRAME CACHE SUCCESS?)
  ...
  (CL:LET* ((PROPOSITION (%PROPOSITION FRAME)))
   ...
   (CL:SETF (%CACHED-GOAL-RESULT? FRAME) TRUE) CACHE))
  ...
  (CL:WHEN
   ...
   (CL:LET*
    ...
    (CL:WHEN
     ...
     (CL:LET*
      ((I NULL-INTEGER) (CACHEBINDING NULL) (GOALBINDING NULL) (GOALARG NULL)
       (VECTOR-000 GOALARGUMENTS) (INDEX-000 0)
     ...
     (CL:LET*
      ...
      (CL:COND ((CL:EQ MODE KWD-GOAL-CACHES-GOAL-INSTANTIATES-CACHE))
       ((CL:EQ MODE KWD-GOAL-CACHES-CACHE-INSTANTIATES-GOAL)
        (CL:SETQ SUBARGUMENTS CACHEARGUMENTS) (CL:SETQ SUBBINDINGS UVECTOR2)
       ...
       (CL:T
        (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
         ...
         (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
      (CL:LET*
       ((I NULL-INTEGER) (SUPERARG NULL) (SUBARG NULL)
        (VECTOR-001 SUBARGUMENTS) (INDEX-001 0)
        (LENGTH-001 (LENGTH VECTOR-001)) (VECTOR-002 SUPERARGUMENTS)
        (INDEX-002 0) (LENGTH-002 (LENGTH VECTOR-002)) (ITER-003 0))
       (CL:DECLARE
        (CL:TYPE CL:FIXNUM INDEX-001 LENGTH-001 INDEX-002 LENGTH-002 ITER-003
         I))
       (CL:LOOP WHILE
        ...
        (CL:TAGBODY (CL:SETQ SUBARGBINDING (CL:AREF SUBBINDINGS I))
         ...
         (CL:LET*
          ((I NULL-INTEGER) (SUPARG NULL) (VECTOR-003 SUPERARGUMENTS)
           (INDEX-003 0) (LENGTH-003 (LENGTH VECTOR-003)) (ITER-004 0))
          (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-003 LENGTH-003 ITER-004 I))
    ...
    (CL:LET* ((TEST-VALUE-000 (%KIND (%PROPOSITION FRAME))))
     (CL:COND
      ((CL:OR (CL:EQ TEST-VALUE-000 KWD-GOAL-CACHES-ISA)
       ...
       (CL:WHEN (TRACE-KEYWORD? KWD-GOAL-CACHES-GOAL-CACHES)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "=========> CACHED "
         (CL:IF SUCCESS? "SUCCESS" "FAILURE") " AT "
         (CL:LET ((CODE (GOAL-HASH-CODE FRAME)) (SIZE 1541))
          (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
          (CL:THE CL:FIXNUM
           (CL:REM
...
(CL:DEFUN ALL-CACHED-PROPOSITIONS (GOAL)
  (CL:LET*
   ...
   (CL:LET* ((SELF-000 (NEW-ATOMIC-GOAL-CACHE-ITERATOR)))
    (CL:SETF (%GOAL SELF-000) GOAL) (CL:SETF (%CACHE-LIST SELF-000) CACHELIST)
...
(CL:DEFUN STARTUP-GOAL-CACHES ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *CACHE-SUCCEEDED-GOALS?* TRUE)
    (CL:SETQ *CACHE-FAILED-GOALS?* TRUE)
...
for some named description\" :ALLOCATION :DYNAMIC)")
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-GOAL-CACHES-LOGIC-STARTUP-GOAL-CACHES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-GOAL-CACHES-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Goal-Caches") NULL-STRING-WRAPPER)))
