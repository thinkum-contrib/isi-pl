...
(CL:DECLAIM
 (CL:SPECIAL STELLA::*STARTUP-TIME-PHASE* STELLA::UNKNOWN-TRUTH-VALUE
  ...
  STELLA::*MODULE* STELLA::NIL STELLA::TRUE STELLA::*BOOTSTRAP-LOCK*
  STELLA::FALSE))

;;; (DEFGLOBAL *POWERLOOM-INITIALIZED?* ...)

(CL:DEFVAR *POWERLOOM-INITIALIZED?* STELLA::FALSE)
...
elements are available, `false' otherwise."
  (CL:LET* ((CURSOR (STELLA::%CURSOR SELF)))
   ...
   (CL:SETF (STELLA::%CURSOR SELF) (STELLA::%%REST CURSOR))
   STELLA::TRUE))
         ...
         (CL:LOOP WHILE (STELLA::NEXT? ITER-001) DO
          ...
          (CL:LET* ((TEST-VALUE-000 STELLA::FALSE))
           (CL:LET* ((ALWAYS?-000 STELLA::TRUE))
            (CL:LET*
             ((ARG2 STELLA::NULL) (INPUTARG STELLA::NULL)
              (ITER-002 ARGUMENTS) (VECTOR-000 (STELLA::%ARGUMENTS P))
...
and that are TRUE in the scope of the module `module'."
  (CL:LET* ((MDL-000 MODULE) (CXT-000 MDL-000))
   ...
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    ...
    (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
     (LIST-TO-PL-ITERATOR
      (STELLA::ALL-FACTS-OF-INSTANCE OBJECT STELLA::FALSE
       STELLA::FALSE))))))
...
Non-reflexive."
  (CL:LET* ((MDL-000 MODULE) (CXT-000 MDL-000))
   ...
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    ...
    (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
     (CONS-TO-PL-ITERATOR
      (STELLA::ALL-SUBRELATIONS RELATION STELLA::TRUE))))))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (STELLA::NEXT? ITER-000) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 STELLA::NULL)
      ...
      (CL:PROGN
       (CL:SETF (STELLA::%%REST COLLECT-000)
        (STELLA::CONS M STELLA::NIL))
       (CL:SETQ COLLECT-000 (STELLA::%%REST COLLECT-000))))))
   (CL:LET* ((MODULES VALUE-000)) (CONS-TO-PL-ITERATOR MODULES))))
    ...
    (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
     ...
     (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
      (CL:LET* ((STREAM STELLA::NULL))
       (CL:UNWIND-PROTECT
        (CL:PROGN
         (CL:SETQ STREAM (STELLA::NEW-OUTPUT-FILE-STREAM FILENAME))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:FIXNUM) CL:DOUBLE-FLOAT)
  GET-NTH-FLOAT))
...
elements are available, `false' otherwise.\")"
   ...
   (STELLA::DEFINE-METHOD-OBJECT
    "(DEFMETHOD (LENGTH INTEGER) ((SELF PL-ITERATOR)) :PUBLIC? TRUE :DOCUMENTATION \"Number of items in `self'.\")"
...
with the name `module-name', then a Stella `no-such-context-exception' is thrown.\")"
   ...
   (STELLA::DEFINE-FUNCTION-OBJECT "GET-NAME"
    "(DEFUN (GET-NAME STRING) ((OBJ OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Return the name of `obj', if it has one.  Otherwise return `null'.\")"
...
`relation-name'.  Include propositions that satisfy subrelations of the relation.\")"
   ...
   (STELLA::DEFINE-FUNCTION-OBJECT "GET-MODULES"
    "(DEFUN (GET-MODULES (PL-ITERATOR OF MODULE)) () :PUBLIC? TRUE :DOCUMENTATION \"Return all modules currently loaded into PowerLoom.\")"
...
the name of the returned object may not match `name' exactly.\")"
   ...
   (STELLA::DEFINE-FUNCTION-OBJECT "CREATE-ENUMERATED-LIST"
    "(DEFUN (CREATE-ENUMERATED-LIST LOGIC-OBJECT) ((MEMBERS CONS)) :PUBLIC? TRUE :DOCUMENTATION \"Create a logical term that denotes a list containing `members'.
Useful for passing lists as arguments to parameterized queries.\")"
    (CL:FUNCTION CREATE-ENUMERATED-LIST) STELLA::NULL)
   (STELLA::DEFINE-FUNCTION-OBJECT "CREATE-ENUMERATED-SET"
    "(DEFUN (CREATE-ENUMERATED-SET LOGIC-OBJECT) ((MEMBERS CONS)) :PUBLIC? TRUE :DOCUMENTATION \"Create a logical term that denotes the enumerated set containing `members'.\")"
...
(CL:DEFUN STARTUP-PLI ()
  (CL:LET*
   ...
   (CL:WHEN (STELLA::CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *POWERLOOM-INITIALIZED?* STELLA::FALSE)
   ...
   (CL:WHEN (STELLA::CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-PLI2)
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-NTH-VALUE"
     "(DEFUN (GET-NTH-VALUE OBJECT) ((SEQUENCE OBJECT) (N INTEGER)) :PUBLIC? TRUE :DOCUMENTATION \"Return the value in the `nth' column of `sequence'.
Counting starts at zero.  `sequence' must be of type proposition, cons,
vector or PL-iterator.  A zero column number returns a proposition's 
relational predicate.  For the PL-iterator case, the number of columns
is for the current value of the iterator.\")"
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-NTH-STRING"
     "(DEFUN (GET-NTH-STRING STRING) ((SEQUENCE OBJECT) (N INTEGER)) :PUBLIC? TRUE :DOCUMENTATION \"Return a string representation of the value in the `nth' column of
`sequence'. Counting starts at zero.  `sequence' must be of type
proposition, cons, vector or PL-iterator.  A zero column number returns
a proposition's relational predicate.  For the PL-iterator case, the
the current value pointed to by the iterator is used.  This will always
...
string reprensentation will be returned.\")"
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-NTH-INTEGER"
     "(DEFUN (GET-NTH-INTEGER INTEGER) ((SEQUENCE OBJECT) (N INTEGER)) :PUBLIC? TRUE :DOCUMENTATION \"Return an integer representation of the value in the `nth' column of
`sequence'. Counting starts at zero.  `sequence' must be of type
proposition, cons, vector or PL-iterator.  A zero column number returns
a proposition's relational predicate.  For the PL-iterator case, the
the current value pointed to by the iterator is used.  If this is not
an integer value, then NULL-INTEGER will be returned.\")"
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-NTH-FLOAT"
     "(DEFUN (GET-NTH-FLOAT FLOAT) ((SEQUENCE OBJECT) (N INTEGER)) :PUBLIC? TRUE :DOCUMENTATION \"Return the floating point value in the `nth' column of
`sequence'. Counting starts at zero.  `sequence' must be of type
proposition, cons, vector or PL-iterator.  A zero column number returns
a proposition's relational predicate.  For the PL-iterator case, the
the current value pointed to by the iterator is used.  If this is not
a floating point value, then NULL-FLOAT will be returned.\")"
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-NTH-LOGIC-OBJECT"
     "(DEFUN (GET-NTH-LOGIC-OBJECT LOGIC-OBJECT) ((SEQUENCE OBJECT) (N INTEGER)) :PUBLIC? TRUE :DOCUMENTATION \"Return a logic object representation of the value in the `nth' column
of `sequence'.  Counting starts at zero.  `sequence' must be of type
proposition, cons, vector or PL-iterator.  A zero column number returns
a proposition's relational predicate.  For the PL-iterator case, the
the current value pointed to by the iterator is used.  A zero column number
returns the proposition's relational predicate.\")"
    ...
    (STELLA::DEFINE-FUNCTION-OBJECT "GET-ENUMERATED-COLLECTION-MEMBERS"
     "(DEFUN (GET-ENUMERATED-COLLECTION-MEMBERS CONS) ((COLLECTION OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Returns the members of an enumerated collection.  This works on all types
of collection, i.e., sets and lists\")"
...
`no-such-context-exception' is thrown.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (STELLA::SET-DYNAMIC-SLOT-VALUE (STELLA::%DYNAMIC-SLOTS FUNCTION)
      ...
      (STELLA::WRAP-STRING "_Startup-Pli")
   ...
   (CL:WHEN (STELLA::CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (CL:SETQ *POWERLOOM-INITIALIZED?* STELLA::TRUE)
    (STELLA::DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
