     ...
     (CL:LET*
      ((ASSERTIONSONLY?
        (CL:AND (CL:NOT (CL:EQ ENVIRONMENT STELLA::NULL))
         (STELLA::STRING-EQL? (STELLA::%LEVEL ENVIRONMENT)
          "ASSERTION")))
       ...
       (CL:WHEN (CL:NOT (CL:EQ DOMAINPROP STELLA::NULL))
        (CL:RETURN-FROM GET-NTH-DOMAIN
         (CL:AREF
          ...
          (CL:THE CL:FIXNUM
           (CL:1-
            (STELLA::LENGTH (STELLA::%ARGUMENTS DOMAINPROP)))))))))))
   (CL:T))
  STELLA::NULL)
   ...
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    ...
    (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
     (STELLA::%LOAD FILENAME
      (STELLA::CONS-LIST KWD-PLI-MODULE MODULE))))))
...
not contain an `in-module' declaration, an error will be signaled.
The remaining commands are evaluated one-by-one, applying the function
`evaluate' to each of them.\")" (CL:FUNCTION LOAD-IN-MODULE)
    STELLA::NULL)
