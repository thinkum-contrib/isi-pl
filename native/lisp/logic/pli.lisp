...
(CL:DECLAIM
 (CL:SPECIAL STELLA::*STARTUP-TIME-PHASE* STELLA::INCONSISTENT-TRUTH-VALUE
  ...
  STELLA::*ROOT-MODULE* STELLA::*LOGIC-MODULE* STELLA::*CONTEXT*
  STELLA::*KIF-SENTENCE-OPERATORS* STELLA::*KIF-TERM-OPERATORS*
  STELLA::NULL-INTEGER STELLA::*MODULE* STELLA::NIL STELLA::*POWERLOOM-LOCK*
  STELLA::*BOOTSTRAP-LOCK*))
...
(CL:DEFUN TEST-ENVIRONMENT-LEVEL? (ENV LEVEL)
  ...
  (CL:AND (CL:NOT (CL:EQ ENV STELLA::NULL))
   (STELLA::STRING-EQL? (STELLA::%LEVEL ENV) LEVEL)))
...
If no such operator exists then a `no-such-object' exception is thrown."
  ...
  (CL:LET*
   ((OPERATOR
     (STELLA::LOOKUP-SYMBOL-IN-MODULE NAME STELLA::*LOGIC-MODULE* CL:NIL)))
   (CL:IF
    (CL:AND (CL:NOT (CL:EQ OPERATOR STELLA::NULL)) (LEGAL-OPERATOR? OPERATOR))
    OPERATOR
    (CL:LET* ((STREAM-000 (STELLA::NEW-OUTPUT-STRING-STREAM)))
     (STELLA::%%PRINT-STREAM (STELLA::%NATIVE-STREAM STREAM-000)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:T CL:T) CL:SIMPLE-STRING) GET-NAME-IN-MODULE))
      ...
      (CL:LET* ((VALUE-000 STELLA::NULL))
       ...
       (CL:LET*
        ...
        (CL:WHEN (CL:NOT ASSERTIONSONLY?)
         (CL:SETQ STELLA::*CONTEXT* (STELLA::GET-PROPERTY-TEST-CONTEXT)))
         ...
         (CL:LOOP WHILE (STELLA::NEXT? ITER-001) DO
          ...
          (CL:LET* ((TEST-VALUE-000 CL:NIL))
           (CL:LET* ((ALWAYS?-000 CL:T))
            (CL:LET*
             ((INPUTARG STELLA::NULL) (ITER-002 ARGUMENTS) (ARG2 STELLA::NULL)
...
the module `module'."
  (CL:LET* ((MDL-000 MODULE) (CXT-000 MDL-000))
   ...
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    ...
    (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
     (CONS-TO-PL-ITERATOR
      (STELLA::CONSIFY (STELLA::ALL-PROPOSITIONS STELLA::*MODULE* CL:T)))))))
          ...
          (STELLA::LIST* SYM-PLI-STELLA-EXISTS
           ...
           (STELLA::LIST* SYM-PLI-STELLA-AND
            (STELLA::LIST* SYM-PLI-PLI-PROPER-SUBRELATION
             ...
             (STELLA::CONS SYM-PLI-LOGIC-?Y STELLA::NIL))
            (STELLA::LIST* SYM-PLI-LOGIC-?Y SYM-PLI-LOGIC-?X STELLA::NIL)
            STELLA::NIL)
           STELLA::NIL)
          STELLA::NIL)
         STELLA::NIL)
  ...
  (CL:LET*
   ((FULL-NAME
     (CL:IF (CL:NOT (CL:EQ PARENT STELLA::NULL))
      (STELLA::CONCATENATE (STELLA::%MODULE-FULL-NAME PARENT) "/" NAME) NAME))
   ...
   (STELLA::DEFINE-MODULE FULL-NAME
    (STELLA::LIST* KWD-PLI-CASE-SENSITIVE? CASE-SENSITIVE-SYMBOL STELLA::NIL))))
     ...
     (CL:LET*
      ...
      (CL:COND
       ...
       ((CL:NOT (CL:EQ CONCEPT STELLA::NULL))
        (CL:SETQ NAME-SYMBOL (STELLA::INTERN-SYMBOL (GET-SHORT-NAME CONCEPT)))
        (CL:SETQ NAME-SYMBOL
      ...
      (CL:SETQ OBJECT-SURROGATE
       (STELLA::LOOKUP-SURROGATE-IN-MODULE (STELLA::%SYMBOL-NAME NAME-SYMBOL)
        (STELLA::%HOME-CONTEXT NAME-SYMBOL) CL:NIL))
      (CL:WHEN (CL:EQ OBJECT-SURROGATE STELLA::NULL)
       (CL:SETQ OBJECT-SURROGATE
        (STELLA::INTERN-SURROGATE-IN-MODULE (STELLA::%SYMBOL-NAME NAME-SYMBOL)
         (STELLA::%HOME-CONTEXT NAME-SYMBOL) CL:NIL)))
      (CL:SETQ OBJECT
       (STELLA::HELP-CREATE-LOGIC-INSTANCE OBJECT-SURROGATE STELLA::NULL))
...
The function found must conform to the signature for specialist functions."
  ...
  (CL:CHECK-TYPE MODULE-NAME CL:SIMPLE-STRING)
  (CL:LET*
   ((MDL-000 (SAFELY-GET-MODULE MODULE-NAME ENVIRONMENT)) (CXT-000 MDL-000))
   (CL:WHEN (CL:EQ MDL-000 STELLA::NULL) (CL:SETQ MDL-000 STELLA::*MODULE*)
    (CL:SETQ CXT-000 STELLA::*CONTEXT*))
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    (CL:DECLARE (CL:SPECIAL STELLA::*MODULE* STELLA::*CONTEXT*))
...
it does not look for the most specific one (which might have to be created)."
  ...
  (CL:WHEN (STELLA::CLASS? RELATION)
   (CL:RETURN-FROM GET-NTH-DOMAIN (CL:IF (CL:= N 0) RELATION STELLA::NULL)))
  (CL:COND
   ((STELLA::SUBTYPE-OF? (STELLA::SAFE-PRIMARY-TYPE RELATION)
     SGT-PLI-LOGIC-NAMED-DESCRIPTION)
    (CL:PROGN
      ...
      (CL:LET*
       ((DOMAINPROP
         (STELLA::%%VALUE
          (HELP-GET-PROPOSITIONS
           (STELLA::%SURROGATE-VALUE SGT-PLI-PL-KERNEL-KB-NTH-DOMAIN)
       ...
       (CL:WHEN (CL:NOT (CL:EQ DOMAINPROP STELLA::NULL))
        (CL:RETURN-FROM GET-NTH-DOMAIN
         (CL:AREF
          (CL:THE CL:SIMPLE-VECTOR
           (STELLA::%THE-ARRAY (STELLA::%ARGUMENTS DOMAINPROP)))
          (CL:THE CL:FIXNUM
           (CL:1- (STELLA::LENGTH (STELLA::%ARGUMENTS DOMAINPROP)))))))))))
   (CL:T))
  STELLA::NULL)
...
`evaluate' to each of them."
  (CL:LET* ((MDL-000 MODULE) (CXT-000 MDL-000))
   ...
   (CL:LET* ((STELLA::*MODULE* MDL-000) (STELLA::*CONTEXT* CXT-000))
    ...
    (CL:SETQ ENVIRONMENT ENVIRONMENT)
    (STELLA::WITH-PROCESS-LOCK STELLA::*POWERLOOM-LOCK*
     (STELLA::LOAD-STREAM-IN-MODULE STREAM MODULE)))))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:FIXNUM CL:T CL:T) CL:FIXNUM) GET-NTH-INTEGER))
  ...
  (CL:PROGN
   ...
   (CL:SETQ SYM-PLI-PLI-F-GET-INFERRED-BINARY-PROPOSITION-VALUES-QUERY-000
    (STELLA::INTERN-RIGID-SYMBOL-WRT-MODULE
     "F-GET-INFERRED-BINARY-PROPOSITION-VALUES-QUERY-000" STELLA::NULL 0))
   ...
   (CL:SETQ SYM-PLI-PLI-PROPER-SUBRELATION
    (STELLA::INTERN-RIGID-SYMBOL-WRT-MODULE "PROPER-SUBRELATION" STELLA::NULL
     0))
   ...
   (CL:SETQ KWD-PLI-CASE-SENSITIVE?
    (STELLA::INTERN-RIGID-SYMBOL-WRT-MODULE "CASE-SENSITIVE?" STELLA::NULL 2))
...
A module name of `null' or the empty string refers to the current module. 
If no module can be found with the name `module-name', then a Stella 
`no-such-context-exception' is thrown.\")" (CL:FUNCTION S-GET-PROPOSITIONS-OF)
    STELLA::NULL)
...
A module name of `null' or the empty string refers to the current module. 
If no module can be found with the name `module-name', then a Stella 
`no-such-context-exception' is thrown.\")" (CL:FUNCTION S-IS-TRUE-PROPOSITION)
    STELLA::NULL)
...
in `module' using `environment'.\")" (CL:FUNCTION CREATE-ENUMERATED-SET)
    STELLA::NULL)
...
A module name of `null' or the empty string refers to the current module. 
If no module can be found with the name `module-name', then a Stella 
`no-such-context-exception' is thrown.\")" (CL:FUNCTION S-DESTROY-OBJECT)
    STELLA::NULL)
...
in either antecedent or consequent position.\")" (CL:FUNCTION GET-RULES)
    STELLA::NULL)))
...
does not supply an `in-module' declaration, an error will be signaled.
The remaining commands are evaluated one-by-one, applying the function
`evaluate' to each of them.\")" (CL:FUNCTION LOAD-STREAM-IN-MODULE)
    STELLA::NULL)
...
does not supply an `in-module' declaration, an error will be signaled.
The remaining commands are evaluated one-by-one, applying the function
`evaluate' to each of them.\")" (CL:FUNCTION LOAD-NATIVE-STREAM-IN-MODULE)
    STELLA::NULL)
...
above.  This allows the use of `get-nth-float' on PL-iterators with only
a single return variable.    If  `sequence' cannot be turned into a floating
point value, an exception will be thrown.\")" (CL:FUNCTION GET-NTH-FLOAT)
    STELLA::NULL)
...
subsumes both sets and lists.\")" (CL:FUNCTION IS-ENUMERATED-COLLECTION)
    STELLA::NULL)
...
The returned truth value represents the logical truth of the queried sentence
as determined by PowerLoom.  It can be be tested via the functions `is-true',
`is-false' and `is-unknown' (which see).\" :PUBLIC? TRUE)" (CL:FUNCTION ASK)
    STELLA::NULL)
...
The returned truth value represents the logical truth of the queried sentence
as determined by PowerLoom.  It can be be tested via the functions `is-true',
`is-false' and `is-unknown' (which see).\" :PUBLIC? TRUE)" (CL:FUNCTION S-ASK)
    STELLA::NULL)
  ...
  (CL:LET*
   ...
   (CL:WHEN (STELLA::CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:LET* ((SELF-059 (NEW-ENVIRONMENT)))
     (CL:SETF (STELLA::%LEVEL SELF-059) "ASSERTION")
     (CL:SETQ ASSERTION-ENV SELF-059))
    (CL:LET* ((SELF-060 (NEW-ENVIRONMENT)))
     (CL:SETF (STELLA::%LEVEL SELF-060) "TAXONOMIC")
     (CL:SETQ TAXONOMIC-ENV SELF-060))
    (CL:LET* ((SELF-061 (NEW-ENVIRONMENT)))
     (CL:SETF (STELLA::%LEVEL SELF-061) "INFERENCE")
     (CL:SETQ INFERENCE-ENV SELF-061))
    (CL:LET* ((SELF-062 (NEW-PL-ITERATOR)))
     (CL:SETF (STELLA::%CURSOR SELF-062) STELLA::NIL)
     (CL:SETQ EMPTY-PL-ITERATOR SELF-062)))
