...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* NULL-INTEGER EOL NULL-FLOAT-WRAPPER
  *MODULE* FALSE-PROPOSITION TRUE-PROPOSITION FALSE-WRAPPER
  TRUE-WRAPPER NIL TRUE FALSE))
     ...
     (CL:LET* ((DISJUNCT ARG))
      (CL:IF (CL:EQ (%KIND DISJUNCT) KWD-NORMALIZE-NOT)
       (CL:SETQ NEGATEDGOALS
        (CONS
         (CL:AREF
          (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS DISJUNCT)))
          0)
         NEGATEDGOALS))
            ...
            (CL:LET* ((VAR NULL) (ITER-000 EXISTSVARIABLES))
             (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
              (CL:PROGN (CL:SETQ VAR (%%VALUE ITER-000))
               (CL:SETQ ITER-000 (%%REST ITER-000)))
              (CL:WHEN
               (VARIABLE-EQL? (%SKOLEM-NAME VAR) (%SKOLEM-NAME V))
               (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
       ...
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN
         (CL:SETQ COLLECT-000
          (CONS
           (CL:AREF
            (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY HOLDSARGUMENTS)) I)
           NIL))
  ...
  (CL:LET*
   ...
   (CL:LET
    ...
    (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
     (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))
   (CL:SETF (%ARGUMENTS SELF) NEWARGUMENTS)
   (NORMALIZE-PROPOSITION SELF)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Normalize") NULL-STRING-WRAPPER)))
