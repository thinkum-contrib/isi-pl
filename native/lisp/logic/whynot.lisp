...
(CL:DEFMETHOD CUTOFF-PARTIAL-MATCH? ((SELF WHYNOT-PARTIAL-MATCH) TRACE?)
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND (CL:NOT CUTOFF?)
     (CL:NOT (CL:= UNKNOWNGOALCUTOFF NULL-INTEGER)))
    (CL:SETQ CUTOFF?
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
      ...
      (CL:LET*
       ((NEWVAR NULL) (NEWVALUE NULL)
        (ITER-001 (ALLOCATE-ITERATOR (%SUBSTITUTION NEWDEV)))
        (REPVAR NULL) (REPVALUE NULL)
        (ITER-002
         (ALLOCATE-ITERATOR
          (%SUBSTITUTION (%DEVIATING-PATTERN CLASS)))))
       (CL:LOOP WHILE (CL:AND (NEXT? ITER-001) (NEXT? ITER-002)) DO
        (CL:SETQ NEWVAR (%KEY ITER-001))
  ...
  (CL:LET* ((SOLUTION NIL) (SUBSTITUTION (%SUBSTITUTION PROOF)))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:LET*
      ((VAR NULL) (VALUE NULL)
       (ITER-000 (ALLOCATE-ITERATOR SUBSTITUTION)))
