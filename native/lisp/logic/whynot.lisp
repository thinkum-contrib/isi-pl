...
(CL:DECLAIM
  ...
  *SIMILAR-WHYNOT-PROOF-CUTOFF* NIL-LIST UNKNOWN-TRUTH-VALUE *TRACED-KEYWORDS*
  ...
  TRUE-TRUTH-VALUE *MINIMUM-SCORE-CUTOFF* NULL-FLOAT-WRAPPER STANDARD-OUTPUT
  EOL *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE *QUERYITERATOR* NIL
...
(CL:DEFMETHOD COMPUTE-UNKNOWN-GOAL-CUTOFF ((SELF WHYNOT-PARTIAL-MATCH))
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ PARENT NULL))
    ...
    (CL:WHEN (CL:= PARENTCUTOFF NULL-INTEGER)
...
(CL:DEFMETHOD CUTOFF-PARTIAL-MATCH? ((SELF WHYNOT-PARTIAL-MATCH) TRACE?)
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND (CL:NOT CUTOFF?) (CL:NOT (CL:= UNKNOWNGOALCUTOFF NULL-INTEGER)))
   ...
   (CL:LET* ((RESULT (CONTINUE-AND-PROOF FRAME LASTMOVE)))
    (CL:COND
     ((CL:EQ RESULT KWD-WHYNOT-MOVE-DOWN)
      ...
      (CL:WHEN
       (CL:AND (CL:NOT (CL:= (%ALLOTTED-CLOCK-TICKS FRAME) NULL-INTEGER))
        (CL:EQ RESULT KWD-WHYNOT-MOVE-DOWN))
...
(CL:DEFMETHOD CONTINUE-PARTIAL-ANTECEDENTS-PROOF ((SELF WHYNOT-PARTIAL-MATCH) LASTMOVE)
  (CL:LET*
   ...
   (CL:WHEN
    (CL:NOT
     (CL:OR (CL:EQ LASTMOVE KWD-WHYNOT-UP-TRUE)
      (CL:= ALLOTTEDCLOCKTICKS NULL-INTEGER)))
    (CL:SETF (%ALLOTTED-CLOCK-TICKS FRAME)
     (CL:TRUNCATE
      (CL:/
       (CL:FLOAT
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((*QUERYITERATOR* QUERY) (*GENERATE-ALL-PROOFS?* CL:T)
     (*RECORD-JUSTIFICATIONS?* CL:T) (*INFERENCELEVEL* INITIALINFERENCELEVEL)
    ...
    (CL:LOOP (CL:WHEN (CL:NOT (NEXT? QUERY)) (CL:RETURN))
     ...
     (CL:WHEN
      (CL:OR (CL:NOT MAXIMIZESCORE?)
       (KNOWN-TRUTH-VALUE? (%TRUTH-VALUE BASEFRAME)))
      (CL:RETURN))))
    ...
    (CL:WHEN (CL:EQ (%INFERENCE-RULE PROOF1) KWD-WHYNOT-PATTERN)
     (CL:LET*
      ((VAR1 NULL) (VALUE1 NULL)
       (ITER-000 (ALLOCATE-ITERATOR (%SUBSTITUTION PROOF1))) (VAR2 NULL)
       (VALUE2 NULL) (ITER-001 (ALLOCATE-ITERATOR (%SUBSTITUTION PROOF2))))
      (CL:LOOP WHILE (CL:AND (NEXT? ITER-000) (NEXT? ITER-001)) DO
       (CL:SETQ VAR1 (%KEY ITER-000)) (CL:SETQ VALUE1 (%VALUE ITER-000))
       (CL:SETQ VAR2 (%KEY ITER-001)) (CL:SETQ VALUE2 (%VALUE ITER-001))
   ...
   (CL:WHEN (CL:NOT (CL:= FOUNDDEVIATIONS NULL-INTEGER))
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
      (CL:SETQ NEWDEV (%KEY ITER-000)) (CL:SETQ REPDEV (%VALUE ITER-000))
      ...
      (CL:LET*
       ((NEWVAR NULL) (NEWVALUE NULL)
        (ITER-001 (ALLOCATE-ITERATOR (%SUBSTITUTION NEWDEV))) (REPVAR NULL)
        (REPVALUE NULL)
        (ITER-002
         (ALLOCATE-ITERATOR (%SUBSTITUTION (%DEVIATING-PATTERN CLASS)))))
       (CL:LOOP WHILE (CL:AND (NEXT? ITER-001) (NEXT? ITER-002)) DO
        (CL:SETQ NEWVAR (%KEY ITER-001)) (CL:SETQ NEWVALUE (%VALUE ITER-001))
     ...
     (CL:TAGBODY (CLEAR (%SOLUTIONS QUERY))
      ...
      (CL:LET* ((TEST-VALUE-000 CL:NIL))
       ...
       (CL:IF TEST-VALUE-000
        (CL:PROGN
         (CL:WHEN (KNOWN-TRUTH-VALUE? (%TRUTH-VALUE BASEFRAME))
      ...
      (CL:SETQ TOPNPARTIALPROOFS
       (CL:IF (CL:= HOWMANY NULL-INTEGER) TOPN
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     (CL:SETQ EXTVAR
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
     (CL:LET*
      ((VAR NULL) (VALUE NULL) (ITER-000 (ALLOCATE-ITERATOR SUBSTITUTION)))
  ...
  (CL:LET*
   ((PARTIALQUERY
     (CREATE-QUERY-ITERATOR
      ...
      (%BINDINGS (%VALUE QUERY)) "PartialQuery"))
      ...
      (CL:WHEN (CL:EQ LABEL STELLA::NULL-STRING)
       (CL:LET*
        ((INFO
           ...
           *MOST-RECENT-EXPLANATION-MAPPING*)))
        (CL:SETF (%LABEL INFO) (INTEGER-TO-STRING (CL:TRUNCATE I)))
   ...
   (CL:COND
    ...
    (SUMMARY?
     (CL:LET* ((FAILURES (NEW-LIST)))
      ...
      (CL:LET*
       ((FAILURE NULL) (ITER-000 (%THE-CONS-LIST FAILURES)) (I NULL-INTEGER)
        (ITER-001 1))
       (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
  ...
  (CL:PROGN
   ...
   (CL:SETQ
    ...
    (INTERN-RIGID-SYMBOL-WRT-MODULE
     "M-WHYNOT-PARTIAL-MATCH.NUMBER-OF-FAILED-SUBGOALS-MEMO-TABLE-000" NULL
     1))
