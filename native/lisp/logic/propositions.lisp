...
(CL:DECLAIM
  ...
  *KIF-SENTENCE-OPERATORS* *REVERSEPOLARITY?*
  ...
  *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE *INVISIBLEASSERTION?*
  *MODULE* EOL TRUE NULL-STRING-WRAPPER *PRINTREADABLY?*
  NULL-FLOAT-WRAPPER NIL-NON-PAGING-INDEX NULL-FLOAT NULL-INTEGER
  *LOADINGREGENERABLEOBJECTS?* TRUE-WRAPPER FALSE-WRAPPER FALSE NIL
  NIL-LIST *CONTEXT*))
...
(CL:DEFUN LOGICAL-SUBTYPE-OF-LITERAL? (TYPE)
  (CL:LET*
   ...
   (CL:COND
    ((CL:EQ DESC NULL)
     (CL:RETURN-FROM LOGICAL-SUBTYPE-OF-LITERAL? FALSE))
...
(CL:DEFUN LOG-NEWLY-CONCEIVED-PROPOSITION (SELF PROPOSITION)
  (CL:LET* ((FOUND?-000 FALSE))
   ...
   (CL:WHEN FOUND?-000
    (CL:WARN "LOGGED QUESTIONABLE CONCEPTION: ~%   `~A'~%"
     PROPOSITION)))
...
(CL:DEFUN ASSERT-DESCRIPTION-IMPLIES-DESCRIPTION (SUB SUPER DONTUPDATE?)
  (CL:LET* ((IMPLIESPROP (CREATE-IMPLIES-PROPOSITION SUB SUPER)))
   (CL:SETQ IMPLIESPROP
    (FASTEN-DOWN-ONE-PROPOSITION IMPLIESPROP FALSE))
...
(CL:DEFUN FINALIZE-PROPOSITION-ANNOTATIONS (PROPOSITION)
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ ANNOTATIONS NULL))
    (CL:LET*
     ((VALUE NULL) (OPTION NULL) (ITER-000 (%THE-PLIST ANNOTATIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN BUILD-AND-OR-NOT-PROPOSITION (TREE)
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ARG NULL) (ITER-001 ARGUMENTS) (ITER-002 0))
...
(CL:DEFUN BUILD-HOLDS-PROPOSITION (TREE)
  (CL:LET*
   ...
   (CL:WHEN EXPLICITHOLDS?
    (CL:LET*
     ((I NULL-INTEGER) (ARG NULL) (ITER-000 (%%REST TREE))
      (ITER-001 0))
     (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 I))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ ARG (%%VALUE ITER-000))
   ...
   (CL:LET* ((OPERATOR NULL))
    ...
    (CL:LET
     ...
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:LET*
     ((I NULL-INTEGER) (ARG NULL) (ITER-002 (%%REST TREE))
      (ITER-003 1))
       ...
       (CL:LET*
        ...
        (CL:LOOP WHILE
         ...
         (CL:WHEN
          ...
          (VERIFY-ONE-ARGUMENT-TYPE ARG TYPE PROPOSITION
           DESCRIPTION))))
       (CL:WHEN FUNCTION?
      ...
      (CL:WHEN
       ...
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
          "ERROR: Illegal implication rule, consequent has variables"
          EOL "   that are not restricted by the antecedent: " EOL
          "   `" PROPOSITION "'." EOL)
         (HELP-SIGNAL-PROPOSITION-ERROR STREAM-000
          KWD-PROPOSITIONS-ERROR))
        (CL:ERROR
...
(CL:DEFUN BEGIN-CREATING-FUNCTION-PROPOSITION (SURROGATE INPUTARGUMENTS)
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ARG NULL) (ITER-000 INPUTARGUMENTS)
     (ITER-001 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 I))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ ARG (%%VALUE ITER-000))
      ...
      (CL:LET* ((FOUND?-001 FALSE))
       (CL:LET* ((ARG NULL) (ITER-001 (%%REST TREE)))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
         (CL:PROGN (CL:SETQ ARG (%%VALUE ITER-001))
          (CL:SETQ ITER-001 (%%REST ITER-001)))
         (CL:WHEN (CL:EQ ARG F) (CL:SETQ FOUND?-001 TRUE)
          (CL:RETURN))))
       (CL:WHEN FOUND?-001
      ...
      (CL:LET* ((FOUND?-002 FALSE))
       (CL:LET* ((ARG NULL) (ITER-002 (%%REST TREE)))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
         (CL:PROGN (CL:SETQ ARG (%%VALUE ITER-002))
          (CL:SETQ ITER-002 (%%REST ITER-002)))
         (CL:WHEN (CL:EQ ARG T) (CL:SETQ FOUND?-002 TRUE)
          (CL:RETURN))))
       (CL:WHEN FOUND?-002
        (CL:RETURN-FROM REMOVE-TRUE-AND-FALSE
...
(CL:DEFUN FLATTEN-NESTED-OPERATORS (TREE)
  ...
  (CL:LET* ((TARGETOPERATOR (%%VALUE TREE)) (TAIL (%%REST TREE)))
   (CL:LOOP
       ...
       ((CL:EQ NESTEDARGS NIL)
        (CL:COND
         ...
         (CL:T
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`"
            TARGETOPERATOR "' is not a valid case option")
           (CL:ERROR
            (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
    ...
    (CL:WHEN (CL:EQ TAIL NIL) (CL:RETURN)))
   (CL:VALUES (CANONICALIZE-PROPOSITION-TREE TREE) TRUE)))
   ...
   (CL:COND
     ...
     (CL:LET* ((WHERECLAUSE (%%VALUE (%%REST (%%REST ARGUMENT)))))
      (CL:RETURN-FROM CANONICALIZE-NEGATION-TREE
       (CANONICALIZE-PROPOSITION-TREE
        (LIST* SYM-PROPOSITIONS-STELLA-FORALL
         (%%VALUE (%%REST ARGUMENT))
    ...
    ((CL:EQ NESTEDOPERATOR SYM-PROPOSITIONS-STELLA-FORALL)
     (CL:LET*
      ...
      (CL:RETURN-FROM CANONICALIZE-NEGATION-TREE
       (CANONICALIZE-PROPOSITION-TREE
        (LIST* SYM-PROPOSITIONS-STELLA-EXISTS
         (%%VALUE (%%REST ARGUMENT))
         (CONS
...
(CL:DEFUN EXPLODE-FORALL-TREE (TREE)
  (CL:LET*
   ...
   (CL:LET*
    ((ANTECEDENTGOAL ANTECEDENT) (CONJUNCTS (%%REST CONSEQUENT)))
...
(CL:DEFUN CLEAR-LOGIC-MODULE-HOOK (MODULE)
  (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
   ...
   (CL:LET*
    ((PROPOSITION NULL) (ITER-001 (ALL-PROPOSITIONS MODULE TRUE)))
    (CL:LOOP WHILE (NEXT? ITER-001) DO
  ...
  (CL:LET* ((I2 NULL) (I1 NULL) (ITER-000 O1) (ITER-001 O2))
   (CL:LOOP WHILE
    ...
    (CL:WHEN (LOGIC-FORM-LESS? I1 I2)
     (CL:RETURN-FROM CONS-LESS-THAN? TRUE))
    (CL:WHEN (LOGIC-FORM-LESS? I2 I1)
     (CL:RETURN-FROM CONS-LESS-THAN? FALSE))))
  (< (LENGTH O1) (LENGTH O2)))
...
(CL:DEFUN OBJECT-SORT-STRING (O)
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE O)))
   (CL:COND
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-PROPOSITIONS-LOGIC-SKOLEM)
     (CL:PROGN
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Skolem " O
       " value = "
...
(CL:DEFUN HELP-STARTUP-PROPOSITIONS6 ()
  (CL:PROGN (CL:SETQ ZERO-VARIABLES-VECTOR (NEW-VECTOR 0))
   ...
   (CL:SETQ *CLIPPINGENABLED?* TRUE)
   (CL:SETQ *NATURALDEDUCTIONMODE?* TRUE)
