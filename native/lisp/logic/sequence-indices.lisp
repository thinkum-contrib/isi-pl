...
(CL:DEFUN ACCESS-SEQUENCE-INDEX-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-SEQUENCE-INDICES-LOGIC-THE-SEQUENCE) (CL:IF SETVALUE? (CL:SETF (%THE-SEQUENCE SELF) VALUE) (CL:SETQ VALUE (%THE-SEQUENCE SELF))))
   ((CL:EQ SLOTNAME SYM-SEQUENCE-INDICES-LOGIC-THE-SEQUENCE-LENGTH)
    (CL:IF SETVALUE? (CL:SETF (%THE-SEQUENCE-LENGTH SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%THE-SEQUENCE-LENGTH SELF)))))
   (CL:T
...
(CL:DEFUN NEW-NON-PAGING-INDEX ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE NON-PAGING-INDEX))) (CL:SETF (%THE-SEQUENCE-LENGTH SELF) NULL-INTEGER)
...
(CL:DEFCLASS PAGING-INDEX (SEQUENCE-INDEX)
  ((SELECTION-PATTERN :ALLOCATION :INSTANCE :ACCESSOR %SELECTION-PATTERN) (STORE :ALLOCATION :INSTANCE :ACCESSOR %STORE))
...
(CL:DEFUN NEW-PAGING-INDEX ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE PAGING-INDEX))) (CL:SETF (%THE-SEQUENCE-LENGTH SELF) NULL-INTEGER) (CL:SETF (%THE-SEQUENCE SELF) NIL)
   (CL:SETF (%STORE SELF) NULL) (CL:SETF (%SELECTION-PATTERN SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-PAGING-INDEX-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SEQUENCE-INDICES-LOGIC-SELECTION-PATTERN) (CL:IF SETVALUE? (CL:SETF (%SELECTION-PATTERN SELF) VALUE) (CL:SETQ VALUE (%SELECTION-PATTERN SELF))))
   ((CL:EQ SLOTNAME SYM-SEQUENCE-INDICES-LOGIC-STORE) (CL:IF SETVALUE? (CL:SETF (%STORE SELF) VALUE) (CL:SETQ VALUE (%STORE SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
  VALUE)
...
(CL:DEFMETHOD FIRST ((SELF SEQUENCE-INDEX))
  (CL:LET* ((ITEM NULL) (ITER-000 (ALLOCATE-ITERATOR SELF))) (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ITEM (%VALUE ITER-000)) (CL:RETURN-FROM FIRST ITEM)))
  NULL)
...
(CL:DEFUN SEQUENCE-INDEX.ESTIMATED-LENGTH (SELF)
  (CL:LET* ((LENGTH (%THE-SEQUENCE-LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:WHEN (CL:= LENGTH NULL-INTEGER) (CL:SETQ LENGTH (LENGTH (%THE-SEQUENCE SELF))) (CL:SETF (%THE-SEQUENCE-LENGTH SELF) LENGTH)) LENGTH))

;;; (DEFMETHOD (REMOVE-DELETED-MEMBERS (LIKE SELF)) ...)

(CL:DEFMETHOD REMOVE-DELETED-MEMBERS ((SELF SEQUENCE-INDEX))
  (CL:LET* ((SEQUENCE (REMOVE-DELETED-MEMBERS (%THE-SEQUENCE SELF)))) (CL:SETF (%THE-SEQUENCE SELF) SEQUENCE) (CL:SETF (%THE-SEQUENCE-LENGTH SELF) NULL-INTEGER) SELF))
   ...
   (CL:LET* ((I NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ I (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS I NIL)) (CL:IF (CL:EQ VALUE-000 NIL) (CL:SETQ VALUE-000 COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST VALUE-000 COLLECT-000)))
...
(CL:DEFUN PRINT-SEQUENCE-INDEX (SELF STREAM)
  (CL:LET* ((LIMIT 10)) (CL:DECLARE (CL:TYPE CL:FIXNUM LIMIT)) (%%PRINT-STREAM STREAM "|SEQ-IDX|(")
   (CL:LET* ((ELT NULL) (ITER-000 (%THE-SEQUENCE SELF)) (I NULL-INTEGER) (ITER-001 1) (UPPER-BOUND-000 LIMIT) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000))) DO (CL:SETQ ELT (%%VALUE ITER-000))
     (CL:SETQ I ITER-001) (CL:WHEN (CL:> I 1) (%%PRINT-STREAM STREAM " ")) (%%PRINT-STREAM STREAM ELT) (CL:WHEN (CL:= I LIMIT) (%%PRINT-STREAM STREAM "..."))
...
(CL:DEFUN NEW-SEQUENCE-INDEX-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE SEQUENCE-INDEX-ITERATOR))) (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) NULL)
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF SEQUENCE-INDEX))
  (CL:LET* ((SELF-000 (NEW-SEQUENCE-INDEX-ITERATOR))) (CL:SETF (%LIST-ITERATOR-CURSOR SELF-000) (%THE-SEQUENCE SELF)) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000)))
...
(CL:DEFMETHOD NEXT? ((SELF SEQUENCE-INDEX-ITERATOR))
  (CL:LET* ((CURSOR (%LIST-ITERATOR-CURSOR SELF)) (VALUE NULL))
   (CL:LOOP (CL:WHEN (CL:EQ CURSOR NIL) (CL:RETURN-FROM NEXT? CL:NIL)) (CL:SETQ VALUE (%%VALUE CURSOR))
    (CL:WHEN (CL:NOT (DELETED? VALUE)) (CL:SETF (%VALUE SELF) VALUE) (CL:SETF (%LIST-ITERATOR-CURSOR SELF) (%%REST CURSOR)) (CL:RETURN-FROM NEXT? CL:T))
...
(CL:DEFMETHOD COPY ((SELF NON-PAGING-INDEX))
  (CL:LET* ((SELF-000 (NEW-NON-PAGING-INDEX))) (CL:SETF (%THE-SEQUENCE SELF-000) (COPY-CONS-LIST (%THE-SEQUENCE SELF))) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000)))
...
(CL:DEFMETHOD REMOVE ((SELF PAGING-INDEX) VALUE)
  (CL:LET* ((SEQUENCE (REMOVE (%THE-SEQUENCE SELF) VALUE))) (CL:SETF (%THE-SEQUENCE SELF) SEQUENCE) (CL:SETF (%THE-SEQUENCE-LENGTH SELF) NULL-INTEGER) SELF))

;;; (DEFUN PRINT-PAGING-INDEX ...)

(CL:DEFUN PRINT-PAGING-INDEX (SELF STREAM)
  (CL:LET* ((LIMIT 10)) (CL:DECLARE (CL:TYPE CL:FIXNUM LIMIT)) (%%PRINT-STREAM STREAM "|PAGING-IDX|(" (%SELECTION-PATTERN SELF))
   (CL:LET* ((ELT NULL) (ITER-000 (%THE-SEQUENCE SELF)) (I NULL-INTEGER) (ITER-001 1) (UPPER-BOUND-000 LIMIT) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000))) DO (CL:SETQ ELT (%%VALUE ITER-000))
     (CL:SETQ I ITER-001) (%%PRINT-STREAM STREAM " " ELT) (CL:WHEN (CL:= I LIMIT) (%%PRINT-STREAM STREAM "...")) (CL:SETQ ITER-000 (%%REST ITER-000))
...
(CL:DEFUN NEW-PAGING-INDEX-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE PAGING-INDEX-ITERATOR))) (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) NULL)
   (CL:SETF (%LIST-ITERATOR-CURSOR SELF) NULL) SELF))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF PAGING-INDEX))
  (CL:LET* ((SELF-000 (NEW-PAGING-INDEX-ITERATOR))) (CL:SETF (%LIST-ITERATOR-CURSOR SELF-000) (%THE-SEQUENCE SELF)) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000)))
...
(CL:DEFUN ACCESS-OBJECT-STORE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-SEQUENCE-INDICES-STELLA-MODULE) (CL:IF SETVALUE? (CL:SETF (%MODULE SELF) VALUE) (CL:SETQ VALUE (%MODULE SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
  VALUE)
...
(CL:DEFUN UNREGISTER-OBJECT-STORE (STORE)
  ...
  (CL:WHEN (CL:NOT (NULL-LIST? *ALL-OBJECT-STORES*)) (REMOVE *ALL-OBJECT-STORES* STORE) (CL:WHEN (EMPTY? *ALL-OBJECT-STORES*) (CL:SETQ *ALL-OBJECT-STORES* NIL-LIST))))
...
(CL:DEFUN HOME-OBJECT-STORE (SELF)
  (CL:LET* ((MODULE (HOME-MODULE SELF)))
   (CL:IF (CL:NOT (CL:EQ MODULE NULL)) (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS MODULE) SYM-SEQUENCE-INDICES-LOGIC-OBJECT-STORE NULL) NULL)))
...
(CL:DEFUN CREATE-SEQUENCE-INDEX (SELF PATTERN)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000) (CL:PROGN (KEYWORD.CREATE-SEQUENCE-INDEX SELF PATTERN)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SEQUENCE-INDICES-STELLA-MODULE) (CL:PROGN (MODULE.CREATE-SEQUENCE-INDEX SELF PATTERN)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SEQUENCE-INDICES-LOGIC-OBJECT-STORE) (CL:PROGN (OBJECT-STORE.CREATE-SEQUENCE-INDEX SELF PATTERN)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Don't know how to create a sequence index on `" SELF "'")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))))
  ...
  (CL:COND
   ((CL:EQ KIND KWD-SEQUENCE-INDICES-PAGING)
    (CL:LET* ((ANCHOR (%%VALUE (%%REST (%%REST PATTERN)))) (STORE NULL))
     (CL:WHEN (CL:EQ ANCHOR NULL) (CL:ERROR "Safety violation: Null argument passed into paging index pattern")) (CL:SETQ STORE (HOME-OBJECT-STORE ANCHOR))
...
(CL:DEFUN MODULE.CREATE-SEQUENCE-INDEX (MODULE PATTERN)
  (CL:LET* ((STORE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS MODULE) SYM-SEQUENCE-INDICES-LOGIC-OBJECT-STORE NULL)))
   (CL:IF (CL:NOT (CL:EQ STORE NULL)) (CREATE-SEQUENCE-INDEX STORE PATTERN) (CREATE-SEQUENCE-INDEX KWD-SEQUENCE-INDICES-PAGING PATTERN))))
...
(CL:DEFMETHOD OBJECT-STORE.CREATE-SEQUENCE-INDEX ((STORE OBJECT-STORE) PATTERN)
  (CL:LET* ((SELF-000 (NEW-PAGING-INDEX))) (CL:SETF (%STORE SELF-000) STORE) (CL:SETF (%SELECTION-PATTERN SELF-000) PATTERN) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000)))

;;; (DEFUN (MAYBE-WRAP-SEQUENCE-INDEX SEQUENCE-INDEX) ...)

(CL:DEFUN MAYBE-WRAP-SEQUENCE-INDEX (INDEX PATTERN KIND ARG1 ARG2)
  (CL:LET* ((STORE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS *MODULE*) SYM-SEQUENCE-INDICES-LOGIC-OBJECT-STORE NULL)) (BASEINDEX INDEX))
   (CL:WHEN (CL:AND (CL:NOT (CL:EQ STORE NULL)) (CL:NOT (DESCRIPTION-MODE?)) (CL:NOT (VARIABLE? ARG1)))
    (CL:COND
     ...
     (CL:T))
    (CL:WHEN (CL:EQ PATTERN NULL) (CL:SETQ PATTERN (CONS KIND (CONS (CL:IF (CL:EQ NULL NULL) NIL NULL) (CONS ARG1 (CL:IF (CL:EQ ARG2 NULL) NIL (CONS ARG2 NIL)))))))
    (CL:SETQ INDEX (OBJECT-STORE.CREATE-SEQUENCE-INDEX STORE PATTERN)) (CL:SETF (%THE-SEQUENCE INDEX) (%THE-SEQUENCE BASEINDEX))
...
specialized on actual OBJECT-STORE implementations."
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Don't know how to fetch relation `" NAME "' from `" STORE "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
specialized on actual OBJECT-STORE implementations."
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Don't know how to fetch instance `" NAME "' from `" STORE "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
specialized on actual OBJECT-STORE implementations."
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Don't know how to fetch a duplicate of `" PROP "' from `" STORE "'")
   (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
...
(CL:DEFUN HELP-STARTUP-SEQUENCE-INDICES1 ()
  (CL:PROGN (CL:SETQ SYM-SEQUENCE-INDICES-LOGIC-SELECTION-PATTERN (INTERN-RIGID-SYMBOL-WRT-MODULE "SELECTION-PATTERN" NULL 0))
   (CL:SETQ SYM-SEQUENCE-INDICES-LOGIC-THE-SEQUENCE (INTERN-RIGID-SYMBOL-WRT-MODULE "THE-SEQUENCE" NULL 0))
   (CL:SETQ SYM-SEQUENCE-INDICES-LOGIC-THE-SEQUENCE-LENGTH (INTERN-RIGID-SYMBOL-WRT-MODULE "THE-SEQUENCE-LENGTH" NULL 0))
   ...
   (CL:SETQ SGT-SEQUENCE-INDICES-LOGIC-SEQUENCE-INDEX-ITERATOR (INTERN-RIGID-SYMBOL-WRT-MODULE "SEQUENCE-INDEX-ITERATOR" NULL 1))
   (CL:SETQ SYM-SEQUENCE-INDICES-STELLA-LIST-ITERATOR-CURSOR (INTERN-RIGID-SYMBOL-WRT-MODULE "LIST-ITERATOR-CURSOR" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   (CL:SETQ SGT-SEQUENCE-INDICES-LOGIC-PAGING-INDEX-ITERATOR (INTERN-RIGID-SYMBOL-WRT-MODULE "PAGING-INDEX-ITERATOR" NULL 1))
   (CL:SETQ SYM-SEQUENCE-INDICES-STELLA-MODULE (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   (CL:SETQ SYM-SEQUENCE-INDICES-LOGIC-OBJECT-STORE (INTERN-RIGID-SYMBOL-WRT-MODULE "OBJECT-STORE" NULL 0))
   (CL:SETQ SGT-SEQUENCE-INDICES-STELLA-MODULE (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   ...
   (CL:SETQ SYM-SEQUENCE-INDICES-LOGIC-STARTUP-SEQUENCE-INDICES (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-SEQUENCE-INDICES" NULL 0))
   (CL:SETQ SYM-SEQUENCE-INDICES-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0))))
...
(CL:DEFUN STARTUP-SEQUENCE-INDICES ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/LOGIC" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ NIL-NON-PAGING-INDEX (NEW-NON-PAGING-INDEX))
    (CL:LET* ((SELF-001 (NEW-PAGING-INDEX))) (CL:SETF (%SELECTION-PATTERN SELF-001) (GET-QUOTED-TREE "((:NIL-SEQUENCE) \"/LOGIC\")" "/LOGIC"))
     (CL:SETQ NIL-PAGING-INDEX SELF-001))
    (CL:SETQ *ALL-OBJECT-STORES* NIL-LIST))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5) (DEFINE-STELLA-TYPE-FROM-STRINGIFIED-SOURCE "(DEFTYPE SELECTION-PATTERN CONS)")
    (CL:LET*
     ((CLASS
    ...
    (CL:LET*
     ((CLASS
       (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "NON-PAGING-INDEX" "(DEFCLASS NON-PAGING-INDEX (SEQUENCE-INDEX) :DOCUMENTATION \"Index that manages an always-in-memory data
strucure containing a sequence of objects.\")")))
...
implementations and facilitated with a proper iterator class.\" :SLOTS ((SELECTION-PATTERN :TYPE SELECTION-PATTERN) (STORE :TYPE OBJECT-STORE)) :PRINT-FORM (PRINT-PAGING-INDEX SELF STREAM))")))
     (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-PAGING-INDEX)) (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-PAGING-INDEX-SLOT-VALUE)))
    (CL:LET*
     ((CLASS
    ...
    (CL:LET* ((CLASS (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "OBJECT-STORE" "(DEFCLASS OBJECT-STORE (STANDARD-OBJECT) :ABSTRACT? TRUE :SLOTS ((MODULE :TYPE MODULE)))")))
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-OBJECT-STORE-SLOT-VALUE))))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT ((SELF SEQUENCE-INDEX) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION INSERT) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD PUSH ((SELF SEQUENCE-INDEX) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION PUSH) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (FIRST OBJECT) ((SELF SEQUENCE-INDEX)) :PUBLIC? TRUE)" (CL:FUNCTION FIRST) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF SEQUENCE-INDEX)) :PUBLIC? TRUE)" (CL:FUNCTION EMPTY?) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (ESTIMATED-LENGTH INTEGER) ((SELF SEQUENCE-INDEX)))" (CL:FUNCTION ESTIMATED-LENGTH) NULL)
    (DEFINE-FUNCTION-OBJECT "SEQUENCE-INDEX.ESTIMATED-LENGTH" "(DEFUN (SEQUENCE-INDEX.ESTIMATED-LENGTH INTEGER) ((SELF SEQUENCE-INDEX)))"
     (CL:FUNCTION SEQUENCE-INDEX.ESTIMATED-LENGTH) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE-DELETED-MEMBERS (LIKE SELF)) ((SELF SEQUENCE-INDEX)) :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-DELETED-MEMBERS) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE (LIKE SELF)) ((SELF SEQUENCE-INDEX) (VALUE OBJECT)))" (CL:FUNCTION REMOVE) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (CONSIFY (CONS OF (LIKE (ANY-VALUE SELF)))) ((SELF SEQUENCE-INDEX)))" (CL:FUNCTION CONSIFY) NULL)
    (DEFINE-FUNCTION-OBJECT "PRINT-SEQUENCE-INDEX" "(DEFUN PRINT-SEQUENCE-INDEX ((SELF SEQUENCE-INDEX) (STREAM NATIVE-OUTPUT-STREAM)))"
     (CL:FUNCTION PRINT-SEQUENCE-INDEX) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (ALLOCATE-ITERATOR (ITERATOR OF (LIKE (ANY-VALUE SELF)))) ((SELF SEQUENCE-INDEX)))" (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT ((SELF PAGING-INDEX) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION INSERT) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD PUSH ((SELF PAGING-INDEX) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION PUSH) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (ESTIMATED-LENGTH INTEGER) ((SELF PAGING-INDEX)) :DOCUMENTATION \"Return the estimated length of the sequences in 'self',
which could be too large if some of the members have been deleted.\" :PUBLIC? TRUE)" (CL:FUNCTION ESTIMATED-LENGTH) NULL)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE (LIKE SELF)) ((SELF PAGING-INDEX) (VALUE OBJECT)))" (CL:FUNCTION REMOVE) NULL)
    (DEFINE-FUNCTION-OBJECT "PRINT-PAGING-INDEX" "(DEFUN PRINT-PAGING-INDEX ((SELF PAGING-INDEX) (STREAM NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION PRINT-PAGING-INDEX) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (ALLOCATE-ITERATOR (ITERATOR OF (LIKE (ANY-VALUE SELF)))) ((SELF PAGING-INDEX)))" (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
    (DEFINE-EXTERNAL-SLOT-FROM-STRINGIFIED-SOURCE "(DEFSLOT MODULE OBJECT-STORE :TYPE OBJECT-STORE :ALLOCATION :DYNAMIC)")
    (DEFINE-FUNCTION-OBJECT "REGISTER-OBJECT-STORE" "(DEFUN REGISTER-OBJECT-STORE ((STORE OBJECT-STORE)) :DOCUMENTATION \"Register `store' as an active object store.\")"
...
PowerLoom, which changes some index access routines to be considerate of that.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NOT (EQL? *ALL-OBJECT-STORES* NIL-LIST))))"
    ...
    (DEFINE-FUNCTION-OBJECT "HOME-OBJECT-STORE" "(DEFUN (HOME-OBJECT-STORE OBJECT-STORE) ((SELF OBJECT)))" (CL:FUNCTION HOME-OBJECT-STORE) NULL)
    (DEFINE-FUNCTION-OBJECT "CREATE-SEQUENCE-INDEX" "(DEFUN (CREATE-SEQUENCE-INDEX SEQUENCE-INDEX) ((SELF OBJECT) (PATTERN SELECTION-PATTERN)))"
     (CL:FUNCTION CREATE-SEQUENCE-INDEX) NULL)
    (DEFINE-FUNCTION-OBJECT "KEYWORD.CREATE-SEQUENCE-INDEX" "(DEFUN (KEYWORD.CREATE-SEQUENCE-INDEX SEQUENCE-INDEX) ((KIND KEYWORD) (PATTERN SELECTION-PATTERN)))"
     (CL:FUNCTION KEYWORD.CREATE-SEQUENCE-INDEX) NULL)
    (DEFINE-FUNCTION-OBJECT "MODULE.CREATE-SEQUENCE-INDEX" "(DEFUN (MODULE.CREATE-SEQUENCE-INDEX SEQUENCE-INDEX) ((MODULE MODULE) (PATTERN SELECTION-PATTERN)))"
     (CL:FUNCTION MODULE.CREATE-SEQUENCE-INDEX) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (OBJECT-STORE.CREATE-SEQUENCE-INDEX SEQUENCE-INDEX) ((STORE OBJECT-STORE) (PATTERN SELECTION-PATTERN)))"
...
change according to `update-mode'.  The default method does nothing.\" :PUBLIC? TRUE)" (CL:FUNCTION UPDATE-PROPOSITION-IN-STORE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-SEQUENCE-INDICES" "(DEFUN STARTUP-SEQUENCE-INDICES () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-SEQUENCE-INDICES) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SEQUENCE-INDICES-LOGIC-STARTUP-SEQUENCE-INDICES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SEQUENCE-INDICES-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupSequenceIndices")
      NULL-STRING-WRAPPER)))
