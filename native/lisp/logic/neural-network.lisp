...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *PARTIAL-MATCH-MODE* ZERO-WRAPPER
  *PRINT-CASE-DISTANCES* *MODULE* *TESTING-EXAMPLES* *CONTEXT*
  *RULE-COMBINATION* *TRAINING-EXAMPLES* STANDARD-ERROR
  *PARTIAL-SUPPORT-CACHE* NULL-FLOAT-WRAPPER STANDARD-OUTPUT EOL))
...
(CL:PROGN (CL:DEFVAR *LEARNING-RATE* 0.1d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *LEARNING-RATE*)))

;;; (DEFGLOBAL *MOMENTUM-TERM* ...)

(CL:PROGN (CL:DEFVAR *MOMENTUM-TERM* 0.9d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *MOMENTUM-TERM*)))

;;; (DEFGLOBAL *WEIGHT-RANGE* ...)

(CL:PROGN (CL:DEFVAR *WEIGHT-RANGE* 0.05d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *WEIGHT-RANGE*)))

;;; (DEFGLOBAL *ERROR-CUTOFF* ...)

(CL:PROGN (CL:DEFVAR *ERROR-CUTOFF* 0.0d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *ERROR-CUTOFF*)))

;;; (DEFGLOBAL *ERROR-PRINT-CYCLE* ...)

(CL:PROGN (CL:DEFVAR *ERROR-PRINT-CYCLE* 25) (CL:DECLAIM (CL:TYPE CL:FIXNUM *ERROR-PRINT-CYCLE*)))

;;; (DEFGLOBAL *SAVE-NETWORK-CYCLE* ...)

(CL:PROGN (CL:DEFVAR *SAVE-NETWORK-CYCLE* 10000) (CL:DECLAIM (CL:TYPE CL:FIXNUM *SAVE-NETWORK-CYCLE*)))

;;; (DEFGLOBAL *SAVE-NETWORK-FILE* ...)

(CL:PROGN (CL:DEFVAR *SAVE-NETWORK-FILE* STELLA::NULL-STRING) (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *SAVE-NETWORK-FILE*)))

;;; (DEFGLOBAL *TRACE-NEURAL-NETWORK-TRAINING* ...)

(CL:DEFVAR *TRACE-NEURAL-NETWORK-TRAINING* FALSE)

;;; (DEFGLOBAL *TRAIN-CACHED-NETWORKS?* ...)

(CL:DEFVAR *TRAIN-CACHED-NETWORKS?* TRUE)

;;; (DEFGLOBAL *MAX-MOVEMENT* ...)

(CL:PROGN (CL:DEFVAR *MAX-MOVEMENT* 1.75d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *MAX-MOVEMENT*)))

;;; (DEFGLOBAL *MODE-SWITCH* ...)

(CL:PROGN (CL:DEFVAR *MODE-SWITCH* 0.0d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *MODE-SWITCH*)))

;;; (DEFGLOBAL *SHRINK-FACTOR* ...)

(CL:PROGN (CL:DEFVAR *SHRINK-FACTOR* NULL-FLOAT) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *SHRINK-FACTOR*)))

;;; (DEFGLOBAL *WEIGHT-DECAY* ...)

(CL:PROGN (CL:DEFVAR *WEIGHT-DECAY* -10.0d-5) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *WEIGHT-DECAY*)))

;;; (DEFGLOBAL *SIGMOID-PRIME-OFFSET* ...)

(CL:PROGN (CL:DEFVAR *SIGMOID-PRIME-OFFSET* 0.1d0) (CL:DECLAIM (CL:TYPE CL:DOUBLE-FLOAT *SIGMOID-PRIME-OFFSET*)))
...
(CL:DEFUN CHECK-MASTER-NETWORK-LIST ()
  (CL:LET*
   ((I NULL-INTEGER) (NET NULL)
    (ITER-000 (%THE-CONS-LIST *MASTER-NEURAL-NETWORK-LIST*)) (ITER-001 0))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Net " I ": "
     (LENGTH (%INPUT NET)) " inputs, " (LENGTH (%HIDDEN NET)) " outputs."
     EOL))))
...
(CL:DEFUN CLEAR-SLOPES (NET)
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 H))
   (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
    (CL:PROGN (CL:SETQ H ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
    (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-001 H))
      (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
       (CL:PROGN (CL:SETQ H ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
       (CL:LET
       ...
       (CL:SETQ K (CL:1+ K))))))
   (CL:LET*
    ((NUM NULL) (H NULL-INTEGER) (ITER-002 0)
     (UPPER-BOUND-002 (CL:1- (LENGTH (%HIDDEN NET)))) (ITER-003 HO))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-002 UPPER-BOUND-002 H))
    (CL:LOOP WHILE
     (CL:AND (CL:<= ITER-002 UPPER-BOUND-002)
      (CL:NOT (CL:EQ ITER-003 NIL)))
     DO
     (CL:PROGN (CL:SETQ H ITER-002) (CL:SETQ ITER-002 (CL:1+ ITER-002)))
     (CL:PROGN (CL:SETQ NUM (%%VALUE ITER-003))
...
(CL:DEFUN ACTIVATE-PROPOSITIONAL-NEURAL-NETWORK (NET)
  (CL:LET*
   ((NUM-IN (LENGTH (%INPUT NET))) (NUM-HIDDEN (LENGTH (%HIDDEN NET)))
    ...
    (NET-HASH-KEY STELLA::NULL-STRING) (INPUT (%INPUT NET))
    (HIDDEN (%HIDDEN NET)) (IH (%IH NET)) (HO (%HO NET)) (SCORE 0.0d0)
    (SUM 0.0d0))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NUM-IN NUM-HIDDEN STOP)
    (CL:TYPE CL:SIMPLE-STRING NET-HASH-KEY)
    ...
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
     ...
     (CL:LET
      ((SELF (%THE-ARRAY HIDDEN))
       (VALUE (WRAP-FLOAT (CL:/ 1.0d0 (CL:+ 1.0d0 (EXP (CL:- 0 SUM))))))
       (POSITION I))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
       (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))
   ...
   (CL:WHEN (CL:= (CL:+ 1.0d0 (EXP (CL:- 0 SCORE))) 0.0d0)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Sum is " SCORE EOL))
   (CL:SETF (%OUTPUT NET) (CL:/ 1.0d0 (CL:+ 1.0d0 (EXP (CL:- 0 SCORE)))))
...
(CL:DEFUN TRAIN-UNCACHED-NEURAL-NETWORKS (CYCLES NUM-TRAINING)
  (CL:DECLARE (CL:TYPE CL:FIXNUM CYCLES NUM-TRAINING))
  (CL:LET*
   ((MSE 0.0d0) (NUM-EX (LENGTH *TRAINING-EXAMPLES*)) (EXPERIENCES 0)
    (LOOK-PTR NULL) (EXAMPLES (NEW-VECTOR NUM-TRAINING)))
   (CL:DECLARE (CL:TYPE CL:DOUBLE-FLOAT MSE)
    (CL:TYPE CL:FIXNUM NUM-EX EXPERIENCES))
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-001 CYCLE))
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
     (CL:PROGN (CL:SETQ CYCLE ITER-001)
     ...
     (CL:LET*
      ((K NULL-INTEGER) (EXAMPLE NULL) (VECTOR-000 EXAMPLES) (INDEX-000 0)
       (LENGTH-000 (LENGTH VECTOR-000)) (ITER-002 0))
      (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-002 K))
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (CL:PROGN (CL:SETQ K ITER-002) (CL:SETQ ITER-002 (CL:1+ ITER-002)))
     ...
     (CL:WHEN
      (CL:AND (CL:> *SAVE-NETWORK-CYCLE* 0)
       (CL:= (CL:THE CL:FIXNUM (CL:MOD CYCLE *SAVE-NETWORK-CYCLE*)) 0))
      (%SAVE-ALL-NEURAL-NETWORKS *SAVE-NETWORK-FILE*))
     (CL:WHEN
      (CL:AND (CL:> *ERROR-PRINT-CYCLE* 0)
       (CL:= (CL:THE CL:FIXNUM (CL:MOD CYCLE *ERROR-PRINT-CYCLE*)) 0))
...
(CL:DEFUN BACKPROPAGATE-ERROR (PROP ERROR)
  ...
  (CL:LET*
   ((NET
    ...
    (RECURSIVE-CONFLICT FALSE) (FULL-ERROR ERROR))
   ...
   (CL:WHEN (CL:NOT (CL:EQ NET NULL))
           ...
           (DYNAMIC-SLOT-VALUE
            (%DYNAMIC-SLOTS
             (CL:AREF
              ...
              (CL:THE CL:FIXNUM (CL:1- I))))
            SYM-NEURAL-NETWORK-LOGIC-MATCH-SCORE NULL-FLOAT-WRAPPER))))
        (CL:SETQ RECURSIVE-CONFLICT TRUE) (CL:RETURN)))))
    ...
    (CL:LET*
     ...
     (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-004 UPPER-BOUND-004 H))
     (CL:LOOP WHILE (CL:<= ITER-004 UPPER-BOUND-004) DO
      (CL:PROGN (CL:SETQ H ITER-004) (CL:SETQ ITER-004 (CL:1+ ITER-004)))
      (CL:LET
    ...
    (CL:LET*
     ...
     (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-005 UPPER-BOUND-005 I))
     (CL:LOOP WHILE (CL:<= ITER-005 UPPER-BOUND-005) DO
      ...
      (CL:LET*
       ...
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-006 UPPER-BOUND-006 H))
       (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-006) DO
        (CL:PROGN (CL:SETQ H ITER-006)
   ...
   (CL:LET* ((TEST-VALUE-000 (%KIND PROP)))
    (CL:COND
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-NEURAL-NETWORK-AND)
       (CL:EQ TEST-VALUE-000 KWD-NEURAL-NETWORK-OR))
      (CL:LET*
       ((I NULL-INTEGER) (CHILD NULL) (VECTOR-000 (%ARGUMENTS PROP))
        (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ITER-007 1)
        (UPPER-BOUND-007 (CL:1- NUM-IN)))
       (CL:DECLARE
        (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-007 UPPER-BOUND-007
         I))
       (CL:LOOP WHILE
      ...
      (CL:LET*
       ...
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-008 UPPER-BOUND-008 H))
       (CL:LOOP WHILE (CL:<= ITER-008 UPPER-BOUND-008) DO
        (CL:PROGN (CL:SETQ H ITER-008)
       ...
       (CL:LET* ((PROOFS (SUPPORT PROP)) (GUILTY NIL))
         ...
         ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-MAX)
          ...
          (CL:LET* ((PROOF NULL) (ITER-011 (%%REST PROOFS)))
           (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-011 NIL)) DO
            (CL:PROGN (CL:SETQ PROOF (%%VALUE ITER-011))
        ...
        (CL:LET* ((PROOF NULL) (ITER-012 GUILTY))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-012 NIL)) DO
          (CL:PROGN (CL:SETQ PROOF (%%VALUE ITER-012))
           ...
           (CL:LET*
              ...
              (%PROPOSITION
               (CL:AREF
                (CL:THE CL:SIMPLE-VECTOR
                 (%THE-ARRAY (%ARGUMENTS (%AXIOM PROOF))))
                0)))
             (FACT (%FACT PROOF)))
            (CL:IF (CL:= (LENGTH (%ARGUMENT-SCORES PROOF)) 1)
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FACT)
              ...
              NULL-FLOAT-WRAPPER)
             (CL:LET*
              ((ARG NULL) (SCORE NULL) (ITER-013 (%ARGUMENT-SCORES PROOF))
              ...
              (CL:LOOP WHILE
               ...
               DO
               (CL:PROGN (CL:SETQ SCORE (%%VALUE ITER-013))
   ...
   (CL:LET* ((TEST-VALUE-000 (%KIND PROP)))
    (CL:COND
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-NEURAL-NETWORK-AND)
      ...
      (CL:LET*
       ((I NULL-INTEGER) (CHILD NULL) (VECTOR-000 (%ARGUMENTS PROP))
        (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ITER-003 1)
        (UPPER-BOUND-003 (CL:1- NUM-IN)))
       (CL:DECLARE
        (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-003 UPPER-BOUND-003
         I))
       (CL:LOOP WHILE
        (CL:AND (CL:< INDEX-000 LENGTH-000)
       ...
       (CL:LET* ((PROOFS (SUPPORT PROP)) (GUILTY NIL))
         ...
         ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-MAX)
          ...
          (CL:LET* ((PROOF NULL) (ITER-004 (%%REST PROOFS)))
           (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-004 NIL)) DO
            (CL:PROGN (CL:SETQ PROOF (%%VALUE ITER-004))
        ...
        (CL:LET* ((PROOF NULL) (ITER-005 GUILTY))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-005 NIL)) DO
          (CL:PROGN (CL:SETQ PROOF (%%VALUE ITER-005))
           ...
           (CL:LET*
            ...
            (CL:IF (CL:= (LENGTH (%ARGUMENT-SCORES PROOF)) 1)
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FACT)
              ...
              NULL-FLOAT-WRAPPER)
             (CL:LET*
              ((ARG NULL) (SCORE NULL) (ITER-006 (%ARGUMENT-SCORES PROOF))
               (VECTOR-001 (%ARGUMENTS FACT)) (INDEX-001 0)
               (LENGTH-001 (LENGTH VECTOR-001)))
              (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-001 LENGTH-001))
              (CL:LOOP WHILE
               (CL:AND (CL:NOT (CL:EQ ITER-006 NIL))
                (CL:< INDEX-001 LENGTH-001))
               DO
               (CL:PROGN (CL:SETQ SCORE (%%VALUE ITER-006))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-000 H))
      (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-000) DO
       ...
       (CL:SETQ DELTA
        (COMPUTE-QP-DELTA
         ...
         (%WRAPPER-VALUE
          (CL:AREF
           (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%HO-PREV-SLOPE NET))) H))
         (%WRAPPER-VALUE
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:<= ITER-002 UPPER-BOUND-001) DO
       ...
       (CL:LET*
        ...
        (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-003 UPPER-BOUND-002 H))
        (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-002) DO
         (CL:PROGN (CL:SETQ H ITER-003)
          (CL:SETQ ITER-003 (CL:1+ ITER-003)))
         (CL:SETQ DELTA
          (COMPUTE-QP-DELTA
           (%WRAPPER-VALUE
...
(CL:DEFUN TEST-OVER-TRAINING-EXAMPLES ()
  (CL:LET*
   ((MSE 0.0d0) (ERROR 0.0d0) (OUTPUT 0.0d0) (FORM NIL) (HOME *CONTEXT*)
    (QUERY NULL))
   (CL:DECLARE (CL:TYPE CL:DOUBLE-FLOAT MSE ERROR OUTPUT))
   (CL:LET*
    ((I NULL-INTEGER) (EXAMPLE NULL)
     (ITER-000 (%THE-CONS-LIST *TRAINING-EXAMPLES*)) (ITER-001 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 I))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ OUTPUT (RETURN-PARTIAL-TRUTH QUERY TRUE))
...
(CL:DEFUN TRAIN-CACHED-NEURAL-NETWORKS (CYCLES NUM-TRAINING)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((EXAMPLE NULL) (I NULL-INTEGER) (ITER-000 0)
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-003 UPPER-BOUND-002 CYCLE))
    (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-002) DO
     (CL:PROGN (CL:SETQ CYCLE ITER-003)
      (CL:SETQ ITER-003 (CL:1+ ITER-003)))
     (CL:SETQ MSE 0.0d0) (CL:SETQ ABS-ERROR 0.0d0)
     (CL:COND
      ((CL:OR
        (CL:EQ *NEURAL-NETWORK-TRAINING-METHOD*
         KWD-NEURAL-NETWORK-BACKPROP)
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-004 UPPER-BOUND-003 K))
      (CL:LOOP WHILE (CL:<= ITER-004 UPPER-BOUND-003) DO
       (CL:PROGN (CL:SETQ K ITER-004) (CL:SETQ ITER-004 (CL:1+ ITER-004)))
       (CL:WHEN *TRACE-NEURAL-NETWORK-TRAINING*
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Example " K ": "
         (%QUERY
...
(CL:DEFUN CACHED-BACKPROPAGATE-ERROR (TREE ERROR)
  ...
  (CL:LET*
   ((NET NULL) (TEMP 0.0d0) (DELTA 0.0d0) (INPUT NULL) (HIDDEN NULL)
    (HIDDEN-ERROR NULL) (INPUT-ERROR NULL) (RECURSIVE-CONFLICT? FALSE))
     ...
     ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-MAX)
      (CL:LET* ((MAX 0.0d0) (GUILTY NIL))
       ...
       (CL:LET* ((NET-DEF NULL) (ITER-000 TREE))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ NET-DEF (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
     ...
     ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-NOISY-OR)
      (CL:LET* ((NET-DEF NULL) (ITER-001 TREE))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ NET-DEF (%%VALUE ITER-001))
         (CL:SETQ ITER-001 (%%REST ITER-001)))
   ...
   (CL:LET*
    ((ELE NULL) (I NULL-INTEGER) (ITER-002 1)
     (UPPER-BOUND-000 (CL:1- (LENGTH (%INPUT NET))))
     (ITER-003 (%%REST TREE)))
          ...
          (CL:COND
           ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-MAX)
            ...
            (CL:LET* ((NET-DEF NULL) (ITER-004 ELE))
             (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-004 NIL)) DO
              (CL:PROGN (CL:SETQ NET-DEF (%%VALUE ITER-004))
               (CL:SETQ ITER-004 (%%REST ITER-004)))
           ...
           ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-NOISY-OR)
            (CL:LET* ((SCORES NIL))
             (CL:LET* ((NET-DEF NULL) (ITER-005 ELE) (COLLECT-000 NULL))
              (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-005 NIL)) DO
               (CL:PROGN (CL:SETQ NET-DEF (%%VALUE ITER-005))
                (CL:SETQ ITER-005 (%%REST ITER-005)))
   ...
   (CL:WHEN RECURSIVE-CONFLICT? (ACTIVATE-CACHED-NETWORK (CONS-LIST TREE))
    ...
    (CL:LET*
     ...
     (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-006 UPPER-BOUND-001 I))
     (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-001) DO
      (CL:PROGN (CL:SETQ I ITER-006) (CL:SETQ ITER-006 (CL:1+ ITER-006)))
      (CL:LET
       ((SELF (%THE-ARRAY INPUT))
        (VALUE
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT NET))) I))
    ...
    (CL:LET*
     ...
     (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-007 UPPER-BOUND-002 H))
     (CL:LOOP WHILE (CL:<= ITER-007 UPPER-BOUND-002) DO
      (CL:PROGN (CL:SETQ H ITER-007) (CL:SETQ ITER-007 (CL:1+ ITER-007)))
      (CL:LET
       ((SELF (%THE-ARRAY HIDDEN))
        (VALUE
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-008 UPPER-BOUND-003 H))
    (CL:LOOP WHILE (CL:<= ITER-008 UPPER-BOUND-003) DO
     (CL:PROGN (CL:SETQ H ITER-008) (CL:SETQ ITER-008 (CL:1+ ITER-008)))
     (CL:LET
      ((SELF (%THE-ARRAY HIDDEN-ERROR))
       (VALUE
        (WRAP-FLOAT
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-009 UPPER-BOUND-004 I))
    (CL:LOOP WHILE (CL:<= ITER-009 UPPER-BOUND-004) DO
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-010 UPPER-BOUND-005 H))
      (CL:LOOP WHILE (CL:<= ITER-010 UPPER-BOUND-005) DO
       (CL:PROGN (CL:SETQ H ITER-010) (CL:SETQ ITER-010 (CL:1+ ITER-010)))
       (CL:LET
        ((SELF (%THE-ARRAY INPUT-ERROR))
         (VALUE
          (WRAP-FLOAT
        ...
        (CL:SETF (CL:AREF SELF POSITION) VALUE))))))
   (CL:LET*
    ((I NULL-INTEGER) (INPUT NULL) (ITER-011 (%%REST TREE)) (ITER-012 1))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-012 I))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-011 NIL)) DO
     (CL:PROGN (CL:SETQ INPUT (%%VALUE ITER-011))
      (CL:SETQ ITER-011 (%%REST ITER-011)))
     (CL:PROGN (CL:SETQ I ITER-012) (CL:SETQ ITER-012 (CL:1+ ITER-012)))
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-013 UPPER-BOUND-006 H))
    (CL:LOOP WHILE (CL:<= ITER-013 UPPER-BOUND-006) DO
     (CL:PROGN (CL:SETQ H ITER-013) (CL:SETQ ITER-013 (CL:1+ ITER-013)))
     (CL:SETQ DELTA
      (CL:+
       (CL:* *MOMENTUM-TERM*
        (%WRAPPER-VALUE
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-014 UPPER-BOUND-007 I))
      (CL:LOOP WHILE (CL:<= ITER-014 UPPER-BOUND-007) DO
       (CL:PROGN (CL:SETQ I ITER-014) (CL:SETQ ITER-014 (CL:1+ ITER-014)))
       (CL:SETQ DELTA
        (CL:+
         (CL:* *MOMENTUM-TERM*
          (%WRAPPER-VALUE
...
(CL:DEFUN CACHED-QUICKPROPAGATE-ERROR (TREE ERROR)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-006 UPPER-BOUND-001 H))
    (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-001) DO
     (CL:PROGN (CL:SETQ H ITER-006) (CL:SETQ ITER-006 (CL:1+ ITER-006)))
     (CL:LET
      ((SELF (%THE-ARRAY (%HO-SLOPE NET)))
       (VALUE
        (WRAP-FLOAT
     ...
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-007 UPPER-BOUND-002 I))
      (CL:LOOP WHILE (CL:<= ITER-007 UPPER-BOUND-002) DO
       ...
       (CL:LET
        ...
        (CL:SETF (CL:AREF SELF POSITION) VALUE))))))
   (CL:LET*
    ((I NULL-INTEGER) (INPUT NULL) (ITER-008 (%%REST TREE)) (ITER-009 1))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-009 I))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-008 NIL)) DO
     (CL:PROGN (CL:SETQ INPUT (%%VALUE ITER-008))
   ...
   ((CL:NOT (CL:EQ (SUPPORT PROP) NIL))
    (CL:LET* ((PROOFS (SUPPORT PROP)) (TREE NIL))
     (CL:LET* ((PROOF NULL) (ITER-000 PROOFS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:PROGN (CL:SETQ PROOF (%%VALUE ITER-000))
        (CL:SETQ ITER-000 (%%REST ITER-000)))
       (CL:LET* ((FACT (%FACT PROOF)) (PROOF-TREE NIL))
        (CL:LET*
         ((SCORE NULL) (ARG NULL) (VECTOR-000 (%ARGUMENTS FACT))
        ...
        (CL:IF
         ...
         (CL:SETQ TREE
          (CONS
           ...
           TREE))))))
     (CL:IF
      (CL:AND (CL:EQ (%%REST TREE) NIL) (CL:NOT (CONS? (%%VALUE TREE))))
      (%%VALUE TREE) TREE)))
   ((CL:NOT
     (CL:EQ
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROP)
    ...
    (CL:LET* ((TREE NIL))
     (CL:LET*
      ((I NULL-INTEGER) (ARG NULL) (VECTOR-001 (%ARGUMENTS PROP))
       (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)) (ITER-002 1)
       (COLLECT-001 NULL))
...
(CL:DEFUN ACTIVATE-CACHED-NETWORK (TREE)
  (CL:LET* ((SCORES (NEW-VECTOR (LENGTH TREE))) (SCORE 0.0d0) (SUM 0.0d0))
   ...
   (CL:LET*
    ((K NULL-INTEGER) (CACHED-NET NULL) (ITER-000 TREE) (ITER-001 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 K))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CACHED-NET (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     ...
     (CL:LET* ((NET (%%VALUE CACHED-NET))) (CL:SETQ SUM 0.0d0)
      ...
      (CL:LET
       ...
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET*
       ((INPUT NULL) (I NULL-INTEGER) (ITER-002 1)
        (UPPER-BOUND-000 (CL:1- (LENGTH (%INPUT NET))))
       ...
       (CL:LOOP WHILE
        ...
        (CL:PROGN (CL:SETQ I ITER-002)
         (CL:SETQ ITER-002 (CL:1+ ITER-002)))
        (CL:PROGN (CL:SETQ INPUT (%%VALUE ITER-003))
         (CL:SETQ ITER-003 (%%REST ITER-003)))
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:<= ITER-004 UPPER-BOUND-001) DO
        ...
        (CL:LET*
         ...
         (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-005 UPPER-BOUND-002 J))
        ...
        (CL:LET
         ((SELF (%THE-ARRAY (%HIDDEN NET)))
          (VALUE
           (WRAP-FLOAT (CL:/ 1.0d0 (CL:+ 1.0d0 (EXP (CL:- 0 SUM))))))
          (POSITION I))
         (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
          (CL:TYPE CL:FIXNUM POSITION))
         (CL:SETF (CL:AREF SELF POSITION) VALUE))
          ...
          (CL:*
           ...
           (%WRAPPER-VALUE
            (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%HO NET)))
             I)))))))
      (CL:SETF (%OUTPUT NET)
       (CL:/ 1.0d0 (CL:+ 1.0d0 (EXP (CL:- 0 SCORE)))))
      (CL:LET
    ...
    ((CL:EQ *RULE-COMBINATION* KWD-NEURAL-NETWORK-MAX)
     (CL:LET*
      ...
      (CL:LET*
       ...
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-006 UPPER-BOUND-003 I))
...
(CL:DEFUN %STRUCTURED-NEURAL-NETWORK-REGRESSION (CLASS-NAME SLOT-NAME CYCLES)
  ...
  (CL:LET*
   ((CLASS (%SURROGATE-VALUE-INVERSE (GET-DESCRIPTION CLASS-NAME)))
    ...
    (SLOT-VECTOR (NEW-VECTOR (LENGTH ILIST))))
   ...
   (CL:LET*
    ((INSTANCE NULL) (I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:1- (LENGTH ILIST)))
    ...
    (CL:LOOP WHILE
     ...
     DO
     (CL:PROGN (CL:SETQ I ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:PROGN (CL:SETQ INSTANCE (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
...
(CL:DEFUN GET-NN-NEAREST-NEIGHBORS (PROBE CASES K)
  ...
  (CL:LET*
   ...
   (CL:LET* ((CASE NULL) (ITER-001 (%THE-CONS-LIST CASES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ CASE (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     ...
     (CL:WHEN
      ...
      (CL:LET* ((J NULL-INTEGER) (ITER-002 1) (UPPER-BOUND-001 (CL:1- K)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-002 UPPER-BOUND-001 J))
       (CL:LOOP WHILE (CL:<= ITER-002 UPPER-BOUND-001) DO
        (CL:PROGN (CL:SETQ J ITER-002)
         (CL:SETQ ITER-002 (CL:1+ ITER-002)))
        (CL:WHEN
         (CL:>
          (%WRAPPER-VALUE
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ITER-003 0) (UPPER-BOUND-002 (CL:1- K))
     (COLLECT-000 NULL))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-003 UPPER-BOUND-002 I))
    (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-002) DO
     (CL:PROGN (CL:SETQ I ITER-003) (CL:SETQ ITER-003 (CL:1+ ITER-003)))
     (CL:WHEN
      (CL:NOT
...
(CL:DEFUN %SWAP-IN-NEW-NETWORKS (OLD-NETS NEW-NETS)
  (CL:LET*
   ((NEW-NET NULL) (OLD-NET NULL) (ITER-000 (%THE-CONS-LIST OLD-NETS))
    (ITER-001 (%THE-CONS-LIST NEW-NETS)))
   (CL:LOOP WHILE
    (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:NOT (CL:EQ ITER-001 NIL)))
    DO
    (CL:PROGN (CL:SETQ OLD-NET (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:PROGN (CL:SETQ NEW-NET (%%VALUE ITER-001))
     (CL:SETQ ITER-001 (%%REST ITER-001)))
...
(CL:DEFUN CREATE-HIDDEN-SIGNATURE (CONS-QUERY)
  (CL:LET*
   ((QUERY
     (MAKE-QUERY NIL (COERCE-TO-TREE (COPY-CONS-TREE CONS-QUERY)) NIL
      NIL))
    (PMF NULL) (MODE *PARTIAL-MATCH-MODE*) (NET NULL) (RESULT NULL))
   (CL:SETQ *PARTIAL-MATCH-MODE* KWD-NEURAL-NETWORK-NN)
   (RETURN-PARTIAL-TRUTH QUERY TRUE) (CL:SETQ *PARTIAL-MATCH-MODE* MODE)
   (CL:SETQ PMF
    (%CHILD (%CHILD (%PARTIAL-MATCH-FRAME (%BASE-CONTROL-FRAME QUERY)))))
   (CL:SETQ NET
    (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%PROPOSITION (%FRAME-PTR PMF)))
     SYM-NEURAL-NETWORK-LOGIC-NEURAL-NETWORK NULL))
...
(CL:DEFCLASS 2-D-ARRAY (MULTI-DIMENSIONAL-ARRAY)
  ((NOF-ROWS :ALLOCATION :INSTANCE :ACCESSOR %NOF-ROWS)
   (NOF-COLUMNS :ALLOCATION :INSTANCE :ACCESSOR %NOF-COLUMNS)
...
(CL:DEFUN NEW-2-D-ARRAY (NOF-ROWS NOF-COLUMNS)
  ...
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%NOF-COLUMNS SELF) NOF-COLUMNS) (INITIALIZE-2-D-ARRAY SELF)
   SELF))
...
Missing values will be padded with NULL, extraneous values will be ignored."
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((VALUE NULL) (I NULL-INTEGER) (ITER-000 0)
...
(CL:DEFMETHOD PRINT-ARRAY ((SELF 2-D-ARRAY) STREAM)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 ROW))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     (CL:PROGN (CL:SETQ ROW ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (%%PRINT-STREAM STREAM "[")
     (CL:LET*
      ((COLUMN NULL-INTEGER) (ITER-001 0)
       (UPPER-BOUND-001 (CL:1- NOF-COLUMNS)))
      (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-001 COLUMN))
      (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
       (CL:PROGN (CL:SETQ COLUMN ITER-001)
        (CL:SETQ ITER-001 (CL:1+ ITER-001)))
       (%%PRINT-STREAM STREAM
...
(CL:DEFUN NEW-FLOAT-VECTOR (ARRAY-SIZE)
  ...
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%THE-ARRAY SELF) STELLA::NULL-NATIVE-VECTOR)
   (CL:SETF (%ARRAY-SIZE SELF) ARRAY-SIZE) (INITIALIZE-FLOAT-VECTOR SELF)
   SELF))
...
(CL:DEFUN CREATE-FLOAT-VECTOR (CL:&REST VALUES)
  ...
  (CL:LET* ((VECTOR (NEW-FLOAT-VECTOR (CL:LENGTH VALUES))))
   (CL:LET*
    ((I NULL-INTEGER) (V NULL-FLOAT) (ITER-000 VALUES) (ITER-001 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 I)
     (CL:TYPE CL:DOUBLE-FLOAT V))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ V (CL:POP ITER-000))
     (CL:PROGN (CL:SETQ I ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
     (CL:SETF
      (%WRAPPER-VALUE
  ...
  (CL:IF (CL:= (%ARRAY-SIZE SELF) 0) (%%PRINT-STREAM STREAM "|i|[]")
   (CL:LET* ((I 0) (LIMIT 9)) (CL:DECLARE (CL:TYPE CL:FIXNUM I LIMIT))
    ...
    (CL:LET*
     ...
     (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000)
      (CL:TYPE CL:DOUBLE-FLOAT ELEMENT))
     (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
      (CL:PROGN
       (CL:SETQ ELEMENT
        (%WRAPPER-VALUE
...
Missing values will be padded with NULL, extraneous values will be ignored."
  ...
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR NATIVEARRAY))
   (CL:LET*
    ((VALUE NULL-FLOAT) (I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:1- (CL:* NOF-ROWS NOF-COLUMNS)))
     (ITER-001 VALUES))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 I)
...
(CL:DEFUN HELP-STARTUP-NEURAL-NETWORK1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-NEURAL-NETWORK-LOGIC-SET-NEURAL-NETWORK-TRAINING-METHOD
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-NEURAL-NETWORK-TRAINING-METHOD"
     NULL 0))
   ...
   (CL:SETQ SYM-NEURAL-NETWORK-LOGIC-TRAIN-AND-TEST-NEURAL-NETWORK
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TRAIN-AND-TEST-NEURAL-NETWORK" NULL
     0))
   (CL:SETQ SYM-NEURAL-NETWORK-LOGIC-MULTIPLE-NETWORK-TRAINING-RUNS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "MULTIPLE-NETWORK-TRAINING-RUNS" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-NEURAL-NETWORK2 ()
  (CL:PROGN
   ...
   (CL:LET*
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-2-D-ARRAY))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS)
...
will be ignored.\" :PUBLIC? TRUE)" (CL:FUNCTION FILL-ARRAY) NULL)
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Neural-Network") NULL-STRING-WRAPPER))))
...
(CL:DEFUN STARTUP-NEURAL-NETWORK ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *WEIGHT-DECAY* FLOAT -10.0e-5)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
