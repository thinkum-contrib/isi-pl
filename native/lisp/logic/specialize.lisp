...
(CL:DECLAIM
  ...
  *CYC-KLUDGES-ENABLED?* *QUERYITERATOR* NIL-PAGING-INDEX
  NIL-NON-PAGING-INDEX FALSE *REVERSEPOLARITY?* TRUE-TRUTH-VALUE NIL
  TRUE))
...
(CL:DEFCLASS ABSTRACT-PROPOSITIONS-ITERATOR (ITERATOR)
  ((SELECTION-PATTERN :ALLOCATION :INSTANCE :ACCESSOR
   ...
   (PROPOSITION-CURSOR :ALLOCATION :INSTANCE :ACCESSOR
    %PROPOSITION-CURSOR)
   (EQUIVALENTS-STACK :ALLOCATION :INSTANCE :ACCESSOR
    %EQUIVALENTS-STACK))
  (:DOCUMENTATION
...
(CL:DEFUN NEW-ABSTRACT-PROPOSITIONS-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE ABSTRACT-PROPOSITIONS-ITERATOR)))
...
(CL:DEFUN NEW-TRUE-PROPOSITIONS-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE TRUE-PROPOSITIONS-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) TRUE) (CL:SETF (%VALUE SELF) NULL)
   (CL:SETF (%EQUIVALENTS-STACK SELF) NULL)
   (CL:SETF (%PROPOSITION-CURSOR SELF) NULL)
   (CL:SETF (%SELECTION-PATTERN SELF) NULL)
  ...
  (CL:LET* ((SPECIALIZINGRELATIONS (%SPECIALIZING-RELATIONS SELF)))
   (CL:LOOP
    ...
    (CL:IF (CL:EQ SPECIALIZINGRELATIONS NULL)
     (CL:COND
      ((CL:EQ (%%VALUE (%SELECTION-PATTERN SELF))
    ...
    (CL:SETF (%PROPOSITION-CURSOR SELF)
     (ALLOCATE-ITERATOR
      (SELECT-PROPOSITIONS (%SELECTION-PATTERN SELF))))
    (CL:SETF (%EQUIVALENTS-STACK SELF) NULL))))
...
(CL:DEFUN ALL-TRUE-DEPENDENT-PROPOSITIONS (SELF RELATION SPECIALIZE?)
  ...
  (CL:LET*
   ...
   (CL:SETF (%PROPOSITION-CURSOR ITERATOR) (ALLOCATE-ITERATOR INDEX))
   ITERATOR))
   ...
   (CL:LET* ((VALUE-000 NIL))
    (CL:LET*
     ...
     (CL:LOOP WHILE (NEXT? ITER-000) DO
      ...
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((I NULL-INTEGER) (A1 NULL) (ITER-001 ARGUMENTS) (ITER-002 0))
   ...
   (CL:LET* ((FOUND?-000 FALSE))
    (CL:LET*
     ...
     (CL:LOOP WHILE (NEXT? ITER-000) DO
      (CL:SETQ PROP (%VALUE ITER-000))
      (CL:WHEN
       (RELATIONREF-SPECIALIZES-RELATIONREF? (%OPERATOR PROP) TYPE)
       (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
    (CL:LET* ((VALUE-000 FOUND?-000)) VALUE-000))))
...
(CL:DEFCLASS TRANSITIVE-CLOSURE-ITERATOR (ITERATOR)
  ((ALLOCATE-ADJACENCY-ITERATOR-FUNCTION :ALLOCATION :INSTANCE
    :ACCESSOR %ALLOCATE-ADJACENCY-ITERATOR-FUNCTION)
  ...
  (CL:COND
   ((CL:EQ SLOTNAME
    ...
    (CL:IF SETVALUE?
     ...
     (CL:SETQ VALUE
      (WRAP-FUNCTION-CODE
       (%ALLOCATE-ADJACENCY-ITERATOR-FUNCTION SELF)))))
  ...
  (CL:LET*
   ...
   (CL:LOOP
    ...
    (CL:WHEN
     ...
     (CL:SETF (%BEEN-THERE-LIST SELF)
      (CONS NODE (%BEEN-THERE-LIST SELF)))
...
(CL:DEFUN NEW-SUPPORTED-CLOSURE-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE SUPPORTED-CLOSURE-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) TRUE) (CL:SETF (%VALUE SELF) NULL)
   (CL:SETF (%BEEN-THERE-LIST SELF) NULL)
   (CL:SETF (%FILTER? SELF) NULL)
...
(CL:DEFMETHOD NEXT? ((SELF SUPPORTED-CLOSURE-ITERATOR))
  (CL:COND
   ((TRANSITIVE-CLOSURE-ITERATOR.NEXT? SELF)
    (CL:SETF (%VALUE SELF)
     (CONS (%VALUE SELF)
      (CONS (%TRUTH-VALUE (%%VALUE (%ADJACENCY-ITERATOR-STACK SELF)))
       NIL)))
    TRUE)
   (CL:T FALSE)))
...
(CL:DEFUN ACCESS-DIRECTLY-LINKED-OBJECTS-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SPECIALIZE-LOGIC-INVERSE?)
    (CL:IF SETVALUE?
     (CL:SETF (%INVERSE? SELF)
      (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE))
     (CL:SETQ VALUE
      (CL:IF (%INVERSE? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
    ...
    (CL:IF (CL:NOT (CL:EQ (%%REST EQUIVALENTS) NIL))
     (CL:LET* ((E NULL) (ITER-000 EQUIVALENTS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       ...
       (CL:LET*
        ((PARENT NULL)
         (ITER-001 (ALL-DIRECT-SUPERCOLLECTIONS E FALSE)))
...
(CL:DEFUN NEW-CLASHING-PROPOSITIONS-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE CLASHING-PROPOSITIONS-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) TRUE) (CL:SETF (%VALUE SELF) NULL)
   (CL:SETF (%EQUIVALENTS-STACK SELF) NULL)
   (CL:SETF (%PROPOSITION-CURSOR SELF) NULL)
   (CL:SETF (%SELECTION-PATTERN SELF) NULL)
...
(CL:DEFMETHOD NEXT? ((SELF CLASHING-PROPOSITIONS-ITERATOR))
  (CL:LET* ((REFERENCEPROP (%REFERENCE-PROPOSITION SELF)))
   (CL:LOOP WHILE (TRUE-PROPOSITIONS-ITERATOR.NEXT? SELF) DO
    (CL:WHEN
     (CLASHES-WITH-FUNCTION-PROPOSITION? (%VALUE SELF) REFERENCEPROP)
     (CL:RETURN-FROM NEXT? TRUE)))
   (CL:SETF (%VALUE SELF) NULL) FALSE))
...
(CL:DEFUN RELATIONS-WITH-DESCRIPTIONS ()
  (CL:LET* ((RESULT (NEW-LIST)))
   (CL:LET*
    ((CLASS NULL) (ITER-000 (ALL-CLASSES NULL FALSE))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO
