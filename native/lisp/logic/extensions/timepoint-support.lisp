   ...
   (CL:CASE (%WRAPPER-VALUE MISSING-ARGUMENT)
    ...
    (0
     (CL:LET* ((TIMEPOINTWRAPPER (HELP-GET-CALENDAR-TIME X2)))
      (CL:SETQ VALUE
       (WRAP-STRING
        (CALENDAR-DATE-TO-STRING (%WRAPPER-VALUE TIMEPOINTWRAPPER)
         0.0d0 CL:NIL CL:T)))))
    (1
...
(CL:DEFUN TIMEPOINT-OF*-COMPUTATION (YY MM DD HR MIN SEC TZ)
  (CL:LET*
   ((YY-VALUE (VALUE-OF YY)) (MM-VALUE (VALUE-OF MM))
    (DD-VALUE (VALUE-OF DD)) (HR-VALUE (VALUE-OF HR))
    (MIN-VALUE (VALUE-OF MIN)) (SEC-VALUE (VALUE-OF SEC))
    (TZ-VALUE (VALUE-OF TZ)) (YEAR (HELP-GET-INTEGER YY-VALUE))
   ...
   (CL:WHEN
    (CL:AND (CL:NOT (CL:= YEAR NULL-INTEGER))
     (CL:NOT (CL:= MONTH NULL-INTEGER))
     (CL:NOT (CL:= DAY NULL-INTEGER)) (CL:NOT (CL:= HOUR NULL-INTEGER))
     (CL:NOT (CL:= MINUTE NULL-INTEGER))
...
(CL:DEFUN TIMEPOINT-OF*-SPECIALIST (FRAME LASTMOVE)
  (CL:LET*
   ...
   (CL:COND
    ((CL:AND (CL:NOT (CL:= YEAR NULL-INTEGER))
      (CL:NOT (CL:= MONTH NULL-INTEGER))
      (CL:NOT (CL:= DAY NULL-INTEGER))
      (CL:NOT (CL:= HOUR NULL-INTEGER))
      (CL:NOT (CL:= MINUTE NULL-INTEGER))
      (CL:NOT (CL:= SECOND-FLOAT NULL-FLOAT))
      (CL:NOT (CL:= ZONE NULL-FLOAT)))
     (CL:SETQ SECOND (FLOOR SECOND-FLOAT))
     ...
     (SELECT-PROOF-RESULT
      (BIND-ARGUMENT-TO-VALUE?
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGS)) 7) THE-TIME
       CL:T)
      CL:NIL CL:T))
    ((CL:NOT (CL:EQ TP NULL))
     (CL:SETQ THE-TIME (HELP-GET-CALENDAR-TIME TP))
  ...
  (CL:IF (CL:NOT (CL:EQ TIMEZONE NULL))
   (CL:LET*
    ...
    (CL:MULTIPLE-VALUE-SETQ (YEAR MONTH DAY DOW)
     (GET-CALENDAR-DATE (%WRAPPER-VALUE TIMEPOINT)
      (%WRAPPER-VALUE TIMEZONE)))
...
(CL:DEFUN HELP-STARTUP-TIMEPOINT-SUPPORT1 ()
  (CL:PROGN
   ...
   (CL:SETQ
    SYM-TIMEPOINT-SUPPORT-TIMEPOINT-SUPPORT-TIMEPOINT-OF*-COMPUTATION
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TIMEPOINT-OF*-COMPUTATION" NULL
     0))
