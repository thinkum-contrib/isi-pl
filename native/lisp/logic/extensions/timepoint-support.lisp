   ...
   (CL:COND
    ((SUBTYPE-OF-STRING? TEST-VALUE-001)
     (CL:PROGN
      (CL:LET* ((VALUE (STRING-TO-CALENDAR-DATE (%WRAPPER-VALUE ITEM))))
       (CL:WHEN (CL:EQ VALUE NULL)
        (CL:SETQ VALUE (STRING-TO-TIME-DURATION (%WRAPPER-VALUE ITEM))))
       (CL:IF (CL:NOT (CL:EQ VALUE NULL)) (WRAP-DATE-TIME VALUE) NULL))))
    ((SUBTYPE-OF? TEST-VALUE-001 SGT-TIMEPOINT-SUPPORT-LOGIC-PATTERN-VARIABLE)
     (CL:PROGN
      (CL:LET* ((VALUE (SAFE-BOUND-TO ITEM)))
  ...
  (CL:IF (CL:EQ OBJ NULL) NULL-INTEGER
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE OBJ)))
    (CL:COND
     ...
     ((SUBTYPE-OF-INTEGER? TEST-VALUE-000) (CL:PROGN (%WRAPPER-VALUE OBJ)))
     (CL:T NULL-INTEGER)))))
...
(CL:DEFUN HELP-GET-FLOAT (OBJ)
  (CL:IF (CL:EQ OBJ NULL) NULL-FLOAT
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE OBJ)))
    (CL:COND
     ((SUBTYPE-OF-FLOAT? TEST-VALUE-000) (CL:PROGN (%WRAPPER-VALUE OBJ)))
  ...
  (CL:LET* ((VALUE NULL))
   (CL:CASE (%WRAPPER-VALUE MISSING-ARGUMENT)
    (-1
     (CL:SETQ VALUE
      (CL:IF
       (OBJECT-EQL? (%WRAPPER-VALUE (HELP-GET-CALENDAR-TIME X1))
        (%WRAPPER-VALUE X2))
    ...
    (0
     (CL:LET* ((TIMEPOINTWRAPPER (HELP-GET-CALENDAR-TIME X2)))
      (CL:SETQ VALUE
       (WRAP-STRING
        (CALENDAR-DATE-TO-STRING (%WRAPPER-VALUE TIMEPOINTWRAPPER) 0.0d0
         CL:NIL CL:T)))))
    (1
...
(CL:DEFUN TIMEPOINT-OF*-EVALUATOR (SELF)
  ...
  (CL:LET*
   ((STOREDVALUE
     (VALUE-OF
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS SELF))) 7)))
    (COMPUTATION
     (LOOKUP-FUNCTION
    ...
    ((CL:AND (CL:NOT (CL:= YEAR NULL-INTEGER))
     ...
     (SELECT-PROOF-RESULT
      (BIND-ARGUMENT-TO-VALUE?
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGS)) 7) THE-TIME CL:T)
      CL:NIL CL:T))
  ...
  (CL:LET* ((VALUE NULL))
   (CL:CASE (%WRAPPER-VALUE MISSING-ARGUMENT)
    (-1
     (CL:SETQ VALUE
      (CL:IF
       (OBJECT-EQL? (%WRAPPER-VALUE (HELP-GET-TIME-DURATION X1))
        (%WRAPPER-VALUE X2))
       TRUE-WRAPPER FALSE-WRAPPER)))
    (0
     (CL:LET* ((DURATIONWRAPPER (HELP-GET-TIME-DURATION X2)))
      (CL:SETQ VALUE
       (WRAP-STRING
        (TIME-DURATION-TO-STRING (%WRAPPER-VALUE DURATIONWRAPPER))))))
    (1
  ...
  (CL:LET* ((VALUE NULL))
   (CL:CASE (%WRAPPER-VALUE MISSING-ARGUMENT)
    (-1
     (CL:SETQ VALUE
      (CL:IF
       (OBJECT-EQL? (TIME-SUBTRACT (%WRAPPER-VALUE X1) (%WRAPPER-VALUE X2))
        (%WRAPPER-VALUE X3))
       TRUE-WRAPPER FALSE-WRAPPER)))
    (0
     (CL:SETQ VALUE
      (WRAP-DATE-TIME (TIME-ADD (%WRAPPER-VALUE X2) (%WRAPPER-VALUE X3)))))
    (1
     (CL:SETQ VALUE
      (WRAP-DATE-TIME
...
(CL:DEFUN TIME-PLUS-CONSTRAINT (MISSING-ARGUMENT X1 X2 X3)
  (CL:LET* ((VALUE NULL))
   (CL:CASE (%WRAPPER-VALUE MISSING-ARGUMENT)
    ...
    (2
     (CL:SETQ VALUE
      (WRAP-DATE-TIME (TIME-ADD (%WRAPPER-VALUE X1) (%WRAPPER-VALUE X2)))))
    (CL:OTHERWISE))
   VALUE))
...
(CL:DEFUN TIMEPOINT-DATE-COMPUTATION (TIMEPOINT TIMEZONE)
  ...
  (CL:IF (CL:NOT (CL:EQ TIMEZONE NULL))
   (WRAP-STRING
    (CALENDAR-DATE-TO-DATE-STRING (%WRAPPER-VALUE TIMEPOINT)
     (%WRAPPER-VALUE TIMEZONE) CL:NIL))
   NULL))
...
(CL:DEFUN TIMEPOINT-TIME-COMPUTATION (TIMEPOINT TIMEZONE)
  ...
  (CL:IF (CL:NOT (CL:EQ TIMEZONE NULL))
   (WRAP-STRING
    (CALENDAR-DATE-TO-TIME-STRING (%WRAPPER-VALUE TIMEPOINT)
     (%WRAPPER-VALUE TIMEZONE) CL:T CL:T CL:T))
   NULL))
