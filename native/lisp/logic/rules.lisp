...
(CL:DECLAIM
  ...
  *CYC-KLUDGES-ENABLED?* *CONTEXT* STANDARD-ERROR TRUE-WRAPPER
  FALSE-WRAPPER FALSE-PROPOSITION TRUE-PROPOSITION NULL-INTEGER EOL
  FALSE TRUE NIL))
        ...
        ((CL:EQ TEST-VALUE-000 KWD-RULES-EXISTS)
         (CL:LET*
          ...
          (CL:WHEN (CL:EQ (%KIND EXISTSBODY) KWD-RULES-AND)
           (CL:LET*
            ((G NULL) (VECTOR-002 (%ARGUMENTS EXISTSBODY))
     ...
     (CL:COND
      ...
      ((CL:EQ HEADORTAIL KWD-RULES-TAIL)
       (CL:LET* ((GOALCOUNT (LENGTH COLLECTION)))
        ...
        (CL:LET*
         ((ARG NULL) (VECTOR-001 (%ARGUMENTS PROPOSITION))
        ...
        (CL:WHEN (CL:= GOALCOUNT (LENGTH COLLECTION))
         (CL:LET*
          ((ARG NULL) (VECTOR-002 (%ARGUMENTS PROPOSITION))
           (INDEX-002 0) (LENGTH-002 (LENGTH VECTOR-002)))
          (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-002 LENGTH-002))
          (CL:LOOP WHILE (CL:< INDEX-002 LENGTH-002) DO
           (CL:PROGN
     ...
     (CL:OR
      (CL:NOT
       (ATOMIC-GOAL-PROPOSITION?
        (CL:AREF
         (CL:THE CL:SIMPLE-VECTOR
          (%THE-ARRAY (%ARGUMENTS MASTERFORALL)))
         0)))
     ...
     (CL:LET*
      ...
      (CL:LET* ((*INVISIBLEASSERTION?* TRUE))
       ...
       (CL:LET*
        ((SATELLITE NULL)
         (ITER-002
          (%THE-CONS-LIST (SATELLITE-PROPOSITIONS FORALLPROP))))
  ...
  (CL:WHEN *LAZY-SATELLITE-RULES?*
   (CL:WHEN
    ...
    (CL:LET* ((MASTERPROPOSITION NULL) (QUEUEDRULES (NEW-LIST)))
     ...
     (CL:LET*
      ((FORALLPROP NULL) (ITER-001 (%THE-CONS-LIST QUEUEDRULES)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ IDX (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:EQ GOAL (%FORWARD-GOAL IDX))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (DELETED? (%MASTER-RULE IDX)))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000) (CONS (%MASTER-RULE IDX) NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
   RULES))
...
so that it will reoptimize subgoal queries upon next invocation.\")"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Rules") NULL-STRING-WRAPPER)))
