...
(CL:DECLAIM
  ...
  *CYC-KLUDGES-ENABLED?* *CONTEXT* TRUE-WRAPPER FALSE-WRAPPER STANDARD-OUTPUT
  FALSE-PROPOSITION TRUE-PROPOSITION NULL-INTEGER EOL NIL))
...
(CL:DEFUN ATOMIC-GOAL-DESCRIPTION? (DESCRIPTION)
  ...
  (CL:LET*
   ...
   (CL:IF
    ...
    CL:T (ATOMIC-GOAL-PROPOSITION? (%PROPOSITION DESCRIPTION)))))
   ...
   (CL:WHEN CONTRAPOSITIVE? (CL:SETQ OLDHEAD (INVERT-PROPOSITION OLDHEAD))
    ...
    (CL:WHEN (CL:EQ NEWGOAL NULL)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
      "Unable to generate contrapositive rule for goal `" GOAL "'" EOL "in `"
      FORALLPROP "'" EOL)
  ...
  (CL:LET*
   ...
   (CL:WHEN
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS IMPLIESPROP)
     SYM-RULES-LOGIC-FORWARD-ONLY? TRUE-WRAPPER FALSE-WRAPPER))))
  ...
  (CL:LET*
   ...
   (CL:LET* ((IMPLIESPROP (CREATE-IMPLIES-PROPOSITION TAIL HEAD)))
    (CL:LET* ((*EVALUATIONMODE* KWD-RULES-EXTENSIONAL-ASSERTION))
     (CL:DECLARE (CL:SPECIAL *EVALUATIONMODE*))
     (CL:LET*
      ((DUPLICATE (RECURSIVELY-FASTEN-DOWN-PROPOSITIONS IMPLIESPROP CL:NIL)))
      (CL:COND
    ...
    (CL:WHEN
     ...
     (CL:IF CONTRAPOSITIVE?
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS IMPLIESPROP)
       SYM-RULES-LOGIC-BACKWARD-ONLY? TRUE-WRAPPER FALSE-WRAPPER)
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS IMPLIESPROP)
       SYM-RULES-LOGIC-FORWARD-ONLY? TRUE-WRAPPER FALSE-WRAPPER)))
    (CL:WHEN
     (%WRAPPER-VALUE
    ...
    (CL:WHEN
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS IMPLIESPROP)
      SYM-RULES-LOGIC-DONT-OPTIMIZE? TRUE-WRAPPER FALSE-WRAPPER)
     (CL:SETQ TAIL
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS IMPLIESPROP)))
       0))
      ...
      (CL:WHEN
       ...
       (CL:COND
        ...
        ((CL:NOT CONTRAPOSITIVEONLY?)
         (DERIVE-ONE-SATELLITE-RULE FORALLPROP GOAL KWD-RULES-HEAD CL:NIL)))
       (CL:WHEN
        ...
        (CL:COND
         ...
         (CL:T
          (DERIVE-ONE-SATELLITE-RULE FORALLPROP GOAL KWD-RULES-HEAD CL:T)))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (CL:WHEN
     (CL:AND
      (CL:NOT
        ...
        (CL:COND
         (LAZYSATELLITES?
          ...
          (INSERT
           ...
           FORALLPROP))
         ((CL:NOT CONTRAPOSITIVEONLY?)
          (DERIVE-ONE-SATELLITE-RULE FORALLPROP GOAL KWD-RULES-TAIL CL:NIL)))
        (CL:WHEN
         (CL:AND (CL:NOT POSITIVEONLY?)
          (CL:NOT
           (%WRAPPER-VALUE
         ...
         (CL:COND
          (LAZYSATELLITES?
           (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS GOALDESC)
            SYM-RULES-LOGIC-DEFERRED-CONTRAPOSITIVES? TRUE-WRAPPER NULL))
          (CL:T
           (DERIVE-ONE-SATELLITE-RULE FORALLPROP GOAL KWD-RULES-TAIL CL:T)))))
       (CL:SETQ ITER-001 (%%REST ITER-001)))))
    (CL:WHEN
      ...
      (CL:NOT
       (CL:EQ
        (ACCESS-IN-CONTEXT (%TRUTH-VALUE FORALLPROP)
         (%HOME-CONTEXT FORALLPROP) CL:NIL)
        NULL)))
     (CL:LET*
      ((*CONTEXT* (%HOME-CONTEXT FORALLPROP))
       (*MODULE* (%BASE-MODULE *CONTEXT*)))
   ...
   (CL:LET* ((TEST-VALUE-000 CL:NIL))
    ...
    (CL:WHEN TEST-VALUE-000 (CL:SETQ TEST-VALUE-000 (CL:= (LENGTH TAILS) 1))
     (CL:WHEN TEST-VALUE-000
      (CL:LET* ((ALWAYS?-000 CL:T))
       (CL:LET*
        ((HV NULL) (VECTOR-000 (%ARGUMENTS HEADGOAL)) (INDEX-000 0)
         (LENGTH-000 (LENGTH VECTOR-000)) (TV NULL)
...
(CL:DEFUN CREATE-FORWARD-CHAINING-INDEX (GOAL MASTERFORALL)
  (CL:LET*
   ...
   (CL:LET*
    ((IOVARIABLES
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FORALLCOPY)
       SYM-RULES-LOGIC-IO-VARIABLES NULL))
     (IOVARIABLENAMES NULL) (GOALVARIABLELIST (LIST)) (GOALVARIABLENAMES NIL)
     (EXISTVARIABLENAMES NIL) (QUERYBODY NULL) (CONSEQUENTTREE NULL))
    ...
    (CL:LET* ((VALUE-003 NIL))
     (CL:LET*
      ((ARG NULL) (ITER-000 (%THE-CONS-LIST GOALVARIABLELIST))
       (COLLECT-000 NULL))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:SETQ ARG (%%VALUE ITER-000))
       (CL:IF (CL:EQ COLLECT-000 NULL)
    ...
    (CL:LET*
     ((V NULL) (ITER-001 IOVARIABLENAMES) (I NULL-INTEGER) (ITER-002 0)
      (COLLECT-001 NULL))
     (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-002))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
      (CL:SETQ V (%%VALUE ITER-001)) (CL:SETQ I ITER-002)
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FORALLCOPY)
     SYM-RULES-LOGIC-IO-VARIABLES IOVARIABLES NULL)
    (CL:SETF (%CONSEQUENT INDEX)
     (CONSTRUCT-DESCRIPTION-FROM-FORALL-PROPOSITION FORALLCOPY
      KWD-RULES-HEAD))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((IDX NULL)
     (ITER-000
      (%THE-CONS-LIST
       (REMOVE-DELETED-MEMBERS (FORWARD-CHAINING-INDICES GOALDESCRIPTION)))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
   ...
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE DESCRIPTION)
     ...
     (CL:PROGN
      (CL:SETQ RULES
       (CONCATENATE
        (APPLICABLE-RULES-OF-DESCRIPTION DESCRIPTION KWD-RULES-FORWARD CL:NIL)
...
so that it will reoptimize subgoal queries upon next invocation.\")"
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-RULES-LOGIC-STARTUP-RULES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-RULES-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupRules")
      NULL-STRING-WRAPPER)))
