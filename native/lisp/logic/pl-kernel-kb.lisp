...
(CL:DEFUN |/PL-KERNEL-KB/INITIALIZE-KERNEL-KB| ()
  ...
  (CL:LET*
   ((KERNELINITFILE (LOOKUP-CONFIGURATION-PROPERTY "PLKernelKBInitFile" (WRAP-STRING "PL:sources;logic;pl-kernel-kb.plm") NULL))
    (KERNELKBDEFINITIONS (COPY-CONS-TREE |/PL-KERNEL-KB/*PL-KERNEL-KB-DEFINITIONS*|)) (DUMMYCONCEPT NULL))
   (CL:SETQ KERNELINITFILE KERNELINITFILE)
   (CL:LET* ((*WARNIFREDEFINE?* CL:NIL)) (CL:DECLARE (CL:SPECIAL *WARNIFREDEFINE?*))
    (CL:LET* ((*MODULE* (GET-STELLA-MODULE "PL-KERNEL-KB" CL:T)) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (CL:LET* ((SELF-000 (NEW-NAMED-DESCRIPTION))) (CL:SETF (%SURROGATE-VALUE-INVERSE SELF-000) |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT|)
      (CL:SETF (%IO-VARIABLE-TYPES SELF-000) (LIST |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT|)) (CL:SETQ DUMMYCONCEPT SELF-000))
     (CL:SETF (%SURROGATE-VALUE |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT|) DUMMYCONCEPT) (GET-DESCRIPTION-EXTENSION DUMMYCONCEPT CL:T)
     (UPDATE-ISA-PROPOSITION DUMMYCONCEPT |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT| |/PL-KERNEL-KB/KWD-PL-KERNEL-KB-ASSERT-TRUE|)
     (CL:SETF (%SURROGATE-VALUE |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CLASS|) DUMMYCONCEPT) (%EVALUATE (%%VALUE KERNELKBDEFINITIONS))
     (CL:SETF (%SURROGATE-VALUE |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CLASS|) (%SURROGATE-VALUE |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT|))
     (CL:LET* ((DEFINITION NULL) (ITER-000 (%%REST KERNELKBDEFINITIONS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ DEFINITION (%%VALUE ITER-000)) (%EVALUATE DEFINITION) (CL:SETQ ITER-000 (%%REST ITER-000))))
     (FINALIZE-OBJECTS) (CL:LET* ((MDL NULL) (ITER-001 (ALL-MODULES))) (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ MDL (%VALUE ITER-001)) (INTRODUCE-MODULE MDL)))
     (CL:LET* ((THING (GET-DESCRIPTION (GET-STELLA-CLASS |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-STELLA-THING| CL:T))))
...
(CL:DEFUN |/PL-KERNEL-KB/STARTUP-PL-KERNEL-KB| ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/PL-KERNEL-KB" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CONCEPT| (INTERN-RIGID-SYMBOL-WRT-MODULE "CONCEPT" NULL 1))
    (CL:SETQ |/PL-KERNEL-KB/KWD-PL-KERNEL-KB-ASSERT-TRUE| (INTERN-RIGID-SYMBOL-WRT-MODULE "ASSERT-TRUE" NULL 2))
    (CL:SETQ |/PL-KERNEL-KB/SGT-PL-KERNEL-KB-PL-KERNEL-KB-CLASS| (INTERN-RIGID-SYMBOL-WRT-MODULE "CLASS" NULL 1))
    (CL:SETQ |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-STELLA-THING| (INTERN-RIGID-SYMBOL-WRT-MODULE "THING" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    (CL:SETQ |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-PL-KERNEL-KB-STARTUP-PL-KERNEL-KB| (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-PL-KERNEL-KB" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-PL-KERNEL-KB" "(DEFUN STARTUP-PL-KERNEL-KB () :PUBLIC? TRUE)" (CL:FUNCTION |/PL-KERNEL-KB/STARTUP-PL-KERNEL-KB|) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-PL-KERNEL-KB-STARTUP-PL-KERNEL-KB|)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-STELLA-METHOD-STARTUP-CLASSNAME| (WRAP-STRING "_StartupPlKernelKb")
      NULL-STRING-WRAPPER)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 8) (FINALIZE-SLOTS) (CLEANUP-UNFINALIZED-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "PL-KERNEL")))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *PL-KERNEL-KB-DEFINITIONS* CONS NULL)")
    (CL:SETQ |/PL-KERNEL-KB/*PL-KERNEL-KB-DEFINITIONS*|
     (GET-QUOTED-TREE "(((DEFCONCEPT CONCEPT (?X RELATION) :DOCUMENTATION \"The class of reified unary relations.  The Powerloom
