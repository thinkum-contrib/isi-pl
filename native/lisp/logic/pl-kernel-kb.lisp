    ...
    (CL:LET*
     ...
     (CL:LET*
      ((THING
        (GET-DESCRIPTION
         (GET-STELLA-CLASS
          |/PL-KERNEL-KB/SYM-PL-KERNEL-KB-STELLA-THING| CL:T))))
...
and bind ?z to the result.  Coerces any type argument to a string if necessary.\" :VARIABLE-ARITY TRUE :COMPUTED TRUE :RELATION-SPECIALIST COMPUTATION-SPECIALIST :RELATION-COMPUTATION STRING-CONCATENATE-COMPUTATION) (DEFFUNCTION SUBSTRING ((?S THING) (?START INTEGER) (?END INTEGER)) :-> (?SUB THING) :DOCUMENTATION \"Generate the substring ?sub of ?s starting at position ?start (zero-based)
and ending just before position ?end.  This is the PowerLoom equivalent to the STELLA method
`subsequence'.  Supplying -1 for ?end indicates the end of ?s.  If ?s and/or ?sub are not
strings, they will be coerced first if possible.  This function supports any binding pattern
where at most one input is unbound (except for ?s), or where both ?start and ?end are unbound
in which case it can locate all occurrences of the ?sub string in ?s.  For example:
	 
      (retrieve all (?start ?end) (substring \\\"foo\\\" ?start ?end \\\"o\\\"))
   ==>  ?start = 1, ?end = 2,
        ?start = 2, ?end = 3
	
Additional examples can be found in the test suite.\" :AXIOMS (AND (COMPUTED SUBSTRING) (RELATION-SPECIALIST SUBSTRING SUBSEQUENCE-SPECIALIST))) (DEFFUNCTION STRING-MATCH ((?PATTERN STRING) (?OBJECT THING) (?START INTEGER) (?END INTEGER)) :-> (?MATCH-POSITION INTEGER) :DOCUMENTATION \"Match ?pattern against ?object between ?start and ?end (zero-based),
