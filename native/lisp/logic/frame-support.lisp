...
(CL:DEFMETHOD LEVELLIZED-TEST-TYPE-ON-INSTANCE? ((LEVEL NORMAL-INFERENCE-LEVEL) SELF TYPE)
  (CL:WHEN (CL:NOT *LEVELLIZED-BACKCHAINING-ENABLED?*)
   (CL:RETURN-FROM LEVELLIZED-TEST-TYPE-ON-INSTANCE?
    (LEVELLIZED-TEST-TYPE-ON-INSTANCE? SUBSUMPTION-INFERENCE SELF
     TYPE)))
  (APPLY-CACHED-ASK
...
(CL:DEFMETHOD LEVELLIZED-ALL-SLOT-VALUE-TYPES ((LEVEL NORMAL-INFERENCE-LEVEL) SELF RELATION)
  (CL:WHEN (CL:NOT *LEVELLIZED-BACKCHAINING-ENABLED?*)
   (CL:RETURN-FROM LEVELLIZED-ALL-SLOT-VALUE-TYPES
    (LEVELLIZED-ALL-SLOT-VALUE-TYPES SUBSUMPTION-INFERENCE SELF
     RELATION)))
  (APPLY-CACHED-RETRIEVE
   (LIST* SYM-FRAME-SUPPORT-LOGIC-?RELATION
  ...
  (CL:PROGN
   ...
   (CL:SETQ
    ...
    (INTERN-RIGID-SYMBOL-WRT-MODULE
     "F-ALL-TAXONOMIC-TYPES-MEMO-TABLE-000" NULL 1))
...
satisfied by 'self', using only assertions and upward
taxonomic reasoning.\" :PUBLIC? TRUE)"
    (CL:FUNCTION ALL-TAXONOMIC-TYPES) NULL)
