...
(CL:DECLAIM
  ...
  *VISITED-INSTANCES* STANDARD-ERROR TRUE-WRAPPER FALSE-WRAPPER
  *CASE-ANTECEDENT-TABLE* *CASE-TYPES-TABLE* *LOG-MATCH-RULES*
  *DROP-FUNCTION-VALUES?* *FAIL-UNBOUND-CLAUSES* *PARTIAL-MATCH-MODE*
  *MODULE* STANDARD-OUTPUT EOL))
...
(CL:DEFCLASS TRAINING-EXAMPLE (STANDARD-OBJECT)
  ((QUERY :ALLOCATION :INSTANCE :ACCESSOR %QUERY)
   ...
   (TEMP :ALLOCATION :INSTANCE :ACCESSOR %TEMP)
...
(CL:DEFUN NEW-TRAINING-EXAMPLE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%CACHED-SOLUTION SELF) NULL)
   (CL:SETF (%TEMP SELF) NULL-FLOAT) (CL:SETF (%OUTPUT SELF) NULL)
   (CL:SETF (%NN-HIDDEN SELF) NULL) (CL:SETF (%FACTS SELF) NULL)
   (CL:SETF (%INPUT SELF) NULL) (CL:SETF (%INPUT-FEATURES SELF) NULL)
   (CL:SETF (%MODULE SELF) NULL) (CL:SETF (%SCORE SELF) NULL-FLOAT)
   (CL:SETF (%NAME SELF) NULL) (CL:SETF (%CONCEPT SELF) NULL)
...
(CL:DEFVAR *CLOSED-WORLD-TRAINING-EXAMPLES* TRUE)
...
(CL:DEFUN SHUFFLE-VECTOR (CARDS)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:< I STOP) DO (CL:SETQ I (CL:1+ I))
    (CL:SETQ ONE (RANDOM NUM)) (CL:SETQ TWO (RANDOM NUM))
    (CL:SETQ TEMP
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY CARDS)) ONE))
    (CL:LET
...
(CL:DEFUN CREATE-PROPOSITIONAL-TRAINING-EXAMPLES (EXAMPLES NAME-FILE DATA-FILE)
  (CL:LET*
   ((NUM (LENGTH EXAMPLES)) (NUM-INPUTS 0) (HOME *MODULE*) (TEMP 0)
    (ALT-NAME (%%VALUE (%QUERY (FIRST EXAMPLES)))) (INPUT NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NUM NUM-INPUTS TEMP))
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       ...
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-000) DO
         (CL:PROGN (CL:SETQ K ITER-003)
          (CL:SETQ ITER-003 (CL:1+ ITER-003)))
         (CL:WHEN
          (CL:NOT
           (EQL?
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE1))) K)
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE2))) K)))
          (CL:SETQ SAME FALSE) (CL:RETURN))))
       (CL:WHEN
        (CL:AND SAME
         (CL:NOT (EQL? (%OUTPUT EXAMPLE1) (%OUTPUT EXAMPLE2))))
  ...
  (CL:LET*
   ((QUERY
     (MAKE-QUERY NIL (COERCE-TO-TREE (COPY-CONS-TREE CONS-QUERY)) NIL
      NIL))
    (RESULT (NEW-LIST)) (PMF NULL) (MODE *PARTIAL-MATCH-MODE*))
   (CL:SETQ *PARTIAL-MATCH-MODE* KWD-MACHINE-LEARNING-BASIC)
   (RETURN-PARTIAL-TRUTH QUERY TRUE) (CL:SETQ *PARTIAL-MATCH-MODE* MODE)
   (CL:SETQ PMF
    (%CHILD (%CHILD (%PARTIAL-MATCH-FRAME (%BASE-CONTROL-FRAME QUERY)))))
   (CL:LET*
    ((PROP NULL) (VECTOR-000 (%ARGUMENTS (%PROPOSITION (%FRAME-PTR PMF))))
...
(CL:DEFUN CREATE-INPUT-SIGNATURE (CONS-QUERY)
  (CL:LET*
   ((QUERY
     (MAKE-QUERY NIL (COERCE-TO-TREE (COPY-CONS-TREE CONS-QUERY)) NIL
      NIL))
    (PMF NULL) (RESULT 0.0d0) (MODE *PARTIAL-MATCH-MODE*) (ARG-SCORES NIL)
    (SIGNATURE NULL))
   (CL:DECLARE (CL:TYPE CL:DOUBLE-FLOAT RESULT))
   (CL:SETQ *PARTIAL-MATCH-MODE* KWD-MACHINE-LEARNING-BASIC)
   (CL:SETQ *FAIL-UNBOUND-CLAUSES* TRUE)
   (CL:SETQ RESULT (RETURN-PARTIAL-TRUTH QUERY TRUE))
   (CL:SETQ *PARTIAL-MATCH-MODE* MODE)
   (CL:SETQ PMF
    (%CHILD (%CHILD (%PARTIAL-MATCH-FRAME (%BASE-CONTROL-FRAME QUERY)))))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (EQL? SCORE (WRAP-FLOAT 1.0d0))
      (CL:LET
       ((SELF (%THE-ARRAY SIGNATURE)) (VALUE (WRAP-INTEGER 1))
        (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET
       ((SELF (%THE-ARRAY SIGNATURE)) (VALUE (WRAP-INTEGER 0))
        (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))))
...
(CL:DEFUN BUILD-CLASSIFICATION-RULE (KIND CLASS-NAME)
  (CL:LET*
   ((RULE-NAME
     (INTERN-SYMBOL
      (CONCATENATE (%SYMBOL-NAME KIND) "-Classification-Rule")))
    (RELATION-NAME
    ...
    (RULE-TYPES (LOOKUP *CASE-TYPES-TABLE* INDEX)) (CONVERTED-TYPES NIL)
    (RELATION
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS CLAUSE NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET*
    ((TYPE NULL) (ITER-001 (%THE-CONS-LIST RULE-TYPES))
     (COLLECT-001 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
   ...
   (CL:WHEN (CL:NOT (EMPTY? RULE-ANTECEDENT))
    (EVALUATE-LOGIC-COMMAND (COPY-CONS-TREE RELATION) FALSE)
    (EVALUATE-LOGIC-COMMAND (COPY-CONS-TREE RULE) FALSE))))
...
(CL:DEFUN OLD-GENERATE-CLASSIFICATION-TRAINING-EXAMPLES (INSTANCES CONCEPT)
  (CL:LET*
   ((NUM-INSTANCES (LENGTH INSTANCES)) (CONS-QUERY NIL) (TARGET NULL)
        ...
        (MAKE-QUERY NIL
         (CONS-LIST CONCEPT-NAME
          (OBJECT-NAME
           (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I)))
         NIL NIL)))
      (CL:WHEN (CL:EQ TARGET NULL) (CL:SETQ TARGET FALSE-WRAPPER))
      (CL:SETQ CONS-QUERY
       (CONS-LIST
        (INTERN-SYMBOL
  ...
  (CL:LET*
   ...
   (CL:LET* ((INSTANCE NULL) (ITER-000 (%THE-CONS-LIST INSTANCES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (TRUE? PROP)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) CONS-QUERY
       " is true" EOL)
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
       KWD-MACHINE-LEARNING-RETRACT-TRUE)
      (CL:SETQ QUERY
...
(CL:DEFUN GENERATE-REGRESSION-TRAINING-EXAMPLES (INSTANCES SLOT)
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE
     (CL:AND (CL:<= ITER-000 UPPER-BOUND-000) (CL:< INDEX-000 LENGTH-000))
     DO
     (CL:PROGN (CL:SETQ I ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:PROGN
      (CL:SETQ INST
...
(CL:PROGN (CL:DEFVAR *MAX-STRUCTURE-DEPTH* 3) (CL:DECLAIM (CL:TYPE CL:FIXNUM *MAX-STRUCTURE-DEPTH*)))

;;; (DEFGLOBAL *DROP-FUNCTION-VALUES?* ...)

(CL:DEFVAR *DROP-FUNCTION-VALUES?* TRUE)
...
(CL:DEFUN HELP-GET-ASSERTION-STRUCTURE (INST DEPTH)
  ...
  (CL:LET*
   ...
   (CL:LET* ((*MODULE* HOME) (*CONTEXT* *MODULE*))
    ...
    (CL:SETQ FACTS
     (ALL-FACTS-OF-INSTANCE (GET-INSTANCE INST) FALSE FALSE)))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      ...
      (CL:LET* ((CONS-PROP (PROPOSITION-TO-CONS FACT)))
       (CL:IF
        (CL:OR (CL:EQ (%KIND FACT) KWD-MACHINE-LEARNING-ISA)
         (CL:< DEPTH *MAX-STRUCTURE-DEPTH*))
        (CL:LET* ((TEST-VALUE-000 (%KIND FACT)))
         (CL:COND
          ((CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-FUNCTION)
           (CL:SETQ CONS-PROP (CONS CONS-PROP NIL))
           (CL:WHEN
            (ISA? (LAST (%%VALUE CONS-PROP))
             SGT-MACHINE-LEARNING-STELLA-NUMBER-WRAPPER)
            (CL:LET*
             ((SKOLEM-NAME
               (REVERSE
                (%%REST (REVERSE (COPY-CONS-TREE (%%VALUE CONS-PROP))))))
              (SKOLEM-VALUE (LAST (%%VALUE CONS-PROP))))
             (CL:SETQ CONS-PROP
              (CONS
               (LIST* SYM-MACHINE-LEARNING-STELLA->=
          ...
          ((CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-ISA)
           (CL:SETQ CONS-PROP (CONS CONS-PROP NIL)))
          (CL:T (CL:SETQ CONS-PROP (CONS CONS-PROP NIL)))))
        (CL:SETQ CONS-PROP NIL))
       (CL:WHEN (CL:NOT (CL:EQ CONS-PROP NIL))
        (CL:LET* ((PROP NULL) (ITER-001 CONS-PROP))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:PROGN (CL:SETQ PROP (%%VALUE ITER-001))
           (CL:SETQ ITER-001 (%%REST ITER-001)))
          (PUSH ANTECEDENT PROP)))
        (CL:LET* ((ELE NULL) (ITER-002 (%%REST (LAST CONS-PROP))))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ARG)))
   (CL:COND
           ...
           (CL:IF (CL:EQ COLLECT-000 NULL)
            (CL:PROGN
             (CL:SETQ COLLECT-000
              (CONS
               (CONSIFY-ARGUMENT
                (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SK-ARGS))
                 I))
               NIL))
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-LOGIC-OBJECT)
     (CL:PROGN
      (INTERN-SYMBOL (%SYMBOL-NAME (%SURROGATE-VALUE-INVERSE ARG)))))
    (CL:T ARG))))
...
(CL:DEFUN %PRINT-FACTS (INSTANCEREF)
  (CL:LET* ((INSTANCE (COERCE-TO-INSTANCE INSTANCEREF NULL)))
   (CL:LET*
    ((FACT NULL)
     (ITER-000
      (%THE-CONS-LIST (ALL-FACTS-OF-INSTANCE INSTANCE FALSE FALSE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ FACT (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN HELP-STARTUP-MACHINE-LEARNING1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-MACHINE-LEARNING-LOGIC-SET-CLOSED-WORLD-TRAINING-EXAMPLES
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-CLOSED-WORLD-TRAINING-EXAMPLES"
     NULL 0))
   ...
   (CL:SETQ SYM-MACHINE-LEARNING-LOGIC-ADD-TRAINING-EXAMPLE-IN-MODULE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ADD-TRAINING-EXAMPLE-IN-MODULE" NULL
     0))
  ...
  testing examples\")" (CL:FUNCTION %ADD-TESTING-EXAMPLE)
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Machine-Learning") NULL-STRING-WRAPPER))))
