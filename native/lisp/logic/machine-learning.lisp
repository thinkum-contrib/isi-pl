...
(CL:DECLAIM
  ...
  *LOG-MATCH-RULES* *DROP-FUNCTION-VALUES?* *FAIL-UNBOUND-CLAUSES?*
  TRUE-WRAPPER TRUE *MODULE* NIL STANDARD-OUTPUT EOL FALSE NULL-FLOAT
  NULL-INTEGER))
...
(CL:DEFCLASS TRAINING-EXAMPLE (STANDARD-OBJECT)
  ((QUERY :ALLOCATION :INSTANCE :ACCESSOR %QUERY)
   ...
   (TEMP :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION
    :INSTANCE :ACCESSOR %TEMP)
...
(CL:DEFUN PRINT-TRAINING-EXAMPLE (EXAMPLE)
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (%QUERY EXAMPLE)
   " in " (%MODULE EXAMPLE) EOL))
   ...
   (CL:LOOP WHILE (CL:< I STOP) DO (CL:SETQ I (CL:1+ I))
    ...
    (CL:LET
     ((SELF (%THE-ARRAY CARDS))
      (VALUE
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY CARDS)) TWO))
      (POSITION ONE))
  ...
  (CL:LET* ((NUM-INPUTS 0) (HOME *MODULE*) (TEMP 0) (INPUT NULL))
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM DATA-FILE) (%OUTPUT EXAMPLE)
      EOL)))
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (CL:EQ (%MODULE EXAMPLE) NULL))
      (CHANGE-MODULE (%MODULE EXAMPLE)))
     (CL:SETQ INSTANCE
      (GET-INSTANCE (%%VALUE (%%REST (%QUERY EXAMPLE)))))
        ...
        (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-000) DO
         ...
         (CL:WHEN
          (CL:NOT
           (EQL?
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE1)))
             K)
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE2)))
             K)))
...
(CL:DEFUN GET-CLASSIFICATION-RELATIONS (CONS-QUERY)
  (CL:LET*
   ...
   (CL:LET*
    ((PROP NULL)
     (VECTOR-000 (%ARGUMENTS (%PROPOSITION (%CONTROL-FRAME PMF))))
...
(CL:DEFUN BUILD-CLASSIFICATION-RULE (KIND CLASS-NAME)
  (CL:LET*
   ((RULE-NAME
    ...
    (RELATION-NAME
     (INTERN-SYMBOL
      (CONCATENATE (%SYMBOL-NAME KIND) "-Classification")))
     ...
     (CL:WHEN
      (CL:NOT
       (CL:EQ
        ...
        NULL))
      (BUILD-CASE-FROM-INSTANCE
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY CASES)) I)
       KIND))))
...
(CL:DEFUN GENERATE-REGRESSION-TRAINING-EXAMPLES (INSTANCES SLOT)
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE
     (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 INDEX-000 LENGTH-000
      I))
    (CL:LOOP WHILE
...
(CL:DEFUN HELP-GET-ASSERTION-STRUCTURE (INST DEPTH)
  ...
  (CL:LET*
   ((HOME (%HOME-CONTEXT INST)) (FACTS (NEW-LIST))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE BIND)))
     (CL:COND
      ...
      ((SUBTYPE-OF? TEST-VALUE-000
       ...
       (CL:PROGN
        (INTERN-SYMBOL
         (%SYMBOL-NAME (%SURROGATE-VALUE-INVERSE BIND)))))
      (CL:T BIND))))))
     ...
     (CL:PROGN
      (CL:IF
       ...
       (CL:IF (CL:NOT (CL:EQ (%DEFINING-PROPOSITION ARG) NULL))
        (CL:LET*
         ((PROP (%DEFINING-PROPOSITION ARG))
          (SK-ARGS (%ARGUMENTS PROP)) (CONS-ARGS NIL))
         (CL:LET*
          ((I NULL-INTEGER) (ITER-000 0)
     ...
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-FORALL)
      ...
      (CL:LET* ((BINDINGS NIL))
       ...
       (CL:WHEN (CL:NOT (CL:EQ BINDINGS NIL))
        (CL:SETQ RESULT
         (CONS-LIST (INTERN-SYMBOL (%SYMBOL-NAME (%KIND PROP)))
          BINDINGS RESULT)))))
     (CL:T
      (CL:SETQ RESULT
