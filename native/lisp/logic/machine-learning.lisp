...
(CL:DECLAIM
  ...
  *FAIL-UNBOUND-CLAUSES?* TRUE-WRAPPER *MODULE* NIL STANDARD-OUTPUT EOL
...
(CL:DEFUN GET-CLASSIFICATION-RELATIONS (CONS-QUERY)
  (CL:LET*
   ((QUERY (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL "Antecedent"))
...
(CL:DEFUN CREATE-INPUT-SIGNATURE (CONS-QUERY)
  (CL:LET*
   ((QUERY (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL "Signature"))
   ...
   (CL:LET* ((INSTANCE NULL) (ITER-000 (%THE-CONS-LIST INSTANCES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (TRUE? PROP)
      ...
      (CL:SETQ QUERY
       (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL "TestUnknown1"))
     ...
     (CL:WHEN (FALSE? PROP)
      ...
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
       KWD-MACHINE-LEARNING-RETRACT-FALSE)
      (CL:SETQ QUERY
       (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL "TestTrue"))
      (CL:WHEN (TRUE-TRUTH-VALUE? (CALL-ASK QUERY)) (CL:SETQ CORRECT? CL:NIL)
  ...
  (CL:LET* ((CORRECT 0.0d0) (QUERY NULL) (CONS-QUERY NIL) (PROP NULL))
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ PROP (CONCEIVE-FORMULA (COPY-CONS-TREE CONS-QUERY)))
     (CL:SETQ QUERY
      (MAKE-QUERY NIL (COPY-CONS-TREE (%QUERY EXAMPLE)) NIL NIL "TestTheory"))
     (CL:COND
     ...
     (CL:IF (TRUE? PROP)
      (CL:PROGN
       (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
        KWD-MACHINE-LEARNING-RETRACT-TRUE)
       (CL:SETQ QUERY
        (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL "AlreadyTrue"))
       (CL:WHEN (UNKNOWN-TRUTH-VALUE? (CALL-ASK QUERY))
  ...
  (CL:LET*
   ...
   (CL:SETQ SCORE
    (RETURN-PARTIAL-TRUTH
     (MAKE-QUERY NIL (COERCE-TO-TREE PROP) NIL NIL "ScoreQuery") CL:T))
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:IF
       (CL:NOT
        (CL:EQ
         (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) CL:NIL)
         NULL))
       (CONSIFY-ARGUMENT
        (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) CL:NIL))
   ...
   (CL:LET* ((TEST-VALUE-000 (%KIND PROP)))
    (CL:COND
     ...
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-PREDICATE)
      ...
      (CL:SETQ RESULT
       (CONS
        (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%OPERATOR PROP))
         (%HOME-CONTEXT (%OPERATOR PROP)) CL:T)
        (REVERSE RESULT))))
