...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *QUERYITERATOR* *VISITED-PROPOSITIONS*
  *VISITED-INSTANCES* STANDARD-ERROR UNKNOWN-TRUTH-VALUE TRUE-TRUTH-VALUE
  DEFAULT-TRUE-TRUTH-VALUE FALSE-WRAPPER *CASE-ANTECEDENT-TABLE*
  *CASE-TYPES-TABLE* *LOG-MATCH-RULES* *DROP-FUNCTION-VALUES?*
  *FAIL-UNBOUND-CLAUSES?* TRUE-WRAPPER TRUE *MODULE* NIL STANDARD-OUTPUT EOL
  FALSE NULL-FLOAT NULL-INTEGER))
...
(CL:DEFCLASS TRAINING-EXAMPLE (STANDARD-OBJECT)
  ((QUERY :ALLOCATION :INSTANCE :ACCESSOR %QUERY)
   ...
   (INPUT-FEATURES :ALLOCATION :INSTANCE :ACCESSOR %INPUT-FEATURES)
   (INPUT :ALLOCATION :INSTANCE :ACCESSOR %INPUT)
   (FACTS :ALLOCATION :INSTANCE :ACCESSOR %FACTS)
   (NN-HIDDEN :ALLOCATION :INSTANCE :ACCESSOR %NN-HIDDEN)
   (OUTPUT :ALLOCATION :INSTANCE :ACCESSOR %OUTPUT)
   (TEMP :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :ALLOCATION :INSTANCE
    :ACCESSOR %TEMP)
...
(CL:DEFUN NEW-TRAINING-EXAMPLE ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE TRAINING-EXAMPLE)))
   (CL:SETF (%TIMESTAMP SELF) NULL-INTEGER)
   (CL:SETF (%CACHED-SOLUTION SELF) NULL) (CL:SETF (%TEMP SELF) NULL-FLOAT)
   (CL:SETF (%OUTPUT SELF) NULL) (CL:SETF (%NN-HIDDEN SELF) NULL)
   (CL:SETF (%FACTS SELF) NULL) (CL:SETF (%INPUT SELF) NULL)
   (CL:SETF (%INPUT-FEATURES SELF) NULL) (CL:SETF (%MODULE SELF) NULL)
   (CL:SETF (%SCORE SELF) NULL-FLOAT) (CL:SETF (%NAME SELF) NULL)
   (CL:SETF (%CONCEPT SELF) NULL) (CL:SETF (%QUERY SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-TRAINING-EXAMPLE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-STELLA-NAME)
    (CL:IF SETVALUE? (CL:SETF (%NAME SELF) VALUE)
     (CL:SETQ VALUE (%NAME SELF))))
   ...
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-INPUT-FEATURES)
    (CL:IF SETVALUE? (CL:SETF (%INPUT-FEATURES SELF) VALUE)
     (CL:SETQ VALUE (%INPUT-FEATURES SELF))))
   ...
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-FACTS)
    (CL:IF SETVALUE? (CL:SETF (%FACTS SELF) VALUE)
     (CL:SETQ VALUE (%FACTS SELF))))
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-NN-HIDDEN)
    (CL:IF SETVALUE? (CL:SETF (%NN-HIDDEN SELF) VALUE)
     (CL:SETQ VALUE (%NN-HIDDEN SELF))))
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-OUTPUT)
    (CL:IF SETVALUE? (CL:SETF (%OUTPUT SELF) VALUE)
     (CL:SETQ VALUE (%OUTPUT SELF))))
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-TEMP)
    (CL:IF SETVALUE? (CL:SETF (%TEMP SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-FLOAT (%TEMP SELF)))))
   ((CL:EQ SLOTNAME SYM-MACHINE-LEARNING-LOGIC-CACHED-SOLUTION)
    (CL:IF SETVALUE? (CL:SETF (%CACHED-SOLUTION SELF) VALUE)
     (CL:SETQ VALUE (%CACHED-SOLUTION SELF))))
...
(CL:DEFVAR *CLOSED-WORLD-TRAINING-EXAMPLES* FALSE)
...
(CL:DEFUN PRINT-TRAINING-EXAMPLE (EXAMPLE)
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (%QUERY EXAMPLE) " in "
   (%MODULE EXAMPLE) EOL))
...
(CL:DEFUN %ADD-TRAINING-EXAMPLE (FORM SCORE)
  ...
  (CL:LET* ((EXAMPLE (NEW-TRAINING-EXAMPLE)))
   (CL:SETF (%QUERY EXAMPLE) (COPY-CONS-LIST FORM))
...
(CL:DEFUN %ADD-TRAINING-EXAMPLE-IN-MODULE (FORM SCORE MODULE)
  ...
  (CL:LET* ((EXAMPLE (NEW-TRAINING-EXAMPLE)))
   (CL:SETF (%QUERY EXAMPLE) (COPY-CONS-LIST FORM))
   (CL:SETF (%SCORE EXAMPLE) SCORE)
   (CL:SETF (%MODULE EXAMPLE) (GET-MODULE MODULE))
...
(CL:DEFUN SHUFFLE-LIST (CARDS)
  (CL:LET*
   ((NUM (LENGTH CARDS)) (STOP (CL:/ NUM 2.0d0)) (TEMP NULL) (I 0) (ONE 0)
    (TWO 0))
   ...
   (CL:LOOP WHILE (CL:< I STOP) DO (CL:SETQ I (CL:1+ I))
    ...
    (CL:SETQ TEMP (NTH CARDS ONE)) (NTH-SETTER CARDS (NTH CARDS TWO) ONE)
    (NTH-SETTER CARDS TEMP TWO))))
...
(CL:DEFUN SHUFFLE-VECTOR (CARDS)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:< I STOP) DO (CL:SETQ I (CL:1+ I))
    ...
    (CL:LET
     ((SELF (%THE-ARRAY CARDS))
      (VALUE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY CARDS)) TWO))
      (POSITION ONE))
...
(CL:DEFUN SAVE-TRAINING-EXAMPLES ()
  (CL:LET*
   ...
   (CREATE-PROPOSITIONAL-TRAINING-EXAMPLES *TRAINING-EXAMPLES* NAME-FILE
    DATA-FILE)
  ...
  (CL:LET* ((NUM-INPUTS 0) (HOME *MODULE*) (TEMP 0) (INPUT NULL))
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ INPUT (CREATE-INPUT-SIGNATURE (%QUERY EXAMPLE)))
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (CL:SETQ TEMP (%WRAPPER-VALUE ELE))
       (%%PRINT-STREAM (%NATIVE-STREAM DATA-FILE) TEMP ",")))
     (%%PRINT-STREAM (%NATIVE-STREAM DATA-FILE) (%OUTPUT EXAMPLE) EOL)))
   (CL:SETQ NUM-INPUTS (LENGTH INPUT))
   (%%PRINT-STREAM (%NATIVE-STREAM NAME-FILE) "true,false" EOL)
   (CL:LET*
    ((I NULL-INTEGER) (ITER-001 0) (UPPER-BOUND-000 (CL:1- NUM-INPUTS)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-000) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM NAME-FILE) "A"
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:EQ (%INPUT EXAMPLE) NULL)
      (CL:WHEN (CL:NOT (CL:EQ (%MODULE EXAMPLE) NULL))
       (CHANGE-MODULE (%MODULE EXAMPLE)))
      (CL:SETF (%INPUT EXAMPLE) (CREATE-INPUT-SIGNATURE (%QUERY EXAMPLE)))
      (CHANGE-MODULE HOME))))
   (CL:SETQ SIG-LENGTH (LENGTH (%INPUT (FIRST EXAMPLES))))
   (CL:LET* ((EXAMPLE1 NULL) (ITER-001 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ EXAMPLE1 (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:LET* ((EXAMPLE2 NULL) (ITER-002 (%THE-CONS-LIST EXAMPLES)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       (CL:PROGN (CL:SETQ EXAMPLE2 (%%VALUE ITER-002))
        (CL:SETQ ITER-002 (%%REST ITER-002)))
       (CL:SETQ SAME TRUE)
       (CL:LET*
        ((K NULL-INTEGER) (ITER-003 0) (UPPER-BOUND-000 (CL:1- SIG-LENGTH)))
        (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-003 UPPER-BOUND-000 K))
        (CL:LOOP WHILE (CL:<= ITER-003 UPPER-BOUND-000) DO
         (CL:PROGN (CL:SETQ K ITER-003) (CL:SETQ ITER-003 (CL:1+ ITER-003)))
         (CL:WHEN
          (CL:NOT
           (EQL?
            (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE1)))
             K)
            (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%INPUT EXAMPLE2)))
             K)))
          (CL:SETQ SAME FALSE) (CL:RETURN))))
       (CL:WHEN
        (CL:AND SAME (CL:NOT (EQL? (%OUTPUT EXAMPLE1) (%OUTPUT EXAMPLE2))))
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
         (LAST (%QUERY EXAMPLE1)) " and " (LAST (%QUERY EXAMPLE2))
...
(CL:DEFUN GET-CLASSIFICATION-RELATIONS (CONS-QUERY)
  (CL:LET*
   ...
   (INSERT-AT (%OPTIONS QUERY) KWD-MACHINE-LEARNING-MATCH-MODE
    KWD-MACHINE-LEARNING-BASIC)
   (INSERT-AT (%OPTIONS QUERY) KWD-MACHINE-LEARNING-MAXIMIZE-SCORE?
    TRUE-WRAPPER)
   (CALL-ASK-PARTIAL QUERY)
   (CL:SETQ PMF
    (%CHILD (%CHILD (%PARTIAL-MATCH-FRAME (%BASE-CONTROL-FRAME QUERY)))))
   (CL:LET*
    ((PROP NULL) (VECTOR-000 (%ARGUMENTS (%PROPOSITION (%CONTROL-FRAME PMF))))
...
(CL:DEFUN CREATE-INPUT-SIGNATURE (CONS-QUERY)
  (CL:LET*
   ...
   (CL:SETQ PMF
    (%CHILD (%CHILD (%PARTIAL-MATCH-FRAME (%BASE-CONTROL-FRAME QUERY)))))
   (CL:SETQ ARG-SCORES (%ARGUMENT-SCORES PMF))
   (CL:SETQ SIGNATURE (NEW-VECTOR (LENGTH ARG-SCORES)))
   (CL:LET*
    ((I NULL-INTEGER) (SCORE NULL) (ITER-000 (REVERSE ARG-SCORES))
     (ITER-001 0))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (EQL? SCORE (WRAP-FLOAT 1.0d0))
      (CL:LET
       ((SELF (%THE-ARRAY SIGNATURE)) (VALUE (WRAP-INTEGER 1)) (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET
       ((SELF (%THE-ARRAY SIGNATURE)) (VALUE (WRAP-INTEGER 0)) (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))))
   SIGNATURE))
...
(CL:DEFUN GENERATE-CLASSIFICATION-RULE (INSTANCES CONCEPT-NAME CLASS-NAME BUILD-RULE?)
  (CL:LET*
   ((NUM-INSTANCES (LENGTH INSTANCES))
    (KIND (%SURROGATE-VALUE-INVERSE (GET-DESCRIPTION CLASS-NAME))) (NAME NULL)
    (PROP NULL) (FUNCTION-VALUES? *DROP-FUNCTION-VALUES?*))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NUM-INSTANCES))
   (CL:SETQ *DROP-FUNCTION-VALUES?* FALSE)
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:WHEN (TRUE? PROP)
      ...
      (BUILD-CASE-FROM-INSTANCE
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I) KIND)
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
...
(CL:DEFUN BUILD-CLASSIFICATION-RULE (KIND CLASS-NAME)
  (CL:LET*
   ((RULE-NAME
     (INTERN-SYMBOL (CONCATENATE (%SYMBOL-NAME KIND) "-Classification-Rule")))
    (RELATION-NAME
     (INTERN-SYMBOL (CONCATENATE (%SYMBOL-NAME KIND) "-Classification")))
    (INDEX (CONCATENATE (%SYMBOL-NAME CLASS-NAME) "-match"))
    (RULE-ANTECEDENT (LOOKUP *CASE-ANTECEDENT-TABLE* INDEX))
    (CONVERTED-ANTECEDENT NIL) (RULE-TYPES (LOOKUP *CASE-TYPES-TABLE* INDEX))
    (CONVERTED-TYPES NIL)
    (RELATION
     (LIST* SYM-MACHINE-LEARNING-LOGIC-DEFRELATION RELATION-NAME
      (CONS
       (CONS
        (LIST* SYM-MACHINE-LEARNING-LOGIC-?P SYM-MACHINE-LEARNING-STELLA-THING
         NIL)
        NIL)
       NIL)))
    (RULE NIL))
   ...
   (CL:LET*
    ((CLAUSE NULL) (ITER-000 (%THE-CONS-LIST RULE-ANTECEDENT))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS CLAUSE NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET*
    ((TYPE NULL) (ITER-001 (%THE-CONS-LIST RULE-TYPES)) (COLLECT-001 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     ...
     (LIST*
      (LIST* SYM-MACHINE-LEARNING-STELLA-FORALL CONVERTED-TYPES
       (CONS
        ...
        NIL))
      KWD-MACHINE-LEARNING-DONT-OPTIMIZE? SYM-MACHINE-LEARNING-STELLA-TRUE
      NIL)))
...
(CL:DEFUN OLD-GENERATE-CLASSIFICATION-TRAINING-EXAMPLES (INSTANCES CONCEPT)
  (CL:LET*
   ((NUM-INSTANCES (LENGTH INSTANCES)) (CONS-QUERY NIL) (TARGET FALSE)
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:LET* ((EXAMPLE (NEW-TRAINING-EXAMPLE)))
      ...
      (CL:SETQ CONS-QUERY
       (CONS-LIST
        (INTERN-SYMBOL
         (CONCATENATE (%SYMBOL-NAME CONCEPT-NAME) "-Classification"))
        (OBJECT-NAME
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I))))
      (CL:SETF (%QUERY EXAMPLE) CONS-QUERY)
      (CL:SETF (%OUTPUT EXAMPLE) (CL:IF TARGET TRUE-WRAPPER FALSE-WRAPPER))
      (CL:SETF (%CONCEPT EXAMPLE) CONCEPT-NAME)
      (CL:SETF (%NAME EXAMPLE)
       (OBJECT-NAME
        (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I)))
      (CL:SETF (%MODULE EXAMPLE) *MODULE*)
      (CL:SETF (%INPUT EXAMPLE) (CREATE-INPUT-SIGNATURE CONS-QUERY))
...
(CL:DEFUN %TEST-DOMAIN-THEORY (RELATION-NAME CLASS-NAME)
  (CL:LET*
   ((CLASS (%SURROGATE-VALUE-INVERSE (GET-DESCRIPTION CLASS-NAME)))
    ...
    (INSTANCES (LISTIFY (ALL-CLASS-INSTANCES CLASS))) (CORRECT? TRUE)
    (NUM-CORRECT 0))
   ...
   (CL:LET* ((INSTANCE NULL) (ITER-000 (%THE-CONS-LIST INSTANCES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ CORRECT? TRUE)
     (CL:WHEN (TRUE? PROP)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) CONS-QUERY " is true"
       EOL)
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
       KWD-MACHINE-LEARNING-RETRACT-TRUE)
      (CL:SETQ QUERY
       (MAKE-QUERY NIL
        ...
        NIL NIL))
      (CL:WHEN (TRUE-TRUTH-VALUE? (CALL-ASK QUERY)) (CL:SETQ CORRECT? FALSE)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
      ...
      (CL:WHEN (UNKNOWN-TRUTH-VALUE? (CALL-ASK QUERY))
       (CL:SETQ CORRECT? FALSE)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
        "  **Theory cannot prove " CONS-QUERY EOL))
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
       KWD-MACHINE-LEARNING-ASSERT-TRUE))
     (CL:WHEN (FALSE? PROP)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) CONS-QUERY " is false"
       EOL)
      (HELP-UPDATE-TOP-LEVEL-PROPOSITION PROP
       KWD-MACHINE-LEARNING-RETRACT-FALSE)
      (CL:SETQ QUERY (MAKE-QUERY NIL (COPY-CONS-TREE CONS-QUERY) NIL NIL))
      (CL:WHEN (TRUE-TRUTH-VALUE? (CALL-ASK QUERY)) (CL:SETQ CORRECT? FALSE)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "  **Theory proves "
        CONS-QUERY "true" EOL))
      (CL:SETQ QUERY
       (MAKE-QUERY NIL
        (COPY-CONS-TREE
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "Theory got "
    NUM-CORRECT " out of " (LENGTH INSTANCES) EOL)))
  ...
  (CL:LET* ((CORRECT 0.0d0) (QUERY NULL) (CONS-QUERY NIL) (PROP NULL))
   ...
   (CL:LET* ((EXAMPLE NULL) (ITER-000 (%THE-CONS-LIST EXAMPLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ PROP (CONCEIVE-FORMULA (COPY-CONS-TREE CONS-QUERY)))
     (CL:SETQ QUERY
      (MAKE-QUERY NIL (COPY-CONS-TREE (%QUERY EXAMPLE)) NIL NIL))
     (CL:COND
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:DOUBLE-FLOAT) TEST-THEORY-OVER-EXAMPLES))

;;; (DEFUN (GENERATE-TRAINING-EXAMPLES INTEGER) ...)

(CL:DEFUN GENERATE-TRAINING-EXAMPLES (INSTANCES CONCEPT)
  (CL:LET*
   ((NUM-INSTANCES (LENGTH INSTANCES)) (CONS-QUERY NIL) (NUM-EXAMPLES 0)
    (EXAMPLE NULL) (PROP NULL) (QUERY NULL)
    (CONCEPT-NAME (INTERN-SYMBOL (%SYMBOL-NAME CONCEPT))))
     ...
     (CL:IF (TRUE? PROP)
      (CL:PROGN
       ...
       (CL:WHEN (UNKNOWN-TRUTH-VALUE? (CALL-ASK QUERY))
        ...
        (CL:SETF (%QUERY EXAMPLE) CONS-QUERY)
        (CL:SETF (%OUTPUT EXAMPLE) TRUE-WRAPPER)
        (CL:SETF (%CONCEPT EXAMPLE) CONCEPT-NAME)
        (CL:SETF (%NAME EXAMPLE)
         (OBJECT-NAME
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I)))
      ...
      (CL:WHEN (CL:OR (FALSE? PROP) *CLOSED-WORLD-TRAINING-EXAMPLES*)
       (CL:PROGN (CL:SETQ EXAMPLE (NEW-TRAINING-EXAMPLE))
        (CL:SETF (%QUERY EXAMPLE) CONS-QUERY)
        (CL:SETF (%OUTPUT EXAMPLE) FALSE-WRAPPER)
        (CL:SETF (%CONCEPT EXAMPLE) CONCEPT-NAME)
        (CL:SETF (%NAME EXAMPLE)
         (OBJECT-NAME
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY INSTANCES)) I)))
   ...
   NUM-EXAMPLES))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:FIXNUM) GENERATE-TRAINING-EXAMPLES))
...
(CL:DEFUN GENERATE-CLASSIFICATION-TRAINING-EXAMPLES (INSTANCES CONCEPT CREATE-SIGNATURE?)
  (CL:LET*
   ((NUM-INSTANCES (LENGTH INSTANCES)) (CONS-QUERY NIL) (NUM-EXAMPLES 0)
    (EXAMPLE NULL) (PROP NULL) (CLASSIFICATION-QUERY NIL) (QUERY NULL)
    (CONCEPT-NAME (INTERN-SYMBOL (%SYMBOL-NAME CONCEPT))))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NUM-INSTANCES NUM-EXAMPLES))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- NUM-INSTANCES)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:IF (TRUE? PROP)
      (CL:PROGN
       ...
       (CL:WHEN (UNKNOWN-TRUTH-VALUE? (CALL-ASK QUERY))
        (CL:SETQ EXAMPLE (NEW-TRAINING-EXAMPLE))
        (CL:SETF (%QUERY EXAMPLE) CLASSIFICATION-QUERY)
        (CL:SETF (%OUTPUT EXAMPLE) TRUE-WRAPPER)
        (CL:SETF (%CONCEPT EXAMPLE) CONCEPT-NAME)
        (CL:SETF (%NAME EXAMPLE)
         (OBJECT-NAME
  ...
  (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (OBJECT-NAME CASE) NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET*
    ((I NULL-INTEGER) (CASE-NAME NULL) (CASE NULL) (VECTOR-001 CASES)
     (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001))
     (ITER-000 (%THE-CONS-LIST CASE-NAMES)) (ITER-001 0))
    ...
    (CL:LOOP WHILE
     ...
     (CL:PROGN (CL:SETQ CASE-NAME (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:PROGN (CL:SETQ I ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:<= ITER-002 UPPER-BOUND-000) DO
     ...
     (CL:WHEN
      (CL:NOT
       (CL:EQ (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SLOT-VALUES)) I)
        NULL))
      (BUILD-CASE-FROM-INSTANCE
   ...
   (CL:SETQ *DROP-FUNCTION-VALUES?* FUNCTION-VALUES?)
   (CL:LET*
    ((CASE-NAME NULL) (VAL NULL) (VECTOR-002 SLOT-VALUES) (INDEX-002 0)
     (LENGTH-002 (LENGTH VECTOR-002)) (ITER-003 (%THE-CONS-LIST CASE-NAMES)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-002 LENGTH-002))
    (CL:LOOP WHILE
     (CL:AND (CL:< INDEX-002 LENGTH-002) (CL:NOT (CL:EQ ITER-003 NIL))) DO
     (CL:PROGN
      (CL:SETQ VAL
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-002)) INDEX-002))
      (CL:SETQ INDEX-002 (CL:1+ INDEX-002)))
     (CL:PROGN (CL:SETQ CASE-NAME (%%VALUE ITER-003))
...
(CL:DEFUN GENERATE-REGRESSION-TRAINING-EXAMPLES (INSTANCES SLOT)
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:FIXNUM NUM-INSTANCES))
   (CL:LET*
    ((INST NULL) (I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:1- NUM-INSTANCES)) (VECTOR-000 INSTANCES)
     (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
    (CL:DECLARE
     (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 INDEX-000 LENGTH-000 I))
    (CL:LOOP WHILE
     ...
     (CL:PROGN
      ...
      (CL:SETQ INDEX-000 (CL:1+ INDEX-000)))
     (CL:LET
      ((SELF (%THE-ARRAY SLOT-VALUES)) (VALUE (GET-SLOT-VALUE INST SLOT))
       (POSITION I))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
       (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))))
     ...
     (CL:WHEN
      ...
      (CL:LET* ((EXAMPLE (NEW-TRAINING-EXAMPLE)))
       ...
       (CL:SETF (%SCORE EXAMPLE)
        (%WRAPPER-VALUE
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SLOT-VALUES)) I)))
       (CL:SETF (%TIMESTAMP EXAMPLE) -1)
       (PUSH *TRAINING-EXAMPLES* EXAMPLE)))))
   NUM-INSTANCES))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:FIXNUM)
...
(CL:DEFUN %APPROXIMATE (INST-NAME SLOT-NAME)
  ...
  (CL:LET* ((SLOT (%SURROGATE-VALUE-INVERSE (GET-DESCRIPTION SLOT-NAME))))
   (CL:IF (FLOAT-FUNCTION? SLOT) (GENERATE-SLOT-APPROXIMATION INST-NAME SLOT)
    (CL:PROGN
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) "ERROR " SLOT-NAME
...
(CL:DEFUN APPROXIMATE-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:LET*
   ((RESULT (%APPROXIMATE (%%VALUE ARGUMENTS) (%%VALUE (%%REST ARGUMENTS)))))
...
(CL:DEFUN GENERATE-SLOT-APPROXIMATION (INST-NAME SLOT)
  (CL:LET*
   ((PROBE (GET-INSTANCE INST-NAME))
    (CLASS
     (%SURROGATE-VALUE-INVERSE (LAST (GET-MOST-SPECIFIC-CLASSES PROBE))))
    (RULE-NAME
     (INTERN-SYMBOL (CONCATENATE (%SYMBOL-NAME SLOT) "-Classification")))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET* ((VALUE (GET-SLOT-VALUE INSTANCE SLOT)))
      (CL:WHEN (CL:NOT (CL:EQ VALUE NULL))
       (CL:LET* ((FL-VALUE (%WRAPPER-VALUE VALUE)))
  ...
  (CL:LET* ((DES (%SURROGATE-VALUE RELATION)))
   (CL:IF
    (CL:AND (FUNCTION-DESCRIPTION? DES)
     (CL:EQ (LAST (%IO-VARIABLE-TYPES DES))
...
(CL:DEFVAR *DROP-FUNCTION-VALUES?* FALSE)
...
(CL:DEFUN HELP-GET-ASSERTION-STRUCTURE (INST DEPTH)
  ...
  (CL:LET*
   ((HOME (%HOME-CONTEXT INST)) (FACTS (NEW-LIST)) (ANTECEDENT (NEW-LIST))
    (CHILDREN NULL))
   (CL:LET* ((*MODULE* HOME) (*CONTEXT* *MODULE*))
    (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:SETQ FACTS (ALL-FACTS-OF-INSTANCE (GET-INSTANCE INST) FALSE FALSE)))
   (CL:LET* ((FACT NULL) (ITER-000 (%THE-CONS-LIST FACTS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ FACT (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:AND (CL:NOT (MEMBER? *VISITED-PROPOSITIONS* FACT))
       (CL:OR (CL:EQ *TABOO-OPERATORS* NULL)
        (CL:NOT (MEMBER? *TABOO-OPERATORS* (%OPERATOR FACT)))))
      (PUSH *VISITED-PROPOSITIONS* FACT)
      (CL:LET* ((CONS-PROP (PROPOSITION-TO-CONS FACT)) (CONS-PROPS NIL))
       (CL:COND
        ((CL:EQ (%KIND FACT) KWD-MACHINE-LEARNING-ISA)
         (CL:SETQ CONS-PROPS (CONS CONS-PROP CONS-PROPS)))
        ((CL:< DEPTH *MAX-STRUCTURE-DEPTH*)
         (CL:COND
          ((CL:EQ (%KIND FACT) KWD-MACHINE-LEARNING-FUNCTION)
           (CL:SETQ CONS-PROPS (CONS CONS-PROP CONS-PROPS))
           (CL:WHEN
            (ISA? (LAST CONS-PROP) SGT-MACHINE-LEARNING-STELLA-NUMBER-WRAPPER)
            (CL:LET*
             ((SKOLEM-NAME
        ...
        (CL:LET* ((PROP NULL) (ITER-001 CONS-PROPS))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:PROGN (CL:SETQ PROP (%%VALUE ITER-001))
           (CL:SETQ ITER-001 (%%REST ITER-001)))
          (PUSH ANTECEDENT PROP)))
        (CL:LET* ((ELE NULL) (ITER-002 (%%REST CONS-PROP)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
          (CL:PROGN (CL:SETQ ELE (%%VALUE ITER-002))
           (CL:SETQ ITER-002 (%%REST ITER-002)))
          (CL:WHEN
           ...
           (CL:IF (CL:EQ CHILDREN NULL)
            ...
            (CL:SETQ CHILDREN
             (CONCATENATE (HELP-GET-ASSERTION-STRUCTURE ELE (CL:1+ DEPTH))
              CHILDREN)))))))))))
...
(CL:DEFUN OLD-RECURSIVE-BUILD-ANTECEDENT (INST DEPTH)
  ...
  (PUSH *VISITED-INSTANCES* INST)
  (CL:LET*
   ((HOME (%HOME-CONTEXT INST)) (FACTS (NEW-LIST)) (ANTECEDENT NIL)
    (CHILDREN NIL))
   (CL:LET* ((*MODULE* HOME) (*CONTEXT* *MODULE*))
    (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:SETQ FACTS (ALL-FACTS-OF-INSTANCE (GET-INSTANCE INST) FALSE FALSE)))
      ...
      (CL:LET* ((CONS-PROP (PROPOSITION-TO-CONS FACT)))
       ...
       (CL:WHEN (CL:NOT (CL:EQ CONS-PROP NIL))
        ...
        (CL:LET* ((ELE NULL) (ITER-001 (%%REST (%%VALUE (%%REST CONS-PROP)))))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:PROGN (CL:SETQ ELE (%%VALUE ITER-001))
           (CL:SETQ ITER-001 (%%REST ITER-001)))
          (CL:WHEN
           (CL:AND (ISA? ELE SGT-MACHINE-LEARNING-STELLA-SYMBOL)
            (CL:NOT (MEMBER? *VISITED-INSTANCES* ELE))
            (CL:< DEPTH *MAX-STRUCTURE-DEPTH*))
           (CL:SETQ CHILDREN
            (CONCATENATE (OLD-RECURSIVE-BUILD-ANTECEDENT ELE (CL:1+ DEPTH))
             CHILDREN))))))))))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE BIND)))
     (CL:COND
      ...
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-SKOLEM)
       (CL:PROGN
        (CL:COND
         ((CL:NOT (CL:EQ (%DEFINING-PROPOSITION BIND) NULL))
          (PROPOSITION-TO-CONS (%DEFINING-PROPOSITION BIND)))
      ...
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-LOGIC-OBJECT)
       (CL:PROGN
        (INTERN-SYMBOL (%SYMBOL-NAME (%SURROGATE-VALUE-INVERSE BIND)))))
      (CL:T BIND))))))
...
(CL:DEFUN CONSIFY-ARGUMENT (ARG)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ARG)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-PROPOSITION)
    ...
    ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000)
     (CL:PROGN (INTERN-SYMBOL (%SYMBOL-NAME ARG))))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-MACHINE-LEARNING-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:IF
       (CL:NOT
        (CL:EQ
         (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) FALSE)
         NULL))
       (CONSIFY-ARGUMENT
        (ACCESS-IN-CONTEXT (%VARIABLE-VALUE ARG) (%HOME-CONTEXT ARG) FALSE))
       (CL:IF (CL:NOT (CL:EQ (%DEFINING-PROPOSITION ARG) NULL))
        (CL:LET*
         ((PROP (%DEFINING-PROPOSITION ARG)) (SK-ARGS (%ARGUMENTS PROP))
          (CONS-ARGS NIL))
         (CL:LET*
          ((I NULL-INTEGER) (ITER-000 0)
           (UPPER-BOUND-000 (CL:- (LENGTH SK-ARGS) 2)) (COLLECT-000 NULL))
          (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 I))
          (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
           (CL:PROGN (CL:SETQ I ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
           (CL:IF (CL:EQ COLLECT-000 NULL)
            (CL:PROGN
             (CL:SETQ COLLECT-000
              (CONS
               (CONSIFY-ARGUMENT
                (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SK-ARGS)) I))
               NIL))
   ...
   (CL:LET* ((TEST-VALUE-000 (%KIND PROP)))
    (CL:COND
     ...
     ((CL:OR (CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-PREDICATE)
       (CL:EQ TEST-VALUE-000 KWD-MACHINE-LEARNING-FUNCTION))
      (CL:SETQ RESULT
       (CONS (SURROGATE-TO-SYMBOL (%OPERATOR PROP)) (REVERSE RESULT))))
      ...
      (CL:IF (CL:NOT (CL:EQ (%%REST RESULT) NIL))
       (CL:SETQ RESULT
        (CONS-LIST SYM-MACHINE-LEARNING-STELLA-OR
         (CONS-LIST SYM-MACHINE-LEARNING-STELLA-NOT (%%VALUE (%%REST RESULT)))
         (%%VALUE RESULT)))
          ...
          (CL:SETQ BINDINGS
           (CONS
            (CONS-LIST (INTERN-SYMBOL (GET-SKOLEM-PRINT-NAME IO))
             (INTERN-SYMBOL (%SYMBOL-NAME (LOGICAL-TYPE IO))))
            BINDINGS)))))
       (CL:WHEN (CL:NOT (CL:EQ BINDINGS NIL))
        (CL:SETQ RESULT
         (CONS-LIST (INTERN-SYMBOL (%SYMBOL-NAME (%KIND PROP))) BINDINGS
          RESULT)))))
     (CL:T
      (CL:SETQ RESULT
       (CONS (INTERN-SYMBOL (%SYMBOL-NAME (%OPERATOR PROP)))
        (REVERSE RESULT))))))
   (CL:IF (FALSE? PROP) (LIST* SYM-MACHINE-LEARNING-STELLA-NOT RESULT NIL)
    RESULT)))

;;; (DEFUN THINGIFY-UNTYPED-INSTANCES ...)

(CL:DEFUN THINGIFY-UNTYPED-INSTANCES ()
  (CL:LET* ((ITEM NULL) (ITER-000 (ALL-NAMED-INSTANCES *MODULE* TRUE)))
...
(CL:DEFUN %PRINT-FACTS (INSTANCEREF)
  (CL:LET* ((INSTANCE (COERCE-TO-INSTANCE INSTANCEREF NULL)))
   (CL:LET*
    ((FACT NULL)
     (ITER-000 (%THE-CONS-LIST (ALL-FACTS-OF-INSTANCE INSTANCE FALSE FALSE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ FACT (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) FACT EOL)))))
...
;;; (DEFUN TRANSLATE-LEARNING ...)

(CL:DEFUN TRANSLATE-LEARNING ()
  (SET-OPTIMIZATION-LEVELS 1 0 3 3)
  (CL-TRANSLATE-FILE "machine-learning.ste" TRUE)
  (CPP-TRANSLATE-FILE "machine-learning.ste")
  (CL-TRANSLATE-FILE "rule-induction.ste" TRUE)
  (CPP-TRANSLATE-FILE "rule-induction.ste")
  (CL-TRANSLATE-FILE "neural-network.ste" TRUE)
  (CPP-TRANSLATE-FILE "neural-network.ste")
  (CL-TRANSLATE-FILE "case-based.ste" TRUE)
  (CPP-TRANSLATE-FILE "case-based.ste"))
...
(CL:DEFUN HELP-STARTUP-MACHINE-LEARNING1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-MACHINE-LEARNING-STELLA-NAME
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NAME" (GET-STELLA-MODULE "/STELLA" TRUE)
     0))
   ...
   (CL:SETQ SYM-MACHINE-LEARNING-LOGIC-SET-CLOSED-WORLD-TRAINING-EXAMPLES
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-CLOSED-WORLD-TRAINING-EXAMPLES" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-MACHINE-LEARNING2 ()
  ...
  testing examples\")" (CL:FUNCTION %ADD-TESTING-EXAMPLE)
   ...
   (DEFINE-FUNCTION-OBJECT "SHUFFLE-LIST"
    "(DEFUN SHUFFLE-LIST ((CARDS LIST)))" (CL:FUNCTION SHUFFLE-LIST) NULL)
   (DEFINE-FUNCTION-OBJECT "SHUFFLE-VECTOR"
    "(DEFUN SHUFFLE-VECTOR ((CARDS VECTOR)))" (CL:FUNCTION SHUFFLE-VECTOR)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "TRANSLATE-LEARNING"
    "(DEFUN TRANSLATE-LEARNING () :COMMAND? TRUE :PUBLIC? TRUE)"
    (CL:FUNCTION TRANSLATE-LEARNING) NULL)
   ...
   (CL:LET*
    ((FUNCTION
      (LOOKUP-FUNCTION SYM-MACHINE-LEARNING-LOGIC-STARTUP-MACHINE-LEARNING)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     SYM-MACHINE-LEARNING-STELLA-METHOD-STARTUP-CLASSNAME
     (WRAP-STRING "_Startup-Machine-Learning") NULL-STRING-WRAPPER))))
...
(CL:DEFUN STARTUP-MACHINE-LEARNING ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *CLOSED-WORLD-TRAINING-EXAMPLES* TRUE)
    (CL:SETQ *DROP-FUNCTION-VALUES?* TRUE)
  ...
  that should be returned and the module it should be evaluated in.\" :SLOTS ((QUERY :TYPE CONS) (CONCEPT :TYPE SYMBOL) (NAME :TYPE SYMBOL) (SCORE :TYPE PARTIAL-MATCH-SCORE) (MODULE :TYPE MODULE) (INPUT-FEATURES :TYPE LIST) (INPUT :TYPE VECTOR) (FACTS :TYPE (CONS OF CONS)) (NN-HIDDEN :TYPE (VECTOR OF FLOAT-WRAPPER)) (OUTPUT :TYPE OBJECT) (TEMP :TYPE FLOAT) (CACHED-SOLUTION :TYPE OBJECT) (TIMESTAMP :TYPE TIMESTAMP)))")))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (REGISTER-NATIVE-NAME SYM-MACHINE-LEARNING-LOGIC-ADD-TRAINING-EXAMPLE
     KWD-MACHINE-LEARNING-COMMON-LISP KWD-MACHINE-LEARNING-FUNCTION)
    (REGISTER-NATIVE-NAME
     SYM-MACHINE-LEARNING-LOGIC-ADD-TRAINING-EXAMPLE-IN-MODULE
     KWD-MACHINE-LEARNING-COMMON-LISP KWD-MACHINE-LEARNING-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-MACHINE-LEARNING-LOGIC-ADD-TESTING-EXAMPLE
     KWD-MACHINE-LEARNING-COMMON-LISP KWD-MACHINE-LEARNING-FUNCTION)
    ...
    (REGISTER-NATIVE-NAME SYM-MACHINE-LEARNING-LOGIC-SET-STRUCTURE-DEPTH
     KWD-MACHINE-LEARNING-COMMON-LISP KWD-MACHINE-LEARNING-FUNCTION)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *TABOO-OPERATORS* (LIST OF SURROGATE) (NEW (LIST OF SURROGATE)))")
    (REGISTER-NATIVE-NAME SYM-MACHINE-LEARNING-LOGIC-ADD-TABOO-OPERATORS
     KWD-MACHINE-LEARNING-COMMON-LISP KWD-MACHINE-LEARNING-FUNCTION)
