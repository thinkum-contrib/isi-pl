...
(CL:DECLAIM
  ...
  *PRINTREADABLY?* EOL *CURRENTJUSTIFICATION* *PRINTMODE*
  STANDARD-OUTPUT *LOGIC-DIALECT* NULL-INTEGER *MODULE*))
...
(CL:DEFUN STRINGIFIED-SURROGATE (OPERATOR)
  (CL:LET*
   ((DESCRIPTION (GET-DESCRIPTION OPERATOR))
    ...
    (VISIBLE?
     (CL:EQ OPERATOR
      (LOOKUP-SURROGATE-IN-MODULE (%SYMBOL-NAME OPERATOR) *MODULE*
       CL:NIL))))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING STRING))
     ...
     (CL:COND
      ...
      (CL:T
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        ...
        (CL:ERROR
         (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))))
   (CL:IF VISIBLE? STRING
    (COMPUTE-FULL-NAME STRING (%HOME-CONTEXT OPERATOR)))))
...
(CL:DEFUN PRINT-KIF-FUNCTION-PROPOSITION (SELF)
  (CL:LET*
   ...
   (INCREASE-INDENT
    (CL:+ (CL:THE CL:FIXNUM (CL:LENGTH DOWNCASEDNAME)) 2))
...
(CL:DEFUN PRINT-KIF-FUNCTION-EXPRESSION (SELF)
  (CL:LET* ((DOWNCASEDNAME (STRINGIFIED-KIF-OPERATOR SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING DOWNCASEDNAME))
   (PRINT-KIF-OPERATOR-WITH-ARGUMENTS DOWNCASEDNAME (%ARGUMENTS SELF)
    CL:NIL CL:T)))
    ...
    (CL:LOOP WHILE
     ...
     (CL:SETQ NEWVAR
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001))
       INDEX-001))
     (CL:WHEN
      (CL:NOT (CL:EQ (%SKOLEM-NAME OLDVAR) (%SKOLEM-NAME NEWVAR)))
   ...
   ((STREAM *PRINTLOGICALFORMSTREAM*)
    ...
    (OPERATORPREFIXINDENT
     (CL:+ 2
      (CL:THE CL:FIXNUM
       (CL:LENGTH
        (CL:THE CL:SIMPLE-STRING (%SYMBOL-NAME OPERATORPREFIX))))))
...
(CL:DEFUN EXCLUDE-ORIGINATED-PROPOSITIONS ()
  (CL:LET* ((D NULL) (ITER-000 (ALL-NAMED-DESCRIPTIONS NULL CL:NIL)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ D (%VALUE ITER-000))
    (CL:LET*
     ((PROP NULL)
      (ITER-001 (%THE-CONS-LIST (ORIGINATED-PROPOSITIONS D))))
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE (NEXT? ITER-002) DO (CL:SETQ P (%VALUE ITER-002))
    (CL:WHEN (CL:NOT (CL:EQ (OBJECT-SURROGATE P) NULL))
     (CL:LET*
      ((PROP NULL)
       (ITER-003 (%THE-CONS-LIST (ORIGINATED-PROPOSITIONS P))))
           ...
           (CL:PROGN
            (CL:SETQ KEYLENGTH
             (CL:1+
              (CL:THE CL:FIXNUM
               (CL:LENGTH
                (CL:THE CL:SIMPLE-STRING (%SYMBOL-NAME KEY))))))
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) ":"
             (%SYMBOL-NAME KEY))))
...
(CL:DEFUN PRETTY-PRINT-NAMED-RULE (RULENAME STREAM)
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM *INDENTCOUNTER*))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "(" OPERATOR " "
     (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME RULENAME)
      (%HOME-CONTEXT RULENAME) CL:T)
     EOL)
    (PRINT-INDENT STREAM NULL-INTEGER)
    (PRETTY-PRINT-LOGICAL-FORM PROPOSITION STREAM)
    (CL:LET*
     ((PROP NULL)
      (ITER-000
       (%THE-CONS-LIST (ORIGINATED-PROPOSITIONS PROPOSITION))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN PRINT-MODULE-FILE-HEADER (MODULE STREAM)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "(IN-MODULE \""
   (%MODULE-FULL-NAME MODULE) "\")" EOL EOL "("
