...
(CL:DEFUN PRINT-KIF-IMPLIES-PROPOSITION (SELF)
  (CL:LET*
   ...
   (CL:IF
    ...
    (PRINT-KIF-OPERATOR-WITH-ARGUMENTS "subset-of" (%ARGUMENTS SELF)
     TRUE FALSE))))
  ...
  (CL:LET* ((MAPPING NULL))
   (CL:LET*
    ((NEWVAR NULL) (OLDVAR NULL) (VECTOR-000 OLDVARS) (INDEX-000 0)
     (LENGTH-000 (LENGTH VECTOR-000)) (VECTOR-001 NEWVARS)
  ...
  (CL:LET*
   ...
   (CL:LET* ((*INDENTCOUNTER* CURRENTINDENTCOUNTER))
    ...
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "("
     (STRINGIFIED-SURROGATE SGT-KIF-OUT-PL-KERNEL-KB-FORALL) " ")
    (PRINT-KIF-QUANTIFIED-VARIABLES
     (CL:IF MAPHEADVARIABLES? (%IO-VARIABLES TAIL)
      (%IO-VARIABLES HEAD))
     FALSE)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) EOL)
    (INCREASE-INDENT NULL-INTEGER) (PRINT-INDENT STREAM NULL-INTEGER)
...
either be an output stream or a persistent OBJECT-STORE."
  (CL:LET* ((*PRINTREADABLY?* TRUE))
   ...
   (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
    ...
    (CL:LET*
     ((DESCRIPTION NULL)
      (ITER-000 (ALL-NAMED-DESCRIPTIONS MODULE TRUE)))
...
(CL:DEFUN STARTUP-KIF-OUT ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *PRETTYPRINTKIF?* FALSE)
    (CL:SETQ *DOWNCASEOPERATORS?* FALSE)
