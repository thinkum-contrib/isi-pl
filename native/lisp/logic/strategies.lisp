...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* INCONSISTENT-TRUTH-VALUE
  TRUE-WRAPPER *CYC-KLUDGES-ENABLED?* *DUPLICATE-RULE-SEARCH-DEPTH*
  NULL-INTEGER EMPTY-PROPOSITIONS-ITERATOR *REVERSEPOLARITY?*
  NULL-FLOAT-WRAPPER *DONTUSEDEFAULTKNOWLEDGE?*
  ...
  *GENERATE-ALL-PROOFS?* *TRACED-KEYWORDS* DEFAULT-TRUE-TRUTH-VALUE
  DEFAULT-FALSE-TRUTH-VALUE STANDARD-OUTPUT EOL TRUE-TRUTH-VALUE
        ...
        (CL:WHEN (TRACE-KEYWORD? KWD-STRATEGIES-GOAL-TREE)
         (CL:LET*
          ...
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
           "CLSH: truth=" (TRUTH-VALUE-TO-STRING TRUTHVALUE CL:T)
           EOL)))
   ...
   (CL:LET* ((TEST-VALUE-000 (%CURRENT-STRATEGY FRAME)))
    (CL:COND
     ...
     ((CL:EQ TEST-VALUE-000
      ...
      (CL:SETQ RESULT
       (RESUME-DISJUNCTIVE-IMPLICATION-PROOF FRAME LASTMOVE)))
...
(CL:DEFUN SPECIALIST-APPLICABLE? (DESCRIPTION PROPOSITION)
  (CL:LET*
   ((D-ARITY (ARITY DESCRIPTION))
    (P-ARITY (LENGTH (%ARGUMENTS PROPOSITION))))
...
(CL:DEFUN CONTINUE-SPECIALIST-PROOF (FRAME LASTMOVE)
  (CL:LET*
   ...
   (CL:WHEN
    ...
    (CL:IF
     (CL:EQ *POWERLOOM-EXECUTION-MODE* KWD-STRATEGIES-DEVELOPMENT)
     (CL:SETQ RESULT (CL:FUNCALL SPECIALISTCODE FRAME LASTMOVE))
     (CL:HANDLER-CASE
      (CL:SETQ RESULT (CL:FUNCALL SPECIALISTCODE FRAME LASTMOVE))
      (CL:CONDITION (NE)
       (CL:WARN "Exception executing specialist for `~A': ~%`~A'"
        DESCRIPTION NE))))
    (CL:WHEN (CL:EQ (%TRUTH-VALUE FRAME) NULL)
     (CL:COND
      ((CL:OR (CL:EQ RESULT KWD-STRATEGIES-FINAL-SUCCESS)
...
(CL:DEFUN TRY-GOAL-CACHES-PROOF (FRAME)
  ...
  (CL:WHEN (ALL-KEY-ARGUMENTS-BOUND? (%PROPOSITION FRAME))
   (CL:IF (CL:EQ (SCAN-CACHED-GOALS FRAME) KWD-STRATEGIES-FAILURE)
    (CL:RETURN-FROM TRY-GOAL-CACHES-PROOF KWD-STRATEGIES-FAILURE)
    (CL:RETURN-FROM TRY-GOAL-CACHES-PROOF
     KWD-STRATEGIES-FINAL-SUCCESS)))
  KWD-STRATEGIES-FAILURE)
     ...
     (CL:WHEN *RECORD-JUSTIFICATIONS?*
      (CL:LET*
       ((FJ NULL)
        (ITER-000
         (%THE-CONS-LIST
          (FORWARD-JUSTIFICATIONS MATCHING-PROPOSITION))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        (CL:SETQ FJ (%%VALUE ITER-000))
        (PUSH (%JUSTIFICATIONS FRAME) FJ) (CL:RETURN)
        (CL:SETQ ITER-000 (%%REST ITER-000))))
      (CL:LET*
       ((SUBSET-JUSTIFICATION
         (CREATE-SUBSET-JUSTIFICATION PROPOSITION
...
(CL:DEFUN CREATE-ANONYMOUS-INSTANCE (PREFIX SKOLEM?)
  ...
  (CL:LET*
   ...
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS INSTANCE)
    SYM-STRATEGIES-LOGIC-HYPOTHESIZED-INSTANCE? TRUE-WRAPPER
    FALSE-WRAPPER)
   INSTANCE))
...
(CL:DEFUN CONTINUE-FAIL-PROOF (FRAME LASTMOVE)
  (CL:COND
   ...
   ((CL:EQ LASTMOVE KWD-STRATEGIES-UP-TRUE)
    (CL:LET*
     ((RESULT
       (CL:IF (%REVERSE-POLARITY? FRAME) KWD-STRATEGIES-FINAL-SUCCESS
        KWD-STRATEGIES-FAILURE)))
     (CL:COND
      ((%REVERSE-POLARITY? FRAME)
       (SET-FRAME-TRUTH-VALUE FRAME TRUE-TRUTH-VALUE)
       (CL:WHEN *RECORD-JUSTIFICATIONS?*
        (RECORD-NEGATED-FAIL-JUSTIFICATION FRAME LASTMOVE)))
   ...
   ((CL:EQ LASTMOVE KWD-STRATEGIES-UP-FAIL)
    (CL:LET*
     ((RESULT
       (CL:IF (%REVERSE-POLARITY? FRAME) KWD-STRATEGIES-FAILURE
        KWD-STRATEGIES-FINAL-SUCCESS)))
     (CL:COND
      ((CL:NOT
        (CL:EQ
      ...
      ((CL:EQ (%TRUTH-VALUE (%RESULT FRAME)) INCONSISTENT-TRUTH-VALUE)
       (SET-FRAME-TRUTH-VALUE FRAME INCONSISTENT-TRUTH-VALUE)
       (CL:SETQ RESULT KWD-STRATEGIES-TERMINAL-FAILURE))
      ((%REVERSE-POLARITY? FRAME)
       (SET-FRAME-TRUTH-VALUE FRAME FALSE-TRUTH-VALUE))
      (CL:T (SET-FRAME-TRUTH-VALUE FRAME TRUE-TRUTH-VALUE)
