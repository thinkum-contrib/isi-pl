...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* UNKNOWN-TRUTH-VALUE
  TRUE-WRAPPER *CYC-KLUDGES-ENABLED?* *CONTEXT*
  *DUPLICATE-RULE-SEARCH-DEPTH* NULL-INTEGER
  ...
  *DUPLICATE-SUBGOAL-STRATEGY* *GENERATE-ALL-PROOFS?* *TRACED-KEYWORDS*
  TRUE NIL DEFAULT-TRUE-TRUTH-VALUE DEFAULT-FALSE-TRUTH-VALUE
  STANDARD-OUTPUT TRUE-TRUTH-VALUE EOL *QUERYITERATOR* NULL-FLOAT FALSE
      ...
      ((CL:OR (CL:EQ RESULT KWD-STRATEGIES-CONTINUING-SUCCESS)
       ...
       (CL:WHEN
        ...
        (SET-FRAME-PARTIAL-TRUTH (%PARTIAL-MATCH-FRAME FRAME)
         TRUE-TRUTH-VALUE NULL-FLOAT NULL-FLOAT FALSE))
       (CL:WHEN
    ...
    (CL:LET* ((ALWAYS?-000 TRUE))
     (CL:LET*
      ...
      (CL:LOOP WHILE
       ...
       (CL:WHEN
        (CL:NOT (BIND-ARGUMENT-TO-VALUE? ARG CACHEDBINDING TRUE))
  ...
  (CL:LET*
   ((ITERATOR
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
      SYM-STRATEGIES-STELLA-ITERATOR NULL))
    (PROPOSITION (%PROPOSITION FRAME))
    (TRUTHVALUE (%TRUTH-VALUE FRAME)) (HASDISPROOF? FALSE))
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
    SYM-STRATEGIES-STELLA-ITERATOR NULL NULL)
    ...
    (CL:IF REVERSEPOLARITY?
     (CL:LET* ((FOUND?-000 FALSE))
      (CL:LET*
       ...
       (CL:LOOP WHILE (NEXT? P) DO (CL:SETQ FOUND?-000 TRUE)
        (CL:RETURN)))
...
(CL:DEFUN TRY-GOAL-COMPLEMENT-PROOF (FRAME)
  (CL:LET*
   ...
   (CL:SETF (%REVERSE-POLARITY? DOWNFRAME) FALSE)
   KWD-STRATEGIES-MOVE-DOWN))
    ...
    ((ISA? MEMBER SGT-STRATEGIES-STELLA-LITERAL-WRAPPER)
     (CL:LET*
      ((SURROGATE (%OPERATOR PROPOSITION))
       (SUCCESS?
        (LOGICAL-SUBTYPE-OF? (LOGICAL-TYPE MEMBER) SURROGATE)))
...
(CL:DEFUN INFERABLE-DESCRIPTION? (SELF)
  (CL:WHEN (CL:EQ SELF NULL)
   (CL:RETURN-FROM INFERABLE-DESCRIPTION? FALSE))
  (CL:COND
...
(CL:DEFUN TRY-SIMPLE-CONTAINED-BY-PROOF (SUBCOLLECTION SUPERCOLLECTION REVERSEPOLARITY?)
  (CL:LET* ((*REVERSEPOLARITY?* FALSE))
   (CL:DECLARE (CL:SPECIAL *REVERSEPOLARITY?*))
   (CL:LET*
    ((MEMBERS (ASSERTED-COLLECTION-MEMBERS SUBCOLLECTION FALSE)))
        ...
        (CL:T
         ...
         (CL:LET*
          ((I NULL-INTEGER) (ARG NULL)
           (ITER-000 (%THE-CONS-LIST BRIDGEARGS)) (ITER-001 0))
