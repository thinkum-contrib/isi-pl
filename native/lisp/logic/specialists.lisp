...
(CL:DEFCLASS COMPUTED-PROCEDURE (THING)
  ((PROCEDURE-NAME :ALLOCATION :INSTANCE :ACCESSOR %PROCEDURE-NAME)
   (PROCEDURE-FUNCTION :ALLOCATION :INSTANCE :ACCESSOR
    %PROCEDURE-FUNCTION))
...
(CL:DEFUN NEW-COMPUTED-PROCEDURE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%PROCEDURE-FUNCTION SELF) NULL)
...
(CL:DEFUN ACCESS-COMPUTED-PROCEDURE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-SPECIALISTS-LOGIC-PROCEDURE-FUNCTION)
    (CL:IF SETVALUE? (CL:SETF (%PROCEDURE-FUNCTION SELF) VALUE)
     (CL:SETQ VALUE (%PROCEDURE-FUNCTION SELF))))
   (CL:T
    (CL:IF SETVALUE?
...
(CL:DEFUN LOOKUP-CONSTRAINT (DESCRIPTION)
  (CL:LET* ((FUNCTION (LOOKUP-CONSTRAINT-FUNCTION DESCRIPTION)))
   (CL:IF (CL:NOT (CL:EQ FUNCTION NULL)) (%FUNCTION-CODE FUNCTION)
    NULL)))
      ...
      ((SUBTYPE-OF-PARAMETRIC-TYPE-SPECIFIER?
       ...
       (CL:PROGN
        (TYPE-SPEC-TO-BASE-TYPE
         (FIRST (%SPECIFIER-PARAMETER-TYPES TARGETTYPE)))))
...
(CL:DEFUN COMPUTE-RELATION-VALUE (PROPOSITION COMPUTATION ERROR?)
  (CL:LET*
   ((ARGUMENTS (%ARGUMENTS PROPOSITION))
    ...
    (ARGTYPES (%IO-VARIABLE-TYPES DESCRIPTION))
   ...
   (CL:SETQ VALUE (APPLY (%FUNCTION-CODE COMPUTATION) BOUNDARGUMENTS))
   VALUE))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ VALUE NULL)
    (CL:RETURN-FROM |/PL-KERNEL-KB/COMPUTATION-SPECIALIST|
     KWD-SPECIALISTS-FAILURE))
   (CL:IF
    (FUNCTION-DESCRIPTION? (GET-DESCRIPTION (%OPERATOR PROPOSITION)))
    (CL:SETQ SUCCESS?
     (CL:IF
...
(CL:DEFUN COMPUTE-SIMPLE-RELATION-CONSTRAINT (PROPOSITION COMPUTATION ERROR?)
  (CL:LET*
   ...
   (CL:CASE NULLCOUNT
    (0
     (CL:SETQ VALUE
      (APPLY (%FUNCTION-CODE COMPUTATION)
...
(CL:DEFUN |/PL-KERNEL-KB/CONSTRAINT-SPECIALIST| (FRAME LASTMOVE)
  ...
  (CL:LET* ((VALUE NULL) (VARIABLEINDEX NULL-INTEGER))
   ...
   (CL:MULTIPLE-VALUE-SETQ (VALUE VARIABLEINDEX)
    (COMPUTE-SIMPLE-RELATION-CONSTRAINT (%PROPOSITION FRAME) NULL
     CL:NIL))
   (CL:WHEN (CL:EQ VALUE NULL)
...
(CL:DEFUN |/PL-KERNEL-KB/TOTAL-VALUE-SPECIALIST| (FRAME LASTMOVE)
  (CL:LET*
   ((VALUEPROPOSITION (%PROPOSITION FRAME))
    (VALUEARGUMENTS (%ARGUMENTS VALUEPROPOSITION))
    (DESCRIPTION
     (ARGUMENT-BOUND-TO
      (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VALUEARGUMENTS))
       0))))
   (CL:COND
    ((CL:NOT (CL:EQ DESCRIPTION NULL))
     (CL:COND
         ...
         (CL:LET* ((VALUE-000 NIL))
             ...
             (CL:PROGN
              (CL:SETQ COLLECT-000
               (CONS
                (ARGUMENT-BOUND-TO
                 (CL:AREF
                  (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VALUEARGUMENTS))
                  I))
                NIL))
          ...
          (CL:LET*
           ...
           (BIND-VARIABLE-TO-VALUE?
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR
              (%THE-ARRAY (%ARGUMENTS VALUEPROPOSITION)))
             (CL:THE CL:FIXNUM
              (CL:1- (LENGTH (%ARGUMENTS VALUEPROPOSITION)))))
            (CL:AREF
             (CL:THE CL:SIMPLE-VECTOR
              (%THE-ARRAY (%ARGUMENTS FUNCTIONPROP)))
             (CL:THE CL:FIXNUM
              (CL:1- (LENGTH (%ARGUMENTS FUNCTIONPROP)))))
            CL:T)
           KWD-SPECIALISTS-FINAL-SUCCESS)))
...
(CL:DEFUN |/PL-KERNEL-KB/PROPOSITION-RELATION-COMPUTATION| (P)
  (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? P)
   (CL:SETQ P (%DEFINING-PROPOSITION P)))
  (CL:COND
   ...
   (CL:COND
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SPECIALISTS-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? P)
       (CL:LET*
        ((PROP (%DEFINING-PROPOSITION P))
         (ARGUMENTS (%ARGUMENTS PROP)))
        (CL:WHEN
         (CL:AND (CL:>= (%WRAPPER-VALUE I) 0)
          (CL:< (%WRAPPER-VALUE I) (CL:1- (LENGTH ARGUMENTS))))
         (CL:RETURN-FROM
          |/PL-KERNEL-KB/PROPOSITION-ARGUMENT-COMPUTATION|
          (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS))
           (CL:THE CL:FIXNUM (%WRAPPER-VALUE I)))))))))
     ...
     (CL:PROGN
      (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? P)
       (CL:RETURN-FROM
        ...
        (CREATE-LOGICAL-LIST
         (LISTIFY
          (BUT-LAST (%ARGUMENTS (%DEFINING-PROPOSITION P)))))))))
    (CL:T)))
  NULL)
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-SPECIALISTS-LOGIC-SKOLEM)
     (CL:PROGN
      (CL:WHEN (FUNCTION-OUTPUT-SKOLEM? P)
       (CL:RETURN-FROM |/PL-KERNEL-KB/PROPOSITION-ARITY-COMPUTATION|
        (WRAP-INTEGER
         (CL:1- (LENGTH (%ARGUMENTS (%DEFINING-PROPOSITION P)))))))))
    (CL:T)))
  NULL)
    ...
    ((CL:EQ LASTMOVE KWD-SPECIALISTS-UP-FAIL)
     (CL:WHEN (CL:> (%ARGUMENT-CURSOR FRAME) 0)
      (CL:RETURN-FROM |/PL-KERNEL-KB/FORK-SPECIALIST|
       KWD-SPECIALISTS-FAILURE))
     (CL:WHEN RECORDJUSTIFICATIONS?
      (RECORD-FAIL-JUSTIFICATION (%RESULT FRAME)
...
(CL:DEFUN LEVELED-QUERY-RELATION-TO-INFERENCE-LEVEL (RELATION)
  (CL:COND
   ...
   (CL:T
    (GET-INFERENCE-LEVEL
     (KEYWORDIFY
      (%%VALUE (SPLIT-STRING (%SYMBOL-NAME RELATION) #\-)))))))
   ...
   (CL:COND
     ...
     (CL:WHEN (CL:EQ ADJUNCT NULL)
      (CL:LET*
       ...
       (CL:LET* ((SELF-000 (NEW-SAVED-INFERENCE-LEVEL-PROOF-ADJUNCT)))
        (CL:SETF (%SAVED-INFERENCE-LEVEL SELF-000)
       ...
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME)
        SYM-SPECIALISTS-LOGIC-PROOF-ADJUNCT ADJUNCT NULL)))
     (CL:SETF (%DOWN FRAME) (%DOWN-FRAME ADJUNCT))
     (CALL-SET-INFERENCE-LEVEL (%KEYWORD (%INFERENCE-LEVEL ADJUNCT))
      NULL)
    ...
    ((CL:EQ LASTMOVE KWD-SPECIALISTS-UP-TRUE)
     ...
     (CALL-SET-INFERENCE-LEVEL
      (%KEYWORD (%SAVED-INFERENCE-LEVEL ADJUNCT)) NULL)
   ...
   (CL:COND
    ...
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-SPECIALISTS-PREDICATE)
     ...
     (CL:COND
      ...
      (CL:T
       (HELP-CLOSED-TERM? (GET-DESCRIPTION (%OPERATOR SELF))
        ACTIVETERMS))))
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000
        (CONS
         (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ELEMENTS)) I)
         NIL))
