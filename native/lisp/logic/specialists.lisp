...
(CL:DECLAIM
  ...
  *RECORD-JUSTIFICATIONS?* EMPTY-PROPOSITIONS-ITERATOR REFUTATION-INFERENCE
  ...
  NULL-FUNCTION-CODE-WRAPPER NULL-METHOD-CODE-WRAPPER STANDARD-OUTPUT
  FALSE-TRUTH-VALUE *QUERYITERATOR* TRUE-TRUTH-VALUE *REVERSEPOLARITY?*
...
(CL:DEFUN FUNCTION-CODE-FROM-PROCEDURE (PROCEDURE)
  (CL:LET* ((STELLAFUNCTION (%PROCEDURE-FUNCTION PROCEDURE)))
   (CL:WHEN (CL:EQ STELLAFUNCTION NULL)
    (CL:SETQ STELLAFUNCTION (STELLA-FUNCTION-FROM-PROCEDURE PROCEDURE)))
...
(CL:DEFUN ARGUMENT-TARGET-TYPE (PARAMETERTYPES NINPUTTYPES ARGINDEX)
  ...
  (CL:LET*
   ((TARGETTYPE (NTH PARAMETERTYPES (MIN ARGINDEX (CL:1- NINPUTTYPES)))))
   (CL:COND
     ...
     (CL:COND
      ...
      ((CL:AND
        (CHECK-STRICT-TYPE? VALUE
         (ARGUMENT-TARGET-TYPE ARGTYPES NINPUTTYPES I) CL:T)
        (SUBTYPE-OF? (PRIMARY-TYPE VALUE)
         (ARGUMENT-TARGET-TYPE COMPARGTYPES (CL:1+ NINPUTTYPES) (CL:1+ I)))))
      ((CL:AND (SKOLEM? VALUE)
        (CL:NOT (BOUND-COMPUTATION-INPUT-SKOLEM? VALUE)))
  ...
  (CL:LET* ((PROPOSITION NULL) (SUBQUERYOPTIONS NULL))
   ...
   (CL:LET*
    ...
    (INITIALIZE-QUERY-ITERATOR SUBQUERYITERATOR SUBQUERYDESCRIPTION
     IOVARIABLEBINDINGS "QuerySpecialist")
