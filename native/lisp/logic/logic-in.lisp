...
(CL:DECLAIM
  ...
  *TRANSLATIONPHASE* *MODULE* STANDARD-INPUT *CONTEXT* FALSE NIL
  STANDARD-ERROR *ITERATIVE-DEEPENING-MODE?* TRUE
  ...
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ LEGALOPTIONS NULL))
     (CL:LET*
      ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST PARSEDOPTIONS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
   ...
   (CL:LET*
    ...
    (CL:LET*
     ((*PRINTMODE* KWD-LOGIC-IN-REALISTIC)
      (*PRETTYPRINTLOGICALFORMS?* TRUE) (*CONTEXT* PRINTCONTEXT))
     (CL:DECLARE
...
(CL:DEFUN LOGIC-COMMAND-LOOP ()
  (CL:LET*
   ((COMMAND NULL) (RESULT NULL) (EXIT? FALSE) (EXITCOMMAND? FALSE))
  ...
  (CL:WHEN (CL:NOT (PROBE-FILE? FILE))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "File `" FILE
     "' does not exist.")
    (CL:ERROR
     (NEW-NO-SUCH-FILE-EXCEPTION (THE-STRING-READER STREAM-000)))))
  (CL:LET*
...
(CL:DEFUN DEMO-FILE (FILE PAUSE?)
  ...
  (CL:LET*
   ((CURRENTLEVEL *DEMO-LEVEL*) (COMMAND NULL) (RESULT NULL)
    (EXIT? FALSE) (INPUTSTREAM (NEW-INPUT-FILE-STREAM FILE)))
...
be diffed with previously validated runs to find deviations."
  (CL:LET*
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
    ...
    "Start time:        "
    (CALENDAR-DATE-TO-STRING START-TIME (GET-LOCAL-TIME-ZONE) TRUE) EOL
    EOL)
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
    ...
    "=====================================" EOL "Finish time:   "
    (CALENDAR-DATE-TO-STRING FINISH-TIME (GET-LOCAL-TIME-ZONE) TRUE)
    EOL "Elapsed time:  "
...
(CL:DEFUN COMPLETE-RULE-DEFINITION (SELF BODY ARROW)
  (CL:LET*
   ((PARAMETERS (YIELD-RELATION-PARAMETERS-TREE SELF FALSE FALSE)))
        ...
        ((CL:EQ TEST-VALUE-000 SGT-LOGIC-IN-STELLA-CONS)
         (CL:PROGN
          (CL:SETQ PROPOSITIONS
           (CONCATENATE (REVERSE RESULT) PROPOSITIONS))))
        ((SUBTYPE-OF? TEST-VALUE-000 SGT-LOGIC-IN-LOGIC-PROPOSITION)
   ...
   (CL:LET*
    ...
    (CL:LET*
     ((I NULL-INTEGER) (VAR NULL) (TYPE NULL)
      (ITER-002 (%THE-CONS-LIST IOVARIABLETYPES))
      (VECTOR-000 IOVARIABLES) (INDEX-000 0)
      (LENGTH-000 (LENGTH VECTOR-000)) (ITER-003 0) (COLLECT-000 NULL))
...
See `set-feature' for a description of supported features.\")"
   ...
   (DEFINE-FUNCTION-OBJECT "LOGIC-COMMAND-LOOP"
    "(DEFUN LOGIC-COMMAND-LOOP ())" (CL:FUNCTION LOGIC-COMMAND-LOOP)
    NULL)
