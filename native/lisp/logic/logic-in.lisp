...
(CL:DECLAIM
  ...
  *STELLA-VERSION-STRING* *POWERLOOM-VERSION-STRING* *SKOLEM-ID-COUNTER*
  FALSE-WRAPPER *CHARACTER-UPCASE-TABLE* NULL-CHARACTER *CHARACTER-TYPE-TABLE*
  NULL-INTEGER *LOGIC-DIALECT* *CURRENTFILE* TRUE-WRAPPER *TRANSLATIONUNITS*
  *POWERLOOM-LOCK* *TRANSLATIONPHASE* STANDARD-INPUT *CONTEXT* *MODULE* NIL
  STANDARD-ERROR *ITERATIVE-DEEPENING-MODE?* *RECORD-JUSTIFICATIONS?*
  *JUST-IN-TIME-FORWARD-INFERENCE?* *EMIT-THINKING-DOTS?* STANDARD-OUTPUT EOL))
...
(CL:DEFUN PRINT-FEATURES ()
   ...
   *CURRENT-POWERLOOM-FEATURES* EOL EOL "Available PowerLoom features:" EOL
...
See `set-feature' for a description of supported features."
  (CL:HANDLER-CASE
   (CL:LET* ((FEATURENAME NULL) (ITER-000 FEATURES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ FEATURENAME (%%VALUE ITER-000))
     (UNSET-POWERLOOM-FEATURE (KEYWORDIFY (COERCE-TO-STRING FEATURENAME)))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (LOGIC-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) (EXCEPTION-MESSAGE E))))
  *CURRENT-POWERLOOM-FEATURES*)
...
(CL:DEFUN RESET-FEATURES ()
  ...
  (CL:LET*
   ((F NULL) (ITER-000 (%THE-CONS-LIST (COPY *CURRENT-POWERLOOM-FEATURES*))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:SETQ F (%%VALUE ITER-000))
    (CL:WHEN (CL:NOT (MEMBER? *DEFAULT-POWERLOOM-FEATURES* F))
     (UNSET-POWERLOOM-FEATURE F))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:LET* ((F NULL) (ITER-001 (%THE-CONS-LIST *DEFAULT-POWERLOOM-FEATURES*)))
...
(CL:DEFUN PARSE-LOGIC-COMMAND-OPTIONS (OPTIONS |LEGALOPTIONS&TYPES| COERCIONERROR? ALLOWOTHERKEYS?)
  (PARSE-OPTIONS OPTIONS |LEGALOPTIONS&TYPES| COERCIONERROR? ALLOWOTHERKEYS?))
...
(CL:DEFUN PRINT-LOGIC-PROMPT ()
  (CL:WHEN *PROMPT-SHOW-MODULE?*
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (%MODULE-NAME *MODULE*)
    " "))
...
(CL:DEFUN PRINT-LOGIC-COMMAND-RESULT (RESULT)
  (CL:LET* ((TEMP-000 (LOOKUP-CONSTRAINT-PROPAGATION-WORLD *CONTEXT*)))
   (CL:LET*
    ((PRINTCONTEXT (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 *CONTEXT*)))
    (CL:LET*
     ((*PRINTMODE* KWD-LOGIC-IN-REALISTIC) (*PRETTYPRINTLOGICALFORMS?* CL:T)
...
(CL:DEFUN LOGIC-COMMAND-LOOP-EXIT? (COMMAND)
  (CL:LET* ((EXIT? CL:NIL))
      ...
      (CL:PROGN
       (CL:COND
        ((CL:OR (CL:EQ COMMAND KWD-LOGIC-IN-BYE)
          (CL:EQ COMMAND KWD-LOGIC-IN-EXIT) (CL:EQ COMMAND KWD-LOGIC-IN-HALT)
          (CL:EQ COMMAND KWD-LOGIC-IN-QUIT) (CL:EQ COMMAND KWD-LOGIC-IN-STOP))
         (CL:SETQ EXIT? CL:T))
        (CL:T))))
   ...
   (CL:WHEN (CL:AND EXIT? *CAREFUL-LOGIC-COMMAND-LOOP-EXIT*?)
    ...
    (CL:WHEN EXIT? (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL))
     ...
     (CL:TAGBODY
      (CL:HANDLER-CASE
       (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL)
        ...
        (CL:WHEN EXIT? (CL:RETURN)) (CL:WHEN EXITCOMMAND? (CL:GO :CONTINUE))
       ...
       (STELLA-EXCEPTION (E)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) ">> Internal Error: "
         (EXCEPTION-MESSAGE E))
...
(CL:DEFUN EVALUATE-LOGIC-COMMAND (COMMAND FINALIZE?)
  (CL:LET*
   ...
   (CL:LET* ((OPERATOR NULL) (OPERATORNAME STELLA::NULL-STRING) (RESULT NULL))
    ...
    (CL:WHEN (CL:AND (CL:NOT (TRANSLATION-ERRORS?)) FINALIZE?)
     (CL:SETQ *TRANSLATIONPHASE* KWD-LOGIC-IN-FINALIZE) (FINALIZE-OBJECTS))
    RESULT)))
...
then an error will be signaled."
  ...
  (CL:LET*
   ((TEMP-000 (FIND-FILE-IN-LOAD-PATH FILE *POWERLOOM-KB-FILE-EXTENSIONS*)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-000))
   (CL:SETQ FILE
    (CL:IF (CL:NOT (CL:EQ TEMP-000 STELLA::NULL-STRING)) TEMP-000 FILE)))
   ...
   ((THEOPTIONS
     (PARSE-LOGIC-COMMAND-OPTIONS OPTIONS
      (LIST* KWD-LOGIC-IN-CHECK-DUPLICATES? SGT-LOGIC-IN-STELLA-BOOLEAN
       KWD-LOGIC-IN-MODULE SGT-LOGIC-IN-STELLA-MODULE NIL)
      CL:T CL:NIL))
       ...
       (CL:NOT
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN
         (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-LOGIC-IN-CHECK-DUPLICATES?
          TRUE-WRAPPER)))))
     (CL:DECLARE
      (CL:SPECIAL *CURRENTFILE* *DONT-CHECK-FOR-DUPLICATE-PROPOSITIONS?*))
     (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *CURRENTFILE*))
     (CL:SETQ INPUTSTREAM (NEW-INPUT-FILE-STREAM FILE))
     (LOAD-STREAM-IN-MODULE INPUTSTREAM
      (LOOKUP THEOPTIONS KWD-LOGIC-IN-MODULE)))
...
(CL:DEFUN LOAD-STREAM (STREAM)
  ...
  (LOAD-STREAM-IN-MODULE STREAM NULL))

;;; (DEFUN LOAD-STREAM-IN-MODULE ...)

(CL:DEFUN LOAD-STREAM-IN-MODULE (STREAM DEFAULT-MODULE)
...
not have an `in-module' form, an error is signaled."
  (CL:LET*
   ((CURRENTDIALECT *LOGIC-DIALECT*) (SKIPCOMMAND? CL:NIL)
    (SEENINMODULE? (CL:NOT (CL:EQ DEFAULT-MODULE NULL))) (COMMANDS NIL)
    (LOAD-MODULE (CL:IF SEENINMODULE? DEFAULT-MODULE *MODULE*)))
   (CL:LET*
    ((*LOGIC-DIALECT* CURRENTDIALECT) (*TRANSLATIONERRORS* 0)
     (*TRANSLATIONWARNINGS* 0) (*TRANSLATIONNOTES* 0))
    (CL:DECLARE
     (CL:SPECIAL *LOGIC-DIALECT* *TRANSLATIONERRORS* *TRANSLATIONWARNINGS*
      *TRANSLATIONNOTES*))
    (CL:DECLARE
     (CL:TYPE CL:FIXNUM *TRANSLATIONERRORS* *TRANSLATIONWARNINGS*
      *TRANSLATIONNOTES*))
    (CL:LET* ((*MODULE* LOAD-MODULE) (*CONTEXT* *MODULE*))
     (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     ...
     (CL:LET* ((TREE NULL) (ITER-001 (REVERSE COMMANDS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:SETQ TREE (%%VALUE ITER-001)) (EVALUATE-LOGIC-COMMAND TREE CL:T)
       (CL:SETQ ITER-001 (%%REST ITER-001)))))
    ...
    (CL:LET* ((FOUND?-000 CL:NIL))
     (CL:LET* ((EXT NULL) (ITER-001 *POWERLOOM-KB-FILE-EXTENSIONS*))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       ...
       (CL:WHEN
        (ENDS-WITH? (%WRAPPER-VALUE FILE) (%WRAPPER-VALUE EXT) NULL-INTEGER)
...
starts with a `-' character."
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ UNPROCESSEDARGS NULL))
    ...
    (CL:LET*
     ((ARG NULL) (ITER-000 UNPROCESSEDARGS) (I NULL-INTEGER) (ITER-001 0))
      ...
      (CL:LET* ((FOUND?-000 CL:NIL))
       (CL:LET* ((EXT NULL) (ITER-003 *POWERLOOM-KB-FILE-EXTENSIONS*))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
         ...
         (CL:WHEN
          (ENDS-WITH? (%WRAPPER-VALUE ARG) (%WRAPPER-VALUE EXT) NULL-INTEGER)
          (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
         (CL:SETQ ITER-003 (%%REST ITER-003))))
       (CL:IF FOUND?-000
        (CL:PROGN
         (CL:LET*
          ((TEMP-000 (FIND-FILE-IN-LOAD-PATH (%WRAPPER-VALUE ARG) NULL)))
          (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-000))
          (CL:SETQ FILE
         ...
         (PL-LOG KWD-LOGIC-IN-MEDIUM (WRAP-STRING "Loading file ")
          (WRAP-STRING FILE))
         (%LOAD FILE NIL))
        (CL:WHEN (PROBE-FILE? (FILE-NAME-AS-DIRECTORY (%WRAPPER-VALUE ARG)))
...
(CL:DEFUN DEMO-SKIP-TO-COMMAND (STREAM)
  (CL:LET*
   ...
   (CL:LOOP (CL:SETF (%ECHO-STREAM STREAM) NULL)
    ...
    (CL:COND
     ((CL:EQ
       (CL:AREF *CHARACTER-TYPE-TABLE* (CL:THE CL:FIXNUM (CL:CHAR-CODE CH)))
     ...
     ((CL:EQL CH #\;) (CL:SETQ ECHO? CL:T)
      ...
      (CL:SETQ EOF? (CL:EQ (NATIVE-READ-LINE STREAM) STELLA::NULL-STRING))
      (CL:WHEN EOF? (CL:RETURN-FROM DEMO-SKIP-TO-COMMAND CL:T)))
...
(CL:DEFUN DEMO-PAUSE? (PAUSE?)
  (CL:LET* ((EXIT? CL:NIL) (INPUT STELLA::NULL-STRING))
   ...
   (CL:COND
    (PAUSE?
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "------ pause ------")
...
(CL:DEFUN DEMO-FILE (FILE PAUSE?)
  ...
  (CL:LET*
   ((CURRENTLEVEL *DEMO-LEVEL*) (COMMAND NULL) (RESULT NULL) (EXIT? CL:NIL))
     ...
     (CL:PROGN (CL:SETQ INPUTSTREAM (OPEN-INPUT-FILE FILE))
      (CL:LET*
       ...
       (CL:LET* ((*MODULE* *MODULE*) (*CONTEXT* *MODULE*))
        (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
        (CL:WHEN PAUSE?
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Now reading from `"
          FILE "'." EOL
         ...
         *LOGIC-COMMAND-RESULT-INDENT* "Finished demo `" FILE "'." EOL))))
     (CL:WHEN (CL:NOT (CL:EQ INPUTSTREAM NULL)) (FREE INPUTSTREAM))))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) DEMO-ENTRY-FILE))
(CL:DEFUN DEMO-ENTRY-FILE (DEMOENTRY)
  (CL:LET* ((DEMOBASEFILE (%WRAPPER-VALUE (%%VALUE DEMOENTRY))))
   ...
   (CL:LET*
    ((TEMP-000
      (FIND-FILE-IN-LOAD-PATH DEMOBASEFILE *POWERLOOM-KB-FILE-EXTENSIONS*)))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-000))
    (CL:LET*
     ((VALUE-000
       (CL:IF (CL:NOT (CL:EQ TEMP-000 STELLA::NULL-STRING)) TEMP-000
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) DEMO-ENTRY-DESCRIPTION))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM) CL:SIMPLE-STRING) SELECT-EXAMPLE-DEMO))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION () CL:SIMPLE-STRING) SELECT-EXAMPLE-DEMO-FROM-MENU))
(CL:DEFUN SELECT-EXAMPLE-DEMO-FROM-MENU ()
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "       Q.   Quit demoing"
   EOL)
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Your choice: ")
  (CL:LET*
   ((SELECTION (READ-S-EXPRESSION STANDARD-INPUT)) (FILE STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FILE))
   (CL:COND
    ((SUBTYPE-OF-INTEGER? (SAFE-PRIMARY-TYPE SELECTION))
     (CL:PROGN (CL:SETQ FILE (SELECT-EXAMPLE-DEMO (%WRAPPER-VALUE SELECTION)))
    ...
    (CL:T
     (CL:WHEN (STRING-EQUAL? (STRINGIFY SELECTION) "Q")
      (CL:RETURN-FROM SELECT-EXAMPLE-DEMO-FROM-MENU STELLA::NULL-STRING))))
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
    ">> ERROR: Illegal choice `" SELECTION "'; enter a number between 1 and "
    (LENGTH *DEMO-FILES*) "!" EOL EOL)
...
Typing `?' at the pause prompt prints a list of available commands."
  (CL:LET*
   ((THEFILEANDPAUSE FILEANDPAUSE) (FILESPEC (%%VALUE THEFILEANDPAUSE))
    (PAUSE? (CL:NOT (EQL? (%%VALUE (%%REST THEFILEANDPAUSE)) FALSE-WRAPPER)))
    (FILE STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FILE))
...
(CL:DEFUN TEST-LOGIC-FILE (FILE)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LET* ((*MODULE* *MODULE*) (*CONTEXT* *MODULE*))
     (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "Now testing file `"
      FILEBASENAME "':" EOL)
     (CL:LOOP
      (CL:HANDLER-CASE
       (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL)
        ...
        (CL:WHEN (CL:NOT EXIT?)
         (CL:MULTIPLE-VALUE-SETQ (COMMAND EXIT?)
          (READ-S-EXPRESSION INPUTSTREAM)))
        (CL:WHEN (CL:OR EXIT? (LOGIC-COMMAND-LOOP-EXIT? COMMAND)) (CL:RETURN))
     ...
     (FREE INPUTSTREAM)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL EOL
      *LOGIC-COMMAND-RESULT-INDENT* "Finished testing file `" FILEBASENAME
      "'." EOL)))))

;;; (DEFUN (GET-FORMATTED-DATE-AND-TIME STRING) ...)

(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION () CL:SIMPLE-STRING) GET-FORMATTED-DATE-AND-TIME))
(CL:DEFUN GET-FORMATTED-DATE-AND-TIME ()
  (CALENDAR-DATE-TO-STRING (MAKE-CURRENT-DATE-TIME) (GET-LOCAL-TIME-ZONE)
   CL:NIL CL:T))
...
be diffed with previously validated runs to find deviations."
  (CL:LET*
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
    "RUNNING POWERLOOM TEST SUITE" EOL "============================" EOL EOL)
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "STELLA version:    "
    *STELLA-VERSION-STRING* EOL "PowerLoom version: "
    *POWERLOOM-VERSION-STRING* EOL "Start time:        "
    (CALENDAR-DATE-TO-STRING START-TIME (GET-LOCAL-TIME-ZONE) CL:NIL CL:T) EOL
    EOL)
   (CLEAR-CONTEXT PLUSER)
   (CL:LET*
    ((CHILD NULL) (ITER-000 (%THE-CONS-LIST (COPY (%CHILD-CONTEXTS PLUSER)))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
    ...
    "=====================================" EOL "Finish time:   "
    (CALENDAR-DATE-TO-STRING FINISH-TIME (GET-LOCAL-TIME-ZONE) CL:NIL CL:T)
    EOL "Elapsed time:  "
    (TIME-DURATION-TO-STRING (TIME-SUBTRACT FINISH-TIME START-TIME)) EOL EOL)
   (%SET-LOGGING-PARAMETERS "PowerLoom"
...
needed for its execution, and then return its result."
  (CL:LET*
   ...
   (CL:WHEN (CL:> (%DAYS ELAPSEDTIME) 0)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (%DAYS ELAPSEDTIME)
     " days, "))
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
...
the required support systems in Lisp and Java if necessary (C++ is not yet supported)."
  (CL:LET*
   ((THEOPTIONS
     (PARSE-LOGIC-COMMAND-OPTIONS OPTIONS
      (LIST* KWD-LOGIC-IN-PORT SGT-LOGIC-IN-STELLA-INTEGER NIL) CL:T CL:NIL))
    (PORT
     (%WRAPPER-VALUE
      (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-LOGIC-IN-PORT (WRAP-INTEGER 0))))
    (ADDRESS STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:FIXNUM PORT) (CL:TYPE CL:SIMPLE-STRING ADDRESS))
   (AUTOLOAD "ONTOSAURUS/CONTROL-PANEL-HANDLER" "ontosaurus" NULL CL:T)
   (CL:SETQ ADDRESS
    (CL:FUNCALL
     (AUTOLOAD "HTTP/START-HTTP-SERVER" (GET-HTTP-SERVER-SYSTEM) NULL CL:T)
     PORT))
   (PL-LOG KWD-LOGIC-IN-LOW
...
This is a no-op if no server is running or the server is not supported."
  (CL:FUNCALL
   (AUTOLOAD "HTTP/STOP-HTTP-SERVER" (GET-HTTP-SERVER-SYSTEM) NULL CL:T)))
...
the required support systems in Lisp and Java if necessary (C++ is not yet supported)."
  (CL:LET*
   ((THEOPTIONS
    ...
    (ADDRESS STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:FIXNUM PORT) (CL:TYPE CL:SIMPLE-STRING ADDRESS))
   (AUTOLOAD "GUI-SERVER/GUI-REQUEST-HANDLER" "powerloom-server" NULL CL:T)
   (CL:SETQ ADDRESS
    (CL:FUNCALL
     (AUTOLOAD "HTTP/START-HTTP-SERVER" (GET-HTTP-SERVER-SYSTEM) NULL CL:T)
     PORT))
   (PL-LOG KWD-LOGIC-IN-LOW (WRAP-STRING "Started PowerLoom server at `")
    (WRAP-STRING ADDRESS) (WRAP-STRING "ploom/soap-rpc/...'"))))
...
an HTTP server (currently Lisp and Java)."
  (CL:LET*
   ((THEOPTIONS
     (PARSE-LOGIC-COMMAND-OPTIONS OPTIONS
      (LIST* KWD-LOGIC-IN-HOST SGT-LOGIC-IN-STELLA-STRING KWD-LOGIC-IN-PORT
       SGT-LOGIC-IN-STELLA-INTEGER KWD-LOGIC-IN-DEBUG-LEVEL
       SGT-LOGIC-IN-STELLA-INTEGER NIL)
      CL:T CL:NIL))
    (HOST
     (%WRAPPER-VALUE
    ...
    (PORT
     (%WRAPPER-VALUE
      (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-LOGIC-IN-PORT (WRAP-INTEGER 9090))))
    (DEBUGLEVEL
     (%WRAPPER-VALUE
      (LOOKUP-WITH-DEFAULT THEOPTIONS KWD-LOGIC-IN-DEBUG-LEVEL
       (WRAP-INTEGER 1))))
    (GUICLASS "edu.isi.powerloom.gui.components.PowerloomApp") (GUIFN NULL)
    (GUIFNARGS (CL:MAKE-ARRAY 6)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING HOST GUICLASS)
    (CL:TYPE CL:FIXNUM PORT DEBUGLEVEL) (CL:TYPE CL:SIMPLE-VECTOR GUIFNARGS))
   (CL:PROGN
...
This is a no-op if the GUI is not running or if it is not supported."
  (CL:LET*
   ((GUICLASS "edu.isi.powerloom.gui.components.PowerloomApp") (EXITFN NULL))
...
(CL:DEFUN PRINT-FORMULA-CONTAINING-ERROR (SELF STREAM)
  (CL:LET*
   ((TEMP (NEW-OUTPUT-STRING-STREAM)) (EXPRESSIONSTRING STELLA::NULL-STRING))
   ...
   (CL:WHEN
    ...
    (CL:SETQ EXPRESSIONSTRING
     (CONCATENATE (SUBSEQUENCE EXPRESSIONSTRING 0 *EXPRESSION-SIZE-CUTOFF*)
      " ......)")))
...
(CL:DEFUN HELP-SIGNAL-PROPOSITION-ERROR (STREAM WARNINGORERROR)
  (CL:LET*
   ((BESTOUTPUTOBJECT *TERMUNDERCONSTRUCTION*)
    (TYPENAME
     (CL:IF (CL:NOT (CL:EQ BESTOUTPUTOBJECT NULL)) "proposition" "relation")))
     ...
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" WARNINGORERROR
        "' is not a valid case option")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " occurred while parsing the "
     TYPENAME ": ")
     ...
     (CL:COND
      ...
      ((SUBTYPE-OF-STRING? TEST-VALUE-000)
       (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM STREAM) EOL)
        (CL:LET*
         ((SEXPRESSION
           (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE BESTOUTPUTOBJECT))))
...
(CL:DEFUN LOOKUP-LOGIC-OBJECT-SURROGATE (NAME DEFINITIONMODULE)
  (CL:SETQ DEFINITIONMODULE
   (CL:IF (CL:NOT (CL:EQ DEFINITIONMODULE NULL)) DEFINITIONMODULE *MODULE*))
  (CL:IF (EXPLICITLY-QUALIFIED-LOGIC-OBJECT-NAME? NAME DEFINITIONMODULE)
   (LOOKUP-SURROGATE-IN-MODULE (%SYMBOL-NAME NAME) (%HOME-CONTEXT NAME) CL:T)
...
(CL:DEFUN INTERN-LOGIC-OBJECT-SURROGATE (NAME)
  (CL:WHEN (CL:NOT (CLEARABLE? *MODULE*))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (CL:LET* ((*PRINTREADABLY?* CL:T))
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
      "ERROR: Can't define the term `" NAME "' in the unclearable module `"
  ...
  (CL:LET*
   ((OLDSURROGATE (LOOKUP-LOGIC-OBJECT-SURROGATE NAME *MODULE*))
    (OLDVALUE
     (CL:IF (CL:NOT (CL:EQ OLDSURROGATE NULL)) (%SURROGATE-VALUE OLDSURROGATE)
      NULL))
    (OLDMODULE
     (CL:IF (CL:NOT (CL:EQ OLDVALUE NULL)) (HOME-MODULE OLDVALUE) NULL)))
   (CL:WHEN
...
(CL:DEFUN LOGIC-OBJECT-DEFINITION-TYPE (NAME OLDOBJECT NEWOBJECT)
  (CL:WHEN (CL:EQ OLDOBJECT NULL)
   (CL:RETURN-FROM LOGIC-OBJECT-DEFINITION-TYPE KWD-LOGIC-IN-DEFINITION))
  (CL:LET*
       ...
       (CL:WHEN
        (CL:NOT
         (CL:OR (CL:= (ARITY OLDOBJECT) (ARITY NEWOBJECT))
          (CL:AND
           (CL:NOT (CL:EQ (STRINGIFIED-SOURCE NEWOBJECT) STELLA::NULL-STRING))
           (CL:NOT
            (CL:=
             (STRING-SEARCH (STRINGIFIED-SOURCE NEWOBJECT) "VARIABLE-ARITY" 0)
             NULL-INTEGER)))))
        (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
         (CL:LET* ((*PRINTREADABLY?* CL:T))
          (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      ...
      (CL:PROGN
       (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NEWOBJECT)))
        (CL:COND
         ...
         (CL:T)))))
     (CL:T
      (CL:WHEN (CL:EQ (PRIMARY-TYPE OLDOBJECT) (PRIMARY-TYPE NEWOBJECT))
       (CL:RETURN-FROM LOGIC-OBJECT-DEFINITION-TYPE
        KWD-LOGIC-IN-REDEFINITION)))))
   (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
    (CL:LET* ((*PRINTREADABLY?* CL:T))
     (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "ERROR: Can't redefine `"
      OLDKIND "' `" NAME "' with a `" NEWKIND "'." EOL
      "   Explicitly destroy the old object first before you redefine it."
      EOL)
...
(CL:DEFUN BIND-LOGIC-OBJECT-TO-SURROGATE (NAME OBJECT)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE OBJECT)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-LOGIC-IN-LOGIC-LOGIC-OBJECT) (CL:PROGN))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-LOGIC-IN-LOGIC-PROPOSITION) (CL:PROGN))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:AND (CL:NOT (CL:EQ OLDOBJECT NULL)) (DELETED? OLDOBJECT))
    (CL:SETQ OLDOBJECT NULL) (CL:SETF (%SURROGATE-VALUE NAMESURROGATE) NULL))
   (CL:WHEN
   ...
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ OLDOBJECT NULL)) (CL:NOT (EQL? OLDOBJECT OBJECT)))
  ...
  (CL:LET*
   ...
   (CL:WHEN
    ...
    (PL-LOG KWD-LOGIC-IN-LOW (WRAP-STRING "Redefining the ")
     (WRAP-STRING (NAME-OBJECT-METACLASS OLDOBJECT)) (WRAP-STRING " named ")
     NAME))
   ...
   (CL:LET* ((PROP NULL) (ITER-002 (%THE-CONS-LIST ORIGINATEDPROPS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
     ...
     (CL:SETQ ITER-002 (%%REST ITER-002))))
   (CL:WHEN (CL:NOT (EQL? OLDOBJECT NEWOBJECT)) (DESTROY-OBJECT OLDOBJECT))))
...
(CL:DEFUN EXPLICITLY-QUALIFIED-LOGIC-OBJECT-NAME? (OBJECTNAME DEFINITIONMODULE)
  (CL:WHEN *HANDLE-EXPLICITLY-QUALIFIED-LOGIC-OBJECTS?*
   ...
   (CL:RETURN-FROM EXPLICITLY-QUALIFIED-LOGIC-OBJECT-NAME?
    (CL:NOT (VISIBLE-FROM? (%HOME-CONTEXT OBJECTNAME) DEFINITIONMODULE))))
  CL:NIL)
   ...
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
    (CL:COND
     ...
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
      (CL:PROGN
       (CL:SETQ SURROGATE
        (LOOKUP-SURROGATE-IN-MODULE (%SYMBOL-NAME SELF) (%HOME-CONTEXT SELF)
         CL:NIL))
       (CL:WHEN
...
(CL:DEFUN DEFINITION-AXIOMS-HANDLER (SELF SLOT THEAXIOMS)
  ...
  (CL:WHEN (STRING? THEAXIOMS)
   (CL:SETQ THEAXIOMS
    (UNSTRINGIFY-IN-MODULE (%WRAPPER-VALUE THEAXIOMS) (HOME-MODULE SELF))))
       ...
       (CL:LET* ((ELT NULL) (ITER-000 NEWAXIOMS))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         ...
         (CL:WHEN (CL:NOT (CONS? ELT)) (CL:SETQ ALWAYS?-000 CL:NIL)
          (CL:RETURN))
         (CL:SETQ ITER-000 (%%REST ITER-000))))
       (CL:IF ALWAYS?-000 (CL:PROGN (CL:SETQ NORMALIZEDNEWAXIOMS NEWAXIOMS))
        (CL:IF (CL:EQ (%%VALUE NEWAXIOMS) SYM-LOGIC-IN-STELLA-AND)
         (CL:PROGN (CL:SETQ NORMALIZEDNEWAXIOMS (%%REST NEWAXIOMS)))
         (CL:PROGN (CL:SETQ NORMALIZEDNEWAXIOMS (CONS NEWAXIOMS NIL))))))))
  ...
  (CL:LET* ((PARAMETERLIST NIL) (PARAMETER NULL))
   (CL:LET*
    ((PNAME NULL) (ITER-000 (%THE-CONS-LIST (%IO-VARIABLE-NAMES SELF)))
     (PTYPE NULL) (ITER-001 (%THE-CONS-LIST (%IO-VARIABLE-TYPES SELF))))
    (CL:LOOP WHILE
     ...
     (CL:WHEN (CL:AND TYPED? (CL:NOT (CL:EQ PTYPE SGT-LOGIC-IN-STELLA-THING)))
      (CL:SETQ PARAMETER
       (CONS PARAMETER
        (CONS
         (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME PTYPE) (%HOME-CONTEXT PTYPE)
          CL:T)
         NIL))))
...
(CL:DEFUN COMPLETE-RULE-DEFINITION (SELF BODY ARROW)
  (CL:LET* ((PARAMETERS (YIELD-RELATION-PARAMETERS-TREE SELF CL:NIL CL:NIL)))
   (LIST* SYM-LOGIC-IN-STELLA-FORALL PARAMETERS
    (CONS
     (CONS ARROW
      (LIST* (CONS (DESCRIPTION-NAME SELF) (CONCATENATE PARAMETERS NIL))
...
(CL:DEFUN HELP-DEFINE-KEYWORD-AXIOMS (SELFNAME KEY VALUE)
  (CL:LET* ((AXIOMS NIL) (RELATIONNAME (INTERN-SYMBOL (%SYMBOL-NAME KEY))))
   (CL:COND
    ((CL:AND (SYMBOL? VALUE) (STRING-EQUAL? (%SYMBOL-NAME VALUE) "TRUE"))
     (CL:SETQ AXIOMS (CONS RELATIONNAME (CONS SELFNAME NIL))))
    ((CL:AND (SYMBOL? VALUE) (STRING-EQUAL? (%SYMBOL-NAME VALUE) "FALSE"))
     (CL:SETQ AXIOMS
      (LIST* SYM-LOGIC-IN-STELLA-NOT (CONS RELATIONNAME (CONS SELFNAME NIL))
       NIL)))
    ((CL:AND (CONS? VALUE)
      (CL:NOT
       (MEMB? (GET-QUOTED-TREE "((SETOFALL KAPPA) \"/LOGIC\")" "/LOGIC")
        (%%VALUE VALUE))))
     (CL:LET* ((VAL NULL) (ITER-000 VALUE) (COLLECT-000 NULL))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       ...
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN
         (CL:SETQ COLLECT-000
          (CONS (CONS RELATIONNAME (CONS SELFNAME (CONS VAL NIL))) NIL))
       ...
       (CL:SETQ ITER-000 (%%REST ITER-000)))))
    (CL:T
     (CL:SETQ AXIOMS (CONS RELATIONNAME (CONS SELFNAME (CONS VALUE NIL))))))
   AXIOMS))
...
(CL:DEFUN ASSERT-KEYWORD-AXIOM (SELF SELFNAME KEY VALUE)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:LET* ((PROPS (UPDATE-PROPOSITION AXIOM KWD-LOGIC-IN-ASSERT-TRUE)))
  ...
  (CL:IF (CL:EQ THEAXIOMS NULL) NIL
   (CL:LET* ((PROPOSITIONS NIL) (RESULT NULL))
    (CL:LET* ((AXIOM NULL) (ITER-000 THEAXIOMS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:SETQ AXIOM (%%VALUE ITER-000))
      (CL:SETQ RESULT (UPDATE-PROPOSITION AXIOM KWD-LOGIC-IN-ASSERT-TRUE))
  ...
  (CL:LET*
   ...
   (CL:LET* ((*MODULE* (HOME-MODULE SELF)) (*CONTEXT* *MODULE*))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
     (CL:COND
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-LOGIC-IN-LOGIC-LOGIC-OBJECT)
       (CL:PROGN (CL:SETQ *TERMSOURCEBEINGPARSED* (STRINGIFIED-SOURCE SELF))
        (CL:WHEN (CL:EQ (AXIOMS SELF) NULL)
         (CL:RETURN-FROM FINALIZE-OBJECT-AXIOMS))
        (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-LOGIC-IN-STELLA-BAD?
         TRUE-WRAPPER NULL)
        (CL:LET* ((THEAXIOMS (HELP-FINALIZE-OBJECT-AXIOMS (AXIOMS SELF))))
         (CL:IF (CL:EQ THEAXIOMS NIL) (CL:RETURN-FROM FINALIZE-OBJECT-AXIOMS)
     ...
     (CL:LET* ((CLASS (GET-DESCRIPTION TYPE)))
      ...
      (CL:WHEN (CL:NOT (CLASS-DESCRIPTION? CLASS))
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Non-class `"
        ...
        "in the definition of relation `" (DESCRIPTION-NAME SELF) "'." EOL
        EOL)))
     (CL:SETQ ITER-000 (%%REST ITER-000))))))
        ...
        (CL:IF (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN (BAD? SELF))
         ...
         (CL:LET* ((FOUND?-000 CL:NIL))
          (CL:LET*
           ...
           (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
            (CL:SETQ TYPE (%%VALUE ITER-000))
            (CL:WHEN (CL:= (%SYMBOL-ID TYPE) -1) (CL:SETQ FOUND?-000 CL:T)
             (CL:RETURN))
            (CL:SETQ ITER-000 (%%REST ITER-000))))
          (CL:SETQ TEST-VALUE-000 FOUND?-000)))
        (CL:WHEN TEST-VALUE-000 (CL:RETURN-FROM FINALIZE-OBJECT)))
       (FINALIZE-OBJECT-AXIOMS SELF) (CHECK-DESCRIPTION-ARGUMENT-TYPES SELF)))
     ((SUBTYPE-OF? TEST-VALUE-001 SGT-LOGIC-IN-LOGIC-LOGIC-OBJECT)
      (CL:PROGN
...
(CL:DEFUN HELP-DEFINE-RELATION-FROM-PARSE-TREE (TREE STRINGIFIEDSOURCE)
  ...
  (CL:LET*
   ((OPERATOR (%%VALUE TREE)) (NAME (%%VALUE (%%REST TREE)))
    (PARAMETERS (%%VALUE (%%REST (%%REST TREE)))) (OPTIONS (NTH-REST TREE 3))
    (CLASS? (CL:EQ OPERATOR SYM-LOGIC-IN-LOGIC-DEFCONCEPT))
    (FUNCTION? (CL:EQ OPERATOR SYM-LOGIC-IN-LOGIC-DEFFUNCTION))
    (DESCRIPTION NULL))
   (CL:SETQ DESCRIPTION (GET-IDENTICAL-RELATION NAME STRINGIFIEDSOURCE))
   (CL:WHEN (CL:NOT (CL:EQ DESCRIPTION NULL))
    (CL:RETURN-FROM HELP-DEFINE-RELATION-FROM-PARSE-TREE DESCRIPTION))
   (CL:LET*
    ((PARAMETERNAMES NULL) (PARAMETERTYPES NULL) (VARIABLEARITY? CL:NIL))
    (CL:MULTIPLE-VALUE-SETQ (PARAMETERNAMES PARAMETERTYPES VARIABLEARITY?)
      ...
      (CL:COND
        ...
        (CL:PROGN (CL:SETQ NAME P)
         (CL:WHEN (CL:NOT (QUESTION-MARK-SYMBOL? NAME))
          (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
           ...
           (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-003)))))
       ...
       (CL:T
        (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
         ...
         (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-004)))))))
     (CL:WHEN (SEQUENCE-VARIABLE-SYMBOL? NAME) (CL:SETQ VARIABLEARITY? CL:T))
...
(CL:DEFUN GET-IDENTICAL-RELATION (NAME STRINGIFIEDSOURCE)
  ...
  (CL:LET*
   ((SURROGATE (LOOKUP-LOGIC-OBJECT-SURROGATE NAME *MODULE*))
    (SURROGATEVALUE
     (CL:IF (CL:NOT (CL:EQ SURROGATE NULL)) (%SURROGATE-VALUE SURROGATE)
      NULL))
    (DESCRIPTION NULL))
   (CL:WHEN
   ...
   (CL:WHEN (CL:NOT (CL:EQ DESCRIPTION NULL))
    (CL:WHEN
     (CL:AND (CL:NOT (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN (BAD? DESCRIPTION)))
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (RUN-OPTION-HANDLER? SELF KEY VALUE))
     (DEFINITION-KEYWORD-AXIOMS-HANDLER SELF (DESCRIPTION-NAME SELF) KEY
      VALUE))
    (CL:SETQ ITER-000 (%%REST (%%REST ITER-000)))))
  (CL:WHEN (CL:NOT (CLASS? SELF))
   (CL:LET*
    ((NAME (DESCRIPTION-NAME SELF)) (IOVARIABLES (%IO-VARIABLES SELF))
     (IOVARIABLETYPES (%IO-VARIABLE-TYPES SELF)) (THEAXIOMS (AXIOMS SELF))
     (TYPEDARGUMENTS NIL) (OPERATOR NULL) (FIRSTARG NULL)
    ...
    (CL:WHEN
     ...
     (CL:LET* ((AXIOM NULL) (ITER-001 THEAXIOMS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       ...
       (CL:COND
        ((CL:EQ (SAFE-PRIMARY-TYPE AXIOM) SGT-LOGIC-IN-STELLA-CONS)
         (CL:PROGN (CL:SETQ OPERATOR (%%VALUE AXIOM))
          ...
          (CL:COND
           ((CL:AND (CL:EQ OPERATOR SYM-LOGIC-IN-PL-KERNEL-KB-NTH-DOMAIN)
             (CL:EQ FIRSTARG NAME)
       ...
       (CL:SETQ ITER-001 (%%REST ITER-001)))))
    (CL:LET*
     ((TYPE NULL) (ITER-002 (%THE-CONS-LIST IOVARIABLETYPES)) (VAR NULL)
      (VECTOR-000 IOVARIABLES) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000))
      (I NULL-INTEGER) (ITER-003 0) (COLLECT-000 NULL))
     (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 I ITER-003))
     (CL:LOOP WHILE
      (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (CL:< INDEX-000 LENGTH-000)) DO
       ...
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN
         ...
         (CL:IF (CL:EQ ARGUMENTTYPEAXIOMS NIL)
          ...
          (ADD-CONS-TO-END-OF-CONS-LIST ARGUMENTTYPEAXIOMS COLLECT-000)))
        (CL:PROGN
         (CL:SETF (%%REST COLLECT-000)
          (CONS
           (LIST* SYM-LOGIC-IN-PL-KERNEL-KB-NTH-DOMAIN NAME
    ...
    (CL:WHEN (CL:NOT (CL:EQ ARGUMENTTYPEAXIOMS NIL))
     (AXIOMS-SETTER SELF (COMBINE-AXIOM-LISTS ARGUMENTTYPEAXIOMS THEAXIOMS))))))
...
"
  (CL:HANDLER-CASE (CL:RETURN-FROM %DEFFUNCTION (CALL-DEFFUNCTION ARGS))
   (LOGIC-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) (EXCEPTION-MESSAGE E))
    NULL)))
...
Expects the same arguments as `deffunction' but supplied as a list."
  (CL:LET*
   ((DEFINITION
     (CONS SYM-LOGIC-IN-LOGIC-DEFFUNCTION (CONCATENATE ARGUMENTS NIL))))
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *TERMSOURCEBEINGPARSED*))
    (CL:SETF (%%REST DEFINITION) (NORMALIZE-DEFFUNCTION-ARGUMENTS ARGUMENTS))
    (INTERN-LOGIC-OBJECT-SURROGATE (%%VALUE (%%REST DEFINITION)))
    (HELP-DEFINE-RELATION-FROM-PARSE-TREE DEFINITION *TERMSOURCEBEINGPARSED*))))

;;; (DEFUN (NORMALIZE-DEFFUNCTION-ARGUMENTS CONS) ...)

(CL:DEFUN NORMALIZE-DEFFUNCTION-ARGUMENTS (ARGUMENTS)
  (CL:LET*
   ((NAME (%%VALUE ARGUMENTS)) (PARAMETERS (%%VALUE (%%REST ARGUMENTS)))
   ...
   (CL:WHEN (CL:NOT (SYMBOL? NAME))
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (CL:LET* ((*PRINTREADABLY?* CL:T))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
       "ERROR: Illegal function name where symbol expected: `" NAME "'." EOL)
      (HELP-SIGNAL-PROPOSITION-ERROR STREAM-000 KWD-LOGIC-IN-ERROR))
     (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-000)))))
   (CL:COND
     ...
     (CL:PROGN
      (CL:LOOP WHILE (CL:NOT (CL:EQ OPTIONS NIL)) DO
       (CL:WHEN
        ...
        (CL:SETQ PARAMETERS
         (CONCATENATE PARAMETERS (CONS (%%VALUE (%%REST OPTIONS)) NIL)))
        (SECOND-SETTER ARGUMENTS PARAMETERS) (FIRST-SETTER OPTIONS NULL)
        (SECOND-SETTER OPTIONS NULL)
...
"
  (CL:HANDLER-CASE (CL:RETURN-FROM %DEFRELATION (CALL-DEFRELATION ARGS))
   (LOGIC-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) (EXCEPTION-MESSAGE E))
    NULL)))
...
Expects the same arguments as `defrelation' but supplied as a list."
  (CL:LET*
   ((DEFINITION
     (CONS SYM-LOGIC-IN-LOGIC-DEFRELATION (CONCATENATE ARGUMENTS NIL))))
   (CL:LET*
    ((*TERMSOURCEBEINGPARSED* (STRINGIFY DEFINITION))
     (*DESCRIPTIONUNDERCONSTRUCTION* NULL))
    (CL:DECLARE
     (CL:SPECIAL *TERMSOURCEBEINGPARSED* *DESCRIPTIONUNDERCONSTRUCTION*))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *TERMSOURCEBEINGPARSED*))
    (CL:SETF (%%REST DEFINITION) (NORMALIZE-DEFRELATION-ARGUMENTS ARGUMENTS))
    (INTERN-LOGIC-OBJECT-SURROGATE (%%VALUE (%%REST DEFINITION)))
    (HELP-DEFINE-RELATION-FROM-PARSE-TREE DEFINITION *TERMSOURCEBEINGPARSED*))))
   ...
   (CL:WHEN (CL:NOT (SYMBOL? NAME))
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (CL:LET* ((*PRINTREADABLY?* CL:T))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
       "ERROR: Illegal relation name where symbol expected: `" NAME "'." EOL)
      (HELP-SIGNAL-PROPOSITION-ERROR STREAM-000 KWD-LOGIC-IN-ERROR))
     (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-000)))))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE
     (CL:SPECIAL *TERMSOURCEBEINGPARSED* *DESCRIPTIONUNDERCONSTRUCTION*))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *TERMSOURCEBEINGPARSED*))
    (CL:SETF (%%REST DEFINITION) (NORMALIZE-DEFCONCEPT-ARGUMENTS ARGUMENTS))
    (INTERN-LOGIC-OBJECT-SURROGATE (%%VALUE (%%REST DEFINITION)))
    (HELP-DEFINE-RELATION-FROM-PARSE-TREE DEFINITION *TERMSOURCEBEINGPARSED*))))
...
(CL:DEFUN NORMALIZE-DEFCONCEPT-ARGUMENTS (ARGUMENTS)
  (CL:LET*
   ((NAME (%%VALUE ARGUMENTS)) (PARAMETERS NULL)
    (SELFVARIABLE SYM-LOGIC-IN-LOGIC-?SELF) (SUPERS NIL) (IMPLIESRULES NIL))
   (CL:WHEN (CL:NOT (SYMBOL? NAME))
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (CL:LET* ((*PRINTREADABLY?* CL:T))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
       "ERROR: Illegal concept name where symbol expected: `" NAME "'." EOL)
      (HELP-SIGNAL-PROPOSITION-ERROR STREAM-000 KWD-LOGIC-IN-ERROR))
     (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-000)))))
      ...
      (CL:T
       (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
        (CL:LET* ((*PRINTREADABLY?* CL:T))
         (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001)
          "ERROR: Illegal parameter list: `" (%%VALUE (%%REST ARGUMENTS)) "'."
          EOL)
         (HELP-SIGNAL-PROPOSITION-ERROR STREAM-001 KWD-LOGIC-IN-ERROR))
        (CL:ERROR (NEW-PROPOSITION-ERROR (THE-STRING-READER STREAM-001))))))
...
Expects the same arguments as `defobject' but supplied as a list."
  (CL:LET*
   ...
   (CL:LET* ((*TERMSOURCEBEINGPARSED* (STRINGIFY DEFINITION)))
    ...
    (CL:COND
     ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE TERM) SGT-LOGIC-IN-LOGIC-LOGIC-OBJECT)
      (CL:PROGN (STRINGIFIED-SOURCE-SETTER TERM *TERMSOURCEBEINGPARSED*)
...
(CL:DEFMETHOD PROCESS-DEFINITION-OPTIONS ((SELF LOGIC-OBJECT) OPTIONS)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:SETQ VALUE (%%VALUE (%%REST ITER-000)))
    (CL:WHEN (CL:NOT (RUN-OPTION-HANDLER? SELF KEY VALUE))
     (DEFINITION-KEYWORD-AXIOMS-HANDLER SELF (OBJECT-NAME SELF) KEY VALUE))
    (CL:SETQ ITER-000 (%%REST (%%REST ITER-000))))))
...
"
  (CL:HANDLER-CASE (CL:RETURN-FROM %DEFPROPOSITION (CALL-DEFPROPOSITION ARGS))
   (LOGIC-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) (EXCEPTION-MESSAGE E))
    NULL)))
...
Expects the same arguments as `defproposition' but supplied as a list."
  (CL:LET*
   ((DEFINITION
     (CONS SYM-LOGIC-IN-LOGIC-DEFPROPOSITION (CONCATENATE ARGUMENTS NIL)))
   ...
   (CL:LET* ((*TERMSOURCEBEINGPARSED* (STRINGIFY DEFINITION)))
    ...
    (CL:SETQ NAME (%%VALUE ARGUMENTS)) (INTERN-LOGIC-OBJECT-SURROGATE NAME)
    (CL:SETQ CONCEPTION
     (SMART-UPDATE-PROPOSITION (%%VALUE (%%REST ARGUMENTS))
      KWD-LOGIC-IN-CONCEIVE))
    (CL:COND
     ((CL:EQ CONCEPTION NULL) (CL:RETURN-FROM CALL-DEFPROPOSITION NULL))
     ((CONS? CONCEPTION)
    ...
    (CL:WHEN (CL:EQ (%KIND PROPOSITION) KWD-LOGIC-IN-FORALL)
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:SETQ SATELLITE (%%VALUE ITER-000)) (DESTROY-PROPOSITION SATELLITE)
       (CL:SETQ ITER-000 (%%REST ITER-000))))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION)
    ...
    (CL:SETQ OLDTRUTHVALUE
     (ACCESS-IN-CONTEXT (%TRUTH-VALUE PROPOSITION) (%HOME-CONTEXT PROPOSITION)
      CL:NIL))
    (CL:COND
     ((CL:EQ (LOOKUP-ANNOTATION PROPOSITION KWD-LOGIC-IN-CONFIDENCE-LEVEL)
       KWD-LOGIC-IN-DEFAULT)
      (REMOVE-ANNOTATION PROPOSITION KWD-LOGIC-IN-CONFIDENCE-LEVEL)
      (UPDATE-PROPOSITION-TRUTH-VALUE PROPOSITION KWD-LOGIC-IN-PRESUME-TRUE))
     (CL:T
      (UPDATE-PROPOSITION-TRUTH-VALUE PROPOSITION KWD-LOGIC-IN-ASSERT-TRUE)))
    (CL:WHEN
     (CL:EQ OLDTRUTHVALUE
...
(CL:DEFUN NORMALIZE-DEFPROPOSITION-ARGUMENTS (ARGUMENTS)
  (CL:LET*
   ((NAME (%%VALUE ARGUMENTS)) (PROPOSITION (%%VALUE (%%REST ARGUMENTS)))
    (OPTIONS NULL) (VALUE NULL) (ANNOTATIONS NIL))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE (%%VALUE ARGUMENTS))))
    (CL:COND ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000))
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:SETQ NAME (INTERN-SYMBOL (%WRAPPER-VALUE (%%VALUE ARGUMENTS))))
  ...
  (CL:LET* ((COMMANDS (NEW-LIST)))
   (CL:LET*
    ((FUNCTION NULL)
     ...
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ FUNCTION (%VALUE ITER-000))
     (CL:WHEN
      (CL:AND
       (%WRAPPER-VALUE
     ...
     (CL:IF (CL:EQ COLLECT-001 NULL)
      (CL:PROGN
       ...
       (CL:IF (CL:EQ (%THE-CONS-LIST COMMANDS) NIL)
        ...
        (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST COMMANDS) COLLECT-001)))
      (CL:PROGN
         ...
         (CL:EQ
          (%WRAPPER-VALUE
           (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS COMMAND)
            SYM-LOGIC-IN-STELLA-DOCUMENTATION NULL-STRING-WRAPPER))
          STELLA::NULL-STRING)
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) (%SLOT-NAME COMMAND)
          ":" EOL))
        (CL:SETQ ITER-001 (%%REST ITER-001))))))
    (CL:T
...
(CL:DEFUN HELP-STARTUP-LOGIC-IN1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-LOGIC-IN-STELLA-LOAD
    (INTERN-RIGID-SYMBOL-WRT-MODULE "LOAD" (GET-STELLA-MODULE "/STELLA" CL:T)
     0))
   ...
   (CL:SETQ SYM-LOGIC-IN-STELLA-DEFUN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DEFUN" (GET-STELLA-MODULE "/STELLA" CL:T)
     0))
   ...
   (CL:SETQ KWD-LOGIC-IN-LOW (INTERN-RIGID-SYMBOL-WRT-MODULE "LOW" NULL 2))
   ...
   (CL:SETQ KWD-LOGIC-IN-NONE (INTERN-RIGID-SYMBOL-WRT-MODULE "NONE" NULL 2))
   (CL:SETQ SYM-LOGIC-IN-LOGIC-TIME-COMMAND
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TIME-COMMAND" NULL 0))
   (CL:SETQ KWD-LOGIC-IN-PORT (INTERN-RIGID-SYMBOL-WRT-MODULE "PORT" NULL 2))
   ...
   (CL:SETQ KWD-LOGIC-IN-HOST (INTERN-RIGID-SYMBOL-WRT-MODULE "HOST" NULL 2))
...
(CL:DEFUN HELP-STARTUP-LOGIC-IN2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-LOGIC-IN-STELLA-BAD?
    (INTERN-RIGID-SYMBOL-WRT-MODULE "BAD?" (GET-STELLA-MODULE "/STELLA" CL:T)
     0))
...
(CL:DEFUN HELP-STARTUP-LOGIC-IN3 ()
  (CL:PROGN
   ...
   (CL:SETQ *DEMO-FILES*
    (LIST*
     (LIST* (WRAP-STRING "basics") (WRAP-STRING "Basic PowerLoom commands")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "classes")
      (WRAP-STRING "Primitive and defined classes") (WRAP-STRING "test-suite")
      NIL)
     (LIST* (WRAP-STRING "collections")
      (WRAP-STRING "Reasoning with collections") (WRAP-STRING "test-suite")
      NIL)
     (LIST* (WRAP-STRING "append") (WRAP-STRING "Prolog-style `append'")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "inequalities")
      (WRAP-STRING "Reasoning with inequalities") (WRAP-STRING "test-suite")
      NIL)
     (LIST* (WRAP-STRING "recursion")
      (WRAP-STRING "Reasoning with recursive rules")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "negation") (WRAP-STRING "Reasoning with negation")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "constraints") (WRAP-STRING "Constraint propagation")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "equations")
      (WRAP-STRING "Simple equational reasoning") (WRAP-STRING "test-suite")
      NIL)
     (LIST* (WRAP-STRING "subsumption")
      (WRAP-STRING "Simple subsumption reasoning") (WRAP-STRING "test-suite")
      NIL)
     (LIST* (WRAP-STRING "family")
      (WRAP-STRING "Subsumption reasoning within a family ontology")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "relation-hierarchy")
      (WRAP-STRING "Finding sub, super and equivalent concepts and relations")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "defaults")
      (WRAP-STRING "Default reasoning with Tweety and friends")
      (WRAP-STRING "test-suite") NIL)
     (LIST* (WRAP-STRING "defaults2") (WRAP-STRING "More default reasoning")
      (WRAP-STRING "test-suite") NIL)
     ...
     (LIST* (WRAP-STRING "test-suite") (WRAP-STRING "PowerLoom test suite")
      (WRAP-STRING "test-suite") NIL)
     NIL))
...
By default, the features `emit-thinking-dots' and `just-in-time-inference'
are enabled, and the others are disabled.
\")" (CL:FUNCTION %SET-FEATURE) (CL:FUNCTION SET-FEATURE-EVALUATOR-WRAPPER))
...
that do NOT have an `in-module' declaration as part of the file.
If this is not set, and no `in-module' declaration is in the file,
then an error will be signaled.\" :COMMAND? TRUE :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "LOAD-STREAM-IN-MODULE"
    "(DEFUN LOAD-STREAM-IN-MODULE ((STREAM INPUT-STREAM) (DEFAULT-MODULE MODULE)) :DOCUMENTATION \"Read logic commands from `stream' and evaluate them.  If `default-module'
is not `null', then any commands will be read into that module unless
an `in-module' declaration is encountered which will over-ride the default
value.  If noe `default-module' is specified, and the input stream does
not have an `in-module' form, an error is signaled.\" :PUBLIC? TRUE)"
...
starts with a `-' character.\" :PUBLIC? TRUE :COMMAND? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "PRINT-DEMO-PROMPT" "(DEFUN PRINT-DEMO-PROMPT ())"
    (CL:FUNCTION PRINT-DEMO-PROMPT) NULL)
...
Typing `?' at the pause prompt prints a list of available commands.\")"
   ...
   (DEFINE-FUNCTION-OBJECT "TEST-LOGIC-FILE"
    "(DEFUN TEST-LOGIC-FILE ((FILE FILE-NAME)))" (CL:FUNCTION TEST-LOGIC-FILE)
    NULL)
...
(CL:DEFUN HELP-STARTUP-LOGIC-IN5 ()
  (CL:PROGN
   (DEFINE-FUNCTION-OBJECT "AXIOMS" "(DEFUN (AXIOMS CONS) ((SELF OBJECT)))"
    (CL:FUNCTION AXIOMS) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "FINALIZE-OBJECT"
    "(DEFUN FINALIZE-OBJECT ((SELF OBJECT)))" (CL:FUNCTION FINALIZE-OBJECT)
    NULL)
...
the last variable in the parameter list is used as the output variable.
See `defrelation' for a description of `<keyword-option>'s.
\")" (CL:FUNCTION %DEFFUNCTION) (CL:FUNCTION DEFFUNCTION-EVALUATOR-WRAPPER))
...
`:foo true' states that `(foo <relconst>)' is true, `:foo false' states that
`(not (foo <relconst>))' is true.
\")" (CL:FUNCTION %DEFRELATION) (CL:FUNCTION DEFRELATION-EVALUATOR-WRAPPER))
...
`load' or `demo' command, try to find it with one of these extensions.\" :PUBLIC? TRUE)")
    ...
    (REGISTER-NATIVE-NAME SYM-LOGIC-IN-STELLA-LOAD KWD-LOGIC-IN-COMMON-LISP
...
invocation of 'finalize-objects'.\")")
    ...
    (REGISTER-NATIVE-NAME SYM-LOGIC-IN-LOGIC-DEFPROPOSITION
     KWD-LOGIC-IN-COMMON-LISP KWD-LOGIC-IN-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-LOGIC-IN-LOGIC-DEFRULE KWD-LOGIC-IN-COMMON-LISP
     KWD-LOGIC-IN-FUNCTION)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
