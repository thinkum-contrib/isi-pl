...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* *SURROGATE-ARRAY* NIL FALSE TRUE))

;;; (DEFUN (CLASS-NAMES-NEXT? BOOLEAN) ...)

(CL:DEFUN CLASS-NAMES-NEXT? (SELF)
  (CL:WHEN (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) FALSE)
...
If `local?', only return propositions conceived locally in `module'."
  (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-OBJECT SELF-000)
    (CL:IF LOCAL? (LIST MODULE) (REVERSE (LISTIFY (VISIBLE-MODULES MODULE)))))
...
only return instances created locally in `module'.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Api-Support") NULL-STRING-WRAPPER)))
