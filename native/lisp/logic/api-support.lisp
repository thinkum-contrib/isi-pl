...
(CL:DEFUN CLASS-NAMES-NEXT? (SELF)
  (CL:WHEN (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)
   (CL:SETF (%VALUE SELF) (%CLASS-TYPE (%ITERATOR-OBJECT SELF))) (CL:RETURN-FROM CLASS-NAMES-NEXT? CL:T))
  (CL:LET* ((NEXT? (NEXT? (%ITERATOR-NESTED-ITERATOR SELF))))
   (CL:WHEN NEXT? (CL:SETF (%VALUE SELF) (%VALUE (%ITERATOR-NESTED-ITERATOR SELF)))) NEXT?))
...
(CL:DEFUN ALL-PROPOSITIONS-NEXT? (SELF)
  (CL:LOOP
   (CL:LET* ((ITERATOR (%ITERATOR-NESTED-ITERATOR SELF)) (MODULE NULL))
    (CL:WHEN (NEXT? ITERATOR) (CL:SETF (%VALUE SELF) (%VALUE ITERATOR)) (CL:RETURN-FROM ALL-PROPOSITIONS-NEXT? CL:T))
    (CL:SETQ MODULE (POP (%ITERATOR-OBJECT SELF)))
    (CL:WHEN (CL:EQ MODULE NULL) (CL:SETF (%VALUE SELF) NULL) (CL:RETURN-FROM ALL-PROPOSITIONS-NEXT? CL:NIL))
    (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF) (ALLOCATE-ITERATOR (LOCALLY-CONCEIVED-PROPOSITIONS MODULE))))))
...
If `local?', only return propositions conceived locally in `module'."
  (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-OBJECT SELF-000)
    (CL:IF LOCAL? (LIST) (LISTIFY (REVERSE (COPY-CONS-LIST (%%REST (VISIBLE-MODULES MODULE)))))))
   (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-000) (ALLOCATE-ITERATOR (LOCALLY-CONCEIVED-PROPOSITIONS MODULE)))
   (CL:SETF (%ITERATOR-NEXT-CODE SELF-000) (CL:FUNCTION ALL-PROPOSITIONS-NEXT?))
...
(CL:DEFUN FILTER-INCONSISTENT-PROPOSITION? (SELF ITERATOR)
  (CL:LET* ((MODULE (%ITERATOR-OBJECT ITERATOR)))
   (CL:LET* ((*CONTEXT* NULL) (*MODULE* NULL)) (CL:DECLARE (CL:SPECIAL *CONTEXT* *MODULE*))
    (CL:LET* ((TEMP-000 (BEST-INFERENCE-CACHE MODULE)))
     (CL:SETQ *CONTEXT* (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 MODULE)))
    (CL:SETQ *MODULE* (%BASE-MODULE *CONTEXT*)) (INCONSISTENT? SELF))))
...
inconsistent propositions conceived locally in `module'."
  (CL:LET* ((PROPOSITIONS-ITERATOR (ALL-PROPOSITIONS MODULE LOCAL?)) (ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-OBJECT ITERATOR) MODULE) (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T)
   (CL:SETF (%ITERATOR-NESTED-ITERATOR ITERATOR) PROPOSITIONS-ITERATOR)
   (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION FILTERED-NESTED-ITERATOR-NEXT?))
   (CL:SETF (%ITERATOR-FILTER-CODE ITERATOR) (CL:FUNCTION FILTER-INCONSISTENT-PROPOSITION?)) ITERATOR))
...
propositions regardless of whether their truth value is defined or not."
  (CL:LET* ((TRUTHVALUE (%TRUTH-VALUE PROPOSITION)))
   (CL:COND
    ((CL:OR (CL:EQ TRUTHVALUE NULL) (ISA? TRUTHVALUE SGT-API-SUPPORT-LOGIC-TRUTH-VALUE))
     (CL:RETURN-FROM LOCALLY-MODIFIED-PROPOSITION? (CL:EQ (HOME-MODULE PROPOSITION) MODULE)))
    (CL:T
     (CL:LET* ((CTXT NULL) (TV NULL) (ITER-000 (%THE-KV-LIST TRUTHVALUE)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ CTXT (%KEY ITER-000)) (CL:SETQ TV (%VALUE ITER-000))
       (CL:SETQ TV TV) (CL:WHEN (CL:EQ CTXT MODULE) (CL:RETURN-FROM LOCALLY-MODIFIED-PROPOSITION? CL:T))
...
(CL:DEFUN FILTER-LOCALLY-MODIFIED-PROPOSITION? (SELF ITERATOR)
  (LOCALLY-MODIFIED-PROPOSITION? SELF (%ITERATOR-OBJECT ITERATOR)))
...
will always include all locally conceived propositions."
  (CL:LET* ((PROPOSITIONS-ITERATOR (ALL-PROPOSITIONS MODULE CL:NIL)) (ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-OBJECT ITERATOR) MODULE) (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T)
   (CL:SETF (%ITERATOR-NESTED-ITERATOR ITERATOR) PROPOSITIONS-ITERATOR)
   (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION FILTERED-NESTED-ITERATOR-NEXT?))
   (CL:SETF (%ITERATOR-FILTER-CODE ITERATOR) (CL:FUNCTION FILTER-LOCALLY-MODIFIED-PROPOSITION?)) ITERATOR))

;;; (DEFUN (VISIBLE-TERM? BOOLEAN) ...)

(CL:DEFUN VISIBLE-TERM? (TERM CONTEXT LOCAL?)
  (CL:IF LOCAL? (CL:EQ (%HOME-CONTEXT TERM) CONTEXT) (VISIBLE-FROM? (%HOME-CONTEXT TERM) CONTEXT)))
...
terms created locally in `module'."
  (CL:LET* ((TERMS NIL) (TERMSWITHDUPLICATES NIL) (TERM NULL))
   (CL:LET* ((PROPOSITION NULL) (ITER-000 (ALL-PROPOSITIONS MODULE LOCAL?)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ PROPOSITION (%VALUE ITER-000))
     (CL:LET* ((ARG NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (CL:TAGBODY
        (CL:WHEN
         (CL:AND (CL:NOT (CL:EQ ARG NULL)) (ISA? ARG SGT-API-SUPPORT-LOGIC-LOGIC-OBJECT) (CL:NOT (DELETED? ARG)))
         (CL:SETQ TERM ARG)
         (CL:WHEN
          (CL:OR (CL:NOT (VISIBLE-TERM? TERM MODULE LOCAL?)) (CL:NOT (CL:EQ (%SURROGATE-VALUE-INVERSE TERM) NULL)))
          (CL:GO :CONTINUE))
         (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE TERM)))
          (CL:COND ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-PATTERN-VARIABLE) (CL:PROGN))
           ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-SKOLEM)
            (CL:PROGN
             (CL:IF (CL:NOT (CL:EQ (%DEFINING-PROPOSITION TERM) NULL))
              (CL:WHEN (CL:EQ (%DEFINING-PROPOSITION TERM) PROPOSITION) (CL:SETQ TERMS (CONS TERM TERMS)))
              (CL:SETQ TERMSWITHDUPLICATES (CONS TERM TERMSWITHDUPLICATES)))))
           (CL:T (CL:SETQ TERMSWITHDUPLICATES (CONS TERM TERMSWITHDUPLICATES))))))
        :CONTINUE)
       (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))))
   (ALLOCATE-ITERATOR (CONCATENATE TERMS (REMOVE-DUPLICATES TERMSWITHDUPLICATES)))))
...
(CL:DEFUN NAMED-TERM? (TERM ITER)
  (CL:SETQ ITER ITER)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE TERM)))
   (CL:COND ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-NAMED-DESCRIPTION) (CL:PROGN (CL:NOT (DELETED? TERM))))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-DESCRIPTION) (CL:PROGN CL:NIL))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-PATTERN-VARIABLE) (CL:PROGN CL:NIL))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-LOGIC-LOGIC-OBJECT) (CL:PROGN (CL:NOT (DELETED? TERM))))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-API-SUPPORT-STELLA-THING) (CL:PROGN (CL:NOT (DELETED? TERM)))) (CL:T CL:NIL))))
...
terms created locally in `module'."
  (CL:IF LOCAL?
   (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR)))
    (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-000) (ALLOCATE-ITERATOR (LOCALLY-CONCEIVED-INSTANCES MODULE)))
    (CL:SETF (%ITERATOR-NEXT-CODE SELF-000) (CL:FUNCTION FILTERED-NESTED-ITERATOR-NEXT?))
    (CL:SETF (%ITERATOR-FILTER-CODE SELF-000) (CL:FUNCTION NAMED-TERM?)) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000))
   (CL:LET* ((ITERATOR1 (ALL-NAMED-TERMS MODULE CL:T)) (ITERATORLIST NIL))
    (CL:LET* ((SUPER NULL) (ITER-000 (%ALL-SUPER-CONTEXTS MODULE)) (COLLECT-000 NULL))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SUPER (%%VALUE ITER-000))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS (ALL-NAMED-TERMS SUPER CL:T) NIL))
        (CL:IF (CL:EQ ITERATORLIST NIL) (CL:SETQ ITERATORLIST COLLECT-000)
         (ADD-CONS-TO-END-OF-CONS-LIST ITERATORLIST COLLECT-000)))
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (ALL-NAMED-TERMS SUPER CL:T) NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (CL:IF (CL:EQ ITERATORLIST NIL) ITERATOR1
     (CL:LET* ((SELF-001 (NEW-ALL-PURPOSE-ITERATOR))) (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-001) ITERATOR1)
      (CL:SETF (%ITERATOR-CONS-LIST SELF-001) ITERATORLIST)
      (CL:SETF (%ITERATOR-NEXT-CODE SELF-001) (CL:FUNCTION FILTERED-CONCATENATED-ITERATOR-NEXT?))
...
terms created locally in `module'."
  (CL:IF LOCAL?
   ...
   (CONCATENATE (ALL-NAMED-TERMS MODULE CL:NIL) (ALL-UNNAMED-TERMS MODULE CL:NIL))))
...
only return instances created locally in `module'."
  (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-000) (ALL-NAMED-TERMS MODULE LOCAL?))
   (CL:SETF (%ITERATOR-NEXT-CODE SELF-000) (CL:FUNCTION FILTERED-NESTED-ITERATOR-NEXT?))
   (CL:SETF (%ITERATOR-FILTER-CODE SELF-000) (CL:FUNCTION FILTER-INSTANCE?)) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000)))
...
Only instances that haven't been deleted will be considered.  If `local?',
only return instances created locally in `module'."
  (CL:LET* ((ITERATOR (ALL-TERMS MODULE LOCAL?)) (ALLPURPOSEITERATOR NULL))
   (CL:IF LOCAL?
    (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR))) (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-000) ITERATOR)
     (CL:SETF (%ITERATOR-NEXT-CODE SELF-000) (CL:FUNCTION FILTERED-NESTED-ITERATOR-NEXT?))
     (CL:SETQ ALLPURPOSEITERATOR SELF-000))
    (CL:SETQ ALLPURPOSEITERATOR ITERATOR))
   (CL:SETF (%ITERATOR-FILTER-CODE ALLPURPOSEITERATOR) (CL:FUNCTION FILTER-INSTANCE?)) ALLPURPOSEITERATOR))
...
(CL:DEFUN STRING-TO-LOGIC-OBJECT (STRING)
  ...
  (CL:LET* ((SGT NULL) (ITER-000 (ALL-SURROGATES NULL CL:NIL)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SGT (%VALUE ITER-000))
    (CL:WHEN
     (CL:AND (STRING-EQL? (%SYMBOL-NAME SGT) STRING) (CL:NOT (CL:EQ (%SURROGATE-VALUE SGT) NULL))
...
(CL:DEFUN STARTUP-API-SUPPORT ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/LOGIC" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-API-SUPPORT-LOGIC-TRUTH-VALUE (INTERN-RIGID-SYMBOL-WRT-MODULE "TRUTH-VALUE" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-LOGIC-LOGIC-OBJECT (INTERN-RIGID-SYMBOL-WRT-MODULE "LOGIC-OBJECT" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-LOGIC-PATTERN-VARIABLE (INTERN-RIGID-SYMBOL-WRT-MODULE "PATTERN-VARIABLE" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-LOGIC-SKOLEM (INTERN-RIGID-SYMBOL-WRT-MODULE "SKOLEM" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-LOGIC-NAMED-DESCRIPTION (INTERN-RIGID-SYMBOL-WRT-MODULE "NAMED-DESCRIPTION" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-LOGIC-DESCRIPTION (INTERN-RIGID-SYMBOL-WRT-MODULE "DESCRIPTION" NULL 1))
    (CL:SETQ SGT-API-SUPPORT-STELLA-THING
     (INTERN-RIGID-SYMBOL-WRT-MODULE "THING" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
    (CL:SETQ SYM-API-SUPPORT-LOGIC-STARTUP-API-SUPPORT (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-API-SUPPORT" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "CLASS-NAMES-NEXT?" "(DEFUN (CLASS-NAMES-NEXT? BOOLEAN) ((SELF ALL-PURPOSE-ITERATOR)))"
     (CL:FUNCTION CLASS-NAMES-NEXT?) NULL)
    (DEFINE-FUNCTION-OBJECT "ALL-PROPOSITIONS-NEXT?"
     "(DEFUN (ALL-PROPOSITIONS-NEXT? BOOLEAN) ((SELF ALL-PURPOSE-ITERATOR)))" (CL:FUNCTION ALL-PROPOSITIONS-NEXT?)
     NULL)
...
Only propositions that haven't been deleted will be considered.
If `local?', only return propositions conceived locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-PROPOSITIONS)
     NULL)
...
that have an inconsistent truth value.  If `local?', only return
inconsistent propositions conceived locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-INCONSISTENT-PROPOSITIONS)
     NULL)
...
an instance (or individual) as well as a description.  Only terms that
haven't been deleted will be considered.  If `local?', only return
terms created locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-UNNAMED-TERMS) NULL)
    (DEFINE-FUNCTION-OBJECT "NAMED-TERM?" "(DEFUN (NAMED-TERM? BOOLEAN) ((TERM OBJECT) (ITER ALL-PURPOSE-ITERATOR)))"
...
instance (or individual) as well as a description.  Only terms that
haven't been deleted will be considered.  If `local?', only return 
terms created locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-NAMED-TERMS) NULL)
...
instance (or individual) as well as a description.  Only terms that
haven't been deleted will be considered.  If `local?', only return 
terms created locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-TERMS) NULL)
...
Only instances that haven't been deleted will be considered.  If `local?',
only return instances created locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-NAMED-INSTANCES) NULL)
...
Only instances that haven't been deleted will be considered.  If `local?',
only return instances created locally in `module'.\" :PUBLIC? TRUE)" (CL:FUNCTION ALL-INSTANCES) NULL)
    (DEFINE-FUNCTION-OBJECT "STRING-TO-LOGIC-OBJECT" "(DEFUN (STRING-TO-LOGIC-OBJECT LOGIC-OBJECT) ((STRING STRING)))"
     (CL:FUNCTION STRING-TO-LOGIC-OBJECT) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-API-SUPPORT" "(DEFUN STARTUP-API-SUPPORT () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-API-SUPPORT) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-API-SUPPORT-LOGIC-STARTUP-API-SUPPORT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-API-SUPPORT-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "LOGIC")))
    (INSERT-AT *STRING-TO-OBJECT-FUNCTIONS* SGT-API-SUPPORT-LOGIC-LOGIC-OBJECT
