...
(CL:DECLAIM
  ...
  *SAVE-NETWORK-FILE* STANDARD-OUTPUT EOL))

;;; (DEFUN (OBJECT-NAME SYMBOL) ...)

(CL:DEFUN OBJECT-NAME (INSTANCE)
  (SURROGATE-TO-SYMBOL (%SURROGATE-VALUE-INVERSE INSTANCE)))

;;; (DEFUN (OBJECT-STRING-NAME STRING) ...)

(CL:DEFUN OBJECT-STRING-NAME (INSTANCE)
  (%SYMBOL-NAME (%SURROGATE-VALUE-INVERSE INSTANCE)))
...
(CL:PROGN (CL:DEFVAR *NUM-NEIGHBORS* 3) (CL:DECLAIM (CL:TYPE CL:FIXNUM *NUM-NEIGHBORS*)))
...
(CL:PROGN (CL:DEFVAR *NUM-CASES* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *NUM-CASES*)))

;;; (DEFGLOBAL *NUM-TRAINING-PER-CASE* ...)

(CL:PROGN (CL:DEFVAR *NUM-TRAINING-PER-CASE* 12) (CL:DECLAIM (CL:TYPE CL:FIXNUM *NUM-TRAINING-PER-CASE*)))

;;; (DEFGLOBAL *CASE-VARIABLE-COUNTER* ...)

(CL:PROGN (CL:DEFVAR *CASE-VARIABLE-COUNTER* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *CASE-VARIABLE-COUNTER*)))

;;; (DEFGLOBAL *BASE-COUNTER* ...)

(CL:PROGN (CL:DEFVAR *BASE-COUNTER* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *BASE-COUNTER*)))
...
(CL:DEFUN BUILD-CASE-FROM-INSTANCE (INST KIND)
  ...
  (CL:LET*
   ((NAME (OBJECT-NAME INST))
    ...
    (RULE-NAME (CONCATENATE (OBJECT-STRING-NAME CLASS) "-match"))
    (RULE-ANTECEDENT (LOOKUP *CASE-ANTECEDENT-TABLE* RULE-NAME))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((INST NULL) (I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:1- NUM)) (ITER-001 ITERATOR))
   ...
   (CL:LET* ((I NULL-INTEGER) (ITER-001 0) (UPPER-BOUND-001 (CL:1- NUM)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
     ...
     (CL:LET
      ...
      (CL:SETF (CL:AREF SELF POSITION) VALUE))))
   (CL:LET*
    ((CASE NULL) (I NULL-INTEGER) (ITER-002 0)
     (UPPER-BOUND-002 (CL:1- NUM)) (ITER-003 (%THE-CONS-LIST CASES)))
...
(CL:DEFUN %TEST-CASE-BASED-REASONER (CLASS-NAME SLOT-NAME NUM-TRAIN NUM-TEST)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((CASE NULL) (I NULL-INTEGER) (ITER-002 0)
     (UPPER-BOUND-001 (CL:1- NUM-TRAIN))
     (ITER-003 (%THE-CONS-LIST CASE-LIST)) (COLLECT-000 NULL))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-002 UPPER-BOUND-001 I))
    (CL:LOOP WHILE
     (CL:AND (CL:<= ITER-002 UPPER-BOUND-001)
      (CL:NOT (CL:EQ ITER-003 NIL)))
     DO
     (CL:PROGN (CL:SETQ I ITER-002) (CL:SETQ ITER-002 (CL:1+ ITER-002)))
     (CL:PROGN (CL:SETQ CASE (%%VALUE ITER-003))
...
(CL:DEFUN %TEST-CASE-MATCHER (CLASS-NAME SLOT-NAME)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((CASE-NAME NULL) (PROBE NULL) (I NULL-INTEGER) (CASE NULL)
     (ITER-003 (%THE-CONS-LIST CASES)) (ITER-004 1)
     (ITER-005 (%THE-CONS-LIST CASES))
     (ITER-006 (%THE-CONS-LIST CASE-NAMES)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-004 I))
    (CL:LOOP WHILE
     (CL:AND (CL:NOT (CL:EQ ITER-003 NIL)) (CL:NOT (CL:EQ ITER-005 NIL))
      (CL:NOT (CL:EQ ITER-006 NIL)))
     DO
     (CL:PROGN (CL:SETQ CASE (%%VALUE ITER-003))
      (CL:SETQ ITER-003 (%%REST ITER-003)))
     (CL:PROGN (CL:SETQ I ITER-004) (CL:SETQ ITER-004 (CL:1+ ITER-004)))
     (CL:PROGN (CL:SETQ PROBE (%%VALUE ITER-005))
      (CL:SETQ ITER-005 (%%REST ITER-005)))
     (CL:PROGN (CL:SETQ CASE-NAME (%%VALUE ITER-006))
    ...
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO
     ...
     (CL:LET*
      ((J NULL-INTEGER) (ITER-002 0)
       (UPPER-BOUND-002 (CL:1- *NUM-TRAINING-PER-CASE*)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-004 UPPER-BOUND-004) DO
     ...
     (CL:LET*
      ((J NULL-INTEGER) (ITER-005 0)
       (UPPER-BOUND-005 (CL:1- *NUM-TRAINING-PER-CASE*)))
...
(CL:DEFUN FILTER-ANTECEDENT (ANTECEDENT RULE-NAME)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 0)
     (UPPER-BOUND-000 (CL:1- (LENGTH ANTECEDENT))))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 I))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     (CL:PROGN (CL:SETQ I ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:LET*
...
(CL:DEFUN HELP-STARTUP-CASE-BASED1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-CASE-BASED-LOGIC-SET-CASE-SOLUTION-COMBINATION
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-CASE-SOLUTION-COMBINATION" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-CASE-BASED3 ()
  (CL:PROGN
   (DEFINE-FUNCTION-OBJECT "OBJECT-NAME"
    "(DEFUN (OBJECT-NAME SYMBOL) ((INSTANCE LOGIC-OBJECT)))"
    (CL:FUNCTION OBJECT-NAME) NULL)
   (DEFINE-FUNCTION-OBJECT "OBJECT-STRING-NAME"
    "(DEFUN (OBJECT-STRING-NAME STRING) ((INSTANCE LOGIC-OBJECT)))"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Case-Based") NULL-STRING-WRAPPER))))
