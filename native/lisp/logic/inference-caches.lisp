...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* STANDARD-ERROR *CONTEXT*
  *QUERYITERATOR* *TRACED-KEYWORDS* STANDARD-OUTPUT EOL
  *JUST-IN-TIME-FORWARD-INFERENCE?* FALSE NIL TRUE-WRAPPER FALSE-WRAPPER TRUE))
...
(CL:DEFUN INITIALIZE-INFERENCE-WORLD (WORLD)
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD)
   SYM-INFERENCE-CACHES-LOGIC-EVALUATION-STATE-TABLE (NEW-HASH-TABLE) NULL)
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD)
   ...
   (CL:COND
    ...
    ((CL:EQ KIND KWD-INFERENCE-CACHES-META)
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD)
      ...
      (CL:IF (CL:NOT (NEXT? (ALL-PROPOSITIONS (%BASE-MODULE CONTEXT) TRUE)))
    ...
    ((CL:OR (CL:EQ KIND KWD-INFERENCE-CACHES-META)
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:PROGN (CL:SETQ PARENTMODULE (%%VALUE ITER-000))
...
(CL:DEFVAR *INVISIBLEASSERTION?* FALSE
...
(CL:DEFUN META-PROPOSITION? (PROPOSITION)
  (CL:WHEN (CL:EQ PROPOSITION NULL) (CL:RETURN-FROM META-PROPOSITION? FALSE))
  (CL:LET* ((POINTSTOMETAOBJECT? FALSE))
   (CL:LET*
     ...
     (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ARG)))
      (CL:COND
       ((SUBTYPE-OF? TEST-VALUE-000 SGT-INFERENCE-CACHES-LOGIC-DESCRIPTION)
        (CL:PROGN (CL:SETQ POINTSTOMETAOBJECT? TRUE)))
       ((SUBTYPE-OF? TEST-VALUE-000
         SGT-INFERENCE-CACHES-STELLA-LITERAL-WRAPPER)
        (CL:PROGN))
       (CL:T (CL:RETURN-FROM META-PROPOSITION? FALSE))))))
   POINTSTOMETAOBJECT?))
...
in the module or world `context'."
  (CL:LET*
   ...
   (CL:HANDLER-CASE
    (CL:LET* ((P NULL) (ITER-000 (ALL-PROPOSITIONS WORLDSTATE TRUE)))
...
called again."
  ...
  (CL:LET* ((OPTIONALNAME NAME) (MODULE NULL))
   (CL:WHEN (CL:NOT (CL:EQ (%%VALUE OPTIONALNAME) NULL))
    (CL:SETQ MODULE (COERCE-TO-MODULE (%%VALUE OPTIONALNAME) TRUE)))
...
updates to the module similar to `propagate-constraints'."
  ...
  (CL:LET*
   ((OPTIONALNAME NAME)
    (CONTEXT (COERCE-TO-MODULE (%%VALUE OPTIONALNAME) TRUE)))
...
called again."
  (CL:LET*
   ((MODULE (COERCE-TO-MODULE MODULEREF TRUE)) (OPTIONALARGUMENTS FORCE)
...
(CL:DEFUN STARTUP-INFERENCE-CACHES ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *INVISIBLEASSERTION?* FALSE))
...
called again.\")" (CL:FUNCTION %RUN-FORWARD-RULES)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Inference-Caches") NULL-STRING-WRAPPER)))
