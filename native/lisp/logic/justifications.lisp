...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* INCONSISTENT-TRUTH-VALUE TRUE-TRUTH-VALUE
  *CONTEXT* *MODULE* *REVERSEPOLARITY?* NIL-LIST NULL-INTEGER
  UNKNOWN-TRUTH-VALUE STANDARD-OUTPUT EOL *QUERYITERATOR*
     ...
     (CL:WHEN (CL:NOT (CL:EQ (%RESULT FRAME) NULL))
      (CL:SETQ ANTECEDENT
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%RESULT FRAME))
        SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)))
     (CL:LET* ((SELF-000 (NEW-JUSTIFICATION)))
      (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-NEGATED-FAIL)
   ...
   (CL:LET*
    ...
    (CL:LET* ((*TYPE-CHECK-STRATEGY* KWD-JUSTIFICATIONS-NONE))
     ...
     (INITIALIZE-QUERY-ITERATOR ASKQUERY (QUERY-DESCRIPTION QUERY)
      (%BINDINGS (NTH (%SOLUTIONS QUERY) SOLUTIONINDEX)) "Justifications"))
...
(CL:DEFCLASS CLASH-JUSTIFICATION (JUSTIFICATION)
  ((DIRECTION :DOCUMENTATION "The inference direction for this inference."
    :ALLOCATION :INSTANCE :ACCESSOR %DIRECTION)))
