...
proposition of this justification." :ALLOCATION :INSTANCE :ACCESSOR %INFERENCE-RULE)
   (PROPOSITION :DOCUMENTATION "The proposition supported by this justification." :ALLOCATION :INSTANCE :ACCESSOR
    %PROPOSITION)
   (ANTECEDENTS :DOCUMENTATION "Antecedents justifications of this justification." :ALLOCATION :INSTANCE :ACCESSOR
    %ANTECEDENTS)
   (SUBSTITUTION :DOCUMENTATION "List of variable bindings recorded for this justification." :ALLOCATION :INSTANCE
    :ACCESSOR %SUBSTITUTION)
   (REVERSE-POLARITY? :DOCUMENTATION "True if proposition was derived in reverse polarity." :ALLOCATION :INSTANCE
    :ACCESSOR %REVERSE-POLARITY?)
   ...
   (POSITIVE-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :DOCUMENTATION
    "Positive partial match score of the derived proposition." :ALLOCATION :INSTANCE :ACCESSOR %POSITIVE-SCORE)
   (NEGATIVE-SCORE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT :DOCUMENTATION
    "Negative partial match score of the derived proposition." :ALLOCATION :INSTANCE :ACCESSOR %NEGATIVE-SCORE)))

(CL:DEFUN NEW-JUSTIFICATION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE JUSTIFICATION)))
   (CL:SETF (%NEGATIVE-SCORE SELF) NULL-FLOAT) (CL:SETF (%POSITIVE-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%TRUTH-VALUE SELF) NULL) (CL:SETF (%REVERSE-POLARITY? SELF) CL:NIL) (CL:SETF (%SUBSTITUTION SELF) NULL)
...
(CL:DEFUN ACCESS-JUSTIFICATION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-INFERENCE-RULE)
    (CL:IF SETVALUE? (CL:SETF (%INFERENCE-RULE SELF) VALUE) (CL:SETQ VALUE (%INFERENCE-RULE SELF))))
   ...
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-ANTECEDENTS)
    (CL:IF SETVALUE? (CL:SETF (%ANTECEDENTS SELF) VALUE) (CL:SETQ VALUE (%ANTECEDENTS SELF))))
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-SUBSTITUTION)
    (CL:IF SETVALUE? (CL:SETF (%SUBSTITUTION SELF) VALUE) (CL:SETQ VALUE (%SUBSTITUTION SELF))))
...
(CL:DEFUN NEW-PRIMITIVE-STRATEGY ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE PRIMITIVE-STRATEGY)))
   (CL:SETF (%NEGATIVE-SCORE SELF) NULL-FLOAT) (CL:SETF (%POSITIVE-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%TRUTH-VALUE SELF) NULL) (CL:SETF (%REVERSE-POLARITY? SELF) CL:NIL) (CL:SETF (%SUBSTITUTION SELF) NULL)
...
(CL:DEFUN ACCESS-PRIMITIVE-STRATEGY-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-STRATEGY)
    (CL:IF SETVALUE? (CL:SETF (%STRATEGY SELF) VALUE) (CL:SETQ VALUE (%STRATEGY SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN JUSTIFICATION-PROPOSITIONS-EQL? (PROPOSITION1 JUST1 PROPOSITION2 JUST2)
  (CL:WHEN
   (CL:AND (CL:EQ (%OPERATOR PROPOSITION1) (%OPERATOR PROPOSITION2))
    (CL:= (LENGTH (%ARGUMENTS PROPOSITION1)) (LENGTH (%ARGUMENTS PROPOSITION2))))
   (CL:LET*
    ((ARG1 NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION1)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ARG2 NULL)
     (VECTOR-001 (%ARGUMENTS PROPOSITION2)) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
  ...
  (CL:LET* ((COPY NULL) (ANTECEDENTS NIL))
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF) SGT-JUSTIFICATIONS-LOGIC-PRIMITIVE-STRATEGY)
     (CL:PROGN
      (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY))) (CL:SETF (%STRATEGY SELF-000) (%STRATEGY SELF))
       (CL:SETQ COPY SELF-000))))
    (CL:T (CL:SETQ COPY (NEW-JUSTIFICATION))))
   (CL:SETF (%INFERENCE-RULE COPY) (%INFERENCE-RULE SELF)) (CL:SETF (%PROPOSITION COPY) (%PROPOSITION SELF))
   (CL:SETF (%SUBSTITUTION COPY) (%SUBSTITUTION SELF)) (CL:SETF (%REVERSE-POLARITY? COPY) (%REVERSE-POLARITY? SELF))
   (CL:SETF (%TRUTH-VALUE COPY) (%TRUTH-VALUE SELF)) (CL:SETF (%POSITIVE-SCORE COPY) (%POSITIVE-SCORE SELF))
...
(CL:DEFUN RECORD-PRIMITIVE-JUSTIFICATION (FRAME LASTMOVE)
  (CL:COND
   ((CL:OR (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE) (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL))
    (CL:LET*
     ((STRATEGY
       (CL:IF (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL) KWD-JUSTIFICATIONS-FAILURE (%CURRENT-STRATEGY FRAME)))
      (JUSTIFICATION NULL))
     (CL:COND
      ((CL:AND (CL:NOT (CL:EQ (%JUSTIFICATIONS FRAME) NULL)) (CL:NOT (EMPTY? (%JUSTIFICATIONS FRAME))))
       (CL:SETQ JUSTIFICATION (FIRST (%JUSTIFICATIONS FRAME))))
      ((CL:NOT (CL:EQ (%RESULT FRAME) NULL))
       (CL:COND
        ((CL:NOT
          (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%RESULT FRAME)) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)
           NULL))
         (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY))) (CL:SETF (%STRATEGY SELF-000) STRATEGY)
          (CL:SETF (%ANTECEDENTS SELF-000)
           (CONS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%RESULT FRAME)) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)
            NIL))
          (CL:SETQ JUSTIFICATION SELF-000)))
        ((CL:AND (CL:NOT (CL:EQ (%PROPOSITION (%RESULT FRAME)) NULL))
          (NON-EMPTY? (FORWARD-JUSTIFICATIONS (%PROPOSITION (%RESULT FRAME)))))
     ...
     (CL:WHEN (CL:EQ JUSTIFICATION NULL)
      (CL:LET* ((SELF-001 (NEW-PRIMITIVE-STRATEGY))) (CL:SETF (%STRATEGY SELF-001) STRATEGY)
...
(CL:DEFUN RECORD-PATTERN-JUSTIFICATION (FRAME LASTMOVE)
  (CL:LET*
   ((ARGUMENT (%RESULT FRAME))
    (JUSTIFICATION (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)))
   (CL:COND
    ((CL:OR (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE) (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL))
     (CL:WHEN (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ARGUMENT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NULL)
      (CL:RETURN-FROM RECORD-PATTERN-JUSTIFICATION))
     (CL:WHEN (CL:EQ JUSTIFICATION NULL)
      (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-PATTERN)
     ...
     (CL:LET*
      ((SUBSTITUTION (NEW-KEY-VALUE-MAP)) (PATTERNRECORD (%PATTERN-RECORD FRAME)) (DESCRIPTION NULL)
       (COLLECTIONFRAME? CL:NIL))
      (CL:WHEN (CL:NOT (CL:EQ PATTERNRECORD NULL))
       (CL:SETQ COLLECTIONFRAME? (CL:NOT (CL:EQ (%COLLECTION-LIST PATTERNRECORD) NULL)))
      ...
      (CL:WHEN (ISA? SUBSTITUTION SGT-JUSTIFICATIONS-STELLA-KEY-VALUE-LIST)
       (CL:LET* ((OLDSUBSTITUTION SUBSTITUTION)) (CL:SETQ SUBSTITUTION (NEW-KEY-VALUE-MAP))
        (CL:LET* ((KEY NULL) (VAL NULL) (ITER-000 (ALLOCATE-ITERATOR OLDSUBSTITUTION)))
         (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VAL (%VALUE ITER-000))
          (INSERT-AT SUBSTITUTION KEY VAL)))))
      (CL:SETF (%SUBSTITUTION JUSTIFICATION) SUBSTITUTION)
      (CL:LET*
       ((ANTECEDENT (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ARGUMENT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)))
       (CL:WHEN COLLECTIONFRAME? (CL:SETF (%INFERENCE-RULE JUSTIFICATION) KWD-JUSTIFICATIONS-AND-INTRODUCTION)
        (CL:LET* ((PATTERNCOPY NULL))
         ...
         (CL:SETF (%SUBSTITUTION PATTERNCOPY) SUBSTITUTION) (CL:SETF (%SUBSTITUTION JUSTIFICATION) NULL)
   ...
   (CL:COND
    ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
     (CL:LET*
      ...
      (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) INFERENCERULE)
       (CL:SETF (%PROPOSITION SELF-000) GOAL)
       (CL:LET* ((SELF-001 (NEW-PRIMITIVE-STRATEGY)))
        (CL:SETF (%STRATEGY SELF-001) KWD-JUSTIFICATIONS-SCAN-PROPOSITIONS) (CL:SETF (%PROPOSITION SELF-001) RULE)
        (CL:SETF (%TRUTH-VALUE SELF-001) (ACCESS-IN-CONTEXT (%TRUTH-VALUE RULE) (%HOME-CONTEXT RULE) CL:NIL))
        (CL:SETF (%POSITIVE-SCORE SELF-001)
         (%POSITIVE-SCORE (ACCESS-IN-CONTEXT (%TRUTH-VALUE RULE) (%HOME-CONTEXT RULE) CL:NIL)))
        (CL:SETF (%ANTECEDENTS SELF-000)
         (CONS SELF-001
          (CONS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ANTECEDENT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NIL))))
...
(CL:DEFUN RECORD-AND-INTRODUCTION-JUSTIFICATION (FRAME LASTMOVE)
  (CL:LET*
   ((CONJUNCTS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FRAME) SYM-JUSTIFICATIONS-STELLA-ARGUMENTS NULL)) (ANTECEDENTS NIL))
   (CL:COND
    ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
     (CL:LET*
      ...
      (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       (CL:SETQ CONJUNCT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000)) (CL:PROGN)
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN
         (CL:SETQ COLLECT-000
          (CONS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CONJUNCT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NIL))
         (CL:IF (CL:EQ ANTECEDENTS NIL) (CL:SETQ ANTECEDENTS COLLECT-000)
          (ADD-CONS-TO-END-OF-CONS-LIST ANTECEDENTS COLLECT-000)))
        (CL:PROGN
         ...
         (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
       (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
     (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-AND-INTRODUCTION)
      (CL:SETF (%ANTECEDENTS SELF-000) ANTECEDENTS) (RECORD-GOAL-JUSTIFICATION FRAME SELF-000)))
    (CL:T))))
   ...
   (CL:COND
    ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
     (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-OR-INTRODUCTION)
      (CL:SETF (%ANTECEDENTS SELF-000)
       (CONS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS DISJUNCT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NIL))
      (RECORD-GOAL-JUSTIFICATION FRAME SELF-000)))
    (CL:T))))
   ...
   (CL:COND
    ((CL:OR (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE) (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL))
     (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-DISPROOF)
      (CL:SETF (%ANTECEDENTS SELF-000)
       (CONS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ARGUMENT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NIL))
      (RECORD-GOAL-JUSTIFICATION FRAME SELF-000)))
    (CL:T))))
...
(CL:DEFUN RECORD-FAIL-JUSTIFICATION (FRAME LASTMOVE)
  (CL:COND
   ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL)
    (CL:LET* ((SELF-000 (NEW-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-FAIL-INTRODUCTION)
     (RECORD-GOAL-JUSTIFICATION FRAME SELF-000)))
   (CL:T)))
   ...
   ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
    (CL:LET* ((ANTECEDENT NULL))
     (CL:WHEN (CL:NOT (CL:EQ (%RESULT FRAME) NULL))
      (CL:SETQ ANTECEDENT
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%RESULT FRAME)) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL)))
  ...
  (CL:COND
   ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-FAIL)
    (CL:LET* ((SELF-000 (NEW-JUSTIFICATION)))
     (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-CLOSED-NOT-INTRODUCTION)
     (CL:SETF (%PROPOSITION SELF-000)
      (CL:IF (CL:EQ (%KIND (%PROPOSITION FRAME)) KWD-JUSTIFICATIONS-FAIL) (%PROPOSITION (%UP FRAME))
       (%PROPOSITION FRAME)))
     (RECORD-GOAL-JUSTIFICATION FRAME SELF-000)))
   (CL:T)))
...
(CL:DEFUN RECORD-EXISTENTIAL-INTRODUCTION-JUSTIFICATION (FRAME LASTMOVE)
  (CL:LET* ((ARGUMENT (%RESULT FRAME)))
   (CL:COND
    ((CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
     (CL:LET* ((SELF-000 (NEW-JUSTIFICATION)))
      (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-EXISTENTIAL-INTRODUCTION)
      (CL:SETF (%ANTECEDENTS SELF-000)
       (CL:IF
        (CL:NOT
         (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS ARGUMENT) SYM-JUSTIFICATIONS-LOGIC-JUSTIFICATION NULL) NULL))
  ...
  (CL:LET* ((TEST-VALUE-000 (%INFERENCE-RULE SELF)))
   (CL:COND
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-JUSTIFICATIONS-PRIMITIVE-STRATEGY)
     ...
     (CL:COND ((CL:EQ (%STRATEGY SELF) KWD-JUSTIFICATIONS-FAILURE) (CL:RETURN-FROM FAILED-GOAL-JUSTIFICATION? CL:T))
      (CL:T)))
    (CL:T)))
  CL:NIL)
...
pattern nodes, and :BRIEF additionally suppresses AND-introduction nodes."
  (CL:LET* ((*EXPLANATION-STYLE* STYLE)) (CL:DECLARE (CL:SPECIAL *EXPLANATION-STYLE*))
   ...
   (CL:LET*
    ((ANTECEDENTS
      (CL:IF (CL:EQ STYLE KWD-JUSTIFICATIONS-RAW) (%ANTECEDENTS SELF) (%THE-CONS-LIST (VISIBLE-ANTECEDENTS SELF))))
    ...
    (CL:WHEN (CL:NOT (CL:= (%POSITIVE-SCORE SELF) NULL-FLOAT))
     (INSERT-AT KEYS KWD-JUSTIFICATIONS-POSITIVE-SCORE (WRAP-FLOAT (%POSITIVE-SCORE SELF))))
     ...
     (CL:WHEN (CL:EQ THESUBSTITUTION NULL)
      (CL:LET* ((ANT NULL) (ITER-000 (%ANTECEDENTS SELF)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ANT (%%VALUE ITER-000))
        (CL:WHEN (CL:EQ (%INFERENCE-RULE ANT) KWD-JUSTIFICATIONS-PATTERN) (CL:SETQ THESUBSTITUTION (%SUBSTITUTION ANT))
         (CL:RETURN))
        (CL:SETQ ITER-000 (%%REST ITER-000)))))
...
(CL:DEFUN GET-QUERY-JUSTIFICATIONS (QUERY SOLUTIONINDEX MAXJUSTIFICATIONS CREATE?)
  ...
  (CL:LET*
   ((ASKQUERY? (QUERY-IS-TRUE-FALSE? QUERY)) (SOLUTIONS (%SOLUTIONS QUERY)) (SOLUTION NULL)
   ...
   (CL:WHEN (CL:>= SOLUTIONINDEX NOFSOLUTIONS)
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "There were only `" NOFSOLUTIONS "' solutions")
   ...
   (CL:COND
    ((CL:EQ (%JUSTIFICATION SOLUTION) NULL)
     (CL:COND
      ...
      (CL:T NIL-LIST)))
    (CL:T
     (CL:WHEN (CL:AND (CL:NOT (CL:EQ (%ALL-JUSTIFICATIONS SOLUTION) NULL)) (NON-EMPTY? (%ALL-JUSTIFICATIONS SOLUTION)))
      (CL:SETQ JUSTIFICATIONS (LIST))
      (CL:LET*
       ((JUST NULL) (ITER-000 (%THE-CONS-LIST (%ALL-JUSTIFICATIONS SOLUTION))) (IGNORE NULL-INTEGER) (ITER-001 1)
        (UPPER-BOUND-000 MAXJUSTIFICATIONS) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)) (COLLECT-000 NULL))
...
(CL:DEFUN DERIVE-QUERY-JUSTIFICATIONS (QUERY OPTIONS SOLUTIONINDEX MAXJUSTIFICATIONS)
  ...
  (CL:CHECK-TYPE MAXJUSTIFICATIONS CL:FIXNUM)
  (CL:LET*
   ((*QUERYITERATOR* QUERY) (*CONTEXT* (%QUERY-CONTEXT QUERY)) (*INFERENCELEVEL* (CURRENT-INFERENCE-LEVEL))
    (*REVERSEPOLARITY?* CL:NIL) (*GENERATE-ALL-PROOFS?* CL:NIL) (*RECORD-JUSTIFICATIONS?* CL:T))
   (CL:DECLARE
...
(CL:DEFUN NEW-FORWARD-GOAL-RECORD ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE FORWARD-GOAL-RECORD)))
   (CL:SETF (%FORWARD-RULE SELF) NULL) (CL:SETF (%FORWARD-GOAL SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-FORWARD-GOAL-RECORD-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-FORWARD-RULE)
    (CL:IF SETVALUE? (CL:SETF (%FORWARD-RULE SELF) VALUE) (CL:SETQ VALUE (%FORWARD-RULE SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFMETHOD FORWARD-CHAINING-GOALS ((SELF PROPOSITION))
  (CL:LET* ((ANSWER (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-JUSTIFICATIONS-LOGIC-FORWARD-CHAINING-GOALS NULL)))
...
(CL:DEFUN RECORD-FORWARD-GOAL (FORWARDRULE ARGUMENTS CONSEQUENTPROPOSITION)
        ...
        (CL:LET* ((ALWAYS?-000 CL:T))
         (CL:LET*
          ...
          (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
           ...
           (CL:WHEN (CL:NOT (EQL? (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS FWDGOAL))) I) ARG))
  ...
  (CL:LET* ((FORWARDGOAL (CREATE-PROPOSITION SYM-JUSTIFICATIONS-STELLA-PREDICATE (CL:1+ (LENGTH ARGUMENTS)))))
   (CL:LET* ((SELF-000 (NEW-FORWARD-GOAL-RECORD))) (CL:SETF (%FORWARD-GOAL SELF-000) FORWARDGOAL)
    ...
    (CL:LET* ((FWDGOALREC SELF-000)) (CL:SETF (%OPERATOR FORWARDGOAL) SGT-JUSTIFICATIONS-PL-KERNEL-KB-HOLDS)
     (CL:LET
      ((SELF (%THE-ARRAY (%ARGUMENTS FORWARDGOAL)))
       (VALUE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ARGUMENTS FORWARDRULE))) 0)) (POSITION 0))
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
       ...
       (CL:LET ((SELF (%THE-ARRAY (%ARGUMENTS FORWARDGOAL))) (VALUE ARG) (POSITION I))
        (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
        (CL:SETF (CL:AREF SELF POSITION) VALUE))
       (CL:SETQ INDEX-001 (CL:1+ INDEX-001)) (CL:SETQ ITER-002 (CL:1+ ITER-002))))
     (CL:IF (EMPTY? (FORWARD-CHAINING-GOALS CONSEQUENTPROPOSITION))
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CONSEQUENTPROPOSITION) SYM-JUSTIFICATIONS-LOGIC-FORWARD-CHAINING-GOALS
  ...
  (CL:LET* ((TEST-VALUE-000 CL:NIL)) (CL:SETQ TEST-VALUE-000 (PREDICATION? CONSEQUENTPROPOSITION))
   (CL:WHEN TEST-VALUE-000 (CL:SETQ TEST-VALUE-000 (ASSERTED-AS-TRUE? FORWARDRULE))
    (CL:WHEN TEST-VALUE-000
     (CL:LET* ((ALWAYS?-000 CL:T))
      (CL:LET*
       ((ARG NULL) (VECTOR-000 (%ARGUMENTS CONSEQUENTPROPOSITION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
       (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
...
(CL:DEFUN GET-FORWARD-GOALS (PROPOSITION)
  (CL:LET* ((GOALS (REMOVE-DELETED-MEMBERS (FORWARD-CHAINING-GOALS PROPOSITION))))
   (CL:WHEN
    (CL:AND (EMPTY? GOALS)
     (CL:NOT
      (CL:EQ (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION) SYM-JUSTIFICATIONS-LOGIC-MASTER-PROPOSITION NULL) NULL)))
    (CL:SETQ GOALS
     (REMOVE-DELETED-MEMBERS
      (FORWARD-CHAINING-GOALS
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION) SYM-JUSTIFICATIONS-LOGIC-MASTER-PROPOSITION NULL)))))
   GOALS))
...
(CL:DEFUN NEW-FORWARD-JUSTIFICATION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE FORWARD-JUSTIFICATION)))
   (CL:SETF (%NEGATIVE-SCORE SELF) NULL-FLOAT) (CL:SETF (%POSITIVE-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%TRUTH-VALUE SELF) NULL) (CL:SETF (%REVERSE-POLARITY? SELF) CL:NIL) (CL:SETF (%SUBSTITUTION SELF) NULL)
   (CL:SETF (%ANTECEDENTS SELF) NIL) (CL:SETF (%PROPOSITION SELF) NULL) (CL:SETF (%INFERENCE-RULE SELF) NULL) SELF))
...
(CL:DEFMETHOD FORWARD-JUSTIFICATIONS ((SELF PROPOSITION))
  (CL:LET* ((ANSWER (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-JUSTIFICATIONS-LOGIC-FORWARD-JUSTIFICATIONS NULL)))
   (CL:IF (CL:EQ ANSWER NULL) NIL-LIST ANSWER)))
...
(CL:DEFUN ADD-FORWARD-JUSTIFICATIONS (PROPOSITION JUSTIFICATION)
  (CL:LET* ((JUSTIFICATIONS (FORWARD-JUSTIFICATIONS PROPOSITION)))
   ...
   (CL:WHEN (CL:EQ JUSTIFICATIONS NIL-LIST) (CL:SETQ JUSTIFICATIONS (LIST))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS PROPOSITION) SYM-JUSTIFICATIONS-LOGIC-FORWARD-JUSTIFICATIONS JUSTIFICATIONS
     NULL))
...
(CL:DEFUN GET-RULE-IO-VARIABLES (RULE)
  (CL:LET* ((VARS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS RULE) SYM-JUSTIFICATIONS-LOGIC-IO-VARIABLES NULL)) (MASTER NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ MASTER NULL))
    (CL:SETQ VARS (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS MASTER) SYM-JUSTIFICATIONS-LOGIC-IO-VARIABLES NULL))
    (CL:WHEN (CL:NOT (CL:EQ VARS NULL)) (CL:RETURN-FROM GET-RULE-IO-VARIABLES VARS)))
   (CL:WHEN (CL:EQ (%KIND RULE) KWD-JUSTIFICATIONS-IMPLIES)
    (CL:LET*
     ((ARGUMENTS (%ARGUMENTS RULE)) (ANT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS)) 0))
      (CQ (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY ARGUMENTS)) 1)))
     (CL:IF (NAMED-DESCRIPTION? ANT) (CL:RETURN-FROM GET-RULE-IO-VARIABLES (%IO-VARIABLES CQ))
      (CL:RETURN-FROM GET-RULE-IO-VARIABLES (%IO-VARIABLES ANT)))))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "get-rule-io-variables: don't know how to access IO-vars of rule: `"
     RULE "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN CREATE-FORWARD-JUSTIFICATION (ANTECEDENTS FORWARDRULE ARGUMENTS CONSEQUENTPROPOSITION BC-JUSTIFICATION)
  (CL:LET* ((SELF-000 (NEW-FORWARD-JUSTIFICATION)))
   ...
   (CL:LET*
    ...
    (CL:SETF (%SUBSTITUTION FJ) SUBSTITUTION) (CL:SETF (%PROPOSITION FJ) CONSEQUENTPROPOSITION)
    (CL:SETF (%TRUTH-VALUE FJ)
     (ACCESS-IN-CONTEXT (%TRUTH-VALUE CONSEQUENTPROPOSITION) (%HOME-CONTEXT CONSEQUENTPROPOSITION) CL:NIL))
...
(CL:DEFUN GET-FORWARD-ANTECEDENT-JUSTIFICATION (ANTECEDENT)
  (CL:IF
   ...
   (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY))) (CL:SETF (%PROPOSITION SELF-000) ANTECEDENT)
    (CL:SETF (%STRATEGY SELF-000) KWD-JUSTIFICATIONS-LOOKUP-ASSERTIONS) (CL:LET* ((VALUE-000 SELF-000)) VALUE-000))))
   ...
   (CL:LET* ((FJ NULL) (ITER-000 (%THE-CONS-LIST JUSTIFICATIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ FJ (%%VALUE ITER-000))
     (CL:WHEN (CL:EQ (%PROPOSITION (%%VALUE (%ANTECEDENTS FJ))) FORWARDRULE)
      (CL:SETF (%SUBSTITUTION FJ) (%SUBSTITUTION NEW-FJ)) (CL:SETF (%ANTECEDENTS FJ) (%ANTECEDENTS NEW-FJ))
   ...
   (CL:WHEN
    ...
    (CL:WHEN (RELATIONREF-SPECIALIZES-RELATIONREF? MATCHING-OPERATOR MAIN-OPERATOR)
     (CL:LET* ((SUBSET-PROPOSITION NULL) (SUBSET-JUSTIFICATIONS NIL))
      ...
      (CL:LET* ((SELF-000 (NEW-PRIMITIVE-STRATEGY))) (CL:SETF (%PROPOSITION SELF-000) SUBSET-PROPOSITION)
       (CL:SETF (%TRUTH-VALUE SELF-000) TRUE-TRUTH-VALUE) (CL:SETF (%STRATEGY SELF-000) KWD-JUSTIFICATIONS-SPECIALIST)
      ...
      (CL:LET* ((SELF-002 (NEW-JUSTIFICATION)))
       (CL:SETF (%INFERENCE-RULE SELF-002) KWD-JUSTIFICATIONS-SUBSUMPTION-REASONING)
...
(CL:DEFUN NEW-CLASH-JUSTIFICATION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE CLASH-JUSTIFICATION)))
   (CL:SETF (%NEGATIVE-SCORE SELF) NULL-FLOAT) (CL:SETF (%POSITIVE-SCORE SELF) NULL-FLOAT)
   (CL:SETF (%TRUTH-VALUE SELF) NULL) (CL:SETF (%REVERSE-POLARITY? SELF) CL:NIL) (CL:SETF (%SUBSTITUTION SELF) NULL)
   (CL:SETF (%ANTECEDENTS SELF) NIL) (CL:SETF (%PROPOSITION SELF) NULL) (CL:SETF (%INFERENCE-RULE SELF) NULL)
   (CL:SETF (%DIRECTION SELF) KWD-JUSTIFICATIONS-FORWARD) SELF))
...
(CL:DEFUN ACCESS-CLASH-JUSTIFICATION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-JUSTIFICATIONS-LOGIC-DIRECTION)
    (CL:IF SETVALUE? (CL:SETF (%DIRECTION SELF) VALUE) (CL:SETQ VALUE (%DIRECTION SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN CREATE-CLASH-JUSTIFICATION (PROP ANTECEDENTS DIRECTION)
  (CL:LET* ((SELF-000 (NEW-CLASH-JUSTIFICATION))) (CL:SETF (%INFERENCE-RULE SELF-000) KWD-JUSTIFICATIONS-CLASH)
   (CL:SETF (%DIRECTION SELF-000) DIRECTION) (CL:SETF (%PROPOSITION SELF-000) PROP)
   (CL:SETF (%TRUTH-VALUE SELF-000) INCONSISTENT-TRUTH-VALUE) (CL:SETF (%ANTECEDENTS SELF-000) ANTECEDENTS)
...
pattern nodes, and :BRIEF additionally suppresses AND-introduction nodes.\" :PUBLIC? TRUE)"
   ...
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-JUSTIFICATIONS-LOGIC-STARTUP-JUSTIFICATIONS)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-JUSTIFICATIONS-STELLA-METHOD-STARTUP-CLASSNAME
...
proposition of this justification.\") (PROPOSITION :TYPE PROPOSITION :DOCUMENTATION \"The proposition supported by this justification.\") (ANTECEDENTS :TYPE (CONS OF JUSTIFICATION) :INITIALLY NIL :DOCUMENTATION \"Antecedents justifications of this justification.\") (SUBSTITUTION :TYPE (ENTITY-MAPPING OF PATTERN-VARIABLE OBJECT) :DOCUMENTATION \"List of variable bindings recorded for this justification.\") (REVERSE-POLARITY? :TYPE BOOLEAN :DOCUMENTATION \"True if proposition was derived in reverse polarity.\") (TRUTH-VALUE :TYPE TRUTH-VALUE :DOCUMENTATION \"Truth value of the derived proposition.\") (POSITIVE-SCORE :TYPE PARTIAL-MATCH-SCORE :DOCUMENTATION \"Positive partial match score of the derived proposition.\") (NEGATIVE-SCORE :TYPE PARTIAL-MATCH-SCORE :DOCUMENTATION \"Negative partial match score of the derived proposition.\")) :METHODS ((INFERENCE-STRATEGY ((SELF JUSTIFICATION)) :TYPE KEYWORD (RETURN NULL)) (INFERENCE-DIRECTION ((SELF JUSTIFICATION)) :TYPE KEYWORD (RETURN :BACKWARD))))")))
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-JUSTIFICATION-SLOT-VALUE)))
    (CL:LET*
     ((CLASS
