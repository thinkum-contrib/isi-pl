...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *CONTEXT* *MODULE* *REVERSEPOLARITY?*
  NIL-LIST NULL-INTEGER UNKNOWN-TRUTH-VALUE *QUERYITERATOR* TRUE
  *CURRENTJUSTIFICATION* TRUE-WRAPPER FALSE-WRAPPER FALSE NIL
  NULL-FLOAT))
  ...
  (CL:COND
   ((CL:OR (CL:EQ LASTMOVE KWD-JUSTIFICATIONS-UP-TRUE)
    ...
    (CL:LET*
     ((ANTECEDENTS
       (CL:IF
        (CL:AND (CL:NOT (CL:EQ (%RESULT FRAME) NULL))
         (CL:NOT
          (CL:EQ
  ...
  (CL:LET* ((TEST-VALUE-000 (%INFERENCE-RULE SELF)))
   (CL:COND
    ((CL:OR (CL:EQ TEST-VALUE-000 KWD-JUSTIFICATIONS-TIMEOUT)
      (CL:EQ TEST-VALUE-000 KWD-JUSTIFICATIONS-DEPTH-CUTOFF))
     (CL:RETURN-FROM CUTOFF-GOAL-JUSTIFICATION? TRUE))
    (CL:T)))
  FALSE)
     ...
     (CL:WHEN (CL:NOT (CL:EQ THESUBSTITUTION NULL))
      (CL:LET*
       ((BINDING NULL) (VAR NULL)
        (ITER-001 (%THE-KV-LIST THESUBSTITUTION)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NULL)) DO
        (CL:SETQ VAR (%KEY ITER-001))
...
(CL:DEFUN GET-QUERY-JUSTIFICATIONS (QUERY SOLUTIONINDEX MAXJUSTIFICATIONS CREATE?)
  ...
  (CL:LET*
   ((ASKQUERY? (QUERY-IS-TRUE-FALSE? QUERY))
    ...
    (NOFSOLUTIONS (LENGTH SOLUTIONS)) (JUSTIFICATION NULL)
    (JUSTIFICATIONS NULL))
   ...
   (CL:COND
    ...
    (CL:T
     (CL:WHEN
      ...
      (CL:LET*
       ((I NULL-INTEGER) (JUST NULL)
        (ITER-000 (%THE-CONS-LIST (%ALL-JUSTIFICATIONS SOLUTION)))
        (ITER-001 1) (UPPER-BOUND-000 MAXJUSTIFICATIONS)
       ...
       (CL:LOOP WHILE
        ...
        DO
        (CL:PROGN (CL:SETQ JUST (%%VALUE ITER-000))
         (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN DERIVE-QUERY-JUSTIFICATIONS (QUERY OPTIONS SOLUTIONINDEX MAXJUSTIFICATIONS)
  ...
  (CL:LET*
   ((*QUERYITERATOR* QUERY) (*CONTEXT* (%QUERY-CONTEXT QUERY))
    ...
    (*REVERSEPOLARITY?* FALSE) (*GENERATE-ALL-PROOFS?* FALSE)
       ...
       (CL:WHEN TEST-VALUE-000
        (CL:LET* ((ALWAYS?-000 TRUE))
         (CL:LET*
          ...
          (CL:DECLARE
           (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-001 I))
          (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
           (CL:PROGN
            (CL:SETQ ARG
             (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
...
pattern nodes, and :BRIEF additionally suppresses AND-introduction nodes.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Justifications") NULL-STRING-WRAPPER)))
