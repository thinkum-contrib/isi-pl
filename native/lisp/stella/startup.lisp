...
(CL:DEFVAR *STELLA-PATCH-LEVEL* 27)
...
(CL:DEFUN STELLA-VERSION-STRING ()
  ...
  (CL:RETURN-FROM STELLA-VERSION-STRING
   (CONCATENATE "STELLA " (INTEGER-TO-STRING (CL:TRUNCATE *STELLA-MAJOR-VERSION-NUMBER*)) "."
...
Useful when reporting problems."
  (CL:RETURN-FROM STELLA-INFORMATION
   (CONCATENATE *STELLA-VERSION-STRING* " [" (%SYMBOL-NAME (RUNNING-IN-LANGUAGE)) "]" (RUNNING-SYSTEM-INFORMATION))))
  ...
  (CL:LET* ((STARTUPTIMEPHASEASNUMBER 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM STARTUPTIMEPHASEASNUMBER))
   ...
   (CL:LET* ((PH NULL) (ITER-000 (%THE-CONS-LIST *STARTUP-TIME-PHASES*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ PH (%%VALUE ITER-000))
     (CL:IF (CL:EQ PH PHASE) (CL:RETURN-FROM ENCODE-STARTUP-TIME-PHASE STARTUPTIMEPHASEASNUMBER)
  ...
  (CL:LET* ((PHASE NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 9))
   ...
   (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ PHASE ITER-000)
    (CL:TAGBODY (CL:SETQ *STARTUP-TIME-PHASE* PHASE)
     ...
     (CL:WHEN (CL:= PHASE 8) (FINALIZE-SLOTS) (CLEANUP-UNFINALIZED-CLASSES) (CL:GO :CONTINUE)) (STARTUP-STARTUP)
     (STARTUP-CL-PRIMAL) (STARTUP-PRIMAL) (STARTUP-TYPE-PREDICATES) (STARTUP-CONSES) (STARTUP-HIERARCHY)
     (STARTUP-TAXONOMIES) (STARTUP-LISTS) (STARTUP-COLLECTIONS) (STARTUP-ITERATORS) (STARTUP-LITERALS)
     (STARTUP-SYMBOLS) (STARTUP-CLASSES) (STARTUP-METHODS) (STARTUP-DEFCLASS) (STARTUP-STELLA-IN) (STARTUP-FOREACH)
     (STARTUP-WALK) (STARTUP-DYNAMIC-SLOTS) (STARTUP-DYNAMIC-LITERAL-SLOTS) (STARTUP-SYSTEMS) (STARTUP-CL-TRANSLATE)
     (STARTUP-MACROS) (STARTUP-MEMOIZE) (STARTUP-STREAMS) (STARTUP-DESCRIBE) (STARTUP-DEMONS) (STARTUP-MORE-DEMONS)
     (STARTUP-NAME-UTILITY) (STARTUP-MODULES) (STARTUP-CONTEXTS) (STARTUP-READ) (STARTUP-XML) (STARTUP-DATE-TIME)
...
(CL:DEFUN STARTUP (VERBOSE?)
  ...
  (CL:LET* ((PHASE NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 9))
   (CL:DECLARE (CL:TYPE CL:FIXNUM PHASE ITER-000 UPPER-BOUND-000))
   (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ PHASE ITER-000) (CL:SETQ *STARTUP-TIME-PHASE* PHASE)
    (STARTUP-TRANSLATE-FILE) (STARTUP-CL-TRANSLATE-FILE) (STARTUP-CPP-TRANSLATOR) (STARTUP-JAVA-TRANSLATOR)
    (STARTUP-IDL-TRANSLATOR) (STARTUP-TOOLS) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
(CL:DEFUN CONSIFY-COMMAND-LINE-ARGUMENTS (COUNT ARGUMENTS)
  ...
  (CL:CHECK-TYPE ARGUMENTS CL:SIMPLE-VECTOR)
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- COUNT)) (COLLECT-000 NULL))
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (WRAP-STRING (CL:AREF ARGUMENTS I)) NIL))
...
(CL:DEFUN MAIN (CL:&REST %%ARGS)
  (CL:LET* ((COUNT (CL:LENGTH %%ARGS)) (ARGUMENTS (CL:MAKE-ARRAY (CL:LENGTH %%ARGS) :INITIAL-CONTENTS %%ARGS)))
   ...
   (CL:LET* ((TESTING? (CL:= COUNT 0))) (STARTUP TESTING?) (STARTUP-STELLA-SYSTEM)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Welcome to " (STELLA-VERSION-STRING) EOL)
...
(CL:DEFUN STARTUP-STARTUP ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-STARTUP-EARLY-INITS (INTERN-RIGID-SYMBOL-WRT-MODULE "EARLY-INITS" NULL 2))
    (CL:SETQ KWD-STARTUP-MODULES (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULES" NULL 2))
    (CL:SETQ KWD-STARTUP-SYMBOLS (INTERN-RIGID-SYMBOL-WRT-MODULE "SYMBOLS" NULL 2))
    (CL:SETQ KWD-STARTUP-QUOTED-CONSTANTS (INTERN-RIGID-SYMBOL-WRT-MODULE "QUOTED-CONSTANTS" NULL 2))
    (CL:SETQ KWD-STARTUP-GLOBALS (INTERN-RIGID-SYMBOL-WRT-MODULE "GLOBALS" NULL 2))
    (CL:SETQ KWD-STARTUP-CLASSES (INTERN-RIGID-SYMBOL-WRT-MODULE "CLASSES" NULL 2))
    ...
    (CL:SETQ KWD-STARTUP-FINAL (INTERN-RIGID-SYMBOL-WRT-MODULE "FINAL" NULL 2))
    (CL:SETQ KWD-STARTUP-WARN (INTERN-RIGID-SYMBOL-WRT-MODULE "WARN" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *STELLA-VERSION-STRING* (STELLA-VERSION-STRING))
    (CL:SETQ *STARTUP-TIME-PHASES*
     (LIST KWD-STARTUP-EARLY-INITS KWD-STARTUP-MODULES KWD-STARTUP-SYMBOLS KWD-STARTUP-QUOTED-CONSTANTS
      KWD-STARTUP-GLOBALS KWD-STARTUP-CLASSES KWD-STARTUP-FINALIZE-CLASSES KWD-STARTUP-METHODS
...
Useful when reporting problems.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION STELLA-INFORMATION)
    ...
    (DEFINE-FUNCTION-OBJECT "ENCODE-STARTUP-TIME-PHASE" "(DEFUN (ENCODE-STARTUP-TIME-PHASE INTEGER) ((PHASE KEYWORD)))"
     (CL:FUNCTION ENCODE-STARTUP-TIME-PHASE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-KERNEL" "(DEFUN STARTUP-KERNEL ((VERBOSE? BOOLEAN)))" (CL:FUNCTION STARTUP-KERNEL)
     NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CPP-TRANSLATOR" "(DEFUN STARTUP-CPP-TRANSLATOR ())"
     (CL:FUNCTION STARTUP-CPP-TRANSLATOR) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-JAVA-TRANSLATOR" "(DEFUN STARTUP-JAVA-TRANSLATOR ())"
     (CL:FUNCTION STARTUP-JAVA-TRANSLATOR) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-IDL-TRANSLATOR" "(DEFUN STARTUP-IDL-TRANSLATOR ())"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-STARTUP" "(DEFUN STARTUP-STARTUP () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-STARTUP)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STARTUP-STELLA-STARTUP-STARTUP)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-STARTUP-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/STELLA")))
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *STELLA-PATCH-LEVEL* INTEGER 27 :PUBLIC? TRUE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
