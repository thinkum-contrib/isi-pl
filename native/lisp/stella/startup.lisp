...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* STANDARD-ERROR STANDARD-OUTPUT EOL
  *CLASS-HIERARCHY-BOOTED?* TRUE NULL-INTEGER NIL))
...
(CL:DEFVAR *STELLA-MINOR-VERSION-NUMBER* 3)
    ...
    (CL:TAGBODY (CL:SETQ *STARTUP-TIME-PHASE* PHASE)
     (CL:WHEN VERBOSE?
      (CL:COND ((CL:= PHASE 0))
       ...
       ((CL:= PHASE 6)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Finalizing classes..."
         EOL))
       ((CL:= PHASE 7)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
         "Creating method objects..." EOL))
       ((CL:= PHASE 8)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Finalizing methods..."
         EOL))
       ((CL:= PHASE 9)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
     ...
     (STARTUP-ITERATORS) (STARTUP-LITERALS) (STARTUP-SYMBOLS) (STARTUP-CLASSES)
     (STARTUP-METHODS) (STARTUP-DEFCLASS) (STARTUP-STELLA-IN) (STARTUP-FOREACH)
     (STARTUP-WALK) (STARTUP-DYNAMIC-SLOTS) (STARTUP-DYNAMIC-LITERAL-SLOTS)
     (STARTUP-SYSTEMS) (STARTUP-CL-TRANSLATE) (STARTUP-MACROS) (STARTUP-MEMOIZE)
     (STARTUP-STREAMS) (STARTUP-DESCRIBE) (STARTUP-DEMONS) (STARTUP-MORE-DEMONS)
     (STARTUP-NAME-UTILITY) (STARTUP-MODULES) (STARTUP-CONTEXTS) (STARTUP-READ)
     (STARTUP-XML) (STARTUP-DATE-TIME) (STARTUP-DATE-TIME-PARSER) :CONTINUE)))
  :VOID)
...
(CL:DEFUN STARTUP (VERBOSE?)
  ...
  (CL:WHEN VERBOSE?
   (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Starting up translators..."
    EOL))
  (CL:LET* ((PHASE NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 9))
   (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 UPPER-BOUND-000 PHASE))
   (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:COND
      ((CL:OR (STRING-EQL? TEST-VALUE-000 "-e")
       ...
       (CL:IF (CL:= CURSOR COUNT)
        ...
        (CL:HANDLER-CASE
         (%EVALUATE (READ-S-EXPRESSION-FROM-STRING (CL:AREF ARGUMENTS CURSOR)))
         (CL:CONDITION (E)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-KERNEL"
     "(DEFUN STARTUP-KERNEL ((VERBOSE? BOOLEAN)))" (CL:FUNCTION STARTUP-KERNEL)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-JAVA-TRANSLATOR"
     "(DEFUN STARTUP-JAVA-TRANSLATOR ())" (CL:FUNCTION STARTUP-JAVA-TRANSLATOR)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP"
     "(DEFUN STARTUP ((VERBOSE? BOOLEAN)) :PUBLIC? TRUE)" (CL:FUNCTION STARTUP)
     NULL)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STARTUP-STELLA-STARTUP-STARTUP)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Startup") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *STELLA-MINOR-VERSION-NUMBER* INTEGER 3 :PUBLIC? TRUE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *STELLA-RELEASE-STATE* STRING \"\" :PUBLIC? TRUE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *STELLA-PATCH-LEVEL* INTEGER 1 :PUBLIC? TRUE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
