...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* FALSE-WRAPPER NULL-INTEGER NIL-LIST
  ONE-WRAPPER *CURRENTTRANSLATIONUNIT* *SAFETY* *STARTUP-TIME-PHASE*
  *UNFINALIZED-CLASSES* STANDARD-ERROR EOL NULL-STRING-WRAPPER
  *COMMON-LISP-MODULE* *MODULE* NIL TRUE FALSE
...
(CL:DEFUN CL-TRANSLATE-SPECIAL-TREE (TREE)
  (CL:LET*
   ((TRANSLATEDTREE (CL-TRANSLATE-LET-TREE TREE TRUE))
    (SPECIALVARS NIL))
   (CL:LET*
  ...
  (CL:LET* ((OTHERWISE? FALSE))
   ...
   (CL:LET* ((COND NULL) (ITER-000 (%%REST (%%REST TREE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETF (%%REST COND)
      (CL-TRANSLATE-LIST-OF-TREES (%%REST COND)))))
   (FIRST-SETTER TREE
       ...
       (CL:LET* ((ELT NULL) (ITER-000 BODY))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ ELT (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:WHEN (CONS? ELT) (CL:SETQ ALWAYS?-000 FALSE)
          (CL:RETURN))))
       (CL:WHEN ALWAYS?-000
        (CL:SETQ OTREE
         (CONS
...
(CL:DEFUN CL-YIELD-METHOD-TYPE-DECLARATION-TREE (METHOD)
  ...
  (CL:LET*
   ((CLTYPE
     (LOOKUP-CL-TYPE-FROM-STELLA-TYPE (%SLOT-BASE-TYPE METHOD))))
   (CL:IF (CL:NOT (CL:EQ CLTYPE NULL))
...
(CL:DEFUN CL-TRANSLATE-METHOD-PARAMETERS (METHOD)
  (CL:LET*
   ((FUNCTION? (%METHOD-FUNCTION? METHOD)) (OTREE NIL)
...
(CL:DEFUN CL-TRANSLATE-DEFINE-METHOD-UNIT (UNIT)
  (CL:LET*
   ((METHOD (%THE-OBJECT UNIT)) (METHODNAME (%SLOT-NAME METHOD))
    ...
    (BODYTREE (%CODE-REGISTER UNIT)) (DECLARATIONS NIL)
    (METHODDECLARATION NULL) (OTREE NIL))
...
(CL:DEFUN YIELD-CLOS-SLOT-TYPE-TREE (SLOT)
  (CL:LET*
   ((CLTYPE (LOOKUP-CL-TYPE-FROM-STELLA-TYPE (%SLOT-BASE-TYPE SLOT))))
   (CL:IF (CL:NOT (CL:EQ CLTYPE NULL))
...
(CL:DEFUN COMPUTE-CL-STRUCT-SLOTS (CLASS)
  (CL:LET* ((VALUE-000 NULL))
   ...
   (CL:LET*
    ...
    (CL:LET*
     ((SUPER NULL)
      (ITER-001 (%THE-CONS-LIST (CLASS-NATIVE-SUPERS CLASS))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
    ...
    (CL:LET*
     ((SLOT NULL)
      (ITER-003 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS)))
      (COLLECT-001 NULL))
