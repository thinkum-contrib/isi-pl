...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NIL *PRINTREADABLY?* FALSE
  NULL-FLOAT NULL-INTEGER PI))
  ...
  (CL:LET* ((JUL 0) (JA 0) (JY 0) (JM 0))
   ...
   (CL:WHEN (CL:> DD 31)
    (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "Day `" DD
      "' must be <= 31")
...
(CL:DEFCLASS DECODED-DATE-TIME (STANDARD-OBJECT)
  ((DATE-TIME-MILLIS :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT
    :ALLOCATION :INSTANCE :ACCESSOR %DATE-TIME-MILLIS)
   ...
   (DATE-TIME-DAY :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION
    :INSTANCE :ACCESSOR %DATE-TIME-DAY)
   ...
   (DATE-TIME-ZONE :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT
    :ALLOCATION :INSTANCE :ACCESSOR %DATE-TIME-ZONE)
...
(CL:DEFUN NEW-DECODED-DATE-TIME ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%DATE-TIME-YEAR SELF) 1900)
   (CL:SETF (%DATE-TIME-MONTH SELF) 1)
   (CL:SETF (%DATE-TIME-DAY SELF) 1) (CL:SETF (%DATE-TIME-HOUR SELF) 0)
   (CL:SETF (%DATE-TIME-MINUTE SELF) 0)
   (CL:SETF (%DATE-TIME-SECOND SELF) 0)
...
(CL:DEFUN SET-CURRENT-TIME (VALUES-STRUCTURE)
  (CL:LET*
   ...
   (CL:PROGN (CL:SETQ YEAR YEAR) (CL:SETQ MON MON) (CL:SETQ DAY DAY)
    (CL:SETQ DOW DOW))
   (CL:SETF (%DATE-TIME-MILLIS VALUES-STRUCTURE)
...
(CL:DEFCLASS DECODED-TIME-DURATION (STANDARD-OBJECT)
  ((DURATION-MILLIS :TYPE CL:DOUBLE-FLOAT :INITFORM NULL-FLOAT
    :ALLOCATION :INSTANCE :ACCESSOR %DURATION-MILLIS)
   ...
   (DURATION-DAYS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION
    :INSTANCE :ACCESSOR %DURATION-DAYS))
  (:DOCUMENTATION
...
(CL:DEFUN NEW-DECODED-TIME-DURATION ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%DURATION-MINUTES SELF) 0)
   (CL:SETF (%DURATION-SECONDS SELF) 0)
...
(CL:DEFCLASS CALENDAR-DATE (DATE-TIME-OBJECT)
  ((MODIFIED-JULIAN-DAY :TYPE CL:FIXNUM :INITFORM NULL-INTEGER
    :ALLOCATION :INSTANCE :ACCESSOR %MODIFIED-JULIAN-DAY)
   (TIME-MILLIS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION
    :INSTANCE :ACCESSOR %TIME-MILLIS))
  (:DOCUMENTATION
...
(CL:DEFUN ACCESS-CALENDAR-DATE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-DATE-TIME-STELLA-TIME-MILLIS)
    (CL:IF SETVALUE?
     (CL:SETF (%TIME-MILLIS SELF) (%WRAPPER-VALUE VALUE))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LET*
     ((YEAR-STRING
       (CL:IF (CL:< YEAR 0)
        (CONCATENATE (INTEGER-TO-STRING (CL:- 0 YEAR)) "BC")
      ...
      (TIMEZONE-STRING
       (CL:IF INCLUDE-TIMEZONE?
        ...
        "")))
     (CL:DECLARE
      (CL:TYPE CL:SIMPLE-STRING YEAR-STRING TIMEZONE-STRING))
     (CL:RETURN-FROM CALENDAR-DATE-TO-STRING
      (CONCATENATE YEAR-STRING "-"
       (%WRAPPER-VALUE
        (CL:AREF
         (CL:THE CL:SIMPLE-VECTOR
          (%THE-ARRAY *MONTH-ABBREVIATION-VECTOR*))
         MONTH))
       "-"
       (FORMAT-WITH-PADDING (INTEGER-TO-STRING DAY) 2 #\0
        KWD-DATE-TIME-RIGHT FALSE)
       " " (INTEGER-TO-STRING HOURS) ":"
       (FORMAT-WITH-PADDING (INTEGER-TO-STRING MINUTES) 2 #\0
        KWD-DATE-TIME-RIGHT FALSE)
       ":"
       (FORMAT-WITH-PADDING (INTEGER-TO-STRING SECONDS) 2 #\0
        KWD-DATE-TIME-RIGHT FALSE)
       "."
       (FORMAT-WITH-PADDING (INTEGER-TO-STRING MILLI) 3 #\0
        KWD-DATE-TIME-RIGHT FALSE)
       TIMEZONE-STRING))))))
(CL:DECLAIM
...
(CL:DEFUN CALENDAR-DATE-TO-NATIVE-DATE-TIME (DATE)
  (CL:LET*
   ((MJD (%MODIFIED-JULIAN-DAY DATE)) (MILLIS (%TIME-MILLIS DATE)))
...
(CL:DEFUN NEW-TIME-DURATION ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%MILLIS SELF) NULL-INTEGER)
   (CL:SETF (%DAYS SELF) NULL-INTEGER)
   ...
   (CL:IF (CL:< N-DAYS 0)
    (CL:IF (CL:<= N-MILLIS 0)
     (CL:RETURN-FROM TIME-DURATION-TO-STRING
      (CONCATENATE "minus " (INTEGER-TO-STRING (CL:- 0 N-DAYS))
       " days; " (INTEGER-TO-STRING (CL:- 0 N-MILLIS)) " ms"))
     (CL:RETURN-FROM TIME-DURATION-TO-STRING
      (CONCATENATE "minus " (INTEGER-TO-STRING (CL:- 0 (CL:1+ N-DAYS)))
       " days; " (INTEGER-TO-STRING (CL:- MILLIS-PER-DAY N-MILLIS))
       " ms")))
    (CL:IF (CL:< N-MILLIS 0)
     (CL:IF (CL:> N-DAYS 0)
      (CL:RETURN-FROM TIME-DURATION-TO-STRING
       (CONCATENATE "plus " (INTEGER-TO-STRING (CL:1- N-DAYS))
        " days; " (INTEGER-TO-STRING (CL:+ MILLIS-PER-DAY N-MILLIS))
        " ms"))
      (CL:RETURN-FROM TIME-DURATION-TO-STRING
  ...
  (CL:LET*
   ...
   (CL:WHEN (DEFINED? MS-START-POSITION)
    ...
    (CL:WHEN (DEFINED? MS-END-POSITION)
     (CL:SETQ N-MILLIS
      (STRING-TO-INTEGER
       (SUBSEQUENCE DURATION MS-START-POSITION
...
(CL:DEFUN MAKE-TIME-DURATION (DAYS MILLIS)
  ...
  (CL:LET* ((SELF-000 (NEW-TIME-DURATION)))
   (CL:SETF (%DAYS SELF-000) DAYS) (CL:SETF (%MILLIS SELF-000) MILLIS)
   (CL:LET* ((VALUE-000 SELF-000))
  ...
  (CL:COND
   ((CL:= TIMEZONE 0.0d0)
    (CL:RETURN-FROM GET-TIME
     (DECODE-TIME-IN-MILLIS (%TIME-MILLIS DATE))))
   ((CL:< TIMEZONE 0.0d0)
    (CL:RETURN-FROM GET-TIME
     (DECODE-TIME-IN-MILLIS
      (CL:THE CL:FIXNUM
   ...
   (CL:LET* ((SELF-000 (NEW-DECODED-TIME-DURATION)))
    ...
    (CL:SETF (%DURATION-MINUTES SELF-000)
     (CL:LET
      ((X
        (CL:THE CL:FIXNUM
         (CL:REM (CL:THE CL:FIXNUM (%MILLIS DURATION))
          MILLIS-PER-HOUR)))
       (Y 60000))
      (CL:DECLARE (CL:TYPE CL:FIXNUM X Y))
      (CL:THE CL:FIXNUM (CL:VALUES (CL:TRUNCATE X Y)))))
    ...
    (CL:SETF (%DURATION-MILLIS SELF-000)
     (CL:FLOAT
      (CL:THE CL:FIXNUM
       (CL:REM (CL:THE CL:FIXNUM (%MILLIS DURATION)) 1000))
      0.0d0))
    (CL:LET* ((VALUE-000 SELF-000))
        ...
        ((SUBTYPE-OF? TEST-VALUE-000
         ...
         (CL:PROGN
          (CL:RETURN-FROM TIME-ADD
           (MAKE-CALENDAR-DATE
            (CL:+ (%MODIFIED-JULIAN-DAY T1) (%DAYS T2))
       ...
       (CL:COND
        ((SUBTYPE-OF? TEST-VALUE-001
         ...
         (CL:PROGN
          (CL:RETURN-FROM TIME-ADD
           (MAKE-CALENDAR-DATE
            (CL:+ (%DAYS T1) (%MODIFIED-JULIAN-DAY T2))
       ...
       (CL:COND
        ...
        ((SUBTYPE-OF-FLOAT? TEST-VALUE-000)
         (CL:PROGN
          (CL:RETURN-FROM TIME-MULTIPLY
           (MAKE-TIME-DURATION
            (ROUND (CL:* (%DAYS T1) (%WRAPPER-VALUE T2)))
       ...
       (CL:COND
        ...
        ((SUBTYPE-OF? TEST-VALUE-002
         ...
         (CL:PROGN
          (CL:RETURN-FROM TIME-MULTIPLY
           (MAKE-TIME-DURATION
            (ROUND (CL:* (%WRAPPER-VALUE T1) (%DAYS T2)))
     ...
     (CL:PROGN
      (CL:RETURN-FROM TIME-DIVIDE
       (WRAP-FLOAT
        (CL:/
         (CL:+ (%DAYS T1)
          (CL:/ (CL:FLOAT (%MILLIS T1) 0.0d0) MILLIS-PER-DAY))
         (CL:+ (%DAYS T2)
     ...
     (CL:PROGN
      (CL:RETURN-FROM TIME-DIVIDE
       (MAKE-TIME-DURATION
        ...
        (ROUND
         (CL:/ (CL:FLOAT (%MILLIS T1) 0.0d0) (%WRAPPER-VALUE T2)))))))
    ((SUBTYPE-OF-FLOAT? TEST-VALUE-000)
     (CL:PROGN
      (CL:RETURN-FROM TIME-DIVIDE
       (MAKE-TIME-DURATION
   ...
   (LIST* SYM-DATE-TIME-STELLA-DEFUN
    ...
    (LIST*
     (LIST* SYM-DATE-TIME-STELLA-T1
      SYM-DATE-TIME-STELLA-DATE-TIME-OBJECT NIL)
     (LIST* SYM-DATE-TIME-STELLA-T2
      SYM-DATE-TIME-STELLA-DATE-TIME-OBJECT NIL)
     NIL)
      ...
      (LIST* SYM-DATE-TIME-STELLA-TYPECASE SYM-DATE-TIME-STELLA-T2
       ...
       (LIST* SYM-DATE-TIME-STELLA-TIME-DURATION
        (LIST* SYM-DATE-TIME-STELLA-ERROR
         ...
         SYM-DATE-TIME-STELLA-T1 (WRAP-STRING "  ")
         SYM-DATE-TIME-STELLA-T2 NIL)
        NIL)
       NIL)
      NIL)
         ...
         (CL:PROGN
          (CL:IF
           ...
           (CL:RETURN-FROM TIME-EQL?
            (CL:= (%TIME-MILLIS T1) (%TIME-MILLIS T2)))
           (CL:RETURN-FROM TIME-EQL?
            (CL:= (%MODIFIED-JULIAN-DAY T1)
             (%MODIFIED-JULIAN-DAY T2))))))
        ((SUBTYPE-OF? TEST-VALUE-000
          SGT-DATE-TIME-STELLA-TIME-DURATION)
         (CL:PROGN
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
...
(CL:DEFMETHOD OBJECT-EQL? ((SELF CALENDAR-DATE) X)
  (CL:RETURN-FROM OBJECT-EQL?
   (CL:AND (ISA? X SGT-DATE-TIME-STELLA-CALENDAR-DATE)
    (TIME-EQL? SELF X))))
...
(CL:DEFMETHOD OBJECT-EQL? ((SELF TIME-DURATION) X)
  (CL:RETURN-FROM OBJECT-EQL?
   (CL:AND (ISA? X SGT-DATE-TIME-STELLA-TIME-DURATION)
    (TIME-EQL? SELF X))))
...
(CL:DEFUN STARTUP-DATE-TIME ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:LET*
     ((ABBREVIATION NULL) (I NULL-INTEGER) (ITER-032 0)
      (UPPER-BOUND-033 12)
      (ITER-034
       (GET-QUOTED-TREE
