...
(CL:DECLAIM
  ...
  *TRANSLATIONUNITS* NULL-STRING-WRAPPER FALSE-WRAPPER *CONTEXT* *STELLA-MODULE*
  *CURRENT-STELLA-FEATURES* *MODULE* FALSE EOL *TRANSLATIONVERBOSITYLEVEL*
  *CURRENT-STREAM* NIL TRUE *TRANSLATOROUTPUTLANGUAGE* STANDARD-OUTPUT))
  ...
  (CL:LET*
   ((BAREFILE
     (JAVA-TRANSLATE-CLASS-NAME
      (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%CLASS-TYPE SOURCE))
       (%HOME-CONTEXT (%CLASS-TYPE SOURCE)) TRUE))))
   (CL:RETURN-FROM JAVA-MAKE-CODE-OUTPUT-FILE-NAME
  ...
  (CL:LET* ((FUNCTIONS NIL) (GLOBALS NIL))
   (CL:LET* ((MODULE NULL) (ITER-000 CODEMODULELIST))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE MODULE NULL GLOBALS
      FUNCTIONS))))
  :VOID)

;;; (DEFUN JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE ...)

(CL:DEFUN JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE (MODULE CLASSUNIT GLOBALS FUNCTIONS)
  (CL:LET*
   ((CLASSOUTPUTFILE STELLA::NULL-STRING) (CLASSOUTPUTSTREAM NULL)
    (TRANSLATION NIL) (CLASS NULL) (CLASSTRANSLATION NIL))
   (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
    (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:SETQ CLASSOUTPUTFILE (JAVA-MAKE-GLOBAL-OUTPUT-FILE-NAME MODULE FALSE))
    (CL:SETQ CLASSOUTPUTSTREAM (NEW-OUTPUT-FILE-STREAM CLASSOUTPUTFILE))
    (CL:WHEN (CL:NOT (CL:EQ CLASSUNIT NULL))
     (CL:SETQ CLASS (%THE-OBJECT CLASSUNIT))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Translating `" CLASS
      "'..." EOL)
     (CL:SETQ CLASSTRANSLATION
      (%%REST (JAVA-TRANSLATE-DEFINE-NATIVE-CLASS CLASS))))
    (CL:LET* ((*CURRENT-STREAM* CLASSOUTPUTSTREAM))
     ...
     (CL:WHEN (CL:NOT (CL:EQ CLASSUNIT NULL))
      (JAVA-OUTPUT-CLASS-CONSTRUCTORS (NTH CLASSTRANSLATION 7)
       (JAVA-YIELD-FLOTSAM-CLASS-NAME MODULE)
       (CL:AND (CL:NOT (CL:EQ CLASS NULL)) (EXCEPTION-CLASS? CLASS)))
     ...
     (CL:LET* ((FUNCTION NULL) (ITER-002 FUNCTIONS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       (CL:PROGN (CL:SETQ FUNCTION (%%VALUE ITER-002))
...
(CL:DEFUN JAVA-OUTPUT-FILE-HEADER (STREAM FILENAME)
  ...
  (CL:LET*
   ...
   (CL:IF (CL:EQ *MODULE* *STELLA-MODULE*)
    ...
    (CL:PROGN
     (CL:WHEN
      (CL:AND (CL:EQ (GET-STELLA-CLASS "NATIVE" FALSE) NULL)
       (CL:NOT (INHERITED-CLASS-NAME-CONFLICTS? "NATIVE")))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "import " (JAVA-STELLA-PACKAGE)
       ".javalib.Native;" EOL))
     (CL:WHEN
      (CL:AND (CL:EQ (GET-STELLA-CLASS "STELLA-SPECIAL-VARIABLE" FALSE) NULL)
      ...
      (CL:WHEN
       ...
       (CL:LET*
        ...
        (CL:SETQ CLASSOUTPUTFILE
         (JAVA-MAKE-CODE-OUTPUT-FILE-NAME (%WRAPPER-VALUE CLASSNAME) FALSE))
        (CL:SETQ CLASSOUTPUTSTREAM (NEW-OUTPUT-FILE-STREAM CLASSOUTPUTFILE))
        ...
        (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Writing `"
          CLASSOUTPUTFILE "'..." EOL))
        (JAVA-OUTPUT-FILE-HEADER CLASSOUTPUTSTREAM
         (JAVA-MAKE-CODE-OUTPUT-FILE-NAME (%WRAPPER-VALUE CLASSNAME) TRUE))
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "public class "
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     ...
     (CL:IF (JAVA-CLASS-UNIT-DEFINES-FLOTSAM-CLASS? CLASSUNIT)
      (CL:LET*
       ...
       (CL:WHEN (CL:EQ FUNCTIONS NULL) (CL:SETQ FUNCTIONS NIL))
       (JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE MODULE CLASSUNIT GLOBALS
        FUNCTIONS)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE"
     "(DEFUN JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE ((MODULE MODULE) (CLASSUNIT TRANSLATION-UNIT) (GLOBALS (CONS OF TRANSLATION-UNIT)) (FUNCTIONS (CONS OF TRANSLATION-UNIT))))"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-JAVA-TRANSLATE-FILE-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Java-Translate-File") NULL-STRING-WRAPPER)))
