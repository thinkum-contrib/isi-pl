...
(CL:DECLAIM
 (CL:SPECIAL TRUE-WRAPPER *JAVA-INDENT-CHARS* *JAVA-STELLA-PACKAGE-MAPPING* *TRANSLATIONUNITS* NULL-STRING-WRAPPER
  FALSE-WRAPPER *CONTEXT* *STELLA-MODULE* *CURRENT-STELLA-FEATURES* EOL *TRANSLATIONVERBOSITYLEVEL* *CURRENT-STREAM*
  NIL *MODULE* *TRANSLATOROUTPUTLANGUAGE* STANDARD-OUTPUT))
...
(CL:DEFUN %JPTRANS (STATEMENT)
  ...
  (CL:LET* ((*TRANSLATOROUTPUTLANGUAGE* *TRANSLATOROUTPUTLANGUAGE*) (*CURRENT-STREAM* STANDARD-OUTPUT))
   ...
   (%SET-TRANSLATOR-OUTPUT-LANGUAGE KWD-JAVA-TRANSLATE-FILE-JAVA) (INCREMENTALLY-TRANSLATE STATEMENT))
  :VOID)
  ...
  (CL:IF DONTTRUNCATE?
   (CL:LET* ((*DONTTRUNCATEFILENAMES?* CL:T)) (CL:DECLARE (CL:SPECIAL *DONTTRUNCATEFILENAMES?*))
    (CL:RETURN-FROM JAVA-MAKE-CODE-OUTPUT-FILE-NAME
     (MAKE-FILE-NAME-FROM-RELATIVE-PATH (WRAP-STRING BAREFILE) KWD-JAVA-TRANSLATE-FILE-JAVA)))
   (CL:RETURN-FROM JAVA-MAKE-CODE-OUTPUT-FILE-NAME
    (MAKE-FILE-NAME-FROM-RELATIVE-PATH (WRAP-STRING BAREFILE) KWD-JAVA-TRANSLATE-FILE-JAVA))))
...
(CL:DEFUN JAVA-MAKE-GLOBAL-OUTPUT-FILE-NAME (MODULE DONTTRUNCATE?)
  (CL:RETURN-FROM JAVA-MAKE-GLOBAL-OUTPUT-FILE-NAME
   (JAVA-MAKE-CODE-OUTPUT-FILE-NAME (JAVA-YIELD-FLOTSAM-CLASS-NAME MODULE) DONTTRUNCATE?)))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ MODULE (%%VALUE ITER-000))
     (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
      ...
      (CL:WHEN (MEMBER? FLOTSAM-FILES (WRAP-STRING FILENAME))
       (CL:WARN "Overwriting Flotsam file `~A'.  This is surely bad." FILENAME))
      (CL:SETQ FLOTSAM-FILES (CONS (WRAP-STRING FILENAME) FLOTSAM-FILES)) (CL:SETQ GLOBALS (LOOKUP GLOBALSHT MODULE))
      (CL:WHEN (CL:EQ GLOBALS NULL) (CL:SETQ GLOBALS NIL)) (CL:SETQ FUNCTIONS (LOOKUP FUNCTIONHT MODULE))
       ...
       (CL:PROGN (CL:SETQ CLASSOUTPUTSTREAM (OPEN-OUTPUT-FILE CLASSOUTPUTFILE))
        ...
        (CL:LET* ((*CURRENT-STREAM* CLASSOUTPUTSTREAM)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
         ...
         (CL:LET* ((GLOBAL NULL) (ITER-000 GLOBALS))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ GLOBAL (%%VALUE ITER-000))
           (CL:SETQ TRANSLATION (JAVA-TRANSLATE-UNIT GLOBAL)) (JAVA-OUTPUT-GLOBAL-DEFINITION (%%REST TRANSLATION))
           (CL:SETF (%TRANSLATION GLOBAL) NULL) (CL:SETF (%CODE-REGISTER GLOBAL) NULL)
           (CL:SETQ ITER-000 (%%REST ITER-000))))
         (CL:WHEN (CL:NOT (CL:EQ CLASSUNIT NULL))
          ...
          (CL:LET* ((STATEMENT NULL) (ITER-001 (NTH CLASSTRANSLATION 8)))
           (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ STATEMENT (%%VALUE ITER-001))
            (JAVA-OUTPUT-STATEMENT (JAVA-TRANSLATE-UNIT STATEMENT)) (CL:SETF (%TRANSLATION STATEMENT) NULL)
            (CL:SETF (%CODE-REGISTER STATEMENT) NULL) (CL:SETQ ITER-001 (%%REST ITER-001)))))
         (CL:LET* ((FUNCTION NULL) (ITER-002 FUNCTIONS))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ FUNCTION (%%VALUE ITER-002))
           (JAVA-OUTPUT-METHOD (%%REST (JAVA-TRANSLATE-UNIT FUNCTION))) (CL:SETF (%TRANSLATION FUNCTION) NULL)
           (CL:SETF (%CODE-REGISTER FUNCTION) NULL) (CL:SETQ ITER-002 (%%REST ITER-002))))
...
(CL:DEFUN JAVA-OUTPUT-FILE-HEADER (STREAM FILENAME)
  ...
  (CL:WHEN (CL:NOT (CL:EQ FILENAME STELLA::NULL-STRING))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "// " (FILE-NAME-WITHOUT-DIRECTORY FILENAME) EOL EOL))
  (CL:LET* ((PACKAGE-NAME (JAVA-PACKAGE-PREFIX *MODULE* ".")) (IMPORTED-PACKAGES NIL) (NAME STELLA::NULL-STRING))
   ...
   (CL:IF (CL:EQ *MODULE* *STELLA-MODULE*)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "import " (JAVA-STELLA-PACKAGE) ".javalib.*;" EOL)
    (CL:PROGN
     (CL:WHEN
      (CL:AND (CL:EQ (GET-STELLA-CLASS "NATIVE" CL:NIL) NULL) (CL:NOT (INHERITED-CLASS-NAME-CONFLICTS? "NATIVE")))
    ...
    (CL:LET* ((MODULE NULL) (ITER-000 (%THE-CONS-LIST (%USES *MODULE*))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ MODULE (%%VALUE ITER-000))
      ...
      (CL:WHEN (CL:NOT (CL:OR (STRING-EQL? NAME PACKAGE-NAME) (MEMBER? IMPORTED-PACKAGES (WRAP-STRING NAME))))
       ...
       (CL:SETQ IMPORTED-PACKAGES (CONS (WRAP-STRING NAME) IMPORTED-PACKAGES)))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
      ...
      (CL:UNWIND-PROTECT
       (CL:PROGN (CL:SETQ CLASSOUTPUTSTREAM (OPEN-OUTPUT-FILE CLASSOUTPUTFILE))
        (CL:LET* ((*CURRENT-STREAM* CLASSOUTPUTSTREAM)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
         ...
         (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "    Writing `" CLASSOUTPUTFILE "'..." EOL))
         (JAVA-OUTPUT-FILE-HEADER CLASSOUTPUTSTREAM (JAVA-MAKE-CODE-OUTPUT-FILE-NAME CLASS CL:T))
...
(CL:DEFUN JAVA-FLOTSAM-FUNCTION? (METHOD)
  (CL:RETURN-FROM JAVA-FLOTSAM-FUNCTION?
   (CL:AND
    (CL:NOT
     (%WRAPPER-VALUE
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD) SYM-JAVA-TRANSLATE-FILE-STELLA-METHOD-CONSTRUCTOR? FALSE-WRAPPER)))
    (CL:NOT
     (%WRAPPER-VALUE
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD) SYM-JAVA-TRANSLATE-FILE-STELLA-METHOD-NATIVE? FALSE-WRAPPER)))
    (CL:NOT (METHOD-STARTUP-FUNCTION? METHOD))
    (CL:OR (ZERO-ARGUMENT-FUNCTION? METHOD) (JAVA-METHOD-OBJECT-DEFINED-ON-NATIVE-TYPE? METHOD)
  ...
  (CL:LET* ((CLASSOUTPUTFILE STELLA::NULL-STRING) (STARTUPFUNCTIONS NULL))
   (CL:LET* ((*CURRENT-STREAM* NULL)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
    (CL:LET* ((CLASSNAME NULL) (ITER-000 KEYLIST))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ CLASSNAME (%%VALUE ITER-000))
      ...
      (CL:WHEN (CL:AND (CL:NOT (CL:EQ STARTUPFUNCTIONS NULL)) (CL:NOT (CL:EQ STARTUPFUNCTIONS NULL)))
       (CL:LET* ((*MODULE* (HOME-MODULE (%%VALUE STARTUPFUNCTIONS))) (*CONTEXT* *MODULE*))
        ...
        (CL:LET* ((CLASSOUTPUTSTREAM NULL))
         (CL:UNWIND-PROTECT
          (CL:PROGN (CL:SETQ CLASSOUTPUTSTREAM (OPEN-OUTPUT-FILE CLASSOUTPUTFILE))
           ...
           (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "public class " (%WRAPPER-VALUE CLASSNAME) " {" EOL)
           (JAVA-BUMP-INDENT)
           (CL:LET* ((FUNCTION NULL) (ITER-001 STARTUPFUNCTIONS))
            (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ FUNCTION (%%VALUE ITER-001))
             (JAVA-OUTPUT-METHOD (%%REST (JAVA-TRANSLATE-UNIT FUNCTION))) (CL:SETF (%TRANSLATION FUNCTION) NULL)
             (CL:SETF (%CODE-REGISTER FUNCTION) NULL) (CL:SETQ ITER-001 (%%REST ITER-001))))
           (JAVA-UNBUMP-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "}" EOL))
          (CL:WHEN (CL:NOT (CL:EQ CLASSOUTPUTSTREAM NULL)) (FREE CLASSOUTPUTSTREAM))))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))))
  :VOID)
...
(CL:DEFUN JAVA-PUSH-INTO-HASH-TABLE (HT KEY OBJECT)
  (CL:LET* ((VALUE (LOOKUP HT KEY)))
   (CL:IF (CL:NOT (CL:EQ VALUE NULL)) (INSERT-AT HT KEY (CONS OBJECT VALUE)) (INSERT-AT HT KEY (CONS OBJECT NIL))))
  :VOID)
...
(CL:DEFUN JAVA-OUTPUT-ALL-UNITS-TO-FILE ()
  (CL:LET*
   ((STARTUPCLASSNAME NULL) (STARTUPHT (NEW-STRING-HASH-TABLE)) (STARTUPCLASSLIST NIL) (METHODS NIL)
    (FLOTSAMFUNCTIONHT (NEW-HASH-TABLE)) (GLOBALSHT (NEW-HASH-TABLE)) (CODEMODULESLIST (LIST)) (VERBATIMSTATEMENTS NIL)
    (CLASSES NIL) (CODEOUTPUTMODULE NULL))
        ...
        (CL:COND
         ...
         ((METHOD-STARTUP-FUNCTION? (%THE-OBJECT UNIT))
            ...
            (WRAP-STRING
             (%WRAPPER-VALUE
              (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%THE-OBJECT UNIT))
               SYM-JAVA-TRANSLATE-FILE-STELLA-METHOD-STARTUP-CLASSNAME NULL-STRING-WRAPPER)))))
          ...
          (CL:WHEN (CL:NOT (MEMBER? STARTUPCLASSLIST STARTUPCLASSNAME))
           (CL:SETQ STARTUPCLASSLIST (CONS STARTUPCLASSNAME STARTUPCLASSLIST))))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ CLASSUNIT (%%VALUE ITER-001))
     (CL:IF (JAVA-CLASS-UNIT-DEFINES-FLOTSAM-CLASS? CLASSUNIT)
      (CL:LET* ((MODULE (HOME-MODULE (%THE-OBJECT CLASSUNIT))) (GLOBALS NIL) (FUNCTIONS NIL))
       (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
        (CL:SETQ GLOBALS (LOOKUP GLOBALSHT MODULE)) (CL:WHEN (CL:EQ GLOBALS NULL) (CL:SETQ GLOBALS NIL))
        (CL:SETQ FUNCTIONS (LOOKUP FLOTSAMFUNCTIONHT MODULE)) (CL:WHEN (CL:EQ FUNCTIONS NULL) (CL:SETQ FUNCTIONS NIL))
        (JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE-FOR-MODULE (JAVA-MAKE-GLOBAL-OUTPUT-FILE-NAME MODULE CL:NIL) MODULE
         CLASSUNIT GLOBALS FUNCTIONS)
        (REMOVE CODEMODULESLIST MODULE)))
      (JAVA-OUTPUT-CLASS-UNIT-TO-FILE CLASSUNIT))
     (CL:SETQ ITER-001 (%%REST ITER-001))))
   (JAVA-OUTPUT-FLOTSAM-UNITS-TO-FILE GLOBALSHT FLOTSAMFUNCTIONHT (%THE-CONS-LIST CODEMODULESLIST))
...
(CL:DEFUN JAVA-TRANSLATE-WALKED-SYSTEM-UNITS (SYSTEMUNITS)
  (CL:LET* ((*TRANSLATIONUNITS* (CONCATENATE-SYSTEM-UNITS SYSTEMUNITS))) (CL:DECLARE (CL:SPECIAL *TRANSLATIONUNITS*))
   (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Generating Java translations..." EOL))
...
(CL:DEFUN JAVA-TRANSLATE-SYSTEM (SYSTEMNAME)
  ...
  (%TRANSLATE-SYSTEM SYSTEMNAME
   (CONS-LIST KWD-JAVA-TRANSLATE-FILE-JAVA KWD-JAVA-TRANSLATE-FILE-TWO-PASS? TRUE-WRAPPER
    KWD-JAVA-TRANSLATE-FILE-FORCE-TRANSLATION? TRUE-WRAPPER KWD-JAVA-TRANSLATE-FILE-PRODUCTION-SETTINGS? TRUE-WRAPPER))
  :VOID)
...
(CL:DEFUN STARTUP-JAVA-TRANSLATE-FILE ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-JAVA-TRANSLATE-FILE-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (DEFINE-FUNCTION-OBJECT "CLT" "(DEFUN CLT ())" (CL:FUNCTION CLT) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-CLASS-TO-FILE" "(DEFUN JAVA-OUTPUT-CLASS-TO-FILE ((CLASS CLASS)))"
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-FLOTSAM-FUNCTION?" "(DEFUN (JAVA-FLOTSAM-FUNCTION? BOOLEAN) ((METHOD METHOD-SLOT)))"
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-ALL-UNITS-TO-FILE" "(DEFUN JAVA-OUTPUT-ALL-UNITS-TO-FILE ())"
     (CL:FUNCTION JAVA-OUTPUT-ALL-UNITS-TO-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-INITIALIZE-FILE-TRANSLATION" "(DEFUN JAVA-INITIALIZE-FILE-TRANSLATION ())"
     (CL:FUNCTION JAVA-INITIALIZE-FILE-TRANSLATION) NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-TRANSLATE-FILE" "(DEFUN JAVA-TRANSLATE-FILE ((FILENAME FILE-NAME)) :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-JAVA-TRANSLATE-FILE" "(DEFUN STARTUP-JAVA-TRANSLATE-FILE () :PUBLIC? TRUE)"
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-JAVA-TRANSLATE-FILE-STELLA-STARTUP-JAVA-TRANSLATE-FILE)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-JAVA-TRANSLATE-FILE-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "STELLA")))
    ...
    (REGISTER-NATIVE-NAME SYM-JAVA-TRANSLATE-FILE-STELLA-JPTRANS KWD-JAVA-TRANSLATE-FILE-COMMON-LISP
