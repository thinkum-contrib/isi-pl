...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-FLOAT NULL-INTEGER *CHARACTER-TYPE-TABLE* NULL-CHARACTER *SAFETY*
...
(CL:DEFUN FORMAT-MESSAGE-ARGUMENTS (MESSAGEARGUMENTSTREE TRAILINGEOL?)
  (CL:LET* ((AUGMENTEDARGUMENTS NIL))
   (CL:LET* ((ITEM NULL) (ITER-000 MESSAGEARGUMENTSTREE))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000))
     (CL:COND ((CL:OR (STRING? ITEM) (CL:EQ ITEM EOL)) (CL:SETQ AUGMENTEDARGUMENTS (CONS ITEM AUGMENTEDARGUMENTS)))
      (CL:T (CL:SETQ AUGMENTEDARGUMENTS (CONS SINGLE-BQUOTE-STRING AUGMENTEDARGUMENTS))
   ...
   (CL:WHEN TRAILINGEOL? (CL:SETQ AUGMENTEDARGUMENTS (CONS EOL AUGMENTEDARGUMENTS)))
...
(CL:DEFUN PRINT (BODY)
  ...
  (CL:RETURN-FROM PRINT
   (LIST* SYM-STREAMS-STELLA-PRINT-STREAM SYM-STREAMS-STELLA-STANDARD-OUTPUT (CONCATENATE BODY NIL))))
...
no stream form is provided, then STANDARD-OUTPUT will be used."
  (CL:IF (CL:EQ (%%REST BODY) NIL)
   (CL:RETURN-FROM PRINT-SPACES
    (LIST* SYM-STREAMS-STELLA-FOREACH SYM-STREAMS-STELLA-I SYM-STREAMS-STELLA-IN
     (LIST* SYM-STREAMS-STELLA-INTERVAL (WRAP-INTEGER 1) (%%VALUE BODY) NIL) SYM-STREAMS-STELLA-DO
   ...
   (CL:RETURN-FROM PRINT-SPACES
    (LIST* SYM-STREAMS-STELLA-FOREACH SYM-STREAMS-STELLA-I SYM-STREAMS-STELLA-IN
     (LIST* SYM-STREAMS-STELLA-INTERVAL (WRAP-INTEGER 1) (%%VALUE (%%REST BODY)) NIL) SYM-STREAMS-STELLA-DO
     (LIST* SYM-STREAMS-STELLA-IGNORE SYM-STREAMS-STELLA-I NIL)
     (LIST* SYM-STREAMS-STELLA-PRINT-STREAM (%%VALUE BODY) (CONS (WRAP-STRING " ") NIL)) NIL))))
...
(CL:DEFUN WARN (BODY)
  ...
  (CL:IF (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-STREAMS-COMMON-LISP)
   (CL:RETURN-FROM WARN (LIST* SYM-STREAMS-STELLA-PRINT-STREAM KWD-STREAMS-WARN (CONCATENATE BODY NIL)))
   (CL:RETURN-FROM WARN
    (LIST* SYM-STREAMS-STELLA-PRINT-STREAM SYM-STREAMS-STELLA-STANDARD-WARNING (WRAP-STRING "Warning: ")
  ...
  (CL:IF (CL:AND (CL:= (LENGTH BODY) 1) (ISA? (%%VALUE BODY) SGT-STREAMS-STELLA-STRING-WRAPPER))
   (CL:RETURN-FROM SIGNAL
    (LIST* SYM-STREAMS-STELLA-SIGNAL-EXCEPTION
     (LIST* SYM-STREAMS-STELLA-NEW TYPE (LIST* KWD-STREAMS-MESSAGE (%%VALUE BODY) NIL)) NIL))
   (CL:LET* ((STREAMVAR (LOCAL-GENSYM "STREAM")))
    (CL:RETURN-FROM SIGNAL
     (LIST* SYM-STREAMS-STELLA-LET
      (CONS (CONS STREAMVAR (CONS (LIST* SYM-STREAMS-STELLA-NEW SYM-STREAMS-STELLA-OUTPUT-STRING-STREAM NIL) NIL)) NIL)
      ...
      (LIST* SYM-STREAMS-STELLA-SIGNAL-EXCEPTION
       (LIST* SYM-STREAMS-STELLA-NEW TYPE
        (LIST* KWD-STREAMS-MESSAGE (LIST* SYM-STREAMS-STELLA-THE-STRING STREAMVAR NIL) NIL))
       NIL)
      NIL)))))
  ...
  (CL:IF (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-STREAMS-JAVA)
   (CL:IF (CL:AND (CL:= (LENGTH BODY) 1) (ISA? (%%VALUE BODY) SGT-STREAMS-STELLA-STRING-WRAPPER))
    (CL:RETURN-FROM CONTINUABLE-ERROR
     (LIST* SYM-STREAMS-STELLA-LET
      (CONS (LIST* SYM-STREAMS-STELLA-ERROR_MESSAGE_ (WRAP-STRING (%WRAPPER-VALUE (%%VALUE BODY))) NIL) NIL)
      (LIST* SYM-STREAMS-STELLA-VERBATIM KWD-STREAMS-JAVA (WRAP-STRING "Native.continuableError(error_message_)") NIL)
      NIL))
    (CL:LET* ((STREAMVAR (LOCAL-GENSYM "STREAM")))
     (CL:RETURN-FROM CONTINUABLE-ERROR
      (LIST* SYM-STREAMS-STELLA-LET
       (CONS (CONS STREAMVAR (CONS (LIST* SYM-STREAMS-STELLA-NEW SYM-STREAMS-STELLA-OUTPUT-STRING-STREAM NIL) NIL))
        NIL)
       (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAMVAR (CONCATENATE (FORMAT-MESSAGE-ARGUMENTS BODY CL:NIL) NIL))
       (LIST* SYM-STREAMS-STELLA-LET
        (CONS (LIST* SYM-STREAMS-STELLA-ERROR_MESSAGE_ (LIST* SYM-STREAMS-STELLA-THE-STRING STREAMVAR NIL) NIL) NIL)
        (LIST* SYM-STREAMS-STELLA-VERBATIM KWD-STREAMS-JAVA (WRAP-STRING "Native.continuableError(error_message_)")
         NIL)
        NIL)
       NIL))))
   (CL:RETURN-FROM CONTINUABLE-ERROR
    (LIST* SYM-STREAMS-STELLA-PRINT-STREAM KWD-STREAMS-CONTINUABLE-ERROR (CONCATENATE BODY NIL)))))
...
(CL:DEFUN SIGNAL-READ-ERROR (BODY)
  ...
  (CL:RETURN-FROM SIGNAL-READ-ERROR
   (LIST* SYM-STREAMS-STELLA-SIGNAL SYM-STREAMS-STELLA-READ-EXCEPTION (CONCATENATE BODY NIL))))
...
context for the error message."
  ...
  (CL:WHEN (CL:NOT (PROBE-FILE? FILENAME))
   (CL:LET* ((SELF-000 (NEW-NO-SUCH-FILE-EXCEPTION (CONCATENATE CONTEXT ": file " FILENAME " does not exist"))))
    (CL:SETF (%FILENAME SELF-000) FILENAME) (CL:LET* ((EX SELF-000)) (CL:ERROR EX))))
  :VOID)
...
with `context' supplying context for the error message."
  ...
  (CL:WHEN (PROBE-FILE? FILENAME)
   (CL:LET* ((SELF-000 (NEW-FILE-ALREADY-EXISTS-EXCEPTION (CONCATENATE CONTEXT ": file " FILENAME " already exists"))))
    (CL:SETF (%FILENAME SELF-000) FILENAME) (CL:LET* ((EX SELF-000)) (CL:ERROR EX))))
  :VOID)
      ...
      ((SUBTYPE-OF-SYMBOL? (SAFE-PRIMARY-TYPE ITEM))
       (CL:PROGN
        (CL:WHEN (CL:NOT (CL:EQ ITEM EOL))
         (VALUE-SETTER IT (LIST* SYM-STREAMS-STELLA-DE-UGLIFY-PARSE-TREE ITEM NIL)))))
      (CL:T)))))
  ...
  (CL:LET* ((MESSAGEARGUMENTS (FORMAT-MESSAGE-ARGUMENTS (FORMAT-WALK-MESSAGE-ARGUMENTS BODY) CL:NIL)))
   (CL:RETURN-FROM WALK-ERROR
    (LIST* SYM-STREAMS-STELLA-SPECIAL
     (CONS (LIST* SYM-STREAMS-STELLA-*PRINTREADABLY?* SYM-STREAMS-STELLA-TRUE NIL) NIL)
   ...
   (CL:RETURN-FROM WALK-INFORM
    (LIST* SYM-STREAMS-STELLA-SPECIAL
     ...
     (LIST* SYM-STREAMS-STELLA-UNLESS (CONS SYM-STREAMS-STELLA-SUPPRESS-WARNINGS? NIL)
      ...
      (LIST* SYM-STREAMS-STELLA-PRINT-STREAM SYM-STREAMS-STELLA-STANDARD-OUTPUT SYM-STREAMS-STELLA-EOL
       (WRAP-STRING " ") (CONCATENATE MESSAGEARGUMENTS (LIST* (WRAP-STRING ".") SYM-STREAMS-STELLA-EOL NIL)))
      NIL)
     NIL))))
  ...
  (CL:LET* ((MESSAGEARGUMENTS (FORMAT-MESSAGE-ARGUMENTS (FORMAT-WALK-MESSAGE-ARGUMENTS BODY) CL:NIL)))
   (CL:RETURN-FROM WALK-WARN
    (LIST* SYM-STREAMS-STELLA-SPECIAL
     ...
     (LIST* SYM-STREAMS-STELLA-UNLESS (CONS SYM-STREAMS-STELLA-SUPPRESS-WARNINGS? NIL)
      (LIST* SYM-STREAMS-STELLA-PRINT-ERROR-CONTEXT (WRAP-STRING ">> WARNING: ") SYM-STREAMS-STELLA-STANDARD-WARNING
       NIL)
      (LIST* SYM-STREAMS-STELLA-PRINT-STREAM SYM-STREAMS-STELLA-STANDARD-WARNING SYM-STREAMS-STELLA-EOL
       (WRAP-STRING " ") (CONCATENATE MESSAGEARGUMENTS (LIST* (WRAP-STRING ".") SYM-STREAMS-STELLA-EOL NIL)))
      NIL)
     NIL))))
...
(CL:DEFUN SAFETY (LEVEL TEST BODY)
  ...
  (CL:RETURN-FROM SAFETY
   (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-NOT TEST NIL)
    (LIST* SYM-STREAMS-STELLA-PRINT-STREAM KWD-STREAMS-ERROR (WRAP-STRING "Safety violation: ") (CONCATENATE BODY NIL))
    NIL)))
   ...
   (CL:RETURN-FROM EVALUATION-ERROR
    (LIST* SYM-STREAMS-STELLA-LET
     ...
     (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAMVAR
      (LIST* (WRAP-STRING "While evaluating '") SYM-STREAMS-STELLA-*EVALUATIONTREE* NIL))
     (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-DEFINED? SYM-STREAMS-STELLA-*EVALUATIONPARENTTREE* NIL)
      (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAMVAR
       (LIST* SYM-STREAMS-STELLA-EOL (WRAP-STRING "' inside '") SYM-STREAMS-STELLA-*EVALUATIONPARENTTREE* NIL))
      NIL)
     (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAMVAR (LIST* (WRAP-STRING "':") SYM-STREAMS-STELLA-EOL NIL))
     (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAMVAR (CONCATENATE (FORMAT-MESSAGE-ARGUMENTS BODY CL:NIL) NIL))
     (LIST* SYM-STREAMS-STELLA-SIGNAL-EXCEPTION
      (LIST* SYM-STREAMS-STELLA-NEW SYM-STREAMS-STELLA-EVALUATION-EXCEPTION KWD-STREAMS-MESSAGE
  ...
  (CL:LET* ((MESSAGEARGUMENTS (FORMAT-STRING-MESSAGE-ARGUMENTS BODY CL:NIL)) (MESSAGEVAR (LOCAL-GENSYM "MESSAGE")))
   (CL:RETURN-FROM MESSAGE-TO-STRING
    (LIST* SYM-STREAMS-STELLA-VRLET
     (CONS (CONS MESSAGEVAR (LIST* SYM-STREAMS-STELLA-STRING SYM-STREAMS-STELLA-NULL NIL)) NIL)
     (LIST* SYM-STREAMS-STELLA-SPECIAL
      (CONS (LIST* SYM-STREAMS-STELLA-*PRINTREADABLY?* SYM-STREAMS-STELLA-TRUE NIL) NIL)
      (LIST* SYM-STREAMS-STELLA-SETQ MESSAGEVAR
       (CONS (CONS SYM-STREAMS-STELLA-CONCATENATE (CONCATENATE MESSAGEARGUMENTS NIL)) NIL))
      NIL)
     MESSAGEVAR NIL))))
...
(CL:DEFUN TERMINATE-OUTPUT-STREAM? (SELF)
  (CL:LET* ((NATIVE-STREAM (%NATIVE-STREAM SELF)))
   (CL:WHEN (CL:NOT (CL:EQ NATIVE-STREAM NULL)) (CL:CLOSE NATIVE-STREAM)) (CL:SETF (%NATIVE-STREAM SELF) NULL)
    ...
    (CL:LET*
     ((THEOPTIONS
       (VET-OPTIONS ARGLIST-000
        (LIST* KWD-STREAMS-IF-EXISTS KWD-STREAMS-IF-EXISTS-ACTION KWD-STREAMS-IF-NOT-EXISTS
     ...
     (CL:WHEN (CL:EQ (%NATIVE-STREAM STREAM) NULL) (CLOSE-STREAM STREAM) (CL:SETQ STREAM NULL))
...
(CL:DEFUN INITIALIZE-FILE-OUTPUT-STREAM (SELF)
  ...
  (CL:LET* ((FILENAME (TRANSLATE-LOGICAL-PATHNAME (%FILENAME SELF))) (APPEND CL:NIL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FILENAME))
    ...
    (CL:COND ((CL:EQ TEST-VALUE-000 KWD-STREAMS-CREATE))
     ...
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "initialize-file-output-stream: illegal :if-not-exists action: `"
    ...
    (CL:COND ((CL:EQ TEST-VALUE-001 KWD-STREAMS-SUPERSEDE))
     ...
     (CL:T
      (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "initialize-file-output-stream: illegal :if-exists action: `"
   ...
   (CL:SETF (%NATIVE-STREAM SELF)
    (CL:PROGN (CL:ENSURE-DIRECTORIES-EXIST FILENAME)
     (CL:OPEN FILENAME :DIRECTION :OUTPUT :IF-EXISTS (CL:IF APPEND :APPEND :SUPERSEDE) :IF-DOES-NOT-EXIST :CREATE)))
   (CL:WHEN (CL:EQ (%NATIVE-STREAM SELF) NULL)
    (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "initialize-file-output-stream: Could not open `" SELF "'")
...
(CL:DEFUN TERMINATE-FILE-OUTPUT-STREAM? (SELF)
  (CL:LET* ((NATIVE-STREAM (%NATIVE-STREAM SELF)))
   (CL:WHEN (CL:NOT (CL:EQ NATIVE-STREAM NULL)) (CL:CLOSE NATIVE-STREAM)) (CL:SETF (%NATIVE-STREAM SELF) NULL)
   (CL:SETF (%STATE SELF) KWD-STREAMS-CLOSED) (REMOVE *OPEN-FILE-STREAMS* SELF)
    ...
    (CL:LET* ((ARG-000 NULL) (ITER-000 OPTIONS) (COLLECT-000 NULL))
     (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG-000 (CL:POP ITER-000))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS ARG-000 NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
    (CL:LET*
     ((THEOPTIONS (VET-OPTIONS ARGLIST-000 (LIST* KWD-STREAMS-IF-NOT-EXISTS KWD-STREAMS-IF-NOT-EXISTS-ACTION NIL))))
  ...
  (CL:LET* ((FILENAME (TRANSLATE-LOGICAL-PATHNAME (%FILENAME SELF)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FILENAME))
   (CL:LET* ((TEST-VALUE-000 (%IF-NOT-EXISTS-ACTION SELF)))
    (CL:COND
     ...
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "initialize-file-input-stream: illegal :if-not-exists action: `"
        (%IF-NOT-EXISTS-ACTION SELF) "'")
       (CL:ERROR (NEW-BAD-ARGUMENT-EXCEPTION (THE-STRING-READER STREAM-000)))))))
   (CL:SETF (%NATIVE-STREAM SELF) (CL:OPEN FILENAME :DIRECTION :INPUT :IF-DOES-NOT-EXIST :ERROR))
   (CL:WHEN (CL:EQ (%NATIVE-STREAM SELF) NULL)
    (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "initialize-file-input-stream: Could not open `" SELF "'")
...
(CL:DEFMETHOD THE-STRING-READER ((SELF OUTPUT-STRING-STREAM))
  (CL:LET* ((NATIVE-STREAM (%NATIVE-STREAM SELF)) (STRING STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING STRING)) (SETQ STRING (CL:GET-OUTPUT-STREAM-STRING NATIVE-STREAM))
   (CL:WHEN (CL:EQ STRING STELLA::NULL-STRING) (CL:SETQ STRING "")) (CL:RETURN-FROM THE-STRING-READER STRING)))
...
(CL:DEFUN CLOSE-STREAM (SELF)
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-INPUT-FILE-STREAM) (CL:PROGN (TERMINATE-FILE-INPUT-STREAM? SELF)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-OUTPUT-FILE-STREAM)
     (CL:PROGN (TERMINATE-FILE-OUTPUT-STREAM? SELF)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-INPUT-STRING-STREAM)
     (CL:PROGN (TERMINATE-STRING-INPUT-STREAM? SELF)))
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-OUTPUT-STRING-STREAM)
...
(CL:DEFUN CLOSE-ALL-FILES ()
  ...
  (CL:LET* ((OPENSTREAMS (COPY *OPEN-FILE-STREAMS*)))
   (CL:LET* ((STREAM NULL) (ITER-000 (%THE-CONS-LIST OPENSTREAMS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ STREAM (%%VALUE ITER-000)) (CLOSE-STREAM STREAM)
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (FREE OPENSTREAMS))
  :VOID)
...
as an input/output stream pair."
  ...
  (CL:LET* ((IN NULL) (OUT NULL)) (setq in (%%open-network-stream host port))
   (setq out in)

   (CL:LET* ((SELF-000 (NEW-INPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-000) IN)
    (CL:SETF (%STATE SELF-000) KWD-STREAMS-OPEN) (CL:SETF (%BUFFERING-SCHEME SELF-000) KWD-STREAMS-BLOCK)
    (CL:LET* ((VALUE-000 SELF-000))
     (CL:LET* ((SELF-001 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-001) OUT)
...
can be any that are legal for `open-input-file' (which see)."
  (CL:LET* ((VAR (%%VALUE BINDING)))
   (CL:RETURN-FROM WITH-INPUT-FILE
    (LIST* SYM-STREAMS-STELLA-LET
     (CONS (CONS VAR (LIST* SYM-STREAMS-STELLA-FILE-INPUT-STREAM SYM-STREAMS-STELLA-NULL NIL)) NIL)
     (LIST* SYM-STREAMS-STELLA-UNWIND-PROTECT
      (LIST* SYM-STREAMS-STELLA-PROGN
       (LIST* SYM-STREAMS-STELLA-SETQ VAR
        (CONS
         (LIST* SYM-STREAMS-STELLA-OPEN-INPUT-FILE (%%VALUE (%%REST BINDING))
          (CONCATENATE (%%REST (%%REST BINDING)) NIL))
         NIL))
       (CONCATENATE BODY NIL))
      (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-DEFINED? VAR NIL)
...
can be any that are legal for `open-output-file' (which see)."
  (CL:LET* ((VAR (%%VALUE BINDING)))
   (CL:RETURN-FROM WITH-OUTPUT-FILE
    (LIST* SYM-STREAMS-STELLA-LET
     (CONS (CONS VAR (LIST* SYM-STREAMS-STELLA-FILE-OUTPUT-STREAM SYM-STREAMS-STELLA-NULL NIL)) NIL)
     (LIST* SYM-STREAMS-STELLA-UNWIND-PROTECT
      (LIST* SYM-STREAMS-STELLA-PROGN
       (LIST* SYM-STREAMS-STELLA-SETQ VAR
        (CONS
         (LIST* SYM-STREAMS-STELLA-OPEN-OUTPUT-FILE (%%VALUE (%%REST BINDING))
          (CONCATENATE (%%REST (%%REST BINDING)) NIL))
         NIL))
       (CONCATENATE BODY NIL))
      (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-DEFINED? VAR NIL)
     ...
     (LIST* SYM-STREAMS-STELLA-UNWIND-PROTECT
      (LIST* SYM-STREAMS-STELLA-PROGN
       (LIST* SYM-STREAMS-STELLA-MV-SETQ (CONS VAR-IN (CONS VAR-OUT NIL))
        (LIST* SYM-STREAMS-STELLA-OPEN-NETWORK-STREAM (%%VALUE (%%REST (%%REST BINDING))) (CONS (FOURTH BINDING) NIL))
        NIL)
       (CONCATENATE BODY NIL))
      (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-DEFINED? VAR-IN NIL)
       (LIST* SYM-STREAMS-STELLA-FREE VAR-IN NIL) NIL)
      (LIST* SYM-STREAMS-STELLA-WHEN (LIST* SYM-STREAMS-STELLA-DEFINED? VAR-OUT NIL)
...
(CL:DEFMETHOD STREAM-POSITION ((SELF INPUT-FILE-STREAM))
  ...
  (CL:LET* ((POSITION (NATIVE-FILE-INPUT-STREAM-POSITION (%NATIVE-STREAM SELF))) (STATE (%TOKENIZER-STATE SELF)))
   (CL:WHEN (CL:NOT (CL:EQ STATE NULL)) (CL:SETQ POSITION (CL:- POSITION (BUFFERED-INPUT-LENGTH STATE))))
...
same as the one following the most recent token."
  (CL:LET* ((NSTREAM (%NATIVE-STREAM SELF)) (STATE (%TOKENIZER-STATE SELF)))
   (CL:WHEN (CL:NOT (CL:EQ STATE NULL))
    (CL:LET*
     ((FILEPOSITION (NATIVE-FILE-INPUT-STREAM-POSITION NSTREAM)) (OFFSET (CL:- FILEPOSITION NEWPOS)) (BUFFERED 0))
     ...
     (CL:WHEN (CL:> OFFSET 0) (CL:SETQ BUFFERED (BUFFERED-INPUT-LENGTH STATE))
      (CL:WHEN (CL:<= OFFSET BUFFERED)
       (CL:SETF (%CURSOR STATE)
        (CL:TRUNCATE (MOD (CL:+ (%CURSOR STATE) (CL:- BUFFERED OFFSET)) (CL:TRUNCATE (%BUFFER-SIZE STATE)))))
...
(CL:DEFUN EXPAND-MARKUP-TAG-FUNCTION (TAG-AND-STREAM BODY XML?)
  (CL:LET*
   ((TAG (%%VALUE TAG-AND-STREAM)) (TAGVAR NULL) (TAGVALUE STELLA::NULL-STRING)
    (STREAM (%%VALUE (%%REST TAG-AND-STREAM))) (EOLARGUMENT (%%VALUE (%%REST (%%REST TAG-AND-STREAM))))
    (EOL?
     (CL:AND (CL:NOT (CL:EQ EOLARGUMENT NULL)) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN (COERCE-TO-BOOLEAN EOLARGUMENT)))))
        ...
        (CL:IF EOL?
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
           (CONS (WRAP-STRING (CONCATENATE "<" (%WRAPPER-VALUE TAG) (CL:IF XML? "/>" ">")))
            (CONS SYM-STREAMS-STELLA-EOL NIL))))
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
           (CONS (WRAP-STRING (CONCATENATE "<" (%WRAPPER-VALUE TAG) (CL:IF XML? "/>" ">"))) NIL))))))
         ...
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
           (LIST* (WRAP-STRING "<") TAG
            (CONS (CL:IF XML? (WRAP-STRING "/>") (WRAP-STRING ">")) (CONS SYM-STREAMS-STELLA-EOL NIL)))))
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
           (LIST* (WRAP-STRING "<") TAG (CONS (CL:IF XML? (WRAP-STRING "/>") (WRAP-STRING ">")) NIL)))))))
      (CL:T
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      ...
      ((SUBTYPE-OF-STRING? TEST-VALUE-001)
       (CL:PROGN (CL:SETQ TAGVALUE (%WRAPPER-VALUE TAG))
        (CL:IF EOL?
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PROGN
           (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM (CONS (WRAP-STRING (CONCATENATE "<" TAGVALUE ">")) NIL))
           (CONCATENATE BODY
            (CONS
             (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
              (CONS (WRAP-STRING (CONCATENATE "</" (SUBSEQUENCE TAGVALUE 0 (POSITION TAGVALUE #\  0)) ">"))
               (CONS SYM-STREAMS-STELLA-EOL NIL)))
             NIL))))
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PROGN
           ...
           (CONCATENATE BODY
            (CONS
             (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
              (CONS (WRAP-STRING (CONCATENATE "</" (SUBSEQUENCE TAGVALUE 0 (POSITION TAGVALUE #\  0)) ">")) NIL))
             NIL)))))))
      ((SUBTYPE-OF-SYMBOL? TEST-VALUE-001)
       (CL:PROGN
        (CL:IF EOL?
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-PROGN
           (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM (LIST* (WRAP-STRING "<") TAG (CONS (WRAP-STRING ">") NIL)))
           (CONCATENATE BODY
            (CONS
             (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
              (LIST* (WRAP-STRING "</")
               (LIST* SYM-STREAMS-STELLA-SUBSEQUENCE TAG
                (LIST* (WRAP-INTEGER 0)
                 (LIST* SYM-STREAMS-STELLA-POSITION TAG (LIST* (WRAP-CHARACTER #\ ) (WRAP-INTEGER 0) NIL)) NIL))
             ...
             (LIST* SYM-STREAMS-STELLA-PRINT-STREAM STREAM
              (LIST* (WRAP-STRING "</")
               (LIST* SYM-STREAMS-STELLA-SUBSEQUENCE TAGVAR
                (LIST* (WRAP-INTEGER 0)
                 (LIST* SYM-STREAMS-STELLA-POSITION TAGVAR (LIST* (WRAP-CHARACTER #\ ) (WRAP-INTEGER 0) NIL)) NIL))
               (WRAP-STRING ">") SYM-STREAMS-STELLA-EOL NIL))
             NIL))))
         (CL:RETURN-FROM EXPAND-MARKUP-TAG-FUNCTION
          (LIST* SYM-STREAMS-STELLA-LET (CONS (CONS TAGVAR (LIST* SYM-STREAMS-STELLA-STRING TAG NIL)) NIL)
...
(CL:DEFUN WRITE-HTML-CHARACTER-QUOTING-SPECIAL-CHARACTERS (STREAM CH)
  (CL:CASE CH (#\> (%%PRINT-STREAM STREAM "&gt;")) (#\< (%%PRINT-STREAM STREAM "&lt;"))
...
(CL:DEFUN HEX-CHARACTER-VALUE (C)
  (CL:LET* ((V (POSITION "0123456789ABCDEF" C 0))) (CL:DECLARE (CL:TYPE CL:FIXNUM V))
   (CL:WHEN (CL:= V NULL-INTEGER) (CL:SETQ V (POSITION "0123456789abcdef" C 0))) (CL:RETURN-FROM HEX-CHARACTER-VALUE V)))
...
character equivalent according to RFC 2396."
  ...
  (CL:IF (CL:= (POSITION INPUT #\% 0) NULL-INTEGER) (CL:RETURN-FROM UNESCAPE-URL-STRING INPUT)
   (CL:LET*
    ((RESULT (MAKE-RAW-MUTABLE-STRING (CL:THE CL:FIXNUM (CL:LENGTH INPUT)))) (I-OUT 0) (I-IN 0)
     (CHAR-SAVE NULL-CHARACTER) (CHAR-VALUE 0) (ESCAPE-VALUE 0) (I-MAX (CL:THE CL:FIXNUM (CL:LENGTH INPUT))))
         ...
         (CL:IF (CL:AND (CL:NOT (CL:= CHAR-VALUE NULL-INTEGER)) (CL:< I-IN I-MAX))
          (CL:PROGN (CL:SETQ ESCAPE-VALUE (CL:* 16 CHAR-VALUE))
           ...
           (CL:IF (CL:NOT (CL:= CHAR-VALUE NULL-INTEGER))
            (CL:PROGN
             (CL:LET
              ((SELF RESULT) (CH (CL:CODE-CHAR (CL:THE CL:FIXNUM (CL:+ ESCAPE-VALUE CHAR-VALUE)))) (POSITION I-OUT))
              (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF) (CL:TYPE CL:FIXNUM POSITION))
            ...
            (CL:PROGN
             (CL:LET ((SELF RESULT) (CH #\%) (POSITION I-OUT))
              (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF) (CL:TYPE CL:FIXNUM POSITION))
              (SETF (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION)) (CL:THE CL:CHARACTER CH)))
             (CL:LET ((SELF RESULT) (CH CHAR-SAVE) (POSITION (CL:SETQ I-OUT (CL:1+ I-OUT))))
              (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF) (CL:TYPE CL:FIXNUM POSITION))
              (SETF (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION)) (CL:THE CL:CHARACTER CH)))
             (CL:LET
              ((SELF RESULT)
...
associated characters."
  ...
  (CL:CHECK-TYPE INPUT CL:SIMPLE-STRING)
  (CL:LET*
   ((INCURSOR 0) (OUTCURSOR 0) (ESCAPESTART (POSITION INPUT #\& INCURSOR)) (ESCAPEEND -1) (INPUTLENGTH -1)
    (OUTPUT STELLA::NULL-STRING) (CHARACTER NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM INCURSOR OUTCURSOR ESCAPESTART ESCAPEEND INPUTLENGTH)
    (CL:TYPE CL:SIMPLE-STRING OUTPUT))
   (CL:LOOP
    (CL:TAGBODY
     (CL:WHEN (CL:= ESCAPESTART NULL-INTEGER)
      (CL:IF (NULL? OUTPUT) (CL:RETURN-FROM UNESCAPE-HTML-STRING INPUT) (CL:RETURN)))
     ...
     (CL:WHEN (NULL? OUTPUT) (CL:SETQ INPUTLENGTH (CL:THE CL:FIXNUM (CL:LENGTH INPUT)))
     ...
     (CL:LET ((SELF OUTPUT) (CH (%WRAPPER-VALUE CHARACTER)) (POSITION OUTCURSOR))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF) (CL:TYPE CL:FIXNUM POSITION))
      (SETF (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION)) (CL:THE CL:CHARACTER CH)))
     (CL:SETQ OUTCURSOR (CL:1+ OUTCURSOR)) (CL:SETQ INCURSOR (CL:1+ ESCAPEEND))
   ...
   (CL:RETURN-FROM UNESCAPE-HTML-STRING
    (SUBSEQUENCE (CL:LET ((S OUTPUT)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING S)) (CL:THE CL:SIMPLE-STRING S)) 0
     OUTCURSOR))))
...
(CL:DEFUN ACCESS-STREAM-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-STREAMS-STELLA-THE-STREAM)
    (CL:IF SETVALUE? (CL:SETF (%THE-STREAM SELF) VALUE) (CL:SETQ VALUE (%THE-STREAM SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
...
(CL:DEFUN NEW-S-EXPRESSION-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE S-EXPRESSION-ITERATOR)))
...
(CL:DEFUN NEW-LINE-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE LINE-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) STELLA::NULL-STRING)
...
(CL:DEFUN NEW-NATIVE-LINE-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE NATIVE-LINE-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) STELLA::NULL-STRING)
   (CL:SETF (%THE-STREAM SELF) NULL) (CL:RETURN-FROM NEW-NATIVE-LINE-ITERATOR SELF)))
...
(CL:DEFUN NEW-CHARACTER-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE CHARACTER-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) NULL-CHARACTER) (CL:SETF (%THE-STREAM SELF) NULL)
...
(CL:DEFUN S-EXPRESSIONS (STREAM)
  (CL:LET* ((SELF-000 (NEW-S-EXPRESSION-ITERATOR))) (CL:SETF (%THE-STREAM SELF-000) STREAM)
   (CL:LET* ((VALUE-000 SELF-000)) (CL:RETURN-FROM S-EXPRESSIONS VALUE-000))))
...
(CL:DEFMETHOD NEXT? ((SELF S-EXPRESSION-ITERATOR))
  ...
  (CL:LET* ((OBJECT NULL) (EOF? CL:NIL)) (CL:MULTIPLE-VALUE-SETQ (OBJECT EOF?) (READ-S-EXPRESSION (%THE-STREAM SELF)))
   (CL:COND (EOF? (CLOSE-STREAM (%THE-STREAM SELF)) (CL:RETURN-FROM NEXT? CL:NIL))
...
(CL:DEFUN NATIVE-LINES (STREAM)
  (CL:LET* ((SELF-000 (NEW-NATIVE-LINE-ITERATOR))) (CL:SETF (%THE-STREAM SELF-000) STREAM)
   (CL:LET* ((VALUE-000 SELF-000)) (CL:RETURN-FROM NATIVE-LINES VALUE-000))))

;;; (DEFMETHOD (NEXT? BOOLEAN) ...)

(CL:DEFMETHOD NEXT? ((SELF NATIVE-LINE-ITERATOR))
  (CL:LET* ((LINE (NATIVE-READ-LINE (%THE-STREAM SELF)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING LINE))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL) (CL:SETF (%VALUE SELF) LINE)
   (CL:COND ((CL:EQ LINE STELLA::NULL-STRING) (CLOSE-STREAM (%THE-STREAM SELF)) (CL:RETURN-FROM NEXT? CL:NIL))
    (CL:T (CL:RETURN-FROM NEXT? CL:T)))))

;;; (DEFUN (LINES LINE-ITERATOR) ...)

(CL:DEFUN LINES (STREAM)
  (CL:LET* ((SELF-000 (NEW-LINE-ITERATOR))) (CL:SETF (%THE-STREAM SELF-000) STREAM)
   (CL:LET* ((VALUE-000 SELF-000)) (CL:RETURN-FROM LINES VALUE-000))))

;;; (DEFMETHOD (NEXT? BOOLEAN) ...)

(CL:DEFMETHOD NEXT? ((SELF LINE-ITERATOR))
  (CL:LET* ((LINE (READ-LINE (%THE-STREAM SELF)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING LINE))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL) (CL:SETF (%VALUE SELF) LINE)
   (CL:COND ((CL:EQ LINE STELLA::NULL-STRING) (CLOSE-STREAM (%THE-STREAM SELF)) (CL:RETURN-FROM NEXT? CL:NIL))
    (CL:T (CL:RETURN-FROM NEXT? CL:T)))))
...
(CL:DEFUN CHARACTERS (STREAM)
  (CL:LET* ((SELF-000 (NEW-CHARACTER-ITERATOR))) (CL:SETF (%THE-STREAM SELF-000) STREAM)
...
(CL:DEFMETHOD NEXT? ((SELF CHARACTER-ITERATOR))
  ...
  (CL:LET* ((CHAR NULL-CHARACTER) (EOF? CL:NIL))
   (CL:MULTIPLE-VALUE-SETQ (CHAR EOF?) (READ-CHARACTER (%THE-STREAM SELF)))
   (CL:COND
    (EOF? (CL:SETF (%VALUE SELF) NULL-CHARACTER) (CLOSE-STREAM (%THE-STREAM SELF)) (CL:RETURN-FROM NEXT? CL:NIL))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ MOD (%KEY ITER-000)) (CL:SETQ PROPS (%VALUE ITER-000))
     (CL:WHEN (STRING-EQUAL? MODULE (%WRAPPER-VALUE MOD))
      (CL:LET* ((TEMP-000 (LOOKUP PROPS PARAMETER)))
       (CL:SETQ VALUE (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 DEFAULT)))
      (CL:RETURN))
     (CL:SETQ ITER-000 (%REST ITER-000))))
  ...
  (CL:LET*
   ((OPTIONS
     (VET-OPTIONS |PARAMS&VALUES|
      (GET-QUOTED-TREE "((:LOG-LEVELS :LEVEL :STREAM :PREFIX :MAX-WIDTH) \"/STELLA\")" "/STELLA")))
    (LOGINFO NULL))
   (CL:LET* ((MOD NULL) (PROPS NULL) (ITER-000 (%THE-KV-LIST *LOGGING-REGISTRY*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ MOD (%KEY ITER-000)) (CL:SETQ PROPS (%VALUE ITER-000))
     (CL:WHEN (STRING-EQUAL? MODULE (%WRAPPER-VALUE MOD)) (CL:SETQ LOGINFO PROPS))
     (CL:SETQ ITER-000 (%REST ITER-000))))
   ...
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-001 (%THE-PLIST OPTIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-001))
     (CL:SETQ VALUE (%%VALUE (%%REST ITER-001))) (INSERT-AT LOGINFO KEY VALUE)
...
log level of `module'.  Return FALSE if any of them are undefined."
  ...
  (CL:CHECK-TYPE MODULE CL:SIMPLE-STRING)
  (CL:LET*
   ((LOGLEVELS (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-LOG-LEVELS NIL))
    (MODULELEVEL (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-LEVEL NULL)) (POS1 (POSITION LOGLEVELS LEVEL 0))
    (POS2 (POSITION LOGLEVELS MODULELEVEL 0)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM POS1 POS2))
   (CL:RETURN-FROM LOG-LEVEL<=
    (CL:AND (CL:NOT (CL:= POS1 NULL-INTEGER)) (CL:NOT (CL:= POS2 NULL-INTEGER)) (CL:<= POS1 POS2)))))
     ...
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN
       (CL:RETURN-FROM GET-LOG-STREAM
        (OPEN-OUTPUT-FILE (%WRAPPER-VALUE STREAMSPEC) KWD-STREAMS-IF-EXISTS KWD-STREAMS-APPEND
  ...
  (CL:WHEN (LOG-LEVEL<= LOGLEVEL MODULE)
   (CL:LET*
    ((LOG (GET-LOG-STREAM MODULE))
     (MODULEPREFIX (%WRAPPER-VALUE (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-PREFIX (WRAP-STRING MODULE))))
     (MAXROOM (%WRAPPER-VALUE (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-MAX-WIDTH (WRAP-INTEGER 10000))))
    ...
    (CL:UNWIND-PROTECT
     (CL:PROGN
      (%%PRINT-STREAM (%NATIVE-STREAM LOG) "["
       (CALENDAR-DATE-TO-STRING (MAKE-CURRENT-DATE-TIME) *LOGGING-LOCAL-TIME-ZONE* CL:NIL CL:NIL)
       (CL:IF (STRING-EQL? MODULEPREFIX "") "" " ") MODULEPREFIX "] ")
      (CL:LET*
       ((I NULL-INTEGER) (ITER-000 1) (UPPER-BOUND-000 *LOG-INDENT-LEVEL*)
       ...
       (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
        (%%PRINT-STREAM (%NATIVE-STREAM LOG) " ") (CL:SETQ ITER-000 (CL:1+ ITER-000))))
          ...
          (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ELT)))
           (CL:COND ((SUBTYPE-OF-STRING? TEST-VALUE-000) (CL:PROGN (CL:SETQ ELTSTRING (%WRAPPER-VALUE ELT))))
            ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-GENERALIZED-SYMBOL)
             (CL:PROGN
              (CL:COND
               ((STRING-EQL? (%SYMBOL-NAME ELT) "EOL") (%%PRINT-STREAM (%NATIVE-STREAM LOG) EOL) (CL:SETQ ROOM MAXROOM)
                (CL:GO :CONTINUE))
               (CL:T (CL:SETQ ELTSTRING (STRINGIFY ELT))))))
            (CL:T (CL:SETQ ELTSTRING (STRINGIFY ELT)))))
          (CL:IF (CL:< ROOM (CL:THE CL:FIXNUM (CL:LENGTH ELTSTRING)))
           (%%PRINT-STREAM (%NATIVE-STREAM LOG) (SUBSEQUENCE ELTSTRING 0 ROOM) "...")
...
(CL:DEFUN HELP-STARTUP-STREAMS1 ()
  (CL:PROGN (CL:SETQ SYM-STREAMS-STELLA-EOL (INTERN-RIGID-SYMBOL-WRT-MODULE "EOL" NULL 0))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-FOREACH (INTERN-RIGID-SYMBOL-WRT-MODULE "FOREACH" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-I (INTERN-RIGID-SYMBOL-WRT-MODULE "I" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-IN (INTERN-RIGID-SYMBOL-WRT-MODULE "IN" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-INTERVAL (INTERN-RIGID-SYMBOL-WRT-MODULE "INTERVAL" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-DO (INTERN-RIGID-SYMBOL-WRT-MODULE "DO" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-IGNORE (INTERN-RIGID-SYMBOL-WRT-MODULE "IGNORE" NULL 0))
   (CL:SETQ KWD-STREAMS-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-NEW (INTERN-RIGID-SYMBOL-WRT-MODULE "NEW" NULL 0))
   (CL:SETQ KWD-STREAMS-MESSAGE (INTERN-RIGID-SYMBOL-WRT-MODULE "MESSAGE" NULL 2))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-SIGNAL (INTERN-RIGID-SYMBOL-WRT-MODULE "SIGNAL" NULL 0))
   (CL:SETQ KWD-STREAMS-JAVA (INTERN-RIGID-SYMBOL-WRT-MODULE "JAVA" NULL 2))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-WHEN (INTERN-RIGID-SYMBOL-WRT-MODULE "WHEN" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-NOT (INTERN-RIGID-SYMBOL-WRT-MODULE "NOT" NULL 0))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-STRING (INTERN-RIGID-SYMBOL-WRT-MODULE "STRING" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-SETQ (INTERN-RIGID-SYMBOL-WRT-MODULE "SETQ" NULL 0))
   (CL:SETQ SYM-STREAMS-STELLA-CONCATENATE (INTERN-RIGID-SYMBOL-WRT-MODULE "CONCATENATE" NULL 0))
   (CL:SETQ KWD-STREAMS-CLOSED (INTERN-RIGID-SYMBOL-WRT-MODULE "CLOSED" NULL 2))
   (CL:SETQ KWD-STREAMS-IF-EXISTS (INTERN-RIGID-SYMBOL-WRT-MODULE "IF-EXISTS" NULL 2))
   ...
   (CL:SETQ KWD-STREAMS-SUPERSEDE (INTERN-RIGID-SYMBOL-WRT-MODULE "SUPERSEDE" NULL 2))
   (CL:SETQ KWD-STREAMS-CREATE (INTERN-RIGID-SYMBOL-WRT-MODULE "CREATE" NULL 2))
   (CL:SETQ KWD-STREAMS-ABORT (INTERN-RIGID-SYMBOL-WRT-MODULE "ABORT" NULL 2))
   (CL:SETQ KWD-STREAMS-PROBE (INTERN-RIGID-SYMBOL-WRT-MODULE "PROBE" NULL 2)))
  :VOID)

(CL:DEFUN HELP-STARTUP-STREAMS2 ()
  (CL:PROGN (CL:SETQ KWD-STREAMS-APPEND (INTERN-RIGID-SYMBOL-WRT-MODULE "APPEND" NULL 2))
   ...
   (CL:SETQ SYM-STREAMS-STELLA-POSITION (INTERN-RIGID-SYMBOL-WRT-MODULE "POSITION" NULL 0))
   (CL:SETQ KWD-STREAMS-LETTER (INTERN-RIGID-SYMBOL-WRT-MODULE "LETTER" NULL 2))
   ...
   (CL:SETQ KWD-STREAMS-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   (CL:SETQ KWD-STREAMS-LEVEL (INTERN-RIGID-SYMBOL-WRT-MODULE "LEVEL" NULL 2))
   (CL:SETQ SYM-STREAMS-STELLA-SET-LOG-LEVEL (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-LOG-LEVEL" NULL 0))
   (CL:SETQ KWD-STREAMS-LOG-LEVELS (INTERN-RIGID-SYMBOL-WRT-MODULE "LOG-LEVELS" NULL 2))
   (CL:SETQ KWD-STREAMS-STREAM (INTERN-RIGID-SYMBOL-WRT-MODULE "STREAM" NULL 2))
   (CL:SETQ KWD-STREAMS-PREFIX (INTERN-RIGID-SYMBOL-WRT-MODULE "PREFIX" NULL 2))
   (CL:SETQ KWD-STREAMS-MAX-WIDTH (INTERN-RIGID-SYMBOL-WRT-MODULE "MAX-WIDTH" NULL 2))
...
(CL:DEFUN HELP-STARTUP-STREAMS3 ()
  (CL:PROGN (CL:SETQ EOL SYM-STREAMS-STELLA-EOL)
   (CL:LET* ((SELF-000 (NEW-INPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-000) CL:*STANDARD-INPUT*)
    (CL:SETQ STANDARD-INPUT SELF-000))
   (CL:SETQ SINGLE-QUOTE-STRING (WRAP-STRING "'")) (CL:SETQ SINGLE-BQUOTE-STRING (WRAP-STRING "`"))
   (CL:SETQ *OPEN-FILE-STREAMS* (NEW-LIST)) (CL:SETQ *HTML-ESCAPE-TABLE* (NEW-KEY-VALUE-LIST))
   (CL:SETQ *LOGGING-REGISTRY* (NEW-KEY-VALUE-LIST)) (CL:SETQ *LOGGING-LOCAL-TIME-ZONE* (GET-LOCAL-TIME-ZONE)))
  :VOID)
...
with `context' supplying context for the error message.\")" (CL:FUNCTION ENSURE-FILE-DOES-NOT-EXIST) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "WALK-ERROR" "(DEFUN WALK-ERROR (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)"
    (CL:FUNCTION WALK-ERROR) NULL)
   (DEFINE-FUNCTION-OBJECT "WALK-INFORM" "(DEFUN WALK-INFORM (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)"
    (CL:FUNCTION WALK-INFORM) NULL)
   (DEFINE-FUNCTION-OBJECT "WALK-WARN" "(DEFUN WALK-WARN (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "EVALUATION-ERROR"
    "(DEFUN EVALUATION-ERROR (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)" (CL:FUNCTION EVALUATION-ERROR) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "MESSAGE-TO-STRING"
    "(DEFUN MESSAGE-TO-STRING (|&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE)" (CL:FUNCTION MESSAGE-TO-STRING) NULL)
   (DEFINE-FUNCTION-OBJECT "TERMINATE-OUTPUT-STREAM?"
    "(DEFUN (TERMINATE-OUTPUT-STREAM? BOOLEAN) ((SELF OUTPUT-STREAM)))" (CL:FUNCTION TERMINATE-OUTPUT-STREAM?) NULL)
   (DEFINE-FUNCTION-OBJECT "TERMINATE-INPUT-STREAM?" "(DEFUN (TERMINATE-INPUT-STREAM? BOOLEAN) ((SELF INPUT-STREAM)))"
...
opening operation will be aborted and NULL will be returned.\" :PUBLIC? TRUE)" (CL:FUNCTION OPEN-OUTPUT-FILE) NULL)
   (DEFINE-FUNCTION-OBJECT "INITIALIZE-FILE-OUTPUT-STREAM"
    "(DEFUN INITIALIZE-FILE-OUTPUT-STREAM ((SELF FILE-OUTPUT-STREAM)))" (CL:FUNCTION INITIALIZE-FILE-OUTPUT-STREAM)
    NULL)
...
the opening operation will be aborted and NULL will be returned.\" :PUBLIC? TRUE)" (CL:FUNCTION OPEN-INPUT-FILE) NULL)
   (DEFINE-FUNCTION-OBJECT "INITIALIZE-FILE-INPUT-STREAM"
    "(DEFUN INITIALIZE-FILE-INPUT-STREAM ((SELF FILE-INPUT-STREAM)))" (CL:FUNCTION INITIALIZE-FILE-INPUT-STREAM) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (THE-STRING-READER STRING) ((SELF OUTPUT-STRING-STREAM)))"
    (CL:FUNCTION THE-STRING-READER) NULL)
   (DEFINE-FUNCTION-OBJECT "INITIALIZE-STRING-INPUT-STREAM"
    "(DEFUN INITIALIZE-STRING-INPUT-STREAM ((SELF INPUT-STRING-STREAM)))" (CL:FUNCTION INITIALIZE-STRING-INPUT-STREAM)
    NULL)
...
same as the one following the most recent token.\" :PUBLIC? TRUE)" (CL:FUNCTION STREAM-POSITION-SETTER) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "HTML-CHARACTER-NEEDS-QUOTING?"
    "(DEFUN (HTML-CHARACTER-NEEDS-QUOTING? BOOLEAN) ((CH CHARACTER)))" (CL:FUNCTION HTML-CHARACTER-NEEDS-QUOTING?)
    NULL)
...
associated characters.\" :PUBLIC? TRUE)" (CL:FUNCTION UNESCAPE-HTML-STRING) NULL)
   (DEFINE-FUNCTION-OBJECT "TERMINATE-STREAM-ITERATOR?"
    "(DEFUN (TERMINATE-STREAM-ITERATOR? BOOLEAN) ((SELF STREAM-ITERATOR)))" (CL:FUNCTION TERMINATE-STREAM-ITERATOR?)
    NULL)
   (DEFINE-FUNCTION-OBJECT "S-EXPRESSIONS"
    "(DEFUN (S-EXPRESSIONS S-EXPRESSION-ITERATOR) ((STREAM INPUT-STREAM)) :PUBLIC? TRUE)" (CL:FUNCTION S-EXPRESSIONS)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF S-EXPRESSION-ITERATOR)) :PUBLIC? TRUE)" (CL:FUNCTION NEXT?)
    NULL))
  :VOID)

(CL:DEFUN STARTUP-STREAMS ()
  (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 0)
   (CL:LET* ((SELF-001 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-001) CL:*STANDARD-OUTPUT*)
    (CL:SETQ STANDARD-OUTPUT SELF-001))
   (CL:WHEN (CL:EQ STANDARD-WARNING NULL)
    (CL:LET* ((SELF-002 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-002) CL:*ERROR-OUTPUT*)
     (CL:SETQ STANDARD-WARNING SELF-002)))
   (CL:LET* ((SELF-003 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-003) CL:*ERROR-OUTPUT*)
    (CL:SETQ STANDARD-ERROR SELF-003)))
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (HELP-STARTUP-STREAMS1) (HELP-STARTUP-STREAMS2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-STREAMS4)
    (DEFINE-FUNCTION-OBJECT "NATIVE-LINES"
     "(DEFUN (NATIVE-LINES NATIVE-LINE-ITERATOR) ((STREAM INPUT-STREAM)) :PUBLIC? TRUE)" (CL:FUNCTION NATIVE-LINES)
     NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF NATIVE-LINE-ITERATOR)) :PUBLIC? TRUE)" (CL:FUNCTION NEXT?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "LINES" "(DEFUN (LINES LINE-ITERATOR) ((STREAM INPUT-STREAM)) :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "CHARACTERS"
     "(DEFUN (CHARACTERS CHARACTER-ITERATOR) ((STREAM INPUT-STREAM)) :PUBLIC? TRUE)" (CL:FUNCTION CHARACTERS) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF CHARACTER-ITERATOR)) :PUBLIC? TRUE)" (CL:FUNCTION NEXT?)
     NULL)
...
convenience function for this common operation.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %SET-LOG-LEVEL)
...
or :EOL to print a line terminator.\" :PUBLIC? TRUE)" (CL:FUNCTION LOGMSG) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-STREAMS" "(DEFUN STARTUP-STREAMS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-STREAMS)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STREAMS-STELLA-STARTUP-STREAMS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-STREAMS-STELLA-METHOD-STARTUP-CLASSNAME
...
ends with the first pass that encountered an error.\" :PUBLIC? TRUE)")
    ...
    (INSERT-AT *HTML-ESCAPE-TABLE* (WRAP-STRING "lt") (WRAP-CHARACTER #\<))
    (INSERT-AT *HTML-ESCAPE-TABLE* (WRAP-STRING "gt") (WRAP-CHARACTER #\>))
    (INSERT-AT *HTML-ESCAPE-TABLE* (WRAP-STRING "amp") (WRAP-CHARACTER #\&))
    (INSERT-AT *HTML-ESCAPE-TABLE* (WRAP-STRING "quot") (WRAP-CHARACTER #\"))
    (INSERT-AT *HTML-ESCAPE-TABLE* (WRAP-STRING "apos") (WRAP-CHARACTER #\'))
