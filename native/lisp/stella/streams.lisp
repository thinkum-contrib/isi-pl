...
(CL:DECLAIM
  ...
  *CHARACTER-TYPE-TABLE* NULL-CHARACTER *SAFETY* *TRANSLATOROUTPUTLANGUAGE*
  NIL))
...
(CL:DEFUN WALK-ERROR (BODY)
  (CL:LET*
   ((MESSAGEARGUMENTS
     (FORMAT-MESSAGE-ARGUMENTS (FORMAT-WALK-MESSAGE-ARGUMENTS BODY) CL:NIL)))
  ...
  (CL:LET*
   ...
   (CL:RETURN-FROM WALK-INFORM
    (LIST* SYM-STREAMS-STELLA-SPECIAL
     ...
     (LIST* SYM-STREAMS-STELLA-UNLESS
      (CONS SYM-STREAMS-STELLA-SUPPRESS-WARNINGS? NIL)
      (LIST* SYM-STREAMS-STELLA-PRINT-ERROR-CONTEXT (WRAP-STRING ">> NOTE: ")
    ...
    (LIST* SYM-STREAMS-STELLA-SPECIAL
     ...
     (LIST* SYM-STREAMS-STELLA-UNLESS
      ...
      (LIST* SYM-STREAMS-STELLA-PRINT-ERROR-CONTEXT
       (WRAP-STRING ">> WARNING: ") SYM-STREAMS-STELLA-STANDARD-WARNING NIL)
      (LIST* SYM-STREAMS-STELLA-PRINT-STREAM
  ...
  (CL:LET* ((STREAM (NEW-OUTPUT-FILE-STREAM STELLA::NULL-STRING)))
   (CL:LET* ((ARGLIST-000 NIL))
    ...
    (CL:LET*
     ((THEOPTIONS
       (VET-OPTIONS ARGLIST-000
        (LIST* KWD-STREAMS-IF-EXISTS KWD-STREAMS-IF-EXISTS-ACTION
         KWD-STREAMS-IF-NOT-EXISTS KWD-STREAMS-IF-NOT-EXISTS-ACTION NIL))))
     ...
     (CL:LET* ((TEMP-002 (LOOKUP THEOPTIONS KWD-STREAMS-IF-NOT-EXISTS)))
      (CL:LET* ((CHOOSE-VALUE-001 NULL))
       (CL:IF (CL:NOT (CL:EQ TEMP-002 NULL))
        ...
        (CL:LET*
         ((TEMP-003 (LOOKUP THEOPTIONS KWD-STREAMS-IF-NOT-EXISTS-ACTION)))
     ...
     (CL:WHEN (CL:EQ (%NATIVE-STREAM STREAM) NULL) (CLOSE-STREAM STREAM)
      (CL:SETQ STREAM NULL))
...
(CL:DEFUN INITIALIZE-FILE-OUTPUT-STREAM (SELF)
  ...
  (CL:LET*
   ((FILENAME (TRANSLATE-LOGICAL-PATHNAME (%FILENAME SELF))) (APPEND CL:NIL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FILENAME))
  ...
  (CL:LET* ((STREAM (NEW-INPUT-FILE-STREAM STELLA::NULL-STRING)))
   (CL:LET* ((ARGLIST-000 NIL))
    ...
    (CL:LET*
     ((THEOPTIONS
       (VET-OPTIONS ARGLIST-000
        (LIST* KWD-STREAMS-IF-NOT-EXISTS KWD-STREAMS-IF-NOT-EXISTS-ACTION
         NIL))))
     (CL:LET* ((TEMP-000 (LOOKUP THEOPTIONS KWD-STREAMS-IF-NOT-EXISTS)))
      (CL:LET* ((CHOOSE-VALUE-000 NULL))
       (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL))
        (CL:SETQ CHOOSE-VALUE-000 TEMP-000)
        (CL:LET*
         ((TEMP-001 (LOOKUP THEOPTIONS KWD-STREAMS-IF-NOT-EXISTS-ACTION)))
         (CL:SETQ CHOOSE-VALUE-000
          (CL:IF (CL:NOT (CL:EQ TEMP-001 NULL)) TEMP-001 KWD-STREAMS-ERROR))))
  ...
  (CL:LET* ((VAR (%%VALUE BINDING)))
   (CL:RETURN-FROM WITH-INPUT-FILE
    (LIST* SYM-STREAMS-STELLA-LET
     (CONS
      (CONS VAR
       (LIST* SYM-STREAMS-STELLA-FILE-INPUT-STREAM SYM-STREAMS-STELLA-NULL
        NIL))
      NIL)
  ...
  (CL:LET* ((VAR (%%VALUE BINDING)))
   (CL:RETURN-FROM WITH-OUTPUT-FILE
    (LIST* SYM-STREAMS-STELLA-LET
     (CONS
      (CONS VAR
       (LIST* SYM-STREAMS-STELLA-FILE-OUTPUT-STREAM SYM-STREAMS-STELLA-NULL
        NIL))
      NIL)
     (LIST* SYM-STREAMS-STELLA-UNWIND-PROTECT
      (LIST* SYM-STREAMS-STELLA-PROGN
...
(CL:DEFMETHOD STREAM-POSITION ((SELF INPUT-FILE-STREAM))
  ...
  (CL:LET*
   ((POSITION (NATIVE-FILE-INPUT-STREAM-POSITION (%NATIVE-STREAM SELF)))
...
(CL:DEFMETHOD STREAM-POSITION ((SELF OUTPUT-FILE-STREAM))
  ...
  (CL:RETURN-FROM STREAM-POSITION
   (CL:LET ((NSTREAM (%NATIVE-STREAM SELF))) (CL:file-position nstream))))
...
(CL:DEFUN EXPAND-MARKUP-TAG-FUNCTION (TAG-AND-STREAM BODY XML?)
  (CL:LET*
   ((TAG (%%VALUE TAG-AND-STREAM)) (TAGVAR NULL)
    (TAGVALUE STELLA::NULL-STRING) (STREAM (%%VALUE (%%REST TAG-AND-STREAM)))
      ...
      (CL:PROGN (CL:SETQ I-IN (CL:1+ I-IN))
       (CL:IF (CL:< I-IN I-MAX)
        (CL:PROGN
         ...
         (CL:IF
          (CL:AND (CL:NOT (CL:= CHAR-VALUE NULL-INTEGER)) (CL:< I-IN I-MAX))
  ...
  (CL:LET*
   ...
   (CL:LOOP
    (CL:TAGBODY
     ...
     (CL:LET
      ((SELF OUTPUT) (CH (%WRAPPER-VALUE CHARACTER)) (POSITION OUTCURSOR))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
       (CL:TYPE CL:FIXNUM POSITION))
      (SETF
       (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION))
...
(CL:DEFUN NEW-CHARACTER-ITERATOR ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%FIRST-ITERATION? SELF) CL:T)
   (CL:SETF (%VALUE SELF) NULL-CHARACTER) (CL:SETF (%THE-STREAM SELF) NULL)
...
(CL:DEFMETHOD NEXT? ((SELF S-EXPRESSION-ITERATOR))
  ...
  (CL:LET* ((OBJECT NULL) (EOF? CL:NIL))
   ...
   (CL:COND
    (EOF? (CLOSE-STREAM (%THE-STREAM SELF)) (CL:RETURN-FROM NEXT? CL:NIL))
...
value is defined."
  ...
  (CL:LET* ((VALUE NULL))
   (CL:LET*
    ((MOD NULL) (PROPS NULL) (ITER-000 (%THE-KV-LIST *LOGGING-REGISTRY*)))
...
log level of `module'.  Return FALSE if any of them are undefined."
  ...
  (CL:LET*
   ((LOGLEVELS (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-LOG-LEVELS NIL))
    (MODULELEVEL (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-LEVEL NULL))
...
(CL:DEFUN GET-LOG-STREAM (MODULE)
  ...
  (CL:LET*
   ((STREAMSPEC
     (LOOKUP-LOGGING-PARAMETER MODULE KWD-STREAMS-STREAM STANDARD-OUTPUT)))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE STREAMSPEC)))
    (CL:COND
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN
       (CL:RETURN-FROM GET-LOG-STREAM
        (OPEN-OUTPUT-FILE (%WRAPPER-VALUE STREAMSPEC) KWD-STREAMS-IF-EXISTS
         KWD-STREAMS-APPEND KWD-STREAMS-IF-NOT-EXISTS KWD-STREAMS-CREATE))))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-STREAMS-STELLA-OUTPUT-STREAM)
...
associated characters.\" :PUBLIC? TRUE)" (CL:FUNCTION UNESCAPE-HTML-STRING)
    NULL)
...
value is defined.\" :PUBLIC? TRUE)" (CL:FUNCTION LOOKUP-LOGGING-PARAMETER)
     NULL)
...
`logLevel' is the same or lower than the `module's log level.  Interprets `EOL'
or :EOL to print a line terminator.\" :PUBLIC? TRUE)" (CL:FUNCTION LOGMSG)
     NULL)
