...
(CL:DECLAIM
  ...
  *CLASSES-WITH-UNRESOLVED-CHILDREN-REFERENCES* NULL-STRING-WRAPPER
  STANDARD-WARNING *DEBUGLEVEL* *CLASS-TAXONOMY-GRAPH* TRUE-WRAPPER
  FALSE-WRAPPER NIL SYMBOL-SYM STANDARD-OUTPUT EOL *MODULE* FALSE
  SURROGATE-SYM TRUE))
...
(%%DEFCONSMETHOD PRIMARY-CLASS ((SELF OBJECT))
  (CL:WHEN (CL:NOT (CL:EQ (PRIMARY-TYPE SELF) NULL))
   (CL:RETURN-FROM PRIMARY-CLASS
    (%SURROGATE-VALUE (PRIMARY-TYPE SELF))))
   ...
   (CL:COND
    ...
    (CLIPOLDVALUE?
     (CL:WHEN *WARNIFREDEFINE?*
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
       "Redefining the `" (%SYMBOL-NAME (PRIMARY-TYPE SELF))
       "' named `" NAME "'" EOL))
     (CL:SETF (%SURROGATE-VALUE SURROGATE) SELF)
     (CL:WHEN
   ...
   (CL:LET* ((CHOOSE-VALUE-000 NULL))
    ...
    (CL:LET*
     ...
     (CL:WHEN (CL:NOT (CL:EQ INITIALVALUES NULL))
      (CL:LET*
       ((VALUE NULL) (SLOTNAME NULL)
        (ITER-002 (%THE-PLIST INITIALVALUES)))
...
(CL:DEFUN COERCE-TO-BOOLEAN (OBJECT)
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE OBJECT)))
   (CL:COND
    ...
    (CL:T)))
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
   "Don't know how to coerce `" OBJECT "' of type `"
...
(CL:DEFUN TRANSFER-EXTERNAL-SLOT-TO-NEW-CLASS (EXTERNALSLOT NEWCLASS)
  (CL:LET*
   ((NEWCLASSSLOT
     (LOOKUP-LOCAL-SLOT NEWCLASS (%SLOT-NAME EXTERNALSLOT))))
   (CL:COND
    ...
    ((CL:OR (INLINE-METHOD? NEWCLASSSLOT)
      ...
      FALSE))
    (CL:T
     (SUBSTITUTE (%CLASS-LOCAL-SLOTS NEWCLASS) EXTERNALSLOT
      NEWCLASSSLOT)
     (FREE NEWCLASSSLOT))))
  :VOID)
...
(CL:DEFUN DESTROY-CLASS-AND-SUBCLASSES (SELF)
  ...
  (CL:LET*
   ((SUBTYPE NULL)
    (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS SELF))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CL:WHEN (CL:EQ (TYPE-TO-CLASS SUB) NULL)
         (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
      (CL:WHEN FOUND?-000
       (CL:SETQ UNRESOLVABLECLASSES
        (CONS CLASS UNRESOLVABLECLASSES))))))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF
      ...
      (CL:WARN
       ...
       (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME ALIAS)
        (%HOME-CONTEXT ALIAS) TRUE)
     ...
     (CL:LET* ((VALUE-000 NULL))
      (CL:LET* ((S NULL) (ITER-000 (CLASS-SLOTS CLASS)))
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ S (%VALUE ITER-000))
        (CL:WHEN (CL:EQ (%SLOT-NAME S) SLOTRENAMES)
...
(CL:DEFUN UNFINALIZE-CLASS-SLOTS (CLASS)
  ...
  (CL:LET*
   ((SUBTYPE NULL)
    (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ SUBTYPE (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUBTYPE) NULL))
   ...
   (CL:LET* ((SUPERCLASS NULL) (ITER-001 (%THE-CONS-LIST PARENTS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ SUPERCLASS (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:LET*
      ((OTHERSUPERCLASS NULL) (ITER-002 (%THE-CONS-LIST PARENTS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
...
(CL:DEFUN INHERIT-SLOTS (CLASS)
  (CL:LET*
   ((SLOTS
     (COPY-CONS-LIST (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS)))))
     ...
     (CL:IF
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
         " Macro `" (DE-UGLIFY-PARSE-TREE NAME)
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
         " Function `" (DE-UGLIFY-PARSE-TREE NAME)
         "' shadows the following methods/slots:" EOL "    `"
         (DE-UGLIFY-PARSE-TREE SLOTS) "'." EOL))))
...
precedence order for classes with multiple parents (similar to CLOS)."
  ...
  (CL:LET* ((OFFSET (%SYMBOL-SLOT-OFFSET SLOT-NAME)) (CACHEDSLOT NULL))
   ...
   (CL:LET*
    ...
    (INSERT-AT (%CLASS-SLOT-AND-METHOD-CACHE CLASS) NEWOFFSET
     CACHEDSLOT)
...
(CL:DEFUN LOOKUP-VISIBLE-SLOT (CLASS SLOT-NAME)
  (CL:LET*
   ...
   (CL:LET*
    ((SYMBOL NULL)
     (ITER-000
      (LOOKUP-VISIBLE-SYMBOLS-IN-MODULE SLOTNAMESTRING MODULE TRUE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFMETHOD CONFORMING-SIGNATURES? ((SELF STORAGE-SLOT) SUPERSLOT)
  (CL:RETURN-FROM CONFORMING-SIGNATURES?
   (CL:AND
    (CONFORMING-TYPE-SPEC? (%SLOT-BASE-TYPE SELF) (TYPE SUPERSLOT))
   ...
   (CL:WHEN TEST-VALUE-000
    (CL:LET* ((ALWAYS?-001 TRUE))
     (CL:LET*
      ((TS2 NULL) (TS1 NULL)
       (ITER-002
        (%THE-CONS-LIST (METHOD-RETURN-TYPE-SPECIFIERS METHOD1)))
       (ITER-003
...
(CL:DEFUN NEW-ALL-CLASS-SLOTS-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE ALL-CLASS-SLOTS-ITERATOR)))
...
(CL:DEFMETHOD ALL-CLASS-SLOTS ((SELF CLASS))
  (CL:LET* ((ITERATOR (NEW-ALL-CLASS-SLOTS-ITERATOR)))
   ...
   (CL:SETF (%ITERATOR-OBJECT ITERATOR)
  ...
  (CL:LET* ((VALUE (DEFAULT-FORM SELF)))
   (CL:COND
    ...
    ((CL:AND (DYNAMIC-STORAGE? SELF)
     ...
     (CL:RETURN-FROM SYSTEM-DEFAULT-VALUE
      (DYNAMIC-SLOT-VALUE
       (%DYNAMIC-SLOTS (%SURROGATE-VALUE (TYPE SELF)))
       SYM-CLASSES-STELLA-CLASS-INITIAL-VALUE NULL)))
...
(CL:DEFUN DISCONNECT-CLASSES ()
  (CL:LET*
   ((SURROGATE NULL) (ITER-000 (ALL-SURROGATES *MODULE* FALSE)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO
...
(CL:DEFUN ROOT-CLASS? (CLASS)
  (CL:RETURN-FROM ROOT-CLASS?
   (CL:AND (EMPTY? (%CLASS-DIRECT-SUPERS CLASS))
    (CL:NOT (%MIXIN? CLASS)))))
  ...
  (CL:LET* ((GRAPH *CLASS-TAXONOMY-GRAPH*))
   ...
   (CL:LET* ((CLASS NULL) (ITER-001 (ALL-CLASSES NULL FALSE)))
    (CL:LOOP WHILE (NEXT? ITER-001) DO
     ...
     (CL:LET*
      ((SUB NULL)
       (ITER-002 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS CLASS))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
...
(CL:DEFUN SLOTREF? (SELF)
  (CL:RETURN-FROM SLOTREF?
   (DEFINED? (POSITION (%SYMBOL-NAME SELF) #\. 0))))
