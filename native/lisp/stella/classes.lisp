...
(CL:DEFVAR KWD-CLASSES-TRUE NULL)
(CL:DEFVAR SYM-CLASSES-STELLA-FALSE NULL)
(CL:DEFVAR KWD-CLASSES-FALSE NULL)
...
(CL:DECLAIM
 (CL:SPECIAL STANDARD-ERROR NULL-INTEGER *FINALIZE-RELATION-HOOKS*
  *STELLA-MODULE* *REDEFINE-RELATION-HOOKS*
  *CLASSES-WITH-UNRESOLVED-CHILDREN-REFERENCES* NULL-STRING-WRAPPER
  STANDARD-WARNING *DEBUGLEVEL* *CLASS-TAXONOMY-GRAPH* TRUE-WRAPPER FALSE-WRAPPER
  NIL SYMBOL-SYM STANDARD-OUTPUT EOL *MODULE* FALSE SURROGATE-SYM TRUE))
...
(CL:DEFUN CLASS-SYMBOL (CLASS)
  (CL:RETURN-FROM CLASS-SYMBOL
   (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%CLASS-TYPE CLASS))
    (%HOME-CONTEXT (%CLASS-TYPE CLASS)) TRUE)))
    ...
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ MODULE (%VALUE ITER-000))
     (CL:LET* ((TEST-VALUE-000 FALSE))
      ...
      (CL:WHEN TEST-VALUE-000
       (CL:LET* () (CL:SETQ CLASS (%SURROGATE-VALUE SURROGATE))
        (CL:SETQ TEST-VALUE-000 (CL:NOT (CL:EQ CLASS NULL))))
       (CL:WHEN TEST-VALUE-000
        (CL:SETQ TEST-VALUE-000 (ISA? CLASS SGT-CLASSES-STELLA-CLASS))))
...
if `error?', else return `null'."
  (CL:LET* ((CLASS (%SURROGATE-VALUE CLASS-NAME)))
   ...
   (CL:WHEN ERROR?
    (CL:CERROR "Continue anyway? " "Class `~A' does not exist." CLASS-NAME))
...
if `error?', else return `null'."
  (CL:RETURN-FROM GET-STELLA-CLASS
   (GET-STELLA-CLASS (%SYMBOL-NAME CLASS-NAME) ERROR?)))

;;; (DEFSPECIAL *WARNIFREDEFINE?* ...)

(CL:DEFVAR *WARNIFREDEFINE?* FALSE
   ...
   (CL:COND
    ((CL:NOT (CL:EQ OLDMODULE *MODULE*))
     ...
     (CL:WHEN ASKFORPERMISSION?
      (CL:WARN "Shadowing the `~A' named `~A'" (%SYMBOL-NAME (PRIMARY-TYPE SELF))
       NAME)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
       ...
       "   object.  Suggestion: Explicitly shadow the name using" EOL
    ...
    (CLIPOLDVALUE?
     (CL:WHEN *WARNIFREDEFINE?*
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Redefining the `"
       (%SYMBOL-NAME (PRIMARY-TYPE SELF)) "' named `" NAME "'" EOL))
     (CL:SETF (%SURROGATE-VALUE SURROGATE) SELF)
     (CL:WHEN (CL:NOT (CL:EQ (PRIMARY-TYPE SELF) (PRIMARY-TYPE OLDVALUE)))
   ...
   (CL:LET* ((TEST-VALUE-000 FALSE))
    ...
    (CL:WHEN TEST-VALUE-000
     (CL:LET* ((FOUND?-000 FALSE))
      (CL:LET*
       ((SHADOW NULL) (ITER-000 (%THE-CONS-LIST (SHADOWED-SURROGATES MODULE))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        (CL:PROGN (CL:SETQ SHADOW (%%VALUE ITER-000))
         (CL:SETQ ITER-000 (%%REST ITER-000)))
        (CL:WHEN (STRING-EQL? (%SYMBOL-NAME SHADOW) SYMBOLNAME)
...
be known at translation time."
  (CL:LET*
   ((CLASS (GET-STELLA-CLASS TYPE TRUE))
   ...
   (CL:LET* ((CHOOSE-VALUE-000 NULL))
    (CL:IF (CL:= (CL:LENGTH INITIAL-VALUE-PAIRS) 0)
     ...
     (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
      (CL:LET* ((ARGLIST-000 NIL))
       (CL:LET*
        ((ARG-000 NULL) (ITER-000 INITIAL-VALUE-PAIRS) (COLLECT-000 NULL))
    ...
    (CL:LET*
     ((INITIALVALUES CHOOSE-VALUE-000)
      (REQUIREDSLOTS (CLASS-REQUIRED-SLOT-NAMES CLASS)) (REQUIREDSLOTVALUES NIL)
      (SLOTVALUE NULL) (SLOT NULL) (OBJECT NULL))
     (CL:WHEN (CL:EQ CONSTRUCTORCODE NULL)
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
        "create-object: no funcallable constructor available for `" TYPE "'")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
     (CL:WHEN (NON-EMPTY? REQUIREDSLOTS)
      (CL:WHEN (CL:EQ INITIALVALUES NULL)
       (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001)
         "create-object: missing initial values for `" TYPE "''s required slots")
        (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-001)))))
      (CL:LET*
       ((REQSLOTNAME NULL) (ITER-001 (%THE-CONS-LIST REQUIREDSLOTS))
        (COLLECT-001 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ REQSLOTNAME (%%VALUE ITER-001))
        ...
        (CL:WHEN (CL:EQ SLOTVALUE NULL)
         (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003)
           "create-object: missing initial value for required slot `" TYPE "'.`"
           REQSLOTNAME "'")
     ...
     (CL:WHEN (CL:NOT (CL:EQ INITIALVALUES NULL))
      (CL:LET*
       ((VALUE NULL) (SLOTNAME NULL) (ITER-002 (%THE-PLIST INITIALVALUES)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
        (CL:SETQ SLOTNAME (%%VALUE ITER-002))
        (CL:PROGN (CL:SETQ VALUE (%%VALUE (%%REST ITER-002)))
          ...
          (CL:PROGN
           ...
           (CL:WHEN (CL:NOT (STORAGE-SLOT? SLOT))
            (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
             (%%PRINT-STREAM (%NATIVE-STREAM STREAM-004) "create-object: slot `"
              SLOTNAME "' does not exist on class `" TYPE "'")
...
(CL:DEFUN COERCE-TO-BOOLEAN (OBJECT)
  (CL:COND
   ((CL:OR (CL:EQ OBJECT SYM-CLASSES-STELLA-TRUE)
     (CL:EQ OBJECT KWD-CLASSES-TRUE))
    (CL:RETURN-FROM COERCE-TO-BOOLEAN TRUE-WRAPPER))
   ((CL:OR (CL:EQ OBJECT SYM-CLASSES-STELLA-FALSE)
     (CL:EQ OBJECT KWD-CLASSES-FALSE))
    (CL:RETURN-FROM COERCE-TO-BOOLEAN FALSE-WRAPPER)))
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE OBJECT)))
   (CL:COND
    ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
     (CL:PROGN
      (CL:COND
       ((STRING-EQUAL? (%SYMBOL-NAME OBJECT) "TRUE")
        (CL:RETURN-FROM COERCE-TO-BOOLEAN TRUE-WRAPPER))
    ...
    ((SUBTYPE-OF-BOOLEAN? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM COERCE-TO-BOOLEAN OBJECT)))
    (CL:T)))
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Don't know how to coerce `"
   OBJECT "' of type `"
   (CL:IF (CL:NOT (CL:EQ OBJECT NULL)) (PRIMARY-TYPE OBJECT)
    SGT-CLASSES-STELLA-UNKNOWN)
   "'" EOL "   into a boolean." EOL)
  (CL:RETURN-FROM COERCE-TO-BOOLEAN NULL))
...
redefined, but the native C++ class is not redefined."
  (CL:LET*
   ((CLASS (NEW-CLASS)) (PARAMETERSLOTS (LOOKUP OPTIONS KWD-CLASSES-PARAMETERS))
    (OLDCLASS NULL) (SUCCESS? FALSE))
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ (%HOME-CONTEXT NAME) *MODULE*))
     (CL:NOT (VISIBLE-FROM? (%HOME-CONTEXT NAME) *MODULE*)))
    (CL:WARN
     "Can't define a class named `~A' because the module ~%   `~A' is not visible from the current module `~A'.~%"
     NAME (CONTEXT-NAME (%HOME-CONTEXT NAME)) (CONTEXT-NAME *MODULE*))
   ...
   (CL:WHEN (CL:NOT SUCCESS?) (CL:RETURN-FROM DEFINE-STELLA-CLASS NULL))
   (CL:SETF (%CLASS-TYPE CLASS) NAME)
    ...
    (CL:LET* ((S NULL) (ITER-000 (%THE-CONS-LIST PARAMETERSLOTS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ S (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:SETF (%ABSTRACT? S) TRUE) (INSERT-LAST SLOTS S))))
   (INITIALIZE-SLOT-AND-METHOD-CACHE CLASS)
   (CL:LET*
    ((S NULL) (ITER-001 (%THE-CONS-LIST SUPERS))
     (INTO-000 (%CLASS-DIRECT-SUPERS CLASS)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ S (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:WHEN
      (CL:AND (CL:NOT (STELLA-CLASS? (%SURROGATE-VALUE S)))
       (CL:NOT (CL:EQ (%HOME-CONTEXT S) *MODULE*))
       (VISIBLE-FROM? (%HOME-CONTEXT S) *MODULE*))
      (CL:WHEN (CL:>= *DEBUGLEVEL* 3)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
        "Automatically shadowing bogus super `" S "' of class `" NAME "'" EOL))
    ...
    (CL:WHEN TEST-VALUE-000
     (CL:LET* ((FOUND?-000 FALSE))
      (CL:LET* ((SLOT NULL) (ITER-002 (%THE-CONS-LIST SLOTS)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
        (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-002))
         (CL:SETQ ITER-002 (%%REST ITER-002)))
        (CL:WHEN (PRIMITIVE? SLOT) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
      (CL:SETQ TEST-VALUE-000 FOUND?-000)))
    (CL:WHEN TEST-VALUE-000
     (CL:WARN "Defined class `~A' illegally specifies primitive slots." CLASS)))
   ...
   (CL:LET* ((SLOT NULL) (ITER-003 (%THE-CONS-LIST SLOTS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
     (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-003))
...
(CL:DEFUN DEFINE-STELLA-SLOT (NAME OWNER BASETYPE TYPESPECIFIER OPTIONS)
  (CL:LET* ((SLOT (NEW-STORAGE-SLOT))) (CL:SETF (%SLOT-NAME SLOT) NAME)
   (CL:SETF (%SLOT-OWNER SLOT) OWNER) (CL:SETF (%SLOT-BASE-TYPE SLOT) BASETYPE)
   (CL:WHEN (CL:NOT (CL:EQ TYPESPECIFIER NIL))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT)
     SYM-CLASSES-STELLA-SLOT-TYPE-SPECIFIER (YIELD-TYPE-SPECIFIER TYPESPECIFIER)
     NULL))
   (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (ALLOCATE-ITERATOR OPTIONS)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000))
     (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:COND
      ((CL:EQ KEY KWD-CLASSES-PUBLIC?)
       (CL:SETF (%SLOT-PUBLIC? SLOT) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-REQUIRED?)
       (CL:SETF (%SLOT-REQUIRED? SLOT)
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-COMPONENT?)
       (CL:SETF (%SLOT-COMPONENT? SLOT)
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-READ-ONLY?)
       (CL:SETF (%SLOT-READ-ONLY? SLOT)
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-ACTIVE?)
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT)
        SYM-CLASSES-STELLA-STORED-ACTIVE? VALUE NULL))
      ((CL:EQ KEY KWD-CLASSES-CONTEXT-SENSITIVE?)
       (CL:SETF (%SLOT-CONTEXT-SENSITIVE? SLOT)
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-HARDWIRED?)
       (CL:SETF (%SLOT-HARDWIRED? SLOT)
        (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ((CL:EQ KEY KWD-CLASSES-ABSTRACT?)
       (CL:SETF (%ABSTRACT? SLOT) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
      ...
      ((CL:EQ KEY KWD-CLASSES-DOCUMENTATION)
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT)
        SYM-CLASSES-STELLA-DOCUMENTATION (WRAP-STRING (%WRAPPER-VALUE VALUE))
        NULL-STRING-WRAPPER))
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          ...
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
           " Skipping invalid slot option `" (DE-UGLIFY-PARSE-TREE KEY) "'" EOL
           "in the definition of slot `"
           (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME OWNER) (%HOME-CONTEXT OWNER)
            TRUE)
    ...
    (CL:COND
     ...
     ((CL:EQ KEY KWD-CLASSES-JAVA-NATIVE-TYPE)
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS)
       ...
       (WRAP-STRING (%WRAPPER-VALUE VALUE)) NULL-STRING-WRAPPER))
     ((CL:EQ KEY KWD-CLASSES-PUBLIC?)
      (CL:SETF (%CLASS-PUBLIC? CLASS) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
     ((CL:EQ KEY KWD-CLASSES-ABSTRACT?)
      (CL:SETF (%ABSTRACT? CLASS) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
     ((CL:EQ KEY KWD-CLASSES-ACTIVE?)
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS)
       SYM-CLASSES-STELLA-STORED-ACTIVE? VALUE NULL))
     ((CL:EQ KEY KWD-CLASSES-CL-STRUCT?)
      (CL:SETF (%CL-STRUCT? CLASS) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)))
     ((CL:EQ KEY KWD-CLASSES-MIXIN?)
      (CL:SETF (%MIXIN? CLASS) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE))
      (CL:WHEN (%MIXIN? CLASS) (CL:SETF (%ABSTRACT? CLASS) TRUE)))
     ...
     ((CL:EQ KEY KWD-CLASSES-KEY)
      (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS) SYM-CLASSES-STELLA-CLASS-KEY
       (NEW-LIST) NULL)
      (CL:LET*
       ((SLOTNAME NULL) (ITER-001 VALUE) (INTO-000 (CLASS-KEY CLASS))
        (COLLECT-000 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ SLOTNAME (%%VALUE ITER-001))
         (CL:SETQ ITER-001 (%%REST ITER-001)))
        (CL:IF (CL:EQ COLLECT-000 NULL)
     ...
     ((CL:EQ KEY KWD-CLASSES-PARAMETERS)
      ...
      (CL:LET*
       ((SLOT NULL) (ITER-002 (%THE-CONS-LIST VALUE))
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
        ...
        (CL:IF (CL:EQ COLLECT-001 NULL)
         (CL:PROGN (CL:SETQ COLLECT-001 (CONS (%SLOT-NAME SLOT) NIL))
          (CL:IF (CL:EQ (%THE-CONS-LIST INTO-001) NIL)
           (CL:SETF (%THE-CONS-LIST INTO-001) COLLECT-001)
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
        ...
        (CL:IF (CL:EQ COLLECT-002 NULL)
         (CL:PROGN (CL:SETQ COLLECT-002 (CONS TYPE NIL))
          (CL:IF (CL:EQ (%THE-CONS-LIST INTO-002) NIL)
           (CL:SETF (%THE-CONS-LIST INTO-002) COLLECT-002)
      ...
      (CL:WHEN (CL:NOT (RUN-OPTION-HANDLER? CLASS KEY VALUE))
       (CL:LET* ((*PRINTREADABLY?* TRUE))
        ...
        (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
         (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
          " Skipping invalid class option `" (DE-UGLIFY-PARSE-TREE KEY) "'" EOL
          "in the definition of class `" (CLASS-SYMBOL CLASS) "'." EOL))))))))
  :VOID)
...
(CL:DEFUN INLINE-METHOD? (SLOT)
  (CL:RETURN-FROM INLINE-METHOD?
   (CL:AND (CL:NOT (%SLOT-EXTERNAL? SLOT))
    (ISA? SLOT SGT-CLASSES-STELLA-METHOD-SLOT) (CL:NOT (%METHOD-FUNCTION? SLOT)))))

;;; (DEFUN TRANSFER-EXTERNAL-SLOT-TO-NEW-CLASS ...)

(CL:DEFUN TRANSFER-EXTERNAL-SLOT-TO-NEW-CLASS (EXTERNALSLOT NEWCLASS)
  (CL:LET*
   ((NEWCLASSSLOT (LOOKUP-LOCAL-SLOT NEWCLASS (%SLOT-NAME EXTERNALSLOT))))
   (CL:COND
    ...
    ((CL:OR (INLINE-METHOD? NEWCLASSSLOT)
      ...
      FALSE))
    (CL:T (SUBSTITUTE (%CLASS-LOCAL-SLOTS NEWCLASS) EXTERNALSLOT NEWCLASSSLOT)
     (FREE NEWCLASSSLOT))))
  :VOID)
...
(CL:DEFUN TRANSFER-DEMONS-FROM-OLDCLASS (OLDCLASS NEWCLASS)
  (CL:LET* ((NEWSLOT NULL))
   (CL:LET*
    ((OLDSLOT NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS OLDCLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ OLDSLOT (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:LET* ()
      (CL:SETQ NEWSLOT (LOOKUP-LOCAL-SLOT NEWCLASS (%SLOT-NAME OLDSLOT)))
         ...
         (CL:PROGN
          (CL:COND
           ((SUBTYPE-OF-STORAGE-SLOT? (SAFE-PRIMARY-TYPE NEWSLOT))
            (CL:PROGN
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWSLOT)
              SYM-CLASSES-STELLA-SLOT-GUARD-DEMONS (SLOT-GUARD-DEMONS OLDSLOT)
              NULL)
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OLDSLOT)
              SYM-CLASSES-STELLA-SLOT-GUARD-DEMONS NULL NULL)
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWSLOT)
              SYM-CLASSES-STELLA-SLOT-DEMONS (SLOT-DEMONS OLDSLOT) NULL)
             (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OLDSLOT)
              SYM-CLASSES-STELLA-SLOT-DEMONS NULL NULL)))
           (CL:T))))
        (CL:T)))))))
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWCLASS)
   SYM-CLASSES-STELLA-CLASS-CONSTRUCTOR-DEMONS
   (CLASS-CONSTRUCTOR-DEMONS OLDCLASS) NULL)
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OLDCLASS)
  ...
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWCLASS)
   SYM-CLASSES-STELLA-CLASS-DESTRUCTOR-DEMONS (CLASS-DESTRUCTOR-DEMONS OLDCLASS)
   NULL)
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS OLDCLASS)
...
(CL:DEFUN UNDEFINE-OLD-CLASS (OLDCLASS NEWCLASS)
  (CL:LET*
   ((NEWSUBS (COPY-CONS-LIST (%THE-CONS-LIST (%CLASS-DIRECT-SUBS NEWCLASS)))))
   (UNFINALIZE-CLASS-AND-SUBCLASSES OLDCLASS) (FORGET-UNFINALIZED-CLASS OLDCLASS)
   (CL:LET*
    ((OLDSUB NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS OLDCLASS)))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ OLDSUB (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:IF (CL:EQ COLLECT-000 NULL)
   ...
   (CL:SETF (%THE-CONS-LIST (%CLASS-DIRECT-SUBS NEWCLASS))
    (REMOVE-DUPLICATES NEWSUBS))
   (CLEAR (%CLASS-DIRECT-SUBS OLDCLASS)))
  (CL:LET* ((TAXONOMYNODE (%CLASS-TAXONOMY-NODE OLDCLASS)))
   (CL:LET*
    ((SUPER NULL) (ITER-001 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS OLDCLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ SUPER (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:WHEN
      (CL:AND (CL:NOT (CL:EQ (%SURROGATE-VALUE SUPER) NULL))
       (CL:NOT (MEMBER? (%CLASS-DIRECT-SUPERS NEWCLASS) SUPER)))
      (UNLINK-TAXONOMY-NODES *CLASS-TAXONOMY-GRAPH*
       (%CLASS-TAXONOMY-NODE (%SURROGATE-VALUE SUPER)) TAXONOMYNODE))))
   (CL:SETF (%CLASS-TAXONOMY-NODE NEWCLASS) TAXONOMYNODE)
   (CL:SETF (%CLASS-TAXONOMY-NODE OLDCLASS) NULL))
  (CL:LET*
   ((OLDSLOT NULL) (ITER-002 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS OLDCLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
    (CL:PROGN (CL:SETQ OLDSLOT (%%VALUE ITER-002))
     (CL:SETQ ITER-002 (%%REST ITER-002)))
    (CL:WHEN (%SLOT-EXTERNAL? OLDSLOT)
...
(CL:DEFUN DESTROY-CLASS-AND-SUBCLASSES (SELF)
  ...
  (CL:LET* ((SUBTYPE NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS SELF))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ SUBTYPE (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN INSERT-PRIMITIVE-SUPER (NEWSUPER SUPERS)
  (CL:LET* ((FOUND?-000 FALSE))
   (CL:LET* ((SUP NULL) (ITER-000 (%THE-CONS-LIST SUPERS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SUP (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (SUBTYPE-OF? NEWSUPER SUP) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
      ...
      (CL:LET* ()
       (CL:LET* ((VALUE-000 NULL))
        (CL:LET* ((SUP NULL) (ITER-001 (%THE-CONS-LIST SUPERS)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:PROGN (CL:SETQ SUP (%%VALUE ITER-001))
           (CL:SETQ ITER-001 (%%REST ITER-001)))
...
(CL:DEFUN COLLECT-MOST-SPECIFIC-PRIMITIVE-SUPERS (CLASS SUPERS)
  (CL:LET*
   ((SUPER NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN CLASS-NATIVE-SUPERS (CLASS)
  (CL:LET* ((ALWAYS?-000 TRUE))
   ...
   (CL:IF ALWAYS?-000
    (CL:RETURN-FROM CLASS-NATIVE-SUPERS (%CLASS-DIRECT-SUPERS CLASS))
...
(CL:DEFUN REMEMBER-UNFINALIZED-CLASS (CLASS FORCE?)
  (CL:WHEN
   (CL:OR FORCE?
    (CL:AND (%CLASS-FINALIZED? CLASS) (%CLASS-SLOTS-FINALIZED? CLASS)))
   (PUSH *UNFINALIZED-CLASSES* CLASS) (CL:SETQ *NEWLY-UNFINALIZED-CLASSES?* TRUE))
  :VOID)
...
(CL:DEFUN FORGET-UNFINALIZED-CLASS (CLASS)
  (CL:WHEN
   (CL:NOT (CL:AND (%CLASS-FINALIZED? CLASS) (%CLASS-SLOTS-FINALIZED? CLASS)))
...
(CL:DEFUN FINALIZE-CLASSES ()
  ...
  (CL:LET* ((HIERARCHYMIGHTHAVECHANGED? FALSE))
   (REMOVE-DELETED-MEMBERS *UNFINALIZED-CLASSES*) (RESOLVE-CHILDREN-REFERENCES)
   (CL:LET* ((CLASS NULL) (ITER-000 (%THE-CONS-LIST *UNFINALIZED-CLASSES*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CLASS (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:AND (CL:NOT (%CLASS-FINALIZED? CLASS))
       (CL:NOT (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN (BAD? CLASS))))
      (FINALIZE-CLASS CLASS) (CL:SETQ HIERARCHYMIGHTHAVECHANGED? TRUE))))
...
(CL:DEFUN RESOLVE-CHILDREN-REFERENCES ()
  ...
  (CL:LET* ((UNRESOLVABLECLASSES NIL))
   ...
   (CL:LET*
    ((CLASS NULL)
     (ITER-000 (%THE-CONS-LIST *CLASSES-WITH-UNRESOLVED-CHILDREN-REFERENCES*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CLASS (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ SUB (%%VALUE ITER-001))
         (CL:SETQ ITER-001 (%%REST ITER-001)))
        (CL:WHEN (CL:EQ (TYPE-TO-CLASS SUB) NULL) (CL:SETQ FOUND?-000 TRUE)
         (CL:RETURN))))
      (CL:WHEN FOUND?-000
       (CL:SETQ UNRESOLVABLECLASSES (CONS CLASS UNRESOLVABLECLASSES))))))
   (CLEAR *CLASSES-WITH-UNRESOLVED-CHILDREN-REFERENCES*)
   (CL:SETF (%THE-CONS-LIST *CLASSES-WITH-UNRESOLVED-CHILDREN-REFERENCES*)
    UNRESOLVABLECLASSES))
  :VOID)
...
and set `*NEWLY-UNFINALIZED-CLASSES?*' to `false'."
  (CL:LET* ((UNFINALIZEDCLASSES NIL))
   (CL:LET*
    ((CLASS NULL) (ITER-000 (%THE-CONS-LIST *UNFINALIZED-CLASSES*))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CLASS (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:OR (CL:NOT (%CLASS-FINALIZED? CLASS))
       (CL:NOT (%CLASS-SLOTS-FINALIZED? CLASS)))
      (CL:IF (CL:EQ COLLECT-000 NULL)
   ...
   (CL:SETF (%THE-CONS-LIST *UNFINALIZED-CLASSES*) UNFINALIZEDCLASSES)
...
(CL:DEFUN ACTIVATE-CLASS (CLASS)
  (CL:LET*
   ((ACTIVEOBJECTCLASS (TYPE-TO-CLASS SGT-CLASSES-STELLA-ACTIVE-OBJECT))
   ...
   (CL:COND
    ((CL:= (LENGTH SUPERS) 0) (INSERT SUPERS SGT-CLASSES-STELLA-ACTIVE-OBJECT))
...
(CL:DEFUN ADD-PRIMARY-TYPE (CLASS)
  (CL:LET* ((CLASSTYPE (%CLASS-TYPE CLASS)) (SLOTTREE NULL))
   (CL:LET* ((TEST-VALUE-000 FALSE))
    (CL:SETQ TEST-VALUE-000 (CL:NOT (%ABSTRACT? CLASS)))
    (CL:WHEN TEST-VALUE-000
     (CL:SETQ TEST-VALUE-000 (SUBTYPE-OF? CLASSTYPE SGT-CLASSES-STELLA-OBJECT))
     (CL:WHEN TEST-VALUE-000
      (CL:LET* ((FOUND?-000 FALSE))
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:WHEN (CL:EQ (%SLOT-NAME SLOT) SYM-CLASSES-STELLA-PRIMARY-TYPE)
          (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
       (CL:SETQ TEST-VALUE-000 FOUND?-000))
       ...
       (CONS
        (LIST* SYM-CLASSES-STELLA-SELF
         (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME CLASSTYPE)
          (%HOME-CONTEXT CLASSTYPE) TRUE)
         NIL)
        NIL)
       KWD-CLASSES-TYPE SYM-CLASSES-STELLA-TYPE KWD-CLASSES-AUXILIARY?
       SYM-CLASSES-STELLA-TRUE (LIST* SYM-CLASSES-STELLA-RETURN CLASSTYPE NIL)
       NIL))
     (PUSH (%CLASS-LOCAL-SLOTS CLASS)
      (DEFINE-INLINE-METHOD SYM-CLASSES-STELLA-PRIMARY-TYPE SLOTTREE)))))
  :VOID)
...
(CL:DEFUN FINALIZE-ONE-CLASS (CLASS)
  (CL:LET* ((*MODULE* (HOME-MODULE CLASS)) (*CONTEXT* *MODULE*))
   ...
   (CL:LET* ((ALIAS NULL) (ITER-000 (%THE-CONS-LIST (CLASS-SYNONYMS CLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ ALIAS (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:IF
      (CL:AND (CL:NOT (CL:EQ (%SURROGATE-VALUE ALIAS) NULL))
       (CL:NOT (CL:EQ (%SURROGATE-VALUE ALIAS) CLASS))
       (CL:NOT
        (STRING-EQL? (CLASS-NAME (%SURROGATE-VALUE ALIAS)) (CLASS-NAME CLASS))))
      (CL:WARN
       "Alias `~A' can't point to `~A' because it already points to ~%the class `~A'."
       (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME ALIAS) (%HOME-CONTEXT ALIAS) TRUE)
   ...
   (CL:SETF (%CLASS-FINALIZED? CLASS) TRUE))
  :VOID)
...
(CL:DEFUN HELP-FINALIZE-CLASS (CLASS FINALIZEDPARENT)
  (CL:WHEN (%CLASS-FINALIZED? CLASS) (CL:RETURN-FROM HELP-FINALIZE-CLASS TRUE))
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS) SYM-CLASSES-STELLA-BAD?
   TRUE-WRAPPER NULL)
  (CL:LET*
   ((SUPER NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (CL:EQ SUPER FINALIZEDPARENT))
     (CL:LET* ((SUPERCLASS (TYPE-TO-CLASS SUPER)))
      (CL:IF (CL:NOT (CL:EQ SUPERCLASS NULL))
       ...
       (CL:RETURN-FROM HELP-FINALIZE-CLASS FALSE))))))
  (CL:WHEN (%CLASS-FINALIZED? CLASS)
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS) SYM-CLASSES-STELLA-BAD? NULL
    NULL)
...
(CL:DEFUN UNFINALIZE-CLASS-AND-SUBCLASSES (CLASS)
  ...
  (CL:LET* ((ALIAS NULL) (ITER-001 (%THE-CONS-LIST (CLASS-SYNONYMS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
    (CL:PROGN (CL:SETQ ALIAS (%%VALUE ITER-001))
     (CL:SETQ ITER-001 (%%REST ITER-001)))
    (CL:WHEN (CL:EQ (%SURROGATE-VALUE ALIAS) CLASS)
     (CL:SETF (%SURROGATE-VALUE ALIAS) NULL))))
  (UNINHERIT-SUPERCLASSES CLASS)
  (UNFINALIZE-CLASS-SLOTS CLASS)
  (REMEMBER-UNFINALIZED-CLASS CLASS FALSE)
  (CL:SETF (%CLASS-FINALIZED? CLASS) FALSE)
  :VOID)
...
(CL:DEFUN ATTACH-SLOT-TO-OWNER (NEWSLOT)
  (CL:LET*
   ((NAME (%SLOT-NAME NEWSLOT))
    (OWNERCLASS (%SURROGATE-VALUE (%SLOT-OWNER NEWSLOT)))
    (OLDSLOT (LOOKUP-LOCAL-SLOT OWNERCLASS NAME)))
   (CL:SETF (%SLOT-EXTERNAL? NEWSLOT) TRUE) (REGISTER-SLOT-NAME NEWSLOT)
   (CL:COND
    ((CL:EQ OLDSLOT NULL) (INSERT (%CLASS-LOCAL-SLOTS OWNERCLASS) NEWSLOT)
...
(CL:DEFUN COMPUTE-SLOT-DIRECT-EQUIVALENT (SELF)
  (CL:LET*
   ((CLASS (%SURROGATE-VALUE (%SLOT-OWNER SELF))) (SLOTNAME (%SLOT-NAME SELF))
    (SLOTRENAMES
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CLASSES-STELLA-SLOT-RENAMES
      NULL))
    (RENAMESSLOT NULL))
   (CL:COND
     ...
     (CL:LET* ((VALUE-000 NULL))
      (CL:LET* ((S NULL) (ITER-000 (CLASS-SLOTS CLASS)))
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ S (%VALUE ITER-000))
        (CL:WHEN (CL:EQ (%SLOT-NAME S) SLOTRENAMES) (CL:SETQ VALUE-000 S)
         (CL:RETURN))))
      (CL:SETQ RENAMESSLOT VALUE-000))
     (CL:COND
      ((CL:NOT (CL:EQ RENAMESSLOT NULL))
       (CL:SETF (%SLOT-DIRECT-EQUIVALENT SELF) RENAMESSLOT)
       (CL:SETF (%SLOT-RENAMED? RENAMESSLOT) TRUE))
      (CL:T
       (CL:WARN
     ...
     (CL:LET* ((VALUE-001 NULL))
      (CL:LET* ((S NULL) (ITER-001 (CLASS-SLOTS CLASS)))
       (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ S (%VALUE ITER-001))
        (CL:WHEN (CL:AND (CL:NOT (CL:EQ S SELF)) (CL:EQ (%SLOT-NAME S) SLOTNAME))
  ...
  (CL:WHEN
   ...
   (CL:SETF (%SLOT-BASE-TYPE SELF)
    (VALIDATE-TYPE-SPECIFIER
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
      SYM-CLASSES-STELLA-SLOT-TYPE-SPECIFIER NULL)
     (%SURROGATE-VALUE (%SLOT-OWNER SELF)) FALSE)))
  (CL:WHEN (CL:EQ (%SLOT-BASE-TYPE SELF) NULL)
   (CL:SETF (%SLOT-BASE-TYPE SELF) (TYPE SELF)))
  (CL:WHEN (CL:EQ (%SLOT-BASE-TYPE SELF) NULL)
   (CL:WHEN (CL:NOT (%ABSTRACT? SELF))
    (CL:WARN "Missing type specification for the slot `~A'" SELF))
   (CL:SETF (%SLOT-BASE-TYPE SELF) SGT-CLASSES-STELLA-UNKNOWN)
...
(CL:DEFUN COMPUTE-SLOT-INVERSES (SELF)
  (CL:WHEN
   (CL:EQ
    (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CLASSES-STELLA-SLOT-INVERSE
     NULL)
    NULL)
  ...
  (CL:WHEN (MULTI-VALUED-SLOT-WITH-DUPLICATES? SELF)
   (CL:WARN
    ...
    SELF)
   (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CLASSES-STELLA-SLOT-INVERSE
    NULL NULL)
   (CL:RETURN-FROM COMPUTE-SLOT-INVERSES))
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ INVERSESLOT NULL))
    (CL:WHEN (MULTI-VALUED-SLOT-WITH-DUPLICATES? INVERSESLOT)
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS INVERSESLOT)
      SYM-CLASSES-STELLA-SLOT-INVERSE NULL NULL)
     (CL:RETURN-FROM COMPUTE-SLOT-INVERSES))
    (CL:WHEN (CL:NOT (ACTIVE? INVERSESLOT))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS INVERSESLOT)
      SYM-CLASSES-STELLA-SLOT-INVERSE (%SLOT-NAME SELF) NULL)
     (FINALIZE-SLOT-TYPE-COMPUTATIONS INVERSESLOT))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS INVERSESLOT)
     SYM-CLASSES-STELLA-INVERSE SELF NULL)
    (ATTACH-INVERSE-SLOT-DEMON SELF)
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CLASSES-STELLA-INVERSE
     INVERSESLOT NULL)
  ...
  (CL:LET* ((OWNER (%SURROGATE-VALUE (%SLOT-OWNER SELF))))
   (CL:WHEN
    (CL:AND (CL:EQ (ALLOCATION SELF) KWD-CLASSES-DYNAMIC)
     (CL:NOT
      (SUBTYPE-OF? (%CLASS-TYPE OWNER) SGT-CLASSES-STELLA-DYNAMIC-SLOTS-MIXIN))
     (CL:NOT (%ABSTRACT? OWNER)))
    (CL:WARN
    ...
    (CL:LET*
     ...
     (CL:COND
      ...
      ((CL:NOT (CL:EQ (ALLOCATION SELF) (ALLOCATION RENAMESSLOT)))
       ...
       (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
        SYM-CLASSES-STELLA-SLOT-ALLOCATION (ALLOCATION RENAMESSLOT) NULL)))))
   (CL:WHEN
    (CL:AND (%SLOT-CONTEXT-SENSITIVE? SELF)
     (CL:NOT
      (SUBTYPE-OF? (%SLOT-OWNER SELF)
       SGT-CLASSES-STELLA-CONTEXT-SENSITIVE-OBJECT))
     (CL:NOT
      (SUBTYPE-OF? (%CLASS-TYPE OWNER)
       SGT-CLASSES-STELLA-CONTEXT-SENSITIVE-MIXIN)))
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Class `"
   ...
   (CL:LET* ((SLOT NULL) (ITER-001 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (REGISTER-SLOT-NAME SLOT) (CL:SETF (%SLOT-OWNER SLOT) CLASSTYPE))))
  (CL:LET* ((SLOT NULL) (ITER-002 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
    (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-002))
     (CL:SETQ ITER-002 (%%REST ITER-002)))
    (FINALIZE-LOCAL-SLOT SLOT)))
  (CL:SETF (%CLASS-SLOTS-FINALIZED? CLASS) TRUE)
  (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS) SYM-CLASSES-STELLA-BAD? NULL
   NULL)
  :VOID)
...
(CL:DEFUN UNFINALIZE-CLASS-SLOTS (CLASS)
  (CL:WHEN (CL:NOT (%CLASS-SLOTS-FINALIZED? CLASS))
   (CL:RETURN-FROM UNFINALIZE-CLASS-SLOTS))
  (CL:LET*
   ((SUBTYPE NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ SUBTYPE (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUBTYPE) NULL))
     (UNFINALIZE-CLASS-SLOTS (%SURROGATE-VALUE SUBTYPE)))))
...
(CL:DEFUN HELP-UNFINALIZE-CLASS-SLOTS (CLASS)
  ...
  (REMEMBER-UNFINALIZED-CLASS CLASS FALSE)
  (CL:SETF (%CLASS-SLOTS-FINALIZED? CLASS) FALSE)
  :VOID)
...
(CL:DEFUN ADD-DIRECT-SUBS-BACK-LINKS (CLASS)
  (CL:LET*
   ((DIRECTSUBS (%CLASS-DIRECT-SUBS CLASS)) (CLASSTYPE (%CLASS-TYPE CLASS)))
...
(CL:DEFUN REMOVE-DIRECT-SUPERS-BACK-LINKS (CLASS)
  (CL:LET* ((CLASSTYPE (%CLASS-TYPE CLASS)))
   (CL:LET*
    ((SUPERTYPE NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS CLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SUPERTYPE (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUPERTYPE) NULL))
...
(CL:DEFUN COLLECT-DIRECT-SUPER-CLASSES (CLASS PARENTS)
  (CL:LET*
   ((DIRECTSUPERTYPES (%CLASS-DIRECT-SUPERS CLASS))
    (NONDIRECTPARENTS (NEW-LIST)))
   (CL:LET*
    ((SUPERTYPE NULL) (ITER-000 (%THE-CONS-LIST DIRECTSUPERTYPES))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SUPERTYPE (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUPERTYPE) NULL))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS (%SURROGATE-VALUE SUPERTYPE) NIL))
        (CL:IF (CL:EQ (%THE-CONS-LIST PARENTS) NIL)
         (CL:SETF (%THE-CONS-LIST PARENTS) COLLECT-000)
         (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST PARENTS) COLLECT-000)))
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000) (CONS (%SURROGATE-VALUE SUPERTYPE) NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
   (CL:LET* ((SUPERCLASS NULL) (ITER-001 (%THE-CONS-LIST PARENTS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ SUPERCLASS (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:LET* ((OTHERSUPERCLASS NULL) (ITER-002 (%THE-CONS-LIST PARENTS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       (CL:PROGN (CL:SETQ OTHERSUPERCLASS (%%VALUE ITER-002))
        (CL:SETQ ITER-002 (%%REST ITER-002)))
   ...
   (CL:LET* ((P NULL) (ITER-003 (%THE-CONS-LIST NONDIRECTPARENTS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
     (CL:PROGN (CL:SETQ P (%%VALUE ITER-003))
      (CL:SETQ ITER-003 (%%REST ITER-003)))
     (REMOVE PARENTS P)))
  ...
  (CL:LET* ((PARENTCLASSES (NEW-LIST)))
   ...
   (CL:WHEN (CL:NOT (MULTIPLE-PARENTS? CLASS))
    (CL:LET* ((ONLYPARENT (FIRST PARENTCLASSES)))
     ...
     (CL:SETF (%CLASS-ALL-SUPER-CLASSES CLASS)
   ...
   (CL:LET* ((ALLSUPERCLASSES NIL) (SUBLIST NULL))
    (CL:LET* ((PARENT NULL) (ITER-000 (%THE-CONS-LIST PARENTCLASSES)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:SETQ SUBLIST NIL)
      (CL:LET*
       ((ANCESTOR NULL) (ITER-001 (%CLASS-ALL-SUPER-CLASSES PARENT))
        (COLLECT-000 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
...
also try to generalize parameter types if necessary."
  ...
  (CL:LET*
   ((BASETYPE1 (TYPE-SPEC-TO-BASE-TYPE TYPE1))
    ...
    (SUPERCLASS
     (TWO-ARGUMENT-LEAST-COMMON-SUPERCLASS (%SURROGATE-VALUE BASETYPE1)
      (%SURROGATE-VALUE BASETYPE2))))
   (CL:WHEN (CL:EQ SUPERCLASS NULL)
    (CL:RETURN-FROM TWO-ARGUMENT-LEAST-COMMON-SUPERTYPE SGT-CLASSES-STELLA-VOID))
   ...
   (CL:LET*
    ((SUPERTYPE (%CLASS-TYPE SUPERCLASS))
     (SUPERTYPESPEC (BASE-TYPE-TO-TYPE-SPEC SUPERTYPE))
     (PARAMETERS (PARAMETERS SUPERCLASS)) (PARAMETERTYPES NULL) (SPTYPE NULL)
     (PTYPE1 NULL) (PTYPE2 NULL) (PARAMETEREXISTS? FALSE) (RESULT NULL))
    (CL:WHEN (EMPTY? PARAMETERS)
     (CL:RETURN-FROM TWO-ARGUMENT-LEAST-COMMON-SUPERTYPE SUPERTYPE))
    (CL:SETQ PARAMETERTYPES (NEW-LIST))
    (CL:LET*
     ((PARAMETER NULL) (ITER-000 (%THE-CONS-LIST PARAMETERS)) (COLLECT-000 NULL))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:WHEN (CL:OR (CL:NOT PARAMETEREXISTS?) (UNKNOWN-TYPE? PTYPE1))
       (CL:RETURN-FROM TWO-ARGUMENT-LEAST-COMMON-SUPERTYPE SUPERTYPE))
      (CL:MULTIPLE-VALUE-SETQ (PTYPE2 PARAMETEREXISTS?)
       (EXTRACT-PARAMETER-TYPE TYPE2 PARAMETER))
      (CL:WHEN (CL:OR (CL:NOT PARAMETEREXISTS?) (UNKNOWN-TYPE? PTYPE2))
       (CL:RETURN-FROM TWO-ARGUMENT-LEAST-COMMON-SUPERTYPE SUPERTYPE))
    ...
    (CL:LET* ((SELF-001 (NEW-PARAMETRIC-TYPE-SPECIFIER)))
     (CL:SETF (%SPECIFIER-BASE-TYPE SELF-001) SUPERTYPE)
   ...
   (CL:COND ((EMPTY? (%CLASS-DIRECT-SUPERS CLASS)))
    ((MULTIPLE-PARENTS? CLASS)
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       ...
       (CL:SETQ SLOTS
        (CONCATENATE SLOTS
         (COPY-CONS-LIST (%CLASS-ALL-SLOTS (%SURROGATE-VALUE SUPERTYPE)))))))
    ...
    (CL:T
     (CL:LET*
      ((PARENTSLOTS
        (%CLASS-ALL-SLOTS
         (%SURROGATE-VALUE (FIRST (%CLASS-DIRECT-SUPERS CLASS))))))
...
(CL:DEFUN INITIALIZE-SLOT-AND-METHOD-CACHE (CLASS)
  (CL:SETF (%CLASS-SLOT-AND-METHOD-CACHE CLASS)
...
slot in the system."
  (CL:RETURN-FROM REGISTERED-SLOT-NAME?
   (DEFINED? (%SYMBOL-SLOT-OFFSET SLOT-NAME))))
...
(CL:DEFUN WARN-ABOUT-SLOT-SHADOWED-BY-A-FUNCTION? (SLOT)
  (CL:LET* ((NAME (%SLOT-NAME SLOT)))
   (CL:WHEN (CL:NOT (DEFINED? (%SYMBOL-SLOT-OFFSET NAME)))
    (CL:WHEN (CL:NOT (CL:EQ (LOOKUP-MACRO NAME) NULL))
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*)) (SIGNAL-TRANSLATION-WARNING)
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL " Slot `"
        (DE-UGLIFY-PARSE-TREE SLOT) "' is shadowed by the macro `"
        (DE-UGLIFY-PARSE-TREE NAME) "'." EOL)))
     (CL:RETURN-FROM WARN-ABOUT-SLOT-SHADOWED-BY-A-FUNCTION? TRUE))
    (CL:WHEN (CL:NOT (CL:EQ (LOOKUP-FUNCTION NAME) NULL))
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL " Slot `"
        (DE-UGLIFY-PARSE-TREE SLOT) "' is shadowed by the function `"
        (DE-UGLIFY-PARSE-TREE NAME) "'." EOL)))
     (CL:RETURN-FROM WARN-ABOUT-SLOT-SHADOWED-BY-A-FUNCTION? TRUE)))
   (CL:RETURN-FROM WARN-ABOUT-SLOT-SHADOWED-BY-A-FUNCTION? FALSE)))
...
(CL:DEFUN WARN-ABOUT-FUNCTION-SHADOWING-SLOTS? (FUNCTION)
  (CL:LET* ((NAME (%SLOT-NAME FUNCTION)))
   (CL:WHEN (DEFINED? (%SYMBOL-SLOT-OFFSET NAME))
    (CL:LET* ((SLOTS NIL))
     (CL:LET* ((MODULE NULL) (ITER-000 (ALL-MODULES)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ MODULE (%VALUE ITER-000))
       (CL:LET*
        ((SLOT NULL) (ITER-001 (ALL-SLOTS MODULE TRUE)) (COLLECT-000 NULL))
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        ...
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL " Macro `"
         (DE-UGLIFY-PARSE-TREE NAME) "' shadows the following methods/slots:" EOL
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        ...
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL " Function `"
         (DE-UGLIFY-PARSE-TREE NAME) "' shadows the following methods/slots:" EOL
         "    `" (DE-UGLIFY-PARSE-TREE SLOTS) "'." EOL))))
     (CL:RETURN-FROM WARN-ABOUT-FUNCTION-SHADOWING-SLOTS? TRUE)))
   (CL:RETURN-FROM WARN-ABOUT-FUNCTION-SHADOWING-SLOTS? FALSE)))
...
precedence order for classes with multiple parents (similar to CLOS)."
  ...
  (CL:LET* ((OFFSET (%SYMBOL-SLOT-OFFSET SLOT-NAME)) (CACHEDSLOT NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM OFFSET))
   (CL:WHEN (DEFINED? OFFSET)
    (CL:SETQ CACHEDSLOT
     (CL:AREF
      (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%CLASS-SLOT-AND-METHOD-CACHE CLASS)))
      (CL:THE CL:FIXNUM (%SYMBOL-SLOT-OFFSET SLOT-NAME))))
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ CACHEDSLOT NULL))
      (CL:EQ (%SLOT-NAME CACHEDSLOT) SLOT-NAME))
   ...
   (CL:LET* ((SLOT NULL) (ITER-000 (CLASS-SLOTS CLASS)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SLOT (%VALUE ITER-000))
     (CL:WHEN (CL:EQ (%SLOT-NAME SLOT) SLOT-NAME) (CL:SETQ CACHEDSLOT SLOT)
      (CL:RETURN))))
    ...
    (CL:COND
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ SLOT (LOOKUP-SLOT CLASS SLOT-NAME))
       ...
       (CL:SETQ SLOTNAMESTRING (%SYMBOL-NAME SLOT-NAME))
       (CL:SETQ MODULE (%HOME-CONTEXT SLOT-NAME))))
     ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ SLOTNAMESTRING (%SYMBOL-NAME SLOT-NAME))
       (CL:SETQ MODULE (%HOME-CONTEXT SLOT-NAME))))
     ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ SLOTNAMESTRING (%SYMBOL-NAME SLOT-NAME))))
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ SLOTNAMESTRING (%WRAPPER-VALUE SLOT-NAME))))
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Illegal slot name: `"
        SLOT-NAME "'")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
   (CL:LET*
    ((SYMBOL NULL)
     (ITER-000 (LOOKUP-VISIBLE-SYMBOLS-IN-MODULE SLOTNAMESTRING MODULE TRUE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN LOOKUP-LOCAL-SLOT (CLASS SLOT-NAME)
  ...
  (CL:LET* ((SLOT NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ SLOT (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:WHEN (CL:EQ (%SLOT-NAME SLOT) SLOT-NAME)
...
(CL:DEFUN RESIZE-SLOT-CACHES (SIZE)
  ...
  (CL:LET* ((C NULL) (ITER-000 (ALL-CLASSES NULL FALSE)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ C (%VALUE ITER-000))
    ...
    (CL:LET* ((S NULL) (ITER-001 (%THE-CONS-LIST (LOCAL-SLOTS C))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
      (CL:PROGN (CL:SETQ S (%%VALUE ITER-001))
       (CL:SETQ ITER-001 (%%REST ITER-001)))
      (CL:SETF (%SYMBOL-SLOT-OFFSET (%SLOT-NAME S)) 0)))))
  :VOID)
  ...
  (CL:RETURN-FROM CONFORMING-TYPE-SPEC?
   (CL:OR (CL:EQ SUB-TYPE-SPEC NULL)
    ...
    (CL:AND (TYPE? SUPER-TYPE-SPEC)
     (CL:EQ (%SURROGATE-VALUE SUPER-TYPE-SPEC) NULL))
  ...
  (CL:RETURN-FROM CONFORMING-SIGNATURES?
   (CL:AND (CONFORMING-TYPE-SPEC? (%SLOT-BASE-TYPE SELF) (TYPE SUPERSLOT))
    (CONFORMING-TYPE-SPEC?
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
      SYM-CLASSES-STELLA-SLOT-TYPE-SPECIFIER NULL)
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SUPERSLOT)
      SYM-CLASSES-STELLA-SLOT-TYPE-SPECIFIER NULL))
       ...
       (CL:LET* ((TEST-VALUE-000 FALSE))
        (CL:LET* ((ALWAYS?-000 TRUE))
         (CL:LET*
          ...
          (CL:LOOP WHILE
           ...
           DO
           (CL:PROGN (CL:SETQ TS1 (%%VALUE ITER-000))
            (CL:SETQ ITER-000 (%%REST ITER-000)))
           (CL:PROGN (CL:SETQ TS2 (%%VALUE ITER-001))
            (CL:SETQ ITER-001 (%%REST ITER-001)))
        ...
        (CL:WHEN TEST-VALUE-000
         (CL:LET* ((ALWAYS?-001 TRUE))
          (CL:LET*
           ((TS2 NULL) (TS1 NULL)
            (ITER-002 (%THE-CONS-LIST (METHOD-RETURN-TYPE-SPECIFIERS SELF)))
            (ITER-003
             (%THE-CONS-LIST (METHOD-RETURN-TYPE-SPECIFIERS SUPERSLOT))))
           (CL:LOOP WHILE
            (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (CL:NOT (CL:EQ ITER-003 NIL)))
            DO
            (CL:PROGN (CL:SETQ TS1 (%%VALUE ITER-002))
             (CL:SETQ ITER-002 (%%REST ITER-002)))
            (CL:PROGN (CL:SETQ TS2 (%%VALUE ITER-003))
             (CL:SETQ ITER-003 (%%REST ITER-003)))
   ...
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SUPERSLOT) SGT-CLASSES-STELLA-TABLE)
     (CL:PROGN
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((TS2 NULL) (TS1 NULL) (ITER-000 (REST (%TUPLE-DOMAINS SELF)))
         (ITER-001 (REST (%TUPLE-DOMAINS SUPERSLOT))))
        (CL:LOOP WHILE
  ...
  (CL:LET* ((TEST-VALUE-000 FALSE))
     ...
     (CL:LOOP WHILE
      ...
      (CL:WHEN
       (CL:NOT
        (CL:AND
         (SUB-TYPE-SPEC-OF?
          (CL:SETQ TS1 (COMPUTE-RELATIVE-TYPE-SPEC TS1 (%SLOT-OWNER METHOD1)))
   ...
   (CL:WHEN TEST-VALUE-000
    (CL:LET* ((ALWAYS?-001 TRUE))
     (CL:LET*
      ((TS2 NULL) (TS1 NULL)
       (ITER-002 (%THE-CONS-LIST (METHOD-RETURN-TYPE-SPECIFIERS METHOD1)))
...
(CL:DEFUN COMPUTE-REQUIRED-SLOT-NAMES (CLASS)
  (CL:LET* ((SLOTNAMES (NEW-LIST)))
   (CL:LET* ((SLOT NULL) (ITER-000 (CLASS-SLOTS CLASS)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SLOT (%VALUE ITER-000))
     (CL:WHEN
      (CL:AND (STORAGE-SLOT? SLOT) (%SLOT-REQUIRED? SLOT)
       (CL:NOT (MEMBER? SLOTNAMES (%SLOT-NAME SLOT))))
      (PUSH SLOTNAMES (%SLOT-NAME SLOT)))))
   (CL:IF (EMPTY? SLOTNAMES) (FREE SLOTNAMES)
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS CLASS)
     SYM-CLASSES-STELLA-CLASS-REQUIRED-SLOT-NAMES (REVERSE SLOTNAMES) NULL)))
  :VOID)
...
(CL:DEFMETHOD PUBLIC? ((SELF SLOT))
  ...
  (CL:RETURN-FROM PUBLIC?
   (CL:OR (%SLOT-PUBLIC? SELF)
    (CL:AND (CL:NOT (CL:EQ (%SLOT-DIRECT-EQUIVALENT SELF) NULL))
...
(CL:DEFMETHOD PUBLIC-SLOTS ((SELF CLASS))
  ...
  (CL:LET* ((PUBLICSLOTS NIL) (ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:LET* ((SLOT NULL) (ITER-000 (CLASS-SLOTS SELF)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SLOT (%VALUE ITER-000))
     (CL:LET* ((TEST-VALUE-000 FALSE))
      (CL:SETQ TEST-VALUE-000 (%SLOT-PUBLIC? SLOT))
      (CL:WHEN TEST-VALUE-000
       (CL:LET* ((FOUND?-000 FALSE))
        (CL:LET* ((PS NULL) (ITER-001 PUBLICSLOTS))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          (CL:PROGN (CL:SETQ PS (%%VALUE ITER-001))
           (CL:SETQ ITER-001 (%%REST ITER-001)))
          (CL:WHEN (CL:EQ (%SLOT-NAME PS) (%SLOT-NAME SLOT))
           (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
        (CL:SETQ TEST-VALUE-000 FOUND?-000))
       (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000)))
   ...
   (CL:SETF (%ITERATOR-CONS-LIST ITERATOR) PUBLICSLOTS)
...
(CL:DEFUN NEW-ALL-CLASS-SLOTS-ITERATOR ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE ALL-CLASS-SLOTS-ITERATOR)))
...
(CL:DEFMETHOD ALL-CLASS-SLOTS ((SELF CLASS))
  (CL:LET* ((ITERATOR (NEW-ALL-CLASS-SLOTS-ITERATOR)))
   (CL:SETF (%ITERATOR-CONS-LIST ITERATOR)
    (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS SELF)))
   (CL:SETF (%ITERATOR-OBJECT ITERATOR) (%CLASS-ALL-SUPER-CLASSES SELF))
   (CL:SETF (%FIRST-ITERATION? ITERATOR) TRUE)
   ...
   (CL:COND
    ((CL:EQ LOCALSLOTS NIL)
     (CL:LET* ((CLASSES (%ITERATOR-OBJECT SELF)))
      (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ CLASSES NIL)) (CL:EQ LOCALSLOTS NIL))
       DO
       (CL:SETQ LOCALSLOTS
        (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS (%%VALUE CLASSES))))
       (CL:SETQ CLASSES (%%REST CLASSES)))
      (CL:WHEN (CL:EQ CLASSES NIL)
       (CL:WHEN (CL:EQ LOCALSLOTS NIL) (CL:SETF (%VALUE SELF) NULL)
        (CL:SETF (%ITERATOR-OBJECT SELF) NULL) (CL:RETURN-FROM NEXT? FALSE)))
      (CL:SETF (%ITERATOR-OBJECT SELF) CLASSES)
      (CL:SETF (%VALUE SELF) (%%VALUE LOCALSLOTS))
      (CL:SETF (%ITERATOR-CONS-LIST SELF) (%%REST LOCALSLOTS))
      (CL:RETURN-FROM NEXT? TRUE)))
    (CL:T (CL:SETF (%VALUE SELF) (%%VALUE LOCALSLOTS))
     (CL:SETF (%ITERATOR-CONS-LIST SELF) (%%REST LOCALSLOTS))
     (CL:RETURN-FROM NEXT? TRUE)))))
...
super classes of `self'."
  (CL:RETURN-FROM DIRECT-SUPER-CLASSES
   (NEW-TYPES-TO-CLASSES-ITERATOR (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS SELF)))))
...
of `self'.  Non-reflexive."
  (CL:LET* ((ITERATOR (NEW-LIST-ITERATOR)))
   (CL:SETF (%LIST-ITERATOR-CURSOR ITERATOR) (%CLASS-ALL-SUPER-CLASSES SELF))
...
(CL:DEFMETHOD ACTIVE? ((SLOT SLOT))
  (CL:RETURN-FROM ACTIVE?
   (CL:IF
    (CL:NOT
     (CL:EQ
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT) SYM-CLASSES-STELLA-STORED-ACTIVE?
       NULL)
      NULL))
    (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT) SYM-CLASSES-STELLA-STORED-ACTIVE?
      NULL))
    (CL:AND
     (CL:NOT (CL:EQ (%SURROGATE-VALUE SGT-CLASSES-STELLA-COLLECTION) NULL))
     (CL:NOT (CL:EQ (%SURROGATE-VALUE (%SLOT-BASE-TYPE SLOT)) NULL))
     (CL:NOT (CL:EQ (%SURROGATE-VALUE (%SLOT-OWNER SLOT)) NULL))
     (PRIMITIVE? SLOT)
     (CL:OR
      (CL:NOT
       (CL:EQ
        (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SLOT) SYM-CLASSES-STELLA-SLOT-INVERSE
         NULL)
        NULL))
...
class representing the type of `self'."
  (CL:LET*
   ...
   (CL:RETURN-FROM INITIAL-VALUE
    (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%SURROGATE-VALUE (TYPE SELF)))
  ...
  (CL:LET* ((VALUE (DEFAULT-FORM SELF)))
   (CL:COND
    ...
    ((CL:AND (DYNAMIC-STORAGE? SELF)
      (CL:NOT (CL:EQ (%SURROGATE-VALUE (TYPE SELF)) NULL)))
     (CL:RETURN-FROM SYSTEM-DEFAULT-VALUE
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%SURROGATE-VALUE (TYPE SELF)))
       SYM-CLASSES-STELLA-CLASS-INITIAL-VALUE NULL)))
...
a value."
  (CL:LET*
   ...
   (CL:COND ((CL:NOT (CL:EQ VALUE NULL)) (CL:RETURN-FROM INITIALLY VALUE))
    ...
    ((CL:EQ (%SLOT-BASE-TYPE SELF) SGT-CLASSES-STELLA-BOOLEAN)
...
(CL:DEFUN REPAIR-SLOTS ()
  (CL:LET* ((TOP (%SURROGATE-VALUE SGT-CLASSES-STELLA-OBJECT)))
...
(CL:DEFUN DISCONNECT-CLASSES ()
  (CL:LET* ((SURROGATE NULL) (ITER-000 (ALL-SURROGATES *MODULE* FALSE)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SURROGATE (%VALUE ITER-000))
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ (%SURROGATE-VALUE SURROGATE) NULL))
  ...
  (CL:WHEN (CL:NOT (CL:EQ NAME NULL))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NAME)))
    (CL:COND
     ...
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN (CL:RETURN-FROM NAME-TO-STRING (%WRAPPER-VALUE NAME))))
     (CL:T))))
   ...
   ((ARGLIST ARGS) (NAME (NAME-TO-STRING (%%VALUE ARGLIST)))
    (MODULE
     (CL:IF (CL:NOT (CL:EQ NAME STELLA::NULL-STRING))
      (GET-STELLA-MODULE NAME TRUE) NULL))
    (LOCAL? FALSE))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
  ...
  (CL:WHEN (CL:NOT (CL:EQ NAME NULL))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NAME)))
    (CL:COND
     ...
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN
       (CL:RETURN-FROM COERCE-TO-SYMBOL (LOOKUP-SYMBOL (%WRAPPER-VALUE NAME)))))
     (CL:T))))
...
(CL:DEFUN %PRINT-UNDEFINED-SUPER-CLASSES (CLASS)
  ...
  (CL:LET* ((TYPE (TYPIFY (COERCE-TO-SYMBOL CLASS))) (BADONES (NEW-LIST)))
   (CL:WHEN (CL:EQ TYPE NULL)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Illegal class name: " CLASS
     EOL)
   ...
   (CL:WHEN (MEMBER? BADONES TYPE)
    (CL:IF (CL:EQ (%SURROGATE-VALUE TYPE) NULL)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) TYPE
      " is itself undefined." EOL)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) TYPE
      " itself points to non-class " (%SURROGATE-VALUE TYPE) EOL))
    (CL:RETURN-FROM %PRINT-UNDEFINED-SUPER-CLASSES))
   (CL:LET* ((FOUND?-000 FALSE))
    (CL:LET* ((SUPER NULL) (ITER-000 (%THE-CONS-LIST BADONES)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ SUPER (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:WHEN (CL:EQ (%SURROGATE-VALUE SUPER) NULL) (CL:SETQ FOUND?-000 TRUE)
       (CL:RETURN))))
    (CL:WHEN FOUND?-000
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
      "The following (indirect) super(s) of " TYPE " are undefined:" EOL)
   ...
   (CL:LET* ((FOUND?-001 FALSE))
    (CL:LET* ((SUPER NULL) (ITER-002 (%THE-CONS-LIST BADONES)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
      (CL:PROGN (CL:SETQ SUPER (%%VALUE ITER-002))
       (CL:SETQ ITER-002 (%%REST ITER-002)))
      (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUPER) NULL))
       (CL:SETQ FOUND?-001 TRUE) (CL:RETURN))))
    (CL:WHEN FOUND?-001
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
      "The following (indirect) super(s) of " TYPE " point to non-classes:" EOL)
     (CL:LET* ((SUPER NULL) (ITER-003 (%THE-CONS-LIST BADONES)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
       (CL:PROGN (CL:SETQ SUPER (%%VALUE ITER-003))
        (CL:SETQ ITER-003 (%%REST ITER-003)))
       (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUPER) NULL))
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "    " SUPER
         " points to " (%SURROGATE-VALUE SUPER) EOL)))))))
  :VOID)
...
(CL:DEFUN COLLECT-BAD-SUPER-CLASSES (TYPE BADONES)
  (CL:LET* ((VALUE (%SURROGATE-VALUE TYPE)))
   (CL:IF (STELLA-CLASS? VALUE)
    (CL:LET*
     ((SUPER NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS VALUE))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ SUPER (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN ROOT-CLASS? (CLASS)
  (CL:RETURN-FROM ROOT-CLASS?
   (CL:AND (EMPTY? (%CLASS-DIRECT-SUPERS CLASS)) (CL:NOT (%MIXIN? CLASS)))))
...
(CL:DEFUN CREATE-CLASS-TAXONOMY ()
  (CL:LET* ((GRAPH *CLASS-TAXONOMY-GRAPH*))
   ...
   (CL:LET* ((CLASS NULL) (ITER-000 (ALL-CLASSES NULL FALSE)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ CLASS (%VALUE ITER-000))
     (CL:SETF (%CLASS-TAXONOMY-NODE CLASS)
      (CREATE-TAXONOMY-NODE GRAPH (%CLASS-TAXONOMY-NODE CLASS) CLASS
       (EMPTY? (%CLASS-DIRECT-SUPERS CLASS))))))
   (CL:LET* ((CLASS NULL) (ITER-001 (ALL-CLASSES NULL FALSE)))
    (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ CLASS (%VALUE ITER-001))
     (CL:LET* ((SUB NULL) (ITER-002 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS CLASS))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       (CL:PROGN (CL:SETQ SUB (%%VALUE ITER-002))
        (CL:SETQ ITER-002 (%%REST ITER-002)))
       (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUB) NULL))
        (LINK-TAXONOMY-NODES GRAPH (%CLASS-TAXONOMY-NODE CLASS)
         (%CLASS-TAXONOMY-NODE (%SURROGATE-VALUE SUB))))))))
...
(CL:DEFUN SUBCLASS-OF? (SUBCLASS SUPERCLASS)
  ...
  (CL:WHEN (CL:EQ SUBCLASS SUPERCLASS) (CL:RETURN-FROM SUBCLASS-OF? TRUE))
  (CL:LET*
   ((SUBNODE (%CLASS-TAXONOMY-NODE SUBCLASS))
    (SUPERNODE (%CLASS-TAXONOMY-NODE SUPERCLASS)) (SUBLABEL NULL-INTEGER))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SUBLABEL))
   (CL:WHEN
    (CL:OR (CL:EQ SUBNODE NULL) (CL:EQ SUPERNODE NULL)
     (CL:= (CL:SETQ SUBLABEL (%LABEL SUBNODE)) NULL-INTEGER))
    (CL:RETURN-FROM SUBCLASS-OF?
     (MEMB? (%CLASS-ALL-SUPER-CLASSES SUBCLASS) SUPERCLASS)))
   (CL:LET* ((INTERVAL NULL) (ITER-000 (%INTERVALS SUPERNODE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ INTERVAL (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:AND (CL:>= SUBLABEL (%LOWER-BOUND INTERVAL))
       (CL:<= SUBLABEL (%UPPER-BOUND INTERVAL)))
      (CL:RETURN-FROM SUBCLASS-OF? TRUE))))
   (CL:RETURN-FROM SUBCLASS-OF? FALSE)))

;;; (DEFUN (FAST-SUBCLASS-OF? BOOLEAN) ...)

(CL:DEFUN FAST-SUBCLASS-OF? (SUBCLASS SUPERCLASS)
  (CL:LET* ((SUBLABEL (%LABEL (%CLASS-TAXONOMY-NODE SUBCLASS))))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SUBLABEL))
   (CL:LET*
    ((INTERVAL NULL) (ITER-000 (%INTERVALS (%CLASS-TAXONOMY-NODE SUPERCLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ INTERVAL (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:AND (CL:>= SUBLABEL (%LOWER-BOUND INTERVAL))
       (CL:<= SUBLABEL (%UPPER-BOUND INTERVAL)))
      (CL:RETURN-FROM FAST-SUBCLASS-OF? TRUE))))
   (CL:RETURN-FROM FAST-SUBCLASS-OF? FALSE)))
...
of the class named `super-type'."
  ...
  (CL:LET*
   ((SUBCLASS (TYPE-TO-CLASS SUB-TYPE)) (SUPERCLASS (TYPE-TO-CLASS SUPER-TYPE)))
    ...
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     ...
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      ...
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Reference to non-existent class: `" (DE-UGLIFY-PARSE-TREE SUB-TYPE) "'"
       EOL "." EOL)))
    (CL:RETURN-FROM SUBTYPE-OF? FALSE))
   (CL:WHEN (CL:EQ SUPERCLASS NULL)
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (SIGNAL-TRANSLATION-ERROR)
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Reference to non-existent class: `" (DE-UGLIFY-PARSE-TREE SUPER-TYPE)
       "'" EOL "." EOL)))
    (CL:RETURN-FROM SUBTYPE-OF? FALSE))
...
(CL:DEFUN ISA? (OBJECT TYPE)
  ...
  (CL:RETURN-FROM ISA?
   (SUBCLASS-OF? (%SURROGATE-VALUE (PRIMARY-TYPE OBJECT))
...
(CL:DEFUN LOOKUP-SLOTREF (SELF SLOTNAME)
  ...
  (CL:LET* ((CLASS (%SURROGATE-VALUE SELF)) (SLOT NULL) (SLOTREF NULL))
   (CL:SETQ SLOT (LOOKUP-FUNCTION SLOTNAME))
   (CL:WHEN (CL:EQ SLOT NULL) (CL:SETQ SLOT (LOOKUP-SLOT CLASS SLOTNAME)))
   (CL:COND
    ((CL:EQ SLOT NULL)
     (CL:WHEN (CL:NOT (CL:EQ CLASS NULL)) (CL:RETURN-FROM LOOKUP-SLOTREF NULL)))
    (CL:T (CL:SETQ SELF (%SLOT-OWNER SLOT))
    ...
    (CL:LET*
     ((*MODULE*
       (CL:IF (CL:NOT (CL:EQ SLOT NULL)) (HOME-MODULE SLOT)
        (%HOME-CONTEXT (PERMANENTIFY SLOTNAME))))
      (*CONTEXT* *MODULE*))
     (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (CL:SETQ SLOTREF
...
(CL:DEFUN SLOTREF? (SELF)
  (CL:RETURN-FROM SLOTREF? (DEFINED? (POSITION (%SYMBOL-NAME SELF) #\. 0))))
...
(CL:DEFUN LOOKUP-SLOT-FROM-SLOTREF (SLOTREF)
  (CL:LET* ((VALUE (%SURROGATE-VALUE SLOTREF)))
   (CL:WHEN (CL:NOT (CL:EQ VALUE NULL))
    ...
    (CL:WARN "lookup-slot-from-slotref: `~A' already points to non-slot `~A'"
     SLOTREF VALUE)
    (CL:RETURN-FROM LOOKUP-SLOT-FROM-SLOTREF NULL)))
  (CL:LET*
   ((SLOTREFNAME (%SYMBOL-NAME SLOTREF)) (SLOTREFMODULE (%HOME-CONTEXT SLOTREF))
    (DOTPOSITION (POSITION SLOTREFNAME #\. 0)) (TYPE NULL) (SLOTNAME NULL)
    (SLOT NULL))
   ...
   (CL:COND
    ((DEFINED? DOTPOSITION)
     (CL:SETQ TYPE
      (LOOKUP-SURROGATE-IN-MODULE (SUBSEQUENCE SLOTREFNAME 0 DOTPOSITION)
       SLOTREFMODULE FALSE)))
   ...
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ TYPE NULL)) (STELLA-CLASS? (%SURROGATE-VALUE TYPE)))
    (CL:SETQ SLOTNAME
     (LOOKUP-SYMBOL-IN-MODULE
      (SUBSEQUENCE SLOTREFNAME (CL:1+ DOTPOSITION) NULL-INTEGER) SLOTREFMODULE
      FALSE))
    (CL:WHEN (CL:NOT (CL:EQ SLOTNAME NULL))
     (CL:SETQ SLOT (LOOKUP-FUNCTION SLOTNAME))
     (CL:WHEN (CL:EQ SLOT NULL)
      (CL:SETQ SLOT (LOOKUP-SLOT (%SURROGATE-VALUE TYPE) SLOTNAME)))
     (CL:WHEN (CL:NOT (CL:EQ SLOT NULL))
      (CL:WHEN (CL:EQ (%SLOT-SLOTREF SLOT) NULL)
       (LOOKUP-SLOTREF
        (CL:IF (CL:EQ (%SLOT-OWNER SLOT) NULL) SGT-CLASSES-STELLA-ANY
         (%SLOT-OWNER SLOT))
        (%SLOT-NAME SLOT)))
      (CL:SETF (%SURROGATE-VALUE SLOTREF) SLOT))))
...
(CL:DEFUN HELP-STARTUP-CLASSES1 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-CLASSES-TRUE (INTERN-RIGID-SYMBOL-WRT-MODULE "TRUE" NULL 2))
   (CL:SETQ SYM-CLASSES-STELLA-FALSE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "FALSE" NULL 0))
   (CL:SETQ KWD-CLASSES-FALSE (INTERN-RIGID-SYMBOL-WRT-MODULE "FALSE" NULL 2))
...
visible surrogates looking for one that has a class defined for it.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "TYPE-TO-CLASS"
    "(DEFUN (TYPE-TO-CLASS CLASS) ((TYPE TYPE)))" (CL:FUNCTION TYPE-TO-CLASS)
    NULL)
   (DEFINE-METHOD-OBJECT
...
if `error?', else return `null'.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-STELLA-CLASS)
    NULL)
   (DEFINE-METHOD-OBJECT
...
if `error?', else return `null'.\" :PUBLIC? TRUE)" (CL:FUNCTION GET-STELLA-CLASS)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD UNBIND-FROM-SURROGATE ((SELF CLASS)))"
...
and then deallocate the storage for self.\")" (CL:FUNCTION FREE) NULL)
   (DEFINE-FUNCTION-OBJECT "COERCE-TO-BOOLEAN"
    "(DEFUN (COERCE-TO-BOOLEAN BOOLEAN-WRAPPER) ((OBJECT OBJECT)) :PUBLIC? TRUE)"
...
redefined, but the native C++ class is not redefined.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "INLINE-METHOD?"
    "(DEFUN (INLINE-METHOD? BOOLEAN) ((SLOT SLOT)))" (CL:FUNCTION INLINE-METHOD?)
    NULL)
...
Unfinalize its subclasses (if it has any).\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD DESTROY-CLASS ((SELF TYPE)) :PUBLIC? TRUE)"
    (CL:FUNCTION DESTROY-CLASS) NULL)
...
and set `*NEWLY-UNFINALIZED-CLASSES?*' to `false'.\")"
   ...
   (DEFINE-FUNCTION-OBJECT "ACTIVATE-CLASS"
    "(DEFUN ACTIVATE-CLASS ((CLASS CLASS)))" (CL:FUNCTION ACTIVATE-CLASS) NULL)
   (DEFINE-FUNCTION-OBJECT "ADD-PRIMARY-TYPE"
    "(DEFUN ADD-PRIMARY-TYPE ((CLASS CLASS)))" (CL:FUNCTION ADD-PRIMARY-TYPE)
    NULL)
   (DEFINE-FUNCTION-OBJECT "FINALIZE-ONE-CLASS"
    "(DEFUN FINALIZE-ONE-CLASS ((CLASS CLASS)))" (CL:FUNCTION FINALIZE-ONE-CLASS)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "ACTIVATE-SLOT"
    "(DEFUN ACTIVATE-SLOT ((SELF STORAGE-SLOT)))" (CL:FUNCTION ACTIVATE-SLOT)
    NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD HELP-FINALIZE-LOCAL-SLOT ((SELF SLOT)))"
   ...
   (DEFINE-FUNCTION-OBJECT "FINALIZE-LOCAL-SLOT"
    "(DEFUN FINALIZE-LOCAL-SLOT ((SELF SLOT)))" (CL:FUNCTION FINALIZE-LOCAL-SLOT)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD UNFINALIZE-LOCAL-SLOT ((SELF SLOT)))"
...
also try to generalize parameter types if necessary.\")"
   ...
   (DEFINE-FUNCTION-OBJECT "INHERIT-SLOTS"
    "(DEFUN INHERIT-SLOTS ((CLASS CLASS)))" (CL:FUNCTION INHERIT-SLOTS) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "UNINHERIT-SLOTS"
    "(DEFUN UNINHERIT-SLOTS ((CLASS CLASS)))" (CL:FUNCTION UNINHERIT-SLOTS) NULL)
...
a value.\" :PUBLIC? TRUE)" (CL:FUNCTION INITIALLY) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "DISCONNECT-CLASSES" "(DEFUN DISCONNECT-CLASSES ())"
    (CL:FUNCTION DISCONNECT-CLASSES) NULL)
...
(CL:DEFUN STARTUP-CLASSES ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *WARNIFREDEFINE?* TRUE)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-CLASSES3)
    ...
    (DEFINE-FUNCTION-OBJECT "ROOT-CLASS?"
     "(DEFUN (ROOT-CLASS? BOOLEAN) ((CLASS CLASS)))" (CL:FUNCTION ROOT-CLASS?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "CREATE-CLASS-TAXONOMY"
     "(DEFUN CREATE-CLASS-TAXONOMY ())" (CL:FUNCTION CREATE-CLASS-TAXONOMY) NULL)
...
of the class named `super-type'.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBTYPE-OF?)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "SLOTREF?"
     "(DEFUN (SLOTREF? BOOLEAN) ((SELF SURROGATE)))" (CL:FUNCTION SLOTREF?) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "SLOTREF-TYPE"
     "(DEFUN (SLOTREF-TYPE TYPE) ((SLOTREF SLOTREF)))" (CL:FUNCTION SLOTREF-TYPE)
     NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CLASSES"
     "(DEFUN STARTUP-CLASSES () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-CLASSES)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CLASSES-STELLA-STARTUP-CLASSES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-CLASSES-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Classes") NULL-STRING-WRAPPER)))
...
which still have some of their children references unresolved.\")")
    ...
    (REGISTER-NATIVE-NAME SYM-CLASSES-STELLA-PRINT-UNDEFINED-SUPER-CLASSES
     KWD-CLASSES-COMMON-LISP KWD-CLASSES-FUNCTION)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
