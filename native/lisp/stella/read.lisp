...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* STANDARD-OUTPUT *CHARACTER-TYPE-TABLE*
  ...
  FALSE NIL TRUE TRUE-WRAPPER FALSE-WRAPPER))
...
(CL:DEFUN PARSE-TOKENIZER-DEFINITION (DEFINITION)
  (CL:LET*
   ((TRANSITIONTABLE (NEW-VECTOR *MAX-TOKENIZER-STATES*))
    ...
    (ALLCHARACTERSETS (NEW-LIST)) (STATEMODIFIERS 0) (ERRORS? FALSE)
    ...
    (CL:LOOP WHILE (CL:<= ITER-005 UPPER-BOUND-002) DO
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-003) DO
       (CL:PROGN (CL:SETQ J ITER-006)
        (CL:SETQ ITER-006 (CL:1+ ITER-006)))
       (CL:PROGN (CL:SETQ K ITER-007)
...
(CL:DEFUN READ-LINE-FROM-TOKENIZER-BUFFER (STREAM)
  (CL:LET*
   ...
   (CL:RETURN-FROM READ-LINE-FROM-TOKENIZER-BUFFER
    (CL:VALUES LINE EOF?))))
(CL:DECLAIM
...
(CL:DEFUN TOKENIZE-S-EXPRESSION (STREAM TOKENLIST)
  (CL:LET*
   ((UPCASE? (CL:NOT (%CASE-SENSITIVE? *MODULE*))) (PARENBALANCE 0)
    (ENDOFFILE? TRUE) (SKIPCLOSINGQUOTE? FALSE)
          ...
          (CL:COND
           ...
           ((CL:EQ TEST-VALUE-000 KWD-READ-FULLY-ESCAPED-SYMBOL)
            (CL:SETF (%CONTENT TOKENCURSOR)
             (UNESCAPE-TOKEN-STRING
              (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_
               (CL:1+ TOK_TOKENSTART_) TOK_CURSOR_ TOK_SIZE_ FALSE)
              #\\ FALSE))
           ...
           ((CL:EQ TEST-VALUE-000 KWD-READ-CL-SYMBOL)
            (CL:SETF (%CONTENT TOKENCURSOR)
             (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_
              (CL:+ TOK_TOKENSTART_ 3) TOK_CURSOR_ TOK_SIZE_ TRUE))
         ...
         (CL:LET*
          ...
          (CL:COND
           ((CL:EQ TEST-VALUE-002 KWD-READ-KEYWORD)
            (CL:SETF (%CONTENT TOKENCURSOR)
             (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_
              (CL:1+ TOK_TOKENSTART_) TOK_CURSOR_ TOK_SIZE_ TRUE)))
    ...
    (CL:TAGBODY (CL:SETQ TOKENTYPE (%LOGICAL-TOKEN-TYPE TOKENLIST))
     (CL:COND
      ...
      ((CL:EQ TOKENTYPE KWD-READ-INTEGER)
       (CL:SETQ PARSEDTOKEN
        (CL:MAKE-INSTANCE (CL:QUOTE INTEGER-WRAPPER)))
...
the input is read.  See also special variable `*USER-QUERY-ACTION*'.\" :PUBLIC? TRUE)"
   ...
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-READ-STELLA-STARTUP-READ)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "_Startup-Read") NULL-STRING-WRAPPER)))
  :VOID)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *STELLA-TOKENIZER-TABLE-DEFINITION*
     (LIST*
      ...
      (LIST* KWD-READ-COMMENT
       (LIST* (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return)
        NIL)
      ...
      (LIST* KWD-READ-PAREN-COMMENT-ESCAPE KWD-READ-ANY
       KWD-READ-PAREN-COMMENT NIL)
      (LIST* KWD-READ-PAREN-COMMENT-BAR (WRAP-STRING "#")
       KWD-READ-START (WRAP-STRING "\\") KWD-READ-PAREN-COMMENT-ESCAPE
      ...
      (LIST* KWD-READ-SYMBOL-OR-NUMBER KWD-READ-INCLUDE
       ...
       (WRAP-STRING "eE")
       (LIST* KWD-READ-SYMBOL-OR-EXPONENT-DELIMITER KWD-READ-SYMBOL
        NIL)
       (WRAP-STRING "|") KWD-READ-ERROR KWD-READ-OTHERWISE
       KWD-READ-SYMBOL NIL)
      ...
      (LIST* KWD-READ-ESCAPED-SYMBOL KWD-READ-INCLUDE
       KWD-READ-DELIMITER (WRAP-STRING "/|") KWD-READ-ERROR
       (WRAP-STRING "\\") KWD-READ-ESCAPED-SYMBOL-ESCAPE
      ...
      (LIST* KWD-READ-OPEN-SURROGATE KWD-READ-INCLUDE
       KWD-READ-DELIMITER (WRAP-STRING "|")
       KWD-READ-FULLY-ESCAPED-SURROGATE (WRAP-STRING "\\")
      ...
      (LIST* KWD-READ-QUALIFIED-NAME KWD-READ-INCLUDE
       KWD-READ-DELIMITER (WRAP-STRING "@")
       KWD-READ-QUALIFIED-SURROGATE (WRAP-STRING ":") KWD-READ-ERROR
      ...
      (LIST* KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL SYM-READ-STELLA-*
       (WRAP-STRING "|") KWD-READ-CLOSE-FULLY-ESCAPED-NAME
       (WRAP-STRING "\\")
       KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL-ESCAPE
       KWD-READ-OTHERWISE KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL NIL)
      (LIST* KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL-ESCAPE
       KWD-READ-ANY KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL NIL)
    ...
    (CL:LET* ((SELF-049 (NEW-PROPERTY-LIST)))
     (CL:SETF (%THE-PLIST SELF-049)
      (LIST* KWD-READ-SYMBOL KWD-READ-SYMBOL KWD-READ-ESCAPED-SYMBOL
       ...
       KWD-READ-COMMA-SPLICE KWD-READ-COMMA-SPLICE
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:<= ITER-051 UPPER-BOUND-052) DO
      (CL:PROGN (CL:SETQ I ITER-051)
       (CL:SETQ ITER-051 (CL:1+ ITER-051)))
      (CL:LET
