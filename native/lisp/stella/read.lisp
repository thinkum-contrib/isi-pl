...
(CL:DECLAIM
  ...
  *TRANSIENTOBJECTS?* KEYWORD-SYM *MODULE* MOST-POSITIVE-LONG-INTEGER
  ...
  STANDARD-ERROR EOL EOL-STRING NIL-LIST NULL-CHARACTER NULL-INTEGER NIL
...
(CL:DEFUN PARSE-TOKENIZER-DEFINITION (DEFINITION)
  (CL:LET*
   ((TRANSITIONTABLE (NEW-VECTOR *MAX-TOKENIZER-STATES*))
    ...
    (ERRORS? CL:NIL) (TABLE (NEW-TOKENIZER-TABLE)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-005 UPPER-BOUND-002) DO (CL:SETQ I ITER-005)
     (CL:LET*
      ((J NULL-INTEGER) (ITER-006 0)
       (UPPER-BOUND-003 (CL:1- *MAX-TOKENIZER-CHARACTERS*)) (K NULL-INTEGER)
       (ITER-007 (CL:* *MAX-TOKENIZER-CHARACTERS* I)))
      (CL:DECLARE (CL:TYPE CL:FIXNUM J ITER-006 UPPER-BOUND-003 K ITER-007))
      (CL:LOOP WHILE (CL:<= ITER-006 UPPER-BOUND-003) DO (CL:SETQ J ITER-006)
       (CL:SETQ K ITER-007)
       (CL:LET
        ((SELF TRANSITIONS)
         (CH
     ...
     (CL:LET* ((TEST-VALUE-000 (%SYMBOL-NAME MODIFIER)))
      ...
      (CL:COND
       ...
       ((CL:OR (STRING-EQL? TEST-VALUE-000 "!")
         (STRING-EQL? TEST-VALUE-000 "*!") (STRING-EQL? TEST-VALUE-000 "!*"))
...
(CL:DEFUN UNSTRINGIFY-TOKENIZER-TABLE (TABLE)
  ...
  (CL:LET*
   ((RESULT (NEW-TOKENIZER-TABLE))
    (ACODE (CL:THE CL:FIXNUM (CL:CHAR-CODE #\A))) (LINE STELLA::NULL-STRING)
    (COUNT 0) (TRANSITIONS STELLA::NULL-STRING) (SEPARATOR #\|) (START 0)
    (END 0))
...
indicated by the return value being equal to start."
  ...
  (CL:IF (CL:NOT (CL:EQ (%TOKENIZER-STATE STREAM) NULL))
   ...
   (CL:RETURN-FROM BYTE-ARRAY-READ-SEQUENCE
    (NATIVE-BYTE-ARRAY-READ-SEQUENCE BUFFER (%NATIVE-STREAM STREAM) START
     END))))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:COND
     ...
     (CL:T
      (CL:SETQ END
       (NATIVE-BYTE-ARRAY-READ-SEQUENCE BUFFER NATIVESTREAM START END))))
...
(CL:DEFUN NEW-TOKENIZER-STREAM-STATE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%TOKEN-LIST SELF) NULL) (CL:SETF (%STATE-DICTIONARY SELF) NULL)
   (CL:SETF (%TABLE SELF) NULL) (CL:SETF (%STATE SELF) 1)
      ...
      (LIST* SYM-READ-STELLA-LET
       (LIST* (LIST* SYM-READ-STELLA-TOK_TABLE_ TABLE NIL)
        (LIST* SYM-READ-STELLA-TOK_TRANSITIONS_
         (LIST* SYM-READ-STELLA-TRANSITIONS SYM-READ-STELLA-TOK_TABLE_ NIL)
         NIL)
        (LIST* SYM-READ-STELLA-TOK_STATENAMES_
         (LIST* SYM-READ-STELLA-THE-ARRAY
          (LIST* SYM-READ-STELLA-STATE-NAMES SYM-READ-STELLA-TOK_TABLE_ NIL)
          NIL)
         NIL)
         ...
         (CL:IF STRING?
          (LIST*
           ...
           (LIST* SYM-READ-STELLA-TOK_BUFFER_
            (LIST* SYM-READ-STELLA-STRING-TO-TOKENIZER-BYTE-ARRAY INPUT NIL)
            NIL)
           (LIST* SYM-READ-STELLA-TOK_STATE_ (WRAP-INTEGER 1) NIL)
           (LIST* SYM-READ-STELLA-TOK_NEXTSTATE_ SYM-READ-STELLA-TOK_STATE_
            NIL)
          ...
          (LIST*
           ...
           (LIST* SYM-READ-STELLA-TOK_STATE_
            (LIST* SYM-READ-STELLA-GET-SAVED-STATE
             SYM-READ-STELLA-TOK_STREAMSTATE_ SYM-READ-STELLA-TOK_TABLE_ NIL)
            NIL)
           (LIST* SYM-READ-STELLA-TOK_NEXTSTATE_ SYM-READ-STELLA-TOK_STATE_
            NIL)
           ...
           (LIST* SYM-READ-STELLA-TOK_END_
            (LIST* SYM-READ-STELLA-END SYM-READ-STELLA-TOK_STREAMSTATE_ NIL)
            NIL)
...
(CL:DEFMETHOD NEXT? ((SELF STREAM-TOKENIZER))
  (CL:LET*
   ((TOK_TABLE_ (%TABLE SELF)) (TOK_TRANSITIONS_ (%TRANSITIONS TOK_TABLE_))
    ...
    (TOK_NEXTSTATE_ TOK_STATE_) (TOK_CURSOR_ (%CURSOR TOK_STREAMSTATE_))
       ...
       ((CL:= TOK_CURSOR_ TOK_END_)
        ...
        (CL:WHEN TOK_ENDOFTOKENS?_ (CL:SETQ TOK_CHECKPOINT_ TOK_CURSOR_)
         (CL:COND ((CL:= TOK_NEXTSTATE_ -1))
          ...
          (CL:T (CL:SETQ TOK_ENDOFTOKENS?_ CL:NIL) (CL:SETQ TOK_STATE_ 0)
           (CL:SETQ TOK_NEXTSTATE_ -1)))
         (CL:RETURN))
...
(CL:DEFUN GET-TOKEN-LONG-INTEGER-INTERNAL (BUFFER START END SIZE)
  ...
  (CL:CHECK-TYPE SIZE CL:FIXNUM)
  (CL:LET*
   ((LENGTH (CL:- END START)) (AUXEND END) (RESULT 0) (NEGATIVE? CL:NIL)
    (DIGIT 0))
...
covering all special cases here; but we are fast :-)"
  ...
  (CL:RETURN-FROM GET-TOKEN-FLOAT
   (LIST* SYM-READ-STELLA-GET-TOKEN-FLOAT-INTERNAL SYM-READ-STELLA-TOK_BUFFER_
    SYM-READ-STELLA-TOK_TOKENSTART_ SYM-READ-STELLA-TOK_CURSOR_
    SYM-READ-STELLA-TOK_SIZE_ NIL)))

;;; (DEFUN (GET-TOKEN-FLOAT-INTERNAL FLOAT) ...)

(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:FIXNUM CL:FIXNUM CL:FIXNUM) CL:DOUBLE-FLOAT)
    ...
    (CL:LOOP WHILE (CL:< START AUXEND) DO
     ...
     (CL:CASE CH
      ...
      (CL:OTHERWISE
       (CL:SETQ DIGIT
        (CL:TRUNCATE
         (CL:- (CL:THE CL:FIXNUM (CL:CHAR-CODE CH)) #.(CL:char-code #\0))))
    ...
    (CL:SETQ RESULT
     (CL:* RESULT
      (CL:THE CL:DOUBLE-FLOAT
       (CL:EXPT 10.0d0 (CL:THE CL:DOUBLE-FLOAT (CL:FLOAT EXPONENT 0.0d0)))))))
...
(CL:DEFUN TOKENIZE-S-EXPRESSION (STREAM TOKENLIST)
  (CL:LET*
   ((UPCASE? (CL:NOT (%CASE-SENSITIVE? *MODULE*))) (PARENBALANCE 0)
    (ENDOFFILE? CL:T) (SKIPCLOSINGQUOTE? CL:NIL) (TOKENCURSOR TOKENLIST))
   ...
   (CL:LET*
    ((TOK_TABLE_ *STELLA-TOKENIZER-TABLE*)
     ...
     (TOK_STATENAMES_ (%THE-ARRAY (%STATE-NAMES TOK_TABLE_)))
     (TOK_TOKENSTART_ -1) (TOK_ENDOFTOKENS?_ CL:NIL) (TOK_INPUTSTREAM_ STREAM)
     (TOK_ECHOSTREAM_ (%ECHO-STREAM TOK_INPUTSTREAM_))
     (TOK_STREAMSTATE_
      (CL:IF (CL:EQ (%TOKENIZER-STATE TOK_INPUTSTREAM_) NULL)
       (CL:SETF (%TOKENIZER-STATE TOK_INPUTSTREAM_)
          ...
          (CL:COND
           ...
           ((CL:EQ TEST-VALUE-000 KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL)
            (CL:LET*
             ((SEPARATORPOS
               (GET-QUALIFIED-SYMBOL-SEPARATOR-POSITION-INTERNAL TOK_BUFFER_
                TOK_TOKENSTART_ TOK_CURSOR_ TOK_SIZE_
...
10-15 times faster than `unstringify'."
  ...
  (CL:LET*
   ((SYMBOLNAME STELLA::NULL-STRING) (MODULENAME STELLA::NULL-STRING)
    ...
    (UPCASE?
     (CL:AND ENABLECASECONVERSION? (CL:NOT (%CASE-SENSITIVE? *MODULE*)))))
   ...
   (CL:LET*
    ((TOK_TABLE_ *STELLA-TOKENIZER-TABLE*)
     (TOK_TRANSITIONS_ (%TRANSITIONS TOK_TABLE_))
     (TOK_STATENAMES_ (%THE-ARRAY (%STATE-NAMES TOK_TABLE_)))
     (TOK_TOKENSTART_ -1) (TOK_ENDOFTOKENS?_ CL:NIL) (TOK_STREAMSTATE_ NULL)
     (TOK_BUFFER_
      (CL:LET ((STRING NAME)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING STRING))
       STRING))
     (TOK_STATE_ 1) (TOK_NEXTSTATE_ TOK_STATE_) (TOK_CURSOR_ 0)
     (TOK_SIZE_ (CL:THE CL:FIXNUM (CL:LENGTH NAME))) (TOK_END_ TOK_SIZE_))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TOK_TRANSITIONS_)
     (CL:TYPE CL:SIMPLE-VECTOR TOK_STATENAMES_)
     (CL:TYPE CL:FIXNUM TOK_TOKENSTART_ TOK_STATE_ TOK_NEXTSTATE_ TOK_CURSOR_
      TOK_SIZE_ TOK_END_))
    (CL:PROGN (CL:SETQ TOK_STATENAMES_ TOK_STATENAMES_)
     (CL:SETQ TOK_ENDOFTOKENS?_ TOK_ENDOFTOKENS?_))
    ...
    (CL:LET*
     ...
     (CL:IF
      ...
      (CL:LET* ((STREAM-006 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-006) "Garbage found after `"
        NAME "'")
         ...
         ((CL:EQ MODULE NULL)
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Module `" MODULENAME
            "' does not exist; current token=`" NAME "'")
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:IF
      ...
      (CL:LET
       ...
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET
       ((SELF (%THE-ARRAY CHARTYPETABLE)) (VALUE KWD-READ-TEXT) (POSITION I))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
        (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
so we leave this for now as a feature."
  (CL:DECLARE
   (CL:TYPE CL:SIMPLE-STRING STRING PUNCTUATIONCHARS QUOTECHARS ESCAPECHARS))
  #+MCL
  (CL:CHECK-TYPE STRING CL:SIMPLE-STRING)
  #+MCL
  (CL:CHECK-TYPE PUNCTUATIONCHARS CL:SIMPLE-STRING)
  ...
  (CL:LET*
   ...
   (CL:WHEN *MEMOIZATION-ENABLED?*
    (CL:SETQ MEMO-TABLE-000
     (%SURROGATE-VALUE SGT-READ-STELLA-F-TOKENIZE-STRING-MEMO-TABLE-000))
    (CL:WHEN (CL:EQ MEMO-TABLE-000 NULL)
     (INITIALIZE-MEMOIZATION-TABLE
      SGT-READ-STELLA-F-TOKENIZE-STRING-MEMO-TABLE-000 "(:MAX-VALUES 10)")
     (CL:SETQ MEMO-TABLE-000
      (%SURROGATE-VALUE SGT-READ-STELLA-F-TOKENIZE-STRING-MEMO-TABLE-000)))
    (CL:SETQ MEMOIZED-ENTRY-000
     (LOOKUP-MRU-MEMOIZED-VALUE MEMO-TABLE-000 (WRAP-STRING PUNCTUATIONCHARS)
      (WRAP-STRING QUOTECHARS) (WRAP-STRING ESCAPECHARS) MEMOIZED-NULL-VALUE
      0))
   ...
   (CL:COND
    ...
    (CL:T
     (CL:SETQ MEMOIZED-VALUE-000
      (CREATE-TOKENIZE-STRING-TABLE PUNCTUATIONCHARS QUOTECHARS ESCAPECHARS))
     (CL:WHEN *MEMOIZATION-ENABLED?*
      (CL:SETF (%%VALUE MEMOIZED-ENTRY-000)
       (CL:IF (CL:EQ MEMOIZED-VALUE-000 NULL) MEMOIZED-NULL-VALUE
        MEMOIZED-VALUE-000)))))
   (CL:LET*
    ((CHARTYPETABLE MEMOIZED-VALUE-000) (I 0) (CH NULL-CHARACTER) (START 0)
     (END (CL:THE CL:FIXNUM (CL:LENGTH STRING))) (STATE KWD-READ-TEXT)
     (NEWSTATE KWD-READ-TEXT) (QUOTEDTOKEN? CL:NIL) (QUOTECHAR NULL-CHARACTER)
     (ENDOFTOKEN? CL:NIL) (ESCAPEPOSITIONS NIL) (TOKEN STELLA::NULL-STRING)
     (RESULT NIL))
        ...
        ((CL:EQ NEWSTATE KWD-READ-TEXT)
         (CL:COND
          ((CL:OR (CL:EQ STATE KWD-READ-PUNCTUATION)
            (CL:EQ STATE KWD-READ-WHITE-SPACE) (CL:EQ STATE KWD-READ-QUOTE))
        ...
        ((CL:EQ NEWSTATE KWD-READ-ESCAPE)
         (CL:COND
          ((CL:OR (CL:EQ STATE KWD-READ-PUNCTUATION)
           ...
           (CL:SETQ ENDOFTOKEN? CL:T))
          (CL:T))
         (CL:SETQ ESCAPEPOSITIONS (CONS (WRAP-INTEGER I) ESCAPEPOSITIONS))
          ...
          (CL:LET* ((PARTS NIL) (S START)) (CL:DECLARE (CL:TYPE CL:FIXNUM S))
           (CL:LET* ((E NULL) (ITER-000 (REVERSE ESCAPEPOSITIONS)))
            (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
             ...
             (CL:SETQ PARTS
              (CONS (WRAP-STRING (SUBSEQUENCE STRING S (%WRAPPER-VALUE E)))
               PARTS))
             (CL:SETQ S (CL:1+ (%WRAPPER-VALUE E)))
             (CL:SETQ ITER-000 (%%REST ITER-000))))
           (CL:SETQ PARTS
            (CONS (WRAP-STRING (SUBSEQUENCE STRING (MIN S END) (MIN I END)))
             PARTS))
           ...
           (CL:LET* ((PART NULL) (ITER-001 (%%REST (%%REST PARTS))))
            (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
             ...
             (CL:SETQ TOKEN (STRING-CONCATENATE TOKEN (%WRAPPER-VALUE PART)))
             (CL:SETQ ITER-001 (%%REST ITER-001)))))
...
(CL:DEFUN MAKE-TOKENIZER-STRING-STREAM (STRING)
  ...
  (CL:LET*
   ...
   (CL:SETF (%TABLE STATE) NULL) (CL:SETF (%STATE-DICTIONARY STATE) NULL)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) (CL:VALUES CL:SIMPLE-STRING CL:T)) READ-LINE2))
...
the input is read.  See also special variable `*USER-QUERY-ACTION*'."
  ...
  (CL:COND
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-YES) (CL:RETURN-FROM YES-OR-NO? CL:T))
   ...
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-NO-VERBOSE)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) MESSAGE " [Auto => NO]"
     EOL)
    (CL:RETURN-FROM YES-OR-NO? CL:NIL))
...
the input is read.  See also special variable `*USER-QUERY-ACTION*'."
  ...
  (CL:CHECK-TYPE MESSAGE CL:SIMPLE-STRING)
  (CL:COND
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-YES) (CL:RETURN-FROM Y-OR-N? CL:T))
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-NO) (CL:RETURN-FROM Y-OR-N? CL:NIL))
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-YES-VERBOSE)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) MESSAGE " [Auto => Y]"
     EOL)
    (CL:RETURN-FROM Y-OR-N? CL:T))
   ((CL:EQ *USER-QUERY-ACTION* KWD-READ-NO-VERBOSE)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) MESSAGE " [Auto => N]"
     EOL)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING) FILE-TO-STRING))
...
(CL:DEFUN HELP-STARTUP-READ3 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-READ-QUALIFIED-ESCAPED-SURROGATE-ESCAPE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "QUALIFIED-ESCAPED-SURROGATE-ESCAPE" NULL
     2))
   (CL:SETQ KWD-READ-QUALIFIED-FULLY-ESCAPED-SURROGATE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "QUALIFIED-FULLY-ESCAPED-SURROGATE" NULL
     2))
   ...
   (CL:SETQ KWD-READ-QUALIFIED-FULLY-ESCAPED-SYMBOL-ESCAPE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "QUALIFIED-FULLY-ESCAPED-SYMBOL-ESCAPE"
     NULL 2))
...
(CL:DEFUN HELP-STARTUP-READ4 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-READ-STELLA-F-TOKENIZE-STRING-MEMO-TABLE-000
    (INTERN-RIGID-SYMBOL-WRT-MODULE "F-TOKENIZE-STRING-MEMO-TABLE-000" NULL
     1))
   ...
   (CL:LET* ((SELF-061 (NEW-PROPERTY-LIST)))
    (CL:SETF (%THE-PLIST SELF-061)
     (LIST* KWD-READ-SYMBOL KWD-READ-SYMBOL KWD-READ-ESCAPED-SYMBOL
      ...
      KWD-READ-CL-SYMBOL KWD-READ-SYMBOL KWD-READ-SURROGATE KWD-READ-SURROGATE
      KWD-READ-ESCAPED-SURROGATE KWD-READ-SURROGATE
      ...
      KWD-READ-COMMA KWD-READ-COMMA KWD-READ-COMMA-SPLICE
      KWD-READ-COMMA-SPLICE KWD-READ-CLOSE-STRING
   ...
   (CL:SETQ *READ-LINE-TOKENIZER-TABLE-DEFINITION*
    (LIST*
     (LIST* KWD-READ-START SYM-READ-STELLA-*
      ...
      KWD-READ-INITIAL-RETURN KWD-READ-EOF KWD-READ-EOF SYM-READ-STELLA-*
     ...
     (LIST* KWD-READ-RETURN SYM-READ-STELLA-*
      ...
      KWD-READ-INITIAL-RETURN KWD-READ-EOF KWD-READ-EOF SYM-READ-STELLA-*
      KWD-READ-OTHERWISE KWD-READ-LINE NIL)
     (LIST* KWD-READ-LINEFEED KWD-READ-EOF KWD-READ-EOF SYM-READ-STELLA-*
      (CONS (WRAP-CHARACTER #\Linefeed) NIL) KWD-READ-INITIAL-LINEFEED
      SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Return) NIL)
      KWD-READ-INITIAL-RETURN SYM-READ-STELLA-* KWD-READ-OTHERWISE
      KWD-READ-LINE NIL)
     (LIST* KWD-READ-LINE SYM-READ-STELLA-*
      (CONS (WRAP-CHARACTER #\Linefeed) NIL) KWD-READ-LINEFEED
      SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Return) NIL) KWD-READ-RETURN
      SYM-READ-STELLA-* KWD-READ-EOF KWD-READ-EOF KWD-READ-OTHERWISE
      KWD-READ-LINE NIL)
     (LIST* KWD-READ-ERROR KWD-READ-INCLUDE KWD-READ-START NIL) NIL))
   (CL:SETQ *READ-LINE2-TOKENIZER-TABLE-DEFINITION*
    (LIST*
     (LIST* KWD-READ-START SYM-READ-STELLA-*
      (CONS (WRAP-CHARACTER #\Linefeed) NIL) KWD-READ-LINEFEED
      SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Return) NIL) KWD-READ-RETURN
      KWD-READ-EOF KWD-READ-EOF SYM-READ-STELLA-* KWD-READ-OTHERWISE
      KWD-READ-LINE NIL)
     (LIST* KWD-READ-LINE SYM-READ-STELLA-*
      (CONS (WRAP-CHARACTER #\Linefeed) NIL) KWD-READ-LINEFEED
      SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Return) NIL) KWD-READ-RETURN
     ...
     (LIST* KWD-READ-RETURN-LINEFEED SYM-READ-STELLA-* KWD-READ-EOF
      KWD-READ-EOF SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Linefeed) NIL)
      KWD-READ-LINEFEED SYM-READ-STELLA-* (CONS (WRAP-CHARACTER #\Return) NIL)
...
`start' stopping just before `end'.\")"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((STATE TOKENIZER-STREAM-STATE)))"
    (CL:FUNCTION CLEAR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD RESET ((STATE TOKENIZER-STREAM-STATE)))"
...
pathname or a module pathname ending with a `/'.  Assumes that `name'
is the printed representation of a STELLA symbol (potentially containing
escape characters).\" :PUBLIC? TRUE)" (CL:FUNCTION QUALIFIED-STELLA-NAME?)
    NULL)
...
the input is read.  See also special variable `*USER-QUERY-ACTION*'.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-READ"
     "(DEFUN STARTUP-READ () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-READ) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-READ-STELLA-STARTUP-READ)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-READ-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupRead")
      NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *TRACE-GET-NEXT-TOKEN?* BOOLEAN FALSE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
