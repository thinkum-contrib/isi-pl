...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* TRUE *CHARACTER-TYPE-TABLE*
  NULL-CHARACTER NULL-INTEGER FALSE TRUE-WRAPPER NIL))
...
easier to use when called from non-Lisp languages."
  ...
  (CL:LET*
   ((LEN (CL:THE CL:FIXNUM (CL:LENGTH NAMESTRING))) (RESULT NIL)
    (LAG 0) (LEAD 0) (INCAPRUN? FALSE) (INNUMBER? FALSE))
     ...
     (CL:LET* ((FOUND?-000 FALSE))
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
        ...
        (CL:WHEN (CL:EQL SEP C) (CL:SETQ FOUND?-000 TRUE)
         (CL:RETURN))))
      (CL:IF FOUND?-000
       (CL:PROGN
        (CL:SETQ RESULT
...
(CL:DEFUN ORDINAL-NUMBER-STRING? (TESTSTRING)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (C NULL-CHARACTER) (VECTOR-000 TESTSTRING)
     (INDEX-000 0)
     (LENGTH-000 (CL:THE CL:FIXNUM (CL:LENGTH VECTOR-000)))
     (ITER-000 0) (UPPER-BOUND-000 (CL:- LEN 3)))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING VECTOR-000)
     (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 ITER-000 UPPER-BOUND-000
      I))
    (CL:LOOP WHILE
