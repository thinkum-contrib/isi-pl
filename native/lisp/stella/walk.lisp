...
(CL:DEFVAR SYM-WALK-STELLA-KWD NULL)
(CL:DEFVAR SYM-WALK-STELLA-IN NULL)
(CL:DEFVAR SYM-WALK-STELLA-QUOTE NULL)
(CL:DEFVAR SYM-WALK-STELLA-WHERE NULL)
(CL:DEFVAR SYM-WALK-STELLA-MEMB? NULL)
(CL:DEFVAR SYM-WALK-STELLA-TRACE-IF NULL)
(CL:DEFVAR SYM-WALK-STELLA-TRACE-IF-EXPANDER NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-MACRO NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-NULL-INTEGER NULL)
(CL:DEFVAR SYM-WALK-STELLA-NULL-FLOAT NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-STRINGIFY NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-STORE-MACRO NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-FUNCTION-CODE NULL)
...
(CL:DEFVAR SYM-WALK-STELLA-INCREMENTALLY-TRANSLATE NULL)
...
(CL:DECLAIM
 (CL:SPECIAL NULL-INTEGER-WRAPPER NULL-FLOAT-WRAPPER
  NULL-FUNCTION-CODE-WRAPPER *CURRENTSYSTEMDEFINITION* NULL-STRING-WRAPPER
  *STELLA-MODULE* *STARTUP-TIME-PHASES* NIL-LIST *INLININGMETHODCALL?*
  KEYWORD-SYM SURROGATE-SYM SYMBOL-SYM STANDARD-ERROR
  *MIXIN-IMPLEMENTATION-STYLE* *CURRENTFILE* STANDARD-WARNING TRUE-WRAPPER
  FALSE-WRAPPER *MODULE* *TRANSLATOROUTPUTLANGUAGE* STANDARD-OUTPUT EOL))
...
features and print a list of all Stella environment features."
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
   "CURRENTLY ENABLED ENVIRONMENT FEATURES:" EOL "   "
   *CURRENT-ENVIRONMENT-FEATURES* EOL EOL
   "AVAILABLE ENVIRONMENT FEATURES:" EOL "   "
   *AVAILABLE-ENVIRONMENT-FEATURES* EOL)
  :VOID)
...
keywords in 'features'."
  (CL:LET* ((F NULL) (ITER-000 FEATURES))
   (CL:LOOP WHILE ITER-000 DO (CL:SETQ F (CL:POP ITER-000))
    (CL:COND
     ((CL:OR (CL:EQ F KWD-WALK-SUPPRESS-WARNINGS)
      ...
      (CL:WHEN (CL:NOT (MEMBER? *CURRENT-ENVIRONMENT-FEATURES* F))
       (INSERT *CURRENT-ENVIRONMENT-FEATURES* F)))
     (CL:T
      (CL:IF (MEMBER? *AVAILABLE-ENVIRONMENT-FEATURES* F)
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
         "Oops. Forgot to implement environment feature:  `" F "'")
        (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
       (CL:WARN "Unrecognizable environment feature:  `~A'" F))))
    (CL:COND
     ((CL:EQ F KWD-WALK-USE-COMMON-LISP-STRUCTS)
      (REMOVE *CURRENT-ENVIRONMENT-FEATURES*
     ...
     ((CL:EQ F KWD-WALK-USE-COMMON-LISP-VECTOR-STRUCTS)
      (CL:WHEN
       ...
       (INSERT *CURRENT-ENVIRONMENT-FEATURES*
        KWD-WALK-USE-COMMON-LISP-STRUCTS)))
     (CL:T))))
  :VOID)
    ...
    (CL:COND
     ...
     (CL:T
      (CL:IF (MEMBER? *AVAILABLE-ENVIRONMENT-FEATURES* F)
       ...
       (CL:WARN "Unrecognizable environment feature:  `~A'" F))))
    (CL:COND
     ((CL:OR (CL:EQ F KWD-WALK-USE-COMMON-LISP-STRUCTS)
       (CL:EQ F KWD-WALK-USE-COMMON-LISP-VECTOR-STRUCTS))
      (REMOVE *CURRENT-ENVIRONMENT-FEATURES*
       KWD-WALK-USE-COMMON-LISP-STRUCTS)
      (REMOVE *CURRENT-ENVIRONMENT-FEATURES*
...
settings."
  (CL:LET*
   ((F NULL) (ITER-000 (%THE-CONS-LIST *CURRENT-ENVIRONMENT-FEATURES*)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ F (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:WHEN (CL:NOT (MEMBER? *DEFAULT-ENVIRONMENT-FEATURES* F))
     (DISABLE-ENVIRONMENT-FEATURE F))))
  (CL:LET*
   ((F NULL) (ITER-001 (%THE-CONS-LIST *DEFAULT-ENVIRONMENT-FEATURES*)))
...
(CL:DEFUN TRACE-IF-EXPANDER (KEYWORD BODY)
  (CL:RETURN-FROM TRACE-IF-EXPANDER
   (LIST* SYM-WALK-STELLA-WHEN
    (LIST* SYM-WALK-STELLA-AND
     (LIST* SYM-WALK-STELLA-DEFINED? SYM-WALK-STELLA-*TRACED-KEYWORDS*
      NIL)
     (CL:IF (CONS? KEYWORD)
      (LIST* SYM-WALK-STELLA-EXISTS SYM-WALK-STELLA-KWD SYM-WALK-STELLA-IN
       (LIST* SYM-WALK-STELLA-QUOTE KEYWORD NIL) SYM-WALK-STELLA-WHERE
       (LIST* SYM-WALK-STELLA-MEMB? SYM-WALK-STELLA-*TRACED-KEYWORDS*
        SYM-WALK-STELLA-KWD NIL)
       NIL)
      (LIST* SYM-WALK-STELLA-MEMB? SYM-WALK-STELLA-*TRACED-KEYWORDS*
       KEYWORD NIL))
     NIL)
    (CONS SYM-WALK-STELLA-PRINT (CONCATENATE (COPY-CONS-LIST BODY) NIL))
    NIL)))
...
(CL:PROGN (CL:DEFVAR *SAFETY* 3 "Integer between 0 and 3.  Higher levels call more
safety checks.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *SAFETY*)))

;;; (DEFSPECIAL *DEBUGLEVEL* ...)

(CL:PROGN (CL:DEFVAR *DEBUGLEVEL* 3 "Integer between 0 and 3.  Higher levels generate more
code to aid debugging.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *DEBUGLEVEL*)))

;;; (DEFSPECIAL *OPTIMIZESPEEDLEVEL* ...)

(CL:PROGN (CL:DEFVAR *OPTIMIZESPEEDLEVEL* 3 "Integer between 0 and 3.  Higher levels optimize for
greater execution speed.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *OPTIMIZESPEEDLEVEL*)))

;;; (DEFSPECIAL *OPTIMIZESPACELEVEL* ...)

(CL:PROGN (CL:DEFVAR *OPTIMIZESPACELEVEL* 3 "Integer between 0 and 3.  Higher levels optimize for
less code size and memory consumption.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *OPTIMIZESPACELEVEL*)))
...
(CL:DEFUN GET-QUOTED-TREE (TREENAME MODULENAME)
  ...
  (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TREENAME MODULENAME))
  (CL:LET* ((QUOTEDTREE (LOOKUP *QUOTATION-TABLE* TREENAME)))
   ...
   (CL:LET* ((*MODULE* (GET-STELLA-MODULE MODULENAME TRUE)))
    ...
    (CL:LET* ((TREE (READ-S-EXPRESSION-FROM-STRING TREENAME)))
...
(CL:DEFCLASS TRANSLATION-UNIT (STANDARD-OBJECT)
  ((TU-HOME-MODULE :ALLOCATION :INSTANCE :ACCESSOR %TU-HOME-MODULE)
   ...
   (ANNOTATION :ALLOCATION :INSTANCE :ACCESSOR %ANNOTATION)
   ...
   (REFERENCED-GLOBALS :ALLOCATION :INSTANCE :ACCESSOR
    %REFERENCED-GLOBALS)))
...
(CL:DEFUN ACCESS-TRANSLATION-UNIT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-WALK-STELLA-AUXILIARY?)
    (CL:IF SETVALUE?
     (CL:SETF (%AUXILIARY? SELF)
      (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE))
     (CL:SETQ VALUE
      (CL:IF (%AUXILIARY? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
  ...
  (CL:LET* ((TEST-VALUE-000 (%CATEGORY SELF)))
   (CL:COND
    ((CL:OR (CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-CLASS)
      ...
      (CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-MACRO)
...
(CL:PROGN (CL:DEFVAR *TRANSLATIONVERBOSITYLEVEL* 1 "The higher the level, the more progress annotations are
generated during the translation of Stella declarations.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *TRANSLATIONVERBOSITYLEVEL*)))
...
(CL:DEFUN %SET-TRANSLATOR-OUTPUT-LANGUAGE (NEWLANGUAGE)
  ...
  (CL:LET* ((OLDLANGUAGE *TRANSLATOROUTPUTLANGUAGE*))
   ...
   (CL:SETQ *TRANSLATOROUTPUTLANGUAGE* NEWLANGUAGE)
   ...
   (CL:LET* ((ANNOTATION STELLA::NULL-STRING))
    ...
    (CL:WHEN CREATEANNOTATION?
     ...
     (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 2)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Defining `"
       ANNOTATION "'" EOL)))
  ...
  (CL:LET*
   ...
   (CL:LET* ((TEST-VALUE-000 (%CATEGORY UNIT)))
    (CL:COND
     ...
     ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-MACRO) (WALK-MACRO-UNIT UNIT))
...
(CL:DEFUN HELP-WALK-AUXILIARY-TREE (TREE FINALIZE?)
  ...
  (CL:LET* ((UNIT (POP *TRANSLATIONUNITS*)))
   (CL:SETF (%AUXILIARY? UNIT) TRUE) (WALK-PHASE-ONE-UNIT UNIT)
...
(CL:PROGN (CL:DEFVAR *TRANSLATIONERRORS* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *TRANSLATIONERRORS*)))

;;; (DEFSPECIAL *TRANSLATIONWARNINGS* ...)

(CL:PROGN (CL:DEFVAR *TRANSLATIONWARNINGS* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *TRANSLATIONWARNINGS*)))
    ...
    (CL:LET*
     ...
     (CL:COND
      ((CL:EQ CATEGORY SYM-WALK-STELLA-CLASS)
       ...
       (CL:COND
        ((SUBTYPE-OF-CLASS? (SAFE-PRIMARY-TYPE OBJECT))
         (CL:PROGN
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (NAME OBJECT))))
      ...
      ((CL:OR (CL:EQ CATEGORY SYM-WALK-STELLA-METHOD)
        ...
        (CL:EQ CATEGORY SYM-WALK-STELLA-MACRO))
       (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE OBJECT)))
        (CL:COND
         ((SUBTYPE-OF-METHOD-SLOT? TEST-VALUE-001)
          (CL:PROGN
           (CL:WHEN (CL:EQ CATEGORY SYM-WALK-STELLA-MACRO)
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "macro expander "))
           (CL:COND
         ...
         ((CL:EQ TEST-VALUE-001 SGT-WALK-STELLA-CONS)
          (CL:PROGN
           (CL:IF (CL:EQ (%%VALUE OBJECT) SYM-WALK-STELLA-DEFUN)
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "function ")
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "method "))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
      ...
      ((CL:EQ CATEGORY SYM-WALK-STELLA-GLOBAL-VARIABLE)
       (CL:LET* ((TEST-VALUE-002 (SAFE-PRIMARY-TYPE OBJECT)))
        (CL:COND
         ((SUBTYPE-OF? TEST-VALUE-002 SGT-WALK-STELLA-GLOBAL-VARIABLE)
          (CL:PROGN
         ...
         (CL:T
          (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003) "`" TEST-VALUE-002
            "' is not a valid case option")
           (CL:ERROR
        ...
        (CL:COND
         ((CL:EQ TEST-VALUE-003 SGT-WALK-STELLA-CONS)
          (CL:PROGN
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
            (%%VALUE (%%REST OBJECT)))))
         ((SUBTYPE-OF? TEST-VALUE-003 SGT-WALK-STELLA-TYPE-SPEC)
          (CL:PROGN
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
            (YIELD-TYPE-SPEC-TREE OBJECT))))
         (CL:T
          (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-004) "`" TEST-VALUE-003
            "' is not a valid case option")
           (CL:ERROR
      ...
      ((CL:EQ CATEGORY SYM-WALK-STELLA-MODULE)
       ...
       (CL:LET* ((TEST-VALUE-004 (SAFE-PRIMARY-TYPE OBJECT)))
        (CL:COND
         ((SUBTYPE-OF? TEST-VALUE-004 SGT-WALK-STELLA-MODULE)
          (CL:PROGN
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
            (%MODULE-NAME OBJECT))))
         ...
         (CL:T
          (CL:LET* ((STREAM-005 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-005) "`" TEST-VALUE-004
            "' is not a valid case option")
           (CL:ERROR
             ...
             (CL:WHEN (CL:NOT TEST-VALUE-001)
              (CL:SETQ TEST-VALUE-001
               (CL:AND (CL:NOT (CL:EQ *CURRENTFILE* STELLA::NULL-STRING))
                (CL:OR
                 (DEFINED? (STRING-SEARCH *CURRENTFILE* "cl-primal" 0))
                 (DEFINED?
                  (STRING-SEARCH *CURRENTFILE* "stella-to-cl"
                   0))))))))))))
...
(CL:PROGN (CL:DEFVAR *LOG-BREAK-POINT-COUNTER* NULL-INTEGER) (CL:DECLAIM (CL:TYPE CL:FIXNUM *LOG-BREAK-POINT-COUNTER*)))
...
(CL:DEFUN VARIABLE-EQL? (VAR1 VAR2)
  (CL:RETURN-FROM VARIABLE-EQL?
   (CL:OR (CL:EQ VAR1 VAR2)
    (CL:AND (STRING-EQL? (%SYMBOL-NAME VAR1) (%SYMBOL-NAME VAR2))
     (CL:OR (CL:EQ (%HOME-CONTEXT VAR1) NULL)
      (CL:EQ (%HOME-CONTEXT VAR2) NULL))))))
...
(CL:DEFUN WARN-ABOUT-UNKNOWN-SOURCE-TYPE (TREE)
  (CL:WHEN (WARN-ABOUT-UNDEFINED-METHODS?)
       ...
       (CL:LET* ((TEST-VALUE-000 (%%VALUE TREE)))
        (CL:COND
         ...
         ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-SYS-CALL-METHOD)
          (CL:WHEN
           (CL:OR (SYMBOL-COMMON-LISP? (%%VALUE (%%REST (%%REST TREE))))
            (CL:EQ
             (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE (%%VALUE (%%REST TREE)))
         ...
         ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-SYS-CALL-FUNCTION)
          (CL:WHEN
           (CL:EQ
            (%SLOT-BASE-TYPE (LOOKUP-FUNCTION (%%VALUE (%%REST TREE))))
            SGT-WALK-STELLA-UNKNOWN)
           (CL:RETURN-FROM WARN-ABOUT-UNKNOWN-SOURCE-TYPE)))
   ...
   (CL:LET* ((*PRINTREADABLY?* TRUE))
    ...
    (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
     (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL " The expression"
      EOL "   `" (DE-UGLIFY-PARSE-TREE TREE) "'" EOL " has UNKNOWN type"
      EOL "."))))
  :VOID)
   ...
   (CL:WHEN (CL:EQ SOURCETYPE SGT-WALK-STELLA-UNKNOWN)
    ...
    (CL:IF
     ...
     (CL:RETURN-FROM HELP-COERCE-A-TREE
      (CL:VALUES (TYPE-TO-WALKED-NULL-VALUE-TREE TARGETBASETYPE)
       TARGETTYPE TRUE))
     (CL:RETURN-FROM HELP-COERCE-A-TREE
      (CL:VALUES TREE TARGETTYPE TRUE))))
    ...
    (CL:WHEN (CL:AND (VOID? SOURCETYPE) (CL:NOT (VOID? TARGETTYPE)))
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Illegal argument to `" (DE-UGLIFY-PARSE-TREE OPERATORNAME) "':"
        EOL "   `" (DE-UGLIFY-PARSE-TREE OEXP) "'" EOL
   ...
   (CL:LOOP WHILE
    ...
    (CL:WHEN
     ...
     (CL:IF (ARRAY-TYPE-SPECIFIER? MOSTGENERALRETURNTYPE)
      ...
      (CL:SETQ MOSTGENERALRETURNTYPE
       (%SLOT-BASE-TYPE MOSTGENERALMETHOD)))))
      ...
      (CL:IF (ANCHORED-TYPE-SPECIFIER? SLOTTYPE)
       ...
       (CL:LET* ((FOUND?-000 FALSE))
        (CL:LET*
         ((TS NULL)
          (ITER-000
           (%THE-CONS-LIST (%SPECIFIER-PARAMETER-TYPES SLOTTYPE))))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET* ((TEST-VALUE-000 (%%VALUE TREE)))
       (CL:COND
        ...
        ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-BAD-SYS)
         (CL:RETURN-FROM HELP-WALK-A-TREE
          (CL:VALUES TREE (%%VALUE (%%REST (%%REST TREE))))))
        ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-SETQ)
...
(CL:PROGN (CL:DEFVAR *CURRENTFILE* STELLA::NULL-STRING "Name of file that is currently being translated.
A NULL value indicates an incremental translation.") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CURRENTFILE*)))
    ...
    (CL:PROGN
     (CL:RETURN-FROM YIELD-SYMBOL-CONSTANT-NAME
      (INTERN-SYMBOL-IN-MODULE
       (STRING-UPCASE (CONSTRUCT-SYMBOL-CONSTANT-NAME SYMBOL)) *MODULE*
       TRUE))))
   (CL:T
    (CL:RETURN-FROM YIELD-SYMBOL-CONSTANT-NAME
     (INTERN-SYMBOL-IN-MODULE
      ...
      (CONS
       (LIST* SYM-WALK-STELLA-SAFE-CAST
        (LIST* SYM-WALK-STELLA-INTERN-RIGID-SYMBOL-WRT-MODULE
         ...
         (CONS
          (CL:IF
           (CL:OR (CL:EQ (%HOME-CONTEXT SYMBOL) *MODULE*)
            (KEYWORD? SYMBOL))
...
(CL:DEFUN LOOKUP-CONSTANT-SYMBOL (SELF)
  (CL:COND
   ((CL:OR (CL:EQ SELF SYM-WALK-STELLA-TRUE)
     (CL:EQ SELF SYM-WALK-STELLA-FALSE))
    (CL:RETURN-FROM LOOKUP-CONSTANT-SYMBOL SGT-WALK-STELLA-BOOLEAN))
   ((CL:EQ SELF SYM-WALK-STELLA-NIL)
    (CL:RETURN-FROM LOOKUP-CONSTANT-SYMBOL SGT-WALK-STELLA-CONS))
   ((CL:EQ SELF SYM-WALK-STELLA-NULL)
    (CL:RETURN-FROM LOOKUP-CONSTANT-SYMBOL SGT-WALK-STELLA-UNKNOWN))
   ((CL:EQ SELF SYM-WALK-STELLA-NULL-INTEGER)
    (CL:RETURN-FROM LOOKUP-CONSTANT-SYMBOL SGT-WALK-STELLA-INTEGER))
   ((CL:EQ SELF SYM-WALK-STELLA-NULL-FLOAT)
    (CL:RETURN-FROM LOOKUP-CONSTANT-SYMBOL SGT-WALK-STELLA-FLOAT))
   ...
   (CL:LET* ((TEST-VALUE-000 OPERATOR))
    (CL:COND
        ...
        (CL:LET*
         ...
         (CL:WHEN (CL:NOT (CL:EQ (%%REST OTHERARGSTEST) NIL))
          (CL:SETQ OTHERARGSTEST
           (CONS
            (CONS SYM-WALK-STELLA-PROGN (CONCATENATE OTHERARGSTEST NIL))
            NIL)))
         (CL:RETURN-FROM HELP-TRANSFORM-BOOLEAN-PROCEDURAL-EXPRESSION
          (CONS
     ...
     ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-VRLET)
      ...
      (CL:RETURN-FROM HELP-TRANSFORM-BOOLEAN-PROCEDURAL-EXPRESSION
       (CONS
        (LIST* SYM-WALK-STELLA-LET FIRSTARG (CONCATENATE OTHERARGS NIL))
        NIL)))
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" TEST-VALUE-000
   ...
   (CL:LET* ((OBJECTREF NULL) (OBJECTTYPE NULL))
    ...
    (CL:LET*
     ((OBJECTCLASS (TYPE-SPEC-TO-CLASS OBJECTTYPE))
      ...
      (SLOT (SAFE-LOOKUP-SLOT OBJECTCLASS SLOTNAME)) (SLOTTYPE NULL)
      ...
      (CL:RETURN-FROM YIELD-DYNAMIC-SLOT-VALUE-TREE
       (LIST* SYM-WALK-STELLA-VRLET
        (CONS
         (CONS ANSWERVAR
          (CONS RETURNTYPE
           (CONS (TYPE-TO-NULL-VALUE-TREE (TYPE SLOT)) NIL)))
         NIL)
      ...
      (CL:RETURN-FROM YIELD-DYNAMIC-SLOT-VALUE-SETTER-TREE
       (LIST* SYM-WALK-STELLA-VRLET
        (LIST*
         ...
         (LIST* SYM-WALK-STELLA-FOUNDMATCHINGENTRY? SYM-WALK-STELLA-FALSE
          NIL)
         NIL)
        (LIST* SYM-WALK-STELLA-FOREACH SYM-WALK-STELLA-IT
         ...
         (CL:IF (SLOT-VALUE-IS-BARE-LITERAL? SLOT)
          (LIST* SYM-WALK-STELLA-LET
           ...
           (LIST* SYM-WALK-STELLA-SETF
            (LIST* SYM-WALK-STELLA-WRAPPER-VALUE SYM-WALK-STELLA-OLDVALUE
             NIL)
        ...
        (LIST* SYM-WALK-STELLA-WHEN
         (LIST* SYM-WALK-STELLA-AND
          (LIST* SYM-WALK-STELLA-NOT SYM-WALK-STELLA-FOUNDMATCHINGENTRY?
           NIL)
         ...
         (LIST* SYM-WALK-STELLA-SETF
          ...
          (LIST* SYM-WALK-STELLA-KV-CONS
           ...
           (CL:IF (SLOT-VALUE-IS-BARE-LITERAL? SLOT)
            (LIST* SYM-WALK-STELLA-WRAP-LITERAL SYM-WALK-STELLA-NEWVALUE
             NIL)
  ...
  (CL:LET* ((OBJECTREF NULL) (OBJECTTYPE NULL))
   ...
   (CL:LET*
    ((SLOTNAME (%%VALUE (%%REST (%%REST TREE))))
     ...
     (OBJECTCLASS (TYPE-SPEC-TO-CLASS OBJECTTYPE))
     (SLOT (SAFE-LOOKUP-SLOT OBJECTCLASS SLOTNAME)) (OTREE NULL)
     (OTYPE NULL))
    (CL:WHEN (CL:EQ SLOT NULL)
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Can't read a non-existent slot `"
  ...
  (CL:LET* ((OBJECTREF NULL) (OBJECTTYPE NULL))
   ...
   (CL:LET*
    ((SLOTNAME (FOURTH TREE)) (CSOPTIONS (NTH-REST TREE 4))
     (OBJECTCLASS (TYPE-SPEC-TO-CLASS OBJECTTYPE))
     (SLOT (SAFE-LOOKUP-SLOT OBJECTCLASS SLOTNAME)) (VALUEREF NULL)
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        (CL:PROGN (CL:SETQ DIM (%%VALUE ITER-000))
         (CL:SETQ ITER-000 (%%REST ITER-000)))
        (CL:WHEN (CL:EQ DIM NULL) (CL:SETQ ALWAYS?-000 FALSE)
         (CL:RETURN))))
   ...
   (CL:WHEN (CL:NOT INPUTPARAMETER?)
    ...
    (CL:MULTIPLE-VALUE-SETQ (OVALUE SOURCETYPE)
     (WALK-EXPRESSION-TREE VALUE TARGETTYPE
      SYM-WALK-STELLA-VARIABLE-DECLARATION TRUE))
    (CL:WHEN (CL:EQ TARGETTYPE NULL) (CL:SETQ TARGETTYPE SOURCETYPE))
...
(CL:DEFUN TRANSFORM-LET-WITH-PROCEDURAL-EXPRESSION (TREE)
  (CL:LET*
   ((FIRSTDECLARATION (%%VALUE (%%VALUE (%%REST TREE))))
    ...
    (TYPETREECONS (%%REST FIRSTDECLARATION))
    (VRLETTREE
     (WALK-WITHOUT-TYPE-TREE
...
(CL:DEFUN WALK-LET-TREE (TREE)
  ...
  (CL:LET*
   ((ORIGINALDECLARATIONS (%%VALUE (%%REST TREE)))
    (LEADINGDECLARATIONS NIL) (TRAILINGDECLARATIONS NIL) (LEADING? TRUE))
...
(CL:PROGN (CL:DEFVAR *NOFSPECIALSATLOOPENTRY* 0 "Number of specials bound at the most recent entry
to a LOOP/WHILE/FOREACH construct.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *NOFSPECIALSATLOOPENTRY*)))
...
(CL:DEFUN PUSH-SPECIAL (VARIABLE OLDVALUEVARIABLE)
  (CL:SETF (%THE-KV-LIST *SPECIALVARIABLESTACK*)
   (KV-CONS VARIABLE OLDVALUEVARIABLE
    (%THE-KV-LIST *SPECIALVARIABLESTACK*)))
  :VOID)
  ...
  (CL:RETURN-FROM NEED-TO-UNBIND-SPECIALS?
   (CL:AND *SPECIALSENABLED?* (CL:> (LENGTH *SPECIALVARIABLESTACK*) 0)
    (CL:NOT
     (MEMBER?
      (GET-QUOTED-TREE
       "((:COMMON-LISP :UNBIND-WITH-DESTRUCTORS) \"/STELLA\")" "/STELLA")
     ...
     (CL:WHEN
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
         " Trying to bind non-special variable `" (%%VALUE BINDING) "'"
      ...
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
        ...
        (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL " `"
         ...
         (LENGTH TARGETTYPES) "' value(s) from `"
         (DE-UGLIFY-PARSE-TREE TREE) "'" EOL "."))))
...
(CL:DEFUN COERCE-MV-TREE (TREE OPERATOR SOURCETYPES TARGETTYPES)
  (CL:LET*
   ...
   (CL:LET*
    ((TGTTS NULL) (SRCTS NULL)
     (IT (ALLOCATE-ITERATOR (%%REST VALUESTREE)))
...
(CL:DEFUN WALK-RETURN-AND-UNBIND-SPECIALS (TREE)
  (CL:LET*
   ((UNBINDTREE (YIELD-RETURN-SPECIAL-UNBIND-TREE))
    (VALUETREES (%%REST TREE)) (VALUEVARIABLES NIL) (VALUEBINDINGS NIL))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000
        (CONS (COMPUTE-RELATIVE-TYPE-SPEC RTYPE (%SLOT-OWNER METHOD))
         NIL))
       ...
       (CL:LET* ((V NULL) (ITER-000 (%%VALUE (%%REST TREE))))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ V (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:WHEN (CL:NOT (SYMBOL? V)) (CL:SETQ ALWAYS?-000 FALSE)
          (CL:RETURN))))
       (CL:SETQ TEST-VALUE-000 ALWAYS?-000))
      (CL:SETQ TEST-VALUE-000 (CL:NOT TEST-VALUE-000)))))
   (CL:WHEN TEST-VALUE-000
   ...
   (CL:LET*
    ((TEST
      (WALK-EXPRESSION-TREE (%%VALUE (%%REST TREE))
       SGT-WALK-STELLA-BOOLEAN SYM-WALK-STELLA-WHILE TRUE))
     (BODY (%%REST (%%REST TREE))))
     ...
     ((CL:EQ TEST-VALUE-000 SYM-WALK-STELLA-IF)
      (CL:WHEN (BAD-ARGUMENT-COUNT? TREE 3)
       (CL:RETURN-FROM WALK-CONDITIONAL-TREE
        (WALK-DONT-CALL-ME-TREE TREE SGT-WALK-STELLA-VOID)))
      (THIRD-SETTER TREE
       (WALK-STATEMENT (%%VALUE (%%REST (%%REST TREE))) TRUE))
       ...
       (CL:LET* ((IT (ALLOCATE-ITERATOR CASETEST)))
        (CL:LOOP WHILE (NEXT? IT) DO (CL:SETQ SYMBOL (%VALUE IT))
         (CL:WHEN (CL:NOT (DEFINED? (%SYMBOL-ID SYMBOL)))
          (CL:SETQ SYMBOL
           (INTERN-PERMANENT-SYMBOL (%SYMBOL-NAME SYMBOL))))
...
(CL:DEFUN COUNT-CASE-TEST-CLAUSES (TREE)
  (CL:LET*
   ((CLAUSES (%%REST (%%REST TREE))) (NOFCLAUSES 0) (OTHERWISE? FALSE))
...
(CL:DEFUN WALK-TYPECASE-TREE (TREE)
  (CL:LET*
   ((TESTTREE (%%VALUE (%%REST TREE))) (TESTEXPRESSIONTYPE NULL)
    (TESTVARIABLE NULL) (EXPANDEDCASES NIL) (NEEDCAST? (SYMBOL? TESTTREE))
...
(CL:DEFUN VERIFY-TYPECASE-CLAUSE-TYPE (TESTTYPE CLAUSETYPE PRECEDINGCLAUSETYPES CLAUSE)
  (CL:LET* ((SHADOWINGTYPE NULL))
   (CL:SETQ CLAUSETYPE (TYPE-TO-WRAPPED-TYPE CLAUSETYPE))
    ...
    (CL:PROGN
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Reference to undefined type in `typecase': `"
        (DE-UGLIFY-PARSE-TREE CLAUSETYPE) "'" EOL ".")))
     (CL:SETF (%%REST CLAUSE)
      (WALK-DONT-CALL-ME-TREE (%%REST CLAUSE) SGT-WALK-STELLA-VOID)))
    (CL:IF
     ...
     (CL:LET* ()
      (CL:LET* ((VALUE-000 NULL))
       (CL:LET*
        ((TYPE NULL) (ITER-000 (%THE-CONS-LIST PRECEDINGCLAUSETYPES)))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ TYPE (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
    ...
    (CL:COND
     ...
     ((WRAPPER? (%%VALUE (%%REST OPERANDS)))
      (SECOND-SETTER OPERANDS
       (WRAP-FLOAT
        (CL:* (%WRAPPER-VALUE (%%VALUE (%%REST OPERANDS))) 1.0d0))))
     (CL:T
      (CL:SETQ OPERANDS
       (CONS
    ...
    (CL:T
     (CL:MULTIPLE-VALUE-SETQ (OTREE OTYPE)
      (WALK-EXPRESSION-TREE STREAMTREE
       SGT-WALK-STELLA-NATIVE-OUTPUT-STREAM (%%VALUE TREE) FALSE))
  ...
  (CL:LET* ((METHOD NULL))
   (CL:LET*
    ((SUB NULL)
     (ITER-000
      (%THE-CONS-LIST
       (%CLASS-DIRECT-SUBS (%SURROGATE-VALUE ABSTRACTTYPE)))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (CL:EQ (%SURROGATE-VALUE SUB) NULL))
      (CL:IF (%ABSTRACT? (%SURROGATE-VALUE SUB))
       ...
       (CL:SETQ METHOD
        (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE SUB) METHODNAME)))
      (CL:WHEN
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE
      ...
      (CL:PROGN (CL:SETQ TARGETTS (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:PROGN (CL:SETQ PINDEX ITER-001)
...
(CL:DEFUN QUOTE-ARGUMENTS (METHOD ARGUMENTS)
  (CL:LET*
   ...
   (CL:LET*
    ((ARGINDEX NULL-INTEGER) (IT (ALLOCATE-ITERATOR ARGUMENTS))
     (ITER-000 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 ARGINDEX))
    (CL:LOOP WHILE (NEXT? IT) DO
     (CL:PROGN (CL:SETQ ARGINDEX ITER-000)
     ...
     (CL:TAGBODY
      (CL:WHEN
       ...
       (CL:WHEN
        ...
        (CL:GO :CONTINUE))
       (VALUE-SETTER IT (LIST* SYM-WALK-STELLA-BQUOTE (%VALUE IT) NIL)))
      :CONTINUE)))
...
(CL:DEFUN WALK-CALL-SLOT-TREE (TREE)
  (CL:LET*
   ((SLOTNAME (%%VALUE TREE)) (WALKEDFIRSTARG NULL)
    ...
    (ILLEGALFIRSTARG? FALSE) (PREVIOUSERRORS *TRANSLATIONERRORS*)
    (SLOT NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM PREVIOUSERRORS))
   (CL:WHEN (CL:EQ SLOTNAME SYM-WALK-STELLA-ERROR)
    (CL:SETQ *FOUNDRETURN?* TRUE))
   (CL:WHEN (CL:NOT (CL:EQ (LOOKUP-MACRO SLOTNAME) NULL))
    (CL:RETURN-FROM WALK-CALL-SLOT-TREE (WALK-MACRO-TREE TREE)))
   (CL:LET* () (CL:SETQ SLOT (LOOKUP-FUNCTION SLOTNAME))
    (CL:IF (CL:NOT (CL:EQ SLOT NULL))
     ...
     (CL:WHEN (CL:NOT (CL:EQ (%%REST TREE) NIL))
      (CL:MULTIPLE-VALUE-SETQ (WALKEDFIRSTARG FIRSTARGTYPE)
       (WALK-EXPRESSION-TREE (%%VALUE (%%REST TREE))
        SGT-WALK-STELLA-UNKNOWN SLOTNAME TRUE))
      (SECOND-SETTER TREE
   ...
   (CL:WHEN
    ...
    (CL:SETQ SLOT
     (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE SGT-WALK-STELLA-CONS) SLOTNAME)))
   (CL:WHEN (CL:EQ SLOT NULL)
    (CL:IF
...
(CL:DEFUN HELP-MOST-SPECIFIC-METHOD? (CLASS METHOD)
  (CL:WHEN (CL:EQ CLASS NULL)
   (CL:RETURN-FROM HELP-MOST-SPECIFIC-METHOD? TRUE))
  (CL:LET* ((TEST-VALUE-000 FALSE))
   (CL:LET* ((FOUND?-000 FALSE))
    (CL:LET*
    ...
    (CL:LET* ((ALWAYS?-000 TRUE))
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       ...
       (CL:WHEN
        (CL:NOT
         (HELP-MOST-SPECIFIC-METHOD? (%SURROGATE-VALUE SUB) METHOD))
        (CL:SETQ ALWAYS?-000 FALSE) (CL:RETURN))))
     (CL:SETQ TEST-VALUE-000 ALWAYS?-000)))
   (CL:LET* ((VALUE-000 TEST-VALUE-000))
...
(CL:DEFUN WALK-INLINE-METHOD-CALL (METHOD WALKEDARGS)
  (CL:LET*
   ...
   (CL:WHEN SUCCESS?
    (CL:LET*
     ((VAR NULL)
      (ITER-003 (%THE-CONS-LIST (METHOD-PARAMETER-NAMES METHOD))))
   ...
   (CL:LET*
    ((VAR NULL)
     (ITER-004 (%THE-CONS-LIST (METHOD-PARAMETER-NAMES METHOD))))
  ...
  (CL:RETURN-FROM YIELD-VERBATIM-INLINE-CALL-TREE
   (LIST* SYM-WALK-STELLA-SYS-INLINE-CALL
    (CONS
     ...
     (CONCATENATE
      (CL:IF (%METHOD-FUNCTION? METHOD) NIL
       (CONS (%SLOT-OWNER METHOD) NIL))
  ...
  (CL:LET* ((METHOD (%THE-OBJECT UNIT)) (BODY (%CODE-REGISTER UNIT)))
   (CL:LET*
    ((*LOCALVARIABLETYPETABLE* (NEW-KEY-VALUE-LIST))
     ...
     (*METHODBEINGWALKED* METHOD) (*FOUNDRETURN?* FALSE))
    (CL:DECLARE
    ...
    (CL:LET* ((METHOD NULL) (ITER-002 UNDEFINEDMETHODS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
      (CL:PROGN (CL:SETQ METHOD (%%VALUE ITER-002))
       (CL:SETQ ITER-002 (%%REST ITER-002)))
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "    " METHOD
       EOL)))))
  :VOID)
...
(CL:DEFUN YIELD-INITIAL-VALUE-EXPRESSION (SLOT)
  (CL:WHEN
   (CL:OR (%ABSTRACT? SLOT) (DYNAMIC-STORAGE? SLOT)
    (%SLOT-HARDWIRED? SLOT) (CL:EQ (ALLOCATION SLOT) KWD-WALK-CLASS))
       ...
       (CL:T
        ...
        (CL:WHEN
         (CL:OR (CL:EQ REASON NULL) (DELETED? REASON)
          ...
          (CL:NOT
           (CL:EQ
            (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE (%SLOT-OWNER REASON))
             (%SLOT-NAME REASON))
            REASON)))
         (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD)
...
(CL:DEFUN REGISTER-AUXILIARY-METHOD (METHOD REASON)
  (CL:WHEN (CL:NOT (CL:EQ METHOD NULL))
   (CL:LET*
    ((CLASS (%SURROGATE-VALUE (%SLOT-OWNER METHOD)))
...
(CL:DEFUN CREATE-ACCESSOR-UNITS-FOR-MIXIN-SLOT (SLOT CLASS)
  ...
  (CL:WHEN (%SLOT-HARDWIRED? SLOT)
   ...
   (CREATE-ACCESSOR-UNITS-FOR-SLOT SLOT
    (%SURROGATE-VALUE SGT-WALK-STELLA-OBJECT) FALSE FALSE))
  :VOID)
  ...
  (CL:LET* ((TEST-VALUE-000 FALSE))
   ...
   (CL:WHEN TEST-VALUE-000
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET*
      ((SUPER NULL)
       (ITER-001 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS CLASS))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
    ...
    ((CL:EQ (SAFE-PRIMARY-TYPE SELFTYPETREE) SGT-WALK-STELLA-CONS)
     (CL:PROGN
      (FIRST-SETTER SELFTYPETREE
       (YIELD-TYPE-SPEC-TREE (%CLASS-TYPE CLASS)))))
    (CL:T
     (CL:SETQ SELFTYPETREE (YIELD-TYPE-SPEC-TREE (%CLASS-TYPE CLASS)))))
   ...
   (WALK-AUXILIARY-TREE
    (LIST* SYM-WALK-STELLA-DEFMETHOD SYM-WALK-STELLA-PRINT-OBJECT
     (LIST* (LIST* SYM-WALK-STELLA-SELF (%CLASS-TYPE CLASS) NIL)
      (LIST* SYM-WALK-STELLA-STREAM SYM-WALK-STELLA-NATIVE-OUTPUT-STREAM
       NIL)
      NIL)
        ...
        (WALK-A-TREE
         (LIST* SYM-WALK-STELLA-IF
          ...
          (LIST* SYM-WALK-STELLA-RETURN
           (LIST* SYM-WALK-STELLA-STRINGIFY SYM-WALK-STELLA-SELF NIL) NIL)
          NIL)))
       (CL:SETF (%AUXILIARY? SELF-002) TRUE)
   ...
   (WALK-AUXILIARY-TREE
    (CL:IF (CL:EQ STARTUPTIMECODE NIL)
     ...
     (LIST* SYM-WALK-STELLA-STARTUP-TIME-PROGN KWD-WALK-CLASSES
      ...
      NIL)))
   (CL:LET*
    ((SYNONYM NULL) (ITER-000 (%THE-CONS-LIST (CLASS-SYNONYMS CLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SYNONYM (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN LOOKUP-MACRO (NAME)
  (CL:LET* ((MACROINFO (LOOKUP *MACRO-TABLE* NAME)) (EXPANDERMETHOD NULL))
   (CL:WHEN (CL:NOT (CL:EQ MACROINFO NULL))
    (CL:SETQ EXPANDERMETHOD (LOOKUP-FUNCTION (%%VALUE MACROINFO)))
    (CL:WHEN (CL:NOT (CL:EQ EXPANDERMETHOD NULL))
     (CL:SETF (%FUNCTION-CODE EXPANDERMETHOD)
      (%WRAPPER-VALUE (%%VALUE (%%REST MACROINFO))))
...
(CL:DEFUN STORE-MACRO (NAME EXPANDERNAME EXPANDERCODE)
  (INSERT-AT *MACRO-TABLE* NAME
   (CONS-LIST EXPANDERNAME (WRAP-FUNCTION-CODE EXPANDERCODE)))
  :VOID)

;;; (DEFUN UNDEFINE-MACRO ...)

(CL:DEFUN UNDEFINE-MACRO (NAME)
  (REMOVE-AT *MACRO-TABLE* NAME)
  :VOID)

;;; (DEFUN (YIELD-MACRO-EXPANDER-FUNCTION-NAME SYMBOL) ...)

(CL:DEFUN YIELD-MACRO-EXPANDER-FUNCTION-NAME (MACRONAME)
  (CL:RETURN-FROM YIELD-MACRO-EXPANDER-FUNCTION-NAME
   (INTERN-DERIVED-SYMBOL MACRONAME
    (CONCATENATE (%SYMBOL-NAME MACRONAME) "-EXPANDER"))))
...
(CL:DEFUN WALK-DEFMACRO-TREE (TREE)
  (CL:LET* ((NAME (%%VALUE (%%REST TREE))))
   ...
   (SECOND-SETTER TREE
    (CONS (YIELD-MACRO-EXPANDER-FUNCTION-NAME NAME)
     (CONS SYM-WALK-STELLA-OBJECT NIL)))
   (WALK-DEFMETHOD-TREE TREE)
   (CL:WHEN (CL:NOT (CL:EQ *CURRENTTRANSLATIONUNIT* NULL))
    (CL:SETF (%CATEGORY *CURRENTTRANSLATIONUNIT*) SYM-WALK-STELLA-MACRO)
...
(CL:DEFUN WALK-MACRO-UNIT (UNIT)
  (CL:LET*
   ((DEFINITION (%CODE-REGISTER UNIT))
    (NAME (%%VALUE (%%REST DEFINITION)))
    (EXPANDERMETHOD (%THE-OBJECT UNIT))
    (EXPANDERFNNAME (%SLOT-NAME EXPANDERMETHOD))
    (NEEDSLISPMACRO? (METHOD-NEEDS-LISP-MACRO? EXPANDERMETHOD)))
   (CL:WHEN NEEDSLISPMACRO? (CREATE-LISP-MACRO-UNITS NAME EXPANDERMETHOD)
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS EXPANDERMETHOD)
     SYM-WALK-STELLA-METHOD-LISP-MACRO? FALSE-WRAPPER NULL))
   (CL:SETF (%CODE-REGISTER UNIT) (NTH-REST DEFINITION 3))
   (WALK-METHOD-UNIT UNIT)
   (CL:WHEN NEEDSLISPMACRO?
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS EXPANDERMETHOD)
     SYM-WALK-STELLA-METHOD-LISP-MACRO? TRUE-WRAPPER NULL))
   (CL:WHEN (CL:> (METHOD-ARGUMENT-COUNT EXPANDERMETHOD) 5)
    (CL:LET* ((*PRINTREADABLY?* TRUE))
     (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*)) (SIGNAL-TRANSLATION-ERROR)
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Too many arguments in macro definition" EOL "."))))
   (WALK-AUXILIARY-TREE
    (LIST* SYM-WALK-STELLA-STARTUP-TIME-PROGN
     (LIST* SYM-WALK-STELLA-STORE-MACRO
      (LIST* SYM-WALK-STELLA-BQUOTE NAME NIL)
      (LIST* SYM-WALK-STELLA-BQUOTE EXPANDERFNNAME NIL)
      (LIST* SYM-WALK-STELLA-THE-CODE KWD-WALK-FUNCTION EXPANDERFNNAME
       NIL)
      NIL)
     NIL)))
  :VOID)
...
(CL:PROGN (CL:DEFVAR *MAX-NUMBER-OF-STARTUP-UNITS* 60 "The maximum number of startup units that can be combined
into a single startup function (this avoids the construction of huge startup
functions that would cause too much stress for some wimpy compilers).") (CL:DECLAIM (CL:TYPE CL:FIXNUM *MAX-NUMBER-OF-STARTUP-UNITS*)))
...
(CL:DEFUN COMBINE-STARTUP-FUNCTION-UNITS (STARTUPFNNAME)
  (CL:LET*
   ...
   (CL:LET*
    ((PHASE NULL) (ITER-000 (%THE-CONS-LIST *STARTUP-TIME-PHASES*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN EXTRACT-STARTUP-FUNCTION-UNITS (STARTUPUNITS STARTUPFNNAME)
  (CL:LET*
   ((REMAININGUNITS (LENGTH STARTUPUNITS))
    ...
    (CURSOR (%THE-CONS-LIST STARTUPUNITS)) (PHASESTART CURSOR)
    (PHASEUNITS 0) (PHASE (%CODE-REGISTER (%%VALUE CURSOR)))
    ...
    (CL:TAGBODY (CL:SETQ UNITPHASE (%CODE-REGISTER (%%VALUE CURSOR)))
     ...
     (CL:LET*
      ...
      (SET-DYNAMIC-SLOT-VALUE
       (%DYNAMIC-SLOTS
        (%THE-OBJECT (HELP-WALK-AUXILIARY-TREE HELPFNTREE TRUE)))
       SYM-WALK-STELLA-METHOD-STARTUP-CLASSNAME
       (WRAP-STRING (STRING-CAPITALIZE (%SYMBOL-NAME STARTUPFNNAME)))
       NULL-STRING-WRAPPER)
...
(CL:DEFUN YIELD-STARTUP-FUNCTION-NAME (FILE)
  ...
  (CL:SETQ FILE
   (CL:IF (CL:EQ FILE STELLA::NULL-STRING) *CURRENTFILE* FILE))
...
(CL:DEFUN WALK-SIGNAL-EXCEPTION-TREE (TREE)
  (CL:RETURN-FROM WALK-SIGNAL-EXCEPTION-TREE
   (CL:VALUES
    (LIST* SYM-WALK-STELLA-SYS-SIGNAL
     (WALK-EXPRESSION-TREE (%%VALUE (%%REST TREE))
      SGT-WALK-STELLA-STELLA-ROOT-EXCEPTION
      SYM-WALK-STELLA-SIGNAL-EXCEPTION FALSE)
     NIL)
    SGT-WALK-STELLA-VOID)))
...
(CL:DEFUN WALK-VERBATIM-UNIT (UNIT)
  ...
  (CL:COND
   ((CL:NOT (CL:EQ (%THE-OBJECT UNIT) NULL))
    (PUSH *TRANSLATIONUNITS* UNIT) (CL:SETF (%CODE-REGISTER UNIT) NULL))
   (CL:T (CL:SETF (%CODE-REGISTER UNIT) NULL)
    (CLEAR-TRANSLATION-UNIT UNIT)))
  :VOID)
...
(CL:DEFUN EXTRACT-REQUIRED-ARGUMENT-VALUES (CLASSTYPE SLOTSANDVALUES)
  (CL:LET*
   ((ARRAYTYPE? (ARRAY-TYPE-SPECIFIER? CLASSTYPE))
    (REQUIREDVALUES (NEW-LIST)) (FOUND? FALSE))
   (CL:WHEN ARRAYTYPE?
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ SIZEREF NULL)
    (CL:WHEN (CL:NOT (INDEFINITE-ARRAY-TYPE? ARRAYTYPE))
     (CL:SETQ SIZEREF
      (%THE-CONS-LIST (ARRAY-TYPE-DIMENSIONS ARRAYTYPE)))))
   (CL:COND
    ((CL:EQ SIZEREF NULL)
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      ...
      ((CL:NOT (CL:= (LENGTH SIZEREF) ARRAYRANK))
       (CL:LET* ((*PRINTREADABLY?* TRUE))
        ...
        (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
         (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
          " Array :size initializer `" (DE-UGLIFY-PARSE-TREE SIZEREF) "'"
          EOL "   doesn't match the rank of" EOL "   `"
     ...
     (CL:LET* ((IT (ALLOCATE-ITERATOR PLIST)))
      (CL:LOOP WHILE (NEXT? IT) DO
       (CL:WHEN (CL:NOT (KEYWORD? (%KEY IT)))
        ...
        (CL:RETURN-FROM YIELD-NEW-ARGUMENTS-TREE (CL:VALUES NIL NIL)))
       (CL:SETQ SLOTNAME
        (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%KEY IT))
         (HOME-MODULE CLASS) FALSE))
...
(CL:DEFUN WALK-MAKE-TREE (TREE)
  (CL:LET* ((CLASSNAME (%%VALUE (%%REST TREE))) (TYPE NULL))
   (CL:COND ((SYMBOL? CLASSNAME))
    ((TYPE? CLASSNAME) (SECOND-SETTER TREE (TYPE-TO-SYMBOL CLASSNAME)))
    (CL:T
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (SIGNAL-TRANSLATION-ERROR)
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Bad argument to 'make':" EOL "   `"
        (DE-UGLIFY-PARSE-TREE CLASSNAME) "' found where symbol expected"
        EOL ".")))
       ...
       (CL:WHEN
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-OUTPUT)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
           " Cannot generate function pointer to main" EOL ".")))
        (CL:LET* ((VALUE-000 NULL) (VALUE-001 NULL))
         (CL:MULTIPLE-VALUE-SETQ (VALUE-000 VALUE-001)
          (WALK-A-TREE
           (LIST* SYM-WALK-STELLA-CAST SYM-WALK-STELLA-NULL
            SYM-WALK-STELLA-FUNCTION-CODE NIL)))
         (CL:RETURN-FROM WALK-THE-CODE-TREE
          (CL:VALUES VALUE-000 VALUE-001)))))
      (CL:T))
     (CL:RETURN-FROM WALK-THE-CODE-TREE
         ...
         (CL:WHEN (SUBTYPE-OF? TYPE SGT-WALK-STELLA-LITERAL)
          (CL:LET* ((*PRINTREADABLY?* TRUE))
           ...
           (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
             ...
             *TRANSLATOROUTPUTLANGUAGE* "', hence, cannot generate" EOL
     ...
     ((CL:EQ LANGUAGE KWD-WALK-COMMON-LISP)
      (CL:SETQ NEWNAME
       (INTERN-DERIVED-SYMBOL NAME
        (CONCATENATE "%" (%SYMBOL-NAME NAME)))))
     (CL:T
      (CL:SETQ NEWNAME
       (INTERN-DERIVED-SYMBOL NAME
        (CONCATENATE (%SYMBOL-NAME NAME) "_")))))
...
(CL:DEFUN YIELD-LISP-MACRO-TREES (NAME METHOD)
  (CL:LET*
   ((MODULE
     (CL:IF (CL:NOT (CL:EQ (%HOME-CONTEXT NAME) NULL))
      (%HOME-CONTEXT NAME) *MODULE*))
        ...
        (CONCATENATE
         ...
         (LIST*
          ...
          (LIST* (INTERN-COMMON-LISP-SYMBOL "LET")
           (CONS
            (LIST* SYM-WALK-STELLA-*IGNORETRANSLATIONERRORS?*
             (CL:IF (COMMAND? METHOD) SYM-WALK-STELLA-FALSE
              SYM-WALK-STELLA-TRUE)
             NIL)
            NIL)
           (LIST* SYM-WALK-STELLA-INCREMENTALLY-TRANSLATE
            SYM-WALK-STELLA-EXPRESSION SYM-WALK-STELLA-FALSE NIL)
           NIL)
          NIL))))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000))
     DO
     (CL:PROGN (CL:SETQ INDEX ITER-000)
      (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000
      ...
      (CL:SETQ CALLTREE
       (LIST* SYM-WALK-STELLA-LET
        ...
        (LIST* SYM-WALK-STELLA-IF
         ...
         (LIST* SYM-WALK-STELLA-RETURN
          (LIST* SYM-WALK-STELLA-WRAP-LITERAL SYM-WALK-STELLA-RESULT NIL)
          NIL)
    ...
    (LIST* SYM-WALK-STELLA-DEFUN NAMETREE
     (LIST*
      (CONS (LIST* SYM-WALK-STELLA-ARGUMENTS SYM-WALK-STELLA-CONS NIL)
       NIL)
      KWD-WALK-AUXILIARY? SYM-WALK-STELLA-TRUE CALLTREE NIL)))))
...
(CL:DEFUN PRINT-EVALUATION-ERROR-CONTEXT (PREFIX STREAM)
  (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING PREFIX))
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) PREFIX "While evaluating '"
   *EVALUATIONTREE*)
  (CL:WHEN (CL:NOT (CL:EQ *EVALUATIONPARENTTREE* NULL))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "' inside '"
...
indicates whether an error occurred during the evaluation."
  ...
  (CL:LET* ((OTREE NULL) (OTYPE SGT-WALK-STELLA-UNKNOWN))
...
evaluation strategies."
  (RESET-EVALUATION-ERROR)
  (CL:LET* ((OTREE TREE) (OTYPE SGT-WALK-STELLA-UNKNOWN))
   ...
   (CL:RETURN-FROM TRY-TO-EVALUATE
    (CL:VALUES OTREE OTYPE (EVALUATION-ERROR?)))))
    ...
    (CL:COND
       ...
       (CL:COND
        ((CL:EQ OPERATORNAME SYM-WALK-STELLA-QUOTE)
         ...
         (CL:LET* ((*PRINTREADABLY?* TRUE))
          ...
          (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
           "   Illegal QUOTE expression." EOL))
     ...
     (CL:T
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
        "   Illegal operator: `" (DE-UGLIFY-PARSE-TREE OPERATORNAME) "'."
        EOL))
      (CL:RETURN-FROM EVALUATE-CONS-TREE (CL:VALUES NULL NULL))))
    (CL:LET*
    ...
    (CL:COND
       ...
       (CL:COND
        ...
        (CL:T
         (CL:LET* ((*PRINTREADABLY?* TRUE))
          ...
          (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
           "   Variable evaluation not yet implemented." EOL))
     ...
     (CL:T
      (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
        "   Unknown atomic expression type: `" (DE-UGLIFY-PARSE-TREE TREE)
        "'." EOL))
      (CL:RETURN-FROM EVALUATE-ATOMIC-TREE (CL:VALUES NULL NULL)))))))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:IF (VOID? SOURCETYPE)
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*)) (SIGNAL-EVALUATION-ERROR)
      (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL "   `"
       (%%VALUE SOURCETREE) "' does not return a value." EOL))
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*)) (SIGNAL-EVALUATION-ERROR)
      (PRINT-EVALUATION-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL
...
(CL:DEFUN HELP-STARTUP-WALK1 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-WALK-TRANSLATE-WITH-COPYRIGHT-HEADER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TRANSLATE-WITH-COPYRIGHT-HEADER" NULL
     2))
   ...
   (CL:SETQ SYM-WALK-STELLA-KWD
    (INTERN-RIGID-SYMBOL-WRT-MODULE "KWD" NULL 0))
   (CL:SETQ SYM-WALK-STELLA-IN
   ...
   (CL:SETQ SYM-WALK-STELLA-TRACE-IF
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TRACE-IF" NULL 0))
   (CL:SETQ SYM-WALK-STELLA-TRACE-IF-EXPANDER
   ...
   (CL:SETQ SYM-WALK-STELLA-SET-TRANSLATOR-OUTPUT-LANGUAGE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SET-TRANSLATOR-OUTPUT-LANGUAGE" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-WALK4 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-WALK-STELLA-INTERN-RIGID-SYMBOL-WRT-MODULE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INTERN-RIGID-SYMBOL-WRT-MODULE" NULL
     0))
   ...
   (CL:SETQ SYM-WALK-STELLA-NULL-INTEGER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NULL-INTEGER" NULL 0))
   (CL:SETQ SYM-WALK-STELLA-NULL-FLOAT
   ...
   (CL:SETQ KWD-WALK-PLACE-WHERE-LAST-EXPRESSION-WAS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PLACE-WHERE-LAST-EXPRESSION-WAS" NULL
     2))
...
(CL:DEFUN HELP-STARTUP-WALK6 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-WALK-STELLA-SUBTYPE-OF-ANCHORED-TYPE-SPECIFIER?
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SUBTYPE-OF-ANCHORED-TYPE-SPECIFIER?"
     NULL 0))
   ...
   (CL:SETQ SYM-WALK-STELLA-STRINGIFY
...
(CL:DEFUN HELP-STARTUP-WALK7 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-WALK-STELLA-STORE-MACRO
   ...
   (CL:SETQ SYM-WALK-STELLA-FUNCTION-CODE
   ...
   (CL:SETQ SYM-WALK-STELLA-INCREMENTALLY-TRANSLATE
...
(CL:DEFUN HELP-STARTUP-WALK8 ()
  (CL:PROGN
   (CL:SETQ *AVAILABLE-ENVIRONMENT-FEATURES*
    (LIST KWD-WALK-WARN-ABOUT-UNDEFINED-METHODS KWD-WALK-SUPPRESS-WARNINGS
     KWD-WALK-WARN-ABOUT-MISSING-METHODS KWD-WALK-USE-HARDCODED-SYMBOLS
     KWD-WALK-USE-COMMON-LISP-STRUCTS
     KWD-WALK-USE-COMMON-LISP-VECTOR-STRUCTS
   ...
   (CL:SETQ *DEFAULT-ENVIRONMENT-FEATURES*
   ...
   (CL:SETQ *WRAPPED-TYPE-TABLE*
    (LIST*
     (LIST* SGT-WALK-STELLA-INTEGER-WRAPPER SGT-WALK-STELLA-INTEGER NIL)
   ...
   (CL:SETQ *COERSION-TABLE*
    (LIST*
     ...
     (LIST* SGT-WALK-STELLA-MUTABLE-STRING
      SGT-WALK-STELLA-MUTABLE-STRING-WRAPPER SYM-WALK-STELLA-WRAP-LITERAL
      NIL)
     ...
     (LIST* SGT-WALK-STELLA-FUNCTION-CODE
      SGT-WALK-STELLA-FUNCTION-CODE-WRAPPER SYM-WALK-STELLA-WRAP-LITERAL
      NIL)
     ...
     (LIST* SGT-WALK-STELLA-OUTPUT-STREAM
      SGT-WALK-STELLA-NATIVE-OUTPUT-STREAM SYM-WALK-STELLA-NATIVE-STREAM
      NIL)
...
(CL:DEFUN HELP-STARTUP-WALK9 ()
  (CL:PROGN
   (DEFINE-FUNCTION-OBJECT "DISPLAY-ENVIRONMENT-FEATURES"
    "(DEFUN DISPLAY-ENVIRONMENT-FEATURES () :DOCUMENTATION \"Print a list of the currently enabled environment 
features and print a list of all Stella environment features.\")"
    (CL:FUNCTION DISPLAY-ENVIRONMENT-FEATURES) NULL)
   (DEFINE-FUNCTION-OBJECT "ENABLE-ENVIRONMENT-FEATURE"
    "(DEFUN ENABLE-ENVIRONMENT-FEATURE (|&REST| (FEATURES KEYWORD)) :DOCUMENTATION \"Enable the environment feature(s) indicated by the
keywords in 'features'.\")" (CL:FUNCTION ENABLE-ENVIRONMENT-FEATURE) NULL)
   (DEFINE-FUNCTION-OBJECT "DISABLE-ENVIRONMENT-FEATURE"
    "(DEFUN DISABLE-ENVIRONMENT-FEATURE (|&REST| (FEATURES KEYWORD)) :DOCUMENTATION \"Disable the environment feature(s) indicated by the
keywords in 'features'.\")" (CL:FUNCTION DISABLE-ENVIRONMENT-FEATURE)
    NULL)
   (DEFINE-FUNCTION-OBJECT "RESET-ENVIRONMENT-FEATURES"
    "(DEFUN RESET-ENVIRONMENT-FEATURES () :DOCUMENTATION \"Reset the current environment features to their default
settings.\")" (CL:FUNCTION RESET-ENVIRONMENT-FEATURES) NULL)
   (DEFINE-FUNCTION-OBJECT "TRACE-IF-EXPANDER"
    "(DEFUN (TRACE-IF-EXPANDER OBJECT) ((KEYWORD OBJECT) |&BODY| (BODY CONS)))"
    (CL:FUNCTION TRACE-IF-EXPANDER) NULL)
...
`speed', and `space'.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "GET-QUOTED-TREE"
    "(DEFUN (GET-QUOTED-TREE CONS) ((TREENAME STRING) (MODULENAME STRING)) :DOCUMENTATION \"Return the quoted tree with name 'treeName'.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "SET-TRANSLATOR-OUTPUT-LANGUAGE"
    "(DEFUN (SET-TRANSLATOR-OUTPUT-LANGUAGE KEYWORD) ((NEWLANGUAGE KEYWORD)) :DOCUMENTATION \"Set output language to `newLanguage'.  Return previous language.\" :COMMAND? TRUE :PUBLIC? TRUE)"
...
(CL:DEFUN HELP-STARTUP-WALK10 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "POP-VARIABLE-BINDING"
    "(DEFUN POP-VARIABLE-BINDING ())" (CL:FUNCTION POP-VARIABLE-BINDING)
    NULL)
...
or 'self' if it is not a bare literal type.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "VERIFY-TYPE"
    "(DEFUN (VERIFY-TYPE TYPE) ((SELF TYPE)))" (CL:FUNCTION VERIFY-TYPE)
    NULL)
...
necessary signatures.\" :ALLOCATION :DYNAMIC)")
   ...
   (DEFINE-FUNCTION-OBJECT "LOOKUP-MACRO"
    "(DEFUN (LOOKUP-MACRO METHOD-SLOT) ((NAME SYMBOL)) :PUBLIC? TRUE)"
    (CL:FUNCTION LOOKUP-MACRO) NULL)
   (DEFINE-FUNCTION-OBJECT "STORE-MACRO"
    "(DEFUN STORE-MACRO ((NAME SYMBOL) (EXPANDERNAME SYMBOL) (EXPANDERCODE FUNCTION-CODE)) :PUBLIC? TRUE)"
    (CL:FUNCTION STORE-MACRO) NULL)
   (DEFINE-FUNCTION-OBJECT "UNDEFINE-MACRO"
    "(DEFUN UNDEFINE-MACRO ((NAME SYMBOL)))" (CL:FUNCTION UNDEFINE-MACRO)
    NULL)
   (DEFINE-FUNCTION-OBJECT "YIELD-MACRO-EXPANDER-FUNCTION-NAME"
    "(DEFUN (YIELD-MACRO-EXPANDER-FUNCTION-NAME SYMBOL) ((MACRONAME SYMBOL)))"
    (CL:FUNCTION YIELD-MACRO-EXPANDER-FUNCTION-NAME) NULL)
   (DEFINE-FUNCTION-OBJECT "WALK-DEFMACRO-TREE"
    "(DEFUN WALK-DEFMACRO-TREE ((TREE CONS)))"
    (CL:FUNCTION WALK-DEFMACRO-TREE) NULL)
   (DEFINE-FUNCTION-OBJECT "WALK-MACRO-UNIT"
    "(DEFUN WALK-MACRO-UNIT ((UNIT TRANSLATION-UNIT)))"
    (CL:FUNCTION WALK-MACRO-UNIT) NULL)
...
polymorphic, i.e., they can only be implemented by functions.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "RESET-EVALUATION-ERROR"
    "(DEFUN RESET-EVALUATION-ERROR () :DOCUMENTATION \"Reset the global evaluation error indicator.\" :PUBLIC? TRUE)"
    (CL:FUNCTION RESET-EVALUATION-ERROR) NULL)
   (DEFINE-FUNCTION-OBJECT "SIGNAL-EVALUATION-ERROR"
...
by calling 'eval-error'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION SIGNAL-EVALUATION-ERROR) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "PRINT-EVALUATION-ERROR-CONTEXT"
    "(DEFUN PRINT-EVALUATION-ERROR-CONTEXT ((PREFIX STRING) (STREAM OUTPUT-STREAM)))"
    (CL:FUNCTION PRINT-EVALUATION-ERROR-CONTEXT) NULL)
...
evaluation strategies.\" :PUBLIC? TRUE)" (CL:FUNCTION TRY-TO-EVALUATE)
   ...
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-WALK-STELLA-STARTUP-WALK)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     SYM-WALK-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "Startup-Walk")
     NULL-STRING-WRAPPER)))
  :VOID)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *AVAILABLE-ENVIRONMENT-FEATURES* (LIST OF KEYWORD) (CAST (LIST :WARN-ABOUT-UNDEFINED-METHODS :SUPPRESS-WARNINGS :WARN-ABOUT-MISSING-METHODS :USE-HARDCODED-SYMBOLS :USE-COMMON-LISP-STRUCTS :USE-COMMON-LISP-VECTOR-STRUCTS :USE-COMMON-LISP-CONSES :USE-CPP-GARBAGE-COLLECTOR :TRANSLATE-WITH-COPYRIGHT-HEADER) (LIST OF KEYWORD)) :DOCUMENTATION \"List of Stella environment features.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *DEFAULT-ENVIRONMENT-FEATURES* (LIST OF KEYWORD) (CAST (LIST :WARN-ABOUT-UNDEFINED-METHODS :WARN-ABOUT-MISSING-METHODS :USE-CPP-GARBAGE-COLLECTOR :USE-COMMON-LISP-CONSES) (LIST OF KEYWORD)) :DOCUMENTATION \"List of features enabled upon call to
'reset-environment-features'.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *TRACED-KEYWORDS* (LIST OF KEYWORD) NULL :PUBLIC? TRUE)")
    (STORE-MACRO SYM-WALK-STELLA-TRACE-IF
     SYM-WALK-STELLA-TRACE-IF-EXPANDER (CL:FUNCTION TRACE-IF-EXPANDER))
