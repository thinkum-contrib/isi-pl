...
(CL:DEFCLASS XML-OBJECT (STANDARD-OBJECT)
  ((NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION
   ...
   (SURFACE-FORM :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %SURFACE-FORM)))

(CL:DEFUN NEW-XML-OBJECT ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-OBJECT)))
...
(CL:DEFUN NEW-XML-ELEMENT ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-ELEMENT)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFUN NEW-XML-SPECIAL ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-SPECIAL)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATA SELF) STELLA::NULL-STRING)
...
(CL:DEFUN NEW-XML-COMMENT ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-COMMENT)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFMETHOD PRINT-OBJECT ((SELF XML-COMMENT) STREAM)
  (CL:IF (CL:EQ (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   ...
   (%%PRINT-STREAM STREAM "<!-- "
    (CL:THE CL:FIXNUM
     (CL:LENGTH (CL:THE CL:SIMPLE-STRING (%SURFACE-FORM SELF))))
    " chars -->"))
  :VOID)
...
(CL:DEFCLASS XML-DOCTYPE (STANDARD-OBJECT)
  ((NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION
   ...
   (PARAMETER-ENTITY-TABLE :ALLOCATION :INSTANCE :ACCESSOR
    %PARAMETER-ENTITY-TABLE)))

(CL:DEFUN NEW-XML-DOCTYPE ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-DOCTYPE)))
   (CL:SETF (%PARAMETER-ENTITY-TABLE SELF) (NEW-KEY-VALUE-LIST))
   (CL:SETF (%ENTITY-TABLE SELF) (NEW-KEY-VALUE-LIST))
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFUN ACCESS-XML-DOCTYPE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-XML-STELLA-ENTITY-TABLE)
    (CL:IF SETVALUE? (CL:SETF (%ENTITY-TABLE SELF) VALUE)
     (CL:SETQ VALUE (%ENTITY-TABLE SELF))))
   ((CL:EQ SLOTNAME SYM-XML-STELLA-PARAMETER-ENTITY-TABLE)
    (CL:IF SETVALUE? (CL:SETF (%PARAMETER-ENTITY-TABLE SELF) VALUE)
     (CL:SETQ VALUE (%PARAMETER-ENTITY-TABLE SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
...
(CL:DEFUN MAKE-XML-CDATA-FORM (THE-DATA)
  ...
  (CL:LET* ((SELF-000 (NEW-XML-SPECIAL))) (CL:SETF (%NAME SELF-000) "CDATA")
   (CL:SETF (%SURFACE-FORM SELF-000) "CDATA")
   (CL:LET* ((TAG SELF-000))
...
(CL:DEFUN MAKE-XML-GLOBAL-ATTRIBUTE-RESPECTING-NAMESPACE (NAME NAMESPACE-NAME SURFACE-FORM NAMESPACE-TABLE)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ NAMESPACE-URI NULL)
    (CL:ERROR
     (NEW-BAD-ARGUMENT-EXCEPTION "Undefined Namespace for Attribute!")))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ ATTRIBUTE NULL)
    (CL:LET* ((SELF-000 (NEW-XML-LOCAL-ATTRIBUTE)))
     (CL:SETF (%NAME SELF-000) NAME)
     (CL:SETF (%PARENT-ELEMENT SELF-000) ELEMENT)
...
(CL:DEFUN XMLNS-ATTRIBUTE? (ATTRIBUTE)
  (CL:RETURN-FROM XMLNS-ATTRIBUTE?
   (CL:AND (ISA? ATTRIBUTE SGT-XML-STELLA-XML-GLOBAL-ATTRIBUTE)
    (STRING-EQL? "xmlns" (%NAMESPACE-NAME ATTRIBUTE)))))
...
(CL:DEFUN GET-DOCTYPE-DEFINITION (DOCTYPE)
  (CL:LET*
   ((DEFINITION
     (%WRAPPER-VALUE (%%VALUE (%%REST (%%VALUE (%%REST DOCTYPE)))))))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING DEFINITION))
...
a PUBLIC or SYSTEM literal for an XML Elternal ID.  (See 4.2.2)"
  (CL:RETURN-FROM EXTERNAL-ID-HEAD?
   (CL:AND (ISA? ATTRIBUTE SGT-XML-STELLA-STRING-WRAPPER)
    (CL:OR (STRING-EQL? (%WRAPPER-VALUE ATTRIBUTE) "SYSTEM")
     (STRING-EQL? (%WRAPPER-VALUE ATTRIBUTE) "PUBLIC")))))
...
(CL:DEFUN DECODE-XML-ENTITY-REF (DOCTYPE NAME PE-REFERENCE-ALLOWED?)
  ...
  (CL:LET* ((VALUE NULL))
   (CL:WHEN (CL:NOT (CL:EQ DOCTYPE NULL))
    (CL:SETQ VALUE (LOOKUP (%ENTITY-TABLE DOCTYPE) (WRAP-STRING NAME))))
   (CL:WHEN (CL:EQ VALUE NULL)
    (CL:SETQ VALUE (LOOKUP *XML-BASE-ENTITY-TABLE* (WRAP-STRING NAME))))
   (CL:IF (CL:EQ VALUE NULL)
    ...
    (CL:RETURN-FROM DECODE-XML-ENTITY-REF
     (DECODE-XML-STRING DOCTYPE (%WRAPPER-VALUE VALUE)
...
(CL:DEFUN DECODE-XML-PARAMETER-ENTITY-REF (DOCTYPE NAME PE-REFERENCE-ALLOWED?)
  ...
  (CL:LET*
   ((VALUE
     (%WRAPPER-VALUE (LOOKUP (%ENTITY-TABLE DOCTYPE) (WRAP-STRING NAME)))))
...
(CL:DEFUN DECODE-XML-STRING (DOCTYPE INPUT PE-REFERENCE-ALLOWED?)
  ...
  (CL:LET*
   ((INPUT-END
     (CL:IF PE-REFERENCE-ALLOWED? (CL:1- (CL:THE CL:FIXNUM (CL:LENGTH INPUT)))
      NULL-INTEGER))
    (REFERENCE-START
     (CL:IF PE-REFERENCE-ALLOWED? (FIND-REFERENCE-START INPUT 0 INPUT-END)
      (POSITION INPUT #\& 0)))
   ...
   (CL:IF (CL:= REFERENCE-START NULL-INTEGER)
    ...
    (CL:LET* ((BUFFER (NEW-OUTPUT-STRING-STREAM)) (REFERENCE-END 0))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:= REFERENCE-START NULL-INTEGER)) DO
      (%%PRINT-STREAM (%NATIVE-STREAM BUFFER)
...
a DOCTYPE object."
  (CL:IF
   ...
   (CL:LET*
    ((DOCTYPE (NEW-XML-DOCTYPE)) (DEFINITION-STREAM NULL) (ITERATOR NULL))
    (CL:UNWIND-PROTECT
     (CL:PROGN
      (CL:SETF (%NAME DOCTYPE)
       (%WRAPPER-VALUE (%%VALUE (%%VALUE (%%REST DOCTYPE-DECLARATION)))))
      ...
      (CL:WHEN (CL:NOT (CL:EQ DEFINITION-STREAM NULL))
       (CL:LET* ((SELF-001 (NEW-XML-EXPRESSION-ITERATOR)))
        (CL:SETF (%THE-STREAM SELF-001) DEFINITION-STREAM)
        (CL:SETF (%DOCTYPE-ITERATOR? SELF-001) CL:T)
           ...
           (CL:IF (STRING-EQL? "%" (%WRAPPER-VALUE (%%VALUE ATTRIBUTES)))
            (CL:IF
             (CL:AND (CL:> (LENGTH ATTRIBUTES) 3)
              (EXTERNAL-ID-HEAD? (%%VALUE (%%REST (%%REST ATTRIBUTES)))))
             (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
              (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
               "Can't handle external entity definitions: `" DECLARATION "'")
              (CL:ERROR
               (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
             (ADD-XML-REFERENCE (%WRAPPER-VALUE (%%VALUE (%%REST ATTRIBUTES)))
              (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ATTRIBUTES))))
            ...
            (CL:IF
             ...
             (ADD-XML-REFERENCE (%WRAPPER-VALUE (%%VALUE ATTRIBUTES))
              (%WRAPPER-VALUE (%%VALUE (%%REST ATTRIBUTES)))
...
(CL:DEFUN TOKENIZE-XML-EXPRESSION (STREAM TOKENLIST REGIONTAGNAME SKIPTOREGION?)
  ...
  (CL:LET*
   ((PARENBALANCE 0) (STARTTAG STELLA::NULL-STRING)
    ...
    (ENDOFFILE? CL:T) (TOKENTYPE NULL) (TOKENCONTENT STELLA::NULL-STRING)
...
The name is available using class accessors."
  (CL:LET*
   ((PARSEDTREE NULL) (PARSEDTREESTACK NIL) (PARSEDTOKEN NULL)
    ...
    (XMLNS-NAME STELLA::NULL-STRING) (COLON-POSITION 0) (ATTRIBUTELIST NIL)
    (CONSCELL NULL) (TOKENTYPE NULL) (NAMESPACE-TABLE (MAKE-NAMESPACE-TABLE))
    (NS-STACK NIL))
     ...
     (CL:COND
      ...
      ((CL:EQ TOKENTYPE KWD-XML-ATTRIBUTE-NAME)
       ...
       (CL:WHEN (XMLNS-ATTRIBUTE-NAME? (%CONTENT TOKENLIST))
        ...
        (CL:LET*
         ((NAMESPACE-VALUE
           (NORMALIZE-ATTRIBUTE-VALUE DOCTYPE (%CONTENT TOKENLIST)
            DOCTYPE-DEFINITION?)))
          ...
          (CL:T
           (CL:SETQ NAMESPACE-TABLE
            (KV-CONS (WRAP-STRING "") (WRAP-STRING NAMESPACE-VALUE)
             NAMESPACE-TABLE))))
         (CL:SETQ PARSEDTOKEN (CL:MAKE-INSTANCE (CL:QUOTE STRING-WRAPPER)))
         (CL:SETF (%WRAPPER-VALUE PARSEDTOKEN) NAMESPACE-VALUE)
         (CL:SETQ CONSCELL (CL:CONS CL:NIL CL:NIL))
         (CL:SETF (%%VALUE CONSCELL) PARSEDTOKEN)
         (CL:SETF (%%REST CONSCELL) ATTRIBUTELIST)
         (CL:SETQ ATTRIBUTELIST CONSCELL)))
      ...
      ((CL:OR (CL:EQ TOKENTYPE KWD-XML-ATTRIBUTE-VALUE)
       ...
       (CL:SETF (%%VALUE CONSCELL) PARSEDTOKEN)
       (CL:SETF (%%REST CONSCELL) ATTRIBUTELIST)
       (CL:SETQ ATTRIBUTELIST CONSCELL) (CL:SETQ TOKENLIST (%NEXT TOKENLIST))
       (CL:GO :CONTINUE))
...
This change will not be backwards-compatible."
  (CL:LET*
   ((ITER (XML-EXPRESSIONS STREAM START-TAG)) (RESULT NULL) (EOF? CL:NIL))
   ...
   (CL:UNWIND-PROTECT
    (CL:PROGN (CL:SETQ IN (OPEN-INPUT-FILE FILENAME))
     (CL:LET* ((VALUE-000 NIL))
      (CL:LET*
       ((ITEM NULL) (ITER-000 (XML-EXPRESSIONS IN NULL)) (COLLECT-000 NULL))
...
(CL:DEFCLASS XML-EXPRESSION-ITERATOR (STREAM-ITERATOR)
  ((REGION-TAG :ALLOCATION :INSTANCE :ACCESSOR %REGION-TAG)
   ...
   (DOCTYPE-ITERATOR? :DOCUMENTATION
    "Set to `true' when iterating over a DTD definition." :ALLOCATION
    :INSTANCE :ACCESSOR %DOCTYPE-ITERATOR?)
...
(CL:DEFUN ACCESS-XML-EXPRESSION-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-XML-STELLA-DOCTYPE)
    (CL:IF SETVALUE? (CL:SETF (%DOCTYPE SELF) VALUE)
     (CL:SETQ VALUE (%DOCTYPE SELF))))
   ((CL:EQ SLOTNAME SYM-XML-STELLA-DOCTYPE-ITERATOR?)
    (CL:IF SETVALUE?
     (CL:SETF (%DOCTYPE-ITERATOR? SELF)
      (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE))
     (CL:SETQ VALUE
      (CL:IF (%DOCTYPE-ITERATOR? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
   ...
   (CL:LOOP
    (CL:TAGBODY
     ...
     (CL:WHEN EOF?
      ...
      (CL:SETF (%THE-STREAM SELF) NULL) (CL:RETURN-FROM NEXT? CL:NIL))
        ...
        (CL:WHEN
         (CL:NOT
          (CL:FUNCALL (%REGION-MATCH-FUNCTION SELF) (%REGION-TAG SELF)
           (XML-TOKEN-LIST-TO-S-EXPRESSION TOKENLIST (%DOCTYPE SELF)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) GET-XML-BASE-ATTRIBUTE-VALUE))
...
element if it exists.  Otherwise NULL."
  (CL:LET*
   ((ATTRIBUTE-LIST (%%VALUE (%%REST EXPRESSION))) (ATTRIBUTE NULL)
    (VALUE STELLA::NULL-STRING) (BASE-ATTRIBUTE-VALUE STELLA::NULL-STRING))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ATTRIBUTE-LIST NIL)) DO
    ...
    (CL:WHEN
     (CL:AND (ISA? ATTRIBUTE SGT-XML-STELLA-XML-GLOBAL-ATTRIBUTE)
      (STRING-EQL? "base" (%NAME ATTRIBUTE))
      ...
      ((SUBTYPE-OF-STRING? TEST-VALUE-000)
       (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "\"")
        (CL:LET* ((FOUND?-000 CL:NIL))
         (CL:LET*
          ((CH NULL-CHARACTER) (VECTOR-000 (%WRAPPER-VALUE ITEM))
           (INDEX-000 0)
...
In that case, each of the forms is indented by `indent' spaces."
  ...
  (CL:WHEN (ISA? (%%VALUE XML-EXPRESSION) SGT-XML-STELLA-CONS)
   (CL:LET* ((EXP NULL) (ITER-000 XML-EXPRESSION))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ EXP (%%VALUE ITER-000)) (PRINT-XML-EXPRESSION STREAM EXP INDENT)
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:RETURN-FROM PRINT-XML-EXPRESSION))
  (CL:LET*
   ((TAG (%%VALUE XML-EXPRESSION))
    (ATTRIBUTES (%%VALUE (%%REST XML-EXPRESSION)))
    (TAGNAME (%SURFACE-FORM TAG)) (PREVIOUS-ITEM-WAS-STRING? CL:NIL)
      ...
      (CL:PROGN
       (CL:COND
        ((CL:EQ (%%REST XML-EXPRESSION) NIL)
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "<" TAGNAME "/>" EOL))
        (CL:T (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "<" TAGNAME)
               ...
               (CL:LET* ((FOUND?-000 CL:NIL))
                ...
                (CL:IF FOUND?-000
                 (WRITE-HTML-QUOTING-SPECIAL-CHARACTERS
                  (%NATIVE-STREAM STREAM) (%WRAPPER-VALUE ITEM))
                 (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
                  (%WRAPPER-VALUE ITEM))))
               (CL:SETQ PREVIOUS-ITEM-WAS-STRING? CL:T)))
             (CL:T
         ...
         (CL:IF IN-CONTENT-FIELD?
          (CL:PROGN
           ...
           (CL:WHEN
            (CL:AND (CL:NOT (CL:= INDENT NULL-INTEGER)) (CL:> INDENT 0))
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) EOL)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "/>" EOL))))))
     (CL:T
      (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
...
(CL:DEFUN HELP-STARTUP-XML1 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-XML-DECLARATION-TAG-MARKUP-DATA-START
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DECLARATION-TAG-MARKUP-DATA-START" NULL
     2))
   (CL:SETQ KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SINGLE-QUOTED-DECLARATION-TAG-DATA" NULL
     2))
   (CL:SETQ KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DOUBLE-QUOTED-DECLARATION-TAG-DATA" NULL
     2))
   ...
   (CL:SETQ KWD-XML-QUOTED-DECLARATION-TAG-DATA
    (INTERN-RIGID-SYMBOL-WRT-MODULE "QUOTED-DECLARATION-TAG-DATA" NULL 2)))
  :VOID)
    ...
    (CL:SETQ *XML-TOKENIZER-TABLE-DEFINITION*
     (LIST* (LIST* KWD-XML-START KWD-XML-INCLUDE KWD-XML-SKIP-WHITESPACE NIL)
      ...
      (LIST* KWD-XML-OPEN-TAG SYM-XML-STELLA-* (WRAP-STRING ">")
       ...
       (LIST* (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-OPEN-TAG SYM-XML-STELLA-* KWD-XML-OTHERWISE KWD-XML-START-TAG
       NIL)
      ...
      (LIST* KWD-XML-SKIP-TO-ATTRIBUTE-VALUE SYM-XML-STELLA-*
       ...
       KWD-XML-SINGLE-QUOTED-ATTRIBUTE-VALUE SYM-XML-STELLA-*
       (WRAP-STRING "\"") KWD-XML-DOUBLE-QUOTED-ATTRIBUTE-VALUE
       SYM-XML-STELLA-* KWD-XML-OTHERWISE KWD-XML-UNQUOTED-ATTRIBUTE-VALUE
       NIL)
      ...
      (LIST* KWD-XML-DECLARATION-WHITESPACE SYM-XML-STELLA-!
       (LIST* (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-DECLARATION-WHITESPACE SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-EMPTY-TAG-END SYM-XML-STELLA-* (WRAP-STRING "[")
       KWD-XML-DECLARATION-TAG-MARKUP-DATA-START SYM-XML-STELLA-*
       (WRAP-STRING "'") KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA
       SYM-XML-STELLA-* (WRAP-STRING "\"")
       KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA SYM-XML-STELLA-*
      ...
      (LIST* KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA (WRAP-STRING "'")
       KWD-XML-QUOTED-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
      ...
      (LIST* KWD-XML-START-SPECIAL-TAG SYM-XML-STELLA-* (WRAP-STRING "[")
       KWD-XML-SPECIAL-TAG-DATA KWD-XML-OTHERWISE KWD-XML-START-SPECIAL-TAG
       NIL)
      ...
      (LIST* KWD-XML-COMMENT-BODY (WRAP-STRING "-")
       KWD-XML-END-COMMENT-OR-COMMENT KWD-XML-OTHERWISE KWD-XML-COMMENT-BODY
       NIL)
...
In that case, each of the forms is indented by `indent' spaces.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-XML"
     "(DEFUN STARTUP-XML () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-XML) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-XML-STELLA-STARTUP-XML)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-XML-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupXml")
      NULL-STRING-WRAPPER)))
