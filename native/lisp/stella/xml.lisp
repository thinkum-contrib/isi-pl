...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* EOL STANDARD-INPUT NIL TRUE FALSE
  *CHARACTER-TYPE-TABLE* NULL-INTEGER))
...
(CL:DEFCLASS XML-OBJECT (STANDARD-OBJECT)
  ((NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION
   ...
   (SURFACE-FORM :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION
    :INSTANCE :ACCESSOR %SURFACE-FORM)))

(CL:DEFUN NEW-XML-OBJECT ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-OBJECT)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
...
(CL:DEFCLASS XML-ELEMENT (XML-OBJECT)
  ((NAMESPACE-NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %NAMESPACE-NAME)
   (NAMESPACE-URI :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %NAMESPACE-URI)
...
(CL:DEFUN NEW-XML-ELEMENT ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-ELEMENT)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%ELEMENT-DICTIONARY SELF) NULL)
   (CL:SETF (%NAMESPACE-URI SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAMESPACE-NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFUN ACCESS-XML-ELEMENT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-XML-STELLA-ELEMENT-DICTIONARY)
    (CL:IF SETVALUE? (CL:SETF (%ELEMENT-DICTIONARY SELF) VALUE)
     (CL:SETQ VALUE (%ELEMENT-DICTIONARY SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
...
(CL:DEFUN NEW-XML-LOCAL-ATTRIBUTE ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-LOCAL-ATTRIBUTE)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%PARENT-ELEMENT SELF) NULL)
...
(CL:DEFUN ACCESS-XML-LOCAL-ATTRIBUTE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-XML-STELLA-PARENT-ELEMENT)
    (CL:IF SETVALUE? (CL:SETF (%PARENT-ELEMENT SELF) VALUE)
     (CL:SETQ VALUE (%PARENT-ELEMENT SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
...
(CL:DEFUN NEW-XML-GLOBAL-ATTRIBUTE ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-GLOBAL-ATTRIBUTE)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAMESPACE-URI SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAMESPACE-NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFCLASS XML-PROCESSING-INSTRUCTION (XML-OBJECT)
  ((DATA :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION
    :INSTANCE :ACCESSOR %DATA)))

(CL:DEFUN NEW-XML-PROCESSING-INSTRUCTION ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-PROCESSING-INSTRUCTION)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATA SELF) STELLA::NULL-STRING)
...
(CL:DEFUN ACCESS-XML-PROCESSING-INSTRUCTION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-XML-STELLA-DATA)
    (CL:IF SETVALUE? (CL:SETF (%DATA SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%DATA SELF)))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
...
(CL:DEFMETHOD PRINT-OBJECT ((SELF XML-PROCESSING-INSTRUCTION) STREAM)
  (CL:IF (CL:EQ (%DATA SELF) STELLA::NULL-STRING)
   (%%PRINT-STREAM STREAM "<?" (%SURFACE-FORM SELF) ">")
...
(CL:DEFUN NEW-XML-DECLARATION ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-DECLARATION)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATA SELF) STELLA::NULL-STRING)
...
(CL:DEFUN NEW-XML-DOCTYPE-DECLARATION ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE XML-DOCTYPE-DECLARATION)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATA SELF) STELLA::NULL-STRING)
...
(CL:DEFMETHOD PRINT-OBJECT ((SELF XML-COMMENT) STREAM)
  (CL:IF (CL:EQ (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (%%PRINT-STREAM STREAM "<!-- -->")
   (%%PRINT-STREAM STREAM "<!-- "
    (CL:THE CL:FIXNUM (CL:LENGTH (CL:THE CL:SIMPLE-STRING (%SURFACE-FORM SELF))))
    " chars -->"))
  :VOID)
  ...
  (CL:LET* ((COLON-POSITION (POSITION NAME #\: 0)))
   ...
   (CL:IF (NULL? COLON-POSITION)
    (CL:LET* ((SELF-000 (NEW-XML-GLOBAL-ATTRIBUTE)))
     (CL:SETF (%NAME SELF-000) "") (CL:SETF (%NAMESPACE-NAME SELF-000) "xmlns")
     (CL:SETF (%SURFACE-FORM SELF-000) NAME)
    ...
    (CL:LET* ((SELF-001 (NEW-XML-GLOBAL-ATTRIBUTE)))
     ...
     (CL:SETF (%NAMESPACE-NAME SELF-001) "xmlns")
     (CL:SETF (%SURFACE-FORM SELF-001) NAME)
...
(CL:DEFUN MAKE-XML-ELEMENT-INTERNAL (NAME NAMESPACE-NAME NAMESPACE SURFACE-FORM)
  (CL:DECLARE
   (CL:TYPE CL:SIMPLE-STRING NAME NAMESPACE-NAME NAMESPACE SURFACE-FORM))
  #+MCL
  (CL:CHECK-TYPE NAME CL:SIMPLE-STRING)
  #+MCL
  ...
  (CL:LET* ((NS-HASH-TABLE NULL) (ELEMENT NULL))
   (CL:COND
    ((CL:NOT (CL:EQ NAMESPACE STELLA::NULL-STRING))
     (CL:SETQ NS-HASH-TABLE (LOOKUP *XML-ELEMENT-HASH-TABLE* NAMESPACE))
   ...
   (CL:WHEN (CL:EQ ELEMENT NULL)
    (CL:LET* ((SELF-001 (NEW-XML-ELEMENT))) (CL:SETF (%NAME SELF-001) NAME)
     (CL:SETF (%NAMESPACE-NAME SELF-001) NAMESPACE-NAME)
     (CL:SETF (%NAMESPACE-URI SELF-001) NAMESPACE)
     (CL:SETF (%SURFACE-FORM SELF-001) SURFACE-FORM)
     (CL:SETF (%ELEMENT-DICTIONARY SELF-001) (NEW-KEY-VALUE-LIST))
...
(CL:DEFUN MAKE-XML-ELEMENT-RESPECTING-NAMESPACE (NAME NAMESPACE-TABLE)
  ...
  (CL:LET*
   ...
   (CL:IF (NULL? COLON-POSITION) (CL:SETQ RAW-NAME NAME)
...
(CL:DEFUN MAKE-XML-GLOBAL-ATTRIBUTE-INTERNAL (NAME NAMESPACE-NAME NAMESPACE-URI SURFACE-FORM)
  (CL:DECLARE
   (CL:TYPE CL:SIMPLE-STRING NAME NAMESPACE-NAME NAMESPACE-URI SURFACE-FORM))
  #+MCL
  (CL:CHECK-TYPE NAME CL:SIMPLE-STRING)
  #+MCL
  (CL:CHECK-TYPE NAMESPACE-NAME CL:SIMPLE-STRING)
  ...
  (CL:WHEN
   (CL:OR (CL:EQ NAMESPACE-URI STELLA::NULL-STRING)
    (CL:EQ NAMESPACE-NAME STELLA::NULL-STRING) (STRING-EQL? NAMESPACE-URI "")
    (STRING-EQL? NAMESPACE-NAME ""))
   (CL:ERROR
    (NEW-STELLA-EXCEPTION "namespace-name and namespace must be specified")))
  (CL:LET*
   ((NS-HASH-TABLE (LOOKUP *XML-GLOBAL-ATTRIBUTE-HASH-TABLE* NAMESPACE-URI))
    (ATTRIBUTE NULL))
   (CL:WHEN (CL:EQ NS-HASH-TABLE NULL)
    (CL:SETQ NS-HASH-TABLE (NEW-STRING-HASH-TABLE))
   ...
   (CL:WHEN (CL:EQ ATTRIBUTE NULL)
    (CL:LET* ((SELF-002 (NEW-XML-GLOBAL-ATTRIBUTE)))
     (CL:SETF (%NAME SELF-002) NAME)
     (CL:SETF (%NAMESPACE-NAME SELF-002) NAMESPACE-NAME)
     (CL:SETF (%NAMESPACE-URI SELF-002) NAMESPACE-URI)
     (CL:SETF (%SURFACE-FORM SELF-002) SURFACE-FORM)
...
(CL:DEFUN MAKE-XML-GLOBAL-ATTRIBUTE-RESPECTING-NAMESPACE (NAME NAMESPACE-NAME SURFACE-FORM NAMESPACE-TABLE)
  ...
  (CL:CHECK-TYPE SURFACE-FORM CL:SIMPLE-STRING)
  (CL:LET*
   ((NAMESPACE-URI (LOOKUP NAMESPACE-TABLE (WRAP-STRING NAMESPACE-NAME))))
   (CL:WHEN (CL:EQ NAMESPACE-URI NULL)
    (CL:ERROR (NEW-STELLA-EXCEPTION "Undefined Namespace for Attribute!")))
   (CL:RETURN-FROM MAKE-XML-GLOBAL-ATTRIBUTE-RESPECTING-NAMESPACE
    (MAKE-XML-GLOBAL-ATTRIBUTE-INTERNAL NAME NAMESPACE-NAME
     (%WRAPPER-VALUE NAMESPACE-URI) SURFACE-FORM))))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ ATTRIBUTE NULL)
    (CL:LET* ((SELF-000 (NEW-XML-LOCAL-ATTRIBUTE)))
     (CL:SETF (%NAME SELF-000) NAME) (CL:SETF (%PARENT-ELEMENT SELF-000) ELEMENT)
     (CL:SETF (%SURFACE-FORM SELF-000) NAME) (CL:SETQ ATTRIBUTE SELF-000))
...
(CL:DEFUN XMLNS-ATTRIBUTE? (NAME)
  ...
  (CL:LET* ((COLON-POSITION (POSITION NAME #\: 0)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM COLON-POSITION))
   (CL:IF (NULL? COLON-POSITION)
    (CL:RETURN-FROM XMLNS-ATTRIBUTE? (STRING-EQL? NAME "xmlns"))
...
(CL:DEFUN EXTRACT-XMLNS-NAME (KEYNAME)
  ...
  (CL:LET* ((COLON-POSITION (POSITION KEYNAME #\: 0)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM COLON-POSITION))
   (CL:IF (NULL? COLON-POSITION) (CL:RETURN-FROM EXTRACT-XMLNS-NAME "")
    (CL:RETURN-FROM EXTRACT-XMLNS-NAME
     (SUBSEQUENCE KEYNAME (CL:1+ COLON-POSITION) NULL-INTEGER)))))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING) EXTRACT-XMLNS-NAME))
...
(CL:DEFUN TOKENIZE-XML-EXPRESSION (STREAM TOKENLIST REGIONTAGNAME SKIPTOREGION?)
  ...
  (CL:LET*
   ((PARENBALANCE 0) (STARTTAG STELLA::NULL-STRING)
    ...
    (REGIONTAGFOUND?
     (CL:OR (CL:EQ REGIONTAGNAME STELLA::NULL-STRING) (CL:NOT SKIPTOREGION?)))
   ...
   (CL:WHEN (CL:EQ TOKENLIST NULL) (CL:SETQ TOKENLIST (NEW-TOKENIZER-TOKEN))
   ...
   (CL:LET*
    ...
    (CL:LOOP
     (CL:TAGBODY
        ...
        (CL:WHEN (CL:NOT (CL:EQ TOK_ECHOSTREAM_ NULL))
         (%%PRINT-STREAM (%NATIVE-STREAM TOK_ECHOSTREAM_)
          (CL:LET ((BUFFER TOK_BUFFER_) (POSITION (CL:1- TOK_CURSOR_)))
           ...
           (CL:SCHAR (CL:THE CL:SIMPLE-STRING BUFFER)
            (CL:THE CL:FIXNUM POSITION)))))))
      (CL:WHEN TOK_ENDOFTOKENS?_ (CL:SETQ ENDOFFILE? TRUE)
      ...
      (CL:COND
       ...
       ((CL:EQ TOKENTYPE KWD-XML-ATTRIBUTE-NAME)
        (CL:SETQ TOKENCONTENT
         (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_ TOK_TOKENSTART_ TOK_CURSOR_
          TOK_SIZE_ FALSE)))
       ((CL:EQ TOKENTYPE KWD-XML-QUOTED-ATTRIBUTE-VALUE)
        (CL:SETQ TOKENCONTENT
          ...
          (CL:LOOP WHILE TRUE DO
           ...
           (CL:WHEN
            ...
            (CL:WHEN (CL:> I 0) (CL:SETQ I (CL:- 0 I))
             (CL:SETQ TOKENCONTENT
              (GET-TOKEN-TEXT-INTERNAL TOK_BUFFER_ TOK_TOKENSTART_
               (CL:IF (CL:< I 0) (CL:+ TOK_CURSOR_ I) (CL:+ TOK_TOKENSTART_ I))
               TOK_SIZE_ FALSE)))
            (CL:RETURN))))))
       ((CL:EQ TOKENTYPE KWD-XML-ERROR)
        (CL:WHEN (CL:NOT (CL:EQ TOK_STREAMSTATE_ NULL))
         ...
         (CL:SETF (%STATE TOK_STREAMSTATE_) TOK_STATE_))
        (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Illegal XML syntax: `"
          TOKENCONTENT "'")
       ...
       (CL:T
        ...
        (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "Illegal XML syntax: `"
          TOKENCONTENT "' in state `" TOKENTYPE "'")
      ...
      (CL:WHEN (CL:< PARENBALANCE 0)
       (CL:COND
        ((CL:AND (CL:NOT (CL:EQ REGIONTAGNAME STELLA::NULL-STRING))
          (CL:NOT SKIPTOREGION?) (STRING-EQL? (%CONTENT TOKENLIST) REGIONTAGNAME)
    ...
    (CL:IF ENDOFFILE?
     ...
     (CL:RETURN-FROM TOKENIZE-XML-EXPRESSION (CL:VALUES TOKENLIST FALSE))))))
...
(CL:DEFUN PROCESS-ATTRIBUTE-LIST (REVERSEATTRIBUTELIST ELEMENT NAMESPACE-TABLE)
  ...
  (CL:LET*
   ((ATTRIBUTELIST NIL) (CONSCELL NULL) (ATTRIBUTENAME STELLA::NULL-STRING))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ REVERSEATTRIBUTELIST NIL)) DO
    ...
    (CL:SETQ REVERSEATTRIBUTELIST (%%REST REVERSEATTRIBUTELIST))
    (CL:SETQ CONSCELL (CL:CONS CL:NIL CL:NIL))
    (CL:IF (XMLNS-ATTRIBUTE? ATTRIBUTENAME)
...
The name is available using class accessors."
  (CL:LET*
   ((PARSEDTREE NULL) (PARSEDTREESTACK NIL) (PARSEDTOKEN NULL)
    (TAGNAME STELLA::NULL-STRING) (ELEMENT NULL) (XMLNS-NAME STELLA::NULL-STRING)
    (COLON-POSITION 0) (ATTRIBUTELIST NIL) (CONSCELL NULL) (TOKENTYPE NULL)
    ...
    (CL:TAGBODY (CL:SETQ TOKENTYPE (%TYPE TOKENLIST))
     (CL:COND
      ((CL:EQ TOKENTYPE KWD-XML-START-TAG)
       ...
       (CL:SETQ PARSEDTREE NIL) (CL:SETQ TAGNAME (%CONTENT TOKENLIST))
       ...
       (CL:COND
        ((CL:EQ TOKENTYPE KWD-XML-START-PI-TAG)
         (CL:LET* ((SELF-000 (NEW-XML-PROCESSING-INSTRUCTION)))
          (CL:SETF (%NAME SELF-000) (%CONTENT TOKENLIST))
          (CL:SETF (%SURFACE-FORM SELF-000) (%CONTENT TOKENLIST))
          (CL:SETQ PARSEDTOKEN SELF-000)))
        ((CL:EQ TOKENTYPE KWD-XML-START-DECLARATION-TAG)
         (CL:LET* ((SELF-001 (NEW-XML-DECLARATION)))
          (CL:SETF (%NAME SELF-001) (%CONTENT TOKENLIST))
          (CL:SETF (%SURFACE-FORM SELF-001) (%CONTENT TOKENLIST))
          (CL:SETQ PARSEDTOKEN SELF-001)))
        ((CL:EQ TOKENTYPE KWD-XML-START-SPECIAL-TAG)
         (CL:LET* ((SELF-002 (NEW-XML-SPECIAL)))
          (CL:SETF (%NAME SELF-002) (%CONTENT TOKENLIST))
          (CL:SETF (%SURFACE-FORM SELF-002) (%CONTENT TOKENLIST))
          (CL:SETQ PARSEDTOKEN SELF-002)))
        (CL:T
         (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" TOKENTYPE
           "' is not a valid case option")
          (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
      ((CL:EQ TOKENTYPE KWD-XML-START-TAG-END)
       (CL:IF (CL:NOT (CL:EQ TAGNAME STELLA::NULL-STRING))
        ...
        (CL:SETQ PARSEDTOKEN
         (CL:IF (CL:EQ ATTRIBUTELIST NIL) NIL (REVERSE ATTRIBUTELIST))))
      ...
      ((CL:EQ TOKENTYPE KWD-XML-ATTRIBUTE-NAME)
       ...
       (CL:SETF (%%REST CONSCELL) ATTRIBUTELIST) (CL:SETQ ATTRIBUTELIST CONSCELL)
       (CL:WHEN (XMLNS-ATTRIBUTE? (%CONTENT TOKENLIST))
        (CL:SETQ XMLNS-NAME (%CONTENT TOKENLIST))
        ...
        (CL:WHEN (CL:NOT (CL:EQ (%TYPE TOKENLIST) KWD-XML-ATTRIBUTE-VALUE))
         (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001)
           "Illegal XMLNS syntax, namespace value is `" (%CONTENT TOKENLIST)
           "' of type `" (%TYPE TOKENLIST) "'")
          (CL:ERROR (NEW-READ-EXCEPTION (THE-STRING-READER STREAM-001)))))
        (CL:SETQ COLON-POSITION (POSITION XMLNS-NAME #\: 0))
        (CL:COND
         ((DEFINED? COLON-POSITION)
          (CL:SETQ NAMESPACE-TABLE
           (KV-CONS
            (WRAP-STRING
             (SUBSEQUENCE XMLNS-NAME (CL:1+ COLON-POSITION) NULL-INTEGER))
            (WRAP-STRING (%CONTENT TOKENLIST)) NAMESPACE-TABLE)))
         ((STRING-EQL? (%CONTENT TOKENLIST) "")
          (CL:SETQ NAMESPACE-TABLE
           (KV-CONS (WRAP-STRING "") NULL NAMESPACE-TABLE)))
         (CL:T
          (CL:SETQ NAMESPACE-TABLE
           (KV-CONS (WRAP-STRING "") (WRAP-STRING (%CONTENT TOKENLIST))
            NAMESPACE-TABLE))))
        (CL:SETQ PARSEDTOKEN (CL:MAKE-INSTANCE (CL:QUOTE STRING-WRAPPER)))
        (CL:SETF (%WRAPPER-VALUE PARSEDTOKEN) (%CONTENT TOKENLIST))
        (CL:SETQ CONSCELL (CL:CONS CL:NIL CL:NIL))
      ...
      (CL:T
       (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "Illegal XML syntax: `"
...
This change will not be backwards-compatible."
  (CL:LET* ((ITER (XML-EXPRESSIONS STREAM START-TAG)) (RESULT NULL) (EOF? FALSE))
   (CL:IF (NEXT? ITER) (CL:SETQ RESULT (%VALUE ITER)) (CL:SETQ EOF? TRUE))
   (CL:WHEN
...
(CL:DEFUN NEW-XML-EXPRESSION-ITERATOR ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%REGION-MATCH-FUNCTION SELF) (CL:FUNCTION XML-REGION-MATCHES?))
...
(CL:DEFMETHOD NEXT? ((SELF XML-EXPRESSION-ITERATOR))
  (CL:LET*
   ((STREAM (%THE-STREAM SELF)) (FIRSTITERATION? (%FIRST-ITERATION? SELF))
    ...
    (REGIONTAGNAME
     (CL:IF (CL:NOT (CL:EQ REGIONTAG NULL)) (%NAME (%%VALUE REGIONTAG))
      STELLA::NULL-STRING))
    (SKIPTOREGION? FALSE) (EOF? FALSE))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING REGIONTAGNAME))
   (CL:WHEN FIRSTITERATION? (CL:SETF (%FIRST-ITERATION? SELF) FALSE)
    (CL:WHEN (CL:NOT (CL:EQ REGIONTAG NULL)) (CL:SETQ SKIPTOREGION? TRUE)))
   (CL:LOOP
    (CL:TAGBODY
     (CL:MULTIPLE-VALUE-SETQ (TOKENLIST EOF?)
      (TOKENIZE-XML-EXPRESSION STREAM TOKENLIST REGIONTAGNAME SKIPTOREGION?))
     (CL:WHEN EOF?
     ...
     (CL:COND
      ((CL:AND SKIPTOREGION? (CL:NOT (CL:EQ REGIONTAG NULL)))
       (CL:LET* ((LASTTOKEN TOKENLIST) (EMPTYTAG? FALSE))
        ...
        (CL:IF (CL:EQ (%TYPE LASTTOKEN) KWD-XML-EMPTY-TAG-END)
        ...
        (CL:WHEN EMPTYTAG? (CL:SETQ REGIONTAG NULL)
         ...
         (CL:SETF (%REGION-TAG SELF) NULL))
        (CL:GO :CONTINUE)))
      (CL:T (CL:SETF (%VALUE SELF) (XML-TOKEN-LIST-TO-S-EXPRESSION TOKENLIST))
   ...
   (CL:LET*
    ...
    (CL:IF
     ...
     (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
      (CL:SETF (%THE-PLIST SELF-000) (%%VALUE (%%REST REGIONSPEC)))
      (CL:SETQ SPECATTRIBUTES SELF-000))
     (CL:RETURN-FROM XML-REGION-MATCHES? TRUE))
    (CL:LET* ((SELF-001 (NEW-PROPERTY-LIST)))
     (CL:SETF (%THE-PLIST SELF-001) (%%VALUE (%%REST REGIONTAG)))
     (CL:SETQ TAGATTRIBUTES SELF-001))
    (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST SPECATTRIBUTES)))
  ...
  (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE REGIONTAG)))
   (CL:COND
    ((SUBTYPE-OF-SYMBOL? TEST-VALUE-001)
     (CL:PROGN
      (CL:RETURN-FROM COERCE-TO-XML-ELEMENT
       (MAKE-XML-ELEMENT (%SYMBOL-NAME REGIONTAG) "" STELLA::NULL-STRING))))
    ...
    ((SUBTYPE-OF-STRING? TEST-VALUE-001)
     (CL:PROGN
      (CL:RETURN-FROM COERCE-TO-XML-ELEMENT
       (MAKE-XML-ELEMENT (%WRAPPER-VALUE REGIONTAG) "" STELLA::NULL-STRING))))
        ...
        (CL:COND ((CL:EQ REGIONTAG NIL))
         ((CL:EQ (%%REST REGIONTAG) NULL)
          (CL:SETF (%REGION-TAG ITERATOR)
           (CONS (COERCE-TO-XML-ELEMENT (%%VALUE REGIONTAG)) (CONS NIL NIL))))
...
(CL:DEFUN XML-CDATA? (ITEM)
  ...
  (CL:COND
   ...
   (CL:T (CL:RETURN-FROM XML-CDATA? FALSE))))
...
associated with it."
  ...
  (CL:RETURN-FROM XML-ELEMENT-MATCH?
   (CL:AND (STRING-EQL? (%NAME TAG) NAME)
    (STRING-EQL? (%NAMESPACE-URI TAG) NAMESPACE))))
...
must not have a namespace associated with it."
  ...
  (CL:RETURN-FROM XML-ATTRIBUTE-MATCH? FALSE))
...
must not have a namespace associated with it."
  ...
  (CL:RETURN-FROM XML-ATTRIBUTE-MATCH?
   (CL:AND (STRING-EQL? (%NAME ATTRIBUTE) NAME)
    (STRING-EQL? (%NAMESPACE-URI ATTRIBUTE) NAMESPACE))))
...
account use `xml-local-attribute-match?'."
  ...
  (CL:RETURN-FROM XML-ATTRIBUTE-MATCH?
   (CL:AND (CL:EQ NAMESPACE STELLA::NULL-STRING)
    (STRING-EQL? (%NAME ATTRIBUTE) NAME))))
  ...
  (CL:RETURN-FROM XML-LOCAL-ATTRIBUTE-MATCH?
   (CL:AND (STRING-EQL? (%NAME ATTRIBUTE) NAME)
    (CL:AND (STRING-EQL? (%NAME (%PARENT-ELEMENT ATTRIBUTE)) ELEMENT-NAME)
     (STRING-EQL? (%NAMESPACE-URI (%PARENT-ELEMENT ATTRIBUTE))
      ELEMENT-NAMESPACE)))))
...
(CL:DECLAIM
 (CL:FTYPE
  (CL:FUNCTION (CL:T CL:SIMPLE-STRING CL:SIMPLE-STRING) CL:SIMPLE-STRING)
...
(CL:DEFUN EXPAND-XML-TAG-CASE (ITEM CLAUSES)
  (CL:LET* ((OUTPUT-CLAUSES NIL) (KEY NULL))
         ...
         (CL:IF (CL:EQ KEY SYM-XML-STELLA-OTHERWISE)
          ...
          (CL:SETQ OUTPUT-CLAUSES
           (CONS
            (CONS
             (LIST* SYM-XML-STELLA-XML-ELEMENT-MATCH? ITEM
              (CONS (WRAP-STRING (%SYMBOL-NAME KEY))
   ...
   (CL:RETURN-FROM EXPAND-XML-TAG-CASE
    (CONS SYM-XML-STELLA-COND (CONCATENATE (REVERSE OUTPUT-CLAUSES) NIL)))))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ITEM)))
     (CL:COND
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-XML-STELLA-XML-ATTRIBUTE)
       (CL:PROGN
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " " (%SURFACE-FORM ITEM) "=")))
      ((SUBTYPE-OF-STRING? TEST-VALUE-000)
       (CL:PROGN
...
In that case, each of the forms is indented by `indent' spaces."
  ...
  (CL:LET*
   ((TAG (%%VALUE XML-EXPRESSION)) (ATTRIBUTES (%%VALUE (%%REST XML-EXPRESSION)))
    (TAGNAME (%SURFACE-FORM TAG)) (PREVIOUS-ITEM-WAS-STRING? FALSE)
    (IN-CONTENT-FIELD? FALSE))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TAGNAME))
   (CL:WHEN (DEFINED? INDENT)
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000)) DO
      ...
      (CL:SETQ I I) (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " "))))
      ...
      (CL:PROGN
       (CL:COND
        ...
        (CL:T (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "<" TAGNAME)
         ...
         (CL:SETQ PREVIOUS-ITEM-WAS-STRING? FALSE)
         (CL:WHEN (DEFINED? INDENT) (CL:SETQ INDENT (CL:+ INDENT 2)))
             ...
             ((SUBTYPE-OF-STRING? TEST-VALUE-000)
              (CL:PROGN
               ...
               (CL:WHEN PREVIOUS-ITEM-WAS-STRING?
                (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " "))
               (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (%WRAPPER-VALUE ITEM))
               (CL:SETQ PREVIOUS-ITEM-WAS-STRING? TRUE)))
             (CL:T
              (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
               (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" TEST-VALUE-000
                "' is not a valid case option")
         ...
         (CL:IF IN-CONTENT-FIELD?
          (CL:PROGN (CL:WHEN (DEFINED? INDENT) (CL:SETQ INDENT (CL:- INDENT 2)))
...
(CL:DEFUN HELP-STARTUP-XML1 ()
  (CL:PROGN
   ...
   (CL:SETQ KWD-XML-DECLARATION-TAG-MARKUP-DATA
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DECLARATION-TAG-MARKUP-DATA" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *XML-TOKENIZER-TABLE-DEFINITION*
     (LIST* (LIST* KWD-XML-START KWD-XML-INCLUDE KWD-XML-SKIP-WHITESPACE NIL)
      ...
      (LIST* KWD-XML-CONTENT SYM-XML-STELLA-! (WRAP-STRING "<") KWD-XML-OPEN-TAG
       KWD-XML-EOF KWD-XML-EOF KWD-XML-OTHERWISE KWD-XML-CONTENT NIL)
      ...
      (LIST* KWD-XML-PI-TAG-DATA-OR-END SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-EMPTY-TAG-END KWD-XML-OTHERWISE KWD-XML-PI-TAG-DATA NIL)
      ...
      (LIST* KWD-XML-OPEN-END-TAG SYM-XML-STELLA-! (WRAP-STRING ">")
       KWD-XML-END-TAG-END SYM-XML-STELLA-* KWD-XML-OTHERWISE KWD-XML-END-TAG
       NIL)
      (LIST* KWD-XML-END-TAG SYM-XML-STELLA-! (WRAP-STRING ">")
       KWD-XML-END-TAG-END KWD-XML-OTHERWISE KWD-XML-END-TAG NIL)
      (LIST* KWD-XML-SKIP-TO-ATTRIBUTE-NAME SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-START-TAG-END SYM-XML-STELLA-! (WRAP-STRING "/")
       KWD-XML-OPEN-EMPTY-TAG-END SYM-XML-STELLA-!
       (LIST* (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-SKIP-TO-ATTRIBUTE-NAME SYM-XML-STELLA-* KWD-XML-OTHERWISE
       KWD-XML-ATTRIBUTE-NAME NIL)
      (LIST* KWD-XML-ATTRIBUTE-NAME SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-START-TAG-END SYM-XML-STELLA-! (WRAP-STRING "/")
       KWD-XML-OPEN-EMPTY-TAG-END SYM-XML-STELLA-!
       (LIST* (WRAP-CHARACTER #\=) (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-SKIP-TO-ATTRIBUTE-VALUE KWD-XML-OTHERWISE KWD-XML-ATTRIBUTE-NAME
       NIL)
      (LIST* KWD-XML-SKIP-TO-ATTRIBUTE-VALUE SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-START-TAG-END SYM-XML-STELLA-! (WRAP-STRING "/")
       KWD-XML-OPEN-EMPTY-TAG-END
       (LIST* (WRAP-CHARACTER #\=) (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-SKIP-TO-ATTRIBUTE-VALUE SYM-XML-STELLA-* (WRAP-STRING "'")
       KWD-XML-SINGLE-QUOTED-ATTRIBUTE-VALUE SYM-XML-STELLA-* (WRAP-STRING "\"")
       KWD-XML-DOUBLE-QUOTED-ATTRIBUTE-VALUE SYM-XML-STELLA-* KWD-XML-OTHERWISE
      ...
      (LIST* KWD-XML-UNQUOTED-ATTRIBUTE-VALUE SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-START-TAG-END SYM-XML-STELLA-! (WRAP-STRING "/")
       KWD-XML-OPEN-EMPTY-TAG-END SYM-XML-STELLA-!
       (LIST* (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
      ...
      (LIST* KWD-XML-OPEN-DECLARATION-TAG (WRAP-STRING "-")
       KWD-XML-START-TAG-OR-COMMENT (WRAP-STRING "[") KWD-XML-OPEN-SPECIAL-TAG
       SYM-XML-STELLA-* KWD-XML-OTHERWISE KWD-XML-START-DECLARATION-TAG NIL)
      (LIST* KWD-XML-START-DECLARATION-TAG SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-EMPTY-TAG-END SYM-XML-STELLA-*
       (LIST* (WRAP-CHARACTER #\ ) (WRAP-CHARACTER #\Tab)
        (WRAP-CHARACTER #\Linefeed) (WRAP-CHARACTER #\Return) NIL)
       KWD-XML-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
       KWD-XML-START-DECLARATION-TAG NIL)
      (LIST* KWD-XML-DECLARATION-TAG-DATA SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-EMPTY-TAG-END SYM-XML-STELLA-* (WRAP-STRING "[")
       KWD-XML-DECLARATION-TAG-MARKUP-DATA-START (WRAP-STRING "'")
       KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA (WRAP-STRING "\"")
       KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
       KWD-XML-DECLARATION-TAG-DATA NIL)
      (LIST* KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA (WRAP-STRING "'")
       KWD-XML-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
       KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA NIL)
      (LIST* KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA (WRAP-STRING "\"")
       KWD-XML-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
       KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA NIL)
      (LIST* KWD-XML-DECLARATION-TAG-MARKUP-DATA-START (WRAP-STRING "]")
       KWD-XML-DECLARATION-TAG-MARKUP-DATA KWD-XML-OTHERWISE
       KWD-XML-DECLARATION-TAG-MARKUP-DATA-START NIL)
      (LIST* KWD-XML-DECLARATION-TAG-MARKUP-DATA SYM-XML-STELLA-*
       (WRAP-STRING ">") KWD-XML-EMPTY-TAG-END KWD-XML-OTHERWISE KWD-XML-ERROR
       NIL)
      (LIST* KWD-XML-OPEN-SPECIAL-TAG SYM-XML-STELLA-* KWD-XML-ANY
       KWD-XML-START-SPECIAL-TAG NIL)
      (LIST* KWD-XML-START-SPECIAL-TAG SYM-XML-STELLA-* (WRAP-STRING "[")
       KWD-XML-SPECIAL-TAG-DATA KWD-XML-OTHERWISE KWD-XML-START-SPECIAL-TAG NIL)
      (LIST* KWD-XML-SPECIAL-TAG-DATA (WRAP-STRING "]")
       KWD-XML-SPECIAL-TAG-DATA-OR-END KWD-XML-OTHERWISE KWD-XML-SPECIAL-TAG-DATA
       NIL)
      (LIST* KWD-XML-SPECIAL-TAG-DATA-OR-END (WRAP-STRING "]")
       (LIST* KWD-XML-SPECIAL-TAG-DATA-OR-END2 KWD-XML-SPECIAL-TAG-DATA NIL)
       KWD-XML-OTHERWISE KWD-XML-SPECIAL-TAG-DATA NIL)
      (LIST* KWD-XML-SPECIAL-TAG-DATA-OR-END2 SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-DATA-TAG-END KWD-XML-OTHERWISE KWD-XML-SPECIAL-TAG-DATA NIL)
      (LIST* KWD-XML-START-TAG-OR-COMMENT (WRAP-STRING "-") KWD-XML-COMMENT-BODY
       SYM-XML-STELLA-* KWD-XML-OTHERWISE KWD-XML-START-TAG NIL)
      (LIST* KWD-XML-COMMENT-BODY (WRAP-STRING "-")
       KWD-XML-END-COMMENT-OR-COMMENT KWD-XML-OTHERWISE KWD-XML-COMMENT-BODY NIL)
      (LIST* KWD-XML-END-COMMENT-OR-COMMENT (WRAP-STRING "-")
       KWD-XML-END-COMMENT-OR-COMMENT2 KWD-XML-OTHERWISE KWD-XML-COMMENT-BODY
       NIL)
      (LIST* KWD-XML-END-COMMENT-OR-COMMENT2 (WRAP-STRING ">") KWD-XML-COMMENT
      ...
      (LIST* KWD-XML-OPEN-EMPTY-TAG-END SYM-XML-STELLA-* (WRAP-STRING ">")
       KWD-XML-EMPTY-TAG-END KWD-XML-OTHERWISE KWD-XML-ERROR NIL)
...
`namespace-name' to refer to  `namespace'.  `namespace'must be a URI.\")"
    ...
    (DEFINE-FUNCTION-OBJECT "XMLNS-ATTRIBUTE?"
     "(DEFUN (XMLNS-ATTRIBUTE? BOOLEAN) ((NAME STRING)))"
    ...
    (DEFINE-FUNCTION-OBJECT "XML-TOKEN-LIST-TO-S-EXPRESSION"
     "(DEFUN (XML-TOKEN-LIST-TO-S-EXPRESSION OBJECT) ((TOKENLIST TOKENIZER-TOKEN)) :DOCUMENTATION \"Convert the XML `tokenList' into a representative s-expression
and return the result.  Every XML tag is represented as a cons-list starting
with the tag as its header, followed by a possibly empty list of keyword
value pairs representing tag attributes, followed by a possibly empty list of
content expressions which might themselves be XML expressions.  For example,
...
can also be (name namespace) pair.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "XML-ELEMENT?"
     "(DEFUN (XML-ELEMENT? BOOLEAN) ((ITEM OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Return `true' if `item' is an XML element object\")"
     (CL:FUNCTION XML-ELEMENT?) NULL)
    (DEFINE-FUNCTION-OBJECT "XML-ATTRIBUTE?"
     "(DEFUN (XML-ATTRIBUTE? BOOLEAN) ((ITEM OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Return `true' if `item' is an XML attribute object\")"
     (CL:FUNCTION XML-ATTRIBUTE?) NULL)
    (DEFINE-FUNCTION-OBJECT "XML-DECLARATION?"
     "(DEFUN (XML-DECLARATION? BOOLEAN) ((ITEM OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Return `true' if `item' is an XML declaration object\")"
     (CL:FUNCTION XML-DECLARATION?) NULL)
    (DEFINE-FUNCTION-OBJECT "XML-CDATA-FORM?"
     "(DEFUN (XML-CDATA-FORM? BOOLEAN) ((FORM OBJECT)) :PUBLIC? TRUE :DOCUMENTATION \"Return `true' if `form' is an CONS headed by a CDATA tag\")"
...
Note that `namespace' must be `null' and that the `attribute's parent element
element is not considered by the match.  To take the parent element into
account use `xml-local-attribute-match?'.\")" (CL:FUNCTION XML-ATTRIBUTE-MATCH?)
     NULL)
...
In that case, each of the forms is indented by `indent' spaces.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-XML" "(DEFUN STARTUP-XML () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-XML) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-XML-STELLA-STARTUP-XML)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-XML-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_Startup-Xml")
      NULL-STRING-WRAPPER)))
