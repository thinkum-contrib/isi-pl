...
(CL:DEFUN ACCESS-XML-OBJECT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-XML-STELLA-SURFACE-FORM)
    (CL:IF SETVALUE?
     (CL:SETF (%SURFACE-FORM SELF) (%WRAPPER-VALUE VALUE))
...
(CL:DEFCLASS XML-ELEMENT (XML-OBJECT)
  ((NAMESPACE-NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
   ...
   (ELEMENT-DICTIONARY :ALLOCATION :INSTANCE :ACCESSOR
    %ELEMENT-DICTIONARY)))
...
(CL:DEFUN ACCESS-XML-ELEMENT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-XML-STELLA-NAMESPACE-NAME)
    (CL:IF SETVALUE?
     (CL:SETF (%NAMESPACE-NAME SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%NAMESPACE-NAME SELF)))))
   ((CL:EQ SLOTNAME SYM-XML-STELLA-NAMESPACE-URI)
    (CL:IF SETVALUE?
     (CL:SETF (%NAMESPACE-URI SELF) (%WRAPPER-VALUE VALUE))
...
(CL:DEFCLASS XML-PROCESSING-INSTRUCTION (XML-OBJECT)
  ((DATA :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %DATA)))

(CL:DEFUN NEW-XML-PROCESSING-INSTRUCTION ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-INSTANCE (CL:QUOTE XML-PROCESSING-INSTRUCTION)))
   (CL:SETF (%SURFACE-FORM SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
...
(CL:DEFUN MAKE-XML-GLOBAL-ATTRIBUTE-RESPECTING-NAMESPACE (NAME NAMESPACE-NAME SURFACE-FORM NAMESPACE-TABLE)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ NAMESPACE-URI NULL)
    (CL:ERROR
     (NEW-STELLA-EXCEPTION "Undefined Namespace for Attribute!")))
...
(CL:DEFUN MAKE-XML-LOCAL-ATTRIBUTE (NAME ELEMENT)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ ATTRIBUTE NULL)
    ...
    (INSERT-AT (%ELEMENT-DICTIONARY ELEMENT) (WRAP-STRING NAME)
     ATTRIBUTE))
     ...
     (CL:COND
      ...
      ((CL:EQ TOKENTYPE KWD-XML-ATTRIBUTE-NAME)
       ...
       (CL:WHEN (XMLNS-ATTRIBUTE? (%CONTENT TOKENLIST))
        ...
        (CL:WHEN
         (CL:NOT (CL:EQ (%TYPE TOKENLIST) KWD-XML-ATTRIBUTE-VALUE))
         (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001)
...
This change will not be backwards-compatible."
  (CL:LET*
   ((ITER (XML-EXPRESSIONS STREAM START-TAG)) (RESULT NULL)
    (EOF? FALSE))
   (CL:IF (NEXT? ITER) (CL:SETQ RESULT (%VALUE ITER))
    (CL:SETQ EOF? TRUE))
   (CL:WHEN
  ...
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-XML-STELLA-REGION-MATCH-FUNCTION)
    (CL:IF SETVALUE?
     ...
     (CL:SETQ VALUE
      (WRAP-FUNCTION-CODE (%REGION-MATCH-FUNCTION SELF)))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME
      "' is not a valid case option")
     ...
     (CL:COND
      ((CL:AND SKIPTOREGION? (CL:NOT (CL:EQ REGIONTAG NULL)))
       (CL:LET* ((LASTTOKEN TOKENLIST) (EMPTYTAG? FALSE))
        (CL:LOOP WHILE (CL:NOT (CL:EQ (%TYPE (%NEXT LASTTOKEN)) NULL))
         DO (CL:SETQ LASTTOKEN (%NEXT LASTTOKEN)))
  ...
  (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE REGIONTAG)))
   (CL:COND
    ((SUBTYPE-OF-SYMBOL? TEST-VALUE-001)
     (CL:PROGN
      (CL:RETURN-FROM COERCE-TO-XML-ELEMENT
       (MAKE-XML-ELEMENT (%SYMBOL-NAME REGIONTAG) ""
        STELLA::NULL-STRING))))
    ...
    ((SUBTYPE-OF-STRING? TEST-VALUE-001)
     (CL:PROGN
      (CL:RETURN-FROM COERCE-TO-XML-ELEMENT
       (MAKE-XML-ELEMENT (%WRAPPER-VALUE REGIONTAG) ""
        STELLA::NULL-STRING))))
...
(CL:DEFUN XML-CDATA? (ITEM)
  ...
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE ITEM) SGT-XML-STELLA-XML-SPECIAL)
    (CL:PROGN
     (CL:RETURN-FROM XML-CDATA? (STRING-EQL? (%NAME ITEM) "CDATA"))))
...
parent element matches `element-name' and `element-namespace'."
  ...
  (CL:RETURN-FROM XML-LOCAL-ATTRIBUTE-MATCH?
   (CL:AND (STRING-EQL? (%NAME ATTRIBUTE) NAME)
    (CL:AND
     (STRING-EQL? (%NAME (%PARENT-ELEMENT ATTRIBUTE)) ELEMENT-NAME)
          ...
          (CL:PROGN
           ...
           (CL:WHEN (CL:NOT PREVIOUS-ITEM-WAS-STRING?)
            (CL:LET*
             ...
             (CL:DECLARE
              (CL:TYPE CL:FIXNUM ITER-003 UPPER-BOUND-001 I))
             (CL:LOOP WHILE
    ...
    (CL:SETQ *XML-TOKENIZER-TABLE-DEFINITION*
     (LIST*
      ...
      (LIST* KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA
       (WRAP-STRING "'") KWD-XML-DECLARATION-TAG-DATA KWD-XML-OTHERWISE
       KWD-XML-SINGLE-QUOTED-DECLARATION-TAG-DATA NIL)
      (LIST* KWD-XML-DOUBLE-QUOTED-DECLARATION-TAG-DATA
       (WRAP-STRING "\"") KWD-XML-DECLARATION-TAG-DATA
...
In that case, each of the forms is indented by `indent' spaces.\" :PUBLIC? TRUE)"
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-XML-STELLA-STARTUP-XML)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Xml") NULL-STRING-WRAPPER)))
