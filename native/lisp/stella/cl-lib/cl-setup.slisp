...
;;; Version: $Id: cl-setup.lisp,v 1.24 2001/03/09 21:03:07 tar Exp $
...
(cl:defvar *condition-message-keyword*
    #+(or :CCL-3 :CCL-4.0 :CCL-4.1 :CCL-4.2) :FORMAT-STRING
    #-(or :CCL-3 :CCL-4.0 :CCL-4.1 :CCL-4.2) :FORMAT-CONTROL)
...
(cl:defun %%get-exception-message (condition)
  ...
  (cl:if (cl:typep condition 'cl:simple-condition)
    (cl:apply #'cl:format cl:nil 
	      #+(or :CCL-3 :CCL-4.0 :CCL-4.1 :CCL-4.2)
	      (cl:simple-condition-format-string condition)
	      #-(or :CCL-3 :CCL-4.0 :CCL-4.1 :CCL-4.2)
	      (cl:simple-condition-format-control condition)
...
(CL:defun clsys-compute-hash-code-statistics (hashCodes)
  ...
  (CL:let ((n (CL:length hashCodes))
    ...
    (CL:setq median (CL:nth (CL:floor n 2.0) hashCodes))
    (CL:loop for code in hashCodes
        do (CL:incf mean code)
           (CL:when (CL:< code min)
             (CL:setq min code))
           (CL:when (CL:> code max)
             (CL:setq max code)))
    (CL:setq mean (CL:/ mean n 1.0))
    (CL:loop for code in hashCodes
        do (CL:incf variance (CL:* (CL:- code mean) (CL:- code mean))))
    (CL:setq variance (CL:/ variance n 1.0))
  ...
  (CL:let* ((n 1000)
    ...
    (CL:multiple-value-bind (min max mean median standardDeviation variance)
      ...
      (CL:and
       ...
       (CL:> mean (CL:* 0.4 (CL:- max min)))
       (CL:< mean (CL:* 0.7 (CL:- max min)))
       ;; the standard deviation should be somewhere around 0.5 of the mean:
       (CL:> standardDeviation (CL:* 0.4 mean))
       (CL:< standardDeviation (CL:* 0.7 mean))))))
    ...
    (CL:when (CL:or (CL:and (clsys-use-structs-p)
      ...
      (CL:setq hashCodeForm
        `(CL:let (,@(CL:when (CL:not (CL:atom object))
           ...
           (CL:when (CL:= code -1)
             (CL:setq code *clsys-hash-code-counter*)
             (CL:setf ,hashCodeSlot code)
             ;; WOULD USING `CL:random' HERE GENERATE BETTER HASHING??
             (CL:setq *clsys-hash-code-counter*
               (CL:logand (CL:1+ *clsys-hash-code-counter*) ,mask)))
           code)))
    hashCodeForm))
