...
(CL:DEFUN + (CL:&REST ARGS)
  (CL:LET* ((SUM 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM SUM))
   (CL:LET* ((ARG NULL) (ITER-000 ARGS))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ SUM (CL:+ SUM ARG))))
   (CL:RETURN-FROM + SUM)))
...
(CL:DEFUN - (X CL:&REST ARGS)
  (CL:IF (CL:= (CL:LENGTH ARGS) 0) (CL:RETURN-FROM - (CL:- X))
   (CL:LET* ((DIFFERENCE X))
    (CL:LET* ((ARG NULL) (ITER-000 ARGS))
     (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ DIFFERENCE (CL:- DIFFERENCE ARG))))
...
(CL:DEFUN * (CL:&REST ARGS)
  (CL:LET* ((PRODUCT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM PRODUCT))
   (CL:LET* ((ARG NULL) (ITER-000 ARGS))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ PRODUCT (CL:* PRODUCT ARG))))
...
(CL:DEFUN / (X CL:&REST ARGS)
  (CL:IF (CL:= (CL:LENGTH ARGS) 0) (CL:RETURN-FROM / (CL:/ X))
   (CL:LET* ((QUOTIENT X))
    (CL:LET* ((ARG NULL) (ITER-000 ARGS))
     (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ QUOTIENT (CL:/ QUOTIENT ARG))))
...
'end' value of NULL stands for the rest of the string."
  ...
  (CL:LET* ((NULLEND? (CL:= END NULL-INTEGER)))
   (CL:RETURN-FROM SUBSEQUENCE (CL:SUBSEQ STRING START (CL:IF NULLEND? CL:NIL END)))))
...
(CL:DEFUN STRINGIFY (EXPRESSION)
  (CL:LET* ((*PRINTPRETTY?* CL:NIL) (*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTPRETTY?* *PRINTREADABLY?*))
   (CL:RETURN-FROM STRINGIFY
    (CL:LET ((CL:*PRINT-PRETTY* CL:NIL)) (CL:WITH-OUTPUT-TO-STRING (S) (%%PRINT-OBJECT S EXPRESSION))))))
    ...
    (STELLA-CODE-TO-CONS-TREE
     (CL:LET
      ((CL:*PACKAGE* (CL:FIND-PACKAGE "STELLA"))
       (CL:*READTABLE* (CL:IF CASESENSITIVE? *CASE-SENSITIVE-READTABLE* *STANDARD-READTABLE*)))
      (CL:HANDLER-CASE (CL:READ-FROM-STRING STRING)
       (CL:STREAM-ERROR () (CL:ERROR (NEW-READ-EXCEPTION ">> Unstringify: Parse error~%")))
       (CL:ERROR (C)
        (CL:ERROR (NEW-UNHANDLED-EXCEPTION (CL:FORMAT NIL "Unstringify: Caught unhandled exception ~A.~%" C))))))))))
...
(CL:DEFUN CL-SYMBOL-TO-STELLA-SYMBOL (NAME)
  ...
  (CL:LET* ((MODULE *MODULE*) (BARENAME NAME)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BARENAME))
   (CL:WHEN (QUALIFIED-STELLA-NAME? NAME)
    (CL:MULTIPLE-VALUE-SETQ (MODULE BARENAME) (COMPUTE-MODULE-AND-BARE-NAME NAME))
   ...
   (CL:COND
    ((KEYWORD-NAME? BARENAME)
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Illegal read syntax: `" NAME "'")
      (CL:ERROR (NEW-READ-EXCEPTION (THE-STRING-READER STREAM-000)))))
    ((SURROGATE-NAME? BARENAME)
     (CL:RETURN-FROM CL-SYMBOL-TO-STELLA-SYMBOL
      (INTERN-RIGID-SYMBOL-WRT-MODULE (SUBSEQUENCE BARENAME 1 NULL-INTEGER) MODULE SURROGATE-SYM)))
...
(CL:DEFUN STELLA-CODE-TO-CONS-TREE (STELLACODE)
  (CL:COND ((CL:NULL STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE NIL))
   ((CL:CONSP STELLACODE)
    (CL:LET* ((CONSTREE NIL))
     (CL:LOOP FOR ELEMENT IN STELLACODE DO (SETQ CONSTREE (CONS (STELLA-CODE-TO-CONS-TREE ELEMENT) CONSTREE)))
    ...
    (CL:LET* ((NAME (CL:SYMBOL-NAME STELLACODE)))
     (CL:COND
      ((CL:KEYWORDP STELLACODE)
       (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (INTERN-RIGID-SYMBOL-WRT-MODULE NAME *MODULE* KEYWORD-SYM)))
      ((CL:AND (CL:EQL (CL:SYMBOL-PACKAGE STELLACODE) (CL:SYMBOL-PACKAGE (CL:QUOTE CL:EVAL)))
   ...
   ((CL:FLOATP STELLACODE)
    (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-FLOAT (CL:FLOAT (CL:float stellaCode 0.0d0) 0.0d0))))
   ((CL:STRINGP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-STRING STELLACODE)))
   ((CL:CHARACTERP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-CHARACTER STELLACODE)))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE STELLATREE)))
   (CL:COND
     ...
     (CL:PROGN
      (CL:IF (CL:EQ STELLATREE NIL) (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE CL-NIL)
       (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE
        (CL:CONS (CONS-TREE-TO-LISP-CODE (%%VALUE STELLATREE)) (CONS-TREE-TO-LISP-CODE (%%REST STELLATREE)))))))
    ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN (%SYMBOL-NAME STELLATREE) "KEYWORD"))))
        ...
        ((SYMBOL? TRANSLATEDSYMBOL)
         (CL:LET* ((NAME (%SYMBOL-NAME TRANSLATEDSYMBOL))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
          (CL:IF (SYMBOL-COMMON-LISP? TRANSLATEDSYMBOL)
           (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN NAME "COMMON-LISP"))
    ...
    ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000)
     (CL:PROGN
      (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN (CONCATENATE "@" (%SYMBOL-NAME STELLATREE)) "STELLA"))))
    ((SUBTYPE-OF-VERBATIM-STRING? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET* ((VERBATIMSTRING (%WRAPPER-VALUE STELLATREE)) (PACKAGENAME (LISP-PACKAGE *MODULE*)))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING VERBATIMSTRING PACKAGENAME))
       (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE
        (CL:LET ((CL:*PACKAGE* (CL:FIND-PACKAGE PACKAGENAME)) (CL:*READTABLE* *STANDARD-READTABLE*))
         (CL:READ-FROM-STRING VERBATIMSTRING CL:NIL CL:NIL))))))
    ((SUBTYPE-OF-INTEGER? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
...
(CL:DEFUN CONS-LIST-TO-LISP-LIST (LIST)
  (CL:LET* ((LISPLIST CL-NIL))
   (CL:LET* ((ITEM NULL) (ITER-000 LIST))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000)) (CL:PUSH ITEM LISPLIST)
...
(CL:DEFUN NATIVE-FILE-WRITE-DATE (FILENAME)
  (CL:RETURN-FROM NATIVE-FILE-WRITE-DATE
   (CL:LET ((DATE (CL:FILE-WRITE-DATE FILENAME))) (CL:IF (CL:NULL DATE) NULL (NATIVE-DATE-TIME-TO-CALENDAR-DATE DATE)))))
...
value of a second."
  (CL:RETURN-FROM TICKTOCK-DIFFERENCE
   (CL:/ (CL:FLOAT (CL:- T2 T1) 0.0D0) (CL:FLOAT CL:INTERNAL-TIME-UNITS-PER-SECOND 0.0D0))))
...
(CL:DEFUN STARTUP-CL-PRIMAL ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-CL-PRIMAL-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" NULL 1))
...
'string1' and 'string2'.\")" (CL:FUNCTION STRING-CONCATENATE) NULL)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY STRING) ((STRING STRING)) :DOCUMENTATION \"Return a copy of 'string'.\")"
...
'end' value of NULL stands for the rest of the string.\")" (CL:FUNCTION SUBSEQUENCE) NULL)
    (DEFINE-FUNCTION-OBJECT "STRINGIFY" "(DEFUN (STRINGIFY STRING) ((EXPRESSION OBJECT)))" (CL:FUNCTION STRINGIFY)
     NULL)
...
float and return the result.\" :PUBLIC? TRUE)" (CL:FUNCTION STRING-TO-FLOAT) NULL)
    (DEFINE-FUNCTION-OBJECT "CL-SYMBOL-TO-STELLA-SYMBOL"
     "(DEFUN (CL-SYMBOL-TO-STELLA-SYMBOL GENERALIZED-SYMBOL) ((NAME STRING)))" (CL:FUNCTION CL-SYMBOL-TO-STELLA-SYMBOL)
     NULL)
    (DEFINE-FUNCTION-OBJECT "STELLA-CODE-TO-CONS-TREE"
     "(DEFUN (STELLA-CODE-TO-CONS-TREE OBJECT) ((STELLACODE LISP-CODE)))" (CL:FUNCTION STELLA-CODE-TO-CONS-TREE) NULL)
    (DEFINE-FUNCTION-OBJECT "CONS-TREE-TO-LISP-CODE" "(DEFUN (CONS-TREE-TO-LISP-CODE LISP-CODE) ((STELLATREE OBJECT)))"
     (CL:FUNCTION CONS-TREE-TO-LISP-CODE) NULL)
    (DEFINE-FUNCTION-OBJECT "CONS-LIST-TO-LISP-LIST" "(DEFUN (CONS-LIST-TO-LISP-LIST LISP-CODE) ((LIST CONS)))"
    ...
    (DEFINE-FUNCTION-OBJECT "TOGGLE-CL-BASED-READERS" "(DEFUN TOGGLE-CL-BASED-READERS ())"
     (CL:FUNCTION TOGGLE-CL-BASED-READERS) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-PROBE-FILE?" "(DEFUN (NATIVE-PROBE-FILE? BOOLEAN) ((FILENAME FILE-NAME)))"
     (CL:FUNCTION NATIVE-PROBE-FILE?) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-FILE-WRITE-DATE"
     "(DEFUN (NATIVE-FILE-WRITE-DATE CALENDAR-DATE) ((FILENAME FILE-NAME)))" (CL:FUNCTION NATIVE-FILE-WRITE-DATE) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-FILE-LENGTH" "(DEFUN (NATIVE-FILE-LENGTH LONG-INTEGER) ((FILENAME FILE-NAME)))"
     (CL:FUNCTION NATIVE-FILE-LENGTH) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-DELETE-FILE" "(DEFUN NATIVE-DELETE-FILE ((FILENAME FILE-NAME)))"
     (CL:FUNCTION NATIVE-DELETE-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-RENAME-FILE" "(DEFUN NATIVE-RENAME-FILE ((FROMFILE FILE-NAME) (TOFILE FILE-NAME)))"
     (CL:FUNCTION NATIVE-RENAME-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "GET-TICKTOCK" "(DEFUN (GET-TICKTOCK TICKTOCK) () :PUBLIC? TRUE)"
...
the timing grain size may be larger than this resolution.\" :PUBLIC? TRUE)" (CL:FUNCTION TICKTOCK-RESOLUTION) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CL-PRIMAL" "(DEFUN STARTUP-CL-PRIMAL () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-CL-PRIMAL) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CL-PRIMAL-STELLA-STARTUP-CL-PRIMAL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CL-PRIMAL-STELLA-METHOD-STARTUP-CLASSNAME
