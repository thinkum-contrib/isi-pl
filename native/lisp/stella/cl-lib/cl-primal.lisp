...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* NIL KEYWORD-SYM SURROGATE-SYM SYMBOL-SYM
  *MODULE* FALSE TRUE))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM) CL:FIXNUM) INTEGER-LENGTH))
...
integer and return the result."
  ...
  #+MCL
  (CL:CHECK-TYPE STRING CL:SIMPLE-STRING)
  (CL:RETURN-FROM STRING-TO-INTEGER
   ...
   ((CL:SYMBOLP STELLACODE)
    (CL:LET* ((NAME (CL:SYMBOL-NAME STELLACODE)))
     (CL:COND
      ...
      ((QUALIFIED-NAME? NAME)
       (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE
        (INTERN-QUALIFIED-NAME NAME FALSE)))
      ((SURROGATE-NAME? NAME)
       (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE
        (INTERN-RIGID-SYMBOL-CASE-SENSITIVELY (CL:SUBSEQ NAME 1)
         SURROGATE-SYM FALSE)))
      (CL:T
       (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:FIXNUM) FILE-LENGTH))
...
the timing grain size may be larger than this resolution.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Cl-Primal") NULL-STRING-WRAPPER)))
