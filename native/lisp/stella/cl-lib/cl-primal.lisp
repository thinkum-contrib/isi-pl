...
(CL:DECLAIM (CL:SPECIAL NIL KEYWORD-SYM *ROOT-MODULE* *STELLA-MODULE* SYMBOL-SYM SURROGATE-SYM *MODULE* NULL-INTEGER))
...
(CL:DEFUN + (CL:&REST ARGS)
  (CL:LET* ((SUM 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM SUM))
   (CL:LET* ((ARG NULL) (ITER-000 ARGS)) (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ SUM (CL:+ SUM ARG)))) (CL:RETURN-FROM + SUM)))
...
(CL:DEFUN - (X CL:&REST ARGS)
  (CL:IF (CL:= (CL:LENGTH ARGS) 0) (CL:RETURN-FROM - (CL:- X))
   (CL:LET* ((DIFFERENCE X))
    (CL:LET* ((ARG NULL) (ITER-000 ARGS)) (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ DIFFERENCE (CL:- DIFFERENCE ARG))))
...
(CL:DEFUN * (CL:&REST ARGS)
  (CL:LET* ((PRODUCT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM PRODUCT))
   (CL:LET* ((ARG NULL) (ITER-000 ARGS)) (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ PRODUCT (CL:* PRODUCT ARG)))) (CL:RETURN-FROM * PRODUCT)))

;;; (DEFUN (/ NUMBER) ...)

(CL:DEFUN / (X CL:&REST ARGS)
  (CL:IF (CL:= (CL:LENGTH ARGS) 0) (CL:RETURN-FROM / (CL:/ X))
   (CL:LET* ((QUOTIENT X)) (CL:LET* ((ARG NULL) (ITER-000 ARGS)) (CL:LOOP WHILE ITER-000 DO (CL:SETQ ARG (CL:POP ITER-000)) (CL:SETQ QUOTIENT (CL:/ QUOTIENT ARG))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:T) CL:SIMPLE-STRING) MAKE-STRING))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING CL:SIMPLE-STRING) CL:SIMPLE-STRING) STRING-CONCATENATE))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING) STRING-UPCASE))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING) STRING-DOWNCASE))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING) STRING-CAPITALIZE))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING CL:SIMPLE-STRING CL:FIXNUM) CL:FIXNUM) STRING-SEARCH))
...
(CL:DEFUN STRINGIFY (EXPRESSION)
  (CL:LET* ((*PRINTPRETTY?* CL:NIL) (*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTPRETTY?* *PRINTREADABLY?*))
   (CL:RETURN-FROM STRINGIFY (CL:LET ((CL:*PRINT-PRETTY* CL:NIL)) (CL:WITH-OUTPUT-TO-STRING (S) (%%PRINT-OBJECT S EXPRESSION))))))
  ...
  (CL:LET* ((CASESENSITIVE? (%CASE-SENSITIVE? *MODULE*)))
   (CL:RETURN-FROM OLD-CL-READ-S-EXPRESSION-FROM-STRING
    (STELLA-CODE-TO-CONS-TREE
     (CL:LET ((CL:*PACKAGE* (CL:FIND-PACKAGE "STELLA")) (CL:*READTABLE* (CL:IF CASESENSITIVE? *CASE-SENSITIVE-READTABLE* *STANDARD-READTABLE*)))
      (CL:HANDLER-CASE (CL:READ-FROM-STRING STRING) (CL:STREAM-ERROR () (CL:ERROR (NEW-READ-EXCEPTION ">> Unstringify: Parse error~%")))
       (CL:ERROR (C) (CL:ERROR (NEW-UNHANDLED-EXCEPTION (CL:FORMAT NIL "Unstringify: Caught unhandled exception ~A.~%" C))))))))))

;;; (DEFUN (INTEGER-TO-STRING-IN-BASE STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:FIXNUM) CL:SIMPLE-STRING) INTEGER-TO-STRING-IN-BASE))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) INTEGER-TO-STRING))
(CL:DEFUN INTEGER-TO-STRING (INTEGER)
  "Return a string representation of `integer'."
  (CL:RETURN-FROM INTEGER-TO-STRING (INTEGER-TO-STRING-IN-BASE INTEGER 10)))

;;; (DEFUN (INTEGER-TO-HEX-STRING STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) INTEGER-TO-HEX-STRING))
(CL:DEFUN INTEGER-TO-HEX-STRING (INTEGER)
  "Return a hex string representation of 'integer'."
  (CL:RETURN-FROM INTEGER-TO-HEX-STRING (INTEGER-TO-STRING-IN-BASE INTEGER 16)))

;;; (DEFUN (FLOAT-TO-STRING STRING) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:DOUBLE-FLOAT) CL:SIMPLE-STRING) FLOAT-TO-STRING))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:DOUBLE-FLOAT CL:FIXNUM) CL:SIMPLE-STRING) FORMAT-FLOAT))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:DOUBLE-FLOAT) STRING-TO-FLOAT))
   ...
   (CL:WHEN (QUALIFIED-STELLA-NAME? NAME) (CL:MULTIPLE-VALUE-SETQ (MODULE BARENAME) (COMPUTE-MODULE-AND-BARE-NAME NAME))
    (CL:WHEN (STRING-EQL? BARENAME "")
     (CL:IF (CL:OR (CL:EQ MODULE *MODULE*) (CL:EQ MODULE *ROOT-MODULE*))
      (CL:RETURN-FROM CL-SYMBOL-TO-STELLA-SYMBOL (INTERN-RIGID-SYMBOL-WRT-MODULE "/" *STELLA-MODULE* SYMBOL-SYM))
      (CL:ERROR (NEW-READ-EXCEPTION "Empty symbol name encountered")))))
   (CL:COND
    ((KEYWORD-NAME? BARENAME)
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Illegal read syntax: `" NAME "'")
      (CL:ERROR (NEW-READ-EXCEPTION (THE-STRING-READER STREAM-000)))))
    ((SURROGATE-NAME? BARENAME) (CL:RETURN-FROM CL-SYMBOL-TO-STELLA-SYMBOL (INTERN-RIGID-SYMBOL-WRT-MODULE (SUBSEQUENCE BARENAME 1 NULL-INTEGER) MODULE SURROGATE-SYM)))
    (CL:T (CL:RETURN-FROM CL-SYMBOL-TO-STELLA-SYMBOL (INTERN-RIGID-SYMBOL-WRT-MODULE BARENAME MODULE SYMBOL-SYM))))))

;;; (DEFUN (STELLA-CODE-TO-CONS-TREE OBJECT) ...)

(CL:DEFUN STELLA-CODE-TO-CONS-TREE (STELLACODE)
  (CL:COND ((CL:NULL STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE NIL))
   ((CL:CONSP STELLACODE)
    (CL:LET* ((CONSTREE NIL)) (CL:LOOP FOR ELEMENT IN STELLACODE DO (SETQ CONSTREE (CONS (STELLA-CODE-TO-CONS-TREE ELEMENT) CONSTREE)))
     (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (REVERSE CONSTREE))))
   ((CL:SYMBOLP STELLACODE)
    (CL:LET* ((NAME (CL:SYMBOL-NAME STELLACODE)))
     (CL:COND ((CL:KEYWORDP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (INTERN-RIGID-SYMBOL-WRT-MODULE NAME *MODULE* KEYWORD-SYM)))
      ((CL:AND (CL:EQL (CL:SYMBOL-PACKAGE STELLACODE) (CL:SYMBOL-PACKAGE (CL:QUOTE CL:EVAL))) (CL:NOT (CL:EQ (CL:FIND-SYMBOL NAME "STELLA") STELLACODE)))
       (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (INTERN-COMMON-LISP-SYMBOL NAME)))
      (CL:T (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (CL-SYMBOL-TO-STELLA-SYMBOL NAME))))))
   ((CL:typep stellaCode 'CL:fixnum) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-INTEGER (CL:TRUNCATE STELLACODE))))
   ((CL:INTEGERP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-LONG-INTEGER (CL:TRUNCATE STELLACODE))))
   ((CL:FLOATP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-FLOAT (CL:FLOAT (CL:float stellaCode 0.0d0) 0.0d0))))
   ((CL:STRINGP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-STRING STELLACODE)))
   ((CL:CHARACTERP STELLACODE) (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE (WRAP-CHARACTER STELLACODE))) (CL:T (CL:RETURN-FROM STELLA-CODE-TO-CONS-TREE STELLACODE))))

;;; (DEFUN (CONS-TREE-TO-LISP-CODE LISP-CODE) ...)

(CL:DEFUN CONS-TREE-TO-LISP-CODE (STELLATREE)
  (CL:WHEN (CL:EQ STELLATREE NULL) (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:QUOTE NULL)))
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE STELLATREE)))
   (CL:COND
    ((CL:EQ TEST-VALUE-000 SGT-CL-PRIMAL-STELLA-CONS)
     (CL:PROGN
      (CL:IF (CL:EQ STELLATREE NIL) (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE CL-NIL)
       (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:CONS (CONS-TREE-TO-LISP-CODE (%%VALUE STELLATREE)) (CONS-TREE-TO-LISP-CODE (%%REST STELLATREE)))))))
    ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN (%SYMBOL-NAME STELLATREE) "KEYWORD"))))
    ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET* ((TRANSLATEDSYMBOL (CL-TRANSLATE-GLOBAL-SYMBOL STELLATREE)))
       (CL:COND
        ((SYMBOL? TRANSLATEDSYMBOL)
         (CL:LET* ((NAME (%SYMBOL-NAME TRANSLATEDSYMBOL))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
          (CL:IF (SYMBOL-COMMON-LISP? TRANSLATEDSYMBOL) (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN NAME "COMMON-LISP"))
           (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN NAME "STELLA")))))
        (CL:T (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CONS-TREE-TO-LISP-CODE TRANSLATEDSYMBOL)))))))
    ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:INTERN (CONCATENATE "@" (%SYMBOL-NAME STELLATREE)) "STELLA"))))
    ((SUBTYPE-OF-VERBATIM-STRING? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET* ((VERBATIMSTRING (%WRAPPER-VALUE STELLATREE)) (PACKAGENAME (LISP-PACKAGE *MODULE*))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING VERBATIMSTRING PACKAGENAME))
       (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE
        (CL:LET ((CL:*PACKAGE* (CL:FIND-PACKAGE PACKAGENAME)) (CL:*READTABLE* *STANDARD-READTABLE*)) (CL:READ-FROM-STRING VERBATIMSTRING CL:NIL CL:NIL))))))
    ((SUBTYPE-OF-INTEGER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    ((SUBTYPE-OF-LONG-INTEGER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    ((SUBTYPE-OF-FLOAT? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    ((SUBTYPE-OF-STRING? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    ((SUBTYPE-OF-CHARACTER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    ((SUBTYPE-OF-BOOLEAN? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (%WRAPPER-VALUE STELLATREE))))
    (CL:T (CL:WARN "cons-tree-to-lisp-code: Don't know how to convert `~A' into LISP-CODE" STELLATREE)
     (CL:RETURN-FROM CONS-TREE-TO-LISP-CODE (CL:FORMAT CL-NIL "~s" STELLATREE))))))
...
(CL:DEFUN CONS-LIST-TO-LISP-LIST (LIST)
  (CL:LET* ((LISPLIST CL-NIL))
   (CL:LET* ((ITEM NULL) (ITER-000 LIST))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000)) (CL:PUSH ITEM LISPLIST) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN OLD-CL-READ-S-EXPRESSION (INPUTSTREAM)
  (CL:LET*
   ((STREAM (%NATIVE-STREAM INPUTSTREAM)) (EOFVALUE :EOF) (CASESENSITIVE? (%CASE-SENSITIVE? *MODULE*))
    (INPUT
     (CL:LET ((CL:*PACKAGE* (CL:FIND-PACKAGE "STELLA")) (CL:*READTABLE* (CL:IF CASESENSITIVE? *CASE-SENSITIVE-READTABLE* *STANDARD-READTABLE*)))
      (CL:READ STREAM CL-NIL EOFVALUE))))
   (CL:IF (CL:EQ INPUT EOFVALUE) (CL:RETURN-FROM OLD-CL-READ-S-EXPRESSION (CL:VALUES NULL CL:T))
    (CL:RETURN-FROM OLD-CL-READ-S-EXPRESSION (CL:VALUES (STELLA-CODE-TO-CONS-TREE INPUT) CL:NIL)))))
...
(CL:DEFUN NATIVE-PROBE-FILE? (FILENAME)
  (CL:RETURN-FROM NATIVE-PROBE-FILE? (CL:NOT (CL:NULL (CL:IGNORE-ERRORS (CL:PROBE-FILE FILENAME))))))

;;; (DEFUN (NATIVE-FILE-WRITE-DATE CALENDAR-DATE) ...)

(CL:DEFUN NATIVE-FILE-WRITE-DATE (FILENAME)
  (CL:RETURN-FROM NATIVE-FILE-WRITE-DATE (CL:LET ((DATE (CL:FILE-WRITE-DATE FILENAME))) (CL:IF (CL:NULL DATE) NULL (NATIVE-DATE-TIME-TO-CALENDAR-DATE DATE)))))

;;; (DEFUN (NATIVE-FILE-LENGTH LONG-INTEGER) ...)

(CL:DEFUN NATIVE-FILE-LENGTH (FILENAME)
  (CL:RETURN-FROM NATIVE-FILE-LENGTH (CL:WITH-OPEN-FILE (FSTREAM FILENAME :DIRECTION :INPUT) (CL:FILE-LENGTH FSTREAM))))
...
(CL:DECLAIM (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:DOUBLE-FLOAT) TICKTOCK-DIFFERENCE))
...
value of a second."
  (CL:RETURN-FROM TICKTOCK-DIFFERENCE (CL:/ (CL:FLOAT (CL:- T2 T1) 0.0D0) (CL:FLOAT CL:INTERNAL-TIME-UNITS-PER-SECOND 0.0D0))))

;;; (DEFUN (TICKTOCK-RESOLUTION FLOAT) ...)

(CL:DECLAIM (CL:FTYPE (CL:FUNCTION () CL:DOUBLE-FLOAT) TICKTOCK-RESOLUTION))
...
the timing grain size may be larger than this resolution."
  (CL:RETURN-FROM TICKTOCK-RESOLUTION (CL:/ (CL:FLOAT CL:INTERNAL-TIME-UNITS-PER-SECOND 0.0D0))))

(CL:DEFUN STARTUP-CL-PRIMAL ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ SGT-CL-PRIMAL-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" NULL 1))
    (CL:SETQ SYM-CL-PRIMAL-STELLA-STARTUP-CL-PRIMAL (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-CL-PRIMAL" NULL 0))
    (CL:SETQ SYM-CL-PRIMAL-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" NULL 0)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (DEFINE-FUNCTION-OBJECT ">" "(DEFUN (> BOOLEAN) ((X NUMBER) (Y NUMBER)))" (CL:FUNCTION >) NULL)
    (DEFINE-FUNCTION-OBJECT ">=" "(DEFUN (>= BOOLEAN) ((X NUMBER) (Y NUMBER)))" (CL:FUNCTION >=) NULL)
    (DEFINE-FUNCTION-OBJECT "<" "(DEFUN (< BOOLEAN) ((X NUMBER) (Y NUMBER)))" (CL:FUNCTION <) NULL)
    (DEFINE-FUNCTION-OBJECT "<=" "(DEFUN (<= BOOLEAN) ((X NUMBER) (Y NUMBER)))" (CL:FUNCTION <=) NULL)
    (DEFINE-FUNCTION-OBJECT "+" "(DEFUN (+ NUMBER) (|&REST| (ARGS NUMBER)))" (CL:FUNCTION +) NULL)
    (DEFINE-FUNCTION-OBJECT "-" "(DEFUN (- NUMBER) ((X NUMBER) |&REST| (ARGS NUMBER)))" (CL:FUNCTION -) NULL)
    (DEFINE-FUNCTION-OBJECT "*" "(DEFUN (* NUMBER) (|&REST| (ARGS NUMBER)))" (CL:FUNCTION *) NULL)
    (DEFINE-FUNCTION-OBJECT "/" "(DEFUN (/ NUMBER) ((X NUMBER) |&REST| (ARGS NUMBER)))" (CL:FUNCTION /) NULL)
    (DEFINE-FUNCTION-OBJECT "CEILING" "(DEFUN (CEILING INTEGER) ((N NUMBER)))" (CL:FUNCTION CEILING) NULL)
    (DEFINE-FUNCTION-OBJECT "FLOOR" "(DEFUN (FLOOR INTEGER) ((N NUMBER)))" (CL:FUNCTION FLOOR) NULL)
    (DEFINE-FUNCTION-OBJECT "ROUND" "(DEFUN (ROUND INTEGER) ((N NUMBER)))" (CL:FUNCTION ROUND) NULL)
    (DEFINE-FUNCTION-OBJECT "TRUNCATE" "(DEFUN (TRUNCATE INTEGER) ((N NUMBER)))" (CL:FUNCTION TRUNCATE) NULL)
...
'string1' and 'string2'.\")" (CL:FUNCTION STRING-CONCATENATE) NULL)
    (DEFINE-FUNCTION-OBJECT "STRING-UPCASE" "(DEFUN (STRING-UPCASE STRING) ((STRING STRING)) :DOCUMENTATION \"Return an upper-case copy of 'string'.\")"
     (CL:FUNCTION STRING-UPCASE) NULL)
    (DEFINE-FUNCTION-OBJECT "STRING-DOWNCASE" "(DEFUN (STRING-DOWNCASE STRING) ((STRING STRING)) :DOCUMENTATION \"Return a lower-case copy of 'string'.\")"
     (CL:FUNCTION STRING-DOWNCASE) NULL)
    (DEFINE-FUNCTION-OBJECT "STRING-CAPITALIZE" "(DEFUN (STRING-CAPITALIZE STRING) ((STRING STRING)) :DOCUMENTATION \"Return a capitalized version of 'string'.\")"
     (CL:FUNCTION STRING-CAPITALIZE) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY STRING) ((STRING STRING)) :DOCUMENTATION \"Return a copy of 'string'.\")" (CL:FUNCTION COPY) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE STRING) ((STRING STRING) (CHAR CHARACTER)) :DOCUMENTATION \"Remove all occurences of 'char' from 'string'.\")"
...
in the string 'self'.\")" (CL:FUNCTION SUBSTITUTE) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (MEMBER? BOOLEAN) ((SELF STRING) (CHAR CHARACTER)))" (CL:FUNCTION MEMBER?) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (REST STRING) ((SELF STRING)))" (CL:FUNCTION REST) NULL)
    (DEFINE-METHOD-OBJECT
...
'start' of 'substring' in 'string'.  Return NULL if it is not a substring.\")" (CL:FUNCTION STRING-SEARCH) NULL)
    (DEFINE-METHOD-OBJECT
...
and ending up to but not including position 'end', counting from zero.  An
'end' value of NULL stands for the rest of the string.\")" (CL:FUNCTION SUBSEQUENCE) NULL)
    (DEFINE-FUNCTION-OBJECT "STRINGIFY" "(DEFUN (STRINGIFY STRING) ((EXPRESSION OBJECT)))" (CL:FUNCTION STRINGIFY) NULL)
    (DEFINE-FUNCTION-OBJECT "OLD-CL-READ-S-EXPRESSION-FROM-STRING" "(DEFUN (OLD-CL-READ-S-EXPRESSION-FROM-STRING OBJECT) ((STRING STRING)))"
...
Helping function for `integer-to-string' and `integer-to-hex-string'\")" (CL:FUNCTION INTEGER-TO-STRING-IN-BASE) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "FLOAT-TO-STRING"
     "(DEFUN (FLOAT-TO-STRING STRING) ((FLOAT FLOAT)) :DOCUMENTATION \"Return a string representation of 'float'.\" :PUBLIC? TRUE)" (CL:FUNCTION FLOAT-TO-STRING) NULL)
...
integer and return the result.\" :PUBLIC? TRUE)" (CL:FUNCTION STRING-TO-INTEGER) NULL)
    (DEFINE-FUNCTION-OBJECT "STRING-TO-FLOAT" "(DEFUN (STRING-TO-FLOAT FLOAT) ((STRING STRING)) :DOCUMENTATION \"Convert the 'string' representation of a float into a
float and return the result.\" :PUBLIC? TRUE)" (CL:FUNCTION STRING-TO-FLOAT) NULL)
    (DEFINE-FUNCTION-OBJECT "CL-SYMBOL-TO-STELLA-SYMBOL" "(DEFUN (CL-SYMBOL-TO-STELLA-SYMBOL GENERALIZED-SYMBOL) ((NAME STRING)))"
     (CL:FUNCTION CL-SYMBOL-TO-STELLA-SYMBOL) NULL)
    (DEFINE-FUNCTION-OBJECT "STELLA-CODE-TO-CONS-TREE" "(DEFUN (STELLA-CODE-TO-CONS-TREE OBJECT) ((STELLACODE LISP-CODE)))" (CL:FUNCTION STELLA-CODE-TO-CONS-TREE) NULL)
    (DEFINE-FUNCTION-OBJECT "CONS-TREE-TO-LISP-CODE" "(DEFUN (CONS-TREE-TO-LISP-CODE LISP-CODE) ((STELLATREE OBJECT)))" (CL:FUNCTION CONS-TREE-TO-LISP-CODE) NULL)
    (DEFINE-FUNCTION-OBJECT "CONS-LIST-TO-LISP-LIST" "(DEFUN (CONS-LIST-TO-LISP-LIST LISP-CODE) ((LIST CONS)))" (CL:FUNCTION CONS-LIST-TO-LISP-LIST) NULL)
    (DEFINE-FUNCTION-OBJECT "OLD-CL-READ-S-EXPRESSION" "(DEFUN (OLD-CL-READ-S-EXPRESSION OBJECT BOOLEAN) ((INPUTSTREAM INPUT-STREAM)))"
     (CL:FUNCTION OLD-CL-READ-S-EXPRESSION) NULL)
    (DEFINE-FUNCTION-OBJECT "TOGGLE-CL-BASED-READERS" "(DEFUN TOGGLE-CL-BASED-READERS ())" (CL:FUNCTION TOGGLE-CL-BASED-READERS) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-PROBE-FILE?" "(DEFUN (NATIVE-PROBE-FILE? BOOLEAN) ((FILENAME FILE-NAME)))" (CL:FUNCTION NATIVE-PROBE-FILE?) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-FILE-WRITE-DATE" "(DEFUN (NATIVE-FILE-WRITE-DATE CALENDAR-DATE) ((FILENAME FILE-NAME)))" (CL:FUNCTION NATIVE-FILE-WRITE-DATE) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-FILE-LENGTH" "(DEFUN (NATIVE-FILE-LENGTH LONG-INTEGER) ((FILENAME FILE-NAME)))" (CL:FUNCTION NATIVE-FILE-LENGTH) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-DELETE-FILE" "(DEFUN NATIVE-DELETE-FILE ((FILENAME FILE-NAME)))" (CL:FUNCTION NATIVE-DELETE-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "NATIVE-RENAME-FILE" "(DEFUN NATIVE-RENAME-FILE ((FROMFILE FILE-NAME) (TOFILE FILE-NAME)))" (CL:FUNCTION NATIVE-RENAME-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "GET-TICKTOCK" "(DEFUN (GET-TICKTOCK TICKTOCK) () :PUBLIC? TRUE)" (CL:FUNCTION GET-TICKTOCK) NULL)
...
The resolution is implementation dependent but will normally be some fractional
value of a second.\" :PUBLIC? TRUE)" (CL:FUNCTION TICKTOCK-DIFFERENCE) NULL)
...
difference in two TICKTOCK time values in seconds.  This resolution is
implementation dependent.  It may also not be realizable in practice, since
the timing grain size may be larger than this resolution.\" :PUBLIC? TRUE)" (CL:FUNCTION TICKTOCK-RESOLUTION) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CL-PRIMAL" "(DEFUN STARTUP-CL-PRIMAL () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-CL-PRIMAL) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CL-PRIMAL-STELLA-STARTUP-CL-PRIMAL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CL-PRIMAL-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupClPrimal") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/STELLA")))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFCONSTANT CL-NIL LISP-CODE (VERBATIM :COMMON-LISP (CL:OR)))")))
  :VOID)
