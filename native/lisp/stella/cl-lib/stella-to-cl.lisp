...
(CL:DEFUN CL-INCREMENTALLY-TRANSLATE (EXPRESSION)
  (CL:LET* ((TREE NULL))
   (CL:LET* ((*TRANSIENTOBJECTS?* CL:T)) (CL:DECLARE (CL:SPECIAL *TRANSIENTOBJECTS?*))
...
due to the head of the result matching the head of 'form'."
  (CL:LET ((HEAD (CL:AND (CL:CONSP FORM) (CL:CAR FORM))) (EXPANDEDFORM (CL-INCREMENTALLY-TRANSLATE FORM)))
   (CL:IF (CL:AND (CL:CONSP EXPANDEDFORM) (CL:EQ HEAD (CL:CAR EXPANDEDFORM)))
    (CL:PROGN (CL:FORMAT CL:T "Can't expand the macro ~A~%" HEAD) (CL:RETURN-FROM SAFELY-EXPAND-MACRO NULL))
...
or Verbatim."
  (CL:LET* ((TYPE SGT-STELLA-TO-CL-STELLA-UNKNOWN) (LISPCODE STELLA::NULL-STRING))
   ...
   (CL:WHEN (UNKNOWN-TYPE? TYPE)
    (CL:RETURN-FROM CLV (LIST* SYM-STELLA-TO-CL-STELLA-VERBATIM KWD-STELLA-TO-CL-COMMON-LISP CODE NIL)))
   (CL:SETQ CODE (NEW-VERBATIM-STRING-WRAPPER LISPCODE))
   (CL:RETURN-FROM CLV
    (LIST* SYM-STELLA-TO-CL-STELLA-CAST (LIST* SYM-STELLA-TO-CL-STELLA-VERBATIM KWD-STELLA-TO-CL-COMMON-LISP CODE NIL)
     TYPE NIL))))
...
(CL:DEFUN EMACS-REGION-TO-STATEMENTS (REGION)
  ...
  (CL:LET* ((START 0) (NEWSTART 0) (LISPSTATEMENT NULL) (STELLASTATEMENT NULL) (EOFVALUE *MODULE*) (RESULT NIL))
   ...
   (CL:LOOP (CL:multiple-value-setq (lispStatement newStart)
    ...
    (CL:HANDLER-CASE (CL:SETQ STELLASTATEMENT (READ-S-EXPRESSION-FROM-STRING (SUBSEQUENCE REGION START NEWSTART)))
     (READ-EXCEPTION () (CL:SETQ STELLASTATEMENT NULL)))
    (CL:IF
     (CL:AND (CONS? STELLASTATEMENT)
      (CL:OR (DECLARATION-TREE? STELLASTATEMENT)
       (CL:AND (SYMBOL? (%%VALUE STELLASTATEMENT)) (CL:NOT (CL:EQ (LOOKUP-COMMAND (%%VALUE STELLASTATEMENT)) NULL)))))
     (CL:SETQ RESULT (CONS (CONS-LIST KWD-STELLA-TO-CL-STELLA STELLASTATEMENT) RESULT))
     (CL:SETQ RESULT (CONS (CONS-LIST KWD-STELLA-TO-CL-LISP LISPSTATEMENT) RESULT)))
...
(CL:DEFUN TRANSLATE-EMACS-REGION-IN-MODULE (MODULENAME REGION)
  ...
  (CL:LET* ((RESULT CL:NIL))
   (CL:LET* ((*MODULE* (GET-STELLA-MODULE MODULENAME CL:T)) (*CONTEXT* *MODULE*))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ STATEMENT (%%VALUE ITER-000))
      (CL:COND
       ((CL:EQ (%%VALUE STATEMENT) KWD-STELLA-TO-CL-STELLA)
        (CL:PUSH (CONS-TREE-TO-LISP-CODE (INCREMENTALLY-TRANSLATE (%%VALUE (%%REST STATEMENT)))) RESULT))
...
(CL:DEFUN LISP-EVAL-FORM-IN-MODULE (MODULENAME FORM)
  ...
  (CL:CHECK-TYPE MODULENAME CL:SIMPLE-STRING)
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE MODULENAME CL:T)) (*CONTEXT* *MODULE*))
   (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*)) (CL:RETURN-FROM LISP-EVAL-FORM-IN-MODULE (CL:EVAL FORM))))
    ...
    (CL:COND
     ((CL:EQ TEST-VALUE-000 SGT-STELLA-TO-CL-STELLA-CONS)
      (CL:PROGN (CL:WHEN (CL:EQ THING NIL) (CL:RETURN-FROM LISPIFY (CL:IDENTITY CL-NIL)))
       (CL:LET* ((FIRST (LISPIFY (%%VALUE THING))) (REST (LISPIFY (%%REST THING))))
      ...
      (CL:PROGN
       (CL:LET* ((RESULT CL-NIL))
        (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-KV-LIST THING)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ KEY (%KEY ITER-000))
          (CL:SETQ VALUE (%VALUE ITER-000)) (CL:PUSH (CL:CONS (LISPIFY KEY) (LISPIFY VALUE)) RESULT)
          (CL:SETQ ITER-000 (%REST ITER-000))))
     ...
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
      (CL:PROGN (CL:RETURN-FROM LISPIFY (CONS-TREE-TO-LISP-CODE (CL-TRANSLATE-GLOBAL-SYMBOL THING)))))
     ...
     ((SUBTYPE-OF-BOOLEAN? TEST-VALUE-000)
      (CL:PROGN (CL:LET* ((TRUTHVALUE (%WRAPPER-VALUE THING))) (CL:RETURN-FROM LISPIFY TRUTHVALUE))))
     ...
     ((SUBTYPE-OF-CHARACTER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING))))
...
Stella boolean."
  (CL:IF (CL:NOT (CL:typep thing 'OBJECT)) (CL:RETURN-FROM LISPIFY-BOOLEAN THING)
...
In a call directly from Lisp 'slotName' can also be supplied as a Lisp symbol."
  ...
  (CL:LET*
   ((OBJECTTYPE (PRIMARY-TYPE OBJECT))
    (SLOT (LOOKUP-SLOT (TYPE-TO-CLASS OBJECTTYPE) (INTERN-DERIVED-SYMBOL OBJECTTYPE SLOTNAME))) (VALUE NULL))
   (CL:WHEN (CL:EQ SLOT NULL) (CL:WARN "slot-value: Slot `~A' does not exist on object `~A'." SLOTNAME OBJECT)
     ...
     (CL:PROGN (CL:SETQ VALUE (READ-SLOT-VALUE OBJECT SLOT))
      (CL:IF (CL:EQ DONTCONVERT? CL:T) (CL:RETURN-FROM CL-SLOT-VALUE VALUE)
       (CL:IF (CL:EQ (TYPE SLOT) SGT-STELLA-TO-CL-STELLA-BOOLEAN)
        (CL:RETURN-FROM CL-SLOT-VALUE (LISPIFY-BOOLEAN VALUE)) (CL:RETURN-FROM CL-SLOT-VALUE (LISPIFY VALUE))))))
...
directly from Lisp 'slotName' can also be supplied as a Lisp symbol."
  ...
  (CL:LET*
   ((OBJECTTYPE (PRIMARY-TYPE OBJECT))
    (SLOT (LOOKUP-SLOT (TYPE-TO-CLASS OBJECTTYPE) (INTERN-DERIVED-SYMBOL OBJECTTYPE SLOTNAME))) (STELLAVALUE VALUE))
   (CL:WHEN (CL:EQ SLOT NULL) (CL:WARN "slot-value-setter: Slot `~A' does not exist on object `~A'." SLOTNAME OBJECT)
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND (CL:NOT (CL:EQ (CL:TYPEP STELLAVALUE (CL:QUOTE OBJECT)) CL-NIL))
      (SUBTYPE-OF? (PRIMARY-TYPE STELLAVALUE) (TYPE-TO-WRAPPED-TYPE (TYPE SLOT)))))
    (CL:WARN "slot-value-setter: Illegal value type for slot `~A'~% on object `~A'.  Type `~A' expected." SLOTNAME
     OBJECT (TYPE SLOT))
    (CL:RETURN-FROM CL-SLOT-VALUE-SETTER NULL))
...
(CL:DEFUN SLOT-VALUE (OBJECT SLOTNAME CL:&OPTIONAL DONTCONVERT?)
  "See 'cl-slot-value'."
  (CL:IF (CL:OR (CL:NOT DONTCONVERT?) (CL:EQL DONTCONVERT? FALSE)) (CL-SLOT-VALUE OBJECT SLOTNAME FALSE)
...
(CL:DEFUN SLOT-VALUE-SETTER (OBJECT SLOTNAME VALUE CL:&OPTIONAL DONTCONVERT?)
  ...
  (CL:IF (CL:OR (CL:NOT DONTCONVERT?) (CL:EQL DONTCONVERT? FALSE)) (CL-SLOT-VALUE-SETTER OBJECT SLOTNAME VALUE FALSE)
...
(CL:DEFUN STARTUP-STELLA-TO-CL ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-UNKNOWN (INTERN-RIGID-SYMBOL-WRT-MODULE "UNKNOWN" NULL 1))
    ...
    (CL:SETQ KWD-STELLA-TO-CL-STELLA (INTERN-RIGID-SYMBOL-WRT-MODULE "STELLA" NULL 2))
    (CL:SETQ KWD-STELLA-TO-CL-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "LISP" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "CL-INCREMENTALLY-TRANSLATE"
     "(DEFUN (CL-INCREMENTALLY-TRANSLATE LISP-CODE) ((EXPRESSION LISP-CODE)))" (CL:FUNCTION CL-INCREMENTALLY-TRANSLATE)
     NULL)
    (DEFINE-FUNCTION-OBJECT "BI-MODAL-TRANSLATE" "(DEFUN (BI-MODAL-TRANSLATE LISP-CODE) ((STATEMENT LISP-CODE)))"
     (CL:FUNCTION BI-MODAL-TRANSLATE) NULL)
    (DEFINE-FUNCTION-OBJECT "HELP-BI-MODAL-TRANSLATE"
     "(DEFUN (HELP-BI-MODAL-TRANSLATE LISP-CODE) ((STATEMENT LISP-CODE)))" (CL:FUNCTION HELP-BI-MODAL-TRANSLATE) NULL)
...
or Verbatim.\" :PUBLIC? TRUE)" (CL:FUNCTION CLV) NULL)
    (DEFINE-FUNCTION-OBJECT "EMACS-REGION-TO-STATEMENTS"
     "(DEFUN (EMACS-REGION-TO-STATEMENTS (CONS OF CONS)) ((REGION STRING)))" (CL:FUNCTION EMACS-REGION-TO-STATEMENTS)
     NULL)
...
directly from Lisp 'slotName' can also be supplied as a Lisp symbol.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-STELLA-TO-CL" "(DEFUN STARTUP-STELLA-TO-CL () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-STELLA-TO-CL) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STELLA-TO-CL-STELLA-STARTUP-STELLA-TO-CL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-STELLA-TO-CL-STELLA-METHOD-STARTUP-CLASSNAME
