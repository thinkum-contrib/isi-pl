...
(CL:DECLAIM (CL:SPECIAL *STELLA-MODULE* EOL TRUE-WRAPPER FALSE-WRAPPER CL-NIL *MODULE* NIL))

;;; (DEFUN (CL-INCREMENTALLY-TRANSLATE LISP-CODE) ...)

(CL:DEFUN CL-INCREMENTALLY-TRANSLATE (EXPRESSION)
  (CL:LET* ((TREE NULL)) (CL:LET* ((*TRANSIENTOBJECTS?* CL:T)) (CL:DECLARE (CL:SPECIAL *TRANSIENTOBJECTS?*)) (CL:SETQ TREE (STELLA-CODE-TO-CONS-TREE EXPRESSION)))
   (CL:RETURN-FROM CL-INCREMENTALLY-TRANSLATE (CONS-TREE-TO-LISP-CODE (INCREMENTALLY-TRANSLATE TREE)))))
...
due to the head of the result matching the head of 'form'."
  (CL:LET ((HEAD (CL:AND (CL:CONSP FORM) (CL:CAR FORM))) (EXPANDEDFORM (CL-INCREMENTALLY-TRANSLATE FORM)))
   (CL:IF (CL:AND (CL:CONSP EXPANDEDFORM) (CL:EQ HEAD (CL:CAR EXPANDEDFORM)))
    (CL:PROGN (CL:FORMAT CL:T "Can't expand the macro ~A~%" HEAD) (CL:RETURN-FROM SAFELY-EXPAND-MACRO NULL)) (CL:RETURN-FROM SAFELY-EXPAND-MACRO EXPANDEDFORM))))
...
(CL:DEFUN HELP-BI-MODAL-TRANSLATE (STATEMENT)
  (CL:RETURN-FROM HELP-BI-MODAL-TRANSLATE (CL-INCREMENTALLY-TRANSLATE STATEMENT)))
...
or Verbatim."
  (CL:LET* ((TYPE SGT-STELLA-TO-CL-STELLA-UNKNOWN) (LISPCODE STELLA::NULL-STRING)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING LISPCODE))
   ...
   (CL:WHEN (UNKNOWN-TYPE? TYPE) (CL:RETURN-FROM CLV (LIST* SYM-STELLA-TO-CL-STELLA-VERBATIM KWD-STELLA-TO-CL-COMMON-LISP CODE NIL)))
   (CL:SETQ CODE (NEW-VERBATIM-STRING-WRAPPER LISPCODE))
   (CL:RETURN-FROM CLV (LIST* SYM-STELLA-TO-CL-STELLA-CAST (LIST* SYM-STELLA-TO-CL-STELLA-VERBATIM KWD-STELLA-TO-CL-COMMON-LISP CODE NIL) TYPE NIL))))
...
(CL:DEFUN EMACS-REGION-TO-STATEMENTS (REGION)
  ...
  (CL:LET* ((START 0) (NEWSTART 0) (LISPSTATEMENT NULL) (STELLASTATEMENT NULL) (EOFVALUE *MODULE*) (RESULT NIL)) (CL:DECLARE (CL:TYPE CL:FIXNUM START NEWSTART))
   (CL:LOOP (CL:multiple-value-setq (lispStatement newStart)
           (CL:read-from-string region CL:NIL eofValue :start start)) (CL:WHEN (CL:EQ LISPSTATEMENT EOFVALUE) (CL:RETURN))
    (CL:HANDLER-CASE (CL:SETQ STELLASTATEMENT (READ-S-EXPRESSION-FROM-STRING (SUBSEQUENCE REGION START NEWSTART))) (READ-EXCEPTION () (CL:SETQ STELLASTATEMENT NULL)))
    (CL:IF
     (CL:AND (CONS? STELLASTATEMENT)
      (CL:OR (DECLARATION-TREE? STELLASTATEMENT) (CL:AND (SYMBOL? (%%VALUE STELLASTATEMENT)) (CL:NOT (CL:EQ (LOOKUP-COMMAND (%%VALUE STELLASTATEMENT)) NULL)))))
     (CL:SETQ RESULT (CONS (CONS-LIST KWD-STELLA-TO-CL-STELLA STELLASTATEMENT) RESULT)) (CL:SETQ RESULT (CONS (CONS-LIST KWD-STELLA-TO-CL-LISP LISPSTATEMENT) RESULT)))
...
(CL:DEFUN TRANSLATE-EMACS-REGION-IN-MODULE (MODULENAME REGION)
  ...
  (CL:LET* ((RESULT CL:NIL))
   (CL:LET* ((*MODULE* (GET-STELLA-MODULE MODULENAME CL:T)) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:LET* ((STATEMENT NULL) (ITER-000 (EMACS-REGION-TO-STATEMENTS REGION)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ STATEMENT (%%VALUE ITER-000))
      (CL:COND ((CL:EQ (%%VALUE STATEMENT) KWD-STELLA-TO-CL-STELLA) (CL:PUSH (CONS-TREE-TO-LISP-CODE (INCREMENTALLY-TRANSLATE (%%VALUE (%%REST STATEMENT)))) RESULT))
...
(CL:DEFMACRO LISP-MACROEXPAND-EMACS-REGION-IN-MODULE (EXPANDER MODULENAME REGION)
  (CL:LET*
   ((TRANSLATEDSTATEMENTS (TRANSLATE-EMACS-REGION-IN-MODULE MODULENAME REGION))
    (EXPANSIONS (CL:LOOP FOR STATEMENT IN TRANSLATEDSTATEMENTS COLLECT (CL:FUNCALL EXPANDER STATEMENT))))
   (CL:IF (CL:= (CL:LENGTH EXPANSIONS) 1) (CL:FIRST EXPANSIONS) (CL:CONS (CL:QUOTE CL:PROGN) EXPANSIONS))))
...
Unsupported types are left unchanged."
  (CL:IF (CL:EQ (CL:OR (CL:TYPEP THING (CL:QUOTE OBJECT)) (CL:AND (CL:LISTP NIL) (CL:LISTP THING))) CL-NIL)
   (CL:IF (DEFINED? THING) (CL:RETURN-FROM LISPIFY THING) (CL:RETURN-FROM LISPIFY CL-NIL))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE THING)))
    (CL:COND
     ((CL:EQ TEST-VALUE-000 SGT-STELLA-TO-CL-STELLA-CONS)
      (CL:PROGN (CL:WHEN (CL:EQ THING NIL) (CL:RETURN-FROM LISPIFY (CL:IDENTITY CL-NIL)))
       (CL:LET* ((FIRST (LISPIFY (%%VALUE THING))) (REST (LISPIFY (%%REST THING)))) (CL:RETURN-FROM LISPIFY (CL:CONS FIRST REST)))))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-STELLA-TO-CL-STELLA-LIST) (CL:PROGN (CL:RETURN-FROM LISPIFY (LISPIFY (%THE-CONS-LIST THING)))))
       ...
       (CL:LET* ((RESULT CL-NIL))
        (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-KV-LIST THING)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
          (CL:PUSH (CL:CONS (LISPIFY KEY) (LISPIFY VALUE)) RESULT) (CL:SETQ ITER-000 (%REST ITER-000))))
        (CL:RETURN-FROM LISPIFY (CL:NREVERSE RESULT)))))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-STELLA-TO-CL-STELLA-ITERATOR) (CL:PROGN (CL:RETURN-FROM LISPIFY (LISPIFY (LISTIFY THING)))))
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (CONS-TREE-TO-LISP-CODE (CL-TRANSLATE-GLOBAL-SYMBOL THING)))))
     ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (CONS-TREE-TO-LISP-CODE THING))))
     ((SUBTYPE-OF-BOOLEAN? TEST-VALUE-000) (CL:PROGN (CL:LET* ((TRUTHVALUE (%WRAPPER-VALUE THING))) (CL:RETURN-FROM LISPIFY TRUTHVALUE))))
     ((SUBTYPE-OF-VERBATIM-STRING? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (CONS-TREE-TO-LISP-CODE THING))))
     ((SUBTYPE-OF-INTEGER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING))))
     ((SUBTYPE-OF-LONG-INTEGER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING))))
     ((SUBTYPE-OF-FLOAT? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING))))
     ((SUBTYPE-OF-STRING? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING))))
     ((SUBTYPE-OF-CHARACTER? TEST-VALUE-000) (CL:PROGN (CL:RETURN-FROM LISPIFY (%WRAPPER-VALUE THING)))) (CL:T (CL:RETURN-FROM LISPIFY THING))))))
...
Stella boolean."
  (CL:IF (CL:NOT (CL:typep thing 'OBJECT)) (CL:RETURN-FROM LISPIFY-BOOLEAN THING) (CL:RETURN-FROM LISPIFY-BOOLEAN (LISPIFY THING))))
...
and makes somewhat different translations."
  (CL:COND
   ((CL:OR (CL:EQ TARGETTYPE SGT-STELLA-TO-CL-STELLA-BOOLEAN) (CL:EQ TARGETTYPE SGT-STELLA-TO-CL-STELLA-WRAPPED-BOOLEAN))
    (CL:RETURN-FROM STELLAFY (CL:IF (CL:NOT (CL:EQ THING CL-NIL)) TRUE-WRAPPER FALSE-WRAPPER)))
   ((CL:EQ TARGETTYPE SGT-STELLA-TO-CL-STELLA-CONS) (CL:RETURN-FROM STELLAFY (STELLA-CODE-TO-CONS-TREE THING)))
   (CL:T (CL:IF (CL:EQ THING CL-NIL) (CL:RETURN-FROM STELLAFY NULL) (CL:RETURN-FROM STELLAFY (STELLA-CODE-TO-CONS-TREE THING))))))
...
In a call directly from Lisp 'slotName' can also be supplied as a Lisp symbol."
  ...
  (CL:LET* ((OBJECTTYPE (PRIMARY-TYPE OBJECT)) (SLOT (LOOKUP-SLOT (TYPE-TO-CLASS OBJECTTYPE) (INTERN-DERIVED-SYMBOL OBJECTTYPE SLOTNAME))) (VALUE NULL))
   (CL:WHEN (CL:EQ SLOT NULL) (CL:WARN "slot-value: Slot `~A' does not exist on object `~A'." SLOTNAME OBJECT) (CL:RETURN-FROM CL-SLOT-VALUE NULL))
   (CL:COND
    ((SUBTYPE-OF-STORAGE-SLOT? (SAFE-PRIMARY-TYPE SLOT))
     (CL:PROGN (CL:SETQ VALUE (READ-SLOT-VALUE OBJECT SLOT))
      (CL:IF (CL:EQ DONTCONVERT? CL:T) (CL:RETURN-FROM CL-SLOT-VALUE VALUE)
       (CL:IF (CL:EQ (TYPE SLOT) SGT-STELLA-TO-CL-STELLA-BOOLEAN) (CL:RETURN-FROM CL-SLOT-VALUE (LISPIFY-BOOLEAN VALUE))
        (CL:RETURN-FROM CL-SLOT-VALUE (LISPIFY VALUE))))))
    (CL:T (CL:WARN "slot-value: Cannot read method slots.") (CL:RETURN-FROM CL-SLOT-VALUE NULL)))))
...
directly from Lisp 'slotName' can also be supplied as a Lisp symbol."
  ...
  (CL:CHECK-TYPE SLOTNAME CL:SIMPLE-STRING)
  (SETQ SLOTNAME (CL:STRING SLOTNAME))
  (CL:LET* ((OBJECTTYPE (PRIMARY-TYPE OBJECT)) (SLOT (LOOKUP-SLOT (TYPE-TO-CLASS OBJECTTYPE) (INTERN-DERIVED-SYMBOL OBJECTTYPE SLOTNAME))) (STELLAVALUE VALUE))
   (CL:WHEN (CL:EQ SLOT NULL) (CL:WARN "slot-value-setter: Slot `~A' does not exist on object `~A'." SLOTNAME OBJECT) (CL:RETURN-FROM CL-SLOT-VALUE-SETTER NULL))
   (CL:WHEN (CL:NOT (CL:EQ DONTCONVERT? CL:T)) (CL:SETQ STELLAVALUE (STELLAFY VALUE (TYPE SLOT))))
   (CL:WHEN
    (CL:NOT (CL:AND (CL:NOT (CL:EQ (CL:TYPEP STELLAVALUE (CL:QUOTE OBJECT)) CL-NIL)) (SUBTYPE-OF? (PRIMARY-TYPE STELLAVALUE) (TYPE-TO-WRAPPED-TYPE (TYPE SLOT)))))
    (CL:WARN "slot-value-setter: Illegal value type for slot `~A'~% on object `~A'.  Type `~A' expected." SLOTNAME OBJECT (TYPE SLOT))
    (CL:RETURN-FROM CL-SLOT-VALUE-SETTER NULL))
   (CL:COND ((SUBTYPE-OF-STORAGE-SLOT? (SAFE-PRIMARY-TYPE SLOT)) (CL:PROGN (WRITE-SLOT-VALUE OBJECT SLOT STELLAVALUE)))
    (CL:T (CL:WARN "slot-value-setter: Cannot write method slots.") (CL:RETURN-FROM CL-SLOT-VALUE-SETTER NULL)))
...
(CL:DEFUN SLOT-VALUE (OBJECT SLOTNAME CL:&OPTIONAL DONTCONVERT?)
  "See 'cl-slot-value'."
  (CL:IF (CL:OR (CL:NOT DONTCONVERT?) (CL:EQL DONTCONVERT? FALSE)) (CL-SLOT-VALUE OBJECT SLOTNAME FALSE) (CL-SLOT-VALUE OBJECT SLOTNAME TRUE)))

;;; (VERBATIM :COMMON-LISP ...)

(CL:DEFUN SLOT-VALUE-SETTER (OBJECT SLOTNAME VALUE CL:&OPTIONAL DONTCONVERT?)
  "See 'cl-slot-value-setter'."
  (CL:IF (CL:OR (CL:NOT DONTCONVERT?) (CL:EQL DONTCONVERT? FALSE)) (CL-SLOT-VALUE-SETTER OBJECT SLOTNAME VALUE FALSE) (CL-SLOT-VALUE-SETTER OBJECT SLOTNAME VALUE TRUE)))
...
(CL:DEFUN CREATE-NATIVE-CLASS-P (SUPERS OPTIONS)
  (CL:DECLARE (CL:IGNORE SUPERS OPTIONS) (CL:SPECIAL *CREATE-NATIVE-CLASS-ENABLED*))
...
(CL:DEFUN STARTUP-STELLA-TO-CL ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ SGT-STELLA-TO-CL-STELLA-UNKNOWN (INTERN-RIGID-SYMBOL-WRT-MODULE "UNKNOWN" NULL 1))
    (CL:SETQ SYM-STELLA-TO-CL-STELLA-VERBATIM (INTERN-RIGID-SYMBOL-WRT-MODULE "VERBATIM" NULL 0))
    (CL:SETQ KWD-STELLA-TO-CL-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
    (CL:SETQ SYM-STELLA-TO-CL-STELLA-CAST (INTERN-RIGID-SYMBOL-WRT-MODULE "CAST" NULL 0))
    (CL:SETQ KWD-STELLA-TO-CL-STELLA (INTERN-RIGID-SYMBOL-WRT-MODULE "STELLA" NULL 2)) (CL:SETQ KWD-STELLA-TO-CL-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "LISP" NULL 2))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" NULL 1))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-LIST (INTERN-RIGID-SYMBOL-WRT-MODULE "LIST" NULL 1))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-KEY-VALUE-LIST (INTERN-RIGID-SYMBOL-WRT-MODULE "KEY-VALUE-LIST" NULL 1))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-ITERATOR (INTERN-RIGID-SYMBOL-WRT-MODULE "ITERATOR" NULL 1))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-BOOLEAN (INTERN-RIGID-SYMBOL-WRT-MODULE "BOOLEAN" NULL 1))
    (CL:SETQ SGT-STELLA-TO-CL-STELLA-WRAPPED-BOOLEAN (INTERN-RIGID-SYMBOL-WRT-MODULE "WRAPPED-BOOLEAN" NULL 1))
    (CL:SETQ SYM-STELLA-TO-CL-STELLA-STARTUP-STELLA-TO-CL (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-STELLA-TO-CL" NULL 0))
    (CL:SETQ SYM-STELLA-TO-CL-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" NULL 0)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "CL-INCREMENTALLY-TRANSLATE" "(DEFUN (CL-INCREMENTALLY-TRANSLATE LISP-CODE) ((EXPRESSION LISP-CODE)))"
     (CL:FUNCTION CL-INCREMENTALLY-TRANSLATE) NULL)
    (DEFINE-FUNCTION-OBJECT "BI-MODAL-TRANSLATE" "(DEFUN (BI-MODAL-TRANSLATE LISP-CODE) ((STATEMENT LISP-CODE)))" (CL:FUNCTION BI-MODAL-TRANSLATE) NULL)
    (DEFINE-FUNCTION-OBJECT "HELP-BI-MODAL-TRANSLATE" "(DEFUN (HELP-BI-MODAL-TRANSLATE LISP-CODE) ((STATEMENT LISP-CODE)))" (CL:FUNCTION HELP-BI-MODAL-TRANSLATE) NULL)
...
or Verbatim.\" :PUBLIC? TRUE)" (CL:FUNCTION CLV) NULL)
    (DEFINE-FUNCTION-OBJECT "EMACS-REGION-TO-STATEMENTS" "(DEFUN (EMACS-REGION-TO-STATEMENTS (CONS OF CONS)) ((REGION STRING)))" (CL:FUNCTION EMACS-REGION-TO-STATEMENTS)
     NULL)
    (DEFINE-FUNCTION-OBJECT "TRANSLATE-EMACS-REGION-IN-MODULE" "(DEFUN (TRANSLATE-EMACS-REGION-IN-MODULE LISP-CODE) ((MODULENAME STRING) (REGION STRING)))"
     (CL:FUNCTION TRANSLATE-EMACS-REGION-IN-MODULE) NULL)
    (DEFINE-FUNCTION-OBJECT "LISP-EVAL-FORM-IN-MODULE" "(DEFUN (LISP-EVAL-FORM-IN-MODULE LISP-CODE) ((MODULENAME STRING) (FORM LISP-CODE)))"
     (CL:FUNCTION LISP-EVAL-FORM-IN-MODULE) NULL)
    (DEFINE-FUNCTION-OBJECT "LISPIFY" "(DEFUN (LISPIFY LISP-CODE) ((THING UNKNOWN)) :DOCUMENTATION \"Convert a Stella 'thing' as much as possible into a
Common-Lisp analogue.  The currently supported 'thing' types are CONS, LIST,
KEY-VALUE-LIST, ITERATOR, SYMBOL, KEYWORD, and all wrapped and unwrapped
literal types.  BOOLEANs are translated into Lisp's CL:T and CL:NIL logic.
Unsupported types are left unchanged.\" :PUBLIC? TRUE)" (CL:FUNCTION LISPIFY) NULL)
...
'thing' into a Stella analogue of type 'targetType'.
Note: See also 'stellify'. it is similar, but guesses 'targetType' on its own,
and makes somewhat different translations.\" :PUBLIC? TRUE)" (CL:FUNCTION STELLAFY) NULL)
...
slot value (see 'lispify').  If 'dontConvert?' is TRUE, the returned slot value
will not be lispified.  Generate a warning if no such slot exists on 'object'.
In a call directly from Lisp 'slotName' can also be supplied as a Lisp symbol.\" :PUBLIC? TRUE)" (CL:FUNCTION CL-SLOT-VALUE) NULL)
...
will not be stellafied before it gets assigned.  Generate a warning if no
such slot exists on 'object', or if 'value' has the wrong type.  In a call
directly from Lisp 'slotName' can also be supplied as a Lisp symbol.\" :PUBLIC? TRUE)" (CL:FUNCTION CL-SLOT-VALUE-SETTER) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-STELLA-TO-CL" "(DEFUN STARTUP-STELLA-TO-CL () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-STELLA-TO-CL) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-STELLA-TO-CL-STELLA-STARTUP-STELLA-TO-CL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-STELLA-TO-CL-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupStellaToCl") NULL-STRING-WRAPPER)))
