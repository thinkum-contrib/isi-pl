...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* EOL TRUE-WRAPPER FALSE-WRAPPER CL-NIL *MODULE* NIL
  TRUE))
...
(CL:DEFUN TRANSLATE-EMACS-REGION-IN-MODULE (MODULENAME REGION)
  ...
  (CL:LET* ((RESULT CL:NIL))
   (CL:LET* ((*MODULE* (GET-STELLA-MODULE MODULENAME TRUE)) (*CONTEXT* *MODULE*))
   ...
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE THING)))
    (CL:COND
     ((CL:EQ TEST-VALUE-000 SGT-STELLA-TO-CL-STELLA-CONS)
      (CL:PROGN
       (CL:WHEN (CL:EQ THING NIL) (CL:RETURN-FROM LISPIFY (CL:IDENTITY CL-NIL)))
       (CL:LET*
   ...
   (CL:COND
    ((SUBTYPE-OF-STORAGE-SLOT? (SAFE-PRIMARY-TYPE SLOT))
     (CL:PROGN (CL:SETQ VALUE (READ-SLOT-VALUE OBJECT SLOT))
      (CL:IF (CL:EQ DONTCONVERT? TRUE) (CL:RETURN-FROM CL-SLOT-VALUE VALUE)
...
directly from Lisp 'slotName' can also be supplied as a Lisp symbol.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-STELLA-TO-CL-STELLA-STARTUP-STELLA-TO-CL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-STELLA-TO-CL-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Stella-To-Cl") NULL-STRING-WRAPPER)))
