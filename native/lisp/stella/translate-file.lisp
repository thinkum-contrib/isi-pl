...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *SAFETY* *GENSYM-COUNTER* TRUE-WRAPPER
  FALSE-WRAPPER LOGICAL-HOST-SEPARATOR NULL-INTEGER
  ...
  *TRANSLATIONVERBOSITYLEVEL* STANDARD-OUTPUT *MODULE* STANDARD-ERROR EOL
  STANDARD-WARNING))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE
      ...
      *TRANSLATIONNOTES* *CURRENTFILE* *MODULE* *CONTEXT*))
    (CL:DECLARE
...
directory and logical host components."
  (CL:LET* ((START 0) (SEPARATOR (DIRECTORY-SEPARATOR-FOR-FILE FILE)))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:= (POSITION FILE SEPARATOR START) NULL-INTEGER))
    DO (CL:SETQ START (CL:1+ (POSITION FILE SEPARATOR START))))
...
return the result."
  (CL:LET* ((START 0) (END NULL-INTEGER))
   ...
   (CL:LOOP WHILE
    (CL:NOT (CL:= (POSITION FILE TYPE-SEPARATOR START) NULL-INTEGER)) DO
...
(CL:DEFUN GET-LOGICAL-HOST-NAME (PATHNAME)
  ...
  (CL:LET* ((SEPARATORPOS (POSITION PATHNAME LOGICAL-HOST-SEPARATOR 0)))
   ...
   (CL:IF
    (CL:AND (CL:NOT (CL:= SEPARATORPOS NULL-INTEGER)) (CL:>= SEPARATORPOS 1))
   ...
   (CL:IF (CL:NOT (CL:EQ *CURRENTSYSTEMDEFINITION* NULL))
    (CL:PROGN
     (CL:LET* ((FOUND?-000 CL:NIL))
      (CL:LET*
       ((FILE NULL)
        (ITER-000 (SYSTEM-DEFINITION-SOURCE-FILES *CURRENTSYSTEMDEFINITION*)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET* ((STARTUP-FILE (SYSTEM-STARTUP-FILE-NAME NULL)))
      ...
      (CL:RETURN-FROM SYSTEM-NEEDS-TRANSLATION?
       (STELLA-NEED-TO-TRANSLATE? (WRAP-STRING STARTUP-FILE) LANGUAGE))))
...
`:production-settings?' (default true): inverse to `:development-settings?'."
  ...
  (CL:LET*
   ...
   (CL:SETQ LANGUAGE
    (LOOKUP-WITH-DEFAULT PLIST KWD-TRANSLATE-FILE-LANGUAGE LANGUAGE))
   (INSERT-AT PLIST KWD-TRANSLATE-FILE-LANGUAGE LANGUAGE)
   (INSERT-AT PLIST KWD-TRANSLATE-FILE-ACTION
    (LOOKUP-WITH-DEFAULT PLIST KWD-TRANSLATE-FILE-ACTION
   ...
   (CL:LET*
    ...
    (CL:DECLARE
      ...
      *RECYCLING-ENABLED?* *CURRENT-STELLA-FEATURES* *CURRENT-SYSTEM-ACTION*))
    (CL:DECLARE (CL:TYPE CL:FIXNUM *SAFETY*))
    (CL:WHEN (CL:NOT (SYSTEM-LOADED-OR-STARTED-UP? SYSTEMNAME))
...
(CL:DEFUN TRANSLATE-SYSTEM-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:RETURN-FROM TRANSLATE-SYSTEM-EVALUATOR-WRAPPER
   (CL:IF
    (%TRANSLATE-SYSTEM (%WRAPPER-VALUE (%%VALUE ARGUMENTS))
     (%%REST ARGUMENTS))
  ...
  (CL:LET* ((NATIVE-FILES NULL))
   (CL:COND
    ...
    ((CL:EQ OUTPUT-LANGUAGE KWD-TRANSLATE-FILE-CPP)
     (CL:SETQ NATIVE-FILES (%CPP-ONLY-FILES *CURRENTSYSTEMDEFINITION*)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" OUTPUT-LANGUAGE
       "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET*
       ...
       (CL:SETQ FROMFILENAME
        (CONCATENATE (ROOT-SOURCE-DIRECTORY) SYSTEM-SUB-DIRECTORY FILENAME))
...
(CL:DEFUN STARTUP-TRANSLATE-FILE ()
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *SYSTEMDEFINITIONSDIRECTORY* STELLA::NULL-STRING)
