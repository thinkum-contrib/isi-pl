...
(CL:DECLAIM
  ...
  *TRANSLATIONVERBOSITYLEVEL* STANDARD-OUTPUT *MODULE* TRUE
  STANDARD-WARNING EOL FALSE))
...
(CL:DEFUN SYSTEM-DEFINITIONS-DIRECTORY ()
  (CL:IF
   (CL:NOT (CL:EQ *SYSTEMDEFINITIONSDIRECTORY* STELLA::NULL-STRING))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:COND
     ...
     (CL:T))
    (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Processing `"
      FILE "':" EOL)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ ROOTDIRECTORY NULL))
     ...
     (CL:SETQ TRANSLATEDPATHNAME
      (CONCATENATE (%WRAPPER-VALUE ROOTDIRECTORY)
       TRANSLATEDPATHNAME))))
   (CL:RETURN-FROM TRANSLATE-LOGICAL-PATHNAME TRANSLATEDPATHNAME)))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING)
...
`defaults' if supplied and return the result."
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ BASEHOST STELLA::NULL-STRING))
    (%%PRINT-STREAM (%NATIVE-STREAM RESULT) BASEHOST
     LOGICAL-HOST-SEPARATOR))
  ...
  (CL:LET* ((EXTENSION (FILE-EXTENSION FILE)))
   ...
   (CL:COND
    ...
    ((CL:OR (STRING-EQL? EXTENSION ".lisp")
      (STRING-EQL? EXTENSION ".java") (STRING-EQL? EXTENSION ".cc")
...
(CL:DEFUN MAKE-FILE-NAME (FILEPATH TYPE RELATIVE?)
  ...
  (CL:LET*
   ...
   (CL:RETURN-FROM MAKE-FILE-NAME
    (CONCATENATE ROOTPATH SPLITTERPATH SUBDIRECTORY FILEPATH
     EXTENSION))))
       ...
       (CL:LET*
        ...
        (CL:LOOP WHILE
         ...
         (CL:PROGN (CL:SETQ NAME (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:PROGN (CL:SETQ N ITER-001)
          (CL:SETQ ITER-001 (CL:1+ ITER-001)))
         (CL:IF (CL:= N PATH-LENGTH)
          (CL:SETQ FILE
           (CONCATENATE FILE
            (HELP-IMPLODE-PATHNAME (%WRAPPER-VALUE NAME))))
          (CL:SETQ FILE
   ...
   (CL:LET*
    ...
    (CL:IF (CL:OR (CL:EQ FILE1-DATE NULL) (CL:EQ FILE2-DATE NULL))
     ...
     (CL:RETURN-FROM FILE-YOUNGER-THAN?
      (CL:IF (TIME-GREATER? FILE1-DATE FILE2-DATE) TRUE-WRAPPER
       FALSE-WRAPPER))))))
...
(CL:DEFUN STELLA-NEED-TO-COMPILE? (FILE)
  (CL:LET*
   ((SOURCEFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-TRANSLATE-FILE-LISP
      TRUE))
    (COMPILEDFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE)
      KWD-TRANSLATE-FILE-LISP-BINARY TRUE)))
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       ...
       (CL:TAGBODY
        (CL:WHEN
         ...
         (CL:SETQ FILENAME
          (MAKE-FILE-NAME (%WRAPPER-VALUE FILE)
           KWD-TRANSLATE-FILE-STELLA TRUE))
...
(CL:DEFUN TRANSFER-NATIVE-SYSTEM-FILES (OUTPUTLANGUAGE)
  (CL:LET*
   ((NATIVEFILES NULL)
    ...
    (FLOTSAMFILENAME ""))
   (CL:DECLARE
    (CL:TYPE CL:SIMPLE-STRING SYSTEMSUBDIRECTORY FLOTSAMFILENAME))
   (CL:COND
    ((CL:EQ OUTPUTLANGUAGE KWD-TRANSLATE-FILE-COMMON-LISP)
     (CL:SETQ NATIVEFILES
      (%LISP-ONLY-FILES *CURRENTSYSTEMDEFINITION*)))
    ((CL:EQ OUTPUTLANGUAGE KWD-TRANSLATE-FILE-JAVA)
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET*
       ...
       (CL:SETQ FROMFILENAME
        (CONCATENATE (ROOT-SOURCE-DIRECTORY) SYSTEMSUBDIRECTORY
         FILENAME))
...
(CL:DEFUN STARTUP-TRANSLATE-FILE ()
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *RECORD-SIGNATURES?* FALSE)
    (CL:SETQ *WALK-WITH-RECORDED-SIGNATURES?* FALSE)
    (CL:SETQ *USERECORDEDSIGNATURES?* FALSE)
