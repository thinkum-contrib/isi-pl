...
(CL:DEFVAR SGT-TRANSLATE-FILE-STELLA-TOKEN-ITERATOR NULL)
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-THE-STRING NULL)
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-DELIMITERS NULL)
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-CURSOR NULL)
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-END NULL)
...
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-STARTUP-TIME-PROGN NULL)
(CL:DEFVAR SYM-TRANSLATE-FILE-STELLA-VERBATIM NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *SAFETY* *GENSYM-COUNTER* TRUE-WRAPPER
  FALSE-WRAPPER NULL-CHARACTER LOGICAL-DIRECTORY-SEPARATOR
  *CURRENT-ENVIRONMENT-FEATURES* *MODULE* *CONTEXT* *TRANSLATIONUNITS*
  *TRANSLATIONPHASE* *TRANSLATOROUTPUTLANGUAGE*
  *TRANSLATIONVERBOSITYLEVEL* STANDARD-OUTPUT STANDARD-WARNING EOL))
...
(CL:PROGN (CL:DEFVAR *ROOTSOURCEDIRECTORY* "PL:sources;") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *ROOTSOURCEDIRECTORY*)))

;;; (DEFSPECIAL *ROOTNATIVEDIRECTORY* ...)

(CL:PROGN (CL:DEFVAR *ROOTNATIVEDIRECTORY* "PL:natives;") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *ROOTNATIVEDIRECTORY*)))

;;; (DEFSPECIAL *ROOTBINARYDIRECTORY* ...)

(CL:PROGN (CL:DEFVAR *ROOTBINARYDIRECTORY* "PL:binaries;") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *ROOTBINARYDIRECTORY*)))

;;; (DEFSPECIAL *SYSTEMDEFINITIONSDIRECTORY* ...)

(CL:PROGN (CL:DEFVAR *SYSTEMDEFINITIONSDIRECTORY* STELLA::NULL-STRING "Points to the directory containing the SYSTEMS file.") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *SYSTEMDEFINITIONSDIRECTORY*)))
...
(CL:PROGN (CL:DEFVAR *CURRENTSYSTEMDEFINITIONSUBDIRECTORY* "" "Points to a path from the root directory down
to a local directory containing sources, natives, or binaries.") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CURRENTSYSTEMDEFINITIONSUBDIRECTORY*)))
  ...
  (CL:LET* ((OPERATOR (%%VALUE TREE)) (FIRSTARG NULL))
   (CL:COND
    ((SUBTYPE-OF-SYMBOL? (SAFE-PRIMARY-TYPE OPERATOR))
     (CL:PROGN
      (CL:LET* ((TEST-VALUE-000 (%SYMBOL-NAME OPERATOR)))
          ...
          (CL:LET* ((*PRINTREADABLY?* TRUE))
           ...
           (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
            ...
            (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
             ...
             EOL ".")))
          (CHANGE-MODULE "STELLA") (CL:SETQ SEENINMODULE? TRUE)))))))
    (CL:T))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:DECLARE
      ...
      *TRANSLATIONWARNINGS* *CURRENTFILE* *MODULE* *CONTEXT*))
    (CL:DECLARE
  ...
  (CL:COND
   ...
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     ...
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
   (%BANNER *CURRENTSYSTEMDEFINITION*))
  (CL:COND
   ((CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-COMMON-LISP)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "|#"))
   ((CL:OR (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-JAVA)
...
(CL:DEFUN OUTPUT-FILE-HEADER (STREAM FILE)
  (CL:COND
   ((CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-COMMON-LISP)
    (CL-OUTPUT-FILE-HEADER STREAM FILE))
   ((CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-JAVA)
    (JAVA-OUTPUT-FILE-HEADER STREAM FILE))
   ((CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-CPP)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "// " FILE EOL EOL)
...
(CL:DEFUN OPERATING-SYSTEM ()
  (CL:COND
   ...
   ((CL:OR (PROBE-FILE? "/usr") (PROBE-FILE? "/etc"))
...
(CL:DEFUN DIRECTORY-SEPARATOR ()
  (CL:LET* ((SEPARATOR #\?))
   (CL:WHEN (CL:NOT (CL:EQ *OS* NULL))
    (CL:COND
     ((CL:EQ *OS* KWD-TRANSLATE-FILE-UNIX) (CL:SETQ SEPARATOR #\/))
     ((CL:EQ *OS* KWD-TRANSLATE-FILE-MAC) (CL:SETQ SEPARATOR #\:))
     ((CL:EQ *OS* KWD-TRANSLATE-FILE-WINDOWS) (CL:SETQ SEPARATOR #\\))
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" *OS*
...
(CL:DEFUN LOGICAL-PATHNAME? (PATHNAME)
  ...
  (CL:RETURN-FROM LOGICAL-PATHNAME?
   (CL:AND (CL:> (CL:THE CL:FIXNUM (CL:LENGTH PATHNAME)) 0)
    (CL:OR
     (CL:AND (DEFINED? (POSITION PATHNAME LOGICAL-HOST-SEPARATOR 1))
      (CL:NOT (DEFINED? (POSITION PATHNAME (DIRECTORY-SEPARATOR) 1))))
     (DEFINED? (POSITION PATHNAME LOGICAL-DIRECTORY-SEPARATOR 0))))))
...
(CL:DEFUN TRANSLATE-LOGICAL-PATHNAME (PATHNAME)
  ...
  (CL:LET* ((TRANSLATEDPATHNAME PATHNAME))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TRANSLATEDPATHNAME))
...
(CL:DEFCLASS TOKEN-ITERATOR (ABSTRACT-ITERATOR)
  ((THE-STRING :ALLOCATION :INSTANCE :ACCESSOR %THE-STRING)
   (DELIMITERS :ALLOCATION :INSTANCE :ACCESSOR %DELIMITERS)
...
(CL:DEFUN NEW-TOKEN-ITERATOR ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%DELIMITERS SELF) STELLA::NULL-STRING)
...
(CL:DEFMETHOD PRIMARY-TYPE ((SELF TOKEN-ITERATOR))
  (CL:RETURN-FROM PRIMARY-TYPE SGT-TRANSLATE-FILE-STELLA-TOKEN-ITERATOR))

(CL:DEFUN ACCESS-TOKEN-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-TRANSLATE-FILE-STELLA-THE-STRING)
    (CL:IF SETVALUE? (CL:SETF (%THE-STRING SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%THE-STRING SELF)))))
   ((CL:EQ SLOTNAME SYM-TRANSLATE-FILE-STELLA-DELIMITERS)
    (CL:IF SETVALUE? (CL:SETF (%DELIMITERS SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%DELIMITERS SELF)))))
   ((CL:EQ SLOTNAME SYM-TRANSLATE-FILE-STELLA-CURSOR)
    (CL:IF SETVALUE? (CL:SETF (%CURSOR SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-INTEGER (%CURSOR SELF)))))
   ((CL:EQ SLOTNAME SYM-TRANSLATE-FILE-STELLA-END)
...
(CL:DEFUN TOKENIZE (STRING DELIMITERS)
  (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING STRING DELIMITERS))
  (CL:LET* ((SELF-000 (NEW-TOKEN-ITERATOR)))
   (CL:SETF (%THE-STRING SELF-000) STRING)
   (CL:LET* ((ITERATOR SELF-000))
    (CL:SETF (%DELIMITERS ITERATOR) DELIMITERS)
    (CL:SETF (%CURSOR ITERATOR) 0)
    (CL:SETF (%END ITERATOR) (CL:THE CL:FIXNUM (CL:LENGTH STRING)))
...
(CL:DEFMETHOD NEXT? ((SELF TOKEN-ITERATOR))
  (CL:LET*
   ((S (%THE-STRING SELF)) (POS (%CURSOR SELF))
    (CLOSESTDELIMITER (%END SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING S)
    (CL:TYPE CL:FIXNUM POS CLOSESTDELIMITER))
   (CL:WHEN (CL:< POS (%END SELF))
    (CL:LET*
     ((C NULL-CHARACTER) (VECTOR-000 (%DELIMITERS SELF)) (INDEX-000 0)
     ...
     (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
      ...
      (CL:SETQ CLOSESTDELIMITER
       (MIN (POSITION S C (CL:1+ POS)) CLOSESTDELIMITER))))
    (CL:SETF (%VALUE SELF)
     (WRAP-STRING (SUBSEQUENCE S POS CLOSESTDELIMITER)))
    (CL:SETF (%CURSOR SELF) (CL:1+ CLOSESTDELIMITER))
...
(CL:DEFUN PARSE-PATHNAME (PATHNAME)
  ...
  (CL:LET* ((RESULT NIL))
   (CL:LET*
    ((TOKEN NULL) (ITER-000 (TOKENIZE PATHNAME "/")) (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ TOKEN (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS TOKEN NIL))
       (CL:IF (CL:EQ RESULT NIL) (CL:SETQ RESULT COLLECT-000)
        (ADD-CONS-TO-END-OF-CONS-LIST RESULT COLLECT-000)))
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS TOKEN NIL))
...
(CL:DEFUN COMPUTE-COMMON-LISP-FILE-EXTENSION (TYPE)
  ...
  (CL:LET*
   ((SUFFIX STELLA::NULL-STRING) (DUMMYPATHNAME STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SUFFIX DUMMYPATHNAME))
   (CL:COND
   ...
   (cl:let* ((lispType 
         ...
         (cl:setq dummyPathName (CL:namestring dummyPath)))
   (CL:WHEN (CL:EQ TYPE KWD-TRANSLATE-FILE-LISP-BINARY)
    (CL:COND
     ((MEMB? *CURRENT-ENVIRONMENT-FEATURES*
       KWD-TRANSLATE-FILE-USE-COMMON-LISP-STRUCTS)
      (CL:SETQ DUMMYPATHNAME "f.sfasl"))
     ((MEMB? *CURRENT-ENVIRONMENT-FEATURES*
       KWD-TRANSLATE-FILE-USE-COMMON-LISP-VECTOR-STRUCTS)
   ...
   ((CL:EQ TYPE KWD-TRANSLATE-FILE-LISP-BINARY)
    (CL:COND
     ((CL:EQ *OS* KWD-TRANSLATE-FILE-MAC)
      (CL:RETURN-FROM DEFAULT-COMMON-LISP-FILE-EXTENSION ".pfsl"))
     ((CL:EQ *OS* KWD-TRANSLATE-FILE-WINDOWS)
...
(CL:PROGN (CL:DEFVAR *LISP-SPLITTER-PATH* "lisp") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *LISP-SPLITTER-PATH*)))

;;; (DEFGLOBAL *JAVA-SPLITTER-PATH* ...)

(CL:PROGN (CL:DEFVAR *JAVA-SPLITTER-PATH* "java") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *JAVA-SPLITTER-PATH*)))

;;; (DEFGLOBAL *CPP-SPLITTER-PATH* ...)

(CL:PROGN (CL:DEFVAR *CPP-SPLITTER-PATH* "cpp") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CPP-SPLITTER-PATH*)))
...
(CL:DEFUN SELECT-SPLITTER-PATH (TYPE)
  (CL:LET* ((SPLITTERPATH ""))
   ...
   (CL:COND
    ...
    ((CL:OR (CL:EQ TYPE KWD-TRANSLATE-FILE-CPP-HEADER)
...
(CL:DEFUN MAKE-FILE-NAME-FROM-RELATIVE-PATH (FILEPATH TYPE)
  (CL:RETURN-FROM MAKE-FILE-NAME-FROM-RELATIVE-PATH
   (MAKE-FILE-NAME (IMPLODE-PATHNAME FILEPATH) TYPE TRUE)))
...
(CL:DEFUN ROOT-DIRECTORY-PREFIX (FILE)
  (CL:LET* ((EXTENSION (FILE-EXTENSION FILE)))
   ...
   (CL:COND
    ...
    ((CL:OR (STRING-EQL? EXTENSION ".lisp")
      (STRING-EQL? EXTENSION ".java") (STRING-EQL? EXTENSION ".cpp"))
     (CL:RETURN-FROM ROOT-DIRECTORY-PREFIX (ROOT-NATIVE-DIRECTORY)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" EXTENSION
       "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))))
...
(CL:DEFUN MAKE-FILE-NAME (FILEPATH TYPE RELATIVE?)
  ...
  (CL:LET*
   ((EXTENSION (SELECT-FILE-EXTENSION TYPE)) (ROOTPATH "")
    (SPLITTERPATH "") (SUBDIRECTORY ""))
   (CL:DECLARE
    (CL:TYPE CL:SIMPLE-STRING EXTENSION ROOTPATH SPLITTERPATH
     SUBDIRECTORY))
   (CL:COND
    (RELATIVE? (CL:SETQ ROOTPATH (SELECT-ROOT-DIRECTORY TYPE))
     ...
     (CL:COND
      ...
      (CL:T
       (CL:WHEN
        (CL:NOT (STRING-EQL? *CURRENTSYSTEMDEFINITIONSUBDIRECTORY* ""))
        (CL:SETQ SUBDIRECTORY
  ...
  (CL:LET* ((FILE STELLA::NULL-STRING))
   ...
   (CL:COND
    ((CL:EQ (SAFE-PRIMARY-TYPE FILEPATH) SGT-TRANSLATE-FILE-STELLA-CONS)
     (CL:PROGN
      (CL:LET* ((DIRECTORIES NULL) (THEFILE NULL))
       (CL:MULTIPLE-VALUE-SETQ (DIRECTORIES THEFILE)
        (SEPARATE-LAST-CONS FILEPATH))
       (CL:SETQ FILE "")
       (CL:LET* ((DIRECTORY NULL) (ITER-000 DIRECTORIES))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ DIRECTORY (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:SETQ FILE
          (CONCATENATE FILE (%WRAPPER-VALUE DIRECTORY)
           (DIRECTORY-SEPARATOR-STRING)))))
       (CL:SETQ FILE
        (CONCATENATE FILE
         (HELP-IMPLODE-PATHNAME (%WRAPPER-VALUE THEFILE)))))))
    (CL:T
     (CL:SETQ FILE (HELP-IMPLODE-PATHNAME (%WRAPPER-VALUE FILEPATH)))))
   (CL:RETURN-FROM IMPLODE-PATHNAME FILE)))

;;; (DEFSPECIAL *DONTTRUNCATEFILENAMES?* ...)

(CL:DEFVAR *DONTTRUNCATEFILENAMES?* FALSE
...
(CL:DEFUN FILE-YOUNGER-THAN? (FILE1 FILE2)
  ...
  (CL:IF
   (CL:OR (CL:NOT (PROBE-FILE? FILE1)) (CL:NOT (PROBE-FILE? FILE2))
    (CL:= (FILE-WRITE-DATE FILE1) NULL-INTEGER)
    (CL:= (FILE-WRITE-DATE FILE2) NULL-INTEGER))
   (CL:RETURN-FROM FILE-YOUNGER-THAN? NULL)
   (CL:RETURN-FROM FILE-YOUNGER-THAN?
    (CL:IF (CL:< (FILE-WRITE-DATE FILE2) (FILE-WRITE-DATE FILE1))
...
(CL:DEFUN STELLA-NEED-TO-TRANSLATE? (FILE LANGUAGE)
  (CL:WHEN (CL:EQ LANGUAGE KWD-TRANSLATE-FILE-JAVA)
   (CL:RETURN-FROM STELLA-NEED-TO-TRANSLATE? TRUE))
  (CL:LET*
   ((SOURCEFILE
    ...
    (TRANSLATEDFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE)
      (LANGUAGE-TO-FILE-TYPE LANGUAGE) TRUE)))
...
(CL:DEFUN STELLA-NEED-TO-COMPILE? (FILE)
  (CL:LET*
   ((SOURCEFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-TRANSLATE-FILE-LISP TRUE))
    (COMPILEDFILE
...
(CL:DEFUN SYSTEM-NEEDS-TRANSLATION? (SYSTEMNAME LANGUAGE)
  ...
  (CL:RETURN-FROM SYSTEM-NEEDS-TRANSLATION? FALSE))
      ...
      (CL:COND
       ((CL:OR
         (CL:EQ TEST-VALUE-000
          SYM-TRANSLATE-FILE-STELLA-STARTUP-TIME-PROGN)
         (CL:EQ TEST-VALUE-000 SYM-TRANSLATE-FILE-STELLA-VERBATIM)))
       (CL:T
...
(CL:DEFUN TRANSLATE-SYSTEM (SYSTEMNAME OUTPUTLANGUAGE CL:&REST OPTIONS)
  "Translate all of the STELLA source files in 'system' into a target language.
If :two-pass? is true, all files will be scanned twice, once to define
   all objects defined in them, and once to actually translate them.
If :force-translation?, files will be translated whether or not they are up-to-date.
If :production-settings?, optimization levels will favor efficient code.
Return TRUE if at least one file was translated."
  ...
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   ...
   (CL:LET*
    ((PLIST SELF-000) (TWOPASS? FALSE) (FORCETRANSLATION? FALSE)
    ...
    (CL:LET*
     ...
     (DISABLE-ENVIRONMENT-FEATURE
      KWD-TRANSLATE-FILE-USE-HARDCODED-SYMBOLS)
     (%SET-TRANSLATOR-OUTPUT-LANGUAGE OUTPUTLANGUAGE)
     (CL:LET*
      ((*CURRENTSYSTEMDEFINITION* (GET-SYSTEM-DEFINITION SYSTEMNAME))
       (*CURRENTSYSTEMDEFINITIONSUBDIRECTORY*
      ...
      (CL:IF (CL:NOT (CL:EQ *CURRENTSYSTEMDEFINITION* NULL))
       ...
       (CL:WARN "Can't find a system named `~A'~%" SYSTEMNAME)))
     (CL:RETURN-FROM TRANSLATE-SYSTEM FALSE)))))
  ...
  (CL:LET* ((*TRANSLATIONUNITS* (NEW-LIST)))
   ...
   (CL:LET*
    ((OUTPUTLANGUAGE *TRANSLATOROUTPUTLANGUAGE*)
     (FILENAME STELLA::NULL-STRING) (TRANSLATEDFILE? FALSE)
        ...
        ((FILENAME (%WRAPPER-VALUE F))
         ...
         (BAREFILENAME
          (RELATIVIZE-FILE-NAME
           (FILE-NAME-WITHOUT-EXTENSION (%WRAPPER-VALUE F))
           (ROOT-SOURCE-DIRECTORY)))
         (SUBDIRECTORY
          (CONCATENATE *CURRENTSYSTEMDEFINITIONSUBDIRECTORY*
           (DIRECTORY-SEPARATOR-STRING)))
         (TYPE NULL) (FROMFILENAME STELLA::NULL-STRING)
         (TOFILENAME STELLA::NULL-STRING))
        (CL:DECLARE
         (CL:TYPE CL:SIMPLE-STRING FILENAME EXTENSION SUBDIRECTORY))
        (CL:WHEN
         (CL:AND (CL:NOT (CL:EQ EXTENSION STELLA::NULL-STRING))
          (CL:NOT (STRING-EQL? EXTENSION ".ste")))
         (CL:COND
          ((STRING-EQL? EXTENSION ".lisp")
           (CL:SETQ TYPE KWD-TRANSLATE-FILE-LISP))
          ((STRING-EQL? EXTENSION ".java")
           (CL:SETQ TYPE KWD-TRANSLATE-FILE-JAVA))
          ((STRING-EQL? EXTENSION ".hh")
           (CL:SETQ TYPE KWD-TRANSLATE-FILE-CPP-HEADER))
          ((STRING-EQL? EXTENSION ".cc")
           (CL:SETQ TYPE KWD-TRANSLATE-FILE-CPP-CODE))
          (CL:T
           (CL:WARN "Unrecognized file extension on file: `~A'~%"
            FILENAME)
           (CL:GO :CONTINUE)))
         (CL:SETQ FROMFILENAME
          (CONCATENATE (ROOT-SOURCE-DIRECTORY) SUBDIRECTORY FILENAME))
         (CL:SETQ TOFILENAME (MAKE-FILE-NAME BAREFILENAME TYPE TRUE))
         (CL:WHEN
          (CL:NOT
...
(CL:DEFUN HELP-STARTUP-TRANSLATE-FILE1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-TRANSLATE-FILE-STELLA-TOKEN-ITERATOR
    (INTERN-RIGID-SYMBOL-WRT-MODULE "TOKEN-ITERATOR" NULL 1))
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-THE-STRING
    (INTERN-RIGID-SYMBOL-WRT-MODULE "THE-STRING" NULL 0))
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-DELIMITERS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DELIMITERS" NULL 0))
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-CURSOR
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CURSOR" NULL 0))
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-END
   ...
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-STARTUP-TIME-PROGN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-TIME-PROGN" NULL 0))
   (CL:SETQ SYM-TRANSLATE-FILE-STELLA-VERBATIM
...
directory separator or the empty string if `file' does not include a directory.\" :PUBLIC? TRUE)"
...
directory separator.\" :PUBLIC? TRUE)" (CL:FUNCTION DIRECTORY-FILE-NAME)
    NULL)
...
not have that many parents.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "FILE-NAME-WITHOUT-DIRECTORY"
    "(DEFUN (FILE-NAME-WITHOUT-DIRECTORY FILE-NAME) ((FILE FILE-NAME)) :PUBLIC? TRUE)"
    (CL:FUNCTION FILE-NAME-WITHOUT-DIRECTORY) NULL)
   (DEFINE-FUNCTION-OBJECT "FILE-NAME-WITHOUT-EXTENSION"
    "(DEFUN (FILE-NAME-WITHOUT-EXTENSION FILE-NAME) ((FILE FILE-NAME)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "FILE-EXTENSION"
    ...
    (CL:FUNCTION FILE-EXTENSION) NULL)
   (DEFINE-FUNCTION-OBJECT "FILE-BASE-NAME"
    "(DEFUN (FILE-BASE-NAME FILE-NAME) ((FILE FILE-NAME)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "TOKENIZE"
    "(DEFUN (TOKENIZE TOKEN-ITERATOR) ((STRING STRING) (DELIMITERS STRING)) :PUBLIC? TRUE)"
    (CL:FUNCTION TOKENIZE) NULL)
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD (NEXT? BOOLEAN) ((SELF TOKEN-ITERATOR)) :PUBLIC? TRUE)"
    (CL:FUNCTION NEXT?) NULL)
   (DEFINE-FUNCTION-OBJECT "PARSE-PATHNAME"
    "(DEFUN (PARSE-PATHNAME (CONS OF STRING-WRAPPER)) ((PATHNAME STRING)) :PUBLIC? TRUE)"
    (CL:FUNCTION PARSE-PATHNAME) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "SEPARATE-LAST-CONS"
    "(DEFUN (SEPARATE-LAST-CONS CONS OBJECT) ((CONSLIST CONS)))"
    (CL:FUNCTION SEPARATE-LAST-CONS) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "TRANSLATE-SYSTEM"
    "(DEFUN (TRANSLATE-SYSTEM BOOLEAN) ((SYSTEMNAME STRING) (OUTPUTLANGUAGE KEYWORD) |&REST| (OPTIONS OBJECT)) :DOCUMENTATION \"Translate all of the STELLA source files in 'system' into a target language.
If :two-pass? is true, all files will be scanned twice, once to define
   all objects defined in them, and once to actually translate them.
If :force-translation?, files will be translated whether or not they are up-to-date.
If :production-settings?, optimization levels will favor efficient code.
Return TRUE if at least one file was translated.\")"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Translate-File") NULL-STRING-WRAPPER)))
  :VOID)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *TYPE-TO-FILE-EXTENSION-TABLE*
     (LIST* (LIST* KWD-TRANSLATE-FILE-STELLA (WRAP-STRING ".ste") NIL)
      ...
      (LIST* KWD-TRANSLATE-FILE-IDL (WRAP-STRING ".idl") NIL) NIL)))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5)
    (CL:LET*
     ((CLASS
       (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "TOKEN-ITERATOR"
        "(DEFCLASS TOKEN-ITERATOR (ABSTRACT-ITERATOR) :PUBLIC? TRUE :DOCUMENTATION \"Iterator that yields tokens from an input string\" :PARAMETERS ((ANY-VALUE :TYPE STRING-WRAPPER)) :SLOTS ((THE-STRING :TYPE STRING) (DELIMITERS :TYPE STRING) (CURSOR :TYPE INTEGER) (END :TYPE INTEGER)))")))
...
try to retrieve a previously recorded signature if normal lookup fails.\")")
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFSPECIAL *ROOTNATIVEDIRECTORY* STRING \"PL:natives;\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFSPECIAL *ROOTBINARYDIRECTORY* STRING \"PL:binaries;\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
...
either :UNIX, :WINDOWS, or :MAC.\")")
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *TYPE-TO-FILE-EXTENSION-TABLE* (CONS OF CONS) (BQUOTE ((:STELLA \".ste\") (:KB \".kb\") (:CPP-CODE \".cc\") (:CPP-HEADER \".hh\") (:JAVA \".java\") (:IDL \".idl\"))) :DOCUMENTATION \"Used by 'select-file-extension'.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
