...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *SAFETY* *GENSYM-COUNTER* TRUE-WRAPPER FALSE-WRAPPER
  NULL-INTEGER LOGICAL-HOST-SEPARATOR LOGICAL-DIRECTORY-SEPARATOR
  *CURRENT-STELLA-FEATURES* NIL *CONTEXT* *TRANSLATIONUNITS* *TRANSLATIONPHASE*
  *TRANSLATOROUTPUTLANGUAGE* *TRANSLATIONVERBOSITYLEVEL* STANDARD-OUTPUT *MODULE*
  TRUE STANDARD-WARNING EOL FALSE))
      ...
      (CL:LET*
       ...
       (CL:COND
        ((STRING-EQL? TEST-VALUE-000 "IN-PACKAGE")
         (CL:RETURN-FROM HANDLE-IN-MODULE-TREE (CL:VALUES TRUE SEENINMODULE?)))
...
(CL:DEFUN OPERATE-ON-FILE (FILE OPERATION)
  (CL:LET*
   ((TARGETLANGUAGE (%SYMBOL-NAME *TRANSLATOROUTPUTLANGUAGE*))
    (CURRENTMODULE *MODULE*) (CURRENTCONTEXT *CONTEXT*) (SEENINMODULE? FALSE)
    (SKIP? FALSE))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TARGETLANGUAGE))
   ...
   (CL:LET*
    ((*TRANSLATIONPHASE* NULL) (*TRANSLATIONERRORS* 0) (*TRANSLATIONWARNINGS* 0)
     (*CURRENTFILE* (FILE-BASE-NAME FILE)) (*MODULE* CURRENTMODULE)
   ...
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" *TRANSLATOROUTPUTLANGUAGE*
      "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
  (CL:LET* ((SUBSTITUTION-LIST (NEW-KEY-VALUE-LIST)))
   (FILL-IN-DATE-SUBSTITUTION SUBSTITUTION-LIST)
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
    (SUBSTITUTE-TEMPLATE-VARIABLES-IN-STRING (%BANNER *CURRENTSYSTEMDEFINITION*)
     SUBSTITUTION-LIST)))
  (CL:COND
   ((CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-TRANSLATE-FILE-COMMON-LISP)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "|#"))
...
(CL:DEFUN OUTPUT-FILE-HEADER (STREAM FILE)
  (CL:COND
   ...
   (CL:T (CL-OUTPUT-FILE-HEADER STREAM FILE FALSE)))
  :VOID)
...
(CL:DEFUN DIRECTORY-SEPARATOR ()
  (CL:LET* ((SEPARATOR #\?))
   (CL:WHEN (CL:NOT (CL:EQ *OS* NULL))
    (CL:COND ((CL:EQ *OS* KWD-TRANSLATE-FILE-UNIX) (CL:SETQ SEPARATOR #\/))
...
(CL:DEFUN TRANSLATE-LOGICAL-PATHNAME (PATHNAME)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((TRANSLATIONS
      (LOOKUP-LOGICAL-HOST-PROPERTY HOST KWD-TRANSLATE-FILE-LISP-TRANSLATIONS))
...
`defaults' if supplied and return the result."
  (CL:LET*
   ((BASESEPARATOR (DIRECTORY-SEPARATOR-FOR-FILE BASEFILE))
    ...
    (DEFAULTSSEPARATOR (DIRECTORY-SEPARATOR-FOR-FILE DEFAULTS)) (MERGED? FALSE)
    (RESULT NULL))
...
(CL:DEFUN COMPUTE-COMMON-LISP-FILE-EXTENSION (TYPE)
  ...
  (CL:LET* ((SUFFIX STELLA::NULL-STRING) (DUMMYPATHNAME STELLA::NULL-STRING))
   ...
   (CL:COND
    ...
    (CL:T
     (CL:LET* ((ROOTPREFIX (ROOT-DIRECTORY-PREFIX FILEPATH)))
      ...
      (CL:WHEN
       (CL:NOT
        (MEMBER? (GET-QUOTED-TREE "((:STELLA :KB) \"/STELLA\")" "/STELLA") TYPE))
    ...
    ((CL:EQ (SAFE-PRIMARY-TYPE FILEPATH) SGT-TRANSLATE-FILE-STELLA-CONS)
     (CL:PROGN
      (CL:LET* ((PATH-LENGTH (LENGTH FILEPATH)))
       ...
       (CL:LET*
        ((N NULL-INTEGER) (NAME NULL) (ITER-000 FILEPATH) (ITER-001 1)
         (UPPER-BOUND-000 PATH-LENGTH) (UNBOUNDED?-000 (NULL? UPPER-BOUND-000)))
  ...
  (CL:LET* ((MAXIMUMLENGTH 999)) (CL:DECLARE (CL:TYPE CL:FIXNUM MAXIMUMLENGTH))
   ...
   (CL:WHEN (CL:> (CL:THE CL:FIXNUM (CL:LENGTH BAREFILENAME)) MAXIMUMLENGTH)
    (CL:LET* ((ORIGINALFILENAME BAREFILENAME))
     ...
     (CL:WARN "Truncating name of file.~%   Original: `~A'    Truncated: `~A'~%"
   ...
   (CL:IF ABSOLUTE?
    (CL:IF (STRING-EQL? FROMROOT TOROOT) (CL:RETURN-FROM REROOT-FILE-NAME FILE)
     (CL:RETURN-FROM REROOT-FILE-NAME
      (CONCATENATE TOROOT
       (SUBSEQUENCE FILE (CL:THE CL:FIXNUM (CL:LENGTH FROMROOT)) NULL-INTEGER))))
...
(CL:DEFUN FILE-YOUNGER-THAN? (FILE1 FILE2)
  ...
  (CL:IF (CL:OR (CL:NOT (PROBE-FILE? FILE1)) (CL:NOT (PROBE-FILE? FILE2)))
   ...
   (CL:LET*
    ((FILE1-DATE (FILE-WRITE-DATE FILE1)) (FILE2-DATE (FILE-WRITE-DATE FILE2)))
    (CL:IF (CL:OR (CL:EQ FILE1-DATE NULL) (CL:EQ FILE2-DATE NULL))
     (CL:RETURN-FROM FILE-YOUNGER-THAN? NULL)
     (CL:RETURN-FROM FILE-YOUNGER-THAN?
      (CL:IF (TIME-GREATER? FILE1-DATE FILE2-DATE) TRUE-WRAPPER FALSE-WRAPPER))))))
...
(CL:DEFUN STELLA-NEED-TO-TRANSLATE? (FILE LANGUAGE)
  ...
  (CL:LET*
   ((SOURCEFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-TRANSLATE-FILE-STELLA TRUE))
    (TRANSLATEDFILE
...
(CL:DEFUN STELLA-NEED-TO-COMPILE? (FILE)
  (CL:LET*
   ((SOURCEFILE
    ...
    (COMPILEDFILE
     (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-TRANSLATE-FILE-LISP-BINARY TRUE)))
   (CL:RETURN-FROM STELLA-NEED-TO-COMPILE?
    (CL:NOT (CL:EQ (FILE-YOUNGER-THAN? COMPILEDFILE SOURCEFILE) TRUE-WRAPPER)))))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%THE-OBJECT UNIT) NULL))
     (CL:LET* ((TEST-VALUE-000 (%CATEGORY UNIT)))
      (CL:COND
       ((CL:OR
         (CL:EQ TEST-VALUE-000 SYM-TRANSLATE-FILE-STELLA-STARTUP-TIME-PROGN)
...
(CL:DEFUN HELP-TRANSLATE-SYSTEM (FILES TWOPASS? FORCETRANSLATION?)
  (CL:LET* ((*TRANSLATIONUNITS* (NEW-LIST)))
   ...
   (CL:LET*
    ((OUTPUTLANGUAGE *TRANSLATOROUTPUTLANGUAGE*) (FILENAME STELLA::NULL-STRING)
     (TRANSLATEDFILE? FALSE) (WALKEDUNITS (NEW-KEY-VALUE-LIST)))
    (CLEAR-SYMBOL-REGISTRY)
    (CL:WHEN TWOPASS?
    ...
    (CL:LET* ((F NULL) (ITER-000 NATIVEFILES))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET*
       ...
       (CL:SETQ TOFILENAME (MAKE-FILE-NAME RELATIVEFILENAME TYPE TRUE))
       (CL:WHEN
        (CL:NOT
...
(CL:DEFUN HELP-STARTUP-TRANSLATE-FILE2 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "OPERATING-SYSTEM"
    "(DEFUN (OPERATING-SYSTEM KEYWORD) ())" (CL:FUNCTION OPERATING-SYSTEM) NULL)
...
`CL:translate-logical-pathname' for more complex translation operations.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "LOGICAL-HOST?"
    "(DEFUN (LOGICAL-HOST? BOOLEAN) ((HOST STRING)))" (CL:FUNCTION LOGICAL-HOST?)
    NULL)
...
(CL:DEFUN STARTUP-TRANSLATE-FILE ()
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *RECORD-SIGNATURES?* FALSE)
    (CL:SETQ *WALK-WITH-RECORDED-SIGNATURES?* FALSE)
    (CL:SETQ *USERECORDEDSIGNATURES?* FALSE) (CL:SETQ *OS* (OPERATING-SYSTEM))
    ...
    (CL:SETQ *DONTTRUNCATEFILENAMES?* FALSE))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-TRANSLATE-FILE2)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Translate-File") NULL-STRING-WRAPPER)))
