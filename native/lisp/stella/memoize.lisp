...
(CL:DEFVAR SYM-MEMOIZE-STELLA-MEMOIZE NULL)
(CL:DEFVAR SYM-MEMOIZE-STELLA-MEMOIZE-EXPANDER NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* STANDARD-ERROR EOL
  *METHODBEINGWALKED*))
...
(CL:DEFVAR *MEMOIZATION-ENABLED?* TRUE)
...
(CL:PROGN (CL:DEFVAR *TOTAL-MEMOIZATION-LOOKUPS* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *TOTAL-MEMOIZATION-LOOKUPS*)))

;;; (DEFGLOBAL *FAILED-MEMOIZATION-LOOKUPS* ...)

(CL:PROGN (CL:DEFVAR *FAILED-MEMOIZATION-LOOKUPS* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *FAILED-MEMOIZATION-LOOKUPS*)))
   ...
   (CL:COND
    ((CL:< EQVECTOR 0)
     ...
     (CL:LOOP (CL:WHEN (CL:EQ ARG2 MEMOIZED-NULL-VALUE) (CL:RETURN))
      ...
      (CL:SETQ HASHCODE
       (CL:THE CL:FIXNUM
        (CL:LOGXOR (CL:THE CL:FIXNUM (CL:ASH HASHCODE 1))
         (CL:THE CL:FIXNUM
          (CL:LET ((SELF ARG2))
           (CL:THE CL:FIXNUM (STELLA::%%OBJECT-HASH-CODE SELF)))))))
    ...
    (CL:T (CL:WHEN (CL:EQ ARG1 NULL) (CL:SETQ ARG1 MEMOIZED-NULL-VALUE))
     ...
     (CL:LOOP (CL:WHEN (CL:EQ ARG2 MEMOIZED-NULL-VALUE) (CL:RETURN))
      ...
      (CL:RETURN))))
   (CL:IF (CL:< HASHCODE 0)
    (CL:RETURN-FROM HASH-MEMOIZED-ARGUMENTS (CL:- 2 HASHCODE))
...
(CL:DEFUN HASH-MEMOIZED-ARGUMENTSN (TUPLE EQVECTOR)
  ...
  (CL:LET* ((HASHCODE 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM HASHCODE))
   ...
   (CL:IF (CL:< HASHCODE 0)
    (CL:RETURN-FROM HASH-MEMOIZED-ARGUMENTSN (CL:- 2 HASHCODE))
    ...
    ((CL:< EQVECTOR 0)
     (CL:LOOP WHILE (CL:NOT (CL:EQ ENTRIES NIL)) DO
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((INPUTARG NULL) (ENTRYARG NULL)
         (ITER-000 (%%REST (%%VALUE ENTRIES))) (ITER-001 TUPLE))
        (CL:LOOP WHILE
...
(CL:DEFCLASS MRU-MEMOIZATION-TABLE (MEMOIZATION-TABLE)
  ((MRU-BUCKETS-VECTOR :ALLOCATION :INSTANCE :ACCESSOR
   ...
   (LRU-BUCKETS-VECTOR :ALLOCATION :INSTANCE :ACCESSOR
    %LRU-BUCKETS-VECTOR)
   (MRU-BUCKETS :ALLOCATION :INSTANCE :ACCESSOR %MRU-BUCKETS)
   (LRU-BUCKETS :ALLOCATION :INSTANCE :ACCESSOR %LRU-BUCKETS)
   (MRU-TIMESTAMP :ALLOCATION :INSTANCE :ACCESSOR %MRU-TIMESTAMP)
   (LRU-TIMESTAMP :ALLOCATION :INSTANCE :ACCESSOR %LRU-TIMESTAMP)
   (NOF-BUCKETS :ALLOCATION :INSTANCE :ACCESSOR %NOF-BUCKETS)
   (FREE-ENTRIES :ALLOCATION :INSTANCE :ACCESSOR %FREE-ENTRIES)
   (MAX-ENTRIES :ALLOCATION :INSTANCE :ACCESSOR %MAX-ENTRIES)))
...
(CL:DEFUN INITIALIZE-MRU-BUCKET-TABLES (MEMOTABLE)
  (CL:SETF (%NOF-BUCKETS MEMOTABLE)
   (PICK-HASHTABLE-SIZE-PRIME (%MAX-ENTRIES MEMOTABLE)))
...
(CL:DEFUN LOOKUP-MRU-MEMOIZED-VALUE (MEMOTABLE ARG1 ARG2 ARG3 ARG4 EQVECTOR)
  ...
  (CL:LET*
   ...
   (CL:SETQ BUCKETINDEX
    (CL:THE CL:FIXNUM
     (CL:MOD HASHCODE (CL:THE CL:FIXNUM (%NOF-BUCKETS MEMOTABLE)))))
   ...
   (CL:WHEN
    (CL:=
     (CL:SETF (%FREE-ENTRIES MEMOTABLE) (CL:1- (%FREE-ENTRIES MEMOTABLE)))
     0)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ MRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE MRUBUCKET) MRUTIMESTAMP)
      (CL:SETQ ENTRY
       (FIND-MEMOIZED-VALUE-ENTRYN MRUBUCKET ARGS EQVECTOR FALSE))
      (CL:WHEN (CL:NOT (CL:EQ ENTRY NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ LRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE LRUBUCKET) (%LRU-TIMESTAMP MEMOTABLE))
      (CL:SETQ ENTRY
       (FIND-MEMOIZED-VALUE-ENTRYN LRUBUCKET ARGS EQVECTOR TRUE)))
     (CL:T (CL:SETF (%%REST LRUBUCKET) NIL))))
     ...
     (CL:LET* ((TS NULL) (ITER-001 (%TIMESTAMPS TABLE)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:PROGN (CL:SETQ TS (%%VALUE ITER-001))
        (CL:SETQ ITER-001 (%%REST ITER-001)))
       (CL:WHEN (CL:EQ TS TIMESTAMPNAME) (CL:SETQ FOUND?-000 TRUE)
        (CL:RETURN))))
     (CL:WHEN FOUND?-000
      (CL:COND
...
(CL:DEFUN PARSE-MEMOIZE-OPTIONS (OPTIONS)
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   ...
   (CL:LET* ((GIVENOPTIONS SELF-000) (PARSEDOPTIONS (NEW-PROPERTY-LIST)))
    (CL:LET*
     ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST GIVENOPTIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN MEMOIZE-EXPANDER (INPUTARGS BODY)
...
(wrapped) literals (with the exception of BOOLEANs)."
  (CL:LET*
   ((MEMONAME NULL) (MEMOTABLEVAR (LOCAL-GENSYM "MEMO-TABLE"))
    ...
    (LOOKUPTREE NIL) (ALLEQ? TRUE) (EQVECTOR 0))
   ...
   (CL:CASE NOFINPUTS
    (0
     ...
     (CL:RETURN-FROM MEMOIZE-EXPANDER
    ...
    (CL:OTHERWISE
     (CL:SETQ LOOKUPTREE
      (CONS LOOKUPFUNCTION
       (CONS MEMOTABLEVAR
        (LIST*
         (CONS SYM-MEMOIZE-STELLA-CONS-LIST (CONCATENATE INPUTARGS NIL))
         (CL:IF ALLEQ? (WRAP-INTEGER -1) (WRAP-INTEGER EQVECTOR))
         NIL))))))
   (CL:RETURN-FROM MEMOIZE-EXPANDER
...
(CL:DEFUN HELP-STARTUP-MEMOIZE1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-MEMOIZE-STELLA-MEMOIZE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "MEMOIZE" NULL 0))
   (CL:SETQ SYM-MEMOIZE-STELLA-MEMOIZE-EXPANDER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "MEMOIZE-EXPANDER" NULL 0))
...
(CL:DEFUN STARTUP-MEMOIZE ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:LET* ((SELF-036 (NEW-SYMBOL "MEMOIZED-NULL-VALUE")))
     (CL:SETF (%HOME-CONTEXT SELF-036) NULL)
     (CL:SETQ MEMOIZED-NULL-VALUE SELF-036)))
...
memoized entries when they get bumped.\")))")))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "MEMOIZE-EXPANDER"
     "(DEFUN (MEMOIZE-EXPANDER OBJECT) ((INPUTARGS CONS) |&BODY| (BODY CONS)) :DOCUMENTATION \"Compute the value of an expression and memoize it relative to
   the values of `inputArgs'.
`inputArgs' should characterize the complete set of values upon which
   the computation of the result depended.
Calls to `memoize' should be of the form
...
(wrapped) literals (with the exception of BOOLEANs).\" :PUBLIC? TRUE)"
     (CL:FUNCTION MEMOIZE-EXPANDER) NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Memoize") NULL-STRING-WRAPPER)))
...
cached NULL.\")")
    ...
    (STORE-MACRO SYM-MEMOIZE-STELLA-MEMOIZE
     SYM-MEMOIZE-STELLA-MEMOIZE-EXPANDER (CL:FUNCTION MEMOIZE-EXPANDER))))
  :VOID)
