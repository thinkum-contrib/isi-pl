...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* STANDARD-ERROR EOL *METHODBEINGWALKED*
  NULL-INTEGER *INTEGER-MSB-MASK* TRUE FALSE NIL))
...
Used at the memoization site for quick memo table lookup." :ALLOCATION :INSTANCE
    :ACCESSOR %TABLE-NAME)
   (CURRENT-TIMESTAMP :DOCUMENTATION
    "Marker value used to indicate valid memoized entries.
This marker changes everytime one of the `timestamps' gets bumped." :ALLOCATION
    :INSTANCE :ACCESSOR %CURRENT-TIMESTAMP)
...
(CL:DEFUN NEW-MEMOIZATION-TABLE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%TIMESTAMPS SELF) NIL) (CL:SETF (%CURRENT-TIMESTAMP SELF) NULL)
...
(CL:DEFUN DISABLE-MEMOIZATION ()
  "Enable memoization and use of memoized expression results."
  (CL:SETQ *MEMOIZATION-ENABLED?* FALSE)
  :VOID)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:FIXNUM) CL:FIXNUM) HASH-MEMOIZED-ARGUMENTSN))
...
(CL:DEFUN MAKE-MEMOIZED-VALUE-ENTRY (VALUE ARG1 ARG2 ARG3 ARG4)
  (CL:LET* ((ENTRY NIL) (CURSOR NIL) (CELL NIL))
   (CL:SETQ ENTRY (CL:CONS CL:NIL CL:NIL)) (CL:SETF (%%VALUE ENTRY) VALUE)
   ...
   (CL:SETF (%%REST CELL) NIL) (CL:RETURN-FROM MAKE-MEMOIZED-VALUE-ENTRY ENTRY)))
...
(CL:DEFUN FIND-MEMOIZED-VALUE-ENTRY (BUCKET ARG1 ARG2 ARG3 ARG4 EQVECTOR DELETEENTRY?)
  ...
  (CL:LET* ((ENTRIES (%%REST BUCKET)) (ARGS NIL) (RESULT NULL))
       ...
       (CL:WHEN (CL:EQ (%%VALUE ARGS) ARG2)
        ...
        (CL:WHEN (CL:EQ (%%VALUE ARGS) ARG3)
         (CL:WHEN (CL:EQ ARG4 MEMOIZED-NULL-VALUE)
          (CL:SETQ RESULT (%%VALUE ENTRIES)) (CL:RETURN))
         (CL:SETQ ARGS (%%REST ARGS))
         (CL:WHEN (CL:EQ (%%VALUE ARGS) ARG4) (CL:SETQ RESULT (%%VALUE ENTRIES))
          (CL:RETURN)))))
      (CL:SETQ ENTRIES (%%REST ENTRIES))))
    (CL:T
     (CL:LET* ((ARG NULL))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ENTRIES NIL)) DO
       (CL:SETQ ARGS (%%REST (%%VALUE ENTRIES))) (CL:SETQ ARG (%%VALUE ARGS))
       (CL:WHEN
   ...
   (CL:WHEN (CL:AND DELETEENTRY? (CL:NOT (CL:EQ RESULT NULL)))
    ...
    (CL:WHEN (CL:EQ (CL:SETF (%%REST BUCKET) (%%REST (%%REST BUCKET))) NIL)
   ...
   (CL:COND
    ((CL:< EQVECTOR 0)
     (CL:LOOP WHILE (CL:NOT (CL:EQ ENTRIES NIL)) DO
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((INPUTARG NULL) (ENTRYARG NULL) (ITER-000 (%%REST (%%VALUE ENTRIES)))
         (ITER-001 TUPLE))
        (CL:LOOP WHILE
         (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:NOT (CL:EQ ITER-001 NIL))) DO
         (CL:PROGN (CL:SETQ ENTRYARG (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:PROGN (CL:SETQ INPUTARG (%%VALUE ITER-001))
          (CL:SETQ ITER-001 (%%REST ITER-001)))
         (CL:WHEN (CL:NOT (CL:EQ ENTRYARG INPUTARG)) (CL:SETQ ALWAYS?-000 FALSE)
          (CL:RETURN))))
    ...
    (CL:T
     (CL:LOOP WHILE (CL:NOT (CL:EQ ENTRIES NIL)) DO
      ...
      (CL:LET*
       ((INPUTARG NULL) (ENTRYARG NULL) (ITER-002 (%%REST RESULT))
        (ITER-003 TUPLE))
       (CL:LOOP WHILE
        (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (CL:NOT (CL:EQ ITER-003 NIL))) DO
        (CL:PROGN (CL:SETQ ENTRYARG (%%VALUE ITER-002))
         (CL:SETQ ITER-002 (%%REST ITER-002)))
        (CL:PROGN (CL:SETQ INPUTARG (%%VALUE ITER-003))
...
(CL:DEFUN LOOKUP-MEMOIZED-VALUE (MEMOTABLE ARG1 ARG2 ARG3 ARG4 EQVECTOR)
  ...
  (CL:CHECK-TYPE EQVECTOR CL:FIXNUM)
  (CL:LET*
   ((HASHTABLE (%HASH-TABLE MEMOTABLE)) (TIMESTAMP NULL) (HASHCODE 0) (ENTRY NIL)
    (BUCKET NIL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ BUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE BUCKET) TIMESTAMP)
      (CL:SETQ ENTRY
       (FIND-MEMOIZED-VALUE-ENTRY BUCKET ARG1 ARG2 ARG3 ARG4 EQVECTOR FALSE))
...
(CL:DEFUN LOOKUP-MEMOIZED-VALUEN (MEMOTABLE ARGS EQVECTOR)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ BUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE BUCKET) TIMESTAMP)
      (CL:SETQ ENTRY (FIND-MEMOIZED-VALUE-ENTRYN BUCKET ARGS EQVECTOR FALSE))
      (CL:WHEN (CL:NOT (CL:EQ ENTRY NULL))
...
(CL:DEFCLASS MRU-MEMOIZATION-TABLE (MEMOIZATION-TABLE)
  ((MRU-BUCKETS-VECTOR :ALLOCATION :INSTANCE :ACCESSOR %MRU-BUCKETS-VECTOR)
   ...
   (MRU-BUCKETS :TYPE CL:SIMPLE-VECTOR :INITFORM STELLA::NULL-NATIVE-VECTOR
    :ALLOCATION :INSTANCE :ACCESSOR %MRU-BUCKETS)
   (LRU-BUCKETS :TYPE CL:SIMPLE-VECTOR :INITFORM STELLA::NULL-NATIVE-VECTOR
    :ALLOCATION :INSTANCE :ACCESSOR %LRU-BUCKETS)
   (MRU-TIMESTAMP :ALLOCATION :INSTANCE :ACCESSOR %MRU-TIMESTAMP)
   (LRU-TIMESTAMP :ALLOCATION :INSTANCE :ACCESSOR %LRU-TIMESTAMP)
   (NOF-BUCKETS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE
    :ACCESSOR %NOF-BUCKETS)
   (FREE-ENTRIES :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE
    :ACCESSOR %FREE-ENTRIES)
   (MAX-ENTRIES :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE
    :ACCESSOR %MAX-ENTRIES)))
...
(CL:DEFUN LOOKUP-MRU-MEMOIZED-VALUE (MEMOTABLE ARG1 ARG2 ARG3 ARG4 EQVECTOR)
  ...
  (CL:CHECK-TYPE EQVECTOR CL:FIXNUM)
  (CL:LET*
   ((MRUBUCKETS (%MRU-BUCKETS MEMOTABLE)) (LRUBUCKETS STELLA::NULL-NATIVE-VECTOR)
    (MRUBUCKET NIL) (LRUBUCKET NIL) (MRUTIMESTAMP NULL) (HASHCODE 0)
    (BUCKETINDEX 0) (ENTRY NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ MRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE MRUBUCKET) MRUTIMESTAMP)
      (CL:SETQ ENTRY
       (FIND-MEMOIZED-VALUE-ENTRY MRUBUCKET ARG1 ARG2 ARG3 ARG4 EQVECTOR FALSE))
      (CL:WHEN (CL:NOT (CL:EQ ENTRY NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ LRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE LRUBUCKET) (%LRU-TIMESTAMP MEMOTABLE))
      (CL:SETQ ENTRY
       (FIND-MEMOIZED-VALUE-ENTRY LRUBUCKET ARG1 ARG2 ARG3 ARG4 EQVECTOR TRUE)))
   ...
   (CL:WHEN
    (CL:= (CL:SETF (%FREE-ENTRIES MEMOTABLE) (CL:1- (%FREE-ENTRIES MEMOTABLE)))
     0)
...
(CL:DEFUN LOOKUP-MRU-MEMOIZED-VALUEN (MEMOTABLE ARGS EQVECTOR)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ MRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE MRUBUCKET) MRUTIMESTAMP)
      (CL:SETQ ENTRY (FIND-MEMOIZED-VALUE-ENTRYN MRUBUCKET ARGS EQVECTOR FALSE))
      (CL:WHEN (CL:NOT (CL:EQ ENTRY NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ LRUBUCKET NULL))
    (CL:COND
     ((CL:EQ (%%VALUE LRUBUCKET) (%LRU-TIMESTAMP MEMOTABLE))
      (CL:SETQ ENTRY (FIND-MEMOIZED-VALUE-ENTRYN LRUBUCKET ARGS EQVECTOR TRUE)))
     (CL:T (CL:SETF (%%REST LRUBUCKET) NIL))))
...
(CL:DEFUN INITIALIZE-MEMOIZATION-TABLE (MEMOTABLESURROGATE OPTIONS)
  ...
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   (CL:SETF (%THE-PLIST SELF-000) (READ-S-EXPRESSION-FROM-STRING OPTIONS))
   (CL:LET*
...
(CL:DEFUN CLEAR-ALL-MEMOIZATION-TABLES ()
  (CL:LET* ((TABLE NULL) (ITER-000 (%THE-CONS-LIST *ALL-MEMOIZATION-TABLES*)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ TABLE (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET* ((TS NULL) (ITER-001 (%TIMESTAMPS TABLE)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:PROGN (CL:SETQ TS (%%VALUE ITER-001))
        (CL:SETQ ITER-001 (%%REST ITER-001)))
       (CL:WHEN (CL:EQ TS TIMESTAMPNAME) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
     (CL:WHEN FOUND?-000
      (CL:COND
  ...
  (CL:WHEN (CL:EQ MEMONAME NULL)
   (CL:SETQ MEMONAME
    (CL:IF (CL:NOT (CL:EQ *METHODBEINGWALKED* NULL))
     (LOCAL-GENSYM
      (CONCATENATE (%SYMBOL-NAME (%SLOT-NAME *METHODBEINGWALKED*))
       "-MEMO-TABLE"))
     (GENSYM "MEMO-TABLE"))))
...
(CL:DEFUN PARSE-MEMOIZE-OPTIONS (OPTIONS)
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   ...
   (CL:LET* ((GIVENOPTIONS SELF-000) (PARSEDOPTIONS (NEW-PROPERTY-LIST)))
    (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST GIVENOPTIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          ...
          (CL:LET* ((ALWAYS?-000 TRUE))
           (CL:LET* ((TS NULL) (ITER-001 VALUE))
            (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
             (CL:PROGN (CL:SETQ TS (%%VALUE ITER-001))
              (CL:SETQ ITER-001 (%%REST ITER-001)))
             (CL:WHEN (CL:NOT (KEYWORD? TS)) (CL:SETQ ALWAYS?-000 FALSE)
              (CL:RETURN))))
         ...
         (CL:IF (CL:NOT (ISA? VALUE SGT-MEMOIZE-STELLA-GENERALIZED-SYMBOL))
          (CL:LET* ((*PRINTREADABLY?* TRUE))
           ...
           (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
            (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
            (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
             " Illegal :name value in memoize: `" (DE-UGLIFY-PARSE-TREE VALUE)
             "'." EOL)))
...
wrapped literals (with the exception of BOOLEANs)."
  (CL:LET*
   ((MEMONAME NULL) (MEMOTABLEVAR (LOCAL-GENSYM "MEMO-TABLE"))
    ...
    (MEMOVALUEVAR (LOCAL-GENSYM "MEMOIZED-VALUE")) (NOFINPUTS (LENGTH INPUTARGS))
    (OPTIONS NULL) (LOOKUPFUNCTION NULL) (LOOKUPTREE NIL) (ALLEQ? TRUE)
    (EQVECTOR 0))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NOFINPUTS EQVECTOR))
   (CL:SETQ BODY (CONS NIL BODY))
   (CL:SETQ OPTIONS (PARSE-MEMOIZE-OPTIONS (EXTRACT-OPTIONS BODY NULL)))
   ...
   (CL:LET*
    ((INDEX NULL-INTEGER) (IT (ALLOCATE-ITERATOR INPUTARGS)) (ITER-000 0))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 INDEX))
    (CL:LOOP WHILE (NEXT? IT) DO
     (CL:PROGN (CL:SETQ INDEX ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:LET* ((WALKEDARG NULL) (TYPE NULL))
      (CL:MULTIPLE-VALUE-SETQ (WALKEDARG TYPE) (WALK-A-TREE (%VALUE IT)))
      (CL:COND
         ...
         (CONS (%%VALUE (%%REST INPUTARGS))
          (CONS (%%VALUE (%%REST (%%REST INPUTARGS)))
           (CONS (FOURTH INPUTARGS)
            (CONS (CL:IF ALLEQ? (WRAP-INTEGER -1) (WRAP-INTEGER EQVECTOR))
             NIL)))))))))
    (CL:OTHERWISE
     (CL:SETQ LOOKUPTREE
      (CONS LOOKUPFUNCTION
    ...
    (LIST* SYM-MEMOIZE-STELLA-VRLET
     (LIST*
      (CONS MEMOTABLEVAR
       (LIST* SYM-MEMOIZE-STELLA-MEMOIZATION-TABLE
        (LIST* SYM-MEMOIZE-STELLA-SURROGATE-VALUE MEMONAME NIL) NIL))
      (CONS MEMOENTRYVAR
     ...
     (LIST* SYM-MEMOIZE-STELLA-WHEN
      ...
      (LIST* SYM-MEMOIZE-STELLA-SETQ MEMOTABLEVAR
       (CONS (LIST* SYM-MEMOIZE-STELLA-SURROGATE-VALUE MEMONAME NIL) NIL))
      NIL)
     ...
     (LIST* SYM-MEMOIZE-STELLA-COND
      ...
      (LIST* SYM-MEMOIZE-STELLA-OTHERWISE
       (LIST* SYM-MEMOIZE-STELLA-SETQ MEMOVALUEVAR (CONCATENATE BODY NIL))
       (LIST* SYM-MEMOIZE-STELLA-WHEN SYM-MEMOIZE-STELLA-*MEMOIZATION-ENABLED?*
...
to the constructor.  Once `self' is initialized it will be cleared." :ALLOCATION
    :INSTANCE :ACCESSOR %BASE-ITERATOR)
...
values to the end of `iterator-and-values'." :ALLOCATION :INSTANCE :ACCESSOR
    %CURSOR))
  (:DOCUMENTATION
...
(CL:DEFUN NEW-MEMOIZABLE-ITERATOR (BASE-ITERATOR)
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%CURSOR SELF) NULL) (CL:SETF (%ITERATOR-AND-VALUES SELF) NULL)
  ...
  (CL:LET* ((ITERANDVALUES (%ITERATOR-AND-VALUES SELF)))
   (CL:COND
    ((CL:NOT (CL:EQ (%%VALUE ITERANDVALUES) NULL))
     (CL:LET* ((CHILD (CL:MAKE-INSTANCE (CL:QUOTE MEMOIZABLE-ITERATOR))))
      (CL:SETF (%FIRST-ITERATION? CHILD) TRUE)
...
generating and saving the next value of the `base-iterator'."
  (CL:LET* ((ITERATOR NULL) (CURSOR (%CURSOR SELF)) (VALUES (%%REST CURSOR)))
   (CL:WHEN (CL:NOT (CL:EQ VALUES NIL)) (CL:SETF (%VALUE SELF) (%%VALUE VALUES))
    (CL:SETF (%CURSOR SELF) VALUES) (CL:RETURN-FROM NEXT? TRUE))
   ...
   (CL:WHEN (CL:NOT (CL:EQ ITERATOR NULL))
    (CL:WHEN (NEXT? ITERATOR) (CL:SETF (%VALUE SELF) (%VALUE ITERATOR))
     ...
     (CL:SETF (%CURSOR SELF) (%%REST CURSOR)) (CL:RETURN-FROM NEXT? TRUE))
...
(CL:DEFUN STARTUP-MEMOIZE ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *MEMOIZATION-ENABLED?* TRUE)
    (CL:LET* ((SELF-057 (NEW-SYMBOL "MEMOIZED-NULL-VALUE")))
     (CL:SETF (%HOME-CONTEXT SELF-057) NULL)
     (CL:SETQ MEMOIZED-NULL-VALUE SELF-057)))
...
maintenance and clearance purposes.\")")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFGLOBAL *MEMOIZATION-ENABLED?* BOOLEAN TRUE)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
