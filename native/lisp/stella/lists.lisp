...
(CL:DEFUN TERMINATE-LIST? (SELF)
  (CL:WHEN (CL:EQ SELF NIL-LIST)
   (CL:WHEN (CL:EQ SELF NIL-LIST) (CL:ERROR "Safety violation: Attempt to free NIL-LIST."))
   (CL:RETURN-FROM TERMINATE-LIST? CL:NIL))
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)) (REST NULL))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CONS NIL)) DO (CL:SETQ REST (%%REST CONS)) (FREE CONS) (CL:SETQ CONS REST))
...
`self' (uses an `eql?' test)."
  (CL:LET* ((I NULL) (ITER-000 (%THE-CONS-LIST SELF)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ I (%%VALUE ITER-000))
    (CL:WHEN (EQL? I OBJECT) (CL:RETURN-FROM MEMBER? CL:T)) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFMETHOD REMOVE-DUPLICATES-EQUAL ((SELF LIST))
  ...
  (CL:WHEN (CL:NOT (CL:EQ (%THE-CONS-LIST SELF) NIL))
   (CL:SETF (%THE-CONS-LIST SELF) (REMOVE-DUPLICATES-EQUAL (%THE-CONS-LIST SELF))))
...
Return NULL if the list is empty."
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)) (VALUE NULL)) (CL:WHEN (CL:EQ CONS NIL) (CL:RETURN-FROM POP NULL))
   (CL:SETQ VALUE (%%VALUE CONS)) (CL:SETF (%THE-CONS-LIST SELF) (%%REST CONS)) (CL:RETURN-FROM POP VALUE)))
...
not relying on the somewhat less efficient variable arguments mechanism."
  (CL:LET* ((FIRSTLIST (%THE-CONS-LIST LIST1)) (LASTLIST (%THE-CONS-LIST LIST2)) (COPY NIL))
   (CL:COND ((CL:EQ FIRSTLIST NIL) (CL:SETQ FIRSTLIST (COPY-CONS-LIST LASTLIST)) (CL:SETQ LASTLIST FIRSTLIST))
    ...
    (CL:LET* ((LIST NULL) (ITER-000 OTHERLISTS))
     (CL:LOOP WHILE ITER-000 DO (CL:SETQ LIST (CL:POP ITER-000))
      (CL:WHEN (NON-EMPTY? LIST) (CL:SETQ COPY (COPY-CONS-LIST (%THE-CONS-LIST LIST)))
       (CL:IF (CL:NOT (CL:EQ LASTLIST NIL)) (CL:SETF (%%REST (LAST-CONS LASTLIST)) COPY) (CL:SETQ FIRSTLIST COPY))
...
(CL:DEFUN CREATE-DERIVED-LIST (SELF)
  ...
  (CL:LET* ((LISTTYPE (PRIMARY-TYPE SELF)))
   (CL:COND ((CL:EQ LISTTYPE SGT-LISTS-STELLA-LIST) (CL:RETURN-FROM CREATE-DERIVED-LIST (NEW-LIST)))
    ((CL:EQ LISTTYPE SGT-LISTS-STELLA-SET) (CL:RETURN-FROM CREATE-DERIVED-LIST (NEW-SET)))
...
freshly allocated."
  (CL:LET* ((COPY (CREATE-DERIVED-LIST SELF))) (CL:SETF (%THE-CONS-LIST COPY) (COPY-CONS-LIST (%THE-CONS-LIST SELF)))
...
equivalent.  Uses `equal?' to test equality of elements."
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-LISTS-STELLA-LIST)
    (CL:PROGN (CL:RETURN-FROM OBJECT-EQUAL? (EQUAL? (%THE-CONS-LIST X) (%THE-CONS-LIST Y)))))
   (CL:T))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CODE 5619580)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:RETURN-FROM EQUAL-HASH-CODE
    (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (EQUAL-HASH-CODE (%THE-CONS-LIST SELF))))))))

;;; (DEFMETHOD (ALLOCATE-ITERATOR (LIST-ITERATOR OF (LIKE (ANY-VALUE SELF)))) ...)

(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF LIST))
  (CL:LET* ((ITERATOR (NEW-LIST-ITERATOR))) (CL:SETF (%LIST-ITERATOR-CURSOR ITERATOR) (%THE-CONS-LIST SELF))
   (CL:SETF (%LIST-ITERATOR-COLLECTION ITERATOR) SELF) (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T)
...
(CL:DEFMETHOD NEXT? ((SELF LIST-ITERATOR))
  (CL:IF (%FIRST-ITERATION? SELF)
   (CL:PROGN (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)
    (CL:SETF (%LIST-ITERATOR-CURSOR SELF) (%THE-CONS-LIST (%LIST-ITERATOR-COLLECTION SELF))))
...
(CL:DEFMETHOD EMPTY? ((SELF LIST-ITERATOR))
  (CL:RETURN-FROM EMPTY?
   (CL:IF (%FIRST-ITERATION? SELF) (CL:EQ (%THE-CONS-LIST (%LIST-ITERATOR-COLLECTION SELF)) NIL)
...
(CL:DEFUN SEARCH-PLIST (PLIST KEY)
  (CL:LOOP WHILE (CL:NOT (CL:EQ PLIST NIL)) DO
   (CL:WHEN (EQL? (%%VALUE PLIST) KEY) (CL:RETURN-FROM SEARCH-PLIST (%%VALUE (%%REST PLIST))))
...
(CL:DEFMETHOD LENGTH ((SELF PROPERTY-LIST))
  (CL:LET* ((CURSOR (%THE-PLIST SELF)) (LENGTH 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (CL:SETQ LENGTH (CL:1+ LENGTH))
...
If a previous entry existed with key `key', that entry is replaced."
  (CL:LET* ((CURSOR (%THE-PLIST SELF)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (CL:WHEN (EQL? (%%VALUE CURSOR) KEY) (CL:SETQ CURSOR (%%REST CURSOR)) (CL:SETF (%%VALUE CURSOR) VALUE)
...
most one entry matches `key'."
  (CL:LET* ((CURSOR (%THE-PLIST SELF)) (PREVIOUSCONS NULL) (VALUE NULL))
   (CL:WHEN (CL:NOT (CL:EQ CURSOR NIL))
    (CL:WHEN (EQL? (%%VALUE CURSOR) KEY) (CL:SETF (%THE-PLIST SELF) (%%REST (%%REST CURSOR)))
     (CL:SETQ VALUE (%%VALUE (%%REST CURSOR))) (CL:RETURN-FROM REMOVE-AT VALUE))
    (CL:SETQ PREVIOUSCONS (%%REST CURSOR)) (CL:SETQ CURSOR (%%REST PREVIOUSCONS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
     (CL:WHEN (EQL? (%%VALUE CURSOR) KEY) (CL:SETF (%%REST PREVIOUSCONS) (%%REST (%%REST CURSOR)))
      (CL:SETQ VALUE (%%VALUE (%%REST CURSOR))) (CL:RETURN-FROM REMOVE-AT VALUE))
     (CL:SETQ PREVIOUSCONS (%%REST CURSOR)) (CL:SETQ CURSOR (%%REST PREVIOUSCONS))))
...
(CL:DEFMETHOD COPY ((SELF PROPERTY-LIST))
  "Return a copy of the list `self'.  The conses in the copy are
freshly allocated."
  (CL:LET* ((COPY (CREATE-OBJECT (PRIMARY-TYPE SELF)))) (CL:SETF (%THE-PLIST COPY) (COPY-CONS-LIST (%THE-PLIST SELF)))
   (CL:RETURN-FROM COPY COPY)))
     ...
     (CL:WHEN (CL:= (LENGTH X) (LENGTH Y))
      (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST X)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-000))
        ...
        (CL:WHEN (CL:NOT (EQUAL? VALUE (LOOKUP Y KEY))) (CL:RETURN-FROM OBJECT-EQUAL? CL:NIL))
  ...
  (CL:LET* ((CODE 108398967)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-000))
     ...
     (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (SAFE-EQUAL-HASH-CODE VALUE)))))
     (CL:SETQ ITER-000 (%%REST (%%REST ITER-000)))))
...
guaranteed to be a set if both `self' and `otherList' are sets."
  (CL:LET* ((RESULT (CREATE-DERIVED-LIST SELF)))
   (CL:SETF (%THE-CONS-LIST RESULT) (INTERSECTION (%THE-CONS-LIST SELF) (%THE-CONS-LIST OTHERLIST)))
...
guaranteed to be a set if both `self' and `otherList' are sets."
  (CL:LET* ((RESULT (CREATE-DERIVED-LIST SELF)))
   (CL:SETF (%THE-CONS-LIST RESULT) (UNION (%THE-CONS-LIST SELF) (%THE-CONS-LIST OTHERLIST)))
...
set if both `self' and `otherList' are sets."
  (CL:LET* ((RESULT (CREATE-DERIVED-LIST SELF)))
   (CL:SETF (%THE-CONS-LIST RESULT) (DIFFERENCE (%THE-CONS-LIST SELF) (%THE-CONS-LIST OTHERLIST)))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF PROPERTY-LIST))
  (CL:LET* ((ITERATOR (NEW-PROPERTY-LIST-ITERATOR))) (CL:SETF (%PLIST-ITERATOR-CURSOR ITERATOR) (%THE-PLIST SELF))
   (CL:SETF (%PLIST-ITERATOR-COLLECTION ITERATOR) SELF) (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T)
   (CL:RETURN-FROM ALLOCATE-ITERATOR ITERATOR)))
...
(CL:DEFMETHOD NEXT? ((SELF PROPERTY-LIST-ITERATOR))
  (CL:IF (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)
   (CL:SETF (%PLIST-ITERATOR-CURSOR SELF) (%%REST (%%REST (%PLIST-ITERATOR-CURSOR SELF)))))
  (CL:LET* ((REMAININGPLIST (%PLIST-ITERATOR-CURSOR SELF)))
   (CL:WHEN (CL:NOT (CL:EQ REMAININGPLIST NIL)) (CL:SETF (%KEY SELF) (%%VALUE REMAININGPLIST))
    (CL:SETF (%VALUE SELF) (%%VALUE (%%REST REMAININGPLIST))) (CL:RETURN-FROM NEXT? CL:T))
...
(CL:DEFUN KV-CONS (KEY VALUE REST)
  ...
  (CL:LET* ((NEWKVC (NEW-KV-CONS))) (CL:SETF (%KEY NEWKVC) KEY) (CL:SETF (%VALUE NEWKVC) VALUE)
   (CL:SETF (%REST NEWKVC) REST) (CL:RETURN-FROM KV-CONS NEWKVC)))
...
(CL:DEFMETHOD LOOKUP ((SELF KV-CONS) KEY)
  (CL:LET* ((CURSOR SELF))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:RETURN-FROM LOOKUP (%VALUE CURSOR))) (CL:SETQ CURSOR (%REST CURSOR)))
...
(CL:DEFMETHOD LENGTH ((SELF KV-CONS))
  (CL:LET* ((CURSOR SELF) (LENGTH 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:SETQ LENGTH (CL:1+ LENGTH)) (CL:SETQ CURSOR (%REST CURSOR)))
   (CL:RETURN-FROM LENGTH LENGTH)))
...
Return `default' if no value was found."
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:RETURN-FROM LOOKUP-WITH-DEFAULT (%VALUE CURSOR)))
...
`self'. If a previous entry existed with key `key', that entry is replaced."
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:SETF (%VALUE CURSOR) VALUE) (CL:RETURN-FROM INSERT-AT))
    (CL:SETQ CURSOR (%REST CURSOR)))
...
Assumes that at most one entry matches `key'."
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (PREVIOUSCURSOR NULL) (VALUE NULL))
   (CL:WHEN (CL:NOT (CL:EQ CURSOR NULL))
    (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:SETF (%THE-KV-LIST SELF) (%REST CURSOR)) (CL:SETQ VALUE (%VALUE CURSOR))
     (FREE-KV-CONS CURSOR) (CL:RETURN-FROM REMOVE-AT VALUE))
    (CL:LOOP WHILE (CL:NOT (CL:EQ (%REST CURSOR) NULL)) DO (CL:SETQ PREVIOUSCURSOR CURSOR)
     (CL:SETQ CURSOR (%REST CURSOR))
     (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:SETF (%REST PREVIOUSCURSOR) (%REST CURSOR)) (CL:SETQ VALUE (%VALUE CURSOR))
      (FREE-KV-CONS CURSOR) (CL:RETURN-FROM REMOVE-AT VALUE))))
   (CL:RETURN-FROM REMOVE-AT NULL)))
  ...
  (CL:LET* ((FOUND?-000 CL:NIL))
   (CL:LET* ((K NULL) (V NULL) (ITER-000 (%THE-KV-LIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ K (%KEY ITER-000)) (CL:SETQ V (%VALUE ITER-000))
     (CL:WHEN (CL:AND (CL:EQ K KEY) (CL:EQ V VALUE)) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
  ...
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (PREVIOUSCURSOR NULL))
   (CL:WHEN (CL:NOT (CL:EQ CURSOR NULL))
    (CL:WHEN (CL:AND (EQL? (%KEY CURSOR) KEY) (EQL? (%VALUE CURSOR) VALUE))
     (CL:SETF (%THE-KV-LIST SELF) (%REST CURSOR)) (FREE-KV-CONS CURSOR) (CL:RETURN-FROM REMOVE-ENTRY))
    (CL:LOOP WHILE (CL:NOT (CL:EQ (%REST CURSOR) NULL)) DO (CL:SETQ PREVIOUSCURSOR CURSOR)
     (CL:SETQ CURSOR (%REST CURSOR))
     (CL:WHEN (CL:AND (EQL? (%KEY CURSOR) KEY) (EQL? (%VALUE CURSOR) VALUE))
      (CL:SETF (%REST PREVIOUSCURSOR) (%REST CURSOR)) (FREE-KV-CONS CURSOR) (CL:RETURN-FROM REMOVE-ENTRY)))))
  :VOID)
...
the list is empty."
  ...
  (CL:LET* ((CONS (%THE-KV-LIST SELF)) (VALUE NULL)) (CL:SETQ VALUE (%VALUE CONS))
   (CL:SETF (%THE-KV-LIST SELF) (%REST CONS)) (FREE-KV-CONS CONS) (CL:RETURN-FROM POP VALUE)))

;;; (DEFMETHOD (REVERSE (LIKE SELF)) ...)

(CL:DEFMETHOD REVERSE ((SELF KEY-VALUE-LIST))
  "Destructively reverse the members of the list `self'."
  (CL:LET* ((REVERSEDLIST (%THE-KV-LIST SELF)) (REMAININGLIST NULL) (TEMP NULL))
   (CL:WHEN (CL:EQ REVERSEDLIST NULL) (CL:RETURN-FROM REVERSE SELF)) (CL:SETQ REMAININGLIST (%REST REVERSEDLIST))
   (CL:WHEN (CL:EQ REMAININGLIST NULL) (CL:RETURN-FROM REVERSE SELF)) (CL:SETF (%REST REVERSEDLIST) NULL)
   (CL:LOOP WHILE (CL:NOT (CL:EQ REMAININGLIST NULL)) DO (CL:SETQ TEMP REMAININGLIST)
    (CL:SETQ REMAININGLIST (%REST REMAININGLIST)) (CL:SETF (%REST TEMP) REVERSEDLIST) (CL:SETQ REVERSEDLIST TEMP))
...
(CL:DEFUN COPY-KV-CONS-LIST (KVCONSLIST)
  ...
  (CL:LET* ((NEWKVCONSLIST (NEW-KV-CONS)) (NEXTKVCONS NEWKVCONSLIST) (COPYFROMKVCONS NULL) (PREVIOUSKVCONS NULL))
   (CL:SETF (%KEY NEXTKVCONS) (%KEY KVCONSLIST)) (CL:SETF (%VALUE NEXTKVCONS) (%VALUE KVCONSLIST))
   (CL:SETQ COPYFROMKVCONS (%REST KVCONSLIST))
   (CL:LOOP WHILE (CL:NOT (CL:EQ COPYFROMKVCONS NULL)) DO (CL:SETQ PREVIOUSKVCONS NEXTKVCONS)
    (CL:SETQ NEXTKVCONS (NEW-KV-CONS)) (CL:SETF (%REST PREVIOUSKVCONS) NEXTKVCONS)
    (CL:SETF (%KEY NEXTKVCONS) (%KEY COPYFROMKVCONS)) (CL:SETF (%VALUE NEXTKVCONS) (%VALUE COPYFROMKVCONS))
...
the copy are freshly allocated."
  (CL:LET* ((COPY (NEW-KEY-VALUE-LIST))) (CL:SETF (%THE-KV-LIST COPY) (COPY-KV-CONS-LIST (%THE-KV-LIST SELF)))
   (CL:RETURN-FROM COPY COPY)))
...
(CL:DEFMETHOD CLEAR ((SELF KEY-VALUE-LIST))
  ...
  (CL:LET* ((NEXT (%THE-KV-LIST SELF)) (CURRENT NULL))
   (CL:LOOP WHILE (CL:NOT (CL:EQ NEXT NULL)) DO (CL:SETQ CURRENT NEXT) (CL:SETQ NEXT (%REST NEXT))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF KEY-VALUE-LIST))
  (CL:LET* ((ITERATOR (NEW-KV-LIST-ITERATOR))) (CL:SETF (%THE-KV-LIST ITERATOR) SELF)
   (CL:SETF (%KV-LIST-ITERATOR-CURSOR ITERATOR) (%THE-KV-LIST SELF)) (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T)
   (CL:RETURN-FROM ALLOCATE-ITERATOR ITERATOR)))
...
(CL:DEFMETHOD NEXT? ((SELF KV-LIST-ITERATOR))
  (CL:IF (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)
   (CL:SETF (%KV-LIST-ITERATOR-CURSOR SELF) (%REST (%KV-LIST-ITERATOR-CURSOR SELF))))
...
ordered keys and values."
  (CL:LET* ((PLIST NIL))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-KV-LIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:SETQ PLIST (CONS VALUE (CONS KEY PLIST))) (CL:SETQ ITER-000 (%REST ITER-000))))
   (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST))) (CL:SETF (%THE-PLIST SELF-000) (REVERSE PLIST))
    (CL:LET* ((VALUE-000 SELF-000)) (CL:RETURN-FROM KVLIST-TO-PLIST VALUE-000)))))
    ...
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ V (CL:POP ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS V NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST LIST) NIL) (CL:SETF (%THE-CONS-LIST LIST) COLLECT-000)
    ...
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ITEM (CL:POP ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS ITEM NIL))
       (CL:IF (CL:EQ COPY NIL) (CL:SETQ COPY COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST COPY COLLECT-000)))
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS ITEM NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:SETQ CURSOR COPY)
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (CL:SETQ KEY (%%VALUE CURSOR))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ K (%KEY ITER-000)) (CL:SETQ V (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS (CONS K (CONS V NIL)) NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST RESULT) NIL) (CL:SETF (%THE-CONS-LIST RESULT) COLLECT-000)
...
(CL:DEFMETHOD LISTIFY ((SELF ITERATOR))
  ...
  (CL:LET* ((LIST (NEW-LIST)))
   (CL:LET* ((VALUE NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ VALUE (%VALUE ITER-000)) (PUSH LIST VALUE)))
...
(CL:DEFUN PRINT-KEY-VALUE-LIST (SELF STREAM)
  ...
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (POSITION 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM POSITION))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN (CL:AND (CL:NOT (CL:= *PRINTLENGTH* NULL-INTEGER)) (CL:> POSITION *PRINTLENGTH*))
     (%%PRINT-STREAM STREAM " ...") (CL:RETURN))
    (CL:WHEN (CL:NOT (CL:= POSITION 1)) (%%PRINT-STREAM STREAM " ")) (%%PRINT-STREAM STREAM CURSOR)
    (CL:SETQ CURSOR (%REST CURSOR)) (CL:SETQ POSITION (CL:1+ POSITION)))
   (%%PRINT-STREAM STREAM ")"))
  :VOID)

(CL:DEFUN HELP-STARTUP-LISTS1 ()
  (CL:PROGN (CL:SETQ SGT-LISTS-STELLA-LIST (INTERN-RIGID-SYMBOL-WRT-MODULE "LIST" NULL 1))
   (CL:SETQ SGT-LISTS-STELLA-SET (INTERN-RIGID-SYMBOL-WRT-MODULE "SET" NULL 1))
...
(CL:DEFUN HELP-STARTUP-LISTS2 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "TERMINATE-LIST?" "(DEFUN (TERMINATE-LIST? BOOLEAN) ((SELF LIST)))"
...
The operation is destructive wrt `list1', but leaves all other lists intact.
The two mandatory parameters allow us to optimize the common binary case by
not relying on the somewhat less efficient variable arguments mechanism.\" :PUBLIC? TRUE)" (CL:FUNCTION CONCATENATE)
    NULL)
   (DEFINE-METHOD-OBJECT
...
freshly allocated.\" :PUBLIC? TRUE)" (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF LIST)) :DOCUMENTATION \"Make `self' an empty list.\" :PUBLIC? TRUE)"
...
is O(N) in the number of elements of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (ALLOCATE-ITERATOR (LIST-ITERATOR OF (LIKE (ANY-VALUE SELF)))) ((SELF LIST)))"
    (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
   (DEFINE-FUNCTION-OBJECT "TERMINATE-LIST-ITERATOR?"
    "(DEFUN (TERMINATE-LIST-ITERATOR? BOOLEAN) ((SELF LIST-ITERATOR)))" (CL:FUNCTION TERMINATE-LIST-ITERATOR?) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (VALUE-SETTER OBJECT) ((SELF LIST-ITERATOR) (VALUE (LIKE (ANY-VALUE SELF)))))"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE-DELETED-MEMBERS (LIKE SELF)) ((SELF LIST)) :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "SEARCH-PLIST" "(DEFUN (SEARCH-PLIST OBJECT) ((PLIST CONS) (KEY OBJECT)))"
...
most one entry matches `key'.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-AT) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NON-EMPTY? BOOLEAN) ((SELF PROPERTY-LIST)) :PUBLIC? TRUE)"
    (CL:FUNCTION NON-EMPTY?) NULL))
  :VOID)

(CL:DEFUN STARTUP-LISTS ()
  (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 0)
   (CL:WHEN (CL:EQ NIL-LIST NULL) (CL:SETQ NIL-LIST (CL:MAKE-INSTANCE (CL:QUOTE LIST))))
...
guaranteed to be a set if `self' is a set.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBTRACT) NULL)
    (DEFINE-METHOD-OBJECT
     ...
     (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF PROPERTY-LIST-ITERATOR)) :PUBLIC? TRUE)"
...
Return `default' if no value was found.\" :PUBLIC? TRUE)" (CL:FUNCTION LOOKUP-WITH-DEFAULT) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF KEY-VALUE-LIST)) :PUBLIC? TRUE)" (CL:FUNCTION LENGTH)
     NULL)
    (DEFINE-METHOD-OBJECT
...
the list is empty.\" :PUBLIC? TRUE)" (CL:FUNCTION POP) NULL)
    (DEFINE-METHOD-OBJECT
     ...
     (CL:FUNCTION REVERSE) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF KEY-VALUE-LIST)) :PUBLIC? TRUE)" (CL:FUNCTION EMPTY?)
     NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (NON-EMPTY? BOOLEAN) ((SELF KEY-VALUE-LIST)) :PUBLIC? TRUE)"
     (CL:FUNCTION NON-EMPTY?) NULL)
...
is O(N) in the number of entries of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL)
    (DEFINE-METHOD-OBJECT
     ...
     (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF KV-LIST-ITERATOR)) :PUBLIC? TRUE)" (CL:FUNCTION NEXT?)
     NULL)
    (DEFINE-METHOD-OBJECT
     "(DEFMETHOD (VALUE-SETTER (LIKE (ANY-VALUE SELF))) ((SELF KV-LIST-ITERATOR) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)"
     (CL:FUNCTION VALUE-SETTER) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (KEY-SETTER OBJECT) ((SELF KV-LIST-ITERATOR) (KEY OBJECT)) :PUBLIC? TRUE)"
     (CL:FUNCTION KEY-SETTER) NULL)
...
@KEY-VALUE-LIST, @KEY-VALUE-MAP and @PROPERTY-LIST.\" :PUBLIC? TRUE :CONSTRUCTOR? TRUE)" (CL:FUNCTION DICTIONARY) NULL)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-LISTS-STELLA-STARTUP-LISTS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-LISTS-STELLA-METHOD-STARTUP-CLASSNAME
