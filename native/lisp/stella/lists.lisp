...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* *PRINTLENGTH* FALSE TRUE NIL))
...
(CL:DEFMETHOD THIRD ((SELF LIST))
  ...
  (CL:RETURN-FROM THIRD
   (%%VALUE (%%REST (%%REST (%THE-CONS-LIST SELF))))))
...
(CL:DEFMETHOD NTH-SETTER ((SELF LIST) VALUE POSITION)
  ...
  (CL:RETURN-FROM NTH-SETTER
   (NTH-SETTER (%THE-CONS-LIST SELF) VALUE POSITION)))
...
will always be relative to the entire list."
  ...
  (CL:RETURN-FROM POSITION
   (POSITION (%THE-CONS-LIST SELF) OBJECT START)))
...
returns TRUE or FALSE.  Returns `self'."
  (CL:SETF (%THE-CONS-LIST SELF)
   (REMOVE-IF (%THE-CONS-LIST SELF) TEST?))
...
Preserves the original order of the remaining members."
  (CL:WHEN (CL:NOT (CL:EQ (%THE-CONS-LIST SELF) NIL))
   (CL:SETF (%THE-CONS-LIST SELF)
    (REMOVE-DUPLICATES (%THE-CONS-LIST SELF))))
...
Return NULL if the list is empty."
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)) (VALUE NULL))
   ...
   (CL:SETF (%THE-CONS-LIST SELF) (%%REST CONS))
...
not relying on the somewhat less efficient variable arguments mechanism."
  (CL:LET*
   ((FIRSTLIST (%THE-CONS-LIST LIST1))
    (LASTLIST (%THE-CONS-LIST LIST2)) (COPY NIL))
   (CL:COND
    ((CL:EQ FIRSTLIST NIL)
...
(CL:DEFMETHOD NEXT? ((SELF LIST-ITERATOR))
  ...
  (CL:RETURN-FROM NEXT?
   (CL:NOT (CL:EQ (%LIST-ITERATOR-CURSOR SELF) NIL))))
...
most one entry matches `key'."
  (CL:LET*
   ((CURSOR (%THE-PLIST SELF)) (PREVIOUSCONS NULL) (VALUE NULL))
...
Assumes that at most one entry matches `key'."
  (CL:LET*
   ((CURSOR (%THE-KV-LIST SELF)) (PREVIOUSCURSOR NULL) (VALUE NULL))
...
Assumes that more than one entry can match `key'."
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (PREVIOUSCURSOR NULL))
   (CL:WHEN (CL:NOT (CL:EQ CURSOR NULL))
    (CL:WHEN
     (CL:AND (EQL? (%KEY CURSOR) KEY) (EQL? (%VALUE CURSOR) VALUE))
...
the list is empty."
  ...
  (CL:LET* ((CONS (%THE-KV-LIST SELF)) (VALUE NULL))
   (CL:SETQ VALUE (%VALUE CONS))
   (CL:SETF (%THE-KV-LIST SELF) (%REST CONS)) (FREE-KV-CONS CONS)
   (CL:RETURN-FROM POP VALUE)))
...
(CL:DEFMETHOD REVERSE ((SELF KEY-VALUE-LIST))
  ...
  (CL:LET*
   ((REVERSEDLIST (%THE-KV-LIST SELF)) (REMAININGLIST NULL)
    (TEMP NULL))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ REMAININGLIST NULL)) DO
    ...
    (CL:SETQ REMAININGLIST (%REST REMAININGLIST))
    (CL:SETF (%REST TEMP) REVERSEDLIST) (CL:SETQ REVERSEDLIST TEMP))
   (CL:SETF (%THE-KV-LIST SELF) REVERSEDLIST)
   (CL:RETURN-FROM REVERSE SELF)))
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS V NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST LIST) NIL)
        ...
        (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST LIST)
         COLLECT-000)))
...
(%%DEFCONSMETHOD LISTIFY ((SELF CL:CONS))
  ...
  (CL:LET* ((SELF-000 (NEW-LIST)))
   (CL:SETF (%THE-CONS-LIST SELF-000) SELF)
...
(CL:DEFMETHOD LISTIFY ((SELF KEY-VALUE-LIST))
  ...
  (CL:LET* ((RESULT (NEW-LIST)))
   (CL:LET*
    ((V NULL) (K NULL) (ITER-000 (%THE-KV-LIST SELF))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
     (CL:SETQ K (%KEY ITER-000))
     (CL:PROGN (CL:SETQ V (%VALUE ITER-000))
  ...
  (CL:LET* ((LIST (NEW-LIST)))
   (CL:LET* ((VALUE NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (NEXT? ITER-000) DO
     (CL:SETQ VALUE (%VALUE ITER-000)) (PUSH LIST VALUE)))
...
(CL:DEFUN PRINT-KEY-VALUE-LIST (SELF STREAM)
  ...
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (POSITION 1))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN
     (CL:AND (DEFINED? *PRINTLENGTH*) (CL:> POSITION *PRINTLENGTH*))
...
`self' (uses an 'eq?' test).\" :PUBLIC? TRUE)" (CL:FUNCTION MEMB?)
    NULL)
   (DEFINE-METHOD-OBJECT
...
the copy are freshly allocated.\" :PUBLIC? TRUE)" (CL:FUNCTION COPY)
     NULL)
    (DEFINE-METHOD-OBJECT
...
@KEY-VALUE-LIST, @KEY-VALUE-MAP and @PROPERTY-LIST.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Lists") NULL-STRING-WRAPPER)))
