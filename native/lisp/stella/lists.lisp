...
(CL:DECLAIM (CL:SPECIAL *STELLA-MODULE* *MODULE* *PRINTLENGTH* FALSE TRUE NIL))
...
(CL:DEFUN TERMINATE-LIST? (SELF)
  ...
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)) (REST NULL))
   ...
   (CL:SETF (%THE-CONS-LIST SELF) NIL) (CL:RETURN-FROM TERMINATE-LIST? TRUE)))
...
(CL:DEFMETHOD FIRST ((SELF LIST))
  ...
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)))
   (CL:RETURN-FROM FIRST (CL:IF (CL:NOT (CL:EQ CONS NIL)) (%%VALUE CONS) NULL))))
...
Return NULL if the list is empty."
  (CL:LET* ((CONS (%THE-CONS-LIST SELF)) (VALUE NULL))
   ...
   (CL:SETQ VALUE (%%VALUE CONS)) (CL:SETF (%THE-CONS-LIST SELF) (%%REST CONS))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CODE 35619580)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   ...
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-LISTS-STELLA-PROPERTY-LIST)
    (CL:PROGN
     (CL:WHEN (CL:= (LENGTH X) (LENGTH Y))
      (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST X)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        ...
        (CL:WHEN (CL:NOT (EQUAL? VALUE (LOOKUP Y KEY)))
         (CL:RETURN-FROM OBJECT-EQUAL? FALSE))))
      (CL:RETURN-FROM OBJECT-EQUAL? TRUE))))
   (CL:T))
  (CL:RETURN-FROM OBJECT-EQUAL? FALSE))
...
is O(N) in the number of entries of `self'."
  (CL:LET* ((CODE 108398967)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ KEY (%%VALUE ITER-000))
     (CL:PROGN (CL:SETQ VALUE (%%VALUE (%%REST ITER-000)))
      (CL:SETQ ITER-000 (%%REST (%%REST ITER-000))))
...
(CL:DEFMETHOD LENGTH ((CURSOR KV-CONS))
  (CL:LET* ((LENGTH 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:SETQ LENGTH (CL:1+ LENGTH))
    (CL:SETQ CURSOR (%REST CURSOR)))
   (CL:RETURN-FROM LENGTH LENGTH)))
  ...
  (CL:LET* ((CURSOR (%THE-KV-LIST SELF)) (PREVIOUSCURSOR NULL) (VALUE NULL))
   (CL:WHEN (CL:NOT (CL:EQ CURSOR NULL))
    (CL:WHEN (EQL? (%KEY CURSOR) KEY)
     (CL:SETF (%THE-KV-LIST SELF) (%REST CURSOR)) (CL:SETQ VALUE (%VALUE CURSOR))
...
entry already exists.  This can generate duplicate entries for `key'."
  (CL:LET* ((FOUND?-000 FALSE))
   (CL:LET* ((V NULL) (K NULL) (ITER-000 (%THE-KV-LIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ K (%KEY ITER-000))
     ...
     (CL:WHEN (CL:AND (CL:EQ K KEY) (CL:EQ V VALUE)) (CL:SETQ FOUND?-000 TRUE)
      (CL:RETURN))))
...
(CL:DEFMETHOD OBJECT-EQUAL? ((X KEY-VALUE-LIST) Y)
  ...
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-LISTS-STELLA-KEY-VALUE-LIST)
    (CL:PROGN
     (CL:WHEN (CL:= (LENGTH X) (LENGTH Y))
      (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-KV-LIST X)))
...
is O(N) in the number of entries of `self'."
  (CL:LET* ((CODE 94797224)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-KV-LIST SELF)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
     (CL:SETQ KEY (%KEY ITER-000))
     (CL:PROGN (CL:SETQ VALUE (%VALUE ITER-000))
      (CL:SETQ ITER-000 (%REST ITER-000)))
...
(CL:DEFUN HELP-STARTUP-LISTS1 ()
  (CL:PROGN
   (CL:SETQ SGT-LISTS-STELLA-LIST (INTERN-RIGID-SYMBOL-WRT-MODULE "LIST" NULL 1))
...
'test?' evaluates to TRUE.  `test' takes a single argument of type OBJECT and
returns TRUE or FALSE.  Returns `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-IF)
    NULL)
   (DEFINE-METHOD-OBJECT
...
is O(N) in the number of elements of `self'.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INITIALIZE-OBJECT ((SELF PROPERTY-LIST)))"
...
guaranteed to be a set if `self' is a set.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "FREE-KV-CONS"
     "(DEFUN FREE-KV-CONS ((KVCONS KV-CONS)))" (CL:FUNCTION FREE-KV-CONS) NULL)
    (DEFINE-METHOD-OBJECT
...
Assumes that more than one entry can match `key'.\")" (CL:FUNCTION REMOVE-ENTRY)
     NULL)
    (DEFINE-METHOD-OBJECT
...
@KEY-VALUE-LIST, @KEY-VALUE-MAP and @PROPERTY-LIST.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-LISTS"
     "(DEFUN STARTUP-LISTS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-LISTS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-LISTS-STELLA-STARTUP-LISTS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-LISTS-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_Startup-Lists")
      NULL-STRING-WRAPPER)))
