...
(CL:DECLAIM
 (CL:SPECIAL *CLEAR-MODULE-HOOKS* STANDARD-OUTPUT TRUE-WRAPPER FALSE-WRAPPER
  NULL-STRING-WRAPPER STANDARD-ERROR *DESTROY-CONTEXT-HOOKS*
  *CONTEXT-NUMBER-COUNTER* *CHANGE-MODULE-HOOKS* *STELLA-MODULE* SYMBOL-SYM
  SURROGATE-SYM EOL NULL-CHARACTER MODULE-SEPARATOR-CHARACTER *ROOT-MODULE*
  NULL-INTEGER *MODULE* *CONTEXT* FALSE TRUE NIL))
  ...
  (CL:LET*
    ...
    (CL:COND
     ...
     (CL:T
      (CL:LET* ((VALUE-000 NULL))
       (CL:LET*
        ((CHILD NULL) (ITER-001 (%THE-CONS-LIST (%CHILD-CONTEXTS CONTEXT))))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
         (CL:PROGN (CL:SETQ CHILD (%%VALUE ITER-001))
          (CL:SETQ ITER-001 (%%REST ITER-001)))
         (CL:LET* ((TEST-VALUE-000 FALSE))
          (CL:SETQ TEST-VALUE-000
           (CL:OR (CL:NOT MODULE?) (ISA? CHILD SGT-MODULES-STELLA-MODULE)))
          (CL:WHEN TEST-VALUE-000
           (CL:IF
            (COMPONENT-MATCH?
             (CL:IF MODULE? (%MODULE-NAME CHILD) (CONTEXT-NAME CHILD)) PATHNAME
             COMPONENTSTART COMPONENTEND)
            (CL:SETQ TEST-VALUE-000 TRUE)
            (CL:PROGN (CL:SETQ TEST-VALUE-000 MODULE?)
             (CL:WHEN TEST-VALUE-000
              (CL:SETQ TEST-VALUE-000 (CL:NOT (CL:EQ (%NICKNAMES CHILD) NULL)))
              (CL:WHEN TEST-VALUE-000
               (CL:LET* ((FOUND?-000 FALSE))
                (CL:LET*
                 ((NICKNAME NULL) (ITER-002 (%THE-CONS-LIST (%NICKNAMES CHILD))))
   ...
   (CL:IF (CL:EQ CONTEXT NULL)
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "No context with name `"
      PATHNAME "'")
...
(CL:DEFUN DWIM-MODULE-AND-BARE-NAME (NAME WARN?)
  ...
  (CL:LET* ((MODULE NULL) (BARENAMESTART NULL-INTEGER))
   (CL:DECLARE (CL:TYPE CL:FIXNUM BARENAMESTART))
   (CL:HANDLER-CASE
    (CL:LET* ((VALUE-000 NULL) (VALUE-001 NULL-INTEGER))
     (CL:DECLARE (CL:TYPE CL:FIXNUM VALUE-001))
     (CL:MULTIPLE-VALUE-SETQ (VALUE-000 VALUE-001)
      (COMPUTE-CONTEXT-OR-MODULE-FROM-PATHNAME NAME TRUE TRUE))
     (CL:PROGN (CL:SETQ MODULE VALUE-000) (CL:SETQ BARENAMESTART VALUE-001)))
    (NO-SUCH-CONTEXT-EXCEPTION ()
     (CL:WHEN (DEFINED? BARENAMESTART)
      (CL:LET*
       ((MODULENAME
         (QUALIFIED-NAME-BARE-NAME (QUALIFIED-NAME-MODULE-NAME NAME))))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING MODULENAME))
       (CL:HANDLER-CASE (CL:SETQ MODULE (GET-STELLA-MODULE MODULENAME FALSE))
        (NO-SUCH-CONTEXT-EXCEPTION ()
         (CL:SETQ MODULE (GET-STELLA-MODULE (STRING-UPCASE MODULENAME) FALSE))))
       (CL:WHEN WARN?
...
(CL:DEFUN INTERN-QUALIFIED-NAME (NAME ENABLE-CASE-CONVERSION?)
  (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
  #+MCL
  (CL:CHECK-TYPE NAME CL:SIMPLE-STRING)
  (CL:LET* ((MODULE NULL) (BARENAME STELLA::NULL-STRING))
   ...
   (CL:WHEN (CL:EQ MODULE NULL)
    (CL:IF
     (CL:OR (CL:EQ BARENAME STELLA::NULL-STRING) (SURROGATE-NAME? BARENAME))
     (CL:RETURN-FROM INTERN-QUALIFIED-NAME SGT-MODULES-STELLA-UNKNOWN-SURROGATE)
     (CL:RETURN-FROM INTERN-QUALIFIED-NAME SYM-MODULES-STELLA-UNKNOWN-SYMBOL)))
   (CL:WHEN (STRING-EQL? BARENAME "")
    (CL:IF (CL:OR (CL:EQ MODULE *MODULE*) (CL:EQ MODULE *ROOT-MODULE*))
     ...
     (CL:PROGN (CL:WARN "Empty symbol name encountered")
      (CL:RETURN-FROM INTERN-QUALIFIED-NAME SYM-MODULES-STELLA-UNKNOWN-SYMBOL))))
   (CL:WHEN (CL:AND ENABLE-CASE-CONVERSION? (CL:NOT (%CASE-SENSITIVE? MODULE)))
    (CL:SETQ BARENAME (STRING-UPCASE BARENAME)))
   (CL:IF (SURROGATE-NAME? BARENAME)
    (CL:RETURN-FROM INTERN-QUALIFIED-NAME
     (INTERN-RIGID-SYMBOL-WRT-MODULE (SUBSEQUENCE BARENAME 1 NULL-INTEGER) MODULE
      SURROGATE-SYM))
    (CL:RETURN-FROM INTERN-QUALIFIED-NAME
...
if no context is found, otherwise silently return `null'."
  ...
  (CL:IF ERROR?
   (CL:RETURN-FROM GET-STELLA-CONTEXT
    (HELP-GET-STELLA-CONTEXT-OR-MODULE PATHNAME FALSE))
   (CL:HANDLER-CASE
    (CL:RETURN-FROM GET-STELLA-CONTEXT
     (HELP-GET-STELLA-CONTEXT-OR-MODULE PATHNAME FALSE))
...
if no such module exists.  The search looks at ancestors and top-most
 ...
 module is found."
  ...
  (CL:LET* ((CONTEXT NULL))
   (CL:IF ERROR?
    (CL:SETQ CONTEXT (HELP-GET-STELLA-CONTEXT-OR-MODULE PATHNAME TRUE))
    (CL:HANDLER-CASE
     (CL:SETQ CONTEXT (HELP-GET-STELLA-CONTEXT-OR-MODULE PATHNAME TRUE))
     (NO-SUCH-CONTEXT-EXCEPTION () (CL:RETURN-FROM GET-STELLA-MODULE NULL))))
   (CL:COND
    ((ISA? CONTEXT SGT-MODULES-STELLA-MODULE)
     (CL:RETURN-FROM GET-STELLA-MODULE CONTEXT))
    (ERROR?
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Context `" CONTEXT
       "' is not a MODULE!")
      (CL:ERROR (NEW-NO-SUCH-CONTEXT-EXCEPTION (THE-STRING-READER STREAM-000)))))
    (CL:T (CL:RETURN-FROM GET-STELLA-MODULE NULL)))))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NAMESPEC)))
   (CL:COND
    ((SUBTYPE-OF-STRING? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM COERCE-TO-MODULE-NAME (%WRAPPER-VALUE NAMESPEC))))
    ...
    ((SUBTYPE-OF-KEYWORD? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM COERCE-TO-MODULE-NAME (%SYMBOL-NAME NAMESPEC))))
     ...
     (CL:PROGN
      (CL:LET* ((MODULE (%HOME-CONTEXT NAMESPEC)))
       (CL:COND
        ...
        (CL:T (CL:RETURN-FROM COERCE-TO-MODULE-NAME (VISIBLE-NAME NAMESPEC)))))))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:T) CL:SIMPLE-STRING) COERCE-TO-MODULE-NAME))
     ...
     (CL:LET* ((OTHERSUPERMODULE NULL) (ITER-001 (%THE-CONS-LIST PARENTS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       ...
       (CL:WHEN (CL:NOT (CL:EQ OTHERSUPERMODULE SUPERMODULE))
        (CL:WHEN (MEMBER? (%ALL-SUPER-CONTEXTS SUPERMODULE) OTHERSUPERMODULE)
...
(CL:DEFMETHOD UNFINALIZE-MODULE ((SELF MODULE))
  ...
  (CL:WHEN (DEFINED? (%CONTEXT-NUMBER SELF))
...
(CL:DEFUN HELP-DESTROY-MODULE (SELF)
  (CL:LET*
   ((CHILD NULL) (ITER-000 (%THE-CONS-LIST (COPY (%CHILD-CONTEXTS SELF)))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
  ...
  (CL:LET* ((PARENTMODULE NULL) (BARENAME STELLA::NULL-STRING))
   ...
   (CL:IF (QUALIFIED-NAME? NAME)
    (CL:PROGN
     ...
     (CL:WHEN (CL:EQ PARENTMODULE NULL)
      (CL:WARN "Bad path name `~A' when defining the module `~A'" NAME BARENAME)
   ...
   (CL:LET*
    ((PLIST
      (VET-OPTIONS SELF-000
       (GET-QUOTED-TREE
        "((:NICKNAME :INCLUDES :USES :DOCUMENTATION :SHADOW :CASE-SENSITIVE? :LISP-PACKAGE :CPP-PACKAGE :JAVA-PACKAGE :JAVA-CATCHALL-CLASS :API? :CLEARABLE? :PROTECT-SURROGATES? :CODE-ONLY?) \"/STELLA\")"
        "/STELLA"))))
         ...
         (CL:LET* ((TEST-VALUE-001 KEY))
          (CL:COND
           ...
           ((CL:EQ TEST-VALUE-001 KWD-MODULES-JAVA-PACKAGE)
            (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
             SYM-MODULES-STELLA-JAVA-PACKAGE (WRAP-STRING (%WRAPPER-VALUE VALUE))
             NULL-STRING-WRAPPER))
...
(CL:DEFUN UNDEFINE-MODULE (OLDMODULE NEWMODULE)
  (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Redefining the module `"
   (CONTEXT-NAME OLDMODULE) "'" EOL)
  (CL:COND
   ...
   ((CL:EQ *SUBCONTEXT-REVISION-POLICY* KWD-MODULES-CLEAR)
    (CL:LET*
     ((C NULL) (ITER-000 (ALL-SUBCONTEXTS OLDMODULE KWD-MODULES-PREORDER)))
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ C (%VALUE ITER-000))
      (CLEAR-CONTEXT C))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" *SUBCONTEXT-REVISION-POLICY*
      "' is not a valid case option")
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE C)))
     (CL:COND
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-MODULES-STELLA-MODULE)
       (CL:PROGN
        (SUBSTITUTE (%THE-CONS-LIST (%PARENT-MODULES C)) NEWMODULE OLDMODULE)))
  ...
  (CL:LET* ((*SHADOWEDSURROGATES* NIL))
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ OLDMODULE NULL))
     (CL:WHEN (IDENTICAL-MODULE-SIGNATURES? OLDMODULE MODULE STELLA::NULL-STRING)
...
(CL:DEFUN IDENTICAL-MODULE-SIGNATURES? (OLDMODULE NEWMODULE NEWOPTIONS)
  ...
  (CL:WHEN
   (CL:AND (CL:NOT (CL:EQ NEWMODULE NULL))
    (CL:NOT
     (STRING-EQL? (%MODULE-FULL-NAME OLDMODULE) (%MODULE-FULL-NAME NEWMODULE))))
  ...
  (CL:LET* ((OLDOPTIONS (%STRINGIFIED-OPTIONS OLDMODULE)))
   ...
   (CL:SETQ NEWOPTIONS
    (CL:IF (CL:EQ NEWMODULE NULL) NEWOPTIONS (%STRINGIFIED-OPTIONS NEWMODULE)))
      ...
      (CL:LET* ((SELF-001 (NEW-PROPERTY-LIST)))
       ...
       (CL:LET* ((NEWPLIST SELF-001)) (REMOVE-AT OLDPLIST KWD-MODULES-INCLUDES)
        ...
        (CL:WHEN (CL:NOT (CL:= (LENGTH OLDPLIST) (LENGTH NEWPLIST)))
         (CL:RETURN-FROM IDENTICAL-MODULE-SIGNATURES? FALSE))
        (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST OLDPLIST)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          (CL:SETQ KEY (%%VALUE ITER-000))
          (CL:PROGN (CL:SETQ VALUE (%%VALUE (%%REST ITER-000)))
           (CL:SETQ ITER-000 (%%REST (%%REST ITER-000))))
...
Optionally, specify a name."
  ...
  (CL:LET* ((WORLD (NEW-WORLD)))
   (CL:WHEN (CL:NOT (CL:EQ NAME STELLA::NULL-STRING))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS WORLD) SYM-MODULES-STELLA-WORLD-NAME
     (WRAP-STRING NAME) NULL-STRING-WRAPPER)
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET*
      ((SIBLING NULL)
...
if there is no current world."
  (CL:WHEN (CL:NOT (CL:EQ *CONTEXT* *MODULE*))
   (CL:LET* ((WORLD *CONTEXT*)) (CL:SETQ *CONTEXT* (%PARENT-CONTEXT WORLD))
    (DESTROY-WORLD WORLD)))
...
(CL:DEFUN ALL-SUBCONTEXTS-NEXT? (SELF)
  (CL:LET*
   ...
   (CL:LET*
    ((CHILD NULL) (ITER-000 (%THE-CONS-LIST (%CHILD-CONTEXTS THISCONTEXT)))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CHILD (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
   ...
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF) SGT-MODULES-STELLA-MODULE)
    (CL:PROGN
     ...
     (CL:LET* ((VARIABLE NULL) (ITER-001 (ALL-VARIABLES SELF TRUE)))
      (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ VARIABLE (%VALUE ITER-001))
     ...
     (CL:WHEN (CL:NOT (PROTECT-SURROGATES? SELF))
      (CL:LET* ((SYMBOL NULL) (ITER-004 (ALL-SYMBOLS SELF TRUE)))
       (CL:LOOP WHILE (NEXT? ITER-004) DO (CL:SETQ SYMBOL (%VALUE ITER-004))
        (UNINTERN-SYMBOL SYMBOL))))
     (CL:LET* ((*MODULE* SELF)) (CL:DECLARE (CL:SPECIAL *MODULE*))
      (CL:LET* ((IT (ALLOCATE-ITERATOR (SHADOWED-SURROGATES SELF))))
       (CL:LOOP WHILE (NEXT? IT) DO
        (VALUE-SETTER IT (SHADOW-SURROGATE (%SYMBOL-NAME (%VALUE IT)))))))))
   (CL:T))
  :VOID)
...
against accidental clearing."
  (CL:LET* ((ARGLIST NAME) (MODULE (COERCE-TO-MODULE (%%VALUE ARGLIST) TRUE)))
...
(CL:DEFUN HELP-STARTUP-MODULES1 ()
  (CL:PROGN
   (CL:SETQ KWD-MODULES-CLEAR (INTERN-RIGID-SYMBOL-WRT-MODULE "CLEAR" NULL 2))
   ...
   (CL:SETQ SYM-MODULES-STELLA-DEFINE-MODULE-FROM-STRINGIFIED-SOURCE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DEFINE-MODULE-FROM-STRINGIFIED-SOURCE" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-MODULES2 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "INTERN-QUALIFIED-NAME"
    "(DEFUN (INTERN-QUALIFIED-NAME GENERALIZED-SYMBOL) ((NAME STRING) (ENABLE-CASE-CONVERSION? BOOLEAN)))"
...
'moduleName'.\" :PUBLIC? TRUE)" (CL:FUNCTION CHANGE-MODULE) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "FINALIZE-MODULE"
    "(DEFUN FINALIZE-MODULE ((SELF MODULE)))" (CL:FUNCTION FINALIZE-MODULE) NULL)
   (DEFINE-FUNCTION-OBJECT "FINALIZE-WORLD"
    "(DEFUN FINALIZE-WORLD ((SELF WORLD)))" (CL:FUNCTION FINALIZE-WORLD) NULL)
...
all contexts that inherit 'self'.\" :PUBLIC? TRUE)" (CL:FUNCTION DESTROY-MODULE)
    NULL)
...
all contexts that inherit 'self'.\" :PUBLIC? TRUE)" (CL:FUNCTION DESTROY-CONTEXT)
    NULL)
   (DEFINE-METHOD-OBJECT
...
context to be its parent.  Return the current context. Nothing happens
if there is no current world.\" :PUBLIC? TRUE)" (CL:FUNCTION POP-WORLD) NULL)
...
'self' (not including 'self') in the order specified by 'traversal' (one
of :preorder, :inorder, or :postorder).\")" (CL:FUNCTION ALL-SUBCONTEXTS) NULL)
...
will start with the module `from'.\")" (CL:FUNCTION VISIBLE-MODULES) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "CLEAR-ONE-CONTEXT"
     "(DEFUN CLEAR-ONE-CONTEXT ((SELF CONTEXT)))" (CL:FUNCTION CLEAR-ONE-CONTEXT)
     NULL)
...
'module' argument.\")")
    (REGISTER-NATIVE-NAME SYM-MODULES-STELLA-DEFMODULE KWD-MODULES-COMMON-LISP
     KWD-MODULES-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-MODULES-STELLA-IN-MODULE KWD-MODULES-COMMON-LISP
     KWD-MODULES-FUNCTION)
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS *ROOT-MODULE*)
     SYM-MODULES-STELLA-CLEARABLE? (CL:IF FALSE TRUE-WRAPPER FALSE-WRAPPER)
     FALSE-WRAPPER)
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS *COMMON-LISP-MODULE*)
     SYM-MODULES-STELLA-CLEARABLE? (CL:IF FALSE TRUE-WRAPPER FALSE-WRAPPER)
     FALSE-WRAPPER)
    (REGISTER-NATIVE-NAME SYM-MODULES-STELLA-CLEAR-MODULE KWD-MODULES-COMMON-LISP
     KWD-MODULES-FUNCTION)))
  :VOID)
