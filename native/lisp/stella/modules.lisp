...
(CL:DECLAIM
 (CL:SPECIAL *CLEAR-MODULE-HOOKS* STANDARD-OUTPUT FALSE-WRAPPER
  NULL-STRING-WRAPPER TRUE-WRAPPER STANDARD-ERROR
  *DESTROY-CONTEXT-HOOKS* *CONTEXT-NUMBER-COUNTER*
  *CHANGE-MODULE-HOOKS* *STELLA-MODULE* SYMBOL-SYM SURROGATE-SYM EOL
  NULL-CHARACTER MODULE-SEPARATOR-CHARACTER *ROOT-MODULE* NULL-INTEGER
  *MODULE* *CONTEXT* FALSE TRUE NIL))
   ...
   (CL:HANDLER-CASE
    (CL:LET* ((VALUE-000 NULL) (VALUE-001 NULL-INTEGER))
     ...
     (CL:MULTIPLE-VALUE-SETQ (VALUE-000 VALUE-001)
      (COMPUTE-CONTEXT-OR-MODULE-FROM-PATHNAME NAME TRUE TRUE))
     (CL:PROGN (CL:SETQ MODULE VALUE-000)
      (CL:SETQ BARENAMESTART VALUE-001)))
    ...
    (CL:IF (CL:OR (CL:EQ MODULE *MODULE*) (CL:EQ MODULE *ROOT-MODULE*))
     ...
     (CL:PROGN (CL:WARN "Empty symbol name encountered")
      (CL:RETURN-FROM INTERN-QUALIFIED-NAME
       SYM-MODULES-STELLA-UNKNOWN-SYMBOL))))
   (CL:WHEN
    (CL:AND ENABLE-CASE-CONVERSION? (CL:NOT (%CASE-SENSITIVE? MODULE)))
 ...
 module is found."
  ...
  (CL:LET* ((CONTEXT NULL))
   (CL:IF ERROR?
    ...
    (CL:HANDLER-CASE
     (CL:SETQ CONTEXT
      (HELP-GET-STELLA-CONTEXT-OR-MODULE PATHNAME TRUE))
     (NO-SUCH-CONTEXT-EXCEPTION ()
      (CL:RETURN-FROM GET-STELLA-MODULE NULL))))
   (CL:COND
  ...
  (CL:LET* ((ALLSUPERCONTEXTS NIL) (SUBLIST NULL))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:SETQ ALLSUPERCONTEXTS
      (CONCATENATE SUBLIST ALLSUPERCONTEXTS))))
      ...
      (CL:T
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         ...
         (CL:LET*
          ((PARENTUSEES NULL)
           (ITER-001 (%THE-CONS-LIST (%USES PARENT))))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
...
(CL:DEFUN FINALIZE-MODULE (SELF)
  ...
  (CL:SETF (%SYMBOL-OFFSET-TABLE SELF)
          ...
          (CL:WHEN
           (CL:AND (CL:NOT (CL:EQ INCLUDEDMODULE PARENTMODULE))
            (CL:OR (CL:EQ INCLUDEDMODULE PARENTMODULE)
             (MEMB? (%ALL-SUPER-CONTEXTS INCLUDEDMODULE)
              PARENTMODULE)))
           (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
        (CL:SETQ TEST-VALUE-000 FOUND?-000)))
  ...
  (CL:LET* ((*SHADOWEDSURROGATES* NIL))
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ OLDMODULE NULL))
     (CL:WHEN
      (IDENTICAL-MODULE-SIGNATURES? OLDMODULE MODULE
       STELLA::NULL-STRING)
...
(CL:DEFUN YIELD-DEFINE-MODULE (MODULE)
  (CL:RETURN-FROM YIELD-DEFINE-MODULE
   (LIST* SYM-MODULES-STELLA-DEFINE-MODULE-FROM-STRINGIFIED-SOURCE
    ...
    (CONS (YIELD-STRING-CONSTANT-TREE (%STRINGIFIED-OPTIONS MODULE))
     NIL))))
...
all contexts that inherit 'self'.\" :PUBLIC? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "TRUE-OPTION?"
    "(DEFUN (TRUE-OPTION? BOOLEAN) ((VALUE OBJECT)) :GLOBALLY-INLINE? TRUE (RETURN (OR (EQL? VALUE (QUOTE TRUE)) (EQL? VALUE TRUE-WRAPPER))))"
...
The generated modules are generated from most- to least-specific and 
will start with the module `from'.\")" (CL:FUNCTION VISIBLE-MODULES)
     NULL)
