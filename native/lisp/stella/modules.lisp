...
(CL:DECLAIM
 (CL:SPECIAL *CLEAR-MODULE-HOOKS* STANDARD-OUTPUT FALSE-WRAPPER
  NULL-STRING-WRAPPER TRUE-WRAPPER STANDARD-ERROR EOL *DESTROY-CONTEXT-HOOKS*
  *CONTEXT-NUMBER-COUNTER* *STELLA-MODULE* *CHANGE-MODULE-HOOKS*
  *MEMOIZATION-ENABLED?* MEMOIZED-NULL-VALUE NULL-CHARACTER
  MODULE-SEPARATOR-CHARACTER *ROOT-MODULE* NULL-INTEGER *MODULE* *CONTEXT*
  NIL))
...
if no such module exists.  The search looks at ancestors and top-most
 ...
 module is found."
  ...
  (CL:CHECK-TYPE PATHNAME CL:SIMPLE-STRING)
  (CL:LET*
   ((WRAPPEDPATHNAME (CL:MAKE-INSTANCE (CL:QUOTE STRING-WRAPPER)))
    (MODULE NULL))
   ...
   (CL:LET*
    ...
    (CL:COND
     ...
     (CL:T
      (CL:SETQ MEMOIZED-VALUE-000 (HELP-GET-STELLA-MODULE PATHNAME CL:NIL))
      (CL:WHEN *MEMOIZATION-ENABLED?*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       ...
       (CL:WHEN (CL:NOT (CL:EQ OTHERSUPERMODULE SUPERMODULE))
        (CL:WHEN (MEMBER? (%ALL-SUPER-CONTEXTS SUPERMODULE) OTHERSUPERMODULE)
         ...
         (REMOVE (%CHILD-CONTEXTS OTHERSUPERMODULE) SELF)
         (CL:SETQ RECOMPUTEFULLNAME? CL:T)))
       (CL:SETQ ITER-001 (%%REST ITER-001))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:WHEN RECOMPUTEFULLNAME?
    (CL:SETF (%MODULE-FULL-NAME SELF) (COMPUTE-FULL-NAME "" SELF))))
  :VOID)
...
(CL:DEFMETHOD INHERIT-SUPERCONTEXTS ((WORLD WORLD))
  (CL:LET* ((PARENT (%PARENT-CONTEXT WORLD)))
   (CL:IF (CL:NOT (CL:EQ PARENT NULL))
    (CL:SETF (%ALL-SUPER-CONTEXTS WORLD)
...
(CL:DEFUN UNINHERIT-USED-MODULES (MODULE)
  (CL:LET* ((USEES (%USES MODULE)))
   (CL:COND ((EMPTY? USEES) (PUSH USEES MODULE))
    ((CL:= (STRING-SEARCH (%STRINGIFIED-OPTIONS MODULE) ":USES " 0)
      NULL-INTEGER)
     (CL:LET* ((USEE NULL) (ITER-000 (%THE-CONS-LIST USEES)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:SETQ USEE (%%VALUE ITER-000)) (REMOVE (%USED-BY USEE) MODULE)
       (CL:SETQ ITER-000 (%%REST ITER-000))))
     (CLEAR USEES))))
  :VOID)
         ...
         (CL:LET* ((TEST-VALUE-001 KEY))
          (CL:COND
           ((CL:EQ TEST-VALUE-001 KWD-MODULES-DOCUMENTATION)
            (CL:SETF (%DOCUMENTATION SELF) (UNWRAP-STRING VALUE)))
...
(CL:DEFUN UNDEFINE-MODULE (OLDMODULE NEWMODULE)
  ...
  (CL:IF (CARDINAL-MODULE? OLDMODULE)
   ...
   (CL:LET* ((P NULL) (ITER-001 (%THE-CONS-LIST (%PARENT-MODULES OLDMODULE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:SETQ P (%%VALUE ITER-001)) (REMOVE (%CHILD-CONTEXTS P) OLDMODULE)
     (CL:SETQ ITER-001 (%%REST ITER-001)))))
  ...
  (CL:LET* ((*SHADOWEDSURROGATES* NIL))
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ OLDMODULE NULL))
     (CL:WHEN
      (IDENTICAL-MODULE-STRUCTURE? OLDMODULE MODULE STELLA::NULL-STRING)
    ...
    (CL:WHEN (CL:NOT (CL:EQ OLDMODULE NULL))
     (CL:LET*
      ((C NULL) (ITER-000 (ALL-SUBCONTEXTS MODULE KWD-MODULES-TOPDOWN)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ C (%VALUE ITER-000))
       (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE C)))
        (CL:COND
         ((SUBTYPE-OF? TEST-VALUE-000 SGT-MODULES-STELLA-MODULE)
          (CL:PROGN (CL:WHEN (EMPTY? (%USES C)) (CL:SETF (%USES C) (LIST C)))
           (FINALIZE-MODULE C)))
         ((SUBTYPE-OF? TEST-VALUE-000 SGT-MODULES-STELLA-WORLD)
...
(CL:DEFUN UPDATE-NON-STRUCTURAL-MODULE-OPTIONS (OLDMODULE NEWMODULE NEWOPTIONS)
  ...
  (CL:WHEN (CL:EQ NEWOPTIONS STELLA::NULL-STRING)
   (CL:SETQ NEWOPTIONS (%STRINGIFIED-OPTIONS NEWMODULE)))
  (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST)))
   (CL:SETF (%THE-PLIST SELF-000)
    (READ-S-EXPRESSION-FROM-STRING (%STRINGIFIED-OPTIONS OLDMODULE)))
    ...
    (CL:LET* ((SELF-001 (NEW-PROPERTY-LIST)))
     ...
     (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        ...
        (CL:WHEN (CL:NOT (EQL? OLDVALUE NEWVALUE))
         (CL:IF (CL:NOT (CL:EQ NEWVALUE NULL))
          ...
          (CL:PROGN
           (CL:WHEN (CL:EQ NEWMODULE NULL) (CL:SETQ NEWMODULE (NEW-MODULE)))
      ...
      (CL:WHEN (CL:NOT (CL:EQ CHANGED-VALUES NIL))
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Updating module `"
        OLDMODULE "'" EOL)
       (INCORPORATE-MODULE-OPTIONS OLDMODULE CHANGED-VALUES)
       (CL:SETF (%STRINGIFIED-OPTIONS OLDMODULE) NEWOPTIONS))))))
  :VOID)
...
(CL:DEFUN ALL-SUBCONTEXTS-NEXT? (SELF)
  (CL:LET*
   ...
   (CL:LET*
    ((CHILD NULL) (ITER-001 (%THE-CONS-LIST (%CHILD-CONTEXTS THISCONTEXT)))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:SETQ CHILD (%%VALUE ITER-001))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ BEENTHERE NULL) (CL:SETQ BEENTHERE (NEW-HASH-TABLE))
    ...
    (CL:LET*
     ((SUB NULL)
      (ITER-000
       (ALL-SUBCONTEXTS (%ITERATOR-OBJECT SELF) KWD-MODULES-PREORDER)))
   ...
   (CL:IF (CL:EQ REMAININGCONTEXTS NIL)
    (CL:PROGN (CL:SETF (%VALUE SELF) NULL)
     (CL:RETURN-FROM ALL-TOPDOWN-SUBCONTEXTS-NEXT? CL:NIL))
    (CL:SETQ REMAININGCONTEXTS (%%REST REMAININGCONTEXTS)))
   (INSERT-AT BEENTHERE THISCONTEXT TRUE-WRAPPER)
   (CL:LET*
    ((CHILD NULL) (ITER-002 (%THE-CONS-LIST (%CHILD-CONTEXTS THISCONTEXT)))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
     ...
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ M (%VALUE ITER-000))
      (CL:WHEN
       (CL:AND
        (CL:NOT
         (%WRAPPER-VALUE
          (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS M) SYM-MODULES-STELLA-CODE-ONLY?
           FALSE-WRAPPER)))
        (CL:NOT
         (%WRAPPER-VALUE
          (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS M) SYM-MODULES-STELLA-NAMESPACE?
           FALSE-WRAPPER))))
       (CL:IF (CL:EQ COLLECT-000 NULL)
...
(CL:DEFUN LIST-MODULES-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:RETURN-FROM LIST-MODULES-EVALUATOR-WRAPPER
   (%LIST-MODULES (%WRAPPER-VALUE (%%VALUE ARGUMENTS)))))
...
least-specific and will start with the module `from'."
  ...
  (CL:LET*
   ...
   (CL:WHEN *MEMOIZATION-ENABLED?*
    (CL:SETQ MEMO-TABLE-000
     (%SURROGATE-VALUE SGT-MODULES-STELLA-F-VISIBLE-MODULES-MEMO-TABLE-000))
    (CL:WHEN (CL:EQ MEMO-TABLE-000 NULL)
     (INITIALIZE-MEMOIZATION-TABLE
     ...
     (CL:SETQ MEMO-TABLE-000
      (%SURROGATE-VALUE SGT-MODULES-STELLA-F-VISIBLE-MODULES-MEMO-TABLE-000)))
    (CL:SETQ MEMOIZED-ENTRY-000
     (LOOKUP-MRU-MEMOIZED-VALUE MEMO-TABLE-000 FROM MEMOIZED-NULL-VALUE NULL
      NULL -1))
    (CL:SETQ MEMOIZED-VALUE-000 (%%VALUE MEMOIZED-ENTRY-000)))
   (CL:COND
    ((CL:NOT (CL:EQ MEMOIZED-VALUE-000 NULL))
...
(CL:DEFUN HELP-STARTUP-MODULES1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-MODULES-STELLA-F-GET-STELLA-MODULE-MEMO-TABLE-000
    (INTERN-RIGID-SYMBOL-WRT-MODULE "F-GET-STELLA-MODULE-MEMO-TABLE-000" NULL
     1))
   ...
   (CL:SETQ SGT-MODULES-STELLA-F-VISIBLE-MODULES-MEMO-TABLE-000
    (INTERN-RIGID-SYMBOL-WRT-MODULE "F-VISIBLE-MODULES-MEMO-TABLE-000" NULL
     1))
...
'moduleName'.\" :PUBLIC? TRUE)" (CL:FUNCTION CHANGE-MODULE) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INHERIT-SUPERCONTEXTS ((WORLD WORLD)))"
    (CL:FUNCTION INHERIT-SUPERCONTEXTS) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INHERIT-SUPERCONTEXTS ((MODULE MODULE)))"
    (CL:FUNCTION INHERIT-SUPERCONTEXTS) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-FUNCTION-OBJECT "FINALIZE-MODULE"
    "(DEFUN FINALIZE-MODULE ((SELF MODULE)))" (CL:FUNCTION FINALIZE-MODULE)
    NULL)
...
is `true', then any modules which are code only or just namespaces are not returned.\")"
     (CL:FUNCTION %LIST-MODULES) (CL:FUNCTION LIST-MODULES-EVALUATOR-WRAPPER))
