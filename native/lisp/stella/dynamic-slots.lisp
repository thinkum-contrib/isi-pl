  ...
  (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-KV-LIST DYNAMICSLOTS)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
    (CL:WHEN (CL:EQ KEY SLOTNAME)
     (CL:IF (CL:EQ VALUE NULL) (CL:RETURN-FROM DYNAMIC-SLOT-VALUE NULLVALUE)
...
(CL:DEFUN SET-DYNAMIC-SLOT-VALUE (DYNAMICSLOTS SLOTNAME VALUE NULLWRAPPER)
  (CL:LET*
   ((DELETEVALUE? (CL:AND (CL:EQ VALUE NULLWRAPPER) (CL:NOT (CL:EQ NULLWRAPPER FALSE-WRAPPER))))
    (CURSOR (%THE-KV-LIST DYNAMICSLOTS)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
    (CL:WHEN (CL:EQ (%KEY CURSOR) SLOTNAME)
     (CL:IF DELETEVALUE? (REMOVE-AT DYNAMICSLOTS SLOTNAME) (CL:SETF (%VALUE CURSOR) VALUE))
     (CL:RETURN-FROM SET-DYNAMIC-SLOT-VALUE VALUE))
    (CL:SETQ CURSOR (%REST CURSOR)))
   (CL:WHEN (CL:NOT DELETEVALUE?)
    (CL:SETF (%THE-KV-LIST DYNAMICSLOTS) (KV-CONS SLOTNAME VALUE (%THE-KV-LIST DYNAMICSLOTS))))
   (CL:RETURN-FROM SET-DYNAMIC-SLOT-VALUE VALUE)))
  ...
  (CL:RETURN-FROM STANDARD-DYNAMIC-SLOT-ACCESS?
   (CL:AND (CL:NOT (ACTIVE? SLOT)) (CL:EQ (ALLOCATION SLOT) KWD-DYNAMIC-SLOTS-DYNAMIC)
    (CL:OR (CL:NOT (SLOT-VALUE-IS-BARE-LITERAL? SLOT))
     (CL:NOT (CL:EQ (LOOKUP-LITERAL-TYPE-INFO (%SLOT-BASE-TYPE SLOT) KWD-DYNAMIC-SLOTS-NULL-WRAPPER) NULL))))))

;;; (DEFUN (YIELD-DOWN-CAST-TREE CONS) ...)

(CL:DEFUN YIELD-DOWN-CAST-TREE (TREE RETURNTYPE)
  (CL:LET* ((OTREE (SYS-TREE TREE RETURNTYPE)))
   (CL:WHEN (CL:NOT (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-DYNAMIC-SLOTS-COMMON-LISP))
     ...
     (LIST* SYM-DYNAMIC-SLOTS-STELLA-WRAPPER-VALUE
      (LIST* SYM-DYNAMIC-SLOTS-STELLA-SAFE-CAST
       (LIST* SYM-DYNAMIC-SLOTS-STELLA-DYNAMIC-SLOT-VALUE (LIST* SYM-DYNAMIC-SLOTS-STELLA-DYNAMIC-SLOTS OBJECTREF NIL)
        ...
        (LOOKUP-LITERAL-TYPE-INFO (TYPE SLOT) KWD-DYNAMIC-SLOTS-NULL-WRAPPER) NIL)
...
(CL:DEFUN YIELD-STANDARD-DYNAMIC-SLOT-VALUE-SETTER-TREE (SLOT OBJECTREF VALUEREF RETURNTYPE)
  (CL:LET*
   ((SLOTNAME (%SLOT-NAME (CANONICAL-SLOT SLOT))) (LITERALSLOT? (SLOT-VALUE-IS-BARE-LITERAL? SLOT)) (SETTERTREE NULL))
   (CL:IF LITERALSLOT?
    (CL:SETQ SETTERTREE
     (LIST* SYM-DYNAMIC-SLOTS-STELLA-SET-DYNAMIC-SLOT-VALUE
      (LIST* SYM-DYNAMIC-SLOTS-STELLA-DYNAMIC-SLOTS OBJECTREF NIL) (LIST* SYM-DYNAMIC-SLOTS-STELLA-BQUOTE SLOTNAME NIL)
      VALUEREF (CONS (LOOKUP-LITERAL-TYPE-INFO (TYPE SLOT) KWD-DYNAMIC-SLOTS-NULL-WRAPPER) NIL)))
    (CL:SETQ SETTERTREE
     (LIST* SYM-DYNAMIC-SLOTS-STELLA-SET-DYNAMIC-SLOT-VALUE
      (LIST* SYM-DYNAMIC-SLOTS-STELLA-DYNAMIC-SLOTS OBJECTREF NIL) (LIST* SYM-DYNAMIC-SLOTS-STELLA-BQUOTE SLOTNAME NIL)
      VALUEREF (CONS SYM-DYNAMIC-SLOTS-STELLA-NULL NIL))))
...
(CL:DEFUN STARTUP-DYNAMIC-SLOTS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-DYNAMIC-SLOTS-DYNAMIC (INTERN-RIGID-SYMBOL-WRT-MODULE "DYNAMIC" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "YIELD-DOWN-CAST-TREE"
     "(DEFUN (YIELD-DOWN-CAST-TREE CONS) ((TREE CONS) (RETURNTYPE TYPE-SPEC)))" (CL:FUNCTION YIELD-DOWN-CAST-TREE)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-DYNAMIC-SLOTS" "(DEFUN STARTUP-DYNAMIC-SLOTS () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-DYNAMIC-SLOTS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-DYNAMIC-SLOTS-STELLA-STARTUP-DYNAMIC-SLOTS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-DYNAMIC-SLOTS-STELLA-METHOD-STARTUP-CLASSNAME
