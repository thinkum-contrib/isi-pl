...
(CL:DEFMETHOD LAST ((SELF VECTOR-SEQUENCE))
  ...
  (CL:RETURN-FROM LAST
   (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF))
    (CL:THE CL:FIXNUM (CL:1- (%SEQUENCE-LENGTH SELF))))))
...
(CL:DEFMETHOD LAST-SETTER ((SELF VECTOR-SEQUENCE) VALUE)
  (CL:RETURN-FROM LAST-SETTER
   (CL:LET
    ((SELF (%THE-ARRAY SELF)) (VALUE VALUE)
     (POSITION (CL:1- (%SEQUENCE-LENGTH SELF))))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
Resize the array if necessary."
  (CL:LET* ((OLDLENGTH (%SEQUENCE-LENGTH SELF)))
   ...
   (CL:WHEN (CL:= OLDLENGTH (%ARRAY-SIZE SELF))
    (RESIZE-VECTOR SELF (CL:* (%ARRAY-SIZE SELF) 2)))
...
Resize the array if necessary."
  (CL:LET* ((OLDLENGTH (%SEQUENCE-LENGTH SELF)))
   ...
   (CL:WHEN (CL:= OLDLENGTH (%ARRAY-SIZE SELF))
    ...
    (RESIZE-VECTOR SELF
     (CL:TRUNCATE (CL:* (%ARRAY-SIZE SELF) (%RESIZE-FACTOR SELF)))))
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION OLDLENGTH))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))
   (CL:SETF (%SEQUENCE-LENGTH SELF) (CL:1+ OLDLENGTH)))
...
(CL:DEFMETHOD REVERSE ((SELF VECTOR-SEQUENCE))
  ...
  (CL:LET*
   ((ARRAY (%THE-ARRAY SELF)) (BOTTOM 0) (TOP (CL:1- (%SEQUENCE-LENGTH SELF)))
    (ELT NULL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY)
...
(CL:DEFUN COPY-VECTOR-SEQUENCE (SOURCE COPY)
  (CL:LET*
   ((SOURCEARRAY (%THE-ARRAY SOURCE)) (COPYARRAY (%THE-ARRAY COPY))
...
(CL:DEFMETHOD COPY ((SELF VECTOR-SEQUENCE))
  ...
  (CL:LET* ((COPY (NEW-VECTOR-SEQUENCE (%ARRAY-SIZE SELF))))
...
(CL:DEFMETHOD COPY ((SELF CUSTOM-VECTOR-SEQUENCE))
  ...
  (CL:LET* ((COPY (NEW-CUSTOM-VECTOR-SEQUENCE (%ARRAY-SIZE SELF))))
   (CL:SETF (%RESIZE-FACTOR COPY) (%RESIZE-FACTOR SELF))
   (COPY-VECTOR-SEQUENCE SELF COPY) (CL:RETURN-FROM COPY COPY)))
   ...
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-VECTOR-SEQUENCE)
    (CL:PROGN
     (CL:WHEN (CL:= (%SEQUENCE-LENGTH X) (%SEQUENCE-LENGTH Y))
      (CL:LET*
       ((ELTX NULL) (VECTOR-000 X) (INDEX-000 0)
        (LENGTH-000 (%SEQUENCE-LENGTH VECTOR-000)) (ELTY NULL) (VECTOR-001 Y)
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF VECTOR))
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   ...
   (CL:SETF (%ITERATOR-SECOND-INTEGER ITERATOR) (LENGTH SELF))
  ...
  (CL:LET* ((LENGTH (LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:COND ((CL:= LENGTH 0) (CL:RETURN-FROM SORT-TUPLES SELF))
    ((CL:EQ PREDICATE NULL)
     (CL:SETQ PREDICATE
      (CHOOSE-SORT-PREDICATE
       (NTH (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0) N)))))
   (CL:LET*
...
(CL:DEFUN HEAP-SORT-SIFT-DOWN (VECTOR START END PREDICATE)
  ...
  (CL:LET* ((ROOT START) (CHILD 0) (AUX NULL))
   ...
   (CL:LOOP (CL:SETQ CHILD (CL:1+ (CL:THE CL:FIXNUM (CL:ASH ROOT 1))))
    ...
    (CL:COND
     ((CL:FUNCALL PREDICATE (CL:AREF VECTOR ROOT) (CL:AREF VECTOR CHILD))
    ...
    (CL:COND
     ((CL:<
       (CL:LET ((ARG (CL:+ START END)) (COUNT 1))
        ...
        (CL:THE CL:FIXNUM (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))
       HIGHCURSOR)
      (QUICK-SORT-NATIVE-VECTOR VECTOR (CL:1+ HIGHCURSOR) END PREDICATE)
...
(CL:DEFCLASS HEAP (VECTOR)
  ((PREDICATE :ALLOCATION :INSTANCE :ACCESSOR %PREDICATE)
   (FILL-POINTER :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE
...
(CL:DEFUN NEW-HEAP (PREDICATE ARRAY-SIZE)
  ...
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE HEAP)))
   (CL:SETF (%PREDICATE SELF) PREDICATE)
   (CL:SETF (%ARRAY-SIZE SELF) ARRAY-SIZE)
   (CL:SETF (%THE-ARRAY SELF) STELLA::NULL-NATIVE-VECTOR)
   (CL:SETF (%FILL-POINTER SELF) 0) (INITIALIZE-VECTOR SELF)
...
(CL:DEFUN HEAP-SIFT-DOWN (HEAP START END VALUE PREDICATE)
  ...
  (CL:CHECK-TYPE END CL:FIXNUM)
  (CL:LET*
   ((PARENT START) (CHILD NULL-INTEGER) (LEFTCHILD NULL) (RIGHTCHILD NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM PARENT CHILD))
   ...
   (CL:LOOP (CL:SETQ CHILD (CL:1+ (CL:THE CL:FIXNUM (CL:ASH PARENT 1))))
    (CL:COND ((CL:> CHILD END) (CL:RETURN))
     ((CL:= CHILD END)
      (CL:WHEN (CL:NOT (CL:FUNCALL PREDICATE (CL:AREF HEAP CHILD) VALUE))
       (CL:RETURN)))
      ...
      (CL:COND
       ((CL:FUNCALL PREDICATE LEFTCHILD VALUE)
        (CL:WHEN
         ...
         (CL:SETQ CHILD (CL:1+ CHILD))))
       ((CL:FUNCALL PREDICATE RIGHTCHILD VALUE) (CL:SETQ CHILD (CL:1+ CHILD)))
       (CL:T (CL:RETURN)))))
...
a Min-heap if `self's `predicate' has `<' semantics; otherwise as a Max-heap."
  (CL:LET*
   ((VALUES (%THE-ARRAY SELF)) (FILLPOINTER (%FILL-POINTER SELF))
    ...
    ((CL:= FILLPOINTER (%ARRAY-SIZE SELF))
     (CL:WHEN
      (CL:FUNCALL PREDICATE
       (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0) VALUE)
      (HEAP-SIFT-DOWN VALUES 0 FILLPOINTER VALUE PREDICATE)))
...
insertion of values or if `predicate' has been changed."
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES)
    (CL:TYPE CL:FIXNUM FILLPOINTER))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- FILLPOINTER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
...
the same."
  (CL:LET*
   ((VALUES (%THE-ARRAY SELF)) (CURSOR (%FILL-POINTER SELF)) (VALUE NULL))
   ...
   (CL:COND ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (%PREDICATE SELF)))
    ((CL:NOT (CL:EQL PREDICATE (%PREDICATE SELF)))
     (CL:SETF (%PREDICATE SELF) PREDICATE) (HEAPIFY SELF)))
   ...
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0))
    ...
    (CL:LOOP WHILE CL:T DO (CL:SETQ I ITER-000)
     (CL:SETQ CURSOR (CL:1- CURSOR)) (CL:WHEN (CL:>= I CURSOR) (CL:RETURN))
  ...
  (CL:LET* ((CHOOSE-VALUE-000 NULL))
   (CL:IF (ISA? SELF SGT-COLLECTIONS-STELLA-HASH-SET)
    ...
    (CL:LET* ((SELF-000 (NEW-HASH-SET)))
     (CL:SETF (%EQUAL-TEST? SELF-000) EQUALTEST?)
      ...
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-COLLECTIONS-STELLA-COLLECTION)
       (CL:PROGN
        (CL:LET* ((ELT NULL) (ITER-002 (ALLOCATE-ITERATOR SELF)))
         (CL:LOOP WHILE (NEXT? ITER-002) DO (CL:SETQ ELT (%VALUE ITER-002))
...
(CL:DEFUN NEW-1D-ARRAY (DIM1)
  ...
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 1D-ARRAY)))
   (CL:SETF (%DIM1 SELF) DIM1)
...
and return the result."
  ...
  (CL:RETURN-FROM 1D-AREF-SETTER
   (CL:SETF (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) I) VALUE)))
...
(CL:DEFMETHOD INITIALIZE-ARRAY ((SELF 1D-ARRAY) INITIALVALUE)
  ...
  (CL:LET* ((ARRAY (%THE-ARRAY SELF)))
   ...
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:SETF (CL:AREF ARRAY I) INITIALVALUE)
     (CL:SETQ ITER-000 (CL:1+ ITER-000)))))
  :VOID)
...
(CL:DEFUN NEW-1D-FLOAT-ARRAY (DIM1)
  ...
  (CL:CHECK-TYPE DIM1 CL:FIXNUM)
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 1D-FLOAT-ARRAY)))
   (CL:SETF (%DIM1 SELF) DIM1)
   (CL:SETF (%THE-ARRAY SELF) STELLA::NULL-1D-ARRAY)
...
(CL:DEFUN NEW-2D-ARRAY (DIM2 DIM1)
  ...
  #+MCL
  (CL:CHECK-TYPE DIM1 CL:FIXNUM)
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 2D-ARRAY)))
   (CL:SETF (%DIM2 SELF) DIM2) (CL:SETF (%DIM1 SELF) DIM1)
   (CL:SETF (%THE-ARRAY SELF) STELLA::NULL-1D-ARRAY)
   (INITIALIZE-DIMENSIONAL-ARRAY SELF) (CL:RETURN-FROM NEW-2D-ARRAY SELF)))
...
(CL:DEFUN NEW-2D-FLOAT-ARRAY (DIM2 DIM1)
  ...
  (CL:CHECK-TYPE DIM1 CL:FIXNUM)
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE 2D-FLOAT-ARRAY)))
   (CL:SETF (%DIM2 SELF) DIM2) (CL:SETF (%DIM1 SELF) DIM1)
   (CL:SETF (%THE-ARRAY SELF) STELLA::NULL-1D-ARRAY)
   (INITIALIZE-DIMENSIONAL-ARRAY SELF)
...
equal test if `equalTest?' is TRUE (`equalTest?' will be ignored if `self'
already is a HASH-SET).\" :PUBLIC? TRUE)" (CL:FUNCTION COERCE-TO-HASH-SET)
    NULL)
   (DEFINE-METHOD-OBJECT
