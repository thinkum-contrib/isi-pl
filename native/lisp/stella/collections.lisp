...
(CL:DECLAIM (CL:SPECIAL *STELLA-MODULE* *MODULE* TRUE-WRAPPER FALSE-WRAPPER *INTEGER-MSB-MASK* NULL-INTEGER NIL))
...
(CL:DEFMETHOD MEMBER? ((SELF COLLECTION) OBJECT)
  ...
  (CL:LET* ((I NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ I (%VALUE ITER-000)) (CL:WHEN (EQL? I OBJECT) (CL:RETURN-FROM MEMBER? CL:T))))
...
(CL:DEFUN STELLA-COLLECTION? (SELF)
  ...
  (CL:RETURN-FROM STELLA-COLLECTION? (ISA? SELF SGT-COLLECTIONS-STELLA-COLLECTION)))
...
(CL:DEFMETHOD INSERT ((SELF SET) VALUE)
  ...
  (CL:WHEN (CL:NOT (MEMBER? SELF VALUE)) (CL:SETF (%THE-CONS-LIST SELF) (CONS VALUE (%THE-CONS-LIST SELF))))
  :VOID)
...
unless `new' is already a member."
  (CL:WHEN (CL:NOT (MEMBER? SELF NEW)) (CL:SETF (%THE-CONS-LIST SELF) (SUBSTITUTE (%THE-CONS-LIST SELF) NEW OLD)))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-000))
     (CL:WHEN (CL:NOT (MEMBER? RESULTSET ELT))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS ELT NIL)) (CL:IF (CL:EQ RESULTSET NIL) (CL:SETQ RESULTSET COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST RESULTSET COLLECT-000)))
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS ELT NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
   ...
   (CL:WHEN (CL:> (CL:LENGTH OTHERSETS) 0)
    (CL:LET* ((OSET NULL) (ITER-001 OTHERSETS))
     (CL:LOOP WHILE ITER-001 DO (CL:SETQ OSET (CL:POP ITER-001))
      (CL:LET* ((ELT NULL) (ITER-002 (%THE-CONS-LIST OSET)) (COLLECT-001 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-002))
        (CL:WHEN (CL:NOT (MEMBER? RESULTSET ELT))
         (CL:IF (CL:EQ COLLECT-001 NULL)
          (CL:PROGN (CL:SETQ COLLECT-001 (CONS ELT NIL))
           (CL:IF (CL:EQ RESULTSET NIL) (CL:SETQ RESULTSET COLLECT-001) (ADD-CONS-TO-END-OF-CONS-LIST RESULTSET COLLECT-001)))
          (CL:PROGN (CL:SETF (%%REST COLLECT-001) (CONS ELT NIL)) (CL:SETQ COLLECT-001 (%%REST COLLECT-001)))))
...
`equivalent-sets?', since that only uses an `eql?' test."
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-SET)
    (CL:PROGN
     (CL:LET* ((TEST-VALUE-000 CL:NIL)) (CL:SETQ TEST-VALUE-000 (CL:= (LENGTH X) (LENGTH Y)))
          ...
          (CL:LET* ((TEST-VALUE-001 CL:NIL))
           (CL:LET* ((FOUND?-000 CL:NIL))
            (CL:LET* ((ELTY NULL) (ITER-001 (ALLOCATE-ITERATOR Y)))
             (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ ELTY (%VALUE ITER-001)) (CL:WHEN (EQUAL? ELTX ELTY) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))))
      ...
      (CL:LET* ((VALUE-000 TEST-VALUE-000)) (CL:RETURN-FROM OBJECT-EQUAL? VALUE-000)))))
   (CL:T))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CURSOR (%THE-CONS-LIST SELF)) (CODE 95880665)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LOOP (CL:WHEN (CL:EQ CURSOR NIL) (CL:RETURN)) (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (SAFE-EQUAL-HASH-CODE (%%VALUE CURSOR))))))
     ...
     (CL:WHEN (CL:NOT (MEMBER? SET V))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS V NIL))
        (CL:IF (CL:EQ (%THE-CONS-LIST SET) NIL) (CL:SETF (%THE-CONS-LIST SET) COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST SET) COLLECT-000)))
...
(CL:DEFMETHOD PRINT-VECTOR ((SELF VECTOR) STREAM)
  (CL:IF (CL:= (LENGTH SELF) 0) (%%PRINT-STREAM STREAM "|i|[]")
   (CL:LET* ((I 0) (LIMIT 9)) (CL:DECLARE (CL:TYPE CL:FIXNUM I LIMIT)) (%%PRINT-STREAM STREAM "|i|[")
    (CL:LET* ((ELEMENT NULL) (VECTOR-000 SELF) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000))) (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
     (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO (CL:SETQ ELEMENT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
      (%%PRINT-STREAM STREAM ELEMENT) (CL:SETQ I (CL:1+ I)) (CL:WHEN (CL:> I LIMIT) (CL:RETURN)) (CL:WHEN (CL:< I (LENGTH SELF)) (%%PRINT-STREAM STREAM " "))
      (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
    (CL:IF (CL:OR (CL:<= I LIMIT) (CL:= I (LENGTH SELF))) (%%PRINT-STREAM STREAM "]") (%%PRINT-STREAM STREAM " ...]"))))
  :VOID)
...
(CL:DEFMETHOD FIRST ((SELF VECTOR))
  (CL:RETURN-FROM FIRST (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0)))

;;; (DEFMETHOD (SECOND (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD SECOND ((SELF VECTOR))
  (CL:RETURN-FROM SECOND (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 1)))

;;; (DEFMETHOD (THIRD (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD THIRD ((SELF VECTOR))
  (CL:RETURN-FROM THIRD (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 2)))

;;; (DEFMETHOD (FOURTH (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD FOURTH ((SELF VECTOR))
  (CL:RETURN-FROM FOURTH (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 3)))

;;; (DEFMETHOD (FIFTH (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD FIFTH ((SELF VECTOR))
  (CL:RETURN-FROM FIFTH (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 4)))
...
(CL:DEFMETHOD LAST ((SELF VECTOR))
  ...
  (CL:RETURN-FROM LAST (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) (CL:THE CL:FIXNUM (CL:1- (%ARRAY-SIZE SELF))))))

;;; (DEFMETHOD (FIRST-SETTER (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD FIRST-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM FIRST-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION 0)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
...
(CL:DEFMETHOD SECOND-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM SECOND-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION 1)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
(CL:DEFMETHOD THIRD-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM THIRD-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION 2)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
(CL:DEFMETHOD FOURTH-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM FOURTH-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION 3)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
(CL:DEFMETHOD FIFTH-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM FIFTH-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION 4)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
(CL:DEFMETHOD LAST-SETTER ((SELF VECTOR) VALUE)
  (CL:RETURN-FROM LAST-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION (CL:1- (%ARRAY-SIZE SELF)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
(CL:DEFMETHOD MEMBER? ((SELF VECTOR) OBJECT)
  (CL:LET* ((ARRAY (%THE-ARRAY SELF))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- (LENGTH SELF)))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:WHEN (EQL? (CL:AREF ARRAY I) OBJECT) (CL:RETURN-FROM MEMBER? CL:T))
...
will always be relative to the entire vector."
  ...
  (CL:LET* ((I NULL-INTEGER) (ITER-000 START) (UPPER-BOUND-000 (CL:1- (LENGTH SELF)))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
   (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
    (CL:WHEN (EQL? (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) I) OBJECT) (CL:RETURN-FROM POSITION I)) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
will always be relative to the entire vector."
  ...
  (CL:LET* ((I NULL-INTEGER) (ITER-000 (REVERSE (INTERVAL 0 END)))) (CL:DECLARE (CL:TYPE CL:FIXNUM I))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ I (%VALUE ITER-000))
    (CL:WHEN (EQL? (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) I) OBJECT) (CL:RETURN-FROM LAST-POSITION I))))
...
(CL:DEFMETHOD COPY ((SELF VECTOR))
  ...
  (CL:LET* ((SIZE (%ARRAY-SIZE SELF)) (SOURCEARRAY (%THE-ARRAY SELF)) (COPY (NEW-VECTOR SIZE)) (COPYARRAY (%THE-ARRAY COPY)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE) (CL:TYPE CL:SIMPLE-VECTOR SOURCEARRAY COPYARRAY))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- SIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:LET ((SELF COPYARRAY) (VALUE (CL:AREF SOURCEARRAY I)) (POSITION I)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
(CL:DEFMETHOD CLEAR ((SELF VECTOR))
  (CL:LET* ((SIZE (%ARRAY-SIZE SELF)) (ARRAY (%THE-ARRAY SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE) (CL:TYPE CL:SIMPLE-VECTOR ARRAY))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- SIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:LET ((SELF ARRAY) (VALUE NULL) (POSITION I)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION)) (CL:SETF (CL:AREF SELF POSITION) VALUE))
     (CL:SETQ ITER-000 (CL:1+ ITER-000)))))
  :VOID)
   ...
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-VECTOR)
    (CL:PROGN
     (CL:WHEN (CL:= (%ARRAY-SIZE X) (%ARRAY-SIZE Y))
      (CL:LET* ((ELTX NULL) (VECTOR-000 X) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (ELTY NULL) (VECTOR-001 Y) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
       ...
       (CL:LOOP WHILE (CL:AND (CL:< INDEX-000 LENGTH-000) (CL:< INDEX-001 LENGTH-001)) DO
        (CL:SETQ ELTX (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
        (CL:SETQ ELTY (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001)) INDEX-001)) (CL:WHEN (CL:NOT (EQUAL? ELTX ELTY)) (CL:RETURN-FROM OBJECT-EQUAL? CL:NIL))
...
(CL:DEFMETHOD EQUAL-HASH-CODE ((SELF VECTOR))
  ...
  (CL:LET* ((CURSOR (LENGTH SELF)) (CODE 66435053)) (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR CODE))
   (CL:LOOP (CL:IF (CL:= CURSOR 0) (CL:RETURN) (CL:SETQ CURSOR (CL:1- CURSOR)))
    (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (SAFE-EQUAL-HASH-CODE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) CURSOR))))))
    (CL:SETQ CODE
...
(CL:DEFMETHOD INSERT-AT ((SELF EXTENSIBLE-SYMBOL-ARRAY) OFFSET VALUE)
  ...
  (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION OFFSET)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
   (CL:SETF (CL:AREF SELF POSITION) VALUE))
  (CL:WHEN (CL:> OFFSET (%TOP-SYMBOL-OFFSET SELF)) (CL:SETF (%TOP-SYMBOL-OFFSET SELF) OFFSET))
  :VOID)
...
(CL:DEFMETHOD EMPTY? ((SELF VECTOR-SEQUENCE))
  "Return `true' if `self' has length 0."
  (CL:RETURN-FROM EMPTY? (CL:= (%SEQUENCE-LENGTH SELF) 0)))

;;; (DEFMETHOD (NON-EMPTY? BOOLEAN) ...)

(CL:DEFMETHOD NON-EMPTY? ((SELF VECTOR-SEQUENCE))
  "Return `true' if `self' has length > 0."
  (CL:RETURN-FROM NON-EMPTY? (> (%SEQUENCE-LENGTH SELF) 0)))
...
(CL:DEFMETHOD LAST ((SELF VECTOR-SEQUENCE))
  "Return the last item in the vector `self'."
  (CL:RETURN-FROM LAST (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) (CL:THE CL:FIXNUM (CL:1- (%SEQUENCE-LENGTH SELF))))))

;;; (DEFMETHOD (LAST-SETTER (LIKE (ANY-VALUE SELF))) ...)

(CL:DEFMETHOD LAST-SETTER ((SELF VECTOR-SEQUENCE) VALUE)
  (CL:RETURN-FROM LAST-SETTER
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION (CL:1- (%SEQUENCE-LENGTH SELF)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))))
...
Resize the array if necessary."
  (CL:LET* ((OLDLENGTH (%SEQUENCE-LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM OLDLENGTH))
   (CL:WHEN (CL:= OLDLENGTH (%ARRAY-SIZE SELF)) (RESIZE-VECTOR SELF (CL:* (%ARRAY-SIZE SELF) 2)))
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION OLDLENGTH)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
Resize the array if necessary."
  (CL:LET* ((OLDLENGTH (%SEQUENCE-LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM OLDLENGTH))
   (CL:WHEN (CL:= OLDLENGTH (%ARRAY-SIZE SELF))
    (CL:WHEN (CL:<= (%RESIZE-FACTOR SELF) 1.0d0) (CL:ERROR "Safety violation: CUSTOM-VECTOR-SEQUENCE.insert: resize factor needs to be > 1"))
    (RESIZE-VECTOR SELF (CL:TRUNCATE (CL:* (%ARRAY-SIZE SELF) (%RESIZE-FACTOR SELF)))))
   (CL:LET ((SELF (%THE-ARRAY SELF)) (VALUE VALUE) (POSITION OLDLENGTH)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
    (CL:SETF (CL:AREF SELF POSITION) VALUE))
   (CL:SETF (%SEQUENCE-LENGTH SELF) (CL:1+ OLDLENGTH)))
  :VOID)
...
the values after it to close the gap."
  (CL:LET* ((ARRAY (%THE-ARRAY SELF)) (LENGTH (%SEQUENCE-LENGTH SELF)) (NHITS 0)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY) (CL:TYPE CL:FIXNUM LENGTH NHITS))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- LENGTH))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:COND ((EQL? (CL:AREF ARRAY I) VALUE) (CL:SETQ NHITS (CL:1+ NHITS)))
      ((CL:> NHITS 0)
       (CL:LET ((SELF ARRAY) (VALUE (CL:AREF ARRAY I)) (POSITION (CL:- I NHITS))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
        (CL:SETF (CL:AREF SELF POSITION) VALUE))))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
   (CL:SETF (%SEQUENCE-LENGTH SELF) (CL:- LENGTH NHITS))
   (CL:LET* ((I NULL-INTEGER) (ITER-001 (%SEQUENCE-LENGTH SELF)) (UPPER-BOUND-001 (CL:1- LENGTH))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001 UPPER-BOUND-001))
...
(CL:DEFMETHOD REVERSE ((SELF VECTOR-SEQUENCE))
  ...
  (CL:LET* ((ARRAY (%THE-ARRAY SELF)) (BOTTOM 0) (TOP (CL:1- (%SEQUENCE-LENGTH SELF))) (ELT NULL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY) (CL:TYPE CL:FIXNUM BOTTOM TOP))
   (CL:LOOP WHILE (CL:< BOTTOM TOP) DO (CL:SETQ ELT (CL:AREF ARRAY BOTTOM))
    (CL:LET ((SELF ARRAY) (VALUE (CL:AREF ARRAY TOP)) (POSITION BOTTOM)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:LET ((SELF ARRAY) (VALUE ELT) (POSITION TOP)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION)) (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
(CL:DEFUN COPY-VECTOR-SEQUENCE (SOURCE COPY)
  (CL:LET* ((SOURCEARRAY (%THE-ARRAY SOURCE)) (COPYARRAY (%THE-ARRAY COPY)) (LENGTH (%SEQUENCE-LENGTH SOURCE)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SOURCEARRAY COPYARRAY) (CL:TYPE CL:FIXNUM LENGTH)) (CL:SETF (%SEQUENCE-LENGTH COPY) LENGTH)
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- LENGTH))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
     (CL:LET ((SELF COPYARRAY) (VALUE (CL:AREF SOURCEARRAY I)) (POSITION I)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
(CL:DEFMETHOD COPY ((SELF VECTOR-SEQUENCE))
  ...
  (CL:LET* ((COPY (NEW-VECTOR-SEQUENCE (%ARRAY-SIZE SELF)))) (COPY-VECTOR-SEQUENCE SELF COPY) (CL:RETURN-FROM COPY COPY)))

;;; (DEFMETHOD (COPY (CUSTOM-VECTOR-SEQUENCE OF (LIKE (ANY-VALUE SELF)))) ...)

(CL:DEFMETHOD COPY ((SELF CUSTOM-VECTOR-SEQUENCE))
  "Return a copy of the vector sequence `self'."
  (CL:LET* ((COPY (NEW-CUSTOM-VECTOR-SEQUENCE (%ARRAY-SIZE SELF)))) (CL:SETF (%RESIZE-FACTOR COPY) (%RESIZE-FACTOR SELF)) (COPY-VECTOR-SEQUENCE SELF COPY)
   (CL:RETURN-FROM COPY COPY)))
...
equivalent.  Uses `equal?' to test equality of elements."
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-VECTOR-SEQUENCE)
    (CL:PROGN
     (CL:WHEN (CL:= (%SEQUENCE-LENGTH X) (%SEQUENCE-LENGTH Y))
      (CL:LET*
       ((ELTX NULL) (VECTOR-000 X) (INDEX-000 0) (LENGTH-000 (%SEQUENCE-LENGTH VECTOR-000)) (ELTY NULL) (VECTOR-001 Y) (INDEX-001 0)
        (LENGTH-001 (%SEQUENCE-LENGTH VECTOR-001)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 INDEX-001 LENGTH-001))
       (CL:LOOP WHILE (CL:AND (CL:< INDEX-000 LENGTH-000) (CL:< INDEX-001 LENGTH-001)) DO
        (CL:SETQ ELTX (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
        (CL:SETQ ELTY (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001)) INDEX-001)) (CL:WHEN (CL:NOT (EQUAL? ELTX ELTY)) (CL:RETURN-FROM OBJECT-EQUAL? CL:NIL))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF VECTOR))
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR))) (CL:SETF (%ITERATOR-OBJECT ITERATOR) SELF) (CL:SETF (%ITERATOR-INTEGER ITERATOR) 0)
   (CL:SETF (%ITERATOR-SECOND-INTEGER ITERATOR) (LENGTH SELF)) (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION VECTOR-NEXT?))
   (CL:RETURN-FROM ALLOCATE-ITERATOR ITERATOR)))

;;; (DEFUN (VECTOR-NEXT? BOOLEAN) ...)

(CL:DEFUN VECTOR-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:WHEN (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) (CL:SETF (%VALUE SELF) (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY (%ITERATOR-OBJECT SELF))) CURSOR))
    (CL:SETF (%ITERATOR-INTEGER SELF) (CL:1+ CURSOR)) (CL:RETURN-FROM VECTOR-NEXT? CL:T))
...
(CL:DEFMETHOD BUT-LAST ((SELF VECTOR))
  ...
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR))) (CL:SETF (%ITERATOR-OBJECT ITERATOR) SELF) (CL:SETF (%ITERATOR-INTEGER ITERATOR) 0)
   (CL:SETF (%ITERATOR-SECOND-INTEGER ITERATOR) (CL:1- (%ARRAY-SIZE SELF))) (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION VECTOR-NEXT?))
...
element types are GENERALIZED-SYMBOL, STRING, INTEGER, and FLOAT)."
  (CL:LET* ((LENGTH (LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:COND ((CL:= LENGTH 0) (CL:RETURN-FROM SORT SELF))
    ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (CHOOSE-SORT-PREDICATE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0)))))
   (HEAP-SORT-NATIVE-VECTOR (%THE-ARRAY SELF) (LENGTH SELF) PREDICATE) (CL:RETURN-FROM SORT SELF)))
...
whose `n'-th element (0-based) will be used for comparison."
  ...
  (CL:LET* ((LENGTH (LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:COND ((CL:= LENGTH 0) (CL:RETURN-FROM SORT-TUPLES SELF))
    ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (CHOOSE-SORT-PREDICATE (NTH (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0) N)))))
   (CL:LET* ((*SORT-TUPLE-COMPARE-PREDICATE* PREDICATE) (*SORT-TUPLE-COMPARE-INDEX* N))
    ...
    (HEAP-SORT-NATIVE-VECTOR (%THE-ARRAY SELF) (LENGTH SELF) (CL:FUNCTION SORT-TUPLE-COMPARE?)) (CL:RETURN-FROM SORT-TUPLES SELF))))
...
it will cause value wrapping everytime `slot' is read."
  (CL:LET* ((LENGTH (LENGTH SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:COND ((CL:= LENGTH 0) (CL:RETURN-FROM SORT-OBJECTS SELF))
    ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (CHOOSE-SORT-PREDICATE (READ-SLOT-VALUE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0) SLOT)))))
   (CL:LET* ((*SORT-TUPLE-COMPARE-PREDICATE* PREDICATE) (*SORT-OBJECTS-COMPARE-SLOT* SLOT))
    ...
    (HEAP-SORT-NATIVE-VECTOR (%THE-ARRAY SELF) (LENGTH SELF) (CL:FUNCTION SORT-OBJECTS-COMPARE?)) (CL:RETURN-FROM SORT-OBJECTS SELF))))
...
(CL:DEFUN HEAP-SORT-NATIVE-VECTOR (VECTOR SIZE PREDICATE)
  ...
  (CL:LET* ((END (CL:1- SIZE)) (AUX NULL)) (CL:DECLARE (CL:TYPE CL:FIXNUM END))
   (CL:LOOP WHILE (CL:> END 0) DO (CL:SETQ AUX (CL:AREF VECTOR 0))
    (CL:LET ((SELF VECTOR) (VALUE (CL:AREF VECTOR END)) (POSITION 0)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:LET ((SELF VECTOR) (VALUE AUX) (POSITION END)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION)) (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:SETQ END (CL:1- END)) (HEAP-SORT-SIFT-DOWN VECTOR 0 END PREDICATE)))
  :VOID)
...
(CL:DEFUN HEAP-SORT-SIFT-DOWN (VECTOR START END PREDICATE)
  ...
  (CL:LET* ((ROOT START) (CHILD 0) (AUX NULL)) (CL:DECLARE (CL:TYPE CL:FIXNUM ROOT CHILD))
   (CL:LOOP (CL:SETQ CHILD (CL:1+ (CL:THE CL:FIXNUM (CL:ASH ROOT 1)))) (CL:WHEN (CL:> CHILD END) (CL:RETURN))
    (CL:WHEN (CL:AND (CL:< CHILD END) (CL:FUNCALL PREDICATE (CL:AREF VECTOR CHILD) (CL:AREF VECTOR (CL:THE CL:FIXNUM (CL:1+ CHILD))))) (CL:SETQ CHILD (CL:1+ CHILD)))
    (CL:COND
     ((CL:FUNCALL PREDICATE (CL:AREF VECTOR ROOT) (CL:AREF VECTOR CHILD)) (CL:SETQ AUX (CL:AREF VECTOR ROOT))
      (CL:LET ((SELF VECTOR) (VALUE (CL:AREF VECTOR CHILD)) (POSITION ROOT)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET ((SELF VECTOR) (VALUE AUX) (POSITION CHILD)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:SETQ ROOT CHILD))
...
(CL:DEFUN QUICK-SORT-PICK-SPLIT-ELEMENT (VECTOR START END PREDICATE)
  ...
  (CL:LET*
   ((SPLITELEMENT (CL:AREF VECTOR START))
    (MIDDLE (CL:LET ((ARG (CL:+ START END)) (COUNT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM ARG COUNT)) (CL:THE CL:FIXNUM (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))))
   (CL:DECLARE (CL:TYPE CL:FIXNUM MIDDLE)) (CL:SETQ END (CL:1- END))
   (CL:IF (CL:FUNCALL PREDICATE SPLITELEMENT (CL:AREF VECTOR MIDDLE))
    (CL:COND
     ((CL:FUNCALL PREDICATE (CL:AREF VECTOR MIDDLE) (CL:AREF VECTOR END)) (CL:SETQ SPLITELEMENT (CL:AREF VECTOR MIDDLE))
      (CL:LET ((SELF VECTOR) (VALUE (CL:AREF VECTOR START)) (POSITION MIDDLE)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))
     ((CL:FUNCALL PREDICATE SPLITELEMENT (CL:AREF VECTOR END)) (CL:SETQ SPLITELEMENT (CL:AREF VECTOR END))
      (CL:LET ((SELF VECTOR) (VALUE (CL:AREF VECTOR START)) (POSITION END)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))
     (CL:T))
    (CL:COND ((CL:FUNCALL PREDICATE SPLITELEMENT (CL:AREF VECTOR END)))
     ((CL:FUNCALL PREDICATE (CL:AREF VECTOR MIDDLE) (CL:AREF VECTOR END)) (CL:SETQ SPLITELEMENT (CL:AREF VECTOR END))
      (CL:LET ((SELF VECTOR) (VALUE (CL:AREF VECTOR START)) (POSITION END)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE)))
...
(CL:DEFUN QUICK-SORT-NATIVE-VECTOR (VECTOR START END PREDICATE)
  ...
  (CL:LOOP
   (CL:LET* ((LENGTH (CL:- END START)) (SPLITELEMENT NULL) (ELEMENT NULL) (LOWCURSOR START) (HIGHCURSOR END))
    ...
    (CL:COND ((CL:<= LENGTH 1) (CL:RETURN-FROM QUICK-SORT-NATIVE-VECTOR))
     ((CL:>= LENGTH 10) (CL:SETQ SPLITELEMENT (QUICK-SORT-PICK-SPLIT-ELEMENT VECTOR START END PREDICATE))) (CL:T (CL:SETQ SPLITELEMENT (CL:AREF VECTOR START))))
    (CL:LOOP WHILE (CL:< LOWCURSOR HIGHCURSOR) DO
     (CL:LOOP WHILE (CL:< LOWCURSOR HIGHCURSOR) DO (CL:SETQ HIGHCURSOR (CL:1- HIGHCURSOR)) (CL:SETQ ELEMENT (CL:AREF VECTOR HIGHCURSOR))
      (CL:WHEN (CL:FUNCALL PREDICATE ELEMENT SPLITELEMENT)
       (CL:LET ((SELF VECTOR) (VALUE ELEMENT) (POSITION LOWCURSOR)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
        (CL:SETF (CL:AREF SELF POSITION) VALUE))
       (CL:RETURN)))
     (CL:LOOP WHILE (CL:< LOWCURSOR HIGHCURSOR) DO (CL:SETQ LOWCURSOR (CL:1+ LOWCURSOR)) (CL:SETQ ELEMENT (CL:AREF VECTOR LOWCURSOR))
      (CL:WHEN (CL:FUNCALL PREDICATE SPLITELEMENT ELEMENT)
       (CL:LET ((SELF VECTOR) (VALUE ELEMENT) (POSITION HIGHCURSOR)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
        (CL:SETF (CL:AREF SELF POSITION) VALUE))
       (CL:RETURN))))
    (CL:LET ((SELF VECTOR) (VALUE SPLITELEMENT) (POSITION HIGHCURSOR)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:COND
     ((CL:< (CL:LET ((ARG (CL:+ START END)) (COUNT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM ARG COUNT)) (CL:THE CL:FIXNUM (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))
       HIGHCURSOR)
      (QUICK-SORT-NATIVE-VECTOR VECTOR (CL:1+ HIGHCURSOR) END PREDICATE) (CL:SETQ END HIGHCURSOR))
     (CL:T (QUICK-SORT-NATIVE-VECTOR VECTOR START HIGHCURSOR PREDICATE) (CL:SETQ START (CL:1+ HIGHCURSOR))))))
  :VOID)

;;; (DEFCLASS HEAP ...)

(CL:DEFCLASS HEAP (VECTOR)
  ((PREDICATE :ALLOCATION :INSTANCE :ACCESSOR %PREDICATE) (FILL-POINTER :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %FILL-POINTER))
...
(CL:DEFUN ACCESS-HEAP-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-PREDICATE)
    (CL:IF SETVALUE? (CL:SETF (%PREDICATE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-FUNCTION-CODE (%PREDICATE SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-FILL-POINTER)
    (CL:IF SETVALUE? (CL:SETF (%FILL-POINTER SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%FILL-POINTER SELF)))))
   (CL:T
...
(CL:DEFMETHOD INITIALIZE-HEAP ((SELF HEAP))
  (CL:LET* ((SIZE (%ARRAY-SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE)) (CL:WHEN (CL:<= SIZE 0) (CL:ERROR "Safety violation: Trying to create an empty heap")))
  :VOID)
...
(CL:DEFMETHOD COPY ((SELF HEAP))
  ...
  (CL:LET* ((LENGTH (%FILL-POINTER SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM LENGTH))
   (CL:LET* ((SELF-000 (NEW-HEAP (%PREDICATE SELF) (%ARRAY-SIZE SELF)))) (CL:SETF (%FILL-POINTER SELF-000) LENGTH)
    (CL:LET* ((COPY SELF-000) (SOURCEARRAY (%THE-ARRAY SELF)) (COPYARRAY (%THE-ARRAY COPY))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SOURCEARRAY COPYARRAY))
     (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- LENGTH))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
      (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000)
       (CL:LET ((SELF COPYARRAY) (VALUE (CL:AREF SOURCEARRAY I)) (POSITION I)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
        (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
(CL:DEFMETHOD LENGTH ((SELF HEAP))
  ...
  (CL:RETURN-FROM LENGTH (%FILL-POINTER SELF)))
...
largest or best item if `self' is a sorted min-heap with a '<' predicate."
  (CL:RETURN-FROM LAST (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) (CL:THE CL:FIXNUM (CL:1- (LENGTH SELF))))))
...
The root contains the minimum element of a min-heap with '<' predicate."
  (CL:RETURN-FROM HEAP-ROOT (CL:IF (EMPTY? SELF) NULL (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0))))
...
(CL:DEFMETHOD FAST-HEAP-ROOT ((SELF HEAP))
  ...
  (CL:RETURN-FROM FAST-HEAP-ROOT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0)))
  ...
  (CL:LET* ((PARENT NULL-INTEGER) (CHILD END)) (CL:DECLARE (CL:TYPE CL:FIXNUM PARENT CHILD))
   ...
   (CL:LOOP WHILE (CL:> CHILD START) DO
    (CL:IF (CL:= (CL:THE CL:FIXNUM (CL:LOGAND CHILD 1)) 0)
     (CL:SETQ PARENT
      (CL:LET ((ARG (CL:- CHILD 2)) (COUNT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM ARG COUNT)) (CL:THE CL:FIXNUM (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT))))))
     (CL:SETQ PARENT
      (CL:LET ((ARG (CL:1- CHILD)) (COUNT 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM ARG COUNT)) (CL:THE CL:FIXNUM (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))))
    (CL:COND
     ((CL:FUNCALL PREDICATE VALUE (CL:AREF HEAP PARENT))
      (CL:LET ((SELF HEAP) (VALUE (CL:AREF HEAP PARENT)) (POSITION CHILD)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
      (CL:LET ((SELF HEAP) (VALUE VALUE) (POSITION PARENT)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
       (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
(CL:DEFUN HEAP-SIFT-DOWN (HEAP START END VALUE PREDICATE)
  ...
  (CL:LET* ((PARENT START) (CHILD NULL-INTEGER) (LEFTCHILD NULL) (RIGHTCHILD NULL)) (CL:DECLARE (CL:TYPE CL:FIXNUM PARENT CHILD))
   ...
   (CL:LOOP (CL:SETQ CHILD (CL:1+ (CL:THE CL:FIXNUM (CL:ASH PARENT 1))))
    (CL:COND ((CL:> CHILD END) (CL:RETURN)) ((CL:= CHILD END) (CL:WHEN (CL:NOT (CL:FUNCALL PREDICATE (CL:AREF HEAP CHILD) VALUE)) (CL:RETURN)))
     (CL:T (CL:SETQ LEFTCHILD (CL:AREF HEAP CHILD)) (CL:SETQ RIGHTCHILD (CL:AREF HEAP (CL:THE CL:FIXNUM (CL:1+ CHILD))))
      (CL:COND
       ((CL:FUNCALL PREDICATE LEFTCHILD VALUE)
        (CL:WHEN (CL:AND (CL:FUNCALL PREDICATE RIGHTCHILD VALUE) (CL:FUNCALL PREDICATE RIGHTCHILD LEFTCHILD)) (CL:SETQ CHILD (CL:1+ CHILD))))
       ((CL:FUNCALL PREDICATE RIGHTCHILD VALUE) (CL:SETQ CHILD (CL:1+ CHILD))) (CL:T (CL:RETURN)))))
    (CL:LET ((SELF HEAP) (VALUE (CL:AREF HEAP CHILD)) (POSITION PARENT)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (CL:LET ((SELF HEAP) (VALUE VALUE) (POSITION CHILD)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
...
a Min-heap if `self's `predicate' has `<' semantics; otherwise as a Max-heap."
  (CL:LET* ((VALUES (%THE-ARRAY SELF)) (FILLPOINTER (%FILL-POINTER SELF)) (PREDICATE (%PREDICATE SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES) (CL:TYPE CL:FIXNUM FILLPOINTER))
   (CL:COND ((CL:< FILLPOINTER (%ARRAY-SIZE SELF)) (HEAP-SIFT-UP VALUES 0 FILLPOINTER VALUE PREDICATE) (CL:SETF (%FILL-POINTER SELF) (CL:1+ FILLPOINTER)))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "HEAP.insert: heap overflow, can't insert new value: `" VALUE "'")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
  :VOID)
...
a Min-heap if `self's `predicate' has `<' semantics; otherwise as a Max-heap."
  (CL:LET* ((VALUES (%THE-ARRAY SELF)) (FILLPOINTER (%FILL-POINTER SELF)) (PREDICATE (%PREDICATE SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES) (CL:TYPE CL:FIXNUM FILLPOINTER))
   (CL:IF (CL:= FILLPOINTER 0) (CL:ERROR (NEW-STELLA-EXCEPTION "HEAP.replace-heap-root: can't replace root of an empty heap"))
...
minimum) of `self' is the currently weakest element at the end of the list."
  (CL:LET* ((VALUES (%THE-ARRAY SELF)) (FILLPOINTER (%FILL-POINTER SELF)) (PREDICATE (%PREDICATE SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES) (CL:TYPE CL:FIXNUM FILLPOINTER))
   (CL:COND
    ((CL:= FILLPOINTER (%ARRAY-SIZE SELF))
     (CL:WHEN (CL:FUNCALL PREDICATE (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY SELF)) 0) VALUE) (HEAP-SIFT-DOWN VALUES 0 FILLPOINTER VALUE PREDICATE)))
    (CL:T (HEAP-SIFT-UP VALUES 0 FILLPOINTER VALUE PREDICATE) (CL:SETF (%FILL-POINTER SELF) (CL:1+ FILLPOINTER)))))
  :VOID)
...
insertion of values or if `predicate' has been changed."
  (CL:LET* ((VALUES (%THE-ARRAY SELF)) (FILLPOINTER (%FILL-POINTER SELF)) (PREDICATE (%PREDICATE SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES) (CL:TYPE CL:FIXNUM FILLPOINTER))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- FILLPOINTER))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (HEAP-SIFT-UP VALUES 0 I (CL:AREF VALUES I) PREDICATE) (CL:SETQ ITER-000 (CL:1+ ITER-000)))))
  :VOID)
...
the same."
  (CL:LET* ((VALUES (%THE-ARRAY SELF)) (CURSOR (%FILL-POINTER SELF)) (VALUE NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR VALUES) (CL:TYPE CL:FIXNUM CURSOR))
   (CL:COND ((CL:EQ PREDICATE NULL) (CL:SETQ PREDICATE (%PREDICATE SELF)))
    ((CL:NOT (CL:EQL PREDICATE (%PREDICATE SELF))) (CL:SETF (%PREDICATE SELF) PREDICATE) (HEAPIFY SELF)))
   (CL:LOOP WHILE (CL:> CURSOR 0) DO (CL:SETQ CURSOR (CL:1- CURSOR)) (CL:SETQ VALUE (CL:AREF VALUES CURSOR))
    (CL:LET ((SELF VALUES) (VALUE (CL:AREF VALUES 0)) (POSITION CURSOR)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE))
    (HEAP-SIFT-DOWN VALUES 0 CURSOR VALUE PREDICATE))
   (CL:SETQ CURSOR (%FILL-POINTER SELF))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000))
    (CL:LOOP WHILE CL:T DO (CL:SETQ I ITER-000) (CL:SETQ CURSOR (CL:1- CURSOR)) (CL:WHEN (CL:>= I CURSOR) (CL:RETURN)) (CL:SETQ VALUE (CL:AREF VALUES I))
     (CL:LET ((SELF VALUES) (VALUE (CL:AREF VALUES CURSOR)) (POSITION I)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))
     (CL:LET ((SELF VALUES) (VALUE VALUE) (POSITION CURSOR)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
(CL:DEFCLASS STELLA-HASH-TABLE (ABSTRACT-HASH-TABLE)
  ((THE-TABLE :ALLOCATION :INSTANCE :ACCESSOR %THE-TABLE) (SIZE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %SIZE)
   (INITIAL-SIZE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "If supplied, the initial hash table
will be sized to hold at least that many elements." :ALLOCATION :INSTANCE :ACCESSOR %INITIAL-SIZE)
   (FREE-ELEMENTS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %FREE-ELEMENTS)
   (EQUAL-TEST? :DOCUMENTATION "If true use `equal?' as the
equality test and `equal-hash-code' as the hash function, otherwise,
use `eql?' and `hash-code' (the default)." :ALLOCATION :INSTANCE :ACCESSOR %EQUAL-TEST?)))

(CL:DEFUN NEW-STELLA-HASH-TABLE ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE STELLA-HASH-TABLE))) (CL:SETF (%THE-STELLA-HASH-TABLE SELF) NULL)
   (CL:SETF (%THE-HASH-TABLE SELF) NULL) (CL:SETF (%EQUAL-TEST? SELF) CL:NIL) (CL:SETF (%FREE-ELEMENTS SELF) NULL-INTEGER) (CL:SETF (%INITIAL-SIZE SELF) 50)
   (CL:SETF (%SIZE SELF) NULL-INTEGER) (CL:SETF (%THE-TABLE SELF) STELLA::NULL-1D-ARRAY) (INITIALIZE-HASH-TABLE SELF) (CL:RETURN-FROM NEW-STELLA-HASH-TABLE SELF)))
...
(CL:DEFUN ACCESS-STELLA-HASH-TABLE-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-SIZE) (CL:IF SETVALUE? (CL:SETF (%SIZE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%SIZE SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-INITIAL-SIZE)
    (CL:IF SETVALUE? (CL:SETF (%INITIAL-SIZE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%INITIAL-SIZE SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-FREE-ELEMENTS)
    (CL:IF SETVALUE? (CL:SETF (%FREE-ELEMENTS SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%FREE-ELEMENTS SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-EQUAL-TEST?)
    (CL:IF SETVALUE? (CL:SETF (%EQUAL-TEST? SELF) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)) (CL:SETQ VALUE (CL:IF (%EQUAL-TEST? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN INITIALIZE-STELLA-HASH-TABLE (SELF)
  (CL:LET* ((SIZE (PICK-HASH-TABLE-SIZE-PRIME (FLOOR (CL:/ (%INITIAL-SIZE SELF) *STELLA-HASH-TABLE-AVG-BUCKET-LENGTH*)))) (TABLE (CL:MAKE-ARRAY SIZE)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE) (CL:TYPE CL:SIMPLE-VECTOR TABLE))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- SIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETF (CL:AREF TABLE I) NULL) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
   (CL:SETF (%THE-TABLE SELF) TABLE) (CL:SETF (%SIZE SELF) SIZE) (CL:SETF (%FREE-ELEMENTS SELF) (FLOOR (CL:* SIZE *STELLA-HASH-TABLE-AVG-BUCKET-LENGTH*))))
  :VOID)
...
(CL:DEFUN REHASH-STELLA-HASH-TABLE (SELF NEWSIZE)
  ...
  (CL:LET*
   ((SIZE (%SIZE SELF)) (TABLE (%THE-TABLE SELF)) (NEWTABLE (CL:MAKE-ARRAY NEWSIZE)) (NEWBUCKETINDEX 0) (NEWBUCKET NULL) (CURSOR NULL) (CURRENT NULL)
    (EQUALTEST? (%EQUAL-TEST? SELF)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE NEWBUCKETINDEX) (CL:TYPE CL:SIMPLE-VECTOR TABLE NEWTABLE))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- NEWSIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETF (CL:AREF NEWTABLE I) NULL) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
   (CL:LET* ((I NULL-INTEGER) (ITER-001 0) (UPPER-BOUND-001 (CL:1- SIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001 UPPER-BOUND-001))
    (CL:LOOP WHILE (CL:<= ITER-001 UPPER-BOUND-001) DO (CL:SETQ I ITER-001) (CL:SETQ CURSOR (CL:AREF TABLE I))
     (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO
      (CL:IF EQUALTEST?
       (CL:SETQ NEWBUCKETINDEX
        (CL:LET ((CODE (EQUAL-HASH-CODE (%KEY CURSOR))) (SIZE NEWSIZE)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
         (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))
       (CL:SETQ NEWBUCKETINDEX
        (CL:LET ((CODE (HASH-CODE (%KEY CURSOR))) (SIZE NEWSIZE)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
         (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE)))))
      (CL:SETQ NEWBUCKET (CL:AREF NEWTABLE NEWBUCKETINDEX)) (CL:SETQ CURRENT CURSOR) (CL:SETQ CURSOR (%REST CURSOR))
      (CL:COND ((CL:NOT (CL:EQ NEWBUCKET NULL)) (CL:SETF (%REST CURRENT) (%REST NEWBUCKET)) (CL:SETF (%REST NEWBUCKET) CURRENT))
       (CL:T (CL:SETF (CL:AREF NEWTABLE NEWBUCKETINDEX) CURRENT) (CL:SETF (%REST CURRENT) NULL))))
     (CL:SETQ ITER-001 (CL:1+ ITER-001))))
   (CL:SETF (%THE-TABLE SELF) NEWTABLE) (CL:SETF (%SIZE SELF) NEWSIZE)
   (CL:SETF (%FREE-ELEMENTS SELF) (MAX (CL:+ (%FREE-ELEMENTS SELF) (FLOOR (CL:* (CL:- NEWSIZE SIZE) *STELLA-HASH-TABLE-AVG-BUCKET-LENGTH*))) 0)))
  :VOID)

;;; (DEFUN STELLA-HASH-TABLE-INSERT-AT ...)

(CL:DEFUN STELLA-HASH-TABLE-INSERT-AT (SELF KEY VALUE)
  (CL:LET* ((TABLE (%THE-TABLE SELF)) (FREE (%FREE-ELEMENTS SELF)) (BUCKETINDEX 0) (BUCKET NULL) (EQUALTEST? (%EQUAL-TEST? SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE) (CL:TYPE CL:FIXNUM FREE BUCKETINDEX))
   (CL:WHEN (NULL-ARRAY? TABLE) (INITIALIZE-STELLA-HASH-TABLE SELF) (CL:SETQ TABLE (%THE-TABLE SELF)) (CL:SETQ FREE (%FREE-ELEMENTS SELF)))
   (CL:WHEN (CL:= FREE 0) (REHASH-STELLA-HASH-TABLE SELF (PICK-HASH-TABLE-SIZE-PRIME (CL:1+ (%SIZE SELF)))) (CL:SETQ TABLE (%THE-TABLE SELF))
    (CL:SETQ FREE (%FREE-ELEMENTS SELF)))
   (CL:IF EQUALTEST?
    (CL:SETQ BUCKETINDEX
     (CL:LET ((CODE (EQUAL-HASH-CODE KEY)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
      (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))
    (CL:SETQ BUCKETINDEX
     (CL:LET ((CODE (HASH-CODE KEY)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
      (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE)))))
   (CL:SETQ BUCKET (CL:AREF TABLE BUCKETINDEX))
   (CL:COND
    ((CL:EQ BUCKET NULL)
     (CL:LET* ((SELF-000 (NEW-KV-CONS))) (CL:SETF (%KEY SELF-000) KEY) (CL:SETF (%VALUE SELF-000) VALUE) (CL:SETF (CL:AREF TABLE BUCKETINDEX) SELF-000))
     (CL:SETF (%FREE-ELEMENTS SELF) (CL:1- FREE)))
    (CL:T
     (CL:LET* ((CURSOR BUCKET))
      (CL:COND (EQUALTEST? (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ CURSOR NULL)) (CL:NOT (EQUAL? (%KEY CURSOR) KEY))) DO (CL:SETQ CURSOR (%REST CURSOR))))
       (CL:T (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ CURSOR NULL)) (CL:NOT (EQL? (%KEY CURSOR) KEY))) DO (CL:SETQ CURSOR (%REST CURSOR)))))
      (CL:COND ((CL:NOT (CL:EQ CURSOR NULL)) (CL:SETF (%VALUE CURSOR) VALUE))
       (CL:T
        (CL:LET* ((SELF-001 (NEW-KV-CONS))) (CL:SETF (%KEY SELF-001) KEY) (CL:SETF (%VALUE SELF-001) VALUE) (CL:SETF (%REST SELF-001) (%REST BUCKET))
...
(CL:DEFUN STELLA-HASH-TABLE-REMOVE-AT (SELF KEY)
  (CL:LET* ((TABLE (%THE-TABLE SELF)) (BUCKETINDEX 0) (BUCKET NULL) (EQUALTEST? (%EQUAL-TEST? SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE) (CL:TYPE CL:FIXNUM BUCKETINDEX)) (CL:WHEN (NULL-ARRAY? TABLE) (CL:RETURN-FROM STELLA-HASH-TABLE-REMOVE-AT))
   (CL:IF EQUALTEST?
    (CL:SETQ BUCKETINDEX
     (CL:LET ((CODE (EQUAL-HASH-CODE KEY)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
      (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))
   ...
   (CL:COND ((CL:EQ BUCKET NULL) (CL:RETURN-FROM STELLA-HASH-TABLE-REMOVE-AT))
    ((CL:AND (CL:NOT EQUALTEST?) (EQL? (%KEY BUCKET) KEY)) (CL:SETF (CL:AREF TABLE BUCKETINDEX) (%REST BUCKET))
     (CL:SETF (%FREE-ELEMENTS SELF) (CL:1+ (%FREE-ELEMENTS SELF))) (CL:RETURN-FROM STELLA-HASH-TABLE-REMOVE-AT))
    ((CL:AND EQUALTEST? (EQUAL? (%KEY BUCKET) KEY)) (CL:SETF (CL:AREF TABLE BUCKETINDEX) (%REST BUCKET)) (CL:SETF (%FREE-ELEMENTS SELF) (CL:1+ (%FREE-ELEMENTS SELF)))
    ...
    (CL:T
     (CL:LET* ((TRAILER BUCKET)) (CL:SETQ BUCKET (%REST BUCKET))
      (CL:LOOP WHILE (CL:NOT (CL:EQ BUCKET NULL)) DO
       (CL:COND
        ((CL:AND (CL:NOT EQUALTEST?) (EQL? (%KEY BUCKET) KEY)) (CL:SETF (%REST TRAILER) (%REST BUCKET)) (CL:SETF (%FREE-ELEMENTS SELF) (CL:1+ (%FREE-ELEMENTS SELF)))
         (CL:RETURN-FROM STELLA-HASH-TABLE-REMOVE-AT))
        ((CL:AND EQUALTEST? (EQUAL? (%KEY BUCKET) KEY)) (CL:SETF (%REST TRAILER) (%REST BUCKET)) (CL:SETF (%FREE-ELEMENTS SELF) (CL:1+ (%FREE-ELEMENTS SELF)))
         (CL:RETURN-FROM STELLA-HASH-TABLE-REMOVE-AT))
...
(CL:DEFUN STELLA-HASH-TABLE-LOOKUP (SELF KEY)
  (CL:LET* ((TABLE (%THE-TABLE SELF)) (BUCKET NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE))
   (CL:WHEN (NULL-ARRAY? TABLE) (CL:RETURN-FROM STELLA-HASH-TABLE-LOOKUP NULL))
   (CL:COND
    ((%EQUAL-TEST? SELF)
     (CL:SETQ BUCKET
      (CL:AREF TABLE
       (CL:THE CL:FIXNUM
        (CL:LET ((CODE (EQUAL-HASH-CODE KEY)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
         (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ BUCKET NULL)) DO
      (CL:IF (EQUAL? (%KEY BUCKET) KEY) (CL:RETURN-FROM STELLA-HASH-TABLE-LOOKUP (%VALUE BUCKET)) (CL:SETQ BUCKET (%REST BUCKET)))))
    (CL:T
     (CL:SETQ BUCKET
      (CL:AREF TABLE
       (CL:THE CL:FIXNUM
        (CL:LET ((CODE (HASH-CODE KEY)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
         (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ BUCKET NULL)) DO
      (CL:IF (EQL? (%KEY BUCKET) KEY) (CL:RETURN-FROM STELLA-HASH-TABLE-LOOKUP (%VALUE BUCKET)) (CL:SETQ BUCKET (%REST BUCKET))))))
...
(CL:DEFUN STELLA-STRING-HASH-TABLE-LOOKUP (SELF KEY)
  ...
  (CL:LET* ((TABLE (%THE-TABLE SELF)) (BUCKET NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE))
   ...
   (CL:SETQ BUCKET
    (CL:AREF TABLE
     (CL:THE CL:FIXNUM
      (CL:LET ((CODE (CL:THE CL:FIXNUM (CL:SXHASH KEY))) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE SIZE))
       (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (CL:LOGAND CODE *INTEGER-UNSIGNED-BITS-MASK*)) SIZE))))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ BUCKET NULL)) DO
    (CL:IF (STRING-EQL? (%WRAPPER-VALUE (%KEY BUCKET)) KEY) (CL:RETURN-FROM STELLA-STRING-HASH-TABLE-LOOKUP (%VALUE BUCKET)) (CL:SETQ BUCKET (%REST BUCKET))))
...
(CL:DEFMETHOD LENGTH ((SELF STELLA-HASH-TABLE))
  ...
  (CL:IF (NULL-ARRAY? (%THE-TABLE SELF)) (CL:RETURN-FROM LENGTH 0) (CL:RETURN-FROM LENGTH (CL:- (%SIZE SELF) (%FREE-ELEMENTS SELF)))))
...
are not copied themselves (similar to what we do for lists, etc.)."
  (CL:LET* ((SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM SIZE))
   (CL:LET* ((SELF-000 (NEW-STELLA-HASH-TABLE))) (CL:SETF (%SIZE SELF-000) SIZE) (CL:SETF (%INITIAL-SIZE SELF-000) (%INITIAL-SIZE SELF))
    (CL:SETF (%FREE-ELEMENTS SELF-000) (%FREE-ELEMENTS SELF)) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF))
    (CL:LET* ((COPY SELF-000) (TABLE (%THE-TABLE SELF)) (TABLECOPY TABLE) (BUCKET NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE TABLECOPY))
     (CL:WHEN (CL:NOT (NULL-ARRAY? TABLE)) (CL:SETQ TABLECOPY (CL:MAKE-ARRAY SIZE)) (CL:SETF (%THE-TABLE COPY) TABLECOPY)
      (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- SIZE))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
       (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETQ BUCKET (CL:AREF TABLE I))
        (CL:IF (CL:NOT (CL:EQ BUCKET NULL)) (CL:SETF (CL:AREF TABLECOPY I) (COPY-KV-CONS-LIST BUCKET)) (CL:SETF (CL:AREF TABLECOPY I) NULL))
        (CL:SETQ ITER-000 (CL:1+ ITER-000)))))
     (CL:RETURN-FROM COPY COPY)))))
...
`(<key> <value>)' pairs and return the result."
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (CONS KEY (CONS VALUE NIL)) NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
...
(CL:DEFMETHOD OBJECT-EQUAL? ((X STELLA-HASH-TABLE) Y)
  ...
  (CL:COND
   ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-STELLA-HASH-TABLE)
    (CL:PROGN
...
is O(N) in the number of entries of `self'."
  (CL:LET* ((CODE 36273463)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (EQUAL-HASH-CODE KEY)))))
     (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (EQUAL-HASH-CODE VALUE)))))))
   (CL:RETURN-FROM EQUAL-HASH-CODE CODE)))

;;; (DEFCLASS STELLA-HASH-TABLE-ITERATOR ...)

(CL:DEFCLASS STELLA-HASH-TABLE-ITERATOR (DICTIONARY-ITERATOR)
  ((SIZE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %SIZE) (BUCKET-TABLE :ALLOCATION :INSTANCE :ACCESSOR %BUCKET-TABLE)
   (BUCKET-INDEX :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %BUCKET-INDEX) (BUCKET-CURSOR :ALLOCATION :INSTANCE :ACCESSOR %BUCKET-CURSOR))
...
(CL:DEFUN NEW-STELLA-HASH-TABLE-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE STELLA-HASH-TABLE-ITERATOR))) (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) NULL)
   (CL:SETF (%KEY SELF) NULL) (CL:SETF (%BUCKET-CURSOR SELF) NULL) (CL:SETF (%BUCKET-INDEX SELF) 0) (CL:SETF (%BUCKET-TABLE SELF) STELLA::NULL-1D-ARRAY)
   (CL:SETF (%SIZE SELF) NULL-INTEGER) (CL:RETURN-FROM NEW-STELLA-HASH-TABLE-ITERATOR SELF)))

(CL:DEFMETHOD PRIMARY-TYPE ((SELF STELLA-HASH-TABLE-ITERATOR))
  (CL:RETURN-FROM PRIMARY-TYPE SGT-COLLECTIONS-STELLA-STELLA-HASH-TABLE-ITERATOR))
...
(CL:DEFUN ACCESS-STELLA-HASH-TABLE-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-SIZE) (CL:IF SETVALUE? (CL:SETF (%SIZE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%SIZE SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-BUCKET-INDEX)
    (CL:IF SETVALUE? (CL:SETF (%BUCKET-INDEX SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%BUCKET-INDEX SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-BUCKET-CURSOR) (CL:IF SETVALUE? (CL:SETF (%BUCKET-CURSOR SELF) VALUE) (CL:SETQ VALUE (%BUCKET-CURSOR SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF STELLA-HASH-TABLE))
  ...
  (CL:LET* ((SELF-000 (NEW-STELLA-HASH-TABLE-ITERATOR))) (CL:SETF (%SIZE SELF-000) (%SIZE SELF)) (CL:SETF (%BUCKET-TABLE SELF-000) (%THE-TABLE SELF))
   (CL:SETF (%BUCKET-INDEX SELF-000) 0) (CL:SETF (%BUCKET-CURSOR SELF-000) NULL) (CL:LET* ((VALUE-000 SELF-000)) (CL:RETURN-FROM ALLOCATE-ITERATOR VALUE-000))))

;;; (DEFMETHOD (NEXT? BOOLEAN) ...)

(CL:DEFMETHOD NEXT? ((SELF STELLA-HASH-TABLE-ITERATOR))
  (CL:LET* ((CURSOR (%BUCKET-CURSOR SELF)))
   (CL:COND ((%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)) ((CL:NOT (CL:EQ CURSOR NULL)) (CL:SETQ CURSOR (%REST CURSOR))))
   (CL:WHEN (CL:EQ CURSOR NULL)
    (CL:LET* ((TABLE (%BUCKET-TABLE SELF)) (INDEX (%BUCKET-INDEX SELF)) (SIZE (%SIZE SELF))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE) (CL:TYPE CL:FIXNUM INDEX SIZE))
     (CL:WHEN (NULL-ARRAY? TABLE) (CL:RETURN-FROM NEXT? CL:NIL))
     (CL:LOOP WHILE (CL:AND (CL:EQ CURSOR NULL) (CL:< INDEX SIZE)) DO (CL:SETQ CURSOR (CL:AREF TABLE INDEX)) (CL:SETQ INDEX (CL:1+ INDEX)))
     (CL:SETF (%BUCKET-INDEX SELF) INDEX) (CL:SETF (%BUCKET-CURSOR SELF) CURSOR)))
   (CL:COND
    ((CL:NOT (CL:EQ CURSOR NULL)) (CL:SETF (%KEY SELF) (%KEY CURSOR)) (CL:SETF (%VALUE SELF) (%VALUE CURSOR)) (CL:SETF (%BUCKET-CURSOR SELF) CURSOR)
...
(CL:DEFMETHOD VALUE-SETTER ((SELF STELLA-HASH-TABLE-ITERATOR) VALUE)
  (CL:LET* ((CURSOR (%BUCKET-CURSOR SELF))) (CL:WHEN (CL:NOT (CL:EQ CURSOR NULL)) (CL:SETF (%VALUE CURSOR) VALUE)) (CL:SETF (%VALUE SELF) VALUE)
...
(CL:DEFMETHOD KEY-SETTER ((SELF STELLA-HASH-TABLE-ITERATOR) KEY)
  (CL:ERROR (NEW-STELLA-EXCEPTION "Cannot change the key of a STELLA-HASH-TABLE item")))
...
equality test (and `equal-hash-code' as the hash function), otherwise,
use `eql?' (and `hash-code') (the default)." :ALLOCATION :INSTANCE :ACCESSOR %EQUAL-TEST?)
   (INITIAL-SIZE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "If supplied, the initial table
will be sized to hold at least that many elements." :ALLOCATION :INSTANCE :ACCESSOR %INITIAL-SIZE)
   (CROSSOVER-POINT :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %CROSSOVER-POINT))
...
(CL:DEFUN NEW-KEY-VALUE-MAP ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE KEY-VALUE-MAP))) (CL:SETF (%CROSSOVER-POINT SELF) *KEY-VALUE-MAP-CROSSOVER-POINT*)
   (CL:SETF (%INITIAL-SIZE SELF) *KEY-VALUE-MAP-CROSSOVER-POINT*) (CL:SETF (%EQUAL-TEST? SELF) CL:NIL) (CL:SETF (%THE-MAP SELF) NULL)
...
(CL:DEFUN ACCESS-KEY-VALUE-MAP-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-THE-MAP) (CL:IF SETVALUE? (CL:SETF (%THE-MAP SELF) VALUE) (CL:SETQ VALUE (%THE-MAP SELF))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-EQUAL-TEST?)
    (CL:IF SETVALUE? (CL:SETF (%EQUAL-TEST? SELF) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE)) (CL:SETQ VALUE (CL:IF (%EQUAL-TEST? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-INITIAL-SIZE)
    (CL:IF SETVALUE? (CL:SETF (%INITIAL-SIZE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%INITIAL-SIZE SELF)))))
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-CROSSOVER-POINT)
    (CL:IF SETVALUE? (CL:SETF (%CROSSOVER-POINT SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%CROSSOVER-POINT SELF)))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
return its value, or NULL if no such entry exists.    Uses an `eql?'
test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((MAP (%THE-MAP SELF)) (CROSSOVER (%CROSSOVER-POINT SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CROSSOVER))
   (CL:COND ((CL:= CROSSOVER 0) (CL:RETURN-FROM LOOKUP (STELLA-HASH-TABLE-LOOKUP MAP KEY)))
    (CL:T
     (CL:LET* ((CURSOR MAP))
      (CL:IF (%EQUAL-TEST? SELF)
       (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:WHEN (EQUAL? (%KEY CURSOR) KEY) (CL:RETURN-FROM LOOKUP (%VALUE CURSOR))) (CL:SETQ CURSOR (%REST CURSOR)))
  ...
  (CL:LET* ((MAP (%THE-MAP SELF)) (CROSSOVER (%CROSSOVER-POINT SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CROSSOVER))
   (CL:WHEN (CL:EQ MAP NULL)
    (CL:COND
     ((CL:> (%INITIAL-SIZE SELF) *KEY-VALUE-MAP-CROSSOVER-POINT*)
      (CL:LET* ((SELF-000 (NEW-STELLA-HASH-TABLE))) (CL:SETF (%INITIAL-SIZE SELF-000) (%INITIAL-SIZE SELF)) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF))
   ...
   (CL:COND ((CL:= CROSSOVER 0) (STELLA-HASH-TABLE-INSERT-AT MAP KEY VALUE))
    (CL:T
     (CL:LET* ((CURSOR MAP))
      (CL:IF (%EQUAL-TEST? SELF)
       (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:WHEN (EQUAL? (%KEY CURSOR) KEY) (CL:SETF (%VALUE CURSOR) VALUE) (CL:RETURN-FROM INSERT-AT))
        (CL:SETQ CURSOR (%REST CURSOR)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:SETF (%VALUE CURSOR) VALUE) (CL:RETURN-FROM INSERT-AT))
        (CL:SETQ CURSOR (%REST CURSOR))))
      (CL:SETQ CURSOR (KV-CONS KEY VALUE MAP)) (CL:SETF (%THE-MAP SELF) CURSOR) (CL:SETQ CROSSOVER (CL:1- CROSSOVER)) (CL:SETF (%CROSSOVER-POINT SELF) CROSSOVER)
      (CL:WHEN (CL:= CROSSOVER 0) (CL:LET* ((SELF-001 (NEW-STELLA-HASH-TABLE))) (CL:SETF (%EQUAL-TEST? SELF-001) (%EQUAL-TEST? SELF)) (CL:SETQ MAP SELF-001))
       (CL:SETF (%THE-MAP SELF) MAP)
       (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 CURSOR))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ KEY (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000)) (STELLA-HASH-TABLE-INSERT-AT MAP KEY VALUE)
         (CL:SETQ ITER-000 (%REST ITER-000)))))))))
  :VOID)
   ...
   (CL:COND ((CL:= CROSSOVER 0) (STELLA-HASH-TABLE-REMOVE-AT MAP KEY))
    (CL:T
     (CL:LET* ((CURSOR MAP) (TRAILER MAP))
      (CL:IF (%EQUAL-TEST? SELF)
       (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:WHEN (EQUAL? (%KEY CURSOR) KEY) (CL:RETURN)) (CL:SETQ TRAILER CURSOR) (CL:SETQ CURSOR (%REST CURSOR)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NULL)) DO (CL:WHEN (EQL? (%KEY CURSOR) KEY) (CL:RETURN)) (CL:SETQ TRAILER CURSOR) (CL:SETQ CURSOR (%REST CURSOR))))
      (CL:COND ((CL:EQ CURSOR NULL) (CL:RETURN-FROM REMOVE-AT)) ((CL:EQ CURSOR TRAILER) (CL:SETF (%THE-MAP SELF) (%REST CURSOR)))
...
(CL:DEFMETHOD LENGTH ((SELF KEY-VALUE-MAP))
  ...
  (CL:LET* ((MAP (%THE-MAP SELF)))
   (CL:COND ((CL:EQ MAP NULL) (CL:RETURN-FROM LENGTH 0)) ((CL:= (%CROSSOVER-POINT SELF) 0) (CL:RETURN-FROM LENGTH (LENGTH MAP)))
...
(CL:DEFMETHOD EMPTY? ((SELF KEY-VALUE-MAP))
  "Return TRUE if `self' has zero entries."
  (CL:LET* ((MAP (%THE-MAP SELF)))
   (CL:COND ((CL:EQ MAP NULL) (CL:RETURN-FROM EMPTY? CL:T)) ((CL:= (%CROSSOVER-POINT SELF) 0) (CL:RETURN-FROM EMPTY? (CL:= (LENGTH MAP) 0)))
...
(CL:DEFMETHOD NON-EMPTY? ((SELF KEY-VALUE-MAP))
  "Return TRUE if `self' has at least 1 entry."
  (CL:LET* ((MAP (%THE-MAP SELF)))
   (CL:COND ((CL:EQ MAP NULL) (CL:RETURN-FROM NON-EMPTY? CL:NIL)) ((CL:= (%CROSSOVER-POINT SELF) 0) (CL:RETURN-FROM NON-EMPTY? (> (LENGTH MAP) 0)))
...
allocated, however, the keys and values of entries are not copied themselves
 (similar to what we do for lists, etc.)."
  (CL:LET* ((SELF-000 (NEW-KEY-VALUE-MAP))) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF)) (CL:SETF (%INITIAL-SIZE SELF-000) (%INITIAL-SIZE SELF))
   (CL:SETF (%CROSSOVER-POINT SELF-000) (%CROSSOVER-POINT SELF))
   (CL:LET* ((COPY SELF-000) (MAP (%THE-MAP SELF)))
    (CL:IF (CL:= (%CROSSOVER-POINT SELF) 0) (CL:SETF (%THE-MAP COPY) (COPY MAP)) (CL:SETF (%THE-MAP COPY) (COPY-KV-CONS-LIST MAP))) (CL:RETURN-FROM COPY COPY))))
...
to corruption or undefined results."
  (CL:LET* ((MAP (%THE-MAP SELF)))
   (CL:COND ((CL:= (%CROSSOVER-POINT SELF) 0) (CL:RETURN-FROM ALLOCATE-ITERATOR (ALLOCATE-ITERATOR MAP)))
    (CL:T
     (CL:LET* ((SELF-000 (NEW-KEY-VALUE-LIST))) (CL:SETF (%THE-KV-LIST SELF-000) MAP)
      (CL:LET* ((VALUE-000 (ALLOCATE-ITERATOR SELF-000))) (CL:RETURN-FROM ALLOCATE-ITERATOR VALUE-000)))))))
...
(CL:DEFUN NEW-HASH-SET ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE HASH-SET))) (CL:SETF (%CROSSOVER-POINT SELF) *KEY-VALUE-MAP-CROSSOVER-POINT*)
   (CL:SETF (%INITIAL-SIZE SELF) *KEY-VALUE-MAP-CROSSOVER-POINT*) (CL:SETF (%EQUAL-TEST? SELF) CL:NIL) (CL:SETF (%THE-MAP SELF) NULL) (CL:RETURN-FROM NEW-HASH-SET SELF)))
...
(CL:DEFUN HASH-SET (CL:&REST VALUES)
  ...
  (CL:LET* ((SET (NEW-HASH-SET))) (CL:LET* ((V NULL) (ITER-000 VALUES)) (CL:LOOP WHILE ITER-000 DO (CL:SETQ V (CL:POP ITER-000)) (INSERT-AT SET V V)))
...
already is a HASH-SET)."
  (CL:LET* ((CHOOSE-VALUE-000 NULL))
   (CL:IF (ISA? SELF SGT-COLLECTIONS-STELLA-HASH-SET) (CL:SETQ CHOOSE-VALUE-000 SELF)
    (CL:LET* ((SELF-000 (NEW-HASH-SET))) (CL:SETF (%EQUAL-TEST? SELF-000) EQUALTEST?) (CL:SETQ CHOOSE-VALUE-000 SELF-000)))
   (CL:LET* ((THESET CHOOSE-VALUE-000))
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
     (CL:COND ((SUBTYPE-OF? TEST-VALUE-000 SGT-COLLECTIONS-STELLA-HASH-SET) (CL:PROGN))
      ((CL:EQ TEST-VALUE-000 SGT-COLLECTIONS-STELLA-CONS)
       (CL:PROGN
        (CL:LET* ((ELT NULL) (ITER-000 SELF))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-000)) (INSERT-AT THESET ELT ELT) (CL:SETQ ITER-000 (%%REST ITER-000))))))
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-COLLECTIONS-STELLA-LIST)
       (CL:PROGN
        (CL:LET* ((ELT NULL) (ITER-001 (%THE-CONS-LIST SELF)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-001)) (INSERT-AT THESET ELT ELT) (CL:SETQ ITER-001 (%%REST ITER-001))))))
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-COLLECTIONS-STELLA-VECTOR)
       (CL:PROGN
        (CL:LET* ((ELT NULL) (VECTOR-000 SELF) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000))) (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
         (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO (CL:SETQ ELT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000)) (INSERT-AT THESET ELT ELT)
          (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))))
      ((SUBTYPE-OF? TEST-VALUE-000 SGT-COLLECTIONS-STELLA-COLLECTION)
       (CL:PROGN
        (CL:LET* ((ELT NULL) (ITER-002 (ALLOCATE-ITERATOR SELF))) (CL:LOOP WHILE (NEXT? ITER-002) DO (CL:SETQ ELT (%VALUE ITER-002)) (INSERT-AT THESET ELT ELT)))))
      (CL:T
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "coerce-to-hash-set: don't know how to coerce a `" (PRIMARY-TYPE SELF) "' into a hash-set")
        (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
...
returns TRUE or FALSE.  Returns `self'."
  (CL:LET* ((ELEMENT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELEMENT (%VALUE ITER-000)) (CL:WHEN (CL:FUNCALL TEST? ELEMENT) (REMOVE-AT SELF ELEMENT))))
   ...
   (CL:WHEN (CL:NOT (CL:EQ SETMAP NULL))
    (CL:COND
     ((CL:= (%CROSSOVER-POINT SELF) 0)
      (CL:LET* ((MAP SETMAP) (TABLE (%THE-TABLE MAP)) (BUCKET NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR TABLE))
       (CL:WHEN (CL:NOT (NULL-ARRAY? TABLE))
        (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- (%SIZE MAP)))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
         (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETQ BUCKET (CL:AREF TABLE I))
          (CL:WHEN (CL:NOT (CL:EQ BUCKET NULL)) (CL:SETQ ELEMENT (%KEY BUCKET)) (CL:RETURN)) (CL:SETQ ITER-000 (CL:1+ ITER-000)))))))
     (CL:T (CL:LET* ((MAP (%THE-MAP SELF))) (CL:WHEN (CL:NOT (CL:EQ MAP NULL)) (CL:SETQ ELEMENT (%KEY MAP)))))))
   (CL:WHEN (CL:NOT (CL:EQ ELEMENT NULL)) (REMOVE-AT SELF ELEMENT)) (CL:RETURN-FROM POP ELEMENT)))
...
if `equal-test?' of `self' is TRUE."
  (CL:WHEN (CL:NOT (MEMBER? SELF NEW)) (REMOVE-AT SELF OLD) (INSERT-AT SELF NEW NEW))
  (CL:RETURN-FROM SUBSTITUTE SELF))
...
do for lists, etc.)."
  (CL:LET* ((SELF-000 (NEW-HASH-SET))) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF)) (CL:SETF (%INITIAL-SIZE SELF-000) (%INITIAL-SIZE SELF))
   (CL:SETF (%CROSSOVER-POINT SELF-000) (%CROSSOVER-POINT SELF))
   (CL:LET* ((COPY SELF-000) (MAP (%THE-MAP SELF)))
    (CL:IF (CL:= (%CROSSOVER-POINT SELF) 0) (CL:SETF (%THE-MAP COPY) (COPY MAP)) (CL:SETF (%THE-MAP COPY) (COPY-KV-CONS-LIST MAP))) (CL:RETURN-FROM COPY COPY))))
...
(CL:DEFMETHOD CONSIFY ((SELF HASH-SET))
  ...
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((VALUE NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS VALUE NIL)) (CL:IF (CL:EQ RESULT NIL) (CL:SETQ RESULT COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST RESULT COLLECT-000)))
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((TEST-VALUE-000 CL:NIL)) (CL:SETQ TEST-VALUE-000 (<= (LENGTH SELF) (LENGTH OTHERSET)))
   (CL:WHEN TEST-VALUE-000
    (CL:LET* ((ALWAYS?-000 CL:T))
     (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:WHEN (CL:EQ (LOOKUP OTHERSET ELT) NULL) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))))
     (CL:SETQ TEST-VALUE-000 ALWAYS?-000)))
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((TEST-VALUE-000 CL:NIL)) (CL:SETQ TEST-VALUE-000 (CL:= (LENGTH SELF) (LENGTH OTHERSET)))
   (CL:WHEN TEST-VALUE-000
    (CL:LET* ((ALWAYS?-000 CL:T))
     (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:WHEN (CL:EQ (LOOKUP OTHERSET ELT) NULL) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))))
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((SELF-000 (NEW-HASH-SET))) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF))
   (CL:LET* ((RESULT SELF-000) (AUX SELF)) (CL:WHEN (CL:> (LENGTH SELF) (LENGTH OTHERSET)) (CL:SETQ SELF OTHERSET) (CL:SETQ OTHERSET AUX))
    (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:WHEN (CL:NOT (CL:EQ (LOOKUP OTHERSET ELT) NULL)) (INSERT-AT RESULT ELT ELT))))
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((RESULT NULL) (AUX SELF)) (CL:WHEN (CL:< (LENGTH SELF) (LENGTH OTHERSET)) (CL:SETQ SELF OTHERSET) (CL:SETQ OTHERSET AUX)) (CL:SETQ RESULT (COPY SELF))
   (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR OTHERSET))) (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (INSERT-AT RESULT ELT ELT)))
...
by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((RESULT NULL))
   (CL:COND
    ((CL:> (LENGTH SELF) (CL:* (LENGTH OTHERSET) 2)) (CL:SETQ RESULT (COPY SELF))
     (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR OTHERSET)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:WHEN (CL:NOT (CL:EQ (LOOKUP RESULT ELT) NULL)) (REMOVE-AT RESULT ELT)))))
    (CL:T (CL:LET* ((SELF-000 (NEW-HASH-SET))) (CL:SETF (%EQUAL-TEST? SELF-000) (%EQUAL-TEST? SELF)) (CL:SETQ RESULT SELF-000))
     (CL:LET* ((ELT NULL) (ITER-001 (ALLOCATE-ITERATOR SELF)))
      (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ ELT (%VALUE ITER-001)) (CL:WHEN (CL:EQ (LOOKUP OTHERSET ELT) NULL) (INSERT-AT RESULT ELT ELT))))))
...
test by default or `equal?' if `equal-test?' of `self' is TRUE."
  (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:WHEN (CL:NOT (CL:EQ (LOOKUP OTHERSET ELT) NULL)) (REMOVE-AT SELF ELT))))
...
This is equivalent to calling `equivalent-sets?'."
  (CL:COND ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE Y) SGT-COLLECTIONS-STELLA-HASH-SET) (CL:PROGN (CL:RETURN-FROM OBJECT-EQUAL? (EQUIVALENT-SETS? X Y)))) (CL:T))
  (CL:RETURN-FROM OBJECT-EQUAL? CL:NIL))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CODE 7971569)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LET* ((ELT NULL) (ITER-000 (ALLOCATE-ITERATOR SELF)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ELT (%VALUE ITER-000)) (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (EQUAL-HASH-CODE ELT)))))))
   (CL:RETURN-FROM EQUAL-HASH-CODE CODE)))

;;; (DEFCLASS ABSTRACT-DIMENSIONAL-ARRAY ...)

(CL:DEFCLASS ABSTRACT-DIMENSIONAL-ARRAY (ABSTRACT-COLLECTION)
  ()
  (:DOCUMENTATION "Array objects that are aware of their dimensions."))

;;; (DEFCLASS 1D-ARRAY ...)

(CL:DEFCLASS 1D-ARRAY (ABSTRACT-DIMENSIONAL-ARRAY)
  ((DIM1 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM1) (THE-ARRAY :ALLOCATION :INSTANCE :ACCESSOR %THE-ARRAY))
...
(CL:DEFMETHOD INITIALIZE-ARRAY ((SELF 1D-ARRAY) INITIALVALUE)
  ...
  (CL:LET* ((ARRAY (%THE-ARRAY SELF))) (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR ARRAY))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- (LENGTH SELF)))) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETF (CL:AREF ARRAY I) INITIALVALUE) (CL:SETQ ITER-000 (CL:1+ ITER-000)))))
  :VOID)
...
(CL:DEFCLASS 1D-FLOAT-ARRAY (ABSTRACT-DIMENSIONAL-ARRAY)
  ((DIM1 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM1) (THE-ARRAY :ALLOCATION :INSTANCE :ACCESSOR %THE-ARRAY))
...
(CL:DEFCLASS 2D-ARRAY (ABSTRACT-DIMENSIONAL-ARRAY)
  ((DIM2 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM2)
   (DIM1 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM1) (THE-ARRAY :ALLOCATION :INSTANCE :ACCESSOR %THE-ARRAY))
...
(CL:DEFMETHOD LENGTH ((SELF 2D-ARRAY))
  "Return the total number of elements in `self'."
  (CL:RETURN-FROM LENGTH (CL:* (%DIM1 SELF) (%DIM2 SELF))))
...
;;; (DEFCLASS 2D-FLOAT-ARRAY ...)

(CL:DEFCLASS 2D-FLOAT-ARRAY (ABSTRACT-DIMENSIONAL-ARRAY)
  ((DIM2 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM2)
   (DIM1 :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %DIM1) (THE-ARRAY :ALLOCATION :INSTANCE :ACCESSOR %THE-ARRAY))
...
(CL:DEFUN HELP-STARTUP-COLLECTIONS1 ()
  (CL:PROGN (CL:SETQ SGT-COLLECTIONS-STELLA-COLLECTION (INTERN-RIGID-SYMBOL-WRT-MODULE "COLLECTION" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-SET (INTERN-RIGID-SYMBOL-WRT-MODULE "SET" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-VECTOR (INTERN-RIGID-SYMBOL-WRT-MODULE "VECTOR" NULL 0)) (CL:SETQ KWD-COLLECTIONS-CPP (INTERN-RIGID-SYMBOL-WRT-MODULE "CPP" NULL 2))
   (CL:SETQ KWD-COLLECTIONS-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   (CL:SETQ SGT-COLLECTIONS-STELLA-VECTOR (INTERN-RIGID-SYMBOL-WRT-MODULE "VECTOR" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-VECTOR-SEQUENCE (INTERN-RIGID-SYMBOL-WRT-MODULE "VECTOR-SEQUENCE" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-HEAP (INTERN-RIGID-SYMBOL-WRT-MODULE "HEAP" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-PREDICATE (INTERN-RIGID-SYMBOL-WRT-MODULE "PREDICATE" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-FILL-POINTER (INTERN-RIGID-SYMBOL-WRT-MODULE "FILL-POINTER" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-STELLA-HASH-TABLE (INTERN-RIGID-SYMBOL-WRT-MODULE "STELLA-HASH-TABLE" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-SIZE (INTERN-RIGID-SYMBOL-WRT-MODULE "SIZE" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-INITIAL-SIZE (INTERN-RIGID-SYMBOL-WRT-MODULE "INITIAL-SIZE" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-FREE-ELEMENTS (INTERN-RIGID-SYMBOL-WRT-MODULE "FREE-ELEMENTS" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-EQUAL-TEST? (INTERN-RIGID-SYMBOL-WRT-MODULE "EQUAL-TEST?" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-STELLA-HASH-TABLE-ITERATOR (INTERN-RIGID-SYMBOL-WRT-MODULE "STELLA-HASH-TABLE-ITERATOR" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-BUCKET-INDEX (INTERN-RIGID-SYMBOL-WRT-MODULE "BUCKET-INDEX" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-BUCKET-CURSOR (INTERN-RIGID-SYMBOL-WRT-MODULE "BUCKET-CURSOR" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-KEY-VALUE-MAP (INTERN-RIGID-SYMBOL-WRT-MODULE "KEY-VALUE-MAP" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-THE-MAP (INTERN-RIGID-SYMBOL-WRT-MODULE "THE-MAP" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-CROSSOVER-POINT (INTERN-RIGID-SYMBOL-WRT-MODULE "CROSSOVER-POINT" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-HASH-SET (INTERN-RIGID-SYMBOL-WRT-MODULE "HASH-SET" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-LIST (INTERN-RIGID-SYMBOL-WRT-MODULE "LIST" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-1D-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "1D-ARRAY" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-OBJECT-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "OBJECT-ARRAY" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-1D-FLOAT-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "1D-FLOAT-ARRAY" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-FLOAT-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "FLOAT-ARRAY" NULL 0))
   (CL:SETQ SGT-COLLECTIONS-STELLA-2D-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "2D-ARRAY" NULL 1))
   (CL:SETQ SGT-COLLECTIONS-STELLA-2D-FLOAT-ARRAY (INTERN-RIGID-SYMBOL-WRT-MODULE "2D-FLOAT-ARRAY" NULL 1))
   (CL:SETQ SYM-COLLECTIONS-STELLA-STARTUP-COLLECTIONS (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-COLLECTIONS" NULL 0))
   (CL:SETQ SYM-COLLECTIONS-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" NULL 0)))
  :VOID)
...
(CL:DEFUN HELP-STARTUP-COLLECTIONS2 ()
  (CL:PROGN
   (CL:LET*
    ((CLASS
      (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "HEAP" "(DEFCLASS HEAP (VECTOR) :DOCUMENTATION \"Implements a Min or Max heap depending on the semantics
of `predicate' (Min if `predicate' has a `<' semantics).  This is useful
for in-place sorting (even though we have specialzed sort routines for that)
or to maintain top-N lists with log(N) insertion time.  We place this under
VECTOR instead of VECTOR-SEQUENCE for now, since sequential order isn't
really maintained or accessible until we sort the heap.\" :PUBLIC? TRUE :PARAMETERS ((ANY-VALUE :TYPE OBJECT)) :SLOTS ((PREDICATE :TYPE FUNCTION-CODE :REQUIRED? TRUE) (FILL-POINTER :TYPE INTEGER :INITIALLY 0)) :INITIALIZER INITIALIZE-HEAP)")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-HEAP)) (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-HEAP-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
...
use `eql?' and `hash-code' (the default).\")))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-STELLA-HASH-TABLE))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-STELLA-HASH-TABLE-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
...
to corruption or undefined results.\" :SLOTS ((SIZE :TYPE INTEGER) (BUCKET-TABLE :TYPE (ARRAY () OF KV-CONS)) (BUCKET-INDEX :TYPE INTEGER :INITIALLY 0) (BUCKET-CURSOR :TYPE KV-CONS)))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-STELLA-HASH-TABLE-ITERATOR))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-STELLA-HASH-TABLE-ITERATOR-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
      (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "KEY-VALUE-MAP" "(DEFCLASS KEY-VALUE-MAP (DICTIONARY) :DOCUMENTATION \"Full-featured dictionary class that supports `eql?' or
`equal?' equality tests, O(1) access operations even for large numbers
of entries by using a hash table, light-weight KV-CONS representation for
small tables and iteration even if the dictionary is represented by a
...
will be sized to hold at least that many elements.\") (CROSSOVER-POINT :TYPE INTEGER :INITIALLY *KEY-VALUE-MAP-CROSSOVER-POINT*)))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-KEY-VALUE-MAP)) (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-KEY-VALUE-MAP-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
...
less equivalent to VECTOR (fewer methods) but kept here for symmetry.\" :PUBLIC? TRUE :SYNONYMS (OBJECT-ARRAY) :PARAMETERS ((ANY-VALUE :TYPE OBJECT)))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-1D-ARRAY)))
   (CL:LET*
    ((CLASS
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-1D-FLOAT-ARRAY)))
   (CL:LET*
    ((CLASS
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-2D-ARRAY)))
   (CL:LET*
    ((CLASS
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-2D-FLOAT-ARRAY))))
  :VOID)
...
(CL:DEFUN HELP-STARTUP-COLLECTIONS3 ()
  (CL:PROGN
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE-DUPLICATES (LIKE SELF)) ((SELF COLLECTION)) :DOCUMENTATION \"Return `self' with duplicates removed.  Preserves the
original order of the remaining members.\" :PUBLIC? TRUE :ABSTRACT? TRUE)" NULL NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NO-DUPLICATES? BOOLEAN) ((SELF SET)) :PUBLIC? TRUE)" (CL:FUNCTION NO-DUPLICATES?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (ORDERED? BOOLEAN) ((SELF COLLECTION)) :PUBLIC? TRUE :DOCUMENTATION \"Return `true' if the collection `self' is ordered.\")"
    (CL:FUNCTION ORDERED?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (ORDERED? BOOLEAN) ((SELF SEQUENCE)) :PUBLIC? TRUE)" (CL:FUNCTION ORDERED?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (ORDERED? BOOLEAN) ((SELF SET)) :PUBLIC? TRUE)" (CL:FUNCTION ORDERED?) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE-DUPLICATES (LIKE SELF)) ((SELF SET)))" (CL:FUNCTION REMOVE-DUPLICATES) NULL)
   (DEFINE-METHOD-OBJECT
...
unless `new' is already a member.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBSTITUTE) NULL)
   (DEFINE-METHOD-OBJECT
...
not relying on the somewhat less efficient variable arguments mechanism.\" :PUBLIC? TRUE)" (CL:FUNCTION CONCATENATE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (OBJECT-EQUAL? BOOLEAN) ((X SET) (Y OBJECT)) :DOCUMENTATION \"Return TRUE iff `x' and `y' are SET's with equivalent members.
Uses `equal?' to test equality of elements.  This is more general than
`equivalent-sets?', since that only uses an `eql?' test.\" :PUBLIC? TRUE)" (CL:FUNCTION OBJECT-EQUAL?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EQUAL-HASH-CODE INTEGER) ((SELF SET)) :DOCUMENTATION \"Return an `equal?' hash code for `self'.  Note that this
is O(N) in the number of elements of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL)
   (DEFINE-FUNCTION-OBJECT "SET" "(DEFUN (SET SET) (|&REST| (VALUES OBJECT)) :DOCUMENTATION \"Return a set containing `values', in order.\" :PUBLIC? TRUE)"
    (CL:FUNCTION SET) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD PRINT-VECTOR ((SELF VECTOR) (STREAM NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION PRINT-VECTOR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF VECTOR)) :DOCUMENTATION \"Return `true' if `self' has length 0.\" :PUBLIC? TRUE)" (CL:FUNCTION EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NON-EMPTY? BOOLEAN) ((SELF VECTOR)) :DOCUMENTATION \"Return `true' if `self' has length > 0.\" :PUBLIC? TRUE)"
    (CL:FUNCTION NON-EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (FIRST (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NTH SELF 0)))" (CL:FUNCTION FIRST)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (SECOND (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NTH SELF 1)))" (CL:FUNCTION SECOND)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (THIRD (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NTH SELF 2)))" (CL:FUNCTION THIRD)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (FOURTH (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NTH SELF 3)))" (CL:FUNCTION FOURTH)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (FIFTH (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (NTH SELF 4)))" (CL:FUNCTION FIFTH)
    NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LAST (LIKE (ANY-VALUE SELF))) ((SELF VECTOR)) :PUBLIC? TRUE :DOCUMENTATION \"Return the last item in the vector `self'.\")"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LAST-SETTER (LIKE (ANY-VALUE SELF))) ((SELF VECTOR) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION LAST-SETTER)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF VECTOR)) :PUBLIC? TRUE)" (CL:FUNCTION LENGTH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (MEMBER? BOOLEAN) ((SELF VECTOR) (OBJECT OBJECT)) :PUBLIC? TRUE)" (CL:FUNCTION MEMBER?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (POSITION INTEGER) ((SELF VECTOR) (OBJECT OBJECT) (START INTEGER)) :DOCUMENTATION \"Return the position of `object' within the vector
`self' (counting from zero); or return `null' if `object' does not occur within 
`self' (uses an `eql?' test).  If `start' was supplied as non-`null', only 
consider the portion starting at `start', however, the returned position 
will always be relative to the entire vector.\" :PUBLIC? TRUE)" (CL:FUNCTION POSITION) NULL)
   (DEFINE-METHOD-OBJECT
...
`self' (uses an `eql?' test).  If `end' was supplied as non-`null', only 
consider the portion ending at index `end', however, the returned position 
will always be relative to the entire vector.\" :PUBLIC? TRUE)" (CL:FUNCTION LAST-POSITION) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT-AT ((SELF VECTOR) (OFFSET INTEGER) (VALUE (LIKE (ANY-VALUE SELF)))))" (CL:FUNCTION INSERT-AT) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY (VECTOR OF (LIKE (ANY-VALUE SELF)))) ((SELF VECTOR)) :DOCUMENTATION \"Return a copy of the vector `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF VECTOR)) :PUBLIC? TRUE)" (CL:FUNCTION CLEAR) NULL)
   (DEFINE-FUNCTION-OBJECT "VECTOR" "(DEFUN (VECTOR VECTOR) (|&REST| (VALUES OBJECT)) :DOCUMENTATION \"Return a vector containing `values', in order.\" :PUBLIC? TRUE)"
    (CL:FUNCTION VECTOR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (OBJECT-EQUAL? BOOLEAN) ((X VECTOR) (Y OBJECT)) :DOCUMENTATION \"Return TRUE iff the vectors `x' and `y' are structurally
equivalent.  Uses `equal?' to test equality of elements.\" :PUBLIC? TRUE)" (CL:FUNCTION OBJECT-EQUAL?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EQUAL-HASH-CODE INTEGER) ((SELF VECTOR)) :DOCUMENTATION \"Return an `equal?' hash code for `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION EQUAL-HASH-CODE) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT-AT ((SELF EXTENSIBLE-VECTOR) (OFFSET INTEGER) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION INSERT-AT) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT-AT ((SELF EXTENSIBLE-SYMBOL-ARRAY) (OFFSET INTEGER) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)" (CL:FUNCTION INSERT-AT)
    NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION NON-EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF VECTOR-SEQUENCE)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (SEQUENCE-LENGTH SELF)))"
    (CL:FUNCTION LENGTH) NULL)
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD (LAST (LIKE (ANY-VALUE SELF))) ((SELF VECTOR-SEQUENCE)) :PUBLIC? TRUE :DOCUMENTATION \"Return the last item in the vector `self'.\")" (CL:FUNCTION LAST)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LAST-SETTER (LIKE (ANY-VALUE SELF))) ((SELF VECTOR-SEQUENCE) (VALUE (LIKE (ANY-VALUE SELF)))) :PUBLIC? TRUE)"
    (CL:FUNCTION LAST-SETTER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT ((SELF VECTOR-SEQUENCE) (VALUE (LIKE (ANY-VALUE SELF)))) :DOCUMENTATION \"Append `value' to the END of the sequence `self'.  
...
the values after it to close the gap.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-FUNCTION-OBJECT "COPY-VECTOR-SEQUENCE" "(DEFUN COPY-VECTOR-SEQUENCE ((SOURCE VECTOR-SEQUENCE) (COPY VECTOR-SEQUENCE)))" (CL:FUNCTION COPY-VECTOR-SEQUENCE)
    NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF VECTOR-SEQUENCE)) :DOCUMENTATION \"Clear `self' by setting its active length to zero.\" :PUBLIC? TRUE)"
    (CL:FUNCTION CLEAR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (OBJECT-EQUAL? BOOLEAN) ((X VECTOR-SEQUENCE) (Y OBJECT)) :DOCUMENTATION \"Return TRUE iff the sequences `x' and `y' are structurally
equivalent.  Uses `equal?' to test equality of elements.\" :PUBLIC? TRUE)" (CL:FUNCTION OBJECT-EQUAL?) NULL))
  :VOID)

(CL:DEFUN HELP-STARTUP-COLLECTIONS4 ()
  (CL:PROGN (DEFINE-METHOD-OBJECT "(DEFMETHOD (ALLOCATE-ITERATOR (ITERATOR OF (LIKE (ANY-VALUE SELF)))) ((SELF VECTOR)))" (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
   (DEFINE-FUNCTION-OBJECT "VECTOR-NEXT?" "(DEFUN (VECTOR-NEXT? BOOLEAN) ((SELF ALL-PURPOSE-ITERATOR)))" (CL:FUNCTION VECTOR-NEXT?) NULL)
   (DEFINE-METHOD-OBJECT
...
it will cause value wrapping everytime `slot' is read.\" :PUBLIC? TRUE)" (CL:FUNCTION SORT-OBJECTS) NULL)
   (DEFINE-FUNCTION-OBJECT "HEAP-SORT-NATIVE-VECTOR" "(DEFUN HEAP-SORT-NATIVE-VECTOR ((VECTOR NATIVE-VECTOR) (SIZE INTEGER) (PREDICATE FUNCTION-CODE)))"
    (CL:FUNCTION HEAP-SORT-NATIVE-VECTOR) NULL)
   (DEFINE-FUNCTION-OBJECT "HEAP-SORT-HEAPIFY" "(DEFUN HEAP-SORT-HEAPIFY ((VECTOR NATIVE-VECTOR) (SIZE INTEGER) (PREDICATE FUNCTION-CODE)))"
    (CL:FUNCTION HEAP-SORT-HEAPIFY) NULL)
   (DEFINE-FUNCTION-OBJECT "HEAP-SORT-SIFT-DOWN" "(DEFUN HEAP-SORT-SIFT-DOWN ((VECTOR NATIVE-VECTOR) (START INTEGER) (END INTEGER) (PREDICATE FUNCTION-CODE)))"
   ...
   (DEFINE-FUNCTION-OBJECT "QUICK-SORT-NATIVE-VECTOR" "(DEFUN QUICK-SORT-NATIVE-VECTOR ((VECTOR NATIVE-VECTOR) (START INTEGER) (END INTEGER) (PREDICATE FUNCTION-CODE)))"
    (CL:FUNCTION QUICK-SORT-NATIVE-VECTOR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INITIALIZE-HEAP ((SELF HEAP)))" (CL:FUNCTION INITIALIZE-HEAP) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY (HEAP OF (LIKE (ANY-VALUE SELF)))) ((SELF HEAP)) :DOCUMENTATION \"Return a copy of the heap `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF HEAP)) :DOCUMENTATION \"Clear `self' by setting its active length to zero.\" :PUBLIC? TRUE)" (CL:FUNCTION CLEAR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF HEAP)) :DOCUMENTATION \"Return the length of the currently filled portion of `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION LENGTH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF HEAP)) :DOCUMENTATION \"Return TRUE if `self' is empty.\" :PUBLIC? TRUE)" (CL:FUNCTION EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LAST (LIKE (ANY-VALUE SELF))) ((SELF HEAP)) :DOCUMENTATION \"Return the last item in the heap `self' which will be the
largest or best item if `self' is a sorted min-heap with a '<' predicate.\" :PUBLIC? TRUE)" (CL:FUNCTION LAST) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (HEAP-ROOT (LIKE (ANY-VALUE SELF))) ((SELF HEAP)) :DOCUMENTATION \"Return the root of `self' (NULL if `self' is empty).
The root contains the minimum element of a min-heap with '<' predicate.\" :PUBLIC? TRUE)" (CL:FUNCTION HEAP-ROOT) NULL)
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION FAST-HEAP-ROOT) NULL)
   (DEFINE-FUNCTION-OBJECT "HEAP-SIFT-UP"
    "(DEFUN HEAP-SIFT-UP ((HEAP NATIVE-VECTOR) (START INTEGER) (END INTEGER) (VALUE (LIKE (ANY-VALUE SELF))) (PREDICATE FUNCTION-CODE)))" (CL:FUNCTION HEAP-SIFT-UP)
    NULL)
   (DEFINE-FUNCTION-OBJECT "HEAP-SIFT-DOWN"
    "(DEFUN HEAP-SIFT-DOWN ((HEAP NATIVE-VECTOR) (START INTEGER) (END INTEGER) (VALUE (LIKE (ANY-VALUE SELF))) (PREDICATE FUNCTION-CODE)))" (CL:FUNCTION HEAP-SIFT-DOWN)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT ((SELF HEAP) (VALUE (LIKE (ANY-VALUE SELF)))) :DOCUMENTATION \"Insert `value' into `self' and restore the heap property.
Signal an error if there is no more room in `self'.  Maintains `self' as
a Min-heap if `self's `predicate' has `<' semantics; otherwise as a Max-heap.\" :PUBLIC? TRUE)" (CL:FUNCTION INSERT) NULL)
   (DEFINE-METHOD-OBJECT
...
the heap property.  Signal an error if `self' is empty.  Maintains `self' as
a Min-heap if `self's `predicate' has `<' semantics; otherwise as a Max-heap.\" :PUBLIC? TRUE)" (CL:FUNCTION REPLACE-HEAP-ROOT) NULL)
   (DEFINE-METHOD-OBJECT
...
and restore the heap property.  This is useful to build and maintain a
heap with some top-N elements (relative to `predicate') where the root (or
minimum) of `self' is the currently weakest element at the end of the list.\" :PUBLIC? TRUE)" (CL:FUNCTION INSERT-IF-BETTER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD HEAPIFY ((SELF HEAP)) :DOCUMENTATION \"Restore the heap property of `self' according to its
`predicate'.  Normally, this is not needed, since insert operations
preserve the heap property.  However, this can be useful after bulk
insertion of values or if `predicate' has been changed.\" :PUBLIC? TRUE)" (CL:FUNCTION HEAPIFY) NULL)
   (DEFINE-METHOD-OBJECT
...
the heap property.  This is slightly different than a regular heap
sort due to the way HEAP's are maintained; however, the complexity is
the same.\" :PUBLIC? TRUE)" (CL:FUNCTION SORT) NULL)
   (DEFINE-FUNCTION-OBJECT "INITIALIZE-STELLA-HASH-TABLE" "(DEFUN INITIALIZE-STELLA-HASH-TABLE ((SELF STELLA-HASH-TABLE)))" (CL:FUNCTION INITIALIZE-STELLA-HASH-TABLE)
    NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD INITIALIZE-HASH-TABLE ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Initialize the STELLA hash table `self'.  This is a
no-op and primarily exists to shadow the standard initializer inherited
from ABSTRACT-HASH-TABLE.  STELLA hash tables are initialized at the
first insertion operation.\")" (CL:FUNCTION INITIALIZE-HASH-TABLE) NULL)
   (DEFINE-FUNCTION-OBJECT "REHASH-STELLA-HASH-TABLE" "(DEFUN REHASH-STELLA-HASH-TABLE ((SELF STELLA-HASH-TABLE) (NEWSIZE INTEGER)))"
    (CL:FUNCTION REHASH-STELLA-HASH-TABLE) NULL)
   (DEFINE-FUNCTION-OBJECT "STELLA-HASH-TABLE-INSERT-AT" "(DEFUN STELLA-HASH-TABLE-INSERT-AT ((SELF STELLA-HASH-TABLE) (KEY OBJECT) (VALUE OBJECT)))"
    (CL:FUNCTION STELLA-HASH-TABLE-INSERT-AT) NULL)
   (DEFINE-FUNCTION-OBJECT "STELLA-HASH-TABLE-REMOVE-AT" "(DEFUN STELLA-HASH-TABLE-REMOVE-AT ((SELF STELLA-HASH-TABLE) (KEY OBJECT)))"
    (CL:FUNCTION STELLA-HASH-TABLE-REMOVE-AT) NULL)
   (DEFINE-FUNCTION-OBJECT "STELLA-HASH-TABLE-LOOKUP" "(DEFUN (STELLA-HASH-TABLE-LOOKUP (LIKE (ANY-VALUE SELF))) ((SELF STELLA-HASH-TABLE) (KEY OBJECT)))"
    (CL:FUNCTION STELLA-HASH-TABLE-LOOKUP) NULL)
   (DEFINE-FUNCTION-OBJECT "STELLA-STRING-HASH-TABLE-LOOKUP" "(DEFUN (STELLA-STRING-HASH-TABLE-LOOKUP (LIKE (ANY-VALUE SELF))) ((SELF STELLA-HASH-TABLE) (KEY STRING)))"
...
return its value, or NULL if no such entry exists.    Uses an `eql?'
test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION LOOKUP) NULL)
   (DEFINE-METHOD-OBJECT
...
to `value' or add a new entry if no entry with `key' exists yet.  Uses an
`eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION INSERT-AT) NULL)
   (DEFINE-METHOD-OBJECT
...
`eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-AT) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Return the number of entries in `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION LENGTH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Return TRUE if `self' has zero entries.\" :PUBLIC? TRUE)"
    (CL:FUNCTION EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NON-EMPTY? BOOLEAN) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Return TRUE if `self' has at least 1 entry.\" :PUBLIC? TRUE)"
    (CL:FUNCTION NON-EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY (LIKE SELF)) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Return a copy of the hash table `self'.  The bucket table
and buckets are freshly allocated, however, the keys and values of entries
are not copied themselves (similar to what we do for lists, etc.).\" :PUBLIC? TRUE)" (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Remove all entries from `self'.  This will result in a
re-initialization of the table upon the first insertion into `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION CLEAR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (CONSIFY (CONS OF CONS)) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Collect all entries of `self' into a cons list of
`(<key> <value>)' pairs and return the result.\" :PUBLIC? TRUE)" (CL:FUNCTION CONSIFY) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EQUAL-HASH-CODE INTEGER) ((SELF STELLA-HASH-TABLE)) :DOCUMENTATION \"Return an `equal?' hash code for `self'.  Note that this
is O(N) in the number of entries of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NEXT? BOOLEAN) ((SELF STELLA-HASH-TABLE-ITERATOR)))" (CL:FUNCTION NEXT?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (VALUE-SETTER (LIKE (ANY-VALUE SELF))) ((SELF STELLA-HASH-TABLE-ITERATOR) (VALUE (LIKE (ANY-VALUE SELF)))))"
    (CL:FUNCTION VALUE-SETTER) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (KEY-SETTER (LIKE (ANY-KEY SELF))) ((SELF STELLA-HASH-TABLE-ITERATOR) (KEY (LIKE (ANY-KEY SELF)))))" (CL:FUNCTION KEY-SETTER) NULL)
   (DEFINE-METHOD-OBJECT
...
`eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-AT) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (LENGTH INTEGER) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Return the number of entries in `self'.\" :PUBLIC? TRUE)"
    (CL:FUNCTION LENGTH) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EMPTY? BOOLEAN) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Return TRUE if `self' has zero entries.\" :PUBLIC? TRUE)"
    (CL:FUNCTION EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (NON-EMPTY? BOOLEAN) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Return TRUE if `self' has at least 1 entry.\" :PUBLIC? TRUE)"
    (CL:FUNCTION NON-EMPTY?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY (LIKE SELF)) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Return a copy of the map `self'.  All entries are freshly
allocated, however, the keys and values of entries are not copied themselves
 (similar to what we do for lists, etc.).\" :PUBLIC? TRUE)" (CL:FUNCTION COPY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CLEAR ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Reset `self' to have zero entries.\" :PUBLIC? TRUE)" (CL:FUNCTION CLEAR) NULL)
   (DEFINE-METHOD-OBJECT
...
operations allowed during iteration are removal of the current element or
changing its value.  All other removal or insertion operations might lead
to corruption or undefined results.\" :PUBLIC? TRUE)" (CL:FUNCTION ALLOCATE-ITERATOR) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (CONSIFY CONS) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Collect all entries of `self' into a cons list of
`(<key> <value>)' pairs and return the result.\" :PUBLIC? TRUE)" (CL:FUNCTION CONSIFY) NULL)
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION OBJECT-EQUAL?) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (EQUAL-HASH-CODE INTEGER) ((SELF KEY-VALUE-MAP)) :DOCUMENTATION \"Return an `equal?' hash code for `self'.  Note that this
is O(N) in the number of entries of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL))
  :VOID)
...
(CL:DEFUN STARTUP-COLLECTIONS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *KEY-VALUE-MAP-CROSSOVER-POINT* *REMOVE-DUPLICATES-CROSSOVER-POINT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5) (HELP-STARTUP-COLLECTIONS2)) (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 6) (FINALIZE-CLASSES))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-COLLECTIONS3) (HELP-STARTUP-COLLECTIONS4)
    (DEFINE-FUNCTION-OBJECT "HASH-SET"
     "(DEFUN (HASH-SET HASH-SET) (|&REST| (VALUES OBJECT)) :DOCUMENTATION \"Return an `eql?' HASH-SET containing `values'.\" :PUBLIC? TRUE)" (CL:FUNCTION HASH-SET) NULL)
...
equal test if `equalTest?' is TRUE (`equalTest?' will be ignored if `self'
already is a HASH-SET).\" :PUBLIC? TRUE)" (CL:FUNCTION COERCE-TO-HASH-SET) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (MEMBER? BOOLEAN) ((SELF HASH-SET) (OBJECT OBJECT)) :DOCUMENTATION \"Return TRUE iff `object' is a member of the set `self'.
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION MEMBER?) NULL)
    (DEFINE-METHOD-OBJECT
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION INSERT) NULL)
    (DEFINE-METHOD-OBJECT
...
returns TRUE or FALSE.  Returns `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION REMOVE-IF) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (POP (LIKE (ANY-VALUE SELF))) ((SELF HASH-SET)) :DOCUMENTATION \"Remove and return an arbitrary element of the set `self'.
Return NULL if the set is empty.  Performance note: for large sets implemented
via hash tables it takes O(N) to empty out the set with repeated calls to `pop',
since the emptier the table gets, the longer it takes to find an element.
Therefore, it is usually better to use iteration with embedded removals for
...
unless `new' is already a member.  Uses an `eql?' test by default or `equal?'
if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBSTITUTE) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (COPY (LIKE SELF)) ((SELF HASH-SET)) :DOCUMENTATION \"Return a copy of the set `self'.  All entries are freshly
allocated, however, the values are not copied themselves (similar to what we
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBSET?) NULL)
    (DEFINE-METHOD-OBJECT
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUIVALENT-SETS?) NULL)
    (DEFINE-METHOD-OBJECT
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION INTERSECTION) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (UNION HASH-SET) ((SELF HASH-SET) (OTHERSET HASH-SET)) :DOCUMENTATION \"Return the set union of `self' and `otherSet' as a new set.
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION UNION) NULL)
    (DEFINE-METHOD-OBJECT
...
all elements that are in `self' but not in `otherSet').  Uses an `eql?' test
by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION DIFFERENCE) NULL)
    (DEFINE-METHOD-OBJECT
...
removing elements from `self' that also occur in `otherSet'.  Uses an `eql?'
test by default or `equal?' if `equal-test?' of `self' is TRUE.\" :PUBLIC? TRUE)" (CL:FUNCTION SUBTRACT) NULL)
    (DEFINE-METHOD-OBJECT
...
Uses an `eql?' test by default or `equal?' if `equal-test?' of `self' is TRUE.
This is equivalent to calling `equivalent-sets?'.\" :PUBLIC? TRUE)" (CL:FUNCTION OBJECT-EQUAL?) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (EQUAL-HASH-CODE INTEGER) ((SELF HASH-SET)) :DOCUMENTATION \"Return an `equal?' hash code for `self'.  Note that this
is O(N) in the number of elements of `self'.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUAL-HASH-CODE) NULL)
    (DEFINE-METHOD-OBJECT
...
This is useful for fast element-wise iteration that doesn't need arithmetic.\" :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE (RETURN (+ (* I (DIM2 SELF)) J)))" NULL NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-COLLECTIONS" "(DEFUN STARTUP-COLLECTIONS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-COLLECTIONS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-COLLECTIONS-STELLA-STARTUP-COLLECTIONS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-COLLECTIONS-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupCollections") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/STELLA")))
    (REGISTER-NATIVE-NAME SYM-COLLECTIONS-STELLA-VECTOR KWD-COLLECTIONS-CPP KWD-COLLECTIONS-FUNCTION)
...
#endif
\" :OTHERWISE FALSE))")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *STELLA-HASH-TABLE-AVG-BUCKET-LENGTH* FLOAT 1.0)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
