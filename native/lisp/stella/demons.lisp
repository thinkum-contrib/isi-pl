...
(CL:DEFUN INJECT-DEMON (DEMONLIST DEMON)
  ...
  (CL:LET* ((IT (ALLOCATE-ITERATOR DEMONLIST)))
   (CL:LOOP WHILE (NEXT? IT) DO
    (CL:WHEN
     (STRING-EQL? (%DEMON-NAME (%VALUE IT)) (%DEMON-NAME DEMON))
     ...
     (CL:IF (%DEMON-ALL? DEMON)
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE
        ...
        (CL:TAGBODY
         (CL:LET*
          ((SLOT (LOOKUP-SLOT (%SURROGATE-VALUE TYPE) SLOTNAME)))
...
(CL:DEFUN RUN-SLOT-DEMONS (SELF SLOT OLDVALUE NEWVALUE)
  (CL:WHEN (CL:NOT (CL:EQ *ALL-SLOTS-DEMONS* NULL))
   (CL:LET*
    ((DEMON NULL) (ITER-000 (%THE-CONS-LIST *ALL-SLOTS-DEMONS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ DEMON (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:FUNCALL (%DEMON-CODE DEMON) SELF SLOT OLDVALUE NEWVALUE))))
  (CL:WHEN (CL:NOT (CL:EQ (SLOT-DEMONS SLOT) NULL))
   (CL:LET*
    ((DEMON NULL) (ITER-001 (%THE-CONS-LIST (SLOT-DEMONS SLOT))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ DEMON (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (CL:FUNCALL (%DEMON-CODE DEMON) SELF SLOT OLDVALUE NEWVALUE))))
...
(CL:DEFCLASS HOOK-LIST (KEY-VALUE-LIST)
  ((SIGNATURE :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %SIGNATURE)
...
each one to `argument'.\")" (CL:FUNCTION RUN-HOOKS) NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Demons") NULL-STRING-WRAPPER)))
