     ...
     (CL:LET* ((RESULT NULL))
      ...
      (CL:LET* ((FEATURE NULL) (ITER-000 TREE) (COLLECT-000 NULL))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        ...
        (CL:TAGBODY
         (CL:WHEN
          (CL:NOT (ISA? FEATURE SGT-STELLA-IN-STELLA-GENERALIZED-SYMBOL))
       ...
       (CL:LET* ((SLOTDEF NULL) (ITER-004 THEVALUE) (COLLECT-003 NULL))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-004 NIL)) DO
         ...
         (CL:IF (CL:EQ COLLECT-003 NULL)
          (CL:PROGN
           (CL:SETQ COLLECT-003
            (CONS (DEFINE-STORAGE-SLOT-FROM-PARSE-TREE SLOTDEF CLASSTYPE)
             NIL))
               ...
               (CL:PROGN (CL:SETQ COLLECT-004 (CONS (TYPIFY CHILD) NIL))
                (CL:IF (CL:EQ (%THE-CONS-LIST CHILDREN) NIL)
                 ...
                 (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST CHILDREN)
                  COLLECT-004)))
               (CL:PROGN
                (CL:SETF (%%REST COLLECT-004) (CONS (TYPIFY CHILD) NIL))
   ...
   (CL:IF (CL:EQ NAMEMODULE DEFINITIONMODULE)
    ...
    (CL:IF (CL:NOT (VISIBLE-FROM? NAMEMODULE DEFINITIONMODULE))
     ...
     (CL:LET* ((TEST-VALUE-000 CL:NIL))
      (CL:LET* ()
       (CL:SETQ SHADOW
        (LOOKUP-SYMBOL-IN-MODULE (%SYMBOL-NAME NAME) DEFINITIONMODULE CL:T))
...
(CL:DEFUN EXTRACT-OPTIONS (TREE OPTIONSTABLE)
  (CL:LET* ((OPTIONS NULL) (TAIL NULL))
   ...
   (CL:WHEN
    (CL:OR (CL:EQ (%%REST TREE) NIL) (CL:EQ (%%REST (%%REST TREE)) NIL))
...
(CL:DEFUN YIELD-SETTER-METHOD-NAME (METHODNAME)
  (CL:LET*
   ((BASE-NAME (%SYMBOL-NAME METHODNAME))
    (SUFFIX (CL:IF (ALL-UPPER-CASE-STRING? BASE-NAME) "-SETTER" "-Setter")))
...
(CL:DEFUN IN-PLACE-NORMALIZE-METHOD-OPTIONS (METHODOPTIONS METHODNAME)
  ...
  (CL:LET*
   ((KEY NULL) (VALUE NULL) (ITER-000 (ALLOCATE-ITERATOR METHODOPTIONS)))
   ...
   (CL:WHEN (CONS? (%%VALUE PARAMETERSTREE))
    ...
    (CL:WHEN (SYMBOL? (LAST PARAMETERSTREE))
     (CL:SETQ EXISTINGSLOT
      (LOOKUP-SLOT
       (TYPE-SPEC-TO-CLASS (YIELD-TYPE-SPECIFIER (LAST PARAMETERSTREE)))
       NAME))
...
module is specified.\" :PUBLIC? TRUE)" (CL:FUNCTION UNSTRINGIFY-IN-MODULE)
     NULL)
