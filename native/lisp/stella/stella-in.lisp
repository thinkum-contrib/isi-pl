...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *SAFETY* STANDARD-WARNING FALSE-WRAPPER
  STANDARD-OUTPUT STANDARD-ERROR EOL *MODULE*))
...
(CL:DEFUN DEFINE-STORAGE-SLOT-FROM-PARSE-TREE (SLOTDEF OWNER)
  (CL:LET*
   ...
   (CL:LET*
    ((THEVALUE NULL) (THEKEY NULL) (ITER-000 (%THE-PLIST THEOPTIONS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
removed from 'methodTree'."
  (CL:LET*
   ((NAMETREE (%%VALUE (%%REST METHODTREE)))
    (PARAMETERSTREE (%%VALUE (%%REST (%%REST METHODTREE)))))
   ...
   (EXTRACT-OPTIONS METHODTREE OPTIONSTABLE)
...
(CL:DEFUN IN-PLACE-NORMALIZE-METHOD-OPTIONS (METHODOPTIONS METHODNAME)
  ...
  (CL:LET*
   ((VALUE NULL) (KEY NULL) (ITER-000 (ALLOCATE-ITERATOR METHODOPTIONS)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ KEY (%KEY ITER-000))
    ...
    (CL:COND
     ...
     ((CL:OR (CL:EQ KEY KWD-STELLA-IN-PUBLIC?)
       (CL:EQ KEY KWD-STELLA-IN-ABSTRACT?)
       (CL:EQ KEY KWD-STELLA-IN-SETTER?) (CL:EQ KEY KWD-STELLA-IN-NATIVE?)
   ...
   (CL:WHEN (CONS? (%%VALUE PARAMETERSTREE))
    ...
    (CL:WHEN (SYMBOL? (LAST PARAMETERSTREE))
     (CL:SETQ EXISTINGSLOT
      (SAFE-LOOKUP-SLOT (TYPE-TO-CLASS (TYPIFY (LAST PARAMETERSTREE)))
       NAME))
...
(CL:DEFUN DEFINE-METHOD-FROM-PARSE-TREE (PARSETREE)
  ...
  (CL:LET* ((METHOD NULL) (IDENTICALMETHOD? FALSE))
   ...
   (CL:WHEN (CL:NOT (CL:EQ (HOME-MODULE METHOD) *MODULE*))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD)
     SYM-STELLA-IN-STELLA-SLOT-HOME-MODULE *MODULE* NULL))
...
(CL:DEFUN DEFINE-EXTERNAL-SLOT-FROM-PARSE-TREE (PARSETREE)
  ...
  (CL:LET*
   ((CLASSNAME (%%VALUE (%%REST PARSETREE))) (OWNER (TYPIFY CLASSNAME))
    ...
    (MARKER (GET-STELLA-CLASS "SLOT" TRUE)) (ABSTRACT? FALSE))
...
(CL:DEFUN DEFAULT-OPTION-HANDLER (SELF SLOT TREE)
  (CL:LET* ((PARSEDVALUE NULL))
   (CL:COND
    ...
    (CL:T (CL:SETQ PARSEDVALUE (PERMANENT-COPY TREE))))
   (CL:WHEN (CL:>= *SAFETY* 2)
    (CL:WHEN
...
objects as opposed to `unstringify-in-module' or the regular `unstringify'.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "DESTRUCTURE-DEFMETHOD-TREE"
     "(DEFUN (DESTRUCTURE-DEFMETHOD-TREE OBJECT CONS CONS) ((METHODTREE CONS) (OPTIONSTABLE KEY-VALUE-LIST)) :PUBLIC? TRUE :DOCUMENTATION \"Return three parse trees representing the name,
parameters, and code body of the parse tree 'methodTree'.  Fill
'optionsTable' with a dictionary of method options.
Storage note:  Options are treated specially because the other return
values are subtrees of 'methodTree', while 'optionsTable' is a newly-created
cons tree.  Note also, the parameter and body trees are destructively
removed from 'methodTree'.\")" (CL:FUNCTION DESTRUCTURE-DEFMETHOD-TREE)
     NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Stella-In") NULL-STRING-WRAPPER)))
