...
returns false."
  ...
  (CL:RETURN-FROM OUTLINE-DEPTH-EXCEEDED?
   (CL:AND (CL:NOT (CL:= DEPTH-LIMIT NULL-INTEGER)) (CL:>= DEPTH-LIMIT 0) (CL:>= CURRENT-DEPTH DEPTH-LIMIT))))
...
variable `*OUTLINE-INDENT-STRING*'"
  ...
  (CL:CHECK-TYPE CURRENT-DEPTH CL:FIXNUM)
  (CL:LET*
   ((I NULL-INTEGER) (ITER-000 1) (UPPER-BOUND-000 CURRENT-DEPTH) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
...
(CL:DEFMETHOD HELP-PRINT-OUTLINE ((TOP CONTEXT) STREAM CURRENT-DEPTH DEPTH NAMED?)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) TOP EOL)
  (CL:WHEN (CL:NOT (CL:AND (CL:NOT (CL:= DEPTH NULL-INTEGER)) (CL:>= DEPTH 0) (CL:>= CURRENT-DEPTH DEPTH)))
   ...
   (CL:LET* ((C NULL) (ITER-000 (%THE-CONS-LIST (%CHILD-CONTEXTS TOP))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000))
     (HELP-PRINT-OUTLINE C STREAM CURRENT-DEPTH DEPTH NAMED?) (CL:SETQ ITER-000 (%%REST ITER-000)))))
  :VOID)
...
(CL:DEFMETHOD HELP-PRINT-OUTLINE ((TOP CLASS) STREAM CURRENT-DEPTH DEPTH NAMED?)
  ...
  (CL:WHEN (CL:NOT (CL:AND (CL:NOT (CL:= DEPTH NULL-INTEGER)) (CL:>= DEPTH 0) (CL:>= CURRENT-DEPTH DEPTH)))
   ...
   (CL:LET* ((C NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS TOP))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000))
     (HELP-PRINT-OUTLINE (TYPE-TO-CLASS C) STREAM CURRENT-DEPTH DEPTH NAMED?) (CL:SETQ ITER-000 (%%REST ITER-000)))))
  :VOID)
...
(CL:DEFUN STARTUP-TOOLS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SYM-TOOLS-STELLA-PRINT-OUTLINE (INTERN-RIGID-SYMBOL-WRT-MODULE "PRINT-OUTLINE" NULL 0))
    (CL:SETQ KWD-TOOLS-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
    (CL:SETQ KWD-TOOLS-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
...
This function is intended to be used on things like modules, contexts,
concepts, etc. that have hierarchical structure.  If `thing' doesn't
have a hierarchical structure, it will just be printed.\")" (CL:FUNCTION %PRINT-OUTLINE)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-TOOLS-STELLA-STARTUP-TOOLS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-TOOLS-STELLA-METHOD-STARTUP-CLASSNAME
