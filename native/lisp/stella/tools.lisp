...
variable `*OUTLINE-INDENT-STRING*'"
  ...
  (CL:LET* ((I NULL-INTEGER) (ITER-000 1) (UPPER-BOUND-000 CURRENT-DEPTH) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
   ...
   (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) *OUTLINE-INDENT-STRING*) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
  :VOID)
...
(CL:DEFUN PRINT-OUTLINE-EVALUATOR-WRAPPER (ARGUMENTS)
  (%PRINT-OUTLINE (%%VALUE ARGUMENTS) (%%VALUE (%%REST ARGUMENTS)) (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ARGUMENTS))))
...
(CL:DEFMETHOD HELP-PRINT-OUTLINE ((TOP CONTEXT) STREAM CURRENT-DEPTH DEPTH NAMED?)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) TOP EOL)
  (CL:WHEN (CL:NOT (CL:AND (CL:NOT (CL:= DEPTH NULL-INTEGER)) (CL:>= DEPTH 0) (CL:>= CURRENT-DEPTH DEPTH))) (CL:SETQ CURRENT-DEPTH (CL:1+ CURRENT-DEPTH))
   (CL:LET* ((C NULL) (ITER-000 (%THE-CONS-LIST (%CHILD-CONTEXTS TOP))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000)) (HELP-PRINT-OUTLINE C STREAM CURRENT-DEPTH DEPTH NAMED?)
...
(CL:DEFMETHOD HELP-PRINT-OUTLINE ((TOP CLASS) STREAM CURRENT-DEPTH DEPTH NAMED?)
  ...
  (CL:WHEN (CL:NOT (CL:AND (CL:NOT (CL:= DEPTH NULL-INTEGER)) (CL:>= DEPTH 0) (CL:>= CURRENT-DEPTH DEPTH))) (CL:SETQ CURRENT-DEPTH (CL:1+ CURRENT-DEPTH))
   (CL:LET* ((C NULL) (ITER-000 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS TOP))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000)) (HELP-PRINT-OUTLINE (TYPE-TO-CLASS C) STREAM CURRENT-DEPTH DEPTH NAMED?)
     (CL:SETQ ITER-000 (%%REST ITER-000)))))
  :VOID)
...
(CL:DEFUN STARTUP-TOOLS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ SYM-TOOLS-STELLA-PRINT-OUTLINE (INTERN-RIGID-SYMBOL-WRT-MODULE "PRINT-OUTLINE" NULL 0))
    (CL:SETQ KWD-TOOLS-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2)) (CL:SETQ KWD-TOOLS-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
    (CL:SETQ SYM-TOOLS-STELLA-STARTUP-TOOLS (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-TOOLS" NULL 0))
    (CL:SETQ SYM-TOOLS-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" NULL 0)))
...
This functions uses the convention that a `null' or negative value of
`depth-limit' means the depth is unlimited.  In those cases it always
returns false.\" (RETURN (AND (DEFINED? DEPTH-LIMIT) (>= DEPTH-LIMIT 0) (>= CURRENT-DEPTH DEPTH-LIMIT))))" (CL:FUNCTION OUTLINE-DEPTH-EXCEEDED?) NULL)
...
`current-depth' on `stream' using the value of the global
variable `*OUTLINE-INDENT-STRING*'\" (FOREACH I IN (INTERVAL 1 CURRENT-DEPTH) DO (IGNORE I) (PRINT-STREAM STREAM *OUTLINE-INDENT-STRING*)))" (CL:FUNCTION INDENT-OUTLINE)
     NULL)
...
This function is intended to be used on things like modules, contexts,
concepts, etc. that have hierarchical structure.  If `thing' doesn't
have a hierarchical structure, it will just be printed.\")" (CL:FUNCTION %PRINT-OUTLINE) (CL:FUNCTION PRINT-OUTLINE-EVALUATOR-WRAPPER))
    ...
    (DEFINE-METHOD-OBJECT
     ...
     (CL:FUNCTION HELP-PRINT-OUTLINE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-TOOLS" "(DEFUN STARTUP-TOOLS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-TOOLS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-TOOLS-STELLA-STARTUP-TOOLS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-TOOLS-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupTools") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "STELLA")))
    ...
    (REGISTER-NATIVE-NAME SYM-TOOLS-STELLA-PRINT-OUTLINE KWD-TOOLS-COMMON-LISP KWD-TOOLS-FUNCTION)))
  :VOID)
