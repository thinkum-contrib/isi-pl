...
(CL:DEFUN IDL-OUTPUT-PARSE-TREE (CONSTREE)
  (CL:LET* ((*IDL-INDENT-CHARS* 0)) (CL:DECLARE (CL:SPECIAL *IDL-INDENT-CHARS*))
   (CL:DECLARE (CL:TYPE CL:FIXNUM *IDL-INDENT-CHARS*)) (IDL-OUTPUT-STATEMENT CONSTREE))
  :VOID)
...
(CL:DEFUN IDL-OUTPUT-STATEMENT (STATEMENT)
  (CL:COND
    ...
    (CL:PROGN (CL:WHEN (CL:EQ STATEMENT NIL) (CL:RETURN-FROM IDL-OUTPUT-STATEMENT))
     (CL:IF (CL:OR (CONS? (%%VALUE STATEMENT)) (CL:EQ (%%VALUE STATEMENT) NULL))
      ...
      (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
       (CL:COND
        ((CL:EQ TEST-VALUE-000 SYM-IDL-OUTPUT-STELLA-IDL_COMMENT) (IDL-OUTPUT-COMMENT (%%VALUE (%%REST STATEMENT))))
   ...
   (CL:T (CL:WHEN (CL:EQ STATEMENT NULL) (CL:RETURN-FROM IDL-OUTPUT-STATEMENT))
...
(CL:DEFUN IDL-OUTPUT-TYPEDEF (TYPEDEF)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "typedef " (%WRAPPER-VALUE (%%VALUE (%%REST TYPEDEF))) " "
   (%WRAPPER-VALUE (%%VALUE TYPEDEF)) ";" EOL EOL)
  :VOID)
...
(CL:DEFUN IDL-OUTPUT-ENUMERANDS (ENUMERANDS)
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE ENUMERANDS) NULL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE (%%VALUE (%%REST (%%VALUE ENUMERANDS))))))
  (CL:LET* ((ENUMERAND NULL) (ITER-000 (%%REST ENUMERANDS)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENUMERAND (%%VALUE ITER-000))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", " (%WRAPPER-VALUE (%%VALUE (%%REST ENUMERAND))))
...
(CL:DEFMETHOD IDL-OUTPUT-LITERAL ((STRING CL:STRING))
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "\""
   (REPLACE-SUBSTRINGS
    (REPLACE-SUBSTRINGS (REPLACE-SUBSTRINGS (REPLACE-SUBSTRINGS STRING "\\\\" "\\") "\\\"" (MAKE-STRING 1 #\")) "\\n"
...
(CL:DEFUN IDL-OUTPUT-DERIVED-CLASSES (CLASS-LIST)
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE CLASS-LIST) NULL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE (%%VALUE CLASS-LIST))))
...
(CL:DEFUN IDL-OUTPUT-TEMPLATE-PARAMETERS (PARAMETERS)
  (CL:WHEN (CL:NOT (CL:EQ PARAMETERS NIL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "template <class " (%WRAPPER-VALUE (%%VALUE PARAMETERS)))
   (CL:LET* ((PARAMETER NULL) (ITER-000 (%%REST PARAMETERS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ PARAMETER (%%VALUE ITER-000))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", class " (%WRAPPER-VALUE PARAMETER))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN IDL-OUTPUT-INTERFACE (CLASSDEF)
  ...
  (CL:LET* ((STATEMENT NULL) (ITER-000 (NTH-REST CLASSDEF 4)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ STATEMENT (%%VALUE ITER-000))
    (IDL-OUTPUT-STATEMENT STATEMENT) (CL:SETQ ITER-000 (%%REST ITER-000))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "};" EOL EOL)
  :VOID)
...
(CL:DEFUN IDL-OUTPUT-ONE-FORMAL-PARAMETER (PARAMETER)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE (%%VALUE PARAMETER)) " "
   (%WRAPPER-VALUE (%%VALUE (%%REST PARAMETER))) " " (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST PARAMETER)))))
  :VOID)

;;; (DEFUN IDL-OUTPUT-FORMAL-PARAMETERS ...)

(CL:DEFUN IDL-OUTPUT-FORMAL-PARAMETERS (PARAMETERS)
  (CL:LET* ((N-PARAMETERS (LENGTH PARAMETERS))) (CL:DECLARE (CL:TYPE CL:FIXNUM N-PARAMETERS))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "(")
   (CL:LET*
     ...
     (CL:IF (CL:= N N-PARAMETERS)
      (CL:IF (CL:EQ (%%VALUE PARAMETER) SYM-IDL-OUTPUT-STELLA-IDL_VAR_ARGS)
       (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "int ")
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE (%%VALUE (%%REST PARAMETER))))
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", ..."))
...
(CL:DEFUN IDL-OUTPUT-DECLARATIONS (DECLARATIONS)
  ...
  (CL:LET* ((DECLARATION NULL) (ITER-000 DECLARATIONS))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ DECLARATION (%%VALUE ITER-000)) (IDL-INDENT)
...
(CL:DEFUN IDL-OUTPUT-SIGNATURE (SIGNATURE OUTPUTMETHODCLASS?)
  ...
  (CL:WHEN (CL:AND OUTPUTMETHODCLASS? (CL:NOT (CL:EQ (%%VALUE SIGNATURE) NULL)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE (%%VALUE SIGNATURE)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "::"))
...
(CL:DEFUN IDL-OUTPUT-SIGNATURES (SIGNATURES)
  ...
  (CL:LET* ((SIGNATURE NULL) (ITER-000 SIGNATURES))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SIGNATURE (%%VALUE ITER-000)) (IDL-INDENT)
    (IDL-OUTPUT-SIGNATURE SIGNATURE CL:NIL) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (IDL-UNBUMP-INDENT)
  :VOID)
...
(CL:DEFUN STARTUP-IDL-OUTPUT ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SYM-IDL-OUTPUT-STELLA-IDL_COMMENT (INTERN-RIGID-SYMBOL-WRT-MODULE "IDL_COMMENT" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-PARSE-TREE" "(DEFUN IDL-OUTPUT-PARSE-TREE ((CONSTREE CONS)))"
     (CL:FUNCTION IDL-OUTPUT-PARSE-TREE) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-STATEMENT" "(DEFUN IDL-OUTPUT-STATEMENT ((STATEMENT OBJECT)))"
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-TYPEDEF" "(DEFUN IDL-OUTPUT-TYPEDEF ((TYPEDEF CONS)))"
     (CL:FUNCTION IDL-OUTPUT-TYPEDEF) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-ENUMERANDS" "(DEFUN IDL-OUTPUT-ENUMERANDS ((ENUMERANDS (CONS OF CONS))))"
     (CL:FUNCTION IDL-OUTPUT-ENUMERANDS) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-ENUM" "(DEFUN IDL-OUTPUT-ENUM ((ENUM CONS)))" (CL:FUNCTION IDL-OUTPUT-ENUM)
     NULL)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((STRING STRING-WRAPPER)))" (CL:FUNCTION IDL-OUTPUT-LITERAL)
     NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((STRING MUTABLE-STRING-WRAPPER)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((TREE QUOTED-EXPRESSION)))" (CL:FUNCTION IDL-OUTPUT-LITERAL)
     NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((SYMBOL SYMBOL)))" (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((SURROGATE SURROGATE)))" (CL:FUNCTION IDL-OUTPUT-LITERAL)
     NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((KEYWORD KEYWORD)))" (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-ATOMIC-EXPRESSION" "(DEFUN IDL-OUTPUT-ATOMIC-EXPRESSION ((ATOM OBJECT)))"
     (CL:FUNCTION IDL-OUTPUT-ATOMIC-EXPRESSION) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-COMMENT" "(DEFUN IDL-OUTPUT-COMMENT ((TREE STRING-WRAPPER)))"
     (CL:FUNCTION IDL-OUTPUT-COMMENT) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-IDENTIFIER" "(DEFUN IDL-OUTPUT-IDENTIFIER ((IDENTIFIER STRING-WRAPPER)))"
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-TEMPLATE-CLASSES?" "(DEFUN (IDL-OUTPUT-TEMPLATE-CLASSES? BOOLEAN) ())"
     (CL:FUNCTION IDL-OUTPUT-TEMPLATE-CLASSES?) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-INTERFACE" "(DEFUN IDL-OUTPUT-INTERFACE ((CLASSDEF CONS)))"
     (CL:FUNCTION IDL-OUTPUT-INTERFACE) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-STRUCT" "(DEFUN IDL-OUTPUT-STRUCT ((CLASSDEF CONS)))"
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-FORMAL-PARAMETERS"
     "(DEFUN IDL-OUTPUT-FORMAL-PARAMETERS ((PARAMETERS (CONS OF CONS))))" (CL:FUNCTION IDL-OUTPUT-FORMAL-PARAMETERS)
     NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-FUNCTION-SIGNATURE" "(DEFUN IDL-OUTPUT-FUNCTION-SIGNATURE ((FUNCTION CONS)))"
     (CL:FUNCTION IDL-OUTPUT-FUNCTION-SIGNATURE) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-DECLARATIONS" "(DEFUN IDL-OUTPUT-DECLARATIONS ((DECLARATIONS (CONS OF CONS))))"
     (CL:FUNCTION IDL-OUTPUT-DECLARATIONS) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-SIGNATURE"
     "(DEFUN IDL-OUTPUT-SIGNATURE ((SIGNATURE CONS) (OUTPUTMETHODCLASS? BOOLEAN)))" (CL:FUNCTION IDL-OUTPUT-SIGNATURE)
     NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-SIGNATURES" "(DEFUN IDL-OUTPUT-SIGNATURES ((SIGNATURES CONS)))"
     (CL:FUNCTION IDL-OUTPUT-SIGNATURES) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-IDL-OUTPUT" "(DEFUN STARTUP-IDL-OUTPUT () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-IDL-OUTPUT) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-IDL-OUTPUT-STELLA-STARTUP-IDL-OUTPUT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-IDL-OUTPUT-STELLA-METHOD-STARTUP-CLASSNAME
