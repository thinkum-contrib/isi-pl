...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* FALSE TRUE EOL *CURRENT-STREAM*
  NULL-INTEGER NIL))
...
(CL:DEFUN IDL-OUTPUT-TYPEDEF (TYPEDEF)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "typedef "
   (%WRAPPER-VALUE (%%VALUE (%%REST TYPEDEF))) " "
...
(CL:DEFMETHOD IDL-OUTPUT-LITERAL ((SURROGATE SURROGATE))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (%WRAPPER-VALUE (IDL-TRANSLATE-SURROGATE-NAME SURROGATE)))
  :VOID)
...
(CL:DEFUN IDL-OUTPUT-COMMENT (TREE)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// " (%WRAPPER-VALUE TREE)
   EOL)
  :VOID)
...
(CL:DEFUN IDL-OUTPUT-DERIVED-CLASSES (CLASS-LIST)
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE CLASS-LIST) NULL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
    (%WRAPPER-VALUE (%%VALUE CLASS-LIST))))
...
(CL:DEFUN IDL-OUTPUT-TEMPLATE-PARAMETERS (PARAMETERS)
  (CL:WHEN (CL:NOT (CL:EQ PARAMETERS NIL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "template <class "
...
(CL:DEFUN IDL-OUTPUT-INTERFACE (CLASSDEF)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "interface "
...
(CL:DEFUN IDL-OUTPUT-STRUCT (CLASSDEF)
  (CL:WHEN (IDL-OUTPUT-TEMPLATE-CLASSES?)
   (IDL-OUTPUT-TEMPLATE-PARAMETERS (%%VALUE (%%REST CLASSDEF))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "struct "
   (%WRAPPER-VALUE (%%VALUE CLASSDEF)) " ")
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE (%%REST (%%REST CLASSDEF))) NIL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ": ")
   (IDL-OUTPUT-DERIVED-CLASSES (%%VALUE (%%REST (%%REST CLASSDEF)))))
...
(CL:DEFUN STARTUP-IDL-OUTPUT ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-UNBUMP-INDENT" "(DEFUN IDL-UNBUMP-INDENT ())"
     (CL:FUNCTION IDL-UNBUMP-INDENT) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-ENUM"
     "(DEFUN IDL-OUTPUT-ENUM ((ENUM CONS)))" (CL:FUNCTION IDL-OUTPUT-ENUM) NULL)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((OBJECT OBJECT)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((STRING STRING)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    ...
    (DEFINE-METHOD-OBJECT
     ...
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((SYMBOL SYMBOL)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT
     "(DEFMETHOD IDL-OUTPUT-LITERAL ((SURROGATE SURROGATE)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD IDL-OUTPUT-LITERAL ((KEYWORD KEYWORD)))"
     (CL:FUNCTION IDL-OUTPUT-LITERAL) NULL)
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-IDL-OUTPUT-STELLA-STARTUP-IDL-OUTPUT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-IDL-OUTPUT-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Idl-Output") NULL-STRING-WRAPPER)))
