...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* EOL NULL-INTEGER FALSE TRUE DELETED-LABEL
  NIL))
  ...
  (CL:COND
   ((CL:EQ NODE NULL)
    (CL:LET* ((SELF-000 (NEW-TAXONOMY-NODE)))
     (CL:SETF (%NATIVE-OBJECT SELF-000) NATIVEOBJECT) (CL:SETQ NODE SELF-000)))
...
(CL:DEFUN REMOVE-TAXONOMY-NODE (GRAPH NODE)
  ...
  (CL:LET*
   ...
   (DELETED?-SETTER NODE TRUE) (REMOVE-TAXONOMY-ROOT GRAPH NODE)
   (CL:SETF (%NUMBER-OF-NODES GRAPH) (CL:1- (%NUMBER-OF-NODES GRAPH))))
  :VOID)
  ...
  (CL:LET* ((PARENTINTERVAL (%INITIAL-INTERVAL PARENT)))
   (CL:WHEN (CL:EQ (%TREE-CHILDREN PARENT) NIL)
    ...
    (CL:LET* ((SELF-000 (NEW-INTERVAL)))
     ...
     (CL:SETF (%UPPER-BOUND SELF-000)
      (FLOOR
       (CL:/ (CL:+ (%LOWER-BOUND PARENTINTERVAL) (%UPPER-BOUND PARENTINTERVAL))
        2.0d0)))
   ...
   (CL:LET*
    ...
    (CL:LET* ((SELF-001 (NEW-INTERVAL)))
     (CL:SETF (%LOWER-BOUND SELF-001) (CL:1+ (%UPPER-BOUND RIGHTCHILDINTERVAL)))
     (CL:SETF (%UPPER-BOUND SELF-001)
      (CEILING
       (CL:/
        (CL:+ (%UPPER-BOUND PARENTINTERVAL) (%UPPER-BOUND RIGHTCHILDINTERVAL))
        2.0d0)))
...
(CL:DEFUN UNLINK-TAXONOMY-NODES (GRAPH PARENT CHILD)
  ...
  (CL:WHEN (%INCREMENTAL-MODE? GRAPH) (CL:SETF (%INCREMENTAL-MODE? GRAPH) FALSE))
...
(CL:DEFUN INCREMENTALLY-UNLINK-TAXONOMY-NODES (GRAPH PARENT CHILD)
  (CL:ERROR
   (NEW-STELLA-EXCEPTION "incrementally-unlink-taxonomy-nodes: Rewrite me!"))
  :VOID)
...
(CL:DEFUN ALL-TAXONOMY-NODES-NEXT? (SELF)
  (CL:LET* ((ROOTS (%ITERATOR-OBJECT SELF)))
   (CL:WHEN (EMPTY? ROOTS) (CL:RETURN-FROM ALL-TAXONOMY-NODES-NEXT? FALSE))
   ...
   (CL:LET* ((CHILD NULL) (ITER-000 (%TREE-CHILDREN (%VALUE SELF))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CHILD (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (PUSH ROOTS CHILD)))
   (CL:RETURN-FROM ALL-TAXONOMY-NODES-NEXT? TRUE)))
...
(CL:DEFUN LABELED-TAXONOMY-NODE? (NODE)
  (CL:RETURN-FROM LABELED-TAXONOMY-NODE? (DEFINED? (%LABEL NODE))))
...
(CL:DEFUN HELP-CLEAR-TAXONOMY-GRAPH (NODE)
  (CL:WHEN (DEFINED? (%LABEL NODE)) (CLEAR-TAXONOMY-NODE NODE)
...
but retain information about the native network and associated links."
  (CL:LET*
   ((ROOT NULL)
    (ITER-000 (%THE-CONS-LIST (REMOVE-DELETED-MEMBERS (%ROOTS GRAPH)))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ ROOT (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (HELP-CLEAR-TAXONOMY-GRAPH ROOT)))
  (CL:SETF (%INCREMENTAL-MODE? GRAPH) FALSE)
...
to be reused, they have to be cleared with `initialize-taxonomy-node'."
  (CL:SETF (%INCREMENTAL-MODE? GRAPH) FALSE)
  (CL:SETF (%RENUMBER-IF-OUT-OF-NUMBERS? GRAPH) FALSE)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ (%TREE-CHILDREN SELF) NIL))
    (CL:SETQ MINCHILDLABEL
     (HELP-CREATE-TAXONOMY-TREE-INTERVALS (%%VALUE (%TREE-CHILDREN SELF)))))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:FIXNUM) HELP-CREATE-TAXONOMY-TREE-INTERVALS))
...
(CL:DEFMETHOD MEMBER? ((INTERVAL INTERVAL) N)
  ...
  (CL:RETURN-FROM MEMBER?
   (CL:AND (CL:>= N (%LOWER-BOUND INTERVAL)) (CL:<= N (%UPPER-BOUND INTERVAL)))))
...
(CL:DEFUN REMOVE-TAXONOMY-NODE-INTERVAL (NODE INTERVAL)
  (CL:LET* ((INTERVALS (%INTERVALS NODE)))
   (CL:SETQ INTERVALS (CL:SETF (%INTERVALS NODE) (REMOVE INTERVALS INTERVAL)))
   (CL:COND
   ...
   (CL:LET* ((INTERVAL NULL) (ITER-000 (%INTERVALS SUPER-NODE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      ...
      (CL:RETURN-FROM TAXONOMY-SUBNODE-OF? TRUE))))
   (CL:RETURN-FROM TAXONOMY-SUBNODE-OF? FALSE)))

;;; (DEFUN (SLOW-TAXONOMY-SUBNODE-OF? BOOLEAN) ...)

(CL:DEFUN SLOW-TAXONOMY-SUBNODE-OF? (SUBNODE SUPERNODE)
  (CL:WHEN (CL:EQ SUBNODE SUPERNODE)
   (CL:RETURN-FROM SLOW-TAXONOMY-SUBNODE-OF? TRUE))
  (CL:LET* ((PARENT NULL) (ITER-000 (%PARENTS SUBNODE)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (SLOW-TAXONOMY-SUBNODE-OF? PARENT SUPERNODE)
     (CL:RETURN-FROM SLOW-TAXONOMY-SUBNODE-OF? TRUE))))
  (CL:RETURN-FROM SLOW-TAXONOMY-SUBNODE-OF? FALSE))
...
IMPORTANT: This will automatically insert a backlink from `child' to
`parent', so, for maximum efficiency it should not be called a second time
with the arguments reversed.\" :PUBLIC? TRUE)" (CL:FUNCTION LINK-TAXONOMY-NODES)
     NULL)
...
to be reused, they have to be cleared with `initialize-taxonomy-node'.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-TAXONOMIES-STELLA-STARTUP-TAXONOMIES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-TAXONOMIES-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Taxonomies") NULL-STRING-WRAPPER)))
