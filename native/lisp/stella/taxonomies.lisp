...
(CL:PROGN (CL:DEFVAR *NUMBERING-INTERVAL* 100 "Spacing between postorder numbers for nodes.  Allows limited
incremental insertions without having to renumber the whole graph.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *NUMBERING-INTERVAL*)))
...
(CL:DEFUN ALL-TAXONOMY-NODES-NEXT? (SELF)
  (CL:LET* ((ROOTS (%ITERATOR-OBJECT SELF)))
   (CL:WHEN (EMPTY? ROOTS)
    (CL:RETURN-FROM ALL-TAXONOMY-NODES-NEXT? FALSE))
...
(CL:PROGN (CL:DEFVAR MARKER-LABEL -99 "Dummy label used for marking a node") (CL:DECLAIM (CL:TYPE CL:FIXNUM MARKER-LABEL)))

;;; (DEFCONSTANT DELETED-LABEL ...)

(CL:PROGN (CL:DEFVAR DELETED-LABEL -99 "Label used for marking deleted nodes") (CL:DECLAIM (CL:TYPE CL:FIXNUM DELETED-LABEL)))
...
(CL:PROGN (CL:DEFVAR *TAXONOMY-POSTORDER-NUMBER* NULL-INTEGER) (CL:DECLAIM (CL:TYPE CL:FIXNUM *TAXONOMY-POSTORDER-NUMBER*)))
...
(CL:DEFUN TAXONOMY-SUBNODE-OF? (SUBNODE SUPERNODE)
  "Return TRUE if `subNode' is a descendant of `superNode'."
  (CL:LET* ((SUBNODELABEL (%LABEL SUBNODE)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM SUBNODELABEL))
   (CL:LET* ((INTERVAL NULL) (ITER-000 (%INTERVALS SUPERNODE)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ INTERVAL (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN
      (CL:AND (CL:>= SUBNODELABEL (%LOWER-BOUND INTERVAL))
       (CL:<= SUBNODELABEL (%UPPER-BOUND INTERVAL)))
...
to be reused, they have to be cleared with `initialize-taxonomy-node'.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "TAXONOMY-SUBNODE-OF?"
     "(DEFUN (TAXONOMY-SUBNODE-OF? BOOLEAN) ((SUBNODE TAXONOMY-NODE) (SUPERNODE TAXONOMY-NODE)) :DOCUMENTATION \"Return TRUE if `subNode' is a descendant of `superNode'.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Taxonomies") NULL-STRING-WRAPPER)))
