...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* *TRANSLATOROUTPUTLANGUAGE* NULL-INTEGER EOL
  TRUE NULL-CHARACTER *CURRENT-STREAM* NIL FALSE))
...
(CL:DEFVAR *OUTPUTTINGDEFPRINT?* FALSE
  ...
  (CL:LET* ((RESULTVAR (LOCAL-GENSYM "RESULT")))
   (CL:RETURN-FROM CPP-OUTPUT-TO-STRING
    (LIST* SYM-CPP-OUTPUT-STELLA-VRLET
     (CONS
      (CONS RESULTVAR
       (LIST* SYM-CPP-OUTPUT-STELLA-STRING SYM-CPP-OUTPUT-STELLA-NULL NIL))
      NIL)
...
(CL:DEFUN CPP-STATEMENT-TO-LIST (STATEMENT)
  (CL:LET* ((FLATTENEDSTATEMENT (CPP-FLATTEN-STATEMENTS STATEMENT)))
   (CL:IF (CL:EQ FLATTENEDSTATEMENT STATEMENT)
    (CL:RETURN-FROM CPP-STATEMENT-TO-LIST (CONS FLATTENEDSTATEMENT NIL))
...
(CL:DEFUN CPP-OUTPUT-STATEMENT (STATEMENT)
  (CL:COND
   ((CL:EQ (SAFE-PRIMARY-TYPE STATEMENT) SGT-CPP-OUTPUT-STELLA-CONS)
    (CL:PROGN
     (CL:WHEN (CL:EQ STATEMENT NIL) (CL:RETURN-FROM CPP-OUTPUT-STATEMENT))
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:CASE CHAR
      ...
      (#\Return (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
       (CPP-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// ")
       (CL:SETQ RETURNP TRUE))
      (CL:OTHERWISE (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR)
       (CL:SETQ RETURNP FALSE)))))
...
(CL:DEFUN CPP-OUTPUT-ARRAY-REFERENCE (TREE)
  ...
  (CL:LET* ((INDEXEXPRESSION NULL) (ITER-000 (%%REST TREE)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ INDEXEXPRESSION (%%VALUE ITER-000))
...
(CL:DEFUN CPP-OUTPUT-VARIABLE-DEFINITION (VARIABLE)
  (CL:LET*
   ((KIND (%%VALUE VARIABLE))
    ...
    (TYPE (%%VALUE (%%REST (%%REST VARIABLE)))) (NAME (FOURTH VARIABLE))
   ...
   (CL:LET* ((RESULT-000 STELLA::NULL-STRING))
    ...
    (CL:LET* ((*CURRENT-STREAM* (NEW-OUTPUT-STRING-STREAM)))
     ...
     (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " (*) (")
      (CL:LET*
       ((I NULL-INTEGER) (PARTYPE NULL) (ITER-000 PARAMETERTYPES) (ITER-001 1)
        (UPPER-BOUND-000 NOFPARAMETERS) (UNBOUNDED?-000 (NULL? UPPER-BOUND-000)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-000 I))
       (CL:LOOP WHILE
        (CL:AND (CL:NOT (CL:EQ ITER-000 NIL))
         (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000)))
        DO
        (CL:PROGN (CL:SETQ PARTYPE (%%VALUE ITER-000))
         (CL:SETQ ITER-000 (%%REST ITER-000)))
     ...
     (CL:LET* ((CONDITION C))
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CPP-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "case ")
        (CPP-OUTPUT-STATEMENT COND)
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ": " EOL)))
      (CPP-BUMP-INDENT) (CPP-OUTPUT-STATEMENT (%%VALUE (%%REST CONDITION)))
      (CPP-UNBUMP-INDENT)
      (CL:WHEN
       (CL:NOT (CPP-LAST-STATEMENT-IS-RETURN? (%%VALUE (%%REST CONDITION))))
       (CPP-INDENT)
       (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "break;" EOL)))))
   (CL:WHEN (CL:EQ DEFAULTCASE NULL)
    (CL:ERROR
     "Safety violation: INTERNAL ERROR: `cpp-output-case' expects an `otherwise' clause."))
   (CPP-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "default:" EOL)
   (CPP-BUMP-INDENT) (CPP-OUTPUT-STATEMENT DEFAULTCASE) (CPP-UNBUMP-INDENT)
...
(CL:DEFUN CPP-PROGN? (TREE)
  (CL:RETURN-FROM CPP-PROGN?
   (CL:AND (CONS? TREE) (CL:EQ (%%VALUE TREE) SYM-CPP-OUTPUT-STELLA-CPP_PROGN))))
     ...
     (CL:LET* ((TEST-VALUE-000 (%%VALUE TREE)))
      (CL:COND
       ((CL:OR (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_BLOCK)
         (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_PROGN))
        (CL:RETURN-FROM CPP-BLOCK? TRUE))
       (CL:T)))))
   (CL:T))
  (CL:RETURN-FROM CPP-BLOCK? FALSE))
...
(CL:DEFUN CPP-OUTPUT-IF (TREE)
  ...
  (CL:IF (CPP-BLOCK? (%%VALUE (%%REST TREE)))
   ...
   (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
    (CPP-BUMP-INDENT) (CPP-INDENT) (CPP-OUTPUT-STATEMENT (%%VALUE (%%REST TREE)))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)
    (CPP-UNBUMP-INDENT)))
...
(CL:DEFUN CPP-OUTPUT-FOREACH (LOOP)
  (CL:LET*
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ") {" EOL) (CPP-BUMP-INDENT)
   (SECOND-SETTER LOOP NULL)
   (THIRD-SETTER LOOP
    (CPP-FLATTEN-STATEMENTS (CONCATENATE NEXTASSIGNMENTS BODY)))
   (CPP-OUTPUT-STATEMENTS (%%VALUE (%%REST (%%REST LOOP)))) (CPP-UNBUMP-INDENT)
   ...
   (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
    (CL:COND
     ((CL:OR (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_ASSIGN)
       ...
       (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_VERBATIM))
      (CL:RETURN-FROM CPP-OUTPUT-SEMICOLON? TRUE))
     (CL:T))))
  (CL:RETURN-FROM CPP-OUTPUT-SEMICOLON? FALSE))
...
(CL:DEFUN CPP-INDENT-STATEMENT? (STATEMENT)
  (CL:IF (CL:NOT (CL:EQ STATEMENT NULL))
   (CL:RETURN-FROM CPP-INDENT-STATEMENT?
    (CL:NOT
     (MEMBER? (GET-QUOTED-TREE "((CPP_STATEMENTS) \"/STELLA\")" "/STELLA")
      (%%VALUE STATEMENT))))
   (CL:RETURN-FROM CPP-INDENT-STATEMENT? FALSE)))
   ...
   (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
    (CL:COND
     ((CL:OR (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_LITERAL)
       (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_IDENT))
      (CL:RETURN-FROM ILLEGAL-CPP-STATEMENT? TRUE))
     (CL:T))))
  (CL:RETURN-FROM ILLEGAL-CPP-STATEMENT? FALSE))
...
(CL:DEFUN CPP-OUTPUT-PRINT-STREAM (EXPS)
  (CL:LET* ((STREAM (%%VALUE EXPS)))
   (CL:COND
    ...
    (CL:T (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "get_ostream(")
...
(CL:DEFUN CPP-OUTPUT-UNARY-OPERATOR (EXPRESSION)
  (CL:LET* ((OP NULL) (ARG NULL) (POSTFIX? FALSE))
   (CL:COND
    ...
    (CL:T (CL:SETQ OP (%%VALUE (%%REST EXPRESSION)))
     (CL:SETQ ARG (%%VALUE EXPRESSION)) (CL:SETQ POSTFIX? TRUE)))
...
(CL:DEFUN CPP-OUTPUT-TERNARY-OPERATOR (EXPRESSION)
  (CL:LET*
   ((ARG1 (%%VALUE EXPRESSION)) (OP1 (%%VALUE (%%REST EXPRESSION)))
    ...
    (ARG3 (FIFTH EXPRESSION)) (TRANSLATEDARG STELLA::NULL-STRING))
     ...
     (CL:LET* ((TEST-VALUE-002 (SAFE-PRIMARY-TYPE TYPE)))
      (CL:COND
       ...
       ((SUBTYPE-OF-STRING? TEST-VALUE-002)
        (CL:PROGN
         (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE TYPE)
          " ")))
       (CL:T
  ...
  (CL:LET* ((DECLARATION NULL) (ITER-000 DECLARATIONS))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%%VALUE (%%REST (%%REST DECLARATION))) NULL))
     (CPP-INDENT) (CPP-OUTPUT-COMMENT (%%VALUE (%%REST (%%REST DECLARATION)))))
    (CPP-INDENT)
  ...
  (CL:LET* ((SIGNATURE NULL) (ITER-000 SIGNATURES))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ SIGNATURE (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CPP-INDENT) (CPP-OUTPUT-SIGNATURE SIGNATURE FALSE)
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)))
  (CL:WHEN (CL:NOT (CL:EQ ACCESSCONTROLMODE NULL)) (CPP-UNBUMP-INDENT))
  :VOID)
  ...
  (CL:LET* ((DEFINITION NULL) (ITER-000 DEFINITIONS))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ DEFINITION (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CPP-INDENT) (CPP-OUTPUT-STATEMENT DEFINITION)))
  (CL:WHEN (CL:NOT (CL:EQ ACCESSCONTROLMODE NULL)) (CPP-UNBUMP-INDENT))
  :VOID)
...
(CL:DEFUN CPP-OUTPUT-BLOCK (BLOCK)
  (CL:LET* ((FIRSTSTATEMENT? TRUE))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "{ ") (CPP-BUMP-INDENT)
   (CL:LET* ((DECLARATION NULL) (ITER-000 (%%VALUE BLOCK)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CPP-OUTPUT-TYPED-ENTITY (%%VALUE DECLARATION)
      (%%VALUE (%%REST DECLARATION)) (%%VALUE (%%REST (%%REST DECLARATION))))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
...
(CL:DEFUN CPP-OUTPUT-WITH-PROCESS-LOCK (SYNCH)
  (CL:LET* ((S (NEW-OUTPUT-STRING-STREAM)))
   (CL:LET* ((*CURRENT-STREAM* S)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
  ...
  (CL:LET* ((CLASS NULL) (ITER-000 (%%REST CLASSLIST)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:COND
     ...
     (CL:T))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE CLASS))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " ")
  :VOID)
   ...
   (CL:LET* ((PARAMETER NULL) (ITER-000 (%%REST PARAMETERS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ PARAMETER (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "class " PARAMETER ", ")))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ">" EOL))
  :VOID)
...
(CL:DEFUN CPP-OUTPUT-FORMAL-PARAMETERS (PARAMETERS)
  (CL:LET* ((N-PARAMETERS (LENGTH PARAMETERS)) (VARIABLE-ARGUMENTS? FALSE))
   (CL:DECLARE (CL:TYPE CL:FIXNUM N-PARAMETERS))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "(")
   (CL:LET*
    ((N NULL-INTEGER) (PARAMETER NULL) (ITER-000 PARAMETERS) (ITER-001 1)
     (UPPER-BOUND-000 N-PARAMETERS) (UNBOUNDED?-000 (NULL? UPPER-BOUND-000)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 UPPER-BOUND-000 N))
    (CL:LOOP WHILE
     (CL:AND (CL:NOT (CL:EQ ITER-000 NIL))
      (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000)))
     DO
     (CL:PROGN (CL:SETQ PARAMETER (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:PROGN (CL:SETQ N ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
     (CL:IF (CL:= N N-PARAMETERS)
      (CL:PROGN
       ...
       (CPP-OUTPUT-TYPED-ENTITY
        (CL:IF VARIABLE-ARGUMENTS? (WRAP-STRING "int") (%%VALUE PARAMETER))
        (%%VALUE (%%REST PARAMETER)) NULL)
       (CL:WHEN VARIABLE-ARGUMENTS?
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", ...")))
      (CL:PROGN
       (CPP-OUTPUT-TYPED-ENTITY (%%VALUE PARAMETER) (%%VALUE (%%REST PARAMETER))
        NULL)
       (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", ")))))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ")"))
  :VOID)
...
(CL:DEFUN CPP-ARGUMENT-IS-STREAM? (ARG)
  (CL:COND
   ...
   (CL:T (CL:RETURN-FROM CPP-ARGUMENT-IS-STREAM? FALSE))))
...
(CL:DEFUN CPP-OUTPUT-DEFPRINT (DEFPRINT)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " {" EOL)
  (CPP-BUMP-INDENT)
  (CL:LET* ((*OUTPUTTINGDEFPRINT?* TRUE))
...
(CL:DEFUN CPP-OUTPUT-FUNCTION-SIGNATURE (FUNCTION)
  (CL:LET* ((RESULT-000 STELLA::NULL-STRING))
   ...
   (CL:LET* ((BARESIGNATURE RESULT-000))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BARESIGNATURE))
    (CPP-OUTPUT-TYPED-ENTITY (%%VALUE FUNCTION) (WRAP-STRING BARESIGNATURE)
     NULL)))
  :VOID)
  ...
  (CL:LET* ((BODY (FOURTH FUNCTION)) (BASEMEMBERS (FIFTH FUNCTION)))
   ...
   (CL:WHEN (CL:NOT (CL:EQ BASEMEMBERS NULL))
    ...
    (CL:LET* ((MEMBER NULL) (ITER-000 BASEMEMBERS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ MEMBER (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CPP-OUTPUT-STATEMENT MEMBER))))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " {" EOL) (CPP-BUMP-INDENT)
   (CPP-OUTPUT-STATEMENT BODY) (CPP-UNBUMP-INDENT)
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "}" EOL)
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL))
  :VOID)
...
(CL:DEFUN HELP-STARTUP-CPP-OUTPUT3 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CAST"
    "(DEFUN CPP-OUTPUT-CAST ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-CAST) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-SIGNAL"
    "(DEFUN CPP-OUTPUT-SIGNAL ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-SIGNAL)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CATCH"
    "(DEFUN CPP-OUTPUT-CATCH ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-CATCH) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-BREAK" "(DEFUN CPP-OUTPUT-BREAK ())"
    (CL:FUNCTION CPP-OUTPUT-BREAK) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CONTINUE" "(DEFUN CPP-OUTPUT-CONTINUE ())"
    (CL:FUNCTION CPP-OUTPUT-CONTINUE) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CASE"
    "(DEFUN CPP-OUTPUT-CASE ((CASE CONS)))" (CL:FUNCTION CPP-OUTPUT-CASE) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-COND"
    "(DEFUN CPP-OUTPUT-COND ((COND CONS)))" (CL:FUNCTION CPP-OUTPUT-COND) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-PROGN?"
    "(DEFUN (CPP-PROGN? BOOLEAN) ((TREE OBJECT)))" (CL:FUNCTION CPP-PROGN?) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-BLOCK?"
    "(DEFUN (CPP-BLOCK? BOOLEAN) ((TREE OBJECT)))" (CL:FUNCTION CPP-BLOCK?) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-IF" "(DEFUN CPP-OUTPUT-IF ((TREE CONS)))"
    (CL:FUNCTION CPP-OUTPUT-IF) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-WHEN"
    "(DEFUN CPP-OUTPUT-WHEN ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-WHEN) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-UNLESS"
    "(DEFUN CPP-OUTPUT-UNLESS ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-UNLESS)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-LOOP"
    "(DEFUN CPP-OUTPUT-LOOP ((LOOP CONS)))" (CL:FUNCTION CPP-OUTPUT-LOOP) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-WHILE"
    "(DEFUN CPP-OUTPUT-WHILE ((LOOP CONS)))" (CL:FUNCTION CPP-OUTPUT-WHILE) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-FOREACH"
    "(DEFUN CPP-OUTPUT-FOREACH ((LOOP CONS)))" (CL:FUNCTION CPP-OUTPUT-FOREACH)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-MAKE"
    "(DEFUN CPP-OUTPUT-MAKE ((STATEMENT CONS)))" (CL:FUNCTION CPP-OUTPUT-MAKE)
    NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-BUMP-INDENT" "(DEFUN CPP-BUMP-INDENT ())"
    (CL:FUNCTION CPP-BUMP-INDENT) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-UNBUMP-INDENT" "(DEFUN CPP-UNBUMP-INDENT ())"
    (CL:FUNCTION CPP-UNBUMP-INDENT) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((OBJECT OBJECT)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD CPP-OUTPUT-LITERAL ((CHARACTER CHARACTER-WRAPPER)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((INT INTEGER-WRAPPER)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((FLOAT FLOAT-WRAPPER)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((STRING STRING)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((SYMBOL SYMBOL)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((SURROGATE SURROGATE)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD CPP-OUTPUT-LITERAL ((KEYWORD KEYWORD)))"
    (CL:FUNCTION CPP-OUTPUT-LITERAL) NULL)
...
(CL:DEFUN STARTUP-CPP-OUTPUT ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *OUTPUTTINGDEFPRINT?* FALSE))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-CPP-OUTPUT3)
    ...
    (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-PROGN"
     "(DEFUN CPP-OUTPUT-PROGN ((PROGN CONS)))" (CL:FUNCTION CPP-OUTPUT-PROGN)
     NULL)
    (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-BLOCK"
     "(DEFUN CPP-OUTPUT-BLOCK ((BLOCK CONS)))" (CL:FUNCTION CPP-OUTPUT-BLOCK)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CLASS"
     "(DEFUN CPP-OUTPUT-CLASS ((CLASSDEF CONS)))" (CL:FUNCTION CPP-OUTPUT-CLASS)
     NULL)
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-CPP-OUTPUT-STELLA-STARTUP-CPP-OUTPUT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-CPP-OUTPUT-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Cpp-Output") NULL-STRING-WRAPPER)))
