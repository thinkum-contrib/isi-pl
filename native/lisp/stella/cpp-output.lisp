...
(CL:DEFVAR SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING NULL)
(CL:DEFVAR SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING-EXPANDER NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* *TRANSLATOROUTPUTLANGUAGE* EOL
  NULL-CHARACTER *CURRENT-STREAM*))

;;; (DEFSPECIAL *CPP-INDENT-CHARS* ...)

(CL:PROGN (CL:DEFVAR *CPP-INDENT-CHARS* 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM *CPP-INDENT-CHARS*)))
...
(CL:DEFUN CPP-OUTPUT-TO-STRING-EXPANDER (STATEMENT)
  (CL:LET*
   ((OLDVAR (LOCAL-GENSYM "OLD-CURRENT-STREAM"))
    (RESULTVAR (LOCAL-GENSYM "RESULT")))
   (CL:RETURN-FROM CPP-OUTPUT-TO-STRING-EXPANDER
...
(CL:DEFUN CPP-OUTPUT-STATEMENT (STATEMENT)
  (CL:COND
      ...
      (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
       (CL:COND
        ...
        ((CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_THROW)
         (CPP-OUTPUT-THROW (%%REST STATEMENT)))
        ...
        ((CL:OR (CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_GLOBAL)
         ...
         (CPP-OUTPUT-GLOBAL-DEFINITION STATEMENT))
        ...
        ((CL:EQ TEST-VALUE-000 SYM-CPP-OUTPUT-STELLA-CPP_PROGN)
         (CPP-OUTPUT-PROGN (%%REST STATEMENT)))
   ...
   (CL:T
    (CL:WHEN (CL:EQ STATEMENT NULL) (CL:RETURN-FROM CPP-OUTPUT-STATEMENT))
...
(CL:DEFUN CPP-OUTPUT-COMMENT (TREE)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// ")
  (CL:LET* ((NEWLINEP FALSE) (RETURNP FALSE))
   (CL:LET*
     ...
     (CL:CASE CHAR
      (#\Linefeed
       (CL:WHEN (CL:NOT RETURNP)
        ...
        (CPP-INDENT)
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// "))
       (CL:SETQ RETURNP FALSE) (CL:SETQ NEWLINEP TRUE))
      (#\Return (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
       (CPP-INDENT)
       (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// ")
       (CL:SETQ RETURNP TRUE) (CL:SETQ NEWLINEP FALSE))
      (CL:OTHERWISE
       (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR)
       (CL:SETQ RETURNP FALSE) (CL:SETQ NEWLINEP FALSE)))))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL))
  :VOID)
...
;;; (DEFUN CPP-OUTPUT-THROW ...)

(CL:DEFUN CPP-OUTPUT-THROW (TREE)
...
(CL:DEFUN CPP-OUTPUT-CATCH (TREE)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (%WRAPPER-VALUE (%%VALUE TREE)))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "& exception) {" EOL)
  (CPP-BUMP-INDENT)
  (CPP-INDENT)
...
(CL:DEFUN CPP-OUTPUT-UNWIND-PROTECT (TREE)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "try ")
  (CPP-OUTPUT-STATEMENT (CPP-PROGNIFY-STATEMENT (%%VALUE TREE)))
  (CPP-INDENT)
...
(CL:DEFUN CPP-OUTPUT-GLOBAL-DECLARATION (GLOBAL)
  ...
  (CL:COND
   ((CL:EQ (%%VALUE GLOBAL) SYM-CPP-OUTPUT-STELLA-CPP_SPECIAL)
    (CL:IF
     ...
     (CPP-OUTPUT-STATEMENT
      (LIST* SYM-CPP-OUTPUT-STELLA-CPP_FUNCTION_CALL
       ...
       (LIST* SYM-CPP-OUTPUT-STELLA-CPP_ACTUALS
        (LIST* SYM-CPP-OUTPUT-STELLA-CPP_IDENT
         (%%VALUE (%%REST (%%REST GLOBAL))) NIL)
        (%%VALUE (%%REST GLOBAL)) NIL)
       NIL))
     (CPP-OUTPUT-TYPED-ENTITY (%%VALUE (%%REST GLOBAL))
      (%%VALUE (%%REST (%%REST GLOBAL))) NULL)))
   (CL:T
    (CPP-OUTPUT-TYPED-ENTITY (%%VALUE (%%REST GLOBAL))
     (%%VALUE (%%REST (%%REST GLOBAL))) NULL)))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)
...
;;; (DEFUN CPP-OUTPUT-GLOBAL-DEFINITION ...)

(CL:DEFUN CPP-OUTPUT-GLOBAL-DEFINITION (GLOBAL)
  (CL:COND
   ((CL:EQ (%%VALUE GLOBAL) SYM-CPP-OUTPUT-STELLA-CPP_SPECIAL)
    (CL:IF
     (CL:EQ (SPECIAL-IMPLEMENTATION-STYLE)
     ...
     (CPP-OUTPUT-STATEMENT
      (LIST* SYM-CPP-OUTPUT-STELLA-CPP_FUNCTION_CALL
       ...
       (LIST* SYM-CPP-OUTPUT-STELLA-CPP_ACTUALS
        (LIST* SYM-CPP-OUTPUT-STELLA-CPP_IDENT
         (%%VALUE (%%REST (%%REST GLOBAL))) NIL)
        (%%VALUE (%%REST GLOBAL)) (CONS (FOURTH GLOBAL) NIL))
       NIL))
     (CPP-OUTPUT-TYPED-ENTITY (%%VALUE (%%REST GLOBAL))
      (%%VALUE (%%REST (%%REST GLOBAL))) (FOURTH GLOBAL))))
   (CL:T
    (CPP-OUTPUT-TYPED-ENTITY (%%VALUE (%%REST GLOBAL))
     (%%VALUE (%%REST (%%REST GLOBAL))) (FOURTH GLOBAL))))
   ...
   (CL:LET* ((RESULT-000 STELLA::NULL-STRING))
    ...
    (CL:LET* ((*CURRENT-STREAM* (NEW-OUTPUT-STRING-STREAM)))
     ...
     (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " (*) (")
      (CL:LET*
       ((I NULL-INTEGER) (PARTYPE NULL) (ITER-000 PARAMETERTYPES)
...
;;; (DEFUN (LAST-STATEMENT-IS-RETURN? BOOLEAN) ...)

(CL:DEFUN LAST-STATEMENT-IS-RETURN? (STATEMENTS)
  (CL:LET* ((ALLBUTLASTITEM NULL) (LASTITEM NULL))
   (CL:MULTIPLE-VALUE-SETQ (ALLBUTLASTITEM LASTITEM)
    (SEPARATE-LAST-CONS (%%REST STATEMENTS)))
   (CL:RETURN-FROM LAST-STATEMENT-IS-RETURN?
    (CL:AND (CL:NOT (CL:EQ LASTITEM NULL))
...
(CL:DEFUN CPP-OUTPUT-CASE (CASE)
  (CL:LET*
   ...
   (CL:LET* ((C NULL) (ITER-000 CONDITIONS))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET* ((CONDITION C))
      ...
      (CPP-UNBUMP-INDENT)
      (CL:WHEN
       (CL:NOT (LAST-STATEMENT-IS-RETURN? (%%VALUE (%%REST CONDITION))))
       (CPP-INDENT)
       (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "break;" EOL)))))
   ...
   (CPP-UNBUMP-INDENT)
   (CL:WHEN (CL:NOT (LAST-STATEMENT-IS-RETURN? DEFAULTCASE)) (CPP-INDENT)
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "break;" EOL))
...
(CL:DEFUN CPP-OUTPUT-IF (TREE)
  ...
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE (%%REST (%%REST TREE))) NULL))
   (CPP-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "else ")
...
(CL:DEFUN CPP-OUTPUT-FOREACH (LOOP)
  (CL:LET*
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ") {" EOL)
   (CPP-BUMP-INDENT) (SECOND-SETTER LOOP NULL)
   (THIRD-SETTER LOOP
    (CPP-FLATTEN-STATEMENTS (CONCATENATE NEXTASSIGNMENTS BODY)))
   (CPP-OUTPUT-STATEMENTS (%%VALUE (%%REST (%%REST LOOP))))
   (CPP-UNBUMP-INDENT) (CPP-INDENT)
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "}" EOL))
  :VOID)
...
(CL:DEFUN CPP-INDENT ()
  (CL:LET*
   ((D NULL-INTEGER) (ITER-000 1) (UPPER-BOUND-000 *CPP-INDENT-CHARS*)
    (UNBOUNDED?-000 (NULL? UPPER-BOUND-000)))
...
;;; (DEFUN (OUTPUT-SEMICOLON? BOOLEAN) ...)

(CL:DEFUN OUTPUT-SEMICOLON? (STATEMENT)
  (CL:IF (CL:NOT (CL:EQ STATEMENT NULL))
   (CL:RETURN-FROM OUTPUT-SEMICOLON?
    (MEMBER?
     (GET-QUOTED-TREE
      "((CPP_ASSIGN CPP_UNARY_OP CPP_BINARY_OP CPP_TERNARY_OP CPP_CAST CPP_FUNCALL CPP_FUNCTION_CALL CPP_FUNCTION_POINTER CPP_MAKE CPP_METHOD_CALL CPP_METHOD_CODE_CALL CPP_METHOD_POINTER CPP_METHOD_SETTER_CALL CPP_NEW CPP_PRINT_NATIVE_STREAM CPP_PRINT_STREAM CPP_REFERENCED_METHOD_CALL CPP_RETURN CPP_SIGNAL CPP_SLOT_VALUE_SETTER CPP_THROW CPP_VERBATIM) \"/STELLA\")"
      "/STELLA")
     (%%VALUE STATEMENT)))
   (CL:RETURN-FROM OUTPUT-SEMICOLON? FALSE)))
...
(CL:DEFUN ILLEGAL-CPP-STATEMENT? (STATEMENT)
  (CL:IF (CL:NOT (CL:EQ STATEMENT NULL))
   (CL:RETURN-FROM ILLEGAL-CPP-STATEMENT?
    (MEMBER?
     (GET-QUOTED-TREE "((CPP_LITERAL CPP_IDENT) \"/STELLA\")" "/STELLA")
     (%%VALUE STATEMENT)))
  ...
  (CL:LET* ((STATEMENT NULL) (ITER-000 STATEMENTLIST))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (ILLEGAL-CPP-STATEMENT? STATEMENT))
     ...
     (CL:WHEN (OUTPUT-SEMICOLON? STATEMENT)
      (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)))))
  :VOID)
...
(CL:DEFUN CPP-OUTPUT-PRINT-STREAM (EXPS)
  (CL:LET* ((STREAM (%%VALUE EXPS)) (LASTITEM NULL) (ALLBUTLASTITEM NULL))
   (CL:MULTIPLE-VALUE-SETQ (ALLBUTLASTITEM LASTITEM)
    (SEPARATE-LAST-CONS (%%REST EXPS)))
   (CL:COND
    ((CL:EQ STREAM SYM-CPP-OUTPUT-STELLA-CPP_STANDARD_OUT)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "cout"))
    ((CL:EQ STREAM SYM-CPP-OUTPUT-STELLA-CPP_STANDARD_ERROR)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "cerr"))
    (CL:T
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "get_ostream(")
     (CPP-OUTPUT-STATEMENT STREAM)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ")")))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " << ")
   (CL:LET* ((E NULL) (ITER-000 ALLBUTLASTITEM))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ E (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CPP-MAYBE-OUTPUT-STATEMENT-WITH-PARENTHESES E)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " << ")))
   (CPP-MAYBE-OUTPUT-STATEMENT-WITH-PARENTHESES LASTITEM))
  :VOID)
...
(CL:DEFUN CPP-OUTPUT-STATEMENT-LINE (STATEMENT)
  (CL:LET* ((LASTTOKEN NULL) (ALLBUTLASTTOKEN NULL))
   (CL:MULTIPLE-VALUE-SETQ (ALLBUTLASTTOKEN LASTTOKEN)
    (SEPARATE-LAST-CONS STATEMENT))
   (CL:LET* ((E NULL) (ITER-000 ALLBUTLASTTOKEN))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ E (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CPP-OUTPUT-STATEMENT E)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " ")))
   (CPP-OUTPUT-STATEMENT LASTTOKEN)
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL))
  :VOID)
      ...
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE TYPE)))
       (CL:COND
        ((CL:EQ TEST-VALUE-000 SGT-CPP-OUTPUT-STELLA-CONS)
         (CL:PROGN
          (CL:WHEN
           (CL:NOT (CL:EQ (%%VALUE TYPE) SYM-CPP-OUTPUT-STELLA-CPP_TYPE))
   ...
   (CL:LET* ((*CURRENT-STREAM* (NEW-OUTPUT-STRING-STREAM)))
    ...
    (CL:PROGN
     (CL:WHEN
      ...
      (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
       (%WRAPPER-VALUE (%%VALUE SIGNATURE)))
      (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "::"))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
      (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST SIGNATURE)))))
     (CPP-OUTPUT-FORMAL-PARAMETERS (FOURTH SIGNATURE)))
    (CL:SETQ RESULT-000 (THE-STRING-READER *CURRENT-STREAM*)))
   (CL:LET* ((BARESIGNATURE RESULT-000))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BARESIGNATURE))
    (CPP-OUTPUT-TYPED-ENTITY (%%VALUE (%%REST SIGNATURE))
...
(CL:DEFUN CPP-OUTPUT-PROGN (PROGN)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "{" EOL)
  (CPP-BUMP-INDENT)
  (CPP-OUTPUT-STATEMENT (%%VALUE PROGN))
  (CPP-UNBUMP-INDENT)
  (CPP-INDENT)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "}" EOL)
  :VOID)
    ...
    (CL:LET* ((PARAMETER NULL) (ITER-000 ALLBUTLASTPARAMETER))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "class "
       (%WRAPPER-VALUE PARAMETER) ", ")))
    (CL:WHEN (CL:NOT (CL:EQ LASTPARAMETER NULL))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "class "
      (%WRAPPER-VALUE LASTPARAMETER)))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ">" EOL)))
  :VOID)
  ...
  (CL:LET* ((LASTPARAMETER NULL) (ALLBUTLASTPARAMETER NULL))
   (CL:LET* ((VALUE-000 NULL) (VALUE-001 NULL))
    (CL:MULTIPLE-VALUE-SETQ (VALUE-000 VALUE-001)
     (SEPARATE-LAST-CONS PARAMETERS))
    (CL:PROGN (CL:SETQ ALLBUTLASTPARAMETER VALUE-000)
     (CL:SETQ LASTPARAMETER VALUE-001)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "(")
   (CL:LET* ((PARAMETER NULL) (ITER-000 ALLBUTLASTPARAMETER))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ PARAMETER (%%VALUE ITER-000))
   ...
   (CL:WHEN (CL:NOT (CL:EQ LASTPARAMETER NULL))
    (CL:LET*
     ((VARIABLEARGUMENTS?
       (CL:EQ (%%VALUE LASTPARAMETER)
        SYM-CPP-OUTPUT-STELLA-CPP_VAR_ARGS)))
     (CPP-OUTPUT-TYPED-ENTITY
      (CL:IF VARIABLEARGUMENTS? (WRAP-STRING "int")
       (%%VALUE LASTPARAMETER))
      (%%VALUE (%%REST LASTPARAMETER)) NULL)
     (CL:WHEN VARIABLEARGUMENTS?
      (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ", ..."))))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ")"))
  :VOID)

;;; (DEFUN (ARGUMENT-IS-STREAM? BOOLEAN) ...)

(CL:DEFUN ARGUMENT-IS-STREAM? (ARG)
  (CL:COND
   ((CL:EQ (SAFE-PRIMARY-TYPE ARG) SGT-CPP-OUTPUT-STELLA-CONS)
    (CL:PROGN
     (CL:RETURN-FROM ARGUMENT-IS-STREAM?
      (CL:AND (CL:EQ (%%VALUE ARG) SYM-CPP-OUTPUT-STELLA-CPP_IDENT)
       (STRING-EQL? (%WRAPPER-VALUE (%%VALUE (%%REST ARG))) "stream")))))
   (CL:T (CL:RETURN-FROM ARGUMENT-IS-STREAM? FALSE))))
  ...
  (CL:LET* ((LASTPARAMETER NULL) (ALLBUTLASTPARAMETER NULL))
   ...
   (CL:LET* ((PARAMETER NULL) (ITER-000 ALLBUTLASTPARAMETER))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ PARAMETER (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:IF (CL:AND *OUTPUTTINGDEFPRINT?* (ARGUMENT-IS-STREAM? PARAMETER))
      (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "&")
   ...
   (CL:WHEN (CL:NOT (CL:EQ LASTPARAMETER NULL))
    (CL:IF
     (CL:AND *OUTPUTTINGDEFPRINT?* (ARGUMENT-IS-STREAM? LASTPARAMETER))
     (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "&")
      (CPP-OUTPUT-STATEMENT LASTPARAMETER))
     (CPP-OUTPUT-STATEMENT LASTPARAMETER)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ")"))
  :VOID)
...
(CL:DEFUN CPP-OUTPUT-FUNCTION (FUNCTION)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " {" EOL)
  (CPP-BUMP-INDENT)
  (CPP-OUTPUT-STATEMENT (FOURTH FUNCTION))
  (CPP-UNBUMP-INDENT)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "}" EOL)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
  :VOID)
...
(CL:DEFUN HELP-STARTUP-CPP-OUTPUT1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CPP-OUTPUT-TO-STRING" NULL 0))
   (CL:SETQ SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING-EXPANDER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CPP-OUTPUT-TO-STRING-EXPANDER" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-CPP-OUTPUT3 ()
  (CL:PROGN
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-TO-STRING-EXPANDER"
    "(DEFUN (CPP-OUTPUT-TO-STRING-EXPANDER OBJECT) ((STATEMENT CONS)))"
    (CL:FUNCTION CPP-OUTPUT-TO-STRING-EXPANDER) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CAST"
    "(DEFUN CPP-OUTPUT-CAST ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-CAST)
    NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-SIGNAL"
    "(DEFUN CPP-OUTPUT-SIGNAL ((TREE CONS)))"
    (CL:FUNCTION CPP-OUTPUT-SIGNAL) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-THROW"
    "(DEFUN CPP-OUTPUT-THROW ((TREE CONS)))"
    (CL:FUNCTION CPP-OUTPUT-THROW) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-GLOBAL-DEFINITION"
    "(DEFUN CPP-OUTPUT-GLOBAL-DEFINITION ((GLOBAL CONS)))"
    (CL:FUNCTION CPP-OUTPUT-GLOBAL-DEFINITION) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CONTINUE"
    ...
    NULL)
   (DEFINE-FUNCTION-OBJECT "LAST-STATEMENT-IS-RETURN?"
    "(DEFUN (LAST-STATEMENT-IS-RETURN? BOOLEAN) ((STATEMENTS CONS)))"
    (CL:FUNCTION LAST-STATEMENT-IS-RETURN?) NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-CASE"
    "(DEFUN CPP-OUTPUT-CASE ((CASE CONS)))" (CL:FUNCTION CPP-OUTPUT-CASE)
    NULL)
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-COND"
    "(DEFUN CPP-OUTPUT-COND ((COND CONS)))" (CL:FUNCTION CPP-OUTPUT-COND)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-WHEN"
    "(DEFUN CPP-OUTPUT-WHEN ((TREE CONS)))" (CL:FUNCTION CPP-OUTPUT-WHEN)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-LOOP"
    "(DEFUN CPP-OUTPUT-LOOP ((LOOP CONS)))" (CL:FUNCTION CPP-OUTPUT-LOOP)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "CPP-UNBUMP-INDENT"
    "(DEFUN CPP-UNBUMP-INDENT ())" (CL:FUNCTION CPP-UNBUMP-INDENT) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "OUTPUT-SEMICOLON?"
    "(DEFUN (OUTPUT-SEMICOLON? BOOLEAN) ((STATEMENT CONS)))"
    (CL:FUNCTION OUTPUT-SEMICOLON?) NULL))
  :VOID)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-CPP-OUTPUT3)
    ...
    (DEFINE-FUNCTION-OBJECT "CPP-OUTPUT-FORMAL-PARAMETERS"
     "(DEFUN CPP-OUTPUT-FORMAL-PARAMETERS ((PARAMETERS CONS)))"
     (CL:FUNCTION CPP-OUTPUT-FORMAL-PARAMETERS) NULL)
    (DEFINE-FUNCTION-OBJECT "ARGUMENT-IS-STREAM?"
     "(DEFUN (ARGUMENT-IS-STREAM? BOOLEAN) ((ARG OBJECT)))"
     (CL:FUNCTION ARGUMENT-IS-STREAM?) NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Cpp-Output") NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFSPECIAL *OUTPUTTINGDEFPRINT?* BOOLEAN FALSE :DOCUMENTATION \"TRUE when outputting a defprint, used to 
            ...
            statements.  Also used to take the address of streams
            when passed to functions inside of a defprint\")")
    (STORE-MACRO SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING
     SYM-CPP-OUTPUT-STELLA-CPP-OUTPUT-TO-STRING-EXPANDER
     (CL:FUNCTION CPP-OUTPUT-TO-STRING-EXPANDER))))
  :VOID)
