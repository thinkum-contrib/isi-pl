...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* EOL
  *TOKENIZER-INITIAL-BUFFER-SIZE* *USE-STELLA-HASH-TABLES?*
  *CHARACTER-UPCASE-TABLE* *CHARACTER-DOWNCASE-TABLE*))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) DIV))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) REM))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) MOD))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) GCD))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) MIN))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) MAX))
...
(CL:DEFMETHOD HASH-CODE ((SELF BOOLEAN-WRAPPER))
  (CL:RETURN-FROM HASH-CODE
   (CL:IF (%WRAPPER-VALUE SELF) 27333705 31891526)))
...
handle NULL components should use this function for recursive calls."
  (CL:IF (CL:EQ SELF NULL)
   (CL:RETURN-FROM SAFE-EQUAL-HASH-CODE 63842277)
...
any data already in `toFile'."
  (CL:WHEN (CL:NOT (PROBE-FILE? FROMFILE))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
     "Can't copy non-existent file: `" FROMFILE "'" EOL)
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
  (CL:LET* ((FROM NULL))
   (CL:UNWIND-PROTECT
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) LOGAND))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) LOGOR))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) LOGXOR))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) SHIFT-LEFT))
...
and is equivalent to dividing `arg' by 2** `count'."
  ...
  (CL:RETURN-FROM SHIFT-RIGHT
   (CL:ASH ARG (CL:THE CL:FIXNUM (CL:- COUNT)))))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:FIXNUM CL:FIXNUM) CL:FIXNUM) SHIFT-RIGHT))
  ...
  (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 0)
    ...
    (CL:LOOP WHILE (CL:<= ITER-009 UPPER-BOUND-010) DO
     ...
     (CL:LET* ((I NULL-INTEGER) (ITER-011 0) (UPPER-BOUND-012 255))
      ...
      (CL:LOOP WHILE (CL:<= ITER-011 UPPER-BOUND-012) DO
       (CL:PROGN (CL:SETQ I ITER-011)
        (CL:SETQ ITER-011 (CL:1+ ITER-011)))
   ...
   (CL:SETQ *HASH-TABLE-SIZE-PRIMES*
    (VECTOR (WRAP-INTEGER 29) (WRAP-INTEGER 53) (WRAP-INTEGER 97)
     ...
     (WRAP-INTEGER 786433) (WRAP-INTEGER 1572869)
     (WRAP-INTEGER 3145739) (WRAP-INTEGER 6291469)
     (WRAP-INTEGER 12582917) (WRAP-INTEGER 25165843)
     (WRAP-INTEGER 50331653) (WRAP-INTEGER 100663319)
     (WRAP-INTEGER 201326611) (WRAP-INTEGER 402653189))))
...
NULL if that cannot be determined.\" :NATIVE? TRUE :PUBLIC? TRUE)" NULL
     NULL)
...
exception.  For example, in Java it prints a stack trace.  In Lisp, it is vendor dependent.\")"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Primal") NULL-STRING-WRAPPER)))
