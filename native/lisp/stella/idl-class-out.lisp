...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* FALSE *METHODBEINGWALKED* TRUE NIL))
...
(CL:DEFUN IDL-YIELD-ATTRIBUTE-TREE (SLOT CLASS)
  (CL:LET*
   ((SLOTNAME (TRUE-SLOT-NAME (%SLOT-NAME SLOT) (%CLASS-TYPE CLASS)))
    (INITIALVALUEFORM (INITIALLY SLOT)) (FACETFORM NIL) (MOSTGENERALSLOT NULL))
   (CL:WHEN
    (CL:EQ (IDL-CLASS-CATEGORY CLASS) SYM-IDL-CLASS-OUT-STELLA-IDL_INTERFACE)
...
(CL:DEFUN IDL-YIELD-PARAMETERIZED-ATTRIBUTE-TREE (SLOT CLASS)
  (CL:LET*
   ...
   (CL:WHEN (CL:EQ INITIALVALUEFORM NULL)
    (CL:SETQ INITIALVALUEFORM (IDL-YIELD-INITIAL-NULL-VALUE)))
   (CL:COND
    ((CL:EQ ALLOCATION KWD-IDL-CLASS-OUT-EMBEDDED) (CL:SETQ ALLOCATIONFORM NIL))
   ...
   (CL:RETURN-FROM IDL-YIELD-PARAMETERIZED-ATTRIBUTE-TREE
    (LIST*
     (CONCATENATE ALLOCATIONFORM
      (CONS
       (IDL-TRANSLATE-CLASS-PARAMETER-NAME (GET-SLOT-CLASS-PARAMETER-NAME SLOT))
       NIL))
     (IDL-TRANSLATE-NAME SLOTNAME) (CONS INITIALVALUEFORM NIL)))))
     ...
     (CL:IF (%METHOD-FUNCTION? METHOD) NULL
      (IDL-TRANSLATE-CLASS-NAME
       (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%SLOT-OWNER METHOD))
        (%HOME-CONTEXT (%SLOT-OWNER METHOD)) TRUE)))
...
(CL:DEFUN ILLEGAL-IDL-METHOD? (METHODSLOT)
  (CL:RETURN-FROM ILLEGAL-IDL-METHOD?
   (MEMBER? (GET-QUOTED-TREE "((DEFINED? NULL?) \"/STELLA\")" "/STELLA")
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      ...
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN
        (CL:SETQ COLLECT-000
         (CONS (IDL-YIELD-PARAMETERIZED-ATTRIBUTE-TREE SLOT CLASS) NIL))
    ...
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SLOT (%VALUE ITER-000))
     (CL:WHEN
      (CL:AND (CL:NOT (%SLOT-MARKED? SLOT))
       ...
       (CL:NOT (%METHOD-FUNCTION? SLOT)) (CL:NOT (ILLEGAL-IDL-METHOD? SLOT))
      ...
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN
        ...
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
   (CL:LET*
    ((AUXILIARY NULL) (ITER-001 (%THE-CONS-LIST (CLASS-AUXILIARY-METHODS CLASS)))
     (COLLECT-001 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ AUXILIARY (%%VALUE ITER-001))
...
(CL:DEFUN IDL-TRANSLATE-DEFINE-NATIVE-CLASS-UNIT (UNIT)
  (CL:LET*
   ((CLASS (%THE-OBJECT UNIT)) (DERIVEDCLASSES NIL)
    (PARAMETERIZEDATTRIBUTEDEFS NIL) (ATTRIBUTEDEFS NIL) (OPERATIONDEFS NIL)
    (CONSTRUCTORDEFS NIL) (PARAMETERS NIL) (STATICATTRIBUTEDEFS NIL))
   (CL:LET*
         ...
         (CONCATENATE
          ...
          (CONCATENATE
           (CL:IF (CL:NOT (CL:EQ ATTRIBUTEDEFS NIL))
            (CONS
             (LIST* SYM-IDL-CLASS-OUT-STELLA-IDL_DECLARATIONS ATTRIBUTEDEFS NIL)
             NIL)
            NIL)
           (CONCATENATE
            (CL:IF (CL:NOT (CL:EQ CONSTRUCTORDEFS NIL))
             (CONS
              (LIST* SYM-IDL-CLASS-OUT-STELLA-IDL_SIGNATURES CONSTRUCTORDEFS NIL)
              NIL)
             NIL)
            (CONCATENATE
             (CL:IF (CL:NOT (CL:EQ OPERATIONDEFS NIL))
              (CONS
               (LIST* SYM-IDL-CLASS-OUT-STELLA-IDL_SIGNATURES OPERATIONDEFS NIL)
               NIL)
              NIL)
             NIL))))))))))))
...
(CL:DEFUN STARTUP-IDL-CLASS-OUT ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-IDL-CLASS-OUT-STELLA-STARTUP-IDL-CLASS-OUT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-IDL-CLASS-OUT-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Idl-Class-Out") NULL-STRING-WRAPPER)))
