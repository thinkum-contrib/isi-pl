...
(CL:DEFVAR SYM-METHODS-STELLA-SLOT-HOME-MODULE NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* SYMBOL-SYM STANDARD-OUTPUT
  NULL-FUNCTION-CODE-WRAPPER *MODULE* *REDEFINE-RELATION-HOOKS*
         ...
         (CL:LET* ((*PRINTREADABLY?* TRUE))
          ...
          (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
           ...
           (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
            ...
            "'" EOL "in the definition of method `"
            (%SLOT-OWNER NEWMETHOD) "'.`" (DE-UGLIFY-PARSE-TREE NAME) "'"
            EOL ".")))))))))
...
(CL:PROGN (CL:DEFVAR *MAXIMUM-STRING-CONSTANT-SIZE* 4000 "Maximum size for a string constant in the target language.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *MAXIMUM-STRING-CONSTANT-SIZE*)))
...
(CL:DEFUN SYNTHESIZE-METHOD-BODY (METHOD)
  ...
  (CL:LET*
   ...
   (CL:COND
    ...
    (CL:T))
   (CL:SETQ STORAGESLOT
    (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE (%SLOT-OWNER METHOD))
     STORAGESLOTNAME))
        ...
        (CONCATENATE
         (CL:IF
          ...
          (CONS
           ...
           NIL))
         (CONS
          (LIST* SYM-METHODS-STELLA-RETURN SYM-METHODS-STELLA-VALUE NIL)
          NIL)))
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          ...
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
           ...
           "' of `" (DE-UGLIFY-PARSE-TREE SUBTYPE) "' is not yet defined"
           EOL ".")))
        (CL:RETURN-FROM COMPATIBLE-PARAMETER-TYPES? FALSE))
       (CL:WHEN
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          ...
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
           ...
           "   for the class `" (DE-UGLIFY-PARSE-TREE OWNERTYPE) "'" EOL
           EOL ".")))
     ...
     (CL:PROGN
      (CL:LET* ((LISTOFTYPES NIL))
       ...
       (CL:RETURN-FROM YIELD-TYPE-SPEC-TREE
        (CONS (TYPE-TO-SYMBOL (%SPECIFIER-BASE-TYPE SELF))
         (CONCATENATE
...
(CL:DEFUN ATTACH-FUNCTION (NEWFUNCTION)
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ (HOME-MODULE NEWFUNCTION) *MODULE*))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWFUNCTION)
     SYM-METHODS-STELLA-SLOT-HOME-MODULE *MODULE* NULL))
...
(CL:DEFUN UNDEFINE-CONFLICTING-DEFINITIONS? (NEWSLOT)
  (CL:COND
   ((%METHOD-FUNCTION? NEWSLOT)
    ...
    (CL:WHEN (CL:NOT (CL:EQ (LOOKUP-MACRO (%SLOT-NAME NEWSLOT)) NULL))
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (SIGNAL-TRANSLATION-WARNING)
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       ...
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL " Function `"
        ...
        "' is shadowed by a pre-exisiting macro." EOL
        " Removing the preexisting macro definition" EOL ".")))
     (UNDEFINE-MACRO (%SLOT-NAME NEWSLOT))))
   (CL:T
   ...
   (CL:WHEN
    (CL:OR (CL:<= (LENGTH TREE) 3)
     (CL:AND
      (MEMBER?
       (GET-QUOTED-TREE
        "((:DOCUMENTATION :PUBLIC? :AUXILIARY?) \"/STELLA\")" "/STELLA")
       INITIALVALUETREE)
...
(CL:DEFUN HELP-STARTUP-METHODS2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-METHODS-STELLA-SLOT-HOME-MODULE
...
'self' to 'value'.\" :PUBLIC? TRUE)" (CL:FUNCTION SET-GLOBAL-VALUE) NULL)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Methods") NULL-STRING-WRAPPER)))
