...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* SYMBOL-SYM STANDARD-OUTPUT *MODULE*
  ...
  STANDARD-WARNING TRUE-WRAPPER FALSE-WRAPPER NULL-INTEGER TRUE STANDARD-ERROR
  EOL NIL FALSE))
...
OBJECT."
  (CL:CASE (LENGTH ARGUMENTS)
   (0 (CL:ERROR (NEW-STELLA-EXCEPTION "Can't call method code on 0 arguments.")))
...
INTEGER."
  (CL:CASE (LENGTH ARGUMENTS)
   (0 (CL:ERROR (NEW-STELLA-EXCEPTION "Can't call method code on 0 arguments.")))
   (1
    (CL:RETURN-FROM APPLY-INTEGER-METHOD (CL:FUNCALL CODE (%%VALUE ARGUMENTS))))
   (2
...
BOOLEAN."
  (CL:CASE (LENGTH ARGUMENTS)
   (0 (CL:ERROR (NEW-STELLA-EXCEPTION "Can't call method code on 0 arguments.")))
   (1
    (CL:RETURN-FROM APPLY-BOOLEAN-METHOD (CL:FUNCALL CODE (%%VALUE ARGUMENTS))))
   (2
...
(CL:DEFUN YIELD-PARAMETRIC-TYPE-SPECIFIER (TYPESPECTREE)
  (CL:LET*
   ...
   (CL:LET* ((HEAD-000 (%%VALUE CURSOR))) (CL:SETQ CURSOR (%%REST CURSOR))
    (CL:LET* ((BASETYPE HEAD-000))
     (CL:LET* ((HEAD-001 (%%VALUE CURSOR))) (CL:SETQ CURSOR (%%REST CURSOR))
      (CL:LET* ((OPERATOR HEAD-001) (ARRAYTYPE? FALSE))
       (CL:SETF (%SPECIFIER-BASE-TYPE TYPESPEC) (VERIFY-TYPE (TYPIFY BASETYPE)))
       (CL:WHEN (ARRAY-TYPE-SPECIFIER? TYPESPEC)
        (CL:WHEN (CL:EQ OPERATOR SYM-METHODS-STELLA-SIZE)
         (CL:LET* ((HEAD-002 (%%VALUE CURSOR))) (CL:SETQ CURSOR (%%REST CURSOR))
          (CL:SETQ OPERATOR HEAD-002)))
        (CL:SETQ ARRAYTYPE? TRUE)
        (CL:SETF (%SPECIFIER-DIMENSIONS TYPESPEC)
         (PARSE-ARRAY-DIMENSIONS-SPEC OPERATOR))
        (CL:LET* ((HEAD-003 (%%VALUE CURSOR))) (CL:SETQ CURSOR (%%REST CURSOR))
         (CL:SETQ OPERATOR HEAD-003)))
    ...
    (CL:PROGN
     (CL:LET* ((BASETYPE (%SPECIFIER-BASE-TYPE TYPESPEC)))
      (CL:WHEN (CL:EQ BASETYPE SGT-METHODS-STELLA-ARRAY)
       (CL:RETURN-FROM ARRAY-TYPE-SPECIFIER? TRUE)))))
   (CL:T))
  (CL:RETURN-FROM ARRAY-TYPE-SPECIFIER? FALSE))
...
(CL:DEFUN INDEFINITE-ARRAY-TYPE? (ARRAYTYPE)
  (CL:LET* ((DIMENSIONS (%SPECIFIER-DIMENSIONS ARRAYTYPE)))
   (CL:IF (CL:EQ DIMENSIONS NULL) (CL:RETURN-FROM INDEFINITE-ARRAY-TYPE? TRUE)
    (CL:LET* ((DIM NULL) (ITER-000 (%THE-CONS-LIST DIMENSIONS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ DIM (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:WHEN (CL:EQ DIM NULL) (CL:RETURN-FROM INDEFINITE-ARRAY-TYPE? TRUE)))))
   (CL:RETURN-FROM INDEFINITE-ARRAY-TYPE? FALSE)))
...
(CL:DEFUN INCORPORATE-INPUT-PARAMETERS (METHOD PARAMETERS)
  ...
  (CL:LET* ((NAME NULL) (TS NULL) (DIRECTION NULL) (VARIABLEARGS? FALSE))
             ...
             ((CL:OR (CL:EQ (%%VALUE P) SYM-METHODS-STELLA-IN)
              ...
              (CL:WHEN (NULL-LIST? (METHOD-PARAMETER-DIRECTIONS METHOD))
               (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD)
                SYM-METHODS-STELLA-METHOD-PARAMETER-DIRECTIONS (NEW-LIST) NULL)
               (CL:LET* ((PAR NULL) (ITER-001 PARAMETERS))
                (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
                 (CL:PROGN (CL:SETQ PAR (%%VALUE ITER-001))
             ...
             (CL:NOT
              (CL:AND (SYMBOL? (%%VALUE P))
               (CL:OR (SYMBOL? (%%VALUE (%%REST P)))
                (SURROGATE? (%%VALUE (%%REST P))) (CONS? (%%VALUE (%%REST P))))))
             (CL:LET* ((*PRINTREADABLY?* TRUE))
              (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
              (SIGNAL-TRANSLATION-ERROR)
              (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
         ...
         (CL:PROGN
          (CL:COND
           ((CL:EQ P SYM-METHODS-STELLA-&REST)
            ...
            (CL:SETQ VARIABLEARGS? TRUE) (CL:GO :CONTINUE))
   ...
   (CL:WHEN (CL:NOT (%METHOD-FUNCTION? METHOD))
    (CL:COND
     ((EMPTY? PARAMETERTYPESPECS)
      ...
      (CL:SETF (%METHOD-FUNCTION? METHOD) TRUE)
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN
       ...
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))
   (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-001 (ALLOCATE-ITERATOR OPTIONS)))
     ...
     (CL:LET* ((TEST-VALUE-000 KEY))
      (CL:COND
           ...
           (CL:PROGN
            (CL:LET* ((FNNAME NULL) (ITER-002 VALUE))
             (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
              (CL:PROGN (CL:SETQ FNNAME (%%VALUE ITER-002))
       ...
       ((CL:EQ TEST-VALUE-000 KWD-METHODS-INHERITS-THROUGH)
        (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS NEWMETHOD)
         SYM-METHODS-STELLA-METHOD-INHERITS-THROUGH (PERMANENTIFY VALUE) NULL))
...
(CL:DEFUN YIELD-STRING-CONSTANT-TREE (STRING)
  ...
  (CL:LET*
   ...
   (CL:LET* ((CHUNK NULL) (ITER-000 (%%REST CHUNKS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ CHUNK (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:SETQ STRINGTREE
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
         ...
         (CL:LET* ((BASECLASS (%SURROGATE-VALUE (TYPE-SPEC-TO-BASE-TYPE SELF))))
          (CL:WHEN (%CLASS-SLOTS-FINALIZED? BASECLASS)
           (CL:LET* ((INHERITEDTYPES (YIELD-CLASS-PARAMETER-TYPES BASECLASS)))
            (CL:LET*
             ((SUBTYPESPEC NULL) (SUPERTYPE NULL) (ITER-000 INHERITEDTYPES)
             ...
             (CL:LOOP WHILE
              ...
              (CL:PROGN (CL:SETQ SUPERTYPE (%%VALUE ITER-000))
               (CL:SETQ ITER-000 (%%REST ITER-000)))
              (CL:PROGN (CL:SETQ SUBTYPESPEC (%%VALUE ITER-001))
               (CL:SETQ ITER-001 (%%REST ITER-001)))
              (CL:WHEN
    ...
    ((SUBTYPE-OF-ANCHORED-TYPE-SPECIFIER? TEST-VALUE-000)
     (CL:PROGN
      (CL:WHEN SKIPANCHOREDTYPE? (CL:RETURN-FROM VALIDATE-TYPE-SPECIFIER NULL))
...
(CL:DEFUN SYNTHESIZE-METHOD-BODY (METHOD)
  ...
  (CL:LET*
   ((STORAGESLOTNAME
     (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS METHOD) SYM-METHODS-STELLA-STORAGE-SLOT
      NULL))
    (INHERITANCESLOTNAME
   ...
   (CL:WHEN (CL:EQ STORAGESLOT NULL)
    (CL:WARN ":storage-slot references non-existent slot `~A' on the class `~A'"
       ...
       (CONS SYM-METHODS-STELLA-WHEN
        (CONCATENATE
         (CL:IF (SUBTYPE-OF? (TYPE STORAGESLOT) SGT-METHODS-STELLA-BOOLEAN)
          ...
          (CONS (LIST* SYM-METHODS-STELLA-DEFINED? SYM-METHODS-STELLA-VALUE NIL)
           NIL))
  ...
  (CL:LET* ((SLOTS NIL))
   (CL:LET* ((MODULE NULL) (ITER-000 (ALL-MODULES)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ MODULE (%VALUE ITER-000))
     (CL:LET* ((SLOT NULL) (ITER-001 (ALL-SLOTS MODULE TRUE)) (COLLECT-000 NULL))
  ...
  (CL:WHEN
   ...
   (CL:LET* ((OWNERCLASS (%SURROGATE-VALUE (%SLOT-OWNER SELF))))
    (CL:WHEN (CL:NOT (CL:EQ OWNERCLASS NULL))
     ...
     (UNFINALIZE-CLASS-SLOTS OWNERCLASS) (DELETED?-SETTER SELF TRUE))))
  :VOID)
...
(CL:DEFUN COMPATIBLE-PARAMETER-TYPES? (SUBTYPE SUPERTYPE)
  ...
  (CL:LET* ((SUBCLASS NULL) (SUPERCLASS NULL))
   ...
   (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE SUPERTYPE)))
    (CL:COND
     ...
     ((SUBTYPE-OF-PARAMETRIC-TYPE-SPECIFIER? TEST-VALUE-001)
      (CL:PROGN
       (CL:SETQ SUPERCLASS (%SURROGATE-VALUE (%SPECIFIER-BASE-TYPE SUPERTYPE)))))
     (CL:T
   ...
   (CL:LET* ((SUPPAR NULL) (ITER-000 (%THE-CONS-LIST (PARAMETERS SUPERCLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SUPPAR (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:TAGBODY
      (CL:LET* ((FOUND?-000 FALSE))
       (CL:LET* ((SUBPAR NULL) (ITER-001 (%THE-CONS-LIST (PARAMETERS SUBCLASS))))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
         (CL:PROGN (CL:SETQ SUBPAR (%%VALUE ITER-001))
          (CL:SETQ ITER-001 (%%REST ITER-001)))
         (CL:WHEN (CL:EQ SUPPAR SUBPAR) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
      ...
      (CL:LET*
       ...
       (CL:WHEN
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL " Parameter type `"
           (TYPE-SPEC-TO-BASE-TYPE SUBPARTYPE) "' of `"
       ...
       (CL:WHEN
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL " Parameter type `"
           (TYPE-SPEC-TO-BASE-TYPE SUPPARTYPE) "' of `"
      ...
      (CL:LET* ((SLOT (LOOKUP-SLOT (%SURROGATE-VALUE SELF) PARAMETER)))
       (CL:IF (CL:NOT (CL:EQ SLOT NULL))
        ...
        (CL:RETURN-FROM EXTRACT-PARAMETER-TYPE
         (CL:VALUES SGT-METHODS-STELLA-OBJECT FALSE))))))
    ((SUBTYPE-OF-PARAMETRIC-TYPE-SPECIFIER? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET*
        ...
        (CL:LET* ((*PRINTREADABLY?* TRUE))
         ...
         (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
          ...
          (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
           ...
           EOL "   for the class `" (DE-UGLIFY-PARSE-TREE OWNERTYPE) "'" EOL "."
           EOL)))
    ...
    ((SUBTYPE-OF-PARAMETRIC-TYPE-SPECIFIER? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET* ((ALWAYS?-000 TRUE))
       (CL:LET*
        ((TS NULL)
         (ITER-000 (%THE-CONS-LIST (%SPECIFIER-PARAMETER-TYPES RELATIVETYPE))))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ TS (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
         (CL:WHEN (CL:NOT (TYPE? TS)) (CL:SETQ ALWAYS?-000 FALSE) (CL:RETURN))))
       (CL:WHEN ALWAYS?-000
        (CL:RETURN-FROM COMPUTE-RELATIVE-TYPE-SPEC RELATIVETYPE)))
      (CL:LET*
       ((TYPESLIST (NEW-LIST)) (RELATIVETS NULL) (FOUNDRELATIVISTICTYPE? FALSE))
       (CL:LET*
        ((TS NULL)
    ...
    ((SUBTYPE-OF-SURROGATE? TEST-VALUE-000)
     (CL:PROGN
      (CL:RETURN-FROM YIELD-TYPE-SPEC-TREE
       (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME SELF) (%HOME-CONTEXT SELF) TRUE))))
    ((SUBTYPE-OF-PARAMETRIC-TYPE-SPECIFIER? TEST-VALUE-000)
     (CL:PROGN
      (CL:LET* ((LISTOFTYPES NIL))
       (CL:LET*
        ((TS NULL) (ITER-000 (%THE-CONS-LIST (%SPECIFIER-PARAMETER-TYPES SELF)))
         (COLLECT-000 NULL))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:PROGN (CL:SETQ TS (%%VALUE ITER-000))
          (CL:SETQ ITER-000 (%%REST ITER-000)))
       ...
       (CL:RETURN-FROM YIELD-TYPE-SPEC-TREE
        (CONS
         (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME (%SPECIFIER-BASE-TYPE SELF))
          (%HOME-CONTEXT (%SPECIFIER-BASE-TYPE SELF)) TRUE)
         (CONCATENATE
          (CL:IF (ARRAY-TYPE-SPECIFIER? SELF)
           (LIST* SYM-METHODS-STELLA-SIZE (YIELD-ARRAY-DIMENSIONS-TREE SELF) NIL)
           NIL)
...
(CL:DEFUN ATTACH-FUNCTION (NEWFUNCTION)
  (CL:LET* ((NAME (%SLOT-NAME NEWFUNCTION)) (OLDFUNCTION (LOOKUP-FUNCTION NAME)))
   ...
   (CL:SETF (%SLOT-EXTERNAL? NEWFUNCTION) TRUE)
    ...
    (CL:LET*
     ((RECORD FUNCTION)
      ...
      (CODE
       (CL:IF
        (CL:NOT
         (CL:EQ (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY RECORD)) 3) NULL))
        (%WRAPPER-VALUE
   ...
   (CL:LET* ((MODULE NULL) (ITER-000 (VISIBLE-MODULES *MODULE*)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ MODULE (%VALUE ITER-000))
     (CL:LET* ((TEST-VALUE-000 FALSE))
      (CL:LET* () (CL:SETQ SYMBOL (LOOKUP-SYMBOL-IN-MODULE NAME MODULE TRUE))
...
(CL:DEFUN DESTROY-FUNCTION (FUNCTION)
  (CL:LET*
   ...
   (DELETED?-SETTER FUNCTION TRUE) (FREE FUNCTION))
  :VOID)
...
(CL:DEFUN UNDEFINE-CONFLICTING-DEFINITIONS? (NEWSLOT)
   ...
   ((%METHOD-FUNCTION? NEWSLOT)
    (CL:WHEN (WARN-ABOUT-FUNCTION-SHADOWING-SLOTS? NEWSLOT)
     (CL:WHEN
      ...
      (CL:RETURN-FROM UNDEFINE-CONFLICTING-DEFINITIONS? FALSE))
  ...
  (CL:RETURN-FROM UNDEFINE-CONFLICTING-DEFINITIONS? TRUE))
...
(CL:DEFUN DEFINE-FUNCTION-OBJECT (NAME DEFINITION CODE WRAPPERCODE)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CL:NOT (CL:EQ CODE NULL))
    (CL:LET
     ((SELF (%THE-ARRAY RECORD)) (VALUE (WRAP-FUNCTION-CODE CODE)) (POSITION 3))
     (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF) (CL:TYPE CL:FIXNUM POSITION))
     (CL:SETF (CL:AREF SELF POSITION) VALUE)))
   (CL:WHEN (CL:NOT (CL:EQ WRAPPERCODE NULL))
    (CL:LET
...
(CL:DEFUN DEFINE-STELLA-GLOBAL-VARIABLE-FROM-PARSE-TREE (TREE STRINGIFIEDSOURCE)
  ...
  (CL:LET*
   ((TYPETREE (%%VALUE (%%REST (%%REST TREE)))) (INITIALVALUETREE (FOURTH TREE))
    ...
    (NOINITIALVALUE? FALSE) (GLOBAL (NEW-GLOBAL-VARIABLE)) (INTERNEDGLOBAL NULL))
   (CL:WHEN
   ...
   (CL:WHEN (CL:NOT (CL:EQ OPTIONSTREE NIL))
    (CL:LET* ((OPTIONS (NEW-PROPERTY-LIST)))
     ...
     (CL:LET* ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST OPTIONS)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
...
(CL:DEFUN DESTROY-VARIABLE (VARIABLE)
  (CL:LET*
   ...
   (DELETED?-SETTER VARIABLE TRUE) (FREE VARIABLE))
  :VOID)
...
to the class identified by the first parameter in 'inputParameters'.\")"
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD UNFINALIZE-LOCAL-SLOT ((SELF METHOD-SLOT)))"
...
Scan all visible symbols looking for one that has a function defined for it.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "UNDEFINE-FUNCTION"
     "(DEFUN UNDEFINE-FUNCTION ((NAME SYMBOL)))" (CL:FUNCTION UNDEFINE-FUNCTION)
     NULL)
...
'self' to 'value'.\" :PUBLIC? TRUE)" (CL:FUNCTION SET-GLOBAL-VALUE) NULL)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-METHODS-STELLA-STARTUP-METHODS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Methods") NULL-STRING-WRAPPER)))
