     ...
     (CL:LET* ((ARG3 (%%VALUE ARGUMENTS)))
      ...
      (CL:LET* ((ARG4 (%%VALUE ARGUMENTS)))
       ...
       (CL:LET* ((ARG5 (%%VALUE ARGUMENTS)))
        (CL:SETQ ARGUMENTS (%%REST ARGUMENTS))
        (CL:WHEN (CL:EQ ARGUMENTS NIL)
         (CL:RETURN-FROM APPLY (CL:FUNCALL CODE ARG1 ARG2 ARG3 ARG4 ARG5)))
         ...
         (CL:LET* ((ARG7 (%%VALUE ARGUMENTS)))
          ...
          (CL:LET* ((ARG8 (%%VALUE ARGUMENTS)))
           ...
           (CL:LET* ((ARG9 (%%VALUE ARGUMENTS)))
            ...
            (CL:WHEN (CL:EQ ARGUMENTS NIL)
             (CL:RETURN-FROM APPLY
              (CL:FUNCALL CODE ARG1 ARG2 ARG3 ARG4 ARG5 ARG6 ARG7 ARG8 ARG9)))
...
(CL:DEFUN DEFINE-FUNCTION-OBJECT-EAGERLY? (FUNCTION)
  (CL:LET*
   ((METHODCLASS (%SURROGATE-VALUE SGT-METHODS-STELLA-METHOD-SLOT))
    (SLOT NULL))
   (CL:LET*
    ((SLOTNAME NULL) (VALUE NULL)
     (ITER-000 (%THE-KV-LIST (%DYNAMIC-SLOTS FUNCTION))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
     (CL:SETQ SLOTNAME (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
     (CL:SETQ VALUE VALUE) (CL:SETQ SLOT (LOOKUP-SLOT METHODCLASS SLOTNAME))
     (CL:WHEN
      ...
      (CL:LET*
       ...
       (CL:IF FOUNDRELATIVISTICTYPE?
        (CL:LET* ((NEWTS (NEW-PARAMETRIC-TYPE-SPECIFIER)))
         ...
         (CL:SETF (%SPECIFIER-DIMENSIONS NEWTS)
          (%SPECIFIER-DIMENSIONS RELATIVETYPE))
          ...
          (CL:WHEN (CL:NOT (RUN-OPTION-HANDLER? GLOBAL KEY VALUE))
           (CL:LET* ((*PRINTREADABLY?* CL:T))
            ...
            (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
             (PRINT-ERROR-CONTEXT ">> WARNING: " STANDARD-WARNING)
             (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-WARNING) EOL
              " Skipping invalid variable option `" (DE-UGLIFY-PARSE-TREE KEY)
              "'" EOL "in the definition of variable `"
              (%VARIABLE-NAME GLOBAL) "'." EOL)))))))
