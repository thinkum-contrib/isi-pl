...
(CL:DECLAIM
  ...
  *TRANSLATIONUNITS* *CURRENT-STELLA-FEATURES* EOL
  *TRANSLATOROUTPUTLANGUAGE* TRUE NIL STANDARD-OUTPUT NULL-INTEGER))
   ...
   (CL:LET*
    ...
    (CL:COND
     ((CL:AND DECLARATION? (CONS? TRANSLATION)
      ...
      (CL:LET*
       ((I NULL-INTEGER) (ELT NULL) (ITER-000 (%%REST TRANSLATION))
        (ITER-001 1))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
      ...
      (CL:WHEN
       ...
       (CL-OUTPUT-ONE-UNIT UNIT NATIVEOUTPUTSTREAM)))))
   (CL:LET*
    ((UNIT NULL) (ITER-002 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
...
`relative?', concatenate root directory to `file'."
  (CL:WHEN RELATIVE?
   (CL:SETQ FILE
    (MAKE-FILE-NAME FILE KWD-CL-TRANSLATE-FILE-STELLA TRUE)))
...
(CL:DEFUN SORT-CL-STRUCT-CLASSES (UNSORTEDCLASSES)
  (CL:LET*
       ...
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN (CL:SETQ COLLECT-000 (CONS CLASS NIL))
         (CL:IF (CL:EQ (%THE-CONS-LIST ROOTS) NIL)
          ...
          (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST ROOTS)
           COLLECT-000)))
   ...
   (CL:LOOP WHILE (NON-EMPTY? ROOTS) DO
    ...
    (CL:LET*
     ((SUB NULL)
      (ITER-003 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS (POP ROOTS)))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
