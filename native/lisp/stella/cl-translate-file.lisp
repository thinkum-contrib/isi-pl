...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* FALSE-WRAPPER TRUE-WRAPPER *CURRENTFILE* *CURRENTSYSTEMDEFINITION* *MODULE*
  *TRANSLATIONVERBOSITYLEVEL* *TRANSLATIONUNITS* *CURRENT-STELLA-FEATURES* EOL *TRANSLATOROUTPUTLANGUAGE* NULL-INTEGER
  NIL STANDARD-OUTPUT))
...
(CL:DEFUN HELP-LPTRANS (STATEMENT)
  (CL:LET* ((*TRANSLATOROUTPUTLANGUAGE* *TRANSLATOROUTPUTLANGUAGE*) (*PRINTPRETTYCODE?* CL:T))
   ...
   (CL:LET* ((DECLARATION? (DECLARATION-TREE? STATEMENT)) (TRANSLATION (INCREMENTALLY-TRANSLATE STATEMENT)))
    (CL:COND
     ((CL:AND DECLARATION? (CONS? TRANSLATION) (CL:EQ (%%VALUE TRANSLATION) (INTERN-COMMON-LISP-SYMBOL "PROGN")))
      (CL:LET* ((ELT NULL) (ITER-000 (%%REST TRANSLATION)) (I NULL-INTEGER) (ITER-001 1))
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-000)) (CL:SETQ I ITER-001)
        (CL:IF (CL:= I 1) (PRINT-STELLA-DEFINITION ELT (%NATIVE-STREAM STANDARD-OUTPUT))
...
(CL:DEFUN AUXILIARY-VARIABLE-UNIT? (UNIT)
  (CL:RETURN-FROM AUXILIARY-VARIABLE-UNIT?
   (CL:AND (%AUXILIARY? UNIT) (CL:EQ (%CATEGORY UNIT) SYM-CL-TRANSLATE-FILE-STELLA-GLOBAL-VARIABLE))))
...
(CL:DEFUN CL-OUTPUT-ONE-UNIT (UNIT STREAM)
  (CL:LET* ((TRANSLATION (%TRANSLATION UNIT)))
   (CL:WHEN (CL:EQ TRANSLATION NULL)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "`" UNIT "' has a NULL translation." EOL)
    (CL:RETURN-FROM CL-OUTPUT-ONE-UNIT))
   (CL:WHEN (CL:NOT (CL:EQ (%CATEGORY UNIT) NULL))
    (CL:WHEN (CL:NOT (AUXILIARY-VARIABLE-UNIT? UNIT)) (%%PRINT-STREAM STREAM EOL))
    ...
    (CL:IF
     ...
     (CL:LET* ((FORM NULL) (ITER-000 (%%REST TRANSLATION)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ FORM (%%VALUE ITER-000))
       (PRINT-STELLA-DEFINITION FORM STREAM) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN CL-OUTPUT-ALL-UNITS-TO-FILE (SOURCEFILE)
  (CL:LET*
   ((OUTPUTFILE (MAKE-FILE-NAME SOURCEFILE KWD-CL-TRANSLATE-FILE-LISP CL:NIL))
    (OUTPUTSTREAM (NEW-OUTPUT-FILE-STREAM OUTPUTFILE)) (NATIVEOUTPUTSTREAM (%NATIVE-STREAM OUTPUTSTREAM)))
   (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Writing `" OUTPUTFILE "'..." EOL))
   ...
   (CL:LET* ((UNIT NULL) (ITER-000 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-000))
     (CL:WHEN (AUXILIARY-VARIABLE-UNIT? UNIT) (CL-OUTPUT-ONE-UNIT UNIT NATIVEOUTPUTSTREAM))
   ...
   (CL:LET* ((UNIT NULL) (ITER-002 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-002))
     (CL:WHEN (CL:NOT (AUXILIARY-VARIABLE-UNIT? UNIT)) (CL-OUTPUT-ONE-UNIT UNIT NATIVEOUTPUTSTREAM))
...
(CL:DEFUN CL-OUTPUT-FILE-HEADER (STREAM FILE ENSUREPACKAGE?)
  (CL:LET* ((PACKAGE (LISP-PACKAGE *MODULE*))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING PACKAGE))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM) ";;; -*- Mode: Lisp; Package: " PACKAGE
   ...
   (CL:LET* ((*TRANSLATOROUTPUTLANGUAGE* KWD-CL-TRANSLATE-FILE-COMMON-LISP))
    (CL:DECLARE (CL:SPECIAL *TRANSLATOROUTPUTLANGUAGE*)) (OUTPUT-COPYRIGHT-HEADER STREAM))
   (CL:WHEN (CL:NOT (CL:OR (STRING-EQL? PACKAGE "STELLA") (CL:NOT ENSUREPACKAGE?)))
    (CL-OUTPUT-ENSURE-PACKAGE-DEFINITION STREAM PACKAGE) (%%PRINT-STREAM (%NATIVE-STREAM STREAM) EOL))
...
(CL:DEFUN CL-OUTPUT-ENSURE-PACKAGE-DEFINITION (STREAM PACKAGE)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "(CL:EVAL-WHEN (:COMPILE-TOPLEVEL :LOAD-TOPLEVEL :EXECUTE)" EOL
   "  (CL:UNLESS (CL:FIND-PACKAGE \"" PACKAGE "\")" EOL "     (CL:DEFPACKAGE \"" PACKAGE "\" (:USE))))" EOL)
  :VOID)
...
(CL:DEFUN CL-OUTPUT-FORWARD-DECLARATIONS (STREAM)
  (CL:LET* ((DEFINEDVARIABLES (NEW-LIST)) (FORWARDREFERENCEDVARIABLES (NEW-LIST)))
   (CL:LET* ((UNIT NULL) (ITER-000 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-000))
     (CL:WHEN (AUXILIARY-VARIABLE-UNIT? UNIT) (PUSH DEFINEDVARIABLES (%THE-OBJECT UNIT)))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((UNIT NULL) (ITER-001 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-001))
     (CL:WHEN (CL:NOT (AUXILIARY-VARIABLE-UNIT? UNIT))
      (CL:WHEN (CL:EQ (%CATEGORY UNIT) SYM-CL-TRANSLATE-FILE-STELLA-GLOBAL-VARIABLE)
      ...
      (CL:LET* ((VARIABLE NULL) (ITER-002 (%THE-CONS-LIST (%REFERENCED-GLOBALS UNIT))))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ VARIABLE (%%VALUE ITER-002))
        (CL:WHEN
         (CL:AND (CL:NOT (MEMBER? DEFINEDVARIABLES VARIABLE)) (CL:NOT (MEMBER? FORWARDREFERENCEDVARIABLES VARIABLE)))
         (PUSH FORWARDREFERENCEDVARIABLES VARIABLE))
        (CL:SETQ ITER-002 (%%REST ITER-002)))))
     (CL:SETQ ITER-001 (%%REST ITER-001))))
   (CL:LET* ((IT (ALLOCATE-ITERATOR FORWARDREFERENCEDVARIABLES)))
    (CL:LOOP WHILE (NEXT? IT) DO (VALUE-SETTER IT (%VARIABLE-NAME (%VALUE IT)))))
    ...
    (PRINT-STELLA-DEFINITION
     (LIST* (INTERN-COMMON-LISP-SYMBOL "DECLAIM")
      (CONS (INTERN-COMMON-LISP-SYMBOL "SPECIAL")
       (CONCATENATE (CL-TRANSLATE-LIST-OF-TREES (%THE-CONS-LIST FORWARDREFERENCEDVARIABLES)) NIL))
      NIL)
     STREAM))
...
(CL:DEFUN CL-TRANSLATE-WALKED-SYSTEM-UNITS (SYSTEMUNITS)
  (CL:LET* ((*CURRENTFILE* STELLA::NULL-STRING) (*TRANSLATIONUNITS* NULL))
   ...
   (CL:LET*
    ((CLASSUNITS (NEW-LIST))
     (USECLSTRUCTS? (MEMB? *CURRENT-STELLA-FEATURES* KWD-CL-TRANSLATE-FILE-USE-COMMON-LISP-STRUCTS))
      ...
      (CL:COND
       (USECLSTRUCTS? (CL:SETQ *TRANSLATIONUNITS* (NEW-LIST))
        (CL:LET* ((UNIT NULL) (ITER-001 (%THE-CONS-LIST UNITS)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-001))
          (CL:IF (CL:EQ (%CATEGORY UNIT) SYM-CL-TRANSLATE-FILE-STELLA-CLASS) (PUSH CLASSUNITS UNIT)
    ...
    (CL:WHEN USECLSTRUCTS?
     (CL:WHEN (CL:>= *TRANSLATIONVERBOSITYLEVEL* 1)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Collecting CL structure class definitions..." EOL))
     (CL:LET*
      ((FILE NULL) (ITER-002 (FILES-PLUS-SYSTEM-STARTUP (SYSTEM-DEFINITION-SOURCE-FILES *CURRENTSYSTEMDEFINITION*))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ FILE (%%VALUE ITER-002))
       (CL:WHEN
        (CL:EQ
         (LOOKUP SYSTEMUNITS
          (WRAP-STRING (CL:SETQ FILENAME (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-CL-TRANSLATE-FILE-STELLA CL:T))))
         NULL)
        (CL:SETQ *TRANSLATIONUNITS* (NEW-LIST)) (CL:SETQ *CURRENTFILE* (FILE-BASE-NAME FILENAME))
        (CL:SETQ *TRANSLATIONUNITS* (NEW-LIST)) (OPERATE-ON-FILE FILENAME KWD-CL-TRANSLATE-FILE-FINALIZE)
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-003))
          (CL:WHEN
           (CL:AND (CL:EQ (%CATEGORY UNIT) SYM-CL-TRANSLATE-FILE-STELLA-CLASS)
            (CREATE-NATIVE-CLASS? (%THE-OBJECT UNIT)) (CL:NOT (%MIXIN? (%THE-OBJECT UNIT))))
  ...
  (CL:LET* ((CLASSTABLE (NEW-HASH-TABLE)) (ROOTS (LIST)) (SORTEDCLASSES (LIST)))
   (CL:LET* ((CLASS NULL) (ITER-000 (%THE-CONS-LIST UNSORTEDCLASSES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ CLASS (%%VALUE ITER-000))
     (INSERT-AT CLASSTABLE (%CLASS-TYPE CLASS) TRUE-WRAPPER) (CL:SETQ ITER-000 (%%REST ITER-000))))
      ...
      (CL:LET* ((FOUND?-000 CL:NIL))
       (CL:LET* ((SUPER NULL) (ITER-002 (%THE-CONS-LIST (%CLASS-DIRECT-SUPERS CLASS))))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ SUPER (%%VALUE ITER-002))
         (CL:WHEN (CL:NOT (CL:EQ (LOOKUP CLASSTABLE SUPER) NULL)) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
         (CL:SETQ ITER-002 (%%REST ITER-002))))
      ...
      (CL:WHEN TEST-VALUE-000
       (CL:IF (CL:EQ COLLECT-000 NULL)
        (CL:PROGN (CL:SETQ COLLECT-000 (CONS CLASS NIL))
         (CL:IF (CL:EQ (%THE-CONS-LIST ROOTS) NIL) (CL:SETF (%THE-CONS-LIST ROOTS) COLLECT-000)
   ...
   (CL:LOOP WHILE (NON-EMPTY? ROOTS) DO (PUSH SORTEDCLASSES (FIRST ROOTS))
    (CL:LET* ((SUB NULL) (ITER-003 (%THE-CONS-LIST (%CLASS-DIRECT-SUBS (POP ROOTS)))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO (CL:SETQ SUB (%%VALUE ITER-003))
      (CL:WHEN (CL:NOT (CL:EQ (LOOKUP CLASSTABLE SUB) NULL)) (PUSH ROOTS (%SURROGATE-VALUE SUB)))
      (CL:SETQ ITER-003 (%%REST ITER-003)))))
...
(CL:DEFUN CL-OUTPUT-SYSTEM-CL-STRUCTS-FILE (CLASSUNITS)
  (CL:LET*
   ((OUTPUTFILE
     (MAKE-FILE-NAME (CL-YIELD-STRUCT-CLASS-FILE-NAME (%NAME *CURRENTSYSTEMDEFINITION*)) KWD-CL-TRANSLATE-FILE-LISP
      CL:T))
    (OUTPUTSTREAM (NEW-OUTPUT-FILE-STREAM OUTPUTFILE)) (CLASSES (NEW-LIST)) (CURRENTMODULE *MODULE*)
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS (%THE-OBJECT UNIT) NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST CLASSES) NIL) (CL:SETF (%THE-CONS-LIST CLASSES) COLLECT-000)
   ...
   (CL:LET* ((*MODULE* *MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    ...
    (CL:LET* ((UNIT NULL) (ITER-001 (%THE-CONS-LIST CLASSUNITS)) (CLASS NULL) (ITER-002 (%THE-CONS-LIST CLASSES)))
     (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-001 NIL)) (CL:NOT (CL:EQ ITER-002 NIL))) DO
      (CL:SETQ UNIT (%%VALUE ITER-001)) (CL:SETQ CLASS (%%VALUE ITER-002)) (CL:SETF (%THE-OBJECT UNIT) CLASS)
      (CL:SETF (%ANNOTATION UNIT) STELLA::NULL-STRING) (CL:SETQ *MODULE* (HOME-MODULE CLASS))
      (CL:WHEN (CL:NOT (CL:EQ *MODULE* CURRENTMODULE)) (CL:SETQ CURRENTMODULE *MODULE*)
       (CL:SETQ PACKAGE (LISP-PACKAGE *MODULE*))
       (CL:WHEN (CL:NOT (MEMBER? ENSUREDPACKAGES (WRAP-STRING PACKAGE))) (PUSH ENSUREDPACKAGES (WRAP-STRING PACKAGE))
        (%%PRINT-STREAM (%NATIVE-STREAM OUTPUTSTREAM) EOL) (CL-OUTPUT-ENSURE-PACKAGE-DEFINITION OUTPUTSTREAM PACKAGE))
       (%%PRINT-STREAM (%NATIVE-STREAM OUTPUTSTREAM) EOL) (CL-OUTPUT-IN-PACKAGE-DECLARATION OUTPUTSTREAM PACKAGE))
      (CL:SETF (%TRANSLATION UNIT) (CL-TRANSLATE-DEFINE-NATIVE-CLASS-UNIT UNIT))
      (CL-OUTPUT-ONE-UNIT UNIT (%NATIVE-STREAM OUTPUTSTREAM)) (CL:SETQ ITER-001 (%%REST ITER-001))
      (CL:SETQ ITER-002 (%%REST ITER-002)))))
   (FREE OUTPUTSTREAM))
  :VOID)
...
(CL:DEFUN CL-TRANSLATE-SYSTEM (SYSTEM-NAME)
  ...
  (%TRANSLATE-SYSTEM SYSTEM-NAME
   (CONS-LIST KWD-CL-TRANSLATE-FILE-COMMON-LISP KWD-CL-TRANSLATE-FILE-TWO-PASS? TRUE-WRAPPER
    KWD-CL-TRANSLATE-FILE-FORCE-TRANSLATION? TRUE-WRAPPER KWD-CL-TRANSLATE-FILE-PRODUCTION-SETTINGS? TRUE-WRAPPER))
  :VOID)
...
(CL:DEFUN CL-COMPILE-AND-LOAD-FILES (FILES RELATIVE? FORCERECOMPILATION?)
  (CL:LET*
   ((LISPKEY KWD-CL-TRANSLATE-FILE-LISP) (BINARYKEY KWD-CL-TRANSLATE-FILE-LISP-BINARY) (TRANSLATEDFILE? CL:NIL))
...
(CL:DEFUN CL-TRANSLATE-STELLA (PRODUCTIONSETTINGS?)
  (%TRANSLATE-SYSTEM "stella"
   (CONS-LIST KWD-CL-TRANSLATE-FILE-COMMON-LISP KWD-CL-TRANSLATE-FILE-FORCE-TRANSLATION? TRUE-WRAPPER
...
(CL:DEFUN CL-RETRANSLATE-STELLA (PRODUCTIONSETTINGS?)
  (%TRANSLATE-SYSTEM "stella"
   (CONS-LIST KWD-CL-TRANSLATE-FILE-COMMON-LISP KWD-CL-TRANSLATE-FILE-TWO-PASS? TRUE-WRAPPER
    KWD-CL-TRANSLATE-FILE-FORCE-TRANSLATION? TRUE-WRAPPER KWD-CL-TRANSLATE-FILE-PRODUCTION-SETTINGS?
...
(CL:DEFUN STARTUP-CL-TRANSLATE-FILE ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SYM-CL-TRANSLATE-FILE-STELLA-LPTRANS (INTERN-RIGID-SYMBOL-WRT-MODULE "LPTRANS" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "AUXILIARY-VARIABLE-UNIT?"
     "(DEFUN (AUXILIARY-VARIABLE-UNIT? BOOLEAN) ((UNIT TRANSLATION-UNIT)))" (CL:FUNCTION AUXILIARY-VARIABLE-UNIT?)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "CL-OUTPUT-ALL-UNITS-TO-FILE"
     "(DEFUN CL-OUTPUT-ALL-UNITS-TO-FILE ((SOURCEFILE FILE-NAME)))" (CL:FUNCTION CL-OUTPUT-ALL-UNITS-TO-FILE) NULL)
...
`relative?', concatenate root directory to `file'.\" :PUBLIC? TRUE)" (CL:FUNCTION CL-TRANSLATE-FILE) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "CL-TRANSLATE-STELLA" "(DEFUN CL-TRANSLATE-STELLA ((PRODUCTIONSETTINGS? BOOLEAN)))"
     (CL:FUNCTION CL-TRANSLATE-STELLA) NULL)
    (DEFINE-FUNCTION-OBJECT "CL-RETRANSLATE-STELLA" "(DEFUN CL-RETRANSLATE-STELLA ((PRODUCTIONSETTINGS? BOOLEAN)))"
     (CL:FUNCTION CL-RETRANSLATE-STELLA) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CL-TRANSLATE-FILE" "(DEFUN STARTUP-CL-TRANSLATE-FILE () :PUBLIC? TRUE)"
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CL-TRANSLATE-FILE-STELLA-STARTUP-CL-TRANSLATE-FILE)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CL-TRANSLATE-FILE-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "STELLA")))
    (REGISTER-NATIVE-NAME SYM-CL-TRANSLATE-FILE-STELLA-LPTRANS KWD-CL-TRANSLATE-FILE-COMMON-LISP
     KWD-CL-TRANSLATE-FILE-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-CL-TRANSLATE-FILE-STELLA-PTRANS KWD-CL-TRANSLATE-FILE-COMMON-LISP
     KWD-CL-TRANSLATE-FILE-FUNCTION)))
  :VOID)
