...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-STRING-WRAPPER FALSE-WRAPPER
  TRUE-WRAPPER ELIPSIS TRUE NULL-INTEGER FALSE STANDARD-OUTPUT EOL NIL))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE TYPEREF)))
     (CL:COND
      ...
      ((SUBTYPE-OF-STRING? TEST-VALUE-000)
       (CL:PROGN
        (CL:SETQ CLASSTYPE
         (LOOKUP-SURROGATE (%WRAPPER-VALUE TYPEREF)))))
     ...
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-001)
      (CL:PROGN
       (CL:RETURN-FROM SEARCH-FOR-OBJECT
        (SEARCH-FOR-OBJECT (WRAP-STRING (%SYMBOL-NAME SELF))
         CLASSTYPE))))
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          ...
          (CL:COND
           ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE FN)
            ...
            (CL:PROGN
             (CL:SETQ OBJECT
              (CL:FUNCALL (%WRAPPER-VALUE FN) INTEGER))))
           (CL:T
            (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
             (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`"
              (SAFE-PRIMARY-TYPE FN) "' is not a valid case option")
     ...
     (CL:PROGN
      (CL:LET* ((CONS (ALLOCATE-ITERATOR TREE)))
       (CL:LOOP WHILE (NEXT? CONS) DO
        (VALUE-SETTER CONS
         (CONSIFY-LISTS-AND-ITERATORS (%VALUE CONS)))))
      (CL:RETURN-FROM CONSIFY-LISTS-AND-ITERATORS TREE)))
...
(CL:DEFMETHOD PRINT-DEFINITION ((SELF MODULE) STREAM)
  (CL:LET*
   ((OPTIONS
     (READ-S-EXPRESSION-FROM-STRING (%STRINGIFIED-OPTIONS SELF))))
...
(CL:DEFUN PRINT-DOCUMENTATION-STRING (STRING INDENT STREAM)
  ...
  (CL:WHEN (NULL? INDENT) (CL:SETQ INDENT 0))
  (CL:LET*
