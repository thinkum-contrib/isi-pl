...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-STRING-WRAPPER FALSE-WRAPPER
  TRUE-WRAPPER ELIPSIS NULL-INTEGER STANDARD-OUTPUT EOL NIL))
  ...
  (CL:LET* ((OBJECT NULL))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO
     (CL:SETQ TYPE (%KEY ITER-000)) (CL:SETQ FN (%VALUE ITER-000))
    ...
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ CLASSTYPE NULL))
      (CL:OR (CL:EQ (%SURROGATE-VALUE CLASSTYPE) NULL)
       (CL:NOT
        (ISA? (%SURROGATE-VALUE CLASSTYPE) SGT-DESCRIBE-STELLA-CLASS))))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          ...
          (CL:COND
           ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE FN)
             SGT-DESCRIBE-STELLA-FUNCTION-CODE-WRAPPER)
            (CL:PROGN
             (CL:SETQ OBJECT (CL:FUNCALL (%WRAPPER-VALUE FN) INTEGER))))
           (CL:T
            (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
             (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`"
              (SAFE-PRIMARY-TYPE FN) "' is not a valid case option")
  ...
  (CL:IF (CL:= (%ITERATOR-INTEGER ITERATOR) 1)
   (CL:RETURN-FROM FILTER-STRING-TO-CLASS?
    (STRING-EQL? (CLASS-NAME SELF)
     (%WRAPPER-VALUE (%ITERATOR-OBJECT ITERATOR))))
   (CL:RETURN-FROM FILTER-STRING-TO-CLASS?
    (CL:NOT
     (CL:=
...
(CL:DEFUN YIELD-EXPLODED-METHOD-NAME (DOTTEDSTRING)
  ...
  (CL:LET*
   ((DOTPOSITION (POSITION DOTTEDSTRING #\. 0))
    (FIRSTSTRING STELLA::NULL-STRING) (SECONDSTRING STELLA::NULL-STRING))
...
(CL:DEFUN STRING-TO-SLOT (STRING)
  ...
  (CL:IF (DOTTED-STRING? STRING)
   (CL:LET*
    ((CLASSSTRING STELLA::NULL-STRING) (SLOTSTRING STELLA::NULL-STRING))
   ...
   (CL:COND
    ...
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-DESCRIBE-STELLA-LIST)
     (CL:PROGN
      (CL:LET* ((CONSLIST NIL))
       (CL:LET*
        ((ITEM NULL) (ITER-000 (%THE-CONS-LIST TREE)) (I NULL-INTEGER)
        ...
        (CL:LOOP WHILE
         ...
         (CL:IF (CL:EQ COLLECT-000 NULL)
          (CL:PROGN
           (CL:SETQ COLLECT-000
            (CONS (CL:IF (CL:= I *PRETTY-PRINT-LIST-CUTOFF*) ELIPSIS ITEM)
             NIL))
      ...
      (CL:LET* ((CONSLIST NIL))
       (CL:LET*
        ...
        (CL:LOOP WHILE
         ...
         DO (CL:SETQ ITEM (%VALUE ITER-002)) (CL:SETQ I ITER-003)
    ...
    ((SLOT NULL)
     (ITER-000
      (PRIVATE-CLASS-STORAGE-SLOTS
       (%SURROGATE-VALUE SGT-DESCRIBE-STELLA-CLASS)))
     (COLLECT-000 NULL))
...
(CL:DEFUN LITERAL-EQL? (X Y)
  (CL:WHEN
   ...
   (CL:RETURN-FROM LITERAL-EQL?
    (CL:OR
     (CL:AND (CL:EQ X NULL) (CL:NOT (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN Y)))
...
(CL:DEFUN PRETTY-PRINT-METHOD-PARAMETERS (STREAM METHOD)
  (CL:LET*
   ...
   (CL:LET* ((*PRINTREADABLY?* CL:T))
    ...
    (CL:LET*
     ((PARAMETER NULL)
      (ITER-000 (%THE-CONS-LIST (METHOD-PARAMETER-NAMES METHOD))) (TSPEC NULL)
    ...
    (PRETTY-PRINT-DEFINITION-TREE
     (LIST* SYM-DESCRIBE-STELLA-SLOT (%SLOT-NAME SELF)
      (LIST* KWD-DESCRIBE-OWNER
       (WRAP-STRING (%SYMBOL-NAME (%SLOT-OWNER SELF)))
        ...
        (LIST* KWD-DESCRIBE-SHADOWS (SHADOWED-SURROGATES SELF)
         (LIST* SYM-DESCRIBE-STELLA->>_INFERRED_SLOTS KWD-DESCRIBE-CHILDREN
          ...
          (LIST* KWD-DESCRIBE-USED-BY (%USED-BY SELF)
           (LIST* KWD-DESCRIBE-CARDINAL-MODULE (%CARDINAL-MODULE SELF)
            NIL)))))))
     STREAM))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
...
(CL:DEFUN SHORT-DOCUMENTATION (DOCUMENTATION)
  ...
  (CL:LET* ((LINEEND NULL-INTEGER) (SENTENCEEND NULL-INTEGER))
   ...
   (CL:IF
    (CL:AND (CL:NOT (CL:= SENTENCEEND NULL-INTEGER)) (CL:<= SENTENCEEND 155))
    (CL:RETURN-FROM SHORT-DOCUMENTATION
