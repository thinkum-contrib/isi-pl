...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-STRING-WRAPPER NULL-CHARACTER
  FALSE-WRAPPER TRUE-WRAPPER ELIPSIS STANDARD-OUTPUT EOL))
     ...
     (CL:WHEN (CL:OR (CL:EQ CLASSTYPE NULL) (SUBTYPE-OF? TYPE CLASSTYPE))
      (CL:COND
       ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE FN)
        ...
        (CL:PROGN
         (CL:SETQ OBJECT (CL:FUNCALL (%WRAPPER-VALUE FN) STRING))))
       (CL:T
     ...
     ((SUBTYPE-OF-INTEGER? TEST-VALUE-001)
      (CL:PROGN
       (CL:LET* ((INTEGER (%WRAPPER-VALUE SELF)) (OBJECT NULL))
        ...
        (CL:LET*
         ((FN NULL)
          (ITER-000 (%THE-CONS-LIST *INTEGER-TO-OBJECT-FUNCTIONS*)))
...
(CL:DEFUN STRING-TO-SLOT (STRING)
  ...
  (CL:IF (DOTTED-STRING? STRING)
   (CL:LET*
    ...
    (CL:LET*
     ((CLASS (GET-STELLA-CLASS CLASSSTRING FALSE)) (SLOTNAME NULL))
     (CL:WHEN (CL:NOT (CL:EQ CLASS NULL))
      (CL:SETQ SLOTNAME (LOOKUP-SYMBOL SLOTSTRING))
      (CL:WHEN (CL:NOT (CL:EQ SLOTNAME NULL))
       (CL:RETURN-FROM STRING-TO-SLOT
        (SAFE-LOOKUP-SLOT CLASS SLOTNAME))))))
...
(CL:PROGN (CL:DEFVAR *PRETTY-PRINT-LIST-CUTOFF* 5 "Lists longer than the cutoff are truncated during
pretty printing.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *PRETTY-PRINT-LIST-CUTOFF*)))
  ...
  (CL:COND
   ((CL:EQ MODE KWD-DESCRIBE-SOURCE)
    (PRETTY-PRINT-DEFINITION-TREE
     (READ-S-EXPRESSION-FROM-STRING (%CLASS-STRINGIFIED-SOURCE SELF))
     STREAM))
       ...
       (LIST* KWD-DESCRIBE-USES (%USES SELF)
        (LIST* KWD-DESCRIBE-SHADOWS (SHADOWED-SURROGATES SELF)
         (LIST* SYM-DESCRIBE-STELLA->>_INFERRED_SLOTS
          KWD-DESCRIBE-CHILDREN (%CHILD-CONTEXTS SELF)
...
to the standard print function.\")" (CL:FUNCTION DESCRIBE-OBJECT) NULL)
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Describe") NULL-STRING-WRAPPER)))
  :VOID)
