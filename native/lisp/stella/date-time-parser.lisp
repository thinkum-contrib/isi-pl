...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-CHARACTER *CHARACTER-TYPE-TABLE* TRUE
  TRUE-WRAPPER FALSE-WRAPPER NULL-FLOAT NULL-INTEGER NIL FALSE))
...
(CL:DEFVAR *ERROR-ON-MISMATCH* FALSE
...
(CL:DEFUN HASHLIST (LIST TABLE)
  (CL:RETURN-FROM HASHLIST
   (LIST* SYM-DATE-TIME-PARSER-STELLA-FOREACH SYM-DATE-TIME-PARSER-STELLA-ITEM
    SYM-DATE-TIME-PARSER-STELLA-IN
    (LIST* SYM-DATE-TIME-PARSER-STELLA-CAST LIST
     (CONS
      (LIST* SYM-DATE-TIME-PARSER-STELLA-CONS SYM-DATE-TIME-PARSER-STELLA-OF
       SYM-DATE-TIME-PARSER-STELLA-CONS NIL)
      NIL))
    SYM-DATE-TIME-PARSER-STELLA-DO
    (LIST* SYM-DATE-TIME-PARSER-STELLA-INSERT-AT TABLE
     (LIST*
      (LIST* SYM-DATE-TIME-PARSER-STELLA-FIRST SYM-DATE-TIME-PARSER-STELLA-ITEM
       NIL)
      (LIST* SYM-DATE-TIME-PARSER-STELLA-SECOND SYM-DATE-TIME-PARSER-STELLA-ITEM
       NIL)
      NIL))
    NIL)))
...
(CL:DEFUN YESTERDAY (PARSED-VALUES)
  (TODAY PARSED-VALUES)
  (CL:LET*
   ((JD
     (COMPUTE-JULIAN-DAY (%DATE-TIME-YEAR PARSED-VALUES)
      (%DATE-TIME-MONTH PARSED-VALUES) (%DATE-TIME-DAY PARSED-VALUES))))
...
(CL:DEFUN TOMORROW (PARSED-VALUES)
  (TODAY PARSED-VALUES)
  (CL:LET*
   ((JD
     (COMPUTE-JULIAN-DAY (%DATE-TIME-YEAR PARSED-VALUES)
...
(CL:DEFUN WEEKDAY (THING)
  (CL:COND
   ((SUBTYPE-OF-STRING? (SAFE-PRIMARY-TYPE THING))
    (CL:PROGN
     (CL:RETURN-FROM WEEKDAY (LOOKUP *WEEKDAY-STRINGS* (%WRAPPER-VALUE THING)))))
    ...
    ((SUBTYPE-OF-STRING? TEST-VALUE-000)
     (CL:PROGN
      ...
      (CL:LET* ((ZONE-NUMBER (LOOKUP *ZONE-STRINGS* (%WRAPPER-VALUE THING))))
       (CL:IF (CL:NOT (CL:EQ ZONE-NUMBER NULL)) (CL:RETURN-FROM ZONE ZONE-NUMBER)
   ...
   ((SUBTYPE-OF-CHARACTER? (SAFE-PRIMARY-TYPE THECHARACTER))
    (CL:PROGN
     (CL:RETURN-FROM TIME-DIVIDER
      (CL:IF (MEMBER? TIME-DIVIDERS THECHARACTER) TRUE-WRAPPER FALSE-WRAPPER))))
   ...
   ((SUBTYPE-OF-CHARACTER? (SAFE-PRIMARY-TYPE THECHARACTER))
    (CL:PROGN
     (CL:RETURN-FROM DATE-DIVIDER
      (CL:IF (MEMBER? DATE-DIVIDERS THECHARACTER) TRUE-WRAPPER FALSE-WRAPPER))))
  ...
  (CL:LET* ((TESTSTRING (STRING-DOWNCASE SUBSTRING)) (TEST-VALUE NULL))
   ...
   (CL:WHEN
    ...
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-DATE-TIME-DIVIDER TEST-VALUE)))
   (CL:WHEN
    (CL:NOT (CL:EQ (CL:SETQ TEST-VALUE (MONTH (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-MONTH TEST-VALUE)))
   (CL:WHEN
    (CL:NOT (CL:EQ (CL:SETQ TEST-VALUE (WEEKDAY (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-WEEKDAY TEST-VALUE)))
   (CL:WHEN
    (CL:NOT (CL:EQ (CL:SETQ TEST-VALUE (AM-PM (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-AM-PM TEST-VALUE)))
   (CL:WHEN
    (CL:NOT
     (CL:EQ (CL:SETQ TEST-VALUE (NOON-MIDN (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-NOON-MIDN TEST-VALUE)))
   (CL:WHEN
    (CL:NOT (CL:EQ (CL:SETQ TEST-VALUE (ZONE (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-ZONE TEST-VALUE)))
   (CL:WHEN
    (CL:NOT
     (CL:EQ (CL:SETQ TEST-VALUE (SPECIALP (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
...
(CL:DEFUN DECOMPOSE-DATE-TIME-STRING (STRING START END)
  ...
  (CL:LET*
   ((STRING-INDEX START) (NEXT-NEGATIVE? FALSE) (PARSING-TIME? FALSE)
    (PARTS-LIST NIL) (NEXT-CHAR NULL-CHARACTER) (PREV-CHAR NULL-CHARACTER))
   (CL:DECLARE (CL:TYPE CL:FIXNUM STRING-INDEX))
   (CL:WHEN (NULL? END) (CL:SETQ END (CL:THE CL:FIXNUM (CL:LENGTH STRING))))
   (CL:LOOP WHILE (CL:< STRING-INDEX END) DO
    ...
    (CL:COND
     ((CL:EQ
      ...
      (CL:SETQ PARSING-TIME? FALSE)
      (CL:LET* ((SCAN-INDEX NULL-INTEGER) (ITER-000 (CL:1+ STRING-INDEX)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-000 SCAN-INDEX))
       (CL:LOOP WHILE TRUE DO
        (CL:PROGN (CL:SETQ SCAN-INDEX ITER-000)
        ...
        (CL:WHEN
         ...
         (CL:LET*
          ((MATCH-SYMBOL
            (MATCH-SUBSTRING (SUBSEQUENCE STRING STRING-INDEX SCAN-INDEX)
             PARSING-TIME?)))
     ...
     ((CL:EQ
      ...
      (CL:LET*
       ((NUMERIC-VALUE (DIGIT-VALUE NEXT-CHAR)) (FRACTIONAL-VALUE 0)
        (FRACTIONAL-DIVISOR 1.0d0) (IN-FRACTION? FALSE))
       ...
       (CL:LET* ((SCAN-INDEX NULL-INTEGER) (ITER-001 (CL:1+ STRING-INDEX)))
        (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 SCAN-INDEX))
        (CL:LOOP WHILE TRUE DO
         (CL:PROGN (CL:SETQ SCAN-INDEX ITER-001)
         ...
         (CL:WHEN
              ...
              (CL:EQL
               (CL:LET ((SELF STRING) (POSITION SCAN-INDEX))
                ...
                (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
                 (CL:THE CL:FIXNUM POSITION)))
               #\.)))))
          (CL:WHEN NEXT-NEGATIVE? (CL:SETQ NEXT-NEGATIVE? FALSE)
          ...
          (CL:IF IN-FRACTION?
           (CL:SETQ PARTS-LIST
            (CONS
             (WRAP-FLOAT
              (CL:+ NUMERIC-VALUE (CL:/ FRACTIONAL-VALUE FRACTIONAL-DIVISOR)))
             PARTS-LIST))
       ...
       (CL:OR (NULL? PREV-CHAR)
        ...
        (CL:AND PARSING-TIME?
         (CL:EQ
          ...
          KWD-DATE-TIME-PARSER-DIGIT))))
      (CL:SETQ NEXT-NEGATIVE? TRUE) (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX)))
     ((MEMBER? TIME-DIVIDERS (WRAP-CHARACTER NEXT-CHAR))
      (CL:SETQ PARTS-LIST
       (CONS
        ...
        PARTS-LIST))
      (CL:SETQ PARSING-TIME? TRUE) (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX)))
      ...
      (CL:SETQ PARTS-LIST
       (CONS
        (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-DATE-DIVIDER
         (WRAP-CHARACTER NEXT-CHAR))
        PARTS-LIST))
      (CL:SETQ PARSING-TIME? FALSE) (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX)))
     ...
     ((MEMBER? WHITESPACE-CHARS (WRAP-CHARACTER NEXT-CHAR))
      (CL:SETQ PARSING-TIME? FALSE) (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX)))
     ((CL:EQL NEXT-CHAR #\+) (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX)))
     ((CL:EQL NEXT-CHAR #\()
      (CL:LOOP WHILE
        ...
        (CL:NOT
         (CL:EQL
          (CL:LET ((SELF STRING) (POSITION STRING-INDEX))
           ...
           (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION)))
          #\))))
       DO (CL:SETQ PARSING-TIME? FALSE)
       (CL:SETQ STRING-INDEX (CL:1+ STRING-INDEX))))
     (CL:T
      (CL:IF *ERROR-ON-MISMATCH*
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
         ">>> Bogus character at position `" STRING-INDEX "' in \"`" STRING
         "'\"")
        (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
       (CL:RETURN-FROM DECOMPOSE-DATE-TIME-STRING NIL)))))
...
(CL:DEFUN MATCH-PATTERN-ELEMENT (PATTERN-ELEMENT DATUM-ELEMENT)
  (CL:COND
   ((ISA? DATUM-ELEMENT SGT-DATE-TIME-PARSER-STELLA-CONS)
    (CL:LET*
     ((DATUM-TYPE
       (CL:IF (CL:EQ (%%VALUE DATUM-ELEMENT) SYM-DATE-TIME-PARSER-STELLA-SPECIAL)
     ...
     (CL:COND
      ...
      (CL:T
       (CL:IF (CL:NOT (CL:EQ MATCH NULL))
        (CL:RETURN-FROM MATCH-PATTERN-ELEMENT (CONS-LIST PATTERN-ELEMENT MATCH))
        (CL:RETURN-FROM MATCH-PATTERN-ELEMENT NIL))))))))
  ...
  (CL:IF (CL:< (LENGTH PATTERN) DATUM-LENGTH) (CL:RETURN-FROM MATCH-PATTERN NIL)
   (CL:LET*
    ((FORM-LIST NIL) (PATTERN-CURSOR PATTERN) (DATUM-CURSOR DATUM)
     (PATTERN-ELEMENT NULL) (DATUM-ELEMENT NULL) (OPTIONAL? FALSE)
     (MATCHING NIL))
    (CL:LOOP WHILE
     (CL:NOT (CL:OR (CL:EQ PATTERN-CURSOR NIL) (CL:EQ DATUM-CURSOR NIL))) DO
     (CL:SETQ PATTERN-ELEMENT (%%VALUE PATTERN-CURSOR))
     ...
     (CL:COND
      ...
      (OPTIONAL? (CL:SETQ DATUM-CURSOR (CONS DATUM-ELEMENT DATUM-CURSOR)))
     ...
     ((CL:EQ DATUM-CURSOR NIL)
      (CL:LET* ((ELEMENT NULL) (ITER-000 PATTERN-CURSOR))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        ...
        (CL:WHEN (CL:NOT (ISA? ELEMENT SGT-DATE-TIME-PARSER-STELLA-CONS))
...
(CL:DEFUN DEAL-WITH-NOON-MIDN (FORM-VALUE PARSED-VALUES)
  (CL:COND
   ...
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Unrecognized symbol: ~A`"
      FORM-VALUE "'")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN SET-TIME-VALUES (STRING-FORM PARSED-VALUES)
  (CL:LET* ((FORM-TYPE NULL) (FORM-VALUE NULL))
   (CL:LET* ((FORM-PART NULL) (ITER-000 STRING-FORM))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ FORM-PART (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     ...
     (CL:LET* ((TEST-VALUE-000 FORM-TYPE))
      (CL:COND
       ...
       ((CL:EQ TEST-VALUE-000 SYM-DATE-TIME-PARSER-STELLA-SPECIAL)
        (CL:FUNCALL (%FUNCTION-CODE (LOOKUP-FUNCTION FORM-VALUE)) PARSED-VALUES))
       (CL:T
        (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
...
and time will be used as determined by the operating system."
  ...
  (CL:LET* ((*ERROR-ON-MISMATCH* ERROR-ON-MISMATCH?))
   ...
   (CL:LET*
    ((STRING-PARTS (DECOMPOSE-DATE-TIME-STRING DATE-TIME-STRING START END))
    ...
    (CL:LET* ((PATTERN NULL) (ITER-000 *DATE-TIME-PATTERNS*))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ PATTERN (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
    ...
    (CL:IF (CL:EQ STRING-FORM NIL)
     (CL:IF *ERROR-ON-MISMATCH*
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "\"`" DATE-TIME-STRING
        "'\" is not a recognized time/date format.")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))
      (CL:RETURN-FROM PARSE-DATE-TIME-IN-TIME-ZONE NULL))
     (CL:LET*
      ((PARSED-VALUES
        (DECODE-CALENDAR-DATE (MAKE-CURRENT-DATE-TIME)
         (CL:IF (CL:= TIME-ZONE NULL-FLOAT) (GET-LOCAL-TIME-ZONE) TIME-ZONE))))
      (CL:SETF (%DATE-TIME-MILLIS PARSED-VALUES) 0.0d0)
      (CL:SETF (%DATE-TIME-SECOND PARSED-VALUES) 0)
      (CL:SETF (%DATE-TIME-MINUTE PARSED-VALUES) 0)
      (CL:SETF (%DATE-TIME-HOUR PARSED-VALUES) 0)
      (CL:SETF (%DATE-TIME-ZONE PARSED-VALUES) TIME-ZONE)
      (SET-TIME-VALUES STRING-FORM PARSED-VALUES)
      (CL:WHEN (CL:= (%DATE-TIME-ZONE PARSED-VALUES) NULL-FLOAT)
       (CL:SETF (%DATE-TIME-ZONE PARSED-VALUES)
        (GET-LOCAL-TIME-ZONE-FOR-DATE (%DATE-TIME-YEAR PARSED-VALUES)
         (%DATE-TIME-MONTH PARSED-VALUES) (%DATE-TIME-DAY PARSED-VALUES)
         (%DATE-TIME-HOUR PARSED-VALUES) (%DATE-TIME-MINUTE PARSED-VALUES)
         (%DATE-TIME-SECOND PARSED-VALUES))))
      (CL:RETURN-FROM PARSE-DATE-TIME-IN-TIME-ZONE PARSED-VALUES))))))
...
`null' is returned."
  ...
  (CL:LET*
   ((DO (PARSE-DATE-TIME-IN-TIME-ZONE INPUT NULL-FLOAT 0 NULL-INTEGER FALSE)))
   (CL:IF (CL:EQ DO NULL) (CL:RETURN-FROM STRING-TO-CALENDAR-DATE NULL)
    (CL:RETURN-FROM STRING-TO-CALENDAR-DATE (ENCODE-CALENDAR-DATE DO)))))
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ DATE-DIVIDERS
     (LIST* (WRAP-CHARACTER #\\) (WRAP-CHARACTER #\/) (WRAP-CHARACTER #\-) NIL))
    (CL:SETQ DATE-TIME-DIVIDERS (CONS (WRAP-CHARACTER #\T) NIL))
    (CL:SETQ *ERROR-ON-MISMATCH* FALSE)
    ...
    (CL:SETQ *SPECIAL-SYMBOLS*
     (LIST* SYM-DATE-TIME-PARSER-STELLA-YESTERDAY
      SYM-DATE-TIME-PARSER-STELLA-TODAY SYM-DATE-TIME-PARSER-STELLA-TOMORROW
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-039 NIL)) DO
      (CL:PROGN (CL:SETQ ITEM (%%VALUE ITER-039))
       (CL:SETQ ITER-039 (%%REST ITER-039)))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-040 NIL)) DO
      (CL:PROGN (CL:SETQ ITEM (%%VALUE ITER-040))
       (CL:SETQ ITER-040 (%%REST ITER-040)))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-041 NIL)) DO
      (CL:PROGN (CL:SETQ ITEM (%%VALUE ITER-041))
       (CL:SETQ ITER-041 (%%REST ITER-041)))
      (INSERT-AT *ZONE-STRINGS* (%WRAPPER-VALUE (%%VALUE ITEM))
       (%%VALUE (%%REST ITEM)))))
    (CL:LET* ((SYMBOL NULL) (ITER-042 *SPECIAL-SYMBOLS*))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-042 NIL)) DO
      (CL:PROGN (CL:SETQ SYMBOL (%%VALUE ITER-042))
       (CL:SETQ ITER-042 (%%REST ITER-042)))
      (INSERT-AT *SPECIAL-STRINGS* (STRING-DOWNCASE (%SYMBOL-NAME SYMBOL))
       SYMBOL)))
    (CL:SETQ *DATE-TIME-PATTERNS*
     (LIST*
      ...
      (LIST* SYM-DATE-TIME-PARSER-STELLA-HOUR
       (CONS SYM-DATE-TIME-PARSER-STELLA-TIME-DIVIDER NIL)
      ...
      (LIST* (CONS SYM-DATE-TIME-PARSER-STELLA-WEEKDAY NIL)
       SYM-DATE-TIME-PARSER-STELLA-MONTH SYM-DATE-TIME-PARSER-STELLA-DAY
       SYM-DATE-TIME-PARSER-STELLA-HOUR SYM-DATE-TIME-PARSER-STELLA-TIME-DIVIDER
       SYM-DATE-TIME-PARSER-STELLA-MINUTE
       (CONS SYM-DATE-TIME-PARSER-STELLA-TIME-DIVIDER NIL)
       (CONS SYM-DATE-TIME-PARSER-STELLA-SECONDP NIL)
       SYM-DATE-TIME-PARSER-STELLA-YEAR NIL)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "YESTERDAY"
     "(DEFUN YESTERDAY ((PARSED-VALUES DECODED-DATE-TIME)))"
     (CL:FUNCTION YESTERDAY) NULL)
    (DEFINE-FUNCTION-OBJECT "TODAY"
     "(DEFUN TODAY ((PARSED-VALUES DECODED-DATE-TIME)))" (CL:FUNCTION TODAY)
     NULL)
    (DEFINE-FUNCTION-OBJECT "TOMORROW"
     "(DEFUN TOMORROW ((PARSED-VALUES DECODED-DATE-TIME)))"
     (CL:FUNCTION TOMORROW) NULL)
    (DEFINE-FUNCTION-OBJECT "NOW"
     "(DEFUN NOW ((PARSED-VALUES DECODED-DATE-TIME)))" (CL:FUNCTION NOW) NULL)
    (DEFINE-FUNCTION-OBJECT "AM-PM" "(DEFUN (AM-PM SYMBOL) ((STRING OBJECT)))"
     (CL:FUNCTION AM-PM) NULL)
    (DEFINE-FUNCTION-OBJECT "NOON-MIDN"
     "(DEFUN (NOON-MIDN SYMBOL) ((STRING OBJECT)))" (CL:FUNCTION NOON-MIDN) NULL)
    (DEFINE-FUNCTION-OBJECT "WEEKDAY"
     "(DEFUN (WEEKDAY KEYWORD) ((THING OBJECT)))" (CL:FUNCTION WEEKDAY) NULL)
    (DEFINE-FUNCTION-OBJECT "MONTH"
     "(DEFUN (MONTH INTEGER-WRAPPER) ((THING OBJECT)))" (CL:FUNCTION MONTH) NULL)
    (DEFINE-FUNCTION-OBJECT "ZONE"
     "(DEFUN (ZONE FLOAT-WRAPPER) ((THING OBJECT)))" (CL:FUNCTION ZONE) NULL)
    (DEFINE-FUNCTION-OBJECT "SPECIALP"
     "(DEFUN (SPECIALP SYMBOL) ((STRING OBJECT)))" (CL:FUNCTION SPECIALP) NULL)
    (DEFINE-FUNCTION-OBJECT "SECONDP"
     "(DEFUN (SECONDP BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION SECONDP)
     NULL)
    (DEFINE-FUNCTION-OBJECT "MINUTE"
     "(DEFUN (MINUTE BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION MINUTE)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "HOUR"
     "(DEFUN (HOUR BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION HOUR) NULL)
    (DEFINE-FUNCTION-OBJECT "DAY"
     "(DEFUN (DAY BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION DAY) NULL)
    (DEFINE-FUNCTION-OBJECT "YEAR"
     "(DEFUN (YEAR BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION YEAR) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "DIGIT-VALUE"
     "(DEFUN (DIGIT-VALUE INTEGER) ((CHAR CHARACTER)))" (CL:FUNCTION DIGIT-VALUE)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "SET-TIME-VALUES"
     "(DEFUN SET-TIME-VALUES ((STRING-FORM (CONS OF CONS)) (PARSED-VALUES DECODED-DATE-TIME)))"
     (CL:FUNCTION SET-TIME-VALUES) NULL)
    (DEFINE-FUNCTION-OBJECT "PARSE-DATE-TIME-IN-TIME-ZONE"
     "(DEFUN (PARSE-DATE-TIME-IN-TIME-ZONE DECODED-DATE-TIME) ((DATE-TIME-STRING STRING) (TIME-ZONE FLOAT) (START INTEGER) (END INTEGER) (ERROR-ON-MISMATCH? BOOLEAN)) :PUBLIC? TRUE :DOCUMENTATION \"Tries very hard to make sense out of the argument `date-time-string' and
returns a time structure if successful.  If not, it returns `null'.
If `error-on-mismatch?' is true, parse-date-time will signal an error instead of
returning `null'.  
Default values are 00:00:00 in the given timezone on the current date.  If the
...
and time will be used as determined by the operating system.\")"
    ...
    (DEFINE-FUNCTION-OBJECT "PARSE-DATE-TIME"
     "(DEFUN (PARSE-DATE-TIME DECODED-DATE-TIME) ((DATE-TIME-STRING STRING) (START INTEGER) (END INTEGER) (ERROR-ON-MISMATCH? BOOLEAN)) :PUBLIC? TRUE :GLOBALLY-INLINE? TRUE :DOCUMENTATION \"Tries very hard to make sense out of the argument `date-time-string' and
returns a time structure if successful.  If not, it returns `null'.
If `error-on-mismatch?' is true, parse-date-time will signal an error instead of
returning `null'.  
Default values are 00:00:00 local time on the current date\" (RETURN (PARSE-DATE-TIME-IN-TIME-ZONE DATE-TIME-STRING NULL START END ERROR-ON-MISMATCH?)))"
    ...
    (DEFINE-FUNCTION-OBJECT "STRING-TO-CALENDAR-DATE"
     "(DEFUN (STRING-TO-CALENDAR-DATE CALENDAR-DATE) ((INPUT STRING)) :PUBLIC? TRUE :DOCUMENTATION \"Returns a calendar date object representing the date and time
parsed from the `input' string.  If no valid parse is found,
`null' is returned.\")" (CL:FUNCTION STRING-TO-CALENDAR-DATE) NULL)
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-DATE-TIME-PARSER-STELLA-STARTUP-DATE-TIME-PARSER)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-DATE-TIME-PARSER-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Date-Time-Parser") NULL-STRING-WRAPPER)))
