...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-CHARACTER *CHARACTER-TYPE-TABLE*
  TRUE-WRAPPER FALSE-WRAPPER NULL-FLOAT NULL-INTEGER NIL))
...
(CL:DEFUN ERA (STRING)
  (CL:COND
   ((SUBTYPE-OF-STRING? (SAFE-PRIMARY-TYPE STRING))
    (CL:PROGN
     (CL:RETURN-FROM ERA (LOOKUP *ERA-STRINGS* (%WRAPPER-VALUE STRING)))))
     ...
     (CL:PROGN
      ...
      (CL:LET* ((ZONE-NUMBER (LOOKUP *ZONE-STRINGS* (%WRAPPER-VALUE THING))))
       (CL:IF (CL:NOT (CL:EQ ZONE-NUMBER NULL))
        (CL:RETURN-FROM ZONE ZONE-NUMBER) (CL:RETURN-FROM ZONE NULL)))))
...
(CL:DEFUN MATCH-SUBSTRING (SUBSTRING PARSING-TIME?)
  ...
  (CL:LET* ((TESTSTRING (STRING-DOWNCASE SUBSTRING)) (TEST-VALUE NULL))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:EQ (CL:SETQ TEST-VALUE (MONTH (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-MONTH TEST-VALUE)))
   (CL:WHEN
    (CL:NOT
     (CL:EQ (CL:SETQ TEST-VALUE (WEEKDAY (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
     (CONS-LIST SYM-DATE-TIME-PARSER-STELLA-WEEKDAY TEST-VALUE)))
   (CL:WHEN
    (CL:NOT
     (CL:EQ (CL:SETQ TEST-VALUE (AM-PM (WRAP-STRING TESTSTRING))) NULL))
    (CL:RETURN-FROM MATCH-SUBSTRING
  ...
  (CL:COND
   ((ISA? DATUM-ELEMENT SGT-DATE-TIME-PARSER-STELLA-CONS)
    (CL:LET*
     ((DATUM-TYPE
       (CL:IF
        (CL:EQ (%%VALUE DATUM-ELEMENT) SYM-DATE-TIME-PARSER-STELLA-SPECIAL)
       ...
       (CL:WHEN (CL:= (%DATE-TIME-ZONE PARSED-VALUES) NULL-FLOAT)
        (CL:SETF (%DATE-TIME-ZONE PARSED-VALUES)
         (GET-LOCAL-TIME-ZONE-FOR-DATE (%DATE-TIME-YEAR BASE-DATE-TIME)
          (%DATE-TIME-MONTH BASE-DATE-TIME) (%DATE-TIME-DAY BASE-DATE-TIME)
          (%DATE-TIME-HOUR BASE-DATE-TIME) (%DATE-TIME-MINUTE BASE-DATE-TIME)
...
relative time strings like \"now\" or \"yesterday\""
  ...
  (CL:RETURN-FROM PARSE-DATE-TIME-RELATIVE-TO-BASE
   (HELP-PARSE-DATE-TIME-RELATIVE-TO-BASE DATE-TIME-STRING BASE-DATE-TIME
    START END ERROR-ON-MISMATCH? MERGE-NULL-FIELDS?)))
...
and time will be used as determined by the operating system."
  ...
  (CL:LET*
   ...
   (CL:RETURN-FROM PARSE-DATE-TIME-IN-TIME-ZONE
    (HELP-PARSE-DATE-TIME-RELATIVE-TO-BASE DATE-TIME-STRING DEFAULT-VALUES
     START END ERROR-ON-MISMATCH? CL:T))))
...
`null' is returned."
  ...
  (CL:RETURN-FROM STRING-TO-DECODED-DATE-TIME
   (PARSE-DATE-TIME-IN-TIME-ZONE INPUT NULL-FLOAT 0 NULL-INTEGER CL:NIL)))
...
If no valid parse is found, `null' is returned."
  ...
  (CL:IF (CL:EQ DEFAULT-DATE NULL)
   ...
   (CL:LET*
    ((DATE-TIME
      (PARSE-DATE-TIME-RELATIVE-TO-BASE INPUT-DATE DEFAULT-DATE 0 NULL-INTEGER
       CL:NIL CL:T)))
...
(CL:DEFUN HELP-STARTUP-DATE-TIME-PARSER2 ()
  (CL:PROGN
   ...
   (CL:SETQ DATE-DIVIDERS
    (LIST* (WRAP-CHARACTER #\\) (WRAP-CHARACTER #\/) (WRAP-CHARACTER #\-)
     (WRAP-CHARACTER #\+) NIL))
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "HOUR"
     "(DEFUN (HOUR BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION HOUR)
     NULL)
    (DEFINE-FUNCTION-OBJECT "DAY"
     "(DEFUN (DAY BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION DAY) NULL)
    (DEFINE-FUNCTION-OBJECT "YEAR"
     "(DEFUN (YEAR BOOLEAN-WRAPPER) ((NUMBER OBJECT)))" (CL:FUNCTION YEAR)
     NULL)
