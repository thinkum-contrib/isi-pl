...
wrapper unless `value' is too large and needs to be stored in a long wrapper."
  (CL:IF
   (CL:AND (CL:>= VALUE NULL-INTEGER) (CL:<= VALUE MOST-POSITIVE-INTEGER))
...
(CL:DEFMETHOD OBJECT-EQL? ((X BOOLEAN-WRAPPER) Y)
  (CL:RETURN-FROM OBJECT-EQL?
   (CL:OR (CL:EQ X Y)
    (CL:AND (CL:NOT (CL:EQ Y NULL))
     (CL:OR (CL:EQ (PRIMARY-TYPE Y) SGT-LITERALS-STELLA-BOOLEAN-WRAPPER)
  ...
  (CL:LET* ((TABLE (CL:MAKE-ARRAY 256)))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-001)) DO
     (CL:SETQ CODE ITER-001) (CL:SETF (CL:AREF TABLE CODE) KWD-LITERALS-DIGIT)
...
(CL:DEFUN HELP-STARTUP-LITERALS2 ()
  (CL:PROGN (CL:SETQ *LITERAL-TYPE-INFO-TABLE* (NEW-HASH-TABLE))
   ...
   (SET-LITERAL-TYPE-INFO SGT-LITERALS-STELLA-LONG-INTEGER
    KWD-LITERALS-NULL-WRAPPER SYM-LITERALS-STELLA-NULL-LONG-INTEGER-WRAPPER)
