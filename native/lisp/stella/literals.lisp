...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* NULL-CHARACTER *MODULE* SYMBOL-SYM
  STANDARD-ERROR EOL))
...
(CL:DEFUN EQL? (X Y)
  "Return TRUE if `x' equals `y'.  Translates into the Lisp `EQ'
or C++ and Java `==', except that it also understands wrapped literals.  For
...
target language and does not actually call this function."
  (CL:RETURN-FROM EQL?
   (CL:IF (CL:EQ X NULL) (CL:EQ Y NULL) (OBJECT-EQL? X Y))))
  ...
  (CL:LET* ((TABLE (NEW-VECTOR 256)))
   (CL:LET* ((CODE NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 255))
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:LET
      ((SELF (%THE-ARRAY TABLE)) (VALUE KWD-LITERALS-OTHER)
       (POSITION CODE))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-001))
     ...
     (CL:LET
      ((SELF (%THE-ARRAY TABLE)) (VALUE KWD-LITERALS-DIGIT)
       (POSITION CODE))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
       (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-001 (CL:<= ITER-002 UPPER-BOUND-002))
     ...
     (CL:LET
      ((SELF (%THE-ARRAY TABLE)) (VALUE KWD-LITERALS-LETTER)
       (POSITION CODE))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR SELF)
       (CL:TYPE CL:FIXNUM POSITION))
      (CL:SETF (CL:AREF SELF POSITION) VALUE))))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-002 (CL:<= ITER-003 UPPER-BOUND-003))
     DO
...
(CL:PROGN (CL:DEFVAR *CHARACTER-UPCASE-TABLE* STELLA::NULL-STRING) (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CHARACTER-UPCASE-TABLE*)))
...
(CL:PROGN (CL:DEFVAR *CHARACTER-DOWNCASE-TABLE* STELLA::NULL-STRING) (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CHARACTER-DOWNCASE-TABLE*)))
...
(CL:DEFUN HELP-STARTUP-LITERALS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-LITERALS-STELLA-INLINE-WRAP-BOOLEAN-EXPANDER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INLINE-WRAP-BOOLEAN-EXPANDER" NULL
     0))
   ...
   (CL:SETQ SYM-LITERALS-STELLA-INLINE-UNWRAP-BOOLEAN-EXPANDER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "INLINE-UNWRAP-BOOLEAN-EXPANDER" NULL
     0))
...
(CL:DEFUN HELP-STARTUP-LITERALS3 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "EQL?"
    "(DEFUN (EQL? BOOLEAN) ((X OBJECT) (Y OBJECT)) :DOCUMENTATION \"Return TRUE if `x' equals `y'.  Translates into the Lisp `EQ'
or C++ and Java `==', except that it also understands wrapped literals.  For
the case where `x' or `y' are plain literals such as strings or integers, the
STELLA translator substitutes the equality test appropriate for the particular
target language and does not actually call this function.\" :PUBLIC? TRUE)"
...
otherwise, return 'char' unmodified.\" (RETURN (NTH *CHARACTER-DOWNCASE-TABLE* (CHARACTER-CODE CHAR))))"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Literals") NULL-STRING-WRAPPER)))
