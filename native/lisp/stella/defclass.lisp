...
(CL:DECLAIM
  ...
  *TRANSLATE-WITH-RECYCLE-LIST-DEBUGGING?* *CURRENTLY-RECYCLED-ITEMS*
  TRUE STANDARD-ERROR EOL *TRANSLATOROUTPUTLANGUAGE*
  NULL-STRING-WRAPPER NIL FALSE))
...
a Stella class object."
  (CL:RETURN-FROM YIELD-DEFINE-STELLA-CLASS
   (LIST* SYM-DEFCLASS-STELLA-DEFINE-CLASS-FROM-STRINGIFIED-SOURCE
    ...
    (CONS
     (YIELD-STRING-CONSTANT-TREE (%CLASS-STRINGIFIED-SOURCE CLASS))
     NIL))))
...
(CL:DEFUN CANONICAL-SLOT (SLOT)
  (CL:LET* ((CANONICALSLOT SLOT))
   (CL:LOOP WHILE
    (CL:AND
     (CL:NOT (CL:EQ (%SLOT-DIRECT-EQUIVALENT CANONICALSLOT) NULL))
...
(CL:DEFUN METHOD-CONTAINS-UNKNOWN-TYPE? (METHOD)
  (CL:LET* ((FOUND?-000 FALSE))
   (CL:LET*
    ((RTYPE NULL)
     (ITER-000
      (%THE-CONS-LIST (METHOD-RETURN-TYPE-SPECIFIERS METHOD))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
  ...
  (CL:LET* ((ASSIGNMENTS NIL) (INITIALVALUEASSIGNMENT NULL))
   ...
   (CL:LET* ((SLOT NULL) (ITER-002 (CLASS-SLOTS CLASS)))
    (CL:LOOP WHILE (NEXT? ITER-002) DO (CL:SETQ SLOT (%VALUE ITER-002))
     (CL:WHEN
      (CL:AND (CL:NOT (%SLOT-MARKED? SLOT)) (STORAGE-SLOT? SLOT))
...
(CL:DEFUN YIELD-LOCAL-INITIAL-VALUE-ASSIGNMENTS (CLASS)
  (CL:LET* ((ASSIGNMENTS NIL) (INITIALVALUEASSIGNMENT NULL))
   ...
   (CL:LET*
    ((SLOT NULL)
     (ITER-002 (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS))))
             ...
             (CONCATENATE
              (CL:IF (CL:NOT (CL:EQ RECYCLESLOT NULL))
               (LIST*
                (LIST* SYM-DEFCLASS-STELLA-SETF
                 (LIST* SYM-DEFCLASS-STELLA-SLOT-VALUE
                  SYM-DEFCLASS-STELLA-SELF (%SLOT-NAME RECYCLESLOT)
                  NIL)
...
(CL:DEFUN CREATE-RECYCLE-LIST (CLASSTYPE ITEMSIZE)
  ...
  (CL:LET* ((LIST (NEW-RECYCLE-LIST)))
   (CL:LET* ((VALUE-000 NULL))
    (CL:LET*
     ((LIST NULL) (ITER-000 (%THE-CONS-LIST *ALL-RECYCLE-LISTS*)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      (CL:AND
       ...
       (CL:NOT
        (DECIDED-TO-RECYCLE?
         (%SURROGATE-VALUE (%RECYCLE-LIST-OF LIST)))))
...
(CL:DEFUN REDUCE-RECYCLE-LIST-SIZE (LIST FRACTION)
  ...
  (CL:LET*
   ((LENGTH (%CURRENT-LENGTH LIST))
    ...
    (ITEMTYPE (%RECYCLE-LIST-OF LIST)) (CURSOR NULL)
...
(CL:DEFUN GENERIC-SLOT-WRITER? (SLOT)
  (CL:RETURN-FROM GENERIC-SLOT-WRITER?
   (CL:AND (CL:NOT (%SLOT-HARDWIRED? SLOT))
...
(CL:DEFUN YIELD-GENERIC-SLOT-ACCESSOR-DEFINITION (CLASS)
  (CL:LET*
   ...
   (CL:WHEN
    (CL:NOT
     (SUBCLASS-OF? CLASS
      (%SURROGATE-VALUE SGT-DEFCLASS-STELLA-OBJECT)))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:TAGBODY
      (CL:WHEN
       (CL:AND (STORAGE-SLOT? SLOT) (GENERIC-SLOT-READER? SLOT))
       (CL:WHEN
     ...
     (CL:WHEN (CL:NOT (%MIXIN? (TYPE-TO-CLASS SUB)))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN
        (CL:SETQ COLLECT-000
         (CONS
          (INTERN-SYMBOL-IN-MODULE (%SYMBOL-NAME SUB)
           (%HOME-CONTEXT SUB) TRUE)
          NIL))
...
(CL:DEFUN YIELD-NATIVE-SLOT-READER-TREE (SLOT CLASS)
  (CL:LET*
   ((READERNAME (YIELD-NATIVE-SLOT-READER-NAME SLOT))
    (TYPETREE (YIELD-SLOT-TYPE-TREE SLOT))
    (SLOTVISIBLE?
     (SUBTYPE-OF? (%CLASS-TYPE CLASS) (%SLOT-OWNER SLOT))))
      ...
      (CL:IF SLOTVISIBLE?
       (CONS
        (LIST* SYM-DEFCLASS-STELLA-RETURN
         (LIST* SYM-DEFCLASS-STELLA-SETF
          (LIST* SYM-DEFCLASS-STELLA-SLOT-VALUE
           SYM-DEFCLASS-STELLA-SELF (%SLOT-NAME SLOT)
           (LIST* KWD-DEFCLASS-CONTEXT-SENSITIVE?
            SYM-DEFCLASS-STELLA-FALSE NIL))
          SYM-DEFCLASS-STELLA-VALUE NIL)
         NIL)
