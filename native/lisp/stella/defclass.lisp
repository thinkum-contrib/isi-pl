...
(CL:DEFVAR SGT-DEFCLASS-STELLA-STELLA-ROOT-EXCEPTION NULL)
...
(CL:DECLAIM
  ...
  *RECYCLE-LISTS-MAINTENANCE-TIMER* *RECYCLE-LISTS-MAINTENANCE-INTERVAL*
  TRUE-WRAPPER FALSE-WRAPPER *CURRENT-ENVIRONMENT-FEATURES*
  *TRANSLATE-WITH-RECYCLE-LIST-DEBUGGING?* *CURRENTLY-RECYCLED-ITEMS*
  STANDARD-ERROR EOL *TRANSLATOROUTPUTLANGUAGE* NULL-STRING-WRAPPER))
...
a Stella class object."
  (CL:RETURN-FROM YIELD-DEFINE-STELLA-CLASS
   (LIST* SYM-DEFCLASS-STELLA-DEFINE-CLASS-FROM-STRINGIFIED-SOURCE
    ...
    (CONS (YIELD-STRING-CONSTANT-TREE (%CLASS-STRINGIFIED-SOURCE CLASS))
     NIL))))
...
(CL:DEFUN EXCEPTION-CLASS? (CLASS)
  (CL:RETURN-FROM EXCEPTION-CLASS?
   (SUBTYPE-OF? (%CLASS-TYPE CLASS)
    SGT-DEFCLASS-STELLA-STELLA-ROOT-EXCEPTION)))
   ...
   (CL:COND
    ...
    ((CL:OR (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-DEFCLASS-COMMON-LISP)
     ...
     (CL:LET* ((NOFSUPERS 0) (NOFMIXINSUPERS 0))
      ...
      (CL:LET* ((SUPER NULL) (ITER-000 (CLASS-NATIVE-SUPERS CLASS)))
    ...
    (CL:LET* ((SUPER NULL) (ITER-000 (%THE-CONS-LIST DIRECTSUPERS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:LET* ((FOUND?-000 FALSE))
       (CL:LET*
        ((OTHERSUPER NULL) (ITER-001 (%THE-CONS-LIST DIRECTSUPERS)))
...
(CL:DEFUN NATIVE-SLOT-HOME (SLOT CLASS)
  (CL:LET*
   ...
   (CL:LET*
    ((SUPERCLASS NULL) (ITER-000 (%CLASS-ALL-SUPER-CLASSES CLASS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF
      (CL:NOT
       (CL:EQ
        ...
        NULL))
      (CONS (CONS (INITIALIZER CLASS) (CONS SYM-DEFCLASS-STELLA-SELF NIL))
       NIL)
      NIL)))
   (CL:LET*
     ...
     (CL:WHEN
      ...
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN
        (CL:SETQ COLLECT-000
         (CONS
          (CONS (INITIALIZER SUPER) (CONS SYM-DEFCLASS-STELLA-SELF NIL))
          NIL))
        (CL:IF (CL:EQ PARENTINITIALIZERS NIL)
         (CL:SETQ PARENTINITIALIZERS COLLECT-000)
         (ADD-CONS-TO-END-OF-CONS-LIST PARENTINITIALIZERS COLLECT-000)))
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000)
         (CONS
          ...
          NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
   (CL:RETURN-FROM YIELD-INITIALIZER-TREES (REVERSE PARENTINITIALIZERS))))
     ...
     (CONS
      (LIST* SYM-DEFCLASS-STELLA-LET
       ...
       (CONCATENATE (YIELD-CONSTRUCTOR-BODY CLASS (REVERSE ASSIGNMENTS))
        (CONS
         (LIST* SYM-DEFCLASS-STELLA-RETURN SYM-DEFCLASS-STELLA-SELF NIL)
         NIL)))
      NIL)))))
...
(CL:DEFUN YIELD-EXCEPTION-CONSTRUCTOR-DEFINITION (CLASS)
  (CL:LET*
   ((REQUIREDPARAMETERS NIL) (ASSIGNMENTS NIL) (SLOT NULL)
    (MESSAGEPARAMETER (FIRST (CLASS-REQUIRED-SLOT-NAMES CLASS))))
   (CL:LET*
    ((SLOTNAME NULL) (ITER-000 (REST (CLASS-REQUIRED-SLOT-NAMES CLASS)))
     (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ SLOTNAME (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
   ...
   (CL:RETURN-FROM YIELD-EXCEPTION-CONSTRUCTOR-DEFINITION
    (LIST* SYM-DEFCLASS-STELLA-DEFUN
     (CONS (YIELD-CONSTRUCTOR-NAME CLASS) (CONS (CLASS-SYMBOL CLASS) NIL))
     (CONS
      (CONS MESSAGEPARAMETER
       (CONS (TYPE-TO-SYMBOL (TYPE (LOOKUP-SLOT CLASS MESSAGEPARAMETER)))
        NIL))
      (CONCATENATE REQUIREDPARAMETERS NIL))
     KWD-DEFCLASS-CONSTRUCTOR? SYM-DEFCLASS-STELLA-TRUE
     KWD-DEFCLASS-PUBLIC?
     (CL:IF (PUBLIC? CLASS) TRUE-WRAPPER FALSE-WRAPPER)
     (CONS
      (LIST* SYM-DEFCLASS-STELLA-LET
...
(CL:DEFUN PARAMETERIZED-CLASS? (CLASS)
  (CL:LET* ((FOUND?-000 FALSE))
   (CL:LET*
    ((PARAMETERS NULL) (ITER-000 (%THE-CONS-LIST (PARAMETERS CLASS))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ PARAMETERS (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:SETQ FOUND?-000 TRUE) (CL:RETURN)))
   (CL:LET* ((VALUE-000 FOUND?-000))
    (CL:RETURN-FROM PARAMETERIZED-CLASS? VALUE-000))))
...
(CL:DEFUN CLASS-WITH-INITIALIZER? (CLASS)
  (CL:LET* ((FOUND?-000 FALSE))
   (CL:LET* ((SLOT NULL) (ITER-000 (CLASS-SLOTS CLASS)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SLOT (%VALUE ITER-000))
     (CL:WHEN (STORAGE-SLOT? SLOT) (CL:SETQ FOUND?-000 TRUE)
      (CL:RETURN))))
   (CL:LET* ((VALUE-000 FOUND?-000))
...
(CL:DEFVAR *RECYCLING-ENABLED?* TRUE
  "If TRUE calls to `new/free' on classes with :recycle-method
...
(CL:PROGN (CL:DEFVAR *MAX-RECYCLE-LIST-BYTES* 3000000 "Maximum number of bytes to be occupied by recycle lists.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *MAX-RECYCLE-LIST-BYTES*)))

;;; (DEFGLOBAL *RECYCLE-LISTS-MAINTENANCE-INTERVAL* ...)

(CL:PROGN (CL:DEFVAR *RECYCLE-LISTS-MAINTENANCE-INTERVAL* 5000 "Total number of recycle list extensions after which a
size maintenance operation is performed to ensure that
the `*max-recycle-list-bytes*' limit is still met.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *RECYCLE-LISTS-MAINTENANCE-INTERVAL*)))

;;; (DEFGLOBAL *RECYCLE-LISTS-MAINTENANCE-TIMER* ...)

(CL:PROGN (CL:DEFVAR *RECYCLE-LISTS-MAINTENANCE-TIMER* NULL-INTEGER) (CL:DECLAIM (CL:TYPE CL:FIXNUM *RECYCLE-LISTS-MAINTENANCE-TIMER*)))
  ...
  (CL:LET*
   ...
   (CL:LET* ((LIST NULL) (ITER-000 (%THE-CONS-LIST *ALL-RECYCLE-LISTS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      (CL:AND
       (CL:NOT (CL:EQ (%SURROGATE-VALUE (%RECYCLE-LIST-OF LIST)) NULL))
       (CL:NOT
   ...
   (CL:WHEN (CL:> TOTALSIZE MAXSIZE)
    ...
    (CL:LOOP WHILE (CL:> TOTALSIZE MAXSIZE) DO
     (CL:TAGBODY
      (CL:LET*
       ((LIST NULL) (ITER-001 (%THE-CONS-LIST *ALL-RECYCLE-LISTS*)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CL:TAGBODY (CL:WHEN (CL:<= TOTALSIZE MAXSIZE) (CL:RETURN))
         ...
         (CL:IF (CL:> LISTSIZE 0)
          (CL:SETQ TOTALSIZE (CL:- TOTALSIZE LISTSIZE)) (CL:GO :CONTINUE))
   ...
   (INTERN-DERIVED-SYMBOL (%SLOT-NAME SLOT)
    (CONCATENATE "*HARDWIRED-"
     (CONCATENATE (%SYMBOL-NAME (%SLOT-NAME SLOT))
      (CONCATENATE "-ON-"
       (CONCATENATE (%SYMBOL-NAME (%SLOT-OWNER SLOT)) "*")))))))
...
(CL:DEFUN YIELD-SLOT-READER-TREE (SLOT CLASS)
  (CL:LET*
   ((PARAMETERS
     (CONS
      (LIST* SYM-DEFCLASS-STELLA-SELF (TYPE-TO-SYMBOL (%CLASS-TYPE CLASS))
       NIL)
      NIL))
     ...
     (LIST* KWD-DEFCLASS-AUXILIARY? SYM-DEFCLASS-STELLA-TRUE
      (CONCATENATE
       (CL:IF SLOTVISIBLE? NIL
        (CONS
         (LIST* SYM-DEFCLASS-STELLA-IGNORE SYM-DEFCLASS-STELLA-VALUE NIL)
         NIL))
       (CONS WRITECODE NIL)))))))
       ...
       (CONS
        (LIST* SYM-DEFCLASS-STELLA-CASE SYM-DEFCLASS-STELLA-SLOTNAME
         (CONCATENATE RENAMECLAUSES
          (CONS
           (LIST* SYM-DEFCLASS-STELLA-OTHERWISE SYM-DEFCLASS-STELLA-NULL
            NIL)
           NIL)))
        NIL)))
     (CL:SETQ CLAUSES
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (CL:NOT (%MIXIN? (TYPE-TO-CLASS SUB)))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS (TYPE-TO-SYMBOL SUB) NIL))
        (CL:IF (CL:EQ RESULT NIL) (CL:SETQ RESULT COLLECT-000)
         (ADD-CONS-TO-END-OF-CONS-LIST RESULT COLLECT-000)))
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000) (CONS (TYPE-TO-SYMBOL SUB) NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
...
(CL:DEFUN HELP-STARTUP-DEFCLASS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SGT-DEFCLASS-STELLA-STELLA-ROOT-EXCEPTION
...
(CL:DEFUN HELP-STARTUP-DEFCLASS2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-DEFCLASS-STELLA-RECYCLE-LIST-DEBUGGING-ENABLED?
    (INTERN-RIGID-SYMBOL-WRT-MODULE "RECYCLE-LIST-DEBUGGING-ENABLED?" NULL
     0))
  ...
  (CL:PROGN
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Defclass") NULL-STRING-WRAPPER)))
  :VOID)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFSPECIAL *RECYCLING-ENABLED?* BOOLEAN TRUE :DOCUMENTATION \"If TRUE calls to `new/free' on classes with :recycle-method
specifications will actually operate on recycle lists.  Otherwise, all
