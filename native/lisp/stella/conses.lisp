...
(CL:DEFUN REMOVE-DUPLICATES-FROM-LONG-LIST (SELF)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    ...
    (CL:SETQ BUCKETINDEX
     (CL:THE CL:FIXNUM
      (CL:REM (CL:THE CL:FIXNUM (HASH-CODE ITEM)) TABLESIZE)))
...
(CL:DEFMETHOD NEXT? ((SELF CONS-ITERATOR))
  ...
  (CL:RETURN-FROM NEXT?
   (CL:NOT (CL:EQ (%CONS-ITERATOR-CURSOR SELF) NIL))))
...
(CL:DEFMETHOD LENGTH ((SELF CONS-ITERATOR))
  ...
  (CL:LET*
   ...
   (CL:SETF (%CONS-ITERATOR-CURSOR SELF) NIL)
...
(CL:DEFUN STARTUP-CONSES ()
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *PRINTPRETTY?* TRUE) (CL:SETQ *PRINTREADABLY?* FALSE)
...
`newValue' in the cons tree `tree'.  Return the tree.  Uses an `eql?' test.\" :PUBLIC? TRUE)"
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Conses") NULL-STRING-WRAPPER)))
