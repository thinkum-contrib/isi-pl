...
(%%DEFCONSMETHOD LENGTH ((SELF CL:CONS))
  ...
  (CL:LET* ((CONS SELF) (I 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM I))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CONS NIL)) DO (CL:SETQ I (CL:1+ I)) (CL:SETQ CONS (%%REST CONS)))
...
will always be relative to the entire list."
  ...
  (CL:LET* ((ELEMENT NULL) (ITER-000 (NTH-REST SELF START)) (POSITION NULL-INTEGER) (ITER-001 START))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELEMENT (%%VALUE ITER-000)) (CL:SETQ POSITION ITER-001)
    (CL:WHEN (EQL? ELEMENT OBJECT) (CL:RETURN-FROM POSITION POSITION)) (CL:SETQ ITER-000 (%%REST ITER-000))
...
will always be relative to the entire list."
  ...
  (CL:LET* ((LAST-POS NULL-INTEGER)) (CL:DECLARE (CL:TYPE CL:FIXNUM LAST-POS))
   ...
   (CL:LET*
    ((ELEMENT NULL) (ITER-000 SELF) (POSITION NULL-INTEGER) (ITER-001 0) (UPPER-BOUND-000 END)
    ...
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000))) DO
     ...
     (CL:WHEN (EQL? ELEMENT OBJECT) (CL:SETQ LAST-POS POSITION)) (CL:SETQ ITER-000 (%%REST ITER-000))
     (CL:SETQ ITER-001 (CL:1+ ITER-001))))
  ...
  (CL:IF (CL:OR (CL:EQ OBJECT NULL) (STANDARD-OBJECT? OBJECT))
   (CL:LET* ((I NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ I (%%VALUE ITER-000))
     (CL:WHEN (CL:EQ I OBJECT) (CL:RETURN-FROM MEMBER? CL:T)) (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET* ((I NULL) (ITER-001 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ I (%%VALUE ITER-001))
     (CL:WHEN (EQL? I OBJECT) (CL:RETURN-FROM MEMBER? CL:T)) (CL:SETQ ITER-001 (%%REST ITER-001)))))
...
`self' (uses an `eq?' test)."
  (CL:LET* ((I NULL) (ITER-000 SELF))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ I (%%VALUE ITER-000))
    (CL:WHEN (CL:EQ I OBJECT) (CL:RETURN-FROM MEMB? CL:T)) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN CONS (VALUE REST)
  ...
  (CL:LET* ((CONS (CL:CONS CL:NIL CL:NIL))) (CL:SETF (%%VALUE CONS) VALUE) (CL:SETF (%%REST CONS) REST)
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%%REST CONS) NIL)) (CL:SETQ PREVIOUSCONS CONS) (CL:SETQ CONS (%%REST CONS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ CONS NIL)) DO
      (CL:IF (EQL? (%%VALUE CONS) VALUE)
       (CL:PROGN (CL:SETF (%%REST PREVIOUSCONS) (%%REST CONS)) (CL:SETQ CONS (%%REST CONS)))
       (CL:PROGN (CL:SETQ PREVIOUSCONS CONS) (CL:SETQ CONS (%%REST CONS))))))
    (CL:WHEN (EQL? (%%VALUE SELF) VALUE) (CL:WHEN (CL:EQ (%%REST SELF) NIL) (CL:RETURN-FROM REMOVE NIL))
     (CL:SETQ CONS (%%REST SELF)) (CL:SETF (%%VALUE SELF) (%%VALUE CONS)) (CL:SETF (%%REST SELF) (%%REST CONS))))
  ...
  (CL:LET* ((CURSOR SELF) (TRAILER NULL))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (CL:IF (CL:AND (CL:FUNCALL TEST? (%%VALUE CURSOR)) (CL:NOT (CL:EQ TRAILER NULL)))
     (CL:PROGN (CL:SETF (%%REST TRAILER) (%%REST CURSOR)) (CL:SETQ CURSOR (%%REST TRAILER)))
     (CL:PROGN (CL:SETQ TRAILER CURSOR) (CL:SETQ CURSOR (%%REST CURSOR)))))
   (CL:SETQ CURSOR SELF)
   (CL:WHEN (CL:AND (CL:NOT (CL:EQ CURSOR NIL)) (CL:FUNCALL TEST? (%%VALUE CURSOR))) (CL:SETQ TRAILER (%%REST CURSOR))
...
Preserves the original order of the remaining members.  Runs in linear time."
  (CL:IF (CL:> (LENGTH SELF) *REMOVE-DUPLICATES-CROSSOVER-POINT*)
   (CL:RETURN-FROM REMOVE-DUPLICATES (REMOVE-DUPLICATES-FROM-LONG-LIST SELF CL:NIL))
   (CL:LET* ((CURSOR SELF) (REST NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (CL:SETQ REST (%%REST CURSOR))
     (CL:WHEN (MEMBER? REST (%%VALUE CURSOR)) (CL:SETQ REST (REMOVE REST (%%VALUE CURSOR)))
...
(CL:DEFUN REMOVE-DUPLICATES-FROM-LONG-LIST (SELF EQUALTEST?)
  (CL:LET*
   ((TABLESIZE (CEILING (CL:* (LENGTH SELF) 0.3d0))) (TABLE (CL:MAKE-ARRAY TABLESIZE)) (CURSOR SELF) (ITEM NULL)
    (BUCKET NULL) (BUCKETINDEX 0))
   (CL:DECLARE (CL:TYPE CL:FIXNUM TABLESIZE BUCKETINDEX) (CL:TYPE CL:SIMPLE-VECTOR TABLE))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- TABLESIZE)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO (CL:SETQ I ITER-000) (CL:SETF (CL:AREF TABLE I) NIL)
    ...
    (CL:LET* ((FOUND?-000 CL:NIL))
     (CL:LET* ((IT NULL) (ITER-001 BUCKET))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ IT (%%VALUE ITER-001))
       (CL:WHEN (CL:IF EQUALTEST? (EQUAL? IT ITEM) (EQL? IT ITEM)) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
...
the somewhat less efficient variable arguments mechanism."
  (CL:IF (CL:EQ LIST1 NIL) (CL:SETQ LIST1 LIST2)
   (CL:IF (CL:EQ LIST2 NIL) (CL:SETQ LIST2 LIST1) (CL:SETF (%%REST (LAST-CONS LIST1)) LIST2)))
   ...
   (CL:LET* ((LIST NULL) (ITER-000 OTHERLISTS))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ LIST (CL:POP ITER-000))
     (CL:WHEN (CL:NOT (CL:EQ LIST NIL))
      (CL:IF (CL:NOT (CL:EQ LIST2 NIL)) (CL:SETF (%%REST (LAST-CONS LIST2)) LIST) (CL:SETQ LIST1 LIST))
      (CL:SETQ LIST2 LIST)))))
...
(%%DEFCONSMETHOD REVERSE ((SELF CL:CONS))
  ...
  (CL:LET* ((REVERSEDCONSLIST SELF) (REMAININGCONSLIST (%%REST REVERSEDCONSLIST)) (DETACHEDCONS NULL))
   (CL:WHEN (CL:EQ REMAININGCONSLIST NIL) (CL:RETURN-FROM REVERSE REVERSEDCONSLIST))
   (CL:SETF (%%REST REVERSEDCONSLIST) NIL)
   (CL:LOOP WHILE (CL:NOT (CL:EQ REMAININGCONSLIST NIL)) DO (CL:SETQ DETACHEDCONS REMAININGCONSLIST)
    (CL:SETQ REMAININGCONSLIST (%%REST REMAININGCONSLIST)) (CL:SETF (%%REST DETACHEDCONS) REVERSEDCONSLIST)
...
`inValue' in the cons list `self'."
  (CL:LET* ((CURSOR SELF))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (CL:WHEN (EQL? (%%VALUE CURSOR) OUTVALUE) (CL:SETF (%%VALUE CURSOR) INVALUE)) (CL:SETQ CURSOR (%%REST CURSOR)))
...
of `self' can be set with `setf'.  Note, that '(nth NIL <pos>)' = `null'."
  ...
  (CL:LET* ((I -1)) (CL:DECLARE (CL:TYPE CL:FIXNUM I))
   (CL:LET* ((VALUE NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ VALUE (%%VALUE ITER-000))
     (CL:WHEN (CL:= (CL:SETQ I (CL:1+ I)) POSITION) (CL:RETURN-FROM NTH VALUE)) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(%%DEFCONSMETHOD NTH-REST ((SELF CL:CONS) POSITION)
  ...
  (CL:LET* ((CURSOR SELF))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 1) (UPPER-BOUND-000 POSITION) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
     (CL:SETQ CURSOR (%%REST CURSOR)) (CL:WHEN (CL:EQ CURSOR NIL) (CL:RETURN-FROM NTH-REST NIL))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
(%%DEFCONSMETHOD SECOND-SETTER ((SELF CL:CONS) VALUE)
  (CL:WHEN (CL:EQ (%%REST SELF) NIL) (CL:WARN "'second setter' applied to too short cons list.")
...
(%%DEFCONSMETHOD NTH-SETTER ((SELF CL:CONS) VALUE POSITION)
  ...
  #+MCL
  (CL:CHECK-TYPE POSITION CL:FIXNUM)
  (CL:WHEN (CL:< POSITION 0) (CL:WARN "Can't apply nth setter to negative position `~A'." POSITION)
  ...
  (CL:LET* ((CURSOR SELF))
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
     (CL:SETQ CURSOR (%%REST CURSOR))
     (CL:WHEN (CL:EQ CURSOR NIL) (CL:WARN "'nth setter' ran off end of cons list.") (CL:RETURN-FROM NTH-SETTER NULL))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
...
(%%DEFCONSMETHOD NTH-REST-SETTER ((SELF CL:CONS) VALUE POSITION)
  ...
  (CL:LET* ((CURSOR SELF))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 2) (UPPER-BOUND-000 POSITION) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
     (CL:SETQ CURSOR (%%REST CURSOR))
     (CL:WHEN (CL:EQ CURSOR NIL) (CL:WARN "'nth-rest setter' ran off end of cons list.")
...
(%%DEFCONSMETHOD ALLOCATE-ITERATOR ((SELF CL:CONS))
  (CL:LET* ((ITERATOR (NEW-CONS-ITERATOR))) (CL:SETF (%CONS-ITERATOR-CURSOR ITERATOR) SELF)
...
(CL:DEFMETHOD NEXT? ((SELF CONS-ITERATOR))
  (CL:IF (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) CL:NIL)
   (CL:SETF (%CONS-ITERATOR-CURSOR SELF) (%%REST (%CONS-ITERATOR-CURSOR SELF))))
...
if one of them is `eql?' to 'value."
  (CL:LET*
   ((RESULT
     (CL:IF (%FIRST-ITERATION? SELF) (MEMBER? (%CONS-ITERATOR-CURSOR SELF) VALUE)
...
(%%DEFCONSMETHOD BUT-LAST ((SELF CL:CONS))
  ...
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR))) (CL:SETF (%ITERATOR-CONS-LIST ITERATOR) SELF)
   (CL:SETF (%FIRST-ITERATION? ITERATOR) CL:T) (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION BUT-LAST-NEXT?))
   ...
   (CL:LET* ((K NULL) (V NULL) (ITER-000 (%THE-KV-LIST SELF)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ K (%KEY ITER-000)) (CL:SETQ V (%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (CONS K (CONS V NIL)) NIL))
...
(CL:DEFMETHOD CONSIFY ((SELF VECTOR))
  "Return a list of elements in `self'."
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((V NULL) (VECTOR-000 SELF) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (COLLECT-000 NULL))
    (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     (CL:SETQ V (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
...
(CL:DEFMETHOD CONSIFY ((SELF ITERATOR))
  ...
  (CL:LET* ((LIST NIL))
   (CL:LET* ((VALUE NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ VALUE (%VALUE ITER-000)) (CL:SETQ LIST (CONS VALUE LIST))))
...
(CL:DEFUN COPY-CONS-LIST (SELF)
  ...
  (CL:LET* ((NEWCONSLIST (CONS (%%VALUE SELF) NIL)) (NEXTCONS NEWCONSLIST) (COPYFROMCONS NULL) (PREVIOUSCONS NULL))
   (CL:SETQ COPYFROMCONS (%%REST SELF))
   (CL:LOOP WHILE (CL:NOT (CL:EQ COPYFROMCONS NIL)) DO (CL:SETQ PREVIOUSCONS NEXTCONS)
    (CL:SETQ NEXTCONS (CONS (%%VALUE COPYFROMCONS) NIL)) (CL:SETF (%%REST PREVIOUSCONS) NEXTCONS)
...
at least one value is passed in."
  (CL:LET* ((HEADCONS NIL) (ANSWER NULL) (LASTCONS NULL))
   (CL:LET* ((V NULL) (ITER-000 VALUES))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ V (CL:POP ITER-000)) (CL:SETQ HEADCONS (CONS V HEADCONS))))
   (CL:SETQ ANSWER (REVERSE (%%REST HEADCONS)))
   (CL:WHEN (CL:EQ ANSWER NIL) (CL:SETQ ANSWER (%%VALUE HEADCONS)) (CL:RETURN-FROM LIST* ANSWER))
   (CL:SETQ LASTCONS ANSWER)
   (CL:LOOP WHILE (CL:NOT (CL:EQ (%%REST LASTCONS) NIL)) DO (CL:SETQ LASTCONS (%%REST LASTCONS)))
   (CL:SETF (%%REST LASTCONS) (%%VALUE HEADCONS)) (CL:RETURN-FROM LIST* ANSWER)))
  ...
  (CL:LET* ((ALWAYS?-000 CL:T))
   (CL:LET* ((ITEM NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000))
     (CL:WHEN (CL:NOT (MEMBER? OTHERLIST ITEM)) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
...
this does not check whether `self' and `otherList' actually are sets."
  (CL:LET* ((ALWAYS?-000 CL:T))
   (CL:LET* ((ITEM NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000))
     (CL:WHEN (MEMBER? OTHERLIST ITEM) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN)) (CL:SETQ ITER-000 (%%REST ITER-000))))
   ...
   (CL:LET* ((I NULL) (ITER-000 SELF) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ I (%%VALUE ITER-000))
     (CL:WHEN (CL:NOT (MEMBER? OTHERLIST I))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       (CL:PROGN (CL:SETQ COLLECT-000 (CONS I NIL))
        (CL:IF (CL:EQ DIFF NIL) (CL:SETQ DIFF COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST DIFF COLLECT-000)))
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS I NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
...
guaranteed to be a set if `self' is a set."
  ...
  (CL:LET* ((I NULL) (ITER-000 OTHERLIST))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ I (%%VALUE ITER-000))
    (CL:WHEN (MEMBER? SELF I) (CL:SETQ SELF (REMOVE SELF I))) (CL:SETQ ITER-000 (%%REST ITER-000))))
...
(CL:DEFUN HELP-SORT-CONS-LIST (LIST LENGTH PREDICATE)
  ...
  (CL:LET* ((FRONTLENGTH (FLOOR (CL:/ LENGTH 2.0d0))) (TEMP LIST) (BACK NIL))
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ITER-000 2) (UPPER-BOUND-000 FRONTLENGTH) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-000 UPPER-BOUND-000))
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000) (CL:SETQ I I)
     (CL:SETQ TEMP (%%REST TEMP)) (CL:SETQ ITER-000 (CL:1+ ITER-000))))
   ...
   (CL:RETURN-FROM HELP-SORT-CONS-LIST
    (MERGE-CONS-LISTS (HELP-SORT-CONS-LIST LIST FRONTLENGTH PREDICATE)
     (HELP-SORT-CONS-LIST BACK (CL:- LENGTH FRONTLENGTH) PREDICATE) PREDICATE))))
...
(CL:DEFUN MERGE-CONS-LISTS (LIST1 LIST2 PREDICATE)
  (CL:LET* ((CURSOR1 LIST1) (CURSOR2 LIST2) (RESULT NIL) (TAIL NIL) (TEMP NIL))
   (CL:LOOP
    (CL:WHEN (CL:EQ CURSOR1 NIL) (CL:WHEN (CL:EQ TAIL NIL) (CL:RETURN-FROM MERGE-CONS-LISTS CURSOR2))
     (CL:SETF (%%REST TAIL) CURSOR2) (CL:RETURN-FROM MERGE-CONS-LISTS RESULT))
    (CL:WHEN (CL:EQ CURSOR2 NIL) (CL:WHEN (CL:EQ TAIL NIL) (CL:RETURN-FROM MERGE-CONS-LISTS CURSOR1))
     (CL:SETF (%%REST TAIL) CURSOR1) (CL:RETURN-FROM MERGE-CONS-LISTS RESULT))
    (CL:COND
     ((CL:FUNCALL PREDICATE (%%VALUE CURSOR2) (%%VALUE CURSOR1)) (CL:SETQ TEMP CURSOR2)
      (CL:SETQ CURSOR2 (%%REST CURSOR2)) (CL:SETQ LIST2 CURSOR2))
   ...
   (CL:COND
      ...
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE Y)))
       (CL:COND
        ((SUBTYPE-OF-FLOAT? TEST-VALUE-000)
         (CL:PROGN (CL:RETURN-FROM NUMBER-LESS-THAN? (< (%WRAPPER-VALUE X) (%WRAPPER-VALUE Y)))))
    ...
    (CL:T
     (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003) "`" TEST-VALUE-003 "' is not a valid case option")
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE FIRSTELEMENT)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000 SGT-CONSES-STELLA-GENERALIZED-SYMBOL)
     (CL:PROGN (CL:RETURN-FROM CHOOSE-SORT-PREDICATE (CL:FUNCTION GENERALIZED-SYMBOL-LESS-THAN?))))
    ((SUBTYPE-OF-INTEGER? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CHOOSE-SORT-PREDICATE (CL:FUNCTION WRAPPED-INTEGER-LESS-THAN?))))
    ((SUBTYPE-OF-LONG-INTEGER? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CHOOSE-SORT-PREDICATE (CL:FUNCTION WRAPPED-LONG-INTEGER-LESS-THAN?))))
    ((SUBTYPE-OF-FLOAT? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CHOOSE-SORT-PREDICATE (CL:FUNCTION WRAPPED-FLOAT-LESS-THAN?))))
    ((SUBTYPE-OF-STRING? TEST-VALUE-000)
     (CL:PROGN (CL:RETURN-FROM CHOOSE-SORT-PREDICATE (CL:FUNCTION WRAPPED-STRING-LESS-THAN?))))
    ...
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "choose-sort-predicate: Don't know how to sort `"
...
whose `n'-th element (0-based) will be used for comparison."
  ...
  (CL:LET* ((*SORT-TUPLE-COMPARE-PREDICATE* PREDICATE) (*SORT-TUPLE-COMPARE-INDEX* N))
   (CL:DECLARE (CL:SPECIAL *SORT-TUPLE-COMPARE-PREDICATE* *SORT-TUPLE-COMPARE-INDEX*))
...
it will cause value wrapping everytime `slot' is read."
  (CL:WHEN (CL:AND (CL:EQ PREDICATE NULL) (CL:NOT (CL:EQ SELF NIL)))
   (CL:SETQ PREDICATE (CHOOSE-SORT-PREDICATE (READ-SLOT-VALUE (%%VALUE SELF) SLOT))))
    ...
    (CL:PROGN
     (CL:IF (CL:EQ TREE NIL) (CL:RETURN-FROM SEARCH-CONS-TREE? (CL:EQ VALUE NIL))
      (CL:RETURN-FROM SEARCH-CONS-TREE?
       (CL:OR (SEARCH-CONS-TREE? (%%VALUE TREE) VALUE) (SEARCH-CONS-TREE? (%%REST TREE) VALUE))))))
      ...
      (CL:T
       (CL:LET* ((FOUND?-000 CL:NIL))
        (CL:LET* ((ITEM NULL) (ITER-000 TREE))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000))
          (CL:WHEN (SEARCH-CONS-TREE-WITH-FILTER? ITEM VALUE FILTER) (CL:SETQ FOUND?-000 CL:T) (CL:RETURN))
          (CL:SETQ ITER-000 (%%REST ITER-000))))
   ...
   ((CL:EQ (SAFE-PRIMARY-TYPE SELF) SGT-CONSES-STELLA-CONS)
    (CL:PROGN
     (CL:IF (CL:EQ SELF NIL) (CL:RETURN-FROM COPY-CONS-TREE NIL)
      (CL:RETURN-FROM COPY-CONS-TREE (CONS (COPY-CONS-TREE (%%VALUE SELF)) (COPY-CONS-TREE (%%REST SELF)))))))
   ...
   ((CL:EQ (SAFE-PRIMARY-TYPE TREE2) SGT-CONSES-STELLA-CONS)
    (CL:PROGN
     (CL:LOOP WHILE (CL:NOT (CL:EQ TREE1 NIL)) DO
      (CL:WHEN (CL:NOT (EQUAL? (%%VALUE TREE1) (%%VALUE TREE2))) (CL:RETURN-FROM OBJECT-EQUAL? CL:NIL))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CODE 18589447)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LOOP (CL:WHEN (CL:EQ SELF NIL) (CL:RETURN))
    (CL:SETQ CODE (CL:THE CL:FIXNUM (CL:LOGXOR CODE (CL:THE CL:FIXNUM (SAFE-EQUAL-HASH-CODE (%%VALUE SELF))))))
  ...
  (CL:LET* ((RESULT TREE))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 INDEX)) (CL:DECLARE (CL:TYPE CL:FIXNUM I))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ I (CL:POP ITER-000))
     (CL:COND ((CL:EQ (SAFE-PRIMARY-TYPE RESULT) SGT-CONSES-STELLA-CONS) (CL:PROGN (CL:SETQ TREE RESULT)))
  ...
  (CL:LET* ((RESULT TREE) (NOFINDICES (CL:LENGTH INDEX))) (CL:DECLARE (CL:TYPE CL:FIXNUM NOFINDICES))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 INDEX) (J NULL-INTEGER) (ITER-001 1))
    ...
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ I (CL:POP ITER-000)) (CL:SETQ J ITER-001)
     ...
     (CL:IF (CL:= J NOFINDICES) (CL:SETQ RESULT (NTH-REST TREE I)) (CL:SETQ RESULT (NTH TREE I)))
     (CL:SETQ ITER-001 (CL:1+ ITER-001))))
           ...
           (CL:LET* ((ALWAYS?-000 CL:T))
            (CL:LET* ((ELT NULL) (ITER-000 TREE) (PAT NULL) (ITER-001 PATTERN))
             (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:NOT (CL:EQ ITER-001 NIL))) DO
              (CL:SETQ ELT (%%VALUE ITER-000)) (CL:SETQ PAT (%%VALUE ITER-001))
              (CL:WHEN (CL:EQ (MATCH-CONS-TREE ELT PAT BINDINGS) NULL) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
    ...
    (CL:LET* ((I 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM I))
     (CL:LET* ((ELEMENT NULL) (ITER-000 TREE))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELEMENT (%%VALUE ITER-000))
       (%%PRINT-STREAM STREAM " " ELEMENT) (CL:SETQ TREE (%%REST TREE)) (CL:SETQ I (CL:1+ I))
       (CL:WHEN (CL:>= I *PRINTLENGTH*) (CL:RETURN)) (CL:SETQ ITER-000 (%%REST ITER-000))))
     (CL:WHEN (CL:NOT (CL:EQ TREE NIL)) (%%PRINT-STREAM STREAM " ...")))
    (CL:LET* ((ELEMENT NULL) (ITER-001 TREE))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ELEMENT (%%VALUE ITER-001))
      (%%PRINT-STREAM STREAM " " ELEMENT) (CL:SETQ ITER-001 (%%REST ITER-001))))))
...
(CL:DEFUN PRINT-STELLA-CODE (TREE STREAM)
  (CL:LET* ((*PRINTPRETTY?* *PRINTPRETTYCODE?*) (*PRINTREADABLY?* CL:T))
   (CL:DECLARE (CL:SPECIAL *PRINTPRETTY?* *PRINTREADABLY?*)) (%%PRINT-STREAM STREAM TREE EOL))
  :VOID)
     ...
     (CL:PROGN
      (CL:COND
       ((CL:>= (LENGTH TREE) 3) (CL:SETQ *PRINTPRETTY?* CL:NIL)
        (%%PRINT-STREAM STREAM "(" (%%VALUE TREE) " " (%%VALUE (%%REST TREE)) " " (%%VALUE (%%REST (%%REST TREE))))
        (CL:SETQ *PRINTPRETTY?* *PRINTPRETTYCODE?*)
        (CL:LET* ((FORM NULL) (ITER-000 (NTH-REST TREE 3)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ FORM (%%VALUE ITER-000))
          (%%PRINT-STREAM STREAM EOL "  " FORM) (CL:SETQ ITER-000 (%%REST ITER-000))))
        (%%PRINT-STREAM STREAM ")"))
   ...
   ((CL:EQ (SAFE-PRIMARY-TYPE SELF) SGT-CONSES-STELLA-CONS)
    (CL:PROGN
     (CL:IF (CL:EQ SELF NIL) (CL:RETURN-FROM TREE-SIZE 0)
      (CL:RETURN-FROM TREE-SIZE (CL:1+ (CL:+ (TREE-SIZE (%%VALUE SELF)) (TREE-SIZE (%%REST SELF))))))))
...
(CL:DEFUN SAFELY-COMPUTE-TREE-SIZE (TREE DEPTHCOUNT CUTOFF)
  ...
  (CL:LET* ((COUNT 1) (VALUE NULL)) (CL:DECLARE (CL:TYPE CL:FIXNUM COUNT))
   ...
   (CL:LOOP (CL:SETQ VALUE (%%VALUE TREE))
    (CL:WHEN (CONS? VALUE)
     (CL:SETQ COUNT (CL:+ COUNT (SAFELY-COMPUTE-TREE-SIZE (%%VALUE TREE) (CL:1+ DEPTHCOUNT) CUTOFF))))
    (CL:WHEN (CL:OR (CL:EQ (%%REST TREE) NULL) (CL:EQ (%%REST TREE) NIL) (CL:> COUNT CUTOFF))
  ...
  (CL:LET* ((*DEPTHEXCEEDED?* CL:NIL)) (CL:DECLARE (CL:SPECIAL *DEPTHEXCEEDED?*))
   (CL:LET* ((CUTOFF 99) (SIZE (SAFELY-COMPUTE-TREE-SIZE TREE 0 CUTOFF))) (CL:DECLARE (CL:TYPE CL:FIXNUM CUTOFF SIZE))
    (CL:RETURN-FROM SAFE-TREE-SIZE
     (CL:VALUES SIZE (CL:IF (CL:OR *DEPTHEXCEEDED?* (CL:>= SIZE CUTOFF)) "CIRCULAR" "OK"))))))
...
(CL:DEFUN HELP-STARTUP-CONSES1 ()
  (CL:PROGN (CL:SETQ SGT-CONSES-STELLA-GENERALIZED-SYMBOL (INTERN-RIGID-SYMBOL-WRT-MODULE "GENERALIZED-SYMBOL" NULL 1))
   ...
   (CL:SETQ SGT-CONSES-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" NULL 1))
...
if one of them is `eql?' to 'value.\")" (CL:FUNCTION MEMBER?) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "BUT-LAST-NEXT?"
    "(DEFUN (BUT-LAST-NEXT? BOOLEAN) ((SELF ALL-PURPOSE-ITERATOR)) :PUBLIC? TRUE)" (CL:FUNCTION BUT-LAST-NEXT?) NULL)
   (DEFINE-METHOD-OBJECT
...
a singleton cons list containing it.\" :PUBLIC? TRUE)" (CL:FUNCTION CONSIFY) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD (REMOVE-DELETED-MEMBERS (LIKE SELF)) ((SELF CONS)) :PUBLIC? TRUE)"
...
Uses an `eql?' test and a simple quadratic-time algorithm.  Note that
this does not check whether `self' and `otherList' actually are sets.\" :PUBLIC? TRUE)" (CL:FUNCTION EQUIVALENT-SETS?)
    NULL)
   (DEFINE-METHOD-OBJECT
...
This is always true if at least one of the two sets is the empty set.
Uses an `eql?' test and a simple quadratic-time algorithm.  Note that
this does not check whether `self' and `otherList' actually are sets.\" :PUBLIC? TRUE)" (CL:FUNCTION DISJOINT-SETS?)
    NULL)
   (DEFINE-METHOD-OBJECT
...
(CL:DEFUN STARTUP-CONSES ()
  (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 0)
   (CL:WHEN (CL:EQ NIL NULL) (CL:SETQ NIL (CL:CONS CL:NIL CL:NIL)) (CL:SETF (%%VALUE NIL) NULL)
    (CL:SETF (%%REST NIL) NIL))
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *PRINTLENGTH* NULL-INTEGER)
...
element types are GENERALIZED-SYMBOL, STRING, INTEGER, and FLOAT).\")" (CL:FUNCTION SORT) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "CHOOSE-SORT-PREDICATE"
     "(DEFUN (CHOOSE-SORT-PREDICATE FUNCTION-CODE) ((FIRSTELEMENT OBJECT)))" (CL:FUNCTION CHOOSE-SORT-PREDICATE) NULL)
    (DEFINE-FUNCTION-OBJECT "SORT-TUPLE-COMPARE?" "(DEFUN (SORT-TUPLE-COMPARE? BOOLEAN) ((X CONS) (Y CONS)))"
...
whose `n'-th element (0-based) will be used for comparison.\")" (CL:FUNCTION SORT-TUPLES) NULL)
    (DEFINE-FUNCTION-OBJECT "SORT-OBJECTS-COMPARE?" "(DEFUN (SORT-OBJECTS-COMPARE? BOOLEAN) ((X OBJECT) (Y OBJECT)))"
...
the cons tree `tree'.  Uses an `eql?' test.  Does not descend into any
cons whose first element matches an element of `filter'.\" :PUBLIC? TRUE)" (CL:FUNCTION SEARCH-CONS-TREE-WITH-FILTER?)
     NULL)
...
structurally equivalent.  Uses `equal?' to test equality of subtrees.\" :PUBLIC? TRUE)" (CL:FUNCTION OBJECT-EQUAL?)
     NULL)
    (DEFINE-METHOD-OBJECT
...
Variables can't be quoted but quoting can effectively be achieved by
inserting to-be-quoted variables bound to themselves into `bindings'.\" :PUBLIC? TRUE)" (CL:FUNCTION MATCH-CONS-TREE)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "PRINT-STELLA-CODE"
     "(DEFUN PRINT-STELLA-CODE ((TREE OBJECT) (STREAM NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION PRINT-STELLA-CODE) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "SAFE-TREE-SIZE" "(DEFUN (SAFE-TREE-SIZE INTEGER STRING) ((TREE CONS)))"
     (CL:FUNCTION SAFE-TREE-SIZE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-CONSES" "(DEFUN STARTUP-CONSES () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-CONSES)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CONSES-STELLA-STARTUP-CONSES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CONSES-STELLA-METHOD-STARTUP-CLASSNAME
...
an unoptimized Common Lisp, 20 is a good crossover point.\")")
    ...
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
     "(DEFSPECIAL *SORT-OBJECTS-COMPARE-SLOT* STORAGE-SLOT NULL)")
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
