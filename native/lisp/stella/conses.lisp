...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* EOL *INTEGER-MSB-MASK* TRUE FALSE
  NULL-INTEGER))
...
will always be relative to the entire list."
  ...
  (CL:LET*
   ((POSITION NULL-INTEGER) (ELEMENT NULL) (ITER-000 (NTH-REST SELF START))
    (ITER-001 START))
   (CL:DECLARE (CL:TYPE CL:FIXNUM ITER-001 POSITION))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ ELEMENT (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (CL:PROGN (CL:SETQ POSITION ITER-001) (CL:SETQ ITER-001 (CL:1+ ITER-001)))
...
assigned to a variable."
  (CL:LET* ((CURSOR SELF) (TRAILER NULL))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (CL:IF
     (CL:AND (CL:FUNCALL TEST? (%%VALUE CURSOR)) (CL:NOT (CL:EQ TRAILER NULL)))
...
Preserves the original order of the remaining members.  Runs in linear time."
  (CL:IF (CL:> (LENGTH SELF) *REMOVE-DUPLICATES-CROSSOVER-POINT*)
   (CL:RETURN-FROM REMOVE-DUPLICATES (REMOVE-DUPLICATES-FROM-LONG-LIST SELF))
  ...
  (CL:LET*
   ...
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (CL:1- TABLESIZE)))
    ...
    (CL:LOOP WHILE (CL:<= ITER-000 UPPER-BOUND-000) DO
     ...
     (CL:SETF (CL:AREF TABLE I) NIL)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (CL:SETQ ITEM (%%VALUE CURSOR))
    (CL:SETQ BUCKETINDEX
     (CL:THE CL:FIXNUM (CL:REM (CL:THE CL:FIXNUM (HASH-CODE ITEM)) TABLESIZE)))
    (CL:SETQ BUCKET (CL:AREF TABLE BUCKETINDEX))
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET* ((IT NULL) (ITER-001 BUCKET))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:PROGN (CL:SETQ IT (%%VALUE ITER-001))
        (CL:SETQ ITER-001 (%%REST ITER-001)))
       (CL:WHEN (EQL? IT ITEM) (CL:SETQ FOUND?-000 TRUE) (CL:RETURN))))
   ...
   (CL:LET* ((LIST NULL) (ITER-000 OTHERLISTS))
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ LIST (CL:POP ITER-000))
     (CL:WHEN (CL:NOT (CL:EQ LIST NIL))
      (CL:IF (CL:NOT (CL:EQ LIST2 NIL)) (CL:SETF (%%REST (LAST-CONS LIST2)) LIST)
       (CL:SETQ LIST1 LIST))
      (CL:SETQ LIST2 LIST)))))
...
`inValue' in the cons list `self'."
  (CL:LET* ((CURSOR SELF))
   (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO
    (CL:WHEN (EQL? (%%VALUE CURSOR) OUTVALUE) (CL:SETF (%%VALUE CURSOR) INVALUE))
    (CL:SETQ CURSOR (%%REST CURSOR)))
   ...
   (CL:LET* ((VALUE NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ VALUE (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:= (CL:SETQ I (CL:1+ I)) POSITION) (CL:RETURN-FROM NTH VALUE))))
...
(%%DEFCONSMETHOD FIRST-SETTER ((SELF CL:CONS) VALUE)
  (CL:WHEN (CL:EQ SELF NIL) (CL:WARN "Can't apply first setter to empty list.")
...
(CL:DEFMETHOD CONSIFY ((SELF KEY-VALUE-LIST))
  ...
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((V NULL) (K NULL) (ITER-000 (%THE-KV-LIST SELF)) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ K (%KEY ITER-000))
     (CL:PROGN (CL:SETQ V (%VALUE ITER-000)) (CL:SETQ ITER-000 (%REST ITER-000)))
...
of `consList1' and `consList2'.  The concatenation is NOT destructive."
  (CL:COND ((CL:EQ CONSLIST1 NIL) (CL:RETURN-FROM APPEND CONSLIST2))
   ...
   (CL:T
    (CL:RETURN-FROM APPEND (CONCATENATE (COPY-CONS-LIST CONSLIST1) CONSLIST2)))))
   ...
   (CL:LET* ((ITEM NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ ITEM (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:NOT (MEMBER? OTHERLIST ITEM)) (CL:SETQ ALWAYS?-000 FALSE)
      (CL:RETURN))))
...
(CL:DEFUN HELP-SORT-CONS-LIST (LIST LENGTH PREDICATE)
  ...
  (CL:LET* ((FRONTLENGTH (FLOOR (CL:/ LENGTH 2.0d0))) (TEMP LIST) (BACK NIL))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO
     (CL:PROGN (CL:SETQ I ITER-000) (CL:SETQ ITER-000 (CL:1+ ITER-000)))
     (CL:SETQ I I) (CL:SETQ TEMP (%%REST TEMP))))
...
(CL:DEFUN MERGE-CONS-LISTS (LIST1 LIST2 PREDICATE)
  (CL:LET* ((CURSOR1 LIST1) (CURSOR2 LIST2) (RESULT NIL) (TAIL NIL) (TEMP NIL))
   (CL:LOOP
    ...
    (CL:IF (CL:EQ TAIL NIL) (CL:SETQ RESULT TEMP) (CL:SETF (%%REST TAIL) TEMP))
    (CL:SETQ TAIL TEMP))))
    ...
    (CL:PROGN
     (CL:COND
      ...
      ((MEMBER? FILTER (%%VALUE TREE))
       (CL:RETURN-FROM SEARCH-CONS-TREE-WITH-FILTER? FALSE))
      (CL:T
       (CL:LET* ((FOUND?-000 FALSE))
    ...
    (CL:PROGN
     (CL:IF (CL:EQ SELF NIL) (CL:RETURN-FROM COPY-CONS-TREE NIL)
      (CL:RETURN-FROM COPY-CONS-TREE
       (CONS (COPY-CONS-TREE (%%VALUE SELF)) (COPY-CONS-TREE (%%REST SELF)))))))
...
is O(N) in the number of elements of `self'."
  (CL:LET* ((CODE 18589447)) (CL:DECLARE (CL:TYPE CL:FIXNUM CODE))
   (CL:LOOP (CL:WHEN (CL:EQ SELF NIL) (CL:RETURN))
    (CL:SETQ CODE
     (CL:THE CL:FIXNUM
      (CL:LOGXOR CODE (CL:THE CL:FIXNUM (SAFE-EQUAL-HASH-CODE (%%VALUE SELF))))))
    (CL:SETQ SELF (%%REST SELF))
    (CL:SETQ CODE
...
(CL:DEFUN PRINT-CONS (TREE STREAM LPAREN RPAREN)
  ...
  (CL:WHEN (CL:NOT (CL:EQ TREE NIL)) (%%PRINT-STREAM STREAM (%%VALUE TREE))
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ELEMENT NULL) (ITER-000 TREE) (ITER-001 2)
     (UPPER-BOUND-000 *PRINTLENGTH*) (UNBOUNDED?-000 (NULL? UPPER-BOUND-000)))
...
(CL:DEFVAR *PRINTPRETTYCODE?* FALSE
...
(CL:DEFUN SAFELY-COMPUTE-TREE-SIZE (TREE DEPTHCOUNT CUTOFF)
  ...
  (CL:LET* ((COUNT 1) (VALUE NULL)) (CL:DECLARE (CL:TYPE CL:FIXNUM COUNT))
   (CL:WHEN (CL:> DEPTHCOUNT *DEPTHCUTOFF*) (CL:SETQ *DEPTHEXCEEDED?* TRUE)
...
(CL:DEFUN SAFE-TREE-SIZE (TREE)
  (CL:LET* ((*DEPTHEXCEEDED?* FALSE)) (CL:DECLARE (CL:SPECIAL *DEPTHEXCEEDED?*))
...
terminated by the last value rather than by `nil'.  Assumes that
at least one value is passed in.\" :PUBLIC? TRUE)" (CL:FUNCTION LIST*) NULL)
...
(CL:DEFUN STARTUP-CONSES ()
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *PRINTPRETTY?* TRUE)
    (CL:SETQ *PRINTREADABLY?* FALSE) (CL:SETQ ELIPSIS SYM-CONSES-STELLA-...)
    (CL:SETQ *PRINTPRETTYCODE?* TRUE) (CL:SETQ *DEPTHEXCEEDED?* FALSE))
...
`newValue' in the cons tree `tree'.  Return the tree.  Uses an `eql?' test.\" :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "TREE-SIZE"
     "(DEFUN (TREE-SIZE INTEGER) ((SELF OBJECT)))" (CL:FUNCTION TREE-SIZE) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-CONSES"
     "(DEFUN STARTUP-CONSES () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-CONSES) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CONSES-STELLA-STARTUP-CONSES)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-CONSES-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_Startup-Conses")
      NULL-STRING-WRAPPER)))
