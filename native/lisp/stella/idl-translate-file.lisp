...
(CL:DECLAIM (CL:SPECIAL *STELLA-MODULE* *MODULE* NIL *TRANSLATIONUNITS* STANDARD-OUTPUT EOL *CURRENT-STREAM*))

;;; (DEFUN IDL-TRANSLATE-CLASS-TO-STREAM ...)

(CL:DEFUN IDL-TRANSLATE-CLASS-TO-STREAM (CLASS NATIVE-STREAM)
  (CL:LET* ((SELF-000 (NEW-TRANSLATION-UNIT))) (CL:SETF (%THE-OBJECT SELF-000) CLASS)
   (CL:LET* ((TEMPUNIT SELF-000))
    (CL:LET* ((*CURRENT-STREAM* NULL)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
     (CL:LET* ((SELF-001 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-001) NATIVE-STREAM)
      (CL:SETQ *CURRENT-STREAM* SELF-001))
     (IDL-OUTPUT-STATEMENT (IDL-TRANSLATE-DEFINE-NATIVE-CLASS-UNIT TEMPUNIT)))))
  :VOID)
...
(CL:DEFUN IDL-MAKE-CODE-OUTPUT-FILE-NAME (SOURCEFILE)
  (CL:RETURN-FROM IDL-MAKE-CODE-OUTPUT-FILE-NAME
   (CONCATENATE (FILE-NAME-WITHOUT-EXTENSION SOURCEFILE) (CONCATENATE *IDL-TRANSLATED-FILE-SUFFIX* ".sidl"))))
...
(CL:DEFUN IDL-OUTPUT-ALL-UNITS-TO-FILE (SOURCEFILE)
  (CL:LET*
   ((CODEOUTPUTFILE (IDL-MAKE-CODE-OUTPUT-FILE-NAME SOURCEFILE))
    (CODEOUTPUTSTREAM (NEW-OUTPUT-FILE-STREAM CODEOUTPUTFILE)) (GLOBALS NIL) (METHODS NIL) (VERBATIMSTATEMENTS NIL)
    (FORMS NIL))
   (CL:LET* ((*CURRENT-STREAM* CODEOUTPUTSTREAM)) (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
    ...
    (CL:LET* ((UNIT NULL) (ITER-000 (%THE-CONS-LIST *TRANSLATIONUNITS*)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ UNIT (%%VALUE ITER-000))
      (CL:LET* ((TEST-VALUE-000 (%CATEGORY UNIT)))
       (CL:COND
        ((CL:OR (CL:EQ TEST-VALUE-000 SYM-IDL-TRANSLATE-FILE-STELLA-METHOD)
          (CL:EQ TEST-VALUE-000 SYM-IDL-TRANSLATE-FILE-STELLA-PRINT-METHOD)
        ...
        ((CL:EQ TEST-VALUE-000 SYM-IDL-TRANSLATE-FILE-STELLA-VERBATIM)
         (CL:SETQ VERBATIMSTATEMENTS (CONS (%TRANSLATION UNIT) VERBATIMSTATEMENTS)))
        ((CL:EQ TEST-VALUE-000 SYM-IDL-TRANSLATE-FILE-STELLA-GLOBAL-VARIABLE)
         (CL:SETQ GLOBALS (CONS (%TRANSLATION UNIT) GLOBALS)))
        (CL:T (CL:SETQ FORMS (CONS (%TRANSLATION UNIT) FORMS)))))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Writing `" CODEOUTPUTFILE "'..." EOL)
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// " CODEOUTPUTFILE EOL EOL)
    (CL:WHEN (TRANSLATE-WITH-COPYRIGHT-HEADER?) (OUTPUT-COPYRIGHT-HEADER *CURRENT-STREAM*))
    (CL:LET* ((FORM NULL) (ITER-001 FORMS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ FORM (%%VALUE ITER-001)) (IDL-OUTPUT-STATEMENT FORM)
...
(CL:DEFUN STARTUP-IDL-TRANSLATE-FILE ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SYM-IDL-TRANSLATE-FILE-STELLA-METHOD (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD" NULL 0))
    (CL:SETQ SYM-IDL-TRANSLATE-FILE-STELLA-PRINT-METHOD (INTERN-RIGID-SYMBOL-WRT-MODULE "PRINT-METHOD" NULL 0))
    (CL:SETQ SYM-IDL-TRANSLATE-FILE-STELLA-MACRO (INTERN-RIGID-SYMBOL-WRT-MODULE "MACRO" NULL 0))
    (CL:SETQ SYM-IDL-TRANSLATE-FILE-STELLA-VERBATIM (INTERN-RIGID-SYMBOL-WRT-MODULE "VERBATIM" NULL 0))
    (CL:SETQ SYM-IDL-TRANSLATE-FILE-STELLA-GLOBAL-VARIABLE (INTERN-RIGID-SYMBOL-WRT-MODULE "GLOBAL-VARIABLE" NULL 0))
    (CL:SETQ KWD-IDL-TRANSLATE-FILE-IDL (INTERN-RIGID-SYMBOL-WRT-MODULE "IDL" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "IDL-OUTPUT-ALL-UNITS-TO-FILE"
     "(DEFUN IDL-OUTPUT-ALL-UNITS-TO-FILE ((SOURCEFILE FILE-NAME)))" (CL:FUNCTION IDL-OUTPUT-ALL-UNITS-TO-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "IDL-TRANSLATE-FILE" "(DEFUN IDL-TRANSLATE-FILE ((FILENAME FILE-NAME)))"
     (CL:FUNCTION IDL-TRANSLATE-FILE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-IDL-TRANSLATE-FILE" "(DEFUN STARTUP-IDL-TRANSLATE-FILE () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-IDL-TRANSLATE-FILE) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-IDL-TRANSLATE-FILE-STELLA-STARTUP-IDL-TRANSLATE-FILE)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-IDL-TRANSLATE-FILE-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/STELLA")))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE "(DEFGLOBAL *IDL-TRANSLATED-FILE-SUFFIX* STRING \"\")")))
  :VOID)
