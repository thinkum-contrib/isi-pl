...
(CL:DEFUN SELECT-SYMBOL-OFFSET-TABLE (MODULE KINDOFSYM)
  ...
  (CL:ECASE KINDOFSYM
   (0
    (CL:RETURN-FROM SELECT-SYMBOL-OFFSET-TABLE (%SYMBOL-OFFSET-TABLE MODULE)))
   (1
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN (SHADOWED-SYMBOL? SYMBOL)
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000) (CONS (%HOME-CONTEXT SYMBOL) NIL))
...
symbols that are shadowed due to some :SHADOW declaration."
  ...
  (CL:LET*
   ...
   (CL:IF
    (CL:AND ENFORCESHADOWING? (CL:NOT (CL:EQ (%%REST VISIBLESYMBOLS) NIL)))
    ...
    (CL:WHEN (CL:NOT (CL:EQ MODULESYMBOL NULL))
     (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001)
       "Can't import, a symbol with name `" NAME "' already exists in `"
       MODULE "'")
...
(CL:DEFUN YIELD-UNIQUE-GENSYM-NAME (PREFIX MODULE)
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE
    (CL:NOT (CL:EQ (LOOKUP-SYMBOL-IN-MODULE GENSYM-NAME MODULE CL:NIL) NULL))
    DO
    (CL:SETQ GENSYM-NAME
     (YIELD-GENSYM-NAME PREFIX
      (CL:SETQ *GENSYM-COUNTER* (CL:1+ *GENSYM-COUNTER*)))))
...
(CL:DEFUN SYMBOL-TO-TYPE (SELF)
  ...
  (CL:RETURN-FROM SYMBOL-TO-TYPE
   (INTERN-SURROGATE-IN-MODULE (%SYMBOL-NAME SELF) (%HOME-CONTEXT SELF) CL:T)))
...
(CL:DEFMETHOD LOCAL-PRINT-NAME ((SELF GENERALIZED-SYMBOL) READABLE?)
  (CL:IF READABLE?
   (CL:RETURN-FROM LOCAL-PRINT-NAME
    (READABLE-SYMBOL-NAME (%SYMBOL-NAME SELF) (%CASE-SENSITIVE? *MODULE*)))
...
(CL:DEFUN VISIBLE-SYMBOL? (SELF)
  ...
  (CL:RETURN-FROM VISIBLE-SYMBOL?
   (CL:EQ SELF (LOOKUP-SYMBOL-IN-MODULE (%SYMBOL-NAME SELF) *MODULE* CL:NIL))))
...
(CL:DEFUN COMPUTE-SYMBOL-ESCAPE-CODE (NAME CASESENSITIVE?)
  ...
  (CL:LET*
   ((TABLE
    ...
    (ESCAPECODE 0) (MAXESCAPECODE 0) (FIRSTCHAR? CL:T) (COULDBENUMBER? CL:T))
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (CL:WHEN (CL:OR (CL:EQL CH #\\) (CL:EQL CH #\|))
        (%%PRINT-STREAM STREAM "\\"))
       (%%PRINT-STREAM STREAM CH) (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
     (%%PRINT-STREAM STREAM "|"))
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" TEST-VALUE-000
   ...
   (CL:COND
    ...
    ((CL:EQ TEST-VALUE-000 KWD-SYMBOLS-COMPLEX-ESCAPED)
     (CL:LET* ((S (NEW-OUTPUT-STRING-STREAM)))
      (PRINT-SYMBOL-NAME-READABLY NAME (%NATIVE-STREAM S) CASESENSITIVE?)
...
Symbol properties can be set with `setf'.\")" (CL:FUNCTION SYMBOL-PROPERTY)
    NULL)
