...
(CL:DECLAIM
  ...
  *STELLA-LOGICAL-STATE-NAMES* NULL-CHARACTER *CHARACTER-TYPE-TABLE*
  NULL-STRING-WRAPPER *TRANSIENTOBJECTS?*))

;;; (DEFUN (NEXT-FREE-OFFSET INTEGER) ...)

(CL:DEFUN NEXT-FREE-OFFSET (SELF)
  (CL:LET*
   ((TOPOFFSET (%TOP-SYMBOL-OFFSET SELF)) (FREEOFFSET (CL:1+ TOPOFFSET)))
...
(CL:PROGN (CL:DEFVAR SYMBOL-SYM 0) (CL:DECLAIM (CL:TYPE CL:FIXNUM SYMBOL-SYM)))

;;; (DEFCONSTANT SURROGATE-SYM ...)

(CL:PROGN (CL:DEFVAR SURROGATE-SYM 1) (CL:DECLAIM (CL:TYPE CL:FIXNUM SURROGATE-SYM)))

;;; (DEFCONSTANT KEYWORD-SYM ...)

(CL:PROGN (CL:DEFVAR KEYWORD-SYM 2) (CL:DECLAIM (CL:TYPE CL:FIXNUM KEYWORD-SYM)))
...
(CL:PROGN (CL:DEFVAR MODULE-SEPARATOR-STRING "/" "String containing character used to concatenate a module name
to a symbol or surrogate name to form a qualified name.") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING MODULE-SEPARATOR-STRING)))
...
(CL:DEFUN GET-KWD (OFFSET)
  ...
  (CL:RETURN-FROM GET-KWD
   (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *FIXED-KEYWORD-ARRAY*))
    OFFSET)))
...
(CL:DEFUN LOOKUP-RIGID-SYMBOL (NAME KINDOFSYM)
  ...
  (CL:LET* ((MODULE *MODULE*))
   (CL:WHEN (QUALIFIED-NAME? NAME)
    (CL:MULTIPLE-VALUE-SETQ (MODULE NAME)
...
(CL:DEFUN LOOKUP-SURROGATE (NAME)
  ...
  (CL:RETURN-FROM LOOKUP-SURROGATE
   (LOOKUP-RIGID-SYMBOL NAME SURROGATE-SYM)))
...
(CL:PROGN (CL:DEFVAR *CONTEXT-NUMBER-COUNTER* -2 "Contains the number of the last-created
context (all 'live' contexts are even-numbered).") (CL:DECLAIM (CL:TYPE CL:FIXNUM *CONTEXT-NUMBER-COUNTER*)))
...
(CL:PROGN (CL:DEFVAR *GENSYM-COUNTER* 0 "Incremented once for each gensym call.") (CL:DECLAIM (CL:TYPE CL:FIXNUM *GENSYM-COUNTER*)))

;;; (DEFGLOBAL *GENSYM-MASK* ...)

(CL:PROGN (CL:DEFVAR *GENSYM-MASK* "-000" "Defines the separator character for gensyms,
and the fill digits.") (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *GENSYM-MASK*)))
...
(CL:DEFUN INTERN-GENERALIZED-SYMBOL (NAME)
  ...
  (CL:COND
   ((QUALIFIED-NAME? NAME)
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL
     (INTERN-QUALIFIED-NAME NAME)))
   ((KEYWORD-NAME? NAME)
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL
     (INTERN-KEYWORD (REST NAME))))
   ((SURROGATE-NAME? NAME)
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL
     (INTERN-SURROGATE (REST NAME))))
...
(CL:PROGN (CL:DEFVAR *SYMBOL-ESCAPE-CODE-TABLE* STELLA::NULL-STRING) (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *SYMBOL-ESCAPE-CODE-TABLE*)))

;;; (DEFGLOBAL *CASE-SENSITIVE-SYMBOL-ESCAPE-CODE-TABLE* ...)

(CL:PROGN (CL:DEFVAR *CASE-SENSITIVE-SYMBOL-ESCAPE-CODE-TABLE* STELLA::NULL-STRING) (CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *CASE-SENSITIVE-SYMBOL-ESCAPE-CODE-TABLE*)))
...
(CL:DEFUN COMPUTE-SYMBOL-ESCAPE-CODE (NAME CASESENSITIVE?)
  ...
  (CL:LET*
   ((TABLE
    ...
    (ESCAPECODE 0) (MAXESCAPECODE 0) (FIRSTCHAR? TRUE)
    (COULDBENUMBER? TRUE))
          ...
          (CL:PROGN (CL:SETQ TOK_TOKENSTART_ -1)
           (CL:LOOP
            (CL:WHEN (CL:= TOK_CURSOR_ TOK_END_)
             (CL:COND
              ((CL:= TOK_NEXTSTATE_ -1) (CL:SETQ TOK_ENDOFTOKENS?_ TRUE))
...
(CL:DEFUN HELP-STARTUP-SYMBOLS2 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "GET-SYM"
    "(DEFUN (GET-SYM SYMBOL) ((OFFSET INTEGER)))" (CL:FUNCTION GET-SYM)
    NULL)
   (DEFINE-FUNCTION-OBJECT "GET-SGT"
    "(DEFUN (GET-SGT SURROGATE) ((OFFSET INTEGER)))" (CL:FUNCTION GET-SGT)
    NULL)
   (DEFINE-FUNCTION-OBJECT "GET-KWD"
    "(DEFUN (GET-KWD KEYWORD) ((OFFSET INTEGER)))" (CL:FUNCTION GET-KWD)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "LOOKUP-SYMBOL"
    "(DEFUN (LOOKUP-SYMBOL SYMBOL) ((NAME STRING)) :PUBLIC? TRUE)"
    (CL:FUNCTION LOOKUP-SYMBOL) NULL)
   (DEFINE-FUNCTION-OBJECT "LOOKUP-SYMBOL-IN-MODULE"
    "(DEFUN (LOOKUP-SYMBOL-IN-MODULE SYMBOL) ((NAME STRING) (MODULE MODULE) (LOCAL? BOOLEAN)) :PUBLIC? TRUE)"
    (CL:FUNCTION LOOKUP-SYMBOL-IN-MODULE) NULL)
   (DEFINE-FUNCTION-OBJECT "LOOKUP-SURROGATE"
    "(DEFUN (LOOKUP-SURROGATE SURROGATE) ((NAME STRING)) :PUBLIC? TRUE)"
    (CL:FUNCTION LOOKUP-SURROGATE) NULL)
   (DEFINE-FUNCTION-OBJECT "LOOKUP-SURROGATE-IN-MODULE"
    "(DEFUN (LOOKUP-SURROGATE-IN-MODULE SURROGATE) ((NAME STRING) (MODULE MODULE) (LOCAL? BOOLEAN)) :PUBLIC? TRUE)"
    (CL:FUNCTION LOOKUP-SURROGATE-IN-MODULE) NULL)
   (DEFINE-FUNCTION-OBJECT "LOOKUP-KEYWORD"
    "(DEFUN (LOOKUP-KEYWORD KEYWORD) ((NAME STRING)) :PUBLIC? TRUE)"
...
(CL:DEFUN HELP-STARTUP-SYMBOLS3 ()
  (CL:PROGN
   ...
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD (KEYWORDIFY KEYWORD) ((SELF SYMBOL)) :PUBLIC? TRUE)"
    (CL:FUNCTION KEYWORDIFY) NULL)
   (DEFINE-METHOD-OBJECT
...
Symbol properties can be set with 'setf'.\")"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "Startup-Symbols") NULL-STRING-WRAPPER)))
  :VOID)
