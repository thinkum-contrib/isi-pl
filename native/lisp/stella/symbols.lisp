...
(CL:DEFUN LOOKUP-RIGID-SYMBOL-OFFSET-WRT-MODULE (NAME MODULE KINDOFSYM)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((SUPER NULL) (ITER-000 (ADVANCE (VISIBLE-MODULES MODULE) 1)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO
...
(CL:DEFUN YIELD-UNIQUE-GENSYM-NAME (PREFIX MODULE)
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE
    (CL:NOT
     (CL:EQ (LOOKUP-SYMBOL-IN-MODULE GENSYM-NAME MODULE FALSE) NULL))
    DO
    (CL:SETQ GENSYM-NAME
     (YIELD-GENSYM-NAME PREFIX
      (CL:SETQ *GENSYM-COUNTER* (CL:1+ *GENSYM-COUNTER*)))))
...
(CL:DEFUN STRING-TO-SURROGATE (SELF)
  ...
  (CL:LET*
   ((SURROGATE (INTERN-SURROGATE-IN-MODULE SELF *MODULE* FALSE)))
   (CL:WHEN
  ...
  (CL:COND
   ((QUALIFIED-NAME? NAME)
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL
     (INTERN-QUALIFIED-NAME NAME TRUE)))
   ((KEYWORD-NAME? NAME)
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL
   ...
   (CL:T
    (CL:RETURN-FROM INTERN-GENERALIZED-SYMBOL (INTERN-SYMBOL NAME)))))
...
Symbol properties can be set with `setf'.\")"
   ...
   (CL:LET*
    ...
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
     ...
     (WRAP-STRING "_Startup-Symbols") NULL-STRING-WRAPPER)))
  :VOID)
