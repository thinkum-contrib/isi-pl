...
(CL:DEFMETHOD GET-STELLA-CONTEXT-SLOWLY ((SELF SYMBOL))
  (CL:RETURN-FROM GET-STELLA-CONTEXT-SLOWLY
   (GET-STELLA-CONTEXT-SLOWLY (RELATIVE-NAME SELF CL:NIL))))
       ...
       (CL:PROGN
        (CL:SETQ CONTEXT
         (GET-STELLA-CONTEXT-SLOWLY
          (CL:TRUNCATE (%WRAPPER-VALUE NAMESPEC))))))
     ...
     (CL:COND
      ...
      (CL:T
       (CL:LET* ((CONTEXTNAME (COERCE-TO-MODULE-NAME NAMESPEC CL:T)))
        ...
        (CL:WHEN (CL:NOT (CL:EQ CONTEXTNAME STELLA::NULL-STRING))
         (CL:SETQ CONTEXT (GET-STELLA-CONTEXT CONTEXTNAME CL:T))
         (CL:SETQ CASESENSITIVE?
          (%CASE-SENSITIVE? (%BASE-MODULE CONTEXT)))))))))
   (CL:if caseSensitive?
...
In CommonLisp, if the new context is case sensitive, then change
the readtable case to the value of CL-USER::*STELLA-CASE-SENSITIVE-READ-MODE*
[default = :INVERT], otherwise to :UPCASE.\")" (CL:FUNCTION %CCC)
