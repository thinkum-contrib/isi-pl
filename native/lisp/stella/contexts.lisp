...
(CL:DECLAIM
  ...
  *CLASS-HIERARCHY-BOOTED?* NULL-STRING-WRAPPER *PRINTREADABLY?* TRUE
  *CONTEXT* *ROOT-MODULE* NIL FALSE))
   ...
   (CL:LET* ((TARGET *CONTEXT*))
    ...
    (CL:LOOP WHILE
     ...
     (CL:IF
      (CL:AND
       (CL:LOGBITP 0
        (CL:THE CL:FIXNUM (%CONTEXT-NUMBER (%KEY KVCONS))))
...
the pre-existing value of the current context.  `cc' is a no-op if the
context reference cannot be successfully evaluated.\")"
     (CL:FUNCTION %CC) (CL:FUNCTION CC-EVALUATOR-WRAPPER))
...
In CommonLisp, if the new context is case sensitive, then change
the readtable case to :INVERT, otherwise to :UPCASE.\")"
     (CL:FUNCTION %CCC) (CL:FUNCTION CCC-EVALUATOR-WRAPPER))
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-CONTEXTS-STELLA-STARTUP-CONTEXTS)))
...
and deallocated in depth-first fashion, and that deallocation of
CS-VALUE entries is taken care of during context destruction.\")")
    (REGISTER-NATIVE-NAME SYM-CONTEXTS-STELLA-CC
     KWD-CONTEXTS-COMMON-LISP KWD-CONTEXTS-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-CONTEXTS-STELLA-CCC
     KWD-CONTEXTS-COMMON-LISP KWD-CONTEXTS-FUNCTION)))
  :VOID)
