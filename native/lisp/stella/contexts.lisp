...
(CL:DECLAIM
  ...
  *CLASS-HIERARCHY-BOOTED?* NULL-STRING-WRAPPER *PRINTREADABLY?* TRUE *CONTEXT*
  *ROOT-MODULE* NIL FALSE))

;;; (DEFGLOBAL *UNLINK-DISCARDED-CONTEXTS-ON-READ?* ...)

(CL:DEFVAR *UNLINK-DISCARDED-CONTEXTS-ON-READ?* FALSE
...
(CL:DEFVAR *UNLINK-DISCARDED-CONTEXTS-ON-WRITE?* FALSE
  "Eliminate pointers to discarded contexts while
   ...
   (CL:WHEN (CL:NOT (CL:EQ NAMESPEC NULL))
    (CL:COND
     ((SUBTYPE-OF-INTEGER? (SAFE-PRIMARY-TYPE NAMESPEC))
      (CL:PROGN
       (CL:SETQ CONTEXT (GET-STELLA-CONTEXT-SLOWLY (%WRAPPER-VALUE NAMESPEC)))))
     (CL:T
    ...
    (CL:COND
     ...
     (CL:T
      (CL:LET* ((CONTEXTNAME (COERCE-TO-MODULE-NAME NAMESPEC TRUE)))
       ...
       (CL:WHEN (CL:NOT (CL:EQ CONTEXTNAME STELLA::NULL-STRING))
        (CL:SETQ CONTEXT (GET-STELLA-CONTEXT CONTEXTNAME TRUE)))))))
   (CL:if (%case-sensitive? (%base-module context))
    ...
    (CL:COND
     ...
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-CONTEXTS-STELLA-WORLD)
      (CL:PROGN
       (CL:SETQ NAME
        (%WRAPPER-VALUE
         (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CONTEXTS-STELLA-WORLD-NAME
          NULL-STRING-WRAPPER)))
       (CL:IF (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER SELF)))
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000)) DO
      ...
      (CL:SETQ I I) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "   ")))
  ...
  (CL:LET* ((KVCONS (%THE-KV-LIST VALUE)) (CONTEXTNUMBER NULL-INTEGER))
   ...
   (CL:LET* ((TARGET *CONTEXT*)) (CL:SETQ CONTEXTNUMBER (%CONTEXT-NUMBER TARGET))
    (CL:LOOP WHILE
     ...
     DO
     (CL:IF
      (CL:AND (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER (%KEY KVCONS))))
       (CL:NOT *CONTEXT-BACKTRACKING-MODE*) *UNLINK-DISCARDED-CONTEXTS-ON-READ?*)
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:LET* ((*CONTEXT* CHILDCXT)) (CL:DECLARE (CL:SPECIAL *CONTEXT*))
     (CL:LET* ((CURRENTVALUE (ACCESS-IN-CONTEXT CSVALUE HOMECONTEXT FALSE)))
...
(CL:DEFUN STARTUP-CONTEXTS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *UNLINK-DISCARDED-CONTEXTS-ON-READ?* TRUE)
    (CL:SETQ *UNLINK-DISCARDED-CONTEXTS-ON-WRITE?* TRUE)
    (CL:SETQ *CONTEXT-BACKTRACKING-MODE* FALSE))
...
the readtable case to :INVERT, otherwise to :UPCASE.\")" (CL:FUNCTION %CCC)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-CONTEXTS"
     "(DEFUN STARTUP-CONTEXTS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-CONTEXTS)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CONTEXTS-STELLA-STARTUP-CONTEXTS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Contexts") NULL-STRING-WRAPPER)))
