...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* STANDARD-OUTPUT EOL NULL-INTEGER *CLASS-HIERARCHY-BOOTED?* NULL-STRING-WRAPPER
  *PRINTREADABLY?* *CONTEXT* *ROOT-MODULE* NIL))
...
(CL:DEFUN CS-VALUE? (SELF)
  (CL:RETURN-FROM CS-VALUE?
   (CL:AND (CL:NOT (CL:EQ SELF NULL)) (CL:EQ (PRIMARY-TYPE SELF) SGT-CONTEXTS-STELLA-CS-VALUE))))

;;; (DEFUN (SUBCONTEXT? BOOLEAN) ...)

(CL:DEFUN SUBCONTEXT? (SUBCONTEXT SUPERCONTEXT)
  (CL:RETURN-FROM SUBCONTEXT?
   (CL:OR (CL:EQ SUBCONTEXT SUPERCONTEXT) (MEMB? (%ALL-SUPER-CONTEXTS SUBCONTEXT) SUPERCONTEXT))))
    ...
    (CL:TAGBODY
     (CL:COND ((CL:= (%CONTEXT-NUMBER C) NUMBER) (CL:RETURN-FROM HELP-FIND-CONTEXT-BY-NUMBER C))
      ((CL:< (%CONTEXT-NUMBER C) NUMBER)
       (CL:RETURN-FROM HELP-FIND-CONTEXT-BY-NUMBER (HELP-FIND-CONTEXT-BY-NUMBER NUMBER (%CHILD-CONTEXTS C))))
      (CL:T (CL:GO :CONTINUE)))
     :CONTINUE)
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
   (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Couldn't find context with number `" NUMBER "'")
  ...
  (CL:LET* ((CONTEXT NULL))
   (CL:LET* ((CXT NULL) (ITER-000 (%THE-CONS-LIST (%CHILD-CONTEXTS *ROOT-MODULE*))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ CXT (%%VALUE ITER-000))
     (CL:WHEN (STRING-EQL? (CONTEXT-NAME CXT) SELF) (CL:RETURN-FROM GET-STELLA-CONTEXT-SLOWLY CXT))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
    ...
    (CL:LOOP WHILE (NEXT? ITER-001) DO (CL:SETQ CXT (%VALUE ITER-001))
     (CL:WHEN
      (CL:OR (STRING-EQL? (CONTEXT-NAME CXT) SELF)
       (CL:AND (ISA? CXT SGT-CONTEXTS-STELLA-MODULE) (CL:NOT (CL:EQ (%NICKNAMES CXT) NULL))
        (MEMBER? (%NICKNAMES CXT) (WRAP-STRING SELF))))
      (CL:WHEN (CL:NOT (CL:EQ CONTEXT NULL))
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "More than one context has the name or nickname `" SELF "': `"
         CONTEXT "' and `" CXT "'")
        (CL:ERROR (NEW-NO-SUCH-CONTEXT-EXCEPTION (THE-STRING-READER STREAM-000)))))
      (CL:SETQ CONTEXT CXT))))
   (CL:IF (CL:NOT (CL:EQ CONTEXT NULL)) (CL:RETURN-FROM GET-STELLA-CONTEXT-SLOWLY CONTEXT)
    (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "No context with name or nickname `" SELF "'")
...
(CL:DEFUN CHANGE-CONTEXT-SLOWLY (SELF)
  (CL:IF (CL:NOT (CL:EQ SELF NULL)) (CL:RETURN-FROM CHANGE-CONTEXT-SLOWLY (CHANGE-CONTEXT SELF))
    ...
    (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NAMESPEC)))
     (CL:COND
      ((SUBTYPE-OF-INTEGER? TEST-VALUE-000)
       (CL:PROGN (CL:SETQ CONTEXT (GET-STELLA-CONTEXT-SLOWLY (CL:TRUNCATE (%WRAPPER-VALUE NAMESPEC))))))
...
(CL:DEFUN PRINT-CONTEXT (SELF STREAM)
  (CL:LET* ((TYPESTRING STELLA::NULL-STRING) (NAME STELLA::NULL-STRING) (NUMBER (%CONTEXT-NUMBER SELF)))
   ...
   (CL:WHEN (CL:NOT *CLASS-HIERARCHY-BOOTED?*) (%%PRINT-STREAM STREAM "|MDL|" (%MODULE-NAME SELF))
    ...
    (CL:COND
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-CONTEXTS-STELLA-MODULE)
      (CL:PROGN (CL:SETQ NAME (%MODULE-FULL-NAME SELF))
       (CL:IF (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER SELF))) (CL:SETQ TYPESTRING "|DeLeTeD MDL|")
        (CL:SETQ TYPESTRING "|MDL|"))))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-CONTEXTS-STELLA-WORLD)
      (CL:PROGN
       (CL:SETQ NAME
        (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF) SYM-CONTEXTS-STELLA-WORLD-NAME NULL-STRING-WRAPPER)))
       (CL:IF (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER SELF))) (CL:SETQ TYPESTRING "|DeLeTeD WLD|")
   ...
   (CL:IF *PRINTREADABLY?*
    (CL:IF (CL:NOT (CL:EQ NAME STELLA::NULL-STRING)) (%%PRINT-STREAM STREAM NAME)
     (%%PRINT-STREAM STREAM "#<" TYPESTRING NUMBER ">"))
    (CL:IF (CL:NOT (CL:EQ NAME STELLA::NULL-STRING)) (%%PRINT-STREAM STREAM TYPESTRING NAME)
     (%%PRINT-STREAM STREAM TYPESTRING NUMBER))))
  :VOID)
...
(CL:DEFUN HELP-PRINT-CONTEXT-TREE (LIST LEVEL)
  ...
  (CL:LET* ((C NULL) (ITER-000 (%THE-CONS-LIST LIST)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000))
    ...
    (CL:LET*
     ((I NULL-INTEGER) (ITER-001 1) (UPPER-BOUND-000 LEVEL) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)))
     ...
     (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000)) DO (CL:SETQ I ITER-001) (CL:SETQ I I)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "   ") (CL:SETQ ITER-001 (CL:1+ ITER-001))))
    ...
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) EOL) (HELP-PRINT-CONTEXT-TREE (%CHILD-CONTEXTS C) (CL:1+ LEVEL))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  :VOID)
...
(CL:DEFUN ACCESS-IN-CONTEXT (VALUE HOMECONTEXT DONTINHERIT?)
  (CL:WHEN
   (CL:OR (CL:EQ VALUE NULL)
    (CL:NOT (CL:AND (CL:NOT (CL:EQ VALUE NULL)) (CL:EQ (PRIMARY-TYPE VALUE) SGT-CONTEXTS-STELLA-CS-VALUE))))
   (CL:IF DONTINHERIT? (CL:WHEN (CL:EQ *CONTEXT* HOMECONTEXT) (CL:RETURN-FROM ACCESS-IN-CONTEXT VALUE))
    (CL:WHEN
     (CL:OR (CL:EQ HOMECONTEXT NULL)
      (CL:OR (CL:EQ *CONTEXT* HOMECONTEXT) (MEMB? (%ALL-SUPER-CONTEXTS *CONTEXT*) HOMECONTEXT))
  ...
  (CL:LET* ((KVCONS (%THE-KV-LIST VALUE)) (CONTEXTNUMBER NULL-INTEGER)) (CL:DECLARE (CL:TYPE CL:FIXNUM CONTEXTNUMBER))
   (CL:LET* ((TARGET *CONTEXT*)) (CL:SETQ CONTEXTNUMBER (%CONTEXT-NUMBER TARGET))
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ KVCONS NULL)) (CL:< CONTEXTNUMBER (%CONTEXT-NUMBER (%KEY KVCONS)))) DO
     (CL:IF
      (CL:AND (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER (%KEY KVCONS)))) (CL:NOT *CONTEXT-BACKTRACKING-MODE*)
       *UNLINK-DISCARDED-CONTEXTS-ON-READ?*)
      (CL:IF (CL:NOT (CL:EQ (%REST KVCONS) NULL))
       (CL:LET* ((NEXTKVCONS (%REST KVCONS))) (CL:SETF (%KEY KVCONS) (%KEY NEXTKVCONS))
        (CL:SETF (%VALUE KVCONS) (%VALUE NEXTKVCONS)) (CL:SETF (%REST KVCONS) (%REST NEXTKVCONS)) (FREE NEXTKVCONS))
    ...
    (CL:WHEN (CL:EQ TARGET (%KEY KVCONS)) (CL:RETURN-FROM ACCESS-IN-CONTEXT (%VALUE KVCONS)))
    (CL:WHEN DONTINHERIT? (CL:RETURN-FROM ACCESS-IN-CONTEXT NULL)))
   (CL:LET* ((TARGET NULL) (ITER-000 (%ALL-SUPER-CONTEXTS *CONTEXT*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ TARGET (%%VALUE ITER-000))
...
(CL:DEFUN HELP-INSERT-A-CS-VALUE (KVCONS NEWVALUE TARGET OVERWRITE?)
  (CL:LET* ((CONTEXTNUMBER (%CONTEXT-NUMBER TARGET))) (CL:DECLARE (CL:TYPE CL:FIXNUM CONTEXTNUMBER))
   (CL:LOOP
    (CL:WHEN
     (CL:AND (CL:NOT *CONTEXT-BACKTRACKING-MODE*) *UNLINK-DISCARDED-CONTEXTS-ON-WRITE?*
      (CL:LOGBITP 0 (CL:THE CL:FIXNUM (%CONTEXT-NUMBER (%KEY KVCONS)))))
     (CL:IF (CL:NOT (CL:EQ (%REST KVCONS) NULL))
      (CL:LET* ((NEXTKVCONS (%REST KVCONS))) (CL:SETF (%KEY KVCONS) (%KEY NEXTKVCONS))
       (CL:SETF (%VALUE KVCONS) (%VALUE NEXTKVCONS)) (CL:SETF (%REST KVCONS) (%REST NEXTKVCONS)) (FREE NEXTKVCONS))
      (CL:PROGN (CL:SETF (%KEY KVCONS) TARGET) (CL:SETF (%VALUE KVCONS) NEWVALUE)
       (CL:RETURN-FROM HELP-INSERT-A-CS-VALUE))))
    (CL:COND
     ((CL:EQ (%KEY KVCONS) TARGET) (CL:WHEN OVERWRITE? (CL:SETF (%VALUE KVCONS) NEWVALUE))
      (CL:RETURN-FROM HELP-INSERT-A-CS-VALUE))
     ((CL:< (%CONTEXT-NUMBER (%KEY KVCONS)) CONTEXTNUMBER)
      (CL:SETF (%REST KVCONS) (KV-CONS (%KEY KVCONS) (%VALUE KVCONS) (%REST KVCONS))) (CL:SETF (%KEY KVCONS) TARGET)
      (CL:SETF (%VALUE KVCONS) NEWVALUE) (CL:RETURN-FROM HELP-INSERT-A-CS-VALUE))
     ((CL:EQ (%REST KVCONS) NULL) (CL:SETF (%REST KVCONS) (KV-CONS TARGET NEWVALUE NULL))
      (CL:RETURN-FROM HELP-INSERT-A-CS-VALUE))
...
(CL:DEFMETHOD INSERT-AT ((SELF CS-VALUE) CONTEXT NEWVALUE)
  (CL:WHEN (CL:EQ (%THE-KV-LIST SELF) NULL)
   (CL:LET* ((KVCONS (NEW-KV-CONS))) (CL:SETF (%KEY KVCONS) CONTEXT) (CL:SETF (%VALUE KVCONS) NEWVALUE)
...
(CL:DEFUN UPDATE-IN-CONTEXT (OLDVALUE NEWVALUE HOMECONTEXT COPYTOCHILDREN?)
  (CL:LET* ((CSVALUE NULL))
   (CL:IF (CL:AND (CL:NOT (CL:EQ OLDVALUE NULL)) (CL:EQ (PRIMARY-TYPE OLDVALUE) SGT-CONTEXTS-STELLA-CS-VALUE))
    (CL:SETQ CSVALUE OLDVALUE)
    (CL:IF (CL:AND (CL:EQ HOMECONTEXT *CONTEXT*) (CL:OR (EMPTY? (%CHILD-CONTEXTS *CONTEXT*)) (CL:NOT COPYTOCHILDREN?)))
     (CL:RETURN-FROM UPDATE-IN-CONTEXT NEWVALUE)
     (CL:PROGN (CL:SETQ CSVALUE (NEW-CS-VALUE))
      (CL:WHEN (CL:NOT (CL:EQ OLDVALUE NULL)) (INSERT-AT CSVALUE HOMECONTEXT OLDVALUE)))))
   (INSERT-AT CSVALUE *CONTEXT* NEWVALUE)
   (CL:WHEN COPYTOCHILDREN? (COPY-CURRENT-VALUE-TO-CHILDREN CSVALUE HOMECONTEXT NEWVALUE))
    ...
    (CL:LET* ((*CONTEXT* CHILDCXT)) (CL:DECLARE (CL:SPECIAL *CONTEXT*))
     (CL:LET* ((CURRENTVALUE (ACCESS-IN-CONTEXT CSVALUE HOMECONTEXT CL:NIL)))
      (CL:WHEN (CL:NOT (EQL? CURRENTVALUE PARENTVALUE))
       (HELP-INSERT-A-CS-VALUE (%THE-KV-LIST CSVALUE) CURRENTVALUE CHILDCXT CL:NIL))))
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  :VOID)
...
(CL:DEFUN STARTUP-CONTEXTS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-CONTEXTS-STELLA-CS-VALUE (INTERN-RIGID-SYMBOL-WRT-MODULE "CS-VALUE" NULL 1))
    (CL:SETQ SGT-CONTEXTS-STELLA-MODULE (INTERN-RIGID-SYMBOL-WRT-MODULE "MODULE" NULL 1))
    (CL:SETQ SYM-CONTEXTS-STELLA-CC (INTERN-RIGID-SYMBOL-WRT-MODULE "CC" NULL 0))
    (CL:SETQ KWD-CONTEXTS-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
    (CL:SETQ KWD-CONTEXTS-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
    (CL:SETQ SYM-CONTEXTS-STELLA-CCC (INTERN-RIGID-SYMBOL-WRT-MODULE "CCC" NULL 0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (DELETED? BOOLEAN) ((CONTEXT CONTEXT)) :PUBLIC? TRUE)" (CL:FUNCTION DELETED?)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "FIND-CONTEXT-BY-NUMBER"
     "(DEFUN (FIND-CONTEXT-BY-NUMBER CONTEXT) ((CONTEXTNUMBER INTEGER)))" (CL:FUNCTION FIND-CONTEXT-BY-NUMBER) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-STELLA-CONTEXT-SLOWLY CONTEXT) ((SELF STRING)))"
     (CL:FUNCTION GET-STELLA-CONTEXT-SLOWLY) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-STELLA-CONTEXT-SLOWLY CONTEXT) ((SELF INTEGER)))"
     (CL:FUNCTION GET-STELLA-CONTEXT-SLOWLY) NULL)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD (GET-STELLA-CONTEXT-SLOWLY CONTEXT) ((SELF SYMBOL)))"
     (CL:FUNCTION GET-STELLA-CONTEXT-SLOWLY) NULL)
    (DEFINE-FUNCTION-OBJECT "CHANGE-CONTEXT-SLOWLY" "(DEFUN (CHANGE-CONTEXT-SLOWLY CONTEXT) ((SELF CONTEXT)))"
...
[default = :INVERT], otherwise to :UPCASE.\")" (CL:FUNCTION %CCC) (CL:FUNCTION CCC-EVALUATOR-WRAPPER))
    (DEFINE-FUNCTION-OBJECT "PRINT-CONTEXT" "(DEFUN PRINT-CONTEXT ((SELF CONTEXT) (STREAM NATIVE-OUTPUT-STREAM)))"
     (CL:FUNCTION PRINT-CONTEXT) NULL)
    (DEFINE-FUNCTION-OBJECT "HELP-PRINT-CONTEXT-TREE"
     "(DEFUN HELP-PRINT-CONTEXT-TREE ((LIST (LIST OF CONTEXT)) (LEVEL INTEGER)))" (CL:FUNCTION HELP-PRINT-CONTEXT-TREE)
     NULL)
    (DEFINE-FUNCTION-OBJECT "PRINT-CONTEXT-TREE" "(DEFUN PRINT-CONTEXT-TREE ((ROOT CONTEXT)))"
    ...
    (DEFINE-METHOD-OBJECT "(DEFMETHOD INSERT-AT ((SELF CS-VALUE) (CONTEXT CONTEXT) (NEWVALUE OBJECT)) :PUBLIC? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-CONTEXTS" "(DEFUN STARTUP-CONTEXTS () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-CONTEXTS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-CONTEXTS-STELLA-STARTUP-CONTEXTS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-CONTEXTS-STELLA-METHOD-STARTUP-CLASSNAME
