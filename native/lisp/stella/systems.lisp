...
(CL:DECLAIM
  ...
  *CURRENT-STELLA-FEATURES* TRUE-WRAPPER FALSE-WRAPPER NIL-LIST
  STANDARD-ERROR *TRANSLATOROUTPUTLANGUAGE* *CURRENTFILE* *MODULE* TRUE
  FALSE *TRANSLATIONUNITS* *EVALUATIONTREE* *EVALUATIONPARENTTREE* EOL
  NIL *TRANSLATIONPHASE* *CURRENTSYSTEMDEFINITION* STANDARD-OUTPUT))
      ...
      (CL:LET* ((PLIST SELF-001))
       (CL:LET* ((VALUE-000 NULL))
        ...
        (CL:LET* ((OLDSYSTEM VALUE-000))
         (CL:LET*
          ((VALUE NULL) (KEY NULL) (ITER-001 (%THE-PLIST PLIST)))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
           ...
           (CL:LET* ((TEST-VALUE-001 KEY))
            (CL:COND
             ...
             ((CL:EQ TEST-VALUE-001 KWD-SYSTEMS-CARDINAL-MODULE)
              (CL:SETF (%CARDINAL-MODULE SYSTEM)
               (%WRAPPER-VALUE VALUE)))
  ...
  (CL:LET* ((VALUE-000 NULL))
   (CL:LET* ((S NULL) (ITER-000 (%THE-CONS-LIST *SYSTEMDEFINITIONS*)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ S (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (STRING-EQUAL? (%NAME S) NAME) (CL:SETQ VALUE-000 S)
      (CL:RETURN))))
    ...
    (CL:WHEN (CL:EQ SYSTEM NULL)
     (CL:LET*
      ...
      (CL:LET* ((VALUE-001 NULL))
       (CL:LET*
        ((S NULL) (ITER-001 (%THE-CONS-LIST *SYSTEMDEFINITIONS*)))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
          ...
          (CL:SETQ FILENAME
           (%WRAPPER-VALUE
            (JAVA-TRANSLATE-CLASS-NAMESTRING
             (WRAP-STRING
              (CONCATENATE "Startup-" (%WRAPPER-VALUE F)))))))
         (CL:T
          (CL:SETQ FILENAME
      ...
      (CL:COND
       ((CL:EQ TYPE KWD-SYSTEMS-JAVA)
        (CL:WHEN
         ...
         (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
          "No cardinal module defined for system `" (%NAME SYSTEM) "'"
          EOL EOL)
        ...
        (CL:LET*
         ...
         (CL:LOOP WHILE (NEXT? ITER-001) DO
          ...
          (CL:SETQ FILENAME
           (JAVA-MAKE-CODE-OUTPUT-FILE-NAME CLASS FALSE))
          (HELP-GET-SYSTEM-FILES FILENAME FILES PROBEFILES?)))
        (CL:SETQ FILENAME
  ...
  (CL:LET* ((SYSTEM (GET-SYSTEM-DEFINITION SYSTEMNAME)))
   ...
   (CL:LET*
    ((F NULL)
     (ITER-000
      (%THE-CONS-LIST
       (GET-SYSTEM-FILES SYSTEM KWD-SYSTEMS-LISP TRUE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ F (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
   ...
   (CL:LET*
    ((F NULL)
     (ITER-001
      (%THE-CONS-LIST
       (GET-SYSTEM-FILES SYSTEM KWD-SYSTEMS-JAVA TRUE))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
...
(CL:DEFUN COMPUTE-OPTIMIZATION-LEVELS (SYSTEM PRODUCTIONMODE?)
  (CL:LET* ((SETTINGS NULL))
   (CL:IF PRODUCTIONMODE?
    (CL:SETQ SETTINGS (%PRODUCTION-SETTINGS SYSTEM))
...
(CL:DEFUN VET-OPTIONS (PLIST LEGALOPTIONS)
  (CL:LET* ((PROPERTYLIST NULL))
   ...
   (CL:LET*
    ((VALUE NULL) (KEY NULL) (ITER-000 (%THE-PLIST PROPERTYLIST)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      (CL:AND LOADSYSTEM?
       (LOAD-SYSTEM SYSTEMNAME LANGUAGE
        KWD-SYSTEMS-FORCE-RECOMPILATION?
  ...
  (CL:LET* ((TEST-VALUE-000 FALSE))
   (CL:IF (STRING-EQUAL? NAME "STELLA") (CL:SETQ TEST-VALUE-000 TRUE)
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET*
      ((SYS NULL) (ITER-000 (%THE-CONS-LIST *SYSTEMDEFINITIONS*)))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       (CL:PROGN (CL:SETQ SYS (%%VALUE ITER-000))
        (CL:SETQ ITER-000 (%%REST ITER-000)))
       (CL:WHEN
        (CL:AND (STRING-EQUAL? (%NAME SYS) NAME) (%LOADED? SYS))
   ...
   (CL:LET* ((VALUE-000 NULL))
    (CL:LET* ((S NULL) (ITER-000 (%THE-CONS-LIST *SYSTEMDEFINITIONS*)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ S (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:WHEN (STRING-EQL? (%NAME S) NAME) (CL:SETQ VALUE-000 S)
       (CL:RETURN))))
...
(CL:DEFUN LOAD-PREPROCESSED-FILES (SYSTEMNAME)
  ...
  (CL:WHEN
   (CL:OR (CL:NOT (RUNNING-AS-LISP?))
    (CL:EQ (%PREPROCESSED-FILES (GET-SYSTEM-DEFINITION SYSTEMNAME))
     NULL))
         ...
         (CL:IF (CL:EQ COLLECT-000 NULL)
          (CL:PROGN
           (CL:SETQ COLLECT-000
            (CONS
             (WRAP-STRING
              (MAKE-FILE-NAME (%WRAPPER-VALUE F) KWD-SYSTEMS-STELLA
               TRUE))
             NIL))
        ...
        (CL:LET*
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
          (CL:PROGN (CL:SETQ F (%%VALUE ITER-002))
           (CL:SETQ ITER-002 (%%REST ITER-002)))
          (DELETE-FILE
           (MAKE-FILE-NAME (%WRAPPER-VALUE F) KWD-SYSTEMS-LISP
            TRUE)))))
       (CL:LET*
        ((STARTUPFN NULL)
         (ITER-003
...
(CL:DEFUN STARTUP-NAME-FROM-FILE (FILE)
  ...
  (CL:LET*
   ((LONGNAME (MAKE-FILE-NAME FILE KWD-SYSTEMS-STELLA TRUE))
    (INPUTSTREAM (NEW-INPUT-FILE-STREAM LONGNAME))
    (SAVEDMODULE *MODULE*) (SEENINMODULE? FALSE) (UNUSED? FALSE)
    (STARTUPFNNAME NULL))
        ...
        ((CL:EQ (SAFE-PRIMARY-TYPE TREE) SGT-SYSTEMS-STELLA-CONS)
         (CL:PROGN
          (CL:WHEN (CL:EQ (%%VALUE TREE) SYM-SYSTEMS-STELLA-DEFMODULE)
           ...
           (CL:SETQ MODULE
            (GET-STELLA-MODULE
             (COERCE-TO-MODULE-NAME (%%VALUE (%%REST TREE)) TRUE)
             TRUE))
    ...
    (CL:SETQ STARTUPFORMS
     (CONS
      (LIST* SYM-SYSTEMS-STELLA-IN-MODULE
       (WRAP-STRING (%MODULE-FULL-NAME (GET-CARDINAL-MODULE SYSTEM)))
       NIL)
      STARTUPFORMS))
    (CL:SETQ STARTUPFORMS
     (CONS
       ...
       (CL:SETQ STARTUPFORMS
        (CONS
         (LIST* SYM-SYSTEMS-STELLA-STARTUP-TIME-PROGN
          ...
          (LIST* SYM-SYSTEMS-STELLA-SETQ
           SYM-SYSTEMS-STELLA-*STARTUP-TIME-PHASE* (WRAP-INTEGER 999)
           NIL)
          NIL)
         STARTUPFORMS)))))
   ...
   (CL:LET* ((ALWAYS?-000 TRUE))
    (CL:LET*
     ((I NULL-INTEGER) (ITER-000 0)
      (UPPER-BOUND-000
       (CL:1- (CL:IF (DEFINED? HASHPOS) HASHPOS LENGTH))))
...
`evaluate' to each of them.\" :COMMAND? TRUE :PUBLIC? TRUE)"
     (CL:FUNCTION %LOAD-FILE)
     (CL:FUNCTION LOAD-FILE-EVALUATOR-WRAPPER))
