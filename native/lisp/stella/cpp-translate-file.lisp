...
(CL:DEFVAR SYM-CPP-TRANSLATE-FILE-STELLA-MAIN NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *CPP-CURRENT-UNIT-MODULE* *CURRENTFILE* TRUE-WRAPPER
  *TRANSLATIONUNITS* NULL-INTEGER *CHARACTER-TYPE-TABLE* *MODULE* NIL
  *CURRENTSYSTEMDEFINITION* TRUE EOL FALSE *TRANSLATOROUTPUTLANGUAGE*
  STANDARD-OUTPUT))
...
(CL:DEFUN CPP-STANDALONE-OUTPUT-IMPORT-DECLARATIONS (HEADERFILENAME)
  (CPP-OUTPUT-INCLUDE-DIRECTIVE
   (MAKE-FILE-NAME "cpp-standalone" KWD-CPP-TRANSLATE-FILE-CPP-HEADER TRUE) TRUE)
...
(CL:DEFUN CPP-RELATIVIZE-HEADER-FILE (HEADERFILE SYSTEMNAME)
  ...
  (CL:LET*
   ...
   (CL:DECLARE
    (CL:SPECIAL *CURRENTSYSTEMDEFINITION* *CURRENTSYSTEMDEFINITIONSUBDIRECTORY*))
...
(CL:DEFUN CPP-OUTPUT-CLASS-HEAD-DECLARATIONS (CLASSES)
  (CL:WHEN (CL:NOT (CL:EQ CLASSES NIL))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// Class headers:" EOL))
...
(CL:DEFUN CPP-OUTPUT-NAMESPACE-HEADER (MODULE)
  ...
  (CL:LET*
   ((USEE NULL) (ITER-000 (REVERSE (%%REST (CONSIFY (VISIBLE-MODULES MODULE))))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "using namespace "
      (CPP-YIELD-NAMESPACE-PREFIX-FROM-MODULE USEE) ";" EOL)))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
   (CPP-OUTPUT-STATEMENT (%TRANSLATION UNIT)) (FREE *CURRENT-STREAM*))
  :VOID)
...
(CL:DEFUN CPP-SORT-UNITS-FOR-HEADER-FILE (FILEUNITS)
  (CL:LET*
   ((GLOBALS NIL) (AUXILIARYGLOBALS NIL) (METHODS NIL) (INCLUDESTATEMENTS NIL)
    (CLASSES NIL) (MAIN NIL))
     ...
     (CL:LET* ((TEST-VALUE-000 (%CATEGORY UNIT)))
      (CL:COND
       ((CL:OR (CL:EQ TEST-VALUE-000 SYM-CPP-TRANSLATE-FILE-STELLA-METHOD)
        ...
        (CL:IF (CPP-UNIT-DEFINES-MAIN? UNIT) (CL:SETQ MAIN (CONS UNIT MAIN))
   ...
   (CL:RETURN-FROM CPP-SORT-UNITS-FOR-HEADER-FILE
    (LIST (REVERSE INCLUDESTATEMENTS) (REVERSE CLASSES) (REVERSE GLOBALS)
     (REVERSE METHODS) (REVERSE AUXILIARYGLOBALS) MAIN))))
...
(CL:DEFUN CPP-OUTPUT-ALL-UNITS-TO-FILE (FILENAME)
  (CL:LET*
   ((CODEOUTPUTFILE
    ...
    (AUXILIARYGLOBALS (FIFTH HEADERUNITS)) (MAIN (%%VALUE (NTH HEADERUNITS 5)))
    (CURRENTNAMESPACEMODULE NULL) (CURRENTUNITMODULE NULL))
   (CL:LET* ((*CURRENT-STREAM* HEADEROUTPUTSTREAM))
    (CL:DECLARE (CL:SPECIAL *CURRENT-STREAM*))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Writing `" HEADEROUTPUTFILE
     "'..." EOL)
    (CPP-OUTPUT-FILE-HEADER HEADEROUTPUTSTREAM HEADEROUTPUTFILE)
    (CL:LET* ((BUCKET NULL) (ITER-000 (%THE-CONS-LIST HEADERUNITS)))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      (CL:PROGN (CL:SETQ BUCKET (%%VALUE ITER-000))
       (CL:SETQ ITER-000 (%%REST ITER-000)))
      (CL:LET* ((I NULL-INTEGER) (UNIT NULL) (ITER-001 BUCKET) (ITER-002 1))
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CL:WHEN
         (CL:AND (CL:NOT (CL:EQ (%THE-OBJECT UNIT) NULL))
          (CL:NOT
           (CL:EQ (%CATEGORY UNIT) SYM-CPP-TRANSLATE-FILE-STELLA-VERBATIM)))
    ...
    (CL:LET* ((UNIT NULL) (ITER-004 AUXILIARYGLOBALS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-004 NIL)) DO
      (CL:PROGN (CL:SETQ UNIT (%%VALUE ITER-004))
...
(CL:DEFUN CPP-TRANSLATE-WALKED-SYSTEM-UNITS (SYSTEMUNITS)
  (CL:LET* ((*CURRENTFILE* STELLA::NULL-STRING) (*TRANSLATIONUNITS* NULL))
   ...
   (CL:LET* ((UNITS NULL) (FILE NULL) (ITER-000 (%THE-KV-LIST SYSTEMUNITS)))
...
(CL:DEFUN CPP-YIELD-SYSTEM-HEADER-FILE-GUARD (SYSTEMNAME)
  ...
  (CL:RETURN-FROM CPP-YIELD-SYSTEM-HEADER-FILE-GUARD
   (CONCATENATE "_STELLA_" (STRING-UPCASE SYSTEMNAME) "_SYSTEM_HH")))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING) CL:SIMPLE-STRING)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%REQUIRED-SYSTEMS SYSTEM) NULL))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// Required systems:"
      EOL)
    ...
    (CL:LET* ((CURRENTNAMESPACEMODULE NULL))
     (CL:LET* ((CLASS NULL) (ITER-001 (ALL-CLASSES *MODULE* TRUE)))
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL EOL))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "// System components:"
     EOL)
    ...
    (CL:LET*
     ((FILE NULL)
      (ITER-002
       (FILES-PLUS-SYSTEM-STARTUP (SYSTEM-DEFINITION-SOURCE-FILES SYSTEM))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
      (CL:PROGN (CL:SETQ FILE (%%VALUE ITER-002))
       (CL:SETQ ITER-002 (%%REST ITER-002)))
      (CPP-OUTPUT-INCLUDE-DIRECTIVE
       (CPP-RELATIVIZE-HEADER-FILE
        (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-CPP-TRANSLATE-FILE-CPP-HEADER
         TRUE)
        SYSTEMNAME)
       FALSE)))
...
(CL:DEFUN CPP-YIELD-SYSTEMS-ROOT-DIRECTORY ()
  (CL:LET*
   ...
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *CURRENTSYSTEMDEFINITIONSUBDIRECTORY*))
   (CL:IF (CL:NOT (CL:EQ *CURRENTSYSTEMDEFINITION* NULL))
    (CL:PROGN (CL:RETURN-FROM CPP-YIELD-SYSTEMS-ROOT-DIRECTORY ".."))
    (CL:WARN "Can't find a system named STELLA~%")))
  (CL:RETURN-FROM CPP-YIELD-SYSTEMS-ROOT-DIRECTORY STELLA::NULL-STRING))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION () CL:SIMPLE-STRING) CPP-YIELD-SYSTEMS-ROOT-DIRECTORY))
   ...
   (CL:IF (CL:NOT (CL:EQ *CURRENTSYSTEMDEFINITION* NULL))
    (CL:PROGN
     (CL:RETURN-FROM CPP-YIELD-STELLA-ROOT-DIRECTORY
      (CONCATENATE (CPP-YIELD-SYSTEMS-ROOT-DIRECTORY) "/"
       (FILE-BASE-NAME
        (DIRECTORY-FILE-NAME
         (DIRECTORY-PARENT-DIRECTORY
          (MAKE-FILE-NAME "dummy" KWD-CPP-TRANSLATE-FILE-CPP-CODE TRUE) 1))))))
    (CL:WARN "Can't find a system named STELLA~%")))
  (CL:RETURN-FROM CPP-YIELD-STELLA-ROOT-DIRECTORY STELLA::NULL-STRING))
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION () CL:SIMPLE-STRING) CPP-YIELD-STELLA-ROOT-DIRECTORY))

;;; (DEFUN CPP-OUTPUT-SYSTEM-MAKEFILE ...)

(CL:DEFUN CPP-OUTPUT-SYSTEM-MAKEFILE ()
  (CL:LET*
   ((MAKEFILE (MAKE-FILE-NAME "Makefile" KWD-CPP-TRANSLATE-FILE-STELLA TRUE))
    (TEMPLATEFILE (CONCATENATE (FILE-NAME-WITHOUT-EXTENSION MAKEFILE) ".in"))
   ...
   (CL:WHEN (CL:NOT (PROBE-FILE? TEMPLATEFILE))
    (CL:LET*
     ...
     (CL:IF (CL:NOT (CL:EQ *CURRENTSYSTEMDEFINITION* NULL))
      (CL:PROGN
       (CL:SETQ MAKEFILE
        (MAKE-FILE-NAME "Makefile-default" KWD-CPP-TRANSLATE-FILE-STELLA TRUE))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (%%PRINT-STREAM (%NATIVE-STREAM SYSTEMFILES)
       (FILE-NAME-WITHOUT-DIRECTORY
        (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-CPP-TRANSLATE-FILE-CPP-CODE
         TRUE))
       " ")))
...
(CL:DEFUN SUBSTITUTE-TEMPLATE-VARIABLES-IN-STRING (STRING VARIABLES)
  ...
  (CL:LET*
   ((INDEX 0) (LENGTH (CL:THE CL:FIXNUM (CL:LENGTH STRING))) (RESULT STRING)
    (VARIABLEEND 0) (VARIABLENAME STELLA::NULL-STRING) (VARIABLEVALUE NULL))
   ...
   (CL:LOOP
    (CL:TAGBODY
     ...
     (CL:SETQ INDEX
      (CL:+ VARIABLEEND 1 (CL:- (CL:THE CL:FIXNUM (CL:LENGTH RESULT)) LENGTH)))
...
(CL:DEFUN SUBSTITUTE-TEMPLATE-VARIABLES-TO-STREAM (TEMPLATESTREAM OUTPUTSTREAM VARIABLES)
  (CL:LET* ((LINE STELLA::NULL-STRING) (ITER-000 (LINES TEMPLATESTREAM)))
...
(CL:DEFUN SUBSTITUTE-TEMPLATE-VARIABLES-TO-MAKEFILE (TEMPLATEFILE OUTPUTFILE VARIABLES)
  ...
  (CL:LET* ((OUT (NEW-OUTPUT-FILE-STREAM OUTPUTFILE)))
   (%%PRINT-STREAM (%NATIVE-STREAM OUT)
    "# This file has been generated automatically.  Modify at your own risk!" EOL
    EOL)
   (SUBSTITUTE-TEMPLATE-VARIABLES-TO-STREAM (NEW-INPUT-FILE-STREAM TEMPLATEFILE)
    OUT VARIABLES)
   (FREE OUT))
  :VOID)
...
(CL:DEFUN HELP-STARTUP-CPP-TRANSLATE-FILE1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-CPP-TRANSLATE-FILE-STELLA-MAIN
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "_Startup-Cpp-Translate-File") NULL-STRING-WRAPPER)))
