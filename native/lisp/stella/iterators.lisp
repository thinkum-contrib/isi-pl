...
(CL:DECLAIM
  ...
  *GLOBAL-VARIABLE-LOOKUP-TABLE* FALSE-WRAPPER *SYMBOL-ARRAY* NIL TRUE FALSE))
...
or NULL if it is empty.  Destructively uses up the first iteration element."
  (CL:IF (NEXT? SELF) (CL:RETURN-FROM POP (%VALUE SELF))
  ...
  (CL:IF (CL:OR (CL:EQ VALUE NULL) (STANDARD-OBJECT? VALUE))
   (CL:LET* ((M NULL) (ITER-000 SELF))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ M (%VALUE ITER-000))
     (CL:WHEN (CL:EQ M VALUE) (CL:RETURN-FROM MEMBER? TRUE))))
...
(CL:DEFMETHOD NEXT? ((SELF DESTRUCTIVE-LIST-ITERATOR))
  (CL:LET* ((CURSOR (%LIST-ITERATOR-CURSOR SELF)))
...
(%%DEFCONSMETHOD ALLOCATE-DESTRUCTIVE-LIST-ITERATOR ((SELF CL:CONS))
  (CL:LET* ((ITERATOR (NEW-DESTRUCTIVE-LIST-ITERATOR)))
   (CL:SETF (%THE-CONS-LIST ITERATOR) SELF)
   (CL:SETF (%LIST-ITERATOR-CURSOR ITERATOR) SELF)
...
(CL:DEFMETHOD ALLOCATE-DESTRUCTIVE-LIST-ITERATOR ((SELF LIST))
  (CL:LET*
   ((ITERATOR (ALLOCATE-DESTRUCTIVE-LIST-ITERATOR (%THE-CONS-LIST SELF))))
   (CL:SETF (%THE-CONS-LIST SELF) NIL) (FREE SELF)
   (CL:RETURN-FROM ALLOCATE-DESTRUCTIVE-LIST-ITERATOR ITERATOR)))
...
(CL:DEFMETHOD EMPTY? ((SELF ALL-PURPOSE-ITERATOR))
  (CL:WHEN (CL:EQ (%ITERATOR-EMPTY-CODE SELF) NULL)
   (CL:ERROR (NEW-STELLA-EXCEPTION "Missing 'empty?' method")))
...
(CL:DEFMETHOD TERMINATE-ALL-PURPOSE-ITERATOR? ((SELF ALL-PURPOSE-ITERATOR))
  (CL:WHEN (CL:NOT (CL:EQ (%ITERATOR-NESTED-ITERATOR SELF) NULL))
...
(CL:DEFUN FILTERED-NESTED-ITERATOR-NEXT? (SELF)
  (CL:LET* ((NESTEDITERATOR (%ITERATOR-NESTED-ITERATOR SELF)) (VALUE NULL))
   (CL:LOOP WHILE (NEXT? NESTEDITERATOR) DO
    (CL:SETQ VALUE (%VALUE NESTEDITERATOR))
    (CL:WHEN (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) VALUE SELF)
...
(CL:DEFUN ITERATOR-CONS-LIST-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-CONS-LIST SELF)))
   (CL:WHEN (CL:EQ CURSOR NIL) (CL:RETURN-FROM ITERATOR-CONS-LIST-NEXT? FALSE))
   (CL:SETF (%VALUE SELF) (%%VALUE CURSOR))
   (CL:SETF (%ITERATOR-CONS-LIST SELF) (%%REST CURSOR))
   (CL:RETURN-FROM ITERATOR-CONS-LIST-NEXT? TRUE)))

;;; (DEFUN (ITERATOR-CONS-LIST-EMPTY? BOOLEAN) ...)

(CL:DEFUN ITERATOR-CONS-LIST-EMPTY? (SELF)
  (CL:RETURN-FROM ITERATOR-CONS-LIST-EMPTY?
   (CL:EQ (%ITERATOR-CONS-LIST SELF) NIL)))

;;; (DEFUN (FILTERED-CONCATENATED-ITERATOR-NEXT? BOOLEAN) ...)

(CL:DEFUN FILTERED-CONCATENATED-ITERATOR-NEXT? (SELF)
  (CL:LET*
   ((NESTEDITERATOR (%ITERATOR-NESTED-ITERATOR SELF))
    (FILTERCODE (%ITERATOR-FILTER-CODE SELF)) (VALUE NULL))
   (CL:LOOP
    (CL:LOOP WHILE (NEXT? NESTEDITERATOR) DO
     (CL:SETQ VALUE (%VALUE NESTEDITERATOR))
     (CL:WHEN
      (CL:IF (CL:NOT (CL:EQ FILTERCODE NULL)) (CL:FUNCALL FILTERCODE VALUE SELF)
       TRUE)
      (CL:SETF (%VALUE SELF) VALUE)
    ...
    (CL:IF (CL:EQ NESTEDITERATOR NULL) (CL:RETURN)
     (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF) NESTEDITERATOR)))
...
resulting iterator."
  (CL:LET* ((ITERATORLIST (CONS ITERATOR2 NIL)))
   ...
   (CL:LET* ((SELF-000 (NEW-ALL-PURPOSE-ITERATOR)))
    (CL:SETF (%ITERATOR-NESTED-ITERATOR SELF-000) ITERATOR1)
...
(CL:DEFMETHOD NEXT? ((SELF INTEGER-INTERVAL-ITERATOR))
  ...
  (CL:LET* ((UPPERBOUND (%UPPER-BOUND SELF)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM UPPERBOUND))
   (CL:SETF (%VALUE SELF) (%INTERVAL-CURSOR SELF))
   (CL:RETURN-FROM NEXT?
    (CL:OR (NULL? UPPERBOUND) (CL:<= (%INTERVAL-CURSOR SELF) UPPERBOUND)))))
...
(CL:DEFMETHOD REVERSE ((SELF INTEGER-INTERVAL-ITERATOR))
  (CL:RETURN-FROM REVERSE
   (REVERSE-INTERVAL (%LOWER-BOUND SELF) (%UPPER-BOUND SELF))))
...
(CL:DEFMETHOD NEXT? ((SELF REVERSE-INTEGER-INTERVAL-ITERATOR))
  (CL:IF (%FIRST-ITERATION? SELF) (CL:SETF (%FIRST-ITERATION? SELF) FALSE)
   (CL:SETF (%INTERVAL-CURSOR SELF) (CL:1- (%INTERVAL-CURSOR SELF))))
  (CL:LET* ((LOWERBOUND (%LOWER-BOUND SELF)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM LOWERBOUND))
   (CL:SETF (%VALUE SELF) (%INTERVAL-CURSOR SELF))
   (CL:RETURN-FROM NEXT?
    (CL:OR (NULL? LOWERBOUND) (CL:>= (%INTERVAL-CURSOR SELF) LOWERBOUND)))))
...
(CL:DEFMETHOD ALLOCATE-ITERATOR ((SELF CL:STRING))
  ...
  (CL:LET* ((SELF-000 (NEW-STRING-ITERATOR)))
   (CL:SETF (%THE-STRING SELF-000) SELF)
   (CL:LET* ((ITERATOR SELF-000)) (CL:SETF (%CURSOR ITERATOR) 0)
    (CL:SETF (%END ITERATOR) (CL:THE CL:FIXNUM (CL:LENGTH SELF)))
...
(CL:DEFMETHOD NEXT? ((SELF STRING-ITERATOR))
  (CL:LET* ((POS (%CURSOR SELF))) (CL:DECLARE (CL:TYPE CL:FIXNUM POS))
   (CL:WHEN (CL:< POS (%END SELF))
    ...
    (CL:SETF (%CURSOR SELF) (CL:1+ POS)) (CL:RETURN-FROM NEXT? TRUE))
      ...
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ MOD (%VALUE ITER-000))
       (CL:IF (CL:EQ COLLECT-000 NULL)
        ...
        (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS MOD NIL))
         (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))))))
   (CL:SETF (%ITERATOR-CONS-LIST ITERATOR) MODULESLIST)
   (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) NEXTCODE)
...
(CL:DEFUN SELECTED-META-OBJECT? (SELF OBJECTMODULE)
  (CL:LET* ((MODULES (%ITERATOR-CONS-LIST SELF)))
   (CL:IF (CL:EQ MODULES NIL) (CL:RETURN-FROM SELECTED-META-OBJECT? TRUE)
    (CL:IF (CL:EQ (%%REST MODULES) NIL)
     (CL:IF (CL:EQ OBJECTMODULE (%%VALUE MODULES))
      (CL:RETURN-FROM SELECTED-META-OBJECT? TRUE)
      (CL:RETURN-FROM SELECTED-META-OBJECT? FALSE))
...
(CL:DEFUN ALL-SYMBOLS-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*)) CURSOR))
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ SYMBOL NULL))
      (SELECTED-META-OBJECT? SELF (%HOME-CONTEXT SYMBOL)))
     (CL:SETF (%VALUE SELF) SYMBOL)
...
If 'module' is null, return all symbols interned everywhere."
  (CL:RETURN-FROM ALL-SYMBOLS
   (ALLOCATE-ALL-META-OBJECTS-ITERATOR
    (CL:1+ (%TOP-SYMBOL-OFFSET *SYMBOL-ARRAY*)) (CL:FUNCTION ALL-SYMBOLS-NEXT?)
    MODULE LOCAL?)))
...
(CL:DEFUN FILTER-PUBLIC-METHOD-SLOT? (SELF ITERATOR)
  ...
  (CL:RETURN-FROM FILTER-PUBLIC-METHOD-SLOT?
   (CL:AND (%SLOT-PUBLIC? SELF) (METHOD-SLOT? SELF))))
...
(CL:DEFUN FILTER-NON-PARAMETER-SLOT? (SELF ITERATOR)
  ...
  (CL:RETURN-FROM FILTER-NON-PARAMETER-SLOT?
   (CL:NOT
    (MEMBER? (CLASS-PARAMETERS (%SURROGATE-VALUE (%SLOT-OWNER SELF)))
     (%SLOT-NAME SELF)))))
  ...
  (CL:LET* ((ALWAYS?-000 TRUE))
   (CL:LET* ((FILTER NULL) (ITER-000 (%ITERATOR-OBJECT ITERATOR)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ FILTER (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN (CL:NOT (CL:FUNCALL (%WRAPPER-VALUE FILTER) SELF ITERATOR))
...
(CL:DEFUN ALL-FUNCTIONS-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL) (FUNCTION NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*)) CURSOR))
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ SYMBOL NULL))
      (SELECTED-META-OBJECT? SELF (LOOKUP-FUNCTION-HOME-MODULE SYMBOL)))
     (CL:SETQ FUNCTION (LOOKUP-FUNCTION SYMBOL))
     (CL:WHEN
      (CL:AND (CL:NOT (CL:EQ FUNCTION NULL))
       (CL:OR (CL:EQ (%ITERATOR-FILTER-CODE SELF) NULL)
        (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) FUNCTION SELF)))
      (CL:SETF (%VALUE SELF) FUNCTION)
      (CL:SETF (%ITERATOR-INTEGER SELF) (CL:1+ CURSOR))
...
If 'module' is null, return all functions defined everywhere."
  (CL:RETURN-FROM ALL-FUNCTIONS
   (ALLOCATE-ALL-META-OBJECTS-ITERATOR
    (CL:1+ (%TOP-SYMBOL-OFFSET *SYMBOL-ARRAY*)) (CL:FUNCTION ALL-FUNCTIONS-NEXT?)
    MODULE LOCAL?)))
...
(CL:DEFUN ALL-VARIABLES-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL) (VARIABLE NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*)) CURSOR))
    (CL:WHEN
     ...
     (CL:WHEN
      (CL:AND (CL:NOT (CL:EQ VARIABLE NULL))
       ...
       (CL:OR (CL:EQ (%ITERATOR-FILTER-CODE SELF) NULL)
        (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) VARIABLE SELF)))
      (CL:SETF (%VALUE SELF) VARIABLE)
      (CL:SETF (%ITERATOR-INTEGER SELF) (CL:1+ CURSOR))
...
If 'module' is null, return all variables defined everywhere."
  (CL:RETURN-FROM ALL-VARIABLES
   (ALLOCATE-ALL-META-OBJECTS-ITERATOR
    (CL:1+ (%TOP-SYMBOL-OFFSET *SYMBOL-ARRAY*)) (CL:FUNCTION ALL-VARIABLES-NEXT?)
    MODULE LOCAL?)))

;;; (DEFUN (ALL-SURROGATES-NEXT? BOOLEAN) ...)

(CL:DEFUN ALL-SURROGATES-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SURROGATE NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SURROGATE
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SURROGATE-ARRAY*)) CURSOR))
    (CL:WHEN
     (CL:AND (CL:NOT (CL:EQ SURROGATE NULL))
      (SELECTED-META-OBJECT? SELF (%HOME-CONTEXT SURROGATE)))
     (CL:SETF (%VALUE SELF) SURROGATE)
     (CL:SETF (%ITERATOR-INTEGER SELF) (CL:1+ CURSOR))
...
(CL:DEFUN ALL-CLASSES-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SURROGATE NULL) (VALUE NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SURROGATE
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SURROGATE-ARRAY*)) CURSOR))
    (CL:WHEN
     ...
     (CL:WHEN
      (CL:AND (CL:NOT (CL:EQ VALUE NULL)) (STELLA-CLASS? VALUE)
       (CL:EQ (%CLASS-TYPE VALUE) SURROGATE))
      (CL:SETF (%VALUE SELF) VALUE)
      (CL:SETF (%ITERATOR-INTEGER SELF) (CL:1+ CURSOR))
...
(CL:DEFUN SLOTS-NEXT? (SELF)
  (CL:LET* ((SLOT NULL))
   (CL:LOOP
    (CL:LOOP
     (CL:LET* () (CL:SETQ SLOT (%%VALUE (%ITERATOR-CONS-LIST SELF)))
      (CL:WHEN (CL:EQ SLOT NULL) (CL:RETURN)))
     (CL:SETF (%ITERATOR-CONS-LIST SELF) (%%REST (%ITERATOR-CONS-LIST SELF)))
     (CL:WHEN
      (CL:OR (CL:EQ (%ITERATOR-FILTER-CODE SELF) NULL)
       (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) SLOT SELF))
      (CL:SETF (%VALUE SELF) SLOT) (CL:RETURN-FROM SLOTS-NEXT? TRUE)))
    (CL:WHEN (CL:NOT (NEXT? (%ITERATOR-NESTED-ITERATOR SELF)))
     (CL:RETURN-FROM SLOTS-NEXT? FALSE))
    (CL:SETF (%ITERATOR-CONS-LIST SELF)
     (%THE-CONS-LIST
      (%CLASS-LOCAL-SLOTS (%VALUE (%ITERATOR-NESTED-ITERATOR SELF))))))))
...
If 'module' is null, return all methods interned everywhere."
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   (CL:SETF (%ITERATOR-NEXT-CODE ITERATOR) (CL:FUNCTION SLOTS-NEXT?))
   (CL:SETF (%ITERATOR-NESTED-ITERATOR ITERATOR) (ALL-CLASSES MODULE LOCAL?))
   (CL:SETF (%ITERATOR-CONS-LIST ITERATOR) NIL)
   ...
   (CL:LOOP
    ...
    (CL:WHEN
     (CL:AND (METHOD-SLOT? SLOT)
      (CL:OR (CL:EQ (%ITERATOR-FILTER-CODE SELF) NULL)
       (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) SLOT SELF)))
     (CL:SETF (%VALUE SELF) SLOT) (CL:RETURN-FROM CLASS-METHODS-NEXT? TRUE)))
...
(CL:DEFUN CLASS-METHODS (CLASS FILTERS)
  (CL:LET* ((ITERATOR (NEW-ALL-PURPOSE-ITERATOR)))
   ...
   (CL:SETF (%ITERATOR-CONS-LIST ITERATOR)
    (%THE-CONS-LIST (%CLASS-LOCAL-SLOTS CLASS)))
   (CL:SETF (%ITERATOR-OBJECT ITERATOR) FILTERS)
   (CL:SETF (%ITERATOR-FILTER-CODE ITERATOR) (CL:FUNCTION FILTER-SLOT?))
   ...
   (CL:LOOP
    ...
    (CL:WHEN
     (CL:AND (ISA? SLOT SGT-ITERATORS-STELLA-TABLE)
      (CL:OR (CL:EQ (%ITERATOR-FILTER-CODE SELF) NULL)
       (CL:FUNCALL (%ITERATOR-FILTER-CODE SELF) SLOT SELF)))
     (CL:SETF (%VALUE SELF) SLOT) (CL:RETURN-FROM CLASS-TABLES-NEXT? TRUE)))
   ...
   (CLASS-STORAGE-SLOTS CLASS
    (CONS (WRAP-FUNCTION-CODE (CL:FUNCTION FILTER-PUBLIC-SLOT?))
     (CONS (WRAP-FUNCTION-CODE (CL:FUNCTION FILTER-NON-PARAMETER-SLOT?))
      (CONS (WRAP-FUNCTION-CODE (CL:FUNCTION FILTER-NON-EXTERNAL-SLOT?))
       (CONS (WRAP-FUNCTION-CODE (CL:FUNCTION FILTER-NON-AUXILIARY-SLOT?))
        NIL)))))))
...
for an iterator (they occupy the same position as a collection
within a 'foreach' statement).\" :PUBLIC? TRUE)" (CL:FUNCTION ALLOCATE-ITERATOR)
    NULL)
   (DEFINE-METHOD-OBJECT
...
(CL:DEFUN STARTUP-ITERATORS ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-ITERATORS1)
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-ITERATORS"
     "(DEFUN STARTUP-ITERATORS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-ITERATORS)
     NULL)
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-ITERATORS-STELLA-STARTUP-ITERATORS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-ITERATORS-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Iterators") NULL-STRING-WRAPPER)))
