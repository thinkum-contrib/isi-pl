...
(CL:DEFUN FILTERED-NESTED-ITERATOR-NEXT? (SELF)
  (CL:LET*
   ((NESTEDITERATOR (%ITERATOR-NESTED-ITERATOR SELF)) (VALUE NULL))
...
(CL:DEFUN ITERATOR-CONS-LIST-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-CONS-LIST SELF)))
   (CL:WHEN (CL:EQ CURSOR NIL)
    (CL:RETURN-FROM ITERATOR-CONS-LIST-NEXT? FALSE))
   (CL:SETF (%VALUE SELF) (%%VALUE CURSOR))
    ...
    (CL:LOOP WHILE (NEXT? NESTEDITERATOR) DO
     ...
     (CL:WHEN
      (CL:IF (CL:NOT (CL:EQ FILTERCODE NULL))
       (CL:FUNCALL FILTERCODE VALUE SELF) TRUE)
      (CL:SETF (%VALUE SELF) VALUE)
...
(CL:DEFUN ALL-SYMBOLS-NEXT? (SELF)
  (CL:LET* ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL))
   ...
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*))
      CURSOR))
    (CL:WHEN
...
(CL:DEFUN ALL-FUNCTIONS-NEXT? (SELF)
  (CL:LET*
   ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL) (FUNCTION NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*))
    ...
    (CL:WHEN
     ...
     (CL:WHEN
      (CL:AND (CL:NOT (CL:EQ FUNCTION NULL))
       (SELECTED-META-OBJECT? SELF (HOME-MODULE FUNCTION))
...
(CL:DEFUN ALL-VARIABLES-NEXT? (SELF)
  (CL:LET*
   ((CURSOR (%ITERATOR-INTEGER SELF)) (SYMBOL NULL) (VARIABLE NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SYMBOL
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SYMBOL-ARRAY*))
...
(CL:DEFUN ALL-CLASSES-NEXT? (SELF)
  (CL:LET*
   ((CURSOR (%ITERATOR-INTEGER SELF)) (SURROGATE NULL) (VALUE NULL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM CURSOR))
   (CL:LOOP WHILE (CL:< CURSOR (%ITERATOR-SECOND-INTEGER SELF)) DO
    (CL:SETQ SURROGATE
     (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *SURROGATE-ARRAY*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-ITERATORS1)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Iterators") NULL-STRING-WRAPPER)))
