...
(CL:DECLAIM
  ...
  *GLOBAL-VARIABLE-LOOKUP-TABLE* FALSE-WRAPPER *SYMBOL-ARRAY* NULL-INTEGER
  NIL))
  ...
  (CL:LET* ((UPPERBOUND (%UPPER-BOUND SELF)))
   ...
   (CL:RETURN-FROM NEXT?
    (CL:OR (CL:= UPPERBOUND NULL-INTEGER)
     (CL:<= (%INTERVAL-CURSOR SELF) UPPERBOUND)))))
  ...
  (CL:LET* ((LOWERBOUND (%LOWER-BOUND SELF)))
   ...
   (CL:RETURN-FROM NEXT?
    (CL:OR (CL:= LOWERBOUND NULL-INTEGER)
     (CL:>= (%INTERVAL-CURSOR SELF) LOWERBOUND)))))
...
If 'module' is null, return all symbols interned everywhere."
  (CL:RETURN-FROM ALL-SYMBOLS
   (ALLOCATE-ALL-META-OBJECTS-ITERATOR
    (CL:1+ (%TOP-SYMBOL-OFFSET *SYMBOL-ARRAY*))
...
'n' elements in the (remainder of the) iteration.\")" (CL:FUNCTION ADVANCE)
    NULL)
   (DEFINE-METHOD-OBJECT
