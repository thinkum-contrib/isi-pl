...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-INTEGER *METHODBEINGWALKED*
  FALSE *SPECIALVARIABLESTACK* TRUE STANDARD-ERROR EOL NIL))
       ...
       (CL:SETQ VALUECLAUSES
        (CONS
         (CONS KEYVAR
          (CONS KEYTYPE
           (CONS
            (LIST* SYM-FOREACH-STELLA-SETQ KEYVAR (CONS KEYTREE NIL))
            NIL)))
         VALUECLAUSES)))
      (CL:T
       (CL:LET* ((*PRINTREADABLY?* TRUE))
       ...
       (CONS
        ...
        (CONS
         (LIST* SYM-FOREACH-STELLA-PROGN
          (LIST* SYM-FOREACH-STELLA-SETQ VALUEVAR
           (CONS
            (LIST* SYM-FOREACH-STELLA-NTH VECTORVAR
             (CONS INDEXVAR NIL))
            NIL))
  ...
  (CL:LET*
   ...
   (CL:LET* ((CLAUSE NULL) (ITER-000 ITERATORCLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (PUSH-VARIABLE-BINDING (%%VALUE CLAUSE)
      (%%VALUE (%%REST CLAUSE)))))
     ...
     (LIST* SYM-FOREACH-STELLA-SYS-FOREACH ITERATORCLAUSES
      (CONS VALUECLAUSES
       (CONS CONTINUATIONTEST
        (CONCATENATE
         (WALK-PREFIX-FOREACH-BODY ITERATORCLAUSES VALUECLAUSES
          WHERETEST DOACTIONS)
         NIL))))
    ...
    (LIST* SYM-FOREACH-STELLA-DEFUN
     ...
     (LIST* SYM-FOREACH-STELLA-LET
      ...
      (LIST* SYM-FOREACH-STELLA-WHILE
       (LIST* SYM-FOREACH-STELLA-NEXT?
        SYM-FOREACH-STELLA-NESTEDITERATOR NIL)
       ...
       (LIST* SYM-FOREACH-STELLA-WHEN WHERETEST
        (LIST*
         ...
         NIL))
       NIL)
      (LIST* SYM-FOREACH-STELLA-RETURN SYM-FOREACH-STELLA-FALSE NIL)
      NIL)
     NIL))
   ...
   (CL:RETURN-FROM WALK-SETOF-TREE
    (CL:VALUES
     (LIST* SYM-FOREACH-STELLA-VRLET
      ...
      (LIST* SYM-FOREACH-STELLA-SETF
       (LIST* SYM-FOREACH-STELLA-ITERATOR-NEXT-CODE ITERATORVARIABLE
        NIL)
