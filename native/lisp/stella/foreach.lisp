...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* *METHODBEINGWALKED*
  *SPECIALVARIABLESTACK* STANDARD-ERROR EOL))
...
(CL:DEFUN EXTRACT-ONE-GENERATOR-TERM (TAIL)
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:EQ (%%VALUE RESIDUE) SYM-FOREACH-STELLA-AS) DO
    (CL:SETQ TEMP RESIDUE) (CL:SETQ RESIDUE (%%REST RESIDUE))
    (CL:MULTIPLE-VALUE-SETQ (INCLAUSE RESIDUE)
    ...
    (CL:COND
     ...
     (CL:T
      (CL:IF (USE-VECTOR-STYLE-ITERATION? COLLECTIONBASETYPE)
       (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
        (YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR COLLECTIONTREE COLLECTIONTYPE
         KEYVAR VALUEVAR))
       (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
...
(CL:DEFUN USE-VECTOR-STYLE-ITERATION? (COLLECTIONTYPE)
  (CL:RETURN-FROM USE-VECTOR-STYLE-ITERATION?
   (CL:OR (CL:EQ COLLECTIONTYPE SGT-FOREACH-STELLA-STRING)
    ...
    (CL:AND
     (SAFE-SUBTYPE-OF? COLLECTIONTYPE SGT-FOREACH-STELLA-SEQUENCE-MIXIN)
     (CL:NOT
      (CL:EQ
       (SAFE-LOOKUP-SLOT (%SURROGATE-VALUE COLLECTIONTYPE)
  ...
  (CL:LET*
   ((UPPERBOUND (%%VALUE (%%REST (%%REST COLLECTIONTREE))))
    ...
    (KNOWNBOUNDED?
     (CL:AND (CL:NOT KNOWNUNBOUNDED?)
      (CL:OR (ISA? UPPERBOUND SGT-FOREACH-STELLA-NUMBER-WRAPPER)
   ...
   (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL
    (CL:VALUES
     (CONS
      (CONS ITERATORVAR
       (LIST* SGT-FOREACH-STELLA-INTEGER (%%VALUE (%%REST COLLECTIONTREE))
        NIL))
      (CONCATENATE
...
(CL:DEFUN YIELD-ON-CURSOR-CLAUSES (ONTREE)
  (CL:LET* ((ITERATORVAR (%%VALUE (%%REST ONTREE))))
   ...
   (CL:LET* ((COLLECTIONTREE NULL) (COLLECTIONTYPE NULL))
    ...
    (CL:WHEN
     (SUB-TYPE-SPEC-OF? COLLECTIONTYPE SGT-FOREACH-STELLA-ARGUMENT-LIST)
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
      (SIGNAL-TRANSLATION-ERROR)
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      ...
      (CL:LET* ((CLAUSE NULL) (ITER-000 (%%REST CURSORSTREE)))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
        (CL:PROGN (CL:SETQ CLAUSE (%%VALUE ITER-000))
         (CL:SETQ ITER-000 (%%REST ITER-000)))
        (CL:LET*
         ((ITERATORCLAUSES NULL) (VALUECLAUSES NULL)
  ...
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ITER-002 1)
     (UPPER-BOUND-000
      (CL:+ (LENGTH ITERATORCLAUSES) (LENGTH VALUECLAUSES))))
...
(CL:DEFUN WRAP-WHERE-TEST (WHERETEST ACTIONS)
  (CL:IF (CL:NOT (CL:EQ WHERETEST NULL))
   (CL:RETURN-FROM WRAP-WHERE-TEST
    (CONS
     (LIST* SYM-FOREACH-STELLA-WHEN WHERETEST (CONCATENATE ACTIONS NIL))
     NIL))
...
(CL:DEFUN YIELD-SETOF-NEXT-FUNCTION-NAME (VALUECLAUSES WHERETEST)
  (CL:LET*
   ((ONLYVALUECLAUSE (%%VALUE VALUECLAUSES))
    ...
    (VALUETYPE (%%VALUE (%%REST ONLYVALUECLAUSE)))
   ...
   (WALK-AUXILIARY-TREE
    (LIST* SYM-FOREACH-STELLA-DEFUN
     ...
     (CONS
      (LIST* SYM-FOREACH-STELLA-SELF
       SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR NIL)
      NIL)
     (LIST* SYM-FOREACH-STELLA-LET
      (LIST*
       (LIST* SYM-FOREACH-STELLA-NESTEDITERATOR
        ...
        NIL)
       (CONS VALUEVARIABLE
        (CONS VALUETYPE (CONS SYM-FOREACH-STELLA-NULL NIL)))
       NIL)
      (LIST* SYM-FOREACH-STELLA-WHILE
       (LIST* SYM-FOREACH-STELLA-NEXT? SYM-FOREACH-STELLA-NESTEDITERATOR
        NIL)
       (LIST* SYM-FOREACH-STELLA-SETQ VALUEVARIABLE
        (CONS
         (LIST* SYM-FOREACH-STELLA-VALUE SYM-FOREACH-STELLA-NESTEDITERATOR
          NIL)
         NIL))
       (LIST* SYM-FOREACH-STELLA-WHEN WHERETEST
        (LIST*
         (LIST* SYM-FOREACH-STELLA-SETF
          ...
          VALUEVARIABLE NIL)
         (LIST* SYM-FOREACH-STELLA-RETURN SYM-FOREACH-STELLA-TRUE NIL)
         NIL))
       NIL)
...
(CL:DEFUN WALK-SETOF-TREE (TREE)
  ...
  (CL:LET*
   ((ITERATORSSUBTREE (%%VALUE (%%REST TREE)))
    (ITERATORVARIABLE (LOCAL-GENSYM "ITER")) (TAIL NULL)
    (ITERATORCLAUSES NULL) (VALUECLAUSES NULL) (CONTINUATIONTEST NULL)
    (WHERETEST NULL))
     ...
     (LIST* SYM-FOREACH-STELLA-VRLET
      (CONS
       (CONS ITERATORVARIABLE
        (CONS
         (LIST* SYM-FOREACH-STELLA-NEW
          SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR NIL)
         NIL))
       NIL)
      (LIST* SYM-FOREACH-STELLA-SETF
       (LIST* SYM-FOREACH-STELLA-ITERATOR-NESTED-ITERATOR ITERATORVARIABLE
        NIL)
...
(CL:DEFUN HELP-STARTUP-FOREACH2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-FOREACH-STELLA-ADD-CONS-TO-END-OF-CONS-LIST
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ADD-CONS-TO-END-OF-CONS-LIST" NULL
     0))
   ...
   (CL:SETQ SYM-FOREACH-STELLA-LET
   ...
   (CL:SETQ SYM-FOREACH-STELLA-NEW
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET*
     ...
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      ...
      (WRAP-STRING "Startup-Foreach") NULL-STRING-WRAPPER)))
