...
(CL:DEFVAR SYM-FOREACH-STELLA-SETOF NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-SET-OF NULL)
...
(CL:DEFVAR SYM-FOREACH-STELLA-DEFUN NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-BOOLEAN NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-SELF NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-LET NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-NESTEDITERATOR NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-ITERATOR-NESTED-ITERATOR NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-WHILE NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-RETURN NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-NEW NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-ITERATOR-NEXT-CODE NULL)
(CL:DEFVAR SYM-FOREACH-STELLA-THE-CODE NULL)
(CL:DEFVAR KWD-FOREACH-FUNCTION NULL)
(CL:DEFVAR SGT-FOREACH-STELLA-ALL-PURPOSE-ITERATOR NULL)
...
(CL:DECLAIM
 (CL:SPECIAL *STELLA-MODULE* *MODULE* NULL-INTEGER *METHODBEINGWALKED* FALSE
  *SPECIALVARIABLESTACK* TRUE STANDARD-ERROR EOL NIL))

;;; (DEFUN (EXTRACT-ONE-IN-CLAUSE CONS CONS) ...)

(CL:DEFUN EXTRACT-ONE-IN-CLAUSE (TAIL)
  (CL:LET*
   ((VARIABLE (%%VALUE TAIL)) (OPERATOR (%%VALUE (%%REST TAIL))) (CLAUSE TAIL))
   (CL:WHEN
    (CL:NOT
    ...
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     ...
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      ...
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Illegal syntax where 'IN', 'ON', or 'ISA' clause expected:" EOL "   `"
       (DE-UGLIFY-PARSE-TREE TAIL) "'." EOL)))
    ...
    (CL:PROGN
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Missing arguments to DO separator." EOL)))
     (CL:SETF (%%REST DOCLAUSE)
      (CONS-LIST (WALK-DONT-CALL-ME-TREE NIL SGT-FOREACH-STELLA-VOID))))
...
(CL:DEFUN FOREACH-TO-PREFIX-FOREACH-TREE (FOREACHTREE)
  (CL:LET*
   ((GENERATORSCLAUSE NULL) (FILTERCLAUSE NULL) (DOCLAUSE NULL)
    (COLLECTCLAUSE NULL) (INTOCLAUSE NULL) (RESIDUE (%%REST FOREACHTREE))
    (OTREE NULL))
   ...
   (CL:WHEN (CL:EQ GENERATORSCLAUSE NULL)
    (CL:RETURN-FROM FOREACH-TO-PREFIX-FOREACH-TREE
     (LIST* SYM-FOREACH-STELLA-PREFIX-FOREACH
      (LIST* SYM-FOREACH-STELLA-IN SYM-FOREACH-STELLA-X SYM-FOREACH-STELLA-NIL
       NIL)
   ...
   (CL:WHEN
    (CL:OR (CL:NOT (CL:EQ COLLECTCLAUSE NULL)) (CL:NOT (CL:EQ INTOCLAUSE NULL)))
    ...
    (LIST* SYM-FOREACH-STELLA-PREFIX-FOREACH GENERATORSCLAUSE
     (CONCATENATE
      ...
      (CONCATENATE (CL:IF (CL:NOT (CL:EQ DOCLAUSE NULL)) (CONS DOCLAUSE NIL) NIL)
       (CONCATENATE
        (CL:IF (CL:NOT (CL:EQ COLLECTCLAUSE NULL)) (CONS COLLECTCLAUSE NIL) NIL)
        NIL)))))
   (CL:WHEN
   ...
   (CL:WHEN (CL:NOT (CL:EQ RESIDUE NIL))
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     ...
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Extra expressions at the end of foreach statement:." EOL)))
    (CL:RETURN-FROM FOREACH-TO-PREFIX-FOREACH-TREE
     (LIST* SYM-FOREACH-STELLA-PREFIX-FOREACH
      (LIST* SYM-FOREACH-STELLA-IN SYM-FOREACH-STELLA-X SYM-FOREACH-STELLA-NIL
       NIL)
  ...
  (CL:LET* ((*NOFSPECIALSATLOOPENTRY* (LENGTH *SPECIALVARIABLESTACK*)))
   ...
   (CL:LET* ((TEST-VALUE-000 (%%VALUE TREE)))
    (CL:COND
     ...
     ((CL:OR (CL:EQ TEST-VALUE-000 SYM-FOREACH-STELLA-SETOF)
       (CL:EQ TEST-VALUE-000 SYM-FOREACH-STELLA-SET-OF))
...
(CL:DEFUN WALK-COLLECTION-TREE (TREE DONTOPTIMIZE?)
    ...
    (CL:PROGN
     (CL:WHEN
      (CL:AND (CL:NOT DONTOPTIMIZE?)
       (CL:EQ (%%VALUE TREE) SYM-FOREACH-STELLA-INTERVAL) (CL:= (LENGTH TREE) 3))
  ...
  (CL:RETURN-FROM WALK-COLLECTION-TREE
   (WALK-EXPRESSION-TREE TREE SGT-FOREACH-STELLA-UNKNOWN
    SYM-FOREACH-STELLA-FOREACH FALSE)))
   ...
   (CL:LET* ((COLLECTIONTREE NULL) (COLLECTIONTYPE NULL))
    ...
    (CL:WHEN
     ...
     (CL:MULTIPLE-VALUE-SETQ (COLLECTIONTREE COLLECTIONTYPE)
      (WALK-COLLECTION-TREE
       (LIST* SYM-FOREACH-STELLA-ALLOCATE-ITERATOR COLLECTIONTREE NIL) TRUE))
     (CL:SETQ COLLECTIONBASETYPE (TYPE-SPEC-TO-BASE-TYPE COLLECTIONTYPE)))
    (CL:COND
      ...
      (CL:PROGN
       (CL:WHEN (CL:NOT (CL:= (LENGTH VARTREE) 2))
        ...
        (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
         (CL:VALUES NIL NIL SYM-FOREACH-STELLA-FALSE)))
    ...
    (CL:COND
     ...
     ((CL:EQ COLLECTIONBASETYPE SGT-FOREACH-STELLA-ARGUMENT-LIST)
      (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
       (YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST COLLECTIONTREE COLLECTIONTYPE
        KEYVAR VALUEVAR)))
     (CL:T
      (CL:IF (USE-VECTOR-STYLE-ITERATION? COLLECTIONBASETYPE)
       (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
        (YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR COLLECTIONTREE COLLECTIONTYPE KEYVAR
         VALUEVAR))
       (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES
  ...
  (CL:RETURN-FROM USE-VECTOR-STYLE-ITERATION?
   (CL:OR (CL:EQ COLLECTIONTYPE SGT-FOREACH-STELLA-STRING)
    ...
    (CL:AND (SAFE-SUBTYPE-OF? COLLECTIONTYPE SGT-FOREACH-STELLA-SEQUENCE-MIXIN)
     (CL:NOT
      (CL:EQ
       (LOOKUP-SLOT (%SURROGATE-VALUE COLLECTIONTYPE)
...
(CL:DEFUN YIELD-IN-CURSOR-CLAUSES-FOR-GENERAL-COLLECTION (COLLECTIONTREE COLLECTIONTYPE KEYVAR VALUEVAR)
  (CL:LET*
   ((COLLECTIONBASETYPE (TYPE-SPEC-TO-BASE-TYPE COLLECTIONTYPE))
    (ITERATORVAR (LOCAL-GENSYM "ITER")) (ITERATORCLAUSES NIL) (VALUECLAUSES NIL)
    (CONTINUATIONTEST NULL) (KEYTREE NIL) (VALUETREE NIL) (BUMPTREES NIL))
   (CL:COND
      ...
      (CONS
       (CONS ITERATORVAR
        (LIST* SGT-FOREACH-STELLA-CONS
         (LIST* SYM-FOREACH-STELLA-THE-CONS-LIST COLLECTIONTREE NIL) NIL))
       NIL))
      ...
      (CONS
       (CONS ITERATORVAR
        (LIST* SGT-FOREACH-STELLA-KV-CONS
         (LIST* SYM-FOREACH-STELLA-THE-KV-LIST COLLECTIONTREE NIL) NIL))
       NIL))
    ...
    (CL:T
     ...
     (CL:SETQ ITERATORCLAUSES
      (CONS
       (CONS ITERATORVAR
        (LIST* SGT-FOREACH-STELLA-ABSTRACT-ITERATOR COLLECTIONTREE NIL))
       NIL))
   ...
   (CL:COND
    ...
    ((CL:EQ COLLECTIONBASETYPE SGT-FOREACH-STELLA-PROPERTY-LIST)
     (CL:WHEN (CL:NOT (CL:EQ KEYVAR NULL))
      (CL:SETQ KEYTREE (LIST* SYM-FOREACH-STELLA-VALUE ITERATORVAR NIL)))
     (CL:SETQ VALUETREE
    ...
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" COLLECTIONBASETYPE
       "' is not a valid case option")
      (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
   (CL:SETQ VALUECLAUSES
    (CONS
     (CONS VALUEVAR
      (CONS (EXTRACT-PARAMETER-TYPE COLLECTIONTYPE SYM-FOREACH-STELLA-ANY-VALUE)
       (CONS
        (OPTIMIZE-PROGN
         (LIST* SYM-FOREACH-STELLA-PROGN
          (LIST* SYM-FOREACH-STELLA-SETQ VALUEVAR (CONS VALUETREE NIL))
          (CONCATENATE BUMPTREES NIL)))
        NIL)))
     VALUECLAUSES))
   (CL:WHEN (CL:NOT (CL:EQ KEYVAR NULL))
    (CL:LET* ((KEYTYPE NULL) (PARAMETEREXISTS? FALSE))
      ...
      (PARAMETEREXISTS?
       (CL:SETQ VALUECLAUSES
        (CONS
         (CONS KEYVAR
          (CONS KEYTYPE
           (CONS (LIST* SYM-FOREACH-STELLA-SETQ KEYVAR (CONS KEYTREE NIL)) NIL)))
         VALUECLAUSES)))
      (CL:T
       (CL:LET* ((*PRINTREADABLY?* TRUE))
        (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*)) (SIGNAL-TRANSLATION-ERROR)
  ...
  (CL:LET*
      ...
      (CL:OR
       (ISA?
        (WALK-EXPRESSION-TREE (COPY-CONS-TREE UPPERBOUND)
         SGT-FOREACH-STELLA-INTEGER SYM-FOREACH-STELLA-FOREACH FALSE)
        SGT-FOREACH-STELLA-NUMBER-WRAPPER)
       (CL:AND (CONS? UPPERBOUND)
        (MEMBER?
         (GET-QUOTED-TREE "((+ - * / 1+ 1- ++ -- LENGTH) \"/STELLA\")" "/STELLA")
         (%%VALUE UPPERBOUND))))))
    (ITERATORVAR (LOCAL-GENSYM "ITER"))
    (UPPERBOUNDVAR (CL:IF KNOWNUNBOUNDED? NULL (LOCAL-GENSYM "UPPER-BOUND")))
    (UNBOUNDEDVAR
   ...
   (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL
    (CL:VALUES
     (CONS
      (CONS ITERATORVAR
       (LIST* SGT-FOREACH-STELLA-INTEGER (%%VALUE (%%REST COLLECTIONTREE)) NIL))
      (CONCATENATE
       (CL:IF KNOWNUNBOUNDED? NIL
        (CONS
         (CONS UPPERBOUNDVAR (LIST* SGT-FOREACH-STELLA-INTEGER UPPERBOUND NIL))
         NIL))
       (CONCATENATE
      ...
      (CONS VALUEVAR
       (LIST* SGT-FOREACH-STELLA-INTEGER
        (LIST* SYM-FOREACH-STELLA-PROGN
         ...
         (LIST* SYM-FOREACH-STELLA-++ ITERATORVAR NIL) NIL)
        NIL))
      NIL)
     (CL:IF KNOWNUNBOUNDED? SYM-FOREACH-STELLA-TRUE
      (CL:IF KNOWNBOUNDED?
       (LIST* SYM-FOREACH-STELLA-<= ITERATORVAR (CONS UPPERBOUNDVAR NIL))
       (LIST* SYM-FOREACH-STELLA-OR UNBOUNDEDVAR
        (CONS (LIST* SYM-FOREACH-STELLA-<= ITERATORVAR (CONS UPPERBOUNDVAR NIL))
         NIL))))))))
...
(CL:DEFUN YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR (COLLECTIONTREE COLLECTIONTYPE KEYVAR VALUEVAR)
  (CL:WHEN (CL:NOT (CL:EQ KEYVAR NULL))
   ...
   (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR
    (CL:VALUES NIL NIL SYM-FOREACH-STELLA-FALSE)))
  (CL:LET*
   ((VECTORVAR (LOCAL-GENSYM "VECTOR")) (INDEXVAR (LOCAL-GENSYM "INDEX"))
    (LENGTHVAR (LOCAL-GENSYM "LENGTH")))
   (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES-FOR-VECTOR
    (CL:VALUES
     (LIST* (CONS VECTORVAR (CONS COLLECTIONTYPE (CONS COLLECTIONTREE NIL)))
        ...
        (CONS
         (LIST* SYM-FOREACH-STELLA-PROGN
          (LIST* SYM-FOREACH-STELLA-SETQ VALUEVAR
           (CONS (LIST* SYM-FOREACH-STELLA-NTH VECTORVAR (CONS INDEXVAR NIL))
            NIL))
          (LIST* SYM-FOREACH-STELLA-++ INDEXVAR NIL) NIL)
         NIL)))
      NIL)
     (LIST* SYM-FOREACH-STELLA-< INDEXVAR (CONS LENGTHVAR NIL))))))

;;; (DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST CONS CONS OBJECT) ...)
...
(CL:DEFUN YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST (COLLECTIONTREE COLLECTIONTYPE KEYVAR VALUEVAR)
  ...
  (CL:WHEN (CL:NOT (PASS-VARIABLE-ARGUMENTS-AS-LIST? *METHODBEINGWALKED*))
   (CL:LET* ((SELF-000 (NEW-PARAMETRIC-TYPE-SPECIFIER)))
    (CL:SETF (%SPECIFIER-BASE-TYPE SELF-000)
  ...
  (CL:LET*
   ((LISTIFIEDTYPE (YIELD-LISTIFIED-VARIABLE-ARGUMENTS-TYPE *METHODBEINGWALKED*))
    (ELEMENTTYPE
     (EXTRACT-PARAMETER-TYPE COLLECTIONTYPE SYM-FOREACH-STELLA-ANY-VALUE)))
   (CL:LET* ((ITERATORCLAUSES NULL) (VALUECLAUSES NULL) (CONTINUATIONTEST NULL))
    (CL:MULTIPLE-VALUE-SETQ (ITERATORCLAUSES VALUECLAUSES CONTINUATIONTEST)
     (YIELD-IN-CURSOR-CLAUSES-FOR-GENERAL-COLLECTION
    ...
    (CL:WHEN (SUB-TYPE-SPEC-OF? ELEMENTTYPE SGT-FOREACH-STELLA-LITERAL)
     (CL:LET* ((CLAUSE NULL) (ITER-000 VALUECLAUSES))
...
(CL:DEFUN YIELD-ON-CURSOR-CLAUSES (ONTREE)
  (CL:LET* ((ITERATORVAR (%%VALUE (%%REST ONTREE))))
   ...
   (CL:LET* ((COLLECTIONTREE NULL) (COLLECTIONTYPE NULL))
    (CL:MULTIPLE-VALUE-SETQ (COLLECTIONTREE COLLECTIONTYPE)
     (WALK-COLLECTION-TREE (%%VALUE (%%REST (%%REST ONTREE))) TRUE))
    (CL:WHEN (SUB-TYPE-SPEC-OF? COLLECTIONTYPE SGT-FOREACH-STELLA-ARGUMENT-LIST)
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " ON iteration not supported for ARGUMENT-LISTs: `"
        (DE-UGLIFY-PARSE-TREE ONTREE) "'." EOL)))
     (CL:RETURN-FROM YIELD-ON-CURSOR-CLAUSES
      (CL:VALUES NIL NIL SYM-FOREACH-STELLA-FALSE)))
     ...
     (CL:VALUES
      (CONS
       (CONS ITERATORVAR
        (LIST* SGT-FOREACH-STELLA-ABSTRACT-ITERATOR COLLECTIONTREE NIL))
       NIL)
      NIL (LIST* SYM-FOREACH-STELLA-NEXT? ITERATORVAR NIL))))))
   ...
   (CL:COND
    ...
    ((CL:EQ TEST-VALUE-000 SYM-FOREACH-STELLA-AS)
     (CL:LET*
      ...
      (CL:RETURN-FROM COLLECT-CURSOR-CLAUSES
       (CL:VALUES ALLITERATORCLAUSES ALLVALUECLAUSES
...
(CL:DEFUN WALK-CURSORS-TREE (CURSORSTREE DONTOPTIMIZE?)
  (CL:LET* ((ITERATORCLAUSES NULL) (VALUECLAUSES NULL) (CONTINUATIONTEST NULL))
     ...
     (CL:LET* ((ITERATORTREE NULL) (ITERATORTYPE NULL))
      (CL:MULTIPLE-VALUE-SETQ (ITERATORTREE ITERATORTYPE)
       (WALK-EXPRESSION-TREE (%%VALUE (%%REST (%%REST CLAUSE)))
        (%%VALUE (%%REST CLAUSE)) SYM-FOREACH-STELLA-FOREACH FALSE))
   ...
   (CL:LET* ((CLAUSE NULL) (ITER-001 VALUECLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ CLAUSE (%%VALUE ITER-001))
      (CL:SETQ ITER-001 (%%REST ITER-001)))
     (PUSH-VARIABLE-BINDING (%%VALUE CLAUSE) (%%VALUE (%%REST CLAUSE)))
     (THIRD-SETTER CLAUSE
      (WALK-WITHOUT-TYPE-TREE (%%VALUE (%%REST (%%REST CLAUSE)))))))
   (CL:SETQ CONTINUATIONTEST (WALK-WITHOUT-TYPE-TREE CONTINUATIONTEST))
   (CL:LET*
    ((I NULL-INTEGER) (ITER-002 1)
     (UPPER-BOUND-000 (CL:+ (LENGTH ITERATORCLAUSES) (LENGTH VALUECLAUSES))))
    ...
    (CL:LOOP WHILE (CL:<= ITER-002 UPPER-BOUND-000) DO
     ...
     (CL:SETQ I I) (POP-VARIABLE-BINDING)))
     ...
     (LIST* SYM-FOREACH-STELLA-PROGN
      ...
      (LIST* SYM-FOREACH-STELLA-IF
       ...
       (LIST* SYM-FOREACH-STELLA-SETF
        (LIST* SYM-FOREACH-STELLA-THE-CONS-LIST INTOVARIABLE NIL) COLLECTVARIABLE
        NIL)
    ...
    (LIST* SYM-FOREACH-STELLA-IF
     ...
     (LIST* SYM-FOREACH-STELLA-PROGN
      ...
      (LIST* SYM-FOREACH-STELLA-IF
       ...
       (LIST* SYM-FOREACH-STELLA-ADD-CONS-TO-END-OF-CONS-LIST INTOVARIABLE
   ...
   (CL:COND
    ((SUBTYPE-OF? INTOVARIABLETYPE SGT-FOREACH-STELLA-LIST)
     (CL:RETURN-FROM YIELD-COLLECT-CODE
      (YIELD-COLLECT-INTO-LIST-CODE COLLECTVARIABLE INTOVARIABLE COLLECTTREE)))
    ((SUBTYPE-OF? INTOVARIABLETYPE SGT-FOREACH-STELLA-CONS)
     (CL:RETURN-FROM YIELD-COLLECT-CODE
      (YIELD-COLLECT-INTO-CONS-CODE COLLECTVARIABLE INTOVARIABLE COLLECTTREE)))
    ...
    (CL:T
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Can't collect into a datatype other than LIST or CONS.." EOL)))
     (CL:RETURN-FROM YIELD-COLLECT-CODE
...
(CL:DEFUN WALK-PREFIX-COLLECT-INTO-TREE (COLLECTINTOTREE ITERATORS VALUEBINDINGS DOCODE)
  (CL:LET*
   ...
   (CL:IF (SYMBOL? INTOTREE)
    ...
    (CL:PROGN
     (CL:LET* ((D NULL) (ITER-000 VALUEBINDINGS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:MULTIPLE-VALUE-SETQ (INTOTREE INTOVARIABLETYPE)
      (WALK-EXPRESSION-TREE INTOTREE NULL SYM-FOREACH-STELLA-FOREACH FALSE))
     (CL:LET* ((D NULL) (ITER-001 VALUEBINDINGS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
...
(CL:DEFUN WRAP-WHERE-TEST (WHERETEST ACTIONS)
  (CL:IF (CL:NOT (CL:EQ WHERETEST NULL))
   (CL:RETURN-FROM WRAP-WHERE-TEST
    (CONS (LIST* SYM-FOREACH-STELLA-WHEN WHERETEST (CONCATENATE ACTIONS NIL))
     NIL))
...
(CL:DEFUN WALK-PREFIX-FOREACH-BODY (ITERATORCLAUSES VALUECLAUSES WHERETEST DOACTIONS)
  (CL:LET* ((OTREE NULL))
   ...
   (CL:LET* ((C NULL) (ITER-001 VALUECLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
   ...
   (CL:LET* ((C NULL) (ITER-002 VALUECLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
     (CL:PROGN (CL:SETQ C (%%VALUE ITER-002))
      (CL:SETQ ITER-002 (%%REST ITER-002)))
     (CL:SETQ C C) (POP-VARIABLE-BINDING)))
   (CL:LET* ((C NULL) (ITER-003 ITERATORCLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
     (CL:PROGN (CL:SETQ C (%%VALUE ITER-003))
...
(CL:DEFUN WALK-PREFIX-FOREACH-TREE (TREE)
  (CL:LET*
   ((TAIL NULL) (ITERATORCLAUSES NULL) (VALUECLAUSES NULL)
    (CONTINUATIONTEST NULL) (WHERETEST NULL) (DOACTIONS NIL))
   (CL:MULTIPLE-VALUE-SETQ (ITERATORCLAUSES VALUECLAUSES CONTINUATIONTEST)
    (WALK-CURSORS-TREE (%%VALUE (%%REST TREE)) FALSE))
   (SECOND-SETTER TREE NULL) (CL:SETQ TAIL (%%REST (%%REST TREE)))
   (CL:WHEN (CL:EQ (%%VALUE (%%VALUE TAIL)) SYM-FOREACH-STELLA-WHERE)
    (CL:SETQ WHERETEST (%%VALUE (%%REST (%%VALUE TAIL))))
    (SECOND-SETTER (%%VALUE TAIL) NULL) (CL:SETQ TAIL (%%REST TAIL)))
   (CL:WHEN (CL:EQ (%%VALUE (%%VALUE TAIL)) SYM-FOREACH-STELLA-DO)
    (CL:SETQ DOACTIONS (%%REST (%%VALUE TAIL)))
    (CL:SETF (%%REST (%%VALUE TAIL)) NIL) (CL:SETQ TAIL (%%REST TAIL)))
   (CL:WHEN (CL:NOT (CL:EQ TAIL NIL))
    (CL:MULTIPLE-VALUE-SETQ (ITERATORCLAUSES DOACTIONS)
     (WALK-PREFIX-COLLECT-INTO-TREE (%%VALUE TAIL) ITERATORCLAUSES VALUECLAUSES
      DOACTIONS))
     ...
     (LIST* SYM-FOREACH-STELLA-SYS-FOREACH ITERATORCLAUSES
      (CONS VALUECLAUSES
       (CONS CONTINUATIONTEST
        (CONCATENATE
         (WALK-PREFIX-FOREACH-BODY ITERATORCLAUSES VALUECLAUSES WHERETEST
          DOACTIONS)
         NIL))))
...
(CL:DEFUN WALK-FORALL-TREE (TREE)
  (CL:LET*
   ((ALWAYS? (LOCAL-GENSYM "ALWAYS?")) (ITERATORSANDWHERE NIL) (ALWAYSTEST NIL)
    (PRECURSOR NIL) (CURSOR TREE))
...
(CL:DEFUN WALK-SOME-TREE (TREE)
  (CL:LET*
   ((VALUEVAR (LOCAL-GENSYM "VALUE")) (ITERATIONVAR (%%VALUE (%%REST TREE)))
    (OTREE NULL))
   (CL:LET* ((SEQUENCEREF NULL) (SEQUENCETYPE NULL))
    ...
    (CL:SETQ OTREE
     (LIST* SYM-FOREACH-STELLA-VRLET
      (CONS
       (CONS VALUEVAR
        (CONS (EXTRACT-PARAMETER-TYPE SEQUENCETYPE SYM-FOREACH-STELLA-ANY-VALUE)
         (CONS SYM-FOREACH-STELLA-NULL NIL)))
       NIL)
      (CONS SYM-FOREACH-STELLA-FOREACH
       (CONCATENATE (%%REST TREE)
        (LIST* SYM-FOREACH-STELLA-DO
         (LIST* SYM-FOREACH-STELLA-SETQ VALUEVAR (CONS ITERATIONVAR NIL))
         (CONS SYM-FOREACH-STELLA-BREAK NIL) NIL)))
      VALUEVAR NIL))
...
(CL:DEFUN EXTRACT-SETOF-NESTED-ITERATOR-TREE (ITERATORCLAUSES)
  (CL:LET*
   ((ITERCLAUSES ITERATORCLAUSES)
    (SUBTREE (%%VALUE (%%REST (%%REST (%%VALUE ITERCLAUSES))))))
   (THIRD-SETTER (%%VALUE ITERCLAUSES) NULL)
   (CL:WHEN (CL:NOT (CL:EQ (%%REST ITERCLAUSES) NIL))
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (SIGNAL-TRANSLATION-ERROR)
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Too many iterators in 'setof' expression." EOL))))
...
(CL:DEFUN YIELD-SETOF-NEXT-FUNCTION-NAME (VALUECLAUSES WHERETEST)
  (CL:LET*
   ((ONLYVALUECLAUSE (%%VALUE VALUECLAUSES))
    (VALUEVARIABLE (%%VALUE ONLYVALUECLAUSE))
    (VALUETYPE (%%VALUE (%%REST ONLYVALUECLAUSE))) (NEXTFUNCTIONNAME NULL))
   (CL:SETQ NEXTFUNCTIONNAME (GENSYM "SETOF-NEXT-P"))
   (WALK-AUXILIARY-TREE
    (LIST* SYM-FOREACH-STELLA-DEFUN
     (CONS NEXTFUNCTIONNAME (CONS SYM-FOREACH-STELLA-BOOLEAN NIL))
     (CONS
      (LIST* SYM-FOREACH-STELLA-SELF SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR NIL)
      NIL)
     (LIST* SYM-FOREACH-STELLA-LET
      (LIST*
       (LIST* SYM-FOREACH-STELLA-NESTEDITERATOR
        (LIST* SYM-FOREACH-STELLA-ITERATOR-NESTED-ITERATOR
         SYM-FOREACH-STELLA-SELF NIL)
        NIL)
       (CONS VALUEVARIABLE (CONS VALUETYPE (CONS SYM-FOREACH-STELLA-NULL NIL)))
       NIL)
      (LIST* SYM-FOREACH-STELLA-WHILE
       (LIST* SYM-FOREACH-STELLA-NEXT? SYM-FOREACH-STELLA-NESTEDITERATOR NIL)
       (LIST* SYM-FOREACH-STELLA-SETQ VALUEVARIABLE
        (CONS
         (LIST* SYM-FOREACH-STELLA-VALUE SYM-FOREACH-STELLA-NESTEDITERATOR NIL)
         NIL))
       (LIST* SYM-FOREACH-STELLA-WHEN WHERETEST
        (LIST*
         (LIST* SYM-FOREACH-STELLA-SETF
          (LIST* SYM-FOREACH-STELLA-VALUE SYM-FOREACH-STELLA-SELF NIL)
          VALUEVARIABLE NIL)
         (LIST* SYM-FOREACH-STELLA-RETURN SYM-FOREACH-STELLA-TRUE NIL) NIL))
       NIL)
      (LIST* SYM-FOREACH-STELLA-RETURN SYM-FOREACH-STELLA-FALSE NIL) NIL)
     NIL))
...
(CL:DEFUN WALK-SETOF-TREE (TREE)
  ...
  (CL:LET*
   ((ITERATORSSUBTREE (%%VALUE (%%REST TREE)))
    (ITERATORVARIABLE (LOCAL-GENSYM "ITER")) (TAIL NULL) (ITERATORCLAUSES NULL)
    (VALUECLAUSES NULL) (CONTINUATIONTEST NULL) (WHERETEST NULL))
   (SECOND-SETTER TREE NULL)
   (CL:WHEN
    (MEMBER? (GET-QUOTED-TREE "((ON AS) \"/STELLA\")" "/STELLA")
     (%%VALUE ITERATORSSUBTREE))
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (SIGNAL-TRANSLATION-ERROR)
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Illegal reserved word `" (%%VALUE ITERATORSSUBTREE)
       "' in 'setof' expression.." EOL))))
   (CL:MULTIPLE-VALUE-SETQ (ITERATORCLAUSES VALUECLAUSES CONTINUATIONTEST)
    (WALK-CURSORS-TREE ITERATORSSUBTREE TRUE))
   (CL:SETQ CONTINUATIONTEST CONTINUATIONTEST)
   (CL:SETQ TAIL (%%REST (%%REST TREE)))
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ TAIL NIL))
     (CL:EQ (%%VALUE (%%VALUE TAIL)) SYM-FOREACH-STELLA-WHERE))
    (CL:SETQ WHERETEST (%%VALUE (%%REST (%%VALUE TAIL))))
    (SECOND-SETTER (%%VALUE TAIL) NULL) (CL:SETQ TAIL (%%REST TAIL)))
   (CL:WHEN (CL:NOT (CL:EQ TAIL NIL))
    (CL:LET* ((*PRINTREADABLY?* TRUE)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     ...
     (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
      (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
       " Illegal 'setof' expression.  Syntax is " EOL
       "   (SETOF <iterators> WHERE <boolean expression>)." EOL))))
   (CL:RETURN-FROM WALK-SETOF-TREE
    (CL:VALUES
     (LIST* SYM-FOREACH-STELLA-VRLET
      (CONS
       (CONS ITERATORVARIABLE
        (CONS
         (LIST* SYM-FOREACH-STELLA-NEW SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR
          NIL)
         NIL))
       NIL)
      (LIST* SYM-FOREACH-STELLA-SETF
       (LIST* SYM-FOREACH-STELLA-ITERATOR-NESTED-ITERATOR ITERATORVARIABLE NIL)
       (EXTRACT-SETOF-NESTED-ITERATOR-TREE ITERATORCLAUSES) NIL)
      (LIST* SYM-FOREACH-STELLA-SETF
       (LIST* SYM-FOREACH-STELLA-ITERATOR-NEXT-CODE ITERATORVARIABLE NIL)
       (LIST* SYM-FOREACH-STELLA-THE-CODE KWD-FOREACH-FUNCTION
        (YIELD-SETOF-NEXT-FUNCTION-NAME VALUECLAUSES WHERETEST) NIL)
       NIL)
      ITERATORVARIABLE NIL)
     SGT-FOREACH-STELLA-ALL-PURPOSE-ITERATOR))))

(CL:DEFUN HELP-STARTUP-FOREACH1 ()
  (CL:PROGN
   (CL:SETQ SYM-FOREACH-STELLA-AS (INTERN-RIGID-SYMBOL-WRT-MODULE "AS" NULL 0))
   ...
   (CL:SETQ SYM-FOREACH-STELLA-SETOF
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SETOF" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-SET-OF
...
(CL:DEFUN HELP-STARTUP-FOREACH2 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-FOREACH-STELLA-DEFUN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DEFUN" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-BOOLEAN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "BOOLEAN" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-SELF
    (INTERN-RIGID-SYMBOL-WRT-MODULE "SELF" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-ALL-PURPOSE-ITERATOR
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ALL-PURPOSE-ITERATOR" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-LET (INTERN-RIGID-SYMBOL-WRT-MODULE "LET" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-NESTEDITERATOR
    (INTERN-RIGID-SYMBOL-WRT-MODULE "NESTEDITERATOR" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-ITERATOR-NESTED-ITERATOR
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ITERATOR-NESTED-ITERATOR" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-WHILE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "WHILE" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-RETURN
    (INTERN-RIGID-SYMBOL-WRT-MODULE "RETURN" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-NEW (INTERN-RIGID-SYMBOL-WRT-MODULE "NEW" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-ITERATOR-NEXT-CODE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "ITERATOR-NEXT-CODE" NULL 0))
   (CL:SETQ SYM-FOREACH-STELLA-THE-CODE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "THE-CODE" NULL 0))
   (CL:SETQ KWD-FOREACH-FUNCTION
    (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   (CL:SETQ SGT-FOREACH-STELLA-ALL-PURPOSE-ITERATOR
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST"
     "(DEFUN (YIELD-IN-CURSOR-CLAUSES-FOR-ARGUMENT-LIST CONS CONS OBJECT) ((COLLECTIONTREE OBJECT) (COLLECTIONTYPE TYPE-SPEC) (KEYVAR SYMBOL) (VALUEVAR SYMBOL)))"
    ...
    (DEFINE-FUNCTION-OBJECT "WALK-PREFIX-FOREACH-BODY"
     "(DEFUN (WALK-PREFIX-FOREACH-BODY CONS) ((ITERATORCLAUSES CONS) (VALUECLAUSES CONS) (WHERETEST OBJECT) (DOACTIONS CONS)))"
    ...
    (DEFINE-FUNCTION-OBJECT "EXTRACT-SETOF-NESTED-ITERATOR-TREE"
     "(DEFUN (EXTRACT-SETOF-NESTED-ITERATOR-TREE CONS) ((ITERATORCLAUSES CONS)))"
     (CL:FUNCTION EXTRACT-SETOF-NESTED-ITERATOR-TREE) NULL)
    (DEFINE-FUNCTION-OBJECT "YIELD-SETOF-NEXT-FUNCTION-NAME"
     "(DEFUN (YIELD-SETOF-NEXT-FUNCTION-NAME SYMBOL) ((VALUECLAUSES CONS) (WHERETEST CONS)))"
     (CL:FUNCTION YIELD-SETOF-NEXT-FUNCTION-NAME) NULL)
    (DEFINE-FUNCTION-OBJECT "WALK-SETOF-TREE"
     "(DEFUN (WALK-SETOF-TREE CONS TYPE-SPEC) ((TREE CONS)))"
     (CL:FUNCTION WALK-SETOF-TREE) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-FOREACH"
     "(DEFUN STARTUP-FOREACH () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-FOREACH)
     NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-FOREACH-STELLA-STARTUP-FOREACH)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-FOREACH-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Foreach") NULL-STRING-WRAPPER)))
