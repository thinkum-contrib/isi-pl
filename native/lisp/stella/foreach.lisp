    ...
    (LIST* SYM-FOREACH-STELLA-PREFIX-FOREACH GENERATORSCLAUSE
     (CONCATENATE
      ...
      (CONCATENATE
       ...
       (CONCATENATE
        (CL:IF (CL:NOT (CL:EQ COLLECTCLAUSE NULL)) (CONS COLLECTCLAUSE NIL)
         NIL)
        NIL)))))
   (CL:WHEN
    (CL:OR
    ...
    (CL:WHEN
     ...
     (CL:MULTIPLE-VALUE-SETQ (COLLECTIONTREE COLLECTIONTYPE)
      (WALK-COLLECTION-TREE
       (LIST* SYM-FOREACH-STELLA-ALLOCATE-ITERATOR COLLECTIONTREE NIL) CL:T))
     (CL:SETQ COLLECTIONBASETYPE (TYPE-SPEC-TO-BASE-TYPE COLLECTIONTYPE)))
    (CL:COND
...
(CL:DEFUN YIELD-IN-CURSOR-CLAUSES-FOR-GENERAL-COLLECTION (COLLECTIONTREE COLLECTIONTYPE KEYVAR VALUEVAR)
  (CL:LET*
   ((COLLECTIONBASETYPE (TYPE-SPEC-TO-BASE-TYPE COLLECTIONTYPE))
    ...
    (VALUECLAUSES NIL) (CONTINUATIONTEST NULL) (KEYTREE NIL) (VALUETREE NIL)
    (BUMPTREES NIL))
   (CL:COND
   ...
   (CL:WHEN (CL:NOT (CL:EQ KEYVAR NULL))
    (CL:LET* ((KEYTYPE NULL) (PARAMETEREXISTS? CL:NIL))
     ...
     (CL:COND
      (PARAMETEREXISTS?
       (CL:SETQ ITERATORCLAUSES
        (CONS (CONS KEYVAR (CONS KEYTYPE (CONS SYM-FOREACH-STELLA-NULL NIL)))
         ITERATORCLAUSES))
       (CL:SETQ VALUECLAUSES
...
(CL:DEFUN YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL (COLLECTIONTREE COLLECTIONTYPE KEYVAR VALUEVAR)
  ...
  (CL:WHEN *AGGRESSIVELY-OPTIMIZE-INTERVAL-FOREACH?*
   (CL:RETURN-FROM YIELD-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL
    (YIELD-OPTIMIZED-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL COLLECTIONTREE
     COLLECTIONTYPE KEYVAR VALUEVAR)))
  (CL:LET*
       ...
       (CONCATENATE
        (CL:IF (CL:OR KNOWNUNBOUNDED? KNOWNBOUNDED?) NIL
         (CONS
          ...
          NIL))
        NIL)))
     (CONS (LIST* SYM-FOREACH-STELLA-SETQ VALUEVAR (CONS ITERATORVAR NIL))
      NIL)
    ...
    (CL:VALUES
     ...
     (CL:IF UNBOUNDED? SYM-FOREACH-STELLA-TRUE
      (CONS TESTOPERATOR
       (CONS ITERATORVAR
        (CONS
         (CL:IF (CL:NOT (CL:EQ UPPERBOUNDVAR NULL)) UPPERBOUNDVAR UPPERBOUND)
         NIL))))))))
  ...
  (CL:LET* ((TEST-VALUE-000 (%%VALUE CURSORSTREE)))
   (CL:COND
    ...
    ((CL:EQ TEST-VALUE-000 SYM-FOREACH-STELLA-AS)
     (CL:LET*
      ((ALLITERATORCLAUSES NIL) (ALLVALUECLAUSES NIL) (ALLBUMPCLAUSES NIL)
...
(CL:DEFUN WALK-CURSORS-TREE (CURSORSTREE DONTOPTIMIZE?)
  (CL:LET*
   ...
   (CL:LET*
    ((I NULL-INTEGER) (ITER-001 1) (UPPER-BOUND-000 (LENGTH ITERATORCLAUSES)))
    ...
    (CL:PROGN
     ...
     (CL:LET* ((D NULL) (ITER-001 ITERATORS))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
       (CL:SETQ D (%%VALUE ITER-001)) (CL:SETQ D D) (POP-VARIABLE-BINDING)
...
(CL:DEFUN WALK-PREFIX-FOREACH-BODY (ITERATORCLAUSES WHERETEST DOACTIONS)
  (CL:LET* ((OTREE NULL))
   ...
   (CL:LET* ((C NULL) (ITER-001 ITERATORCLAUSES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:SETQ C (%%VALUE ITER-001)) (CL:SETQ C C) (POP-VARIABLE-BINDING)
     (CL:SETQ ITER-001 (%%REST ITER-001))))
...
(CL:DEFUN WALK-PREFIX-FOREACH-TREE (TREE)
  (CL:LET*
   ((TAIL NULL) (ITERATORCLAUSES NULL) (VALUECLAUSES NULL) (BUMPCLAUSES NULL)
   ...
   (CL:WHEN (CL:EQ (%%VALUE (%%VALUE TAIL)) SYM-FOREACH-STELLA-DO)
    (CL:SETQ DOACTIONS (%%REST (%%VALUE TAIL))) (CL:SETQ TAIL (%%REST TAIL)))
      ...
      (CONS VALUECLAUSES
       (CONS BUMPCLAUSES
        (CONS CONTINUATIONTEST
         (CONCATENATE
          (WALK-PREFIX-FOREACH-BODY ITERATORCLAUSES WHERETEST DOACTIONS)
          NIL)))))
    ...
    (CL:SETQ OTREE
     (LIST* SYM-FOREACH-STELLA-VRLET
      (CONS
       (CONS VALUEVAR
        (CONS
         (EXTRACT-PARAMETER-TYPE SEQUENCETYPE SYM-FOREACH-STELLA-ANY-VALUE)
         (CONS SYM-FOREACH-STELLA-NULL NIL)))
       NIL)
      (CONS SYM-FOREACH-STELLA-FOREACH
       (CONCATENATE (%%REST TREE)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT
     ...
     (CL:FUNCTION YIELD-OPTIMIZED-IN-CURSOR-CLAUSES-FOR-INTEGER-INTERVAL)
     NULL)
