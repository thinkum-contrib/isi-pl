...
(CL:DECLAIM
  ...
  *METHODBEINGWALKED* *CURRENT-STELLA-FEATURES* *TRANSLATOROUTPUTLANGUAGE*
      ...
      (LIST* SYM-MACROS-STELLA-SETQ VARIABLE
       (CONS
        (LIST* SYM-MACROS-STELLA-CHOOSE
         ...
         VARIABLE
         (CONS (LIST* SYM-MACROS-STELLA-CONS VALUEVAR (CONS VARIABLE NIL))
          NIL))
        NIL))
      NIL)))
   (CL:RETURN-FROM PUSHQ-NEW
   ...
   (LIST* SYM-MACROS-STELLA-SPECIAL
    (LIST* (LIST* SYM-MACROS-STELLA-*CONTEXT* CONTEXTFORM NIL)
     (LIST* SYM-MACROS-STELLA-*MODULE*
      (LIST* SYM-MACROS-STELLA-BASE-MODULE SYM-MACROS-STELLA-*CONTEXT* NIL)
      NIL)
     NIL)
    (CONCATENATE BODY NIL))))
...
(CL:DEFUN HELP-STARTUP-MACROS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-MACROS-STELLA-*CURRENTSYSTEMDEFINITIONSUBDIRECTORY*
    (INTERN-RIGID-SYMBOL-WRT-MODULE "*CURRENTSYSTEMDEFINITIONSUBDIRECTORY*"
     NULL 0))
...
is already a member of the list.\" :PUBLIC? TRUE)" (CL:FUNCTION PUSHQ-NEW)
     NULL)
...
FALSE.  There can only be one DEFMAIN per module.\" :PUBLIC? TRUE :LISP-MACRO? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-MACROS"
     "(DEFUN STARTUP-MACROS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-MACROS)
     NULL)
