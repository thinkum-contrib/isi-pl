...
(CL:DECLAIM
  ...
  *METHODBEINGWALKED* *CURRENT-STELLA-FEATURES* *TRANSLATOROUTPUTLANGUAGE* TRUE
  STANDARD-ERROR EOL ONE-WRAPPER NIL))
  ...
  (CL:LET* ((VALUEVAR (LOCAL-GENSYM "HEAD")))
   (CL:RETURN-FROM POPQ
    (LIST* SYM-MACROS-STELLA-VRLET
     (CONS
      (CONS VALUEVAR (CONS (LIST* SYM-MACROS-STELLA-VALUE VARIABLE NIL) NIL))
      NIL)
     (LIST* SYM-MACROS-STELLA-SETQ VARIABLE
  ...
  (CL:LET* ((PLACECOPY (COPY-CONS-TREE PLACE)))
   (CL:RETURN-FROM PUSHF
    (LIST* SYM-MACROS-STELLA-SETF PLACE
     (CONS (LIST* SYM-MACROS-STELLA-CONS VALUE (CONS PLACECOPY NIL)) NIL)))))
  ...
  (CL:IF (CONS? VALUE)
      ...
      (LIST* SYM-MACROS-STELLA-SETQ VARIABLE
       (CONS
        (LIST* SYM-MACROS-STELLA-CHOOSE
         (LIST* SYM-MACROS-STELLA-MEMBER? VARIABLE (CONS VALUEVAR NIL)) VARIABLE
   ...
   (CL:RETURN-FROM PUSHQ-NEW
    (LIST* SYM-MACROS-STELLA-SETQ VARIABLE
     (CONS
      (LIST* SYM-MACROS-STELLA-CHOOSE
       (LIST* SYM-MACROS-STELLA-MEMBER? VARIABLE (CONS VALUE NIL)) VARIABLE
       (CONS (LIST* SYM-MACROS-STELLA-CONS VALUE (CONS VARIABLE NIL)) NIL))
      NIL)))))
  ...
  (CL:LET* ((INCR (%%VALUE INCREMENT)))
    ...
    (CL:IF (CL:EQ PLACE NULL)
     ...
     (CL:LET* ((*PRINTREADABLY?* TRUE))
      ...
      (CL:WHEN (CL:NOT (SUPPRESS-WARNINGS?))
       (PRINT-ERROR-CONTEXT ">> ERROR: " STANDARD-ERROR)
       (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-ERROR) EOL
        " Illegal place in `++' expression: `" (DE-UGLIFY-PARSE-TREE PLACE) "'."
        EOL))))
    (CL:RETURN-FROM ++ NIL))
   ...
   (CL:RETURN-FROM ++
    (CL:IF (SYMBOL? PLACE)
     ...
     (LIST* SYM-MACROS-STELLA-SETF PLACE
      (CONS (LIST* SYM-MACROS-STELLA-+ (COPY-CONS-TREE PLACE) (CONS INCR NIL))
       NIL))))))
  ...
  (CL:LET* ((DECR (%%VALUE DECREMENT)))
   ...
   (CL:RETURN-FROM --
    (CL:IF (SYMBOL? PLACE)
     ...
     (LIST* SYM-MACROS-STELLA-SETF PLACE
      (CONS (LIST* SYM-MACROS-STELLA-- (COPY-CONS-TREE PLACE) (CONS DECR NIL))
       NIL))))))
   ...
   (2
    (CL:IF (SYMBOL? (%%VALUE FORMS))
     (CL:RETURN-FROM FIRST-DEFINED
      (LIST* SYM-MACROS-STELLA-CHOOSE
       (LIST* SYM-MACROS-STELLA-DEFINED? (%%VALUE FORMS) NIL) (%%VALUE FORMS)
   ...
   (CL:OTHERWISE
    (CL:RETURN-FROM FIRST-DEFINED
     (LIST* SYM-MACROS-STELLA-FIRST-DEFINED (%%VALUE FORMS)
      (CONS
       (CONS SYM-MACROS-STELLA-FIRST-DEFINED (CONCATENATE (%%REST FORMS) NIL))
       NIL))))))
...
(CL:DEFUN COLLECT (COLLECTVARIABLE BODY)
...
objects within the scope of this declaration."
  (CL:RETURN-FROM WITH-PERMANENT-OBJECTS
   (LIST* SYM-MACROS-STELLA-SPECIAL
    (CONS
     (LIST* SYM-MACROS-STELLA-*TRANSIENTOBJECTS?* SYM-MACROS-STELLA-FALSE NIL)
     NIL)
...
account for transient objects will continue to work correctly."
  (CL:RETURN-FROM WITH-TRANSIENT-OBJECTS
   (LIST* SYM-MACROS-STELLA-SPECIAL
    (CONS
     (LIST* SYM-MACROS-STELLA-*TRANSIENTOBJECTS?* SYM-MACROS-STELLA-TRUE NIL)
     NIL)
    (CONCATENATE BODY NIL))))
   ...
   (CL:LET* ((VARIABLE NULL) (ITER-000 VARIABLES) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000
        (CONS (LIST* SYM-MACROS-STELLA-SETQ VARIABLE (CONS VARIABLE NIL)) NIL))
   ...
   (LIST* SYM-MACROS-STELLA-SPECIAL
    (LIST* (LIST* SYM-MACROS-STELLA-*CONTEXT* CONTEXTFORM NIL)
     (LIST* SYM-MACROS-STELLA-*MODULE*
      (LIST* SYM-MACROS-STELLA-BASE-MODULE SYM-MACROS-STELLA-*CONTEXT* NIL) NIL)
     NIL)
    (CONCATENATE BODY NIL))))
   ...
   (CL:LET* ((ARGVAR (LOCAL-GENSYM "ARG")) (LISTVAR (LOCAL-GENSYM "ARGLIST")))
    (CL:RETURN-FROM COERCE-&REST-TO-CONS
     (LIST* SYM-MACROS-STELLA-VRLET
      (CONS
       (CONS LISTVAR
        (CONS (YIELD-LISTIFIED-VARIABLE-ARGUMENTS-TYPE *METHODBEINGWALKED*)
   ...
   (LIST* SYM-MACROS-STELLA-SPECIAL
    (LIST*
     (LIST* SYM-MACROS-STELLA-*CURRENTSYSTEMDEFINITION*
      (LIST* SYM-MACROS-STELLA-GET-SYSTEM-DEFINITION SYSTEMNAMEEXPRESSION NIL)
      NIL)
    ...
    (LIST* SYM-MACROS-STELLA-IF
     ...
     (LIST* SYM-MACROS-STELLA-WARN (WRAP-STRING "Can't find a system named ")
      SYSTEMNAMEEXPRESSION (CONS SYM-MACROS-STELLA-EOL NIL))
     NIL)
    NIL)))
...
FALSE.  There can only be one DEFMAIN per module."
  (CL:LET*
   ((PROCESSCMDLINEARGS? (CL:NOT (CL:EQ VARLIST NIL))) (V1NAME (%%VALUE VARLIST))
    (V2NAME (%%VALUE (%%REST VARLIST))) (BODYWITHHEADER (CONS NULL BODY)))
     ...
     ((OPTIONS SELF-000)
      (STARTUPFUNCTIONCALL
       (CL:IF
        ...
        (CONS
         (CONS (SYSTEM-STARTUP-FUNCTION-SYMBOL *CURRENTSYSTEMDEFINITION*) NIL)
         NIL)
        NIL)))
        ...
        ((CL:OR (CL:EQ *TRANSLATOROUTPUTLANGUAGE* KWD-MACROS-CPP)
         ...
         (CL:RETURN-FROM %DEFMAIN
          (LIST* SYM-MACROS-STELLA-DEFUN
           (LIST* SYM-MACROS-STELLA-MAIN SYM-MACROS-STELLA-INTEGER NIL) NIL
           KWD-MACROS-PUBLIC? SYM-MACROS-STELLA-TRUE
         ...
         (CL:RETURN-FROM %DEFMAIN
          (LIST* SYM-MACROS-STELLA-DEFUN SYM-MACROS-STELLA-MAIN
           (LIST* SYM-MACROS-STELLA-&REST
            (LIST* SYM-MACROS-STELLA-%%ARGS SYM-MACROS-STELLA-STRING NIL) NIL)
           KWD-MACROS-PUBLIC? SYM-MACROS-STELLA-TRUE
             ...
             (CONS V1NAME
              (LIST* SYM-MACROS-STELLA-INTEGER
               (LIST* SYM-MACROS-STELLA-VERBATIM KWD-MACROS-JAVA
                (WRAP-STRING
                 (CONCATENATE (%WRAPPER-VALUE (JAVA-TRANSLATE-NAME V2NAME))
                  ".length"))
                NIL)
               NIL))
             NIL)
...
(CL:DEFUN HELP-STARTUP-MACROS1 ()
  (CL:PROGN
   ...
   (CL:SETQ SYM-MACROS-STELLA-*CURRENTSYSTEMDEFINITIONSUBDIRECTORY*
    (INTERN-RIGID-SYMBOL-WRT-MODULE "*CURRENTSYSTEMDEFINITIONSUBDIRECTORY*" NULL
     0))
...
return TRUE if none of them are NULL.\")" (CL:FUNCTION ALL-DEFINED?) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "COLLECT"
     "(DEFUN COLLECT ((COLLECTVARIABLE SYMBOL) |&BODY| (BODY CONS)) :TYPE OBJECT :MACRO? TRUE :DOCUMENTATION \"Use a VRLET to collect values.  Input has the form
'(collect <x> in <expression> where (<test> <x>))'.\")" (CL:FUNCTION COLLECT)
     NULL)
...
FALSE.  There can only be one DEFMAIN per module.\" :PUBLIC? TRUE :LISP-MACRO? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-MACROS"
     "(DEFUN STARTUP-MACROS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-MACROS) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-MACROS-STELLA-STARTUP-MACROS)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-MACROS-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_Startup-Macros")
      NULL-STRING-WRAPPER)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    (REGISTER-NATIVE-NAME SYM-MACROS-STELLA--- KWD-MACROS-JAVA
     KWD-MACROS-FUNCTION)
    (REGISTER-NATIVE-NAME SYM-MACROS-STELLA-DEFMAIN KWD-MACROS-COMMON-LISP
     KWD-MACROS-FUNCTION)))
  :VOID)
