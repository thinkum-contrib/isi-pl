...
(CL:DECLAIM
 (CL:TYPE CL:FIXNUM *HARDWIRED-RESIZE-INCREMENT-ON-VECTOR-SEQUENCE*))
...
(CL:DECLAIM
  ...
  *HARDWIRED-CLASS-ARITY-ON-CLASS* *JAVA-STELLA-PACKAGE-MAPPING*
  NULL-STRING-WRAPPER NIL-LIST
  *HARDWIRED-TRANSIENT?-ON-TRANSIENT-MIXIN* *CONTEXT*
  *HARDWIRED-RESIZE-INCREMENT-ON-SHORT-VECTOR-SEQUENCE*
  *HARDWIRED-RESIZE-INCREMENT-ON-VECTOR-SEQUENCE* NULL-CHARACTER NIL
  TRUE TRUE-WRAPPER FALSE-WRAPPER NULL-INTEGER FALSE))
...
(CL:DEFMETHOD PRIMARY-TYPE ((SELF SHORT-VECTOR-SEQUENCE))
  (CL:RETURN-FROM PRIMARY-TYPE
   SGT-HIERARCHY-STELLA-SHORT-VECTOR-SEQUENCE))
...
(CL:DEFUN NEW-CLASS ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%CLASS-PUBLIC? SELF) TRUE)
   (CL:SETF (%CL-STRUCT? SELF) FALSE)
...
(CL:DEFUN NEW-MODULE ()
  (CL:LET* ((SELF NULL))
   ...
   (CL:SETF (%CASE-SENSITIVE? SELF) FALSE)
   ...
   (CL:LET*
    ((CLASS
      (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "NO-SUCH-FILE-EXCEPTION"
       "(DEFCLASS NO-SUCH-FILE-EXCEPTION (INPUT-OUTPUT-EXCEPTION) :PUBLIC? TRUE)")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS)
