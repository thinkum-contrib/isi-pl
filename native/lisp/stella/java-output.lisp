...
(CL:DECLAIM
 (CL:SPECIAL *CHARACTER-TYPE-TABLE* *MODULE* NULL-CHARACTER STANDARD-OUTPUT
  EOL NIL NULL-INTEGER *CURRENT-STREAM* *STELLA-MODULE*))
...
(CL:DEFUN JAVA-OUTPUT-STATEMENT (STATEMENT)
  (CL:COND
   ...
   (CL:T
    (CL:WHEN (CL:EQ STATEMENT NULL) (CL:RETURN-FROM JAVA-OUTPUT-STATEMENT))
...
(CL:DEFUN JAVA-OUTPUT-CLASS-VARIABLE-DEFINITIONS (VARIABLETRANSLATIONS)
  (CL:LET* ((STATEMENT NULL) (ITER-000 VARIABLETRANSLATIONS))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:SETQ STATEMENT (%%VALUE ITER-000)) (JAVA-OUTPUT-STATEMENT STATEMENT)
    (CL:SETQ ITER-000 (%%REST ITER-000))))
  :VOID)
     ...
     (CL:RETURN-FROM JAVA-HEURISTICALLY-TRANSLATE-NAME
      (JAVA-TRANSLATE-NAMESTRING
       (MAKE-MATCHING-NAME-FULL STELLA-NAME KWD-JAVA-OUTPUT-NO
        KWD-JAVA-OUTPUT-NO "- " "" "" "" "" KWD-JAVA-OUTPUT-PRESERVE "_"))))
...
(CL:DEFUN JAVA-OUTPUT-COMMENT-STRING (COMMENT COMMENT-START COMMENT-CONTINUATION COMMENT-END JAVADOC-COMMENT? CASE-CONVERTED-NAME-LIST)
  ...
  (CL:LET*
    ...
    (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
     ...
     (CL:TAGBODY
      (CL:CASE CHAR
       ...
       (#\\
        (CL:IF ESCAPE?
         (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR)
   ...
   (CL:WHEN INSIDE-QUOTE?
    (CL:WARN "Comment ended while inside a ` form in comment `~A'" COMMENT)
  ...
  (CL:LET* ((PARAMETER-NAMES NIL))
   ...
   (CL:WHEN
    (CL:AND (CL:NOT (CL:EQ AUTHOR STELLA::NULL-STRING))
     (CL:NOT (CL:EQ AUTHOR "")))
    (JAVA-INDENT)
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " * @author " AUTHOR
     EOL))
   (CL:WHEN
...
(CL:DEFUN JAVA-OUTPUT-GLOBAL-DEFINITION (GLOBAL)
  (CL:WHEN
   (CL:NOT (CL:EQ (%WRAPPER-VALUE (%%VALUE GLOBAL)) STELLA::NULL-STRING))
   (JAVA-INDENT)
...
(CL:DEFUN JAVA-STATEMENT-RETURNS? (STATEMENT)
  (CL:COND
   ((CL:EQ STATEMENT NULL) (CL:RETURN-FROM JAVA-STATEMENT-RETURNS? CL:NIL))
...
(CL:DEFUN JAVA-OUTPUT-IF (TREE)
  ...
  (CL:WHEN (CL:NOT (CL:EQ (%%VALUE (%%REST (%%REST TREE))) NULL))
   (JAVA-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "else ")
   (CL:IF
  ...
  (CL:LET*
   ((CONTINUATIONTEST (%%VALUE (%%REST LOOP)))
    ...
    (EOLSEPARATEEXPRESSIONS?
     (CL:OR (CL:> NOFNEXTASSIGNMENTS 2) (CL:> *JAVA-INDENT-CHARS* 35))))
   ...
   (CL:WHEN (CL:NOT (CL:EQ NEXTASSIGNMENTS NIL))
    (CL:WHEN EOLSEPARATEEXPRESSIONS?
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL))
    (CL:LET*
     ((NEXT NULL) (ITER-000 NEXTASSIGNMENTS) (I NULL-INTEGER) (ITER-001 1))
...
(CL:DEFUN JAVA-OUTPUT-PRINT-STREAM (EXPS NATIVESTREAM?)
  (CL:LET*
      ...
      (CL:IF (JAVA-END-OF-LINE-TOKEN? ITEM)
       (CL:PROGN (CL:WHEN EMBEDDEDEOL? (JAVA-INDENT))
        (JAVA-HELP-OUTPUT-PRINT-STREAM STREAM
         (JAVA-REVERSE-AND-CONSOLIDATE-STRINGS PRINTITEMS) NATIVESTREAM? CL:T)
        (CL:WHEN EMBEDDEDEOL?
         (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ PRINTITEMS NIL))
    ...
    (JAVA-HELP-OUTPUT-PRINT-STREAM STREAM
     (JAVA-REVERSE-AND-CONSOLIDATE-STRINGS PRINTITEMS) NATIVESTREAM? CL:NIL)
    (CL:WHEN EMBEDDEDEOL?
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)))
...
(CL:DEFUN HELP-STARTUP-JAVA-OUTPUT3 ()
  (CL:PROGN
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-CAST"
    "(DEFUN JAVA-OUTPUT-CAST ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-CAST)
    NULL)
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *JAVA-SEMICOLON-STATEMENTS*
     (LIST* SYM-JAVA-OUTPUT-STELLA-JAVA_ASSIGN
      ...
      SYM-JAVA-OUTPUT-STELLA-JAVA_RETURN SYM-JAVA-OUTPUT-STELLA-JAVA_SIGNAL
