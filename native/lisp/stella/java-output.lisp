...
(CL:DECLAIM
 (CL:SPECIAL *CHARACTER-TYPE-TABLE* *MODULE* NULL-CHARACTER STANDARD-OUTPUT EOL
  NIL FALSE TRUE *CURRENT-STREAM* NULL-INTEGER *STELLA-MODULE*))
...
(CL:DEFUN JAVA-INDENT ()
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO
    ...
    (CL:SETQ I I) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " ")))
  :VOID)
...
(CL:DEFUN JAVA-OUTPUT-STATEMENT (STATEMENT)
  (CL:COND
   ((CL:EQ (SAFE-PRIMARY-TYPE STATEMENT) SGT-JAVA-OUTPUT-STELLA-CONS)
    (CL:PROGN
     (CL:WHEN (CL:EQ STATEMENT NIL) (CL:RETURN-FROM JAVA-OUTPUT-STATEMENT))
     (CL:IF (CL:OR (CONS? (%%VALUE STATEMENT)) (CL:EQ (%%VALUE STATEMENT) NULL))
      ...
      (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
       (CL:COND
        ...
        ((CL:EQ TEST-VALUE-000 SYM-JAVA-OUTPUT-STELLA-JAVA_ARRAY_INITIALIZER)
   ...
   (CL:T (CL:WHEN (CL:EQ STATEMENT NULL) (CL:RETURN-FROM JAVA-OUTPUT-STATEMENT))
...
(CL:DEFUN JAVA-OUTPUT-CLASS-DECLARATION (CLASSDEF)
  (CL:WHEN
   (CL:NOT (CL:EQ (UNWRAP-STRING (%%VALUE CLASSDEF)) STELLA::NULL-STRING))
  ...
  (CL:LET* ((M NULL) (ITER-000 (%%VALUE (%%REST CLASSDEF))))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    (CL:PROGN (CL:SETQ M (%%VALUE ITER-000))
     (CL:SETQ ITER-000 (%%REST ITER-000)))
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) (%WRAPPER-VALUE M) " ")))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
...
(CL:DEFUN JAVA-OUTPUT-EXCEPTION-CONSTRUCTOR (EXCEPTIONNAME)
  ...
  (JAVA-INDENT)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "public " EXCEPTIONNAME
   " (String message) {" EOL)
  (JAVA-BUMP-INDENT)
  (JAVA-INDENT)
  ...
  (CL:IF
   ...
   (CL:LET* ((COMMENT (%%VALUE (%%REST BODY))))
    ...
    (CL:IF (CL:EQ (%%REST BODY) NIL)
     (CL:RETURN-FROM JAVA-SEPARATE-COMMENT-FROM-BODY (CL:VALUES NIL COMMENT))
...
(CL:DEFUN JAVA-OUTPUT-METHOD (METHOD)
  (CL:LET* ((BODY NULL) (COMMENT NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ COMMENT NULL)) (JAVA-INDENT)
    (JAVA-OUTPUT-JAVADOC-COMMENT (%%VALUE (%%REST COMMENT)) (FOURTH METHOD)
     (%%VALUE (%%VALUE (%%REST METHOD))) STELLA::NULL-STRING
     STELLA::NULL-STRING))
   (JAVA-INDENT) (JAVA-OUTPUT-METHOD-SIGNATURE METHOD)
   (CL:COND
    ((CL:NOT (CL:EQ BODY NIL))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " {" EOL)
     (JAVA-BUMP-INDENT) (JAVA-OUTPUT-STATEMENT BODY) (JAVA-UNBUMP-INDENT)
    ...
    ((MEMBER? (%%VALUE (%%REST METHOD)) (WRAP-STRING "void"))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Note: Void method `"
      (%%VALUE (%%REST (%%REST METHOD))) "' has an empty body." EOL)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " {}" EOL))
    (CL:T
     (CL:WARN "Method `~A' has an empty body but is not abstract or void!"
...
(%%DEFCONSMETHOD JAVA-OUTPUT-LITERAL ((CONS CL:CONS))
  (CL:IF (CL:EQ CONS NIL)
   ...
   (CL:PROGN
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "(" (%%VALUE CONS) " . ")
...
(CL:DEFUN JAVA-INDENT-STATEMENT? (STATEMENT)
  (CL:IF (CL:NOT (CL:EQ STATEMENT NULL))
   (CL:RETURN-FROM JAVA-INDENT-STATEMENT?
    (CL:NOT
     (MEMBER?
      (GET-QUOTED-TREE "((JAVA_STATEMENTS JAVA_CATCH) \"/STELLA\")" "/STELLA")
      (%%VALUE STATEMENT))))
  ...
  (CL:LET* ((DECLARATION NULL) (ITER-000 DECLARATIONS))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:WHEN (CL:NOT (CL:EQ (%%VALUE (%%REST (%%REST DECLARATION))) NULL))
     (JAVA-INDENT)
     (JAVA-OUTPUT-JAVADOC-COMMENT (%%VALUE (%%REST (%%REST DECLARATION))) NIL
      NULL STELLA::NULL-STRING STELLA::NULL-STRING))
...
(CL:DEFUN JAVA-HEURISTICALLY-TRANSLATE-NAME (STELLA-NAME CASE-CONVERTED-NAME-LIST)
  ...
  (CL:LET* ((TEST-VALUE-000 FALSE))
   (CL:IF (%CASE-SENSITIVE? *MODULE*) (CL:SETQ TEST-VALUE-000 TRUE)
    (CL:LET* ((FOUND?-000 FALSE))
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (CL:WHEN (UPPER-CASE-CHARACTER? CH) (CL:SETQ FOUND?-000 TRUE)
        (CL:RETURN))))
      ...
      (CL:RETURN-FROM JAVA-HEURISTICALLY-TRANSLATE-NAME
       (JAVA-TRANSLATE-NAMESTRING
        (MAKE-MATCHING-NAME-FULL STELLA-NAME KWD-JAVA-OUTPUT-CLEVER
         KWD-JAVA-OUTPUT-CLEVER "- " "" "" "" "" KWD-JAVA-OUTPUT-UPCASE "_"))))
     (CL:PROGN
      (CL:RETURN-FROM JAVA-HEURISTICALLY-TRANSLATE-NAME
       (JAVA-TRANSLATE-NAMESTRING
        (MAKE-MATCHING-NAME-FULL STELLA-NAME KWD-JAVA-OUTPUT-CLEVER
         KWD-JAVA-OUTPUT-CLEVER "- " "" "" "" "" KWD-JAVA-OUTPUT-TITLECASEX
         ""))))))))
(CL:DECLAIM
...
(CL:DEFUN JAVA-OUTPUT-COMMENT-STRING (COMMENT COMMENT-START COMMENT-CONTINUATION COMMENT-END JAVADOC-COMMENT? CASE-CONVERTED-NAME-LIST)
  ...
  (CL:LET*
   ((RETURN? FALSE) (ESCAPE? FALSE) (INSIDE-QUOTE? FALSE)
    (SAVED-STREAM *CURRENT-STREAM*) (STRING-STREAM NULL)
    (STELLA-NAME STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING STELLA-NAME))
      ...
      (CL:CASE CHAR
       (#\Linefeed
        (CL:WHEN (CL:NOT RETURN?)
         (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL) (JAVA-INDENT)
         (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
          COMMENT-CONTINUATION)))
       (#\Return (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
        (JAVA-INDENT)
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) COMMENT-CONTINUATION)
        (CL:SETQ RETURN? TRUE) (CL:SETQ ESCAPE? FALSE) (CL:GO :CONTINUE))
       (#\`
        (CL:COND
         (ESCAPE? (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR))
         (INSIDE-QUOTE?
          (CL:WARN "Encountered unescaped ` inside ` form in comment `~A'"
           COMMENT)
        ...
        (CL:COND
         ...
         (INSIDE-QUOTE? (CL:SETQ *CURRENT-STREAM* SAVED-STREAM)
          ...
          (CL:IF JAVADOC-COMMENT?
           (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "</code>")
           (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "'"))
          (CL:SETQ INSIDE-QUOTE? FALSE))
         (CL:T (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR))))
       (#\\
        (CL:IF ESCAPE? (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) CHAR)
         (CL:PROGN (CL:SETQ ESCAPE? TRUE) (CL:SETQ RETURN? FALSE)
          (CL:GO :CONTINUE))))
       (CL:OTHERWISE
        ...
        (CL:SETQ RETURN? FALSE) (CL:SETQ ESCAPE? FALSE)))
      :CONTINUE)))
   (CL:IF
...
(CL:DEFUN JAVA-OUTPUT-JAVADOC-COMMENT (TREE PARAMETER-NAMES-AND-TYPES RETURN-VALUE AUTHOR VERSION)
  ...
  (CL:LET* ((PARAMETER-NAMES NIL))
   ...
   (JAVA-OUTPUT-COMMENT-STRING (%WRAPPER-VALUE TREE) "/** " " * "
    STELLA::NULL-STRING TRUE PARAMETER-NAMES)
   (CL:LET* ((N NULL) (ITER-001 PARAMETER-NAMES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:PROGN (CL:SETQ N (%%VALUE ITER-001))
   ...
   (CL:WHEN
    ...
    (JAVA-INDENT)
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " * @version " VERSION
     EOL))
   (JAVA-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " */" EOL))
  :VOID)
...
(CL:DEFUN FIXUP-DOCUMENTATION-STRING-QUOTING (DOCUMENTATION)
  ...
  (CL:LET*
   ((CLOSINGQUOTEPOSITION NULL-INTEGER) (PRECEDINGCHARACTER NULL-CHARACTER)
    (OPENPARENTHESES "([{")
   ...
   (CL:LET*
    ((I NULL-INTEGER) (CHAR NULL-CHARACTER) (VECTOR-000 DOCUMENTATION)
      ...
      (CL:CASE CHAR
       ((#\` #\') (CL:WHEN (CL:= I CLOSINGQUOTEPOSITION) (CL:GO :CONTINUE))
        ...
        (CL:WHEN
         ...
         (CL:WHEN (DEFINED? CLOSINGQUOTEPOSITION)
...
(CL:DEFUN JAVA-OUTPUT-VERBATIM (VERBATIMSTATEMENT)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (SUBSTITUTE-TEMPLATE-VARIABLES-IN-STRING (%WRAPPER-VALUE VERBATIMSTATEMENT)
...
(CL:DEFUN JAVA-OUTPUT-BLOCK (BLOCK)
  (CL:LET* ((FIRSTSTATEMENT? TRUE))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "{ ") (JAVA-BUMP-INDENT)
...
(CL:DEFUN JAVA-OUTPUT-GLOBAL-DEFINITION (GLOBAL)
  (CL:WHEN (CL:NOT (CL:EQ (%WRAPPER-VALUE (%%VALUE GLOBAL)) STELLA::NULL-STRING))
   (JAVA-INDENT)
   (JAVA-OUTPUT-JAVADOC-COMMENT (%%VALUE GLOBAL) NIL NULL STELLA::NULL-STRING
    STELLA::NULL-STRING))
  (JAVA-INDENT)
   ...
   (CL:T
    (CL:LET* ((TEST-VALUE-000 (%%VALUE STATEMENT)))
     (CL:COND
      ...
      ((CL:EQ TEST-VALUE-000 SYM-JAVA-OUTPUT-STELLA-JAVA_BLOCK)
       (CL:RETURN-FROM JAVA-STATEMENT-RETURNS?
        (JAVA-LAST-STATEMENT-RETURNS? (%%VALUE (%%REST (%%REST STATEMENT))))))
      ((CL:EQ TEST-VALUE-000 SYM-JAVA-OUTPUT-STELLA-JAVA_IF)
       (CL:RETURN-FROM JAVA-STATEMENT-RETURNS?
...
(CL:DEFUN JAVA-OUTPUT-CASE (CASE)
  (CL:LET*
      ...
      (CL:LET*
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        (CL:PROGN (CL:SETQ COND (%%VALUE ITER-001))
         (CL:SETQ ITER-001 (%%REST ITER-001)))
        (JAVA-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "case ")
        (JAVA-OUTPUT-STATEMENT COND)
        (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ": " EOL)))
      (JAVA-BUMP-INDENT) (JAVA-OUTPUT-STATEMENT (%%VALUE (%%REST CONDITION)))
      (JAVA-UNBUMP-INDENT)
      (CL:WHEN
       (CL:NOT (JAVA-LAST-STATEMENT-RETURNS? (%%VALUE (%%REST CONDITION))))
       (JAVA-INDENT)
   ...
   (JAVA-BUMP-INDENT) (JAVA-OUTPUT-STATEMENT DEFAULTCASE) (JAVA-UNBUMP-INDENT)
...
(CL:DEFUN JAVA-OUTPUT-COND (COND)
  (CL:LET*
   ((DEFAULTCONDITION (%%VALUE COND)) (CONDITIONS (%%VALUE (%%REST COND)))
    (FIRSTCONDITION (%%VALUE CONDITIONS)) (RESTCONDITIONS (%%REST CONDITIONS)))
   (CL:WHEN
...
(CL:DEFUN JAVA-PROGN? (TREE)
  (CL:RETURN-FROM JAVA-PROGN?
   (CL:AND (CONS? TREE) (CL:EQ (%%VALUE TREE) SYM-JAVA-OUTPUT-STELLA-JAVA_PROGN))))

;;; (DEFUN (JAVA-BLOCK? BOOLEAN) ...)

(CL:DEFUN JAVA-BLOCK? (TREE)
  (CL:RETURN-FROM JAVA-BLOCK?
   (CL:AND (CONS? TREE) (CL:EQ (%%VALUE TREE) SYM-JAVA-OUTPUT-STELLA-JAVA_BLOCK))))
...
(CL:DEFUN JAVA-OUTPUT-FOREACH (LOOP)
  (CL:LET*
   ((CONTINUATIONTEST (%%VALUE LOOP)) (NEXTASSIGNMENTS (%%VALUE (%%REST LOOP)))
...
(CL:DEFUN JAVA-OUTPUT-PRINT-STREAM (EXPS NATIVESTREAM?)
  (CL:LET*
   ((STREAM (%%VALUE EXPS)) (FOUNDEOL? FALSE) (EMBEDDEDEOL? FALSE)
    (PRINTITEMS NIL))
   (CL:LET* ((I NULL) (ITER-000 (%%REST EXPS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ I (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
     (CL:WHEN FOUNDEOL? (CL:SETQ EMBEDDEDEOL? TRUE) (CL:RETURN))
     (CL:WHEN (JAVA-END-OF-LINE-TOKEN? I) (CL:SETQ FOUNDEOL? TRUE))))
   (CL:WHEN EMBEDDEDEOL?
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "{" EOL)
    (JAVA-BUMP-INDENT))
      ...
      (CL:IF (JAVA-END-OF-LINE-TOKEN? ITEM)
       (CL:PROGN (CL:WHEN EMBEDDEDEOL? (JAVA-INDENT))
        (JAVA-HELP-OUTPUT-PRINT-STREAM STREAM
         (JAVA-REVERSE-AND-CONSOLIDATE-STRINGS PRINTITEMS) NATIVESTREAM? TRUE)
        (CL:WHEN EMBEDDEDEOL?
         (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ PRINTITEMS NIL)) (CL:WHEN EMBEDDEDEOL? (JAVA-INDENT))
    (JAVA-HELP-OUTPUT-PRINT-STREAM STREAM
     (JAVA-REVERSE-AND-CONSOLIDATE-STRINGS PRINTITEMS) NATIVESTREAM? FALSE)
    (CL:WHEN EMBEDDEDEOL?
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ";" EOL)))
...
(CL:DEFUN JAVA-REVERSE-AND-CONSOLIDATE-STRINGS (ITEMS)
  ...
  (CL:LET* ((OUTPUT-ITEMS NIL) (LAST-ITEM (%%VALUE ITEMS)))
   (CL:LET* ((NEW-ITEM NULL) (ITER-000 (%%REST ITEMS)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:PROGN (CL:SETQ NEW-ITEM (%%VALUE ITER-000))
      (CL:SETQ ITER-000 (%%REST ITER-000)))
...
(CL:DEFUN JAVA-HELP-OUTPUT-PRINT-STREAM (STREAM EXPS NATIVESTREAM? ENDOFLINE?)
  (CL:COND
   ((CL:EQ STREAM SYM-JAVA-OUTPUT-STELLA-JAVA_STANDARD_OUT)
    (CL:IF
     (CL:OR (CL:NOT (CL:EQ (GET-STELLA-CLASS "SYSTEM" FALSE) NULL))
   ...
   (CL:T (JAVA-OUTPUT-STATEMENT STREAM)
    (CL:WHEN (CL:NOT NATIVESTREAM?)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ".nativeStream"))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
...
(CL:DEFUN JAVA-HELP-OUTPUT-BINARY-OPERATOR (EXPRESSION NESTLEVEL)
  ...
  (CL:LET*
   ...
   (CL:IF (JAVA-INDENTABLE-BINARY-OPERATOR? OP)
    (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) EOL)
     (JAVA-INDENT) (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) "    ")
     (CL:LET*
...
(CL:DEFUN JAVA-OUTPUT-TERNARY-OPERATOR (EXPRESSION)
  (CL:LET*
   ((ARG1 (%%VALUE EXPRESSION)) (OP1 (%%VALUE (%%REST EXPRESSION)))
    (ARG2 (%%VALUE (%%REST (%%REST EXPRESSION)))) (OP2 (FOURTH EXPRESSION))
...
(CL:DEFUN HELP-STARTUP-JAVA-OUTPUT3 ()
  (CL:PROGN
   (DEFINE-FUNCTION-OBJECT "JAVA-STELLA-PACKAGE"
    "(DEFUN (JAVA-STELLA-PACKAGE STRING) ())" (CL:FUNCTION JAVA-STELLA-PACKAGE)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-BUMP-INDENT" "(DEFUN JAVA-BUMP-INDENT ())"
    (CL:FUNCTION JAVA-BUMP-INDENT) NULL)
   (DEFINE-FUNCTION-OBJECT "JAVA-UNBUMP-INDENT" "(DEFUN JAVA-UNBUMP-INDENT ())"
    (CL:FUNCTION JAVA-UNBUMP-INDENT) NULL)
   ...
   (DEFINE-METHOD-OBJECT "(DEFMETHOD JAVA-OUTPUT-LITERAL ((OBJECT OBJECT)))"
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD JAVA-OUTPUT-LITERAL ((STRING STRING)))"
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-AREF"
    "(DEFUN JAVA-OUTPUT-AREF ((STATEMENT CONS)))" (CL:FUNCTION JAVA-OUTPUT-AREF)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-METHOD"
    "(DEFUN JAVA-OUTPUT-METHOD ((METHOD CONS)))" (CL:FUNCTION JAVA-OUTPUT-METHOD)
    NULL)
   ...
   (DEFINE-METHOD-OBJECT
    ...
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD JAVA-OUTPUT-LITERAL ((SYMBOL SYMBOL)))"
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT
    "(DEFMETHOD JAVA-OUTPUT-LITERAL ((SURROGATE SURROGATE)))"
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   (DEFINE-METHOD-OBJECT "(DEFMETHOD JAVA-OUTPUT-LITERAL ((KEYWORD KEYWORD)))"
    (CL:FUNCTION JAVA-OUTPUT-LITERAL) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-MAKE"
    "(DEFUN JAVA-OUTPUT-MAKE ((STATEMENT CONS)))" (CL:FUNCTION JAVA-OUTPUT-MAKE)
    NULL)
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-CAST"
    "(DEFUN JAVA-OUTPUT-CAST ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-CAST) NULL)
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-PROGN"
    "(DEFUN JAVA-OUTPUT-PROGN ((PROGN CONS)))" (CL:FUNCTION JAVA-OUTPUT-PROGN)
    NULL)
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-BLOCK"
    "(DEFUN JAVA-OUTPUT-BLOCK ((BLOCK CONS)))" (CL:FUNCTION JAVA-OUTPUT-BLOCK)
    NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-THROW"
    "(DEFUN JAVA-OUTPUT-THROW ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-THROW)
    NULL)
   (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-CATCH"
    "(DEFUN JAVA-OUTPUT-CATCH ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-CATCH)
    NULL)
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:SETQ *JAVA-SEMICOLON-STATEMENTS*
     (LIST* SYM-JAVA-OUTPUT-STELLA-JAVA_ASSIGN
      SYM-JAVA-OUTPUT-STELLA-JAVA_UNARY_OP SYM-JAVA-OUTPUT-STELLA-JAVA_BINARY_OP
      SYM-JAVA-OUTPUT-STELLA-JAVA_TERNARY_OP SYM-JAVA-OUTPUT-STELLA-JAVA_CAST
      SYM-JAVA-OUTPUT-STELLA-JAVA_FUNCALL
      SYM-JAVA-OUTPUT-STELLA-JAVA_FUNCTION_CALL
      SYM-JAVA-OUTPUT-STELLA-JAVA_FUNCTION_POINTER
      SYM-JAVA-OUTPUT-STELLA-JAVA_MAKE SYM-JAVA-OUTPUT-STELLA-JAVA_METHOD_CALL
      ...
      SYM-JAVA-OUTPUT-STELLA-JAVA_PRINT_STREAM SYM-JAVA-OUTPUT-STELLA-JAVA_RETURN
      SYM-JAVA-OUTPUT-STELLA-JAVA_SIGNAL
      SYM-JAVA-OUTPUT-STELLA-JAVA_SLOT_VALUE_SETTER
      SYM-JAVA-OUTPUT-STELLA-JAVA_THROW SYM-JAVA-OUTPUT-STELLA-JAVA_VERBATIM
      NIL)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7) (HELP-STARTUP-JAVA-OUTPUT3)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-SIGNAL"
     "(DEFUN JAVA-OUTPUT-SIGNAL ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-SIGNAL)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-BREAK"
     "(DEFUN JAVA-OUTPUT-BREAK ((TAG CONS)))" (CL:FUNCTION JAVA-OUTPUT-BREAK)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-CASE"
     "(DEFUN JAVA-OUTPUT-CASE ((CASE CONS)))" (CL:FUNCTION JAVA-OUTPUT-CASE)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-COND"
     "(DEFUN JAVA-OUTPUT-COND ((COND CONS)))" (CL:FUNCTION JAVA-OUTPUT-COND)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-PROGN?"
     "(DEFUN (JAVA-PROGN? BOOLEAN) ((TREE OBJECT)))" (CL:FUNCTION JAVA-PROGN?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-BLOCK?"
     "(DEFUN (JAVA-BLOCK? BOOLEAN) ((TREE OBJECT)))" (CL:FUNCTION JAVA-BLOCK?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-IF"
     "(DEFUN JAVA-OUTPUT-IF ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-IF) NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-WHEN"
     "(DEFUN JAVA-OUTPUT-WHEN ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-WHEN)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-UNLESS"
     "(DEFUN JAVA-OUTPUT-UNLESS ((TREE CONS)))" (CL:FUNCTION JAVA-OUTPUT-UNLESS)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-LOOP"
     "(DEFUN JAVA-OUTPUT-LOOP ((LOOP CONS)))" (CL:FUNCTION JAVA-OUTPUT-LOOP)
     NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-OUTPUT-WHILE"
     "(DEFUN JAVA-OUTPUT-WHILE ((LOOP CONS)))" (CL:FUNCTION JAVA-OUTPUT-WHILE)
     NULL)
    ...
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-JAVA-OUTPUT-STELLA-STARTUP-JAVA-OUTPUT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION)
      SYM-JAVA-OUTPUT-STELLA-METHOD-STARTUP-CLASSNAME
      (WRAP-STRING "_Startup-Java-Output") NULL-STRING-WRAPPER)))
