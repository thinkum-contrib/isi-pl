...
(CL:DEFUN JAVA-OUTPUT-CLASS (CLASSDEF EXCEPTIONCLASS?)
  ...
  (CL:WHEN EXCEPTIONCLASS?
   (JAVA-OUTPUT-EXCEPTION-CONSTRUCTOR (%WRAPPER-VALUE (FOURTH CLASSDEF))))
  ...
  (CL:LET* ((LASTPARAMETER NULL) (ALLBUTLASTPARAMETER NULL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ LASTPARAMETER NULL))
    (CL:LET* ((CASTEDLASTPARAMETER LASTPARAMETER))
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
      (%WRAPPER-VALUE (%%VALUE CASTEDLASTPARAMETER)) " "
      (%WRAPPER-VALUE (%%VALUE (%%REST CASTEDLASTPARAMETER))))))
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ")"))
  :VOID)
...
(CL:DEFUN JAVA-OUTPUT-COMMENT-STRING (COMMENT COMMENT-START COMMENT-CONTINUATION COMMENT-END)
  ...
  (CL:LET* ((RETURNP FALSE))
   ...
   (CL:WHEN
    ...
    (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) COMMENT-END EOL)))
  :VOID)
   ...
   (CL:LET* ((N NULL) (ITER-000 PARAMETERNAMES))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " * @param "
      (%WRAPPER-VALUE (%%VALUE (%%REST N))) EOL))))
  (CL:WHEN
   (CL:AND (CL:NOT (CL:EQ RETURNVALUE NULL))
    (CL:NOT (STRING-EQL? (%WRAPPER-VALUE RETURNVALUE) "void")))
   (JAVA-INDENT)
   (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) " * @return "
    (%WRAPPER-VALUE RETURNVALUE) EOL))
  (CL:WHEN
...
(CL:DEFUN JAVA-OUTPUT-VERBATIM (VERBATIMSTATEMENT)
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (SUBSTITUTE-TEMPLATE-VARIABLES-IN-STRING
    (%WRAPPER-VALUE VERBATIMSTATEMENT) *JAVA-STELLA-PACKAGE-MAPPING*))
  :VOID)
...
(CL:DEFUN JAVA-OUTPUT-GLOBAL-DEFINITION (GLOBAL)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (%WRAPPER-VALUE (%%VALUE (%%REST GLOBAL))))
...
(CL:DEFUN JAVA-OUTPUT-SPECIAL-DEFINITION (SPECIAL)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
   (%WRAPPER-VALUE (%%VALUE (%%REST SPECIAL))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
  ...
  (CL:COND
   ...
   (CL:T (JAVA-OUTPUT-STATEMENT STREAM)
    (CL:WHEN (CL:NOT NATIVESTREAM?)
     (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*) ".native_stream"))))
  (%%PRINT-STREAM (%NATIVE-STREAM *CURRENT-STREAM*)
