...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* STANDARD-OUTPUT FALSE-WRAPPER
  NULL-STRING-WRAPPER NULL-INTEGER NIL-LIST NIL EOL *CHARACTER-UPCASE-TABLE*
  NULL-CHARACTER))
...
(CL:DEFUN TEXINFO-ESCAPE-STRING (STRING)
  ...
  (CL:LET*
    ...
    (CL:SETQ ESCAPED-STRING
     (CL:THE CL:SIMPLE-STRING
      (CL:COPY-SEQ
       (CL:THE CL:SIMPLE-STRING
        (MAKE-STRING
         (CL:+ (CL:THE CL:FIXNUM (CL:LENGTH STRING)) N-OF-SPECIAL-CHARS)
         NULL-CHARACTER)))))
   ...
   (CL:LET ((S ESCAPED-STRING)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING S))
    (CL:THE CL:SIMPLE-STRING S))))

;;; (DEFUN (YIELD-TEXINFO-TYPE-SPEC STRING) ...)

(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) YIELD-TEXINFO-TYPE-SPEC))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) YIELD-TEXINFO-BARE-NAME))
(CL:DEFUN YIELD-TEXINFO-BARE-NAME (NAME)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE NAME)))
   (CL:COND
    ((SUBTYPE-OF-STRING? TEST-VALUE-000) (CL:PROGN (%WRAPPER-VALUE NAME)))
    ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
     (CL:PROGN (TEXINFO-ESCAPE-STRING (STRING-DOWNCASE (%SYMBOL-NAME NAME)))))
    ((CL:EQ TEST-VALUE-000 SGT-MANUALS-STELLA-CONS)
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) YIELD-TEXINFO-CONSTANT))
(CL:DEFUN YIELD-TEXINFO-CONSTANT (NAME)
  (CONCATENATE "@code{"
   (CONCATENATE (TEXINFO-ESCAPE-STRING (STRING-DOWNCASE (%SYMBOL-NAME NAME)))
    "}")))
  ...
  (CL:LET* ((COMMAND-INDEX 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM COMMAND-INDEX))
   (CL:LOOP (CL:SETQ START (CL:1+ START))
    (CL:CASE
     ...
     (CL:OTHERWISE
      (CL:RETURN-FROM TEXINFO-GET-PARAGRAPH-COMMAND STELLA::NULL-STRING))))
...
(CL:DEFUN TEXINFO-DESCRIBE-DOCUMENTATION-STRING (OBJECT DOCUMENTATION STREAM PARAMETERS)
  ...
  (CL:LET*
   ((QUOTED-NAME STELLA::NULL-STRING) (CLOSING-QUOTE-POSITION NULL-INTEGER)
    (BEGINNING-OF-LINE? CL:T) (INSIDE-EXAMPLE? CL:NIL)
         ...
         (CL:WHEN (CL:NOT INSIDE-EXAMPLE?)
          (CL:COND
           ((CL:= I CLOSING-QUOTE-POSITION)
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "}") (CL:GO :CONTINUE))
           ((CL:OR (CL:= I 0)
            ...
            (CL:WHEN (CL:NOT (CL:= CLOSING-QUOTE-POSITION NULL-INTEGER))
             (CL:SETQ QUOTED-NAME
              (STRING-UPCASE
               (SUBSEQUENCE DOCUMENTATION (CL:1+ I) CLOSING-QUOTE-POSITION)))
             (CL:LET* ((FOUND?-000 CL:NIL))
...
(CL:DEFUN TEXINFO-DESCRIBE-VARIABLE (VARIABLE STREAM)
  ...
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
   ...
   EOL)
  (TEXINFO-DESCRIBE-DOCUMENTATION-STRING VARIABLE (%DOCUMENTATION VARIABLE)
   STREAM NULL)
  ...
  (CL:LET*
   ...
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
    ...
    (CL:IF (CL:= I 1) (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " : ")
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM) ", "))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (YIELD-TEXINFO-TYPE-SPEC SUPER))
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:LET* ((VALUE-000 NULL-INTEGER))
      ...
      (CL:LET*
       ((I NULL-INTEGER) (ITER-001 (INTERVAL 1 (CL:- END P-NAME-LENGTH))))
...
(CL:DEFUN TEXINFO-GET-REAL-METHOD-PARAMETERS-AND-TYPES (METHOD)
  (CL:LET*
   ((STRINGIFIED-SOURCE (%METHOD-STRINGIFIED-SOURCE METHOD))
    ...
    (DEFINITION-TREE NULL) (RETURN-TYPES-TREE NIL) (PARAMETERS-TREE NIL))
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS (YIELD-TYPE-SPEC-TREE P-TYPE) NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST REAL-PARAMETER-TYPES) NIL)
        ...
        (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST REAL-PARAMETER-TYPES)
         COLLECT-000)))
      (CL:PROGN
       (CL:SETF (%%REST COLLECT-000) (CONS (YIELD-TYPE-SPEC-TREE P-TYPE) NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:LET*
    ((RTYPE NULL)
     ...
     (CL:IF (CL:EQ COLLECT-001 NULL)
      (CL:PROGN (CL:SETQ COLLECT-001 (CONS (YIELD-TYPE-SPEC-TREE RTYPE) NIL))
       (CL:IF (CL:EQ (%THE-CONS-LIST REAL-RETURN-TYPES) NIL)
        ...
        (ADD-CONS-TO-END-OF-CONS-LIST (%THE-CONS-LIST REAL-RETURN-TYPES)
         COLLECT-001)))
      (CL:PROGN
       (CL:SETF (%%REST COLLECT-001) (CONS (YIELD-TYPE-SPEC-TREE RTYPE) NIL))
   ...
   (CL:WHEN (CL:NOT (CL:EQ STRINGIFIED-SOURCE STELLA::NULL-STRING))
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (NEXT? IT)) DO
      (CL:SETQ PTREE (%%VALUE ITER-002)) (VALUE-SETTER IT (LAST PTREE))
      (CL:SETQ ITER-002 (%%REST ITER-002))))
    (CL:LET*
     ((RTREE NULL) (ITER-003 RETURN-TYPES-TREE)
      (IT (ALLOCATE-ITERATOR REAL-RETURN-TYPES)))
     (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-003 NIL)) (NEXT? IT)) DO
      (CL:SETQ RTREE (%%VALUE ITER-003)) (VALUE-SETTER IT RTREE)
      (CL:SETQ ITER-003 (%%REST ITER-003)))))
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE
       ...
       (CL:IF (CL:AND (CL:= I 1) (STRING-EQL? CATEGORY "Method"))
        ...
        (TEXINFO-DESCRIBE-TYPE-DECLARATION (%WRAPPER-VALUE P-NAME) P-TYPE
         MODIFIER STREAM))
       (CL:WHEN (CL:< I N-OF-ARGUMENTS)
        (CL:IF
...
(CL:DEFUN TEXINFO-DESCRIBE-UNIMPLEMENTED-OBJECT (SPECIFICATION STREAM)
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "@deffn ??? "
   (YIELD-TEXINFO-BARE-NAME SPECIFICATION) EOL "Not yet implemented." EOL
   "@end deffn" EOL))
    ...
    (CL:COND
     ...
     ((SUBTYPE-OF-SYMBOL? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ OBJECT (LOOKUP-FUNCTION SPECIFICATION))
       ...
       (CL:WHEN (CL:EQ OBJECT NULL)
        (CL:SETQ OBJECT (GET-OBJECT (%SYMBOL-NAME SPECIFICATION) NULL)))))
     ((CL:EQ TEST-VALUE-000 SGT-MANUALS-STELLA-CONS)
      (CL:PROGN
     ...
     (CL:WHEN
      ...
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS FUNCTION NIL))
        (CL:SETQ COLLECT-000 (%%REST COLLECT-000)))))))
   (CL:LET*
    ((METHOD NULL) (ITER-001 (ALL-METHODS MODULE CL:T)) (COLLECT-001 NULL))
...
(CL:DEFUN MANUAL-DESCRIBE-OBJECT (OBJECT SPECIFICATION STREAM)
  ...
  (CL:COND
         ...
         (CL:LET*
          ((HANDLERNAME
            (INTERN-SYMBOL-IN-MODULE
             ...
             (%HOME-CONTEXT SYM-MANUALS-UTILITIES-MANUAL-DESCRIBE-OBJECT)
             CL:T))
   ...
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" *MANUAL-OUTPUT-LANGUAGE*
      "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
...
(CL:DEFUN MANUAL-EXPAND-TEMPLATE-COMMANDS (LINE)
  ...
  (CL:LET*
   ((COMMANDSTART (STRING-SEARCH LINE *MANUAL-TEMPLATE-COMMAND-PREFIX* 0)))
   ...
   (CL:HANDLER-CASE
    (CL:LET*
     ((PREFIX (SUBSEQUENCE LINE 0 COMMANDSTART))
      (RESTLINE
       (NEW-INPUT-STRING-STREAM
        (SUBSEQUENCE LINE
         (CL:+ COMMANDSTART
          (CL:THE CL:FIXNUM (CL:LENGTH *MANUAL-TEMPLATE-COMMAND-PREFIX*)))
         NULL-INTEGER)))
     ...
     (CL:LET* ((*MANUAL-OUTPUT-STREAM* (NEW-OUTPUT-STRING-STREAM)))
      (CL:DECLARE (CL:SPECIAL *MANUAL-OUTPUT-STREAM*)) (%EVALUATE EXPRESSION)
      (CL:SETQ LINE (READ-LINE RESTLINE))
    ...
    (CL:CONDITION (E)
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
      "*** ERROR while expanding template command: " (EXCEPTION-MESSAGE E) EOL
...
(CL:DEFUN HELP-STARTUP-MANUALS1 ()
  (CL:PROGN
   ...
   (CL:SETQ |SYM-MANUALS-STELLA-&REST|
    (INTERN-RIGID-SYMBOL-WRT-MODULE "&REST" (GET-STELLA-MODULE "/STELLA" CL:T)
     0))
   (CL:SETQ |SYM-MANUALS-STELLA-&BODY|
    (INTERN-RIGID-SYMBOL-WRT-MODULE "&BODY" (GET-STELLA-MODULE "/STELLA" CL:T)
     0))
...
documentation string defined will be documented (this is a catch-all option).\")"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP-MANUALS"
     "(DEFUN STARTUP-MANUALS () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-MANUALS)
     NULL)
    (CL:LET*
     ((FUNCTION (LOOKUP-FUNCTION SYM-MANUALS-UTILITIES-STARTUP-MANUALS)))
...
:INFIX-PARAMETER-LISTS.\")")
    ...
    (CL:LET*
     ...
     (CL:SETQ *TEXINFO-WORD-DELIMITERS*
      (CL:LET ((S DELIMITERS)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING S))
       (CL:THE CL:SIMPLE-STRING S))))
    (DEFINE-STELLA-GLOBAL-VARIABLE-FROM-STRINGIFIED-SOURCE
