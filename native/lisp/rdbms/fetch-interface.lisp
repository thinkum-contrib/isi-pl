...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* NULL-STRING-WRAPPER *MODULE* *POWERLOOM-LOCK* *CONTEXT* *REVERSEPOLARITY?* *MEMOIZATION-ENABLED?* TRUE-WRAPPER FALSE-WRAPPER
  MEMOIZED-NULL-VALUE NULL-INTEGER EOL NIL))
...
the server."
  ...
  (CL:LET* ((*PRINTREADABLY?* CL:NIL)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
   (CL:LET* ((URL (NEW-OUTPUT-STRING-STREAM)) (REPLY NULL)) (%%PRINT-STREAM (%NATIVE-STREAM URL) "/agent/runner?plan=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) QUERY-ID) (%%PRINT-STREAM (%NATIVE-STREAM URL) "&TaskID=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) TASK-ID) (%%PRINT-STREAM (%NATIVE-STREAM URL) "-")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) SUBTASK-ID) (%%PRINT-STREAM (%NATIVE-STREAM URL) "&Database=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) DATABASE) (%%PRINT-STREAM (%NATIVE-STREAM URL) "&") (%%PRINT-STREAM (%NATIVE-STREAM URL) PARAMETERS)
    (CL:SETQ REPLY (CL:FUNCALL (AUTOLOAD "HTTP/HTTP-GET-DATA" "webtools" SGT-FETCH-INTERFACE-RDBMS-RUN-FETCH-QUERY CL:T) HOST PORT (THE-STRING-READER URL) NULL))
    (CL:LET* ((EXP NULL) (ITER-000 (XML-EXPRESSIONS REPLY NULL)))
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ EXP (%VALUE ITER-000))
      (CL:WHEN (CL:AND (ISA? EXP SGT-FETCH-INTERFACE-STELLA-CONS) (ISA? (%%VALUE EXP) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT))
       (CL:LET* ((ITEM-000 (%%VALUE EXP)))
        (CL:COND
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "PlanExecution") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING)) (CL:RETURN-FROM RUN-FETCH-QUERY EXP))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST EXP)))) "'")
...
(CL:DEFUN NORMALIZE-FETCH-QUERY-ARGUMENTS (PROPOSITION ARGUMENTS)
  (CL:WHEN (CL:NOT (STRING? ARGUMENTS))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `" ARGUMENTS "'." EOL)
  ...
  (CL:LET*
   ((ARGS (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE ARGUMENTS))) (CURSOR NULL) (QUERYID STELLA::NULL-STRING) (OUTPUTARGS NIL) (PARAMETERS NIL)
    (PARAMETERVALUE NULL))
   ...
   (CL:COND
     ...
     (CL:PROGN
      (CL:LET* ((TEST-VALUE-000 CL:NIL))
       (CL:SETQ TEST-VALUE-000
        (CL:OR (CL:AND (CONS? (%%VALUE ARGS)) (CL:LET ((X (LENGTH ARGS))) (CL:DECLARE (CL:TYPE CL:FIXNUM X)) (CL:NOT (CL:LOGBITP 0 X))))
         (CL:AND (CL:NOT (CONS? (%%VALUE ARGS))) (CL:LOGBITP 0 (CL:THE CL:FIXNUM (LENGTH ARGS))))))
       (CL:WHEN TEST-VALUE-000
        (CL:LET* ((ALWAYS?-000 CL:T))
         (CL:LET* ((ARG NULL) (ITER-000 (%%REST ARGS)))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ARG (%%VALUE ITER-000)) (CL:WHEN (CONS? ARG) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
           (CL:SETQ ITER-000 (%%REST ITER-000))))
         (CL:SETQ TEST-VALUE-000 ALWAYS?-000)))
       (CL:WHEN (CL:NOT TEST-VALUE-000)
        (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
         (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `" ARGUMENTS "'." EOL)
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ OUTARG (%%VALUE ITER-001))
          (CL:IF (CL:EQ COLLECT-000 NULL)
           (CL:PROGN (CL:SETQ COLLECT-000 (CONS (WRAP-STRING (COERCE-TO-STRING OUTARG)) NIL))
            (CL:IF (CL:EQ OUTPUTARGS NIL) (CL:SETQ OUTPUTARGS COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST OUTPUTARGS COLLECT-000)))
           (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (WRAP-STRING (COERCE-TO-STRING OUTARG)) NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
          (CL:SETQ ITER-001 (%%REST ITER-001))))
        (CL:SETQ CURSOR (%%REST CURSOR)))
       (CL:T (CL:SETQ OUTPUTARGS (GET-TABLE-COLUMN-NAMES (GET-DESCRIPTION (%OPERATOR PROPOSITION))))
        (CL:WHEN (CL:EQ OUTPUTARGS NULL)
         (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
          (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "PARSING ERROR: Fetch-EDB-query: missing output arg/column specifications for: `"
            (GET-DESCRIPTION (%OPERATOR PROPOSITION)) "'." EOL)
      ...
      (CL:SETQ QUERYID (COERCE-TO-STRING (%%VALUE CURSOR))) (CL:SETQ CURSOR (%%REST CURSOR)) (CL:SETQ PARAMETERS CURSOR)
      (CL:LOOP WHILE (CL:NOT (CL:EQ CURSOR NIL)) DO (FIRST-SETTER CURSOR (WRAP-STRING (COERCE-TO-STRING (%%VALUE CURSOR))))
       (CL:SETQ PARAMETERVALUE (%%VALUE (%%REST CURSOR)))
       (CL:COND
        ((QUESTION-MARK-SYMBOL? PARAMETERVALUE)
         (CL:LET* ((VALUE-000 NULL))
          (CL:LET*
           ((ARG NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (VAR NULL)
            (ITER-002 (%THE-CONS-LIST (%IO-VARIABLE-NAMES (GET-DESCRIPTION (%OPERATOR PROPOSITION))))))
           (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000))
           (CL:LOOP WHILE (CL:AND (CL:< INDEX-000 LENGTH-000) (CL:NOT (CL:EQ ITER-002 NIL))) DO
            (CL:SETQ ARG (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000)) (CL:SETQ VAR (%%VALUE ITER-002))
            (CL:WHEN (CL:EQ VAR PARAMETERVALUE) (CL:SETQ VALUE-000 ARG) (CL:RETURN)) (CL:SETQ INDEX-000 (CL:1+ INDEX-000)) (CL:SETQ ITER-002 (%%REST ITER-002))))
          (CL:SETQ PARAMETERVALUE VALUE-000))
         (CL:WHEN (CL:EQ PARAMETERVALUE NULL)
          (CL:LET* ((STREAM-003 (NEW-OUTPUT-STRING-STREAM)))
           (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM-003) "PARSING ERROR: Fetch-EDB-query: non-existent input variable: `" (%%VALUE (%%REST CURSOR)) "'." EOL)
    ...
    (CL:T
     (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
      (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-004) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `" ARGUMENTS "'." EOL)
   ...
   (CL:LET* ((SELF-010 (NEW-PROPERTY-LIST))) (CL:SETF (%THE-PLIST SELF-010) PARAMETERS) (CL:LET* ((VALUE-001 SELF-010)) (CL:VALUES QUERYID OUTPUTARGS VALUE-001)))))

;;; (DEFUN (HELP-MEMOIZE-FETCH-EDB-QUERY (ITERATOR OF CONS)) ...)

(CL:DEFUN HELP-MEMOIZE-FETCH-EDB-QUERY (PROPOSITION DATABASE ARGUMENTS)
  (CL:LET* ((QUERYID STELLA::NULL-STRING) (OUTPUTARGS NIL) (PARAMETERS NULL) (RELATION (GET-DESCRIPTION (%OPERATOR PROPOSITION))) (PARAMS (NEW-OUTPUT-STRING-STREAM)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING QUERYID)) (CL:MULTIPLE-VALUE-SETQ (QUERYID OUTPUTARGS PARAMETERS) (NORMALIZE-FETCH-QUERY-ARGUMENTS PROPOSITION ARGUMENTS))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST PARAMETERS)) (I NULL-INTEGER) (ITER-001 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-000)) (CL:SETQ VALUE (%%VALUE (%%REST ITER-000))) (CL:SETQ I ITER-001)
     (CL:WHEN (CL:OR (CL:EQ KEY NULL) (CL:EQ VALUE NULL)) (CL:RETURN-FROM HELP-MEMOIZE-FETCH-EDB-QUERY NULL))
     (CL:WHEN (CL:> I 1) (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) "&")) (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) (UNWRAP-STRING KEY))
     (CL:SETQ VALUE (ARGUMENT-BOUND-TO VALUE)) (CL:WHEN (CL:EQ VALUE NULL) (CL:RETURN-FROM HELP-MEMOIZE-FETCH-EDB-QUERY NULL))
     (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) "=" (COERCE-POWERLOOM-OBJECT-TO-STRING VALUE NULL)) (CL:SETQ ITER-000 (%%REST (%%REST ITER-000)))
     (CL:SETQ ITER-001 (CL:1+ ITER-001))))
   (CL:LET* ((TEMP-000 (UNWRAP-STRING (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-HOST)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-000))
    (CL:LET* ((TEMP-001 (UNWRAP-INTEGER (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-PORT)))) (CL:DECLARE (CL:TYPE CL:FIXNUM TEMP-001))
     (CL:LET* ((TEMP-002 (UNWRAP-STRING (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-TASK-ID)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-002))
      (CL:LET* ((TEMP-003 (UNWRAP-STRING (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-SUBTASK-ID)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-003))
       (CL:LET* ((TEMP-004 (UNWRAP-STRING (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-DB-NAME)))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-004))
        (CL:LET*
         ((VALUE-000
           (ALLOCATE-ITERATOR
            (PARSE-FETCH-XML-ANSWER
             (RUN-FETCH-QUERY (CL:IF (CL:NOT (CL:EQ TEMP-000 STELLA::NULL-STRING)) TEMP-000 "pegasus2.isi.edu")
              (CL:IF (CL:NOT (CL:= TEMP-001 NULL-INTEGER)) TEMP-001 8080) QUERYID (CL:IF (CL:NOT (CL:EQ TEMP-002 STELLA::NULL-STRING)) TEMP-002 "TASK")
              (CL:IF (CL:NOT (CL:EQ TEMP-003 STELLA::NULL-STRING)) TEMP-003 "SUBTASK") (CL:IF (CL:NOT (CL:EQ TEMP-004 STELLA::NULL-STRING)) TEMP-004 "EELD-DATABASE")
...
(CL:DEFUN FETCH-EDB-QUERY (PROPOSITION DATABASE ARGUMENTS)
  ...
  (CL:LET* ((MEMO-TABLE-000 NULL) (MEMOIZED-ENTRY-000 NULL) (MEMOIZED-VALUE-000 NULL))
   (CL:WHEN *MEMOIZATION-ENABLED?* (CL:SETQ MEMO-TABLE-000 (%SURROGATE-VALUE SGT-FETCH-INTERFACE-RDBMS-F-FETCH-EDB-QUERY-MEMO-TABLE-000))
    (CL:WHEN (CL:EQ MEMO-TABLE-000 NULL)
     (INITIALIZE-MEMOIZATION-TABLE SGT-FETCH-INTERFACE-RDBMS-F-FETCH-EDB-QUERY-MEMO-TABLE-000 "(:MAX-VALUES 10000 :TIMESTAMPS (:META-KB-UPDATE :EXTERNAL-DB-UPDATE))")
     (CL:SETQ MEMO-TABLE-000 (%SURROGATE-VALUE SGT-FETCH-INTERFACE-RDBMS-F-FETCH-EDB-QUERY-MEMO-TABLE-000)))
   ...
   (CL:COND ((CL:NOT (CL:EQ MEMOIZED-VALUE-000 NULL)) (CL:WHEN (CL:EQ MEMOIZED-VALUE-000 MEMOIZED-NULL-VALUE) (CL:SETQ MEMOIZED-VALUE-000 NULL)))
    (CL:T (CL:SETQ MEMOIZED-VALUE-000 (NEW-MEMOIZABLE-ITERATOR (HELP-MEMOIZE-FETCH-EDB-QUERY PROPOSITION DATABASE ARGUMENTS)))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND (ISA? (%%VALUE XMLANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XMLANSWER)) "PlanExecution") (STRING-EQL? (%NAMESPACE-URI (%%VALUE XMLANSWER)) STELLA::NULL-STRING))))
    (CL:WARN "Fetch-EDB-query Error: Didn't find a Fetch Answer in `~A'" XMLANSWER) (CL:RETURN-FROM PARSE-FETCH-XML-ANSWER NIL))
   (CL:LET* ((ANSWER NULL) (ITER-000 (%%REST (%%REST XMLANSWER))))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ANSWER (%%VALUE ITER-000))
     (CL:WHEN (CL:AND (ISA? ANSWER SGT-FETCH-INTERFACE-STELLA-CONS) (ISA? (%%VALUE ANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT))
      (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
       (CL:COND
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "OBJECT") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:SETQ TUPLES (CONS (PARSE-ONE-FETCH-XML-OBJECT (%%REST (%%REST ANSWER)) RELATION OUTPUTCOLUMNS) TUPLES)))
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (UNWRAP-STRING (%%VALUE (%%REST (%%REST ANSWER)))) "'")
          (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "ERROR") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "`" (UNWRAP-STRING (%%VALUE (%%REST (%%REST ANSWER)))) "'")
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ COLUMN (%%VALUE ITER-000))
     (CL:LET* ((ITEM NULL) (ITER-001 OBJECT))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-001))
       (CL:WHEN (EQL-TO-STRING? COLUMN (%NAME (%%VALUE ITEM))) (CL:SETQ TUPLE (CONS (%%VALUE (%%REST (%%REST ITEM))) TUPLE))
    ...
    (CL:LET* ((MDL-000 (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 *MODULE*)) (CXT-000 MDL-000))
     ...
     (CL:LET* ((*MODULE* MDL-000) (*CONTEXT* CXT-000)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
      (WITH-PROCESS-LOCK *POWERLOOM-LOCK*
       (CL:LET*
        ((SECONDARYDBNAME (LOOKUP KEYS (WRAP-STRING "sdatabase"))) (FETCHDB (%GET (COPY-CONS-TREE (WRAP-STRING "fetch-edb")) NIL))
         (TASKID (LOOKUP KEYS (WRAP-STRING "task"))) (SUBTASKID (LOOKUP KEYS (WRAP-STRING "subtask"))) (COMMAND (LOOKUP KEYS (WRAP-STRING "command"))) (RESULT NULL))
        (CL:LET* ((SELF-000 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-000) OUT)
         (CL:LET* ((WRAPPEDOUT SELF-000))
          (CL:WHEN (CL:NOT (CL:EQ FETCHDB NULL))
           (CL:WHEN (CL:NOT (CL:EQ SECONDARYDBNAME NULL))
            (PLI::ASSERT-NARY-PROPOSITION (CONS (GET-RELATION SGT-FETCH-INTERFACE-RDBMS-DB-NAME) (CONS FETCHDB (CONS SECONDARYDBNAME NIL))) NULL NULL))
           (CL:WHEN (CL:NOT (CL:EQ TASKID NULL))
            (PLI::ASSERT-NARY-PROPOSITION (CONS (GET-RELATION SGT-FETCH-INTERFACE-RDBMS-TASK-ID) (CONS FETCHDB (CONS TASKID NIL))) NULL NULL))
           (CL:WHEN (CL:NOT (CL:EQ SUBTASKID NULL))
            (PLI::ASSERT-NARY-PROPOSITION (CONS (GET-RELATION SGT-FETCH-INTERFACE-RDBMS-SUBTASK-ID) (CONS FETCHDB (CONS SUBTASKID NIL))) NULL NULL)))
          (CL:SETQ RESULT (EVALUATE-LOGIC-COMMAND (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE COMMAND)) CL:T))
          (CL:COND
           ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE RESULT) SGT-FETCH-INTERFACE-LOGIC-QUERY-ITERATOR)
            (CL:PROGN
             (CL:LET* ((SOLUTION NULL) (ITER-000 (ALLOCATE-ITERATOR (%SOLUTIONS RESULT))))
              (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SOLUTION (%VALUE ITER-000))
               (CL:LET* ((ELT NULL) (VECTOR-000 (%BINDINGS SOLUTION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000)) (I NULL-INTEGER) (ITER-001 0))
                (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-000 LENGTH-000 I ITER-001))
                (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO (CL:SETQ ELT (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000)) INDEX-000)) (CL:SETQ I ITER-001)
                 (CL:WHEN (CL:> I 0) (%%PRINT-STREAM OUT " ")) (PRINT-LOGICAL-FORM ELT WRAPPEDOUT) (CL:SETQ INDEX-000 (CL:1+ INDEX-000))
                 (CL:SETQ ITER-001 (CL:1+ ITER-001))))
               (%%PRINT-STREAM OUT "<br>")))))
...
(CL:DEFUN BUILD-ONE-TABLE-ASSERTION (TABLE-RELATION TUPLE DB-MODULE)
  (CL:LET* ((ASSERTION NIL) (COLNO -1)) (CL:DECLARE (CL:TYPE CL:FIXNUM COLNO))
   (CL:LET* ((I NULL-INTEGER) (ITER-000 0) (UPPER-BOUND-000 (ARITY TABLE-RELATION)) (UNBOUNDED?-000 (CL:= UPPER-BOUND-000 NULL-INTEGER)) (COLLECT-000 NULL))
    ...
    (CL:LOOP WHILE (CL:OR UNBOUNDED?-000 (CL:<= ITER-000 UPPER-BOUND-000)) DO (CL:SETQ I ITER-000)
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN (CL:SETQ COLLECT-000 (CONS NULL-STRING-WRAPPER NIL))
       (CL:IF (CL:EQ ASSERTION NIL) (CL:SETQ ASSERTION COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST ASSERTION COLLECT-000)))
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS NULL-STRING-WRAPPER NIL)) (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (CL:1+ ITER-000))))
   (CL:LET* ((ITEM NULL) (ITER-001 TUPLE))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-001))
     (CL:SETQ COLNO (GET-TABLE-COLUMN-INDEX TABLE-RELATION (%NAME (%%VALUE ITEM)) DB-MODULE))
     ...
     (CL:WHEN (CL:NOT (CL:EQ (%%REST (%%REST (%%REST ITEM))) NIL)) (CL:WARN "Extra elements found in `~A'" ITEM)) (CL:SETQ ITER-001 (%%REST ITER-001))))
   (CL:LET* ((ITEM NULL) (ITER-002 (%%REST ASSERTION)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-002))
     (CL:WHEN (CL:EQ (UNWRAP-STRING ITEM) STELLA::NULL-STRING) (CL:WARN "Some table relation values not set: `~A'" ASSERTION) (CL:RETURN))
     (CL:SETQ ITER-002 (%%REST ITER-002))))
   (CL:SETF (%%REST ASSERTION) (COERCE-VALUES (%%REST ASSERTION) (GET-RELATION-TABLE-INFO TABLE-RELATION))) (NTH-SETTER ASSERTION TABLE-RELATION 0) ASSERTION))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND (ISA? (%%VALUE XML-ANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XML-ANSWER)) "PlanExecution") (STRING-EQL? (%NAMESPACE-URI (%%VALUE XML-ANSWER)) STELLA::NULL-STRING))))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ANSWER (%%VALUE ITER-000))
      (CL:WHEN (CL:AND (ISA? ANSWER SGT-FETCH-INTERFACE-STELLA-CONS) (ISA? (%%VALUE ANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT))
       (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
        (CL:COND
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "OBJECT") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:SETQ ASSERTIONS (CONS (BUILD-ONE-TABLE-ASSERTION TABLE-RELATION (%%REST (%%REST ANSWER)) DB-MODULE) ASSERTIONS)))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ANSWER)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "ERROR") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM))) (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ANSWER)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-001))))))))
      (CL:SETQ ITER-000 (%%REST ITER-000)))))
   ASSERTIONS))
...
(CL:DEFUN LOOKUP-STRING-VALUE (RELATION INSTANCE MOD)
  (CL:LET* ((ITER (PLI::RETRIEVE (LIST* (WRAP-INTEGER 1) (CONS (OBJECT-NAME RELATION) (CONS INSTANCE (CONS SYM-FETCH-INTERFACE-LOGIC-?X NIL))) NIL) MOD NULL)))
   (CL:IF (NEXT? ITER) (%WRAPPER-VALUE (%VALUE ITER)) STELLA::NULL-STRING)))
...
to find the appropriate relations."
  ...
  (CL:LET*
   ((DB-TABLE (LOOKUP-STRING-VALUE (PLI::GET-RELATION "QUERY-TABLE-NAME" DB-MODULE NULL) FETCH-QUERY DB-MODULE))
    (PARAMETER-NAME (LOOKUP-STRING-VALUE (PLI::GET-RELATION "QUERY-PARAMETER-NAME" DB-MODULE NULL) FETCH-QUERY DB-MODULE))
    (FETCH-ANSWER (RUN-FETCH-QUERY HOST PORT (%SYMBOL-NAME (OBJECT-NAME FETCH-QUERY)) TASK-ID SUBTASK-ID DATABASE (CONCATENATE PARAMETER-NAME "=" PARAMETER-VALUE))))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING DB-TABLE PARAMETER-NAME)) (GENERATE-TABLE-ASSERTIONS DB-TABLE FETCH-ANSWER TARGET-MODULE DB-MODULE)))
...
(CL:DEFUN STARTUP-FETCH-INTERFACE ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/RDBMS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2) (CL:SETQ SGT-FETCH-INTERFACE-RDBMS-RUN-FETCH-QUERY (INTERN-RIGID-SYMBOL-WRT-MODULE "RUN-FETCH-QUERY" NULL 1))
    (CL:SETQ SGT-FETCH-INTERFACE-STELLA-CONS (INTERN-RIGID-SYMBOL-WRT-MODULE "CONS" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
    (CL:SETQ SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT (INTERN-RIGID-SYMBOL-WRT-MODULE "XML-ELEMENT" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
    ...
    (CL:SETQ SGT-FETCH-INTERFACE-RDBMS-F-FETCH-EDB-QUERY-MEMO-TABLE-000 (INTERN-RIGID-SYMBOL-WRT-MODULE "F-FETCH-EDB-QUERY-MEMO-TABLE-000" NULL 1))
    (CL:SETQ SGT-FETCH-INTERFACE-LOGIC-QUERY-ITERATOR (INTERN-RIGID-SYMBOL-WRT-MODULE "QUERY-ITERATOR" (GET-STELLA-MODULE "/LOGIC" CL:T) 1))
    (CL:SETQ SYM-FETCH-INTERFACE-LOGIC-?X (INTERN-RIGID-SYMBOL-WRT-MODULE "?X" (GET-STELLA-MODULE "/LOGIC" CL:T) 0))
    (CL:SETQ SYM-FETCH-INTERFACE-RDBMS-STARTUP-FETCH-INTERFACE (INTERN-RIGID-SYMBOL-WRT-MODULE "STARTUP-FETCH-INTERFACE" NULL 0))
    (CL:SETQ SYM-FETCH-INTERFACE-STELLA-METHOD-STARTUP-CLASSNAME (INTERN-RIGID-SYMBOL-WRT-MODULE "METHOD-STARTUP-CLASSNAME" (GET-STELLA-MODULE "/STELLA" CL:T) 0)))
...
the server.\" :PUBLIC? TRUE)" (CL:FUNCTION RUN-FETCH-QUERY) NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "FETCH-EDB-QUERY" "(DEFUN (FETCH-EDB-QUERY (ITERATOR OF CONS)) ((PROPOSITION PROPOSITION) (DATABASE LOGIC-OBJECT) (ARGUMENTS OBJECT)))"
    ...
    (DEFINE-FUNCTION-OBJECT "START-FETCH-EDB-GATEWAY" "(DEFUN START-FETCH-EDB-GATEWAY ((PORT INTEGER)))" (CL:FUNCTION START-FETCH-EDB-GATEWAY) NULL)
    (DEFINE-FUNCTION-OBJECT "EVALUATE-FETCH-QUERY" "(DEFUN EVALUATE-FETCH-QUERY ((KEYS KEY-VALUE-LIST) (OUT NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION EVALUATE-FETCH-QUERY)
     NULL)
    (DEFINE-FUNCTION-OBJECT "BUILD-ONE-TABLE-ASSERTION" "(DEFUN (BUILD-ONE-TABLE-ASSERTION CONS) ((TABLE-RELATION NAMED-DESCRIPTION) (TUPLE CONS) (DB-MODULE MODULE)))"
...
meta information is found in `db-module'.\")" (CL:FUNCTION GENERATE-TABLE-ASSERTIONS) NULL)
    (DEFINE-FUNCTION-OBJECT "LOOKUP-STRING-VALUE" "(DEFUN (LOOKUP-STRING-VALUE STRING) ((RELATION NAMED-DESCRIPTION) (INSTANCE LOGIC-OBJECT) (MOD MODULE)))"
...
to find the appropriate relations.\")" (CL:FUNCTION FETCH-QUERY-ASSERTIONS) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-FETCH-INTERFACE" "(DEFUN STARTUP-FETCH-INTERFACE () :PUBLIC? TRUE)" (CL:FUNCTION STARTUP-FETCH-INTERFACE) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-FETCH-INTERFACE-RDBMS-STARTUP-FETCH-INTERFACE)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-FETCH-INTERFACE-STELLA-METHOD-STARTUP-CLASSNAME (WRAP-STRING "_StartupFetchInterface") NULL-STRING-WRAPPER)))
