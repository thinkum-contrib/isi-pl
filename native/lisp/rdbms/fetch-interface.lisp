...
the server."
  ...
  (CL:LET* ((*PRINTREADABLY?* CL:NIL))
   ...
   (CL:LET* ((URL (NEW-OUTPUT-STRING-STREAM)) (REPLY NULL))
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "/agent/runner?plan=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) QUERY-ID)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&TaskID=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) TASK-ID)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "-")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL)
     SUBTASK-ID)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&Database=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) DATABASE)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&")
    (%%PRINT-STREAM (%NATIVE-STREAM URL) PARAMETERS)
    (CL:SETQ REPLY
     (CL:FUNCALL
      ...
      (CL:WHEN
       ...
       (CL:LET* ((ITEM-000 (%%VALUE EXP)))
        (CL:COND
         ...
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error")
           (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`"
            (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST EXP)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
         (CL:T))))))
    (CL:ERROR
     (NEW-STELLA-EXCEPTION "Fetch Query Error: No answer element found.")))))

;;; (DEFUN (NORMALIZE-FETCH-QUERY-ARGUMENTS STRING (CONS OF STRING-WRAPPER) PROPERTY-LIST) ...)

(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T CL:T) (CL:VALUES CL:SIMPLE-STRING CL:T CL:T))
...
(CL:DEFUN NORMALIZE-FETCH-QUERY-ARGUMENTS (PROPOSITION ARGUMENTS)
  (CL:WHEN (CL:NOT (STRING? ARGUMENTS))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (CL:LET* ((*PRINTREADABLY?* CL:T))
     (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
      "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `" ARGUMENTS
      "'." EOL)
  ...
  (CL:LET*
    ...
    ((CL:EQ (SAFE-PRIMARY-TYPE ARGS) SGT-FETCH-INTERFACE-STELLA-CONS)
     (CL:PROGN
       ...
       (CL:SETQ TEST-VALUE-000
        (CL:OR
         (CL:AND (CONS? (%%VALUE ARGS))
          (CL:LET ((X (LENGTH ARGS))) (CL:DECLARE (CL:TYPE CL:FIXNUM X))
        ...
        (CL:LET* ((ALWAYS?-000 CL:T))
         (CL:LET* ((ARG NULL) (ITER-000 (%%REST ARGS)))
          (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
           ...
           (CL:WHEN (CONS? ARG) (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
      ...
      (CL:COND
       ...
       (CL:T
        (CL:SETQ OUTPUTARGS
         (GET-TABLE-COLUMN-NAMES (GET-DESCRIPTION (%OPERATOR PROPOSITION))))
        ...
        ((QUESTION-MARK-SYMBOL? PARAMETERVALUE)
         (CL:LET* ((VALUE-000 NULL))
          (CL:LET*
           ...
           (CL:LOOP WHILE
            (CL:AND (CL:< INDEX-000 LENGTH-000) (CL:NOT (CL:EQ ITER-002 NIL)))
            DO
            (CL:SETQ ARG
   ...
   (CL:LET* ((SELF-010 (NEW-PROPERTY-LIST)))
    (CL:SETF (%THE-PLIST SELF-010) PARAMETERS)
...
(CL:DEFUN HELP-MEMOIZE-FETCH-EDB-QUERY (PROPOSITION DATABASE ARGUMENTS)
  (CL:LET*
   ...
   (CL:LET*
    ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST PARAMETERS))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) (UNWRAP-STRING KEY))
     (CL:SETQ VALUE (ARGUMENT-BOUND-TO VALUE))
     (CL:WHEN (CL:EQ VALUE NULL)
      (CL:RETURN-FROM HELP-MEMOIZE-FETCH-EDB-QUERY NULL))
     (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) "="
   ...
   (CL:LET*
    ...
    (CL:LET*
     ((TEMP-001
       (UNWRAP-INTEGER
        (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-PORT))))
     (CL:DECLARE (CL:TYPE CL:FIXNUM TEMP-001))
     (CL:LET*
      ((TEMP-002
        (UNWRAP-STRING
         (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-TASK-ID))))
      ...
      (CL:LET*
       ...
       (CL:LET*
        ((TEMP-004
          (UNWRAP-STRING
           (ACCESS-BINARY-VALUE DATABASE SGT-FETCH-INTERFACE-RDBMS-DB-NAME))))
            ...
            (PARSE-FETCH-XML-ANSWER
             (RUN-FETCH-QUERY
              (CL:IF (CL:NOT (CL:EQ TEMP-000 STELLA::NULL-STRING)) TEMP-000
               "pegasus2.isi.edu")
              (CL:IF (CL:NOT (CL:= TEMP-001 NULL-INTEGER)) TEMP-001 8080)
              QUERYID
              (CL:IF (CL:NOT (CL:EQ TEMP-002 STELLA::NULL-STRING)) TEMP-002
               "TASK")
              (CL:IF (CL:NOT (CL:EQ TEMP-003 STELLA::NULL-STRING)) TEMP-003
               "SUBTASK")
              (CL:IF (CL:NOT (CL:EQ TEMP-004 STELLA::NULL-STRING)) TEMP-004
               "EELD-DATABASE")
    ...
    (CL:NOT
     (CL:AND (ISA? (%%VALUE XMLANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XMLANSWER)) "PlanExecution")
       (STRING-EQL? (%NAMESPACE-URI (%%VALUE XMLANSWER))
        STELLA::NULL-STRING))))
    (CL:WARN "Fetch-EDB-query Error: Didn't find a Fetch Answer in `~A'"
     XMLANSWER)
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:WHEN
      ...
      (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
       (CL:COND
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "OBJECT")
          (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:SETQ TUPLES
          (CONS
           (PARSE-ONE-FETCH-XML-OBJECT (%%REST (%%REST ANSWER)) RELATION
            OUTPUTCOLUMNS)
           TUPLES)))
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error")
          (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
...
(CL:DEFUN EVALUATE-FETCH-QUERY (KEYS OUT)
  (CL:HANDLER-CASE
   (CL:LET* ((TEMP-000 (GET-STELLA-MODULE "EELD-EDB" CL:NIL)))
    (CL:LET*
     ((MDL-000 (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 *MODULE*))
      (CXT-000 MDL-000))
      ...
      (WITH-PROCESS-LOCK *POWERLOOM-LOCK*
       (CL:LET*
        ((SECONDARYDBNAME (LOOKUP KEYS (WRAP-STRING "sdatabase")))
         (FETCHDB (%GET (COPY-CONS-TREE (WRAP-STRING "fetch-edb")) NIL))
        ...
        (CL:LET* ((SELF-000 (NEW-OUTPUT-STREAM)))
         (CL:SETF (%NATIVE-STREAM SELF-000) OUT)
         (CL:LET* ((WRAPPEDOUT SELF-000))
          ...
          (CL:SETQ RESULT
           (EVALUATE-LOGIC-COMMAND
            (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE COMMAND)) CL:T))
          (CL:COND
           ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE RESULT)
            ...
            (CL:PROGN
             (CL:LET*
              ((SOLUTION NULL)
               (ITER-000 (ALLOCATE-ITERATOR (%SOLUTIONS RESULT))))
              (CL:LOOP WHILE (NEXT? ITER-000) DO
               (CL:SETQ SOLUTION (%VALUE ITER-000))
               (CL:LET*
                ((ELT NULL) (VECTOR-000 (%BINDINGS SOLUTION)) (INDEX-000 0)
  ...
  (CL:LET* ((ASSERTION NIL) (COLNO -1)) (CL:DECLARE (CL:TYPE CL:FIXNUM COLNO))
   ...
   (CL:LET* ((ITEM NULL) (ITER-001 TUPLE))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     (CL:SETQ ITEM (%%VALUE ITER-001))
     (CL:SETQ COLNO
      (GET-TABLE-COLUMN-INDEX TABLE-RELATION (%NAME (%%VALUE ITEM))
       DB-MODULE))
     (CL:IF (CL:< COLNO 0)
      (CL:WARN "Couldn't find column number for `~A' on relation `~A'" ITEM
       TABLE-RELATION)
...
meta information is found in `db-module'."
  ...
  (CL:LET*
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND
      (ISA? (%%VALUE XML-ANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XML-ANSWER)) "PlanExecution")
       (STRING-EQL? (%NAMESPACE-URI (%%VALUE XML-ANSWER))
        STELLA::NULL-STRING))))
       ...
       (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
        (CL:COND
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "OBJECT")
          ...
          (CL:SETQ ASSERTIONS
           (CONS
            (BUILD-ONE-TABLE-ASSERTION TABLE-RELATION (%%REST (%%REST ANSWER))
             DB-MODULE)
            ASSERTIONS)))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error")
           (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`"
            (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ANSWER)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "ERROR")
           (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
...
to find the appropriate relations."
  (CL:DECLARE
   (CL:TYPE CL:SIMPLE-STRING HOST TASK-ID SUBTASK-ID DATABASE PARAMETER-VALUE)
   (CL:TYPE CL:FIXNUM PORT))
  #+MCL
  (CL:CHECK-TYPE HOST CL:SIMPLE-STRING)
  #+MCL
  ...
  (CL:LET*
   ((DB-TABLE
    ...
    (PARAMETER-NAME
     (LOOKUP-STRING-VALUE
      (PLI::GET-RELATION "QUERY-PARAMETER-NAME" DB-MODULE NULL) FETCH-QUERY
      DB-MODULE))
    (FETCH-ANSWER
     (RUN-FETCH-QUERY HOST PORT (%SYMBOL-NAME (OBJECT-NAME FETCH-QUERY))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ SGT-FETCH-INTERFACE-RDBMS-F-FETCH-EDB-QUERY-MEMO-TABLE-000
     (INTERN-RIGID-SYMBOL-WRT-MODULE "F-FETCH-EDB-QUERY-MEMO-TABLE-000" NULL
      1))
    ...
    (CL:SETQ SYM-FETCH-INTERFACE-LOGIC-?X
     (INTERN-RIGID-SYMBOL-WRT-MODULE "?X" (GET-STELLA-MODULE "/LOGIC" CL:T)
      0))
...
is made to a remote http query server running on `host' port number `port'.
Returns a set of assertions for `target-module' using the information in `db-module'
to find the appropriate relations.\")" (CL:FUNCTION FETCH-QUERY-ASSERTIONS)
     NULL)
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-FETCH-INTERFACE-RDBMS-STARTUP-FETCH-INTERFACE)))
