       ...
       (CL:COND
        ((QUESTION-MARK-SYMBOL? PARAMETERVALUE)
         (CL:LET* ((VALUE-000 NULL))
          (CL:LET*
           ((ARG NULL) (VECTOR-000 (%ARGUMENTS PROPOSITION))
         ...
         (CL:LET* ((WRAPPEDOUT SELF-000))
          ...
          (CL:SETQ RESULT
           (EVALUATE-LOGIC-COMMAND
            (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE COMMAND))
            CL:T))
          (CL:COND
            ...
            (CL:PROGN
             (CL:LET*
              ...
              (CL:LOOP WHILE (NEXT? ITER-000) DO
               ...
               (CL:LET*
                ((ELT NULL) (VECTOR-000 (%BINDINGS SOLUTION))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND
      ...
      (CL:AND
       (STRING-EQL? (%NAME (%%VALUE XML-ANSWER)) "PlanExecution")
   ...
   ((ITER
     (PLI::RETRIEVE
      (LIST* (WRAP-INTEGER 1)
       ...
       NIL)
      MOD NULL)))
   (CL:IF (NEXT? ITER) (%WRAPPER-VALUE (%VALUE ITER))
    STELLA::NULL-STRING)))
   ...
   ((DB-TABLE
    ...
    (FETCH-ANSWER
     (RUN-FETCH-QUERY HOST PORT
      (%SYMBOL-NAME (OBJECT-NAME FETCH-QUERY)) TASK-ID SUBTASK-ID
      DATABASE (CONCATENATE PARAMETER-NAME "=" PARAMETER-VALUE))))
