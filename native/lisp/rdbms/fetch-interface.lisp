...
the server."
  ...
  (CL:LET* ((*PRINTREADABLY?* CL:NIL)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
   (CL:LET* ((URL (NEW-OUTPUT-STRING-STREAM)) (REPLY NULL)) (%%PRINT-STREAM (%NATIVE-STREAM URL) "/agent/runner?plan=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) QUERY-ID)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&TaskID=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) TASK-ID) (%%PRINT-STREAM (%NATIVE-STREAM URL) "-")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) SUBTASK-ID)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&Database=")
    (WRITE-HTML-ESCAPING-URL-SPECIAL-CHARACTERS (%NATIVE-STREAM URL) DATABASE)
    (%%PRINT-STREAM (%NATIVE-STREAM URL) "&") (%%PRINT-STREAM (%NATIVE-STREAM URL) PARAMETERS)
    (CL:SETQ REPLY
     ...
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ EXP (%VALUE ITER-000))
      (CL:WHEN
       ...
       (CL:LET* ((ITEM-000 (%%VALUE EXP)))
        (CL:COND
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "PlanExecution")
           (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         ...
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST EXP)))) "'")
...
(CL:DEFUN NORMALIZE-FETCH-QUERY-ARGUMENTS (PROPOSITION ARGUMENTS)
  (CL:WHEN (CL:NOT (STRING? ARGUMENTS))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `"
      ARGUMENTS "'." EOL)
  ...
  (CL:LET*
   ((ARGS (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE ARGUMENTS))) (CURSOR NULL) (QUERYID STELLA::NULL-STRING)
    (OUTPUTARGS NIL) (PARAMETERS NIL) (PARAMETERVALUE NULL))
   ...
   (CL:COND
      ...
      (CL:LET* ((TEST-VALUE-000 CL:NIL))
       ...
       (CL:WHEN (CL:NOT TEST-VALUE-000)
        (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
         (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `"
           ARGUMENTS "'." EOL)
    ...
    (CL:T
     (CL:LET* ((STREAM-004 (NEW-OUTPUT-STRING-STREAM)))
      (CL:LET* ((*PRINTREADABLY?* CL:T)) (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-004) "PARSING ERROR: Fetch-EDB-query: illegal pattern arguments: `"
        ARGUMENTS "'." EOL)
   ...
   (CL:LET* ((SELF-010 (NEW-PROPERTY-LIST))) (CL:SETF (%THE-PLIST SELF-010) PARAMETERS)
...
(CL:DEFUN HELP-MEMOIZE-FETCH-EDB-QUERY (PROPOSITION DATABASE ARGUMENTS)
  (CL:LET*
   ((QUERYID STELLA::NULL-STRING) (OUTPUTARGS NIL) (PARAMETERS NULL)
    (RELATION (GET-DESCRIPTION (%OPERATOR PROPOSITION))) (PARAMS (NEW-OUTPUT-STRING-STREAM)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING QUERYID))
   (CL:MULTIPLE-VALUE-SETQ (QUERYID OUTPUTARGS PARAMETERS) (NORMALIZE-FETCH-QUERY-ARGUMENTS PROPOSITION ARGUMENTS))
   (CL:LET* ((KEY NULL) (VALUE NULL) (ITER-000 (%THE-PLIST PARAMETERS)) (I NULL-INTEGER) (ITER-001 1))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ KEY (%%VALUE ITER-000))
     ...
     (CL:WHEN (CL:> I 1) (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) "&"))
     (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) (UNWRAP-STRING KEY)) (CL:SETQ VALUE (ARGUMENT-BOUND-TO VALUE))
     (CL:WHEN (CL:EQ VALUE NULL) (CL:RETURN-FROM HELP-MEMOIZE-FETCH-EDB-QUERY NULL))
     (%%PRINT-STREAM (%NATIVE-STREAM PARAMS) "=" (COERCE-POWERLOOM-OBJECT-TO-STRING VALUE NULL))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND (ISA? (%%VALUE XMLANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XMLANSWER)) "PlanExecution")
       (STRING-EQL? (%NAMESPACE-URI (%%VALUE XMLANSWER)) STELLA::NULL-STRING))))
     ...
     (CL:WHEN
      ...
      (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
       (CL:COND
        ...
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (UNWRAP-STRING (%%VALUE (%%REST (%%REST ANSWER)))) "'")
          (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
        ((CL:AND (STRING-EQL? (%NAME ITEM-000) "ERROR") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
         (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "`" (UNWRAP-STRING (%%VALUE (%%REST (%%REST ANSWER)))) "'")
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ COLUMN (%%VALUE ITER-000))
     (CL:LET* ((ITEM NULL) (ITER-001 OBJECT))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-001))
       (CL:WHEN (EQL-TO-STRING? COLUMN (%NAME (%%VALUE ITEM)))
            ...
            (CL:PROGN
             (CL:LET* ((SOLUTION NULL) (ITER-000 (ALLOCATE-ITERATOR (%SOLUTIONS RESULT))))
              (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ SOLUTION (%VALUE ITER-000))
               (CL:LET*
                ((ELT NULL) (VECTOR-000 (%BINDINGS SOLUTION)) (INDEX-000 0) (LENGTH-000 (LENGTH VECTOR-000))
   ...
   (CL:WHEN
    (CL:NOT
     (CL:AND (ISA? (%%VALUE XML-ANSWER) SGT-FETCH-INTERFACE-STELLA-XML-ELEMENT)
      (CL:AND (STRING-EQL? (%NAME (%%VALUE XML-ANSWER)) "PlanExecution")
       (STRING-EQL? (%NAMESPACE-URI (%%VALUE XML-ANSWER)) STELLA::NULL-STRING))))
      ...
      (CL:WHEN
       ...
       (CL:LET* ((ITEM-000 (%%VALUE ANSWER)))
        (CL:COND
         ...
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "Error") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ANSWER)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
         ((CL:AND (STRING-EQL? (%NAME ITEM-000) "ERROR") (STRING-EQL? (%NAMESPACE-URI ITEM-000) STELLA::NULL-STRING))
          (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
           (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "`" (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ANSWER)))) "'")
           (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-001))))))))
      (CL:SETQ ITER-000 (%%REST ITER-000)))))
   ASSERTIONS))
...
to find the appropriate relations."
  ...
  (CL:LET*
   ((DB-TABLE (LOOKUP-STRING-VALUE (PLI::GET-RELATION "QUERY-TABLE-NAME" DB-MODULE NULL) FETCH-QUERY DB-MODULE))
    ...
    (FETCH-ANSWER
     (RUN-FETCH-QUERY HOST PORT (%SYMBOL-NAME (OBJECT-NAME FETCH-QUERY)) TASK-ID SUBTASK-ID DATABASE
...
to find the appropriate relations.\")" (CL:FUNCTION FETCH-QUERY-ASSERTIONS) NULL)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-FETCH-INTERFACE-RDBMS-STARTUP-FETCH-INTERFACE)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-FETCH-INTERFACE-STELLA-METHOD-STARTUP-CLASSNAME
