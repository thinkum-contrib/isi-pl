...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* NULL-CHARACTER EOL STANDARD-OUTPUT *CHARACTER-TYPE-TABLE* NULL-INTEGER NIL
...
(CL:DEFCLASS CONNECTION (STANDARD-OBJECT)
  ((NATIVE-CONNECTION :ALLOCATION :INSTANCE :ACCESSOR %NATIVE-CONNECTION)
   (CONNECTION-STRING :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR
    %CONNECTION-STRING)
   (PROTOCOL :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %PROTOCOL)
   (DBMS-NAME :DOCUMENTATION "Indicates to what kind of DBMS we are connected." :ALLOCATION :INSTANCE :ACCESSOR
    %DBMS-NAME)
   (STATEMENT :DOCUMENTATION "Prepared statement associated with this connection." :ALLOCATION :INSTANCE :ACCESSOR
    %STATEMENT)
   (AUTO-COMMIT? :DOCUMENTATION "State of connection's auto-commit feature." :ALLOCATION :INSTANCE :ACCESSOR
    %AUTO-COMMIT?)
   (CREATION-TIME :DOCUMENTATION "Time at which connection was created." :ALLOCATION :INSTANCE :ACCESSOR
    %CREATION-TIME)
   (LAST-ACCESS-TIME :DOCUMENTATION "Time at which connection was last accessed." :ALLOCATION :INSTANCE :ACCESSOR
    %LAST-ACCESS-TIME)
   (RENEWAL-TIME :DOCUMENTATION "Time at or after which connection should be renewed." :ALLOCATION :INSTANCE :ACCESSOR
    %RENEWAL-TIME)))

(CL:DEFUN NEW-CONNECTION ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE CONNECTION))) (CL:SETF (%RENEWAL-TIME SELF) NULL)
   (CL:SETF (%LAST-ACCESS-TIME SELF) NULL) (CL:SETF (%CREATION-TIME SELF) NULL) (CL:SETF (%AUTO-COMMIT? SELF) CL:T)
   (CL:SETF (%STATEMENT SELF) NULL) (CL:SETF (%DBMS-NAME SELF) NULL) (CL:SETF (%PROTOCOL SELF) STELLA::NULL-STRING)
   (CL:SETF (%CONNECTION-STRING SELF) STELLA::NULL-STRING) (CL:SETF (%NATIVE-CONNECTION SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-CONNECTION-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-CONNECTION-STRING)
    (CL:IF SETVALUE? (CL:SETF (%CONNECTION-STRING SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%CONNECTION-STRING SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-PROTOCOL)
    (CL:IF SETVALUE? (CL:SETF (%PROTOCOL SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%PROTOCOL SELF)))))
   ...
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-STATEMENT)
    (CL:IF SETVALUE? (CL:SETF (%STATEMENT SELF) VALUE) (CL:SETQ VALUE (%STATEMENT SELF))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-AUTO-COMMIT?)
    (CL:IF SETVALUE? (CL:SETF (%AUTO-COMMIT? SELF) (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN VALUE))
     (CL:SETQ VALUE (CL:IF (%AUTO-COMMIT? SELF) TRUE-WRAPPER FALSE-WRAPPER))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-CREATION-TIME)
    (CL:IF SETVALUE? (CL:SETF (%CREATION-TIME SELF) VALUE) (CL:SETQ VALUE (%CREATION-TIME SELF))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-LAST-ACCESS-TIME)
    (CL:IF SETVALUE? (CL:SETF (%LAST-ACCESS-TIME SELF) VALUE) (CL:SETQ VALUE (%LAST-ACCESS-TIME SELF))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-RENEWAL-TIME)
    (CL:IF SETVALUE? (CL:SETF (%RENEWAL-TIME SELF) VALUE) (CL:SETQ VALUE (%RENEWAL-TIME SELF))))
   (CL:T
...
(CL:DEFCLASS PREPARED-STATEMENT (STANDARD-OBJECT)
  ((CONNECTION :ALLOCATION :INSTANCE :ACCESSOR %CONNECTION) (STATEMENT :ALLOCATION :INSTANCE :ACCESSOR %STATEMENT)
   ...
   (PARAMETERS :DOCUMENTATION "Stores parameters bound so far to support connection renewal." :ALLOCATION :INSTANCE
    :ACCESSOR %PARAMETERS)))

(CL:DEFUN NEW-PREPARED-STATEMENT ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE PREPARED-STATEMENT)))
   (CL:SETF (%PARAMETERS SELF) (NEW-KEY-VALUE-LIST)) (CL:SETF (%SQL SELF) STELLA::NULL-STRING)
   (CL:SETF (%STATEMENT SELF) NULL) (CL:SETF (%CONNECTION SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-PREPARED-STATEMENT-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-CONNECTION)
    (CL:IF SETVALUE? (CL:SETF (%CONNECTION SELF) VALUE) (CL:SETQ VALUE (%CONNECTION SELF))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-SQL)
    (CL:IF SETVALUE? (CL:SETF (%SQL SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%SQL SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-STELLA-PARAMETERS)
    (CL:IF SETVALUE? (CL:SETF (%PARAMETERS SELF) VALUE) (CL:SETQ VALUE (%PARAMETERS SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN NEW-DATABASE-EXCEPTION (MESSAGE)
  ...
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF
    (CL:MAKE-CONDITION (CL:QUOTE DATABASE-EXCEPTION) *CONDITION-MESSAGE-KEYWORD*
...
(CL:DEFUN NEW-RESULT-SET-ITERATOR ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE RESULT-SET-ITERATOR)))
   (CL:SETF (%FIRST-ITERATION? SELF) CL:T) (CL:SETF (%VALUE SELF) NULL) (CL:SETF (%TYPES SELF) NULL)
   (CL:SETF (%RESULT-SET SELF) NULL) SELF))
...
(CL:DEFUN ACCESS-RESULT-SET-ITERATOR-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-TYPES) (CL:IF SETVALUE? (CL:SETF (%TYPES SELF) VALUE) (CL:SETQ VALUE (%TYPES SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
   ...
   ((RESULT (NEW-CONNECTION)) (NATIVECONNECTION NULL)
    (OPTIONS
     (VET-OPTIONS KEYS-AND-VALUES
      (GET-QUOTED-TREE
       "((:PROTOCOL :SERVER-TYPE :DSN :DB-NAME :USER :PASSWORD :HOST :PORT :CONNECTION-STRING) \"/SDBC\")" "/SDBC"))))
   (CL:LET* ((TEMP-000 (LOOKUP OPTIONS KWD-SDBC-PROTOCOL)))
    (CL:LET*
     ((PROTOCOL
       (STRING-UPCASE
        (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) (%WRAPPER-VALUE TEMP-000) #+:CLSQL "CLSQL" #-:CLSQL "ODBC")))
     ...
     (CL:COND
      ...
      ((STRING-EQL? PROTOCOL "JDBC") (CL:SETQ CONNECTIONSTRING (MAKE-JDBC-CONNECTION-STRING OPTIONS))
       (CL:SETQ NATIVECONNECTION NULL))
      (CL:T
       (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
        (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Database Error: unsupported protocol `" PROTOCOL "'")
        (CL:ERROR (NEW-DATABASE-EXCEPTION (THE-STRING-READER STREAM-000))))))
     (CL:WHEN (CL:EQ NATIVECONNECTION NULL)
      (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "Database Error: `" PROTOCOL "' not supported in `" "Lisp" "'")
       (CL:ERROR (NEW-DATABASE-EXCEPTION (THE-STRING-READER STREAM-001)))))
     (CL:SETF (%NATIVE-CONNECTION RESULT) NATIVECONNECTION) (CL:SETF (%CONNECTION-STRING RESULT) CONNECTIONSTRING)
     (CL:SETF (%PROTOCOL RESULT) PROTOCOL) (CL:SETF (%CREATION-TIME RESULT) (MAKE-CURRENT-DATE-TIME))
     (CL:SETF (%LAST-ACCESS-TIME RESULT) (%CREATION-TIME RESULT))
     (CL:SETF (%RENEWAL-TIME RESULT)
      (TIME-ADD (%CREATION-TIME RESULT) (MAKE-TIME-DURATION 0 (CL:* (GET-MAX-CONNECTION-LIFETIME RESULT) 1000))))
     (LOGMSG "SDBC" KWD-SDBC-LOW (WRAP-STRING "Connect via ") (WRAP-STRING PROTOCOL) (WRAP-STRING ": ")
...
(CL:DEFUN MAYBE-RENEW-CONNECTION (CON)
  ...
  (CL:LET* ((CURRENTTIME (MAKE-CURRENT-DATE-TIME)) (STATEMENT (%STATEMENT CON)) (NEWCON NULL))
   (CL:SETF (%LAST-ACCESS-TIME CON) CURRENTTIME)
   (CL:WHEN (GREATER-EQUAL? CURRENTTIME (%RENEWAL-TIME CON))
    (LOGMSG "SDBC" KWD-SDBC-LOW (WRAP-STRING "Renewing ") (WRAP-STRING (%PROTOCOL CON)) (WRAP-STRING " connection: ")
     (WRAP-STRING (%CONNECTION-STRING CON)))
    (CL:SETQ NEWCON
     (%CONNECT
      (CONS-LIST KWD-SDBC-PROTOCOL (WRAP-STRING (%PROTOCOL CON)) KWD-SDBC-CONNECTION-STRING
       (WRAP-STRING (%CONNECTION-STRING CON)))))
    (%DISCONNECT CON) (CL:SETF (%NATIVE-CONNECTION CON) (%NATIVE-CONNECTION NEWCON))
    (CL:SETF (%CONNECTION-STRING CON) (%CONNECTION-STRING NEWCON)) (CL:SETF (%PROTOCOL CON) (%PROTOCOL NEWCON))
    (CL:SETF (%CREATION-TIME CON) (%CREATION-TIME NEWCON)) (CL:SETF (%LAST-ACCESS-TIME CON) (%LAST-ACCESS-TIME NEWCON))
    (CL:SETF (%RENEWAL-TIME CON) (%RENEWAL-TIME NEWCON))
    (CL:WHEN (CL:NOT (CL:EQ STATEMENT NULL)) (%PREPARE-STATEMENT CON (%SQL STATEMENT))
     (CL:SETF (%STATEMENT STATEMENT) (%STATEMENT (%STATEMENT CON))) (CL:SETF (%STATEMENT CON) STATEMENT)
     (CL:LET* ((POS NULL) (ENTRY NULL) (ITER-000 (%THE-KV-LIST (%PARAMETERS STATEMENT))))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ POS (%KEY ITER-000)) (CL:SETQ ENTRY (%VALUE ITER-000))
       (%BIND-PARAMETER STATEMENT (%WRAPPER-VALUE POS) (%%VALUE ENTRY) (%%VALUE (%%REST ENTRY)))
...
(CL:DEFUN MAKE-JDBC-CONNECTION-STRING (OPTIONS)
  ...
  (CL:LET* ((CON (NEW-OUTPUT-STRING-STREAM)) (VALUE STELLA::NULL-STRING)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING VALUE))
   (%%PRINT-STREAM (%NATIVE-STREAM CON) "jdbc:")
   (CL:LET* ((TEMP-000 (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-SERVER-TYPE))))
   ...
   (CL:COND
    ((STRING-EQL? VALUE "mysql") (%%PRINT-STREAM (%NATIVE-STREAM CON) "//")
     ...
     (CL:LET* ((PORT (LOOKUP OPTIONS KWD-SDBC-PORT)))
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE PORT)))
       (CL:COND
        ((SUBTYPE-OF-STRING? TEST-VALUE-000) (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM CON) ":" (UNWRAP-STRING PORT))))
        ((SUBTYPE-OF-INTEGER? TEST-VALUE-000)
         (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM CON) ":" (UNWRAP-INTEGER PORT))))
        (CL:T))))
       ...
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ OPTION (%%VALUE ITER-000))
        ...
        (CL:TAGBODY
         (CL:WHEN (CL:NOT (CL:EQ VAL NULL))
          (CL:COND
           ((CL:OR (CL:EQ OPTION KWD-SDBC-SERVER-TYPE) (CL:EQ OPTION KWD-SDBC-DSN) (CL:EQ OPTION KWD-SDBC-HOST)
             (CL:EQ OPTION KWD-SDBC-PORT) (CL:EQ OPTION KWD-SDBC-DB-NAME) (CL:EQ OPTION KWD-SDBC-PROTOCOL))
            (CL:GO :CONTINUE))
    ...
    ((STRING-EQL? VALUE "oracle") (%%PRINT-STREAM (%NATIVE-STREAM CON) "thin:")
     (CL:SETQ VALUE (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-USER)))
     (CL:WHEN (CL:NOT (CL:EQ VALUE STELLA::NULL-STRING)) (%%PRINT-STREAM (%NATIVE-STREAM CON) VALUE))
     (CL:LET* ((TEMP-001 (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-PASSWORD))))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEMP-001))
      (CL:SETQ VALUE (CL:IF (CL:NOT (CL:EQ TEMP-001 STELLA::NULL-STRING)) TEMP-001 "")))
     (%%PRINT-STREAM (%NATIVE-STREAM CON) "/" VALUE)
     (CL:LET* ((TEMP-002 (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-HOST))))
       ...
       (CL:LET* ((TEST-VALUE-001 (SAFE-PRIMARY-TYPE PORT)))
        (CL:COND
         ((SUBTYPE-OF-STRING? TEST-VALUE-001)
          (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM CON) ":" (UNWRAP-STRING PORT))))
...
(CL:DEFUN ODBC-CONNECTION-STRING? (STRING)
  ...
  #+MCL
  (CL:CHECK-TYPE STRING CL:SIMPLE-STRING)
  (CL:AND (CL:> (CL:THE CL:FIXNUM (CL:LENGTH STRING)) 0) (CL:NOT (JDBC-CONNECTION-STRING? STRING))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ OPTION (%%VALUE ITER-000))
     ...
     (CL:TAGBODY
      (CL:WHEN (CL:NOT (CL:EQ VALUE NULL))
       (CL:COND ((CL:EQ OPTION KWD-SDBC-USER) (%%PRINT-STREAM (%NATIVE-STREAM CON) "UID"))
        ((CL:EQ OPTION KWD-SDBC-PASSWORD) (%%PRINT-STREAM (%NATIVE-STREAM CON) "PWD"))
        ((CL:EQ OPTION KWD-SDBC-DB-NAME) (%%PRINT-STREAM (%NATIVE-STREAM CON) "DATABASE"))
        ((CL:EQ OPTION KWD-SDBC-HOST) (%%PRINT-STREAM (%NATIVE-STREAM CON) "SERVER"))
...
(CL:DEFUN CL-NATIVE-ODBC-CONNECT (CONNECTIONSTRING)
  ...
  (CL:LET* ((ERRORMSG STELLA::NULL-STRING) (NATIVECONNECTION NULL)) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING ERRORMSG))
   #+:allegro
       (cl:handler-case
          (cl:progn
            (cl:require :aodbc-v2)
...
(CL:DEFUN CL-NATIVE-CLSQL-CONNECT (OPTIONS SERVER-TYPE)
  ...
  (CL:LET*
   ((CONNECTION-STRING (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-CONNECTION-STRING)))
    (USER (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-USER))) (PASSWORD (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-PASSWORD)))
    (DB-NAME (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-DB-NAME))) (HOST (UNWRAP-STRING (LOOKUP OPTIONS KWD-SDBC-HOST)))
   ...
   (CL:WHEN (CL:NOT (CL:EQ ERROR-MESSAGE STELLA::NULL-STRING)) (CL:ERROR (NEW-DATABASE-EXCEPTION ERROR-MESSAGE)))
...
the disconnect is unsuccessful."
  (CL:LET* ((NATIVECONNECTION (%NATIVE-CONNECTION CONNECTION)))
   (CL:WHEN (CL:NOT (CL:EQ NATIVECONNECTION NULL)) (cl-native-disconnect nativeConnection)
    (LOGMSG "SDBC" KWD-SDBC-LOW (WRAP-STRING "Disconnecting ") (WRAP-STRING (%PROTOCOL CONNECTION))
     (WRAP-STRING " connection: ") (WRAP-STRING (%CONNECTION-STRING CONNECTION)))
...
values."
  ...
  (CL:LET* ((START 0) (END 0) (OPERATOR ""))
   (CL:DECLARE (CL:TYPE CL:FIXNUM START END) (CL:TYPE CL:SIMPLE-STRING OPERATOR))
   (CL:LOOP WHILE
    (CL:EQ
   ...
   (CL:OR (STRING-EQUAL? OPERATOR "SELECT") (STRING-EQUAL? OPERATOR "SHOW") (STRING-EQUAL? OPERATOR "DESCRIBE")
    (STRING-EQUAL? OPERATOR "EXPLAIN") (STRING-EQUAL? OPERATOR "ANALYZE"))))
   ...
   (CL:COND ((CL:OR (CL:EQ STELLATYPE NULL) (CL:EQ STELLATYPE SYM-SDBC-STELLA-NULL)) (cl:setq result :varchar))
    (CL:T
     (CL:LET* ((TEST-VALUE-000 STELLATYPE))
      (CL:COND ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-CALENDAR-DATE) (cl:setq result :varchar))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-STRING) (cl:setq result :varchar))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-INTEGER) (cl:setq result :int))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-LONG-INTEGER) (cl:setq result :long))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-DOUBLE-FLOAT) (cl:setq result :double))
...
(CL:DEFUN CL-TRANSLATE-SQL-TYPE-SPECIFIER (STELLATYPE)
  (CL:LET* ((RESULT NULL))
   (CL:COND ((CL:OR (CL:EQ STELLATYPE NULL) (CL:EQ STELLATYPE SYM-SDBC-STELLA-NULL)) (cl:setq result cl:t))
    (CL:T
     (CL:LET* ((TEST-VALUE-000 STELLATYPE))
      (CL:COND ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-CALENDAR-DATE) (cl:setq result cl:t))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-STRING) (cl:setq result cl:t))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-INTEGER) (cl:setq result :int))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-LONG-INTEGER) (cl:setq result :long))
       ((CL:EQ TEST-VALUE-000 SGT-SDBC-STELLA-DOUBLE-FLOAT) (cl:setq result :double))
       (CL:T (cl:setq result :unknown))))))
   ...
   (CL:LET* ((TYPE NULL) (ITER-000 STELLATYPES) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ TYPE (%%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (CL-TRANSLATE-SQL-TYPE-SPECIFIER TYPE) NIL))
...
(CL:DEFUN %EXECUTE-SQL (CONNECTION SQL OPTIONS)
  ...
  (CL:LET*
   ((RESULT NULL) (NCON (%NATIVE-CONNECTION CONNECTION))
    (PROPLIST (VET-OPTIONS OPTIONS (GET-QUOTED-TREE "((:TYPES) \"/SDBC\")" "/SDBC")))
    (TYPES (LOOKUP PROPLIST KWD-SDBC-TYPES)))
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SQL)))
    (CL:COND
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN (LOGMSG "SDBC" KWD-SDBC-HIGH (WRAP-STRING "execute-sql: sql=") SQL)
       (CL:SETQ RESULT (cl-execute-sql ncon (unwrap-string sql) types))))
     ((SUBTYPE-OF? TEST-VALUE-000 SGT-SDBC-SDBC-PREPARED-STATEMENT)
      (CL:PROGN
       (LOGMSG "SDBC" KWD-SDBC-HIGH (WRAP-STRING "execute-sql: prepared sql=") (WRAP-STRING (%SQL SQL))
        (WRAP-STRING ", parameters=") (%PARAMETERS SQL))
...
(CL:DEFUN GET-RESULT-SET (CONNECTION SQL CL:&REST OPTIONS)
  ...
  (MAYBE-RENEW-CONNECTION CONNECTION)
  (CL:LET*
   ((NATIVERESULTSET NULL) (NATIVECONNECTION (%NATIVE-CONNECTION CONNECTION))
   ...
   (CL:LET* ((ARGLIST-000 NIL))
    ...
    (CL:LET*
     ((PROPLIST (VET-OPTIONS ARGLIST-000 (GET-QUOTED-TREE "((:TYPES) \"/SDBC\")" "/SDBC")))
      (TYPES (LOOKUP PROPLIST KWD-SDBC-TYPES)))
     (CL:SETF (%TYPES RESULTSETITERATOR) TYPES)
     (LOGMSG "SDBC" KWD-SDBC-HIGH (WRAP-STRING "get-result-set: sql=") (WRAP-STRING SQL))
     (CL:SETQ NATIVERESULTSET (cl-get-native-result-set nativeConnection sql types))
     (CL:SETF (%RESULT-SET RESULTSETITERATOR) NATIVERESULTSET) RESULTSETITERATOR))))
...
(CL:DEFMETHOD NEXT? ((SELF RESULT-SET-ITERATOR))
  (CL:LET* ((NATIVE-RESULT-SET (%RESULT-SET SELF)) (RESULT NULL))
   ...
   (CL:PROGN
    ...
    (CL:WHEN (CL:NOT (CL:EQ RESULT NULL)) (CL:SETQ RESULT (STELLIFY-WITH-NIL-TO-NULL RESULT))))
   (CL:SETF (%VALUE SELF)
    (CL:IF (CL:NOT (CL:EQ RESULT NULL)) (COERCE-UNCOERCED-COLUMN-VALUES RESULT (%TYPES SELF)) NULL))
   (CL:COND ((CL:NOT (CL:EQ RESULT NULL)) CL:T)
    (CL:T #+:EXCL (dbi:close-query native-result-set) (CL:SETF (%RESULT-SET SELF) NULL) CL:NIL))))
...
(CL:DEFUN SQL-QUOTE-CHARACTER (DBMS-NAME LEXICALTYPE)
  (CL:COND ((CL:EQ LEXICALTYPE KWD-SDBC-STRING-CONSTANT) #\')
   ((CL:EQ LEXICALTYPE KWD-SDBC-IDENTIFIER) (CL:COND ((CL:EQ DBMS-NAME KWD-SDBC-MYSQL) #\`) (CL:T #\")))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" LEXICALTYPE "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
...
(CL:DEFUN PRINT-VALUE-LIST (STREAM VALUES QUOTECHAR)
  (CL:LET* ((VALUE NULL) (ITER-000 VALUES) (I NULL-INTEGER) (ITER-001 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ VALUE (%%VALUE ITER-000)) (CL:SETQ I ITER-001)
    ...
    (CL:COND
     ((CL:OR (CL:EQ VALUE NULL) (CL:EQ VALUE SYM-SDBC-STELLA-NULL) (CL:EQ VALUE KWD-SDBC-NULL))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "NULL"))
     ((CL:OR (CL:EQ VALUE SYM-SDBC-STELLA-DEFAULT) (CL:EQ VALUE KWD-SDBC-DEFAULT))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "DEFAULT"))
     (CL:T
      (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE VALUE)))
       (CL:COND
        ((SUBTYPE-OF-INTEGER? TEST-VALUE-000)
         (CL:PROGN (%%PRINT-STREAM (%NATIVE-STREAM STREAM) (%WRAPPER-VALUE VALUE))))
        ...
        ((SUBTYPE-OF-STRING? TEST-VALUE-000)
         (CL:PROGN
          (%%PRINT-STREAM (%NATIVE-STREAM STREAM) QUOTECHAR (SQL-ESCAPE-STRING (%WRAPPER-VALUE VALUE) QUOTECHAR)
           QUOTECHAR)))
        (CL:T
         (%%PRINT-STREAM (%NATIVE-STREAM STREAM) QUOTECHAR (SQL-ESCAPE-STRING (STRINGIFY VALUE) QUOTECHAR)
          QUOTECHAR))))))
...
(CL:DEFUN INSERT-ROW-EVALUATOR-WRAPPER (ARGUMENTS)
  (%INSERT-ROW (%%VALUE ARGUMENTS) (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS))) (%%VALUE (%%REST (%%REST ARGUMENTS)))
...
insert which is significantly faster than inserting rows one by one."
  ...
  (CL:LET*
   ((SQL NULL) (IDQUOTE (SQL-QUOTE-CHARACTER (GET-DBMS-NAME CONNECTION) KWD-SDBC-IDENTIFIER))
    (VALUEQUOTE (SQL-QUOTE-CHARACTER (GET-DBMS-NAME CONNECTION) KWD-SDBC-STRING-CONSTANT)) (BLOCKLENGTH 1000)
   ...
   (CL:LOOP (CL:SETQ SQL (NEW-OUTPUT-STRING-STREAM))
    (%%PRINT-STREAM (%NATIVE-STREAM SQL) "INSERT INTO " IDQUOTE (SQL-ESCAPE-STRING TABLE IDQUOTE) IDQUOTE)
    (CL:WHEN (CL:AND (CL:NOT (CL:EQ COLUMNS NULL)) (CL:NOT (CL:EQ COLUMNS NIL)))
     (%%PRINT-STREAM (%NATIVE-STREAM SQL) "(") (PRINT-VALUE-LIST SQL COLUMNS IDQUOTE)
     (%%PRINT-STREAM (%NATIVE-STREAM SQL) ")"))
    (CL:COND
     ((CL:AND (CL:EQ (GET-DBMS-NAME CONNECTION) KWD-SDBC-ORACLE) (CL:NOT (CL:EQ (%%REST ROWS) NIL)))
      (CL:LET*
       ((VALUES NULL) (ITER-000 ROWS) (I NULL-INTEGER) (ITER-001 1) (UPPER-BOUND-000 ORACLEBLOCKLENGTH)
       ...
       (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:OR UNBOUNDED?-000 (CL:<= ITER-001 UPPER-BOUND-000))) DO
        (CL:SETQ VALUES (%%VALUE ITER-000)) (CL:SETQ I ITER-001)
        (CL:WHEN (CL:> I 1) (%%PRINT-STREAM (%NATIVE-STREAM SQL) " UNION"))
        (%%PRINT-STREAM (%NATIVE-STREAM SQL) " SELECT ") (PRINT-VALUE-LIST SQL VALUES VALUEQUOTE)
        (%%PRINT-STREAM (%NATIVE-STREAM SQL) " FROM DUAL") (CL:SETQ ROWS (%%REST ROWS))
        (CL:SETQ ITER-000 (%%REST ITER-000)) (CL:SETQ ITER-001 (CL:1+ ITER-001)))))
     (CL:T (%%PRINT-STREAM (%NATIVE-STREAM SQL) " VALUES")
      (CL:LET*
       ((VALUES NULL) (ITER-002 ROWS) (I NULL-INTEGER) (ITER-003 1) (UPPER-BOUND-001 BLOCKLENGTH)
        (UNBOUNDED?-001 (CL:= UPPER-BOUND-001 NULL-INTEGER)))
       (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-003 UPPER-BOUND-001))
       (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (CL:OR UNBOUNDED?-001 (CL:<= ITER-003 UPPER-BOUND-001))) DO
        (CL:SETQ VALUES (%%VALUE ITER-002)) (CL:SETQ I ITER-003)
        (CL:WHEN (CL:> I 1) (%%PRINT-STREAM (%NATIVE-STREAM SQL) ",")) (%%PRINT-STREAM (%NATIVE-STREAM SQL) "(")
        (PRINT-VALUE-LIST SQL VALUES VALUEQUOTE) (%%PRINT-STREAM (%NATIVE-STREAM SQL) ")") (CL:SETQ ROWS (%%REST ROWS))
...
(CL:DEFUN INSERT-ROWS-EVALUATOR-WRAPPER (ARGUMENTS)
  (%INSERT-ROWS (%%VALUE ARGUMENTS) (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS))) (%%VALUE (%%REST (%%REST ARGUMENTS)))
   (%%VALUE (%%REST (%%REST (%%REST ARGUMENTS))))))
   ...
   (CL:LET* ((ROW NULL) (ITER-000 TABLE) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ROW (%%VALUE ITER-000))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN (CL:SETF (%%REST COLLECT-000) (CONS (COERCE-UNCOERCED-COLUMN-VALUES ROW TYPES) NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   RESULT))
  ...
  (CL:LET* ((RESULT NIL))
   (CL:LET* ((VALUE NULL) (ITER-000 ROW) (TYPE NULL) (ITER-001 TYPES) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:NOT (CL:EQ ITER-001 NIL))) DO
     (CL:SETQ VALUE (%%VALUE ITER-000)) (CL:SETQ TYPE (%%VALUE ITER-001))
     (CL:IF (CL:EQ COLLECT-000 NULL)
      (CL:PROGN
       (CL:SETQ COLLECT-000
        (CONS (CL:IF (STRING? VALUE) (CONVERT-STRING-TO-TYPED-OBJECT (%WRAPPER-VALUE VALUE) TYPE) VALUE) NIL))
       (CL:IF (CL:EQ RESULT NIL) (CL:SETQ RESULT COLLECT-000) (ADD-CONS-TO-END-OF-CONS-LIST RESULT COLLECT-000)))
      (CL:PROGN
...
(CL:DEFUN CONVERT-STRING-TO-TYPED-OBJECT (VALUESTRING TYPESPEC)
  ...
  (CL:COND ((CL:EQ TYPESPEC SGT-SDBC-STELLA-CALENDAR-DATE) (STRING-TO-CALENDAR-DATE VALUESTRING))
...
IMPORTANT: Only one prepared statement can be associated with a connection at a time."
  ...
  (CL:CHECK-TYPE SQL CL:SIMPLE-STRING)
  (MAYBE-RENEW-CONNECTION CONNECTION)
  (CL:LET* ((RESULT (NEW-PREPARED-STATEMENT)) (NATIVECONNECTION (%NATIVE-CONNECTION CONNECTION)))
   (CL:SETF (%CONNECTION RESULT) CONNECTION) (CL:SETF (%STATEMENT CONNECTION) RESULT) (CL:SETF (%SQL RESULT) SQL)
...
`value' for proper translation and coercion."
  ...
  (CL:LET* ((CONNECTION (%CONNECTION STATEMENT)) (NCON NULL)) (MAYBE-RENEW-CONNECTION CONNECTION)
...
(CL:DEFUN BIND-PARAMETER-EVALUATOR-WRAPPER (ARGUMENTS)
  (%BIND-PARAMETER (%%VALUE ARGUMENTS) (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS)))
   (%%VALUE (%%REST (%%REST ARGUMENTS))) (%%VALUE (%%REST (%%REST (%%REST ARGUMENTS))))))
...
(CL:DEFUN CL-DO-BIND-PARAMETER (STATEMENT POSITION TYPESPEC VALUE)
  ...
  (CL:CHECK-TYPE POSITION CL:FIXNUM)
  (CL:LET*
   ((NATIVECONNECTION (%NATIVE-CONNECTION (%CONNECTION STATEMENT))) (NULL? (CL:EQ VALUE NULL))
...
(CL:DEFCLASS TABLE-INFO (STANDARD-OBJECT)
  ((DATABASE :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %DATABASE)
   ...
   (REMARKS :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %REMARKS)
   (COLUMNS :ALLOCATION :INSTANCE :ACCESSOR %COLUMNS)))

(CL:DEFUN NEW-TABLE-INFO ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE TABLE-INFO))) (CL:SETF (%COLUMNS SELF) NULL)
   (CL:SETF (%REMARKS SELF) STELLA::NULL-STRING) (CL:SETF (%TYPE SELF) STELLA::NULL-STRING)
   (CL:SETF (%NAME SELF) STELLA::NULL-STRING) (CL:SETF (%SCHEMA SELF) STELLA::NULL-STRING)
...
(CL:DEFUN ACCESS-TABLE-INFO-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-DATABASE)
    (CL:IF SETVALUE? (CL:SETF (%DATABASE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%DATABASE SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-CATALOG)
    (CL:IF SETVALUE? (CL:SETF (%DATABASE SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%DATABASE SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-SCHEMA)
    (CL:IF SETVALUE? (CL:SETF (%SCHEMA SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%SCHEMA SELF)))))
   ...
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-REMARKS)
    (CL:IF SETVALUE? (CL:SETF (%REMARKS SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-STRING (%REMARKS SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-COLUMNS)
    (CL:IF SETVALUE? (CL:SETF (%COLUMNS SELF) VALUE) (CL:SETQ VALUE (%COLUMNS SELF))))
   (CL:T
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" SLOTNAME "' is not a valid case option")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFCLASS COLUMN-INFO (STANDARD-OBJECT)
  ((DATABASE :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %DATABASE)
   ...
   (NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %NAME)
   (DATA-TYPE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "SQL type from java.sql.Types." :ALLOCATION
    :INSTANCE :ACCESSOR %DATA-TYPE)
   (DATA-TYPE-NAME :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :DOCUMENTATION
    "Data source dependent type name, for a UDT the type name is fully qualified." :ALLOCATION :INSTANCE :ACCESSOR
    %DATA-TYPE-NAME)
...
for numeric or decimal types this is precision." :ALLOCATION :INSTANCE :ACCESSOR %COLUMN-SIZE)
   (BUFFER-LENGTH :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "Not used." :ALLOCATION :INSTANCE :ACCESSOR
    %BUFFER-LENGTH)
   (DECIMAL-DIGITS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "The number of fractional digits." :ALLOCATION
    :INSTANCE :ACCESSOR %DECIMAL-DIGITS)
   (RADIX :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "Radix (typically either 10 or 2)." :ALLOCATION
    :INSTANCE :ACCESSOR %RADIX)
...
allow NULL values, NULL means nullability unknown." :ALLOCATION :INSTANCE :ACCESSOR %IS-NULLABLE?)
   (DOCUMENTATION :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :DOCUMENTATION
    "Comment describing column (may be null)." :ALLOCATION :INSTANCE :ACCESSOR %DOCUMENTATION)
   (DEFAULT-VALUE :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %DEFAULT-VALUE)
   (SQL-DATA-TYPE :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "Not used." :ALLOCATION :INSTANCE :ACCESSOR
    %SQL-DATA-TYPE)
   (SQL-DATETIME-SUB :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "Not used." :ALLOCATION :INSTANCE :ACCESSOR
    %SQL-DATETIME-SUB)
   (CHAR-OCTET-LENGTH :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION
    "For char types the maximum number of bytes in the column." :ALLOCATION :INSTANCE :ACCESSOR %CHAR-OCTET-LENGTH)
   (ORDINAL-POSITION :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :DOCUMENTATION "Index of column in table (starting at 1)."
    :ALLOCATION :INSTANCE :ACCESSOR %ORDINAL-POSITION)))

(CL:DEFUN NEW-COLUMN-INFO ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE COLUMN-INFO)))
   (CL:SETF (%ORDINAL-POSITION SELF) NULL-INTEGER) (CL:SETF (%CHAR-OCTET-LENGTH SELF) NULL-INTEGER)
   (CL:SETF (%SQL-DATETIME-SUB SELF) NULL-INTEGER) (CL:SETF (%SQL-DATA-TYPE SELF) NULL-INTEGER)
   (CL:SETF (%DEFAULT-VALUE SELF) STELLA::NULL-STRING) (CL:SETF (%DOCUMENTATION SELF) STELLA::NULL-STRING)
   (CL:SETF (%IS-NULLABLE? SELF) NULL) (CL:SETF (%NULLABLE? SELF) NULL) (CL:SETF (%RADIX SELF) NULL-INTEGER)
   (CL:SETF (%DECIMAL-DIGITS SELF) NULL-INTEGER) (CL:SETF (%BUFFER-LENGTH SELF) NULL-INTEGER)
   (CL:SETF (%COLUMN-SIZE SELF) NULL-INTEGER) (CL:SETF (%DATA-TYPE-NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATA-TYPE SELF) NULL-INTEGER) (CL:SETF (%NAME SELF) STELLA::NULL-STRING)
   (CL:SETF (%TABLE-NAME SELF) STELLA::NULL-STRING) (CL:SETF (%SCHEMA SELF) STELLA::NULL-STRING)
   (CL:SETF (%DATABASE SELF) STELLA::NULL-STRING) SELF))
...
(CL:DEFUN ACCESS-COLUMN-INFO-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-DATA-TYPE-NAME)
    (CL:IF SETVALUE? (CL:SETF (%DATA-TYPE-NAME SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-STRING (%DATA-TYPE-NAME SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-COLUMN-SIZE)
    (CL:IF SETVALUE? (CL:SETF (%COLUMN-SIZE SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-INTEGER (%COLUMN-SIZE SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-BUFFER-LENGTH)
    (CL:IF SETVALUE? (CL:SETF (%BUFFER-LENGTH SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-INTEGER (%BUFFER-LENGTH SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-DECIMAL-DIGITS)
    (CL:IF SETVALUE? (CL:SETF (%DECIMAL-DIGITS SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-INTEGER (%DECIMAL-DIGITS SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-RADIX)
    (CL:IF SETVALUE? (CL:SETF (%RADIX SELF) (%WRAPPER-VALUE VALUE)) (CL:SETQ VALUE (WRAP-INTEGER (%RADIX SELF)))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-NULLABLE?)
    (CL:IF SETVALUE? (CL:SETF (%NULLABLE? SELF) VALUE) (CL:SETQ VALUE (%NULLABLE? SELF))))
   ((CL:EQ SLOTNAME SYM-SDBC-SDBC-IS-NULLABLE?)
    (CL:IF SETVALUE? (CL:SETF (%IS-NULLABLE? SELF) VALUE) (CL:SETQ VALUE (%IS-NULLABLE? SELF))))
...
manual filtering here."
  ...
  (CL:LET*
   ((NCON (%NATIVE-CONNECTION CONNECTION)) (RESULT-SET NULL) (RESULT-ITERATOR NULL)
    (FILTERTABLES?
     (CL:AND TRUE
      (CL:OR (CL:NOT (CL:EQ CATALOG-NAME STELLA::NULL-STRING)) (CL:NOT (CL:EQ SCHEMA-NAME STELLA::NULL-STRING))
       (CL:NOT (CL:EQ TABLE-NAME STELLA::NULL-STRING)) (CL:NOT (CL:EQ TABLE-TYPE STELLA::NULL-STRING)))))
   ...
   (CL:WHEN (CL:EQ RESULT-ITERATOR NULL)
    (CL:LET* ((SELF-000 (NEW-RESULT-SET-ITERATOR))) (CL:SETF (%RESULT-SET SELF-000) RESULT-SET)
    ...
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ TABLE-ROW (%VALUE ITER-000))
     (CL:TAGBODY
       ...
       (CL:AND FILTERTABLES?
        (CL:OR
         ...
         (CL:AND (CL:NOT (CL:EQ SCHEMA-NAME STELLA::NULL-STRING))
          (CL:NOT (EQL-TO-STRING? (%%VALUE (%%REST TABLE-ROW)) SCHEMA-NAME)))
         (CL:AND (CL:NOT (CL:EQ TABLE-NAME STELLA::NULL-STRING))
          (CL:NOT (EQL-TO-STRING? (%%VALUE (%%REST (%%REST TABLE-ROW))) TABLE-NAME)))
      ...
      (CL:LET* ((SELF-001 (NEW-TABLE-INFO))) (CL:SETF (%DATABASE SELF-001) (%WRAPPER-VALUE (%%VALUE TABLE-ROW)))
       (CL:SETF (%SCHEMA SELF-001) (%WRAPPER-VALUE (%%VALUE (%%REST TABLE-ROW))))
       (CL:SETF (%NAME SELF-001) (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST TABLE-ROW)))))
       (CL:SETF (%TYPE SELF-001) (%WRAPPER-VALUE (FOURTH TABLE-ROW)))
       (CL:SETF (%REMARKS SELF-001) (%WRAPPER-VALUE (FIFTH TABLE-ROW))) (CL:SETQ TABLE-INFO SELF-001))
      (CL:WHEN (STRING-EQL? (%DATABASE TABLE-INFO) "") (CL:SETF (%DATABASE TABLE-INFO) STELLA::NULL-STRING))
      (CL:WHEN (STRING-EQL? (%SCHEMA TABLE-INFO) "") (CL:SETF (%SCHEMA TABLE-INFO) STELLA::NULL-STRING))
      (CL:SETQ TABLES (CONS TABLE-INFO TABLES)) :CONTINUE)))
   (REVERSE TABLES)))

(CL:DEFUN GET-TABLES-EVALUATOR-WRAPPER (ARGUMENTS)
  (%GET-TABLES (%%VALUE ARGUMENTS) (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS)))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ARGUMENTS))))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST (%%REST ARGUMENTS)))))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST (%%REST (%%REST ARGUMENTS))))))))
...
(CL:DEFUN GET-COLUMNS-EVALUATOR-WRAPPER (ARGUMENTS)
  (%GET-COLUMNS (%%VALUE ARGUMENTS) (%WRAPPER-VALUE (%%VALUE (%%REST ARGUMENTS)))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ARGUMENTS))))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST (%%REST ARGUMENTS)))))
   (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST (%%REST (%%REST ARGUMENTS))))))))
   ...
   (CL:LET* ((TEST-VALUE-001 (STRING-UPCASE PROPERTY))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TEST-VALUE-001))
    (CL:COND
     ((STRING-EQL? TEST-VALUE-001 "DBMS-NAME")
      (CL:RETURN-FROM %GET-SERVER-INFO (WRAP-STRING (GUESS-DBMS-NAME CONNECTION))))
       ...
       (CL:COND
        ((CL:EQ TEST-VALUE-002 KWD-SDBC-MYSQL)
         (CL:RETURN-FROM %GET-SERVER-INFO
          (CONS-TREE-NTH (%EXECUTE-SQL CONNECTION (WRAP-STRING "select version()") NIL) 0 0)))
        ((CL:EQ TEST-VALUE-002 KWD-SDBC-ORACLE)
         (CL:RETURN-FROM %GET-SERVER-INFO
          (CONS-TREE-NTH
           (%EXECUTE-SQL CONNECTION (WRAP-STRING "select * from v$version where banner like 'Oracle%'") NIL) 0 0)))
        (CL:T))))
     (CL:T)))
   NULL))
...
(CL:DEFUN GUESS-DBMS-NAME (CONNECTION)
  (CL:LET* ((DBMSNAME STELLA::NULL-STRING) (NCON (%NATIVE-CONNECTION CONNECTION)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING DBMSNAME))
   #+:CLSQL (cl:return-from guess-dbms-name (cl:symbol-name (clsql:database-type ncon)))
   (CL:HANDLER-CASE
    (CL:PROGN (%EXECUTE-SQL CONNECTION (WRAP-STRING "select @@myisam_max_sort_file_size") NIL)
     (CL:SETQ DBMSNAME "MySQL"))
    (DATABASE-EXCEPTION ()))
   (CL:WHEN (CL:EQ DBMSNAME STELLA::NULL-STRING)
    (CL:HANDLER-CASE
     (CL:PROGN (%EXECUTE-SQL CONNECTION (WRAP-STRING "select 1 from dual") NIL) (CL:SETQ DBMSNAME "Oracle"))
     (DATABASE-EXCEPTION ())))
   DBMSNAME))
...
(CL:DEFUN GET-DBMS-NAME (CONNECTION)
  (CL:LET* ((CACHEDDBMSNAME (%DBMS-NAME CONNECTION)) (DBMSNAME NULL))
   (CL:WHEN (CL:NOT (CL:EQ CACHEDDBMSNAME NULL)) (CL:RETURN-FROM GET-DBMS-NAME CACHEDDBMSNAME))
   (CL:HANDLER-CASE (CL:SETQ DBMSNAME (%GET-SERVER-INFO CONNECTION "DBMS-NAME")) (STELLA-EXCEPTION ()))
   (CL:IF (CL:NOT (CL:EQ DBMSNAME NULL))
    (CL:SETQ CACHEDDBMSNAME (INTERN-KEYWORD (STRING-UPCASE (%WRAPPER-VALUE DBMSNAME))))
...
(CL:DEFUN PRINT-TABLE (TABLE)
  ...
  (CL:LET* ((CONSTABLE TABLE))
   (CL:LET* ((ROW NULL) (ITER-000 CONSTABLE))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ROW (%%VALUE ITER-000))
     (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "row = " ROW EOL) (CL:SETQ ITER-000 (%%REST ITER-000))))))
    ...
    (CL:PROGN (CL:SETQ IN (OPEN-INPUT-FILE FILE))
     (CL:LET* ((COMMAND NULL) (ITER-000 (S-EXPRESSIONS IN)))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ COMMAND (%VALUE ITER-000))
       (CL:LET* ((SELF-000 (NEW-PROPERTY-LIST))) (CL:SETF (%THE-PLIST SELF-000) COMMAND)
...
(CL:DEFUN INSERT-SQL-COMMAND-TEMPLATE (TEMPLATE)
  (CL:LET* ((COMMANDNAME (LOOKUP TEMPLATE KWD-SDBC-COMMAND)))
   (CL:LET* ((TEMP-000 (LOOKUP TEMPLATE KWD-SDBC-DATA-SOURCE)))
    (CL:LET*
     ((DATASOURCES (CONSIFY (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 KWD-SDBC-DEFAULT)))
      (SQL (LOOKUP TEMPLATE KWD-SDBC-SQL)) (TEMPLATES (LOOKUP *SQL-COMMAND-TEMPLATES* COMMANDNAME)))
     (CL:WHEN (CL:EQ COMMANDNAME NULL)
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Missing :command name in SQL template: `" TEMPLATE "'")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
     (CL:WHEN (CL:NOT (STRING? SQL))
      (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "Illegal or missing :sql specification in SQL template: `" SQL "'"
        EOL "   String expected.")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-001)))))
     (CL:WHEN (CL:EQ TEMPLATES NULL) (CL:SETQ TEMPLATES (NEW-KEY-VALUE-LIST))
     ...
     (CL:LET* ((DATASOURCE NULL) (ITER-000 DATASOURCES))
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ DATASOURCE (%%VALUE ITER-000))
       (CL:WHEN (CL:EQ DATASOURCE KWD-SDBC-OTHERWISE) (CL:SETQ DATASOURCE KWD-SDBC-DEFAULT))
       (INSERT-AT TEMPLATES DATASOURCE TEMPLATE) (CL:SETQ ITER-000 (%%REST ITER-000))))))))

;;; (DEFUN (LOOKUP-SQL-CMD-TEMPLATE PROPERTY-LIST) ...)

(CL:DEFUN LOOKUP-SQL-CMD-TEMPLATE (COMMANDNAME DATASOURCE)
  (CL:LET* ((TEMPLATES (LOOKUP *SQL-COMMAND-TEMPLATES* COMMANDNAME)) (TEMPLATE NULL))
   (CL:WHEN (CL:EQ TEMPLATES NULL) (CL:RETURN-FROM LOOKUP-SQL-CMD-TEMPLATE NULL))
...
(CL:DEFUN INSTANTIATE-SQL-CMD (CMDNAME DATASOURCE CL:&REST |VARS&VALUES|)
  (CL:LET* ((TEMPLATE (LOOKUP-SQL-CMD-TEMPLATE CMDNAME DATASOURCE)) (VARIABLES (NEW-KEY-VALUE-LIST)) (KEY NULL))
   (CL:WHEN (CL:EQ TEMPLATE NULL)
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Undefined SQL command template: `" CMDNAME "'")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
    ...
    (CL:LOOP WHILE ITER-000 DO (CL:SETQ ITEM (CL:POP ITER-000))
     (CL:COND
      ((CL:NOT (CL:EQ KEY NULL))
       (INSERT-AT VARIABLES KEY (CL:IF (CL:EQ ITEM STELLA::NULL-STRING) NULL (WRAP-STRING ITEM))) (CL:SETQ KEY NULL))
...
(CL:DEFUN INSTANTIATE-SQL-CMD-TEMPLATE (TEMPLATE VARIABLES)
  ...
  (CL:LET*
   ((PREFIX *SQL-CMD-TEMPLATE-VARIABLE-PREFIX*) (PREFIXLENGTH (CL:THE CL:FIXNUM (CL:LENGTH PREFIX))) (CURSOR 0)
    (VARIABLESTART 0) (LENGTH (CL:THE CL:FIXNUM (CL:LENGTH TEMPLATE))) (RESULT NULL) (VARIABLE STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING PREFIX VARIABLE) (CL:TYPE CL:FIXNUM PREFIXLENGTH CURSOR VARIABLESTART LENGTH))
   (CL:WHEN (CL:= (STRING-SEARCH TEMPLATE PREFIX NULL-INTEGER) NULL-INTEGER)
    (CL:RETURN-FROM INSTANTIATE-SQL-CMD-TEMPLATE TEMPLATE))
   (CL:SETQ RESULT (NEW-OUTPUT-STRING-STREAM))
   (CL:LOOP (CL:SETQ VARIABLESTART (STRING-SEARCH TEMPLATE PREFIX CURSOR))
    (%%PRINT-STREAM (%NATIVE-STREAM RESULT) (SUBSEQUENCE TEMPLATE CURSOR VARIABLESTART))
    (CL:WHEN (CL:= VARIABLESTART NULL-INTEGER) (CL:RETURN))
    (CL:MULTIPLE-VALUE-SETQ (VARIABLE CURSOR)
     (READ-SQL-CMD-TEMPLATE-VARIABLE TEMPLATE (CL:+ VARIABLESTART PREFIXLENGTH) LENGTH))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:SIMPLE-STRING CL:FIXNUM CL:FIXNUM) (CL:VALUES CL:SIMPLE-STRING CL:FIXNUM))
      ...
      (CL:LET*
       ((C
         (CL:LET ((SELF TEMPLATE) (POSITION CURSOR))
          (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF) (CL:TYPE CL:FIXNUM POSITION))
          (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION)))))
       (CL:COND ((CL:EQL C #\\) (CL:SETQ ESCAPED? CL:T) (CL:SETQ CURSOR (CL:1+ CURSOR))) ((CL:EQL C #\}) (CL:RETURN))
        (CL:T))
       (CL:SETQ CURSOR (CL:1+ CURSOR))))
     (CL:WHEN (CL:< CURSOR END)
      (CL:IF ESCAPED?
       (CL:RETURN-FROM READ-SQL-CMD-TEMPLATE-VARIABLE
        (CL:VALUES (UNESCAPE-TOKEN-STRING (SUBSEQUENCE TEMPLATE (CL:1+ START) CURSOR) #\\ CL:NIL) (CL:1+ CURSOR)))
       (CL:RETURN-FROM READ-SQL-CMD-TEMPLATE-VARIABLE
        (CL:VALUES (SUBSEQUENCE TEMPLATE (CL:1+ START) CURSOR) (CL:1+ CURSOR)))))))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Illegal template variable syntax at position `" CURSOR "' in" EOL EOL
     "    `" TEMPLATE "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN INSTANTIATE-SQL-CMD-TEMPLATE-VARIABLE (TEMPLATEVARIABLE VARIABLES)
  ...
  (CL:LET* ((START 0) (QUOTES "'`\"") (QUOTE NULL-CHARACTER))
   (CL:DECLARE (CL:TYPE CL:FIXNUM START) (CL:TYPE CL:SIMPLE-STRING QUOTES))
   (CL:LET* ((VAR NULL) (VALUE NULL) (ITER-000 (%THE-KV-LIST VARIABLES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NULL)) DO (CL:SETQ VAR (%KEY ITER-000)) (CL:SETQ VALUE (%VALUE ITER-000))
     ...
     (CL:WHEN (CL:NOT (CL:= START NULL-INTEGER))
      (CL:LET* ((VARSPEC STELLA::NULL-STRING) (FROM NULL-INTEGER) (TO NULL-INTEGER) (DEFAULT STELLA::NULL-STRING))
       ...
       (CL:MULTIPLE-VALUE-SETQ (VARSPEC FROM TO DEFAULT)
        (PARSE-SQL-CMD-TEMPLATE-VARIABLE-RANGE-AND-DEFAULT TEMPLATEVARIABLE (%WRAPPER-VALUE VAR) START))
       (CL:WHEN
        (CL:AND (CL:> START 0)
         (MEMBER? QUOTES
       ...
       (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE VALUE)))
        (CL:COND
         ((SUBTYPE-OF-STRING? TEST-VALUE-000)
          (CL:PROGN
           (CL:WHEN (CL:OR (CL:EQ (UNWRAP-STRING VALUE) STELLA::NULL-STRING) (CL:> FROM 0))
            (CL:RETURN-FROM INSTANTIATE-SQL-CMD-TEMPLATE-VARIABLE DEFAULT))
           ...
           (CL:RETURN-FROM INSTANTIATE-SQL-CMD-TEMPLATE-VARIABLE
            (REPLACE-SUBSTRINGS TEMPLATEVARIABLE (%WRAPPER-VALUE VALUE) VARSPEC))))
          ...
          (CL:PROGN
           (CL:IF (CL:OR (CL:EQ VALUE NIL) (CL:>= FROM (LENGTH VALUE)))
            ...
            (CL:LET* ((RESULT (NEW-OUTPUT-STRING-STREAM)))
             (CL:LET* ((ELT NULL) (ITER-001 VALUE) (I NULL-INTEGER) (ITER-002 0))
              ...
              (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-001)) (CL:SETQ I ITER-002)
               (CL:WHEN (CL:AND (CL:>= I FROM) (CL:OR (CL:= TO NULL-INTEGER) (CL:<= I TO)))
                ...
                (%%PRINT-STREAM (%NATIVE-STREAM RESULT)
                 (REPLACE-SUBSTRINGS TEMPLATEVARIABLE (%WRAPPER-VALUE ELT) VARSPEC)))
     ...
     (CL:SETQ ITER-000 (%REST ITER-000))))
   (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
    (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Undefined template variable: `" TEMPLATEVARIABLE "'")
    (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
...
(CL:DEFUN PARSE-SQL-CMD-TEMPLATE-VARIABLE-RANGE-AND-DEFAULT (TEMPLATEVARIABLE VARIABLENAME START)
  ...
  (CL:CHECK-TYPE START CL:FIXNUM)
  (CL:LET*
   ((VARIABLESTART START) (END (CL:THE CL:FIXNUM (CL:LENGTH TEMPLATEVARIABLE))) (STARTIDX 0) (ENDIDX 0) (DEFAULT "1=1")
    (CURSOR 0) (RANGE NIL) (ELEMENT STELLA::NULL-STRING) (NOFELEMENTS 0))
   (CL:DECLARE (CL:TYPE CL:FIXNUM VARIABLESTART END STARTIDX ENDIDX CURSOR NOFELEMENTS)
   ...
   (CL:WHEN (CL:= END NULL-INTEGER)
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "Missing right parenthesis in template variable range: `"
      TEMPLATEVARIABLE "'")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
    ...
    (CL:COND
     ((CL:AND
       (CL:EQL
        ...
        #\,)
       (CL:< NOFELEMENTS 2))
      (CL:SETQ RANGE (CONS (WRAP-STRING (SUBSEQUENCE TEMPLATEVARIABLE START CURSOR)) RANGE))
      (CL:SETQ NOFELEMENTS (CL:1+ NOFELEMENTS)) (CL:SETQ CURSOR (CL:1+ CURSOR)) (CL:SETQ START CURSOR))
     (CL:T (CL:SETQ CURSOR (CL:1+ CURSOR)))))
   (CL:SETQ NOFELEMENTS (CL:1+ NOFELEMENTS))
   (CL:SETQ RANGE (CONS (WRAP-STRING (SUBSEQUENCE TEMPLATEVARIABLE START CURSOR)) RANGE))
   (CL:LET* ((ELT NULL) (ITER-000 (REVERSE RANGE)) (I NULL-INTEGER) (ITER-001 1))
       ...
       (CL:IF (CL:= I NOFELEMENTS)
            ...
            (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
             (CL:SETQ CHAR
              (CL:LET ((SELF VECTOR-000) (POSITION INDEX-000))
               ...
               (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF) (CL:THE CL:FIXNUM POSITION))))
             (CL:WHEN
              (CL:NOT (CL:EQ (CL:AREF *CHARACTER-TYPE-TABLE* (CL:THE CL:FIXNUM (CL:CHAR-CODE CHAR))) KWD-SDBC-DIGIT))
              (CL:SETQ ALWAYS?-000 CL:NIL) (CL:RETURN))
             (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
           (CL:IF ALWAYS?-000
            (CL:PROGN (CL:SETQ STARTIDX (CL:TRUNCATE (STRING-TO-INTEGER ELEMENT))) (CL:SETQ ENDIDX STARTIDX))
        ...
        (CL:LET* ((ALWAYS?-001 CL:T))
         ...
         (CL:IF ALWAYS?-001
          ...
          (CL:PROGN
           (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
            (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "Illegal template variable range: `" TEMPLATEVARIABLE "'")
            (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-001)))))))))
      (2
       (CL:IF (CL:= I NOFELEMENTS)
        (CL:PROGN
         (CL:IF (STRING-EQL? ELEMENT "") (CL:PROGN (CL:SETQ DEFAULT ELEMENT))
          (CL:IF (STRING-EQL? ELEMENT ":") (CL:PROGN (CL:SETQ ENDIDX NULL-INTEGER))
           (CL:LET* ((ALWAYS?-002 CL:T))
            ...
            (CL:IF ALWAYS?-002 (CL:PROGN (CL:SETQ ENDIDX (CL:TRUNCATE (STRING-TO-INTEGER ELEMENT))))
             (CL:PROGN (CL:SETQ DEFAULT ELEMENT)))))))
           ...
           (CL:IF ALWAYS?-003 (CL:PROGN (CL:SETQ ENDIDX (CL:TRUNCATE (STRING-TO-INTEGER ELEMENT))))
            (CL:PROGN
             (CL:LET* ((STREAM-002 (NEW-OUTPUT-STRING-STREAM)))
              (%%PRINT-STREAM (%NATIVE-STREAM STREAM-002) "Illegal template variable range: `" TEMPLATEVARIABLE "'")
...
(CL:DEFUN TEST-PREPARED-STATEMENT ()
  (CL:HANDLER-CASE
   (CL:LET*
    ((CON
      (%CONNECT
       (CONS-LIST KWD-SDBC-DSN (WRAP-STRING "myodbc3") KWD-SDBC-USER (WRAP-STRING "scott") KWD-SDBC-PASSWORD
        (WRAP-STRING "tiger") KWD-SDBC-CONNECTIONSTRING
        (WRAP-STRING "jdbc:mysql://localhost:3306/menagerie?user=scott&password=tiger"))))
     (PREPAREDSQL "insert into customer4 values (?, ?, ?, ?)") (SELECTSQL "select * from customer4")
     (PREPAREDSTATEMENT NULL) (RESULT NULL))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING PREPAREDSQL SELECTSQL))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table before executing prepared statement:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL))
    (CL:SETQ PREPAREDSTATEMENT (%PREPARE-STATEMENT CON PREPAREDSQL))
    (%BIND-PARAMETER PREPAREDSTATEMENT 1 SGT-SDBC-STELLA-INTEGER (WRAP-INTEGER 23))
    (%BIND-PARAMETER PREPAREDSTATEMENT 2 SGT-SDBC-STELLA-STRING (WRAP-STRING "k"))
    (%BIND-PARAMETER PREPAREDSTATEMENT 3 SGT-SDBC-STELLA-DOUBLE-FLOAT (WRAP-FLOAT 5.3d0))
    (%BIND-PARAMETER PREPAREDSTATEMENT 4 SGT-SDBC-STELLA-CALENDAR-DATE (STRING-TO-CALENDAR-DATE "1997-02-14"))
    (CL:SETQ RESULT (%EXECUTE-SQL CON PREPAREDSTATEMENT NIL))
    (%BIND-PARAMETER PREPAREDSTATEMENT 1 SGT-SDBC-STELLA-INTEGER (WRAP-INTEGER 24))
    (%BIND-PARAMETER PREPAREDSTATEMENT 2 SGT-SDBC-STELLA-STRING NULL)
    (%BIND-PARAMETER PREPAREDSTATEMENT 3 SGT-SDBC-STELLA-DOUBLE-FLOAT NULL)
    (%BIND-PARAMETER PREPAREDSTATEMENT 4 SGT-SDBC-STELLA-CALENDAR-DATE NULL)
    (CL:SETQ RESULT (%EXECUTE-SQL CON PREPAREDSTATEMENT NIL))
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table after executing prepared statements:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)))
   (DATABASE-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Caught Database Exception: " (EXCEPTION-MESSAGE E) EOL))))
...
(CL:DEFUN TEST-TRANSACTIONS ()
  ...
  (CL:HANDLER-CASE
   (CL:LET*
    ((CON
      (%CONNECT
       (CONS-LIST KWD-SDBC-DSN (WRAP-STRING "myodbc3") KWD-SDBC-USER (WRAP-STRING "scott") KWD-SDBC-PASSWORD
        (WRAP-STRING "tiger") KWD-SDBC-CONNECTIONSTRING
        (WRAP-STRING "jdbc:mysql://localhost:3306/menagerie?user=scott&password=tiger"))))
     (SELECTSQL "select * from customer2") (INSERTSQL1 "insert into customer2 values (28, 'aa')")
    ...
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table before insert:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)) (%EXECUTE-SQL CON (WRAP-STRING INSERTSQL1) NIL)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table after insert1:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)) (%COMMIT CON)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table after commit:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)) (%EXECUTE-SQL CON (WRAP-STRING INSERTSQL2) NIL)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table after insert2:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)) (%ROLLBACK CON)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Table after rollback:" EOL)
    (PRINT-TABLE (%EXECUTE-SQL CON (WRAP-STRING SELECTSQL) NIL)) (%DISCONNECT CON))
   (DATABASE-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Caught Database Exception: " (EXCEPTION-MESSAGE E) EOL))))
   ...
   (CL:LET*
    ((CON
      (%CONNECT
       (CONS-LIST KWD-SDBC-DSN (WRAP-STRING "myodbc3") KWD-SDBC-USER (WRAP-STRING "scott") KWD-SDBC-PASSWORD
        ...
        (WRAP-STRING "jdbc:mysql://localhost:3306/menagerie?user=scott&password=tiger"))))
     (TYPES
      (LIST* SGT-SDBC-STELLA-INTEGER SGT-SDBC-STELLA-DOUBLE-FLOAT SGT-SDBC-STELLA-LONG-INTEGER SYM-SDBC-STELLA-NULL
    ...
    (CL:LET* ((ROW NULL) (ITER-000 RESULT))
     (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ ROW (%VALUE ITER-000))
      (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Retrieved row : " (%VALUE RESULT) EOL)))
    (%DISCONNECT CON))
   (DATABASE-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Caught Database Exception: " (EXCEPTION-MESSAGE E) EOL))))
...
(CL:DEFUN TEST-RETRIEVE-ALL ()
  ...
  (CL:HANDLER-CASE
   (CL:LET*
    ...
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SQL)) (CL:WHEN (CL:NOT (CL:EQ RESULT NULL)) (PRINT-TABLE RESULT))
    (%DISCONNECT CON) (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "connection string: " CON EOL))
   (DATABASE-EXCEPTION (E)
    (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT) "Caught Database Exception: " (EXCEPTION-MESSAGE E) EOL))))
...
(CL:DEFUN HELP-STARTUP-SDBC1 ()
  (CL:PROGN (CL:SETQ KWD-SDBC-LOG-LEVELS (INTERN-RIGID-SYMBOL-WRT-MODULE "LOG-LEVELS" NULL 2))
   (CL:SETQ KWD-SDBC-LEVEL (INTERN-RIGID-SYMBOL-WRT-MODULE "LEVEL" NULL 2))
   (CL:SETQ KWD-SDBC-NONE (INTERN-RIGID-SYMBOL-WRT-MODULE "NONE" NULL 2))
   (CL:SETQ KWD-SDBC-MAX-WIDTH (INTERN-RIGID-SYMBOL-WRT-MODULE "MAX-WIDTH" NULL 2))
   ...
   (CL:SETQ SYM-SDBC-SDBC-CONNECTION (INTERN-RIGID-SYMBOL-WRT-MODULE "CONNECTION" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-SQL (INTERN-RIGID-SYMBOL-WRT-MODULE "SQL" NULL 0))
   (CL:SETQ SYM-SDBC-STELLA-PARAMETERS
    (INTERN-RIGID-SYMBOL-WRT-MODULE "PARAMETERS" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   ...
   (CL:SETQ SYM-SDBC-SDBC-TYPES (INTERN-RIGID-SYMBOL-WRT-MODULE "TYPES" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-CONNECT (INTERN-RIGID-SYMBOL-WRT-MODULE "CONNECT" NULL 0))
   (CL:SETQ KWD-SDBC-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
   (CL:SETQ KWD-SDBC-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   (CL:SETQ KWD-SDBC-PROTOCOL (INTERN-RIGID-SYMBOL-WRT-MODULE "PROTOCOL" NULL 2))
   (CL:SETQ KWD-SDBC-SERVER-TYPE (INTERN-RIGID-SYMBOL-WRT-MODULE "SERVER-TYPE" NULL 2))
   (CL:SETQ KWD-SDBC-LOW (INTERN-RIGID-SYMBOL-WRT-MODULE "LOW" NULL 2))
   (CL:SETQ KWD-SDBC-CONNECTION-STRING (INTERN-RIGID-SYMBOL-WRT-MODULE "CONNECTION-STRING" NULL 2))
   (CL:SETQ KWD-SDBC-HOST (INTERN-RIGID-SYMBOL-WRT-MODULE "HOST" NULL 2))
   (CL:SETQ KWD-SDBC-PORT (INTERN-RIGID-SYMBOL-WRT-MODULE "PORT" NULL 2))
   (CL:SETQ KWD-SDBC-DB-NAME (INTERN-RIGID-SYMBOL-WRT-MODULE "DB-NAME" NULL 2))
   (CL:SETQ KWD-SDBC-DSN (INTERN-RIGID-SYMBOL-WRT-MODULE "DSN" NULL 2))
   (CL:SETQ KWD-SDBC-USER (INTERN-RIGID-SYMBOL-WRT-MODULE "USER" NULL 2))
   (CL:SETQ KWD-SDBC-PASSWORD (INTERN-RIGID-SYMBOL-WRT-MODULE "PASSWORD" NULL 2))
   (CL:SETQ SYM-SDBC-SDBC-DISCONNECT (INTERN-RIGID-SYMBOL-WRT-MODULE "DISCONNECT" NULL 0))
   (CL:SETQ KWD-SDBC-WHITE-SPACE (INTERN-RIGID-SYMBOL-WRT-MODULE "WHITE-SPACE" NULL 2))
   (CL:SETQ KWD-SDBC-LETTER (INTERN-RIGID-SYMBOL-WRT-MODULE "LETTER" NULL 2))
   ...
   (CL:SETQ SGT-SDBC-STELLA-CALENDAR-DATE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "CALENDAR-DATE" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   ...
   (CL:SETQ SGT-SDBC-STELLA-LONG-INTEGER
    (INTERN-RIGID-SYMBOL-WRT-MODULE "LONG-INTEGER" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ SGT-SDBC-STELLA-DOUBLE-FLOAT
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DOUBLE-FLOAT" (GET-STELLA-MODULE "/STELLA" CL:T) 1))
   (CL:SETQ KWD-SDBC-TYPES (INTERN-RIGID-SYMBOL-WRT-MODULE "TYPES" NULL 2))
   (CL:SETQ KWD-SDBC-HIGH (INTERN-RIGID-SYMBOL-WRT-MODULE "HIGH" NULL 2))
   ...
   (CL:SETQ KWD-SDBC-IDENTIFIER (INTERN-RIGID-SYMBOL-WRT-MODULE "IDENTIFIER" NULL 2))
   (CL:SETQ KWD-SDBC-MYSQL (INTERN-RIGID-SYMBOL-WRT-MODULE "MYSQL" NULL 2))
   ...
   (CL:SETQ KWD-SDBC-DEFAULT (INTERN-RIGID-SYMBOL-WRT-MODULE "DEFAULT" NULL 2))
   (CL:SETQ SYM-SDBC-SDBC-INSERT-ROW (INTERN-RIGID-SYMBOL-WRT-MODULE "INSERT-ROW" NULL 0))
   (CL:SETQ KWD-SDBC-ORACLE (INTERN-RIGID-SYMBOL-WRT-MODULE "ORACLE" NULL 2))
   (CL:SETQ SYM-SDBC-SDBC-INSERT-ROWS (INTERN-RIGID-SYMBOL-WRT-MODULE "INSERT-ROWS" NULL 0))
   ...
   (CL:SETQ SYM-SDBC-SDBC-COMMIT (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMIT" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-ROLLBACK (INTERN-RIGID-SYMBOL-WRT-MODULE "ROLLBACK" NULL 0))))

(CL:DEFUN HELP-STARTUP-SDBC2 ()
  (CL:PROGN (CL:SETQ SGT-SDBC-SDBC-TABLE-INFO (INTERN-RIGID-SYMBOL-WRT-MODULE "TABLE-INFO" NULL 1))
   (CL:SETQ SYM-SDBC-SDBC-DATABASE (INTERN-RIGID-SYMBOL-WRT-MODULE "DATABASE" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-CATALOG (INTERN-RIGID-SYMBOL-WRT-MODULE "CATALOG" NULL 0))
   ...
   (CL:SETQ SYM-SDBC-SDBC-REMARKS (INTERN-RIGID-SYMBOL-WRT-MODULE "REMARKS" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-COLUMNS (INTERN-RIGID-SYMBOL-WRT-MODULE "COLUMNS" NULL 0))
   ...
   (CL:SETQ SYM-SDBC-SDBC-RADIX (INTERN-RIGID-SYMBOL-WRT-MODULE "RADIX" NULL 0))
   (CL:SETQ SYM-SDBC-SDBC-NULLABLE? (INTERN-RIGID-SYMBOL-WRT-MODULE "NULLABLE?" NULL 0))
   ...
   (CL:SETQ SYM-SDBC-STELLA-DEFAULT-VALUE
    (INTERN-RIGID-SYMBOL-WRT-MODULE "DEFAULT-VALUE" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
   ...
   (CL:SETQ KWD-SDBC-COMMAND (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMAND" NULL 2))
   (CL:SETQ KWD-SDBC-DATA-SOURCE (INTERN-RIGID-SYMBOL-WRT-MODULE "DATA-SOURCE" NULL 2))
   (CL:SETQ KWD-SDBC-SQL (INTERN-RIGID-SYMBOL-WRT-MODULE "SQL" NULL 2))
   (CL:SETQ KWD-SDBC-OTHERWISE (INTERN-RIGID-SYMBOL-WRT-MODULE "OTHERWISE" NULL 2))
...
(CL:DEFUN HELP-STARTUP-SDBC3 ()
  (CL:PROGN
   ...
   (CL:LET*
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-CONNECTION))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-CONNECTION-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
   ...
   (CL:LET*
    ((CLASS
      (DEFINE-CLASS-FROM-STRINGIFIED-SOURCE "DATABASE-EXCEPTION" "(DEFCLASS DATABASE-EXCEPTION (STELLA-EXCEPTION))")))
   ...
   (CL:LET*
    ...
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-TABLE-INFO))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-TABLE-INFO-SLOT-VALUE)))
   (CL:LET*
    ((CLASS
...
allow NULL values, NULL means nullability unknown.\") (DOCUMENTATION :TYPE STRING :DOCUMENTATION \"Comment describing column (may be null).\") (REMARKS :RENAMES DOCUMENTATION) (DEFAULT-VALUE :TYPE STRING) (SQL-DATA-TYPE :TYPE INTEGER :DOCUMENTATION \"Not used.\") (SQL-DATETIME-SUB :TYPE INTEGER :DOCUMENTATION \"Not used.\") (CHAR-OCTET-LENGTH :TYPE INTEGER :DOCUMENTATION \"For char types the maximum number of bytes in the column.\") (ORDINAL-POSITION :TYPE INTEGER :DOCUMENTATION \"Index of column in table (starting at 1).\")))")))
    (CL:SETF (%CLASS-CONSTRUCTOR-CODE CLASS) (CL:FUNCTION NEW-COLUMN-INFO))
    (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-COLUMN-INFO-SLOT-VALUE)))))
...
A DATABASE-EXCEPTION is thrown if the connection is unsuccessful.\" :PUBLIC? TRUE :COMMAND? TRUE)"
   ...
   (DEFINE-FUNCTION-OBJECT "GET-MAX-CONNECTION-LIFETIME"
    "(DEFUN (GET-MAX-CONNECTION-LIFETIME INTEGER) ((CON CONNECTION)))" (CL:FUNCTION GET-MAX-CONNECTION-LIFETIME) NULL)
   (DEFINE-FUNCTION-OBJECT "MAYBE-RENEW-CONNECTION" "(DEFUN MAYBE-RENEW-CONNECTION ((CON CONNECTION)))"
    (CL:FUNCTION MAYBE-RENEW-CONNECTION) NULL)
   (DEFINE-FUNCTION-OBJECT "JDBC-CONNECTION-STRING?" "(DEFUN (JDBC-CONNECTION-STRING? BOOLEAN) ((STRING STRING)))"
   ...
   (DEFINE-FUNCTION-OBJECT "ODBC-CONNECTION-STRING?" "(DEFUN (ODBC-CONNECTION-STRING? BOOLEAN) ((STRING STRING)))"
...
the disconnect is unsuccessful.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %DISCONNECT) NULL)
   (DEFINE-FUNCTION-OBJECT "CL-NATIVE-DISCONNECT"
    "(DEFUN CL-NATIVE-DISCONNECT ((NATIVE-CONNECTION NATIVE-CONNECTION)))" (CL:FUNCTION CL-NATIVE-DISCONNECT) NULL)
   (DEFINE-FUNCTION-OBJECT "CL-NATIVE-ODBC-DISCONNECT"
    "(DEFUN CL-NATIVE-ODBC-DISCONNECT ((NATIVECONNECTION NATIVE-CONNECTION)))" (CL:FUNCTION CL-NATIVE-ODBC-DISCONNECT)
    NULL)
...
values.\" :PUBLIC? FALSE)" (CL:FUNCTION SQL-QUERY?) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "EXECUTE-SQL"
    "(DEFUN (EXECUTE-SQL OBJECT) ((CONNECTION CONNECTION) (SQL OBJECT) |&REST| (OPTIONS OBJECT)) :PUBLIC? TRUE :COMMAND? TRUE :DOCUMENTATION \"Execute a SQL statement, and return a table as the reuslt
                  ...
                  @INTEGER, @LONG-INTEGER, @DOUBLE-FLOAT, @CALENDAR-DATE, @STRING, and
                  NULL.  If NULL is specified, values are returned as strings.
                  A DATABASE-EXCEPTION is thrown if execution is unsuccessful.\")" (CL:FUNCTION %EXECUTE-SQL)
   ...
   (DEFINE-FUNCTION-OBJECT "SQL-ESCAPE-STRING"
    "(DEFUN (SQL-ESCAPE-STRING STRING) ((VALUE STRING) (QUOTECHAR CHARACTER)))" (CL:FUNCTION SQL-ESCAPE-STRING) NULL)
...
values :DEFAULT or (quote DEFAULT) translate into DEFAULT.  Arbitrary
unquoted strings can be generated via VERBATIM-STRING-WRAPPERs as
values (e.g., for something like `seq.nextval' in Oracle).\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %INSERT-ROW)
...
insert which is significantly faster than inserting rows one by one.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION %INSERT-ROWS) (CL:FUNCTION INSERT-ROWS-EVALUATOR-WRAPPER))
...
IMPORTANT: Only one prepared statement can be associated with a connection at a time.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    (CL:FUNCTION %PREPARE-STATEMENT) (CL:FUNCTION PREPARE-STATEMENT-EVALUATOR-WRAPPER))
...
statement `statement' to `value'.  `typeSpec' characterizes the type of
`value' for proper translation and coercion.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %BIND-PARAMETER)
...
and committed as individual transactions.  Otherwise, its SQL statements are grouped
into transactions that are terminated by a call to either `commit' or `rollback'.
By default, new connections are in auto-commit mode.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %SET-AUTO-COMMIT)
...
This is less general than what is supported by JDBC and ODBC, but Allegro
ODBC does not support filtering based on patterns, so we only do simple
manual filtering here.\" :PUBLIC? TRUE :COMMAND? TRUE)" (CL:FUNCTION %GET-TABLES)
   ...
   (DEFINE-FUNCTION-OBJECT "GUESS-DBMS-NAME" "(DEFUN (GUESS-DBMS-NAME STRING) ((CONNECTION CONNECTION)))"
    (CL:FUNCTION GUESS-DBMS-NAME) NULL)
   (DEFINE-FUNCTION-OBJECT "GET-DBMS-NAME" "(DEFUN (GET-DBMS-NAME KEYWORD) ((CONNECTION CONNECTION)))"
   ...
   (DEFINE-FUNCTION-OBJECT "CALENDAR-DATE-TO-SQL-STRING"
    "(DEFUN (CALENDAR-DATE-TO-SQL-STRING STRING) ((DATE CALENDAR-DATE)))" (CL:FUNCTION CALENDAR-DATE-TO-SQL-STRING)
    NULL)
   (DEFINE-FUNCTION-OBJECT "LOAD-SQL-CMD-TEMPLATES" "(DEFUN LOAD-SQL-CMD-TEMPLATES ((FILE STRING)))"
    (CL:FUNCTION LOAD-SQL-CMD-TEMPLATES) NULL)
   (DEFINE-FUNCTION-OBJECT "INSERT-SQL-COMMAND-TEMPLATE"
    "(DEFUN INSERT-SQL-COMMAND-TEMPLATE ((TEMPLATE PROPERTY-LIST)))" (CL:FUNCTION INSERT-SQL-COMMAND-TEMPLATE) NULL)
   ...
   (DEFINE-FUNCTION-OBJECT "TEST-PREPARED-STATEMENT" "(DEFUN TEST-PREPARED-STATEMENT ())"
   ...
   (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SDBC-SDBC-STARTUP-SDBC)))
    (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SDBC-STELLA-METHOD-STARTUP-CLASSNAME
...
(CL:DEFUN STARTUP-SDBC ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/SDBC" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4) (CL:SETQ *SQL-COMMAND-TEMPLATES* (NEW-KEY-VALUE-MAP)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "/SDBC")))
    (%SET-LOGGING-PARAMETERS "SDBC"
     (CONS-LIST KWD-SDBC-LOG-LEVELS (GET-QUOTED-TREE "((:NONE :LOW :MEDIUM :HIGH) \"/SDBC\")" "/SDBC") KWD-SDBC-LEVEL
      KWD-SDBC-NONE KWD-SDBC-MAX-WIDTH (WRAP-INTEGER 250)))
