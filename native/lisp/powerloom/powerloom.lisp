...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* FALSE-WRAPPER *STELLA-EXIT-HOOKS*
  TRUE-WRAPPER NIL STANDARD-OUTPUT EOL))
...
(CL:DEFUN START-POWERLOOM-GUI-OPTION-HANDLER (OPTION VALUE)
  ...
  (CL:PROGN (CL:SETQ OPTION OPTION) (CL:SETQ VALUE VALUE))
  (SET-CONFIGURATION-PROPERTY "powerloom.runInteractively" TRUE-WRAPPER NULL)
...
(CL:DEFUN MAIN (CL:&REST %%ARGS)
  (CL:LET*
   ...
   (CL:WHEN
    (CL:AND
     (CL:NOT
      (EQL? (%GET-PROPERTY (WRAP-STRING "powerloom.runInteractively") NIL)
       FALSE-WRAPPER))
     (CL:NOT
...
(CL:DEFUN STARTUP-POWERLOOM ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-POWERLOOM-KEY (INTERN-RIGID-SYMBOL-WRT-MODULE "KEY" NULL 2))
    ...
    (CL:SETQ SYM-POWERLOOM-POWERLOOM-CODE-LOAD-ALL-EXTENSIONS-OPTION-HANDLER
     (INTERN-RIGID-SYMBOL-WRT-MODULE "LOAD-ALL-EXTENSIONS-OPTION-HANDLER" NULL
      0))
    ...
    (CL:SETQ SYM-POWERLOOM-POWERLOOM-CODE-START-POWERLOOM-GUI-OPTION-HANDLER
     (INTERN-RIGID-SYMBOL-WRT-MODULE "START-POWERLOOM-GUI-OPTION-HANDLER" NULL
      0))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (DEFINE-FUNCTION-OBJECT "MAIN"
     "(DEFUN MAIN (|&REST| (%%ARGS STRING)) :PUBLIC? TRUE)" (CL:FUNCTION MAIN)
     NULL)
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION SYM-POWERLOOM-POWERLOOM-CODE-STARTUP-POWERLOOM)))
...
dynamically (not yet in C++).\" :PUBLIC? TRUE)")
  ...
  command line arguments have been processed.")
     ...
     (WRAP-STRING "powerloom.runInteractively") KWD-POWERLOOM-VALUE-TYPE
     SGT-POWERLOOM-STELLA-BOOLEAN KWD-POWERLOOM-DEFAULT-VALUE TRUE-WRAPPER)
  ...
  command line arguments and main function code and then exit.")
     KWD-POWERLOOM-N-ARGUMENTS (WRAP-INTEGER 0) KWD-POWERLOOM-PROPERTY
     (WRAP-STRING "powerloom.runInteractively") KWD-POWERLOOM-VALUE-TYPE
     SGT-POWERLOOM-STELLA-BOOLEAN KWD-POWERLOOM-DEFAULT-VALUE FALSE-WRAPPER)
  ...
  Evaluate the STELLA <s-expression> in the PL-USER module.")
     ...
     SYM-POWERLOOM-STELLA-EVAL-OPTION-HANDLER KWD-POWERLOOM-ERROR-ACTION
     KWD-POWERLOOM-WARN))))
