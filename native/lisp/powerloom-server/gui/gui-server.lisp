...
(CL:DEFUN SERVER-PROCESS-SOAP-REQUEST (REQUESTBODY)
  ...
  (CL:LET* ((EXCEPTION STELLA::NULL-STRING) (REALRESULT "") (SOAPRESULT STELLA::NULL-STRING))
   ...
   (SERVER-LOG KWD-GUI-SERVER-HIGH (WRAP-STRING "Response: ") (WRAP-STRING SOAPRESULT)) SOAPRESULT))
...
(CL:DEFUN SERVER-MARSHAL-EXCEPTION (TYPE EXCEPTION)
  ...
  (CL:LET*
   ((MESSAGE (EXCEPTION-MESSAGE EXCEPTION)) (SERVEREXCEPTION (|new-ServerException|)) (XMLSTRING STELLA::NULL-STRING))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING MESSAGE XMLSTRING)) (CL:SETF (|%Message| SERVEREXCEPTION) MESSAGE)
   (CL:SETF (|%Type| SERVEREXCEPTION) TYPE) (CL:SETQ XMLSTRING (TO-XML-STRING SERVEREXCEPTION)) XMLSTRING))
...
(CL:DEFUN SERVER-EXTRACT-SOAP-BODY-CONTENTS (PARSETREE)
  (CL:LET* ((BODY (%%VALUE (%%REST (%%REST (%%VALUE PARSETREE))))) (BODYCONTENTS (%%VALUE (%%REST (%%REST BODY)))))
   BODYCONTENTS))
    ...
    (CL:LET* ((ARG NULL) (ITER-000 RAWARGS) (COLLECT-000 NULL))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ARG (%%VALUE ITER-000))
      (CL:IF (CL:EQ COLLECT-000 NULL)
       ...
       (CL:PROGN
        (CL:SETF (%%REST COLLECT-000) (CONS (FROM-XML-STRING (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ARG))))) NIL))
...
(CL:DEFUN SERVER-INVOKE-POWERLOOM-METHOD (BODYCONTENTS)
  (CL:LET*
   ((METHODNAME (SERVER-EXTRACT-METHOD-NAME BODYCONTENTS)) (ARGUMENTS (SERVER-EXTRACT-ARGUMENTS BODYCONTENTS))
    (RESULT (DISPATCH-CALL METHODNAME ARGUMENTS)) (MARSHALEDRESULT (TO-XML-STRING RESULT)))
...
(CL:DEFUN GUI-REQUEST-HANDLER (XCHG)
  (CL:LET* ((REQUEST (GET-REQUEST-BODY XCHG)) (RESPONSE (SERVER-PROCESS-SOAP-REQUEST REQUEST)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING REQUEST RESPONSE)) (%%PRINT-STREAM (GET-REPLY-STREAM XCHG) RESPONSE)))
...
(CL:DEFUN STARTUP-GUI-SERVER ()
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ KWD-GUI-SERVER-HIGH (INTERN-RIGID-SYMBOL-WRT-MODULE "HIGH" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "SERVER-PROCESS-SOAP-REQUEST"
     "(DEFUN (SERVER-PROCESS-SOAP-REQUEST STRING) ((REQUESTBODY STRING)))" (CL:FUNCTION SERVER-PROCESS-SOAP-REQUEST)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "SERVER-EXTRACT-METHOD-NAME"
     "(DEFUN (SERVER-EXTRACT-METHOD-NAME STRING) ((BODYCONTENTS CONS)))" (CL:FUNCTION SERVER-EXTRACT-METHOD-NAME) NULL)
    (DEFINE-FUNCTION-OBJECT "SERVER-EXTRACT-ARGUMENTS"
     "(DEFUN (SERVER-EXTRACT-ARGUMENTS (CONS OF OBJECT)) ((BODYCONTENTS CONS)))" (CL:FUNCTION SERVER-EXTRACT-ARGUMENTS)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "GUI-REQUEST-HANDLER" "(DEFUN GUI-REQUEST-HANDLER ((XCHG /HTTP/HTTP-EXCHANGE)))"
     (CL:FUNCTION GUI-REQUEST-HANDLER) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-GUI-SERVER" "(DEFUN STARTUP-GUI-SERVER () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-GUI-SERVER) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-GUI-SERVER-GUI-SERVER-STARTUP-GUI-SERVER)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-GUI-SERVER-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "GUI-SERVER")))
    ...
    (PUBLISH-HANDLER "/ploom/soap-rpc/powerloom-gui-service" (CL:FUNCTION GUI-REQUEST-HANDLER)
     KWD-GUI-SERVER-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-GUI-SERVER-XML NULL))
     KWD-GUI-SERVER-DOCUMENTATION (WRAP-STRING "SOAP service for PowerLoom GUI.")))))
