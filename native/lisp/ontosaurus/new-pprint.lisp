...
(CL:DEFUN PPRINT-AS-ATOMIC-OBJECT? (SELF)
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE SELF)))
   (CL:COND
    ((SUBTYPE-OF? TEST-VALUE-000
      SGT-NEW-PPRINT-LOGIC-NAMED-DESCRIPTION)
     (CL:PROGN CL:T))
...
(CL:DEFUN OPERATOR-TO-STRING (OPERATOR)
  (CL:WHEN (CL:EQ OPERATOR NULL)
   (CL:RETURN-FROM OPERATOR-TO-STRING "NULL"))
...
(CL:DEFUN PPRINT-OPERATOR-WITH-ARGUMENTS (OPERATORNAME ARGUMENTS SEPARATELINES? OMITLASTARGUMENT? STREAM)
  ...
  (CL:LET*
   ((OPERATORINDENT
     (CL:+ (CL:THE CL:FIXNUM (CL:LENGTH OPERATORNAME)) 2)))
         ...
         (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
          (CL:SETQ ARG
           (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-000))
            INDEX-000))
          (CL:WHEN (SHORT-TERM? ARG) (CL:SETQ ALWAYS?-000 CL:NIL)
           (CL:RETURN))
      ...
      (CL:LOOP WHILE (CL:< INDEX-002 LENGTH-002) DO
       (CL:SETQ ARG
        (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-002))
         INDEX-002))
       (CL:IF (PPRINT-AS-ATOMIC-OBJECT? ARG)
...
(CL:DEFUN PPRINT-FUNCTION-TERM (SELF STREAM)
  ...
  (PPRINT-OBJECT (%OPERATOR SELF) STREAM)
  (CL:LET* ((TEST-VALUE-000 CL:NIL))
   (CL:IF (CL:>= (LENGTH (%ARGUMENTS SELF)) 5)
    (CL:SETQ TEST-VALUE-000 CL:T)
    (CL:PROGN
     (CL:SETQ TEST-VALUE-000 (CL:= (LENGTH (%ARGUMENTS SELF)) 4))
     (CL:WHEN TEST-VALUE-000
      (CL:LET* ((ALWAYS?-000 CL:T))
       (CL:LET*
        ((ARG NULL) (VECTOR-000 (%ARGUMENTS SELF)) (INDEX-000 0)
...
(CL:DEFUN PPRINT-FUNCTION-PROPOSITION (SELF STREAM)
  (CL:LET* ((LASTARG (LAST (%ARGUMENTS SELF))))
   (%%PRINT-STREAM STREAM "(= ")
   (CL:SETQ *PPRINT-INDENT* (CL:+ *PPRINT-INDENT* 3))
   (PPRINT-FUNCTION-TERM SELF STREAM)
   (CL:IF (PPRINT-AS-ATOMIC-OBJECT? LASTARG)
    (%%PRINT-STREAM STREAM " ") (PPRINT-NEWLINE STREAM))
...
(CL:DEFUN PPRINT-PROPOSITION (SELF STREAM)
  ...
  (CL:LET* ((TEST-VALUE-000 (%KIND SELF)))
   (CL:COND
    ...
    ((CL:EQ TEST-VALUE-000 KWD-NEW-PPRINT-AND)
     (%%PRINT-STREAM STREAM "(")
     (PPRINT-OBJECT (%OPERATOR SELF) STREAM)
     ...
     (CL:SETQ *PPRINT-INDENT* (CL:- *PPRINT-INDENT* 5))
     (%%PRINT-STREAM STREAM ")"))
    ((CL:EQ TEST-VALUE-000 KWD-NEW-PPRINT-OR)
     (%%PRINT-STREAM STREAM "(")
     (PPRINT-OBJECT (%OPERATOR SELF) STREAM)
    ...
    ((CL:EQ TEST-VALUE-000 KWD-NEW-PPRINT-EXISTS)
     (%%PRINT-STREAM STREAM "(")
     (PPRINT-OBJECT (%OPERATOR SELF) STREAM)
     ...
     (CL:SETQ *PPRINT-INDENT* (CL:- *PPRINT-INDENT* 3))
     (%%PRINT-STREAM STREAM ")"))
    ((CL:EQ TEST-VALUE-000 KWD-NEW-PPRINT-FORALL)
     (%%PRINT-STREAM STREAM "(")
     (PPRINT-OBJECT (%OPERATOR SELF) STREAM)
     (CL:SETQ *PPRINT-INDENT* (CL:+ *PPRINT-INDENT* 8))
     (%%PRINT-STREAM STREAM " ")
...
(CL:DEFUN PPRINT-QUANTIFIED-VARIABLES (VARIABLES INCLUDETYPES? STREAM)
  (CL:WHEN (CL:EQ VARIABLES NULL)
   (CL:RETURN-FROM PPRINT-QUANTIFIED-VARIABLES))
  (%%PRINT-STREAM STREAM "(")
...
(CL:DEFUN PPRINT-DESCRIPTION (SELF STREAM)
  (CL:COND
   ((NAMED-DESCRIPTION? SELF) (PPRINT-NAMED-DESCRIPTION SELF STREAM))
   ((CL:NOT
     (CL:EQ
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SELF)
    ...
    (CL:IF (CL:NOT (CL:EQ *SKOLEMNAMEMAPPINGTABLE* NULL))
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:< INDEX-000 LENGTH-000) DO
       ...
       (%%PRINT-STREAM STREAM " ")
       (CL:IF
        (CL:NOT (CL:EQ (LOOKUP *SKOLEMNAMEMAPPINGTABLE* VAR) NULL))
...
(CL:DEFUN PPRINT-NAMED-DESCRIPTIONS-AS-RULE (HEAD TAIL OPERATORPREFIX STREAM)
  ...
  (CL:LET*
   ((OPERATORINDENT
     (CL:+ (CL:THE CL:FIXNUM (CL:LENGTH OPERATORPREFIX)) 2))
    (VARIABLES NIL))
   (CL:DECLARE (CL:TYPE CL:FIXNUM OPERATORINDENT))
   (CL:LET*
...
(CL:DEFUN PPRINT-ONTOSAURUS-OBJECT (SELF STREAM)
  (CL:LET*
   ...
   (CL:DECLARE
    (CL:SPECIAL *LOGIC-DIALECT* *PPRINT-ATOMIC-OBJECT-RENDERER*))
   (CL:COND
    ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE SELF)
      SGT-NEW-PPRINT-LOGIC-PROPOSITION)
     (CL:PROGN
      (CL:LET* ((TEST-VALUE-000 (%KIND SELF)))
       (CL:COND
