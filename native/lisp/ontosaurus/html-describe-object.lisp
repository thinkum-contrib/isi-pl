  ...
  (CL:COND
   ...
   (CAPITALIZED?
    (CL:COND ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-CONCEPT) "Concept")
     ...
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-OBJECT) "Object") (CL:T (STRING-CAPITALIZE (%SYMBOL-NAME TERM)))))
   (PLURAL?
    (CL:COND ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-CONCEPT) "concepts")
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-RELATION) "relations")
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-CONTEXT) "modules")
   ...
   (CL:T
    (CL:COND ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-CONCEPT) "concept")
     ...
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-CONTEXT) "module")
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-INSTANCE) "instance") ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-QUERY) "query")
     ((CL:EQ TERM KWD-HTML-DESCRIBE-OBJECT-OBJECT) "object") (CL:T (STRING-DOWNCASE (%SYMBOL-NAME TERM)))))))
      ...
      (CL:LET* ((DOC NULL) (ITER-000 DOCSTRINGS))
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ DOC (%VALUE ITER-000))
        (CL:WHEN (CL:AND (CL:NOT (CL:EQ DOC NULL)) (CL:NOT (CL:EQ (%WRAPPER-VALUE DOC) STELLA::NULL-STRING)))
         (CL:PROGN (%%PRINT-STREAM STREAM "<DT>") (HTML-DISPLAY-STRING-IN-PARAGRAPHS (%WRAPPER-VALUE DOC) STREAM)
      ...
      (CL:WHEN
       ...
       (CL:WHEN (CL:NOT (CLASS? (%OPERATOR FACT)))
        (CL:WHEN (CL:NOT (CL:EQ (%KIND FACT) KWD-HTML-DESCRIBE-OBJECT-EQUIVALENT))
         (CL:WHEN (CL:NOT (CL:AND FILTER-RULES? (CL:EQ (%KIND FACT) KWD-HTML-DESCRIBE-OBJECT-IMPLIES)))
...
(CL:DEFUN HTML-DESCRIBE-ONE-MODULE-SUPER-LEVEL (MODULE ALREADYOUTPUT STREAM)
  (CL:LET*
   ((PARENTS (SORT (COPY-CONS-LIST (%THE-CONS-LIST (%PARENT-MODULES MODULE))) (CL:FUNCTION MODULE-NAME-LESS-THAN?)))
  ...
  (CL:LET* ((NAMESTRING (NAME MODULE))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAMESTRING))
   (CL:WHEN
    (CL:AND (VIZ-SERVER-AVAILABLE?)
     (CL:NOT
      (%WRAPPER-VALUE
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS MODULE) SYM-HTML-DESCRIBE-OBJECT-STELLA-CODE-ONLY? FALSE-WRAPPER)))
     (CL:NOT
      (%WRAPPER-VALUE
       (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS MODULE) SYM-HTML-DESCRIBE-OBJECT-STELLA-NAMESPACE? FALSE-WRAPPER))))
   ...
   (CL:COND
        ...
        (CL:WHEN (ISA? OBJ SGT-HTML-DESCRIBE-OBJECT-LOGIC-LOGIC-OBJECT) (CL:SETQ CANDIDATEOBJECTNAME (OBJECT-NAME OBJ))
         (CL:WHEN (CL:NOT (CL:EQ CANDIDATEOBJECTNAME NULL))
          (CL:WHEN
           (CL:AND
            (CL:NOT
             (CL:= (STRING-SEARCH (STRING-UPCASE (%SYMBOL-NAME CANDIDATEOBJECTNAME)) UPCASEDOBJECTNAME 0)
              NULL-INTEGER))
            (CL:OR (CL:EQ OBJECTMODULE NULL) (CL:EQ (HOME-MODULE CANDIDATEOBJECTNAME) OBJECTMODULE)))
           (PUSH RESULTS OBJ) (CL:GO :CONTINUE)))
    ...
    (CL:T
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "`" MATCHTYPE "' is not a valid case option")
...
(CL:DEFUN RESOLVE-SYNONYMS (OBJ)
  (CL:LET* ((SURROGATE (%SURROGATE-VALUE-INVERSE OBJ)))
   (CL:IF (CL:AND (CL:NOT (CL:EQ SURROGATE NULL)) (SYNONYM-SURROGATE? SURROGATE)) (%SURROGATE-VALUE SURROGATE) OBJ)))
...
(CL:DEFUN HTML-POWERLOOM-RESPONSE (ACTION OBJECTTYPE MODULENAME OBJECTNAME STREAM)
  ...
  (CL:LET*
   ((*PPRINT-ATOMIC-OBJECT-RENDERER*
     (%FUNCTION-CODE (LOOKUP-FUNCTION SYM-HTML-DESCRIBE-OBJECT-ONTOSAURUS-PPRINT-ATOMIC-OBJECT-FOR-HTML)))
    ...
    (CL:COND
     ((CL:>= (LENGTH KEYS) 4)
      (CL:COND ((STARTS-WITH? (%WRAPPER-VALUE ACTION) "graph" 0) (VIZ-SERVER-HANDLER XCHG))
       (CL:T
        (HTML-POWERLOOM-RESPONSE (%WRAPPER-VALUE ACTION) (%WRAPPER-VALUE OBJECTTYPE) (%WRAPPER-VALUE MODULENAME)
  ...
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/ONTOSAURUS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-HTML-DESCRIBE-OBJECT-ONTOSAURUS-STARTUP-HTML-DESCRIBE-OBJECT)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-HTML-DESCRIBE-OBJECT-STELLA-METHOD-STARTUP-CLASSNAME
