...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING) SAFE-OBJECT-NAME-STRING))
...
(CL:DEFUN HTML-DESCRIBE-CONTEXT-VIEW (SELF STREAM)
  (CL:LET*
   ((HOMEC (HOME-MODULE SELF)) (OBJECTNAME (PLI::GET-NAME SELF))
...
(CL:DEFUN HTML-DISPLAY-IMAGES (SELF STREAM)
  (CL:LET*
   ((FIRST? CL:T)
    (IMAGES
     (PLI::GET-INFERRED-BINARY-PROPOSITION-VALUES *IMAGE-URL-RELATION* SELF
      NULL NULL)))
    ...
    (CL:PROGN (%%PRINT-STREAM STREAM "<P>")
     (CL:LET* ((I NULL) (ITER-000 IMAGES))
      (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ I (%VALUE ITER-000))
       ...
       (HTML-EMIT-IMAGE STREAM (%WRAPPER-VALUE I) "" NULL-INTEGER NULL-INTEGER
        "BOTTOM" 0)))
...
(CL:DEFUN HTML-DISPLAY-DOCUMENTATION (SELF STREAM)
  (CL:LET*
   ((DOCSTRINGS
     (PLI::GET-INFERRED-BINARY-PROPOSITION-VALUES *DOCUMENTATION-RELATION*
      SELF NULL NULL)))
       ...
       (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ DOC (%VALUE ITER-000))
        (CL:WHEN
         ...
         (CL:PROGN (%%PRINT-STREAM STREAM "<DT>")
          (HTML-DISPLAY-STRING-IN-PARAGRAPHS (%WRAPPER-VALUE DOC) STREAM)
   ...
   (CL:WHEN (CL:NOT (CL:EQ SYNONYMS NIL))
    (CL:PROGN (%%PRINT-STREAM STREAM "<P>")
     (CL:PROGN (%%PRINT-STREAM STREAM "<B>")
      (%%PRINT-STREAM STREAM "Synonyms: ") (%%PRINT-STREAM STREAM "</B>"))
...
(CL:DEFUN HTML-DESCRIBE-EQUIVALENTS (TITLE HEAD RELATION STREAM RULE?)
  ...
  (CL:LET*
   ((EQUIVALENTS (COPY-CONS-LIST (ALL-EQUIVALENT-RELATIONS RELATION CL:T))))
...
(CL:DEFUN HTML-DESCRIBE-ONE-SUB-LEVEL (OBJECT DEPTH CUTOFF STREAM)
  ...
  (CL:LET*
   ((DIRECTSUBS (CONSIFY (PLI::GET-DIRECT-SUBRELATIONS OBJECT NULL NULL))))
   ...
   (CL:PROGN
    (%%PRINT-STREAM STREAM "<DL STYLE=' margin-top: 0; margin-bottom:0'>")
      ...
      (CL:PROGN (%%PRINT-STREAM STREAM "<DT STYLE='margin-top: 0'>")
       ...
       (CL:WHEN
        (CL:AND (CL:>= DEPTH CUTOFF)
         (CL:NOT (EMPTY? (PLI::GET-DIRECT-SUBRELATIONS SUB NULL NULL))))
        (%%PRINT-STREAM STREAM "    +..."))
...
(CL:DEFUN HTML-DESCRIBE-SUBS (TITLE HEAD RELATION STREAM RULE?)
  ...
  (CL:LET*
   ...
   (CL:IF (CL:= SUBLENGTH 0)
    (CL:PROGN (%%PRINT-STREAM STREAM "<I>") (%%PRINT-STREAM STREAM "None")
...
(CL:DEFUN HTML-DESCRIBE-ONE-SUPER-LEVEL (DIRECTSUPERS ALREADYOUTPUT STREAM)
  (CL:LET* ((ALREADYOUTPUT? CL:NIL))
   ...
   (CL:SETQ DIRECTSUPERS
    (SORT (COPY-CONS-LIST DIRECTSUPERS) (CL:FUNCTION OBJECT-NAME-LESS-THAN?)))
   (CL:PROGN
    (%%PRINT-STREAM STREAM "<DL STYLE=' margin-top: 0; margin-bottom:0'>")
...
(CL:DEFUN HTML-DESCRIBE-ONE-PARENT-WITH-SIBLINGS (PARENT CHILD STREAM)
  (CL:LET*
   ((DIRECTSUBS (CONSIFY (PLI::GET-DIRECT-SUBRELATIONS PARENT NULL NULL))))
   (CL:SETQ DIRECTSUBS (SORT DIRECTSUBS (CL:FUNCTION OBJECT-NAME-LESS-THAN?)))
   (CL:PROGN (%%PRINT-STREAM STREAM "<DT>")
...
(CL:DEFUN HAS-CHILD-OTHER-THAN? (PARENT CHILD)
  (CL:LET*
   ((C NULL) (ITER-000 (PLI::GET-DIRECT-SUBRELATIONS PARENT NULL NULL)))
   (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ C (%VALUE ITER-000))
...
(CL:DEFUN HTML-DESCRIBE-SIBLINGS (TITLE HEAD RELATION STREAM RULE?)
  ...
  (CL:LET* ((PARENTSOFSIBS NIL))
   (CL:LET*
    ((P NULL) (ITER-000 (PLI::GET-DIRECT-SUPERRELATIONS RELATION NULL NULL))
     (COLLECT-000 NULL))
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:WHEN
       ...
       (CL:WHEN (CL:NOT (CLASS? (%OPERATOR FACT)))
        (CL:WHEN
         (CL:NOT (CL:EQ (%KIND FACT) KWD-HTML-DESCRIBE-OBJECT-EQUIVALENT))
         (CL:WHEN
          (CL:NOT
           (CL:AND FILTER-RULES?
  ...
  (CL:LET*
   ...
   (CL:IF (CL:EQ INSTANCES NIL)
    ...
    (CL:PROGN
     ...
     (HTML-EMIT-IMAGE STREAM "icons/small-circ-filled.gif" "x" 11 11 "BOTTOM"
      0)
     (%%PRINT-STREAM STREAM ")" EOL)
     (CL:PROGN (%%PRINT-STREAM STREAM "<DL>")
      (CL:SETQ INSTANCES (SORT INSTANCES (CL:FUNCTION LOGIC-FORM-LESS?)))
        ...
        (CL:PROGN (%%PRINT-STREAM STREAM "<DT>")
         ...
         (CL:IF (MEMBER? ASSERTEDINSTANCES INSTANCE)
          ...
          (CL:PROGN (%%PRINT-STREAM STREAM "<I>")
           (PPRINT-OBJECT INSTANCE STREAM) (%%PRINT-STREAM STREAM "</I>")))
         (%%PRINT-STREAM STREAM "</DT>" EOL))
        (CL:SETQ ITER-000 (%%REST ITER-000))))
      (%%PRINT-STREAM STREAM "</DL>" EOL))))
   (CL:WHEN RULE? (%%PRINT-STREAM STREAM "<HR SIZE=2>" EOL)) NULL))
...
(CL:DEFUN HTML-DESCRIBE-TUPLES (TITLE HEAD RELATION STREAM RULE?)
  ...
  (CL:CHECK-TYPE HEAD CL:SIMPLE-STRING)
  (CL:LET*
   ((TUPLES (CONSIFY (PLI::GET-RELATION-EXTENSION RELATION NULL NULL))))
   (HTML-WRITE-HEADER-WITH-TABLE STREAM TITLE HEAD)
   (CL:IF (CL:EQ TUPLES NIL)
    (CL:PROGN (%%PRINT-STREAM STREAM "<I>") (%%PRINT-STREAM STREAM "None")
     (%%PRINT-STREAM STREAM "</I>" EOL))
    (CL:PROGN (%%PRINT-STREAM STREAM "<PRE>")
     (CL:SETQ TUPLES (SORT TUPLES (CL:FUNCTION PROPOSITION-LESS-THAN?)))
...
(CL:DEFUN HTML-DESCRIBE-NAMED-DESCRIPTION (CONCEPT STREAM)
  ...
  (CL:LET*
   ...
   (HTML-DESCRIBE-EQUIVALENTS
    (STRING-CONCATENATE "Equivalent " DESCRIPTIONTERM) NAMESTRING CONCEPT
    STREAM CL:T)
   ...
   (HTML-DESCRIBE-SUBS (STRING-CONCATENATE "Sub" DESCRIPTIONTERM) NAMESTRING
    CONCEPT STREAM CL:T)
   ...
   (CL:LET*
    ...
    (CL:IF (CL:= (PLI::GET-ARITY CONCEPT) 1)
     (HTML-DESCRIBE-INSTANCES "Instances" NAMESTRING CONCEPT STREAM CL:T)
...
(CL:DEFUN HTML-DESCRIBE-INSTANCE (INSTANCE STREAM)
  ...
  (CL:LET* ((NAMESTRING (OBJECT-NAME-STRING INSTANCE)))
    ...
    (CL:LET* ((SUB NULL) (ITER-000 DIRECTSUBMODULES))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:PROGN (%%PRINT-STREAM STREAM "<DT STYLE='margin-top: 0'>")
       ...
       (CL:WHEN
        (CL:AND (CL:>= DEPTH CUTOFF) (CL:NOT (CL:EQ (CHILD-MODULES SUB) NIL)))
        (%%PRINT-STREAM STREAM "    +..."))
       (%%PRINT-STREAM STREAM "</DT>" EOL))
      (CL:WHEN (CL:< DEPTH CUTOFF)
       (CL:PROGN (%%PRINT-STREAM STREAM "<DD>")
        (HTML-DESCRIBE-ONE-MODULE-SUB-LEVEL SUB (CL:1+ DEPTH) CUTOFF STREAM)
        (%%PRINT-STREAM STREAM "</DD>" EOL)))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (%%PRINT-STREAM STREAM "</DL>" EOL))
   NULL))
    ...
    (CL:LET* ((PARENT NULL) (ITER-000 PARENTS))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:WHEN (CL:NOT ALREADYOUTPUT?)
       ...
       (CL:PROGN (%%PRINT-STREAM STREAM "<DD>")
        (HTML-DESCRIBE-ONE-MODULE-SUPER-LEVEL PARENT ALREADYOUTPUT STREAM)
        (%%PRINT-STREAM STREAM "</DD>" EOL)))
      (CL:SETQ ITER-000 (%%REST ITER-000))))
    (%%PRINT-STREAM STREAM "</DL>" EOL)))
  NULL)
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     ...
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
       ...
       (CL:WHEN (MEMBER? ALLCLASSES PARENT) (CL:SETQ ADD? CL:NIL) (CL:RETURN))
...
(CL:DEFUN HTML-DESCRIBE-MODULE (MODULE STREAM)
  ...
  (CL:LET* ((NAMESTRING (NAME MODULE)))
   ...
   (CL:WHEN
    ...
    (HTML-WRITE-HREF-IN-DETAIL STREAM "Graph" "graph" "context" (NAME MODULE)
     (NAME MODULE) (CONCATENATE "Graph " (NAME MODULE)) CL:NIL))
   ...
   (HTML-DESCRIBE-MODULE-SUPERS "Parent Modules" NAMESTRING MODULE STREAM
    CL:T)
...
(CL:DEFUN HTML-UNKNOWN-MODULE-RESPONSE (ITEMTYPE ITEMNAME MODULENAME STREAM)
  ...
  (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
   ...
   (CL:LET*
    ((TAG-001 (CONCATENATE "BODY BGCOLOR='#" *ERROR-BACKGROUND-COLOR* "'")))
...
(CL:DEFUN HTML-UNKNOWN-OBJECT-RESPONSE (ITEMTYPE ITEMNAME STREAM)
  ...
  (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
   ...
   (%%PRINT-STREAM STREAM EOL)
   (CL:LET*
    ((TAG-002 (CONCATENATE "BODY BGCOLOR='#" *ERROR-BACKGROUND-COLOR* "'")))
...
(CL:DEFUN HTML-WRITE-RELATION-RESPONSE-PAGE (RELATION STREAM)
  (CL:LET* ((OBJECTNAME (OBJECT-NAME-STRING RELATION)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING OBJECTNAME))
   (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
    (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>")
...
(CL:DEFUN HTML-WRITE-INSTANCE-RESPONSE-PAGE (INSTANCE STREAM)
  (CL:LET* ((OBJECTNAME (OBJECT-NAME-STRING INSTANCE)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING OBJECTNAME))
   (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
    (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>")
     (%%PRINT-STREAM STREAM "<BASE TARGET='content'>" EOL)
    ...
    (CL:LET*
     ...
     (CL:PROGN (%%PRINT-STREAM STREAM "<H1>")
      ...
      (%%PRINT-STREAM STREAM "</H1>" EOL))
     (%%PRINT-STREAM STREAM "<HR SIZE=4>" EOL)
     (HTML-DESCRIBE-INSTANCE INSTANCE STREAM) (WRITE-POWERLOOM-TRAILER STREAM)
     (%%PRINT-STREAM STREAM "</"
...
(CL:DEFUN HTML-WRITE-OBJECT-RESPONSE-PAGE-INTERNAL (OBJECT STREAM)
  (CL:LET* ((OBJECTNAME (OBJECT-NAME-STRING OBJECT)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING OBJECTNAME))
  ...
  (CL:LET* ((MODULENAME (NAME MODULE)))
   ...
   (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
    (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>")
     ...
     (%%PRINT-STREAM STREAM "</HEAD>" EOL))
    (CL:LET*
     ((TAG-006 (CONCATENATE "BODY BGCOLOR='#" *MODULE-BACKGROUND-COLOR* "'")))
     ...
     (CL:PROGN (%%PRINT-STREAM STREAM "<H1>")
      ...
      (%%PRINT-STREAM STREAM "</H1>" EOL))
     (%%PRINT-STREAM STREAM "<HR SIZE=4>" EOL)
     (HTML-DESCRIBE-MODULE MODULE STREAM) (WRITE-POWERLOOM-TRAILER STREAM)
     (%%PRINT-STREAM STREAM "</"
...
(CL:DEFUN HTML-WRITE-MODULE-RESPONSE-PAGE (MODULENAME STREAM)
  ...
  (CL:LET* ((MODULE (GET-STELLA-MODULE MODULENAME CL:NIL)))
   (CL:COND
    ((CL:EQ MODULE NULL)
     (HTML-UNKNOWN-OBJECT-RESPONSE KWD-HTML-DESCRIBE-OBJECT-CONTEXT MODULENAME
      STREAM))
    (CL:T
...
(CL:DEFUN HTML-WRITE-OBJECT-RESPONSE-PAGE (MODULENAME OBJECTNAME STREAM)
  ...
  (CL:LET* ((MODULE (GET-STELLA-MODULE MODULENAME CL:NIL)))
   (CL:WHEN (CL:EQ MODULE NULL)
    (HTML-UNKNOWN-MODULE-RESPONSE KWD-HTML-DESCRIBE-OBJECT-RELATION OBJECTNAME
     MODULENAME STREAM)
    (CL:RETURN-FROM HTML-WRITE-OBJECT-RESPONSE-PAGE))
   (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
    (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
    (CL:LET* ((INSTANCE (PLI::GET-OBJECT OBJECTNAME NULL NULL)))
     (CL:IF (CL:NOT (CL:EQ INSTANCE NULL))
      ...
      (HTML-UNKNOWN-OBJECT-RESPONSE KWD-HTML-DESCRIBE-OBJECT-OBJECT OBJECTNAME
       STREAM))))))
...
(CL:DEFUN FIND-CANDIDATE-OBJECTS (OBJECTNAME OBJECTTYPE MATCHTYPE)
  ...
  (CL:LET*
       ...
       (CL:TAGBODY
        (CL:WHEN (ISA? OBJ SGT-HTML-DESCRIBE-OBJECT-LOGIC-LOGIC-OBJECT)
         ...
         (CL:WHEN (CL:NOT (CL:EQ CANDIDATEOBJECTNAME NULL))
          (CL:WHEN
           (CL:AND (STRING-EQL? (%SYMBOL-NAME CANDIDATEOBJECTNAME) BARENAME)
   ...
   (REMOVE-DUPLICATES RESULTS) (SORT RESULTS (CL:FUNCTION LOGIC-FORM-LESS?))
   RESULTS))
...
(CL:DEFUN RESOLVE-SYNONYMS (OBJ)
  (CL:LET* ((SURROGATE (%SURROGATE-VALUE-INVERSE OBJ)))
   (CL:IF
    (CL:AND (CL:NOT (CL:EQ SURROGATE NULL)) (SYNONYM-SURROGATE? SURROGATE))
      ...
      (0
       (HTML-UNKNOWN-OBJECT-RESPONSE
        (CL:IF (STRING-EQL? OBJECTTYPE "instance")
         KWD-HTML-DESCRIBE-OBJECT-OBJECT KWD-HTML-DESCRIBE-OBJECT-RELATION)
        OBJECTNAME STREAM))
      (1
       (CL:LET*
  ...
  (CL:LET*
   ...
   (CL:COND
    ((CL:OR (STRING-EQL? OBJECTTYPE "object")
      (STRING-EQL? OBJECTTYPE "concept") (STRING-EQL? OBJECTTYPE "relation")
     ...
     (CL:COND
      ...
      ((CL:OR (STRING-EQL? ACTION "find")
       ...
       (CL:LET* ((*ONTOSAURUS-URL-ACTION* "show"))
        ...
        (HTML-WRITE-FIND-OBJECT-RESPONSE-PAGE MODULENAME OBJECTNAME OBJECTTYPE
         KWD-HTML-DESCRIBE-OBJECT-SUBSTRING STREAM))
       (CL:RETURN-FROM HTML-POWERLOOM-RESPONSE))
      ((STRING-EQL? ACTION "findexact")
       (CL:LET* ((*ONTOSAURUS-URL-ACTION* "show"))
        ...
        (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING *ONTOSAURUS-URL-ACTION*))
        (HTML-WRITE-FIND-OBJECT-RESPONSE-PAGE MODULENAME OBJECTNAME OBJECTTYPE
         KWD-HTML-DESCRIBE-OBJECT-EXACT STREAM))
       (CL:RETURN-FROM HTML-POWERLOOM-RESPONSE))
    ...
    ((STRING-EQL? OBJECTTYPE "query")
     (CL:COND
      ((STRING-EQL? ACTION "explain")
       (HTML-WRITE-EXPLANATION-RESPONSE-PAGE MODULENAME OBJECTNAME STREAM)
       (CL:RETURN-FROM HTML-POWERLOOM-RESPONSE))))))
  (CL:PROGN (%%PRINT-STREAM STREAM "<FONT COLOR='#BB0000'>")
   (CL:PROGN (%%PRINT-STREAM STREAM "<B>")
    (%%PRINT-STREAM STREAM "ERROR: Unrecognized action-type pair: '" ACTION
     "' '" OBJECTTYPE "'.")
    (%%PRINT-STREAM STREAM "</B>"))
   (%%PRINT-STREAM STREAM "</FONT>" EOL))
  NULL)
   ...
   (CL:COND
    ...
    (CL:T (SET-RESPONSE-CODE XCHG KWD-HTML-DESCRIBE-OBJECT-BAD-REQUEST)
     (CL:LET* ((TAG-008 (MAKE-HTML-BODY-TAG *ERROR-BACKGROUND-COLOR*)))
      ...
      (%%PRINT-STREAM STREAM "ERROR: expected four parameters, only received "
       (LENGTH KEYS) EOL)
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (PUBLISH-HANDLER "/ploom/ontosaurus/do-action"
     (CL:FUNCTION DO-ACTION-HANDLER) KWD-HTML-DESCRIBE-OBJECT-CONTENT-TYPE
     (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-HTML-DESCRIBE-OBJECT-HTML NULL))
