...
(CL:DEFUN NEW-THREAD (RUN-FUNCTION RUN-ARGUMENTS)
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE THREAD)))
   (CL:SETF (%RUN-FUNCTION SELF) RUN-FUNCTION) (CL:SETF (%RUN-ARGUMENTS SELF) RUN-ARGUMENTS)
...
(CL:DEFUN ACCESS-THREAD-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-SHELL-ONTOSAURUS-RUN-ARGUMENTS)
    (CL:IF SETVALUE? (CL:SETF (%RUN-ARGUMENTS SELF) VALUE) (CL:SETQ VALUE (%RUN-ARGUMENTS SELF))))
   (CL:T
...
(CL:DEFMETHOD INITIALIZE-THREAD ((SELF THREAD))
  (CL:WHEN (CL:EQ (%RUN-FUNCTION SELF) NULL)
   (CL:ERROR (NEW-STELLA-EXCEPTION "initialize-thread: run-function not defined")))
...
(CL:DEFUN CL-INITIALIZE-THREAD (SELF)
  (CL:LET*
   ((NAME (YIELD-UNIQUE-GENSYM-NAME "STELLA-THREAD" NULL))
    (THREAD #+allegro (mp:make-process :name name) #-allegro NULL) (FUNCTION (%RUN-FUNCTION SELF)) (ARGUMENTS NIL))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
   (CL:WHEN (NULL? THREAD)
    (CL:ERROR (NEW-STELLA-EXCEPTION "initialize-thread: threads not yet supported in this Lisp dialect")))
...
(CL:DEFUN JAVA-INITIALIZE-THREAD (SELF)
  (CL:LET* ((NAME (YIELD-UNIQUE-GENSYM-NAME "STELLA-THREAD" NULL)) (THREAD NULL) (FUNCTION (%RUN-FUNCTION SELF)))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING NAME))
   (CL:LET* ((TEMP-000 (%RUN-ARGUMENTS SELF)))
    (CL:LET* ((ARGUMENTS (CL:IF (CL:NOT (CL:EQ TEMP-000 NULL)) TEMP-000 (VECTOR))) (NATARGS (%THE-ARRAY ARGUMENTS)))
     (CL:DECLARE (CL:TYPE CL:SIMPLE-VECTOR NATARGS)) (CL:SETF (%NATIVE-THREAD SELF) THREAD)))))
...
(CL:DEFMETHOD KILL-THREAD ((SELF THREAD))
  ...
  (CL:LET* ((THREAD (%NATIVE-THREAD SELF))) (STOP-THREAD SELF) #+allegro (mp:process-kill thread)
...
(CL:DEFCLASS SHELL-PROCESS (STANDARD-OBJECT)
  ((COMMAND :ALLOCATION :INSTANCE :ACCESSOR %COMMAND)
   (DIRECTORY :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING :ALLOCATION :INSTANCE :ACCESSOR %DIRECTORY)
   ...
   (EXIT-STATUS :TYPE CL:FIXNUM :INITFORM NULL-INTEGER :ALLOCATION :INSTANCE :ACCESSOR %EXIT-STATUS)))

(CL:DEFUN NEW-SHELL-PROCESS ()
  (CL:LET* ((SELF NULL)) (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE SHELL-PROCESS)))
   (CL:SETF (%EXIT-STATUS SELF) NULL-INTEGER) (CL:SETF (%NATIVE-STDERR SELF) NULL) (CL:SETF (%ERROR-STREAM SELF) NULL)
   (CL:SETF (%NATIVE-STDOUT SELF) NULL) (CL:SETF (%OUTPUT-STREAM SELF) NULL) (CL:SETF (%NATIVE-STDIN SELF) NULL)
   (CL:SETF (%INPUT-STREAM SELF) NULL) (CL:SETF (%NATIVE-PROCESS SELF) NULL) (CL:SETF (%DIRECTORY SELF) ".")
...
(CL:DEFUN ACCESS-SHELL-PROCESS-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ...
   ((CL:EQ SLOTNAME SYM-SHELL-STELLA-INPUT-STREAM)
    (CL:IF SETVALUE? (CL:SETF (%INPUT-STREAM SELF) VALUE) (CL:SETQ VALUE (%INPUT-STREAM SELF))))
   ((CL:EQ SLOTNAME SYM-SHELL-ONTOSAURUS-NATIVE-STDIN)
    (CL:IF SETVALUE? (CL:SETF (%NATIVE-STDIN SELF) VALUE) (CL:SETQ VALUE (%NATIVE-STDIN SELF))))
   ((CL:EQ SLOTNAME SYM-SHELL-STELLA-OUTPUT-STREAM)
    (CL:IF SETVALUE? (CL:SETF (%OUTPUT-STREAM SELF) VALUE) (CL:SETQ VALUE (%OUTPUT-STREAM SELF))))
   ((CL:EQ SLOTNAME SYM-SHELL-ONTOSAURUS-NATIVE-STDOUT)
    (CL:IF SETVALUE? (CL:SETF (%NATIVE-STDOUT SELF) VALUE) (CL:SETQ VALUE (%NATIVE-STDOUT SELF))))
   ((CL:EQ SLOTNAME SYM-SHELL-ONTOSAURUS-ERROR-STREAM)
    (CL:IF SETVALUE? (CL:SETF (%ERROR-STREAM SELF) VALUE) (CL:SETQ VALUE (%ERROR-STREAM SELF))))
   ((CL:EQ SLOTNAME SYM-SHELL-ONTOSAURUS-NATIVE-STDERR)
    (CL:IF SETVALUE? (CL:SETF (%NATIVE-STDERR SELF) VALUE) (CL:SETQ VALUE (%NATIVE-STDERR SELF))))
   ...
   (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE COMMAND)))
    (CL:COND ((CL:EQ TEST-VALUE-000 SGT-SHELL-STELLA-CONS) (CL:PROGN (CL:SETQ PARSEDCOMMAND COMMAND)))
     ((SUBTYPE-OF-STRING? TEST-VALUE-000)
      (CL:PROGN (CL:SETQ PARSEDCOMMAND (SPLIT-STRING (%WRAPPER-VALUE COMMAND) #\ ))))
     (CL:T
      (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
       (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000) "parse-shell-command: illegal shell command format: `" COMMAND "'")
       (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))))
   (CL:WHEN #+allegro CL:t #-allegro CL:nil (CL:SETQ PARSEDCOMMAND (CONS (%%VALUE PARSEDCOMMAND) PARSEDCOMMAND)))
   (CL:SETQ RESULT (CL:MAKE-ARRAY (LENGTH PARSEDCOMMAND)))
   (CL:LET* ((ELT NULL) (ITER-000 PARSEDCOMMAND) (I NULL-INTEGER) (ITER-001 0))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ELT (%%VALUE ITER-000)) (CL:SETQ I ITER-001)
     (CL:SETF (CL:AREF RESULT I) (%WRAPPER-VALUE ELT)) (CL:SETQ ITER-000 (%%REST ITER-000))
...
(CL:DEFMETHOD START-PROCESS ((SELF SHELL-PROCESS))
  ...
  (CL:LET*
   ((PROCESS NULL) (COMMAND (%COMMAND SELF)) (DIRECTORY (%DIRECTORY SELF)) (INPUT (%INPUT-STREAM SELF))
    (OUTPUT (%OUTPUT-STREAM SELF)) (ERROR (%ERROR-STREAM SELF)) (STDIN NULL) (STDOUT NULL) (STDERR NULL))
   ...
   (CL:WHEN (NULL? PROCESS) (CL:ERROR (NEW-STELLA-EXCEPTION "shell-process.start-process: not yet implemented")))
   (CL:SETF (%NATIVE-PROCESS SELF) PROCESS)
   (CL:WHEN (DEFINED? STDIN)
    (CL:LET* ((SELF-001 (NEW-OUTPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-001) STDIN)
     (CL:SETF (%NATIVE-STDIN SELF) SELF-001)))
   (CL:WHEN (DEFINED? STDOUT)
    (CL:LET* ((SELF-002 (NEW-INPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-002) STDOUT)
     (CL:SETF (%BUFFERING-SCHEME SELF-002) KWD-SHELL-BLOCK) (CL:SETF (%NATIVE-STDOUT SELF) SELF-002)))
   (CL:WHEN (DEFINED? STDERR)
    (CL:LET* ((SELF-003 (NEW-INPUT-STREAM))) (CL:SETF (%NATIVE-STREAM SELF-003) STDERR)
     (CL:SETF (%BUFFERING-SCHEME SELF-003) KWD-SHELL-BLOCK) (CL:SETF (%NATIVE-STDERR SELF) SELF-003)))))
...
completion.  Conceivably, we could execute a `kill' command to achieve that."
  (CL:LET* ((PROCESS (%NATIVE-PROCESS SELF))) #+allegro (sys:reap-os-subprocess :pid process :wait CL:t)
   ...
   (CL:WHEN (CL:NOT (CL:EQ (%NATIVE-STDOUT SELF) NULL)) (TERMINATE-INPUT-STREAM? (%NATIVE-STDOUT SELF))
    (CL:SETF (%TOKENIZER-STATE (%NATIVE-STDOUT SELF)) NULL))
   (CL:WHEN (CL:NOT (CL:EQ (%NATIVE-STDERR SELF) NULL)) (TERMINATE-INPUT-STREAM? (%NATIVE-STDERR SELF))
...
streams (if any) and do any shell error handling based on exit status or error output."
  ...
  (CL:LET* ((SELF-000 (NEW-SHELL-PROCESS))) (CL:SETF (%COMMAND SELF-000) (PARSE-SHELL-COMMAND COMMAND))
   (CL:SETF (%DIRECTORY SELF-000) (CL:IF (CL:NOT (CL:EQ DIRECTORY STELLA::NULL-STRING)) DIRECTORY "."))
   (CL:SETF (%INPUT-STREAM SELF-000) INPUT) (CL:SETF (%OUTPUT-STREAM SELF-000) OUTPUT)
   (CL:SETF (%ERROR-STREAM SELF-000) ERROR)
   (CL:LET* ((PROCESS SELF-000)) (START-PROCESS PROCESS)
    (CL:LET*
     ((OUTPUTTHREAD
       (CL:IF (CL:NOT (CL:EQ OUTPUT NULL))
        (NEW-THREAD (CL:FUNCTION COPY-STREAM-TO-STREAM) (VECTOR (%NATIVE-STDOUT PROCESS) (%OUTPUT-STREAM PROCESS)))
        NULL))
      (ERRORTHREAD
       (CL:IF (CL:NOT (CL:EQ ERROR NULL))
        (NEW-THREAD (CL:FUNCTION COPY-STREAM-TO-STREAM) (VECTOR (%NATIVE-STDERR PROCESS) (%ERROR-STREAM PROCESS)))
        NULL)))
     ...
     (CL:WHEN (CL:NOT (CL:EQ INPUT NULL)) (COPY-STREAM-TO-STREAM INPUT (%NATIVE-STDIN PROCESS))
      (CLOSE-STREAM (%NATIVE-STDIN PROCESS)))
     (CL:WHEN (CL:NOT (CL:EQ OUTPUTTHREAD NULL)) (WAIT-FOR-COMPLETION OUTPUTTHREAD))
     (CL:WHEN (CL:NOT (CL:EQ ERRORTHREAD NULL)) (WAIT-FOR-COMPLETION ERRORTHREAD)) (WAIT-FOR-COMPLETION PROCESS)
     (DESTROY-PROCESS PROCESS) (%EXIT-STATUS PROCESS)))))
...
If :directory is given, execute `command' in that directory."
  (CL:WHEN *BLOCK-SHELL-COMMAND?*
   (CL:ERROR (NEW-STELLA-EXCEPTION "Execution of `shell-command' is blocked in this context")))
  (CL:LET*
   ((THEOPTIONS
     (PARSE-LOGIC-COMMAND-OPTIONS OPTIONS
      (LIST* KWD-SHELL-DIRECTORY SGT-SHELL-STELLA-STRING KWD-SHELL-INPUT SGT-SHELL-STELLA-STRING
       KWD-SHELL-IGNORE-ERRORS? SGT-SHELL-STELLA-BOOLEAN NIL)
      CL:T CL:NIL))
    (DIRECTORY (UNWRAP-STRING (LOOKUP THEOPTIONS KWD-SHELL-DIRECTORY)))
    (INPUTSTRING (UNWRAP-STRING (LOOKUP THEOPTIONS KWD-SHELL-INPUT)))
    (INPUTSTREAM (CL:IF (CL:NOT (BLANK-STRING? INPUTSTRING)) (NEW-INPUT-STRING-STREAM INPUTSTRING) NULL))
    (OUTPUTSTREAM (NEW-OUTPUT-STRING-STREAM)) (ERRORSTREAM (NEW-OUTPUT-STRING-STREAM))
   ...
   (CL:WHEN (CL:NOT (CL:EQ INPUTSTREAM NULL)) (CLOSE-STREAM INPUTSTREAM)) (CLOSE-STREAM OUTPUTSTREAM)
   ...
   (CL:WHEN (CL:AND (CL:NOT (CL:= EXIT 0)) (CL:NOT (COERCE-WRAPPED-BOOLEAN-TO-BOOLEAN IGNOREERRORS?)))
    (CL:LET* ((STREAM-001 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-001) "shell-command: non-zero exit status=`" EXIT "'")
...
(CL:DEFUN STARTUP-SHELL ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/ONTOSAURUS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    (CL:SETQ SGT-SHELL-ONTOSAURUS-THREAD (INTERN-RIGID-SYMBOL-WRT-MODULE "THREAD" NULL 1))
    ...
    (CL:SETQ SYM-SHELL-STELLA-DIRECTORY
     (INTERN-RIGID-SYMBOL-WRT-MODULE "DIRECTORY" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    ...
    (CL:SETQ SYM-SHELL-STELLA-OUTPUT-STREAM
     (INTERN-RIGID-SYMBOL-WRT-MODULE "OUTPUT-STREAM" (GET-STELLA-MODULE "/STELLA" CL:T) 0))
    ...
    (CL:SETQ KWD-SHELL-DIRECTORY (INTERN-RIGID-SYMBOL-WRT-MODULE "DIRECTORY" NULL 2))
    ...
    (CL:SETQ KWD-SHELL-IGNORE-ERRORS? (INTERN-RIGID-SYMBOL-WRT-MODULE "IGNORE-ERRORS?" NULL 2))
    ...
    (CL:SETQ KWD-SHELL-COMMON-LISP (INTERN-RIGID-SYMBOL-WRT-MODULE "COMMON-LISP" NULL 2))
    (CL:SETQ KWD-SHELL-FUNCTION (INTERN-RIGID-SYMBOL-WRT-MODULE "FUNCTION" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 5)
    ...
    (CL:LET*
     ...
     (CL:SETF (%CLASS-SLOT-ACCESSOR-CODE CLASS) (CL:FUNCTION ACCESS-THREAD-SLOT-VALUE)))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-METHOD-OBJECT "(DEFMETHOD INITIALIZE-THREAD ((SELF THREAD)))" (CL:FUNCTION INITIALIZE-THREAD) NULL)
    (DEFINE-FUNCTION-OBJECT "CL-INITIALIZE-THREAD" "(DEFUN CL-INITIALIZE-THREAD ((SELF THREAD)))"
     (CL:FUNCTION CL-INITIALIZE-THREAD) NULL)
    (DEFINE-FUNCTION-OBJECT "JAVA-INITIALIZE-THREAD" "(DEFUN JAVA-INITIALIZE-THREAD ((SELF THREAD)))"
    ...
    (DEFINE-FUNCTION-OBJECT "PARSE-SHELL-COMMAND"
     "(DEFUN (PARSE-SHELL-COMMAND (ARRAY () OF STRING)) ((COMMAND OBJECT)))" (CL:FUNCTION PARSE-SHELL-COMMAND) NULL)
    (DEFINE-METHOD-OBJECT
...
If :directory is given, execute `command' in that directory.\" :COMMAND? TRUE :PUBLIC? TRUE)"
     (CL:FUNCTION %SHELL-COMMAND) (CL:FUNCTION SHELL-COMMAND-EVALUATOR-WRAPPER))
    ...
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-SHELL-ONTOSAURUS-STARTUP-SHELL)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-SHELL-STELLA-METHOD-STARTUP-CLASSNAME
