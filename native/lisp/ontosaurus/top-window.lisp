...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *MODULE* *ERROR-BACKGROUND-COLOR* EOL *CONTROL-BACKGROUND-COLOR* NIL FALSE-WRAPPER))
  ...
  (CL:LET* ((CONTEXTNAMES NIL))
   (CL:LET* ((M NULL) (ITER-000 (ALL-MODULES)))
    (CL:LOOP WHILE (NEXT? ITER-000) DO (CL:SETQ M (%VALUE ITER-000))
     (CL:WHEN
      (CL:AND
       (CL:NOT (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS M) SYM-TOP-WINDOW-STELLA-CODE-ONLY? FALSE-WRAPPER)))
       (CL:NOT
        (%WRAPPER-VALUE (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS M) SYM-TOP-WINDOW-STELLA-NAMESPACE? FALSE-WRAPPER))))
...
(CL:DEFUN SERVE-CONTROL-PANEL-INTERNAL (STREAM)
  (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
   (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>")
    (CL:PROGN (%%PRINT-STREAM STREAM "<TITLE>") (%%PRINT-STREAM STREAM "PowerLoom Browser Control Panel")
     (%%PRINT-STREAM STREAM "</TITLE>" EOL))
    (WRITE-CONTROL-PANEL-SCRIPT STREAM) (%%PRINT-STREAM STREAM "</HEAD>" EOL))
   (CL:LET* ((TAG-009 (CONCATENATE "BODY BGCOLOR='#" *CONTROL-BACKGROUND-COLOR* "'")))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TAG-009)) (%%PRINT-STREAM STREAM "<" TAG-009 ">") (WRITE-TOP-SECTION STREAM)
...
(CL:DEFUN WRITE-THEORY-SECTION (STREAM)
  ...
  (%%PRINT-STREAM STREAM
   "<SELECT NAME=\"theory\" onChange='document.cookie=\"theory=\" + this[this.selectedIndex].value + \";\";'>" EOL)
  (CL:LET* ((C NULL) (ITER-000 (SORTED-CONTEXTS)))
   (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ C (%%VALUE ITER-000))
    (%%PRINT-STREAM STREAM "<option VALUE='" (%WRAPPER-VALUE C) "'>" (%WRAPPER-VALUE C) "</option>" EOL)
...
(CL:DEFUN WRITE-TOP-SECTION (STREAM)
  (%%PRINT-STREAM STREAM
   "<FORM NAME=\"navform\" TARGET=\"content\" METHOD=\"POST\" onSubmit=\"return formTest(this)\">" EOL "<TABLE BORDER>"
   EOL "<TR>" EOL "<TD ALIGN=center COLSPAN=1 ROWSPAN=1>" EOL
   "<A HREF=\"help/intro.html\" TARGET=\"content\" onMouseOver=\"window.status='Shows directions for using this browser'; return true\">"
   EOL "<IMG SRC=\"/ploom/icons/button-34-help.gif\" BORDER=0 WIDTH=12 HEIGHT=14></a></TD>" EOL
...
"))
  ...
  (CL:IF (EDITTING-POSSIBLE?)
   (CL:PROGN (%%PRINT-STREAM STREAM "<TD ALIGN=LEFT ROWSPAN=2")
    (CL:COND ((EDITTING-ALLOWED?) (%%PRINT-STREAM STREAM " BGCOLOR='#FFCCCC'"))
  ...
  (%%PRINT-STREAM STREAM EOL EOL "<SELECT NAME=\"objecttype\">" EOL "<option VALUE=\"object\" SELECTED>any</option>"
   EOL "<option VALUE=\"concept\">" (LOOKUP-TERM KWD-TOP-WINDOW-CONCEPT CL:NIL CL:NIL) "</option>" EOL
   "<option VALUE=\"relation\">" (LOOKUP-TERM KWD-TOP-WINDOW-RELATION CL:NIL CL:NIL) "</option>" EOL
...
"))
  (%%PRINT-STREAM STREAM "  <img src=\"/ploom/icons/eighth-inch-space.gif\" ALT=\" \" WIDTH=9 HEIGHT=2>" EOL
   "Match&nbsp;<SELECT NAME=\"matchtype\">" EOL "<option VALUE=\"exact\">Exactly</option>" EOL
  ...
  (%%PRINT-STREAM STREAM
   "var cookieValue = document.cookie.substring(document.cookie.indexOf('theory=')+7,document.cookie.length);" EOL)
...
(CL:DEFUN SERVE-CONTROL-PANEL-NEEDS-JAVASCRIPT (STREAM)
  (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>") (%%PRINT-STREAM STREAM EOL)
   (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>") (%%PRINT-STREAM STREAM EOL)
    (CL:PROGN (%%PRINT-STREAM STREAM "<TITLE>") (%%PRINT-STREAM STREAM "Loom Browser No Javascript")
     (%%PRINT-STREAM STREAM "</TITLE>" EOL))
    (%%PRINT-STREAM STREAM "</HEAD>" EOL))
   (CL:LET* ((TAG-010 (CONCATENATE "BODY BGCOLOR='#" *ERROR-BACKGROUND-COLOR* "'")))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TAG-010)) (%%PRINT-STREAM STREAM "<" TAG-010 ">")
    (CL:PROGN (%%PRINT-STREAM STREAM "<B>")
     (CL:PROGN (%%PRINT-STREAM STREAM "<FONT SIZE=6>")
      (%%PRINT-STREAM STREAM "I'm sorry, but a Javascript enabled browser is needed to use this site." EOL)
...
(CL:DEFUN CONTROL-PANEL-HANDLER (XCHG)
  (CL:LET* ((STREAM (GET-REPLY-STREAM XCHG)))
   (CL:COND ((CL:EQ (GET-REQUEST-METHOD XCHG) KWD-TOP-WINDOW-GET) (SERVE-CONTROL-PANEL-INTERNAL STREAM))
...
(CL:DEFUN STARTUP-TOP-WINDOW ()
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/ONTOSAURUS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ KWD-TOP-WINDOW-CONTEXT (INTERN-RIGID-SYMBOL-WRT-MODULE "CONTEXT" NULL 2))
    (CL:SETQ KWD-TOP-WINDOW-CONCEPT (INTERN-RIGID-SYMBOL-WRT-MODULE "CONCEPT" NULL 2))
    (CL:SETQ KWD-TOP-WINDOW-RELATION (INTERN-RIGID-SYMBOL-WRT-MODULE "RELATION" NULL 2))
    (CL:SETQ KWD-TOP-WINDOW-INSTANCE (INTERN-RIGID-SYMBOL-WRT-MODULE "INSTANCE" NULL 2))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "SAVING-ALLOWED?" "(DEFUN (SAVING-ALLOWED? BOOLEAN) ())" (CL:FUNCTION SAVING-ALLOWED?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "LOADING-ALLOWED?" "(DEFUN (LOADING-ALLOWED? BOOLEAN) ())" (CL:FUNCTION LOADING-ALLOWED?)
     NULL)
    (DEFINE-FUNCTION-OBJECT "EDITTING-ALLOWED?" "(DEFUN (EDITTING-ALLOWED? BOOLEAN) ())"
     (CL:FUNCTION EDITTING-ALLOWED?) NULL)
    (DEFINE-FUNCTION-OBJECT "EDITTING-POSSIBLE?" "(DEFUN (EDITTING-POSSIBLE? BOOLEAN) ())"
     (CL:FUNCTION EDITTING-POSSIBLE?) NULL)
    (DEFINE-FUNCTION-OBJECT "CURRENT-EDIT-LOCK?" "(DEFUN (CURRENT-EDIT-LOCK? BOOLEAN) ())"
     (CL:FUNCTION CURRENT-EDIT-LOCK?) NULL)
    (DEFINE-FUNCTION-OBJECT "SORTED-CONTEXTS" "(DEFUN (SORTED-CONTEXTS (CONS OF STRING-WRAPPER)) ())"
     (CL:FUNCTION SORTED-CONTEXTS) NULL)
    (DEFINE-FUNCTION-OBJECT "SERVE-CONTROL-PANEL-INTERNAL"
     "(DEFUN SERVE-CONTROL-PANEL-INTERNAL ((STREAM NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION SERVE-CONTROL-PANEL-INTERNAL)
     NULL)
    (DEFINE-FUNCTION-OBJECT "WRITE-THEORY-SECTION" "(DEFUN WRITE-THEORY-SECTION ((STREAM NATIVE-OUTPUT-STREAM)))"
     (CL:FUNCTION WRITE-THEORY-SECTION) NULL)
    (DEFINE-FUNCTION-OBJECT "WRITE-TOP-SECTION" "(DEFUN WRITE-TOP-SECTION ((STREAM NATIVE-OUTPUT-STREAM)))"
     (CL:FUNCTION WRITE-TOP-SECTION) NULL)
    (DEFINE-FUNCTION-OBJECT "WRITE-BOTTOM-SECTION" "(DEFUN WRITE-BOTTOM-SECTION ((STREAM NATIVE-OUTPUT-STREAM)))"
    ...
    (DEFINE-FUNCTION-OBJECT "CONTROL-PANEL-HANDLER" "(DEFUN CONTROL-PANEL-HANDLER ((XCHG /HTTP/HTTP-EXCHANGE)))"
     (CL:FUNCTION CONTROL-PANEL-HANDLER) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-TOP-WINDOW" "(DEFUN STARTUP-TOP-WINDOW () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-TOP-WINDOW) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-TOP-WINDOW-ONTOSAURUS-STARTUP-TOP-WINDOW)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-TOP-WINDOW-STELLA-METHOD-STARTUP-CLASSNAME
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9) (%IN-MODULE (COPY-CONS-TREE (WRAP-STRING "ONTOSAURUS")))
    ...
    (PUBLISH-HANDLER "/ploom/ontosaurus/top-window.html" (CL:FUNCTION CONTROL-PANEL-HANDLER)
     KWD-TOP-WINDOW-CONTENT-TYPE (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-TOP-WINDOW-HTML NULL))
     KWD-TOP-WINDOW-DOCUMENTATION (WRAP-STRING "Ontosaurus control-panel handler.")))))
