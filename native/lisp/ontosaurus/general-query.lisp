...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING)
  YIELD-PRESET-QUERIES-SCRIPT))
(CL:DEFUN YIELD-PRESET-QUERIES-SCRIPT (QUERIES)
  (CL:WHEN (CL:EQ QUERIES NIL)
   (CL:RETURN-FROM YIELD-PRESET-QUERIES-SCRIPT ""))
  (CL:LET*
   ((NQUERIES (LENGTH QUERIES)) (SCRIPT (NEW-OUTPUT-STRING-STREAM)))
   (CL:DECLARE (CL:TYPE CL:FIXNUM NQUERIES))
   (%%PRINT-STREAM (%NATIVE-STREAM SCRIPT)
...
var querySourceList = new makeArray(")
   ...
   (CL:LET*
    ((ENTRY NULL) (ITER-000 QUERIES) (J NULL-INTEGER) (ITER-001 1))
...
(CL:DEFUN YIELD-CHOICE-MENU-STRING (ITEMS SELECTOPTIONS)
  ...
  (CL:LET*
   ...
   (%%PRINT-STREAM (%NATIVE-STREAM MENU) "<SELECT " SELECTOPTIONS ">"
    EOL)
   (CL:LET*
    ((ITEM NULL) (ITER-000 ITEMS) (I NULL-INTEGER) (ITER-001 1))
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM MENU) "<OPTION VALUE='" OPTIONITEM
      "'" (CL:IF (CL:= I 1) " SELECTED" "") ">" MENUITEM "</OPTION>"
      EOL)
     (CL:SETQ ITER-000 (%%REST ITER-000))
     (CL:SETQ ITER-001 (CL:1+ ITER-001))))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING)
  YIELD-MODULE-MENU-STRING))
(CL:DEFUN YIELD-MODULE-MENU-STRING (MODULES)
  (CL:WHEN (CL:EQ MODULES NIL)
   (CL:RETURN-FROM YIELD-MODULE-MENU-STRING ""))
...
(CL:DECLAIM
 (CL:FTYPE (CL:FUNCTION (CL:T) CL:SIMPLE-STRING)
  YIELD-QUERY-MENU-STRING))
(CL:DEFUN YIELD-QUERY-MENU-STRING (QUERIES)
  (CL:WHEN (CL:EQ QUERIES NIL)
   (CL:RETURN-FROM YIELD-QUERY-MENU-STRING ""))
   ...
   (CL:LET* ((ENTRY NULL) (ITER-000 QUERIES) (COLLECT-000 NULL))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     ...
     (CL:IF (CL:EQ COLLECT-000 NULL)
      ...
      (CL:PROGN
       (CL:SETF (%%REST COLLECT-000) (CONS (%%VALUE ENTRY) NIL))
       (CL:SETQ COLLECT-000 (%%REST COLLECT-000))))
     (CL:SETQ ITER-000 (%%REST ITER-000))))
   (CL:SETQ QUERYITEMS
    (CONS
     (GET-QUOTED-TREE "((\"None\" \"----\") \"/ONTOSAURUS\")"
      "/ONTOSAURUS")
     QUERYITEMS))
...
(CL:DEFUN YIELD-TIMEOUTS-MENU-STRING (TIMEOUTS)
  (CL:WHEN (CL:EQ TIMEOUTS NIL)
   (CL:RETURN-FROM YIELD-TIMEOUTS-MENU-STRING ""))
...
(CL:DEFUN HTML-WRITE-QUERY-FORM-PAGE-FROM-TEMPLATE (STREAM TEMPLATESTREAM)
  (CL:LET*
   ...
   (INSERT-AT VARIABLES (WRAP-STRING "FORM-TRAILER")
    (WRAP-STRING "</FORM>"))
   (CL:SETQ FORMHEADER
...
for (var i = 0; i < document.QUERY.MODULE.length; i++) {
   ...
   (INSERT-AT VARIABLES (WRAP-STRING "FORM-HEADER")
    (WRAP-STRING FORMHEADER))
   (SUBSTITUTE-TEMPLATE-VARIABLES-TO-STREAM TEMPLATESTREAM STREAM
    VARIABLES)))
...
(CL:DEFUN HTML-EXECUTE-QUERY (QUERYDATA STREAM)
  (CL:LET*
   ((MODULENAME
     (%WRAPPER-VALUE (LOOKUP QUERYDATA (WRAP-STRING "MODULE"))))
    ...
    (NVALUEOBJECT
     (CL:IF (CL:NOT (CL:EQ NVALUEENTRY NULL))
      (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE NVALUEENTRY))
      NULL))
    (NVALUES
    ...
    (TIMEOUTOBJECT
     (CL:IF (CL:NOT (CL:EQ TIMEOUTENTRY NULL))
      (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE TIMEOUTENTRY))
      NULL))
    (TIMEOUT
     (CL:IF (FLOAT? TIMEOUTOBJECT) (%WRAPPER-VALUE TIMEOUTOBJECT)
      NULL-FLOAT))
    (QUERY NIL)
    (OPTIONS
     (LIST* KWD-GENERAL-QUERY-SORT-BY KWD-GENERAL-QUERY-VALUES NIL))
    (VARIABLES (NEW-LIST)))
    ...
    (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*))
     ...
     (CL:SETQ QUERY
      (READ-S-EXPRESSION-FROM-STRING
       (%WRAPPER-VALUE
        (LOOKUP QUERYDATA (WRAP-STRING "QUERYSOURCE")))))
     ...
     (CL:IF (EMPTY? VARIABLES)
      ...
      (HTML-EXECUTE-RETRIEVE-QUERY NVALUES (%THE-CONS-LIST VARIABLES)
  ...
  (CL:LET* ((*PRINTREADABLY?* CL:T))
   ...
   (CL:LET* ((QUERYITERATOR NULL) (SOLUTIONS NIL))
    ...
    (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
     ...
     (CL:LET*
      ((TAG-011 (MAKE-HTML-BODY-TAG *ANSWER-BACKGROUND-COLOR*)))
      ...
      (CL:IF (CL:EQ SOLUTIONS NIL)
       ...
       (CL:LET*
        ...
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
         (CL:SETQ SOLN (%%VALUE ITER-000))
         (CL:SETQ ANSWERCOUNTER ITER-001)
         (CL:PROGN (%%PRINT-STREAM STREAM "<DL>")
          ...
          (CL:LET*
           ((BINDING NULL) (ITER-002 SOLN) (VAR NULL)
            (ITER-003 VARIABLES))
           (CL:LOOP WHILE
            ...
            (CL:PROGN (%%PRINT-STREAM STREAM "<DD>")
             ...
             (CL:COND
              ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE BINDING)
               ...
               (CL:PROGN
                (HTML-WRITE-HREF STREAM "show" "concept" BINDING)))
  ...
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE
     ...
     DO (CL:SETQ BINDING (%%VALUE ITER-000))
     (CL:SETQ VAR
   ...
   (HTML-WRITE-HREF-IN-DETAIL STREAM STELLA::NULL-STRING "explain"
    "query" (NAME MODULE) (STRINGIFY PROPOSITION)
...
(CL:DEFUN HTML-WRITE-EXPLANATION-RESPONSE-PAGE (MODULENAME QUERY STREAM)
  ...
  (CL:LET* ((MODULE (GET-STELLA-MODULE MODULENAME CL:NIL)))
   (CL:WHEN (CL:EQ MODULE NULL)
    (HTML-UNKNOWN-MODULE-RESPONSE KWD-GENERAL-QUERY-RELATION QUERY
     MODULENAME STREAM)
     ...
     (WITH-PROCESS-LOCK *POWERLOOM-LOCK*
      (CL:LET* ((*RECORD-JUSTIFICATIONS?* CL:T))
       ...
       (CL:LET*
        ((QUERYITERATOR
          (CREATE-ASK-QUERY
           (CONS (READ-S-EXPRESSION-FROM-STRING QUERY) NIL)))
        ...
        (CL:LET* ((TAG-012 (MAKE-HTML-BODY-TAG "FFFFFF")))
         ...
         (%%PRINT-STREAM STREAM "</"
          (SUBSEQUENCE TAG-012 0 (POSITION TAG-012 #\  0)) ">"
          EOL)))))))))
...
(CL:DEFUN PLOOM-QUERY-HANDLER (XCHG)
  (CL:LET*
   ...
   (CL:COND
    ((CL:EQ METHOD KWD-GENERAL-QUERY-GET)
    ...
    (CL:T
     (CL:LET* ((MESSAGE-000 STELLA::NULL-STRING))
      ...
      (CL:LET* ((*PRINTREADABLY?* CL:T))
       (CL:DECLARE (CL:SPECIAL *PRINTREADABLY?*))
       (CL:SETQ MESSAGE-000
        (CONCATENATE "Unhandled request method: " "`"
         (STRINGIFY METHOD) "'")))
      (GENERATE-ERROR-RESPONSE XCHG KWD-GENERAL-QUERY-BAD-REQUEST
       MESSAGE-000))))))
  ...
  (CL:LET*
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ SYM-GENERAL-QUERY-LOGIC-ALL
     (INTERN-RIGID-SYMBOL-WRT-MODULE "ALL"
...
")))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    ...
    (CL:LET*
     ((FUNCTION
       (LOOKUP-FUNCTION
        SYM-GENERAL-QUERY-ONTOSAURUS-STARTUP-GENERAL-QUERY)))
