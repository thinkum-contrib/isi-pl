...
(CL:DECLAIM
 (CL:SPECIAL *STARTUP-TIME-PHASE* *EXPLANATION-AUDIENCE* *CONTEXT* *POWERLOOM-LOCK* *ANSWER-BACKGROUND-COLOR*
  TRUE-TRUTH-VALUE DEFAULT-TRUE-TRUTH-VALUE FALSE-TRUTH-VALUE DEFAULT-FALSE-TRUTH-VALUE NULL-FLOAT *MODULE* EOL-STRING
  NIL EOL NULL-INTEGER))
...
var querySourceList = new makeArray(")
   ...
   (CL:LET* ((ENTRY NULL) (ITER-000 QUERIES) (J NULL-INTEGER) (ITER-001 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM J ITER-001))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ENTRY (%%VALUE ITER-000)) (CL:SETQ J ITER-001)
     (%%PRINT-STREAM (%NATIVE-STREAM SCRIPT) "queryModuleList[" J "] = '" (%WRAPPER-VALUE (%%VALUE (%%REST ENTRY)))
      "';" EOL)
     (%%PRINT-STREAM (%NATIVE-STREAM SCRIPT) "querySourceList[" J "] = '"
      (REPLACE-SUBSTRINGS
       (REPLACE-SUBSTRINGS
        (REPLACE-SUBSTRINGS (REPLACE-SUBSTRINGS (%WRAPPER-VALUE (%%VALUE (%%REST (%%REST ENTRY)))) "\\\\" "\\") "\\\""
         (MAKE-STRING 1 #\"))
  ...
  (CL:LET* ((MENU (NEW-OUTPUT-STRING-STREAM)) (OPTIONITEM STELLA::NULL-STRING) (MENUITEM STELLA::NULL-STRING))
   ...
   (CL:LET* ((ITEM NULL) (ITER-000 ITEMS) (I NULL-INTEGER) (ITER-001 1)) (CL:DECLARE (CL:TYPE CL:FIXNUM I ITER-001))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ ITEM (%%VALUE ITER-000)) (CL:SETQ I ITER-001)
     (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE ITEM)))
      (CL:COND
       ((SUBTYPE-OF-STRING? TEST-VALUE-000)
        (CL:PROGN (CL:SETQ OPTIONITEM (%WRAPPER-VALUE ITEM)) (CL:SETQ MENUITEM (%WRAPPER-VALUE ITEM))))
     ...
     (%%PRINT-STREAM (%NATIVE-STREAM MENU) "<OPTION VALUE='" OPTIONITEM "'" (CL:IF (CL:= I 1) " SELECTED" "") ">"
      MENUITEM "</OPTION>" EOL)
     (CL:SETQ ITER-000 (%%REST ITER-000)) (CL:SETQ ITER-001 (CL:1+ ITER-001))))
...
(CL:DEFUN HTML-WRITE-QUERY-FORM-PAGE-FROM-TEMPLATE (STREAM TEMPLATESTREAM)
  (CL:LET*
   ((VARIABLES (NEW-KEY-VALUE-LIST))
    (FORMHEADER "<FORM NAME='QUERY' METHOD='POST' ACTION='do-query' ENCTYPE='application/www-url-form-encoded'>"))
   (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING FORMHEADER))
   (INSERT-AT VARIABLES (WRAP-STRING "MODULE-MENU") (WRAP-STRING (YIELD-MODULE-MENU-STRING (SORTED-CONTEXTS))))
   ...
   (INSERT-AT VARIABLES (WRAP-STRING "NOF-ANSWERS-MENU")
    (WRAP-STRING
     (YIELD-NOF-ANSWERS-MENU-STRING
      (GET-QUOTED-TREE "((\"All\" \"1\" \"2\" \"5\" \"10\" \"20\" \"50\" \"100\") \"/ONTOSAURUS\")" "/ONTOSAURUS"))))
   (INSERT-AT VARIABLES (WRAP-STRING "TIMEOUTS-MENU")
    (WRAP-STRING
     (YIELD-TIMEOUTS-MENU-STRING
      (GET-QUOTED-TREE "((\"None\" \"1.0\" \"2.0\" \"5.0\" \"10.0\" \"30.0\" \"60.0\" \"120.0\") \"/ONTOSAURUS\")"
       "/ONTOSAURUS"))))
   (INSERT-AT VARIABLES (WRAP-STRING "QUERY-INPUT-AREA")
    (WRAP-STRING "<TEXTAREA NAME='QUERYSOURCE' ROWS='4' COLS='80'></TEXTAREA>"))
...
(CL:DEFUN HTML-EXECUTE-QUERY (QUERYDATA STREAM)
  (CL:LET*
   ((MODULENAME (%WRAPPER-VALUE (LOOKUP QUERYDATA (WRAP-STRING "MODULE"))))
    (MODULE (GET-MODULE (WRAP-STRING MODULENAME))) (NVALUEENTRY (LOOKUP QUERYDATA (WRAP-STRING "NANSWERS")))
    (NVALUEOBJECT
     (CL:IF (CL:NOT (CL:EQ NVALUEENTRY NULL)) (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE NVALUEENTRY)) NULL))
    (NVALUES (CL:IF (INTEGER? NVALUEOBJECT) (%WRAPPER-VALUE NVALUEOBJECT) NULL-INTEGER))
    (TIMEOUTENTRY (LOOKUP QUERYDATA (WRAP-STRING "TIMEOUT")))
    (TIMEOUTOBJECT
     (CL:IF (CL:NOT (CL:EQ TIMEOUTENTRY NULL)) (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE TIMEOUTENTRY)) NULL))
   ...
   (CL:IF (CL:NOT (CL:EQ MODULE NULL))
    (CL:LET* ((*MODULE* MODULE) (*CONTEXT* *MODULE*)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (CL:SETQ QUERY (READ-S-EXPRESSION-FROM-STRING (%WRAPPER-VALUE (LOOKUP QUERYDATA (WRAP-STRING "QUERYSOURCE")))))
     (COLLECT-UNDECLARED-VARIABLES QUERY NIL VARIABLES)
     (CL:WHEN (CL:NOT (CL:= TIMEOUT NULL-FLOAT))
      (CL:SETQ OPTIONS (CONCATENATE OPTIONS (LIST* KWD-GENERAL-QUERY-TIMEOUT (WRAP-FLOAT TIMEOUT) NIL))))
  ...
  (CL:LET* ((ANSWER (PLI::ASK (CONS QUERY OPTIONS) MOD NULL)))
   (CL:COND
    ((CL:OR (CL:EQ ANSWER TRUE-TRUTH-VALUE) (CL:EQ ANSWER DEFAULT-TRUE-TRUTH-VALUE))
     (CL:PROGN (%%PRINT-STREAM STREAM "<FONT COLOR='#33FF66'>") (%%PRINT-STREAM STREAM "TRUE")
      (%%PRINT-STREAM STREAM "</FONT>" EOL)))
    ((CL:OR (CL:EQ ANSWER FALSE-TRUTH-VALUE) (CL:EQ ANSWER DEFAULT-FALSE-TRUTH-VALUE))
     (CL:PROGN (%%PRINT-STREAM STREAM "<FONT COLOR='#FF6633'>") (%%PRINT-STREAM STREAM "FALSE")
      (%%PRINT-STREAM STREAM "</FONT>" EOL)))
    (CL:T
     (CL:PROGN (%%PRINT-STREAM STREAM "<FONT COLOR='#888888'>") (%%PRINT-STREAM STREAM "UNKNOWN")
      (%%PRINT-STREAM STREAM "</FONT>" EOL))))
   ...
   (CL:LET* ((QUERYITERATOR NULL) (SOLUTIONS NIL))
     ...
     (CL:LET* ((*MODULE* MDL-000) (*CONTEXT* CXT-000)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
      (WITH-PROCESS-LOCK *POWERLOOM-LOCK*
       (CL:SETQ QUERYITERATOR
        (CALL-RETRIEVE
         (CONS (CL:IF (CL:> NVALUES 0) (WRAP-INTEGER NVALUES) SYM-GENERAL-QUERY-LOGIC-ALL)
    ...
    (CL:PROGN (%%PRINT-STREAM STREAM "<HTML>")
     (CL:PROGN (%%PRINT-STREAM STREAM "<HEAD>") (%%PRINT-STREAM STREAM "<BASE TARGET='content'>" EOL)
      (CL:PROGN (%%PRINT-STREAM STREAM "<TITLE>") (%%PRINT-STREAM STREAM "Answer Page")
       (%%PRINT-STREAM STREAM "</TITLE>" EOL))
      (%%PRINT-STREAM STREAM "</HEAD>" EOL))
     (CL:LET* ((TAG-011 (MAKE-HTML-BODY-TAG *ANSWER-BACKGROUND-COLOR*)))
      (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TAG-011)) (%%PRINT-STREAM STREAM "<" TAG-011 ">")
      ...
      (CL:IF (CL:EQ SOLUTIONS NIL)
       (CL:PROGN (%%PRINT-STREAM STREAM "<FONT COLOR='#CC3366'>")
        (CL:PROGN (%%PRINT-STREAM STREAM "<I>") (%%PRINT-STREAM STREAM "No Solutions") (%%PRINT-STREAM STREAM "</I>"))
        (%%PRINT-STREAM STREAM "</FONT>" EOL))
       (CL:LET* ((SOLN NULL) (ITER-000 SOLUTIONS) (ANSWERCOUNTER NULL-INTEGER) (ITER-001 1))
        (CL:DECLARE (CL:TYPE CL:FIXNUM ANSWERCOUNTER ITER-001))
        (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO (CL:SETQ SOLN (%%VALUE ITER-000))
         ...
         (CL:PROGN (%%PRINT-STREAM STREAM "<DL>")
          ...
          (CL:LET* ((BINDING NULL) (ITER-002 SOLN) (VAR NULL) (ITER-003 VARIABLES))
           (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-002 NIL)) (CL:NOT (CL:EQ ITER-003 NIL))) DO
            (CL:SETQ BINDING (%%VALUE ITER-002)) (CL:SETQ VAR (%%VALUE ITER-003))
            (CL:PROGN (%%PRINT-STREAM STREAM "<DD>") (%%PRINT-STREAM STREAM VAR " = ")
             (CL:COND
              ((SUBTYPE-OF? (SAFE-PRIMARY-TYPE BINDING) SGT-GENERAL-QUERY-LOGIC-LOGIC-OBJECT)
...
(CL:DEFUN WRITE-EXPLANATION-ICON (QUERY BINDINGS STREAM)
  (CL:LET*
   ...
   (CL:LET*
    ((BINDING NULL) (ITER-000 BINDINGS) (VAR NULL) (VECTOR-000 (%IO-VARIABLES DESCRIPTION)) (INDEX-000 0)
    ...
    (CL:LOOP WHILE (CL:AND (CL:NOT (CL:EQ ITER-000 NIL)) (CL:< INDEX-000 LENGTH-000)) DO
     ...
     (INSERT-AT MAPPING VAR BINDING) (CL:SETQ ITER-000 (%%REST ITER-000)) (CL:SETQ INDEX-000 (CL:1+ INDEX-000))))
   (CL:LET* ((VAR NULL) (VECTOR-001 EXISTSVARS) (INDEX-001 0) (LENGTH-001 (LENGTH VECTOR-001)))
    (CL:DECLARE (CL:TYPE CL:FIXNUM INDEX-001 LENGTH-001))
    (CL:LOOP WHILE (CL:< INDEX-001 LENGTH-001) DO
     (CL:SETQ VAR (CL:AREF (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY VECTOR-001)) INDEX-001)) (INSERT-AT MAPPING VAR VAR)
   ...
   (CL:WHEN (NON-EMPTY? EXISTSVARS)
    (CL:SETQ PROPOSITION
     (LIST* SYM-GENERAL-QUERY-STELLA-EXISTS (GENERATE-VARIABLES EXISTSVARS CL:NIL) (CONS PROPOSITION NIL))))
...
(CL:DEFUN HTML-WRITE-EXPLANATION-RESPONSE-PAGE (MODULENAME QUERY STREAM)
  ...
  (CL:LET* ((MODULE (GET-STELLA-MODULE MODULENAME CL:NIL)))
   (CL:WHEN (CL:EQ MODULE NULL) (HTML-UNKNOWN-MODULE-RESPONSE KWD-GENERAL-QUERY-RELATION QUERY MODULENAME STREAM)
    ...
    (CL:LET* ((*MODULE* MDL-000) (*CONTEXT* CXT-000)) (CL:DECLARE (CL:SPECIAL *MODULE* *CONTEXT*))
     (WITH-PROCESS-LOCK *POWERLOOM-LOCK*
      (CL:LET* ((*RECORD-JUSTIFICATIONS?* CL:T)) (CL:DECLARE (CL:SPECIAL *RECORD-JUSTIFICATIONS?*))
       (CL:LET*
        ((QUERYITERATOR (CREATE-ASK-QUERY (CONS (READ-S-EXPRESSION-FROM-STRING QUERY) NIL)))
         (EXPLANATION (NEW-OUTPUT-STRING-STREAM)) (JUSTIFICATION NULL))
        (CALL-ASK QUERYITERATOR)
        (CL:SETQ JUSTIFICATION
         (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS (%BASE-CONTROL-FRAME QUERYITERATOR)) SYM-GENERAL-QUERY-LOGIC-JUSTIFICATION
          NULL))
        (CL:LET*
         ((*EXPLANATION-FORMAT* KWD-GENERAL-QUERY-HTML) (*EXPLANATION-AUDIENCE* KWD-GENERAL-QUERY-LAY)
        ...
        (CL:LET* ((TAG-012 (MAKE-HTML-BODY-TAG "FFFFFF"))) (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING TAG-012))
         (%%PRINT-STREAM STREAM "<" TAG-012 ">") (%%PRINT-STREAM STREAM (THE-STRING-READER EXPLANATION) EOL)
         (%%PRINT-STREAM STREAM "</" (SUBSEQUENCE TAG-012 0 (POSITION TAG-012 #\  0)) ">" EOL)))))))))
  ...
  (CL:LET* ((STREAM (GET-REPLY-STREAM XCHG)) (METHOD (GET-REQUEST-METHOD XCHG)))
   (CL:COND ((CL:EQ METHOD KWD-GENERAL-QUERY-GET) (HTML-WRITE-QUERY-FORM-PAGE STREAM))
    ((CL:EQ METHOD KWD-GENERAL-QUERY-POST)
     (CL:HANDLER-CASE
      (CL:LET* ((BODY (GET-REQUEST-BODY XCHG)) (VALUES (PARSE-AND-DECODE-FORM-VALUES BODY)))
       (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING BODY)) (HTML-EXECUTE-QUERY VALUES STREAM))
  ...
  (CL:LET* ((*MODULE* (GET-STELLA-MODULE "/ONTOSAURUS" (> *STARTUP-TIME-PHASE* 1))) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 2)
    ...
    (CL:SETQ SYM-GENERAL-QUERY-LOGIC-JUSTIFICATION
     (INTERN-RIGID-SYMBOL-WRT-MODULE "JUSTIFICATION" (GET-STELLA-MODULE "/LOGIC" CL:T) 0))
    (CL:SETQ KWD-GENERAL-QUERY-HTML (INTERN-RIGID-SYMBOL-WRT-MODULE "HTML" NULL 2))
    (CL:SETQ KWD-GENERAL-QUERY-LAY (INTERN-RIGID-SYMBOL-WRT-MODULE "LAY" NULL 2))
    (CL:SETQ KWD-GENERAL-QUERY-KIF-ONTOSAURUS (INTERN-RIGID-SYMBOL-WRT-MODULE "KIF-ONTOSAURUS" NULL 2))
    (CL:SETQ KWD-GENERAL-QUERY-GET (INTERN-RIGID-SYMBOL-WRT-MODULE "GET" NULL 2))
    (CL:SETQ KWD-GENERAL-QUERY-POST (INTERN-RIGID-SYMBOL-WRT-MODULE "POST" NULL 2))
...
")))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 7)
    (DEFINE-FUNCTION-OBJECT "HTML-WRITE-QUERY-FORM-PAGE"
     "(DEFUN HTML-WRITE-QUERY-FORM-PAGE ((STREAM NATIVE-OUTPUT-STREAM)))" (CL:FUNCTION HTML-WRITE-QUERY-FORM-PAGE)
     NULL)
    ...
    (DEFINE-FUNCTION-OBJECT "PLOOM-QUERY-HANDLER" "(DEFUN PLOOM-QUERY-HANDLER ((XCHG /HTTP/HTTP-EXCHANGE)))"
     (CL:FUNCTION PLOOM-QUERY-HANDLER) NULL)
    (DEFINE-FUNCTION-OBJECT "STARTUP-GENERAL-QUERY" "(DEFUN STARTUP-GENERAL-QUERY () :PUBLIC? TRUE)"
     (CL:FUNCTION STARTUP-GENERAL-QUERY) NULL)
    (CL:LET* ((FUNCTION (LOOKUP-FUNCTION SYM-GENERAL-QUERY-ONTOSAURUS-STARTUP-GENERAL-QUERY)))
     (SET-DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS FUNCTION) SYM-GENERAL-QUERY-STELLA-METHOD-STARTUP-CLASSNAME
...
\"))")
    (PUBLISH-HANDLER "/ploom/ontosaurus/do-query" (CL:FUNCTION PLOOM-QUERY-HANDLER) KWD-GENERAL-QUERY-CONTENT-TYPE
     (WRAP-STRING (GET-HTTP-MIME-TYPE KWD-GENERAL-QUERY-HTML NULL)) KWD-GENERAL-QUERY-DOCUMENTATION
