...
public class QueryIterator extends Iterator {
  ...
  public static QueryIterator deriveJustifiedPartialQuery(QueryIterator query) {
    ...
    { QueryIterator partialquery = Description.createQueryIterator(query.baseControlFrame.patternRecord.description, ((QuerySolution)(query.value)).bindings, "PartialQuery");
        ...
        { QueryIterator askquery = Logic.allocateQueryIterator();
          ...
          { Object old$TypeCheckStrategy$000 = Logic.$TYPE_CHECK_STRATEGY$.get();
            ...
            try {
              ...
              QueryIterator.initializeQueryIterator(askquery, query.queryDescription(), query.solutions.nth(solutionindex).bindings, "Justifications");
  ...
  public static boolean tryToDefeatLastAnswerP(QueryIterator self, Object [] MV_returnarray) {
    { QueryIterator negatedquery = (self.queryIsTrueFalseP() ? self : ((QueryIterator)(KeyValueList.dynamicSlotValue(self.dynamicSlots, Logic.SYM_LOGIC_AUXILIARY_QUERY, null))));
      ...
      if (negatedquery == null) {
        negatedquery = Description.createQueryIterator(self.queryDescription(), ((QuerySolution)(self.value)).bindings, "negatedQuery");
...
;
  ...
  public static QueryIterator initializeQueryIterator(QueryIterator queryiterator, Description description, Vector outsidebindings, String debugid) {
...
;
  ...
  public static void printQueryIterator(QueryIterator self, java.io.PrintStream stream) {
    if (Logic.$DEBUG_MODEp$) {
      stream.print("[" + self.debugId + "] ");
    }
  ...
  public static Stella_Object accessQueryIteratorSlotValue(QueryIterator self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    ...
    else if (slotname == Logic.SYM_LOGIC_DEBUG_ID) {
      if (setvalueP) {
        self.debugId = ((StringWrapper)(value)).wrapperValue;
      }
      else {
        value = StringWrapper.wrapString(self.debugId);
      }
    }
