...
public class SystemDefinition extends StandardObject {
  ...
  public static SystemDefinition newSystemDefinition() {
    { SystemDefinition self = null;
      ...
      self.upToDateP = Stella.FALSE;
      self.loadedP = Stella.FALSE;
...
;
    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        { String file = Stella.makeFileName(SystemDefinition.systemStartupFileName(system), Stella.KWD_STELLA, Stella.TRUE);
          OutputFileStream outputstream = OutputFileStream.newOutputFileStream(file);
          ...
          OutputStream.clOutputFileHeader(outputstream, file, Stella.FALSE);
          startupforms = Stella_Object.cons(Stella.list$(Stella_Object.cons(Stella.SYM_STELLA_IN_MODULE, Stella_Object.cons(Stella.wrapString(SystemDefinition.getCardinalModule(system).moduleFullName), Stella_Object.cons(Stella.NIL, Stella.NIL)))), startupforms);
          ...
          if (Stella.stringEqlP(Native.stringUpcase(system.name), "STELLA")) {
            startupforms = Stella_Object.cons(Stella.list$(Stella_Object.cons(Stella.SYM_STELLA_STARTUP_TIME_PROGN, Stella_Object.cons(Stella.KWD_EARLY_INITS, Stella_Object.cons(Stella.list$(Stella_Object.cons(Stella.SYM_STELLA_STARTUP, Stella_Object.cons(Stella.SYM_STELLA_FALSE, Stella_Object.cons(Stella.NIL, Stella.NIL)))), Stella_Object.cons(Stella.NIL, Stella.NIL))))), startupforms);
          }
          else {
...
;
  ...
  public static Cons collectStartupFormsFromSystemFile(SystemDefinition system) {
    { String systemfilename = Stella.makeSystemDefinitionFileName(system.name);
      InputFileStream inputstream = InputFileStream.newInputFileStream(systemfilename);
      Module module = null;
              ...
              if (Stella_Object.safePrimaryType(tree) == Stella.SGT_STELLA_CONS) {
                { Cons tree000 = ((Cons)(tree));
                  ...
                  if (tree000.value == Stella.SYM_STELLA_DEFMODULE) {
                    ...
                    module = Stella.getStellaModule(Stella_Object.coerceToModuleName(tree000.rest.value, Stella.TRUE), Stella.TRUE);
                    if (module != null) {
  ...
  public static Symbol systemStartupFunctionSymbol(SystemDefinition system) {
    return (Stella.internSymbolInModule(system.systemStartupFunctionName(), SystemDefinition.getCardinalModule(system), Stella.TRUE));
  }
    ...
    { Cons files = Cons.copyConsList(system.files);
      ...
      if (((Keyword)(Stella.$TRANSLATOROUTPUTLANGUAGE$.get())) == Stella.KWD_COMMON_LISP) {
        ...
        if (((List)(Stella.$CURRENT_STELLA_FEATURES$.get())).membP(Stella.KWD_USE_COMMON_LISP_STRUCTS)) {
          files = Stella_Object.cons(Stella.wrapString(Stella.clYieldStructClassFileName(system.name)), files);
        }
      }
              ...
              { StringWrapper f = null;
                ...
                while (!(iter000 == Stella.NIL)) {
                  ...
                  if (type == Stella.KWD_JAVA) {
                    filename = StringWrapper.javaTranslateClassNamestring(Stella.wrapString("Startup-" + f.wrapperValue)).wrapperValue;
                  }
                  else {
...
;
                ...
                { Stella_Class renamed_Class = null;
                  Iterator iter001 = Module.allClasses(SystemDefinition.getCardinalModule(((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get()))), Stella.TRUE);

                  while (iter001.nextP()) {
                    renamed_Class = ((Stella_Class)(iter001.value));
                    filename = renamed_Class.javaMakeCodeOutputFileName(Stella.FALSE);
                    Stella.helpGetSystemFiles(filename, files, probefilesP);
                  }
                }
                filename = Stella.string_javaMakeCodeOutputFileName(Module.javaYieldFlotsamClassName(SystemDefinition.getCardinalModule(((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())))), Stella.FALSE);
                Stella.helpGetSystemFiles(filename, files, probefilesP);
              }
              else {
              }
...
;
  ...
  public static Module getCardinalModule(SystemDefinition system) {
    { String name = system.cardinalModule;
      ...
      if (name != null) {
        module = Stella.getStellaModule(name, Stella.FALSE);
        if (module == null) {
          module = Stella.getStellaModule(Native.stringUpcase(name), Stella.FALSE);
        }
      }
      if (module == null) {
        {
  ...
  public static Stella_Object accessSystemDefinitionSlotValue(SystemDefinition self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    if (slotname == Stella.SYM_STELLA_NAME) {
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.name);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.directory);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.cardinalModule);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.sourceRootDirectory);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.nativeRootDirectory);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.binaryRootDirectory);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.banner);
      }
    }
