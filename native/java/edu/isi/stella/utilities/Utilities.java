          ...
          { Cons specification000 = ((Cons)(specification));
            ...
            { Symbol classsymbol = ((Symbol)(specification000.rest.value));
              ...
              if (renamed_Class == null) {
                System.out.print("Can't find a class named " + specification000.rest.value);
                return (null);
              }
  ...
  public static String manualExpandTemplateCommands(String line) {
    { int commandstart = Native.stringSearch(line, Utilities.$MANUAL_TEMPLATE_COMMAND_PREFIX$, 0);
      ...
      try {
        ...
        {
          System.out.println("*** ERROR while expanding template command: " + Stella.exceptionMessage(e));
          System.out.println("    Error occured in line: `" + line + "'");
        }
;
        return (line);
      }
  ...
  public static void showMeasures() {
    { Measure m = null;
      ...
      for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
        ...
        System.out.println(m.name + "   base-unit=" + m.baseUnit + "   prime-id=" + m.primeId + ((m.baseMeasureP() ? "   BASE" : "   DERIVED")));
      }
    }
  }
  ...
  public static void showMeasure(String measureName) {
    { Measure m = ((Measure)(Utilities.$NAME_TO_MEASURE_TABLE$.lookup(measureName)));

      if (m != null) {
        { KeyValueList theScale = m.scale;

          System.out.println(measureName + "   base-unit=" + m.baseUnit + "   prime-id=" + m.primeId + ((m.baseMeasureP() ? "   BASE" : "   DERIVED")));
          { StringWrapper u = null;
            Cons iter000 = m.getUnits().theConsList;

            for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
              u = ((StringWrapper)(iter000.value));
              System.out.println("  " + StringWrapper.unwrapString(u) + "   factor=" + FloatWrapper.unwrapFloat(((FloatWrapper)(theScale.lookup(u)))));
            }
          }
        }
      }
      else {
        System.out.println("No measure with name `" + measureName + "' found.");
      }
    }
  }
