...
public class ActiveSet extends List {
    public StorageSlot activeSlot;
    public StandardObject ownerInstance;

  public static ActiveSet newActiveSet() {
    { ActiveSet self = null;
    ...
    { ActiveSet self = this;
      ...
      { StorageSlot slot = self.activeSlot;
        ...
        if (self.theConsList.memberP(value)) {
          if (StandardObject.runSlotGuardDemonsP(owner, slot, value, null)) {
            self.theConsList = self.theConsList.remove(value);
            StandardObject.runSlotDemons(owner, slot, value, null);
          }
        }
        return (self);
      }
  ...
  public void insert(Stella_Object value) {
    { ActiveSet self = this;
      ...
      { StorageSlot slot = self.activeSlot;
        ...
        if (!(self.theConsList.memberP(value))) {
          if (StandardObject.runSlotGuardDemonsP(owner, slot, self, value)) {
            self.theConsList = Cons.cons(value, self.theConsList);
            StandardObject.runSlotDemons(owner, slot, null, value);
          }
        }
      }
    }
