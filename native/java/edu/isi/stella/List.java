    ...
    { int remainingunits = startupunits.length();
      ...
      loop000 : while (!(cursor == Stella.NIL)) {
        ...
        { Symbol functionname = GeneralizedSymbol.internDerivedSymbol(startupfnname, "HELP-" + startupfnname.symbolName + Native.integerToString(helpfncounter = helpfncounter + 1));
          ...
          KeyValueList.setDynamicSlotValue(((MethodSlot)(Cons.helpWalkAuxiliaryTree(helpfntree, true).theObject)).dynamicSlots, Stella.SYM_STELLA_METHOD_STARTUP_CLASSNAME, Stella.wrapString(Symbol.yieldStartupFunctionClassname(startupfnname)), Stella.NULL_STRING_WRAPPER);
