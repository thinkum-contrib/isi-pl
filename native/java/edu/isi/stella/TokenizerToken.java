...
public class TokenizerToken extends StandardObject {
  ...
  /** Convert the XML <code>tokenlist</code> into a representative s-expression
    ...
    { Cons parsedtree = null;
      ...
      loop000 : for (;;) {
        ...
        if (tokentype == Stella.KWD_START_TAG) {
          ...
          if (Stella.xmlnsAttributeP(tokenlist.content)) {
            ...
            if (colonPosition != Stella.NULL_INTEGER) {
              namespaceTable = Stella_Object.kvCons(Stella.wrapString(Native.string_subsequence(xmlnsName, colonPosition + 1, Stella.NULL_INTEGER)), Stella.wrapString(tokenlist.content), namespaceTable);
            }
            else if (Stella.stringEqlP(tokenlist.content, "")) {
              namespaceTable = Stella_Object.kvCons(Stella.wrapString(""), null, namespaceTable);
            }
            else {
              namespaceTable = Stella_Object.kvCons(Stella.wrapString(""), Stella.wrapString(tokenlist.content), namespaceTable);
            }
            parsedtoken = new StringWrapper();
            ((StringWrapper)(parsedtoken)).wrapperValue = tokenlist.content;
            conscell = new Cons();
          ...
          continue loop000;
            ...
            ((tokentype == Stella.KWD_PI_TAG_DATA) ||
  ...
  public static Stella_Object accessTokenizerTokenSlotValue(TokenizerToken self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    if (slotname == Stella.SYM_STELLA_TYPE) {
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapString(self.content);
      }
    }
