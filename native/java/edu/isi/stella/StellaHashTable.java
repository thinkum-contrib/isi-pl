...
public class StellaHashTable extends AbstractHashTable {
  ...
  public static void rehashStellaHashTable(StellaHashTable self, int newsize) {
    ...
    { int size = self.size;
      ...
      self.size = newsize;
      self.freeElements = Stella.max(self.freeElements + Native.floor((newsize - size) * Stella.$STELLA_HASH_TABLE_AVG_BUCKET_LENGTH$), 0);
    }
  }
  ...
  public static Stella_Object accessStellaHashTableSlotValue(StellaHashTable self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    if (slotname == Stella.SYM_STELLA_SIZE) {
      if (setvalueP) {
        ...
        value = Stella.wrapInteger(self.size);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapInteger(self.initialSize);
      }
    }
      ...
      if (setvalueP) {
        ...
        value = Stella.wrapInteger(self.freeElements);
      }
    }
