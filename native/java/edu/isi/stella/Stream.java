...
public abstract class Stream extends StandardObject {
    public Keyword state;

  /** Close the stream <code>self</code>.
   * @param self
   */
  public static void closeStream(Stream self) {
    { Surrogate testValue000 = Stella_Object.safePrimaryType(self);

      if (Surrogate.subtypeOfP(testValue000, Stella.SGT_STELLA_FILE_INPUT_STREAM)) {
        { InputFileStream self000 = ((InputFileStream)(self));

          InputFileStream.terminateFileInputStreamP(self000);
        }
      }
      else if (Surrogate.subtypeOfP(testValue000, Stella.SGT_STELLA_FILE_OUTPUT_STREAM)) {
  ...
  public static Stella_Object accessStreamSlotValue(Stream self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    if (slotname == Stella.SYM_STELLA_STATE) {
      if (setvalueP) {
