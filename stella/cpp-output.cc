...
void cpp_output_parse_tree(Cons* constree) {
}

void startup_cpp_output() {
      ...
      if ((!oPENDINGEXCEPTIONHANDLERpo) ||
          setup_long_jumpP(oEXCEPTIONHANDLERADDRESSo)) {
        {
          ...
          if (current_startup_time_phaseP(7)) {
            { Method_Slot* method = define_method_from_stringified_source("CPP-OUTPUT-PARSE-TREE", "CONS", "(DEFUN CPP-OUTPUT-PARSE-TREE ((CONSTREE CONS)))");

              method->function_code = ((cpp_function_code)(&cpp_output_parse_tree));
            }
            define_method_from_stringified_source("STARTUP-CPP-OUTPUT", NULL, "(DEFUN STARTUP-CPP-OUTPUT ())");
          }
