...
void hello_world(char* saywhat) {
  cout << endl << "*****************************" << endl << "    " << saywhat << endl << "*****************************" << endl;
}

void startup_your_file() {
      ...
      if ((!oPENDINGEXCEPTIONHANDLERpo) ||
          setup_long_jumpP(oEXCEPTIONHANDLERADDRESSo)) {
        {
          ...
          if (current_startup_time_phaseP(7)) {
            { Method_Slot* method = define_method_from_stringified_source("HELLO-WORLD", "STRING", "(DEFUN HELLO-WORLD ((SAYWHAT STRING)))");

              method->function_code = ((cpp_function_code)(&hello_world));
            }
            define_method_from_stringified_source("STARTUP-YOUR-FILE", NULL, "(DEFUN STARTUP-YOUR-FILE ())");
          }
