...
void idl_output_parse_tree(Cons* constree) {
}

void startup_idl_output() {
      ...
      if ((!oPENDINGEXCEPTIONHANDLERpo) ||
          setup_long_jumpP(oEXCEPTIONHANDLERADDRESSo)) {
        {
          ...
          if (current_startup_time_phaseP(7)) {
            { Method_Slot* method = define_method_from_stringified_source("IDL-OUTPUT-PARSE-TREE", "CONS", "(DEFUN IDL-OUTPUT-PARSE-TREE ((CONSTREE CONS)))");

              method->function_code = ((cpp_function_code)(&idl_output_parse_tree));
            }
            define_method_from_stringified_source("STARTUP-IDL-OUTPUT", NULL, "(DEFUN STARTUP-IDL-OUTPUT ())");
          }
